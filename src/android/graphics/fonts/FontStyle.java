...
public final class FontStyle {
    ...
    public FontStyle(int i, int i2) {
        Preconditions.checkArgument(1 <= i && i <= 1000, "weight value must be [1, 1000]");
        Preconditions.checkArgument(i2 == 0 || i2 == 1, "slant value must be FONT_SLANT_UPRIGHT or FONT_SLANT_UPRIGHT");
        this.mWeight = i;
        this.mSlant = i2;
    }
    ...
    public int getMatchScore(FontStyle fontStyle) {
        return (Math.abs(getWeight() - fontStyle.getWeight()) / 100) + (getSlant() == fontStyle.getSlant() ? 0 : 2);
    }
