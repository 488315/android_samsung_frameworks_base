...
public class ListPopupWindow implements ShowableListMenu {
    ...
    public void setWidth(int i) {
        if (this.mIsDexMode && i < -2) {
            this.mDropDownWidth = -2;
            Log.d(TAG, "An abnormal value has been passed. width: " + i);
            return;
        }
    ...
    public void setContentWidth(int i) {
        ...
        if (background != null) {
            ...
            if (anchorView != null) {
                ...
                this.mDropDownWidth = Math.min(this.mTempRect.left + this.mTempRect.right + i, rect.right - rect.left);
                return;
            }
    ...
    public void show() {
        ...
        if (i2 == -1) {
            iBuildDropDown = -1;
        } else if (i2 != -2 && iBuildDropDown >= i2) {
            iBuildDropDown = Math.max(1, i2);
        }
        setBlurEffect();
    ...
    private int buildDropDown() {
        ...
        if (i5 == -2) {
            ...
            iMakeMeasureSpec = View.MeasureSpec.makeMeasureSpec(i5 - (this.mTempRect.left + this.mTempRect.right), 1073741824);
        }
    ...
    private void setBlurEffect() {
        if (this.mContext != null && this.mIsDeviceDefault && this.mPopup.semIsAvailableBlurBackground()) {
            TypedValue typedValue = new TypedValue();
            this.mContext.getTheme().resolveAttribute(R.attr.parentIsDeviceDefaultDark, typedValue, true);
            boolean z = typedValue.data != 0;
            int i = !z ? 110 : 125;
            float dimensionPixelSize = this.mContext.getResources().getDimensionPixelSize(R.dimen.sem_popup_menu_corner_radius);
            SemBlurInfo.Builder builder = new SemBlurInfo.Builder(0);
            builder.setColorCurvePreset(i);
            if (z) {
                builder.setBackgroundColor(this.mContext.getResources().getColor(R.color.sem_popup_menu_blur_background_dark, this.mContext.getTheme()));
            }
            builder.setBackgroundCornerRadius(dimensionPixelSize);
