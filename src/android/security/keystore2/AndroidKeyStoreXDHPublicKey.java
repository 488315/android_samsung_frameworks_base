...
public class AndroidKeyStoreXDHPublicKey extends AndroidKeyStorePublicKey implements XECPublicKey {
    ...
    public AndroidKeyStoreXDHPublicKey(KeyDescriptor keyDescriptor, KeyMetadata keyMetadata, String str, KeyStoreSecurityLevel keyStoreSecurityLevel, byte[] bArr) {
    ...
    private static int matchesPreamble(byte[] bArr, byte[] bArr2) {
        if (bArr2.length == bArr.length + 32 && Arrays.compare(bArr, 0, bArr.length, bArr2, 0, bArr.length) == 0) {
            return bArr.length;
        }
        return 0;
    }
