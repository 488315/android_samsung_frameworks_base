...
public abstract class ContentCaptureSession implements AutoCloseable {
    ...
    abstract void internalNotifySessionFlushEvent(int i);
    ...
    public final void notifyViewAppeared(ViewStructure viewStructure) {
        ...
        if (isContentCaptureEnabled()) {
            ...
            internalNotifyViewAppeared(this.mId, (ViewNode.ViewStructureImpl) viewStructure);
        }
    }
    ...
    public final void notifyViewsAppeared(List<ViewStructure> list) {
        Preconditions.checkCollectionElementsNotNull(list, "appearedNodes");
        if (isContentCaptureEnabled()) {
            for (int i = 0; i < list.size(); i++) {
                ViewStructure viewStructure = list.get(i);
                if (!(viewStructure instanceof ViewNode.ViewStructureImpl)) {
                    throw new IllegalArgumentException("Invalid class: " + viewStructure.getClass());
                }
            }
            internalNotifyViewTreeEvent(this.mId, true);
            for (int i2 = 0; i2 < list.size(); i2++) {
                internalNotifyViewAppeared(this.mId, (ViewNode.ViewStructureImpl) list.get(i2));
            }
    ...
    public final void notifyViewsDisappeared(AutofillId autofillId, long[] jArr) {
        ...
        if (isContentCaptureEnabled()) {
            ...
            for (long j : jArr) {
                internalNotifyViewDisappeared(this.mId, new AutofillId(autofillId, j, this.mId));
            }
            if (CompatChanges.isChangeEnabled(NOTIFY_NODES_DISAPPEAR_NOW_SENDS_TREE_EVENTS)) {
                internalNotifyViewTreeEvent(this.mId, false);
            }
    ...
    public void flush() {
        internalNotifySessionFlushEvent(this.mId);
    }
