...
public abstract class RestrictionsReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (RestrictionsManager.ACTION_REQUEST_PERMISSION.equals(intent.getAction())) {
            onRequestPermission(context, intent.getStringExtra(RestrictionsManager.EXTRA_PACKAGE_NAME), intent.getStringExtra(RestrictionsManager.EXTRA_REQUEST_TYPE), intent.getStringExtra(RestrictionsManager.EXTRA_REQUEST_ID), (PersistableBundle) intent.getParcelableExtra(RestrictionsManager.EXTRA_REQUEST_BUNDLE, PersistableBundle.class));
        }
    }
}
