...
public class AlwaysOnHotwordDetector extends AbstractDetector {
    ...
    public static class EventPayload {
        ...
        private EventPayload(boolean z, AudioFormat audioFormat, int i, int i2, byte[] bArr, HotwordDetectedResult hotwordDetectedResult, ParcelFileDescriptor parcelFileDescriptor, List<SoundTrigger.KeyphraseRecognitionExtra> list, long j, boolean z2) {
        ...
        public static final class Builder {
            ...
            private List<SoundTrigger.KeyphraseRecognitionExtra> mKeyphraseExtras = Collections.EMPTY_LIST;
            ...
            public Builder setIsRecognitionStopped(boolean z) {
    ...
    public AlwaysOnHotwordDetector(String str, Locale locale, Executor executor, Callback callback, KeyphraseEnrollmentInfo keyphraseEnrollmentInfo, IVoiceInteractionManagerService iVoiceInteractionManagerService, int i, boolean z, String str2) {
        ...
        this.mExternalExecutor = executor == null ? new HandlerExecutor(new Handler(Looper.myLooper())) : executor;
    ...
    void initialize(PersistableBundle persistableBundle, SharedMemory sharedMemory, SoundTrigger.ModuleProperties moduleProperties) {
        AlwaysOnHotwordDetector alwaysOnHotwordDetector;
        if (this.mSupportSandboxedDetectionService) {
            alwaysOnHotwordDetector = this;
            alwaysOnHotwordDetector.initAndVerifyDetector(persistableBundle, sharedMemory, this.mInternalCallback, 1, this.mAttributionTag);
        } else {
            alwaysOnHotwordDetector = this;
        }
        try {
            ...
            if (moduleProperties == null) {
                moduleProperties = alwaysOnHotwordDetector.mModelManagementService.listModuleProperties(identity).stream().filter(new Predicate() { // from class: android.service.voice.AlwaysOnHotwordDetector$$ExternalSyntheticLambda0
            ...
            alwaysOnHotwordDetector.mSoundTriggerSession = alwaysOnHotwordDetector.mModelManagementService.createSoundTriggerSessionAsOriginator(identity, alwaysOnHotwordDetector.mBinder, moduleProperties);
            alwaysOnHotwordDetector.new RefreshAvailabilityTask().execute(new Void[0]);
        } catch (RemoteException e) {
            throw e.rethrowAsRuntimeException();
        }
    }

    static /* synthetic */ boolean lambda$initialize$0(SoundTrigger.ModuleProperties moduleProperties) {
    ...
    public void overrideAvailability(int i) {
        synchronized (this.mLock) {
            ...
            if (this.mKeyphraseMetadata == null && i == 2) {
    ...
    public boolean startRecognition(int i) {
        boolean z;
        synchronized (this.mLock) {
            z = startRecognitionLocked(i, new byte[0]) == 0;
        }
        return z;
    }
    ...
    public ModelParamRange queryParameter(int i) {
        ModelParamRange modelParamRangeQueryParameterLocked;
        synchronized (this.mLock) {
            int i2 = this.mAvailability;
            if (i2 == -3 || i2 == 3) {
                throw new IllegalStateException("queryParameter called on an invalid detector or error state");
            }
            modelParamRangeQueryParameterLocked = queryParameterLocked(i);
        }
    ...
    void onSoundModelsChanged() {
        synchronized (this.mLock) {
            ...
            if (i != -3 && i != -2 && i != 3) {
                ...
                if (i == 2) {
                    try {
                        int iStopRecognitionLocked = stopRecognitionLocked();
                        if (iStopRecognitionLocked == 0) {
                            sendSoundTriggerFailure(new SoundTriggerFailure(0, "stopped recognition because of enrollment update", 4));
                        }
                        Log.w(TAG, "Failed to stop recognition after enrollment update: code=" + iStopRecognitionLocked);
                    } catch (Exception e) {
    ...
    private int startRecognitionLocked(int i, byte[] bArr) {
        ...
        arrayList.add(new SoundTrigger.KeyphraseRecognitionExtra(this.mKeyphraseMetadata.getId(), this.mKeyphraseMetadata.getRecognitionModeFlags(), 0, new SoundTrigger.ConfidenceLevel[0]));
        boolean z = (i & 1) != 0;
        boolean z2 = (i & 2) != 0;
        boolean z3 = (i & 16) != 0;
        int i3 = (i & 4) == 0 ? 0 : 1;
        ...
        try {
            int iStartRecognition = this.mSoundTriggerSession.startRecognition(this.mKeyphraseMetadata.getId(), this.mLocale.toLanguageTag(), this.mInternalCallback, new SoundTrigger.RecognitionConfig.Builder().setCaptureRequested(z).setMultipleTriggersAllowed(z2).setKeyphrases(arrayList).setData(bArr).setAudioCapabilities(i3).build(), z3);
    ...
    private int stopRecognitionLocked() {
        try {
            int iStopRecognition = this.mSoundTriggerSession.stopRecognition(this.mKeyphraseMetadata.getId(), this.mInternalCallback);
    ...
    private ModelParamRange queryParameterLocked(int i) {
        try {
            SoundTrigger.ModelParamRange modelParamRangeQueryParameter = this.mSoundTriggerSession.queryParameter(this.mKeyphraseMetadata.getId(), i);
            if (modelParamRangeQueryParameter == null) {
                return null;
            }
            return new ModelParamRange(modelParamRangeQueryParameter);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void sendUnknownFailure(String str) {
    ...
    static final class SoundTriggerListener extends IHotwordRecognitionStatusCallback.Stub {
        ...
        public void onKeyphraseDetectedFromExternalSource(HotwordDetectedResult hotwordDetectedResult) {
            ...
            EventPayload.Builder builder = new EventPayload.Builder();
            if (Flags.enableHotwordWearableSensingApi()) {
                builder.setIsRecognitionStopped(false);
            }
            Message.obtain(this.mHandler, 2, builder.setHotwordDetectedResult(hotwordDetectedResult).build()).sendToTarget();
        }
        ...
        public void onUnknownFailure(String str) throws RemoteException {
            Slog.v(AlwaysOnHotwordDetector.TAG, "onUnknownFailure: " + str);
            Handler handler = this.mHandler;
            if (TextUtils.isEmpty(str)) {
                str = "Error data is null";
            }
            Message.obtain(handler, 11, str).sendToTarget();
        }
        ...
        public void onStatusReported(int i) {
            ...
            Message messageObtain = Message.obtain(this.mHandler, 7);
    ...
    class MyHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            synchronized (AlwaysOnHotwordDetector.this.mLock) {
                ...
                final Message messageObtain = Message.obtain(message);
                Binder.withCleanCallingIdentity(new FunctionalUtils.ThrowingRunnable() { // from class: android.service.voice.AlwaysOnHotwordDetector$MyHandler$$ExternalSyntheticLambda1
                    ...
                    public final void runOrThrow() throws Exception {
                        this.f$0.lambda$handleMessage$1(messageObtain);
                    }
                });
            }
        }
        ...
        public /* synthetic */ void lambda$handleMessage$1(final Message message) throws Exception {
            AlwaysOnHotwordDetector.this.mExternalExecutor.execute(new Runnable() { // from class: android.service.voice.AlwaysOnHotwordDetector$MyHandler$$ExternalSyntheticLambda0
                ...
                public final void run() {
                    this.f$0.lambda$handleMessage$0(message);
                }
            });
        }
    ...
    class RefreshAvailabilityTask extends AsyncTask<Void, Void, Void> {
        ...
        public Void doInBackground(Void... voidArr) {
            try {
                ...
                synchronized (AlwaysOnHotwordDetector.this.mLock) {
                    if (iInternalGetInitialAvailability == 0) {
                        internalUpdateEnrolledKeyphraseMetadata();
                        iInternalGetInitialAvailability = AlwaysOnHotwordDetector.this.mKeyphraseMetadata != null ? 2 : 1;
                        AlwaysOnHotwordDetector.this.updateAndNotifyStateChangedLocked(iInternalGetInitialAvailability);
                    } else {
                        AlwaysOnHotwordDetector.this.updateAndNotifyStateChangedLocked(iInternalGetInitialAvailability);
                    }
                }
