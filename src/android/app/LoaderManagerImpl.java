...
class LoaderManagerImpl extends LoaderManager {
    ...
    void doStart() {
        ...
        if (this.mStarted) {
            Log.w(TAG, "Called doStart when already started: " + this, new RuntimeException("here"));
            return;
        }
        this.mStarted = true;
    ...
    void doStop() {
        ...
        if (!this.mStarted) {
            Log.w(TAG, "Called doStop when not started: " + this, new RuntimeException("here"));
            return;
        }
        for (int size = this.mLoaders.size() - 1; size >= 0; size--) {
    ...
    void doRetain() {
        ...
        if (!this.mStarted) {
            Log.w(TAG, "Called doRetain when not started: " + this, new RuntimeException("here"));
            return;
        }
        this.mRetaining = true;
        this.mStarted = false;
