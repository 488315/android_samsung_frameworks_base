...
public final class AudioHalCapParameter implements Parcelable {
    ...
    public void readFromParcel(Parcel parcel) {
        ...
        if (readInt == 0) {
            _set(readInt, (StrategyDevice) parcel.readTypedObject(StrategyDevice.CREATOR));
            return;
        }
        if (readInt == 1) {
            _set(readInt, (InputSourceDevice) parcel.readTypedObject(InputSourceDevice.CREATOR));
            return;
        }
        if (readInt == 2) {
            _set(readInt, (StrategyDeviceAddress) parcel.readTypedObject(StrategyDeviceAddress.CREATOR));
        } else if (readInt == 3) {
            _set(readInt, (StreamVolumeProfile) parcel.readTypedObject(StreamVolumeProfile.CREATOR));
        } else {
