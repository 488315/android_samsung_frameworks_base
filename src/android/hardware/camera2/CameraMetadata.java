...
public abstract class CameraMetadata<TKey> {
    ...
    <TKey> ArrayList<TKey> getKeys(Class<?> cls, Class<TKey> cls2, CameraMetadata<TKey> cameraMetadata, int[] iArr, boolean z) throws IllegalAccessException, IllegalArgumentException {
        ...
        CameraMetadataNative cameraMetadataNative = this.mNativeInstance;
        if (cameraMetadataNative != null && (allVendorKeys = cameraMetadataNative.getAllVendorKeys(cls2)) != null) {
            Iterator it = allVendorKeys.iterator();
            while (it.hasNext()) {
                Object next = it.next();
                if (next instanceof CaptureRequest.Key) {
                    CaptureRequest.Key key = (CaptureRequest.Key) next;
                    name = key.getName();
                    vendorId = key.getVendorId();
                } else if (next instanceof CaptureResult.Key) {
                    CaptureResult.Key key2 = (CaptureResult.Key) next;
                    name = key2.getName();
                    vendorId = key2.getVendorId();
                } else if (next instanceof CameraCharacteristics.Key) {
                    CameraCharacteristics.Key key3 = (CameraCharacteristics.Key) next;
                    name = key3.getName();
    ...
    /* JADX WARN: Multi-variable type inference failed */
    private static <TKey> boolean shouldKeyBeAdded(TKey tkey, Field field, int[] iArr, boolean z) {
        ...
        return field.getAnnotation(SyntheticKey.class) != null ? z : Arrays.binarySearch(iArr, nativeKey.getTag()) >= 0;
    }
}
