...
public class ImsManager {
    ...
    public ImsRcsManager getImsRcsManager(int i) {
        ...
        return new ImsRcsManager(this.mContext, i, sRcsCache, sTelephonyCache);
    }
    ...
    public ImsMmTelManager getImsMmTelManager(int i) {
        ...
        return new ImsMmTelManager(this.mContext, i, sTelephonyCache);
    }

    @SystemApi
    public SipDelegateManager getSipDelegateManager(int i) {
        if (!SubscriptionManager.isValidSubscriptionId(i)) {
            throw new IllegalArgumentException("Invalid subscription ID: " + i);
        }
        return new SipDelegateManager(this.mContext, i, sRcsCache, sTelephonyCache);
    }

    public ProvisioningManager getProvisioningManager(int i) {
        if (!SubscriptionManager.isValidSubscriptionId(i)) {
            throw new IllegalArgumentException("Invalid subscription ID: " + i);
        }
