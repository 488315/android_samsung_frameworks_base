...
public class EuiccManager {
    ...
    private EuiccManager(Context context, int i) {
        this.mContext = context;
        this.mCardId = i;
    }
    ...
    public void continueOperation(Intent intent, Bundle bundle) {
        if (!isEnabled()) {
            PendingIntent pendingIntent = (PendingIntent) intent.getParcelableExtra(EXTRA_EMBEDDED_SUBSCRIPTION_RESOLUTION_CALLBACK_INTENT, PendingIntent.class);
    ...
    public void switchToSubscription(int i, int i2, PendingIntent pendingIntent) {
        ...
        try {
            if (this.mContext.checkCallingOrSelfPermission(Manifest.permission.WRITE_EMBEDDED_SUBSCRIPTIONS) != 0 && !getIEuiccController().hasCarrierPrivilegesForPackageOnAnyPhone(this.mContext.getOpPackageName())) {
    ...
    public void setSupportedCountries(List<String> list) {
        if (isEnabled()) {
            try {
                getIEuiccController().setSupportedCountries(true, (List) list.stream().map(new EuiccManager$$ExternalSyntheticLambda0()).collect(Collectors.toList()));
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
    ...
    public void setUnsupportedCountries(List<String> list) {
        if (isEnabled()) {
            try {
                getIEuiccController().setSupportedCountries(false, (List) list.stream().map(new EuiccManager$$ExternalSyntheticLambda0()).collect(Collectors.toList()));
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
    ...
    public boolean isSupportedCountry(String str) {
        ...
        try {
            return getIEuiccController().isSupportedCountry(str.toUpperCase(Locale.ROOT));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    private int getCardIdForDefaultEuicc() {
        ...
        if (packageManager == null || !packageManager.hasSystemFeature(PackageManager.FEATURE_TELEPHONY_EUICC)) {
            return -2;
        }
        return ((TelephonyManager) this.mContext.getSystemService(TelephonyManager.class)).getCardIdForDefaultEuicc();
    }
    ...
    @SystemApi
    public void setPsimConversionSupportedCarriers(Set<Integer> set) {
        if (!isEnabled()) {
            throw new IllegalStateException("Euicc is not enabled");
        }
        try {
            getIEuiccController().setPsimConversionSupportedCarriers(set.stream().mapToInt(new PreferentialNetworkServiceConfig$$ExternalSyntheticLambda2()).toArray());
        } catch (RemoteException e) {
