    ...
    static class SmHandler extends Handler {
        ...
        static class StateInfo {
            ...
            public String toString() {
                ...
                sb.append(stateInfo == null ? PerfettoProtoLogImpl.NULL_STRING : stateInfo.state.getName());
                return sb.toString();
            }
        }
        ...
        private void performTransitions(State state, Message message) {
            ...
            boolean z = this.mSm.recordLogRec(message) && message.obj != mSmHandlerObj;
            if (this.mLogRecords.logOnlyTransitions()) {
                if (this.mDestState != null) {
                    LogRecords logRecords = this.mLogRecords;
                    StateMachine stateMachine = this.mSm;
                    logRecords.add(stateMachine, message, stateMachine.getLogRecString(message), state, state2, this.mDestState);
                }
            } else if (z) {
                LogRecords logRecords2 = this.mLogRecords;
                StateMachine stateMachine2 = this.mSm;
                logRecords2.add(stateMachine2, message, stateMachine2.getLogRecString(message), state, state2, this.mDestState);
            }
