    ...
    public void dump(android.util.IndentingPrintWriter indentingPrintWriter) {
        synchronized (this.mLock) {
            ...
            for (int i = 0; i < this.SIZE; i++) {
                ...
                indentingPrintWriter.println(String.format("%s: count=%d, total=%.1fms, avg=%.3fms, max calls/s=%d max dur/s=%.1fms max time=%.1fms", this.mLabels[i], Integer.valueOf(i2), Double.valueOf(d), Double.valueOf(i2 == 0 ? SContextConstants.ENVIRONMENT_VALUE_UNKNOWN : d / i2), Integer.valueOf(this.mMaxCallsPerSecond[i]), Double.valueOf(this.mMaxDurationPerSecond[i] / 1000.0d), Double.valueOf(this.mMaxDurationStats[i] / 1000.0d)));
            }
