...
public class BitwiseInputStream {
    ...
    public int read(int i) throws AccessException {
        ...
        if (i < 0 || i > 8 || i2 + i > this.mEnd) {
            throw new AccessException("illegal read (pos " + this.mPos + ", end " + this.mEnd + ", bits " + i + NavigationBarInflaterView.KEY_CODE_END);
        }
        byte[] bArr = this.mBuf;
    ...
    public void skip(int i) throws AccessException {
        int i2 = this.mPos;
        if (i2 + i > this.mEnd) {
            throw new AccessException("illegal skip (pos " + this.mPos + ", end " + this.mEnd + ", bits " + i + NavigationBarInflaterView.KEY_CODE_END);
        }
        this.mPos = i2 + i;
    }
}
