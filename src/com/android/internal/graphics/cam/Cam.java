...
public class Cam {
    ...
    public static Cam fromIntInFrame(int i, Frame frame) {
        ...
        float fPow = (float) Math.pow((frame.getFl() * Math.abs(f9)) / 100.0d, 0.42d);
        float fPow2 = (float) Math.pow((frame.getFl() * Math.abs(f10)) / 100.0d, 0.42d);
        float fPow3 = (float) Math.pow((frame.getFl() * Math.abs(f11)) / 100.0d, 0.42d);
        ...
        float c = (4.0f / frame.getC()) * ((float) Math.sqrt(fPow4 / 100.0f)) * (frame.getAw() + 4.0f) * frame.getFlRoot();
        float fPow5 = ((float) Math.pow((((((((float) (Math.cos((((((double) f17) < 20.14d ? 360.0f + f17 : f17) * 3.141592653589793d) / 180.0d) + 2.0d) + 3.8d)) * 0.25f) * 3846.1538f) * frame.getNc()) * frame.getNcb()) * ((float) Math.sqrt((f12 * f12) + (f13 * f13)))) / (f15 + 0.305f), 0.9d)) * ((float) Math.pow(1.64d - Math.pow(0.29d, frame.getN()), 0.73d)) * ((float) Math.sqrt(fPow4 / 100.0d));
        float flRoot = fPow5 * frame.getFlRoot();
        float fSqrt = ((float) Math.sqrt((r1 * frame.getC()) / (frame.getAw() + 4.0f))) * 50.0f;
        float f19 = (1.7f * fPow4) / ((0.007f * fPow4) + 1.0f);
        float fLog = ((float) Math.log((0.0228f * flRoot) + 1.0f)) * 43.85965f;
        double d2 = f18;
        return new Cam(f17, fPow5, fPow4, c, flRoot, fSqrt, f19, fLog * ((float) Math.cos(d2)), fLog * ((float) Math.sin(d2)));
    }
    ...
    private static Cam fromJchInFrame(float f, float f2, float f3, Frame frame) {
        ...
        float fSqrt = ((float) Math.sqrt(((f2 / ((float) Math.sqrt(r4))) * frame.getC()) / (frame.getAw() + 4.0f))) * 50.0f;
        float f4 = (1.7f * f) / ((0.007f * f) + 1.0f);
        float fLog = ((float) Math.log((flRoot * 0.0228d) + 1.0d)) * 43.85965f;
        double d = (3.1415927f * f3) / 180.0f;
        return new Cam(f3, f2, f, c, flRoot, fSqrt, f4, ((float) Math.cos(d)) * fLog, fLog * ((float) Math.sin(d)));
    }
    ...
    public int viewed(Frame frame) {
        float fPow = (float) Math.pow(((((double) getChroma()) == SContextConstants.ENVIRONMENT_VALUE_UNKNOWN || ((double) getJ()) == SContextConstants.ENVIRONMENT_VALUE_UNKNOWN) ? 0.0f : getChroma() / ((float) Math.sqrt(getJ() / 100.0d))) / Math.pow(1.64d - Math.pow(0.29d, frame.getN()), 0.73d), 1.1111111111111112d);
        ...
        float fSignum = Math.signum(f5) * (100.0f / frame.getFl()) * ((float) Math.pow((float) Math.max(SContextConstants.ENVIRONMENT_VALUE_UNKNOWN, (Math.abs(f5) * 27.13d) / (400.0d - Math.abs(f5))), 2.380952380952381d));
        float fSignum2 = Math.signum(f6) * (100.0f / frame.getFl()) * ((float) Math.pow((float) Math.max(SContextConstants.ENVIRONMENT_VALUE_UNKNOWN, (Math.abs(f6) * 27.13d) / (400.0d - Math.abs(f6))), 2.380952380952381d));
        float fSignum3 = Math.signum(((f4 - (f2 * 220.0f)) - (f3 * 6300.0f)) / 1403.0f) * (100.0f / frame.getFl()) * ((float) Math.pow((float) Math.max(SContextConstants.ENVIRONMENT_VALUE_UNKNOWN, (Math.abs(r8) * 27.13d) / (400.0d - Math.abs(r8))), 2.380952380952381d));
        float f7 = fSignum / frame.getRgbD()[0];
        float f8 = fSignum2 / frame.getRgbD()[1];
        float f9 = fSignum3 / frame.getRgbD()[2];
    ...
    public static int getInt(float f, float f2, float f3, Frame frame) {
        ...
        while (Math.abs(f4 - f2) >= 0.4f) {
            Cam camFindCamByJ = findCamByJ(fMin, f5, f3);
            if (!z) {
    ...
    private static Cam findCamByJ(float f, float f2, float f3) {
        ...
        while (Math.abs(f6 - f4) > 0.01f) {
            ...
            if (fAbs < 0.2f) {
                ...
                float fDistance = camFromInt.distance(fromJch(camFromInt.getJ(), camFromInt.getChroma(), f));
                if (fDistance <= 1.0f) {
                    cam = camFromInt;
                    f5 = fAbs;
                    f7 = fDistance;
