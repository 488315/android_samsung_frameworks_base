...
public class HeightInModifierOperation extends DimensionInModifierOperation {
    ...
    public HeightInModifierOperation(float f, float f2) {
        super(232, f, f2);
    }
    ...
    public static void read(WireBuffer wireBuffer, List<Operation> list) {
        list.add(new HeightInModifierOperation(wireBuffer.readFloat(), wireBuffer.readFloat()));
    }
    ...
    public static void documentation(DocumentationBuilder documentationBuilder) {
        documentationBuilder.operation("Layout Operations", 232, CLASS_NAME).description("Add additional constraints to the height").field(1, ParameterCapability.CAPABILITY_MIN, "The minimum height, -1 if not applied").field(1, "max", "The maximum height, -1 if not applied");
    }

    public static void apply(WireBuffer wireBuffer, float f, float f2) {
        wireBuffer.start(232);
    ...
    public void serializeToString(int i, StringSerializer stringSerializer) {
        stringSerializer.append(i, "HEIGHT_IN = [" + getMin() + ", " + getMax() + NavigationBarInflaterView.SIZE_MOD_END);
    }
