...
public class MenuPopupHelper implements MenuHelper {
    ...
    private MenuPopup createPopup() {
        ...
        Rect bounds = ((WindowManager) this.mContext.getSystemService(WindowManager.class)).getMaximumWindowMetrics().getBounds();
        if (Math.min(bounds.width(), bounds.height()) >= this.mContext.getResources().getDimensionPixelSize(R.dimen.cascading_menus_min_smallest_width)) {
    ...
    private void showPopup(int i, int i2, boolean z, boolean z2) {
        ...
        if (z) {
            int absoluteGravity = Gravity.getAbsoluteGravity(this.mDropDownGravity, this.mAnchorView.getLayoutDirection()) & 7;
            if (this.mIsThemeDeviceDefaultFamily && !this.mIsContextMenu) {
                boolean z3 = this.mAnchorView.getLayoutDirection() == 1;
                i -= this.mContext.getResources().getDimensionPixelOffset(R.dimen.sem_menu_popup_horizontal_offset);
                popup.setHorizontalOffset(z3 ? -i : i);
            } else {
    ...
    protected void setIsContextMenuPopup(boolean z) {
