...
public class ImsConfig {
    ...
    public int getConfigInt(int i) throws ImsException {
        try {
            int configInt = this.miConfig.getConfigInt(i);
            if (this.DBG) {
                Rlog.d(TAG, "getInt(): item = " + i + ", ret =" + configInt);
            }
            return configInt;
    ...
    public String getProvisionedStringValue(int i) throws ImsException {
        return getConfigString(i);
    }

    public String getConfigString(int i) throws ImsException {
        try {
            String configString = this.miConfig.getConfigString(i);
            if (this.DBG) {
                Rlog.d(TAG, "getConfigString(): item = " + i + ", ret =" + configString);
            }
            return configString;
        } catch (RemoteException e) {
    ...
    public int setProvisionedStringValue(int i, String str) throws ImsException {
        return setConfig(i, str);
    }

    public int setConfig(int i, int i2) throws ImsException {
        if (this.DBG) {
            Rlog.d(TAG, "setConfig(): item = " + i + "value = " + i2);
        }
        try {
            int configInt = this.miConfig.setConfigInt(i, i2);
            if (this.DBG) {
                Rlog.d(TAG, "setConfig(): item = " + i + " value = " + i2 + " ret = " + configInt);
            }
            return configInt;
        } catch (RemoteException e) {
    ...
    public int setConfig(int i, String str) throws ImsException {
        if (this.DBG) {
            Rlog.d(TAG, "setConfig(): item = " + i + "value = " + str);
        }
        try {
            int configString = this.miConfig.setConfigString(i, str);
            if (this.DBG) {
                Rlog.d(TAG, "setConfig(): item = " + i + " value = " + str + " ret = " + configString);
            }
            return configString;
        } catch (RemoteException e) {
            throw new ImsException("setConfig()", e, 131);
    ...
    public void addConfigCallback(IImsConfigCallback iImsConfigCallback) throws ImsException {
        if (this.DBG) {
            Rlog.d(TAG, "addConfigCallback: " + iImsConfigCallback);
        }
        try {
    ...
    public void removeConfigCallback(IImsConfigCallback iImsConfigCallback) throws ImsException {
        if (this.DBG) {
            Rlog.d(TAG, "removeConfigCallback: " + iImsConfigCallback);
        }
        try {
