...
public class ImsUceManager {
    ...
    public void createUceService(boolean z) {
        if (z && ServiceManager.checkService(getUceServiceName()) == null) {
            return;
        }
        IBinder service = ServiceManager.getService(getUceServiceName());
        if (service != null) {
            try {
    ...
    private class UceServiceDeathRecipient implements IBinder.DeathRecipient {
        ...
        public void binderDied() {
            ...
            if (ImsUceManager.this.mContext != null) {
                ImsUceManager.this.mContext.sendBroadcast(new Intent(new Intent(ImsUceManager.ACTION_UCE_SERVICE_DOWN)));
            }
        }
    }
}
