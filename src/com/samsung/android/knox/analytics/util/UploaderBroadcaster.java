...
public class UploaderBroadcaster {
    ...
    private static final String TAG = "[KnoxAnalytics] UploaderBroadcaster";
    ...
    public static void broadcastAnalyticsStatus(Context context, boolean z, boolean z2) {
        Log.d(TAG, "broadcastAnalyticsStatus(" + String.valueOf(z) + "," + String.valueOf(z2) + NavigationBarInflaterView.KEY_CODE_END);
        Intent intent = new Intent(BROADCAST_STATUS_TO_UPLOADER_ACTION);
        intent.setComponent(new ComponentName(UPLOADER_PACKAGENAME, UPLOADER_COMPONENT)).putExtra(BROADCAST_STATUS_TO_UPLOADER_EXTRA_KEY, getStatusValue(z, z2));
        context.sendBroadcastAsUser(intent, UserHandle.SYSTEM);
    }

    public static void broadcastKAStatusToMDM(Context context, boolean z, boolean z2, boolean z3) {
        Intent intent;
        Log.d(TAG, "broadcastKAServiceStatusToMDM( start:" + String.valueOf(z) + " , isNewActivation:" + String.valueOf(z2) + " , isForceful:" + String.valueOf(z3) + NavigationBarInflaterView.KEY_CODE_END);
        if (z) {
            intent = new Intent(ACTIVATE_KNOX_ANALYTICS_DEVICE_STATUS);
            intent.setPackage(EnterpriseProxyConstants.ENTERPRISE_AUTH_DIALOG_PACKAGE).putExtra("isNewActivation", z2);
        } else {
            intent = new Intent(DEACTIVATE_KNOX_ANALYTICS_DEVICE_STATUS);
            intent.setPackage(EnterpriseProxyConstants.ENTERPRISE_AUTH_DIALOG_PACKAGE).putExtra("isForceful", z3);
        }
        context.sendBroadcastAsUser(intent, UserHandle.SYSTEM);
    }

    private static String getStatusValue(boolean z, boolean z2) {
        if (z) {
