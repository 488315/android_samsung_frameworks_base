package com.samsung.android.rune;

import android.os.SystemProperties;

/* loaded from: classes6.dex */
public class ResRune {
    public static final boolean ALARM_MANAGER_AUTO_POWER_UP = true;
    public static final boolean ALARM_MANAGER_DUMP = true;
    public static final boolean ALARM_MANAGER_FIX_WAKELOCK_SET_WORKSOURCE = true;
    public static final boolean ALARM_MANAGER_GET_NEXT_ALARM_CLOCKS = true;
    public static final boolean APP_ICON = true;
    public static final boolean CLEAR_THEMEPARK_OVERLAYS_ON_UNINSTALL = true;
    public static final boolean COLOR_PALETTE_DISABLE_AOSP = true;
    public static final boolean COLOR_THEME_VALIDATOR_SERVICE = true;
    public static final boolean CST_FEATURE_ENABLED = true;
    public static final boolean DISABLE_COLOR_PALETTE_MAINTENANCE_MODE = true;
    public static final boolean DISABLE_THEME = true;
    public static final boolean DISABLE_USER_TRACKER_DYNAMIC_COLOR = true;
    public static final boolean ENABLE_AOSP_DYNAMIC_COLOR = true;
    public static final boolean ICON_EXPAND_SCOPE_OF_LIVEICON = true;
    public static final boolean ICON_FIX_REDUCTION = true;
    public static final boolean ICON_LOCKSCREEN_SHORTCUT_ICON = true;
    public static final boolean ICON_SELECTIVE_DAY_NIGHT_MODE = true;
    public static final boolean ICON_SEM_ARCHIVED_APP_ICON = true;
    public static final boolean IDMAP_PREVENT_OVERLAY_XML_FOR_SAMSUNGTHEME = true;
    public static final boolean OMS_DUMP_THEME_INFO = true;
    public static final boolean OMS_EXT_SUPPORT = true;
    public static final boolean OM_BACKUP_SETTINGS = true;
    public static final boolean OM_BUG_FIX_LOST_OVERLAY_WHEN_UPDATE_UNINSTALL = true;
    public static final boolean REMOVE_COLOR_CACHE_ON_UIMODE_CONFIG_CHANGE = true;
    public static final boolean REMOVE_STALE_OVERLAYS_ON_REBOOT = true;
    public static final boolean RESOURCES_BUG_FIX = true;
    public static final boolean RESOURCES_BUG_FIX_EMPTY_APK_ASSETS_ISSUE = true;
    public static final boolean RESOURCES_BUG_FIX_NPE_IN_DUMP_HISTORY = true;
    public static final boolean RESOURCES_BUG_FIX_UPDATE_INVALID_OVERLAY_PATHS = true;
    public static final boolean RESOURCES_BUG_FIX_UPDATE_SERVICE_RECORD = true;
    public static final boolean RESOURCES_FIX_OLD_ASSETS_ADDED = true;
    public static final boolean RES_FIX_ASSET_NAVIBAR = true;
    public static final boolean RM_SEP = true;
    public static final boolean SEM_WALLPAPER_THEME_REGISTER_IN_MUM = true;
    public static final boolean THEMEPARK_DUMMY_PROGRESS_ON_COVERSCREEN = true;
    public static final boolean THEMEPARK_FULL_THEME = true;
    public static final boolean THEMEPARK_ICON_PACK = true;
    public static final boolean THEMEPARK_SINGLE_THEME = true;
    public static final boolean THEMEPARK_SINGLE_THEME_PRIORITY = true;
    public static final boolean THEME_CREATE_IDMAP_EARLIER = true;
    public static final boolean THEME_FIX_APP_ICON_PALETTE_RESTORATION = true;
    public static final boolean THEME_FIX_TOO_MUCH_LOG = true;
    public static final boolean THEME_OVERLAYS_SANITY_CHECK = true;
    public static final boolean THEME_RESOURCE_MAPPING = true;
    public static final boolean THEME_SUPPORT = true;
    public static final boolean THEME_WALLPAPER_THEMING = true;
    public static final boolean THEME_WALLPAPER_THEMING_DEBUG = SystemProperties.getBoolean("debug.wallpaper.theme.enable", false);
    public static final boolean THEME_WINDOW_BACKGROUND_CROP = true;
    public static final boolean WARNING_ABNORMAL_METADATA = true;
    public static final boolean ZIPPED_OVERLAY_SUPPORT_BLOCKED = true;
}
