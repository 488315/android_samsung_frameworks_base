...
public class SettingsWrapper {
    ...
    public boolean isBugReportMode() {
        int i = Settings.Global.getInt(this.mResolver, "bugreport_in_power_menu", State.OFF);
        this.mLogWrapper.i(TAG, "(BUGREPORT_IN_POWER_MENU)current value : " + i);
        return i == State.ON;
    }

    public boolean isTaskBarEnabled() {
        boolean z = Settings.Global.getInt(this.mResolver, Settings.Global.SEM_TASK_BAR, 0) == 1;
        this.mLogWrapper.i(TAG, "(SEM_TASK_BAR)current value : " + z);
        return z;
    }

    public boolean isNavBarGestureType() {
        boolean z = Settings.Global.getInt(this.mResolver, Settings.Global.NAVIGATION_BAR_GESTURE_WHILE_HIDDEN, 0) > 0;
        this.mLogWrapper.i(TAG, "(NAVIGATION_BAR_GESTURE_WHILE_HIDDEN)current value : " + z);
        return z;
    }
    ...
    public boolean isUltraPowerSavingMode() {
        int i = Settings.System.getInt(this.mResolver, Settings.System.SEM_ULTRA_POWERSAVING_MODE, State.OFF);
        this.mLogWrapper.i(TAG, "(SEM_ULTRA_POWERSAVING_MODE)current value : " + i);
        return i == State.ON;
    }

    public boolean isShopDemo() {
        int i = Settings.Secure.getInt(this.mContext.getContentResolver(), "shopdemo", State.OFF);
        this.mLogWrapper.i(TAG, "(isShopDemo)current value : " + i);
        return i == State.ON;
    }
    ...
    public boolean isBikeMode() {
        int i = Settings.Secure.getInt(this.mResolver, AudioConstants.SETTING_BIKE_MODE, State.OFF);
        this.mLogWrapper.i(TAG, "(isBikeMode)current value : " + i);
        return i == State.ON;
    }
