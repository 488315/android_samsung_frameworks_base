...
public class KeyEventDispatcher implements Handler.Callback {
  ...
  public KeyEventDispatcher(Handler handler, int i, Object obj, PowerManager powerManager) {
    if (InputEventConsistencyVerifier.isInstrumentationEnabled()) {
      this.mSentEventsVerifier =
          new InputEventConsistencyVerifier(this, 0, KeyEventDispatcher.class.getSimpleName());
    } else {
  ...
  public void setOnKeyEventResult(KeyEventFilter keyEventFilter, boolean z, int i) {
    synchronized (this.mLock) {
      PendingKeyEvent removeEventFromListLocked =
          removeEventFromListLocked((List) this.mPendingEventsMap.get(keyEventFilter), i);
