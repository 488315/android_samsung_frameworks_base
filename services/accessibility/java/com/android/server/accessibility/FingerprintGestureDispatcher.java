...
public class FingerprintGestureDispatcher extends IFingerprintClientActiveCallback.Stub
  ...
  public FingerprintGestureDispatcher(
      IFingerprintService iFingerprintService, Resources resources, Object obj) {
    this.mFingerprintService = iFingerprintService;
    this.mHardwareSupportsGestures = resources.getBoolean(R.bool.config_enableSafetyCenter);
    this.mLock = obj;
  }
    ...
    if (this.mHardwareSupportsGestures) {
      synchronized (this.mLock) {
        ...
        for (int i = 0; i < list.size(); i++) {
          FingerprintGestureClient fingerprintGestureClient =
              (FingerprintGestureClient) list.get(i);
    ...
    if (this.mHardwareSupportsGestures) {
      synchronized (this.mLock) {
        for (int i = 0; i < this.mCapturingClients.size(); i++) {
          ((FingerprintGestureClient) this.mCapturingClients.get(i))
              .onFingerprintGestureDetectionActiveChanged(!z);
        }
      }
    }
  }
  ...
  public boolean handleMessage(Message message) {
      ...
      try {
        try {
          ...
          Slog.e(
              ...
              "Failed to register for fingerprint activity callbacks");
        }
        return false;
      } finally {
      }
    ...
    if (i == 2) {
      ...
      try {
        ...
        Slog.e(
            "FingerprintGestureDispatcher",
            "Failed to unregister for fingerprint activity callbacks");
      }
