...
public class MagnificationProcessor {
  ...
  public MagnificationConfig getMagnificationConfig(int i) {
    ...
    if (controllingMode == 1) {
      ...
      builder
          .setMode(controllingMode)
          .setActivated(this.mController.isActivated(i, 1))
      ...
      WindowMagnificationManager windowMagnificationMgr =
          this.mController.getWindowMagnificationMgr();
      builder
          .setMode(controllingMode)
          .setActivated(this.mController.isActivated(i, 2))
          .setScale(windowMagnificationMgr.getScale(i))
          .setCenterX(windowMagnificationMgr.getCenterX(i))
          .setCenterY(windowMagnificationMgr.getCenterY(i));
    } else {
  ...
  public boolean setMagnificationConfig(
      int i, MagnificationConfig magnificationConfig, boolean z, int i2) {
    ...
    if (mode == 1) {
      if (isActivated) {
        return setScaleAndCenterForFullScreenMagnification(
            i,
  ...
  public final boolean setScaleAndCenterForFullScreenMagnification(
      int i, float f, float f2, float f3, boolean z, int i2) {
    if (!isRegistered(i)) {
      register(i);
    }
    return this.mController
        .getFullScreenMagnificationController()
        .setScaleAndCenter(i, f, f2, f3, z, i2);
  }

  public final boolean transitionModeIfNeeded(
      int i, MagnificationConfig magnificationConfig, boolean z, int i2) {
    int controllingMode = getControllingMode(i);
    ...
    if (controllingMode == magnificationConfig.getMode()
        && !this.mController.hasDisableMagnificationCallback(i)) {
      return false;
    }
    ...
    if (controllingMode == 2) {
      return this.mController
          ...
          .disableWindowMagnification(
              i, false, z ? MagnificationAnimationCallback.STUB_ANIMATION_CALLBACK : null);
    }
    return false;
  }
  ...
  public int getControllingMode(int i) {
    ...
    return (!this.mController.isActivated(i, 1)
            && this.mController.getLastMagnificationActivatedMode(i) == 2)
        ? 2
        : 1;
  }
  ...
  public void dump(PrintWriter printWriter, ArrayList arrayList) {
    for (int i = 0; i < arrayList.size(); i++) {
      ...
      printWriter
          .append(
              (CharSequence)
                  ("    IdOfLastServiceToMagnify="
                      + getIdOfLastServiceToMagnify(magnificationConfig.getMode(), displayId)))
          .println();
    ...
    printWriter
        .append(
            (CharSequence)
                ("    SupportWindowMagnification=" + this.mController.supportWindowMagnification()))
        .println();
    printWriter
        .append(
            (CharSequence)
                ("    WindowMagnificationConnectionState="
                    + this.mController.getWindowMagnificationMgr().getConnectionState()))
        .println();
  }
  ...
  public final int getIdOfLastServiceToMagnify(int i, int i2) {
    if (i == 1) {
      return this.mController
          .getFullScreenMagnificationController()
          .getIdOfLastServiceToMagnify(i2);
    }
