...
public class AccessibilityServiceConnection extends AbstractAccessibilityServiceConnection {
  ...
  public AccessibilityServiceConnection(
    ...
    try {
      component.putExtra(
          ...
          this.mSystemSupport.getPendingIntentActivity(
              this.mContext, 0, new Intent("android.settings.ACCESSIBILITY_SETTINGS"), 67108864));
    } finally {
    ...
    try {
      ...
      if (this.mService == null
          && this.mContext.bindServiceAsUser(
              this.mIntent, this, i, new UserHandle(accessibilityUserState.mUserId))) {
      ...
      this.mActivityTaskManagerService.setAllowAppSwitches(
          this.mComponentName.flattenToString(),
          this.mAccessibilityServiceInfo.getResolveInfo().serviceInfo.applicationInfo.uid,
  ...
  public void unbindLocked() {
    ...
    this.mActivityTaskManagerService.setAllowAppSwitches(
        this.mComponentName.flattenToString(), -1, accessibilityUserState.mUserId);
    resetLocked();
  }

  public boolean canRetrieveInteractiveWindowsLocked() {
    return this.mSecurityPolicy.canRetrieveWindowContentLocked(this)
  ...
  public void disableSelf() {
    ...
    synchronized (this.mLock) {
      ...
      if (accessibilityUserState.getEnabledServicesLocked().remove(this.mComponentName)) {
        long clearCallingIdentity = Binder.clearCallingIdentity();
        try {
          this.mSystemSupport.persistComponentNamesToSettingLocked(
              "enabled_accessibility_services",
              accessibilityUserState.getEnabledServicesLocked(),
              accessibilityUserState.mUserId);
          Binder.restoreCallingIdentity(clearCallingIdentity);
      ...
      this.mMainHandler.sendMessage(
          PooledLambda.obtainMessage(
              new Consumer() { // from class:
                // com.android.server.accessibility.AccessibilityServiceConnection$$ExternalSyntheticLambda1
  ...
  */
  public final void initializeService() {
    ...
    synchronized (this.mLock) {
      ...
      Set bindingServicesLocked = accessibilityUserState.getBindingServicesLocked();
      Set crashedServicesLocked = accessibilityUserState.getCrashedServicesLocked();
      if (!bindingServicesLocked.contains(this.mComponentName)
          && !crashedServicesLocked.contains(this.mComponentName)) {
        iAccessibilityServiceClient = null;
        if (iAccessibilityServiceClient == null
            && !accessibilityUserState.getEnabledServicesLocked().contains(this.mComponentName)) {
          this.mSystemSupport.onClientChangeLocked(false);
          return;
        }
        if (iAccessibilityServiceClient != null) {
        ...
        try {
          if (svcClientTracingEnabled()) {
            logTraceSvcClient(
                "init", this + "," + this.mId + "," + this.mOverlayWindowTokens.get(0));
          }
          iAccessibilityServiceClient.init(
              this, this.mId, (IBinder) this.mOverlayWindowTokens.get(0));
          return;
        } catch (RemoteException e) {
          Slog.w(
              "AccessibilityServiceConnection",
              "Error while setting connection for service: " + iAccessibilityServiceClient,
              e);
          binderDied();
          return;
        }
  ...
  public boolean hasRightsToCurrentUserLocked() {
    ...
    return callingUid == 0
        ...
        || this.mSecurityPolicy.hasPermission("android.permission.INTERACT_ACROSS_USERS")
        || this.mSecurityPolicy.hasPermission("android.permission.INTERACT_ACROSS_USERS_FULL");
  }
  ...
  public boolean isAccessibilityButtonAvailable() {
    ...
    synchronized (this.mLock) {
      ...
      AccessibilityUserState accessibilityUserState =
          (AccessibilityUserState) this.mUserStateWeakReference.get();
      if (accessibilityUserState != null
          && isAccessibilityButtonAvailableLocked(accessibilityUserState)) {
        z = true;
      }
      return z;
    }
  ...
  public boolean isCapturingFingerprintGestures() {
    return this.mServiceInterface != null
        && this.mSecurityPolicy.canCaptureFingerprintGestures(this)
  ...
  public void dispatchGesture(int i, ParceledListSlice parceledListSlice, int i2) {
    synchronized (this.mLock) {
      if (this.mServiceInterface != null && this.mSecurityPolicy.canPerformGestures(this)) {
        MotionEventInjector motionEventInjectorForDisplayLocked =
            this.mSystemSupport.getMotionEventInjectorForDisplayLocked(i2);
        if (wmTracingEnabled()) {
          logTraceWM("isTouchOrFaketouchDevice", "");
        }
        if (motionEventInjectorForDisplayLocked != null
            && this.mWindowManagerService.isTouchOrFaketouchDevice()) {
          motionEventInjectorForDisplayLocked.injectEvents(
              parceledListSlice.getList(), this.mServiceInterface, i, i2);
        } else {
          try {
            if (svcClientTracingEnabled()) {
            ...
            Slog.e(
                "AccessibilityServiceConnection",
                "Error sending motion event injection failure to " + this.mServiceInterface,
                e);
          }
        }
      }
    ...
    synchronized (this.mLock) {
      if (hasRightsToCurrentUserLocked()) {
        if (this.mSecurityPolicy.checkAccessibilityAccess(this)) {
          if (accessibilityUserState.getFocusStrokeWidthLocked() == i
              && accessibilityUserState.getFocusColorLocked() == i2) {
            return;
          }
    ...
    this.mMainHandler.sendMessage(
        PooledLambda.obtainMessage(
            new BiConsumer() { // from class:
              // com.android.server.accessibility.AccessibilityServiceConnection$$ExternalSyntheticLambda0
              @Override // java.util.function.BiConsumer
              public final void accept(Object obj, Object obj2) {
                ((AccessibilityServiceConnection) obj)
                    .notifyMotionEventInternal((MotionEvent) obj2);
              }
            },
            this,
            motionEvent));
  ...
  public final void notifyMotionEventInternal(MotionEvent motionEvent) {
    ...
    if (serviceInterfaceSafely != null) {
      try {
        ...
        Slog.e(
            "AccessibilityServiceConnection", "Error sending motion event to" + this.mService, e);
      }
    }
  }
