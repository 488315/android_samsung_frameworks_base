...
public class PolicyWarningUIController {
    ...
    public class NotificationController extends BroadcastReceiver {
        ...
        public final void sendNotification(int i, ComponentName componentName, CharSequence charSequence, Bitmap bitmap) {
            ...
            builder.setSmallIcon(R.drawable.ft_avd_tooverflow_animation).setSmallIcon(R.drawable.pointer_wait_vector_49).setColor(this.mContext.getResources().getColor(R.color.accent_primary_variant_light_device_default)).setCategory("recommendation").setContentTitle(this.mContext.getString(17043222)).setContentText(this.mContext.getString(17043221, charSequence)).setStyle(new Notification.BigTextStyle().bigText(this.mContext.getString(17043221, charSequence))).setTicker(this.mContext.getString(17043222)).setOnlyAlertOnce(true).setDeleteIntent(PolicyWarningUIController.createPendingIntent(this.mContext, i, PolicyWarningUIController.ACTION_DISMISS_NOTIFICATION, componentName)).setContentIntent(PolicyWarningUIController.createPendingIntent(this.mContext, i, PolicyWarningUIController.ACTION_A11Y_SETTINGS, componentName));
            if (bitmap != null) {
