...
public class MotionEventInjector extends BaseEventStreamTransformation implements Handler.Callback {
  ...
  public void injectEvents(
      List list, IAccessibilityServiceClient iAccessibilityServiceClient, int i, int i2) {
  ...
  public void onMotionEvent(MotionEvent motionEvent, MotionEvent motionEvent2, int i) {
    ...
    if (motionEvent.isFromSource(8194)
        ...
        && ((Boolean) this.mOpenGesturesInProgress.get(4098, Boolean.FALSE)).booleanValue()) {
      return;
    }
    cancelAnyPendingInjectedEvents();
    sendMotionEventToNext(
        motionEvent, motionEvent2, i | IInstalld.FLAG_CLEAR_APP_DATA_KEEP_ART_PROFILES);
  }
  ...
  public boolean handleMessage(Message message) {
    ...
    if (i == 2) {
      ...
      injectEventsMainThread(
          (List) someArgs.arg1,
          (IAccessibilityServiceClient) someArgs.arg2,
          someArgs.argi1,
          someArgs.argi2);
      someArgs.recycle();
      return true;
    ...
    if (message.arg1 != 0) {
      notifyService(
          this.mServiceInterfaceForCurrentGesture, this.mSequencesInProgress.get(0), true);
      this.mSequencesInProgress.remove(0);
    }
    return true;
  }

  public final void injectEventsMainThread(
      List list, IAccessibilityServiceClient iAccessibilityServiceClient, int i, int i2) {
    if (this.mIsDestroyed) {
      try {
        ...
        Slog.e(
            "MotionEventInjector",
            "Error sending status with mIsDestroyed to " + iAccessibilityServiceClient,
            e);
        return;
    ...
    if (newGestureTriesToContinueOldOne
        && (iAccessibilityServiceClient != this.mServiceInterfaceForCurrentGesture
            || !prepareToContinueOldGesture(list))) {
      cancelAnyPendingInjectedEvents();
      notifyService(iAccessibilityServiceClient, i, false);
      return;
    }
    ...
    List motionEventsFromGestureSteps =
        getMotionEventsFromGestureSteps(
            list,
            this.mSequencesInProgress.size() == 0 ? uptimeMillis : this.mLastScheduledEventTime);
    ...
    while (i3 < motionEventsFromGestureSteps.size()) {
      ...
      Message obtainMessage =
          this.mHandler.obtainMessage(
              1, i3 == motionEventsFromGestureSteps.size() - 1 ? 1 : 0, 0, motionEvent);
      this.mLastScheduledEventTime = motionEvent.getEventTime();
      this.mHandler.sendMessageDelayed(
          obtainMessage, Math.max(0L, motionEvent.getEventTime() - uptimeMillis));
      i3++;
    }
  }
    ...
    for (int i2 = 0; i2 < gestureStep.numTouchPoints; i2++) {
      ...
      if (!touchPoint.mIsStartOfPath) {
        ...
        if (i3 == -1) {
          Slog.w(
              "MotionEventInjector",
              "Can't continue gesture due to unknown continued stroke id in " + touchPoint);
          return false;
        }
        this.mStrokeIdToPointerId.put(touchPoint.mStrokeId, i3);
        int findPointByStrokeId =
            findPointByStrokeId(
                this.mLastTouchPoints, this.mNumLastTouchPoints, touchPoint.mContinuedStrokeId);
        if (findPointByStrokeId < 0) {
          Slog.w(
              "MotionEventInjector",
              "Can't continue gesture due continued gesture id of "
                  + touchPoint
                  + " not matching any previous strokes in "
                  + Arrays.asList(this.mLastTouchPoints));
          return false;
        }
        GestureDescription.TouchPoint touchPoint2 = this.mLastTouchPoints[findPointByStrokeId];
        if (touchPoint2.mIsEndOfPath
            || touchPoint2.mX != touchPoint.mX
            || touchPoint2.mY != touchPoint.mY) {
          Slog.w(
              "MotionEventInjector",
              "Can't continue gesture due to points mismatch between "
                  + this.mLastTouchPoints[findPointByStrokeId]
                  + " and "
                  + touchPoint);
          return false;
        }
  ...
  public final void cancelAnyPendingInjectedEvents() {
    if (this.mHandler.hasMessages(1)) {
      ...
      for (int size = this.mSequencesInProgress.size() - 1; size >= 0; size--) {
        notifyService(
            this.mServiceInterfaceForCurrentGesture, this.mSequencesInProgress.get(size), false);
  ...
  public final List getMotionEventsFromGestureSteps(List list, long j) {
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      appendMoveEventIfNeeded(
          arrayList, gestureStep.touchPoints, i2, j + gestureStep.timeSinceGestureStart);
  ...
  public final void appendMoveEventIfNeeded(
      List list, GestureDescription.TouchPoint[] touchPointArr, int i, long j) {
    GestureDescription.TouchPoint[] lastTouchPoints = getLastTouchPoints();
    boolean z = false;
    for (int i2 = 0; i2 < i; i2++) {
      int findPointByStrokeId =
          findPointByStrokeId(
              lastTouchPoints, this.mNumLastTouchPoints, touchPointArr[i2].mStrokeId);
      if (findPointByStrokeId >= 0) {
  ...
  public final void appendUpEvents(
      List list, GestureDescription.TouchPoint[] touchPointArr, int i, long j) {
    int findPointByStrokeId;
    int i2;
    GestureDescription.TouchPoint[] lastTouchPoints = getLastTouchPoints();
    for (int i3 = 0; i3 < i; i3++) {
      ...
      if (touchPoint.mIsEndOfPath
          && (findPointByStrokeId =
                  findPointByStrokeId(
                      lastTouchPoints, this.mNumLastTouchPoints, touchPoint.mStrokeId))
              >= 0) {
        ...
        list.add(
            obtainMotionEvent(
                this.mDownTime,
                j,
                (i4 != 1 ? 6 : 1) | (findPointByStrokeId << 8),
                lastTouchPoints,
                i4));
        while (true) {
  ...
  public final void appendDownEvents(
      List list, GestureDescription.TouchPoint[] touchPointArr, int i, long j) {
    GestureDescription.TouchPoint[] lastTouchPoints = getLastTouchPoints();
    for (int i2 = 0; i2 < i; i2++) {
