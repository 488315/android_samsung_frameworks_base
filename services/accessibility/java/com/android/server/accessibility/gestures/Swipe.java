...
public class Swipe extends GestureMatcher {
  ...
  public static String directionToString(int i) {
    return i != 0
        ? i != 1
            ? i != 2 ? i != 3 ? "Unknown Direction" : INetd.IF_STATE_DOWN : INetd.IF_STATE_UP
            : "right"
        : "left";
  }
  ...
  public Swipe(
      Context context, int[] iArr, int i, GestureMatcher.StateChangeListener stateChangeListener) {
    ...
    this.mGestureDetectionThresholdPixels =
        TypedValue.applyDimension(5, GestureUtils.MM_PER_CM, displayMetrics) * 1.0f;
    ...
    if (TouchExplorer.DEBUG) {
      Slog.d(
          ...
          "moveDelta:"
              ...
              + Float.toString(this.mGestureDetectionThresholdPixels));
    }
    if (getState() == 0) {
  ...
  public final void recognizeGesturePath(
      MotionEvent motionEvent, MotionEvent motionEvent2, int i, ArrayList arrayList) {
        ...
        if (TouchExplorer.DEBUG) {
          Slog.d(
              ...
              "Found direction "
                  ...
                  + " when expecting "
                  + directionToString(this.mDirections[i2]));
        }
        cancelGesture(motionEvent, motionEvent2, i);
        return;
      }
  ...
  public static int toDirection(float f, float f2) {
    return Math.abs(f) > Math.abs(f2)
        ? f < DisplayPowerController2.RATE_FROM_DOZE_TO_ON ? 0 : 1
        : f2 < DisplayPowerController2.RATE_FROM_DOZE_TO_ON ? 2 : 3;
  }
