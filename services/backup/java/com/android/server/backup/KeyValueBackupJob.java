...
public class KeyValueBackupJob extends JobService {
  ...
  public static ComponentName sKeyValueJobService =
  ...
  public static void schedule(
      int i, Context context, long j, UserBackupManagerService userBackupManagerService) {
    ...
    synchronized (KeyValueBackupJob.class) {
      ...
      if (!sparseBooleanArray.get(i) && userBackupManagerService.isFrameworkSchedulingEnabled()) {
        ...
        if (j <= 0) {
          j =
              new Random().nextInt((int) keyValueBackupFuzzMilliseconds)
                  + keyValueBackupIntervalMilliseconds;
        }
        Slog.v("KeyValueBackupJob", "Scheduling k/v pass in " + ((j / 1000) / 60) + " minutes");
        JobInfo.Builder overrideDeadline =
            new JobInfo.Builder(getJobIdForUserId(i), sKeyValueJobService)
                .setMinimumLatency(j)
                .setRequiredNetworkType(keyValueBackupRequiredNetworkType)
                .setRequiresCharging(keyValueBackupRequireCharging)
                .setOverrideDeadline(
        ...
        ((JobScheduler) context.getSystemService("jobscheduler"))
            .schedule(overrideDeadline.build());
