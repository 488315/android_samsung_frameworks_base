...
public class PerformAdbBackupTask extends FullBackupTask implements BackupRestoreTask {
    ...
    while (it.hasNext()) {
      ...
      if (!treeMap.containsKey(str)) {
        try {
          treeMap.put(
              str,
              this.mUserBackupManagerService.getPackageManager().getPackageInfo(str, 134217728));
  ...
  public final OutputStream emitAesBackupHeader(StringBuilder sb, OutputStream outputStream) {
    ...
    SecretKey buildPasswordKey =
        PasswordUtils.buildPasswordKey(
            "PBKDF2WithHmacSHA1", this.mEncryptPassword, randomBytes, 10000);
    ...
    byte[] makeKeyChecksum =
        PasswordUtils.makeKeyChecksum(
            "PBKDF2WithHmacSHA1", secretKeySpec.getEncoded(), randomBytes2, 10000);
    ByteArrayOutputStream byteArrayOutputStream =
        new ByteArrayOutputStream(iv.length + encoded.length + makeKeyChecksum.length + 3);
