...
public class SystemBackupAgent extends BackupAgentHelper {
  ...
  public static final String WALLPAPER_IMAGE_DIR =
  ...
  public static final String WALLPAPER_INFO_DIR =
      Environment.getUserSystemDirectory(0).getAbsolutePath();
  ...
  static {
    ArraySet newArraySet =
        Sets.newArraySet(
            new String[] {"permissions", "notifications", "account_sync_settings", "app_locales"});
    sEligibleHelpersForProfileUser = newArraySet;
    sEligibleHelpersForNonSystemUser =
        SetUtils.union(
            newArraySet,
            Sets.newArraySet(
                new String[] {
                  "account_manager", "usage_stats", "preferred_activities", "shortcut_manager"
                }));
  }
  ...
  public void onCreate(UserHandle userHandle, int i) {
    ...
    if (identifier != 0) {
      this.mIsProfileUser =
          ((UserManager) createContextAsUser(userHandle, 0).getSystemService(UserManager.class))
              .isProfile();
    }
    ...
    if (str.equals("r")) {
      ...
      if (file2 == null) {
        try {
          Slog.w(
              "SystemBackupAgent",
              "Skipping unrecognized system file: [ " + str + " : " + str2 + " ]");
