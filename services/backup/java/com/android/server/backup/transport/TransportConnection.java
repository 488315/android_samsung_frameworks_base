...
public class TransportConnection {
  ...
  public TransportConnection(
    ...
    this.mPrefixForLog =
        componentName.getShortClassName().replaceFirst(".*\\.", "")
            + "#"
            + str
    ...
    synchronized (this.mStateLock) {
      ...
      if (i == 0) {
        ...
        if (i == 2) {
          ...
          this.mBindIntent, this.mConnection, 1, UserHandle.of(this.mUserId))) {
  ...
  public BackupTransportClient connect(String str) {
    ...
    synchronized (this.mStateLock) {
      ...
      TransportConnectionListener transportConnectionListener =
          new TransportConnectionListener() { // from class:
                                              // com.android.server.backup.transport.TransportConnection$$ExternalSyntheticLambda1
            @Override // com.android.server.backup.transport.TransportConnectionListener
            public final void onTransportConnectionResult(
      ...
      try {
        ...
        log(
            6,
            str,
            e.getClass().getSimpleName() + " while waiting for transport: " + e.getMessage());
        return null;
      }
    }
  }
  ...
  public String toString() {
    return "TransportClient{"
        + this.mTransportComponent.flattenToShortString()
        + "#"
        + this.mIdentifier
        + "}";
  }
  ...
  public final void notifyListener(
      final TransportConnectionListener transportConnectionListener,
    ...
    this.mListenerHandler.post(
        new Runnable() { // from class:
          ...
          public final void run() {
            TransportConnection.this.lambda$notifyListener$1(
                transportConnectionListener, backupTransportClient);
          }
        });
  }
  ...
  public final void notifyListenersAndClearLocked(BackupTransportClient backupTransportClient) {
    for (Map.Entry entry : this.mListeners.entrySet()) {
      notifyListener(
          (TransportConnectionListener) entry.getKey(),
  ...
  public final void onStateTransition(int i, int i2) {
    ...
    if (transitionThroughState != 0) {
      EventLog.writeEvent(
          2850, flattenToShortString, Integer.valueOf(transitionThroughState == 1 ? 1 : 0));
    }
    if (transitionThroughState2 != 0) {
      EventLog.writeEvent(
          2851, flattenToShortString, Integer.valueOf(transitionThroughState2 == 1 ? 1 : 0));
    }
  }
  ...
  public final void checkStateIntegrityLocked() {
    ...
    if (i == 0) {
      ...
      if (i == 3) {
        ...
        checkState(
            this.mTransport != null, "Transport expected to be non-null when state = CONNECTED");
        return;
      } else {
  ...
  public final void saveLogEntry(String str) {
    String str2 =
        ((Object) DateFormat.format("yyyy-MM-dd HH:mm:ss", System.currentTimeMillis())) + " " + str;
    ...
    public final void referenceLost(String str) {
      try {
        ...
        TransportUtils.log(
            ...
            str + " called but TransportClient reference has been GC'ed");
      } catch (IllegalArgumentException e) {
        TransportUtils.log(
            5,
            TransportConnection.TAG,
            str + " called but unbindService failed: " + e.getMessage());
      }
    }
  }
}
