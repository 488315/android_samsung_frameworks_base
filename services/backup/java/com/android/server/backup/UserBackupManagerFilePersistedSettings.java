...
public abstract class UserBackupManagerFilePersistedSettings {
  public static boolean readBackupEnableState(int i) {
    boolean readBackupEnableState =
        readBackupEnableState(UserBackupManagerFiles.getBaseStateDir(i));
    Slog.d(
        "BackupManagerService",
        "user:" + i + " readBackupEnableState enabled:" + readBackupEnableState);
  ...
  public static void writeBackupEnableState(File file, boolean z) {
    ...
    try {
      ...
      Slog.e(
          "BackupManagerService",
          "Unable to record backup enable state; reverting to disabled: " + e.getMessage());
      file2.delete();
      file3.delete();
    }
  }
