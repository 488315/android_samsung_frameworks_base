    ...
    try {
      try {
        ...
        for (String str : this.mQueue) {
          try {
            TransportConnection transportClient =
                this.mTransportManager.getTransportClient(str, "PerformInitializeTask.run()");
            if (transportClient == null) {
              Slog.e("BackupManagerService", "Requested init for " + str + " but not found");
            } else {
              arrayList.add(transportClient);
              Slog.i(
                  "BackupManagerService", "Initializing (wiping) backup transport storage: " + str);
              ...
              BackupTransportClient connectOrThrow =
                  transportClient.connectOrThrow("PerformInitializeTask.run()");
              ...
              if (initializeDevice == 0) {
                ...
                this.mBackupManagerService.resetBackupState(
                    new File(this.mBaseStateDir, transportDirName));
                ...
                try {
                  ...
                  Slog.w(
                      ...
                      "Init failed on " + str + " resched in " + requestBackupTime);
                  this.mBackupManagerService
                      .getAlarmManager()
                      .set(
                          0,
                          System.currentTimeMillis() + requestBackupTime,
                          this.mBackupManagerService.getRunInitIntent());
                  i2 = initializeDevice;
                } catch (Exception e) {
                  e = e;
                  i = initializeDevice;
                  ...
                  for (TransportConnection transportConnection : arrayList) {
                    ...
                    transportManager.disposeOfTransportClient(
                        transportConnection, "PerformInitializeTask.run()");
                  ...
                  while (it.hasNext()) {
                    this.mTransportManager.disposeOfTransportClient(
                        (TransportConnection) it.next(), "PerformInitializeTask.run()");
                  }
                  notifyFinished(i);
                  this.mListener.onFinished("PerformInitializeTask.run()");
                  throw th;
        ...
        for (TransportConnection transportConnection2 : arrayList) {
          ...
          transportManager2.disposeOfTransportClient(
              transportConnection2, "PerformInitializeTask.run()");
