...
public class BackupManagerConstants extends KeyValueSettingObserver {
  ...
  public String getSettingValue(ContentResolver contentResolver) {
    return Settings.Secure.getStringForUser(
        contentResolver, "backup_manager_constants", contentResolver.getUserId());
  }

  public synchronized void update(KeyValueListParser keyValueListParser) {
    this.mKeyValueBackupIntervalMilliseconds =
        keyValueListParser.getLong(
            KEY_VALUE_BACKUP_INTERVAL_MILLISECONDS, DEFAULT_KEY_VALUE_BACKUP_INTERVAL_MILLISECONDS);
    this.mKeyValueBackupFuzzMilliseconds =
        keyValueListParser.getLong(KEY_VALUE_BACKUP_FUZZ_MILLISECONDS, 600000L);
    this.mKeyValueBackupRequireCharging =
        keyValueListParser.getBoolean(KEY_VALUE_BACKUP_REQUIRE_CHARGING, true);
    this.mKeyValueBackupRequiredNetworkType =
        keyValueListParser.getInt(KEY_VALUE_BACKUP_REQUIRED_NETWORK_TYPE, 1);
    this.mFullBackupIntervalMilliseconds =
        keyValueListParser.getLong(
            FULL_BACKUP_INTERVAL_MILLISECONDS, DEFAULT_FULL_BACKUP_INTERVAL_MILLISECONDS);
    this.mFullBackupRequireCharging =
        keyValueListParser.getBoolean(FULL_BACKUP_REQUIRE_CHARGING, true);
    this.mFullBackupRequiredNetworkType =
        keyValueListParser.getInt(FULL_BACKUP_REQUIRED_NETWORK_TYPE, 2);
  ...
  public synchronized long getKeyValueBackupFuzzMilliseconds() {
    Slog.v(
        "BackupManagerConstants",
        "getKeyValueBackupFuzzMilliseconds(...) returns " + this.mKeyValueBackupFuzzMilliseconds);
  ...
  public synchronized boolean getKeyValueBackupRequireCharging() {
    Slog.v(
        "BackupManagerConstants",
        "getKeyValueBackupRequireCharging(...) returns " + this.mKeyValueBackupRequireCharging);
  ...
  public synchronized long getFullBackupIntervalMilliseconds() {
    Slog.v(
        "BackupManagerConstants",
        "getFullBackupIntervalMilliseconds(...) returns " + this.mFullBackupIntervalMilliseconds);
  ...
  public synchronized boolean getFullBackupRequireCharging() {
    Slog.v(
        "BackupManagerConstants",
        "getFullBackupRequireCharging(...) returns " + this.mFullBackupRequireCharging);
  ...
  public synchronized int getFullBackupRequiredNetworkType() {
    Slog.v(
        "BackupManagerConstants",
        "getFullBackupRequiredNetworkType(...) returns " + this.mFullBackupRequiredNetworkType);
  ...
  public synchronized String[] getBackupFinishedNotificationReceivers() {
    Slog.v(
        ...
        "getBackupFinishedNotificationReceivers(...) returns "
            + TextUtils.join(", ", this.mBackupFinishedNotificationReceivers));
