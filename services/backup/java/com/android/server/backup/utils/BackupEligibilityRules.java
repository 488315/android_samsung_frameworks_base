...
public class BackupEligibilityRules {
  ...
  static {
    ...
    systemPackagesAllowedForNonSystemUsers =
        SetUtils.union(
            newArraySet,
            Sets.newArraySet(
                new String[] {"com.android.wallpaperbackup", "com.android.providers.settings"}));
  }
    ...
    if (UserHandle.isCore(applicationInfo.uid)) {
      if (this.mUserId != 0) {
        if (this.mIsProfileUser
            && !systemPackagesAllowedForProfileUser.contains(applicationInfo.packageName)) {
          return false;
        }
        if (!this.mIsProfileUser
            && !systemPackagesAllowedForNonSystemUsers.contains(applicationInfo.packageName)) {
          return false;
        }
      }
    ...
    if (i2 == 1) {
      return (!((i & 1) != 0)
              && CompatChanges.isChangeEnabled(
                  183147249L, applicationInfo.packageName, UserHandle.of(this.mUserId)))
          || z;
    }
    if (i2 == 2) {
      ...
      try {
        return this.mPackageManager
            .getPropertyAsUser("android.backup.ALLOW_ADB_BACKUP", str, null, this.mUserId)
            .getBoolean();
      ...
      if (appIsEligibleForBackup(applicationInfo)
        ...
        try {
          return transportConnection
              .connectOrThrow("AppBackupUtils.appIsRunningAndEligibleForBackupWithTransport")
              .isAppEligibleForBackup(packageInfoAsUser, appGetsFullBackup(packageInfoAsUser));
        } catch (Exception e) {
  ...
  public boolean appIsDisabled(ApplicationInfo applicationInfo) {
    int applicationEnabledState =
        this.mPackageManagerInternal.getApplicationEnabledState(
            applicationInfo.packageName, this.mUserId);
    if (applicationEnabledState != 0) {
      return applicationEnabledState == 2
          || applicationEnabledState == 3
          || applicationEnabledState == 4;
    }
  ...
  public boolean signaturesMatch(Signature[] signatureArr, PackageInfo packageInfo) {
    ...
    if (signatureArr.length == 1) {
      return this.mPackageManagerInternal.isDataRestoreSafe(
          signatureArr[0], packageInfo.packageName);
    }
