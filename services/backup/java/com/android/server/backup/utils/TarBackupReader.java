...
public class TarBackupReader {
    ...
    try {
      ...
      fileMetadata.path =
          extractString(
              ...
              FrameworkStatsLog.f677xec66d03f);
  ...
  public Signature[] readAppManifestAndReturnSignatures(FileMetadata fileMetadata) {
    ...
    if (readExactly == j2) {
      ...
      try {
        ...
        Slog.w(
            "BackupManagerService",
            "Corrupt restore manifest for package " + fileMetadata.packageName);
        this.mMonitor =
            BackupManagerMonitorUtils.monitorEvent(
                this.mMonitor,
                46,
  ...
  */
  public RestorePolicy chooseRestorePolicy(
    ...
    try {
      ...
      applicationInfo = packageInfoAsUser.applicationInfo;
      int i2 = applicationInfo.flags;
    } catch (PackageManager.NameNotFoundException unused) {
      if (z) {
        Slog.i(
            "BackupManagerService",
            "Package " + fileMetadata.packageName + " not installed; requiring apk in dataset");
    ...
    if (!backupEligibilityRules.isAppBackupAllowed(applicationInfo) && !z2) {
      Slog.i(
          "BackupManagerService",
          "Restore manifest from " + fileMetadata.packageName + " but allowBackup=false");
      this.mMonitor =
          BackupManagerMonitorUtils.monitorEvent(this.mMonitor, 39, packageInfoAsUser, 3, null);
      if (restorePolicy2 == RestorePolicy.ACCEPT_IF_APK && !fileMetadata.hasApk) {
        Slog.i(
            "BackupManagerService",
            "Cannot restore package " + fileMetadata.packageName + " without the matching .apk");
        this.mMonitor =
            BackupManagerMonitorUtils.monitorEvent(
                this.mMonitor,
                41,
    ...
    if (UserHandle.isCore(packageInfoAsUser.applicationInfo.uid)
      ...
      Slog.w(
          "BackupManagerService",
          "Package " + fileMetadata.packageName + " is system level with no agent");
      this.mMonitor =
          BackupManagerMonitorUtils.monitorEvent(this.mMonitor, 38, packageInfoAsUser, 2, null);
      if (restorePolicy2 == RestorePolicy.ACCEPT_IF_APK) {
        Slog.i(
            "BackupManagerService",
            "Cannot restore package " + fileMetadata.packageName + " without the matching .apk");
        this.mMonitor =
    ...
    if (backupEligibilityRules.signaturesMatch(signatureArr, packageInfoAsUser)) {
      if ((packageInfoAsUser.applicationInfo.flags
              & IInstalld.FLAG_CLEAR_APP_DATA_KEEP_ART_PROFILES)
          != 0) {
        Slog.i("BackupManagerService", "Package has restoreAnyVersion; taking data");
        this.mMonitor =
            BackupManagerMonitorUtils.monitorEvent(this.mMonitor, 34, packageInfoAsUser, 3, null);
        ...
        restorePolicy2 = RestorePolicy.ACCEPT;
        this.mMonitor =
            BackupManagerMonitorUtils.monitorEvent(this.mMonitor, 35, packageInfoAsUser, 3, null);
      } else if (z) {
        Slog.i(
            "BackupManagerService",
            "Data version "
                ...
                + packageInfoAsUser.getLongVersionCode()
                + " - requiring apk");
        restorePolicy2 = RestorePolicy.ACCEPT_IF_APK;
      } else {
        Slog.i(
            "BackupManagerService",
            "Data requires newer version " + fileMetadata.version + "; ignoring");
        this.mMonitor =
            BackupManagerMonitorUtils.monitorEvent(
                this.mMonitor,
                36,
      ...
      Slog.w(
          "BackupManagerService",
          "Restore manifest signatures do not match installed application for "
              + fileMetadata.packageName);
      this.mMonitor =
          BackupManagerMonitorUtils.monitorEvent(this.mMonitor, 37, packageInfoAsUser, 3, null);
    }
    if (restorePolicy2 == RestorePolicy.ACCEPT_IF_APK) {}
    return restorePolicy2;
  }
