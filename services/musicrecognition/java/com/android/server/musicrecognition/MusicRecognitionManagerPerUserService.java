...
public final class MusicRecognitionManagerPerUserService extends AbstractPerUserSystemService
  ...
  public MusicRecognitionManagerPerUserService(
      MusicRecognitionManagerService musicRecognitionManagerService, Object obj, int i) {
    super(musicRecognitionManagerService, obj, i);
    this.mAppOpsManager =
        (AppOpsManager)
            getContext()
                .createAttributionContext(MusicRecognitionManagerService.TAG)
                .getSystemService(AppOpsManager.class);
    this.mAttributionMessage =
        String.format("MusicRecognitionManager.invokedByUid.%s", Integer.valueOf(i));
  ...
  public ServiceInfo newServiceInfoLocked(ComponentName componentName) {
    try {
      ServiceInfo serviceInfo =
          AppGlobals.getPackageManager().getServiceInfo(componentName, 128L, this.mUserId);
      if ("android.permission.BIND_MUSIC_RECOGNITION_SERVICE".equals(serviceInfo.permission)) {
        return serviceInfo;
      }
      Slog.w(
          TAG,
          "MusicRecognitionService from '"
              + serviceInfo.packageName
              + "' does not require permission android.permission.BIND_MUSIC_RECOGNITION_SERVICE");
      throw new SecurityException(
          "Service does not require permission android.permission.BIND_MUSIC_RECOGNITION_SERVICE");
    } catch (RemoteException unused) {
      throw new PackageManager.NameNotFoundException("Could not get service for " + componentName);
    }
  }

  public final RemoteMusicRecognitionService ensureRemoteServiceLocked(
      IMusicRecognitionManagerCallback iMusicRecognitionManagerCallback) {
    if (this.mRemoteService == null) {
      ...
      this.mRemoteService =
          new RemoteMusicRecognitionService(
              getContext(),
              ComponentName.unflattenFromString(componentNameLocked),
              this.mUserId,
              this,
              new MusicRecognitionServiceCallback(iMusicRecognitionManagerCallback),
              ((MusicRecognitionManagerService) this.mMaster).isBindInstantServiceAllowed(),
              ((MusicRecognitionManagerService) this.mMaster).verbose);
      try {
        this.mServiceInfo =
            getContext()
                .getPackageManager()
                .getServiceInfo(this.mRemoteService.getComponentName(), 128);
  ...
  public void beginRecognitionLocked(final RecognitionRequest recognitionRequest, IBinder iBinder) {
    final IMusicRecognitionManagerCallback asInterface =
        IMusicRecognitionManagerCallback.Stub.asInterface(iBinder);
    RemoteMusicRecognitionService ensureRemoteServiceLocked =
        ensureRemoteServiceLocked(asInterface);
    ...
    if (createPipe == null) {
      ...
      this.mAttributionTagFuture.thenAcceptAsync(
          new Consumer() { // from class:
                           // com.android.server.musicrecognition.MusicRecognitionManagerPerUserService$$ExternalSyntheticLambda0
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
              MusicRecognitionManagerPerUserService.this.lambda$beginRecognitionLocked$0(
                  recognitionRequest, asInterface, parcelFileDescriptor, (String) obj);
            }
          },
          (Executor) ((MusicRecognitionManagerService) this.mMaster).mExecutorService);
      this.mRemoteService.onAudioStreamStarted(
          parcelFileDescriptor2, recognitionRequest.getAudioFormat());
    }
  }

  /* renamed from: streamAudio, reason: merged with bridge method [inline-methods] */
  public final void lambda$beginRecognitionLocked$0(
      String str,
    ...
    try {
      ...
      Slog.e(
          TAG,
          "RECORD_AUDIO op not permitted on behalf of " + this.mServiceInfo.getComponentName(),
          e3);
      try {
  ...
  public final class MusicRecognitionServiceCallback extends IMusicRecognitionServiceCallback.Stub {
    ...
    public MusicRecognitionServiceCallback(
        IMusicRecognitionManagerCallback iMusicRecognitionManagerCallback) {
  ...
  public final void startRecordAudioOp(String str) {
    ...
    if (startProxyOp != 0) {
      throw new SecurityException(
          String.format(
              "Failed to obtain RECORD_AUDIO permission (status: %d) for receiving service: %s",
              Integer.valueOf(startProxyOp), this.mServiceInfo.getComponentName()));
    }
    String str2 = TAG;
    ...
    Slog.i(
        ...
        String.format(
            ...
            serviceInfo2.packageName, Integer.valueOf(serviceInfo2.applicationInfo.uid), str));
  }
  ...
  public final void finishRecordAudioOp(String str) {
    ...
    appOpsManager.finishProxyOp(
        permissionToOp, serviceInfo.applicationInfo.uid, serviceInfo.packageName, str);
  }

  public static AudioRecord createAudioRecord(RecognitionRequest recognitionRequest, int i) {
    return new AudioRecord(
        recognitionRequest.getAudioAttributes(),
        recognitionRequest.getAudioFormat(),
        getBufferSizeInBytes(recognitionRequest.getAudioFormat().getSampleRate(), i),
  ...
  public static Pair createPipe() {
    try {
      ...
      if (!createPipe[0].getFileDescriptor().valid()
        ...
        Slog.e(
            TAG,
            "Failed to create audio stream pipe, didn't receive a pair of valid file descriptors.");
        return null;
      }
