...
public final class VoiceInteractionSessionConnection
      ...
      synchronized (VoiceInteractionSessionConnection.this.mLock) {
        ...
        if (Instant.now().isBefore(this.mExpiryTime)) {
          ...
          if (VoiceInteractionSessionConnection.this.mSetPowerBoostRunnable != null) {
            VoiceInteractionSessionConnection.this.mFgHandler.postDelayed(
                VoiceInteractionSessionConnection.this.mSetPowerBoostRunnable,
  ...
  public VoiceInteractionSessionConnection(
    ...
    this.mFullConnection =
        new ServiceConnection() { // from class:
          // com.android.server.voiceinteraction.VoiceInteractionSessionConnection.2
    ...
    this.mShowAssistDisclosureRunnable =
        new Runnable() { // from class:
          // com.android.server.voiceinteraction.VoiceInteractionSessionConnection.3
          @Override // java.lang.Runnable
          public void run() {
    ...
    this.mLowPowerStandbyControllerInternal =
        (LowPowerStandbyControllerInternal)
            LocalServices.getService(LowPowerStandbyControllerInternal.class);
    ...
    this.mPermissionOwner =
        uriGrantsManagerInternal.newUriPermissionOwner(
  ...
  public int getUserDisabledShowContextLocked() {
    int i =
        Settings.Secure.getIntForUser(
                    this.mContext.getContentResolver(), "assist_structure_enabled", 1, this.mUser)
                == 0
            ? 1
            : 0;
    return Settings.Secure.getIntForUser(
                this.mContext.getContentResolver(), "assist_screenshot_enabled", 1, this.mUser)
            == 0
        ? i | 2
        : i;
  }
  ...
  public boolean showLocked(
    ...
    if (!this.mFullyBound) {
      this.mFullyBound =
          this.mContext.bindServiceAsUser(
              this.mBindIntent, this.mFullConnection, 135790593, new UserHandle(this.mUser));
    }
    this.mShown = true;
    ...
    if (z3) {
      ...
      this.mAssistDataRequester.requestAssistData(
          arrayList,
          z,
          z2,
          (userDisabledShowContextLocked & 1) == 0,
          (userDisabledShowContextLocked & 2) == 0,
          this.mCallingUid,
          this.mSessionComponentName.getPackageName(),
          str,
          CoreRune.FW_SUPPORT_SEARCLE_HOME_LONG
              && ((i3 = this.mShowArgs.getInt("omni.entry_point")) == 65537 || i3 == 65538));
      if ((this.mAssistDataRequester.getPendingDataCount() > 0
              || this.mAssistDataRequester.getPendingScreenshotCount() > 0)
          && AssistUtils.shouldDisclose(this.mContext, this.mSessionComponentName)) {
    ...
    if (iVoiceInteractionSession != null) {
      try {
        iVoiceInteractionSession.show(
            this.mShowArgs, this.mShowFlags, iVoiceInteractionSessionShowCallback);
        this.mShowArgs = null;
        this.mShowFlags = 0;
    ...
    LowPowerStandbyControllerInternal lowPowerStandbyControllerInternal =
        this.mLowPowerStandbyControllerInternal;
    ...
    for (int i = 0; i < size; i++) {
      ...
      try {
        this.mSession.handleAssist(
            activityAssistInfo.getTaskId(),
            activityAssistInfo.getAssistToken(),
            (Bundle) null,
    ...
    try {
      if (bundle == null) {
        iVoiceInteractionSession.handleAssist(
            -1, (IBinder) null, (Bundle) null, (AssistStructure) null, (AssistContent) null, 0, 0);
      } else {
        ...
        if (i4 >= 0 && assistContent != null) {
          ...
          if (clipData2 != null) {
            grantClipDataPermissions(
                clipData2, 1, i4, this.mCallingUid, this.mSessionComponentName.getPackageName());
          }
        }
      ...
      try {
        try {
          this.mUgmInternal.checkGrantUriPermission(
              ...
              i,
              ContentProvider.getUserIdFromUri(uri, UserHandle.getUserId(i2)));
  ...
  public boolean deliverNewSessionLocked(
      IVoiceInteractionSession iVoiceInteractionSession, IVoiceInteractor iVoiceInteractor) {
    ...
    if (this.mListeningVisibleActivity) {
      this.mScheduledExecutorService.execute(
          new Runnable() { // from class:
            ...
            @Override // java.lang.Runnable
            public final void run() {
              VoiceInteractionSessionConnection.this.lambda$notifyActivityEventChangedLocked$0(
                  iBinder, i);
            }
          });
    }
  ...
  public final ArrayMap getTopVisibleActivityInfosLocked() {
    ...
    for (int i = 0; i < size; i++) {
      ...
      arrayMap.put(
          activityAssistInfo.getActivityToken(),
          new VisibleActivityInfo(
              activityAssistInfo.getTaskId(), activityAssistInfo.getAssistToken()));
    }
    return arrayMap;
  }
  ...
  public final void handleVisibleActivitiesLocked(IBinder iBinder, int i) {
    ...
    if (this.mListeningVisibleActivity && this.mShown && !this.mCanceled && this.mSession != null) {
      if (i != 1 && i != 2) {
        z = false;
        if (i == 3) {
          if (getVisibleActivityInfoFromTopVisibleActivity(iBinder) != null
              || (visibleActivityInfoFromTopVisibleActivity =
          ...
          Slog.w(
              "VoiceInteractionServiceManager",
              "notifyActivityEventChangedLocked unexpected type=" + i);
          return;
        }
      } else if (this.mVisibleActivityInfoForToken.containsKey(iBinder)
          || (visibleActivityInfoFromTopVisibleActivity =
                  getVisibleActivityInfoFromTopVisibleActivity(iBinder))
              == null) {
        return;
      } else {
        z = true;
      ...
      try {
        this.mSession.notifyVisibleActivityInfoChanged(
            visibleActivityInfoFromTopVisibleActivity, z ? 1 : 2);
      } catch (RemoteException unused) {
      }
      if (z) {
  ...
  public final void notifyVisibleActivitiesChangedLocked(ArrayMap arrayMap, int i) {
    ...
    for (int i2 = 0; i2 < arrayMap.size(); i2++) {
      try {
        this.mSession.notifyVisibleActivityInfoChanged(
            (VisibleActivityInfo) arrayMap.valueAt(i2), i);
      } catch (RemoteException unused) {
        return;
      }
    }
  ...
  public void notifyActivityDestroyedLocked(final IBinder iBinder) {
    if (this.mListeningVisibleActivity) {
      this.mScheduledExecutorService.execute(
          new Runnable() { // from class:
            // com.android.server.voiceinteraction.VoiceInteractionSessionConnection$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
              VoiceInteractionSessionConnection.this.lambda$notifyActivityDestroyedLocked$1(
                  iBinder);
            }
          });
    }
      ...
      if (this.mListeningVisibleActivity) {
        if (this.mShown && !this.mCanceled && this.mSession != null) {
          VisibleActivityInfo visibleActivityInfo =
              (VisibleActivityInfo) this.mVisibleActivityInfoForToken.remove(iBinder);
