...
public class RecognitionServiceInfo {
  ...
  public static List getAvailableServices(Context context, int i) {
    ...
    Iterator it =
        context
            .getPackageManager()
            .queryIntentServicesAsUser(new Intent("android.speech.RecognitionService"), 786432, i)
            .iterator();
    while (it.hasNext()) {
      RecognitionServiceInfo parseInfo =
          parseInfo(context.getPackageManager(), ((ResolveInfo) it.next()).serviceInfo);
      if (!TextUtils.isEmpty(parseInfo.mParseError)) {
        Log.w(
            "RecognitionServiceInfo",
            "Parse error in getAvailableServices: " + parseInfo.mParseError);
      }
  ...
  public static RecognitionServiceInfo parseInfo(
    ...
    if (loadXmlMetaData == null) {
      RecognitionServiceInfo recognitionServiceInfo =
          new RecognitionServiceInfo(
              serviceInfo, true, "No android.speech meta-data for " + serviceInfo.packageName);
      if (loadXmlMetaData != null) {
        loadXmlMetaData.close();
      }
    ...
    TypedArray obtainAttributes =
        resourcesForApplication.obtainAttributes(asAttributeSet, R.styleable.RecognitionService);
