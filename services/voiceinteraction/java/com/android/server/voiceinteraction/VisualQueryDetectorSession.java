...
public final class VisualQueryDetectorSession extends DetectorSession {
  ...
  public VisualQueryDetectorSession(
  ...
  public void startPerceivingLocked(
      final IVisualQueryDetectionVoiceInteractionCallback
          iVisualQueryDetectionVoiceInteractionCallback) {
    final IDetectorSessionVisualQueryDetectionCallback.Stub stub =
        new IDetectorSessionVisualQueryDetectionCallback.Stub() { // from class:
          ...
          public void onAttentionGained() {
            ...
            try {
              ...
              try {
                iVisualQueryDetectionVoiceInteractionCallback.onVisualQueryDetectionServiceFailure(
                    new VisualQueryDetectionServiceFailure(
                        3, "Attention listener failed to switch to GAINED state."));
              } catch (RemoteException unused) {
                Slog.v(
                    "VisualQueryDetectorSession",
                    "Fail to call onVisualQueryDetectionServiceFailure");
              }
            }
          }
          ...
          public void onAttentionLost() {
            ...
            try {
              ...
              try {
                iVisualQueryDetectionVoiceInteractionCallback.onVisualQueryDetectionServiceFailure(
                    new VisualQueryDetectionServiceFailure(
                        3, "Attention listener failed to switch to LOST state."));
              } catch (RemoteException unused) {
                Slog.v(
                    "VisualQueryDetectorSession",
                    "Fail to call onVisualQueryDetectionServiceFailure");
          ...
          public void onQueryDetected(String str) {
            ...
            if (!VisualQueryDetectorSession.this.mEgressingData) {
              Slog.v(
                  "VisualQueryDetectorSession",
                  "Query should not be egressed within the unattention state.");
              iVisualQueryDetectionVoiceInteractionCallback.onVisualQueryDetectionServiceFailure(
                  new VisualQueryDetectionServiceFailure(
                      4, "Cannot stream queries without attention signals."));
            } else {
          ...
          public void onQueryFinished() {
            ...
            if (!VisualQueryDetectorSession.this.mQueryStreaming) {
              Slog.v(
                  "VisualQueryDetectorSession",
                  "Query streaming state signal FINISHED is block since there is no active query"
                      + " being streamed.");
              iVisualQueryDetectionVoiceInteractionCallback.onVisualQueryDetectionServiceFailure(
                  new VisualQueryDetectionServiceFailure(
                      4, "Cannot send FINISHED signal with no query streamed."));
            } else {
          ...
          public void onQueryRejected() {
            ...
            if (!VisualQueryDetectorSession.this.mQueryStreaming) {
              Slog.v(
                  "VisualQueryDetectorSession",
                  "Query streaming state signal REJECTED is block since there is no active query"
                      + " being streamed.");
              iVisualQueryDetectionVoiceInteractionCallback.onVisualQueryDetectionServiceFailure(
                  new VisualQueryDetectionServiceFailure(
                      4, "Cannot send REJECTED signal with no query streamed."));
            } else {
  ...
  public void startListeningFromExternalSourceLocked(
    ...
    throw new UnsupportedOperationException(
        "HotwordDetectionService method should not be called from VisualQueryDetectorSession.");
  }
