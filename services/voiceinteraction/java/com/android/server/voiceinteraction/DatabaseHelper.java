...
public class DatabaseHelper extends SQLiteOpenHelper implements IEnrolledModelDb {
  ...
  public void onCreate(SQLiteDatabase sQLiteDatabase) {
    sQLiteDatabase.execSQL(
        "CREATE TABLE sound_model(model_uuid TEXT,vendor_uuid TEXT,keyphrase_id INTEGER,type"
            + " INTEGER,data BLOB,recognition_modes INTEGER,locale TEXT,hint_text TEXT,users"
            + " TEXT,model_version INTEGER,PRIMARY KEY (keyphrase_id,locale,users))");
  }
          ...
          if (soundModelRecord.ifViolatesV6PrimaryKeyIsFirstOfAnyDuplicates(arrayList)) {
            try {
              ...
              if (writeToDatabase == -1) {
                Slog.e(
                    "SoundModelDBHelper",
                    "Database write failed " + soundModelRecord.modelUuid + ": " + writeToDatabase);
              }
            } catch (Exception e2) {
              Slog.e(
                  "SoundModelDBHelper",
                  "Failed to update V6 record " + soundModelRecord.modelUuid,
                  e2);
            }
          }
        }
  ...
  public boolean updateKeyphraseSoundModel(SoundTrigger.KeyphraseSoundModel keyphraseSoundModel) {
    synchronized (this) {
      ...
      contentValues.put(
          "keyphrase_id", Integer.valueOf(keyphraseSoundModel.getKeyphrases()[0].getId()));
      contentValues.put(
          "recognition_modes",
          Integer.valueOf(keyphraseSoundModel.getKeyphrases()[0].getRecognitionModes()));
      contentValues.put(
          "users", getCommaSeparatedString(keyphraseSoundModel.getKeyphrases()[0].getUsers()));
      contentValues.put(
          "locale", keyphraseSoundModel.getKeyphrases()[0].getLocale().toLanguageTag());
  ...
  */
  public final SoundTrigger.KeyphraseSoundModel getValidKeyphraseSoundModelForUser(
      String str, int i) {
    boolean z;
        ...
        while (true) {
          if (rawQuery.getInt(rawQuery.getColumnIndex("type")) == 0) {
            ...
            if (string == null) {
              ...
              int[] arrayForCommaSeparatedString =
                  getArrayForCommaSeparatedString(
                      rawQuery.getString(rawQuery.getColumnIndex("users")));
              Locale forLanguageTag =
                  Locale.forLanguageTag(rawQuery.getString(rawQuery.getColumnIndex("locale")));
  ...
  public class SoundModelRecord {
    ...
    public final boolean V6PrimaryKeyMatches(SoundModelRecord soundModelRecord) {
      return this.keyphraseId == soundModelRecord.keyphraseId
          && stringComparisonHelper(this.locale, soundModelRecord.locale)
          && stringComparisonHelper(this.users, soundModelRecord.users);
    }
    ...
    public boolean ifViolatesV6PrimaryKeyIsFirstOfAnyDuplicates(List list) {
      ...
      while (it.hasNext()) {
        ...
        if (this != soundModelRecord
            && V6PrimaryKeyMatches(soundModelRecord)
            && !Arrays.equals(this.data, soundModelRecord.data)) {
          return false;
        }
      }
