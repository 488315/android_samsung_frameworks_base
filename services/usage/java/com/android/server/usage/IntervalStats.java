  ...
  */
  public UsageEvents.Event buildEvent(ProtoInputStream protoInputStream, List list) {
    ...
    while (true) {
      switch (protoInputStream.nextField()) {
        ...
        case 2:
          event.mPackage =
              getCachedStringRef((String) list.get(protoInputStream.readInt(1120986464258L) - 1));
          break;
        case 3:
        ...
        case 4:
          event.mClass =
              getCachedStringRef((String) list.get(protoInputStream.readInt(1120986464260L) - 1));
          break;
        case 5:
        ...
        case 13:
          event.mNotificationChannelId =
              getCachedStringRef((String) list.get(protoInputStream.readInt(1120986464269L) - 1));
          break;
        case 14:
          event.mInstanceId = protoInputStream.readInt(1120986464270L);
          break;
        case 15:
          event.mTaskRootPackage =
              getCachedStringRef((String) list.get(protoInputStream.readInt(1120986464271L) - 1));
          break;
        case 16:
          event.mTaskRootClass =
              getCachedStringRef((String) list.get(protoInputStream.readInt(1120986464272L) - 1));
          break;
        case 17:
          event.mLocusId =
              getCachedStringRef((String) list.get(protoInputStream.readInt(1120986464273L) - 1));
          break;
      }
    }
  }
  ...
  public final boolean deobfuscateUsageStats(PackagesTokenData packagesTokenData) {
    ...
    while (i < size) {
      ...
      if (packageString == null) {
        ...
        while (i2 < size2) {
          ...
          String string =
              packagesTokenData2.getString(keyAt, usageStats.mChooserCountsObfuscated.keyAt(i2));
          if (string != null) {
            SparseIntArray sparseIntArray =
                (SparseIntArray) usageStats.mChooserCountsObfuscated.valueAt(i2);
    ...
    if (z) {
      Slog.d(
          "IntervalStats",
          "Unable to parse usage stats packages: " + Arrays.toString(arraySet.toArray()));
    }
    return z;
  }
  ...
  public final boolean deobfuscateEvents(PackagesTokenData packagesTokenData) {
        ...
        if (i5 != 5) {
          if (i5 == 8) {
            ...
            if (string == null) {
              Slog.v(
                  "IntervalStats",
                  "Unable to parse shortcut " + event.mShortcutIdToken + " for package " + i);
              this.events.remove(size);
            }
          } else if (i5 == 12) {
            ...
            if (string2 == null) {
              Slog.v(
                  "IntervalStats",
                  "Unable to parse notification channel "
            ...
            if (string3 == null) {
              Slog.v(
                  "IntervalStats",
                  "Unable to parse locus " + event.mLocusIdToken + " for package " + i);
              this.events.remove(size);
            }
          }
    ...
    if (z) {
      Slog.d(
          "IntervalStats",
          "Unable to parse event packages: " + Arrays.toString(arraySet.toArray()));
    }
    return z;
  }
  ...
  public final void obfuscateUsageStatsData(PackagesTokenData packagesTokenData) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (usageStats != null
          && (packageTokenOrAdd =
                  packagesTokenData.getPackageTokenOrAdd(str, usageStats.mEndTimeStamp))
              != -1) {
        ...
        for (int i2 = 0; i2 < size2; i2++) {
          ...
          if (arrayMap != null) {
            ...
            usageStats.mChooserCountsObfuscated.put(
                packagesTokenData.getTokenOrAdd(packageTokenOrAdd, str, str2), sparseIntArray);
          }
        }
  ...
  public final void obfuscateEventsData(PackagesTokenData packagesTokenData) {
    for (int size = this.events.size() - 1; size >= 0; size--) {
      ...
      if (event != null) {
        int packageTokenOrAdd =
            packagesTokenData.getPackageTokenOrAdd(event.mPackage, event.mTimeStamp);
        if (packageTokenOrAdd == -1) {
          ...
          if (!TextUtils.isEmpty(event.mClass)) {
            event.mClassToken =
                packagesTokenData.getTokenOrAdd(packageTokenOrAdd, event.mPackage, event.mClass);
          }
          if (!TextUtils.isEmpty(event.mTaskRootPackage)) {
            event.mTaskRootPackageToken =
                packagesTokenData.getTokenOrAdd(
                    packageTokenOrAdd, event.mPackage, event.mTaskRootPackage);
          }
          if (!TextUtils.isEmpty(event.mTaskRootClass)) {
            event.mTaskRootClassToken =
                packagesTokenData.getTokenOrAdd(
                    packageTokenOrAdd, event.mPackage, event.mTaskRootClass);
          }
          ...
          if (i != 8) {
            if (i == 12) {
              if (!TextUtils.isEmpty(event.mNotificationChannelId)) {
                event.mNotificationChannelIdToken =
                    packagesTokenData.getTokenOrAdd(
                        packageTokenOrAdd, event.mPackage, event.mNotificationChannelId);
              }
            } else if (i == 30 && !TextUtils.isEmpty(event.mLocusId)) {
              event.mLocusIdToken =
                  packagesTokenData.getTokenOrAdd(
                      packageTokenOrAdd, event.mPackage, event.mLocusId);
            }
          } else if (!TextUtils.isEmpty(event.mShortcutId)) {
            event.mShortcutIdToken =
                packagesTokenData.getTokenOrAdd(
                    packageTokenOrAdd, event.mPackage, event.mShortcutId);
          }
        }
      }
    }
