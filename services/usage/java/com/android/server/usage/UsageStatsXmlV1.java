...
public abstract class UsageStatsXmlV1 {
  public static void loadUsageStats(XmlPullParser xmlPullParser, IntervalStats intervalStats) {
    ...
    orCreateUsageStats.mLastTimeUsed =
        intervalStats.beginTime + XmlUtils.readLongAttribute(xmlPullParser, "lastTimeActive");
    try {
      orCreateUsageStats.mLastTimeVisible =
          intervalStats.beginTime + XmlUtils.readLongAttribute(xmlPullParser, "lastTimeVisible");
    ...
    try {
      orCreateUsageStats.mLastTimeForegroundServiceUsed =
          intervalStats.beginTime
              + XmlUtils.readLongAttribute(xmlPullParser, "lastTimeServiceUsed");
    ...
    while (true) {
      ...
      if (next == 2 && name.equals("chosen_action")) {
        loadChooserCounts(
            xmlPullParser, orCreateUsageStats, XmlUtils.readStringAttribute(xmlPullParser, "name"));
      }
    }
  }
  ...
  public static void loadChooserCounts(
      XmlPullParser xmlPullParser, UsageStats usageStats, String str) {
    if (str == null) {
      return;
    }
    ...
    while (true) {
      ...
      if (next == 2 && name.equals("category")) {
        ((ArrayMap) usageStats.mChooserCounts.get(str))
            .put(
                XmlUtils.readStringAttribute(xmlPullParser, "name"),
                Integer.valueOf(XmlUtils.readIntAttribute(xmlPullParser, "count")));
      }
    }
  }
  ...
  public static void loadEvent(XmlPullParser xmlPullParser, IntervalStats intervalStats) {
    ...
    UsageEvents.Event buildEvent =
        intervalStats.buildEvent(
            readStringAttribute, XmlUtils.readStringAttribute(xmlPullParser, "class"));
    buildEvent.mFlags = XmlUtils.readIntAttribute(xmlPullParser, "flags", 0);
    buildEvent.mTimeStamp =
        intervalStats.beginTime + XmlUtils.readLongAttribute(xmlPullParser, "time");
    ...
    if (i != 5) {
      if (i == 8) {
        ...
        buildEvent.mShortcutId =
            readStringAttribute2 != null ? readStringAttribute2.intern() : null;
      } else if (i == 11) {
        buildEvent.mBucketAndReason = XmlUtils.readIntAttribute(xmlPullParser, "standbyBucket", 0);
      } else if (i == 12) {
        String readStringAttribute3 =
            XmlUtils.readStringAttribute(xmlPullParser, "notificationChannel");
        buildEvent.mNotificationChannelId =
            readStringAttribute3 != null ? readStringAttribute3.intern() : null;
      }
    } else {
      Configuration configuration = new Configuration();
  ...
  */
  public static void read(XmlPullParser xmlPullParser, IntervalStats intervalStats) {
    ...
    intervalStats.endTime =
        intervalStats.beginTime + XmlUtils.readLongAttribute(xmlPullParser, "endTime");
    try {
