...
public class UserBroadcastResponseStats {
  ...
  public BroadcastResponseStats getOrCreateBroadcastResponseStats(BroadcastEvent broadcastEvent) {
    BroadcastResponseStats broadcastResponseStats =
        (BroadcastResponseStats) this.mResponseStats.get(broadcastEvent);
    ...
    BroadcastResponseStats broadcastResponseStats2 =
        new BroadcastResponseStats(
            broadcastEvent.getTargetPackage(), broadcastEvent.getIdForResponseEvent());
  ...
  public void populateAllBroadcastResponseStats(List list, String str, long j) {
    for (int size = this.mResponseStats.size() - 1; size >= 0; size--) {
      ...
      if ((j == 0 || j == broadcastEvent.getIdForResponseEvent())
          && (str == null || str.equals(broadcastEvent.getTargetPackage()))) {
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter) {
    for (int i = 0; i < this.mResponseStats.size(); i++) {
      ...
      BroadcastResponseStats broadcastResponseStats =
          (BroadcastResponseStats) this.mResponseStats.valueAt(i);
