    ...
    if (!isBatteryLevelSufficient()) {
      notifyCallbackOnError(
          ...
          "The battery level must be above "
              + this.mConstants.BATTERY_THRESHOLD_NOT_CHARGING
              + " while not charging or above "
              + this.mConstants.BATTERY_THRESHOLD_CHARGING
              + " while charging");
      return;
    }
    Thread thread =
  ...
  public final void copyToFile(File file) {
    ...
    try {
      ParcelFileDescriptor.AutoCloseInputStream autoCloseInputStream =
          new ParcelFileDescriptor.AutoCloseInputStream(this.mUpdateFileDescriptor);
      try {
