...
public class RemoteBugreportManager {
  ...
  public final Runnable mRemoteBugreportTimeoutRunnable = new Runnable() { // from class:
  ...
  public final BroadcastReceiver mRemoteBugreportFinishedReceiver =
      new BroadcastReceiver() { // from class:
        // com.android.server.devicepolicy.RemoteBugreportManager.1
        @Override // android.content.BroadcastReceiver
        public void onReceive(Context context, Intent intent) {
          if ("android.intent.action.REMOTE_BUGREPORT_DISPATCH".equals(intent.getAction())
              && RemoteBugreportManager.this.mRemoteBugreportServiceIsActive.get()) {
  ...
  public final BroadcastReceiver mRemoteBugreportConsentReceiver =
      new BroadcastReceiver() { // from class:
        // com.android.server.devicepolicy.RemoteBugreportManager.2
        @Override // android.content.BroadcastReceiver
        public void onReceive(Context context, Intent intent) {
          String action = intent.getAction();
          RemoteBugreportManager.this
              .mInjector
              .getNotificationManager()
              .cancel("DevicePolicyManager", 678432343);
          ...
          RemoteBugreportManager.this.mContext.unregisterReceiver(
              RemoteBugreportManager.this.mRemoteBugreportConsentReceiver);
        }
      };
        ...
        new Notification.Builder(this.mContext, SystemNotificationChannels.DEVICE_ADMIN)
            ...
            .setContentIntent(
                PendingIntent.getActivityAsUser(
                    this.mContext, i, intent, 67108864, null, UserHandle.CURRENT))
    ...
    if (i == 2) {
      ...
      PendingIntent broadcast =
          PendingIntent.getBroadcast(
              this.mContext,
              678432343,
              new Intent("com.android.server.action.REMOTE_BUGREPORT_SHARING_ACCEPTED"),
              335544320);
      extend
          .addAction(
              new Notification.Action.Builder(
                      (Icon) null,
                      this.mContext.getString(R.string.fingerprint_error_hw_not_available),
                      PendingIntent.getBroadcast(
                          this.mContext,
                          678432343,
                          new Intent("com.android.server.action.REMOTE_BUGREPORT_SHARING_DECLINED"),
                          335544320))
                  .build())
          .addAction(
              new Notification.Action.Builder(
                      (Icon) null, this.mContext.getString(17042805), broadcast)
                  .build())
          .setContentTitle(this.mContext.getString(17042807))
          .setContentText(this.mContext.getString(17042806))
          .setStyle(new Notification.BigTextStyle().bigText(this.mContext.getString(17042806)));
    }
    return extend.build();
  }
  ...
  public boolean requestBugreport() {
    long nextLong;
    if (this.mRemoteBugreportServiceIsActive.get()
        || this.mService.getDeviceOwnerRemoteBugreportUriAndHash() != null) {
      Slogf.m86d(
          "DevicePolicyManager",
          "Remote bugreport wasn't started because there's already one running");
      return false;
    }
    ...
    do {
      try {
        ...
        Slogf.m91e(
            "DevicePolicyManager",
            "Failed to make remote calls to start bugreportremote service",
            e);
        return false;
      } finally {
    ...
    this.mInjector
        .getNotificationManager()
        .notifyAsUser("DevicePolicyManager", 678432343, buildNotification(1), UserHandle.ALL);
  ...
  public final void registerRemoteBugreportReceivers() {
    try {
      ...
      Slogf.m106w(
          "DevicePolicyManager", e, "Failed to set type %s", "application/vnd.android.bugreport");
    }
  ...
  public final void onBugreportFinished(Intent intent) {
    ...
    if (longExtra == 0 || this.mRemoteBugreportNonce.get() != longExtra) {
      Slogf.m102w("DevicePolicyManager", "Invalid nonce provided, ignoring " + longExtra);
      return;
    }
    ...
    if (this.mRemoteBugreportSharingAccepted.get()) {
      ...
      this.mInjector
          .getNotificationManager()
          .notifyAsUser("DevicePolicyManager", 678432343, buildNotification(3), UserHandle.ALL);
    }
  ...
  public final void onBugreportSharingAccepted() {
    ...
    Pair deviceOwnerRemoteBugreportUriAndHash =
        this.mService.getDeviceOwnerRemoteBugreportUriAndHash();
    if (deviceOwnerRemoteBugreportUriAndHash != null) {
      shareBugreportWithDeviceOwnerIfExists(
          (String) deviceOwnerRemoteBugreportUriAndHash.first,
          (String) deviceOwnerRemoteBugreportUriAndHash.second);
    } else if (this.mRemoteBugreportServiceIsActive.get()) {
      this.mInjector
          .getNotificationManager()
          .notifyAsUser("DevicePolicyManager", 678432343, buildNotification(2), UserHandle.ALL);
    }
  }
