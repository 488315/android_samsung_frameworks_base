...
public final class DevicePolicyManagerServiceShellCommand extends ShellCommand {
  ...
  public int onCommand(String str) {
    ...
    try {
      ...
      switch (c) {
        ...
        case '\f':
          int runMarkProfileOwnerOnOrganizationOwnedDevice =
              runMarkProfileOwnerOnOrganizationOwnedDevice(outPrintWriter);
          if (outPrintWriter != null) {
            outPrintWriter.close();
          }
  ...
  public final void showHelp(PrintWriter printWriter) {
    ...
    printWriter.printf(
        "    Disables an active admin, the admin must have declared android:testOnly in the"
            + " application in its manifest. This will also remove device and profile owners.\n\n",
        new Object[0]);
    printWriter.printf("  %s\n", "clear-freeze-period-record");
    printWriter.printf(
        "    Clears framework-maintained record of past freeze periods that the device went"
            + " through. For use during feature development to prevent triggering restriction on"
            + " setting freeze periods.\n\n",
        new Object[0]);
    printWriter.printf("  %s\n", "force-network-logs");
    printWriter.printf(
    ...
    printWriter.printf(
        "    Marks the profile owner of the given user as managing an organization-owneddevice."
            + " That will give it access to device identifiers (such as serial number, IMEI and"
            + " MEID), as well as other privileges.\n\n",
        new Object[0]);
  }
  ...
  public final int runIsSafeOperation(PrintWriter printWriter) {
    ...
    printWriter.printf(
        ...
        safeToString(unsafeOperationReason == -1),
        DevicePolicyManager.operationSafetyReasonToString(unsafeOperationReason));
    return 0;
  }
  ...
  public final int runIsSafeOperationByReason(PrintWriter printWriter) {
    ...
    printWriter.printf(
        ...
        DevicePolicyManager.operationSafetyReasonToString(parseInt),
        safeToString(this.mService.isSafeOperation(parseInt)));
    return 0;
  }
  ...
  public final int runListOwners(PrintWriter printWriter) {
    ...
    for (int i = 0; i < printAndGetSize; i++) {
      ...
      printWriter.printf(
          "User %2d: admin=%s",
          Integer.valueOf(ownerShellData.userId), ownerShellData.admin.flattenToShortString());
      ...
      if (ownerShellData.isManagedProfileOwner) {
        printWriter.printf(
            ",ManagedProfileOwner(parentUserId=%d)", Integer.valueOf(ownerShellData.parentUserId));
      }
