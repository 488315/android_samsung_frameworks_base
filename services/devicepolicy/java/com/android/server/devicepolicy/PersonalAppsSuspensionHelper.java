...
public final class PersonalAppsSuspensionHelper {
  ...
  public String[] getPersonalAppsForSuspension() {
    ...
    for (String str :
        this.mPackageManager.getUnsuspendablePackages((String[]) arraySet.toArray(new String[0]))) {
      arraySet.remove(str);
    }
    if (Log.isLoggable("DevicePolicyManager", 4)) {
      Slogf.m96i(
          "DevicePolicyManager", "Packages subject to suspension: %s", String.join(",", arraySet));
    }
  ...
  public final List getSystemLauncherPackages() {
    ...
    for (ResolveInfo resolveInfo : this.mPackageManager.queryIntentActivities(intent, 786432)) {
      ...
      if (activityInfo == null || TextUtils.isEmpty(activityInfo.packageName)) {
        Slogf.wtf(
            "DevicePolicyManager", "Could not find package name for launcher app %s", resolveInfo);
      } else {
        ...
        try {
          ...
          Slogf.m92e(
              "DevicePolicyManager", "Could not find application info for launcher app: %s", str);
        }
      }
    }
    return arrayList;
  ...
  public final List getInputMethodPackages() {
    List enabledInputMethodListAsUser =
        InputMethodManagerInternal.get().getEnabledInputMethodListAsUser(this.mContext.getUserId());
    ArrayList arrayList = new ArrayList();
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter) {
    ...
    DevicePolicyManagerService.dumpApps(
        indentingPrintWriter, "critical packages", getCriticalPackages());
    DevicePolicyManagerService.dumpApps(
        indentingPrintWriter, "launcher packages", getSystemLauncherPackages());
    DevicePolicyManagerService.dumpApps(
        indentingPrintWriter, "accessibility services", getAccessibilityServices());
    DevicePolicyManagerService.dumpApps(
        indentingPrintWriter, "input method packages", getInputMethodPackages());
    ...
    DevicePolicyManagerService.dumpApps(
        indentingPrintWriter, "Packages subject to suspension", getPersonalAppsForSuspension());
