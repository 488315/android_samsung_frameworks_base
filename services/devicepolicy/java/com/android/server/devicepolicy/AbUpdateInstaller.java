...
public class AbUpdateInstaller extends UpdateInstaller {
  ...
  public static Map buildErrorStringsMap() {
    ...
    hashMap.put(
        20,
        "The delta update payload was targeted for another version or the source partitionwas"
            + " modified after it was installed");
    ...
    hashMap.put(
        52, "The payload has been successfully installed,but the active slot was not flipped.");
    return hashMap;
  }

  public AbUpdateInstaller(
      Context context,
  ...
  public void installUpdateInThread() {
    ...
    try {
      setState();
      applyPayload(
          Paths.get(this.mCopiedUpdateFile.getAbsolutePath(), new String[0]).toUri().toString());
  ...
  public final void applyPayload(String str) {
    if (updateStateForPayload()) {
              ...
              this.mProperties.stream()
                  .toArray(
                      new IntFunction() { // from class:
                        // com.android.server.devicepolicy.AbUpdateInstaller$$ExternalSyntheticLambda0
      ...
      try {
        buildBoundUpdateEngine()
            .applyPayload(str, this.mOffsetForUpdate, this.mSizeForUpdate, strArr);
      } catch (Exception e) {
  ...
  public final void updatePropertiesForEntry(ZipEntry zipEntry) {
    BufferedReader bufferedReader =
        new BufferedReader(new InputStreamReader(this.mPackedUpdateFile.getInputStream(zipEntry)));
    while (true) {
      try {
  ...
  public class DelegatingUpdateEngineCallback extends UpdateEngineCallback {
    ...
    public DelegatingUpdateEngineCallback(
    ...
    public void onPayloadApplicationComplete(int i) {
      ...
      this.mUpdateInstaller.notifyCallbackOnError(
          ((Integer) AbUpdateInstaller.errorCodesMap.getOrDefault(Integer.valueOf(i), 1))
              .intValue(),
          (String)
              AbUpdateInstaller.errorStringsMap.getOrDefault(
                  Integer.valueOf(i), "Unknown error with error code = " + i));
    }
  }
}
