...
public class CertificateMonitor {
  ...
  public CertificateMonitor(
    ...
    BroadcastReceiver broadcastReceiver = new BroadcastReceiver() { // from class:
          // com.android.server.devicepolicy.CertificateMonitor.1
          @Override // android.content.BroadcastReceiver
          public void onReceive(Context context, Intent intent) {
            CertificateMonitor.this.updateInstalledCertificates(
                UserHandle.of(
    ...
    injector.mContext.registerReceiverAsUser(
        broadcastReceiver, UserHandle.ALL, intentFilter, null, handler);
  }
    ...
    try {
      ...
      try {
        KeyChain.KeyChainConnection keyChainBindAsUser =
            this.mInjector.keyChainBindAsUser(userHandle);
        try {
          String installCaCertificate =
              keyChainBindAsUser.getService().installCaCertificate(convertToPem);
          keyChainBindAsUser.close();
          return installCaCertificate;
        } finally {
        }
      } catch (RemoteException e) {
        Slogf.m91e("DevicePolicyManager", "installCaCertsToKeyChain(): ", e);
        return null;
      } catch (InterruptedException e2) {
        Slogf.m103w("DevicePolicyManager", "installCaCertsToKeyChain(): ", e2);
        Thread.currentThread().interrupt();
        return null;
      }
    } catch (IOException | CertificateException e3) {
      Slogf.m91e("DevicePolicyManager", "Problem converting cert", e3);
      return null;
    }
  }
  ...
  public void uninstallCaCerts(UserHandle userHandle, String[] strArr) {
    try {
      ...
      Slogf.m91e("DevicePolicyManager", "from CaCertUninstaller: ", e);
    } catch (InterruptedException e2) {
      Slogf.m103w("DevicePolicyManager", "CaCertUninstaller: ", e2);
      Thread.currentThread().interrupt();
    }
  }
  ...
  public void onCertificateApprovalsChanged(final int i) {
    this.mHandler.post(
        new Runnable() { // from class:
          // com.android.server.devicepolicy.CertificateMonitor$$ExternalSyntheticLambda0
    ...
    if (this.mInjector.getUserManager().isUserUnlocked(identifier)) {
      try {
        ...
        try {
          ...
          if (licenseService != null && licenseService.getAllLicenseInfo() != null) {
            this.mInjector
                ...
                .cancelAsUser("DevicePolicyManager", 33, userHandle);
            return;
          }
        } catch (RemoteException unused) {
        }
        int size =
            installedCaCertificates.size()
                - this.mService.getAcceptedCaCertificates(userHandle).size();
        if (size != 0) {
          this.mInjector
              .getNotificationManager()
              .notifyAsUser(
                  "DevicePolicyManager", 33, buildNotification(userHandle, size), userHandle);
        } else {
          this.mInjector
              .getNotificationManager()
              .cancelAsUser("DevicePolicyManager", 33, userHandle);
        ...
        Slogf.m93e(
            "DevicePolicyManager",
            e,
            "Could not retrieve certificates from KeyChain service for user %d",
  ...
  public final Notification buildNotification(UserHandle userHandle, int i) {
    ...
    try {
      ...
      if (this.mService.lambda$isProfileOwner$71(userHandle.getIdentifier()) != null) {
        str =
            resources.getString(
                17042894, this.mService.getProfileOwnerName(userHandle.getIdentifier()));
      ...
      ActivityInfo resolveActivityInfo =
          intent.resolveActivityInfo(this.mInjector.getPackageManager(), 1048576);
      if (resolveActivityInfo != null) {
        intent.setComponent(resolveActivityInfo.getComponentName());
      }
      PendingIntent pendingIntentGetActivityAsUser =
          this.mInjector.pendingIntentGetActivityAsUser(
              createContextAsUser, 0, intent, 201326592, null, UserHandle.of(identifier));
      HashMap hashMap = new HashMap();
      hashMap.put("count", Integer.valueOf(i));
      return new Notification.Builder(createContextAsUser, SystemNotificationChannels.SECURITY)
          .setSmallIcon(i3)
          .setContentTitle(PluralsMessageFormatter.format(resources, hashMap, 17042895))
          .setContentText(str)
          .setContentIntent(pendingIntentGetActivityAsUser)
          .setShowWhen(false)
          .setColor(R.color.system_notification_accent_color)
          .build();
    } catch (PackageManager.NameNotFoundException e) {
      Slogf.m93e("DevicePolicyManager", e, "Create context as %s failed", userHandle);
      return null;
    }
  }
