    ...
    public Intent getModuleServiceIntent(
      ...
      try {
        int packageUidAsUser =
            packageManager.getPackageUidAsUser(resolveSystemService.getPackageName(), 0);
  ...
  public void startModuleService(
    ...
    if (!this.mContext.bindServiceAsUser(
      ...
      maybeCrashWithTerribleFailure(
          "Could not bind to networking module in-process, or in app with " + moduleServiceIntent,
          packageName);
      return;
    }
  ...
  public class ModuleServiceConnection implements ServiceConnection {
    ...
    public void onServiceDisconnected(ComponentName componentName) {
      ConnectivityModuleConnector.this.maybeCrashWithTerribleFailure(
          "Lost network stack. This is not the root cause of any issue, it is a side effect of a"
              + " crash that happened earlier. Earlier logs should point to the actual issue.",
          this.mPackageName);
    }
  }
  ...
  public synchronized void maybeCrashWithTerribleFailure(String str, String str2) {
    ...
    long j =
        DeviceConfig.getLong(
            "connectivity", CONFIG_MIN_CRASH_INTERVAL_MS, DEFAULT_MIN_CRASH_INTERVAL_MS);
    long j2 = DeviceConfig.getLong("connectivity", CONFIG_MIN_UPTIME_BEFORE_CRASH_MS, 1800000L);
    boolean z = false;
    boolean z2 =
        DeviceConfig.getBoolean("connectivity", CONFIG_ALWAYS_RATELIMIT_NETWORKSTACK_CRASH, false);
    ...
    if ((tryGetLastCrashTime != 0 && tryGetLastCrashTime < currentTimeMillis)
  ...
  private SharedPreferences getSharedPreferences() {
    try {
      return this.mContext
          ...
          .getSharedPreferences(new File(Environment.getDataSystemDeDirectory(0), PREFS_FILE), 0);
    } catch (Throwable th) {
