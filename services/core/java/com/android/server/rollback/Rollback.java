...
public class Rollback {
  ...
  public boolean enableForPackage(
      String str, long j, long j2, boolean z, String str2, String[] strArr, int i) {
    assertInWorkerThread();
    try {
      ...
      this.info
          .getPackages()
          .add(
              new PackageRollbackInfo(
                  new VersionedPackage(str, j),
                  new VersionedPackage(str, j2),
                  new ArrayList(),
                  new ArrayList(),
                  z,
                  false,
  ...
  public void snapshotUserData(
      String str, int[] iArr, AppDataRollbackHelper appDataRollbackHelper) {
    assertInWorkerThread();
    if (isEnabling()) {
        ...
        if (packageRollbackInfo.getPackageName().equals(str)) {
          if (packageRollbackInfo.getRollbackDataPolicy() == 0) {
            appDataRollbackHelper.snapshotAppData(
                this.info.getRollbackId(), packageRollbackInfo, iArr);
  ...
  public void commitPendingBackupAndRestoreForUser(
      int i, AppDataRollbackHelper appDataRollbackHelper) {
    assertInWorkerThread();
  ...
  public void commit(
      final Context context, final List list, String str, final IntentSender intentSender) {
    boolean z;
    File[] fileArr;
    ...
    if (containsApex() && wasCreatedAtLowerExtensionVersion()) {
      if (extensionVersionReductionWouldViolateConstraint(
          this.mExtensionVersions,
          (PackageManagerInternal) LocalServices.getService(PackageManagerInternal.class))) {
        RollbackManagerServiceImpl.sendFailure(
            context, intentSender, 1, "Rollback may violate a minExtensionVersion constraint");
        return;
      }
    }
    try {
      ...
      PackageManager packageManager =
          context
              .createPackageContextAsUser(str, 0, UserHandle.of(this.mUserId))
              .getPackageManager();
      try {
        ...
        for (PackageRollbackInfo packageRollbackInfo : this.info.getPackages()) {
          ...
          if (!packageRollbackInfo.isApkInApex()) {
            PackageInstaller.SessionParams sessionParams2 =
                new PackageInstaller.SessionParams(z2 ? 1 : 0);
            ...
            File[] packageCodePaths =
                RollbackStore.getPackageCodePaths(this, packageRollbackInfo.getPackageName());
        ...
        RollbackManagerServiceImpl.sendFailure(
            context, intentSender, 1, "IOException: " + e.toString());
      }
  ...
  public /* synthetic */ void lambda$commit$0(
      Intent intent, Context context, IntentSender intentSender, List list) {
    assertInWorkerThread();
    ...
    while (it.hasNext()) {
      context.sendBroadcastAsUser(
          intent3, (UserHandle) it.next(), "android.permission.MANAGE_ROLLBACKS");
    }
  }

  public boolean restoreUserDataForPackageIfInProgress(
      String str, int[] iArr, int i, String str2, AppDataRollbackHelper appDataRollbackHelper) {
    assertInWorkerThread();
    ...
    for (PackageRollbackInfo packageRollbackInfo : this.info.getPackages()) {
      if (packageRollbackInfo.getPackageName().equals(str)) {
        ...
        for (int i2 : iArr) {
          z |=
              appDataRollbackHelper.restoreAppData(
                  this.info.getRollbackId(), packageRollbackInfo, i2, i, str2);
        }
        if (z) {
          RollbackStore.saveRollback(this);
        }
      ...
      if (packageRollbackInfo.isApex()) {
        ...
        for (int i = 0; i < snapshottedUsers.size(); i++) {
          appDataRollbackHelper.destroyAppDataSnapshot(
              this.info.getRollbackId(),
              packageRollbackInfo,
              ((Integer) snapshottedUsers.get(i)).intValue());
        }
      }
    }
    if (z) {
      ...
      while (it.hasNext()) {
        appDataRollbackHelper.destroyApexCeSnapshots(
            ((Integer) it.next()).intValue(), this.info.getRollbackId());
      }
    }
  ...
  public boolean includesPackageWithDifferentVersion(String str, long j) {
    ...
    for (PackageRollbackInfo packageRollbackInfo : this.info.getPackages()) {
      if (packageRollbackInfo.getPackageName().equals(str)
          && packageRollbackInfo.getVersionRolledBackFrom().getLongVersionCode() != j) {
        return true;
      }
    }
    return false;
  ...
  public static boolean extensionVersionReductionWouldViolateConstraint(
    ...
    for (int i = 0; i < packageNames.size(); i++) {
      SparseIntArray minExtensionVersions =
          packageManagerInternal.getPackage((String) packageNames.get(i)).getMinExtensionVersions();
  ...
  public final boolean wasCreatedAtLowerExtensionVersion() {
    for (int i = 0; i < this.mExtensionVersions.size(); i++) {
      if (SdkExtensions.getExtensionVersion(this.mExtensionVersions.keyAt(i))
          > this.mExtensionVersions.valueAt(i)) {
        return true;
      }
    }
    return false;
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter) {
    ...
    if (isCommitted()) {
      ...
      for (VersionedPackage versionedPackage : this.info.getCausePackages()) {
        indentingPrintWriter.println(
            versionedPackage.getPackageName() + " " + versionedPackage.getLongVersionCode());
      }
      indentingPrintWriter.decreaseIndent();
