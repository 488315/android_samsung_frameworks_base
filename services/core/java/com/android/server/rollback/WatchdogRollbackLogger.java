...
public abstract class WatchdogRollbackLogger {
  ...
  public static String rollbackReasonToString(int i) {
    return i != 1
        ? i != 2
            ? i != 3
                ? i != 4
                    ? i != 5 ? "UNKNOWN" : "REASON_NATIVE_CRASH_DURING_BOOT"
  ...
  public static String rollbackTypeToString(int i) {
    return i != 1
        ? i != 2
            ? i != 3 ? i != 4 ? "UNKNOWN" : "ROLLBACK_BOOT_TRIGGERED" : "ROLLBACK_FAILURE"
            : "ROLLBACK_SUCCESS"
        : "ROLLBACK_INITIATE";
  }
  ...
  public static String getLoggingParentName(Context context, String str) {
    try {
      Bundle bundle =
          context.getPackageManager().getPackageInfo(str, 1073741952).applicationInfo.metaData;
      if (bundle == null) {
        return null;
      }
  ...
  public static VersionedPackage getLogPackage(Context context, VersionedPackage versionedPackage) {
    ...
    try {
      return new VersionedPackage(
          loggingParentName,
          context.getPackageManager().getPackageInfo(loggingParentName, 0).getLongVersionCode());
  ...
  public static void logRollbackStatusOnBoot(Context context, int i, String str, List list) {
    ...
    if (sessionInfo == null) {
      Slog.e(
          ...
          "On boot completed, could not load session id " + committedSessionId);
      return;
    }
  ...
  public static void logEvent(VersionedPackage versionedPackage, int i, int i2, String str) {
    Slog.i(
        "WatchdogRollbackLogger",
        "Watchdog event occurred with type: "
            + rollbackTypeToString(i)
            + " logPackage: "
            + versionedPackage
            + " rollbackReason: "
            + rollbackReasonToString(i2)
            + " failedPackageName: "
            + str);
  ...
  public static void logTestProperties(
      VersionedPackage versionedPackage, int i, int i2, String str) {
    if (SystemProperties.getBoolean("persist.sys.rollbacktest.enabled", false)) {
      ...
      SystemProperties.set(
          str2 + ".logPackage", versionedPackage != null ? versionedPackage.toString() : "");
