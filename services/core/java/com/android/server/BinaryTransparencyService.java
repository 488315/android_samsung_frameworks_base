...
public class BinaryTransparencyService extends SystemService {
  ...
  public static final Histogram digestAllPackagesLatency =
      new Histogram(
          ...
          new Histogram.UniformOptions(50, DisplayPowerController2.RATE_FROM_DOZE_TO_ON, 500.0f));
  ...
  public final class BinaryTransparencyServiceImpl extends IBinaryTransparencyService.Stub {
      ...
      for (Signature signature : apkContentsSigners) {
        arrayList.add(
            HexEncoding.encodeToString(
                PackageUtils.computeSha256DigestBytes(signature.toByteArray()), false));
      }
    ...
    public final List collectAppInfo(PackageState packageState, int i) {
      ...
      InstallSourceInfo installSourceInfo =
          BinaryTransparencyService.this.getInstallSourceInfo(packageState.getPackageName());
    ...
    public void recordMeasurementsForAllPackages() {
      ...
      if (CompatChanges.isChangeEnabled(245692487L)) {
        for (IBinaryTransparencyService.AppInfo appInfo2 :
            collectAllSilentInstalledMbaInfo(bundle)) {
    ...
    public List collectAllApexInfo(boolean z) {
      ...
      for (PackageInfo packageInfo : BinaryTransparencyService.this.getCurrentInstalledApexs()) {
        PackageStateInternal packageStateInternal =
            BinaryTransparencyService.this.mPackageManagerInternal.getPackageStateInternal(
                packageInfo.packageName);
        if (packageStateInternal == null) {
          Slog.w(
              "TransparencyService",
              "Package state is unavailable, ignoring the APEX " + packageInfo.packageName);
        } else {
          AndroidPackage androidPackage = packageStateInternal.getAndroidPackage();
          if (androidPackage == null) {
            Slog.w(
                "TransparencyService", "Skipping the missing APK in " + androidPackage.getPath());
          } else {
            Digest measureApk = measureApk(androidPackage.getPath());
            if (measureApk == null) {
              Slog.w(
                  "TransparencyService",
                  "Skipping the missing APEX in " + androidPackage.getPath());
            } else {
              ...
              apexInfo.signerDigests =
                  computePackageSignerSha256Digests(packageStateInternal.getSigningInfo());
              if (z) {
                apexInfo.moduleName =
                    BinaryTransparencyService.this.apexPackageNameToModuleName(
    ...
    public List collectAllUpdatedPreloadInfo(final Bundle bundle) {
      ...
      BinaryTransparencyService.this.mPackageManagerInternal.forEachPackageState(
          new Consumer() { // from class:
            // com.android.server.BinaryTransparencyService$BinaryTransparencyServiceImpl$$ExternalSyntheticLambda0
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
              BinaryTransparencyService.BinaryTransparencyServiceImpl.this
                  .lambda$collectAllUpdatedPreloadInfo$0(
                      bundle, arrayList, (PackageStateInternal) obj);
            }
          });
      return arrayList;
    }
    ...
    public /* synthetic */ void lambda$collectAllUpdatedPreloadInfo$0(
        Bundle bundle, ArrayList arrayList, PackageStateInternal packageStateInternal) {
      if (packageStateInternal.isUpdatedSystemApp()
          && !bundle.containsKey(packageStateInternal.getPackageName())) {
        Slog.d(
            "TransparencyService",
            "Preload "
                + packageStateInternal.getPackageName()
      ...
      for (PackageInfo packageInfo : BinaryTransparencyService.this.getNewlyInstalledMbas()) {
        if (!bundle.containsKey(packageInfo.packageName)) {
          ...
          if (packageStateInternal == null) {
            Slog.w(
                "TransparencyService",
                "Package state is unavailable, ignoring the package " + packageInfo.packageName);
          } else {
    ...
    public final Map computeApkContentDigest(String str) {
      ...
      if (verifySignaturesInternal.isError()) {
        Slog.e(
            "TransparencyService",
            "Failed to compute content digest for "
                + str
                + " due to: "
                + verifySignaturesInternal.getErrorMessage());
        return null;
      }
      return ((ApkSignatureVerifier.SigningDetailsWithDigests) verifySignaturesInternal.getResult())
          .contentDigests;
    }
      ...
      new ShellCommand() { // from class:
        ...
        public final void printPackageMeasurements(
          ...
          Map computeApkContentDigest =
              BinaryTransparencyServiceImpl.this.computeApkContentDigest(
          ...
          if (z) {
            printWriter.print(
                PackageUtils.computeSha256DigestForLargeFile(
                        packageInfo.applicationInfo.sourceDir, PackageUtils.createLargeFileBuffer())
                    + ",");
          }
          for (Map.Entry entry : computeApkContentDigest.entrySet()) {
            ...
            printWriter.print(
                BinaryTransparencyService.this.translateContentDigestAlgorithmIdToString(
                    num.intValue()));
        ...
        public final void printPackageInstallationInfo(
          ...
          if (packageInfo.applicationInfo.sourceDir.startsWith("/data/apex/")) {
            String originalApexPreinstalledLocation =
                BinaryTransparencyService.this.getOriginalApexPreinstalledLocation(
                    packageInfo.packageName);
            printWriter.println(
                "|--> Pre-installed package install location: " + originalApexPreinstalledLocation);
            if (!originalApexPreinstalledLocation.equals("could-not-be-determined")) {
              if (z) {
                printWriter.println(
                    "|--> Pre-installed package SHA-256 digest: "
                        + PackageUtils.computeSha256DigestForLargeFile(
                            originalApexPreinstalledLocation,
                ...
                for (Map.Entry entry : computeApkContentDigest.entrySet()) {
                  ...
                  printWriter.println(
                      "|--> Pre-installed package content digest: "
                          + HexEncoding.encodeToString((byte[]) entry.getValue(), false));
                  printWriter.println(
                      "|--> Pre-installed package content digest algorithm: "
                          + BinaryTransparencyService.this
                              .translateContentDigestAlgorithmIdToString(num.intValue()));
                }
              }
            }
          }
          ...
          printWriter.println(
              "Is preloaded: " + (packageInfo.firstInstallTime == packageInfo.lastUpdateTime));
          InstallSourceInfo installSourceInfo =
              BinaryTransparencyService.this.getInstallSourceInfo(packageInfo.packageName);
          if (installSourceInfo == null) {
            printWriter.println(
                "ERROR: Unable to obtain installSourceInfo of " + packageInfo.packageName);
          } else {
            printWriter.println(
                "Installation initiated by: " + installSourceInfo.getInitiatingPackageName());
            printWriter.println(
                "Installation done by: " + installSourceInfo.getInstallingPackageName());
            printWriter.println(
                "Installation originating from: " + installSourceInfo.getOriginatingPackageName());
          }
        ...
        public final void printPackageSignerDetails(
          ...
          printWriter.println(
              "Signing key has been rotated: " + signingInfo.hasPastSigningCertificates());
          Signature[] apkContentsSigners = signingInfo.getApkContentsSigners();
          ...
          for (int i = 0; i < length; i++) {
            ...
            try {
              printWriter.println(
                  "Signing key algorithm: " + signature.getPublicKey().getAlgorithm());
            } catch (CertificateException e) {
              Slog.e(
                  "ShellCommand",
                  "Failed to obtain public key of signer for cert with hash: " + encodeToString,
                  e);
            }
          }
          ...
          printWriter.println(
              "(Certs are sorted in the order of rotation, beginning with the original signing"
                  + " cert)");
          ...
          while (i2 < signingCertificateHistory.length - 1) {
            ...
            String encodeToString2 =
                HexEncoding.encodeToString(
                    PackageUtils.computeSha256DigestBytes(signature2.toByteArray()), false);
            ...
            try {
              printWriter.println(
                  "  Signing key algorithm: " + signature2.getPublicKey().getAlgorithm());
            } catch (CertificateException e2) {
              Slog.e(
                  "ShellCommand",
                  "Failed to obtain public key of signer for cert with hash: " + encodeToString2,
                  e2);
            }
          }
        }
        ...
        public final void printAppDetails(
          ...
          printWriter.println(
              "Target SDK version: " + packageInfo.applicationInfo.targetSdkVersion);
          ...
          printWriter.println(
              "Component factory: " + packageInfo.applicationInfo.appComponentFactory);
          ...
          if (z) {
            ...
            while (i < sharedLibraryInfos.size()) {
              ...
              printWriter.print(KnoxVpnFirewallHelper.DELIMITER_IP_RESTORE);
              printWriter.println(
                  "  Lib package name (if available): " + sharedLibraryInfo.getPackageName());
          ...
          while (true) {
            ...
            if (nextOption == null) {
              ...
              for (PackageInfo packageInfo :
                  BinaryTransparencyService.this.getCurrentInstalledApexs()) {
                if (z && z3) {
                  printHeadersHelper("APEX", z2, outPrintWriter);
                }
                outPrintWriter.print(
                    packageInfo.packageName + "," + packageInfo.getLongVersionCode() + ",");
                printPackageMeasurements(packageInfo, z2, outPrintWriter);
                if (z) {
                  try {
                    ModuleInfo moduleInfo =
                        packageManager.getModuleInfo(packageInfo.packageName, 0);
          ...
          while (true) {
            ...
            if (nextOption == null) {
              ...
              for (ModuleInfo moduleInfo :
                  packageManager2.getInstalledModules(
                ...
                try {
                  ...
                  try {
                    ...
                    outPrintWriter.println(
                        packageName + ",ERROR:Unable to find PackageInfo for this module.");
                    if (z2) {
                      printModuleDetails(moduleInfo, outPrintWriter);
                      outPrintWriter.println("");
                    }
          ...
          while (true) {
            ...
            if (nextOption == null) {
              ...
              Iterator<PackageInfo> it =
                  packageManager
                      .getInstalledPackages(PackageManager.PackageInfoFlags.of(136314880L))
                      .iterator();
              while (it.hasNext()) {
                ...
                if (next.signingInfo == null) {
                  try {
                    packageManager.getPackageInfo(
                        next.packageName, PackageManager.PackageInfoFlags.of(134348800L));
                  } catch (PackageManager.NameNotFoundException unused) {
                    Slog.e(
                        "ShellCommand",
                        "Failed to obtain an updated PackageInfo of " + next.packageName);
                  }
                }
                if (z2 && z) {
                  printHeadersHelper("Preload", z4, outPrintWriter);
              ...
              for (PackageInfo packageInfo :
                  BinaryTransparencyService.this.getNewlyInstalledMbas()) {
                if (z2 && z) {
                  printHeadersHelper("MBA", z4, outPrintWriter);
                }
                outPrintWriter.print(packageInfo.packageName + ",");
        ...
        public final void printHelpMenu() {
          ...
          outPrintWriter.println(
              "      -o: also uses the old digest scheme (SHA256) to compute APEX hashes. WARNING:"
                  + " This can be a very slow and CPU-intensive computation.");
          ...
          outPrintWriter.println(
              "      -o: also uses the old digest scheme (SHA256) to compute module hashes."
                  + " WARNING: This can be a very slow and CPU-intensive computation.");
          ...
          outPrintWriter.println(
              "      -o: also uses the old digest scheme (SHA256) to compute MBA hashes. WARNING:"
                  + " This can be a very slow and CPU-intensive computation.");
          outPrintWriter.println("      -v: lists more verbose information about each app.");
          outPrintWriter.println(
              "      -l: lists shared library info. (This option only works when -v option is also"
                  + " specified)");
          outPrintWriter.println("      --no-headers: does not print the header if specified.");
          outPrintWriter.println(
              "      --preloads-only: lists only preloaded apps. This options can also be combined"
                  + " with others.");
          outPrintWriter.println("");
        }
  ...
  public class BiometricLogger {
    ...
    public void logStats(
        ...
        String str5) {
      FrameworkStatsLog.write(
          FrameworkStatsLog.BIOMETRIC_PROPERTIES_COLLECTED,
          i,
          i2,
          i3,
          i4,
  ...
  public class UpdateMeasurementsJobService extends JobService {
          ...
          .newThread(
              new Runnable() { // from class:
                ...
                public final void run() {
                  BinaryTransparencyService.UpdateMeasurementsJobService.this.lambda$onStartJob$0(
                      jobParameters);
                }
              })
          .start();
    ...
    public /* synthetic */ void lambda$onStartJob$0(JobParameters jobParameters) {
      try {
        IBinaryTransparencyService.Stub.asInterface(ServiceManager.getService("transparency"))
    ...
    public static void scheduleBinaryMeasurements(
        Context context, BinaryTransparencyService binaryTransparencyService) {
      ...
      if (sTimeLastRanMs != 0) {
        ...
        Slog.d(
            "TransparencyService",
            "Scheduling the next measurement to be done at least " + j + "ms from now.");
      }
      if (jobScheduler.schedule(
              new JobInfo.Builder(
                      1740526926,
                      new ComponentName(context, (Class<?>) UpdateMeasurementsJobService.class))
  ...
  public final void logBiometricProperties(SensorProperties sensorProperties, int i, int i2) {
    ...
    for (SensorProperties.ComponentInfo componentInfo : sensorProperties.getComponentInfo()) {
      this.mBiometricLogger.logStats(
          sensorId,
          i,
          i2,
          sensorStrength,
      ...
      if (fingerprintManager != null) {
        fingerprintManager.addAuthenticatorsRegisteredCallback(
            new IFingerprintAuthenticatorsRegisteredCallback
                .Stub() { // from class: com.android.server.BinaryTransparencyService.1
              public void onAllAuthenticatorsRegistered(List list) {
                ...
                while (it.hasNext()) {
                  ...
                  BinaryTransparencyService binaryTransparencyService =
                      BinaryTransparencyService.this;
                  binaryTransparencyService.logBiometricProperties(
                      from,
                      1,
                      binaryTransparencyService.toFingerprintSensorType(from.getSensorType()));
                }
              }
            });
      }
      if (faceManager != null) {
        faceManager.addAuthenticatorsRegisteredCallback(
            new IFaceAuthenticatorsRegisteredCallback
                .Stub() { // from class: com.android.server.BinaryTransparencyService.2
              public void onAllAuthenticatorsRegistered(List list) {
                Iterator it = list.iterator();
                while (it.hasNext()) {
                  FaceSensorProperties from =
                      FaceSensorProperties.from((FaceSensorPropertiesInternal) it.next());
                  BinaryTransparencyService binaryTransparencyService =
                      BinaryTransparencyService.this;
                  binaryTransparencyService.logBiometricProperties(
                      from, 4, binaryTransparencyService.toFaceSensorType(from.getSensorType()));
                }
              }
            });
      }
  ...
  public final void registerStagedApexUpdateObserver() {
    ...
    try {
      asInterface.registerStagedApexObserver(
          new IStagedApexObserver
              .Stub() { // from class: com.android.server.BinaryTransparencyService.3
            public void onApexStaged(ApexStagedEvent apexStagedEvent) {
              Slog.d(
                  "TransparencyService",
                  "A new APEX has been staged for update. There are currently "
              ...
              UpdateMeasurementsJobService.scheduleBinaryMeasurements(
                  BinaryTransparencyService.this.mContext, BinaryTransparencyService.this);
            }
          });
  ...
  public final boolean isPackagePreloaded(String str) {
    try {
      this.mContext
          .getPackageManager()
          .getPackageInfo(str, PackageManager.PackageInfoFlags.of(2097152L));
      return true;
    } catch (PackageManager.NameNotFoundException unused) {
      return false;
    }
  ...
  public final boolean isPackageAnApex(String str) {
    try {
      return this.mContext
          .getPackageManager()
          .getPackageInfo(str, PackageManager.PackageInfoFlags.of(1073741824L))
          .isApex;
    } catch (PackageManager.NameNotFoundException unused) {
      return false;
    }
  ...
  public class PackageUpdatedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      if (intent.getAction().equals("android.intent.action.PACKAGE_ADDED")) {
        ...
        if (BinaryTransparencyService.this.isPackagePreloaded(schemeSpecificPart)
            || BinaryTransparencyService.this.isPackageAnApex(schemeSpecificPart)) {
          Slog.d(
              "TransparencyService",
  ...
  public final List getCurrentInstalledApexs() {
    ...
    List<PackageInfo> installedPackages =
        packageManager.getInstalledPackages(PackageManager.PackageInfoFlags.of(1207959552L));
        ...
        installedPackages.stream()
            .filter(
                new Predicate() { // from class:
                  // com.android.server.BinaryTransparencyService$$ExternalSyntheticLambda0
