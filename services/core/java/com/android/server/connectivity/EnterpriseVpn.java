...
public class EnterpriseVpn {
                ...
                if (str.equals(EnterpriseVpn.this.mInterface) && EnterpriseVpn.this.jniCheck(str) == 0) {
                    ...
                    try {
                        if (EnterpriseVpn.this.mConnection != null) {
                            ...
                            EnterpriseVpn.getService().disconnectKnoxVpn(EnterpriseVpn.this.mProfileName, EnterpriseVpn.this.mOwnerUID);
    ...
    public void updateState(NetworkInfo.DetailedState detailedState, String str) {
        ...
        int i = AbstractC10384.$SwitchMap$android$net$NetworkInfo$DetailedState[detailedState.ordinal()];
        if (i == 1) {
    ...
    public final Notification createNotification(PendingIntent pendingIntent) {
        ...
        return new Notification.Builder(this.mContext, SystemNotificationChannels.VPN).setSmallIcon(17304616).setContentTitle(string).setContentText(this.mContext.getString(R.string.older)).setContentIntent(pendingIntent).setDefaults(0).setOngoing(true).setPriority(2).getNotification();
    }
