...
public class MultipathPolicyTracker {
  ...
  public void start() {
    ...
    this.mUserAllContext.registerReceiver(
        this.mConfigChangeReceiver, intentFilter, null, this.mHandler);
  }

  public Integer getMultipathPreference(Network network) {
    MultipathTracker multipathTracker;
    if (network == null
        || (multipathTracker = (MultipathTracker) this.mMultipathTrackers.get(network)) == null) {
      return null;
    }
  ...
  public class MultipathTracker {
    ...
    public MultipathTracker(final Network network, NetworkCapabilities networkCapabilities) {
      ...
      if (networkSpecifier instanceof TelephonyNetworkSpecifier) {
        ...
        TelephonyManager telephonyManager =
            (TelephonyManager)
                MultipathPolicyTracker.this.mContext.getSystemService(TelephonyManager.class);
        ...
        if (createForSubscriptionId == null) {
          throw new IllegalStateException(
              String.format(
                  "Can't get TelephonyManager for subId %d", Integer.valueOf(subscriptionId)));
        }
        ...
        this.mNetworkTemplate =
            new NetworkTemplate.Builder(1)
                ...
                .build();
        this.mUsageCallback =
            new NetworkStatsManager.UsageCallback() { // from class:
        ...
        NetworkStatsManager networkStatsManager =
            (NetworkStatsManager)
                MultipathPolicyTracker.this.mContext.getSystemService(NetworkStatsManager.class);
      ...
      throw new IllegalStateException(
          String.format(
              "Can't get subId from mobile network %s (%s)", network, networkCapabilities));
    }
    ...
    public final long getDailyNonDefaultDataUsage() {
      ZonedDateTime ofInstant =
          ZonedDateTime.ofInstant(
              MultipathPolicyTracker.this.mClock.instant(), ZoneId.systemDefault());
      return getNetworkTotalBytes(
    ...
    public final NetworkIdentity getTemplateMatchingNetworkIdentity(
        NetworkCapabilities networkCapabilities) {
      return new NetworkIdentity.Builder()
          .setType(0)
    ...
    public final long getRemainingDailyBudget(long j, Range range) {
      ...
      return (networkTotalBytes != -1 ? Math.max(0L, j - networkTotalBytes) : 0L)
          / Math.max(
              1L,
              (((epochMilli2 - MultipathPolicyTracker.this.mClock.millis()) - 1)
                      / TimeUnit.DAYS.toMillis(1L))
                  + 1);
    }

    public final long getUserPolicyOpportunisticQuotaBytes() {
      NetworkIdentity templateMatchingNetworkIdentity =
          getTemplateMatchingNetworkIdentity(this.mNetworkCapabilities);
      long j = Long.MAX_VALUE;
      for (NetworkPolicy networkPolicy : MultipathPolicyTracker.this.mNPM.getNetworkPolicies()) {
        if (networkPolicy.hasCycle()
            && networkPolicy.template.matches(templateMatchingNetworkIdentity)) {
          long epochMilli =
              ((ZonedDateTime) ((Range) networkPolicy.cycleIterator().next()).getLower())
                  .toInstant()
                  .toEpochMilli();
            ...
            j =
                Math.min(
                    ...
                    getRemainingDailyBudget(
                        activeWarning, (Range) networkPolicy.cycleIterator().next()));
          }
        }
      }
      if (j == Long.MAX_VALUE) {
    ...
    public void updateMultipathBudget() {
      ...
      if (subscriptionOpportunisticQuota == -1) {
        subscriptionOpportunisticQuota =
            MultipathPolicyTracker.this.getDefaultDailyMultipathQuotaBytes();
      }
      ...
      long max =
          dailyNonDefaultDataUsage != -1
              ? Math.max(0L, subscriptionOpportunisticQuota - dailyNonDefaultDataUsage)
              : 0L;
      if (max > 2097152) {
  ...
  public final long getDefaultDailyMultipathQuotaBytes() {
    String string =
        Settings.Global.getString(
            this.mContext.getContentResolver(), "network_default_daily_multipath_quota_bytes");
    if (string != null) {
      try {
    ...
    return this.mContext.getResources().getInteger(R.integer.config_notificationsBatteryLowARGB);
  }
    ...
    ConnectivityManager.NetworkCallback networkCallback =
        new ConnectivityManager.NetworkCallback() { // from class:
          ...
          public void onCapabilitiesChanged(
            ...
            MultipathTracker multipathTracker =
                (MultipathTracker) MultipathPolicyTracker.this.mMultipathTrackers.get(network);
            ...
            try {
              MultipathPolicyTracker.this.mMultipathTrackers.put(
                  network,
                  MultipathPolicyTracker.this.new MultipathTracker(network, networkCapabilities));
            } catch (IllegalStateException e) {
              Log.e(
                  MultipathPolicyTracker.TAG,
                  "Can't track mobile network " + network + ": " + e.getMessage());
            }
          }
  ...
  public class C10412 extends NetworkPolicyManager.Listener {
    public C10412() {}
      ...
      MultipathPolicyTracker.this.mHandler.post(
          new Runnable() { // from class:
            ...
            public final void run() {
              MultipathPolicyTracker.C10412.this.lambda$onMeteredIfacesChanged$0();
            }
          });
    }
  }
  ...
  public final void registerNetworkPolicyListener() {
    C10412 c10412 = new C10412();
    this.mPolicyListener = c10412;
    this.mNPM.registerListener(c10412);
  }
    ...
    for (MultipathTracker multipathTracker : this.mMultipathTrackers.values()) {
      indentingPrintWriter.println(
          String.format(
              ...
              Long.valueOf(multipathTracker.getQuota()),
              Long.valueOf(multipathTracker.getMultipathBudget()),
              DebugUtils.flagsToString(
                  ConnectivityManager.class,
                  "MULTIPATH_PREFERENCE_",
                  multipathTracker.getMultipathPreference())));
    }
