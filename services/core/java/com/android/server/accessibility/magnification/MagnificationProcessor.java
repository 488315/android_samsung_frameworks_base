...
public final class MagnificationProcessor {
    ...
    public final void dump(PrintWriter printWriter, ArrayList arrayList) {
        ...
        for (int i2 = 0; i2 < arrayList.size(); i2++) {
            ...
            if (!region.isEmpty()) {
                printWriter
                        .append("    Magnification region=")
                        .append((CharSequence) region.toString())
                        .println();
            }
            ...
            if (magnificationConfig.getMode() == 1) {
                ...
                MagnificationConnectionManager magnificationConnectionManager =
                        this.mController.getMagnificationConnectionManager();
                    ...
                    try {
                        MagnificationConnectionManager.WindowMagnifier windowMagnifier =
                                (MagnificationConnectionManager.WindowMagnifier)
                                        magnificationConnectionManager.mWindowMagnifiers.get(
                                                displayId);
                        i =
                                windowMagnifier != null
                                        ? windowMagnifier.mIdOfLastServiceToControl
                                        : -1;
                    } finally {
                    }
                }
                ...
                .append(
                        (CharSequence)
                                ("    SupportWindowMagnification="
                                        + this.mController.mSupportWindowMagnification))
                .println();
        printWriter
                .append(
                        (CharSequence)
                                ("    WindowMagnificationConnectionState="
    ...
    public final void getFullscreenMagnificationRegion(int i, Region region, boolean z) {
        ...
        try {
            magnificationController
                    .getFullScreenMagnificationController()
                    .getMagnificationRegion(i, region);
        } finally {
            if (z2) {
                unregister(i);
            }
    ...
    public final MagnificationConfig getMagnificationConfig(int i) {
        ...
        if (controllingMode == 1) {
            ...
            builder.setMode(controllingMode)
                    .setActivated(magnificationController.isActivated(i, 1))
                    .setScale(fullScreenMagnificationController.getScale(i))
                    .setCenterX(fullScreenMagnificationController.getCenterX(i))
                    .setCenterY(fullScreenMagnificationController.getCenterY(i));
        } else if (controllingMode == 2) {
            MagnificationConnectionManager magnificationConnectionManager =
                    magnificationController.getMagnificationConnectionManager();
            builder.setMode(controllingMode)
                    .setActivated(magnificationController.isActivated(i, 2))
                    .setScale(magnificationConnectionManager.getScale(i))
                    .setCenterX(magnificationConnectionManager.getCenterX(i))
                    .setCenterY(magnificationConnectionManager.getCenterY(i));
        } else {
    ...
    public final void resetAllIfNeeded(int i) {
        int i2;
        FullScreenMagnificationController fullScreenMagnificationController =
                this.mController.getFullScreenMagnificationController();
            ...
            for (int i3 = 0; i3 < fullScreenMagnificationController.mDisplays.size(); i3++) {
                try {
                    fullScreenMagnificationController.resetIfNeeded(
                            fullScreenMagnificationController.mDisplays.keyAt(i3), i);
                } finally {
                }
            }
        }
        ...
        synchronized (magnificationConnectionManager.mLock) {
            for (i2 = 0; i2 < magnificationConnectionManager.mWindowMagnifiers.size(); i2++) {
                try {
                    MagnificationConnectionManager.WindowMagnifier windowMagnifier =
                            (MagnificationConnectionManager.WindowMagnifier)
                                    magnificationConnectionManager.mWindowMagnifiers.valueAt(i2);
                    if (windowMagnifier != null
                            && windowMagnifier.mEnabled
                            && i == windowMagnifier.mIdOfLastServiceToControl) {
    ...
    public final boolean setMagnificationConfig(
        ...
        if (magnificationConfig.getMode() != 0) {
            if (controllingMode == magnificationConfig.getMode()) {
                ...
                synchronized (magnificationController.mLock) {
                    try {
                        if (((MagnificationController.DisableMagnificationCallback)
                                        magnificationController.mMagnificationEndRunnableSparseArray
                                                .get(i))
                                != null) {}
                    } finally {
                    }
            ...
            synchronized (magnificationController2.mLock) {
                try {
                    ...
                    PointF currentMagnificationCenterLocked =
                            magnificationController2.getCurrentMagnificationCenterLocked(i, mode);
                    PointF pointF =
                            new PointF(
                                    magnificationConfig.getCenterX(),
                                    magnificationConfig.getCenterY());
                    if (currentMagnificationCenterLocked != null) {
                        pointF.set(
                                Float.isNaN(magnificationConfig.getCenterX())
                                        ? currentMagnificationCenterLocked.x
                                        : magnificationConfig.getCenterX(),
                                Float.isNaN(magnificationConfig.getCenterY())
                                        ? currentMagnificationCenterLocked.y
                                        : magnificationConfig.getCenterY());
                    }
                    MagnificationController.DisableMagnificationCallback
                            disableMagnificationCallback =
                                    (MagnificationController.DisableMagnificationCallback)
                                            magnificationController2
                                                    .mMagnificationEndRunnableSparseArray.get(i);
                    if (disableMagnificationCallback != null) {
                        ...
                        MagnificationController.this.setDisableMagnificationCallbackLocked(
                                disableMagnificationCallback.mDisplayId, null);
                    }
                    FullScreenMagnificationController fullScreenMagnificationController =
                            magnificationController2.getFullScreenMagnificationController();
                    MagnificationConnectionManager magnificationConnectionManager =
                            magnificationController2.getMagnificationConnectionManager();
                    float scale =
                            Float.isNaN(magnificationConfig.getScale())
                                    ? mode == 2
                                            ? magnificationController2
                                                    .getFullScreenMagnificationController()
                                                    .getScale(i)
                                            : magnificationController2
                                                    .getMagnificationConnectionManager()
                                                    .getScale(i)
                                    : magnificationConfig.getScale();
                    magnificationController2.setTransitionState(
                            Integer.valueOf(i), Integer.valueOf(mode));
                            ...
                            z
                                    ? new MagnificationAnimationCallback() { // from class:
                                        ...
                                        public final void onResult(boolean z2) {
                                            MagnificationController magnificationController3 =
                                                    MagnificationController.this;
                                            magnificationController3.mAms.changeMagnificationMode(
                                                    i, mode);
                                        }
                                    }
                                    : null;
                    if (mode == 2) {
                        ...
                        fullScreenMagnificationController.reset(
                                i, (MagnificationAnimationCallback) null);
                        if (isActivated) {
        ...
        if (mode2 != 1) {
            ...
            return this.mController
                    ...
                    .disableWindowMagnification(
                            i, false, MagnificationAnimationCallback.STUB_ANIMATION_CALLBACK);
        }
        if (!isActivated2) {
        ...
        return this.mController
                .getFullScreenMagnificationController()
                .setScaleAndCenter(i, scale2, centerX, centerY, z, i2);
    }
