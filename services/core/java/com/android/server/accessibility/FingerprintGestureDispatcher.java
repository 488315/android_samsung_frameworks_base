...
public final class FingerprintGestureDispatcher extends IFingerprintClientActiveCallback.Stub
    ...
    public FingerprintGestureDispatcher(
        ...
        this.mHardwareSupportsGestures =
                resources.getBoolean(R.bool.config_handleVolumeKeysInWindowManager);
        this.mLock = obj;
    }
        ...
        if (this.mHardwareSupportsGestures) {
            synchronized (this.mLock) {
                for (int i = 0; i < ((ArrayList) this.mCapturingClients).size(); i++) {
                    try {
                        ((FingerprintGestureClient) ((ArrayList) this.mCapturingClients).get(i))
