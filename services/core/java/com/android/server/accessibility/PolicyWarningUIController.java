            ...
            ComponentName componentName =
                    (ComponentName)
                            intent.getParcelableExtra(
                                    "android.intent.extra.COMPONENT_NAME", ComponentName.class);
                ...
                if (!PolicyWarningUIController.ACTION_A11Y_SETTINGS.equals(action)) {
                    if (PolicyWarningUIController.ACTION_DISMISS_NOTIFICATION.equals(action)) {
                        ...
                        if (intExtra == this.mCurrentUserId
                                && this.mNotifiedA11yServices.add(componentName)) {
                ...
                if (intExtra == this.mCurrentUserId) {
                    Intent m =
                            BatteryService$$ExternalSyntheticOutline0.m(
                                    268468224, "android.settings.ACCESSIBILITY_DETAILS_SETTINGS");
                    m.putExtra(
                    ...
                    this.mContext.startActivityAsUser(
                            ...
                            ActivityOptions.makeBasic()
                                    .setLaunchDisplayId(this.mContext.getDisplayId())
                                    .toBundle(),
                            UserHandle.of(intExtra));
                    ((StatusBarManager) this.mContext.getSystemService(StatusBarManager.class))
                            .collapsePanels();
                    ...
                    if (componentName
                        ...
                        Bitmap buildScaledBitmap =
                                ImageUtils.buildScaledBitmap(
                                        loadIcon, dimensionPixelSize, dimensionPixelSize);
                        Notification.Builder builder =
                                new Notification.Builder(
                                        this.mContext,
                                        SystemNotificationChannels.ACCESSIBILITY_SECURITY_POLICY);
                        builder.setSmallIcon(R.drawable.ic_battery)
                                .setSmallIcon(R.drawable.scrubber_progress_horizontal_holo_dark)
                                .setColor(
                                        this.mContext
                                                .getResources()
                                                .getColor(
                                                        R.color
                                                                .accessibility_magnification_thumbnail_container_stroke_color))
                                .setCategory("recommendation")
                                .setContentTitle(this.mContext.getString(17043442))
                                .setContentText(this.mContext.getString(17043441, loadLabel))
                                .setStyle(
                                        new Notification.BigTextStyle()
                                                .bigText(
                                                        this.mContext.getString(
                                                                17043441, loadLabel)))
                                .setTicker(this.mContext.getString(17043442))
                        ...
                        this.mNotificationManager.notify(
                                componentName.flattenToShortString(), 1005, builder.build());
                        ((ArrayList) this.mSentA11yServiceNotification).add(componentName);
                        AccessibilityStatsLogUtils.logNonA11yToolServiceWarningReported(
                                componentName.getPackageName(),
                                AccessibilityStatsLogUtils
                                        .ACCESSIBILITY_PRIVACY_WARNING_STATUS_SHOWN,
                                elapsedRealtime);
                        return;
                    }
                }
