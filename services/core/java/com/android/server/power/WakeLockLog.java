...
public final class WakeLockLog {
  ...
  public static final String[] LEVEL_TO_STRING = {
    "unknown", "partial", "full", "screen-dim", "screen-bright", "prox", "doze", "draw"
  };
  ...
  public final void onWakeLockEvent(int i, String str, int i2, int i3) {
    ...
    handleWakeLockEventInternal(
        i,
        tagNameReducer(str),
        i2,
        i == 1 ? translateFlagsFromPowerManager(i3) : 0,
  ...
  public int translateFlagsFromPowerManager(int i) {
    ...
    if (i2 != 1) {
      ...
      if (i2 == 6) {
        ...
        if (i2 != 128) {
          android.util.Slog.w(
              "PowerManagerService.WLLog", "Unsupported lock level for logging, flags: " + i);
          i3 = 0;
        } else {
          i3 = 7;
        }
  ...
  public class EntryByteTranslator {
      ...
      if (i2 != 0) {
        if (i2 != 1) {
          if (i2 == 2) {
            ...
            android.util.Slog.w(
                "PowerManagerService.WLLog", "Type not recognized [" + i2 + "]", new Exception());
          }
        } else if (bArr.length >= 3) {
          int i3 = i & 63;
          logEntry.set(
              (bArr[2] & 255) + j, 1, this.mTagDatabase.getTag(bArr[1] & Byte.MAX_VALUE), i3);
          return logEntry;
        }
    ...
    public int toBytes(LogEntry logEntry, byte[] bArr, long j) {
      ...
      if (i == 1) {
        ...
        bArr[2] =
            (byte) (relativeTime & IDnsResolverUnsolicitedEventListener.DNS_HEALTH_RESULT_TIMEOUT);
        return 3;
      }
      if (i == 2) {
        if (bArr != null && bArr.length >= 2) {
          ...
          bArr[1] =
              (byte)
                  (relativeTime2 & IDnsResolverUnsolicitedEventListener.DNS_HEALTH_RESULT_TIMEOUT);
        }
        return 2;
      }
  ...
  public class TheLog {
    ...
    public TheLog(
        Injector injector, EntryByteTranslator entryByteTranslator, TagDatabase tagDatabase) {
      this.mBuffer = new byte[Math.max(injector.getLogSize(), 10)];
      this.mTranslator = entryByteTranslator;
      this.mTagDatabase = tagDatabase;
      tagDatabase.setCallback(
          new TagDatabase.Callback() { // from class: com.android.server.power.WakeLockLog.TheLog.1
    ...
    public void addEntry(LogEntry logEntry) {
      ...
      if (bytes > 9 || bytes <= 0) {
        android.util.Slog.w(
            "PowerManagerService.WLLog", "Log entry size is out of expected range: " + bytes);
        return;
      }
      if (makeSpace(bytes)) {
      ...
      return new Iterator() { // from class: com.android.server.power.WakeLockLog.TheLog.2
        ...
        public LogEntry next() {
          ...
          LogEntry readEntryAt =
              TheLog.this.readEntryAt(this.mCurrent, this.mCurrentTimeReference, logEntry);
          this.mCurrent =
              (this.mCurrent
                      + TheLog.this.mTranslator.toBytes(readEntryAt, null, TheLog.this.mStartTime))
    ...
    public final void removeTagIndex(int i) {
      ...
      while (i2 != this.mEnd) {
        ...
        if (readEntryAt == null) {
          android.util.Slog.w(
              "PowerManagerService.WLLog", "Entry is unreadable - Unexpected @ " + i2);
          return;
        }
    ...
    public final void removeOldestItem() {
      ...
      this.mStart =
          (this.mStart + this.mTranslator.toBytes(readEntryAt, null, this.mStartTime))
              % this.mBuffer.length;
