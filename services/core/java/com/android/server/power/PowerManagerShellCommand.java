...
public class PowerManagerShellCommand extends ShellCommand {
  ...
  public final int runSetFixedPerformanceModeEnabled() {
    boolean powerModeChecked =
        this.mService.setPowerModeChecked(3, Boolean.parseBoolean(getNextArgRequired()));
    if (!powerModeChecked) {
      PrintWriter errPrintWriter = getErrPrintWriter();
      errPrintWriter.println("Failed to set FIXED_PERFORMANCE mode");
      errPrintWriter.println(
          "This is likely because Power HAL AIDL is not implemented on this device");
    }
  ...
  public final int runListAmbientDisplaySuppressionTokens() {
    ...
    outPrintWriter.println(
        String.format("[%s]", String.join(", ", ambientDisplaySuppressionTokens)));
    return 0;
  }
  ...
  public final int runSetProx() {
    ...
    if ("-d".equals(getNextArg())) {
      ...
      if (parseInt < 0) {
        outPrintWriter.println(
            "Error: Specified displayId (" + nextArg + ") must a non-negative int.");
        return -1;
      }
      i = parseInt;
    }
    ...
    if (wakeLock == null) {
      wakeLock =
          ((PowerManager) this.mContext.getSystemService(PowerManager.class))
              .newWakeLock(32, "PowerManagerShellCommand[" + i + "]", i);
