    ...
    public void handleBatterySaverStateChanged(boolean z, int i) {
        ...
        synchronized (this.mLock) {
            if (!getFullEnabledLocked() && !getAdaptiveEnabledLocked()) {
                objArr = false;
                EventLogTags.writeBatterySaverMode(!this.mFullPreviouslyEnabled ? 1 : 0, !this.mAdaptivePreviouslyEnabled ? 1 : 0, !getFullEnabledLocked() ? 1 : 0, getAdaptiveEnabledLocked() ? 1 : 0, isInteractive ? 1 : 0, objArr == false ? this.mBatterySaverPolicy.toEventLogString() : "", i);
        ...
        if (z) {
            ...
            if (getPowerSaveModeChangedListenerPackage().isPresent()) {
                this.mContext.sendBroadcastAsUser(new Intent("android.os.action.POWER_SAVE_MODE_CHANGED").setPackage((String) getPowerSaveModeChangedListenerPackage().get()).addFlags(285212672), UserHandle.ALL);
            }
            ...
            for (PowerManagerInternal.LowPowerModeListener lowPowerModeListener : lowPowerModeListenerArr) {
                PowerSaveState batterySaverPolicy = this.mBatterySaverPolicy.getBatterySaverPolicy(lowPowerModeListener.getServiceType());
                Slog.m72d("BatterySaverController", "onLowPowerModeChanged: ServiceType: " + serviceTypeToString(lowPowerModeListener.getServiceType()) + ": " + batterySaverPolicy.batterySaverEnabled);
