    ...
    final class BinderService extends IHintManager.Stub {
        ...
        public IHintSession createHintSession(IBinder iBinder, int[] iArr, long j) {
            ...
            try {
                ...
                AppHintSession appHintSession = HintManagerService.this.new AppHintSession(callingUid, threadGroupLeader, iArr, iBinder, halCreateHintSession, j);
        ...
        public void close() {
            synchronized (HintManagerService.this.mLock) {
                ...
                try {
                    ...
                    Slogf.m105w("HintManagerService", "Error during unlinkToDeath, UID %d, PID %d", Integer.valueOf(this.mUid), Integer.valueOf(this.mPid));
                }
                ArrayMap arrayMap = (ArrayMap) HintManagerService.this.mActiveSessions.get(Integer.valueOf(this.mUid));
                if (arrayMap == null) {
                    Slogf.m105w("HintManagerService", "UID %d is not present in active session map", Integer.valueOf(this.mUid));
                    return;
                }
                ArraySet arraySet = (ArraySet) arrayMap.get(this.mToken);
                if (arraySet == null) {
                    Slogf.m105w("HintManagerService", "Token %s is not present in token map", this.mToken.toString());
                    return;
                }
