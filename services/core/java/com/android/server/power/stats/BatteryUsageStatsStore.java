...
public class BatteryUsageStatsStore {
  public static final List BATTERY_USAGE_STATS_QUERY =
      List.of(
  ...
  public BatteryUsageStatsStore(
      Context context, BatteryStatsImpl batteryStatsImpl, File file, Handler handler) {
    this(context, batteryStatsImpl, file, handler, 102400L);
  }

  public BatteryUsageStatsStore(
      Context context, BatteryStatsImpl batteryStatsImpl, File file, Handler handler, long j) {
    ...
    batteryStatsImpl.setBatteryResetListener(
        new BatteryStatsImpl
            .BatteryResetListener() { // from class:
                                      // com.android.server.power.stats.BatteryUsageStatsStore$$ExternalSyntheticLambda0
  ...
  public final void prepareForBatteryStatsReset(int i) {
    ...
    final List batteryUsageStats =
        this.mBatteryUsageStatsProvider.getBatteryUsageStats(BATTERY_USAGE_STATS_QUERY);
    if (batteryUsageStats.isEmpty()) {
      Slog.wtf("BatteryUsageStatsStore", "No battery usage stats generated");
    } else {
      this.mHandler.post(
          new Runnable() { // from class:
                           // com.android.server.power.stats.BatteryUsageStatsStore$$ExternalSyntheticLambda1
  ...
  public final void storeBatteryUsageStats(BatteryUsageStats batteryUsageStats) {
    ...
    try {
      if (!this.mStoreDir.exists() && !this.mStoreDir.mkdirs()) {
        Slog.e(
            "BatteryUsageStatsStore",
            "Could not create a directory for battery usage stats snapshots");
        return;
      }
      try {
        writeXmlFileLocked(
            batteryUsageStats, makeSnapshotFilename(batteryUsageStats.getStatsEndTimestamp()));
      } catch (Exception e) {
      ...
      for (File file : this.mStoreDir.listFiles()) {
        ...
        if (name.endsWith(".bus")) {
          try {
            ...
            Slog.wtf(
                "BatteryUsageStatsStore",
                "Invalid format of BatteryUsageStats snapshot file name: " + name);
          }
        }
      }
      unlockSnapshotDirectory();
    ...
    try {
      ...
      try {
        ...
        return Long.parseLong(
            properties.getProperty("BATTERY_USAGE_STATS_BEFORE_RESET_TIMESTAMP", "0"));
      } catch (Throwable th) {
        if (openRead != null) {
          try {
            openRead.close();
  ...
  public final File makeSnapshotFilename(long j) {
    return new File(
        this.mStoreDir, String.format(Locale.ENGLISH, "%019d", Long.valueOf(j)) + ".bus");
  }
