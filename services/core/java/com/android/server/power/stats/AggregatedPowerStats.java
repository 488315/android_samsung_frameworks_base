...
public final class AggregatedPowerStats {
    ...
    public AggregatedPowerStats(
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            AggregatedPowerStatsConfig.PowerComponent powerComponent =
                    (AggregatedPowerStatsConfig.PowerComponent) arrayList.get(i);
            if (sparseBooleanArray.get(powerComponent.mPowerComponentId, true)) {
                this.mPowerComponentStats.put(
                        powerComponent.mPowerComponentId,
        ...
        PowerComponentAggregatedPowerStats powerComponentAggregatedPowerStats =
                new PowerComponentAggregatedPowerStats(this, powerComponent2);
        powerComponentAggregatedPowerStats.mPowerStatsDescriptor =
                new PowerStats.Descriptor(-1, 0, (SparseArray) null, 0, 0, new PersistableBundle());
    ...
    public final void addClockUpdate(long j, long j2) {
        ...
        Slog.i(
                ...
                "Too many clock updates. Replacing the previous update with "
                        + ((Object) DateFormat.format("yyyy-MM-dd-HH-mm-ss", j2)));
    ...
    public final void addPowerStats(PowerStats powerStats, long j) {
        ...
        if (powerComponentAggregatedPowerStats == null) {
            ...
            if (length == iArr2.length) {
                ...
                if (powerComponentAggregatedPowerStats3.mDeviceStats != null) {
                    ...
                    MultiStateStats multiStateStats =
                            powerComponentAggregatedPowerStats2.mDeviceStats;
                    if (multiStateStats != null) {
                        multiStateStats.mCounter.copyStatesFrom(
                                powerComponentAggregatedPowerStats3.mDeviceStats.mCounter);
                    }
                }
            } else {
                Slog.wtf(
                        "AggregatePowerStats",
                        "State configurations have different lengths: "
                                + powerComponentAggregatedPowerStats3.mDeviceStates.length
                                + " vs "
                                + iArr2.length);
            }
            for (int size = powerComponentAggregatedPowerStats3.mUidStats.size() - 1;
                    size >= 0;
                    size--) {
                int keyAt = powerComponentAggregatedPowerStats3.mUidStats.keyAt(size);
                PowerComponentAggregatedPowerStats.UidStats uidStats =
                        (PowerComponentAggregatedPowerStats.UidStats)
                                powerComponentAggregatedPowerStats3.mUidStats.valueAt(size);
                int[] iArr3 = uidStats.states;
                if (iArr3 != null) {
                    PowerComponentAggregatedPowerStats.UidStats uidStats2 =
                            new PowerComponentAggregatedPowerStats.UidStats();
            ...
            if (powerComponentAggregatedPowerStats2.mProcessor == null) {
                ...
                powerComponentAggregatedPowerStats2.mProcessor =
                        supplier == null
                                ? AggregatedPowerStatsConfig.NO_OP_PROCESSOR
                                : (PowerStatsProcessor) supplier.get();
            }
            powerComponentAggregatedPowerStats2.mProcessor.start(
                    powerComponentAggregatedPowerStats2, j);
            this.mPowerComponentStats.put(i, powerComponentAggregatedPowerStats2);
            powerComponentAggregatedPowerStats = powerComponentAggregatedPowerStats2;
        }
        powerComponentAggregatedPowerStats.mProcessor.addPowerStats(
                powerComponentAggregatedPowerStats, powerStats, j);
    }
    ...
    public final void dump(IndentingPrintWriter indentingPrintWriter) {
        ...
        for (int i2 = 0; i2 < this.mPowerComponentStats.size(); i2++) {
            ((PowerComponentAggregatedPowerStats) this.mPowerComponentStats.valueAt(i2))
                    .dumpDevice(indentingPrintWriter);
        }
        indentingPrintWriter.decreaseIndent();
        HashSet hashSet = new HashSet();
        for (int i3 = 0; i3 < this.mPowerComponentStats.size(); i3++) {
            ((PowerComponentAggregatedPowerStats) this.mPowerComponentStats.valueAt(i3))
                    .collectUids(hashSet);
        }
        ...
        for (Integer num : numArr) {
            ...
            for (int i4 = 0; i4 < this.mPowerComponentStats.size(); i4++) {
                PowerComponentAggregatedPowerStats powerComponentAggregatedPowerStats =
                        (PowerComponentAggregatedPowerStats) this.mPowerComponentStats.valueAt(i4);
                PowerComponentAggregatedPowerStats.UidStats uidStats =
                        (PowerComponentAggregatedPowerStats.UidStats)
                                powerComponentAggregatedPowerStats.mUidStats.get(intValue);
                if (uidStats != null && (multiStateStats = uidStats.stats) != null) {
                    PowerStats.Descriptor descriptor =
                            powerComponentAggregatedPowerStats.mPowerStatsDescriptor;
                    PowerComponentAggregatedPowerStats.dumpMultiStateStats(
    ...
    public final void reset() {
        ...
        for (int i = 0; i < this.mPowerComponentStats.size(); i++) {
            PowerComponentAggregatedPowerStats powerComponentAggregatedPowerStats =
                    (PowerComponentAggregatedPowerStats) this.mPowerComponentStats.valueAt(i);
            ...
            for (int size = powerComponentAggregatedPowerStats.mUidStats.size() - 1;
                    size >= 0;
                    size--) {
                ((PowerComponentAggregatedPowerStats.UidStats)
                                        powerComponentAggregatedPowerStats.mUidStats.valueAt(size))
                                .stats =
                        null;
            }
        }
    ...
    public final void setDeviceState(int i, int i2, long j) {
        for (int i3 = 0; i3 < this.mPowerComponentStats.size(); i3++) {
            ...
            MultiStateStats.States[] statesArr =
                    powerComponentAggregatedPowerStats.mDeviceStateConfig;
            if (statesArr[i].mTracked) {
                ...
                for (int size = powerComponentAggregatedPowerStats.mStateStats.size() - 1;
                        size >= 0;
                        size--) {
                    ((MultiStateStats) powerComponentAggregatedPowerStats.mStateStats.valueAt(size))
                            .setState(i, i2, j);
                }
            }
            String str = statesArr[i].mName;
            MultiStateStats.States[] statesArr2 =
                    powerComponentAggregatedPowerStats.mUidStateConfig;
            ...
            if (findTrackedStateByName != -1 && statesArr2[findTrackedStateByName].mTracked) {
                for (int size2 = powerComponentAggregatedPowerStats.mUidStats.size() - 1;
                        size2 >= 0;
                        size2--) {
                    PowerComponentAggregatedPowerStats.UidStats uidStats =
                            (PowerComponentAggregatedPowerStats.UidStats)
                                    powerComponentAggregatedPowerStats.mUidStats.valueAt(size2);
                    if (uidStats.stats == null) {
    ...
    public final void setUidState(int i, int i2, long j) {
        for (int i3 = 0; i3 < this.mPowerComponentStats.size(); i3++) {
            ...
            if (powerComponentAggregatedPowerStats.mUidStateConfig[2].mTracked) {
                PowerComponentAggregatedPowerStats.UidStats uidStats =
                        powerComponentAggregatedPowerStats.getUidStats(i);
                if (uidStats.stats == null) {
                    powerComponentAggregatedPowerStats.createUidStats(uidStats, j);
                }
