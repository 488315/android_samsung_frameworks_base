...
public class BatteryUsageStatsProvider {
  ...
  public BatteryUsageStatsProvider(
      Context context, BatteryStats batteryStats, BatteryUsageStatsStore batteryUsageStatsStore) {
  ...
  public final List getPowerCalculators() {
    synchronized (this.mLock) {
      if (this.mPowerCalculators == null) {
        ...
        this.mPowerCalculators.add(
            new ScreenPowerCalculator(this.mPowerProfile, this.mStats.hasDisplayPowerReporting()));
  ...
  public List getBatteryUsageStats(List list) {
    ...
    synchronized (this.mStats) {
      ...
      for (int i = 0; i < list.size(); i++) {
        arrayList.add(
            getBatteryUsageStats((BatteryUsageStatsQuery) list.get(i), currentTimeMillis));
      }
    }
    return arrayList;
  }
  ...
  public final BatteryUsageStats getBatteryUsageStats(
      BatteryUsageStatsQuery batteryUsageStatsQuery, long j) {
  ...
  public final BatteryUsageStats getCurrentBatteryUsageStats(
      BatteryUsageStatsQuery batteryUsageStatsQuery, long j) {
    int i;
    int i2;
    int[] iArr;
    ...
    boolean z2 =
        (batteryUsageStatsQuery.getFlags() & 8) != 0 && this.mStats.isProcessStateDataAvailable();
    boolean z3 = (batteryUsageStatsQuery.getFlags() & 16) != 0;
    BatteryUsageStats.Builder builder2 =
        new BatteryUsageStats.Builder(this.mStats.getCustomEnergyConsumerNames(), z, z2);
    ...
    for (int size = uidStats.size() - 1; size >= 0; size--) {
      ...
      if (z3 || uid.getUid() != 1090) {
        builder2
            .getOrCreateUidBatteryConsumerBuilder(uid)
            .setTimeInStateMs(1, getProcessBackgroundTimeMs(uid, elapsedRealtime))
            .setTimeInStateMs(0, getProcessForegroundTimeMs(uid, elapsedRealtime));
      }
    }
    ...
    while (i5 < size2) {
      ...
      powerCalculator.calculate(
          builder2, this.mStats, elapsedRealtime, uptimeMillis, batteryUsageStatsQuery);
      i5 = i2 + 1;
      builder2 = builder;
      size2 = i;
      powerComponents = iArr;
        ...
        if (d > 2.0d + consumedPower) {
          String str =
              "Sum of states exceeds total. UID = "
                  ...
                  + " "
                  + BatteryConsumer.powerComponentIdToString(i2)
                  + " total = "
                  + consumedPower
                  + " states = "
                  + d;
  ...
  public final long getProcessForegroundTimeMs(BatteryStats.Uid uid, long j) {
    ...
    return (Math.min(
                ...
                foregroundActivityTimer != null
                    ? foregroundActivityTimer.getTotalTimeLocked(j, 0)
                    : 0L)
            + uid.getProcessStateTime(2, j, 0))
        / 1000;
  }
  ...
  public final BatteryUsageStats getAggregatedBatteryUsageStats(
    ...
    BatteryUsageStats.Builder builder =
        new BatteryUsageStats.Builder(customEnergyConsumerNames, z, z2);
    ...
    for (long j : batteryUsageStatsStore.listBatteryUsageStatsTimestamps()) {
      if (j > batteryUsageStatsQuery.getFromTimestamp()
          && j <= batteryUsageStatsQuery.getToTimestamp()
          && (loadBatteryUsageStats = this.mBatteryUsageStatsStore.loadBatteryUsageStats(j))
              != null) {
        if (!Arrays.equals(
            loadBatteryUsageStats.getCustomPowerComponentNames(), customEnergyConsumerNames)) {
          Log.w(
              "BatteryUsageStatsProv",
              "Ignoring older BatteryUsageStats snapshot, which has different custom power"
                  + " components: "
                  + Arrays.toString(loadBatteryUsageStats.getCustomPowerComponentNames()));
        } else if (z2 && !loadBatteryUsageStats.isProcessStateDataIncluded()) {
          Log.w(
              "BatteryUsageStatsProv",
              "Ignoring older BatteryUsageStats snapshot, which  does not include process state"
                  + " data");
        } else {
