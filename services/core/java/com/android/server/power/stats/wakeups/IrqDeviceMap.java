...
public final class IrqDeviceMap {
            ...
            try {
                try {
                    ...
                    while (true) {
                        ...
                        if (str != null
                                && eventType == 3
                                && xmlResourceParser.getName().equals("device")) {
                            if (arraySet.size() > 0) {
                                this.mSubsystemsForDevice.put(
                                        str, Collections.unmodifiableList(new ArrayList(arraySet)));
                            }
                            arraySet.clear();
                            str = null;
                        }
                        if (str != null
                                && eventType == 2
                                && xmlResourceParser.getName().equals("subsystem")) {
    ...
    public final void dump(IndentingPrintWriter indentingPrintWriter) {
        ...
        indentingPrintWriter.println(
                "Loaded from xml resource: "
                        + (indexOfValue >= 0
                                ? "0x" + Long.toHexString(longSparseArray.keyAt(indexOfValue))
                                : null));
