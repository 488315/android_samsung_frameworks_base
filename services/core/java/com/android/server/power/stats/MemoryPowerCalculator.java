...
public class MemoryPowerCalculator extends PowerCalculator {
  ...
  public MemoryPowerCalculator(PowerProfile powerProfile) {
    ...
    for (int i = 0; i < numElements; i++) {
      this.mPowerEstimators[i] =
          new UsageBasedPowerEstimator(powerProfile.getAveragePower("memory.bandwidths", i));
    }
  }
  ...
  public void calculate(
    ...
    builder
        ...
        .setUsageDurationMillis(13, calculateDuration)
        .setConsumedPower(13, calculatePower(batteryStats, j, 0));
  }
  ...
  public final long calculateDuration(BatteryStats batteryStats, long j, int i) {
    ...
    for (int i2 = 0; i2 < kernelMemoryStats.size(); i2++) {
      ...
      j2 +=
          usageBasedPowerEstimatorArr[i2].calculateDuration(
              (BatteryStats.Timer) kernelMemoryStats.valueAt(i2), j, i);
    }
    return j2;
  }
  ...
  public final double calculatePower(BatteryStats batteryStats, long j, int i) {
    ...
    for (int i2 = 0; i2 < kernelMemoryStats.size(); i2++) {
      ...
      UsageBasedPowerEstimator usageBasedPowerEstimator =
          usageBasedPowerEstimatorArr[(int) kernelMemoryStats.keyAt(i2)];
      d +=
          usageBasedPowerEstimator.calculatePower(
              usageBasedPowerEstimator.calculateDuration(
                  (BatteryStats.Timer) kernelMemoryStats.valueAt(i2), j, i));
    }
    return d;
  }
