...
public class ScreenPowerCalculator extends PowerCalculator {
  ...
  public ScreenPowerCalculator(PowerProfile powerProfile, boolean z) {
    ...
    for (int i = 0; i < numDisplays; i++) {
      this.mScreenOnPowerEstimators[i] =
          new UsageBasedPowerEstimator(
              powerProfile.getAveragePowerForOrdinal("screen.on.display", i));
      this.mScreenFullPowerEstimators[i] =
          new UsageBasedPowerEstimator(
              powerProfile.getAveragePowerForOrdinal("screen.full.display", i));
    }
  ...
  public void calculate(
    ...
    int powerModel =
        PowerCalculator.getPowerModel(screenOnEnergyConsumptionUC, batteryUsageStatsQuery);
    calculateTotalDurationAndPower(
        powerAndDuration2, powerModel, batteryStats, j, 0, screenOnEnergyConsumptionUC);
    ...
    if (this.mHasDisplayPowerReporting) {
      for (int size = uidBatteryConsumerBuilders.size() - 1; size >= 0; size--) {
        UidBatteryConsumer.Builder builder3 =
      ...
      if (powerModel == 2) {
        ...
        while (size2 >= 0) {
          UidBatteryConsumer.Builder builder4 =
              (UidBatteryConsumer.Builder) uidBatteryConsumerBuilders.valueAt(size2);
          ...
          builder4
              .setUsageDurationMillis(0, powerAndDuration3.durationMs)
              .setConsumedPower(0, powerAndDuration3.powerMah, powerModel);
        ...
        builder2
            ...
            .setConsumedPower(0, Math.max(powerAndDuration5.powerMah, d), powerModel)
            .setUsageDurationMillis(0, powerAndDuration5.durationMs);
        builder2
            .getAggregateBatteryConsumerBuilder(1)
            .setConsumedPower(0, d, powerModel)
            .setUsageDurationMillis(0, j4);
      }
    ...
    builder2
        .getAggregateBatteryConsumerBuilder(0)
        .setConsumedPower(0, Math.max(powerAndDuration52.powerMah, d), powerModel)
        .setUsageDurationMillis(0, powerAndDuration52.durationMs);
    builder2
        .getAggregateBatteryConsumerBuilder(1)
        .setConsumedPower(0, d, powerModel)
        .setUsageDurationMillis(0, j4);
  ...
  public final void calculateTotalDurationAndPower(
      PowerAndDuration powerAndDuration,
      int i,
      BatteryStats batteryStats,
      long j,
  ...
  public final void calculateAppUsingEnergyConsumption(
      PowerAndDuration powerAndDuration, BatteryStats.Uid uid, long j) {
    ...
    if (screenOnEnergyConsumptionUC < 0) {
      Slog.wtf(
          ...
          "Screen energy not supported, so calculateApp shouldn't de called");
  ...
  public final double calculateTotalPowerFromBrightness(BatteryStats batteryStats, long j) {
    ...
    for (int i = 0; i < length; i++) {
      long j2 = 1000;
      d +=
          this.mScreenOnPowerEstimators[i].calculatePower(
              batteryStats.getDisplayScreenOnTime(i, j) / 1000);
      int i2 = 0;
      while (i2 < 5) {
        d +=
            (this.mScreenFullPowerEstimators[i].calculatePower(
                        batteryStats.getDisplayScreenBrightnessTime(i, i2, j) / j2)
                    * (i2 + 0.5f))
                / 5.0d;
        i2++;
        j2 = 1000;
  ...
  public final void smearScreenBatteryDrain(
      SparseArray sparseArray, PowerAndDuration powerAndDuration, long j) {
    ...
    if (j3 >= 600000) {
      ...
      while (size2 >= 0) {
        UidBatteryConsumer.Builder builder2 =
            (UidBatteryConsumer.Builder) sparseArray.valueAt(size2);
  ...
  public long getProcessForegroundTimeMs(BatteryStats.Uid uid, long j) {
    return Math.min(
            0 + uid.getProcessStateTime(new int[] {0}[0], j, 0),
            getForegroundActivityTotalTimeUs(uid, j))
        / 1000;
  }
