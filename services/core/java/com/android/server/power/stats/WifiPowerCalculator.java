...
public class WifiPowerCalculator extends PowerCalculator {
  ...
  public WifiPowerCalculator(PowerProfile powerProfile) {
    ...
    this.mHasWifiPowerController =
        usageBasedPowerEstimator.isSupported()
            && usageBasedPowerEstimator2.isSupported()
            && usageBasedPowerEstimator3.isSupported();
  }
  ...
  public void calculate(
    ...
    while (size >= 0) {
      ...
      int powerModel =
          PowerCalculator.getPowerModel(wifiEnergyConsumptionUC, batteryUsageStatsQuery2);
      double d2 = d;
      int i = size;
      calculateApp(
    ...
    int powerModel2 =
        PowerCalculator.getPowerModel(wifiEnergyConsumptionUC2, batteryUsageStatsQuery);
    calculateRemaining(
        powerDurationAndTraffic,
        powerModel2,
        batteryStats,
        ...
        j3,
        d3,
        wifiEnergyConsumptionUC2);
    builder
        .getAggregateBatteryConsumerBuilder(0)
        .setUsageDurationMillis(11, powerDurationAndTraffic.durationMs)
        .setConsumedPower(11, d3 + powerDurationAndTraffic.powerMah, powerModel2);
  ...
  public final void calculateApp(
    ...
    if (z && this.mHasWifiPowerController) {
      ...
      if (wifiControllerActivity != null) {
        ...
        if (i == 1) {
          powerDurationAndTraffic.powerMah =
              calcPowerFromControllerDataMah(countLocked, countLocked2, countLocked3);
        } else {
        ...
        while (true) {
          ...
          if (i4 != 0) {
            if (i == 1) {
              powerDurationAndTraffic.powerPerKeyMah[i3] =
              ...
              powerDurationAndTraffic.powerPerKeyMah[i3] =
                  PowerCalculator.uCtoMah(uid.getWifiEnergyConsumptionUC(i4));
            }
          }
          i3++;
        }
      ...
      if (i == 1) {
        ...
        powerDurationAndTraffic.powerMah =
            calcPowerWithoutControllerDataMah(
  ...
  public final void calculateRemaining(
      PowerDurationAndTraffic powerDurationAndTraffic,
      int i,
      BatteryStats batteryStats,
      long j,
    ...
    if (z && this.mHasWifiPowerController) {
      ...
      if (i == 1) {
        double countLocked4 =
            wifiControllerActivity.getPowerCounter().getCountLocked(i2) / 3600000.0d;
        uCtoMah =
            countLocked4 == 0.0d
                ? calcPowerFromControllerDataMah(countLocked3, countLocked2, countLocked)
                : countLocked4;
      }
    } else {
  ...
  public double calcPowerFromControllerDataMah(long j, long j2, long j3) {
    return this.mRxPowerEstimator.calculatePower(j)
        + this.mTxPowerEstimator.calculatePower(j2)
        + this.mIdlePowerEstimator.calculatePower(j3);
  }

  public double calcPowerWithoutControllerDataMah(long j, long j2, long j3, long j4, long j5) {
    return ((j + j2) * this.mWifiPowerPerPacket)
        + this.mPowerOnPowerEstimator.calculatePower(j3)
        + this.mScanPowerEstimator.calculatePower(j4)
        + this.mBatchScanPowerEstimator.calculatePower(j5);
  }
