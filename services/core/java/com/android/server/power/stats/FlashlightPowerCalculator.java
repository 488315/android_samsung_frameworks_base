...
public final class FlashlightPowerCalculator extends PowerCalculator {
    ...
    public FlashlightPowerCalculator(PowerProfile powerProfile) {
        this.mPowerEstimator =
                new UsageBasedPowerEstimator(powerProfile.getAveragePower("camera.flashlight"));
    }
    ...
    public final void calculate(
        ...
        for (int size = uidBatteryConsumerBuilders.size() - 1; size >= 0; size--) {
            ...
            BatteryStats.Timer flashlightTurnedOnTimer =
                    builder2.getBatteryStatsUid().getFlashlightTurnedOnTimer();
            ...
            long calculateDuration =
                    UsageBasedPowerEstimator.calculateDuration(flashlightTurnedOnTimer, j);
            builder2.setUsageDurationMillis(6, calculateDuration)
                    .setConsumedPower(
                            6, usageBasedPowerEstimator.mAveragePowerMahPerMs * calculateDuration);
        }
        ...
        builder.getAggregateBatteryConsumerBuilder(0)
                .setUsageDurationMillis(6, flashlightOnTime)
                .setConsumedPower(6, d);
