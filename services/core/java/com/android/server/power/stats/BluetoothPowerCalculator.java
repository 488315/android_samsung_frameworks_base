...
public class BluetoothPowerCalculator extends PowerCalculator {
  ...
  public BluetoothPowerCalculator(PowerProfile powerProfile) {
    ...
    this.mHasBluetoothPowerController =
        (averagePower == 0.0d || averagePower2 == 0.0d || averagePower3 == 0.0d) ? false : true;
  }
  ...
  public void calculate(
    ...
    if (batteryStats.hasBluetoothActivityReporting()) {
      ...
      int powerModel =
          PowerCalculator.getPowerModel(bluetoothEnergyConsumptionUC, batteryUsageStatsQuery);
      ...
      builder
          ...
          .setConsumedPower(
              2, Math.max(powerAndDuration.powerMah, powerAndDuration.totalPowerMah), powerModel);
      builder
          .getAggregateBatteryConsumerBuilder(1)
          .setUsageDurationMillis(2, powerAndDuration.totalDurationMs)
          .setConsumedPower(2, powerAndDuration.totalPowerMah, powerModel);
    }
  }
  ...
  public final void calculateApp(
    ...
    long bluetoothEnergyConsumptionUC =
        builder.getBatteryStatsUid().getBluetoothEnergyConsumptionUC();
    int powerModel =
        PowerCalculator.getPowerModel(bluetoothEnergyConsumptionUC, batteryUsageStatsQuery);
    calculatePowerAndDuration(
        builder.getBatteryStatsUid(),
        powerModel,
        bluetoothEnergyConsumptionUC,
        builder.getBatteryStatsUid().getBluetoothControllerActivity(),
        batteryUsageStatsQuery.shouldForceUsePowerProfileModel(),
        powerAndDuration);
    builder
        .setUsageDurationMillis(2, powerAndDuration.durationMs)
    ...
    if (i == 2) {
      ...
      while (true) {
        ...
        if (i3 != 0) {
          powerAndDuration.powerPerKeyMah[i2] =
              PowerCalculator.uCtoMah(uid.getBluetoothEnergyConsumptionUC(i3));
        }
        i2++;
      }
    } else {
      ...
      if (this.mHasBluetoothPowerController) {
        ...
        while (true) {
          ...
          if (i5 != 0) {
            powerAndDuration.powerPerKeyMah[i4] =
                calculatePowerMah(
                    rxTimeCounter.getCountForProcessState(i5),
                    longCounter.getCountForProcessState(i5),
                    idleTimeCounter.getCountForProcessState(i5));
          }
          i4++;
        }
      } else {
