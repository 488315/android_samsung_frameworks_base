...
public class SystemServicePowerCalculator extends PowerCalculator {
  ...
  public SystemServicePowerCalculator(PowerProfile powerProfile) {
    ...
    for (int i4 = 0; i4 < numCpuClusters; i4++) {
      ...
      while (i5 < numSpeedStepsInCpuCluster) {
        this.mPowerEstimators[i3] =
            new UsageBasedPowerEstimator(powerProfile.getAveragePowerForCpuCore(i4, i5));
        i5++;
        i3++;
      }
    }
  ...
  public void calculate(
    ...
    if (powerModel == 2) {
      calculatePowerUsingPowerProfile =
          calculatePowerUsingEnergyConsumption(batteryStats, uid, cpuEnergyConsumptionUC);
    } else {
  ...
  public final double calculatePowerUsingEnergyConsumption(
      BatteryStats batteryStats, BatteryStats.Uid uid, long j) {
    double calculatePowerUsingPowerProfile = calculatePowerUsingPowerProfile(batteryStats);
    double calculateUidModeledPowerMah =
        this.mCpuPowerCalculator.calculateUidModeledPowerMah(uid, 0);
    if (calculateUidModeledPowerMah > 0.0d) {
      return (PowerCalculator.uCtoMah(j) * calculatePowerUsingPowerProfile)
