...
public class MobileRadioPowerCalculator extends PowerCalculator {
  ...
  public MobileRadioPowerCalculator(PowerProfile powerProfile) {
    ...
    if (!Double.isNaN(powerProfile.getAveragePowerOrDefault("radio.on", Double.NaN))) {
      for (int i2 = 0; i2 < NUM_SIGNAL_STRENGTH_LEVELS; i2++) {
        this.mIdlePowerEstimators[i2] =
            new UsageBasedPowerEstimator(powerProfile.getAveragePower("radio.on", i2));
      }
    } else {
      ...
      for (int i3 = 1; i3 < NUM_SIGNAL_STRENGTH_LEVELS; i3++) {
        this.mIdlePowerEstimators[i3] =
            new UsageBasedPowerEstimator(Math.max(1.0d, averagePower2 / 256.0d));
      }
    }
    this.mScanPowerEstimator =
        new UsageBasedPowerEstimator(powerProfile.getAveragePowerOrDefault("radio.scanning", 0.0d));
  }
  ...
  */
  public void calculate(
    ...
    int powerModel =
        PowerCalculator.getPowerModel(mobileRadioEnergyConsumptionUC, batteryUsageStatsQuery);
    if (powerModel == 2) {
      longArrayQueue = null;
    ...
    while (size >= 0) {
      ...
      if (keyArr3 == UNINITIALIZED_KEYS) {
        if (batteryUsageStatsQuery.isProcessStateDataNeeded()) {
          ...
          if (powerModel != 2) {
            long mobileRadioEnergyConsumptionUC2 =
                batteryStatsUid.getMobileRadioEnergyConsumptionUC();
                ...
                while (i3 < length) {
                  ...
                  if (i4 == 0) {
                    ...
                    builder2.setConsumedPower(
                        key,
                        PowerCalculator.uCtoMah(
                            batteryStatsUid.getMobileRadioEnergyConsumptionUC(i4)),
                        powerModel);
                  }
                  i3++;
        ...
        if (!batteryUsageStatsQuery.isProcessStateDataNeeded() || keyArr3 == null) {
          ...
          while (i7 < length2) {
            ...
            if (i9 == 0) {
              ...
              j3 = mobileRadioActiveTime;
              long mobileRadioActiveTimeInProcessState =
                  batteryStatsUid2.getMobileRadioActiveTimeInProcessState(i9) / 1000;
              uid = batteryStatsUid2;
              powerAndDuration = powerAndDuration2;
              double d6 = j7;
              builder3.setConsumedPower(
                  key2,
                  d6 == 0.0d ? 0.0d : (mobileRadioActiveTimeInProcessState * d4) / d6,
                  powerModel);
            }
            i7++;
    ...
    if (powerModel == 2) {
      ...
      if (modemControllerActivity != null) {
        i = 0;
        d =
            calcInactiveStatePowerMah(
                modemControllerActivity.getSleepTimeCounter().getCountLocked(0),
      ...
      if (Double.isNaN(d)) {
        double calcScanTimePowerMah =
            calcScanTimePowerMah(batteryStats.getPhoneSignalScanningTime(j, i) / 1000);
        int i10 = i;
        while (i10 < NUM_SIGNAL_STRENGTH_LEVELS) {
          calcScanTimePowerMah +=
              calcIdlePowerAtSignalStrengthMah(
                  batteryStats.getPhoneSignalStrengthTime(i10, j, i) / 1000, i10);
          i10++;
          i = 0;
        }
    ...
    builder
        .getAggregateBatteryConsumerBuilder(0)
        .setUsageDurationMillis(
            8, powerAndDuration3.remainingDurationMs + powerAndDuration3.totalAppDurationMs)
        .setConsumedPower(
            8,
            powerAndDuration3.remainingPowerMah + powerAndDuration3.totalAppPowerMah,
            powerModel);
    builder
        .getAggregateBatteryConsumerBuilder(1)
        .setUsageDurationMillis(8, powerAndDuration3.totalAppDurationMs)
        .setConsumedPower(8, powerAndDuration3.totalAppPowerMah, powerModel);
  }
  ...
  public double calcRxStatePowerMah(int i, int i2, long j) {
    ...
    double averageBatteryDrainOrDefaultMa =
        this.mPowerProfile.getAverageBatteryDrainOrDefaultMa(buildModemPowerProfileKey, Double.NaN);
    if (!Double.isNaN(averageBatteryDrainOrDefaultMa)) {
      return (averageBatteryDrainOrDefaultMa * j) / 3600000.0d;
    }
    Log.w(
        "MobRadioPowerCalculator",
        "Unavailable Power Profile constant for key 0x"
            + Long.toHexString(buildModemPowerProfileKey));
    return Double.NaN;
  }
