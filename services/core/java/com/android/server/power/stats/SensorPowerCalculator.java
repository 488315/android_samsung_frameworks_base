...
public class SensorPowerCalculator extends PowerCalculator {
  ...
  public final double calculateApp(
      UidBatteryConsumer.Builder builder, BatteryStats.Uid uid, long j) {
    double calculatePowerMah = calculatePowerMah(uid, j, 0);
    builder
        .setUsageDurationMillis(9, calculateDuration(uid, j, 0))
        .setConsumedPower(9, calculatePowerMah);
  ...
  public final long calculateDuration(BatteryStats.Uid uid, long j, int i) {
    ...
    for (int i2 = 0; i2 < size; i2++) {
      if (sensorStats.keyAt(i2) != -10000) {
        j2 +=
            ((BatteryStats.Uid.Sensor) sensorStats.valueAt(i2))
                    .getSensorTime()
                    .getTotalTimeLocked(j, i)
                / 1000;
      }
    }
    return j2;
  ...
  public final double calculatePowerMah(BatteryStats.Uid uid, long j, int i) {
    ...
    for (int i2 = 0; i2 < size; i2++) {
      ...
      if (keyAt != -10000) {
        if (((BatteryStats.Uid.Sensor) sensorStats.valueAt(i2))
                        .getSensorTime()
                        .getTotalTimeLocked(j, i)
                    / 1000
                != 0
            && (sensor = (Sensor) this.mSensors.get(keyAt)) != null) {
