...
public class SemBatteryUsageStatsProvider {
  ...
  public SemBatterySipper getBatteryUsageStats() {
    ...
    synchronized (this.mStats) {
      BatteryUsageStats batteryUsageStats =
          new BatteryUsageStatsProvider(this.mContext, this.mStats)
      ...
      if (batteryUsageStats != null) {
        updateBatteryUsage(
            batteryUsageStats, elapsedRealtime, uptimeMillis, semDevicePowerInfo, arrayList);
    ...
    for (int size = uidStats.size() - 1; size >= 0; size--) {
      ...
      if (uid != null && hashMap.containsKey(Integer.valueOf(uid.getUid()))) {
        arrayList.add(
            processAppUsage(
                uid,
                (UidBatteryConsumer) hashMap.get(Integer.valueOf(uid.getUid())),
                semDevicePowerInfo,
                j));
      }
    }
  ...
  public final SemUidPowerInfo processAppUsage(
    ...
    long processStateTime2 =
        processStateTime
            + ((uid.getProcessStateTime(1, j, 0) + uid.getProcessStateTime(2, j, 0)) / 1000);
    long processStateTime3 =
        ((uid.getProcessStateTime(3, j, 0) + uid.getProcessStateTime(4, j, 0))
    ...
    semUidPowerInfo.smearedPower =
        semUidPowerInfo.shouldHide ? 0.0d : uidBatteryConsumer.getConsumedPower();
    semUidPowerInfo.power =
        Math.max(0.0d, uidBatteryConsumer.getConsumedPower() - semUidPowerInfo.screenPower);
    ...
    semUidPowerInfo.mobileData =
        uid.getNetworkActivityBytes(0, 0) + uid.getNetworkActivityBytes(1, 0);
    semUidPowerInfo.mobilePackets =
        uid.getNetworkActivityPackets(0, 0) + uid.getNetworkActivityPackets(1, 0);
    semUidPowerInfo.wifiPackets =
        uid.getNetworkActivityPackets(2, 0) + uid.getNetworkActivityPackets(3, 0);
    semUidPowerInfo.wifiData =
        uid.getNetworkActivityBytes(2, 0) + uid.getNetworkActivityBytes(3, 0);
    ...
    while (i4 < this.mStats.getDisplayCount()) {
      semUidPowerInfo.displayTopActivityTime[i4] =
          (uid.getDisplayTopActivityTime(i4, j, i3) + 500) / 1000;
      i4++;
      i3 = 0;
    }
  ...
  public final void updateUidGpsDuration(
      BatteryStats.Uid uid, SemUidPowerInfo semUidPowerInfo, long j) {
  ...
  public final long calculateWakelockTime(BatteryStats.Uid uid, long j, int i) {
    ...
    for (int i2 = 0; i2 < size; i2++) {
      BatteryStats.Timer wakeTime =
          ((BatteryStats.Uid.Wakelock) wakelockStats.valueAt(i2)).getWakeTime(0);
      if (wakeTime != null) {
  ...
  public final void processDeviceUsage(
      BatteryUsageStats batteryUsageStats, SemDevicePowerInfo semDevicePowerInfo, long j, long j2) {
    ...
    semDevicePowerInfo.screenOnTime =
        (this.mStats.computeBatteryRealtime(j, 0) / 1000) - semDevicePowerInfo.screenOffTime;
    ...
    semDevicePowerInfo.screenOffDischarge =
        this.mStats.getDischargeAmountScreenOffSinceChargePermil();
    semDevicePowerInfo.screenOnDischarge =
        this.mStats.getDischargeAmountScreenOnSinceChargePermil();
    semDevicePowerInfo.subScreenOffDischarge =
        this.mStats.getDischargeAmountScreenOffSinceChargePermil();
    semDevicePowerInfo.subScreenOnDischarge =
    ...
    semDevicePowerInfo.screenOffCoulombCounter =
        this.mStats.getDischargeAmountScreenOffSinceChargeCoulombCounter();
    semDevicePowerInfo.screenOnCoulombCounter =
  ...
  public final void processScreenUsage(SemDevicePowerInfo semDevicePowerInfo, long j) {
    for (int i = 0; i < 5; i++) {
      semDevicePowerInfo.screenBrightnessTime[i] =
          this.mStats.getScreenBrightnessTime(i, j, 0) / 1000;
      semDevicePowerInfo.screenAutoBrightnessTime[i] =
          this.mStats.getScreenAutoBrightnessTime(i, j, 0) / 1000;
      semDevicePowerInfo.subScreenBrightnessTime[i] =
          this.mStats.getSubScreenBrightnessTime(i, j, 0) / 1000;
      semDevicePowerInfo.subScreenAutoBrightnessTime[i] =
          this.mStats.getSubScreenAutoBrightnessTime(i, j, 0) / 1000;
    }
    semDevicePowerInfo.screenHighBrightnessTime =
        this.mStats.getScreenHighBrightnessTime(j, 0) / 1000;
    semDevicePowerInfo.subScreenHighBrightnessTime =
  ...
  public final void processRadioUsage(SemDevicePowerInfo semDevicePowerInfo, long j) {
    ...
    for (int i = 0; i < numSignalStrengthLevels; i++) {
      semDevicePowerInfo.signalStrengthTime[i] =
          this.mStats.getPhoneSignalStrengthTime(i, j, 0) / 1000;
    }
  ...
  public final void processBluetoothUsage(SemDevicePowerInfo semDevicePowerInfo, long j) {
    ...
    semDevicePowerInfo.btTotalBytes =
        this.mStats.getNetworkActivityBytes(4, 0) + this.mStats.getNetworkActivityBytes(5, 0);
  }

  public final void processNetworkUsage(SemDevicePowerInfo semDevicePowerInfo) {
    semDevicePowerInfo.mobileTotalBytes =
        this.mStats.getNetworkActivityBytes(0, 0) + this.mStats.getNetworkActivityBytes(1, 0);
    semDevicePowerInfo.wifiTotalBytes =
        this.mStats.getNetworkActivityBytes(2, 0) + this.mStats.getNetworkActivityBytes(3, 0);
    semDevicePowerInfo.mobileTotalPackets =
        this.mStats.getNetworkActivityPackets(0, 0) + this.mStats.getNetworkActivityPackets(1, 0);
    semDevicePowerInfo.wifiTotalPackets =
        this.mStats.getNetworkActivityPackets(2, 0) + this.mStats.getNetworkActivityPackets(3, 0);
  }
  ...
  public final void processModemUsage(SemDevicePowerInfo semDevicePowerInfo) {
    ...
    semDevicePowerInfo.nrRxTime =
        networkModemControllerActivity
            .getNrModemActivityInfo()
            .getRxTimeCounter()
            .getCountLocked(0);
    semDevicePowerInfo.nrTxByte =
        networkModemControllerActivity
            .getNrModemActivityInfo()
            .getTxByteCounter()
            .getCountLocked(0);
    semDevicePowerInfo.nrRxByte =
        networkModemControllerActivity
            .getNrModemActivityInfo()
            .getRxByteCounter()
            .getCountLocked(0);
    ...
    semDevicePowerInfo.lcRxTime =
        networkModemControllerActivity
            .getLcModemActivityInfo()
            .getRxTimeCounter()
            .getCountLocked(0);
    semDevicePowerInfo.lcTxByte =
        networkModemControllerActivity
            .getLcModemActivityInfo()
            .getTxByteCounter()
            .getCountLocked(0);
    semDevicePowerInfo.lcRxByte =
        networkModemControllerActivity
            .getLcModemActivityInfo()
            .getRxByteCounter()
            .getCountLocked(0);
  ...
  public final int getWakeupAlarmCount(BatteryStats.Uid uid, int i) {
    ...
    for (int size = packageStats.size() - 1; size >= 0; size--) {
      ArrayMap wakeupAlarmStats =
          ((BatteryStats.Uid.Pkg) packageStats.valueAt(size)).getWakeupAlarmStats();
    ...
    if (wakeupReasonStats.size() > 0) {
      for (Map.Entry entry : wakeupReasonStats.entrySet()) {
        if (entry != null && (timer = (BatteryStats.Timer) entry.getValue()) != null) {
          ...
          long computeWakeupReasonTime =
              computeWakeupReasonTime(timer, SystemClock.elapsedRealtime() * 1000, 0);
          ...
          if (makeWakeupReasonListName != null && countLocked > 0 && computeWakeupReasonTime != 0) {
            arrayList.add(
                new SemWakeupReasonInfo(
                    makeWakeupReasonListName, countLocked, computeWakeupReasonTime));
          }
        }
      }
  ...
  public final String makeWakeupReasonListName(String str) {
    if (!str.contains("Abort:")
        || str.contains("Some devices failed to suspend, or early wake event detected")) {
      return null;
    }
    ...
    for (int i = 0; i < 2; i++) {
      if (str.contains(strArr[i])) {
        Slog.i(
            "SemBatteryUsageStatsProvider",
            "makeWakeupReasonListName WhiteList Tag/tableName=" + str);
        return null;
      }
    }
    return str;
    ...
    if (kernelWakelockStats != null && kernelWakelockStats.size() > 0) {
      ...
      while (it2.hasNext()) {
        ...
        if (this.mLastKWakelockMap.containsKey(tag)) {
          semKernelWakelockInfo2.calculateDelta(
              (SemKernelWakelockInfo) this.mLastKWakelockMap.get(tag));
  ...
  public final int getBluetoothScanCount(BatteryStats.Uid uid, long j, int i) {
    ...
    if (bluetoothScanTimer == null
        || (bluetoothScanTimer.getTotalTimeLocked(j, i) + 500) / 1000 == 0) {
      return 0;
    }
