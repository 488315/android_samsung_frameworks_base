...
public class GnssPowerCalculator extends PowerCalculator {
  ...
  public void calculate(
    ...
    while (size >= 0) {
      ...
      d =
          !builder2.isVirtualUid()
              ? d2
                  + calculateApp(
                      builder2,
                      builder2.getBatteryStatsUid(),
                      PowerCalculator.getPowerModel(
                          gnssEnergyConsumptionUC, batteryUsageStatsQuery),
                      j,
                      averageGnssPower,
                      gnssEnergyConsumptionUC)
              : d2;
      size = i - 1;
    ...
    builder
        ...
        .setConsumedPower(
            10,
            powerModel == 2 ? PowerCalculator.uCtoMah(gnssEnergyConsumptionUC2) : d,
            powerModel);
    builder.getAggregateBatteryConsumerBuilder(1).setConsumedPower(10, d, powerModel);
  }

  public final double calculateApp(
      UidBatteryConsumer.Builder builder, BatteryStats.Uid uid, int i, long j, double d, long j2) {
    double uCtoMah;
