...
public class EnergyConsumerSnapshot {
  ...
  public class EnergyConsumerDeltaData {
    ...
    public boolean isEmpty() {
      return this.bluetoothChargeUC <= 0
          && isEmpty(this.cpuClusterChargeUC)
          && isEmpty(this.displayChargeUC)
          && this.gnssChargeUC <= 0
          && this.mobileRadioChargeUC <= 0
          && this.wifiChargeUC <= 0
          && isEmpty(this.otherTotalChargeUC);
    }
  ...
  public EnergyConsumerDeltaData updateAndGetDelta(
    ...
    if (i <= 0) {
      Slog.wtf(
          ...
          "Unexpected battery voltage (" + i + " mV) when taking energy consumer snapshot");
      return null;
    }
    ...
    while (i4 < length) {
      ...
      if (energyConsumer == null) {
        ...
        int i8 = ((i7 + i) + 1) / 2;
        SparseLongArray updateAndGetDeltaForTypeOther =
            updateAndGetDeltaForTypeOther(energyConsumer, energyConsumerAttributionArr, i8);
        if (j2 >= 0 && j != j2) {
          i2 = length;
          i3 = i4;
          long j3 = j - j2;
          if (j3 < 0 || i7 <= 0) {
            ...
            switch (b) {
              ...
              case 2:
                if (energyConsumerDeltaData.cpuClusterChargeUC == null) {
                  energyConsumerDeltaData.cpuClusterChargeUC =
                      new long[this.mNumCpuClusterOrdinals];
                }
              ...
              default:
                ...
                Slog.w(
                    str,
                    "Ignoring consumer " + energyConsumer.name + " of unknown type " + ((int) b));
                continue;
            }
          }
        } else {
  ...
  public final SparseLongArray updateAndGetDeltaForTypeOther(
    ...
    int i2 = 0;
    EnergyConsumerAttribution[] energyConsumerAttributionArr3 =
        energyConsumerAttributionArr == null
            ? new EnergyConsumerAttribution[0]
            : energyConsumerAttributionArr;
    SparseLongArray sparseLongArray2 =
        (SparseLongArray) this.mAttributionSnapshots.get(energyConsumer.f5id, null);
  ...
  public BatteryStats.EnergyConsumerDetails getEnergyConsumerDetails(
      EnergyConsumerDeltaData energyConsumerDeltaData) {
    ...
    while (true) {
      ...
      BatteryStats.EnergyConsumerDetails.EnergyConsumer[] energyConsumerArr =
          energyConsumerDetails.consumers;
  ...
  */
  public final BatteryStats.EnergyConsumerDetails createEnergyConsumerDetails() {
    BatteryStats.EnergyConsumerDetails energyConsumerDetails =
        new BatteryStats.EnergyConsumerDetails();
    energyConsumerDetails.consumers =
        new BatteryStats.EnergyConsumerDetails.EnergyConsumer[this.mEnergyConsumers.size()];
    for (int i = 0; i < this.mEnergyConsumers.size(); i++) {
      EnergyConsumer energyConsumer = (EnergyConsumer) this.mEnergyConsumers.valueAt(i);
      BatteryStats.EnergyConsumerDetails.EnergyConsumer energyConsumer2 =
          new BatteryStats.EnergyConsumerDetails.EnergyConsumer();
      byte b = energyConsumer.type;
      energyConsumer2.type = b;
      energyConsumer2.ordinal = energyConsumer.ordinal;
      switch (b) {
        case 0:
          energyConsumer2.name = sanitizeCustomBucketName(energyConsumer.name);
          break;
        case 1:
          energyConsumer2.name = "BLUETOOTH";
          break;
        case 2:
          energyConsumer2.name = "CPU";
          break;
        case 3:
          energyConsumer2.name = "DISPLAY";
          break;
        case 4:
          energyConsumer2.name = "GNSS";
          break;
        case 5:
          energyConsumer2.name = "MOBILE_RADIO";
          break;
        case 6:
          energyConsumer2.name = "WIFI";
          break;
        default:
          energyConsumer2.name = "UNKNOWN";
          break;
      }
      if (energyConsumer2.type != 0) {
        boolean z = true;
        boolean z2 = energyConsumer2.ordinal != 0;
        if (!z2) {
          for (int i2 = 0; i2 < this.mEnergyConsumers.size(); i2++) {
            EnergyConsumer energyConsumer3 = (EnergyConsumer) this.mEnergyConsumers.valueAt(i2);
            if (energyConsumer3.type == energyConsumer2.type && energyConsumer3.ordinal != 0) {
              if (!z) {
                energyConsumer2.name += "/" + energyConsumer.ordinal;
              }
            }
          }
        }
      ...
      energyConsumerDetails.consumers[i] = energyConsumer2;
    }
