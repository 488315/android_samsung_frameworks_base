...
public class CpuPowerCalculator extends PowerCalculator {
  ...
  public CpuPowerCalculator(PowerProfile powerProfile) {
    ...
    for (int i2 = 0; i2 < this.mNumCpuClusters; i2++) {
      this.mPerClusterPowerEstimators[i2] =
          new UsageBasedPowerEstimator(powerProfile.getAveragePowerForCpuCluster(i2));
    }
    int i3 = 0;
    int i4 = 0;
    while (true) {
    ...
    for (int i6 = 0; i6 < this.mNumCpuClusters; i6++) {
      ...
      this.mPerCpuFreqPowerEstimatorsByCluster[i6] =
          new UsageBasedPowerEstimator[numSpeedStepsInCpuCluster];
      int i7 = 0;
      while (i7 < numSpeedStepsInCpuCluster) {
        UsageBasedPowerEstimator usageBasedPowerEstimator =
            new UsageBasedPowerEstimator(powerProfile.getAveragePowerForCpuCore(i6, i7));
  ...
  public final void calculateApp(
    ...
    builder
        .setConsumedPower(1, result.powerMah, powerModel)
        .setUsageDurationMillis(1, result.durationMs)
  ...
  public final void calculateEnergyConsumptionPerProcessState(
      UidBatteryConsumer.Builder builder, BatteryStats.Uid uid, BatteryConsumer.Key[] keyArr) {
  ...
  public final void calculateModeledPowerPerProcessState(
      UidBatteryConsumer.Builder builder,
      BatteryStats.Uid uid,
      BatteryConsumer.Key[] keyArr,
      Result result) {
    ...
    for (int i = 0; i < 7; i++) {
      int mapUidProcessStateToBatteryConsumerProcessState =
          BatteryStats.mapUidProcessStateToBatteryConsumerProcessState(i);
      if (mapUidProcessStateToBatteryConsumerProcessState != 0
          && uid.getCpuFreqTimes(result.cpuFreqTimes, i)) {
        double[] dArr2 = result.perProcStatePowerMah;
        dArr2[mapUidProcessStateToBatteryConsumerProcessState] =
            dArr2[mapUidProcessStateToBatteryConsumerProcessState]
                + calculateUidModeledPowerMah(uid, 0L, null, result.cpuFreqTimes);
      }
    }
    for (BatteryConsumer.Key key : keyArr) {
      ...
      if (i2 != 0) {
        ...
        builder
            .setConsumedPower(
                key,
                result.perProcStatePowerMah[key.processState]
                    + this.mCpuActivePowerEstimator.calculatePower(cpuActiveTime),
                1)
  ...
  public final void calculatePowerAndDuration(
      BatteryStats.Uid uid, int i, long j, int i2, Result result) {
    double uCtoMah;
    double d;
    int i3 = i2;
  ...
  public double calculateUidModeledPowerMah(BatteryStats.Uid uid, int i) {
    return calculateUidModeledPowerMah(
        uid, uid.getCpuActiveTime(), uid.getCpuClusterTimes(), uid.getCpuFreqTimes(i));
  }
    ...
    if (jArr != null) {
      if (jArr.length == this.mNumCpuClusters) {
        ...
        Log.w(
            ...
            "UID "
                + uid.getUid()
                + " CPU cluster # mismatch: Power Profile # "
                + this.mNumCpuClusters
                + " actual # "
                + jArr.length);
      }
    }
    if (jArr2 != null) {
      if (jArr2.length == this.mPerCpuFreqPowerEstimators.length) {
        for (int i2 = 0; i2 < jArr2.length; i2++) {
          calculateActiveCpuPowerMah +=
              this.mPerCpuFreqPowerEstimators[i2].calculatePower(jArr2[i2]);
        }
      } else {
        Log.w(
            "CpuPowerCalculator",
            "UID "
                + uid.getUid()
                + " CPU freq # mismatch: Power Profile # "
                + this.mPerCpuFreqPowerEstimators.length
                + " actual # "
                + jArr2.length);
      }
    }
