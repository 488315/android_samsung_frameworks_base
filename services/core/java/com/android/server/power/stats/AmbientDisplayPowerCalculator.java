...
public class AmbientDisplayPowerCalculator extends PowerCalculator {
    ...
    for (int i = 0; i < numDisplays; i++) {
      this.mPowerEstimators[i] =
          new UsageBasedPowerEstimator(
              powerProfile.getAveragePowerForOrdinal("ambient.on.display", i));
    }
  }
  ...
  public void calculate(
    ...
    int powerModel =
        PowerCalculator.getPowerModel(screenDozeEnergyConsumptionUC, batteryUsageStatsQuery);
    long calculateDuration = calculateDuration(batteryStats, j, 0);
    builder
        .getAggregateBatteryConsumerBuilder(0)
        .setUsageDurationMillis(15, calculateDuration)
        .setConsumedPower(
            15,
            calculateTotalPower(powerModel, batteryStats, j, screenDozeEnergyConsumptionUC),
            powerModel);
  }
  ...
  public final double calculateEstimatedPower(BatteryStats batteryStats, long j) {
    ...
    for (int i = 0; i < length; i++) {
      d +=
          this.mPowerEstimators[i].calculatePower(
              batteryStats.getDisplayScreenDozeTime(i, j) / 1000);
    }
    return d;
  }
}
