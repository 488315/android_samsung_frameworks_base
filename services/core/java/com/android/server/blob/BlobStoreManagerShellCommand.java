...
public class BlobStoreManagerShellCommand extends ShellCommand {
  ...
  public final int runQueryBlobExistence(PrintWriter printWriter) {
    ...
    printWriter.println(
        this.mService.isBlobAvailable(parsedArgs.blobId, parsedArgs.userId) ? 1 : 0);
    return 0;
  }
  ...
  public void onHelp() {
    ...
    outPrintWriter.println(
        "delete-blob [-u | --user USER_ID] [--digest DIGEST] [--expiry EXPIRY_TIME] [--label LABEL]"
            + " [--tag TAG]");
  ...
  */
  ...
  public class ParsedArgs {
    ...
    public BlobHandle getBlobHandle() {
      return BlobHandle.create(
          this.algorithm, this.digest, this.label, this.expiryTimeMillis, this.tag);
    }
  }
}
