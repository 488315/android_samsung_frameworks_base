...
class BlobStoreSession extends IBlobStoreSession.Stub {
    ...
    synchronized (this.mSessionLock) {
      if (isFinalized()) {
        throw new IllegalStateException(
            "Not allowed to open session with state: " + stateToString(this.mState));
      }
      this.mState = 1;
    }
  }
  ...
  public ParcelFileDescriptor openWrite(long j, long j2) {
    ...
    synchronized (this.mSessionLock) {
      if (this.mState != 1) {
        throw new IllegalStateException(
            "Not allowed to write in state: " + stateToString(this.mState));
      }
    }
    try {
  ...
  public ParcelFileDescriptor openRead() {
    ...
    synchronized (this.mSessionLock) {
      if (this.mState != 1) {
        throw new IllegalStateException(
            "Not allowed to read in state: " + stateToString(this.mState));
      }
  ...
  public void allowPackageAccess(String str, byte[] bArr) {
    ...
    synchronized (this.mSessionLock) {
      if (this.mState != 1) {
        throw new IllegalStateException(
            "Not allowed to change access type in state: " + stateToString(this.mState));
      }
      if (this.mBlobAccessMode.getAllowedPackagesCount()
          >= BlobStoreConfig.getMaxPermittedPackages()) {
        throw new ParcelableException(
            new LimitExceededException(
                "Too many packages permitted to access the blob: "
                    + this.mBlobAccessMode.getAllowedPackagesCount()));
      }
  ...
  public boolean isPackageAccessAllowed(String str, byte[] bArr) {
    ...
    synchronized (this.mSessionLock) {
      if (this.mState != 1) {
        throw new IllegalStateException(
            "Not allowed to get access type in state: " + stateToString(this.mState));
      }
  ...
  public final void closeSession(int i, boolean z) {
    ...
    synchronized (this.mSessionLock) {
      ...
      throw new IllegalStateException(
          "Not allowed to delete or abandon a session with state: " + stateToString(this.mState));
    }
  }
  ...
  public void computeDigest() {
    try {
      try {
        Trace.traceBegin(
            524288L, "computeBlobDigest-i" + this.mSessionId + "-l" + getSessionFile().length());
    ...
    synchronized (this.mSessionLock) {
      ...
      if (bArr != null && Arrays.equals(bArr, this.mBlobHandle.digest)) {
        ...
        FrameworkStatsLog.write(
            FrameworkStatsLog.BLOB_COMMITTED, getOwnerUid(), this.mSessionId, getSize(), 3);
    ...
    revocableFileDescriptor.addOnCloseListener(
        new ParcelFileDescriptor
            .OnCloseListener() { // from class:
                                 // com.android.server.blob.BlobStoreSession$$ExternalSyntheticLambda0
          @Override // android.os.ParcelFileDescriptor.OnCloseListener
          public final void onClose(IOException iOException) {
            BlobStoreSession.this.lambda$trackRevocableFdLocked$0(
                revocableFileDescriptor, iOException);
          }
        });
  }

  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ void lambda$trackRevocableFdLocked$0(
      RevocableFileDescriptor revocableFileDescriptor, IOException iOException) {
    synchronized (this.mRevocableFds) {
  ...
  public void dump(
    ...
    synchronized (this.mSessionLock) {
      ...
      indentingPrintWriter.println(
          "creation time: " + BlobStoreUtils.formatTime(this.mCreationTimeMs));
      indentingPrintWriter.println(
