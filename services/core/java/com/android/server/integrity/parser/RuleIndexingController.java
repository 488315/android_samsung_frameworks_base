...
public class RuleIndexingController {
  ...
  public List identifyRulesToEvaluate(AppInstallMetadata appInstallMetadata) {
    ...
    while (it.hasNext()) {
      arrayList.add(
          searchIndexingKeysRangeContainingKey(sAppCertificateBasedIndexes, (String) it.next()));
    }
    arrayList.add(
        new RuleIndexRange(
  ...
  public final LinkedHashMap getNextIndexGroup(BitInputStream bitInputStream) {
    ...
    while (bitInputStream.hasNext()) {
      ...
      linkedHashMap.put(
          stringValue, Integer.valueOf(BinaryFileOperations.getIntValue(bitInputStream)));
