...
public class WatchlistConfig {
    ...
    if (this.mXmlFile.exists()) {
      try {
        ...
        try {
          ...
          this.mDomainDigests =
              new CrcShaDigests(new HarmfulCrcs(arrayList), new HarmfulDigests(arrayList2));
          this.mIpDigests =
              new CrcShaDigests(new HarmfulCrcs(arrayList3), new HarmfulDigests(arrayList4));
  ...
  public void setTestMode(InputStream inputStream) {
    ...
    FileUtils.copyToFileOrThrow(
        inputStream, new File("/data/misc/network_watchlist/network_watchlist_for_test.xml"));
