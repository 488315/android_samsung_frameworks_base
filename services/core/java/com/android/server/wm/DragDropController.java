...
public class DragDropController {
  ...
  public AtomicReference mCallback =
      new AtomicReference(
          new WindowManagerInternal
              .IDragDropCallback() { // from class: com.android.server.wm.DragDropController.1
          });
  ...
  public IBinder performDrag(
    ...
    return performDragWithArea(
        i, i2, iWindow, i3, surfaceControl, i4, f, f2, f3, f4, clipData, null, null);
  }
  ...
  public IBinder performDragWithArea(
    ...
    boolean prePerformDrag =
        ((WindowManagerInternal.IDragDropCallback) this.mCallback.get())
            .prePerformDrag(iWindow, binder, i4, f, f2, f3, f4, clipData);
    try {
      WindowManagerGlobalLock windowManagerGlobalLock = this.mService.mGlobalLock;
      WindowManagerService.boostPriorityForLockedSection();
      synchronized (windowManagerGlobalLock) {
        try {
          try {
            if (prePerformDrag) {
              try {
                if (!dragDropActiveLocked()) {
                  ...
                  if (windowForClientLocked != null
                      && windowForClientLocked.canReceiveTouchInput()) {
                    ...
                    if (displayContent != null) {
                      if (displayContent.isDefaultDisplay) {
                        ...
                        if ((i3 & 1073741824) != 0
                          ...
                          Slog.d(
                              StartingSurfaceController.TAG,
                              "Not support to launch multi-window mode with drag-and-drop if your"
                                  + " app is in multi-window mode.");
                      ...
                      try {
                        ...
                        if (task2 != null && (i3 & 256) != 0) {
                          Slog.d(
                              StartingSurfaceController.TAG,
                              "[TWODND] Set DragSourceTask=" + task2);
                        ...
                        if (CoreRune.FW_DND_OBJECT_CAPTURE
                            && (4194304 & i3) != 0
                            && rectF != null) {
                          DragState dragState2 = this.mDragState;
                        ...
                        if ((i3 & 1024) != 0) {
                          ...
                          sendTimeoutMessage(
                              0,
                              windowForClientLocked.mClient.asBinder(),
                              getAccessibilityManager().getRecommendedTimeoutMillis(60000, 4));
                          WindowManagerService.resetPriorityAfterLockedSection();
                          return binder;
                        }
                        CompletableFuture registerInputChannel =
                            ((WindowManagerInternal.IDragDropCallback) this.mCallback.get())
                                .registerInputChannel(
                                    this.mDragState,
                                    displayContent.getDisplay(),
                                    this.mService.mInputManager,
                                    windowForClientLocked.mInputChannel);
                        this.mDragState.mCallingPackageName =
                            windowForClientLocked.getOwningPackage();
                        ...
                        try {
                          z =
                              ((Boolean) registerInputChannel.get(5000L, TimeUnit.MILLISECONDS))
                                  .booleanValue();
                        } catch (Exception e) {
                          Slog.e(
                              StartingSurfaceController.TAG,
                              "Exception thrown while waiting for touch focus transfer",
                              e);
                          z = false;
                        }
                        ...
                        synchronized (windowManagerGlobalLock2) {
                          try {
                            if (!z) {
                              Slog.e(
                                  StartingSurfaceController.TAG, "Unable to transfer touch focus");
              ...
              Slog.w(
                  StartingSurfaceController.TAG,
                  "IDragDropCallback rejects the performDrag request");
              if (surfaceControl != null) {
                surfaceControl.release();
              }
            }
        ...
        try {
          ...
          if (dragState == null) {
            ...
            if (windowForClientLocked == null) {
              ...
              dragState2.mRelinquishDragSurfaceToDropTarget =
                  z && dragState2.targetInterceptsGlobalDrag(windowForClientLocked);
              if (z
                  && (windowForClientLocked.isEavesdropDragEvent()
                      || (this.mDragState.mRelinquishDragSurfaceToDropTarget
                          && windowForClientLocked.getWindowType() == 2024))) {
                DragState dragState3 = this.mDragState;
      ...
      synchronized (windowManagerGlobalLock) {
        try {
          ...
          if (dragState.mToken != iBinder) {
            Slog.w(
                StartingSurfaceController.TAG, "cancelDragAndDrop() does not match prepareDrag()");
    ...
    synchronized (windowManagerGlobalLock) {
      try {
        ...
        boolean reportDropWindowLock =
            this.mDragState.reportDropWindowLock(windowForClientLocked.mInputChannelToken, f, f2);
        WindowManagerService.resetPriorityAfterLockedSection();
  ...
  public class DragHandler extends Handler {
    ...
    public void handleMessage(Message message) {
      ...
      if (i == 0) {
        Slog.w(
            StartingSurfaceController.TAG, "Timeout ending drag to win " + ((IBinder) message.obj));
        WindowManagerGlobalLock windowManagerGlobalLock = this.mService.mGlobalLock;
        WindowManagerService.boostPriorityForLockedSection();
        synchronized (windowManagerGlobalLock) {
          try {
        ...
        synchronized (windowManagerGlobalLock4) {
          try {
            ((SurfaceControl.Transaction) this.mService.mTransactionFactory.get())
                .reparent((SurfaceControl) message.obj, null)
                .apply();
          } finally {
          }
        }
  ...
  public void setDragSurfaceToOverlay(boolean z) {
    ...
    if (z) {
      dragState.mTransaction.setRelativeLayer(
          dragState.mSurfaceControl, displayContent.getOverlayLayer(), -1);
      return;
    }
    ...
    DragState dragState2 = this.mDragState;
    dragState2.mTransaction.setRelativeLayer(
        dragState2.mSurfaceControl, windowEavesdropDragEvent.getSurfaceControl(), -1);
  }
