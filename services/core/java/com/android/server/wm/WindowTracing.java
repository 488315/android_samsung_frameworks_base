...
public class WindowTracing {
  ...
  public static WindowTracing createDefaultAndStartLooper(
      WindowManagerService windowManagerService, Choreographer choreographer) {
    return new WindowTracing(
  ...
  public WindowTracing(
      File file, WindowManagerService windowManagerService, Choreographer choreographer, int i) {
  ...
  public void stopTrace(PrintWriter printWriter) {
    ...
    synchronized (this.mEnabledLock) {
      logAndPrintln(
          printWriter, "Stop tracing to " + this.mTraceFile + ". Waiting for traces to flush.");
