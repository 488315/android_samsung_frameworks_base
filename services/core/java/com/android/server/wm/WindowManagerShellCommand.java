...
public class WindowManagerShellCommand extends ShellCommand {
    ...
    */
    public final int runSetLetterboxDefaultPositionForHorizontalReachability(PrintWriter printWriter) {
        char c;
    ...
    */
    ...
    public final int runResetMultiWindowConfig() {
        int integer = this.mInternal.mContext.getResources().getInteger(R.integer.config_undockedHdmiRotation);
        int integer2 = this.mInternal.mContext.getResources().getInteger(R.integer.config_screenBrightnessCapForWearBedtimeMode);
        WindowManagerGlobalLock windowManagerGlobalLock = this.mInternal.mAtmService.mGlobalLock;
        WindowManagerService.boostPriorityForLockedSection();
        synchronized (windowManagerGlobalLock) {
            try {
            ...
            if (nextArg2 != null && (nextArg != null || "reset".equals(nextArg2))) {
                try {
                    if (parseDimension < 0 && parseDimension2 >= 0 && i > 0) {
                        this.mInterface.setForcedDisplaySizeDensityWithInfo(new MultiResolutionChangeRequestInfo.Builder(0).setWidth(parseDimension).setHeight(parseDimension2).setDensity(i).setSaveToSettings(false).setForcedHideCutout(-1).build());
                    } else {
                        this.mInterface.clearForcedDisplaySizeDensity(displayId);
                    }
                } catch (RemoteException unused) {
