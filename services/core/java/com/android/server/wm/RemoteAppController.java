...
public class RemoteAppController implements IController {
  ...
  */
  public boolean notifyStartActivityInterceptedLocked(
      Intent intent,
    ...
    if (neededUriGrants != null) {
      if (activityRecord != null) {
        ...
        synchronized (this.mLock) {
          ...
          while (it.hasNext()) {
            z2 =
                ((RemoteAppControllerCallbacks) it.next())
                    .onStartActivityInterceptedLocked(
                        intent,
                        activityOptions,
                        activityInfo,
                        i,
  ...
  public boolean interceptStartActivityFromRecentsLocked(
      Task task, ActivityOptions activityOptions, int i, String str) {
    ...
    if (rootActivity != null && rootActivity.intent != null && displayContent != null) {
      if (rootActivity.isTaskOverlay()) {
        if (CoreRune.SAFE_DEBUG) {
          Slog.d(
              "RemoteAppController",
              "interceptStartActivityFromRecentsLocked: Can't intercept taskOverlay, r="
                  + rootActivity);
        }
        return false;
      }
      if (rootActivity.isResolverOrDelegateActivity()) {
        if (CoreRune.SAFE_DEBUG) {
          Slog.d(
              "RemoteAppController",
              "interceptStartActivityFromRecentsLocked: Can't intercept Chooser/Resolver, r="
                  + rootActivity);
        }
        return false;
      }
      if (i == 0 && isRemoteAppDisplayLocked(task.getDisplayId())) {
        boolean notifyStartActivityInterceptedLocked =
            notifyStartActivityInterceptedLocked(
                rootActivity.intent,
                activityOptions,
                rootActivity.info,
                i,
        ...
        Slog.d(
            "RemoteAppController",
            "interceptStartActivityFromRecentsLocked: intercepted="
                + notifyStartActivityInterceptedLocked
                + ", reason="
                + str
                + ", r="
                ...
                + task
                + ", intercept_reason="
                + RemoteAppControllerCallbacks.interceptReasonToString(2));
  ...
  */
    ...
    if (activityRecord.intent != null && displayContent != null) {
      if (activityRecord.isTaskOverlay()) {
        if (CoreRune.SAFE_DEBUG) {
          Slog.d(
              "RemoteAppController",
              "interceptStartActivityLocked: Can't intercept taskOverlay, r=" + activityRecord);
        }
        return false;
      }
      if (activityRecord.isResolverOrDelegateActivity()) {
        if (CoreRune.SAFE_DEBUG) {
          Slog.d(
              "RemoteAppController",
              "interceptStartActivityLocked: Can't intercept Chooser/Resolver, r="
                  + activityRecord);
        }
        return false;
      }
      ...
      if (isInterceptRequested(activityRecord.intent)) {
        ...
        if (displayContent.isRemoteAppDisplay()) {
          ...
          i2 = i5;
          i5 = i4;
        } else if (displayContent.getDisplayId() == 0
            && task != null
            && isRemoteAppDisplayLocked(task.getDisplayId())) {
          i3 = 2;
        } else {
          i5 = 0;
          i2 = 0;
        ...
        if (i5 != 0) {
          ...
          Slog.d(
              "RemoteAppController",
              "interceptStartActivityLocked: intercepted="
                  + notifyStartActivityInterceptedLocked
                  + ", reason="
                  + str
                  + ", r="
                  ...
                  + neededUriGrants
                  + ", intercept_reason="
                  + RemoteAppControllerCallbacks.interceptReasonToString(i2));
          return notifyStartActivityInterceptedLocked;
        }
      }
      i2 = i3;
  ...
  public static boolean isValidActivityTypeLocked(ActivityRecord activityRecord) {
    return activityRecord.isActivityTypeStandardOrUndefined()
  ...
  public void startActivityAsCaller(Intent intent, CallerInfo callerInfo, int i, Bundle bundle) {
    this.mAtmService
        ...
        .setCallingUid(callerInfo.launchedFromUid)
        .setCallingPackage(callerInfo.launchedFromPackage)
        .setCallingFeatureId(callerInfo.launchedFromFeatureId)
        .setResolvedType(callerInfo.resolvedType)
        .setRequestCode(-1)
        .setActivityOptions(bundle)
        .setUserId(i)
        .setIgnoreTargetSecurity(false)
        .setFilterCallingUid(callerInfo.isResolver ? 0 : callerInfo.launchedFromUid)
