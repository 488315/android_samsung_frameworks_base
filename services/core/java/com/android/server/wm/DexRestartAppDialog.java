...
public class DexRestartAppDialog extends AlertDialog implements DialogInterface.OnClickListener {
  ...
  public DexRestartAppDialog(
      Context context,
      ActivityTaskManagerService activityTaskManagerService,
      DexController dexController,
      DexActivityStartInterceptor.DexRestartAppDialogController dexRestartAppDialogController,
      DexRestartAppInfo dexRestartAppInfo,
      int i) {
    super(context, 16975230);
    this.mReceiver =
        new BroadcastReceiver() { // from class: com.android.server.wm.DexRestartAppDialog.1
          ...
          public void onReceive(Context context2, Intent intent) {
            if ("android.intent.action.CLOSE_SYSTEM_DIALOGS".equals(intent.getAction())) {
              if (DexRestartAppDialog.this.mTargetDisplayId == intent.getIntExtra("displayId", 0)) {
                Slog.d(
                    "DexRestartAppDialog",
                    "Finish dex restart app dialog reason = " + intent.getStringExtra("reason"));
    ...
    String format =
        String.format(
            context.getString(R.string.fp_power_button_enrollment_title),
            dexRestartAppInfo.getAppName(context, activityTaskManagerService.mTaskSupervisor));
    setTitle(format);
    View inflate =
        ((LayoutInflater) context2.getSystemService("layout_inflater"))
            .inflate(R.layout.floating_popup_container, (ViewGroup) null);
    this.mDoNotShowAgainCheckBox = (CheckBox) inflate.findViewById(R.id.email);
    setView(inflate);
    setButton(-1, context.getText(R.string.fp_power_button_enrollment_button_text), this);
  ...
  public void onStart() {
    ...
    this.mContext.registerReceiver(
        this.mReceiver, new IntentFilter("android.intent.action.CLOSE_SYSTEM_DIALOGS"));
  }
  ...
  public void onClick(DialogInterface dialogInterface, int i) {
    ...
    if (isChecked) {
      Settings.System.putInt(
          this.mAtmService.mContext.getContentResolver(), "display_chooser_do_not_show_again", 1);
    }
    ...
    this.mDexController.scheduleMoveTasksBackAndStartPendingActivity(
        this.mInfo, this.mTargetDisplayId);
    this.mDialogController.setRestartDialogShowRequested(false);
    this.mRun = true;
  }
  ...
  public final void dismissSplitIfNeeded() {
    if (this.mTargetDisplayId == 0) {
      TaskDisplayArea defaultTaskDisplayArea =
          this.mAtmService.mRootWindowContainer.getDefaultTaskDisplayArea();
      Task fromWindowContainerToken =
          Task.fromWindowContainerToken(this.mInfo.mOptions.getLaunchRootTask());
      if (fromWindowContainerToken == null
          || !fromWindowContainerToken.inSplitScreenWindowingMode()
          || fromWindowContainerToken.hasChild()) {
        return;
      }
      if (defaultTaskDisplayArea.isSplitScreenModeActivated()
          || defaultTaskDisplayArea.hasChildTaskInSideStage()) {
        ...
        defaultTaskDisplayArea.onStageSplitScreenDismissed(
            defaultTaskDisplayArea
                .getTopRootTaskInStageType(stageType == 1 ? 2 : 1)
                .getTopMostTask());
      }
    }
  }
