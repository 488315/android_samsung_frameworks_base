...
public class AppPairController implements IController {
  ...
  public void handleAutoPipIfNeededLocked(WindowContainerTransaction windowContainerTransaction) {
    ...
    if (topRootTaskInWindowingMode == null
        || defaultTaskDisplayArea.isSplitScreenModeActivated()
        || (findEnterPipOnTaskSwitchCandidate =
                Task.findEnterPipOnTaskSwitchCandidate(topRootTaskInWindowingMode))
            == null
        || findEnterPipOnTaskSwitchCandidate.supportsEnterPipOnTaskSwitch) {
      return;
    }
    if (hasSamePackageInStartIntentsLocked(
        windowContainerTransaction, findEnterPipOnTaskSwitchCandidate.packageName)) {
      Slog.d(
          "AppPairController",
    ...
    Task.enableEnterPipOnTaskSwitch(
        findEnterPipOnTaskSwitchCandidate, topRootTaskInWindowingMode, null, null);
    Slog.d(
        "AppPairController",
        "handleAutoPipIfNeededLocked: enable autoPip, r=" + findEnterPipOnTaskSwitchCandidate);
    setShouldAutoPipByAppPair(findEnterPipOnTaskSwitchCandidate.supportsEnterPipOnTaskSwitch);
  }

  public final boolean hasSamePackageInStartIntentsLocked(
      WindowContainerTransaction windowContainerTransaction, String str) {
    if (str == null) {
      return false;
    }
