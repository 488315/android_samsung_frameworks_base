...
public class DexActivityStartInterceptor {
  ...
  public final DexRestartAppDialogController mDexRestartAppDialogController =
      new DexRestartAppDialogController();

  public DexActivityStartInterceptor(
      DexController dexController, ActivityTaskManagerService activityTaskManagerService) {
  ...
  public boolean intercept(
    ...
    ActivityRecord activityRecord3 =
        (rootActivity == null || activityRecord.processName.equals(rootActivity.processName))
            ? rootActivity
            : null;
    if (this.mDexController.getDexModeLocked() != 2) {
      ...
      if (activityRecord3.getState() != ActivityRecord.State.STOPPING) {
        this.mDexController.moveTaskToDefaultDisplayAndLayoutTask(
            task, activityRecord3, activityRecord2, activityOptions);
        return false;
      }
      Slog.i(
          "DexActivityStartInterceptor",
          "delay stopping dex activity process kill, " + activityRecord);
    }
    ...
    if (processController == null
        || (processController.getDisplayId() == i2 && !processController.isKeepProcessAlive())) {
      ...
      Slog.w(
          "DexActivityStartInterceptor",
          "Skip intercept " + activityRecord4 + " is system process activity");
      return false;
    }
    if (SAFE_DEBUG) {
      Slog.d(
          "DexActivityStartInterceptor",
          "intercept " + activityRecord + " to d" + i2 + " reusedTask=" + task);
    }
    DexRestartAppInfo createPendingActivityLaunchType =
        DexRestartAppInfo.createPendingActivityLaunchType(
            new PendingActivityLaunch(
                activityRecord,
                activityRecord2,
                i,
                activityRecord.getRootTask(),
                null,
                neededUriGrants),
            str,
            uid,
            activityRecord4.info.applicationInfo,
            task,
            task2,
            activityOptions,
            i2);
    if (task != null) {
      ...
      Iterator it =
          createPendingActivityLaunchType.getMovingTaskLocked(this.mDexController, i2).iterator();
      while (true) {
        if (!it.hasNext()) {
          break;
        }
      ...
      if (task.isVisible() || z) {
        this.mDexController.scheduleMoveTasksBackAndStartPendingActivity(
            createPendingActivityLaunchType, i2);
        return true;
      }
      if (processController != null
          && this.mDexController.shouldKillProcess(processController, i2)
          && !this.mDexController.getTaskLocked(str, uid, true, true, i2).isEmpty()) {
    ...
    if (!this.mDexController.getTaskLocked(str, uid, true, i2).isEmpty()) {
      this.mDexRestartAppDialogController.showRestartAppDialogLocked(
          createPendingActivityLaunchType, i2);
      return true;
    }
  ...
  public boolean interceptStartFromRecents(Task task, ActivityOptions activityOptions) {
    ...
    if (this.mDexController.getDexModeLocked() != 2) {
      ...
      if (rootActivity.getState() != ActivityRecord.State.STOPPING) {
        this.mDexController.moveTaskToDefaultDisplayAndLayoutTask(
            task, rootActivity, null, activityOptions);
        return false;
      }
      Slog.i(
          "DexActivityStartInterceptor",
          "delay stopping dex activity process kill, " + rootActivity);
    }
    WindowProcessController processController =
        this.mAtmService.getProcessController(rootActivity.processName, rootActivity.getUid());
    if (processController == null
        || processController.getDisplayId() == launchDisplayId
        || task.getDisplayId() == launchDisplayId) {
      Slog.i("DexActivityStartInterceptor", "do not need process kill, " + rootActivity);
      return false;
    }
    DexRestartAppInfo createStartActivityFromRecentsType =
        DexRestartAppInfo.createStartActivityFromRecentsType(
            rootActivity.processName,
            rootActivity.getUid(),
            rootActivity.info.applicationInfo,
            task,
            activityOptions,
            launchDisplayId);
    if (processController.isKeepProcessAlive()) {
      ...
      this.mDexController.scheduleMoveTasksBackAndStartPendingActivity(
          createStartActivityFromRecentsType, launchDisplayId);
      return true;
    }
    Iterator it =
        createStartActivityFromRecentsType
            .getMovingTaskLocked(this.mDexController, launchDisplayId)
            .iterator();
    while (true) {
      if (!it.hasNext()) {
        break;
    ...
    if (task.isVisible() || z) {
      ...
      this.mDexController.scheduleKillProcessAndStartActivity(
          createStartActivityFromRecentsType, launchDisplayId);
    }
    return true;
  }
  ...
  public final boolean cancelIntercept(ActivityRecord activityRecord, int i) {
    if (!DexController.isDefaultOrDexDisplay(i)
  ...
  public class DexRestartAppDialogController {
    ...
    public void showRestartAppDialogLocked(DexRestartAppInfo dexRestartAppInfo, int i) {
      ...
      if (DexActivityStartInterceptor.SAFE_DEBUG) {
        Slog.d(
            "DexActivityStartInterceptor",
            "Skip to show Restart app DialogmDoNotShowAgainChecked="
                + this.mIsDoNotShowAgainChecked);
      }
      DexActivityStartInterceptor.this.mDexController.scheduleMoveTasksBackAndStartPendingActivity(
          dexRestartAppInfo, i);
    }
    ...
    public final class UiHandler extends Handler {
      ...
      public void handleMessage(Message message) {
        ...
        WindowManagerGlobalLock windowManagerGlobalLock =
            DexActivityStartInterceptor.this.mAtmService.mGlobalLock;
            ...
            try {
              if (DexActivityStartInterceptor.this.mDexDisplay != null) {
                createDisplayContext =
                    DexActivityStartInterceptor.this
                        .mAtmService
                        .getUiContext()
                        .createDisplayContext(
                            DexActivityStartInterceptor.this.mDexDisplay.getDisplay());
                context = createDisplayContext;
              }
            } catch (Throwable th) {
        ...
        WindowManagerService.resetPriorityAfterLockedSection();
        new DexRestartAppDialog(
                context,
                DexActivityStartInterceptor.this.mAtmService,
                DexActivityStartInterceptor.this.mDexController,
                DexActivityStartInterceptor.this.mDexRestartAppDialogController,
                dexRestartAppInfo,
                i)
            .show();
      }
