...
public abstract class SizeCompatMultiTaskingPolicy extends SizeCompatPolicy {
  ...
  public void ensureTransaction(int i, WindowConfiguration windowConfiguration, int i2) {
    ...
    if (i >= 0 && i != task.getWindowingMode()) {
      ...
      if (CoreRune.MT_SUPPORT_SIZE_COMPAT_DRAG
          && i2 != 0
          && supportsDragResizable()
          && (asSizeCompatDragPolicy = asSizeCompatDragPolicy()) != null) {
      ...
      setFreeformConfiguration(
          ...
          new BiConsumer() { // from class:
            ...
            public final void accept(Object obj, Object obj2) {
              SizeCompatMultiTaskingPolicy.lambda$onOrientationChanged$0(
                  bounds, bounds2, (Rect) obj, (Rect) obj2);
            }
          });
    }
    if (DEBUG_CONFIG) {
      logDebugConfig(
          "onOrientationChanged", this.mTask, requestedOverrideConfiguration.windowConfiguration);
    }
  }

  public static /* synthetic */ void lambda$onOrientationChanged$0(
      Rect rect, Rect rect2, Rect rect3, Rect rect4) {
    int i;
    int i2;
    ...
    if (i < rect2.left
        || i2 < rect2.top
        || i + width3 > rect2.right
        || i2 + height3 > rect2.bottom) {
      i = (width2 - width3) >> 1;
  ...
  public final Rect getTaskBounds(Rect rect, int i, float f, boolean z) {
    ...
    this.mTmpContainingBounds.set(
        0,
        0,
        shouldUseAspectRatio ? fullScreenBounds.height() : fullScreenBounds.width(),
        shouldUseAspectRatio ? fullScreenBounds.width() : fullScreenBounds.height());
    CompatUtils.getScaledBounds(this.mTmpContainingBounds, f, shouldUseAspectRatio);
    if (z
        && !shouldUseAspectRatio
        && isSmallerContainer(
            width, height, this.mTmpContainingBounds.width(), this.mTmpContainingBounds.height())) {
  ...
  public void setFreeformConfiguration(
  ...
  public final boolean supportsSandbox(ActivityRecord activityRecord) {
    ...
    return (sizeCompatAttributes != null && sizeCompatAttributes.hasBounds())
        || !this.mTask.inFullscreenWindowingMode()
        || (taskDisplayArea = this.mTask.getTaskDisplayArea()) == null
  ...
  public static void logDebugConfig(
      String str, Task task, WindowConfiguration windowConfiguration) {
    Slog.d(
        "SizeCompatPolicy",
        str
            + ": bounds="
            ...
            + ", mode="
            + WindowConfiguration.windowingModeToString(windowConfiguration.getWindowingMode())
            + ", task="
            + task);
  }

  public static void logDebugConfig(
      String str, Task task, LaunchParamsController.LaunchParams launchParams) {
    Slog.d(
        "SizeCompatPolicy",
        str
            + ": bounds="
            + launchParams.mBounds
            + ", mode="
            + WindowConfiguration.windowingModeToString(launchParams.mWindowingMode)
            + ", task="
            + task);
  }
  ...
  public static void logWindowingModeWarning(String str, Task task, int i) {
    Slog.w(
        "SizeCompatPolicy",
        str
            + ": Unsupported windowing mode, mode="
            + WindowConfiguration.windowingModeToString(i)
            + ", task="
            + task);
  }
}
