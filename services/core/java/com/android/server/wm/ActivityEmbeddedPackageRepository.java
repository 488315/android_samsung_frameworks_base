...
public class ActivityEmbeddedPackageRepository {
        ...
        public void onReceive(Context context, Intent intent) {
          ...
          if (!ActivityEmbeddedPackageRepository.this.allowSystemOverride(str)) {
            if (action == "android.intent.action.PACKAGE_REMOVED"
                && ActivityEmbeddedPackageRepository.this.remove(str)) {
              ActivityEmbeddedPackageRepository.this.mAtm.mMultiTaskingController
                  .updateEmbedActivityPackageEnabled(str, 0, userId, true);
              return;
            }
            return;
          }
          if (action == "android.intent.action.PACKAGE_ADDED"
            ...
            ActivityEmbeddedPackageRepository.this.mAtm.mMultiTaskingController
                .updateEmbedActivityPackageEnabled(str, 1, userId, false);
  ...
  public boolean allowSystemOverride(String str) {
    try {
      return this.mAtm
              .mContext
              .getPackageManager()
              .getProperty("android.window.PROPERTY_ACTIVITY_EMBEDDING_ALLOW_SYSTEM_OVERRIDE", str)
          != null;
  ...
  public void loadActivityEmbeddedPackages() {
    ...
    for (ApplicationInfo applicationInfo :
        this.mAtm.mContext.getPackageManager().getInstalledApplications(0)) {
      if (allowSystemOverride(applicationInfo.packageName)) {
        ActivityTaskManagerService activityTaskManagerService = this.mAtm;
        activityTaskManagerService.mMultiTaskingController.updateEmbedActivityPackageEnabled(
            applicationInfo.packageName, 1, activityTaskManagerService.getCurrentUserId(), false);
  ...
  public void registerActivityEmbeddedPackageReceiver() {
    ...
    ActivityTaskManagerService activityTaskManagerService = this.mAtm;
    activityTaskManagerService.mContext.registerReceiver(
        this.mPackageReceiver, intentFilter, null, activityTaskManagerService.f1734mH);
  }
