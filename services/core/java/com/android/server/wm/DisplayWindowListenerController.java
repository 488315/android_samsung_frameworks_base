...
public class DisplayWindowListenerController {
        ...
        this.mService.mAtmService.mRootWindowContainer.forAllDisplays(
            new Consumer() { // from class:
              ...
              public final void accept(Object obj) {
                DisplayWindowListenerController.lambda$registerListener$0(
                    intArray, (DisplayContent) obj);
              }
            });
  ...
  public static /* synthetic */ void lambda$registerListener$0(
      IntArray intArray, DisplayContent displayContent) {
  ...
  public void dispatchDisplayChanged(DisplayContent displayContent, Configuration configuration) {
    ...
    if (z) {
      ...
      for (int i2 = 0; i2 < beginBroadcast; i2++) {
        try {
          this.mDisplayListeners
              .getBroadcastItem(i2)
              .onDisplayConfigurationChanged(displayContent.getDisplayId(), configuration);
        } catch (RemoteException unused) {
        }
      }
      this.mDisplayListeners.finishBroadcast();
  ...
  public void dispatchFixedRotationStarted(DisplayContent displayContent, int i) {
    ...
    for (int i2 = 0; i2 < beginBroadcast; i2++) {
      try {
        this.mDisplayListeners
            .getBroadcastItem(i2)
            .onFixedRotationStarted(displayContent.mDisplayId, i);
      } catch (RemoteException unused) {
      }
    }
    this.mDisplayListeners.finishBroadcast();
  ...
  public void dispatchFixedRotationFinished(DisplayContent displayContent) {
    ...
    for (int i = 0; i < beginBroadcast; i++) {
      try {
        this.mDisplayListeners
            .getBroadcastItem(i)
            .onFixedRotationFinished(displayContent.mDisplayId);
      } catch (RemoteException unused) {
      }
    }
    this.mDisplayListeners.finishBroadcast();
      ...
      try {
        this.mDisplayListeners
            ...
            .onKeepClearAreasChanged(
                displayContent.mDisplayId, new ArrayList(set), new ArrayList(set2));
      } catch (RemoteException unused) {
      }
    }
    this.mDisplayListeners.finishBroadcast();
