...
public final class InputConsumerImpl implements IBinder.DeathRecipient {
    ...
    public InputConsumerImpl(
        ...
        InputChannel createInputChannel =
                windowManagerService.mInputManager.createInputChannel(str);
        ...
        InputWindowHandle inputWindowHandle =
                new InputWindowHandle(new InputApplicationHandle(binder, str, j), i2);
        ...
        SurfaceControl build =
                windowManagerService
                        .makeSurfaceBuilder(
                                windowManagerService.mRoot.getDisplayContent(i2).mSession)
                        .setContainerLayer()
                        .setName("Input Consumer " + str)
        ...
        synchronized (this.mService.mGlobalLock) {
            try {
                DisplayContent displayContent =
                        this.mService.mRoot.getDisplayContent(this.mWindowHandle.displayId);
    ...
    public final void layout(SurfaceControl.Transaction transaction, Rect rect) {
        ...
        if (this.mNeedOneHandOpSpec
                && this.mDisplayContent.hasOneHandOpSpec()
                && (magnificationSpec = this.mDisplayContent.mMagnificationSpec) != null) {
            float f = rect.left;
            float f2 = magnificationSpec.scale;
            rect.offsetTo((int) (f * f2), (int) (rect.top * f2));
        }
        this.mTmpClipRect.set(0, 0, rect.width(), rect.height());
        if (this.mOldPosition.equals(rect.left, rect.top)
                && this.mOldWindowCrop.equals(this.mTmpClipRect)) {
            return;
        }
        if (!this.mNeedOneHandOpSpec || this.mDisplayContent.getOneHandOpPolicy() == null) {
            ...
            if (!displayContent.getOneHandOpPolicy().mHasOneHandOpSpec
                ...
                transaction.setPosition(
                        this.mInputSurface,
                        rect.left + magnificationSpec2.offsetX,
                        rect.top + magnificationSpec2.offsetY);
            }
        }
