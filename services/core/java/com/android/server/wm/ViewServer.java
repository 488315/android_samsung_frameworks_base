  ...
  public static boolean writeValue(Socket socket, String str) {
    ...
    try {
      try {
        bufferedWriter =
            new BufferedWriter(
                new OutputStreamWriter(socket.getOutputStream()), IInstalld.FLAG_FORCE);
    ...
    */
    public void run() {
      ...
      try {
        try {
          bufferedReader =
              new BufferedReader(new InputStreamReader(this.mClient.getInputStream()), 1024);
          try {
            try {
              ...
              if ("PROTOCOL".equalsIgnoreCase(readLine)) {
                ...
                viewServerWindowCommand =
                    ViewServer.this.mWindowManager.viewServerListWindows(this.mClient);
              } else if ("GET_FOCUS".equalsIgnoreCase(readLine)) {
                viewServerWindowCommand =
                    ViewServer.this.mWindowManager.viewServerGetFocusedWindow(this.mClient);
              } else if ("AUTOLIST".equalsIgnoreCase(readLine)) {
                viewServerWindowCommand = windowManagerAutolistLoop();
              } else {
                viewServerWindowCommand =
                    ViewServer.this.mWindowManager.viewServerWindowCommand(
                        this.mClient, readLine, substring);
              }
              if (!viewServerWindowCommand) {
                Slog.w(
                    StartingSurfaceController.TAG,
                    "An error occurred with the command: " + readLine);
              }
              try {
      ...
      try {
        try {
          BufferedWriter bufferedWriter2 =
              new BufferedWriter(new OutputStreamWriter(this.mClient.getOutputStream()));
