...
public class SurfaceAnimator {
  ...
  public interface Animatable {
    ...
    default void onLeashAnimationStarting(
        SurfaceControl.Transaction transaction, SurfaceControl surfaceControl) {}
  ...
  public final OnAnimationFinishedCallback getFinishedCallback(
      final OnAnimationFinishedCallback onAnimationFinishedCallback) {
    return new OnAnimationFinishedCallback() { // from class:
                                               // com.android.server.wm.SurfaceAnimator$$ExternalSyntheticLambda0
      @Override // com.android.server.wm.SurfaceAnimator.OnAnimationFinishedCallback
      public final void onAnimationFinished(int i, AnimationAdapter animationAdapter) {
        SurfaceAnimator.this.lambda$getFinishedCallback$1(
            onAnimationFinishedCallback, i, animationAdapter);
      }
    };
  }

  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ void lambda$getFinishedCallback$1(
      final OnAnimationFinishedCallback onAnimationFinishedCallback,
      final int i,
      final AnimationAdapter animationAdapter) {
    WindowManagerGlobalLock windowManagerGlobalLock = this.mService.mGlobalLock;
    WindowManagerService.boostPriorityForLockedSection();
    synchronized (windowManagerGlobalLock) {
      try {
        SurfaceAnimator surfaceAnimator =
            (SurfaceAnimator) this.mService.mAnimationTransferMap.remove(animationAdapter);
        if (surfaceAnimator != null) {
          ...
          Runnable runnable =
              new Runnable() { // from class:
                               // com.android.server.wm.SurfaceAnimator$$ExternalSyntheticLambda1
                @Override // java.lang.Runnable
                public final void run() {
                  SurfaceAnimator.this.lambda$getFinishedCallback$0(
                      animationAdapter, onAnimationFinishedCallback, i);
                }
              };
          if (!this.mAnimatable.shouldDeferAnimationFinish(runnable)
              && !animationAdapter.shouldDeferAnimationFinish(runnable)) {
            runnable.run();
          }
  ...
  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ void lambda$getFinishedCallback$0(
    ...
    OnAnimationFinishedCallback onAnimationFinishedCallback2 =
        this.mSurfaceAnimationFinishedCallback;
  ...
  public void startAnimation(
    ...
    SurfaceControl takeLeashForAnimation =
        surfaceFreezer != null ? surfaceFreezer.takeLeashForAnimation() : null;
    this.mLeash = takeLeashForAnimation;
    if (takeLeashForAnimation == null) {
      Animatable animatable = this.mAnimatable;
      SurfaceControl createAnimationLeash =
          createAnimationLeash(
              animatable,
              surfaceControl,
              transaction,
              i,
              animatable.getSurfaceWidth(),
              this.mAnimatable.getSurfaceHeight(),
              0,
              0,
              z,
    ...
    if (this.mAnimationStartDelayed) {
      if (ProtoLogCache.WM_DEBUG_ANIM_enabled) {
        ProtoLogImpl.i(
            ...
            0,
            (String) null,
            new Object[] {String.valueOf(this.mAnimatable)});
        return;
      }
      return;
    }
    this.mAnimation.startAnimation(
        this.mLeash, transaction, i, this.mInnerAnimationFinishedCallback);
    if (ProtoLogImpl.isEnabled(ProtoLogGroup.WM_DEBUG_ANIM)) {
      ...
      if (ProtoLogCache.WM_DEBUG_ANIM_enabled) {
        ProtoLogImpl.d(
            ...
            0,
            (String) null,
            new Object[] {String.valueOf(this.mAnimatable), String.valueOf(stringWriter)});
      }
    }
    if (animationAdapter2 != null) {
      ...
      if (takeSnapshotForAnimation == null) {
        Slog.e(
            StartingSurfaceController.TAG,
            "No snapshot target to start animation on for " + this.mAnimatable);
        return;
      }
  ...
  public void endDelayingAnimationStart() {
    ...
    animationAdapter.startAnimation(
        this.mLeash,
        this.mAnimatable.getSyncTransaction(),
        this.mAnimationType,
        this.mInnerAnimationFinishedCallback);
  ...
  public void transferAnimation(SurfaceAnimator surfaceAnimator) {
    ...
    if (surfaceControl == null || animationLeashParent == null) {
      Slog.w(
          StartingSurfaceController.TAG, "Unable to transfer animation, surface or parent is null");
      cancelAnimation();
      return;
    }
    if (surfaceAnimator.mAnimationFinished) {
      Slog.w(
          StartingSurfaceController.TAG,
          "Unable to transfer animation, because " + surfaceAnimator + " animation is finished");
      return;
    }
  ...
  public void cancelAnimation(SurfaceControl.Transaction transaction, boolean z, boolean z2) {
    ...
    if (this.mLeash != null) {
      ...
      ProtoLogImpl.i(
          ...
          3,
          (String) null,
          new Object[] {Boolean.valueOf(z), String.valueOf(this.mAnimatable)});
    }
    SurfaceControl surfaceControl = this.mLeash;
    ...
    if (animationAdapter != null) {
      ...
      if (!z) {
        OnAnimationFinishedCallback onAnimationFinishedCallback2 =
            this.mStaticAnimationFinishedCallback;
        if (onAnimationFinishedCallback2 != null) {
          onAnimationFinishedCallback2.onAnimationFinished(i, animationAdapter);
        }
        if (onAnimationFinishedCallback != null) {
  ...
  public static boolean removeLeash(
      SurfaceControl.Transaction transaction,
      Animatable animatable,
    ...
    boolean z3 =
        surfaceControl2 != null
            && (animationLeash == null || animationLeash.equals(surfaceControl));
    if (z3) {
      StringBuilder sb = new StringBuilder();
  ...
  public static SurfaceControl createAnimationLeash(
      Animatable animatable,
      SurfaceControl surfaceControl,
      SurfaceControl.Transaction transaction,
      int i,
    ...
    SurfaceControl.Builder callsite =
        animatable
            ...
            .setName(surfaceControl + " - animation-leash of " + animationTypeToString(i))
            .setHidden(z)
            .setEffectLayer()
    ...
    Slog.d(
        StartingSurfaceController.TAG,
        "makeSurface duration=" + (SystemClock.uptimeMillis() - uptimeMillis) + " leash=" + build);
