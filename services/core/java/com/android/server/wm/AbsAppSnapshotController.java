...
public abstract class AbsAppSnapshotController {
  ...
  public abstract ActivityManager.TaskDescription getTaskDescription(
    ...
    return Math.max(
        Math.min(
            this.mService
                .mContext
                .getResources()
                .getFloat(R.dimen.config_closeToSquareDisplayMaxAspectRatio),
            1.0f),
        0.1f);
  }
  ...
  public final TaskSnapshot recordSnapshotInner(WindowContainer windowContainer, boolean z) {
    ...
    TaskSnapshot captureSnapshot =
        captureSnapshot(windowContainer, z && windowContainer.isActivityTypeHome());
    ...
    if (hardwareBuffer.getWidth() == 0 || hardwareBuffer.getHeight() == 0) {
      ...
      Slog.e(
          ...
          "Invalid snapshot dimensions "
              + hardwareBuffer.getWidth()
              + "x"
              + hardwareBuffer.getHeight());
      return null;
    }
  ...
  public int getSnapshotMode(WindowContainer windowContainer) {
    ...
    if (windowContainer.isActivityTypeStandardOrUndefined()
        || windowContainer.isActivityTypeAssistant()) {
  ...
  public TaskSnapshot snapshot(WindowContainer windowContainer, int i) {
    ...
    if (!prepareTaskSnapshot(windowContainer, i, builder)
        || (createSnapshot = createSnapshot(windowContainer, builder)) == null) {
      return null;
    }
  ...
  public ScreenCapture.ScreenshotHardwareBuffer createSnapshot(
      WindowContainer windowContainer, TaskSnapshot.Builder builder) {
    Point point = new Point();
    Trace.traceBegin(32L, "createSnapshot");
    ScreenCapture.ScreenshotHardwareBuffer createSnapshot =
        createSnapshot(
            windowContainer, this.mHighResSnapshotScale, builder.getPixelFormat(), point, builder);
    Trace.traceEnd(32L);
  ...
  public ScreenCapture.ScreenshotHardwareBuffer createSnapshot(
      WindowContainer windowContainer, float f, int i, Point point, TaskSnapshot.Builder builder) {
    ...
    if (windowContainer.getSurfaceControl() == null) {
      Slog.w(
          StartingSurfaceController.TAG,
          "Failed to take screenshot. No surface control for " + windowContainer);
      return null;
    }
    this.mTmpRect.setEmpty();
    if (windowContainer.mTransitionController.inFinishingTransition(windowContainer)
        && (changeInfo =
                (Transition.ChangeInfo)
                    windowContainer.mTransitionController.mFinishingTransition.mChanges.get(
                        windowContainer))
            != null) {
    ...
    if (windowContainer.getDisplayContent() != null) {
      ...
      boolean z2 =
          (windowState == null
                  || windowState.getSurfaceControl() == null
                  || windowContainer.getDisplayContent().shouldImeAttachedToApp())
              ? false
              : true;
      WindowState navigationBar =
          windowContainer.getDisplayContent().getDisplayPolicy().getNavigationBar();
    ...
    ScreenCapture.ScreenshotHardwareBuffer captureLayersExcluding =
        ScreenCapture.captureLayersExcluding(
            windowContainer.getSurfaceControl(), this.mTmpRect, f, i, surfaceControlArr);
    if (point != null) {
      point.x = this.mTmpRect.width();
      point.y = this.mTmpRect.height();
    }
    HardwareBuffer hardwareBuffer =
        captureLayersExcluding == null ? null : captureLayersExcluding.getHardwareBuffer();
    if (!isInvalidHardwareBuffer(hardwareBuffer)) {
      return captureLayersExcluding;
    }
    Slog.w(
        StartingSurfaceController.TAG,
        "Failed to take screenshot. isInvalidHardwareBuffer buffer=" + hardwareBuffer);
    return null;
  }

  public static boolean isInvalidHardwareBuffer(HardwareBuffer hardwareBuffer) {
    return hardwareBuffer == null
        || hardwareBuffer.isClosed()
        || hardwareBuffer.getWidth() <= 1
        || hardwareBuffer.getHeight() <= 1;
  }

  public boolean prepareTaskSnapshot(
      WindowContainer windowContainer, int i, TaskSnapshot.Builder builder) {
    ...
    if (!activityRecord.inFreeformWindowingMode()) {
      systemBarInsets =
          getSystemBarInsetsWithoutCaptionBar(
              windowState.getFrame(), windowState.getInsetsStateWithVisibilityOverride());
    } else {
      systemBarInsets =
          getSystemBarInsets(
              windowState.getFrame(), windowState.getInsetsStateWithVisibilityOverride());
  ...
  public Pair checkIfReadyToSnapshot(WindowContainer windowContainer) {
    ...
    if (!windowManagerPolicy.isScreenOn(
        displayContent != null ? displayContent.getDisplayId() : 0)) {
    ...
    if (findAppTokenForSnapshot == null) {
      Slog.w(
          StartingSurfaceController.TAG,
          "Failed to take screenshot. No visible windows for " + windowContainer);
      return null;
    }
    if (findAppTokenForSnapshot.hasCommittedReparentToAnimationLeash()) {
      Slog.w(
          StartingSurfaceController.TAG,
          "Failed to take screenshot. App is animating " + findAppTokenForSnapshot);
      return null;
    }
    ...
    if (findMainWindow == null) {
      Slog.w(
          StartingSurfaceController.TAG,
          "Failed to take screenshot. No main window for " + windowContainer);
      return null;
    }
    if (findAppTokenForSnapshot.hasFixedRotationTransform()) {
      Slog.i(
          StartingSurfaceController.TAG,
          "Skip taking screenshot. App has fixed rotation " + findAppTokenForSnapshot);
      return null;
    }
  ...
  public final TaskSnapshot drawAppThemeSnapshot(WindowContainer windowContainer) {
    ...
    InsetsState insetsStateWithVisibilityOverride =
        findMainWindow.getInsetsStateWithVisibilityOverride();
    if (!topActivity.inFreeformWindowingMode()) {
      systemBarInsets =
          getSystemBarInsetsWithoutCaptionBar(
              findMainWindow.getFrame(), insetsStateWithVisibilityOverride);
    } else {
      systemBarInsets =
          getSystemBarInsets(findMainWindow.getFrame(), insetsStateWithVisibilityOverride);
    }
    Rect rect = systemBarInsets;
    SnapshotDrawerUtils.SystemBarBackgroundPainter systemBarBackgroundPainter =
        new SnapshotDrawerUtils.SystemBarBackgroundPainter(
            attrs.flags,
            attrs.privateFlags,
            attrs.insetsFlags.appearance,
            taskDescription,
            this.mHighResSnapshotScale,
            findMainWindow.getRequestedVisibleTypes());
    ...
    return new TaskSnapshot(
        ...
        topActivity.mActivityComponent,
        createHardwareBitmap.getHardwareBuffer(),
        createHardwareBitmap.getColorSpace(),
        findMainWindow.getConfiguration().orientation,
        findMainWindow.getWindowConfiguration().getRotation(),
        new Point(width, height),
        rect2,
        letterboxInsets,
        false,
        false,
        windowContainer.getWindowingMode(),
        getAppearance(windowContainer),
        false,
        false,
        getCutoutInsets(findMainWindow.getFrame(), insetsStateWithVisibilityOverride));
  }
  ...
  public static Rect getSystemBarInsetsWithoutCaptionBar(Rect rect, InsetsState insetsState) {
    return insetsState
        .calculateInsets(rect, WindowInsets.Type.systemBarsWithoutCaptionBar(), false)
        .toRect();
  }
  ...
  public final int getAppearance(WindowContainer windowContainer) {
    ...
    WindowState topFullscreenOpaqueWindow =
        topFullscreenActivity != null ? topFullscreenActivity.getTopFullscreenOpaqueWindow() : null;
