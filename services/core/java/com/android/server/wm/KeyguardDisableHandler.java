...
public class KeyguardDisableHandler {
  ...
  public KeyguardDisableHandler(Injector injector, Handler handler) {
    UserTokenWatcher.Callback callback =
        new UserTokenWatcher
            .Callback() { // from class: com.android.server.wm.KeyguardDisableHandler.1
    ...
    this.mSystemTokenWatcher =
        new UserTokenWatcher(callback, handler, StartingSurfaceController.TAG);
  }
  ...
  public final boolean shouldKeyguardBeEnabled(int i) {
    ...
    boolean z = false;
    boolean z2 =
        (dpmRequiresPassword || this.mInjector.isKeyguardSecure(this.mCurrentUser)) ? false : true;
    boolean z3 = !dpmRequiresPassword;
    if ((z2 && this.mAppTokenWatcher.isAcquired(i))
        || (z3 && this.mSystemTokenWatcher.isAcquired(i))) {
      z = true;
    }
    return !z;
  }

  public static KeyguardDisableHandler create(
      Context context, final WindowManagerPolicy windowManagerPolicy, Handler handler) {
    final UserManagerInternal userManagerInternal =
        (UserManagerInternal) LocalServices.getService(UserManagerInternal.class);
    return new KeyguardDisableHandler(
        new Injector() { // from class: com.android.server.wm.KeyguardDisableHandler.2
