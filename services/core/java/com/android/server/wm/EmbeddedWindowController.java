...
public class EmbeddedWindowController {
  ...
  public final void updateProcessController(EmbeddedWindow embeddedWindow) {
    ...
    WindowProcessController processController =
        this.mAtmService.getProcessController(embeddedWindow.mOwnerPid, embeddedWindow.mOwnerUid);
  ...
  public void onActivityRemoved(ActivityRecord activityRecord) {
    ...
    for (int size = this.mWindows.size() - 1; size >= 0; size--) {
      EmbeddedWindow embeddedWindow = (EmbeddedWindow) this.mWindows.valueAt(size);
      if (embeddedWindow.mHostActivityRecord == activityRecord
          && (processController =
  ...
  public class EmbeddedWindow implements InputTarget {
    ...
    public EmbeddedWindow(
      ...
      if (windowState != null) {
        str2 =
            PackageManagerShellCommandDataLoader.STDIN_PATH + windowState.getWindowTag().toString();
      } else {
        str2 = "";
      }
      this.mIsFocusable = z;
    ...
    public InputApplicationHandle getApplicationHandle() {
      ...
      if (windowState == null
          || windowState.mInputWindowHandle.getInputApplicationHandle() == null) {
        return null;
      }
      return new InputApplicationHandle(
          this.mHostWindowState.mInputWindowHandle.getInputApplicationHandle());
    }

    public InputChannel openInputChannel() {
      InputChannel createInputChannel =
          this.mWmService.mInputManager.createInputChannel(toString());
    ...
    public void onGrantFocusChanged(boolean z) {
      if (this.mFocusGranted != z) {
        ...
        Slog.d(
            "EmbeddedWindow",
            "onGrantFocusChanged: " + z + ", this=" + this + ", Callers=" + Debug.getCallers(2));
      }
    }
    ...
    public final void handleTap(boolean z) {
      if (this.mInputChannel != null) {
        WindowState windowState = this.mHostWindowState;
        if (windowState != null) {
          this.mWmService.grantEmbeddedWindowFocus(
              null, windowState.mClient, this.mFocusGrantToken, z);
          if (z) {
