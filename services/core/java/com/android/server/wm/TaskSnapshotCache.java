...
public class TaskSnapshotCache extends SnapshotCache {
  ...
  public TaskSnapshotCache(
      WindowManagerService windowManagerService, AppSnapshotLoader appSnapshotLoader) {
  ...
  public void putSnapshot(Task task, TaskSnapshot taskSnapshot) {
    SnapshotCache.CacheEntry cacheEntry =
        (SnapshotCache.CacheEntry) this.mRunningCache.get(Integer.valueOf(task.mTaskId));
    if (cacheEntry != null) {
    ...
    this.mRunningCache.put(
        Integer.valueOf(task.mTaskId), new SnapshotCache.CacheEntry(taskSnapshot, topMostActivity));
  ...
  public final void removeOldestCacheIfNeeded(Task task) {
    ...
    if (shouldApplySplitModeSnapshotPolicy()
    ...
    while (it.hasNext()) {
      ...
      SnapshotCache.CacheEntry cacheEntry =
          (SnapshotCache.CacheEntry) this.mRunningCache.get(Integer.valueOf(intValue));
      if (cacheEntry != null) {
        if (z
            && (activityRecord = cacheEntry.topApp) != null
            && WindowConfiguration.isSplitScreenWindowingMode(
                activityRecord.getWindowConfiguration())) {
          this.mTaskIdsInSplitMode.add(
              new Pair(Integer.valueOf(intValue), Long.valueOf(cacheEntry.timestamp)));
        } else {
  ...
  public final void updateSplitModeSnapshotCache() {
    if (!shouldApplySplitModeSnapshotPolicy()
        || this.mTaskIdsInSplitMode.size() <= sSplitModeMaxCacheSize) {
      return;
    }
    removeRunningEntry(
        Integer.valueOf(
                                    ...
                                    this.mTaskIdsInSplitMode.stream()
                                        .sorted(
                                            new Comparator() { // from class:
                                                               // com.android.server.wm.TaskSnapshotCache$$ExternalSyntheticLambda0
                                              @Override // java.util.Comparator
                                              public final int compare(Object obj, Object obj2) {
                                                int lambda$updateSplitModeSnapshotCache$0;
                                                lambda$updateSplitModeSnapshotCache$0 =
                                                    TaskSnapshotCache
                                                        .lambda$updateSplitModeSnapshotCache$0(
                                                            (Pair) obj, (Pair) obj2);
