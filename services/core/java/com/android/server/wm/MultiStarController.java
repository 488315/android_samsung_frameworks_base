...
public final class MultiStarController implements IController {
    public static final List DEFAULT_ALLOW_LIST =
            Arrays.asList(
    ...
    public static void checkMultiStarPackageAndPermission(String str, List list) {
        ...
        try {
            String[] packagesForUid =
                    AppGlobals.getPackageManager().getPackagesForUid(Binder.getCallingUid());
        ...
        StringBuilder m =
                DumpUtils$$ExternalSyntheticOutline0.m("Permission Denial: ", str, " from pid=");
    ...
    public final void setCustomDensityEnabled(int i) {
        ...
        m.append(
                multiWindowEnableController.mSimpleDateFormat.format(
        ...
        while (it.hasNext()) {
            multiWindowEnableController.mCoreStateController.setVolatileState(
                    "custom_density",
                    Integer.valueOf(i),
    ...
    public final boolean toggleFreeformWindowingMode() {
        ...
        Task rootTask =
                activityTaskManagerService
                        ...
                        .getRootTask(1, 1);
        if (rootTask == null
                || (activityRecord = rootTask.topRunningActivityLocked()) == null
                || activityRecord.task == null
                || !activityRecord.supportsFreeform()) {
            return false;
        }
        activityTaskManagerService.mActivityClientController.toggleFreeformWindowingMode(
                activityRecord.token);
        if (activityRecord.inFreeformWindowingMode()
                && ((rect = activityRecord.task.mLastNonFullscreenBounds) == null
                        || rect.isEmpty())) {
            ActivityOptions makeBasic = ActivityOptions.makeBasic();
            makeBasic.setLaunchWindowingMode(5);
            LaunchParamsController launchParamsController =
                    activityTaskManagerService.mTaskSupervisor.mLaunchParamsController;
            Task task = activityRecord.task;
            launchParamsController.layoutTask(
                    task,
                    activityRecord.info.windowLayout,
                    activityRecord,
                    null,
                    makeBasic,
                    task.getDisplayId());
