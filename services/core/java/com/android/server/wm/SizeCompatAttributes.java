...
public class SizeCompatAttributes {
  ...
  public boolean updateScale(Rect rect, float f, SizeCompatPolicy sizeCompatPolicy) {
    ...
    if (!isSameReason(sizeCompatPolicy)) {
      Slog.w(
          "SizeCompatPolicy",
          "The reason is changed from "
              + SizeCompatInfo.sizeCompatModeToString(this.mReason.getMode())
              + " to "
              + SizeCompatInfo.sizeCompatModeToString(sizeCompatPolicy.getMode())
              + ", scale="
              + f
              + ", bounds="
              + rect);
  ...
  public void cleanUp(SizeCompatPolicy sizeCompatPolicy) {
    ...
    Slog.w(
        "SizeCompatPolicy",
        "Failed to clean up by "
            + SizeCompatInfo.sizeCompatModeToString(sizeCompatPolicy.getMode())
            + ", SizeCompatAttributes created by "
            + SizeCompatInfo.sizeCompatModeToString(this.mReason.getMode())
            + ", r="
