...
public class DexCompatController implements IController {
  ...
  public boolean resolveDexCompatConfigurationLocked(
  ...
  public boolean shouldBeApplyDexCompatConfigurationLocked(
      ActivityRecord activityRecord, ApplicationInfo applicationInfo, int i) {
    ...
    return ((dexModeLocked == 1 && i == 0) || (dexModeLocked == 2 && i == 2))
        && (getResolvedLaunchPolicyForPackage(activityRecord, applicationInfo) & 1) != 0;
  }
  ...
  */
  public void applyDexCompatConfigurationLocked(
      ActivityRecord activityRecord,
      ApplicationInfo applicationInfo,
      Configuration configuration,
      String str) {
      ...
      if (task.getMergedOverrideConfiguration().orientation == 2) {
        ...
        if (!z) {
          ...
          DisplayContent displayContent =
              this.mAtm.mRootWindowContainer.getDisplayContent(dexModeLocked != 1 ? 2 : 0);
          ...
          if (defaultWidth <= 0 || defaultHeight <= 0) {
            Slog.w(
                "DexCompatController",
                "applyDexCompatConfigurationLocked: Invalid size, "
                    + defaultWidth
                    + "x"
                    + defaultHeight);
          }
          ...
          configuration.screenLayout =
              Configuration.reduceScreenLayout(resetScreenLayout, max2, min3);
        }
        configuration.dexCompatEnabled = 2;
        if (!DEBUG_DEX_COMPAT || CoreRune.SAFE_DEBUG) {
          Slog.i(
              "DexCompatController",
              "[DexCompat] applyDexCompatConfigurationLocked:  packageName="
  ...
  public final ActivityRecord getTopCompatActivityLocked(final String str) {
    ...
    if (dexModeLocked == 2) {
      ...
      displayContent =
          dexModeLocked == 1 ? this.mAtm.mRootWindowContainer.getDisplayContent(0) : null;
    }
    if (str != null && displayContent != null) {
      final ArrayList arrayList = new ArrayList();
      displayContent.forAllTasks(
          new Consumer() { // from class:
                           // com.android.server.wm.DexCompatController$$ExternalSyntheticLambda0
            ...
            ((Task) it.next())
                .getActivity(
                    new Predicate() { // from class:
                                      // com.android.server.wm.DexCompatController$$ExternalSyntheticLambda1
                      @Override // java.util.function.Predicate
                      public final boolean test(Object obj) {
                        boolean lambda$getTopCompatActivityLocked$1;
                        lambda$getTopCompatActivityLocked$1 =
                            DexCompatController.lambda$getTopCompatActivityLocked$1(
  ...
  public static /* synthetic */ void lambda$getTopCompatActivityLocked$0(
      ArrayList arrayList, Task task) {
    if (task.isDexCompatEnabled()) {
      arrayList.add(task);
    }
  }

  public static /* synthetic */ boolean lambda$getTopCompatActivityLocked$1(
      String str, ActivityRecord activityRecord) {
    return str.equals(activityRecord.processName) && !activityRecord.finishing;
  }

  public final int getResolvedLaunchPolicyForPackage(
      ActivityRecord activityRecord, ApplicationInfo applicationInfo) {
    if (((DesktopModeManagerInternal) LocalServices.getService(DesktopModeManagerInternal.class))
        == null) {
      Slog.w(
          "DexCompatController",
          "[DexCompat] DexCompatLaunchPolicy: task="
              + this
              + ", DesktopModeService is null, caller="
              + Debug.getCallers(6));
      return 0;
    }
    if (activityRecord == null) {
    ...
    if (i == 0) {
      activityRecord.mProcessAppLaunchPolicy =
          this.mAtm.mDexController.getDexPolicyFlags(applicationInfo);
    ...
    if (activityRecord.info == null
        ...
        || !activityRecord.getTask().isDexCompatEnabled()) {
      return 0;
    }
  ...
  public void loadResources(int i) {
    ...
    this.mDecorCaptionHeightInFullscreen.put(
        i, Integer.valueOf(resources.getDimensionPixelSize(R.dimen.toast_width)));
    this.mDecorCaptionHeightInFreeform.put(
        i, Integer.valueOf(resources.getDimensionPixelSize(R.dimen.toast_text_size)));
  }

  public void updateDexCompatLaunchPolicy(Task task, ActivityInfo activityInfo) {
    String str;
    if (task.isDexMode()) {
      task.mDexLaunchPolicy =
          this.mAtm.mDexController.getDexPolicyFlags(activityInfo.applicationInfo);
  ...
  public void changeWindowingModeIfNeeded(Task task, Task task2, ActivityRecord activityRecord) {
    ...
    this.mAtm
        ...
        .calculate(task2, null, activityRecord, null, null, null, 3, launchParams);
    ...
    Slog.d(
        "DexCompatController",
        "[DexCompat] changeWindowingModeIfNeeded: prev="
            + windowingMode
            + ", next="
  ...
  public boolean isOrientationChangedLocked(Task task, ActivityRecord activityRecord) {
    ...
    if ((activityRecord2 != null && activityRecord2 != activityRecord)
        || (convertToConfigurationOrientation =
                convertToConfigurationOrientation(activityRecord.getRequestedOrientation()))
            == 0) {
      return false;
    }
    this.mTmpRect.set(task.getBounds());
    return ((this.mTmpRect.isEmpty() || this.mTmpRect.width() > this.mTmpRect.height()) ? 2 : 1)
        != convertToConfigurationOrientation;
  }
  ...
  public final int getAppOrientation(Task task, ActivityRecord activityRecord) {
    ...
    if (topMostActivity != null) {
      return topMostActivity.getOrientation() != -2
          ? topMostActivity.getOrientation()
          : topMostActivity.info.screenOrientation;
    }
    if (activityRecord == null) {
      return -1;
    }
  ...
  public void rotateDexCompatTaskLocked(ActivityRecord activityRecord) {
    ...
    if (CoreRune.SAFE_DEBUG) {
      Slog.i(
          "DexCompatController",
          "[DexCompat] rotateCompatTaskLocked: " + task + ", prev=" + bounds + ", new=" + rect);
    }
  }
  ...
  */
  ...
  public void startActivityForDexRestart(Task task) {
    ...
    try {
      this.mAtm.mTaskSupervisor.removeTaskById(
          task.mTaskId, false, true, "startActivityForDexRestart", request.callingUid);
  ...
  public void toggleFreeformForDexCompatApp(Task task) {
    ...
    try {
      this.mAtm.mTaskSupervisor.removeTaskById(
          task.mTaskId, false, true, "toggleFreeformWindowingMode", request.callingUid);
      scheduleStartActivityAsToggleFreeform(request, displayId);
    } finally {
      this.mAtm.continueWindowLayout();
    }
    ...
    synchronized (windowManagerGlobalLock) {
      try {
        ...
        if (displayContent == null) {
          Slog.w(
              "DexCompatController",
              "startActivityAsToggleFreeform: failed, cannot find display#" + i);
  ...
  public final class HandlerC2890H extends Handler {
    ...
    public void handleMessage(Message message) {
      ...
      DexCompatController.this.startActivityAsToggleFreeform(
          (Request) message.obj, message.arg1, message.arg2 != 0);
    }
  }
  ...
  public void dumpLocked(PrintWriter printWriter, String str) {
    ...
    if (dexModeLocked != 0) {
      DisplayContent displayContent =
          this.mAtm.mRootWindowContainer.getDisplayContent(dexModeLocked == 1 ? 0 : 2);
      if (displayContent == null) {
        return;
      }
      displayContent.getStableRect(this.mTmpRect);
      ...
      printWriter.println(
          str
              + "DexCompat isDefaultSizeCompatible="
              + DexCompatBoundsProvider.isDefaultSizeCompatible(min));
      printWriter.println(
          str + "DexCompat DefaultSize=(" + defaultWidth + "x" + defaultHeight + ")");
    }
    printWriter.println();
  }
}
