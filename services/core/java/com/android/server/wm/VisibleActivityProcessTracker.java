  ...
  public boolean hasResumedActivity(int i) {
    return match(
        ...
        new Predicate() { // from class:
                          // com.android.server.wm.VisibleActivityProcessTracker$$ExternalSyntheticLambda0
    ...
    synchronized (this.mProcMap) {
      for (int size = this.mProcMap.size() - 1; size >= 0; size--) {
        WindowProcessController windowProcessController =
            (WindowProcessController) this.mProcMap.keyAt(size);
        if (windowProcessController.mUid == i
            && (predicate == null || predicate.test(windowProcessController))) {
          return true;
        }
      }
      return false;
    ...
    public void run() {
      ...
      if (cpuTime > 0) {
        ActivityManagerInternal activityManagerInternal =
            VisibleActivityProcessTracker.this.mAtms.mAmInternal;
        ApplicationInfo applicationInfo = this.mProc.mInfo;
        activityManagerInternal.updateForegroundTimeIfOnBattery(
            applicationInfo.packageName, applicationInfo.uid, cpuTime);
      }
    }
  }
}
