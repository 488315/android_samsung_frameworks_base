...
public class PinnedTaskController {
  ...
  public final Runnable mDeferOrientationTimeoutRunnable =
      new Runnable() { // from class:
                       // com.android.server.wm.PinnedTaskController$$ExternalSyntheticLambda0
  ...
  public final void reloadResources() {
    ...
    this.mMinAspectRatio = resources.getFloat(R.dimen.config_letterboxHorizontalPositionMultiplier);
    this.mMaxAspectRatio =
        resources.getFloat(R.dimen.config_letterboxDefaultMinAspectRatioForUnresizableApps);
  }
  ...
  public boolean isValidPictureInPictureAspectRatio(float f) {
    return Float.compare(this.mMinAspectRatio, f) <= 0
        && Float.compare(f, this.mMaxAspectRatio) <= 0;
  }
  ...
  public void deferOrientationChangeForEnteringPipFromFullScreenIfNeeded() {
    ...
    ActivityRecord activity =
        this.mDisplayContent.getActivity(
            new Predicate() { // from class:
                              // com.android.server.wm.PinnedTaskController$$ExternalSyntheticLambda1
              @Override // java.util.function.Predicate
              public final boolean test(Object obj) {
                boolean m114xf3c914b3;
                m114xf3c914b3 = PinnedTaskController.m114xf3c914b3((ActivityRecord) obj);
                return m114xf3c914b3;
              }
            });
    if (activity == null
        || activity.hasFixedRotationTransform()
        || (rotationForActivityInDifferentOrientation =
                this.mDisplayContent.rotationForActivityInDifferentOrientation(activity))
            == -1) {
      return;
    }
    this.mDisplayContent.setFixedRotationLaunchingApp(
        activity, rotationForActivityInDifferentOrientation);
    this.mDeferOrientationChanging = true;
    this.mService.f1749mH.removeCallbacks(this.mDeferOrientationTimeoutRunnable);
    this.mService.f1749mH.postDelayed(
        this.mDeferOrientationTimeoutRunnable,
        (int) (Math.max(1.0f, this.mService.getCurrentAnimatorScale()) * 1000.0f));
  }

  /* renamed from: lambda$deferOrientationChangeForEnteringPipFromFullScreenIfNeeded$1 */
  public static /* synthetic */ boolean m114xf3c914b3(ActivityRecord activityRecord) {
  ...
  public void setEnterPipTransaction(
  ...
  public void startSeamlessRotationIfNeeded(SurfaceControl.Transaction transaction, int i, int i2) {
    ...
    boolean z =
        pictureInPictureSurfaceTransaction == null
            || pictureInPictureSurfaceTransaction.mPosition == null;
    if ((rect == null && z)
        || (rootPinnedTask =
                (defaultTaskDisplayArea = this.mDisplayContent.getDefaultTaskDisplayArea())
                    .getRootPinnedTask())
            == null) {
      return;
    }
    ...
    if (!z) {
      ...
      if (pictureInPictureSurfaceTransaction.hasCornerRadiusSet()) {
        transaction.setCornerRadius(
            surfaceControl, pictureInPictureSurfaceTransaction.mCornerRadius);
      }
      Slog.i(
          StartingSurfaceController.TAG,
          "Seamless rotation PiP tx="
    ...
    Slog.i(
        StartingSurfaceController.TAG,
        "Seamless rotation PiP bounds=" + rect + " hintRect=" + rect2);
    ...
    if (rect2 != null && deltaRotation == 3 && rootPinnedTask.getDisplayCutoutInsets() != null) {
      Rect rect3 =
          RotationUtils.rotateInsets(
                  Insets.of(rootPinnedTask.getDisplayCutoutInsets()),
                  RotationUtils.deltaRotation(i2, i))
              .toRect();
  ...
  public boolean isFreezingTaskConfig(Task task) {
    return this.mFreezingTaskConfig
        && task == this.mDisplayContent.getDefaultTaskDisplayArea().getRootPinnedTask();
  }
