...
public final class ContentRecorder implements WindowContainerListener {
  ...
  public ContentRecorder(
      DisplayContent displayContent, MediaProjectionManagerWrapper mediaProjectionManagerWrapper) {
  ...
  public void updateRecording() {
    if (isCurrentlyRecording()
        && (this.mDisplayContent.getLastHasContent()
            || this.mDisplayContent.getDisplayInfo().state == 1)) {
      pauseRecording();
    } else {
  ...
  public void onConfigurationChanged(int i) {
    ...
    if (this.mRecordedWindowContainer == null) {
      if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
        ProtoLogImpl.v(
            ...
            -1156314529,
            1,
            "Content Recording: Unexpectedly null window container; unable to update recording for"
                + " display %d",
            new Object[] {Long.valueOf(this.mDisplayContent.getDisplayId())});
        return;
      }
      return;
    }
    if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
      ProtoLogImpl.v(
          ...
          339482207,
          1,
          "Content Recording: Display %d was already recording, so apply transformations if"
              + " necessary",
          new Object[] {Long.valueOf(this.mDisplayContent.getDisplayId())});
    }
    ...
    if (fetchSurfaceSizeIfPresent != null) {
      if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
        ProtoLogImpl.v(
            ...
            -452750194,
            17,
            "Content Recording: Going ahead with updating recording for display %d to new bounds %s"
                + " and/or orientation %d.",
            new Object[] {
      ...
      updateMirroredSurface(
          this.mRecordedWindowContainer.getSyncTransaction(), bounds, fetchSurfaceSizeIfPresent);
      return;
    }
    if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
      ProtoLogImpl.v(
          ...
          -1480264178,
          17,
          "Content Recording: Unable to update recording for display %d to new bounds %s and/or"
              + " orientation %d, since the surface is not available.",
          new Object[] {
            Long.valueOf(this.mDisplayContent.getDisplayId()),
            String.valueOf(bounds),
            Long.valueOf(orientation)
  ...
  public void pauseRecording() {
    ...
    if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
      ProtoLogImpl.v(
          ...
          new Object[] {
            Long.valueOf(this.mDisplayContent.getDisplayId()),
            Boolean.valueOf(this.mDisplayContent.getLastHasContent())
          });
    }
    ((SurfaceControl.Transaction) this.mDisplayContent.mWmService.mTransactionFactory.get())
        .remove(this.mRecordedSurface)
        .reparent(
            this.mDisplayContent.getWindowingLayer(), this.mDisplayContent.getSurfaceControl())
        .reparent(this.mDisplayContent.getOverlayLayer(), this.mDisplayContent.getSurfaceControl())
        .apply();
    this.mRecordedSurface = null;
  }
  ...
  public final void startRecordingIfNeeded() {
    ...
    if (this.mDisplayContent.getDisplayInfo().state == 1
    ...
    if (contentRecordingSession.isWaitingForConsent()) {
      if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
        ProtoLogImpl.v(
            ...
            -125383273,
            0,
            "Content Recording: waiting to record, so do nothing",
            (Object[]) null);
        return;
      }
      return;
    ...
    if (fetchSurfaceSizeIfPresent == null) {
      if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
        ProtoLogImpl.v(
            ...
            1712935427,
            1,
            "Content Recording: Unable to start recording for display %d since the surface is not"
                + " available.",
            new Object[] {Long.valueOf(this.mDisplayContent.getDisplayId())});
        return;
      }
    ...
    if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
      ProtoLogImpl.v(
          ...
          -1217596375,
          5,
          "Content Recording: Display %d has no content and is on, so start recording for state %d",
          new Object[] {
            Long.valueOf(this.mDisplayContent.getDisplayId()),
            Long.valueOf(this.mDisplayContent.getDisplayInfo().state)
          });
    }
    if (CoreRune.FW_SCREENSHOT_FOR_HDR) {
    ...
    this.mRecordedSurface =
        SurfaceControl.mirrorSurface(this.mRecordedWindowContainer.getSurfaceControl());
    SurfaceControl.Transaction reparent =
        ((SurfaceControl.Transaction) this.mDisplayContent.mWmService.mTransactionFactory.get())
            .reparent(this.mRecordedSurface, this.mDisplayContent.getSurfaceControl())
            .reparent(this.mDisplayContent.getWindowingLayer(), null)
            .reparent(this.mDisplayContent.getOverlayLayer(), null);
    updateMirroredSurface(
        reparent, this.mRecordedWindowContainer.getBounds(), fetchSurfaceSizeIfPresent);
    reparent.apply();
    if (this.mContentRecordingSession.getContentToRecord() == 1) {
      this.mMediaProjectionManager.notifyActiveProjectionCapturedContentVisibilityChanged(
          this.mRecordedWindowContainer.asTask().isVisibleRequested());
    } else {
      this.mMediaProjectionManager.notifyActiveProjectionCapturedContentVisibilityChanged(
          this.mRecordedWindowContainer.asDisplayContent().getDisplayInfo().state != 1);
    }
  }
  ...
  public final WindowContainer retrieveRecordedWindowContainer() {
    ...
    if (contentToRecord == 0) {
      DisplayContent displayContent =
          this.mDisplayContent.mWmService.mRoot.getDisplayContent(
              this.mContentRecordingSession.getDisplayToRecord());
      if (displayContent != null) {
        return displayContent;
      }
      DisplayContent displayContent2 = this.mDisplayContent;
      displayContent2.mWmService.mDisplayManagerInternal.setWindowManagerMirroring(
          displayContent2.getDisplayId(), false);
      ...
      if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
        ProtoLogImpl.v(
            ...
            -732715767,
            1,
            "Unable to retrieve window container to start recording for display %d",
            new Object[] {Long.valueOf(this.mDisplayContent.getDisplayId())});
      }
      return null;
    }
      ...
      if (asTask == null) {
        ...
        if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
          ProtoLogImpl.v(
              ...
              -2074882083,
              1,
              "Content Recording: Unable to retrieve task to start recording for display %d",
              new Object[] {Long.valueOf(this.mDisplayContent.getDisplayId())});
        }
      } else {
    ...
    if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
      ProtoLogImpl.v(
          ...
          -869242375,
          1,
          "Content Recording: Unable to start recording due to invalid region for display %d",
          new Object[] {Long.valueOf(this.mDisplayContent.getDisplayId())});
    }
    return null;
  }
  ...
  public void updateMirroredSurface(
    ...
    transaction
        ...
        .setMatrix(
            ...
            DisplayPowerController2.RATE_FROM_DOZE_TO_ON,
            min)
        .setPosition(this.mRecordedSurface, i2, i4);
  ...
  public final Point fetchSurfaceSizeIfPresent() {
    ...
    ProtoLogImpl.v(
        ...
        1750878635,
        1,
        "Content Recording: Provided surface for recording on display %d is not present, so do not"
            + " update the surface",
        new Object[] {Long.valueOf(this.mDisplayContent.getDisplayId())});
    return null;
  }
      ...
      try {
        ...
        if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
          ProtoLogImpl.e(
              ...
              -88873335,
              0,
              "Content Recording: Unable to tell MediaProjectionManagerService to stop the active"
                  + " projection: %s",
      ...
      try {
        ...
        if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
          ProtoLogImpl.e(
              ...
              1661414284,
              0,
              "Content Recording: Unable to tell MediaProjectionManagerService about resizing the"
                  + " active projection: %s",
              new Object[] {String.valueOf(e)});
        }
      }
      ...
      try {
        ...
        if (ProtoLogCache.WM_DEBUG_CONTENT_RECORDING_enabled) {
          ProtoLogImpl.e(
              ...
              -180594244,
              0,
              "Content Recording: Unable to tell MediaProjectionManagerService about visibility"
                  + " change on the active projection: %s",
              new Object[] {String.valueOf(e)});
        }
      }
    }
  ...
  public /* synthetic */ void lambda$invalidateForRecording$0(
