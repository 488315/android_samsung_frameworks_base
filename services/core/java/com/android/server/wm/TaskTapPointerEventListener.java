    ...
    if (actionMasked == 0) {
      ...
      synchronized (this) {
        if (this.mDisplayContent.isDexMode()
            && (motionEvent.getFlags() & 268435456) != 0
            && (motionEvent.getButtonState() & 1) == 0) {
          return;
        }
        if (!this.mTouchExcludeRegion.contains(x, i2)) {
          this.mService.mTaskPositioningController.handleTapOutsideTask(
              this.mDisplayContent, x, i2);
        }
        return;
      }
    }
            ...
            if (x2 < rect.left) {
              ...
              if (this.mPointerIconType != i) {
                ...
                if (i == 1) {
                  ...
                  this.mService
                      .f1749mH
                      .obtainMessage(55, x2, y2, this.mDisplayContent)
                      .sendToTarget();
                } else {
