...
public class FreeformTaskPinningController {
  ...
  public void startPinning(Task task) {
    ...
    if (hasTaskPinned()
        || (CoreRune.MT_NEW_DEX_TASK_PINNING
            && task.getConfiguration().windowConfiguration.isAlwaysOnTop())) {
      Slog.d(
          "FreeformTaskPinningController", "Failed to start freeform task pinning, already pinned");
      return;
    }
    if (!task.inFreeformWindowingMode()) {
      Slog.d(
          "FreeformTaskPinningController",
          "Failed to start freeform task pinning, task isn't in freeform.");
      return;
    }
    if (!this.mTaskDisplayArea.isDesktopModeEnabled()) {
      Slog.d(
          "FreeformTaskPinningController",
          "Failed to start freeform task pinning, it's not in dex mode.");
      return;
    }
    if (DEBUG) {
    ...
    if (!task.isForceHidden()) {
      ...
      if (CoreRune.MT_NEW_DEX_TASK_PINNING
          && (displayContent = this.mTaskDisplayArea.getDisplayContent()) != null) {
  ...
  public void stopPinning(Task task, boolean z, String str) {
    if (this.mPinnedTask != task
        && CoreRune.MT_NEW_DEX_TASK_PINNING
        && !this.mTaskDisplayArea.isNewDexMode()) {
      Slog.d(
          "FreeformTaskPinningController",
    ...
    if (DEBUG) {
      Slog.d(
          "FreeformTaskPinningController",
          "stopFreeformTaskPinning : taskId #" + task.mTaskId + ", reason : " + str);
    }
    if (CoreRune.MT_NEW_DEX_TASK_PINNING && !this.mTaskDisplayArea.isNewDexMode()) {
  ...
  public boolean hasTaskPinned() {
    return ((CoreRune.MT_NEW_DEX_TASK_PINNING && this.mTaskDisplayArea.isNewDexMode())
            || this.mPinnedTask == null)
        ? false
        : true;
  }
