...
public class CoreStateController implements CoreStateObserverController.Callback {
  ...
  public void initializeLocked() {
    ...
    this.f1736mH = new Handler(this.mAtm.f1734mH.getLooper());
    CoreStateObserverController coreStateObserverController =
        new CoreStateObserverController(this.mAtm.mContext, this, this.f1736mH);
  ...
  public final void notifyCoreStatesChangedLocked(int i, Runnable runnable) {
    ...
    for (int size = pidMap.size() - 1; size >= 0; size--) {
      ...
      if (windowProcessController.getPid() != ActivityManagerService.MY_PID
          && (windowProcessController.mUserId == i
              || (z && isPrimarySystemUIProcess(windowProcessController)))) {
        try {
          IApplicationThread thread = windowProcessController.getThread();
          if (thread != null) {
            if (DEBUG) {
              Slog.d(
                  TAG, "notifyCoreStatesChangedLocked: schedule CoreStateChangeItem, pid=" + keyAt);
            }
            ClientTransaction.obtain(thread, (IBinder) null)
                .addCallback(CoreStatesChangeItem.obtain(coreStates));
            this.mAtm
                .getLifecycleManager()
                .scheduleTransaction(
                    thread,
                    (IBinder) null,
                    (ClientTransactionItem) CoreStatesChangeItem.obtain(coreStates));
          }
  ...
  public static boolean isPrimarySystemUIProcess(WindowProcessController windowProcessController) {
    return windowProcessController.mUserId == 0
        && "com.android.systemui".equals(windowProcessController.mName);
  }
  ...
  public void setVolatileState(
      String str, Object obj, int i, boolean z, boolean z2, Runnable runnable) {
    WindowManagerGlobalLock windowManagerGlobalLock = this.mAtm.mGlobalLock;
    WindowManagerService.boostPriorityForLockedSection();
    synchronized (windowManagerGlobalLock) {
      try {
