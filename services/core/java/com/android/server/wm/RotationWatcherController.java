...
public class RotationWatcherController {
    ...
    for (int size = this.mProposedRotationListeners.size() - 1; size >= 0; size--) {
      ...
      if (iBinder == proposedRotationListener.mToken
        ...
        Slog.w(
            ...
            "Register rotation listener to a registered token, uid=" + Binder.getCallingUid());
        return;
      }
    }
    register(
        ...
        this.mProposedRotationListeners);
    this.mHasProposedRotationListeners = !this.mProposedRotationListeners.isEmpty();
  }

  public static void register(
      IBinder iBinder, RotationWatcher rotationWatcher, ArrayList arrayList) {
    try {
  ...
  public void dispatchProposedRotation(DisplayContent displayContent, int i) {
    for (int size = this.mProposedRotationListeners.size() - 1; size >= 0; size--) {
      ProposedRotationListener proposedRotationListener =
          (ProposedRotationListener) this.mProposedRotationListeners.get(size);
      WindowContainer associatedWindowContainer =
          getAssociatedWindowContainer(proposedRotationListener.mToken);
  ...
  public WindowContainer getAssociatedWindowContainer(IBinder iBinder) {
    ...
    return forTokenLocked != null
        ? forTokenLocked
        : this.mService.mWindowContextListenerController.getContainer(iBinder);
  }
  ...
  public void dump(PrintWriter printWriter) {
    ...
    for (int size2 = this.mProposedRotationListeners.size() - 1; size2 >= 0; size2--) {
      printWriter.print(' ');
      ProposedRotationListener proposedRotationListener =
          (ProposedRotationListener) this.mProposedRotationListeners.get(size2);
  ...
  public class DisplayRotationWatcher extends RotationWatcher {
    ...
    public DisplayRotationWatcher(
        WindowManagerService windowManagerService, IRotationWatcher iRotationWatcher, int i) {
