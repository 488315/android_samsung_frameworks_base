...
public final class SnapshotStartingData extends StartingData {
    ...
    public SnapshotStartingData(
            WindowManagerService windowManagerService, TaskSnapshot taskSnapshot, int i) {
    ...
    public final StartingSurfaceController.StartingSurface createStartingSurface(
        ...
        if (task == null) {
            Slog.w(
                    "WindowManager",
                    "TaskSnapshotSurface.create: Failed to find task for activity="
                            + activityRecord);
            return null;
        }
        if (task.getWindow(new Task$$ExternalSyntheticLambda0(7)) == null) {
            Slog.w(
                    "WindowManager",
                    "TaskSnapshotSurface.create: no main window in " + activityRecord);
            return null;
        }
        if (activityRecord.mDisplayContent.mDisplayRotation.mRotation
                != taskSnapshot.getRotation()) {
            activityRecord.mDisplayContent.handleTopActivityLaunchingInDifferentOrientation(
                    activityRecord, activityRecord, false);
        }
