...
public abstract class ForceLaunchWindowingModeUtils {
  public static boolean shouldApplyForceLaunchWindowingMode(
      ActivityOptions activityOptions, ActivityRecord activityRecord, Task task) {
    int resolveForceLaunchWindowingMode =
        resolveForceLaunchWindowingMode(activityOptions, activityRecord, task);
    return resolveForceLaunchWindowingMode != 0
        && (task == null || task.getWindowingMode() != resolveForceLaunchWindowingMode);
  }

  public static boolean shouldDismissSplitBeforeLaunch(
      ActivityOptions activityOptions, ActivityRecord activityRecord) {
  ...
  public static int resolveForceLaunchWindowingMode(
    ...
    if (activityRecord != null) {
      if (activityRecord.supportsMultiWindowInDefaultDisplayArea()
          && activityRecord.isActivityTypeStandardOrUndefined()) {
        return activityOptions.getForceLaunchWindowingMode();
      }
      return (activityOptions.getForceLaunchWindowingMode() != 1 || activityRecord.isResizeable())
          ? 0
          : 1;
    }
    if (task != null
