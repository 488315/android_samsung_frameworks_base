    ...
    public /* synthetic */ void lambda$finishAnimation$0(int i, boolean z, RecentsAnimationController recentsAnimationController) {
        ...
        try {
            try {
                ...
                if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                    ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 1781673113, 0, "onAnimationFinished(): targetRootTask=%s targetActivity=%s mRestoreTargetBehindRootTask=%s", new Object[]{String.valueOf(rootTask), String.valueOf(isInTask), String.valueOf(this.mRestoreTargetBehindRootTask)});
                }
                if (isInTask != null) {
                    ...
                    if (i == 1) {
                        ...
                        if (ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS.isLogToAny() && (topNonAlwaysOnTopRootTask = getTopNonAlwaysOnTopRootTask()) != rootTask && ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                            ProtoLogImpl.w(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, -302468788, 0, "Expected target rootTask=%s to be top most but found rootTask=%s", new Object[]{String.valueOf(rootTask), String.valueOf(topNonAlwaysOnTopRootTask)});
                        }
                    } else if (i == 2) {
                        ...
                        if (ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS.isLogToAny()) {
                            ...
                            if (task2 != null && rootTaskAbove != task2 && ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                                ProtoLogImpl.w(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 1822314934, 0, "Expected target rootTask=%s to restored behind rootTask=%s but it is behind rootTask=%s", new Object[]{String.valueOf(rootTask), String.valueOf(this.mRestoreTargetBehindRootTask), String.valueOf(rootTaskAbove)});
                            }
                        }
                        if (CoreRune.FW_BOUNDS_COMPAT_UI && (task = this.mRestoreTargetBehindRootTask) != null) {
                            task.dispatchTaskInfoChangedByBoundsCompat();
