...
public final class DefaultImeVisibilityApplier implements ImeVisibilityApplier {
  ...
  public final ImeTargetVisibilityPolicy mImeTargetVisibilityPolicy =
      (ImeTargetVisibilityPolicy) LocalServices.getService(ImeTargetVisibilityPolicy.class);
    ...
    if (curMethodLocked != null) {
      Slog.v(
          ...
          "Calling "
              ...
              + resultReceiver
              + ") for reason: "
              + InputMethodDebug.softInputDisplayReasonToString(i2));
      if (curMethodLocked.showSoftInput(iBinder, token, i, resultReceiver)) {
        if (ImeTracker.DEBUG_IME_VISIBILITY) {
          EventLog.writeEvent(
              32001,
              token.getTag(),
              Objects.toString(this.mService.mCurFocusedWindow),
              InputMethodDebug.softInputDisplayReasonToString(i2),
              InputMethodDebug.softInputModeToString(this.mService.mCurFocusedWindowSoftInputMode));
        }
  ...
  public void performHideIme(
      IBinder iBinder, ImeTracker.Token token, ResultReceiver resultReceiver, int i) {
    IInputMethodInvoker curMethodLocked = this.mService.getCurMethodLocked();
    if (curMethodLocked != null) {
      Slog.v(
          "InputMethodManagerService",
          "Calling "
              ...
              + resultReceiver
              + ") for reason: "
              + InputMethodDebug.softInputDisplayReasonToString(i));
  ...
  public void applyImeVisibility(IBinder iBinder, ImeTracker.Token token, int i, int i2) {
    ...
    if (i == 0) {
      if (this.mService.hasAttachedClient()) {
        ...
        this.mWindowManagerInternal.hideIme(
            iBinder, this.mService.getDisplayIdToShowImeLocked(), token);
        return;
      } else {
  ...
  public boolean removeImeScreenshot(int i) {
    ...
    inputMethodManagerService.onShowHideSoftInputRequested(
        false, inputMethodManagerService.mCurFocusedWindow, 35, null);
    return true;
  }
}
