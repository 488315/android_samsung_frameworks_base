    ...
    while (true) {
        ...
        if (str != null) {
          ...
          if (parseZoneIds == null) {
            geolocationTimeZoneSuggestion =
                GeolocationTimeZoneSuggestion.createUncertainSuggestion(elapsedRealtime);
          } else {
            geolocationTimeZoneSuggestion =
                GeolocationTimeZoneSuggestion.createCertainSuggestion(
                    elapsedRealtime, parseZoneIds);
          }
        }
        LocationAlgorithmEvent locationAlgorithmEvent =
            new LocationAlgorithmEvent(
                locationTimeZoneAlgorithmStatus, geolocationTimeZoneSuggestion);
      ...
      if (nextArg.equals("--suggestion")) {
        ...
        locationTimeZoneAlgorithmStatus =
            LocationTimeZoneAlgorithmStatus.parseCommandlineArg(shellCommand.getNextArgRequired());
      } else {
