...
public class RealLocationTimeZoneProviderProxy extends LocationTimeZoneProviderProxy
  ...
  public RealLocationTimeZoneProviderProxy(
      Context context,
      Handler handler,
    ...
    if (z) {
      create =
          CurrentUserServiceSupplier.createUnsafeForTestsOnly(
              context, str, str2, "android.permission.BIND_TIME_ZONE_PROVIDER_SERVICE", null);
    } else {
      create =
          CurrentUserServiceSupplier.create(
              context,
    ...
    this.mServiceWatcher =
        ServiceWatcher.create(context, handler, "RealLocationTimeZoneProviderProxy", create, this);
  }
    ...
    this.mServiceWatcher.runOnBinder(
        new ServiceWatcher
            .BinderOperation() { // from class:
                                 // com.android.server.timezonedetector.location.RealLocationTimeZoneProviderProxy$$ExternalSyntheticLambda0
          @Override // com.android.server.servicewatcher.ServiceWatcher.BinderOperation
          public final void run(IBinder iBinder) {
            RealLocationTimeZoneProviderProxy.lambda$trySendCurrentRequest$0(
                TimeZoneProviderRequest.this, managerProxy, iBinder);
          }
        });
  }

  public static /* synthetic */ void lambda$trySendCurrentRequest$0(
      TimeZoneProviderRequest timeZoneProviderRequest, ManagerProxy managerProxy, IBinder iBinder) {
    ...
    if (timeZoneProviderRequest.sendUpdates()) {
      asInterface.startUpdates(
          managerProxy,
          timeZoneProviderRequest.getInitializationTimeout().toMillis(),
          timeZoneProviderRequest.getEventFilteringAgeThreshold().toMillis());
    } else {
