...
public class LocationTimeZoneProviderController implements Dumpable {
  ...
  public void initialize(Environment environment, Callback callback) {
    ...
    synchronized (this.mSharedLock) {
      ...
      LocationTimeZoneProvider.ProviderListener providerListener =
          new LocationTimeZoneProvider
              .ProviderListener() { // from class:
                                    // com.android.server.timezonedetector.location.LocationTimeZoneProviderController$$ExternalSyntheticLambda0
            @Override // com.android.server.timezonedetector.location.LocationTimeZoneProvider.ProviderListener
            public final void onProviderStateChange(
  ...
  public void onConfigurationInternalChanged() {
    ...
    synchronized (this.mSharedLock) {
      ...
      ConfigurationInternal currentUserConfigurationInternal =
          this.mEnvironment.getCurrentUserConfigurationInternal();
      ...
      if (!currentUserConfigurationInternal.equals(configurationInternal)) {
        if (currentUserConfigurationInternal.getUserId() != configurationInternal.getUserId()) {
          String str =
              "User changed. old="
                  + configurationInternal.getUserId()
                  + ", new="
                  + currentUserConfigurationInternal.getUserId();
  ...
  public final void reportSuggestionEvent(
    ...
    LocationAlgorithmEvent locationAlgorithmEvent =
        new LocationAlgorithmEvent(generateCurrentAlgorithmStatus(), geolocationTimeZoneSuggestion);
    ...
    switch (locationTimeZoneProvider.getCurrentState().stateEnum) {
      ...
      default:
        LocationTimeZoneManagerService.warnLog(
            "Unknown provider state: " + locationTimeZoneProvider);
        break;
    }
  }

  public final void alterProvidersStartedStateIfRequired(
      ConfigurationInternal configurationInternal, ConfigurationInternal configurationInternal2) {
    boolean z =
        configurationInternal != null && configurationInternal.isGeoDetectionExecutionEnabled();
  ...
  public final void tryStartProvider(
    ...
    switch (locationTimeZoneProvider.getCurrentState().stateEnum) {
      ...
      case 4:
        ...
        locationTimeZoneProvider.startUpdates(
            configurationInternal,
            this.mEnvironment.getProviderInitializationTimeout(),
            this.mEnvironment.getProviderInitializationTimeoutFuzz(),
            this.mEnvironment.getProviderEventFilteringAgeThreshold());
        return;
      case 5:
      case 6:
        LocationTimeZoneManagerService.debugLog(
  ...
  public void onProviderStateChange(LocationTimeZoneProvider.ProviderState providerState) {
    ...
    synchronized (this.mSharedLock) {
      if (Objects.equals(this.mState.get(), "PROVIDERS_INITIALIZING")) {
        LocationTimeZoneManagerService.warnLog(
            "onProviderStateChange: Ignoring provider state change because both providers have not"
                + " yet completed initialization. providerState="
                + providerState);
        return;
      }
      switch (providerState.stateEnum) {
        ...
        case 6:
          LocationTimeZoneManagerService.warnLog(
              "onProviderStateChange: Unexpected state change for provider, provider="
                  + locationTimeZoneProvider);
          break;
        case 2:
        case 3:
          LocationTimeZoneManagerService.debugLog(
              "onProviderStateChange: Received notification of a state change while started,"
                  + " provider="
                  + locationTimeZoneProvider);
        ...
        default:
          LocationTimeZoneManagerService.warnLog(
              "onProviderStateChange: Unexpected provider=" + locationTimeZoneProvider);
          break;
      }
    }
  }
  ...
  public final void handleProviderFailedStateChange(
    ...
    } else if (locationTimeZoneProvider == this.mSecondaryProvider
        && currentState.stateEnum != 3
        && !currentState.isTerminated()) {
      LocationTimeZoneManagerService.warnLog(
          "Secondary provider unexpected reported a failure: failed provider="
              + locationTimeZoneProvider.getName()
              + ", primary provider="
              + this.mPrimaryProvider
              + ", secondary provider="
              + this.mSecondaryProvider);
    }
  ...
  public final void handleProviderStartedStateChange(
    ...
    if (timeZoneProviderEvent == null) {
      handleProviderUncertainty(
          locationTimeZoneProvider,
          this.mEnvironment.elapsedRealtimeMillis(),
  ...
  public final void handleProviderSuggestion(
      LocationTimeZoneProvider locationTimeZoneProvider,
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter, String[] strArr) {
    synchronized (this.mSharedLock) {
      ...
      indentingPrintWriter.println(
          "providerInitializationTimeout=" + this.mEnvironment.getProviderInitializationTimeout());
      indentingPrintWriter.println(
  ...
  public void handleProviderUncertainty(
      ...
      this.mUncertaintyTimeoutQueue.runDelayed(
          new Runnable() { // from class:
            ...
            public final void run() {
              LocationTimeZoneProviderController.this.lambda$handleProviderUncertainty$0(
                  locationTimeZoneProvider, j, uncertaintyDelay);
            }
          },
  ...
  public final void lambda$handleProviderUncertainty$0(
      LocationTimeZoneProvider locationTimeZoneProvider, long j, Duration duration) {
    this.mThreadingDomain.assertCurrentThread();
    synchronized (this.mSharedLock) {
      long elapsedRealtimeMillis = this.mEnvironment.elapsedRealtimeMillis();
      setState("UNCERTAIN");
      reportSuggestionEvent(
          GeolocationTimeZoneSuggestion.createUncertainSuggestion(j),
          "Uncertainty timeout triggered for "
              ...
              + Duration.ofMillis(elapsedRealtimeMillis)
              + ", uncertaintyDelay="
              + duration);
    }
  }
  ...
  public final LocationTimeZoneAlgorithmStatus generateCurrentAlgorithmStatus() {
    return createAlgorithmStatus(
        (String) this.mState.get(),
        this.mPrimaryProvider.getCurrentState(),
        this.mSecondaryProvider.getCurrentState());
  }

  public static LocationTimeZoneAlgorithmStatus createAlgorithmStatus(
      String str,
      LocationTimeZoneProvider.ProviderState providerState,
      LocationTimeZoneProvider.ProviderState providerState2) {
    return new LocationTimeZoneAlgorithmStatus(
        mapControllerStateToDetectionAlgorithmStatus(str),
        providerState.getProviderStatus(),
        providerState.getReportedStatus(),
        providerState2.getProviderStatus(),
        providerState2.getReportedStatus());
  }
  ...
  public LocationTimeZoneManagerServiceState getStateForTests() {
    ...
    synchronized (this.mSharedLock) {
      ...
      builder
          .setControllerState((String) this.mState.get())
          .setStateChanges(this.mRecordedStates)
          .setPrimaryProviderStateChanges(this.mPrimaryProvider.getRecordedStates())
          .setSecondaryProviderStateChanges(this.mSecondaryProvider.getRecordedStates());
      build = builder.build();
    }
    return build;
  }
