...
public class LocationTimeZoneManagerShellCommand extends ShellCommand {
  ...
  public void onHelp() {
    ...
    outPrintWriter.printf(
        "    Starts the service with test provider packages configured / provider permission checks"
            + " disabled.\n",
        new Object[0]);
    outPrintWriter.printf(
    ...
    outPrintWriter.printf(
        "    Sets the amount of time the service waits when uncertain before making an 'uncertain'"
            + " suggestion to the time zone detector.\n",
        new Object[0]);
    ...
    outPrintWriter.printf(
        "    Sets the amount of time that must pass between equivalent LTZP events before they will"
            + " be reported to the system server.\n",
        new Object[0]);
    outPrintWriter.println();
    outPrintWriter.printf(
        "Typically, use '%s' to stop the service before setting individual flags and '%s' after to"
            + " restart it.\n",
        "stop", "start");
    outPrintWriter.println();
    outPrintWriter.printf(
  ...
  public final int runStartWithTestProviders() {
    try {
      this.mService.startWithTestProviders(
          parseProviderPackageName(getNextArgRequired()),
          parseProviderPackageName(getNextArgRequired()),
    ...
    try {
      ...
      if ("--proto".equals(getNextOption())) {
        dualDumpOutputStream =
            new DualDumpOutputStream(new ProtoOutputStream(getOutFileDescriptor()));
      } else {
        dualDumpOutputStream =
      ...
      writeProviderStates(
          dualDumpOutputStream,
          stateForTests.getPrimaryProviderStates(),
          "primary_provider_states",
          2246267895810L);
      writeProviderStates(
          dualDumpOutputStream,
          stateForTests.getSecondaryProviderStates(),
  ...
  public static void writeControllerStates(DualDumpOutputStream dualDumpOutputStream, List list) {
    ...
    while (it.hasNext()) {
      dualDumpOutputStream.write(
          "controller_states",
          2259152797700L,
          convertControllerStateToProtoEnum((String) it.next()));
    }
  }
