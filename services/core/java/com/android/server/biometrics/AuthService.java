...
public class AuthService extends SystemService {
  ...
  public final class AuthServiceImpl extends IAuthService.Stub {
    ...
    public String getUiPackage() {
      ...
      return AuthService.this
          .getContext()
          .getResources()
          .getString(R.string.config_wwan_data_service_package);
    }
    ...
    public long authenticate(
      ...
      if (i == callingUserId) {
        ...
        Slog.w(
            "AuthService",
            "User " + callingUserId + " is requesting authentication of userid: " + i);
      ...
      if (iBinder == null
        ...
        authenticateFastFail(
            "Unable to authenticate, one or more null arguments", iBiometricServiceReceiver);
        return -1L;
      }
      ...
      if (promptInfo.containsTestConfigurations()
          && AuthService.this
                  .getContext()
                  .checkCallingOrSelfPermission("android.permission.TEST_BIOMETRIC")
              != 0) {
        AuthService.this.checkInternalPermission();
      }
      ...
      try {
        VirtualDeviceManagerInternal virtualDeviceManagerInternal =
            (VirtualDeviceManagerInternal)
                AuthService.this.getLocalService(VirtualDeviceManagerInternal.class);
        if (virtualDeviceManagerInternal != null) {
          virtualDeviceManagerInternal.onAuthenticationPrompt(callingUid);
        }
        return AuthService.this.mBiometricService.authenticate(
    ...
    public final void authenticateFastFail(
    ...
    public int canAuthenticate(String str, int i, int i2) {
      ...
      try {
        int canAuthenticate =
            AuthService.this.mBiometricService.canAuthenticate(str, i, callingUserId, i2);
        Slog.d(
            "AuthService",
    ...
    public CharSequence getButtonLabel(int i, String str, int i2) {
      ...
      long clearCallingIdentity = Binder.clearCallingIdentity();
      try {
        int credentialBackupModality =
            AuthService.getCredentialBackupModality(
                AuthService.this.mBiometricService.getCurrentModality(str, i, callingUserId, i2));
        if (credentialBackupModality == 0) {
          ...
          str2 = AuthService.this.getContext().getString(R.string.launchBrowserDefault);
        } else if (credentialBackupModality == 8) {
          str2 =
              AuthService.this
                  .getContext()
                  .getString(R.string.keyguard_accessibility_unlock_area_expanded);
        } else {
          str2 = AuthService.this.getContext().getString(R.string.config_defaultDndDeniedPackages);
        }
        return str2;
      } finally {
        Binder.restoreCallingIdentity(clearCallingIdentity);
          ...
          if (credentialBackupModality != 8) {
            if (isCredentialRequested) {
              str2 =
                  AuthService.this
                      .getContext()
                      .getString(R.string.config_defaultNetworkRecommendationProviderPackage);
            } else {
              str2 =
                  AuthService.this
                      .getContext()
            ...
            str2 = AuthService.this.getContext().getString(R.string.kg_failed_attempts_now_wiping);
          } else {
            str2 =
                AuthService.this
                    .getContext()
                    .getString(R.string.keyguard_accessibility_widget_changed);
          }
        } else if (isCredentialRequested) {
          str2 = AuthService.this.getContext().getString(R.string.lockscreen_access_pattern_start);
        } else {
          str2 = AuthService.this.getContext().getString(R.string.launch_warning_replace);
        }
        return str2;
      } finally {
        Binder.restoreCallingIdentity(clearCallingIdentity);
        ...
        if (supportedModalities == 0) {
          ...
          if (i3 == 2) {
            str2 =
                AuthService.this
                    .getContext()
                    .getString(R.string.lockscreen_access_pattern_detected);
          } else if (i3 == 8) {
            str2 =
                AuthService.this.getContext().getString(R.string.kg_failed_attempts_almost_at_wipe);
          } else {
            str2 =
                AuthService.this
                    .getContext()
                    .getString(R.string.config_defaultNearbySharingSliceUri);
          }
        }
        return str2;
      } finally {
    ...
    if (this.mInjector.isHidlDisabled(getContext())) {
      ...
      int i =
          SystemProperties.getInt(
              "ro.board.api_level", SystemProperties.getInt("ro.board.first_api_level", 0));
  ...
  public final void registerAuthenticators(SensorConfig[] sensorConfigArr) {
    ...
    if (sensorConfigArr != null) {
      for (SensorConfig sensorConfig : sensorConfigArr) {
        Slog.d(
            "AuthService",
            "Registering HIDL ID: "
                + sensorConfig.f1652id
                + " Modality: "
                + sensorConfig.modality
                + " Strength: "
                + sensorConfig.strength);
        int i = sensorConfig.modality;
        if (i == 2) {
          arrayList.add(getHidlFingerprintSensorProps(sensorConfig.f1652id, sensorConfig.strength));
        } else if (i == 4) {
          arrayList3.add(getHidlIrisSensorProps(sensorConfig.f1652id, sensorConfig.strength));
        } else if (i == 8) {
          arrayList2.add(getHidlFaceSensorProps(sensorConfig.f1652id, sensorConfig.strength));
        } else {
    ...
    if (fingerprintService != null) {
      ...
      Slog.e(
          "AuthService", "HIDL fingerprint configuration exists, but FingerprintService is null.");
    }
  ...
  public final void checkPermission() {
    if (getContext().checkCallingOrSelfPermission("android.permission.USE_FINGERPRINT") != 0) {
      getContext()
          .enforceCallingOrSelfPermission(
              "android.permission.USE_BIOMETRIC", "Must have USE_BIOMETRIC permission");
    }
  }
  ...
  public final FingerprintSensorPropertiesInternal getHidlFingerprintSensorProps(int i, int i2) {
    return new FingerprintSensorPropertiesInternal(
        i,
        Utils.authenticatorStrengthToPropertyStrength(i2),
        FingerprintUtils.semGetMaxTemplateNumberFromSPF(),
        new ArrayList(),
        FingerprintUtils.semGetSensorType(),
        false);
  }

  public final FaceSensorPropertiesInternal getHidlFaceSensorProps(int i, int i2) {
    boolean z = getContext().getResources().getBoolean(R.bool.config_enableNightMode);
    return new FaceSensorPropertiesInternal(
        i,
        Utils.authenticatorStrengthToPropertyStrength(i2),
        getContext().getResources().getInteger(R.integer.config_letterboxActivityCornersRadius),
        new ArrayList(),
        0,
        false,
        z,
  ...
  public final SensorPropertiesInternal getHidlIrisSensorProps(int i, int i2) {
    return new SensorPropertiesInternal(
        i, Utils.authenticatorStrengthToPropertyStrength(i2), 1, new ArrayList(), false, false);
  }
