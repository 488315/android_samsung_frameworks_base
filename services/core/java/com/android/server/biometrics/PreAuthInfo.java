...
public class PreAuthInfo {
  ...
  public static PreAuthInfo create(
      ...
      boolean z,
      Context context) {
    int sepBiometricPromptTypeToBiometricAuthenticatorModality =
        Utils.sepBiometricPromptTypeToBiometricAuthenticatorModality(
            promptInfo.semGetBiometricType());
    if (sepBiometricPromptTypeToBiometricAuthenticatorModality != 0) {
      promptInfo.setAuthenticators(
    ...
    Slog.d(
        ...
        "create: "
            ...
            + ", "
            + Integer.toHexString(publicBiometricStrength)
            + ", "
            + sepBiometricPromptTypeToBiometricAuthenticatorModality);
    ...
    if (isBiometricRequested) {
      ...
      while (it.hasNext()) {
        ...
        if (sepBiometricPromptTypeToBiometricAuthenticatorModality == 0
            || (biometricSensor.modality & sepBiometricPromptTypeToBiometricAuthenticatorModality)
                != 0) {
          int i2 = sepBiometricPromptTypeToBiometricAuthenticatorModality;
          ArrayList arrayList3 = arrayList2;
          int statusForBiometricAuthenticator =
          ...
          Slog.d(
              ...
              "Package: "
                  ...
                  + " Modality: "
                  + biometricSensor.modality
                  + " Status: "
                  + statusForBiometricAuthenticator);
          if (statusForBiometricAuthenticator == 1 || statusForBiometricAuthenticator == 12) {
            arrayList.add(biometricSensor);
          } else {
            arrayList3.add(
                new Pair(biometricSensor, Integer.valueOf(statusForBiometricAuthenticator)));
          }
  ...
  public static int getStatusForBiometricAuthenticator(
      DevicePolicyManager devicePolicyManager,
      BiometricService.SettingObserver settingObserver,
    ...
    if (sensorPrivacyManager != null
        && biometricSensor.modality == 8
        && sensorPrivacyManager.isSensorPrivacyEnabled(2, i)) {
      return 12;
    }
  ...
  public static boolean isEnabledForApp(
      BiometricService.SettingObserver settingObserver, int i, int i2) {
    return settingObserver.getEnabledForApps(i2);
  }

  public static boolean isBiometricDisabledByDevicePolicy(
      DevicePolicyManager devicePolicyManager, int i, int i2) {
    ...
    boolean z =
        (devicePolicyManager.getKeyguardDisabledFeatures(null, i2) & mapModalityToDevicePolicyType)
            != 0;
    Slog.w(
        "BiometricService/PreAuthInfo",
        "isBiometricDisabledByDevicePolicy(" + i + "," + i2 + ")=" + z);
    return z;
  }
  ...
  */
  public final Pair getInternalStatus() {
    SensorPrivacyManager sensorPrivacyManager =
        (SensorPrivacyManager) this.context.getSystemService(SensorPrivacyManager.class);
    int i = 2;
    int i2 = 0;
    boolean isSensorPrivacyEnabled =
        sensorPrivacyManager != null
            ? sensorPrivacyManager.isSensorPrivacyEnabled(2, this.userId)
            : false;
    ...
    Slog.d(
        "BiometricService/PreAuthInfo",
        "getCanAuthenticateInternal Modality: " + i2 + " AuthenticatorStatus: " + i);
  ...
  public int getCanAuthenticateResult() {
    return Utils.biometricConstantsToBiometricManager(
        Utils.authenticatorStatusToBiometricConstant(
            ((Integer) getInternalStatus().second).intValue()));
  }

  public Pair getPreAuthenticateStatus() {
    Pair internalStatus = getInternalStatus();
    int authenticatorStatusToBiometricConstant =
        Utils.authenticatorStatusToBiometricConstant(((Integer) internalStatus.second).intValue());
