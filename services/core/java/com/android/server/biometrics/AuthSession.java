...
public final class AuthSession implements IBinder.DeathRecipient {
    ...
    public final void setSensorsToStateUnknown() {
        for (BiometricSensor biometricSensor : this.mPreAuthInfo.eligibleSensors) {
            Slog.v("BiometricService/AuthSession", "set to unknown state sensor: " + biometricSensor.f1650id);
            ...
            if (this.mState != 5) {
                try {
                    ...
                    for (int i2 = 0; i2 < this.mPreAuthInfo.eligibleSensors.size(); i2++) {
                        this.mSensors[i2] = ((BiometricSensor) this.mPreAuthInfo.eligibleSensors.get(i2)).f1650id;
                    }
            ...
            try {
                switch (i) {
                    ...
                    case 4:
                        ...
                        if (bArr2 == null) {
                            Slog.d("BiometricService/AuthSession", "addAuthToken: " + this.mKeyStore.addAuthToken(bArr2));
                        } else {
                        ...
                        if (this.mPromptInfo.semGetPrivilegedFlag() == 0) {
                            ...
                            BiometricSensor biometricSensor = this.mCurrentSensor;
                            if (biometricSensor != null) {
                                authenticationResult = (AuthenticationResult) this.mAuthenticationResults.get(biometricSensor.f1650id, authenticationResult);
                            }
                            this.mClientReceiver.onSemAuthenticationSucceeded(Utils.getAuthenticationTypeForResult(i), authenticationResult.f1649id, authenticationResult.challengeToken);
                            break;
                        } else {
