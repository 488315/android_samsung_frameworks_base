...
public abstract class InternalEnumerateClient extends HalClientMonitor
    ...
    public InternalEnumerateClient(
            Context context,
            Supplier supplier,
            IBinder iBinder,
            int i,
    ...
    public final void onEnumerationResult(BiometricAuthenticator.Identifier identifier, int i) {
        ...
        if (identifier == null) {
            ...
            Slog.v(
                    "Biometrics/InternalEnumerateClient",
                    "handleEnumeratedTemplate: " + identifier.getBiometricId());
            int i2 = 0;
            while (true) {
                if (i2 >= this.mEnrolledList.size()) {
                    ...
                    if (((BiometricAuthenticator.Identifier) this.mEnrolledList.get(i2))
                                    .getBiometricId()
        ...
        if (i == 0) {
            if (this.mEnrolledList == null) {
                ...
                for (int i3 = 0; i3 < this.mEnrolledList.size(); i3++) {
                    BiometricAuthenticator.Identifier identifier2 =
                            (BiometricAuthenticator.Identifier) this.mEnrolledList.get(i3);
                    arrayList.add(identifier2.getName().toString());
                    Slog.e(
                            "Biometrics/InternalEnumerateClient",
                            "doTemplateCleanup(): Removing dangling template from framework: "
                                    + identifier2.getBiometricId()
                                    + " "
                                    + ((Object) identifier2.getName()));
                    this.mUtils.removeBiometricForUser(
                            this.mContext, this.mTargetUserId, identifier2.getBiometricId());
    ...
    public void sendDanglingNotification(List list) {
        ...
        Slog.e(
                "Biometrics/InternalEnumerateClient",
                "sendDanglingNotification(): initial enrolledSize="
                        + this.mInitialEnrolledSize
                        + ", after clean up size="
                        + this.mEnrolledList.size());
        ...
        StringBuilder m =
                DumpUtils$$ExternalSyntheticOutline0.m("Showing ", str, " notification :[");
        m.append(list.size());
        ...
        String string3 =
                context.getString(
                        z2
                                ? R.string.lockscreen_sim_unlock_progress_dialog_message
                                : R.string.language_picker_section_suggested);
        if (!z2) {
                        ...
                        String.format(
                                context.getString(
                                        z
                                                ...
                                                : R.string.lockscreen_sim_locked_message),
                                sb,
                                str2);
            } else {
                int i2 =
                        z
                                ? R.string.lockscreen_sim_puk_locked_instructions
                                : R.string.lockscreen_screen_locked;
                sb.append(bidiFormatter.unicodeWrap("\""));
        ...
        PendingIntent broadcastAsUser =
                PendingIntent.getBroadcastAsUser(context, 0, intent2, 67108864, userHandle);
        BiometricNotificationUtils.showNotificationHelper(
                ...
                new Notification.Action.Builder(
                        ...
                        .build(),
                "sys",
                z2 ? "FingerprintReEnrollNotificationChannel" : "FaceReEnrollNotificationChannel",
                z2 ? "FingerprintReEnroll" : "FaceReEnroll",
                -1,
                false,
                32);
    }
