...
public abstract class SemFpBaseAuthenticationClient extends AuthenticationClient
    implements SemUdfpsConstraintStatusListener, SemFpTspBlockStatusHandler {
  ...
  public class Injector {
    public PromptInfo getBiometricPromptInfo(int i) {
      try {
        ...
        Slog.w(
            "FingerprintService", "getBiometricPrompt: failed to get prompt info" + e.getMessage());
        return null;
      }
    }

    public SemUdfpsSysUiImpl createUdfpsSysUiImpl(
        Context context, IBinder iBinder, String str, int i, boolean z) {
    ...
    public boolean checkTDDIDoubleTap(Context context) {
      ...
      return !(InputManager.getInstance() == null
          ...
          || Settings.System.getInt(context.getContentResolver(), "double_tab_to_wake_up", 0) == 0;
    }
  ...
  public SemFpBaseAuthenticationClient(
    ...
    this.mIsSettingApp =
        !isRestricted() && Utils.isSettings(context, authenticateOptions.getOpPackageName());
  }
  ...
  public void setExtraAttribute(Bundle bundle) {
    ...
    Slog.i(
        "FingerprintService",
        "FingerprintClientExt: "
            + Integer.toBinaryString(this.mPrivilegedFlags)
            + ", "
            + Integer.toBinaryString(getPromptPrivilegedFlags()));
  }
  ...
  private void initForUdfps() {
    SemUdfpsSysUiImpl createUdfpsSysUiImpl =
        this.mInjector.createUdfpsSysUiImpl(
            getContext(),
            getToken(),
            getOwnerString(),
            getTargetUserId(),
            hasPrivilegedFlag(16) || hasExtraAuthenticationFlag(32768));
    ...
    this.mUdfpsImpl.setSysUiListener(
        new SemUdfpsSysUiImpl
            .SysUiCallback() { // from class:
                               // com.android.server.biometrics.sensors.fingerprint.SemFpBaseAuthenticationClient.1
  ...
  public void start(ClientMonitorCallback clientMonitorCallback) {
    ...
    if (SemBiometricFeature.FP_FEATURE_SENSOR_IS_IN_DISPLAY_TYPE
        && (semUdfpsSysUiImpl = this.mUdfpsImpl) != null) {
      semUdfpsSysUiImpl.start();
    }
    showBackgroundAuthenticationNotificationIfNeeded();
    getBioLoggingManager()
        .fpStart((int) getRequestId(), isBiometricPrompt() ? "AP" : "A", getOwnerString());
  }
  ...
  private void showBackgroundAuthenticationNotificationIfNeeded() {
    ...
    if (this.mBackgroundNotification == null) {
      this.mBackgroundNotification =
          this.mInjector.createBiometricNotification(getContext(), getOwnerString());
    }
    ...
    if (this.mBackgroundNotificationAction == null) {
      this.mBackgroundNotificationAction =
          new BroadcastReceiver() { // from class:
                                    // com.android.server.biometrics.sensors.fingerprint.SemFpBaseAuthenticationClient.2
      ...
      Utils.registerBroadcastAsUser(
          getContext(),
          this.mBackgroundNotificationAction,
          new IntentFilter("com.samsung.android.server.biometrics.BIOMETRICS_NOTIFICATION"),
          UserHandle.CURRENT,
          SemFpMainThread.get().getHandler());
    }
    this.mBackgroundNotification.postNotification(intent);
  }

  public final void handleNotificationAction(Intent intent) {
    if ("com.samsung.android.server.biometrics.BIOMETRICS_NOTIFICATION".equals(intent.getAction())
        && getOwnerString().equals(intent.getStringExtra("package"))
        && intent.getIntExtra("authenticator", 0) == 2) {
  ...
  public final void handleAuthenticationResult(int i) {
    ...
    handleBigDataOnBackgroundThread(
        this.mIsAuthenticated,
        this.mQualityErrorCount,
        this.mRejectCount,
        currentTimeMillis,
        this.mIsScreenOnWhenStartCapture);
    ...
    if (this.mIsAuthenticated) {
      ...
      getBioLoggingManager()
          .fpNoMatch(
              ...
              (int) getRequestId(),
              j,
              this.mAuthenticationFailedReason,
              this.mQualityErrorCount,
              this.mIsScreenOnWhenStartCapture);
    }
  }

  public final void handleBigDataOnBackgroundThread(
      final boolean z, final int i, final int i2, final long j, final boolean z2) {
    SemBioFgThread.get()
        .post(
            new Runnable() { // from class:
                             // com.android.server.biometrics.sensors.fingerprint.SemFpBaseAuthenticationClient$$ExternalSyntheticLambda0
              @Override // java.lang.Runnable
              public final void run() {
                SemFpBaseAuthenticationClient.this.lambda$handleBigDataOnBackgroundThread$0(
                    z, j, i2, i, z2);
              }
            });
  }
  ...
  public /* synthetic */ void lambda$handleBigDataOnBackgroundThread$0(
    ...
    if (i == 1006) {
      ...
      getBioLoggingManager()
          .fpCaptureFailed(
              getContext(),
              (int) getRequestId(),
              System.currentTimeMillis() - this.mCaptureStartTime,
              i,
              this.mIsScreenOnWhenStartCapture);
    } else if (i == 10011) {
      setEarlyWakeUp();
  ...
  public final void handleAuthenticationFailedReasonEvent(int i) {
    ...
    PerformanceTracker.getInstanceForSensorId(getSensorId())
        .semIncrementNoMatchReason(getTargetUserId(), i);
  ...
  public final void handleQualityFailedEvent(int i, int i2) {
    ...
    this.mTotalQualityErrorCount++;
    PerformanceTracker.getInstanceForSensorId(getSensorId())
        .semIncrementQualityForUser(getTargetUserId(), isCryptoOperation());
  ...
  public final void setEarlyWakeUp() {
    if (this.mIsSetEarlyWakeUp
        || !isKeyguard()
        || this.mInjector.isInteractive(getContext())
        || this.mInjector.isFoldedInFlipType(getContext())
        || !this.mInjector.checkTDDIDoubleTap(getContext())) {
      return;
    }
  ...
  public final void pauseFaceAuth() {
    if (isKeyguard()) {
      SemBioFgThread.get()
          .post(
              new Runnable() { // from class:
                               // com.android.server.biometrics.sensors.fingerprint.SemFpBaseAuthenticationClient$$ExternalSyntheticLambda1
                @Override // java.lang.Runnable
                public final void run() {
  ...
  public final void resumeFaceAuth() {
    if (isKeyguard()) {
      SemBioFgThread.get()
          .post(
              new Runnable() { // from class:
                               // com.android.server.biometrics.sensors.fingerprint.SemFpBaseAuthenticationClient$$ExternalSyntheticLambda2
                @Override // java.lang.Runnable
                public final void run() {
  ...
  public void destroy() {
    ...
    getBioLoggingManager()
        .fpStop((int) getRequestId(), "U", 0L, this.mLastErrorCode, this.mTotalQualityErrorCount);
  }
