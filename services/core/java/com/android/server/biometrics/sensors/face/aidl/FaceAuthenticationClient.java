...
public class FaceAuthenticationClient extends AuthenticationClient implements LockoutConsumer {
  ...
  public FaceAuthenticationClient(
          ...
          public void onClientFinished(BaseClientMonitor baseClientMonitor, boolean z5) {
            ...
            if (FaceAuthenticationClient.this.mCancellationSignal != null) {
              FaceAuthenticationClient.this
                  .getServiceExtImpl()
                  .daemonCancel(FaceAuthenticationClient.this.mCancellationSignal, true);
            }
          }
        };
    setRequestId(j);
    ...
    this.mBiometricPromptIgnoreList = resources.getIntArray(R.array.fingerprint_acquired_vendor);
    this.mBiometricPromptIgnoreListVendor = resources.getIntArray(R.array.maps_starting_lat_lng);
    this.mKeyguardIgnoreList = resources.getIntArray(R.array.imAddressTypes);
    this.mKeyguardIgnoreListVendor = resources.getIntArray(R.array.networkAttributes);
    ...
    if (bundle != null) {
      ...
      Slog.d(
          ...
          "mPrivilegedAttr = " + this.mPrivilegedAttr + ", bundle = " + bundle);
    } else {
      this.mPrivilegedAttr = 0;
    }
    if (isBiometricPrompt()
        && (biometricPromptInfo = getBiometricPromptInfo(getCookie())) != null) {
  ...
  public ClientMonitorCallback wrapCallbackForStart(ClientMonitorCallback clientMonitorCallback) {
    return new ClientMonitorCompositeCallback(
        this.mSemCancelDaemonCallback,
        getLogger().getAmbientLightProbe(true),
  ...
  public final ICancellationSignal doAuthenticate() {
    ...
    if (aidlSession.hasContextMethods()) {
      ...
      ICancellationSignal authenticateWithContext =
          aidlSession
              .getSession()
              .authenticateWithContext(
                  this.mOperationId,
                  operationContext.toAidlContext((FaceAuthenticateOptions) getOptions()));
      getBiometricContext()
          .subscribe(
              operationContext,
              new Consumer() { // from class:
                               // com.android.server.biometrics.sensors.face.aidl.FaceAuthenticationClient$$ExternalSyntheticLambda0
                @Override // java.util.function.Consumer
                public final void accept(Object obj) {
                  FaceAuthenticationClient.lambda$doAuthenticate$0(
                      AidlSession.this, (OperationContext) obj);
                }
              });
  ...
  public static /* synthetic */ void lambda$doAuthenticate$0(
      AidlSession aidlSession, OperationContext operationContext) {
    try {
  ...
  public void onAuthenticated(
    ...
    this.mUsageStats.addEvent(
        new UsageStats.AuthenticationEvent(
            getStartTimeMs(),
            System.currentTimeMillis() - getStartTimeMs(),
            z,
            0,
            0,
            getTargetUserId()));
  ...
  public void onLockoutTimed(long j) {
    this.mAuthSessionCoordinator.lockOutTimed(
        getTargetUserId(), getSensorStrength(), getSensorId(), j, getRequestId());
  ...
  public void onLockoutPermanent() {
    this.mAuthSessionCoordinator.lockedOutFor(
        getTargetUserId(), getSensorStrength(), getSensorId(), getRequestId());
  ...
  public final void showBackgroundAuthenticationNotificationIfNeeded() {
    if (!SemFaceUtils.hasPrivilegedAttr(this.mBundle, 4)
        || isKeyguard()
    ...
    if (this.mBackgroundNotification == null) {
      this.mBackgroundNotification =
          new SemBiometricNotification(getContext(), 8, getOwnerString());
    }
          ...
          new BroadcastReceiver() { // from class:
            ...
            public void onReceive(Context context, Intent intent2) {
              if ("com.samsung.android.server.biometrics.BIOMETRICS_NOTIFICATION"
                  ...
                  && FaceAuthenticationClient.this
                      .getOwnerString()
                      .equals(intent2.getStringExtra("package"))
                  && intent2.getIntExtra("authenticator", 0) == 8
                  && FaceAuthenticationClient.this.mCancellationSignal != null) {
                Slog.i("FaceAuthenticationClient", "Cancel authentication by Notification action");
                FaceAuthenticationClient.this
                    .getServiceExtImpl()
                    .daemonCancel(FaceAuthenticationClient.this.mCancellationSignal, false);
              }
            }
          };
