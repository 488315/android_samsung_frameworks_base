...
public abstract class InternalCleanupClient extends HalClientMonitor
  ...
  public InternalCleanupClient(
    ...
    this.mEnumerateCallback =
        new ClientMonitorCallback() { // from class:
          ...
          public void onClientFinished(BaseClientMonitor baseClientMonitor, boolean z) {
            List unknownHALTemplates =
                ((InternalEnumerateClient) InternalCleanupClient.this.mCurrentTask)
                    .getUnknownHALTemplates();
            Slog.d(
                "Biometrics/InternalCleanupClient",
                "Enumerate onClientFinished: " + baseClientMonitor + ", success: " + z);
            if (!unknownHALTemplates.isEmpty()) {
              Slog.w(
                  "Biometrics/InternalCleanupClient",
                  "Adding " + unknownHALTemplates.size() + " templates for deletion");
            }
            Iterator it = unknownHALTemplates.iterator();
            while (it.hasNext()) {
              InternalCleanupClient.this.mUnknownHALTemplates.add(
                  new UserTemplate(
                      (BiometricAuthenticator.Identifier) it.next(),
                      InternalCleanupClient.this.mCurrentTask.getTargetUserId()));
            }
            ...
            try {
              ...
              while (it2.hasNext()) {
                ...
                Slog.i(
                    "Biometrics/InternalCleanupClient",
                    "Adding unknown HAL template: " + userTemplate.mIdentifier.getBiometricId());
                InternalCleanupClient.this.onAddUnknownTemplate(
                    userTemplate.mUserId, userTemplate.mIdentifier);
              }
            } finally {
    ...
    this.mRemoveCallback =
        new ClientMonitorCallback() { // from class:
                                      // com.android.server.biometrics.sensors.InternalCleanupClient.2
          @Override // com.android.server.biometrics.sensors.ClientMonitorCallback
          public void onClientFinished(BaseClientMonitor baseClientMonitor, boolean z) {
            Slog.d(
                "Biometrics/InternalCleanupClient",
                "Remove onClientFinished: " + baseClientMonitor + ", success: " + z);
            if (InternalCleanupClient.this.mUnknownHALTemplates.isEmpty()) {
              InternalCleanupClient internalCleanupClient = InternalCleanupClient.this;
              internalCleanupClient.mCallback.onClientFinished(internalCleanupClient, z);
            } else {
  ...
  public final void startCleanupUnknownHalTemplates() {
    Slog.d(
        "Biometrics/InternalCleanupClient",
        "startCleanupUnknownHalTemplates, size: " + this.mUnknownHALTemplates.size());
  ...
  public void start(ClientMonitorCallback clientMonitorCallback) {
    ...
    List biometricsForUser =
        this.mBiometricUtils.getBiometricsForUser(getContext(), getTargetUserId());
    this.mCurrentTask =
        getEnumerateClient(
            getContext(),
            this.mLazyDaemon,
  ...
  public void onRemoved(BiometricAuthenticator.Identifier identifier, int i) {
    ...
    if (!(baseClientMonitor instanceof RemovalClient)) {
      Slog.e(
          "Biometrics/InternalCleanupClient",
          "onRemoved received during client: " + this.mCurrentTask.getClass().getSimpleName());
      return;
    }
    ...
    if (!(this.mCurrentTask instanceof InternalEnumerateClient)) {
      Slog.e(
          ...
          "onEnumerationResult received during client: "
              + this.mCurrentTask.getClass().getSimpleName());
      return;
    }
