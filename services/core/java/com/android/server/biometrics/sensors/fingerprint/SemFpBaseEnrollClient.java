...
public abstract class SemFpBaseEnrollClient extends EnrollClient
  ...
  public class Injector {
    ...
    public SemUdfpsSysUiImpl createUdfpsSysUiImpl(
        Context context, IBinder iBinder, String str, int i) {
      return new SemUdfpsSysUiImpl(context, iBinder, str, i, true);
    }
  }

  public SemFpBaseEnrollClient(
      Context context,
      Supplier supplier,
      IBinder iBinder,
  ...
  public void initForUdfps() {
    SemUdfpsSysUiImpl createUdfpsSysUiImpl =
        this.mInjector.createUdfpsSysUiImpl(
            getContext(), getToken(), getOwnerString(), getTargetUserId());
    ...
    this.mUdfpsImpl.setSysUiListener(
        new SemUdfpsSysUiImpl
            .SysUiCallback() { // from class:
                               // com.android.server.biometrics.sensors.fingerprint.SemFpBaseEnrollClient.1
  ...
  public boolean hasReachedEnrollmentLimit() {
    if (FingerprintUtils.getInstance(getSensorId())
  ...
  public void onEnrollResult(BiometricAuthenticator.Identifier identifier, int i) {
    if (i == 0) {
      ...
      this.mInjector.stopLogging(
          (int) getRequestId(), "S", this.mDuplicatedImgCnt, this.mTotalQualityErrorCount, 0);
    }
  ...
  public void destroy() {
    ...
    this.mInjector.stopLogging(
        (int) getRequestId(), "U", 0, this.mTotalQualityErrorCount, this.mLastErrorCode);
  }
