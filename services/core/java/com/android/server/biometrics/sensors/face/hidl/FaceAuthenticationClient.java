...
public abstract class FaceAuthenticationClient extends AuthenticationClient
  ...
  public FaceAuthenticationClient(
    ...
    this.mSemCancelDaemonCallback =
        new ClientMonitorCallback() { // from class:
                                      // com.android.server.biometrics.sensors.face.hidl.FaceAuthenticationClient.2
          ...
          public void onClientStarted(BaseClientMonitor baseClientMonitor) {
            ...
            FaceAuthenticationClient.this
                ...
                .authStartedFor(
                    FaceAuthenticationClient.this.getTargetUserId(),
                    FaceAuthenticationClient.this.getSensorId(),
                    FaceAuthenticationClient.this.getRequestId());
          }
          ...
          public void onClientFinished(BaseClientMonitor baseClientMonitor, boolean z5) {
            ...
            FaceAuthenticationClient.this
                ...
                .authEndedFor(
                    FaceAuthenticationClient.this.getTargetUserId(),
                    Utils.getCurrentStrength(FaceAuthenticationClient.this.getSensorId()),
                    FaceAuthenticationClient.this.getSensorId(),
                    FaceAuthenticationClient.this.getRequestId(),
                    z5);
          }
      ...
      try {
        promptInfo =
            IBiometricService.Stub.asInterface(ServiceManager.getService("biometric"))
                .semGetPromptInfo(getCookie());
      } catch (RemoteException e) {
        Slog.w(
            "FaceAuthenticationClient",
            "FaceAuthenticationClient getPrompt Info Exception" + e.getMessage());
        promptInfo = null;
      }
      if (promptInfo != null) {
  ...
  public void onAuthenticated(
    ...
    if (z2) {
      ...
      try {
        ...
        Slog.e(
            "FaceAuthenticationClient",
            "onAuthenticated : Unable to notify listener, finishing",
            e);
        this.mCallback.onClientFinished(this, false);
      }
    }
    ...
    if (this.mBackgroundNotificationAction == null) {
      this.mBackgroundNotificationAction =
          new BroadcastReceiver() { // from class:
                                    // com.android.server.biometrics.sensors.face.hidl.FaceAuthenticationClient.1
            ...
            public void onReceive(Context context, Intent intent2) {
              if ("com.samsung.android.server.biometrics.BIOMETRICS_NOTIFICATION"
                  ...
                  && !FaceAuthenticationClient.this.mCancellationSignal.isCanceled()) {
