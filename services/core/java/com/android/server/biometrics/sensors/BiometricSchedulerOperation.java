...
public class BiometricSchedulerOperation {
  ...
  public BiometricSchedulerOperation(
      BaseClientMonitor baseClientMonitor, ClientMonitorCallback clientMonitorCallback, int i) {
    this(
        baseClientMonitor,
        clientMonitorCallback,
        i,
        new BooleanSupplier() { // from class:
                                // com.android.server.biometrics.sensors.BiometricSchedulerOperation$$ExternalSyntheticLambda1
  ...
  public boolean startWithCookie(ClientMonitorCallback clientMonitorCallback, int i) {
    if (this.mClientMonitor.getCookie() != i) {
      Slog.e(
          ...
          "Mismatched cookie for operation: " + this + ", received: " + i);
      return false;
    }
    if (errorWhenNoneOf("start", 0, 4, 1)) {
      return false;
  ...
  public void cancel(Handler handler, ClientMonitorCallback clientMonitorCallback) {
    ...
    if (i == 3) {
      Slog.w(
          "BiometricSchedulerOperation", "Cannot cancel - already invoked for operation: " + this);
      return;
    }
    this.mState = 3;
    if (i == 0 || i == 1 || i == 4) {
      Slog.d(
          "BiometricSchedulerOperation",
          "[Cancelling] Current client (without start): " + this.mClientMonitor);
  ...
  public boolean isAuthenticationOrDetectionOperation() {
    ...
    return (baseClientMonitor instanceof AuthenticationConsumer)
        || (baseClientMonitor instanceof DetectionConsumer);
  }
