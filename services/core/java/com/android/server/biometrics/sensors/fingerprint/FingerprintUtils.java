...
public final class FingerprintUtils implements BiometricUtils {
    static final String INTENT_ACTION_ADD_FINGERPRINT =
            "com.samsung.android.intent.action.FINGERPRINT_ADDED";
    static final String INTENT_ACTION_REMOVE_FINGERPRINT =
            "com.samsung.android.intent.action.FINGERPRINT_REMOVED";
    static final String INTENT_ACTION_RESET_FINGERPRINT =
    ...
    public static FingerprintAuthenticateOptions copyOptions(
            int i, FingerprintAuthenticateOptions fingerprintAuthenticateOptions) {
        return new FingerprintAuthenticateOptions.Builder()
                .setUserId(i)
                .setSensorId(fingerprintAuthenticateOptions.getSensorId())
                .setOpPackageName(fingerprintAuthenticateOptions.getOpPackageName())
                .setAttributionTag(fingerprintAuthenticateOptions.getAttributionTag())
                .setDisplayState(fingerprintAuthenticateOptions.getDisplayState())
                .setIgnoreEnrollmentState(fingerprintAuthenticateOptions.isIgnoreEnrollmentState())
                .build();
    }
    ...
    public final void addBiometricForUser(Context context, int i, Fingerprint fingerprint) {
        ...
        semSendTemplateChangedBroadCast(
                i,
                fingerprint.getBiometricId(),
                context,
                new Intent(INTENT_ACTION_ADD_FINGERPRINT));
    }
    ...
    public final void semSendTemplateChangedBroadCast(
            final int i, final int i2, final Context context, final Intent intent) {
        Runnable runnable =
                new Runnable() { // from class:
                                 // com.android.server.biometrics.sensors.fingerprint.FingerprintUtils$$ExternalSyntheticLambda0
                    @Override // java.lang.Runnable
                    public final void run() {
                        ...
                        try {
                            ...
                            if (Utils.DEBUG) {
                                Slog.i(
                                        ...
                                        "["
                                                + intent2.getAction()
                                                + "] is sent with fingerId "
                                                + i3);
                            } else {
                                Slog.i(
                                        "FingerprintService",
                                        "[" + intent2.getAction() + "] is sent");
                            }
                        } catch (Exception e) {
                            NandswapManager$$ExternalSyntheticOutline0.m(
                                    e,
                                    new StringBuilder("semSendTemplateChangedBroadCast: failed, "),
                                    "FingerprintService");
                        }
                    }
                };
