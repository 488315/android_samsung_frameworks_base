...
public class UserAwareBiometricScheduler extends SemConcurrentBiometricScheduler {
  ...
  public class ClientFinishedCallback implements ClientMonitorCallback {
    ...
    public void onClientFinished(final BaseClientMonitor baseClientMonitor, final boolean z) {
      UserAwareBiometricScheduler.this.mHandler.post(
          new Runnable() { // from class:
                           // com.android.server.biometrics.sensors.UserAwareBiometricScheduler$ClientFinishedCallback$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
              UserAwareBiometricScheduler.ClientFinishedCallback.this.lambda$onClientFinished$0(
                  baseClientMonitor, z);
            }
          });
    }
    ...
    public /* synthetic */ void lambda$onClientFinished$0(
      ...
      Slog.d(
          UserAwareBiometricScheduler.this.getTag(),
          "[Client finished] " + baseClientMonitor + ", success: " + z);
      ...
      BiometricSchedulerOperation biometricSchedulerOperation =
          UserAwareBiometricScheduler.this.mCurrentOperation;
  ...
  public UserAwareBiometricScheduler(
      ...
      UserSwitchCallback userSwitchCallback) {
  ...
  public void startNextOperationIfIdle() {
    ...
    int targetUserId =
        ((BiometricSchedulerOperation) this.mPendingOperations.getFirst()).getTargetUserId();
    ...
    if (currentUserId == -10000) {
      ...
      this.mCurrentOperation =
          new BiometricSchedulerOperation(startUserClient, clientFinishedCallback, 2);
    ...
    Slog.d(
        getTag(),
        "[Stopping User] current: "
            + currentUserId
            + ", next: "
            + targetUserId
            + ". "
            + this.mStopUserClient);
    this.mCurrentOperation =
        new BiometricSchedulerOperation(this.mStopUserClient, clientFinishedCallback2, 2);
