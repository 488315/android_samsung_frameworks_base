...
public class BiometricStateCallback implements ClientMonitorCallback, IBinder.DeathRecipient {
  ...
  public void onClientStarted(BaseClientMonitor baseClientMonitor) {
    ...
    if (baseClientMonitor instanceof AuthenticationClient) {
      ...
      Slog.w(
          ...
          "Other authentication client: " + Utils.getClientName(baseClientMonitor));
  ...
  public void onClientFinished(BaseClientMonitor baseClientMonitor, boolean z) {
    ...
    Slog.d(
        "BiometricStateCallback",
        "Client finished, state updated to "
            + this.mBiometricState
            + ", client "
            + baseClientMonitor);
  ...
  public synchronized void registerBiometricStateListener(
      IBiometricStateListener iBiometricStateListener) {
  ...
  public final synchronized void broadcastCurrentEnrollmentState(
      IBiometricStateListener iBiometricStateListener) {
    for (BiometricServiceProvider biometricServiceProvider : this.mProviders) {
      for (SensorPropertiesInternal sensorPropertiesInternal :
          biometricServiceProvider.getSensorProperties()) {
        for (UserInfo userInfo : this.mUserManager.getAliveUsers()) {
          boolean hasEnrollments =
              biometricServiceProvider.hasEnrollments(
                  sensorPropertiesInternal.sensorId, userInfo.id);
          if (iBiometricStateListener != null) {
            notifyEnrollmentStateChanged(
                iBiometricStateListener,
                userInfo.id,
                sensorPropertiesInternal.sensorId,
                hasEnrollments);
          } else {
            notifyAllEnrollmentStateChanged(
                userInfo.id, sensorPropertiesInternal.sensorId, hasEnrollments);
          }
        }
      }
    }
  ...
  public final void notifyEnrollmentStateChanged(
      IBiometricStateListener iBiometricStateListener, int i, int i2, boolean z) {
    try {
  ...
  public void binderDied(final IBinder iBinder) {
    ...
    if (this.mBiometricStateListeners.removeIf(
        new Predicate() { // from class:
                          // com.android.server.biometrics.sensors.BiometricStateCallback$$ExternalSyntheticLambda0
          @Override // java.util.function.Predicate
          public final boolean test(Object obj) {
            boolean lambda$binderDied$0;
            lambda$binderDied$0 =
                BiometricStateCallback.lambda$binderDied$0(iBinder, (IBiometricStateListener) obj);
  ...
  public static /* synthetic */ boolean lambda$binderDied$0(
      IBinder iBinder, IBiometricStateListener iBiometricStateListener) {
