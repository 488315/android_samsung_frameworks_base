        ...
        context.registerReceiver(
                new BroadcastReceiver() { // from class:
                    ...
                    public final void onReceive(Context context2, Intent intent) {
                        ...
                        if (SemFpAidlLockoutHalImpl.ACTION_LOCKOUT_RESET.equals(
                                intent.getAction())) {
                            int intExtra =
                                    intent.getIntExtra(
                                            SemFpAidlLockoutHalImpl.KEY_LOCKOUT_RESET_USER, 0);
                            if (SemFpAidlLockoutHalImpl.this.getLockoutModeForUser(intExtra) == 1) {
                                ((Sensor)
                                                ((Sensor$$ExternalSyntheticLambda0)
                                                                lockoutResetCallback)
                                                        .f$0)
                                        .handleOnLockoutReset(intExtra);
                            }
                        }
                    }
                },
