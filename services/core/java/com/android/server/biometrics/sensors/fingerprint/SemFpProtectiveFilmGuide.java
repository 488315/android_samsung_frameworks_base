...
public class SemFpProtectiveFilmGuide
  ...
  static final String NOTIFICATION_INTENT_ACTION_NAME =
      "com.samsung.android.server.biometrics.fingerprint.BIOMETRICS_NOTIFICATION_PF_GUIDE";
  ...
  public class Injector {
    public Notification getNotification(Context context, int i) {
      return new Notification.Builder(context, "FingerprintProtectiveFilmNotificationChannel")
          .setSmallIcon(R.drawable.text_cursor_holo_light)
          .setSubText(getSubText(context))
          .setContentTitle(getContentTitle(context))
          .setContentText(getContentText(context))
          .setOnlyAlertOnce(true)
          .setLocalOnly(true)
          .setOngoing(true)
          .setCategory(getCategory(context))
          .addAction(getCloseNotificationAction(context, i))
          .addAction(getGoToSettingsNotificationAction(context, i))
          .setStyle(new Notification.BigTextStyle().bigText(getContentText(context)))
    ...
    public void setGuideStateToNotificationDoneByAuthentication(Context context, int i) {
      Settings.Secure.putIntForUser(
          context.getContentResolver(), "fingerprint_protective_film_guideline_displayed", 0, i);
    }
    ...
    public final Notification.Action getCloseNotificationAction(Context context, int i) {
      return new Notification.Action.Builder(
              ...
              getPendingIntentForAction(
                  context, SemFpProtectiveFilmGuide.NOTIFICATION_ACTION_CLOSE_BUTTON, 0, i))
          .build();
    }
    ...
    public final Notification.Action getGoToSettingsNotificationAction(Context context, int i) {
      return new Notification.Action.Builder(
              ...
              getPendingIntentForAction(
                  context, SemFpProtectiveFilmGuide.NOTIFICATION_ACTION_SETTINGS_BUTTON, 1, i))
          .build();
    }

    public final PendingIntent getPendingIntentForAction(
  ...
  public SemFpProtectiveFilmGuide(
  ...
  public final void showNotification(int i) {
    ...
    notificationManager.createNotificationChannel(
        new NotificationChannel(
            "FingerprintProtectiveFilmNotificationChannel", this.mContext.getString(17042744), 4));
    notificationManager.notifyAsUser(
        "FingerprintServiceProtectiveFilm",
        1,
        this.mInjector.getNotification(this.mContext, i),
        UserHandle.of(i));
  }
    ...
    if (this.mNotificationActionReceiver == null) {
      this.mNotificationActionReceiver =
          new BroadcastReceiver() { // from class:
                                    // com.android.server.biometrics.sensors.fingerprint.SemFpProtectiveFilmGuide.1
            @Override // android.content.BroadcastReceiver
            public void onReceive(Context context, Intent intent) {
              if (SemFpProtectiveFilmGuide.NOTIFICATION_INTENT_ACTION_NAME.contentEquals(
                  intent.getAction())) {
                int intExtra =
                    intent.getIntExtra(SemFpProtectiveFilmGuide.NOTIFICATION_ACTION_USER, -2);
                String stringExtra =
                    intent.getStringExtra(SemFpProtectiveFilmGuide.NOTIFICATION_ACTION_KEY);
                if (Utils.DEBUG) {
                  Slog.d(
                      SemFpProtectiveFilmGuide.TAG, "received: " + intExtra + ", " + stringExtra);
                }
                if (TextUtils.isEmpty(stringExtra)) {
                  return;
                }
                if (SemFpProtectiveFilmGuide.NOTIFICATION_ACTION_CLOSE_BUTTON.contentEquals(
                    stringExtra)) {
                  SemFpProtectiveFilmGuide.this.cancelNotification(intExtra);
                } else if (SemFpProtectiveFilmGuide.NOTIFICATION_ACTION_SETTINGS_BUTTON
                    .contentEquals(stringExtra)) {
      ...
      Utils.registerBroadcastAsUser(
          this.mContext,
          this.mNotificationActionReceiver,
          new IntentFilter(NOTIFICATION_INTENT_ACTION_NAME),
          UserHandle.ALL,
  ...
  public final void cancelNotification(int i) {
    ((NotificationManager) this.mContext.getSystemService("notification"))
        .cancelAsUser("FingerprintServiceProtectiveFilm", 1, UserHandle.of(i));
  }
