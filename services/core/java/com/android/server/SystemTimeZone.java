...
public abstract class SystemTimeZone {
  ...
  public static boolean setTimeZoneId(String str, int i, String str2) {
    ...
    if (TextUtils.isEmpty(str) || !isValidTimeZoneId(str)) {
      addDebugLogEntry(
          "setTimeZoneId: Invalid time zone ID. timeZoneId="
              + str
              + ", confidence="
              + i
              + ", logInfo="
    ...
    synchronized (SystemTimeZone.class) {
      ...
      if (z || timeZoneConfidence) {
        addDebugLogEntry(
            "Time zone or confidence set:  (new) timeZoneId="
                + str
                + ", (new) confidence="
                + i
                + ", logInfo="
  ...
  public static boolean isValidTimeZoneId(String str) {
    return (str == null || str.isEmpty() || !ZoneInfoDb.getInstance().hasTimeZone(str))
        ? false
        : true;
  }
}
