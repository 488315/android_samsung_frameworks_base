...
public class TrustArchive {
  ...
  public void dump(PrintWriter printWriter, int i, int i2, String str, boolean z) {
    ...
    while (descendingIterator.hasNext() && i4 < i) {
      ...
      if (i2 == -1 || i2 == (i3 = event.userId) || i3 == -1) {
        ...
        printWriter.printf(
            "#%-2d %s %s: ",
            Integer.valueOf(i4), formatElapsed(event.elapsedTimestamp), dumpType(event.type));
        if (i2 == -1) {
        ...
        if (i5 == 0) {
          printWriter.printf(
              ", message=\"%s\", duration=%s, flags=%s",
              event.message, formatDuration(event.duration), dumpGrantFlags(event.flags));
        } else if (i5 == 6) {
  ...
  public static String getSimpleName(ComponentName componentName) {
    ...
    return (lastIndexOf >= className.length() || lastIndexOf < 0)
        ? className
