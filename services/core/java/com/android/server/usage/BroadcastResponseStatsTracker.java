...
public final class BroadcastResponseStatsTracker {
    ...
    public final BroadcastResponseStatsTracker$$ExternalSyntheticLambda0
            mRoleHoldersChangedListener =
                    new OnRoleHoldersChangedListener() { // from class:
                                                         // com.android.server.usage.BroadcastResponseStatsTracker$$ExternalSyntheticLambda0
    ...
    public final boolean doesPackageHoldExemptedPermission(String str, UserHandle userHandle) {
        try {
            int packageUidAsUser =
                    this.mContext
                            .getPackageManager()
                            .getPackageUidAsUser(str, userHandle.getIdentifier());
            List broadcastResponseExemptedPermissions =
                    this.mAppStandby.getBroadcastResponseExemptedPermissions();
            for (int size = broadcastResponseExemptedPermissions.size() - 1; size >= 0; size--) {
                if (this.mContext.checkPermission(
                                (String) broadcastResponseExemptedPermissions.get(size),
                                -1,
                                packageUidAsUser)
                        == 0) {
                    return true;
        ...
        synchronized (this.mLock) {
            try {
                for (int size = broadcastResponseExemptedRoles.size() - 1; size >= 0; size--) {
                    ...
                    ArrayMap arrayMap =
                            (ArrayMap)
                                    this.mExemptedRoleHoldersCache.get(userHandle.getIdentifier());
                    if (arrayMap == null) {
    ...
    public final void dumpBroadcastEventsLocked(IndentingPrintWriter indentingPrintWriter) {
        ...
        for (int i = 0; i < this.mUserBroadcastEvents.size(); i++) {
            ...
            UserBroadcastEvents userBroadcastEvents =
                    (UserBroadcastEvents) this.mUserBroadcastEvents.valueAt(i);
        ...
        for (int i = 0; i < this.mUserResponseStats.size(); i++) {
            ...
            for (int i2 = 0; i2 < sparseArray.size(); i2++) {
                ...
                UserBroadcastResponseStats userBroadcastResponseStats =
                        (UserBroadcastResponseStats) sparseArray.valueAt(i2);
                indentingPrintWriter.println("User " + keyAt2 + ":");
                indentingPrintWriter.increaseIndent();
                for (int i3 = 0; i3 < userBroadcastResponseStats.mResponseStats.size(); i3++) {
                    BroadcastEvent broadcastEvent =
                            (BroadcastEvent) userBroadcastResponseStats.mResponseStats.keyAt(i3);
                    BroadcastResponseStats broadcastResponseStats =
                            (BroadcastResponseStats)
                                    userBroadcastResponseStats.mResponseStats.valueAt(i3);
    ...
    public final ArraySet getOrCreateBroadcastEventsLocked(String str, UserHandle userHandle) {
        UserBroadcastEvents userBroadcastEvents =
                (UserBroadcastEvents) this.mUserBroadcastEvents.get(userHandle.getIdentifier());
    ...
    public final BroadcastResponseStats getOrCreateBroadcastResponseStats(
            BroadcastEvent broadcastEvent) {
        ...
        UserBroadcastResponseStats userBroadcastResponseStats =
                (UserBroadcastResponseStats) sparseArray.get(i2);
        ...
        BroadcastResponseStats broadcastResponseStats =
                (BroadcastResponseStats)
                        userBroadcastResponseStats.mResponseStats.get(broadcastEvent);
        if (broadcastResponseStats != null) {
            return broadcastResponseStats;
        }
        BroadcastResponseStats broadcastResponseStats2 =
                new BroadcastResponseStats(
                        broadcastEvent.mTargetPackage, broadcastEvent.mIdForResponseEvent);
    ...
    public final void recordAndPruneOldBroadcastDispatchTimestamps(BroadcastEvent broadcastEvent) {
        LongArrayQueue longArrayQueue = broadcastEvent.mTimestampsMs;
        long broadcastResponseWindowDurationMs =
                this.mAppStandby.getBroadcastResponseWindowDurationMs();
    ...
    public final void reportNotificationEvent(int i, String str, UserHandle userHandle, long j) {
        ...
        BroadcastResponseStatsLogger broadcastResponseStatsLogger =
                broadcastResponseStatsTracker.mLogger;
        synchronized (broadcastResponseStatsLogger.mLock) {
            try {
                if (UsageStatsService.DEBUG_RESPONSE_STATS) {
                    Slog.d(
                            "ResponseStatsTracker",
                            BroadcastResponseStatsLogger.getNotificationEventLog(
                                    i, userHandle.getIdentifier(), j, str));
                }
                BroadcastResponseStatsLogger.Data data =
                        (BroadcastResponseStatsLogger.Data)
                                broadcastResponseStatsLogger.mNotificationEventsBuffer
                                        .getNextSlot();
                if (data != null) {
                    data.reset();
                    BroadcastResponseStatsLogger.NotificationEvent notificationEvent =
                            (BroadcastResponseStatsLogger.NotificationEvent) data;
        ...
        synchronized (broadcastResponseStatsTracker.mLock) {
            try {
                UserBroadcastEvents userBroadcastEvents =
                        (UserBroadcastEvents)
                                broadcastResponseStatsTracker.mUserBroadcastEvents.get(
                                        userHandle.getIdentifier());
                ArraySet arraySet =
                        userBroadcastEvents == null
                                ? null
                                : (ArraySet) userBroadcastEvents.mBroadcastEvents.get(str);
                if (arraySet == null) {
                    return;
                }
                long broadcastResponseWindowDurationMs =
                        broadcastResponseStatsTracker.mAppStandby
                                .getBroadcastResponseWindowDurationMs();
                long broadcastSessionsWithResponseDurationMs =
                        broadcastResponseStatsTracker.mAppStandby
                ...
                while (size >= 0) {
                    ...
                    broadcastResponseStatsTracker.recordAndPruneOldBroadcastDispatchTimestamps(
                            broadcastEvent);
                    LongArrayQueue longArrayQueue = broadcastEvent.mTimestampsMs;
                    long j2 = 0;
                    long j3 = 0;
                    while (longArrayQueue.size() > 0 && longArrayQueue.peekFirst() <= j) {
                        ...
                        if (j - peekFirst <= broadcastResponseWindowDurationMs && peekFirst >= j3) {
                            ...
                            BroadcastResponseStats orCreateBroadcastResponseStats =
                                    broadcastResponseStatsTracker.getOrCreateBroadcastResponseStats(
                                            broadcastEvent);
                            orCreateBroadcastResponseStats.incrementBroadcastsDispatchedCount(i3);
                            j3 = peekFirst + broadcastSessionsWithResponseDurationMs;
                            if (i == 0) {
                                i2 = i3;
                                orCreateBroadcastResponseStats.incrementNotificationsPostedCount(
                                        i2);
                            } else if (i == i3) {
                                i2 = i3;
                                orCreateBroadcastResponseStats.incrementNotificationsUpdatedCount(
                                        i2);
                            } else if (i != 2) {
                                ...
                                i2 = 1;
                                orCreateBroadcastResponseStats.incrementNotificationsCancelledCount(
                                        1);
                            }
                        } else {
                            i2 = i3;
