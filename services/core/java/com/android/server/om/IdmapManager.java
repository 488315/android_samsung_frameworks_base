...
public final class IdmapManager {
  ...
  public int createIdmap(
    ...
    try {
      int calculateFulfilledPolicies =
          calculateFulfilledPolicies(androidPackage, packageState, androidPackage2, i);
      boolean enforceOverlayable = enforceOverlayable(packageState, androidPackage2);
      if (this.mIdmapDaemon.verifyIdmap(
          path, str, str2, calculateFulfilledPolicies, enforceOverlayable, i)) {
        return 1;
      }
      return this.mIdmapDaemon.createIdmap(
                  path, str, str2, calculateFulfilledPolicies, enforceOverlayable, i)
              != null
          ? 3
          : 0;
  ...
  public FabricatedOverlayInfo createFabricatedOverlay(
  ...
  public final int calculateFulfilledPolicies(
      AndroidPackage androidPackage,
      PackageState packageState,
      AndroidPackage androidPackage2,
      int i) {
    int i2 =
        this.mPackageManager.signaturesMatching(
                androidPackage.getPackageName(), androidPackage2.getPackageName(), i)
            ? 17
            : 1;
    if (matchesActorSignature(androidPackage, androidPackage2, i)) {
      i2 |= 128;
    }
    if (!TextUtils.isEmpty(this.mConfigSignaturePackage)
        && this.mPackageManager.signaturesMatching(
            this.mConfigSignaturePackage, androidPackage2.getPackageName(), i)) {
      i2 |= 256;
    }
    return packageState.isVendor()
        ? i2 | 4
        : packageState.isProduct()
            ...
            : packageState.isOdm()
                ...
                : packageState.isOem()
                    ? i2 | 64
                    : (packageState.isSystem() || packageState.isSystemExt()) ? i2 | 2 : i2;
  }

  public final boolean matchesActorSignature(
      AndroidPackage androidPackage, AndroidPackage androidPackage2, int i) {
    String str;
    ...
    try {
      OverlayableInfo overlayableForTarget =
          this.mPackageManager.getOverlayableForTarget(
              androidPackage.getPackageName(), overlayTargetOverlayableName, i);
      if (overlayableForTarget == null || (str = overlayableForTarget.actor) == null) {
        return false;
      }
      return this.mPackageManager.signaturesMatching(
          (String)
              OverlayActorEnforcer.getPackageNameForActor(
                      str, this.mPackageManager.getNamedActors())
                  .first,
          androidPackage2.getPackageName(),
          i);
