...
public class OverlayReferenceMapper {
  ...
  public OverlayReferenceMapper(boolean z, Provider provider) {
    ...
    this.mProvider =
        provider == null
            ? new Provider() { // from class: com.android.server.om.OverlayReferenceMapper.1
              ...
              public String getActorPkg(String str) {
                return (String)
                    OverlayActorEnforcer.getPackageNameForActor(
                            str, SystemConfig.getInstance().getNamedActors())
                        .first;
              }
    ...
    synchronized (this.mLock) {
      for (int size = this.mActorToTargetToOverlays.size() - 1; size >= 0; size--) {
        ...
        if (arrayMap.containsKey(str)) {
          arrayMap.remove(str);
          collection.add(
              this.mProvider.getActorPkg((String) this.mActorToTargetToOverlays.keyAt(size)));
          if (arrayMap.isEmpty()) {
  ...
  public final void ensureMapBuilt() {
    if (this.mDeferRebuild) {
      ...
      Slog.w(
          ...
          "The actor map was queried before the system was ready, which mayresult in decreased"
              + " performance.");
    }
  }
