...
public class TemplateManager {
  ...
  public final void initUidTemplate(JSONArray jSONArray, List list) {
    ...
    for (int i = 0; i < jSONArray.length(); i++) {
      ...
      uidItem.opacity =
          Integer.valueOf(jSONObject.isNull("opacity") ? 100 : jSONObject.getInt("opacity"));
      list.add(uidItem);
    }
  }
  ...
  public final void initColorTemplate(JSONArray jSONArray, HashMap hashMap) {
    ...
    for (int i = 0; i < jSONArray.length(); i++) {
      ...
      if (!jSONObject.isNull("name") && !jSONObject.isNull("colorLight")) {
        ...
        colorItem.colorDark =
            jSONObject.isNull("colorDark")
                ? colorItem.colorLight
                : jSONObject.getString("colorDark");
        colorItem.colorLightGray =
            jSONObject.isNull("colorLightGray")
                ? colorItem.colorLight
                : jSONObject.getString("colorLightGray");
        colorItem.colorDarkGray =
            jSONObject.isNull("colorDarkGray")
                ? colorItem.colorDark
                : jSONObject.getString("colorDarkGray");
  ...
  public void update(AndroidPackage androidPackage) {
    ...
    if (string != null && !string.isEmpty()) {
      ...
      for (String str : split) {
        int identifier =
            packageResources.getIdentifier(str, "raw", androidPackage.getPackageName());
        if (identifier > 0) {
          try {
            ...
            SemWallpaperThemeManager.saveSWTLog(
                this.TAG, "update template from apk, error = " + e2);
          }
        } else {
  ...
  public final void updateUidTemplate(JSONArray jSONArray, String str) {
    String str2 =
        jSONArray
            .getJSONObject(0)
            .getString("uid")
            .split(PackageManagerShellCommandDataLoader.STDIN_PATH)[0];
    SemWallpaperThemeManager.saveSWTLog(
        this.TAG, "template rpUID [" + str2 + "] replaced by " + str);
      ...
      if (jSONObject.isNull("name") || jSONObject.isNull("colorLight")) {
        ...
        while (true) {
          ...
          if (colorItem2.name.equals(colorItem.name)) {
            SemWallpaperThemeManager.saveSWTLog(
                this.TAG, "template COLOR [" + colorItem.name + "] replaced by " + str);
  ...
  public List getColors(String str) {
    return (List)
        search(
            str,
            new Function() { // from class: com.android.server.om.wallpapertheme.TemplateManager.1
              ...
              public List apply(UidItem uidItem) {
                ...
                if (strArr[0].startsWith("#")) {
                  ...
                  ColorItem colorItem =
                      (ColorItem) TemplateManager.this.mColorTemplate.get(strArr[0]);
                  if (colorItem == null) {
                    return null;
                  }
                  TemplateManager templateManager = TemplateManager.this;
                  colorFromName =
                      templateManager.getColorFromName(
                          templateManager.mThemePalette.mIsGray
                              ? colorItem.colorLightGray
                              : colorItem.colorLight);
                }
                ...
                if (strArr[1].startsWith("#")) {
                  ...
                  ColorItem colorItem2 =
                      (ColorItem) TemplateManager.this.mColorTemplate.get(strArr[1]);
                  if (colorItem2 == null) {
                    return null;
                  }
                  TemplateManager templateManager2 = TemplateManager.this;
                  colorFromName2 =
                      templateManager2.getColorFromName(
                          templateManager2.mThemePalette.mIsGray
                ...
                if (uidItem.opacity.intValue() != 100) {
                  colorFromName =
                      ThemeUtil.adjustAlpha(
                          uidItem.opacity.intValue() / 100.0f, colorFromName.intValue());
                  colorFromName2 =
                      ThemeUtil.adjustAlpha(
                          uidItem.opacity.intValue() / 100.0f, colorFromName2.intValue());
                }
  ...
  public List getBooleans(String str) {
    return (List)
        search(
            str,
            new Function() { // from class: com.android.server.om.wallpapertheme.TemplateManager.2
              @Override // java.util.function.Function
              public List apply(UidItem uidItem) {
                Boolean bool;
                Boolean bool2;
  ...
  public Integer getInteger(String str) {
    return (Integer)
        search(
            str,
            new Function() { // from class: com.android.server.om.wallpapertheme.TemplateManager.3
              @Override // java.util.function.Function
  ...
  public void dump(PrintWriter printWriter) {
    ...
    for (UidItem uidItem : this.mUidTemplate) {
      printWriter.println(
          "   - UID:" + uidItem.uid + ", THEME:" + uidItem.theme + ", OPA:" + uidItem.opacity);
    }
