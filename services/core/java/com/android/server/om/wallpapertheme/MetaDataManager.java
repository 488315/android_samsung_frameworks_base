...
public class MetaDataManager {
  ...
  public void loadStaticMetadata(Context context) {
    ...
    SemWallpaperThemeManager.saveSWTLog(
        this.TAG, "load static metadatas, uidMap size: " + this.mUidMap.size());
  }
    ...
    try {
      ...
      if (string != null && !string.isEmpty()) {
        ...
        for (String str : split) {
          ...
          if (identifier > 0) {
            MetaData metaData2 =
                new MetaData(packageResources.getXml(identifier), this.mSeslMetaData);
            ...
            SemWallpaperThemeManager.saveSWTLog(
                this.TAG, "metadata rpUID [" + rpUID + "] replaced by " + packageName);
          } else {
      ...
      if (i > 0) {
        ...
        SemWallpaperThemeManager.saveSWTLog(
            this.TAG, "metadata rpUID [" + rpUID2 + "] replaced by " + packageName);
      }
    } catch (Exception e) {
      SemWallpaperThemeManager.saveSWTLog(
          this.TAG,
          "Package : " + androidPackage.getPackageName() + " metadata update error = " + e);
    }
  }
  ...
  public String getRefUid(String str) {
    ...
    if (hashMap == null
        || (uid = (Uid) hashMap.get(str)) == null
        || (reference = uid.getReference()) == null
        || reference.isEmpty()) {
      return null;
    }
    return reference;
    ...
    for (Map.Entry entry : this.mUidMap.entrySet()) {
      if (entry.getKey() != null) {
        if (((String) entry.getKey())
            .startsWith(str + PackageManagerShellCommandDataLoader.STDIN_PATH)) {
  ...
  public class MetaData {
    ...
    public final void addSeslMetaData() {
      this.mCurrentPackage
          .getUidList()
          .addAll(((Package) MetaDataManager.this.mPackageList.get(0)).getUidList());
    }
    ...
    public final void addUID(XmlPullParser xmlPullParser) {
      ...
      if (attributeValue == null) {
        SemWallpaperThemeManager.saveSWTLog(
            this.TAG, "Parsing xml error, uid is empty. destAttributeName : " + attributeValue3);
        return;
      }
      this.mCurrentPackage.addUid(
          new Uid(
              attributeValue,
              attributeValue2,
              attributeValue3,
