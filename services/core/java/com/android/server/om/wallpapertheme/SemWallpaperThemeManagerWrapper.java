...
public class SemWallpaperThemeManagerWrapper implements ISemWallpaperThemeManager {
  ...
  public static OverlayIdentifier sColorThemeOverlayId =
  ...
  public SemWallpaperThemeManager mWallpaperThemeManager =
      SemWallpaperThemeManager.getInstance(mContext);

  public SemWallpaperThemeManagerWrapper() {
    OverlayPolicyManager.registerPolicy(
        new SemWallpaperThemeOverlayPolicy(this.mWallpaperThemeManager));
  }

  public static synchronized ISemWallpaperThemeManager getInstance(
      Context context, OverlayManagerSettingsHelper overlayManagerSettingsHelper) {
    SemWallpaperThemeManagerWrapper semWallpaperThemeManagerWrapper;
    synchronized (SemWallpaperThemeManagerWrapper.class) {
      mContext = context;
      if (instance == null) {
        SemWallpaperThemeManagerWrapper semWallpaperThemeManagerWrapper2 =
            new SemWallpaperThemeManagerWrapper();
  ...
  public void initWallpaperTheme() {
    try {
      ...
      if (getColorThemeState() == 1) {
        ...
        Slog.d(
            "SWT_WTM_Wrapper",
            "init color theme metadata has finished : "
        ...
        if (!readLastPalette.isEmpty()) {
          ...
          this.mWallpaperThemeManager.setPalette(
              arrayList,
              arrayList2,
              this.mWallpaperThemeManager.splitPalette(readLastPalette, arrayList, arrayList2));
        }
        if (mContext.getPackageManager().isDeviceUpgrading()) {
          ...
          this.mWallpaperThemeManager.restoreMonetOverlay(
              builder,
              this.mSettings.getNullableOverlayInfo(
                  new OverlayIdentifier("android", "MonetPalette"), 0));
          ...
          Set<Pair> allIdentifiersAndBaseCodePaths =
              this.mSettings.getAllIdentifiersAndBaseCodePaths();
          ArrayList arrayList3 = new ArrayList();
          for (Pair pair : allIdentifiersAndBaseCodePaths) {
            if (((OverlayIdentifier) pair.first).getOverlayName() != null
                && ((OverlayIdentifier) pair.first).getOverlayName().startsWith("SemWT_")) {
              arrayList3.add((OverlayIdentifier) pair.first);
            }
          }
          if (!arrayList3.isEmpty()) {
            Slog.i(
                "SWT_WTM_Wrapper", "unregister unnecessary color theme overlays for fota upgrade");
          ...
          Slog.d(
              "SWT_WTM_Wrapper",
              "restoring color theme has finished, ready:"
      ...
      Slog.e(
          "SWT_WTM_Wrapper",
          "failed initWallpaperTheme, wallpaper theming will not working, ex = " + e);
    }
  }
  ...
  public boolean isRequestForColorTheme(OverlayManagerTransaction overlayManagerTransaction) {
    ...
    if (overlayManagerTransaction.getRequests() == null
        || !overlayManagerTransaction.getRequests().hasNext()) {
      return false;
    }
    OverlayManagerTransaction.Request request =
        (OverlayManagerTransaction.Request) overlayManagerTransaction.getRequests().next();
    String overlayName =
        (request == null || (overlayIdentifier = request.overlay) == null)
            ? null
  ...
  public void applyWallpaperColor(List list, List list2, boolean z, boolean z2) {
    ...
    if (mContext.getPackageManager().checkSignatures(1000, callingUid) != 0) {
      SemWallpaperThemeManager.saveSWTLog(
          "SWT_WTM_Wrapper", "applyWallpaperColor called by abnormal uid : " + callingUid);
      return;
    }
    Log.i(
        "SWT_WTM_Wrapper",
        "invoked applyWallpaperColor, isGray=" + z + ", colorSS=" + list + ", colorGG=" + list2);
    try {
      try {
        ...
        if (list != null) {
          ...
          try {
            ...
            SemWallpaperThemeManager.saveSWTLog(
                "SWT_WTM_Wrapper",
                "[Enable] commit OverlayManagerTransaction, dur1:"
                    + (uptimeMillis2 - uptimeMillis)
                    + ", dur2:"
                    + (SystemClock.uptimeMillis() - uptimeMillis2));
          } catch (Exception e) {
          ...
          try {
            ...
            IOverlayManager.Stub.asInterface(ServiceManager.getService("overlay"))
                .commit(builder2.build());
            this.mWallpaperThemeManager.saveWallpaperThemeColor();
            this.mWallpaperThemeManager.saveWallpaperThemeState(0);
            SemWallpaperThemeManager.saveSWTLog(
                "SWT_WTM_Wrapper",
                "[Disable] commit OverlayManagerTransaction, dur:"
                    + (SystemClock.uptimeMillis() - uptimeMillis3));
          } catch (Exception e2) {
  ...
  public final int getColorThemeState() {
    OverlayInfo nullableOverlayInfo =
        this.mSettings.getNullableOverlayInfo(sColorThemeOverlayId, 0);
  ...
  public void applyThemeParkWallpaperColor(Uri uri) {
    ...
    try {
      ...
      try {
        ArrayList arrayList = new ArrayList();
        JSONArray jSONArray =
            new JSONObject(readFromInputStream(openInputStream)).getJSONArray("wallpaperColors");
  ...
  public final void unregisterThemeParkOverlays(OverlayManagerTransaction.Builder builder) {
    ...
    for (Pair pair : allIdentifiersAndBaseCodePaths) {
      if (((OverlayIdentifier) pair.first).getOverlayName() != null
          && ((OverlayIdentifier) pair.first).getOverlayName().startsWith("ThemePark_")) {
