...
public class ContainerDependencyWrapper {
    ...
    */
    public static void setApplicationBlackList(Context context2, List list, int i) {
        ...
        ContextInfo contextInfo = new ContextInfo(getEdmStorageProvider(context2).getMUMContainerOwnerUid(i), i);
        List list2 = null;
        try {
        ...
        for (Map.Entry entry : hashMap.entrySet()) {
            ...
            if (intValue == -1) {
                ...
                try {
                    iApplicationPolicy.removeAppPackageNameFromBlackList(contextInfo, (String) entry.getKey());
    ...
    public static void handlePwdChangeNotificationForDeviceOwner(Context context2, int i) {
        ...
        Notification build = new Notification.Builder(context2, SystemNotificationChannels.DEVICE_ADMIN).setSmallIcon(R.drawable.ic_test_badge_experiment).setWhen(0L).setOngoing(true).setTicker(charSequence).setColor(context2.getColor(R.color.system_notification_accent_color)).setContentTitle(charSequence).setContentText(charSequence2).setVisibility(1).setContentIntent(activity).setStyle(new Notification.BigTextStyle().bigText(charSequence2)).setAutoCancel(true).build();
        build.semFlags |= 8;
            ...
            for (UserInfo userInfo : users) {
                if ((userInfo != null && userInfo.isManagedProfile() && !userInfo.isSecureFolder()) || SemPersonaManager.isDoEnabled(0)) {
                    ...
                    if (i != 0 && i2 != 0) {
                        ...
                        str = isTablet() ? context2.getString(17042463) + "\n - " + context2.getString(R.string.gnss_time_update_service) : context2.getString(17042462) + "\n - " + context2.getString(R.string.gnss_time_update_service);
                    } else {
                        Settings.System.putInt(context2.getContentResolver(), "marketing_info_agree", 0);
                        ...
                        str = isTablet() ? context2.getString(17042463) + "\n - " + context2.getString(R.string.permdesc_sdcardRead) : context2.getString(17042462) + "\n - " + context2.getString(R.string.permdesc_sdcardRead);
                    }
