...
public class BasicContainerAnalytics {
    ...
    if (!SemPersonaManager.isSecureFolderId(i) && !SemDualAppManager.isDualAppId(i)) {
      ...
      if (i != 0 || SemPersonaManager.isDoEnabled(0)) {
        ...
        try {
          ...
          boolean z2 = true;
          if (this.ifKnoxAnalyticsContainer.isOrganizationOwned()
              && !this.ifKnoxAnalyticsContainer.isDoEnabled(0)) {
            z = true;
          }
      ...
      if (str == null) {
        str =
            ((DevicePolicyManager) this.context.getSystemService("device_policy"))
                .getProfileOwnerAsUser(new UserHandle(i))
                .getPackageName();
      }
      bundle.putString("pN", str);
  ...
  public void logProfilePolicyRestriction(String str, int i, int i2) {
    ...
    try {
      str2 =
          ((DevicePolicyManager) this.context.getSystemService("device_policy"))
              .getProfileOwnerAsUser(new UserHandle(i2))
              .getPackageName();
    } catch (Exception e) {
      e.printStackTrace();
      str2 = null;
