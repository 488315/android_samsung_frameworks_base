...
public class StateMachineImpl {
  ...
  static {
    ...
    enumMap5.put(
        (EnumMap) Event.DDAR_WORKSPACE_AUTH_SUCCESS, (Event) State.DEVICE_UNLOCK_DATA_UNLOCK);
  ...
  public void setInitialState(int i, State state) {
    DDLog.m33d(
        ...
        "setInitialState() initialState: " + state + " for user " + i,
        new Object[0]);
  ...
  public boolean processEvent(Event event, int i) {
    ...
    synchronized (this.stateLock) {
      ...
      if (event != null && map.containsKey(event)) {
        ...
        DDLog.m34e(
            "DualDAR::StateMachine",
            "Invalid event " + event + " in state: " + currentState + " for user " + i,
            new Object[0]);
        state = null;
      }
      if (state == State.IDLE) {
  ...
  */
  public com.samsung.android.knox.dar.ddar.fsm.State getPreviousState(int r3) {
    ...
    */
    throw new UnsupportedOperationException(
        "Method not decompiled:"
            + " com.android.server.knox.dar.ddar.fsm.StateMachineImpl.getPreviousState(int):com.samsung.android.knox.dar.ddar.fsm.State");
  }
}
