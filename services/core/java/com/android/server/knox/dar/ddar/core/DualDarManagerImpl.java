...
public class DualDarManagerImpl {
      ...
      getDarManagerService()
          .ifPresent(
              new Consumer() { // from class:
                               // com.android.server.knox.dar.ddar.core.DualDarManagerImpl$$ExternalSyntheticLambda1
  ...
  public boolean handleEnsureDataUnlocked() {
    if (this.mLockPatternUtils.isSecure(0)) {
      Log.d(
          "DualDarManagerImpl",
          "handleEnsureDataUnlocked - Not able to ensure while device lockscreen is set");
      return false;
    }
  ...
  public boolean isInnerLayerUnlocked() {
    boolean dualDARLockstate =
        EnterprisePartitionManager.getInstance(this.mContext).getDualDARLockstate();
    ...
    if (DualDarManager.isOnDeviceOwner(i)) {
      ...
      try {
        ...
        if (telecomManager != null && telecomManager.isInCall()) {
          ...
          DDLog.m33d(
              "DualDarManagerImpl", "Perform hard reboot when datalock in DO case", new Object[0]);
  ...
  public boolean handleClearDualDarTrialExpiryTimer(Bundle bundle) {
    ...
    DDLog.m33d(
        "DualDarManagerImpl",
        "handleRemoveLicenseExpiryTimer cancelled alarm successful for user : " + i,
        new Object[0]);
