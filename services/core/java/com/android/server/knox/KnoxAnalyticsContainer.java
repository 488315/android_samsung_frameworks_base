...
public class KnoxAnalyticsContainer {
  ...
  public void postActiveUserChange(
      final int i, final ComponentName componentName, final boolean z) {
    this.analyticsHandler.post(
        new Runnable() { // from class: com.android.server.knox.KnoxAnalyticsContainer.1
    ...
    if (z && componentName != null && componentName.getPackageName() != null) {
      ...
      if (this.mPostActiveUserId != i || !this.mPostActivePackage.equals(packageName)) {
        if (SemPersonaManager.isKnoxId(this.mPostActiveUserId)) {
          checkTimeAndSendAnalytics(
              this.mPostActiveUserId,
              this.mPostActivePackage,
              SystemClock.elapsedRealtime() - this.mPostActiveTime);
        }
      ...
      if ((this.ifKnoxAnalyticsContainer.isDoEnabled(0)
              || this.ifKnoxAnalyticsContainer.isKnoxId(i)
              || (i == this.ifKnoxAnalyticsContainer.getSeparatedAppsUserId() && i > 0))
          && isActvtStmpNeeded(i)) {
        try {
          if (i == this.ifKnoxAnalyticsContainer.getSeparatedAppsUserId() && i > 0) {
            this.separatedAppsAnalytics.onAcitivtyChange(
                this.mPostActiveUserId, this.mPostActivePackage);
          } else {
            this.basicContainerAnalytics.logActivityChange(
                i, this.mPostActiveUserId, this.mPostActivePackage);
          }
        } catch (Exception e) {
  ...
  public final void handleSendLocationLog(int i) {
    try {
      if (i == 0) {
        if (((Integer) this.locationRestrictionMap.get(Integer.valueOf(i))).intValue()
            == (this.ifKnoxAnalyticsContainer.getLocationProvidersAllowed(i) == 0 ? 0 : 1)) {
          return;
        }
      } else {
        if (((Integer) this.locationRestrictionMap.get(Integer.valueOf(i))).intValue()
            == (true ^ this.ifKnoxAnalyticsContainer.hasUserRestriction("no_share_location", i))) {
          return;
        }
      }
  ...
  public final void sendSnapshotLog(int i) {
    ...
    try {
      ...
      Object[] objArr = this.ifKnoxAnalyticsContainer.getKnoxFingerprintPlus(i) == 1;
      bundle.putInt(
          "lckTp", getPasswordType(this.ifKnoxAnalyticsContainer.getActivePasswordQuality(i)));
      if (DEBUG) {
        Log.d("STATUS_SNAPSHOT", "bioInf " + (biometricState | biometricState2));
      }
      bundle.putInt("bioInf", biometricState2 | biometricState);
      bundle.putInt("mfaEnb", objArr != false ? 1 : 0);
      ...
      bundle.putInt(
          "nSntz", this.ifKnoxAnalyticsContainer.getLockScreenAllowPrivateNotification(i));
      if (i != 0) {
      ...
      if (containerType == 2) {
        ComponentName adminComponentForLegacy =
            this.ifKnoxAnalyticsContainer.getAdminComponentForLegacy(i);
        String packageName =
            adminComponentForLegacy != null ? adminComponentForLegacy.getPackageName() : "unknown";
        bundle.putString("daPn", packageName);
        bundle.putString(
            "daPv", this.ifKnoxAnalyticsContainer.getPackageInfo(packageName, 0).versionName);
      } else {
  ...
  public void onBroadcastIntentReceived(Context context, Intent intent) {
    ...
    if ("com.samsung.android.knox.profilepolicy.intent.action.update".equals(intent.getAction())) {
      this.basicContainerAnalytics.logProfilePolicyRestriction(
          intent.getStringExtra("restrictionName"),
          intent.getIntExtra("restrictionAllowed", 0),
          intent.getIntExtra(KnoxCustomManagerService.CONTAINER_ID_ZERO, 0));
    }
    if (((this.ifKnoxAnalyticsContainer.isDoEnabled(intExtra) || intExtra != 0)
            && !this.ifKnoxAnalyticsContainer.isSecureFolderId(intExtra)
            && (intExtra == 0
                || this.ifKnoxAnalyticsContainer.isKnoxId(intExtra)
                || intExtra == this.ifKnoxAnalyticsContainer.getSeparatedAppsUserId()))
        || "samsung.knox.intent.action.RCP_POLICY_CHANGED".equals(intent.getAction())
        || "android.intent.action.ACTION_SHUTDOWN".equals(intent.getAction())
        || "android.intent.action.SCREEN_OFF".equals(intent.getAction())
        || "samsung.knox.intent.action.rcp.MOVEMENT".equals(intent.getAction())) {
      if (("android.intent.action.PACKAGE_ADDED".equals(intent.getAction())
              || "android.intent.action.PACKAGE_REMOVED".equals(intent.getAction()))
          && SemPersonaManager.getKnoxContainerVersion()
                  .compareTo(SemPersonaManager.KnoxContainerVersion.KNOX_CONTAINER_VERSION_3_3_0)
              >= 0) {
        ...
        if (intExtra == this.ifKnoxAnalyticsContainer.getSeparatedAppsUserId()) {
          if (intExtra != 0) {
            this.separatedAppsAnalytics.onPackageChanged(
                intExtra,
                schemeSpecificPart,
                "android.intent.action.PACKAGE_ADDED".equals(intent.getAction()) ? 1 : 0);
            return;
          }
          return;
        } else {
          if (this.ifKnoxAnalyticsContainer.isKnoxId(intExtra)) {
            this.basicContainerAnalytics.logPackageChanged(
                intExtra,
                schemeSpecificPart,
                "android.intent.action.PACKAGE_ADDED".equals(intent.getAction()) ? 1 : 0);
            return;
      ...
      if ("android.intent.action.ACTION_SHUTDOWN".equals(intent.getAction())
        ...
        if (i != 0
            ...
            && SemPersonaManager.isKnoxId(this.mPostActiveUserId)) {
          checkTimeAndSendAnalytics(
              this.mPostActiveUserId,
              this.mPostActivePackage,
              SystemClock.elapsedRealtime() - this.mPostActiveTime);
      ...
      if ("samsung.knox.intent.action.rcp.MOVEMENT".equals(intent.getAction())) {
        this.basicContainerAnalytics.logMoveToKnox(
            intExtra, intent.getBooleanExtra("move_to_knox", false));
  ...
  public void knoxAnalyticsContainer(int i, String str) {
    ...
    if (str.equals("DUAL_DAR_USER_ADDED")) {
      KnoxAnalyticsData knoxAnalyticsData =
          new KnoxAnalyticsData("KNOX_DUALDAR", 1, "TYPE_OF_DUALDAR");
      IDualDARPolicy asInterface =
          IDualDARPolicy.Stub.asInterface(ServiceManager.getService("DualDARPolicy"));
      long clearCallingIdentity = Binder.clearCallingIdentity();
      boolean isDualDARIntentProvision =
          ContainerDependencyWrapper.isDualDARIntentProvision(this.context);
      ...
      Log.d(
          "PersonaManagerService:DualDARAnalytics",
          "Dual Dar Container Type Created: " + dualDARType);
  ...
  public final void saveJSONObjectForGivenPackage(
      SharedPreferences sharedPreferences, SharedPreferences.Editor editor, String str, long j) {
    JSONArray jSONArray;
    try {
  ...
  public final boolean ifSameDate() {
    ...
    return calendar2.get(1) == calendar.get(1)
        && calendar2.get(2) == calendar.get(2)
        && calendar2.get(5) == calendar.get(5);
  }
  ...
  public final void sendWorkUsageLogs(SharedPreferences sharedPreferences, int i) {
    ...
    try {
      ...
      for (int i2 = 0; i2 < jSONArray.length(); i2++) {
        ...
        this.basicContainerAnalytics.logWorkAppUsgae(
            i,
            jSONObject.getString("package_name"),
            jSONObject.getLong("usageTime"),
            jSONObject.getInt("appLaunchCount"));
      }
      if (DEBUG) {
