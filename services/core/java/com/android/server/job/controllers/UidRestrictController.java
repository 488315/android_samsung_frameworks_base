...
public final class UidRestrictController extends StateController {
                ...
                try {
                    if ("android.intent.ACTION_JOB_RESTRICT_UID".equals(intent.getAction())) {
                        ...
                        if (intExtra > 0) {
                            ...
                            UidRestrictController.this.setUidRestrictedLocked(
    ...
    public UidRestrictController(JobSchedulerService jobSchedulerService) {
        ...
        this.mContext.registerReceiver(
                new UidRestrictOperationListener(),
                BatteryService$$ExternalSyntheticOutline0.m(
                        "android.intent.ACTION_JOB_RESTRICT_UID"));
    }
    ...
    public final void dumpControllerStateLocked(
        ...
        for (Map.Entry entry : this.mTrackedTasks.entrySet()) {
            ...
            if (!arraySet.isEmpty()) {
                indentingPrintWriter.println(
                        "  uid=" + entry.getKey() + "(" + arraySet.size() + "):");
                ...
                for (int i = 0; i < arraySet.size(); i++) {
                    ...
                    if (jobSchedulerService$$ExternalSyntheticLambda5.test(jobStatus)) {
                        ...
                        protoOutputStream.write(
                                1133871366147L, this.mRestrictedUids.contains(Integer.valueOf(i2)));
    ...
    public final void maybeStartTrackingJobLocked(JobStatus jobStatus, JobStatus jobStatus2) {
        ...
        jobStatus.setConstraintSatisfied(
                1048576, elapsedRealtime, !this.mRestrictedUids.contains(Integer.valueOf(i)));
    }
    ...
    public final void maybeStopTrackingJobLocked(JobStatus jobStatus, JobStatus jobStatus2) {
        ...
        if (!jobStatus.clearTrackingController(256)
                || (arraySet =
                                (ArraySet)
                                        this.mTrackedTasks.get(
                                                Integer.valueOf(jobStatus.sourceUid)))
                        == null) {
            return;
        }
