...
public final class TimeController extends StateController {
  ...
  public TimeController(JobSchedulerService jobSchedulerService) {
    ...
    this.mDeadlineExpiredListener =
        new AlarmManager
            .OnAlarmListener() { // from class: com.android.server.job.controllers.TimeController.1
    ...
    this.mNextDelayExpiredListener =
        new AlarmManager
            .OnAlarmListener() { // from class: com.android.server.job.controllers.TimeController.2
          @Override // android.app.AlarmManager.OnAlarmListener
          public void onAlarm() {
            if (TimeController.DEBUG) {
              Slog.d("JobScheduler.Time", "Delay-expired alarm fired");
            }
            TimeController.this.mLastFiredDelayExpiredElapsedMillis =
  ...
  public void maybeStartTrackingJobLocked(JobStatus jobStatus, JobStatus jobStatus2) {
    ...
    if (jobStatus.hasTimingDelayConstraint() || jobStatus.hasDeadlineConstraint()) {
      ...
      if (jobStatus.hasTimingDelayConstraint()
          && evaluateTimingDelayConstraint(jobStatus, millis)
      ...
      while (true) {
        if (!listIterator.hasPrevious()) {
          z = false;
          break;
        } else if (((JobStatus) listIterator.previous()).getLatestRunTimeElapsed()
  ...
  public void evaluateStateLocked(JobStatus jobStatus) {
    ...
    if (jobStatus.hasDeadlineConstraint()
        ...
        && jobStatus.getLatestRunTimeElapsed() <= this.mNextJobExpiredElapsedMillis) {
    ...
    if (!jobStatus.hasTimingDelayConstraint()
        ...
        || jobStatus.getEarliestRunTime() > this.mNextDelayExpiredElapsedMillis) {
      return;
    }
  ...
  public final boolean canStopTrackingJobLocked(JobStatus jobStatus) {
    return (!jobStatus.hasTimingDelayConstraint()
            || jobStatus.isConstraintSatisfied(Integer.MIN_VALUE))
        && (!jobStatus.hasDeadlineConstraint() || jobStatus.isConstraintSatisfied(1073741824));
  }
        ...
        if (jobStatus.hasDeadlineConstraint()) {
          if (evaluateDeadlineConstraint(jobStatus, millis)) {
            ...
            if (DEBUG) {
              Slog.i(
                  "JobScheduler.Time",
                  "Skipping " + jobStatus + " because deadline won't make it ready.");
            }
          } else {
        ...
        if (jobStatus.hasTimingDelayConstraint()) {
          if (evaluateTimingDelayConstraint(jobStatus, millis)) {
            ...
            if (DEBUG) {
              Slog.i(
                  "JobScheduler.Time",
                  "Skipping " + jobStatus + " because delay won't make it ready.");
            }
          } else {
  ...
  public final void setDelayExpiredAlarmLocked(long j, WorkSource workSource) {
    long maybeAdjustAlarmTime =
        maybeAdjustAlarmTime(Math.max(j, this.mLastFiredDelayExpiredElapsedMillis + 30000));
    if (this.mNextDelayExpiredElapsedMillis == maybeAdjustAlarmTime) {
      return;
    }
    this.mNextDelayExpiredElapsedMillis = maybeAdjustAlarmTime;
    updateAlarmWithListenerLocked(
        "*job.delay*", 3, this.mNextDelayExpiredListener, maybeAdjustAlarmTime, workSource);
  }
  ...
  public final void setDeadlineExpiredAlarmLocked(long j, WorkSource workSource) {
    ...
    updateAlarmWithListenerLocked(
        "*job.deadline*", 2, this.mDeadlineExpiredListener, maybeAdjustAlarmTime, workSource);
  }
  ...
  public final void updateAlarmWithListenerLocked(
      String str,
      int i,
  ...
  public void dumpControllerStateLocked(
    ...
    TimeUtils.formatDuration(
        millis, this.mLastFiredDelayExpiredElapsedMillis, indentingPrintWriter);
    indentingPrintWriter.println();
    ...
    for (JobStatus jobStatus : this.mTrackedJobs) {
      if (predicate.test(jobStatus)) {
        ...
        if (jobStatus.hasDeadlineConstraint()) {
          TimeUtils.formatDuration(
              jobStatus.getLatestRunTimeElapsed(), millis, indentingPrintWriter);
        } else {
          indentingPrintWriter.print("N/A");
        }
        indentingPrintWriter.println();
