...
public class PrefetchController extends StateController {
  ...
  public final UsageStatsManagerInternal.EstimatedLaunchTimeChangedListener
      mEstimatedLaunchTimeChangedListener;
  ...
  public PrefetchController(JobSchedulerService jobSchedulerService) {
    ...
    UsageStatsManagerInternal.EstimatedLaunchTimeChangedListener
        estimatedLaunchTimeChangedListener =
            new UsageStatsManagerInternal
                .EstimatedLaunchTimeChangedListener() { // from class:
                                                        // com.android.server.job.controllers.PrefetchController.1
    ...
    this.mThresholdAlarmListener =
        new ThresholdAlarmListener(this.mContext, AppSchedulingModuleThread.get().getLooper());
  ...
  public void maybeStartTrackingJobLocked(JobStatus jobStatus, JobStatus jobStatus2) {
    if (jobStatus.getJob().isPrefetch()) {
      ...
      if (arraySet.add(jobStatus)
          && arraySet.size() == 1
          && !willBeLaunchedSoonLocked(sourceUserId, sourcePackageName, millis)) {
  ...
  public void maybeStopTrackingJobLocked(JobStatus jobStatus, JobStatus jobStatus2) {
    ...
    if (arraySet != null && arraySet.remove(jobStatus) && arraySet.size() == 0) {
      this.mThresholdAlarmListener.removeAlarmForKey(
          UserPackage.of(sourceUserId, sourcePackageName));
    }
  }
  ...
  public long getNextEstimatedLaunchTimeLocked(JobStatus jobStatus) {
    return getNextEstimatedLaunchTimeLocked(
  ...
  public final void maybeUpdateConstraintForUid(int i) {
    synchronized (this.mLock) {
      ...
      while (size >= 0) {
        ArraySet arraySet2 =
            (ArraySet) this.mTrackedJobs.get(userId, (String) packagesForUidLocked.valueAt(size));
        if (arraySet2 != null) {
          int i2 = 0;
  ...
  public final void processUpdatedEstimatedLaunchTime(int i, String str, long j) {
    ...
    if (z) {
      Slog.d(
          ...
          "Estimated launch time for "
              + StateController.packageToString(i, str)
              + " changed to "
              + j
              + " ("
              + TimeUtils.formatDuration(j - JobSchedulerService.sSystemClock.millis())
              + " from now)");
    }
    synchronized (this.mLock) {
      ArraySet arraySet = (ArraySet) this.mTrackedJobs.get(i, str);
      if (arraySet != null) {
        ...
        if (!arraySet.isEmpty()) {
          ...
          while (i2 < this.mPrefetchChangedListeners.size()) {
            ((PrefetchChangedListener) this.mPrefetchChangedListeners.valueAt(i2))
                .onPrefetchCacheUpdated(arraySet, i, str, longValue, j, millis2);
            i2++;
            arraySet = arraySet;
          }
        ...
        Slog.i(
            "JobScheduler.Prefetch",
            "Not caching launch time since we haven't seen any prefetch jobs for "
                + StateController.packageToString(i, str));
      }
    }
  }
  ...
  public final boolean updateConstraintLocked(JobStatus jobStatus, long j, long j2) {
    ...
    if (!(this.mService.getUidBias(jobStatus.getSourceUid()) == 40)) {
      int sourceUserId = jobStatus.getSourceUserId();
      String sourcePackageName = jobStatus.getSourcePackageName();
      if (!willBeLaunchedSoonLocked(sourceUserId, sourcePackageName, j)
          && ((appWidgetManager = this.mAppWidgetManager) == null
              || !appWidgetManager.isBoundWidgetPackage(sourcePackageName, sourceUserId))) {
        z = false;
      }
    } else {
  ...
  public final void updateThresholdAlarmLocked(int i, String str, long j, long j2) {
    ...
    if (nextEstimatedLaunchTimeLocked != Long.MAX_VALUE) {
      ...
      if (j3 > j4) {
        this.mThresholdAlarmListener.addAlarm(
            UserPackage.of(i, str), j2 + (nextEstimatedLaunchTimeLocked - (j + j4)));
        return;
      }
    }
    this.mThresholdAlarmListener.removeAlarmForKey(UserPackage.of(i, str));
  ...
  public final boolean willBeLaunchedSoonLocked(int i, String str, long j) {
    return getNextEstimatedLaunchTimeLocked(i, str, j)
        <= (j + this.mLaunchTimeThresholdMs) - this.mLaunchTimeAllowanceMs;
  }
      ...
      AppSchedulingModuleThread.getHandler()
          .post(
              new Runnable() { // from class:
                               // com.android.server.job.controllers.PrefetchController$$ExternalSyntheticLambda1
    ...
    public void processExpiredAlarms(ArraySet arraySet) {
      ...
      synchronized (PrefetchController.this.mLock) {
        ...
        while (i < arraySet.size()) {
          ...
          if (!PrefetchController.this.willBeLaunchedSoonLocked(
              userPackage.userId, userPackage.packageName, millis)) {
            Slog.e(
                "JobScheduler.Prefetch",
                "Alarm expired for "
                    + StateController.packageToString(userPackage.userId, userPackage.packageName)
                    + " at the wrong time");
            PrefetchController.this.updateThresholdAlarmLocked(
                userPackage.userId, userPackage.packageName, millis, millis2);
            j = millis;
          } else {
            j = millis;
            if (PrefetchController.this.maybeUpdateConstraintForPkgLocked(
                millis, millis2, userPackage.userId, userPackage.packageName)) {
              arraySet2.addAll(
                  (ArraySet)
                      PrefetchController.this.mTrackedJobs.get(
    ...
    public void handleMessage(Message message) {
      ...
      if (i != 0) {
        if (i == 1) {
          ...
          PrefetchController.this.processUpdatedEstimatedLaunchTime(
              someArgs.argi1, (String) someArgs.arg1, someArgs.argl1);
          someArgs.recycle();
          return;
        } else {
          if (i != 2) {
      ...
      long estimatedPackageLaunchTime =
          PrefetchController.this.mUsageStatsManagerInternal.getEstimatedPackageLaunchTime(str, i2);
      if (PrefetchController.DEBUG) {
        Slog.d(
            "JobScheduler.Prefetch",
            "Retrieved launch time for "
                + StateController.packageToString(i2, str)
                + " of "
                + estimatedPackageLaunchTime
                + " ("
                + TimeUtils.formatDuration(
                    estimatedPackageLaunchTime - JobSchedulerService.sSystemClock.millis())
                + " from now)");
      }
      synchronized (PrefetchController.this.mLock) {
        ...
        if (l == null || estimatedPackageLaunchTime != l.longValue()) {
          PrefetchController.this.processUpdatedEstimatedLaunchTime(
              i2, str, estimatedPackageLaunchTime);
        }
      }
    }
  }
  ...
  class PcConstants {
      ...
      if (str.equals(KEY_LAUNCH_TIME_THRESHOLD_MS)) {
        ...
        long min2 =
            Math.min(
                ...
                Math.max(ClipboardService.DEFAULT_CLIPBOARD_TIMEOUT_MILLIS, j2));
        if (PrefetchController.this.mLaunchTimeThresholdMs != min2) {
          PrefetchController.this.mLaunchTimeThresholdMs = min2;
          this.mShouldReevaluateConstraints = true;
          PrefetchController.this.mThresholdAlarmListener.setMinTimeBetweenAlarmsMs(
              PrefetchController.this.mLaunchTimeThresholdMs / 10);
        }
      }
    }
    ...
    public final void dump(IndentingPrintWriter indentingPrintWriter) {
      ...
      indentingPrintWriter
          .print(KEY_LAUNCH_TIME_THRESHOLD_MS, Long.valueOf(this.LAUNCH_TIME_THRESHOLD_MS))
          .println();
      indentingPrintWriter
          .print(KEY_LAUNCH_TIME_ALLOWANCE_MS, Long.valueOf(this.LAUNCH_TIME_ALLOWANCE_MS))
          .println();
  ...
  public void dumpControllerStateLocked(
      final IndentingPrintWriter indentingPrintWriter, final Predicate predicate) {
    long millis = JobSchedulerService.sSystemClock.millis();
    ...
    indentingPrintWriter.decreaseIndent();
    indentingPrintWriter.println();
    this.mTrackedJobs.forEach(
        new Consumer() { // from class:
                         // com.android.server.job.controllers.PrefetchController$$ExternalSyntheticLambda0
          @Override // java.util.function.Consumer
          public final void accept(Object obj) {
            PrefetchController.lambda$dumpControllerStateLocked$1(
                predicate, indentingPrintWriter, (ArraySet) obj);
          }
        });
    indentingPrintWriter.println();
  ...
  public static /* synthetic */ void lambda$dumpControllerStateLocked$1(
      Predicate predicate, IndentingPrintWriter indentingPrintWriter, ArraySet arraySet) {
