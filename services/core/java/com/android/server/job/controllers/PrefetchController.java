...
public final class PrefetchController extends StateController {
        ...
        public final void handleMessage(Message message) {
            ...
            if (i == 0) {
                ...
                long estimatedPackageLaunchTime =
                        UsageStatsService.this.getEstimatedPackageLaunchTime(i2, str);
                ...
                synchronized (PrefetchController.this.mLock) {
                    try {
                        ...
                        PrefetchController.m626$$Nest$mprocessUpdatedEstimatedLaunchTime(
                                PrefetchController.this, i2, str, estimatedPackageLaunchTime);
                    } finally {
                    }
                }
                return;
            ...
            synchronized (prefetchController.mLock) {
                try {
                    ArraySet packagesForUidLocked =
                            prefetchController.mService.getPackagesForUidLocked(i3);
                    ...
                    while (size >= 0) {
                        ArraySet arraySet2 =
                                (ArraySet)
                                        prefetchController.mTrackedJobs.get(
                                                userId,
                                                (String) packagesForUidLocked.valueAt(size));
                        if (arraySet2 != null) {
                            int i4 = 0;
                            while (i4 < arraySet2.size()) {
                                ...
                                int i6 = i4;
                                if (prefetchController.updateConstraintLocked(
                                        jobStatus, millis, elapsedRealtime)) {
                    ...
                    while (i < arraySet.size()) {
                        ...
                        if (PrefetchController.this.willBeLaunchedSoonLocked(
                            ...
                            Slog.e(
                                    "JobScheduler.Prefetch",
                                    "Alarm expired for "
                                            + StateController.packageToString(
    ...
    public static void m626$$Nest$mprocessUpdatedEstimatedLaunchTime(
            PrefetchController prefetchController, int i, String str, long j) {
        boolean z = DEBUG;
        if (z) {
            ...
            Slog.d(
                    ...
                    "Estimated launch time for "
                            ...
                            + " ("
                            + TimeUtils.formatDuration(
                                    j - JobSchedulerService.sSystemClock.millis())
                            + " from now)");
        }
        synchronized (prefetchController.mLock) {
            try {
                ...
                if (arraySet != null) {
                    long longValue =
                            ((Long) prefetchController.mEstimatedLaunchTimes.get(i, str))
                                    .longValue();
                    ...
                    if (!arraySet.isEmpty()) {
                        ...
                        JobSchedulerService.sElapsedRealtimeClock.getClass();
                        long elapsedRealtime = SystemClock.elapsedRealtime();
                        prefetchController.updateThresholdAlarmLocked(
                                i, millis, elapsedRealtime, str);
                        int i2 = 0;
                        while (i2 < prefetchController.mPrefetchChangedListeners.size()) {
                            ((FlexibilityController.AnonymousClass1)
                                            ((PrefetchChangedListener)
                                                    prefetchController.mPrefetchChangedListeners
                                                            .valueAt(i2)))
                                    .onPrefetchCacheUpdated(
                                            arraySet, i, str, longValue, j, elapsedRealtime);
                            i2++;
                            arraySet = arraySet;
                        }
                        ArraySet arraySet2 = arraySet;
                        if (prefetchController.maybeUpdateConstraintForPkgLocked(
                                i, millis, elapsedRealtime, str)) {
                            prefetchController.mStateChangedListener.onControllerStateChanged(
                                    arraySet2);
                        }
                    }
                } else if (z) {
    ...
    public final void dumpConstants(IndentingPrintWriter indentingPrintWriter) {
        ...
        JobSchedulerService$Constants$$ExternalSyntheticOutline0.m(
                pcConstants.LAUNCH_TIME_THRESHOLD_MS,
                indentingPrintWriter,
                "pc_launch_time_threshold_ms");
        indentingPrintWriter
                .print(
                        "pc_launch_time_allowance_ms",
                        Long.valueOf(pcConstants.LAUNCH_TIME_ALLOWANCE_MS))
                .println();
    ...
    public final void dumpControllerStateLocked(
            ...
            final JobSchedulerService$$ExternalSyntheticLambda5
    ...
    public final void maybeStartTrackingJobLocked(JobStatus jobStatus, JobStatus jobStatus2) {
        if (jobStatus.job.isPrefetch()) {
            ...
            if (arraySet.add(jobStatus)
                    && arraySet.size() == 1
                    && !willBeLaunchedSoonLocked(i, str, millis)) {
                                ...
                                public final void run() {
                                    ...
                                    synchronized (prefetchController.mLock) {
                                        try {
                                            ...
                                            long millis = JobSchedulerService.sSystemClock.millis();
                                            for (int i = 0;
                                                    i < prefetchController.mTrackedJobs.numMaps();
                                                    i++) {
                                                int keyAt =
                                                        prefetchController.mTrackedJobs.keyAt(i);
                                                int i2 = 0;
                                                while (i2
                                                        < prefetchController.mTrackedJobs
                                                                .numElementsForKey(keyAt)) {
                                                    String str =
                                                            (String)
                                                                    prefetchController.mTrackedJobs
                                                                            .keyAt(i, i2);
                                    ...
                                    if (arraySet.size() > 0) {
                                        prefetchController.mStateChangedListener
                                                .onControllerStateChanged(arraySet);
                                    }
                                }
                            });
        }
    ...
    public final void startTrackingLocked() {
        UsageStatsService.this.mEstimatedLaunchTimeChangedListeners.add(
                this.mEstimatedLaunchTimeChangedListener);
    }
    ...
    public final boolean updateConstraintLocked(JobStatus jobStatus, long j, long j2) {
        ...
        if (jobSchedulerService.getUidBias(i) == 40) {
            ...
            String str = jobStatus.sourcePackageName;
            z =
                    willBeLaunchedSoonLocked(i2, str, j)
                            || ((appWidgetManager = this.mAppWidgetManager) != null
                                    && appWidgetManager.isBoundWidgetPackage(str, i2));
        }
    ...
    public final void updateThresholdAlarmLocked(int i, long j, long j2, String str) {
        ...
        if (nextEstimatedLaunchTimeLocked != Long.MAX_VALUE) {
            ...
            if (j3 > j4) {
                thresholdAlarmListener.addAlarm(
                        j2 + (nextEstimatedLaunchTimeLocked - (j + j4)), UserPackage.of(i, str));
                return;
            }
        }
        thresholdAlarmListener.removeAlarmForKey(UserPackage.of(i, str));
