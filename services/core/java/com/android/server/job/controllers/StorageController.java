      ...
      for (int size = this.mTrackedTasks.size() - 1; size >= 0; size--) {
        z |=
            ((JobStatus) this.mTrackedTasks.valueAt(size))
                .setStorageNotLowConstraintSatisfied(millis, isStorageNotLow);
      }
    }
    if (isStorageNotLow) {
    ...
    public void onReceiveInternal(Intent intent) {
      ...
      if ("android.intent.action.DEVICE_STORAGE_LOW".equals(action)) {
        if (StorageController.DEBUG) {
          Slog.d(
              "JobScheduler.Storage",
              "Available storage too low to do work. @ "
      ...
      if ("android.intent.action.DEVICE_STORAGE_OK".equals(action)) {
        if (StorageController.DEBUG) {
          Slog.d(
              "JobScheduler.Storage",
              "Available storage high enough to do work. @ "
                  + JobSchedulerService.sElapsedRealtimeClock.millis());
        }
