...
public final class JobStatus {
  ...
  */
  public JobStatus(
    ...
    if (i2 != -1 && str != null) {
      ...
      String str6 =
          str4 == null
              ...
              : str5 + jobInfo.getService().flattenToShortString();
      this.batteryName = str6;
      this.tag = "*job*/" + str6 + "#" + jobInfo.getId();
      ...
      constraintFlags =
          jobInfo.getRequiredNetwork() != null ? constraintFlags | 268435456 : constraintFlags;
      ...
      constraintFlags =
          UidRestrictController.isUidIgnored(this.sourceUid)
              ? constraintFlags
              : constraintFlags | 1048576;
      if (jobInfo.getTriggerContentUris() != null) {
        ...
        this.mRequiredConstraintsOfInterest = constraintFlags & (-1800404977);
        ...
        if (jobInfo.getRequiredNetwork() != null) {
          ...
          NetworkRequest.Builder builder2 =
              new NetworkRequest.Builder(jobInfo.getRequiredNetwork());
          builder2.setUids(
              Collections.singleton(
                  new Range(Integer.valueOf(this.sourceUid), Integer.valueOf(this.sourceUid))));
    ...
    if (UidRestrictController.isUidIgnored(this.sourceUid)) {}
    if (jobInfo.getTriggerContentUris() != null) {}
    z = false;
    this.mHasExemptedMediaUrisOnly = z;
    preferredConstraintFlags = jobInfo.getPreferredConstraintFlags();
    ...
    if (preferredConstraintFlags == 0) {}
    this.mNumRequiredFlexibleConstraints = 0;
    this.requiredConstraints = constraintFlags2;
    this.mRequiredConstraintsOfInterest = constraintFlags2 & (-1800404977);
    addDynamicConstraints(i7);
    this.mReadyNotDozing = canRunInDoze();
    if (i3 == 5) {}
    this.mCumulativeExecutionTimeMs = j5;
  ...
  public JobStatus(JobStatus jobStatus) {
    this(
        ...
        jobStatus.getSourceTag(),
        jobStatus.getNumFailures(),
        jobStatus.getNumSystemStops(),
  ...
  public static JobStatus createFromJobInfo(
    ...
    if (jobInfo.isPeriodic()) {
      long max =
          Math.max(
              JobInfo.getMinPeriodMillis(), Math.min(31536000000L, jobInfo.getIntervalMillis()));
      maxExecutionDelayMillis = millis + max;
      minLatencyMillis =
          maxExecutionDelayMillis
              - Math.max(JobInfo.getMinFlexMillis(), Math.min(max, jobInfo.getFlexMillis()));
    } else {
      minLatencyMillis = jobInfo.hasEarlyConstraint() ? jobInfo.getMinLatencyMillis() + millis : 0L;
      maxExecutionDelayMillis =
          jobInfo.hasLateConstraint()
              ? jobInfo.getMaxExecutionDelayMillis() + millis
              : Long.MAX_VALUE;
    }
    return new JobStatus(
        ...
        JobSchedulerService.standbyBucketForPackage(
            str != null ? str : jobInfo.getService().getPackageName(), i2, millis),
        str2,
        str3,
        0,
        0,
  ...
  public void enqueueWorkLocked(JobWorkItem jobWorkItem) {
    ...
    if (jobWorkItem.getIntent() != null
        && GrantedUriPermissions.checkGrantFlags(jobWorkItem.getIntent().getFlags())) {
      jobWorkItem.setGrants(
          GrantedUriPermissions.createFromIntent(
  ...
  public void prepareLocked() {
    ...
    if (clipData != null) {
      this.uriPerms =
          GrantedUriPermissions.createFromClip(
              clipData,
              this.sourceUid,
              this.sourcePackageName,
              this.sourceUserId,
              this.job.getClipGrantFlags(),
              toShortString());
    }
  }
  ...
  public int getEffectiveStandbyBucket() {
    ...
    boolean isAppConsideredBuggy =
        ((JobSchedulerInternal) LocalServices.getService(JobSchedulerInternal.class))
            .isAppConsideredBuggy(
                getUserId(),
                getServiceComponent().getPackageName(),
  ...
  public boolean updateMediaBackupExemptionStatus() {
    boolean z =
        this.mHasExemptedMediaUrisOnly
            && !this.job.hasLateConstraint()
            && this.job.getRequiredNetwork() != null
            && getEffectivePriority() >= 300
            && this.sourcePackageName.equals(
                ((JobSchedulerInternal) LocalServices.getService(JobSchedulerInternal.class))
                    .getCloudMediaProviderPackage(this.sourceUserId));
  ...
  public int getEffectivePriority() {
    int min =
        Math.min(
            (getInternalFlags() & 2) != 0
                    || (this.job.isUserInitiated() && (getInternalFlags() & 4) != 0)
                ? 400
                : 500,
  ...
  public void maybeAddForegroundExemption(Predicate predicate) {
    if (this.job.hasEarlyConstraint()
        ...
        || !predicate.test(Integer.valueOf(getSourceUid()))) {
      return;
    }
    addInternalFlags(1);
  }
  ...
  public boolean canRunInBatterySaver() {
    ...
    return (shouldTreatAsExpeditedJob() || this.startedAsExpeditedJob)
        && (this.mDynamicConstraints & 4194304) == 0;
  }
  ...
  public void adjustNumRequiredFlexibleConstraints(int i) {
    this.mNumDroppedFlexibleConstraints =
        Math.max(
            0,
            Math.min(
                this.mNumRequiredFlexibleConstraints, this.mNumDroppedFlexibleConstraints - i));
  }
  ...
  public final boolean isReady(int i) {
    if (((this.mReadyWithinQuota && this.mReadyTareWealth)
            || this.mReadyDynamicSatisfied
  ...
  public String toString() {
    ...
    if (this.earliestRunTimeElapsedMillis != 0
        || this.latestRunTimeElapsedMillis != Long.MAX_VALUE) {
      long millis = JobSchedulerService.sElapsedRealtimeClock.millis();
      sb.append(" TIME=");
  ...
  public final void dumpJobWorkItem(
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter, boolean z, long j) {
    ...
    if (z) {
      ...
      if (this.job.getTriggerContentUris() != null) {
        ...
        indentingPrintWriter
            .print("Has media backup exemption", Boolean.valueOf(this.mHasMediaBackupExemption))
            .println();
      }
    ...
    if (z) {
      ...
      dumpConstraints(
          indentingPrintWriter,
          (this.requiredConstraints | 16777216 | 134217728) & (~this.satisfiedConstraints));
      indentingPrintWriter.println();
    ...
    formatRunTime(
        (PrintWriter) indentingPrintWriter, this.latestRunTimeElapsedMillis, Long.MAX_VALUE, j);
    indentingPrintWriter.print(", original latest=");
    formatRunTime(
        (PrintWriter) indentingPrintWriter,
        this.mOriginalLatestRunTimeElapsedMillis,
        Long.MAX_VALUE,
        j);
    indentingPrintWriter.println();
  ...
  public void dump(ProtoOutputStream protoOutputStream, long j, boolean z, long j2) {
    ...
    if (z) {
      ...
      dumpConstraints(
          protoOutputStream,
          2259152797705L,
          (this.requiredConstraints | 16777216) & (~this.satisfiedConstraints));
    ...
    if (this.executingWork != null) {
      for (int i5 = 0; i5 < this.executingWork.size(); i5++) {
        dumpJobWorkItem(
            protoOutputStream, 2246267895824L, (JobWorkItem) this.executingWork.get(i5));
      }
    }
