...
public final class JobServiceContext implements ServiceConnection {
  ...
  static {
    ...
    sEnqueuedJwiAtJobStart =
        new Histogram(
            "job_scheduler.value_hist_w_uid_enqueued_work_items_at_job_start",
            new Histogram.ScaledRangeOptions(20, 1, 3.0f, 1.4f));
    sTransferredNetworkDownloadKBHighWaterMarkLogger =
        new Histogram(
    ...
    sTransferredNetworkUploadKBHighWaterMarkLogger =
        new Histogram(
            ...
            new Histogram.ScaledRangeOptions(50, 0, 32.0f, 1.31f));
    sUpdatedEstimatedNetworkDownloadKBLogger =
        new Histogram(
            "job_scheduler.value_hist_updated_estimated_network_download_kilobytes",
            new Histogram.ScaledRangeOptions(50, 0, 32.0f, 1.31f));
    sUpdatedEstimatedNetworkUploadKBLogger =
        new Histogram(
  ...
  */
  public boolean executeRunnableJob(JobStatus jobStatus, int i) {
    ...
    synchronized (this.mLock) {
      ...
      ArraySet arraySet = jobStatus.changedUris;
      if (arraySet != null) {
      ...
      ArraySet arraySet2 = jobStatus.changedAuthorities;
      if (arraySet2 != null) {
      ...
      this.mParams =
          new JobParameters(
              ...
              jobStatus.getJobId(),
              job.getExtras(),
              job.getTransientExtras(),
              job.getClipData(),
              job.getClipGrantFlags(),
              z3,
              jobStatus.shouldTreatAsExpeditedJob(),
              jobStatus.shouldTreatAsUserInitiatedJob(),
              uriArr,
              strArr,
              canGetNetworkInformation(jobStatus) ? jobStatus.network : null);
      ...
      this.mMaxExecutionTimeMillis =
          Math.max(
              this.mService.getMaxJobExecutionTimeMs(jobStatus), this.mMinExecutionGuaranteeMillis);
      ...
      newWakeLock.setWorkSource(
          this.mService.deriveWorkSource(
      ...
      this.mEconomyManagerInternal.noteInstantaneousEvent(
          ...
          getStartActionId(jobStatus),
          String.valueOf(jobStatus.getJobId()));
      this.mVerb = 0;
        ...
        try {
          z2 =
              this.mContext.bindServiceAsUser(
                  flags, this, of, UserHandle.of(jobStatus.getUserId()));
        } catch (SecurityException e) {
          e = e;
          Slog.w(
              "JobServiceContext",
              "Job service "
                  + jobStatus.getServiceComponent().getShortClassName()
                  + " cannot be executed: "
                  + e.getMessage());
          z2 = false;
          boolean z4 = z;
      ...
      if (z2) {
        if (DEBUG) {
          Slog.d(
              "JobServiceContext",
              jobStatus.getServiceComponent().getShortClassName() + " unavailable.");
        }
      ...
      FrameworkStatsLog.write_non_chained(
          ...
          this.mExecutionStartTimeElapsed - jobStatus.enqueueTime,
          ...
          ActivityManager.processStateAmToProto(this.mService.getUidProcState(jobStatus.getUid())),
      ...
      if (Trace.isTagEnabled(524288L)) {
        ...
        if (!sourcePackageName.equals(packageName)) {
          str = str + XmlUtils.STRING_ARRAY_SEPARATOR + packageName;
        }
        String str2 = str + "/" + jobStatus.getServiceComponent().getShortClassName();
        ...
        Trace.asyncTraceForTrackBegin(
            524288L, "JobScheduler", str2 + "#" + jobStatus.getJobId(), getId());
      }
      try {
      ...
      ((UsageStatsManagerInternal) LocalServices.getService(UsageStatsManagerInternal.class))
          .setLastJobRunTime(
              sourcePackageName, jobStatus.getSourceUserId(), this.mExecutionStartTimeElapsed);
  ...
  public final boolean canGetNetworkInformation(JobStatus jobStatus) {
    ...
    return !CompatChanges.isChangeEnabled(271850009L, uid)
        || hasPermissionForDelivery(
            uid,
            jobStatus.getServiceComponent().getPackageName(),
            "android.permission.ACCESS_NETWORK_STATE");
  }

  public final boolean hasPermissionForDelivery(int i, String str, String str2) {
    return PermissionChecker.checkPermissionForDataDelivery(
            this.mContext, str2, -1, i, str, (String) null, "network info via JS")
        == 0;
  }
  ...
  public static int getStartActionId(JobStatus jobStatus) {
    ...
    Slog.wtf(
        "JobServiceContext",
        "Unknown priority: " + JobInfo.getPriorityString(jobStatus.getEffectivePriority()));
    return 1610612740;
  }
  ...
  public void markForProcessDeathLocked(int i, int i2, String str) {
    if (this.mVerb == 4) {
      if (DEBUG) {
        Slog.d(
            "JobServiceContext", "Too late to mark for death (verb=" + this.mVerb + "), ignoring.");
        return;
      }
      return;
    }
    if (DEBUG) {
      Slog.d(
          "JobServiceContext",
          "Marking "
              + this.mRunningJob.toShortString()
              + " for death because "
              + i
  ...
  public void informOfNetworkChangeLocked(Network network) {
    JobStatus jobStatus;
    if (network != null
        && (jobStatus = this.mRunningJob) != null
        && !canGetNetworkInformation(jobStatus)) {
      if (DEBUG) {
    ...
    if (this.mVerb != 2) {
      Slog.w(
          "JobServiceContext",
          "Sending onNetworkChanged for a job that isn't started. " + this.mRunningJob);
      int i = this.mVerb;
      if (i == 0 || i == 1) {
  ...
  public boolean isWithinExecutionGuaranteeTime() {
    return JobSchedulerService.sElapsedRealtimeClock.millis()
        < this.mExecutionStartTimeElapsed + this.mMinExecutionGuaranteeMillis;
  }

  public boolean stopIfExecutingLocked(
      String str, int i, String str2, boolean z, int i2, int i3, int i4) {
    ...
    if ((str != null && !str.equals(runningJobLocked.getSourcePackageName()))
        || !Objects.equals(str2, runningJobLocked.getNamespace())) {
      return false;
    }
  ...
  public Pair getEstimatedNetworkBytes() {
    return Pair.create(
        Long.valueOf(this.mEstimatedDownloadBytes), Long.valueOf(this.mEstimatedUploadBytes));
  }

  public Pair getTransferredNetworkBytes() {
    return Pair.create(
        Long.valueOf(this.mTransferredDownloadBytes), Long.valueOf(this.mTransferredUploadBytes));
  }
  ...
  public final void doAcknowledgeGetTransferredDownloadBytesMessage(
      JobCallback jobCallback, int i, int i2, long j) {
    synchronized (this.mLock) {
      if (verifyCallerLocked(jobCallback)) {
  ...
  public final void doAcknowledgeGetTransferredUploadBytesMessage(
      JobCallback jobCallback, int i, int i2, long j) {
    synchronized (this.mLock) {
      if (verifyCallerLocked(jobCallback)) {
  ...
  public final void doUpdateEstimatedNetworkBytes(
      JobCallback jobCallback, int i, JobWorkItem jobWorkItem, long j, long j2) {
    synchronized (this.mLock) {
      if (verifyCallerLocked(jobCallback)) {
        Counter.logIncrementWithUid(
            "job_scheduler.value_cntr_w_uid_estimated_network_bytes_updated",
            this.mRunningJob.getUid());
        sUpdatedEstimatedNetworkDownloadKBLogger.logSample(
            JobSchedulerService.safelyScaleBytesToKBForHistogram(j));
        sUpdatedEstimatedNetworkUploadKBLogger.logSample(
            JobSchedulerService.safelyScaleBytesToKBForHistogram(j2));
  ...
  public final void doUpdateTransferredNetworkBytes(
      JobCallback jobCallback, int i, JobWorkItem jobWorkItem, long j, long j2) {
    synchronized (this.mLock) {
      if (verifyCallerLocked(jobCallback)) {
        Counter.logIncrementWithUid(
  ...
  public final void doSetNotification(
      JobCallback jobCallback, int i, int i2, Notification notification, int i3) {
      ...
      synchronized (this.mLock) {
        if (verifyCallerLocked(jobCallback)) {
          if (callingUid != this.mRunningJob.getUid()) {
            Slog.wtfStack(
                "JobServiceContext", "Calling UID isn't the same as running job's UID...");
            throw new SecurityException("Can't post notification on behalf of another app");
          }
          this.mNotificationCoordinator.enqueueNotification(
              this,
              this.mRunningJob.getServiceComponent().getPackageName(),
              callingPid,
              callingUid,
              i2,
              notification,
  ...
  public void onServiceDisconnected(ComponentName componentName) {
    synchronized (this.mLock) {
      ...
      if (i != 0) {
        this.mParams.setStopReason(
            i, this.mDeathMarkInternalStopReason, this.mDeathMarkDebugReason);
      } else {
        JobStatus jobStatus = this.mRunningJob;
        if (jobStatus != null) {
          Counter.logIncrementWithUid(
              "job_scheduler.value_cntr_w_uid_unexpected_service_disconnects", jobStatus.getUid());
        }
      }
    ...
    synchronized (this.mLock) {
      ...
      if (jobStatus == null) {
        ...
        Slog.e(
            "JobServiceContext",
            "Binding died for "
                + this.mRunningJob.getSourceUserId()
                + XmlUtils.STRING_ARRAY_SEPARATOR
                + componentName.getPackageName());
      } else {
        Slog.e(
  ...
  public final boolean assertCallerLocked(JobCallback jobCallback) {
    ...
    if (!this.mPreviousJobHadSuccessfulFinish
        && millis - this.mLastUnsuccessfulFinishElapsed < 15000) {
      return false;
    }
    StringBuilder sb = new StringBuilder(128);
          ...
          if (message.obj == JobServiceContext.this.mRunningCallback) {
            ...
            if (jobCallback.mStoppedReason != null) {
              ...
              TimeUtils.formatDuration(
                  JobSchedulerService.sElapsedRealtimeClock.millis() - jobCallback.mStoppedTime,
                  sb);
              sb.append(" because: ");
              sb.append(jobCallback.mStoppedReason);
            }
  ...
  public void doCallbackLocked(boolean z, String str) {
    ...
    if (z2) {
      Slog.d(
          "JobServiceContext",
          "doCallback of : " + this.mRunningJob + " v:" + VERB_STRINGS[this.mVerb]);
    }
    removeOpTimeOutLocked();
    int i = this.mVerb;
    if (i == 1) {
  ...
  public final void doCancelLocked(int i, int i2, String str) {
    ...
    if (i3 == 4 || i3 == 3) {
      if (DEBUG) {
        Slog.d(
            "JobServiceContext",
            "Too late to process cancel for context (verb=" + this.mVerb + "), ignoring.");
        return;
      }
      return;
    }
    if (this.mRunningJob.startedAsExpeditedJob && i == 10) {
      if (JobSchedulerService.sElapsedRealtimeClock.millis()
          < this.mExecutionStartTimeElapsed + this.mMinExecutionGuaranteeMillis) {
  ...
  public final void handleServiceBoundLocked() {
    ...
    if (this.mVerb != 0) {
      Slog.e(
          "JobServiceContext",
          "Sending onStartJob for a job that isn't pending. " + VERB_STRINGS[this.mVerb]);
    ...
    if (this.mCancelled) {
      if (z) {
        Slog.d(
            "JobServiceContext",
            "Job cancelled while waiting for bind to complete. " + this.mRunningJob);
      }
    ...
    try {
      ...
      Slog.e(
          "JobServiceContext",
          "Error sending onStart message to '"
              + this.mRunningJob.getServiceComponent().getShortClassName()
              + "' ",
          e);
    }
  }
  ...
  public final void handleStartedLocked(boolean z) {
    if (this.mVerb == 1) {
      ...
      scheduleOpTimeOutLocked();
      if (this.mPendingNetworkChange != null
          && !Objects.equals(this.mParams.getNetwork(), this.mPendingNetworkChange)) {
    ...
    Slog.e(
        "JobServiceContext",
        "Handling started job but job wasn't starting! Was " + VERB_STRINGS[this.mVerb] + ".");
  }
  ...
  public final void handleFinishedLocked(boolean z, String str) {
    ...
    Slog.e(
        "JobServiceContext",
        "Got an execution complete message for a job that wasn't beingexecuted. Was "
            + VERB_STRINGS[this.mVerb]
            + ".");
  }
  ...
  public final void handleCancelLocked(String str) {
    if (JobSchedulerService.DEBUG) {
      Slog.d(
          "JobServiceContext",
          "Handling cancel for: " + this.mRunningJob.getJobId() + " " + VERB_STRINGS[this.mVerb]);
    }
    int i = this.mVerb;
    if (i == 0 || i == 1) {
  ...
  public final void handleOpTimeoutLocked() {
    ...
    if (i == 1) {
      onSlowAppResponseLocked(
          ...
          CompatChanges.isChangeEnabled(258236856L, this.mRunningJob.getUid()));
      return;
    }
    if (i != 2) {
      if (i == 3) {
      ...
      Slog.e(
          "JobServiceContext",
          "Handling timeout for an invalid job state: "
    ...
    if (this.mPendingStopReason != 0) {
      if (this.mService.isReadyToBeExecutedLocked(this.mRunningJob, false)) {
        ...
        Slog.i(
            "JobServiceContext",
            "JS was waiting to stop this job. Sending onStop: " + getRunningJobNameLocked());
        this.mParams.setStopReason(
            this.mPendingStopReason, this.mPendingInternalStopReason, this.mPendingDebugStopReason);
    ...
    if (millis >= j2) {
      Slog.i(
          "JobServiceContext",
          "Client timed out while executing (no jobFinished received). Sending onStop: "
              + getRunningJobNameLocked());
    ...
    if (millis >= j3) {
      String shouldStopRunningJobLocked =
          this.mJobConcurrencyManager.shouldStopRunningJobLocked(this);
      if (shouldStopRunningJobLocked != null) {
        Slog.i(
            "JobServiceContext",
            "Stopping client after min execution time: "
                + getRunningJobNameLocked()
                + " because "
      ...
      Slog.i(
          "JobServiceContext",
          "Letting " + getRunningJobNameLocked() + " continue to run past min execution time");
      scheduleOpTimeOutLocked();
      return;
    }
    if (this.mAwaitingNotification) {
      onSlowAppResponseLocked(
          ...
          "timed out while stopping",
          "required notification not provided",
          true);
      return;
    }
  ...
  public final void sendStopMessageLocked(String str) {
    ...
    if (this.mVerb != 2) {
      Slog.e(
          "JobServiceContext",
          "Sending onStopJob for a job that isn't started. " + this.mRunningJob);
  ...
  public final void onSlowAppResponseLocked(
      boolean z, boolean z2, String str, String str2, String str3, boolean z3) {
    ...
    if (z3) {
      ...
      JobStatus jobStatus = this.mRunningJob;
      activityManagerInternal.appNotResponding(
          jobStatus.serviceProcessName, jobStatus.getUid(), TimeoutRecord.forJobService(str3));
    }
  ...
  public final void closeAndCleanupJobLocked(boolean z, String str) {
    ...
    applyStoppedReasonLocked(str);
    JobStatus jobStatus = this.mRunningJob;
    jobStatus.incrementCumulativeExecutionTime(
        millis - this.mLastExecutionDurationStampTimeElapsed);
    ...
    if (this.mDeathMarkStopReason != 0) {
      if (z2) {
        Slog.d(
            "JobServiceContext",
            "Job marked for death because of "
                + JobParameters.getInternalReasonCodeDescription(this.mDeathMarkInternalStopReason)
                + ": "
                + this.mDeathMarkDebugReason);
      }
    ...
    FrameworkStatsLog.write_non_chained(
        ...
        jobStatus.getNumPreviousAttempts(),
        jobStatus.getJob().getMaxExecutionDelayMillis(),
        this.mParams.isOverrideDeadlineExpired(),
        jobStatus.isConstraintSatisfied(1),
        jobStatus.isConstraintSatisfied(2),
        jobStatus.isConstraintSatisfied(8),
        jobStatus.isConstraintSatisfied(Integer.MIN_VALUE),
    ...
    try {
      this.mBatteryStats.noteJobFinish(
          this.mRunningJob.getBatteryName(),
          this.mRunningJob.getSourceUid(),
          internalStopReasonCode);
    } catch (RemoteException unused) {
    }
    if (stopReason == 3) {
      this.mEconomyManagerInternal.noteInstantaneousEvent(
          this.mRunningJob.getSourceUserId(),
          this.mRunningJob.getSourcePackageName(),
          1610612746,
          String.valueOf(this.mRunningJob.getJobId()));
    }
  ...
  public final void scheduleOpTimeOutLocked() {
    ...
    if (DEBUG) {
      Slog.d(
          "JobServiceContext",
          "Scheduling time out for '"
              + this.mRunningJob.getServiceComponent().getShortClassName()
              + "' jId: "
              + this.mParams.getJobId()
              + ", in "
              + (j / 1000)
              + " s");
    }
    this.mCallbackHandler.sendMessageDelayed(
        this.mCallbackHandler.obtainMessage(0, this.mRunningCallback), j);
  ...
  public void dumpLocked(IndentingPrintWriter indentingPrintWriter, long j) {
    ...
    TimeUtils.formatDuration(
        (this.mExecutionStartTimeElapsed + this.mMinExecutionGuaranteeMillis) - j,
        indentingPrintWriter);
    indentingPrintWriter.print(", ");
    TimeUtils.formatDuration(
        (this.mExecutionStartTimeElapsed + this.mMaxExecutionTimeMillis) - j, indentingPrintWriter);
    indentingPrintWriter.print("]");
    if (this.mPendingStopReason != 0) {
