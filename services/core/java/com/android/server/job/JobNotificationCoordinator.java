...
public class JobNotificationCoordinator {
  ...
  public final NotificationManagerInternal mNotificationManagerInternal =
  ...
  public void enqueueNotification(
    ...
    NotificationDetails notificationDetails =
        (NotificationDetails) this.mNotificationDetails.get(jobServiceContext);
    ...
    if (runningJobLocked.startedAsUserInitiatedJob) {
      ...
      synchronized (this.mUijLock) {
        ...
        ((ArraySet) this.mUijNotificationChannels.get(userId, str))
            .add(notification.getChannelId());
      }
    }
    UserPackage of = UserPackage.of(userId, str);
    NotificationDetails notificationDetails2 =
        new NotificationDetails(of, i, i2, i3, notification.getChannelId(), i4);
    ...
    this.mNotificationManagerInternal.enqueueNotification(
        str, str, i2, i, null, i3, notification, userId);
  }

  public void removeNotificationAssociation(
      JobServiceContext jobServiceContext, int i, JobStatus jobStatus) {
    NotificationDetails notificationDetails =
        (NotificationDetails) this.mNotificationDetails.remove(jobServiceContext);
    if (notificationDetails == null) {
      return;
    }
    SparseSetArray sparseSetArray =
        (SparseSetArray) this.mCurrentAssociations.get(notificationDetails.userPackage);
    if (sparseSetArray == null
        || !sparseSetArray.remove(notificationDetails.notificationId, jobServiceContext)) {
    ...
    if (arraySet == null || arraySet.isEmpty()) {
      if (notificationDetails.jobEndNotificationPolicy == 1 || i == 11 || i == 13) {
        this.mNotificationManagerInternal.cancelNotification(
            str, str, notificationDetails.appUid, notificationDetails.appPid, null, i2, userId);
        z = false;
      }
    } else {
  ...
  public final boolean isNotificationUsedForAnyUij(int i, String str, int i2) {
    ArraySet arraySet;
    SparseSetArray sparseSetArray =
        (SparseSetArray) this.mCurrentAssociations.get(UserPackage.of(i, str));
    if (sparseSetArray == null || (arraySet = sparseSetArray.get(i2)) == null) {
      return false;
    }
    for (int size = arraySet.size() - 1; size >= 0; size--) {
      JobStatus runningJobLocked =
          ((JobServiceContext) arraySet.valueAt(size)).getRunningJobLocked();
  ...
  public final void maybeDeleteNotificationChannelAssociation(int i, String str, String str2) {
    ...
    for (int size = this.mNotificationDetails.size() - 1; size >= 0; size--) {
      JobServiceContext jobServiceContext =
          (JobServiceContext) this.mNotificationDetails.keyAt(size);
      NotificationDetails notificationDetails =
          (NotificationDetails) this.mNotificationDetails.get(jobServiceContext);
      if (notificationDetails != null
          && UserHandle.getUserId(notificationDetails.appUid) == i
          && notificationDetails.userPackage.packageName.equals(str)
          && notificationDetails.notificationChannel.equals(str2)
          && (runningJobLocked = jobServiceContext.getRunningJobLocked()) != null
          && runningJobLocked.startedAsUserInitiatedJob) {
        return;
      }
    }
    synchronized (this.mUijLock) {
  ...
  public final void validateNotification(String str, int i, Notification notification, int i2) {
    ...
    if (this.mNotificationManagerInternal.getNotificationChannel(
            str, i, notification.getChannelId())
        == null) {
