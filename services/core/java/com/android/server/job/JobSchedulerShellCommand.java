...
public final class JobSchedulerShellCommand extends BasicShellCommandHandler {
  ...
  public final void checkPermission(String str) {
    ...
    if (callingUid == 0
        || this.mPM.checkUidPermission("android.permission.CHANGE_APP_IDLE_STATE", callingUid)
            == 0) {
      return;
    }
      ...
      if (nextOption == null) {
        ...
        try {
          int executeRunCommand =
              this.mInternal.executeRunCommand(nextArgRequired, i, str, parseInt, z, z2);
  ...
  public final int timeout(PrintWriter printWriter) {
    ...
    while (true) {
      ...
      if (nextOption != null) {
        ...
        long clearCallingIdentity = Binder.clearCallingIdentity();
        try {
          return this.mInternal.executeStopCommand(
        ...
        try {
          int estimatedNetworkBytes =
              this.mInternal.getEstimatedNetworkBytes(
                  printWriter, nextArgRequired, currentUser, str, parseInt, i);
  ...
  public final int getJobState(PrintWriter printWriter) {
    ...
    while (true) {
      ...
      if (nextOption != null) {
        ...
        try {
          int jobState =
              this.mInternal.getJobState(printWriter, nextArgRequired, currentUser, str, parseInt);
  ...
  */
  ...
  public void onHelp() {
    ...
    outPrintWriter.println(
        "  run [-f | --force] [-s | --satisfied] [-u | --user USER_ID] [-n | --namespace NAMESPACE]"
            + " PACKAGE JOB_ID");
    outPrintWriter.println(
    ...
    outPrintWriter.println(
        "  stop [-u | --user USER_ID] [-n | --namespace NAMESPACE] [-s | --stop-reason STOP_REASON]"
            + " [-i | --internal-stop-reason STOP_REASON] [PACKAGE] [JOB_ID]");
    outPrintWriter.println(
    ...
    outPrintWriter.println(
        "  get-estimated-download-bytes [-u | --user USER_ID] [-n | --namespace NAMESPACE] PACKAGE"
            + " JOB_ID");
    ...
    outPrintWriter.println("         the primary or system user");
    outPrintWriter.println(
        "  get-estimated-upload-bytes [-u | --user USER_ID] [-n | --namespace NAMESPACE] PACKAGE"
            + " JOB_ID");
    ...
    outPrintWriter.println(
        "  get-transferred-download-bytes [-u | --user USER_ID] [-n | --namespace NAMESPACE]"
            + " PACKAGE JOB_ID");
    ...
    outPrintWriter.println("         the primary or system user");
    outPrintWriter.println(
        "  get-transferred-upload-bytes [-u | --user USER_ID] [-n | --namespace NAMESPACE] PACKAGE"
            + " JOB_ID");
