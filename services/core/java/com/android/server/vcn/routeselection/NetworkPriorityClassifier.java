...
public abstract class NetworkPriorityClassifier {
  ...
  public static int calculatePriorityClass(
    ...
    return (vcnContext.isInTestMode() && networkCapabilities.hasTransport(7))
        ? Integer.MAX_VALUE
        : -1;
  }
  ...
  public static boolean checkMatchesPriorityRule(
    ...
    NetworkCapabilities networkCapabilities = underlyingNetworkRecord.networkCapabilities;
    boolean z =
        underlyingNetworkRecord2 != null
            && Objects.equals(underlyingNetworkRecord2.network, underlyingNetworkRecord.network);
    ...
    if ((metered != 1 || z2)
        && ((metered != 2 || !z2)
            ...
            && ((networkCapabilities.getLinkUpstreamBandwidthKbps()
                ...
                && (networkCapabilities.getLinkDownstreamBandwidthKbps()
                        >= vcnUnderlyingNetworkTemplate.getMinEntryDownstreamBandwidthKbps()
                    || z)))) {
      for (Map.Entry entry :
      ...
      logWtf(
          "Got unknown VcnUnderlyingNetworkTemplate class: "
              + vcnUnderlyingNetworkTemplate.getClass().getSimpleName());
    }
    return false;
  }

  public static boolean checkMatchesWifiPriorityRule(
      VcnWifiUnderlyingNetworkTemplate vcnWifiUnderlyingNetworkTemplate,
      UnderlyingNetworkRecord underlyingNetworkRecord,
      UnderlyingNetworkRecord underlyingNetworkRecord2,
      PersistableBundleUtils.PersistableBundleWrapper persistableBundleWrapper) {
    NetworkCapabilities networkCapabilities = underlyingNetworkRecord.networkCapabilities;
    if (networkCapabilities.hasTransport(1)
        && isWifiRssiAcceptable(
            underlyingNetworkRecord, underlyingNetworkRecord2, persistableBundleWrapper)) {
      return vcnWifiUnderlyingNetworkTemplate.getSsids().isEmpty()
          || vcnWifiUnderlyingNetworkTemplate.getSsids().contains(networkCapabilities.getSsid());
    }
    return false;
  }

  public static boolean isWifiRssiAcceptable(
      UnderlyingNetworkRecord underlyingNetworkRecord,
      UnderlyingNetworkRecord underlyingNetworkRecord2,
      PersistableBundleUtils.PersistableBundleWrapper persistableBundleWrapper) {
    NetworkCapabilities networkCapabilities = underlyingNetworkRecord.networkCapabilities;
    return ((underlyingNetworkRecord2 != null
                && underlyingNetworkRecord.network.equals(underlyingNetworkRecord2.network))
            && networkCapabilities.getSignalStrength()
                >= getWifiExitRssiThreshold(persistableBundleWrapper))
        || networkCapabilities.getSignalStrength()
            >= getWifiEntryRssiThreshold(persistableBundleWrapper);
  }
  ...
  public static boolean checkMatchesCellPriorityRule(
    ...
    boolean isOpportunistic =
        isOpportunistic(telephonySubscriptionSnapshot, networkCapabilities.getSubscriptionIds());
    if (opportunistic == 1) {
      if (!isOpportunistic) {
        return false;
      }
      if (telephonySubscriptionSnapshot
              .getAllSubIdsInGroup(parcelUuid)
              .contains(Integer.valueOf(SubscriptionManager.getActiveDataSubscriptionId()))
          && !networkCapabilities
              .getSubscriptionIds()
              .contains(Integer.valueOf(SubscriptionManager.getActiveDataSubscriptionId()))) {
        return false;
  ...
  public static int getWifiEntryRssiThreshold(
      PersistableBundleUtils.PersistableBundleWrapper persistableBundleWrapper) {
    return persistableBundleWrapper != null
        ? persistableBundleWrapper.getInt(
            "vcn_network_selection_wifi_entry_rssi_threshold", WIFI_ENTRY_RSSI_THRESHOLD_DEFAULT)
        : WIFI_ENTRY_RSSI_THRESHOLD_DEFAULT;
  }

  public static int getWifiExitRssiThreshold(
      PersistableBundleUtils.PersistableBundleWrapper persistableBundleWrapper) {
    return persistableBundleWrapper != null
        ? persistableBundleWrapper.getInt(
            "vcn_network_selection_wifi_exit_rssi_threshold", WIFI_EXIT_RSSI_THRESHOLD_DEFAULT)
