...
public class WatchedSparseIntArray extends WatchableImpl implements Snappable {
  ...
  public static void snapshot(
      WatchedSparseIntArray watchedSparseIntArray, WatchedSparseIntArray watchedSparseIntArray2) {
    if (watchedSparseIntArray.size() != 0) {
      throw new IllegalArgumentException("snapshot destination is not empty");
    }
    int size = watchedSparseIntArray2.size();
    for (int i = 0; i < size; i++) {
      watchedSparseIntArray.mStorage.put(
          watchedSparseIntArray2.keyAt(i), watchedSparseIntArray2.valueAt(i));
    }
    watchedSparseIntArray.seal();
  }
}
