...
public class WebViewUpdateService extends SystemService {
  ...
  public void onStart() {
    this.mWebViewUpdatedReceiver =
        new BroadcastReceiver() { // from class: com.android.server.webkit.WebViewUpdateService.1
            ...
            switch (action) {
              ...
              case "android.intent.action.PACKAGE_CHANGED":
                if (WebViewUpdateService.entirePackageChanged(intent)) {
                  WebViewUpdateService.this.mImpl.packageStateChanged(
                      WebViewUpdateService.packageNameFromIntent(intent), 0, intExtra);
                  break;
                }
                break;
              case "android.intent.action.PACKAGE_REMOVED":
                if (!intent.getExtras().getBoolean("android.intent.extra.REPLACING")) {
                  WebViewUpdateService.this.mImpl.packageStateChanged(
                      WebViewUpdateService.packageNameFromIntent(intent), 3, intExtra);
                  break;
                }
                break;
              case "android.intent.action.PACKAGE_ADDED":
                WebViewUpdateService.this.mImpl.packageStateChanged(
                    WebViewUpdateService.packageNameFromIntent(intent),
                    intent.getExtras().getBoolean("android.intent.extra.REPLACING") ? 2 : 1,
                    intExtra);
                break;
            }
          }
    ...
    getContext()
        .registerReceiverAsUser(
            this.mWebViewUpdatedReceiver, UserHandle.ALL, intentFilter, null, null);
    IntentFilter intentFilter2 = new IntentFilter();
    intentFilter2.addAction("android.intent.action.USER_STARTED");
    intentFilter2.addAction("android.intent.action.USER_REMOVED");
    getContext()
        .registerReceiverAsUser(
            this.mWebViewUpdatedReceiver, UserHandle.ALL, intentFilter2, null, null);
  ...
  public static boolean entirePackageChanged(Intent intent) {
    return Arrays.asList(
        ...
        .contains(intent.getDataString().substring(8));
  }
  ...
  public class BinderService extends IWebViewUpdateService.Stub {
    ...
    public final void grantVisibilityToCaller(String str, int i) {
      ...
      packageManagerInternal.grantImplicitAccess(
          UserHandle.getUserId(i),
          null,
          UserHandle.getAppId(i),
          packageManagerInternal.getPackageUid(str, 0L, UserHandle.getUserId(i)),
          true);
    }

    public String changeProviderAndSetting(String str) {
      if (WebViewUpdateService.this
              .getContext()
              .checkCallingPermission("android.permission.WRITE_SECURE_SETTINGS")
          != 0) {
        String str2 =
            "Permission Denial: changeProviderAndSetting() from pid="
    ...
    public void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
      if (DumpUtils.checkDumpPermission(
          WebViewUpdateService.this.getContext(), "WebViewUpdateService", printWriter)) {
