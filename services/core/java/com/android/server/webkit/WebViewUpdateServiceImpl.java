...
public class WebViewUpdateServiceImpl {
  ...
  public static String getInvalidityReason(int i) {
    return i != 1
        ? i != 2
            ? i != 3
                ? i != 4 ? "Unexcepted validity-reason" : "No WebView-library manifest flag"
                : "Incorrect signature"
    ...
    for (WebViewProviderInfo webViewProviderInfo : this.mSystemInterface.getWebViewPackages()) {
      if (webViewProviderInfo.packageName.equals(str)) {
        synchronized (this.mLock) {
          ...
          if (!webViewProviderInfo.packageName.equals(findPreferredWebViewPackage.packageName)
              && !webViewProviderInfo.packageName.equals(str2)) {
  ...
  public void prepareWebViewInSystemServer() {
    ...
    try {
      synchronized (this.mLock) {
        ...
        String userChosenWebViewProvider =
            this.mSystemInterface.getUserChosenWebViewProvider(this.mContext);
        if (userChosenWebViewProvider != null
            && !userChosenWebViewProvider.equals(this.mCurrentWebViewPackage.packageName)) {
    ...
    if (getCurrentWebViewPackage() == null) {
      WebViewProviderInfo fallbackProvider =
          getFallbackProvider(this.mSystemInterface.getWebViewPackages());
      if (fallbackProvider != null) {
        ...
        this.mSystemInterface.enablePackageForAllUsers(
            this.mContext, fallbackProvider.packageName, true);
        return;
      }
  ...
  public WebViewProviderResponse waitForAndGetProvider() {
    ...
    synchronized (this.mLock) {
      ...
      if (webViewIsReadyLocked) {
        ...
        String str =
            "Timed out waiting for relro creation, relros started "
                + this.mNumRelroCreationsStarted
                + " relros finished "
                + this.mNumRelroCreationsFinished
                + " package dirty? "
                + this.mWebViewPackageDirty;
        Slog.e(TAG, str);
  ...
  */
  ...
  public final void onWebViewProviderChanged(PackageInfo packageInfo) {
    synchronized (this.mLock) {
      ...
      if (this.mNumRelroCreationsStarted == this.mNumRelroCreationsFinished) {
        ...
        this.mNumRelroCreationsStarted =
            this.mSystemInterface.onWebViewProviderChanged(packageInfo);
        checkIfRelrosDoneLocked();
      } else {
    ...
    for (int i = 0; i < webViewPackages.length; i++) {
      try {
        PackageInfo packageInfoForProvider =
            this.mSystemInterface.getPackageInfoForProvider(webViewPackages[i]);
  ...
  public final PackageInfo findPreferredWebViewPackage() {
    ...
    for (ProviderAndPackageInfo providerAndPackageInfo : validWebViewPackagesAndInfos) {
      if (providerAndPackageInfo.provider.packageName.equals(userChosenWebViewProvider)
          && isInstalledAndEnabledForAllUsers(
              this.mSystemInterface.getPackageInfoForProviderAllUsers(
                  this.mContext, providerAndPackageInfo.provider))) {
    ...
    for (ProviderAndPackageInfo providerAndPackageInfo2 : validWebViewPackagesAndInfos) {
      ...
      if (webViewProviderInfo.availableByDefault
          && isInstalledAndEnabledForAllUsers(
              this.mSystemInterface.getPackageInfoForProviderAllUsers(
                  this.mContext, webViewProviderInfo))) {
  ...
  public final boolean webViewIsReadyLocked() {
    return !this.mWebViewPackageDirty
        && this.mNumRelroCreationsStarted == this.mNumRelroCreationsFinished
        && this.mAnyWebViewInstalled;
  }
  ...
  public final long getMinimumVersionCode() {
    ...
    for (WebViewProviderInfo webViewProviderInfo : this.mSystemInterface.getWebViewPackages()) {
      if (webViewProviderInfo.availableByDefault) {
        try {
          long factoryPackageVersion =
              this.mSystemInterface.getFactoryPackageVersion(webViewProviderInfo.packageName);
  ...
  public static boolean providerHasValidSignature(
  ...
  public static WebViewProviderInfo getFallbackProvider(
      WebViewProviderInfo[] webViewProviderInfoArr) {
  ...
  public boolean isMultiProcessEnabled() {
    ...
    return this.mSystemInterface.isMultiProcessDefaultEnabled()
        ? multiProcessSetting > Integer.MIN_VALUE
        : multiProcessSetting >= Integer.MAX_VALUE;
  }

  public void enableMultiProcess(boolean z) {
    PackageInfo currentWebViewPackage = getCurrentWebViewPackage();
    this.mSystemInterface.setMultiProcessSetting(
        this.mContext, z ? Integer.MAX_VALUE : Integer.MIN_VALUE);
  ...
  public void dumpState(PrintWriter printWriter) {
    ...
    printWriter.println(
        String.format("  Multiprocess enabled: %b", Boolean.valueOf(isMultiProcessEnabled())));
    synchronized (this.mLock) {
      PackageInfo packageInfo = this.mCurrentWebViewPackage;
      if (packageInfo == null) {
      ...
      printWriter.println(
          String.format(
              "  Minimum WebView version code: %d", Long.valueOf(this.mMinimumVersionCode)));
      printWriter.println(
          String.format(
              "  Number of relros started: %d", Integer.valueOf(this.mNumRelroCreationsStarted)));
      printWriter.println(
          String.format(
              "  Number of relros finished: %d", Integer.valueOf(this.mNumRelroCreationsFinished)));
      printWriter.println(
          String.format("  WebView package dirty: %b", Boolean.valueOf(this.mWebViewPackageDirty)));
      printWriter.println(
          String.format(
              "  Any WebView package installed: %b", Boolean.valueOf(this.mAnyWebViewInstalled)));
      try {
        PackageInfo findPreferredWebViewPackage = findPreferredWebViewPackage();
        printWriter.println(
            String.format(
                "  Preferred WebView package (name, version): (%s, %s)",
                findPreferredWebViewPackage.packageName, findPreferredWebViewPackage.versionName));
      } catch (WebViewPackageMissingException unused) {
  ...
  public final void dumpAllPackageInformationLocked(PrintWriter printWriter) {
    ...
    for (WebViewProviderInfo webViewProviderInfo : webViewPackages) {
      PackageInfo packageInfo =
          ((UserPackage)
                  this.mSystemInterface
                      .getPackageInfoForProviderAllUsers(this.mContext, webViewProviderInfo)
                      .get(0))
              .getPackageInfo();
      if (packageInfo == null) {
        printWriter.println(
            String.format("    %s is NOT installed.", webViewProviderInfo.packageName));
      } else {
        ...
        if (validityResult == 0) {
          boolean isInstalledAndEnabledForAllUsers =
              isInstalledAndEnabledForAllUsers(
                  this.mSystemInterface.getPackageInfoForProviderAllUsers(
                      this.mContext, webViewProviderInfo));
          ...
          printWriter.println(
              String.format(
                  "    Valid package %s (%s) is %s installed/enabled for all users", objArr));
        } else {
          printWriter.println(
              String.format(
                  "    Invalid package %s (%s), reason: %s",
                  packageInfo.packageName, format, getInvalidityReason(validityResult)));
        }
      }
    }
  }
