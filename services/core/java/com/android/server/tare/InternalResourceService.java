...
public class InternalResourceService extends SystemService {
  ...
  public InternalResourceService(Context context) {
    ...
    this.mApbListener =
        new IAppOpsCallback
            .Stub() { // from class: com.android.server.tare.InternalResourceService.1
          public void opChanged(int i, int i2, String str) {
            ...
            synchronized (InternalResourceService.this.mLock) {
              ...
              } else if (InternalResourceService.this.mRestrictedApps.remove(
                  UserHandle.getUserId(i2), str)) {
    ...
    this.mBroadcastReceiver =
        new BroadcastReceiver() { // from class: com.android.server.tare.InternalResourceService.2
          ...
          public void onReceive(Context context2, Intent intent) {
            ...
            switch (c) {
              case 0:
                InternalResourceService.this.onUserRemoved(
                    intent.getIntExtra("android.intent.extra.user_handle", 0));
                break;
              case 1:
                boolean booleanExtra =
                    intent.getBooleanExtra("present", InternalResourceService.this.mHasBattery);
                ...
                break;
              case 2:
                InternalResourceService.this.onPackageForceStopped(
                    UserHandle.getUserId(intent.getIntExtra("android.intent.extra.UID", -1)),
              ...
              case 4:
                ...
                break;
              case 5:
                InternalResourceService.this.onUserAdded(
                    intent.getIntExtra("android.intent.extra.user_handle", 0));
                break;
              case 6:
                if (!intent.getBooleanExtra("android.intent.extra.REPLACING", false)) {
                  InternalResourceService.this.onPackageAdded(
                      intent.getIntExtra("android.intent.extra.UID", -1), getPackageName(intent));
                  break;
                }
                break;
              case 7:
                InternalResourceService.this.onPackageRemoved(
                    intent.getIntExtra("android.intent.extra.UID", -1), getPackageName(intent));
                break;
            }
          }
        };
    this.mSurveillanceAgent =
        new UsageStatsManagerInternal
            .UsageEventListener() { // from class: com.android.server.tare.InternalResourceService.3
    ...
    this.mUnusedWealthReclamationListener =
        new AlarmManager
            .OnAlarmListener() { // from class: com.android.server.tare.InternalResourceService.4
          @Override // android.app.AlarmManager.OnAlarmListener
          public void onAlarm() {
            synchronized (InternalResourceService.this.mLock) {
              InternalResourceService.this.mAgent.reclaimUnusedAssetsLocked(
                  0.10000000149011612d, 259200000L, false);
              InternalResourceService.this.mScribe.setLastReclamationTimeLocked(
                  TareUtils.getCurrentTimeMillis());
  ...
  public List getInstalledPackages(int i) {
    ...
    synchronized (this.mLock) {
      ...
      for (int numElementsForKeyAt = this.mPkgCache.numElementsForKeyAt(indexOfKey) - 1;
          numElementsForKeyAt >= 0;
          numElementsForKeyAt--) {
        arrayList.add(
            (InstalledPackageInfo) this.mPkgCache.valueAt(indexOfKey, numElementsForKeyAt));
      }
      return arrayList;
    }
  }
  ...
  public long getMinBalanceLocked(int i, String str) {
    return (this.mCurrentBatteryLevel * this.mCompleteEconomicPolicy.getMinSatiatedBalance(i, str))
        / 100;
  }
  ...
  public void onPackageAdded(int i, String str) {
    ...
    try {
      PackageInfo packageInfoAsUser =
          this.mPackageManager.getPackageInfoAsUser(str, 1074532352, userId);
      synchronized (this.mPackageToUidCache) {
        this.mPackageToUidCache.add(userId, str, Integer.valueOf(i));
      }
      synchronized (this.mLock) {
        InstalledPackageInfo installedPackageInfo =
            new InstalledPackageInfo(getContext(), userId, packageInfoAsUser);
        maybeUpdateInstallerStatusLocked(
            (InstalledPackageInfo) this.mPkgCache.add(userId, str, installedPackageInfo),
  ...
  public void onPackageRemoved(int i, String str) {
    ...
    synchronized (this.mLock) {
      ...
      InstalledPackageInfo installedPackageInfo =
          (InstalledPackageInfo) this.mPkgCache.delete(userId, str);
      this.mInstallers.delete(userId, str);
      if (installedPackageInfo != null
          && (str2 = installedPackageInfo.installerPackageName) != null
          && (arraySet = (ArraySet) this.mInstallers.get(userId, str2)) != null) {
        arraySet.remove(str);
      }
      ...
      for (int size = installedPackagesAsUser.size() - 1; size >= 0; size--) {
        InstalledPackageInfo installedPackageInfo =
            new InstalledPackageInfo(
                getContext(), i, (PackageInfo) installedPackagesAsUser.get(size));
        maybeUpdateInstallerStatusLocked(
            (InstalledPackageInfo)
                this.mPkgCache.add(i, installedPackageInfo.packageName, installedPackageInfo),
            installedPackageInfo);
      }
    ...
    synchronized (this.mLock) {
      ...
      if (indexOfKey >= 0) {
        for (int numElementsForKeyAt = this.mPkgCache.numElementsForKeyAt(indexOfKey) - 1;
            numElementsForKeyAt >= 0;
            numElementsForKeyAt--) {
          this.mUidToPackageCache.remove(
              ((InstalledPackageInfo) this.mPkgCache.valueAt(indexOfKey, numElementsForKeyAt)).uid);
        }
      }
  ...
  public void maybePerformQuantitativeEasingLocked() {
    ...
    long min =
        Math.min(
            (((this.mCurrentBatteryLevel - 50) * satiatedConsumptionLimitLocked) / 100)
                + satiatedConsumptionLimitLocked,
            this.mCompleteEconomicPolicy.getMaxSatiatedConsumptionLimit());
    if (min != satiatedConsumptionLimitLocked) {
      Slog.i(
          "TARE-IRS",
          "Increasing consumption limit from "
              + TareUtils.cakeToString(satiatedConsumptionLimitLocked)
              + " to "
              + TareUtils.cakeToString(min));
  ...
  public void maybeAdjustDesiredStockLevelLocked() {
    ...
    if (this.mConfigObserver.ENABLE_TIP3 && getRealtimeSinceFirstSetupMs() >= 432000000) {
      ...
      if (currentTimeMillis - this.mScribe.getLastStockRecalculationTimeLocked() < 57600000
          || this.mCurrentBatteryLevel <= 80) {
        return;
      }
      ...
      long j =
          ((batteryFullCharge * batteryScreenOffDurationMs) / batteryScreenOffDischargeMah)
              / ClipboardService.DEFAULT_CLIPBOARD_TIMEOUT_MILLIS;
      long j2 = (j * 100) / this.mTargetBackgroundBatteryLifeHours;
      if (DEBUG) {
        Slog.d(
            "TARE-IRS",
            "maybeAdjustDesiredStockLevelLocked: screenOffMs="
                + batteryScreenOffDurationMs
                + " dischargeMah="
                + batteryScreenOffDischargeMah
                + " capacityMah="
                + batteryFullCharge
                + " estimatedLifeHours="
                + j
      ...
      if (j2 > 105) {
        max =
            Math.min(
                (long) (satiatedConsumptionLimitLocked * 1.01d),
                this.mCompleteEconomicPolicy.getMaxSatiatedConsumptionLimit());
      } else if (j2 >= 100) {
        return;
      } else {
        max =
            Math.max(
                (long) (satiatedConsumptionLimitLocked * 0.98d),
                this.mCompleteEconomicPolicy.getMinSatiatedConsumptionLimit());
      }
      if (max != satiatedConsumptionLimitLocked) {
        Slog.i(
            "TARE-IRS",
            "Adjusting consumption limit from "
                + TareUtils.cakeToString(satiatedConsumptionLimitLocked)
                + " to "
                + TareUtils.cakeToString(max)
                + " because drain was "
                + j2
                + "% of target");
  ...
  public void postAffordabilityChanged(
      int i, String str, Agent.ActionAffordabilityNote actionAffordabilityNote) {
    if (DEBUG) {
      Slog.d(
          "TARE-IRS",
          i
              ...
              + str
              + " affordability changed to "
              + actionAffordabilityNote.isCurrentlyAffordable());
    }
  ...
  public final void adjustCreditSupplyLocked(boolean z) {
    ...
    if (remainingConsumableCakesLocked > consumptionLimitLocked) {
      this.mScribe.adjustRemainingConsumableCakesLocked(
          consumptionLimitLocked - remainingConsumableCakesLocked);
    } else if (z) {
      this.mScribe.adjustRemainingConsumableCakesLocked(
          (long)
              ((this.mCurrentBatteryLevel / 100.0d)
                  * (consumptionLimitLocked - remainingConsumableCakesLocked)));
    }
  ...
  public final void grantInstallersTemporaryVipStatusLocked(int i, long j, long j2) {
    ...
    for (int numElementsForKey = this.mPkgCache.numElementsForKey(indexOfKey) - 1;
        numElementsForKey >= 0;
        numElementsForKey--) {
      InstalledPackageInfo installedPackageInfo =
          (InstalledPackageInfo) this.mPkgCache.valueAt(indexOfKey, numElementsForKey);
      if (installedPackageInfo.isSystemInstaller
          && ((l = (Long) this.mTemporaryVips.get(i, installedPackageInfo.packageName)) == null
              || l.longValue() < j3)) {
  ...
  public final void processUsageEventLocked(int i, UsageEvents.Event event) {
    ...
    if (DEBUG) {
      Slog.d(
          ...
          "Processing event "
              ...
              + event.mInstanceId
              + ") for "
              + TareUtils.appToString(i, packageName));
    }
    long elapsedRealtime = SystemClock.elapsedRealtime();
    int eventType = event.getEventType();
    if (eventType == 1) {
      this.mAgent.noteOngoingEventLocked(
          i, packageName, -2147483646, String.valueOf(event.mInstanceId), elapsedRealtime);
      return;
    }
    if (eventType != 2) {
      if (eventType != 7) {
    ...
    this.mAgent.stopOngoingActionLocked(
        i,
        packageName,
        -2147483646,
        String.valueOf(event.mInstanceId),
        elapsedRealtime,
        TareUtils.getCurrentTimeMillis());
  }
  ...
  public final void scheduleUnusedWealthReclamationLocked() {
    ...
    final long max =
        Math.max(
            ...
            this.mScribe.getLastReclamationTimeLocked()
                + BackupManagerConstants.DEFAULT_FULL_BACKUP_INTERVAL_MILLISECONDS);
    this.mHandler.post(
        new Runnable() { // from class:
                         // com.android.server.tare.InternalResourceService$$ExternalSyntheticLambda1
          @Override // java.lang.Runnable
          public final void run() {
            InternalResourceService.this.lambda$scheduleUnusedWealthReclamationLocked$0(
  ...
  public /* synthetic */ void lambda$scheduleUnusedWealthReclamationLocked$0(long j, long j2) {
    ...
    if (alarmManager != null) {
      alarmManager.setWindow(
          3,
          SystemClock.elapsedRealtime() + (j - j2),
          1800000L,
          "*tare.reclamation*",
          this.mUnusedWealthReclamationListener,
          this.mHandler);
  ...
  public final void maybeUpdateInstallerStatusLocked(
      InstalledPackageInfo installedPackageInfo, InstalledPackageInfo installedPackageInfo2) {
    ArraySet arraySet;
    boolean z = true;
    if (installedPackageInfo == null) {
      ...
      z =
          true
              ^ Objects.equals(
                  installedPackageInfo.installerPackageName,
                  installedPackageInfo2.installerPackageName);
    }
    if (z) {
      ...
      if (installedPackageInfo != null
          && (arraySet =
                  (ArraySet)
                      this.mInstallers.get(userId, installedPackageInfo.installerPackageName))
              != null) {
        arraySet.remove(str);
      }
  ...
  public final void registerListeners() {
    ...
    getContext()
        .registerReceiverAsUser(this.mBroadcastReceiver, UserHandle.ALL, intentFilter3, null, null);
    ((UsageStatsManagerInternal) LocalServices.getService(UsageStatsManagerInternal.class))
        .registerListener(this.mSurveillanceAgent);
    try {
  ...
  public final void setupHeavyWork() {
    ...
    synchronized (this.mLock) {
      ...
      if (z) {
        ...
        this.mScribe.setConsumptionLimitLocked(
            this.mCompleteEconomicPolicy.getInitialSatiatedConsumptionLimit());
        ...
        if (this.mScribe.getSatiatedConsumptionLimitLocked()
                >= this.mCompleteEconomicPolicy.getMinSatiatedConsumptionLimit()
            && this.mScribe.getSatiatedConsumptionLimitLocked()
                <= this.mCompleteEconomicPolicy.getMaxSatiatedConsumptionLimit()) {
          adjustCreditSupplyLocked(true);
          realtimeSinceUsersAddedLocked =
              this.mScribe.getRealtimeSinceUsersAddedLocked(elapsedRealtime);
        }
        this.mScribe.setConsumptionLimitLocked(
            this.mCompleteEconomicPolicy.getInitialSatiatedConsumptionLimit());
        realtimeSinceUsersAddedLocked =
  ...
  public final void onBootPhaseSystemServicesReady() {
    ...
    synchronized (this.mLock) {
      ...
      if (registerReceiver != null
          && this.mHasBattery
              != (booleanExtra = registerReceiver.getBooleanExtra("present", true))) {
  ...
  public final void onBootPhaseThirdPartyAppsCanStart() {
    ...
    this.mHandler.post(
        new Runnable() { // from class:
                         // com.android.server.tare.InternalResourceService$$ExternalSyntheticLambda0
          @Override // java.lang.Runnable
          public final void run() {
  ...
  public final void tearDownEverything() {
    ...
    synchronized (this.mLock) {
      ...
      this.mHandler.post(
          new Runnable() { // from class:
                           // com.android.server.tare.InternalResourceService$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
      ...
      ((UsageStatsManagerInternal) LocalServices.getService(UsageStatsManagerInternal.class))
          .unregisterListener(this.mSurveillanceAgent);
      try {
  ...
  public final class IrsHandler extends Handler {
    ...
    public void handleMessage(Message message) {
      ...
      if (i == 0) {
        ...
        Agent.ActionAffordabilityNote actionAffordabilityNote =
            (Agent.ActionAffordabilityNote) someArgs.arg2;
        actionAffordabilityNote
            .getListener()
            .onAffordabilityChanged(
                i2,
                str,
                actionAffordabilityNote.getActionBill(),
                actionAffordabilityNote.isCurrentlyAffordable());
        someArgs.recycle();
        return;
      }
          ...
          while (i4 < size) {
            ...
            if ((keyAt & i5) != 0) {
              ...
              for (int size2 = arraySet.size() - 1; size2 >= 0; size2--) {
                ((EconomyManagerInternal.TareStateChangeListener) arraySet.valueAt(size2))
                    .onTareEnabledModeChanged(enabledMode);
              }
            }
            i4++;
          }
      ...
      if (i == 4) {
        ((EconomyManagerInternal.TareStateChangeListener) message.obj)
            .onTareEnabledModeChanged(InternalResourceService.this.getEnabledMode(message.arg1));
        return;
      }
      if (i != 5) {
        return;
      ...
      synchronized (InternalResourceService.this.mLock) {
        ...
        while (i4 < InternalResourceService.this.mTemporaryVips.numMaps()) {
          ...
          for (int numElementsForKeyAt =
                  InternalResourceService.this.mTemporaryVips.numElementsForKeyAt(i4) - 1;
              numElementsForKeyAt >= 0;
              numElementsForKeyAt--) {
            String str2 =
                (String) InternalResourceService.this.mTemporaryVips.keyAt(i4, numElementsForKeyAt);
            Long l =
                (Long) InternalResourceService.this.mTemporaryVips.valueAt(i4, numElementsForKeyAt);
  ...
  public final class EconomyManagerStub extends IEconomyManager.Stub {
    ...
    public void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
      ...
      if (!DumpUtils.checkDumpAndUsageStatsPermission(
          InternalResourceService.this.getContext(), "TARE-IRS", printWriter)) {
        return;
      }
      try {
    ...
    public int handleShellCommand(
        ...
        String[] strArr) {
      return new TareShellCommand(InternalResourceService.this)
          .exec(
              this,
  ...
  public final class LocalService implements EconomyManagerInternal {
    ...
    public void registerAffordabilityChangeListener(
        int i,
        String str,
        EconomyManagerInternal.AffordabilityChangeListener affordabilityChangeListener,
        EconomyManagerInternal.ActionBill actionBill) {
      if (!InternalResourceService.this.isTareSupported()
          || InternalResourceService.this.isSystem(i, str)) {
        return;
      }
      synchronized (InternalResourceService.this.mLock) {
        InternalResourceService.this.mAgent.registerAffordabilityChangeListenerLocked(
            i, str, affordabilityChangeListener, actionBill);
      }
    }

    @Override // com.android.server.tare.EconomyManagerInternal
    public void unregisterAffordabilityChangeListener(
        int i,
        String str,
        EconomyManagerInternal.AffordabilityChangeListener affordabilityChangeListener,
        EconomyManagerInternal.ActionBill actionBill) {
      ...
      synchronized (InternalResourceService.this.mLock) {
        InternalResourceService.this.mAgent.unregisterAffordabilityChangeListenerLocked(
            i, str, affordabilityChangeListener, actionBill);
      }
    }

    @Override // com.android.server.tare.EconomyManagerInternal
    public void registerTareStateChangeListener(
        EconomyManagerInternal.TareStateChangeListener tareStateChangeListener, int i) {
      if (InternalResourceService.this.isTareSupported()) {
        synchronized (InternalResourceService.this.mStateChangeListeners) {
          if (InternalResourceService.this.mStateChangeListeners.add(i, tareStateChangeListener)) {
            InternalResourceService.this
                .mHandler
                .obtainMessage(4, i, 0, tareStateChangeListener)
                .sendToTarget();
          }
        }
      }
    ...
    public boolean canPayFor(int i, String str, EconomyManagerInternal.ActionBill actionBill) {
      ...
      if (InternalResourceService.this.mEnabledMode == 0
          || InternalResourceService.this.isVip(i, str)) {
        return true;
      }
      ...
      synchronized (obj2) {
        ...
        while (i2 < anticipatedActions.size()) {
          try {
            anticipatedAction =
                (EconomyManagerInternal.AnticipatedAction) anticipatedActions.get(i2);
            j =
                InternalResourceService.this.mCompleteEconomicPolicy.getCostOfAction(
                        anticipatedAction.actionId, i, str)
                    .price;
            obj = obj2;
          } catch (Throwable th) {
            th = th;
          ...
          try {
            j2 +=
                (anticipatedAction.numInstantaneousCalls * j)
                    + (j * (anticipatedAction.ongoingDurationMs / 1000));
            i2++;
            obj2 = obj;
          } catch (Throwable th2) {
            th = th2;
        ...
        Object obj3 = obj2;
        boolean z =
            InternalResourceService.this.mAgent.getBalanceLocked(i, str) >= j2
                && InternalResourceService.this.mScribe.getRemainingConsumableCakesLocked() >= j2;
        return z;
      }
    }
    ...
    public long getMaxDurationMs(int i, String str, EconomyManagerInternal.ActionBill actionBill) {
      ...
      synchronized (InternalResourceService.this.mLock) {
        ...
        return (Math.min(
                    InternalResourceService.this.mAgent.getBalanceLocked(i, str),
                    InternalResourceService.this.mScribe.getRemainingConsumableCakesLocked())
                * 1000)
            / j;
      }
    }
    ...
    public void noteOngoingEventStarted(int i, String str, int i2, String str2) {
      ...
      synchronized (InternalResourceService.this.mLock) {
        InternalResourceService.this.mAgent.noteOngoingEventLocked(
            i, str, i2, str2, SystemClock.elapsedRealtime());
      }
    }
    ...
    public void noteOngoingEventStopped(int i, String str, int i2, String str2) {
      ...
      long currentTimeMillis = TareUtils.getCurrentTimeMillis();
      synchronized (InternalResourceService.this.mLock) {
        InternalResourceService.this.mAgent.stopOngoingActionLocked(
            i, str, i2, str2, elapsedRealtime, currentTimeMillis);
      }
    }
  }
  ...
  public class ConfigObserver extends ContentObserver
    ...
    public void start() {
      ...
      this.mContentResolver.registerContentObserver(
          Settings.Global.getUriFor("enable_tare"), false, this);
      this.mContentResolver.registerContentObserver(
    ...
    */
              ...
              if (hashCode == 1536945124 && str.equals("target_bg_battery_life_hrs")) {
                ...
                if (c != 0) {
                  ...
                  synchronized (InternalResourceService.this.mLock) {
                    ...
                    internalResourceService.mTargetBackgroundBatteryLifeHours =
                        properties.getInt(
                            str, internalResourceService.mDefaultTargetBackgroundBatteryLifeHours);
                    InternalResourceService.this.maybeAdjustDesiredStockLevelLocked();
                  }
                } else if (!z
                    && (str.startsWith("am")
                        || str.startsWith("js")
                        || str.startsWith("enable_policy"))) {
                  updateEconomicPolicy();
                  z = true;
                }
              }
    ...
    public final void updateEnabledStatus() {
      int i =
          InternalResourceService.this.isTareSupported()
              ? Settings.Global.getInt(
                  this.mContentResolver,
                  "enable_tare",
                  DeviceConfig.getInt("tare", "enable_tare_mode", 0))
              : 0;
      boolean z = true;
      if (i != 0 && i != 1 && i != 2) {
        i = 0;
    ...
    public final void updateEconomicPolicy() {
      synchronized (InternalResourceService.this.mLock) {
        long minSatiatedConsumptionLimit =
            InternalResourceService.this.mCompleteEconomicPolicy.getMinSatiatedConsumptionLimit();
        long maxSatiatedConsumptionLimit =
            InternalResourceService.this.mCompleteEconomicPolicy.getMaxSatiatedConsumptionLimit();
        int enabledPolicyIds =
            InternalResourceService.this.mCompleteEconomicPolicy.getEnabledPolicyIds();
        InternalResourceService.this.mCompleteEconomicPolicy.tearDown();
        InternalResourceService.this.mCompleteEconomicPolicy =
            new CompleteEconomicPolicy(InternalResourceService.this);
        if (InternalResourceService.this.mEnabledMode != 0
            && InternalResourceService.this.mBootPhase >= 600) {
          InternalResourceService.this.mCompleteEconomicPolicy.setup(
              getAllDeviceConfigProperties());
          if (minSatiatedConsumptionLimit
                  != InternalResourceService.this.mCompleteEconomicPolicy
                      .getMinSatiatedConsumptionLimit()
              || maxSatiatedConsumptionLimit
                  != InternalResourceService.this.mCompleteEconomicPolicy
                      .getMaxSatiatedConsumptionLimit()) {
            InternalResourceService.this.mScribe.setConsumptionLimitLocked(
                InternalResourceService.this.mCompleteEconomicPolicy
                    .getInitialSatiatedConsumptionLimit());
          }
          InternalResourceService.this.mAgent.onPricingChangedLocked();
          int enabledPolicyIds2 =
              InternalResourceService.this.mCompleteEconomicPolicy.getEnabledPolicyIds();
          if (enabledPolicyIds != enabledPolicyIds2) {
            InternalResourceService.this
                .mHandler
                .obtainMessage(3, enabledPolicyIds2 ^ enabledPolicyIds, 0)
                .sendToTarget();
          }
        }
      }
  ...
  public int executeClearVip(PrintWriter printWriter) {
    synchronized (this.mLock) {
      ...
      for (int numMaps = this.mVipOverrides.numMaps() - 1; numMaps >= 0; numMaps--) {
        ...
        for (int numElementsForKeyAt = this.mVipOverrides.numElementsForKeyAt(numMaps) - 1;
            numElementsForKeyAt >= 0;
            numElementsForKeyAt--) {
          sparseSetArray.add(
              keyAt, (String) this.mVipOverrides.keyAt(numMaps, numElementsForKeyAt));
        }
      }
  ...
  public int executeSetVip(PrintWriter printWriter, int i, String str, Boolean bool) {
    ...
    printWriter.println(
        TareUtils.appToString(i, str)
            + " VIP status set to "
            + bool
            + ". Final VIP state changed? "
            + z);
    return 0;
  }
  ...
  public final void dumpInternal(IndentingPrintWriter indentingPrintWriter, boolean z) {
    ...
    synchronized (this.mLock) {
      ...
      indentingPrintWriter.print(
          TareUtils.cakeToString(
              this.mCompleteEconomicPolicy.getInitialSatiatedConsumptionLimit()));
      indentingPrintWriter.print("/");
      indentingPrintWriter.println(
          TareUtils.cakeToString(this.mScribe.getSatiatedConsumptionLimitLocked()));
      indentingPrintWriter.print("Target bg battery life (hours): ");
      indentingPrintWriter.print(this.mTargetBackgroundBatteryLifeHours);
      indentingPrintWriter.print(" (");
      indentingPrintWriter.print(
          String.format("%.2f", Float.valueOf(100.0f / this.mTargetBackgroundBatteryLifeHours)));
      ...
      indentingPrintWriter.print(
          String.format(
              "%.2f",
              Float.valueOf((remainingConsumableCakesLocked * 100.0f) / consumptionLimitLocked)));
      indentingPrintWriter.println("% of current limit)");
      indentingPrintWriter.print("Device wealth: ");
      indentingPrintWriter.println(
          TareUtils.cakeToString(this.mScribe.getCakesInCirculationForLoggingLocked()));
      indentingPrintWriter.println();
      ...
      for (int i5 = 0; i5 < this.mInstallers.numMaps(); i5++) {
        ...
        for (int i6 = 0; i6 < this.mInstallers.numElementsForKeyAt(i5); i6++) {
          indentingPrintWriter.print(
              TareUtils.appToString(keyAt3, (String) this.mInstallers.keyAt(i5, i6)));
          indentingPrintWriter.print(": ");
      ...
      for (int i7 = 0; i7 < this.mPkgCache.numMaps(); i7++) {
        for (int i8 = 0; i8 < this.mPkgCache.numElementsForKeyAt(i7); i8++) {
          InstalledPackageInfo installedPackageInfo =
              (InstalledPackageInfo) this.mPkgCache.valueAt(i7, i8);
          if (installedPackageInfo.hasCode
              && installedPackageInfo.isHeadlessSystemApp
              && !UserHandle.isCore(installedPackageInfo.uid)) {
            indentingPrintWriter.println();
