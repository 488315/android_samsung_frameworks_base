...
public class Ledger {
  ...
  public Transaction getEarliestTransaction() {
    ...
    while (true) {
      ...
      Transaction transaction =
          transactionArr[(this.mTransactionIndex + i) % transactionArr.length];
      if (transaction != null) {
        return transaction;
      }
      i++;
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter, int i) {
    indentingPrintWriter
        .print("Current balance", TareUtils.cakeToString(getCurrentBalance()))
        .println();
    ...
    for (int i3 = 0; i3 < 4; i3++) {
      ...
      if (rewardBucket != null && rewardBucket.startTimeMs != 0) {
        ...
        TimeUtils.formatDuration(
            currentTimeMillis - rewardBucket.startTimeMs, indentingPrintWriter);
        indentingPrintWriter.println(" ago):");
        indentingPrintWriter.increaseIndent();
        for (int i4 = 0; i4 < rewardBucket.cumulativeDelta.size(); i4++) {
          indentingPrintWriter.print(
              EconomicPolicy.eventToString(rewardBucket.cumulativeDelta.keyAt(i4)));
          indentingPrintWriter.print(": ");
          indentingPrintWriter.println(
              TareUtils.cakeToString(rewardBucket.cumulativeDelta.valueAt(i4)));
        }
        indentingPrintWriter.decreaseIndent();
      }
    }
