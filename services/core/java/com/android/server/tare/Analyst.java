...
public class Analyst {
  ...
  public void noteBatteryLevelChange(int i) {
    ...
    if ((report != null
            ...
            && report.cumulativeBatteryDischarge >= 25)
        || (report != null
            && report.currentBatteryLevel < i
            && (report.screenOffDurationMs > 86400000L
                    ? 1
                    : (report.screenOffDurationMs == 86400000L ? 0 : -1))
                >= 0)) {
      int i3 = (i2 + 1) % 8;
    ...
    if (i < i4) {
      ...
      report3.screenOffDurationMs +=
          latestBatteryScreenOffRealtimeMs - report3.bsScreenOffRealtimeBase;
      report3.screenOffDischargeMah +=
          latestScreenOffDischargeMah - report3.bsScreenOffDischargeMahBase;
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (report != null) {
        ...
        indentingPrintWriter.print(
            padStringWithSpaces(Integer.toString(report.cumulativeBatteryDischarge), 15));
        if (report.numProfitableActions > 0) {
          ...
          indentingPrintWriter.print(
              padStringWithSpaces(
                  String.format(
                      "%s (%s : %s)",
                      TareUtils.cakeToString(report.cumulativeProfit),
                      TareUtils.cakeToString(report.cumulativeProfit / report.numProfitableActions),
                      i2 > 0 ? TareUtils.cakeToString(report.cumulativeProfit / i2) : "N/A"),
                  47));
        } else {
        ...
        if (report.numUnprofitableActions > 0) {
          ...
          indentingPrintWriter.print(
              padStringWithSpaces(
                  String.format(
                      "%s (%s : %s)",
                      TareUtils.cakeToString(report.cumulativeLoss),
                      TareUtils.cakeToString(report.cumulativeLoss / report.numUnprofitableActions),
                      i3 > 0 ? TareUtils.cakeToString(report.cumulativeLoss / i3) : "N/A"),
                  47));
        } else {
          indentingPrintWriter.print(padStringWithSpaces("N/A", 47));
        }
        if (report.numRewards > 0) {
          ...
          indentingPrintWriter.print(
              padStringWithSpaces(
                  String.format(
                      "%s (%s : %s)",
                      TareUtils.cakeToString(report.cumulativeRewards),
                      TareUtils.cakeToString(report.cumulativeRewards / report.numRewards),
                      i4 > 0 ? TareUtils.cakeToString(report.cumulativeRewards / i4) : "N/A"),
                  47));
        } else {
          indentingPrintWriter.print(padStringWithSpaces("N/A", 47));
        }
        if (report.numPositiveRegulations > 0) {
          ...
          indentingPrintWriter.print(
              padStringWithSpaces(
                  String.format(
                      "%s (%s : %s)",
                      TareUtils.cakeToString(report.cumulativePositiveRegulations),
                      TareUtils.cakeToString(
                          report.cumulativePositiveRegulations / report.numPositiveRegulations),
                      i5 > 0
                          ? TareUtils.cakeToString(report.cumulativePositiveRegulations / i5)
                          : "N/A"),
                  47));
        } else {
          indentingPrintWriter.print(padStringWithSpaces("N/A", 47));
        ...
        if (report.numNegativeRegulations > 0) {
          ...
          indentingPrintWriter.print(
              padStringWithSpaces(
                  String.format(
                      "%s (%s : %s)",
                      TareUtils.cakeToString(report.cumulativeNegativeRegulations),
                      TareUtils.cakeToString(
                          report.cumulativeNegativeRegulations / report.numNegativeRegulations),
                      i6 > 0
                          ? TareUtils.cakeToString(report.cumulativeNegativeRegulations / i6)
                          : "N/A"),
                  47));
        } else {
          indentingPrintWriter.print(padStringWithSpaces("N/A", 47));
        ...
        if (report.screenOffDurationMs > 0) {
          indentingPrintWriter.print(
              padStringWithSpaces(
                  String.format(
                      "%d mAh (%.2f%%/hr)",
                      Long.valueOf(report.screenOffDischargeMah),
                      Double.valueOf(
                          ((report.screenOffDischargeMah * 100.0d) * 3600000.0d)
                              / (report.screenOffDurationMs * batteryFullCharge))),
                  47));
        } else {
          indentingPrintWriter.print(padStringWithSpaces("N/A", 47));
        }
