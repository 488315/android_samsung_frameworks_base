            ...
            try {
                ...
                if (strArr != null) {
                    ...
                    long elapsedRealtime =
                            this.mTemporaryServiceExpiration - SystemClock.elapsedRealtime();
                ...
                printWriter.println(
                        this.mDefaultServicesDisabled.get(i) ? " (disabled)" : " (enabled)");
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public final String[] getDefaultServiceNameList(int i) {
        synchronized (this.mLock) {
            if (!this.mIsMultiple) {
                ...
                return TextUtils.isEmpty(readServiceName)
                        ? new String[0]
                        : new String[] {readServiceName};
            }
            ...
            try {
                ...
                if (strArr != null) {
                    Slog.w(
                            ...
                            "getServiceName(): using temporary name "
                ...
                Slog.w(
                        "ServiceNameBaseResolver",
                        "getServiceName(): temporary name not set and default disabled for user "
                                + i);
                return null;
            } catch (Throwable th) {
                throw th;
    ...
    public final void resetTemporaryService(int i) {
        synchronized (this.mLock) {
            try {
                Slog.i(
                        "ServiceNameBaseResolver",
                        "resetting temporary service for user "
                                + i
                                + " from "
                                + Arrays.toString(
                                        (Object[]) this.mTemporaryServiceNamesList.get(i)));
        ...
        synchronized (this.mLock) {
            try {
                if ((!this.mDefaultServicesDisabled.get(i)) == z) {
                    Slog.i(
                            "ServiceNameBaseResolver",
                            "setDefaultServiceEnabled(" + i + "): already " + z);
                    return false;
                }
                if (z) {
        ...
        synchronized (this.mLock) {
            try {
                            ...
                            new Handler(
                                    Looper
                                            .getMainLooper()) { // from class:
                                                                // com.android.server.infra.ServiceNameBaseResolver.1
                ...
                for (String str : strArr) {
                    ServiceNameResolver$NameResolverListener
                            serviceNameResolver$NameResolverListener = this.mOnSetCallback;
                    if (serviceNameResolver$NameResolverListener != null) {
