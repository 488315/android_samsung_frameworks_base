...
public class SyncOperation {
  public static String[] REASON_NAMES = {
    "DataSettingsChanged",
    "AccountsUpdated",
    "ServiceChanged",
    "Periodic",
  ...
  public SyncOperation(SyncOperation syncOperation, long j, long j2) {
    this(
        ...
        syncOperation.mImmutableExtras,
        syncOperation.allowParallelSyncs,
        syncOperation.isPeriodic,
        syncOperation.sourcePeriodicId,
        j,
        j2,
        0);
  }
  ...
  public boolean isConflict(SyncOperation syncOperation) {
    ...
    if (this.target.account.type.equals(endPoint.account.type)
        && this.target.provider.equals(endPoint.provider)) {
      SyncStorageEngine.EndPoint endPoint2 = this.target;
      if (endPoint2.userId == endPoint.userId
          && (!this.allowParallelSyncs || endPoint2.account.name.equals(endPoint.account.name))) {
        return true;
      }
    }
    return false;
  ...
  public boolean matchesPeriodicOperation(SyncOperation syncOperation) {
    return this.target.matchesSpec(syncOperation.target)
        && SyncManager.syncExtrasEquals(this.mImmutableExtras, syncOperation.mImmutableExtras, true)
  ...
  public final String toKey() {
    ...
    sb.append(
        " account {name="
            + this.target.account.name
            + ", user="
            + this.target.userId
            + ", type="
            + this.target.account.type
            + "}");
  ...
  public String dump(
    ...
    if (!z) {
      ...
      if (syncAdapterStateFetcher != null) {
        sb.append(" [");
        sb.append(
            syncAdapterStateFetcher.getStandbyBucket(
                UserHandle.getUserId(this.owningUid), this.owningPackage));
        sb.append("]");
  ...
  public String wakeLockName() {
    ...
    String str2 =
        this.target.provider + "/" + this.target.account.type + "/" + this.target.account.name;
  ...
  public Object[] toEventLog(int i) {
    ...
    return new Object[] {
      endPoint.provider, valueOf, valueOf2, Integer.valueOf(endPoint.account.name.hashCode())
    };
  }
