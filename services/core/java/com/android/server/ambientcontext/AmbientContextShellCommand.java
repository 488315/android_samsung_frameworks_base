...
public final class AmbientContextShellCommand extends ShellCommand {
  ...
  public class TestableCallbackInternal {
    ...
    public final IAmbientContextObserver createAmbientContextObserver() {
      return new IAmbientContextObserver
          .Stub() { // from class:
                    // com.android.server.ambientcontext.AmbientContextShellCommand.TestableCallbackInternal.1
      ...
      return new RemoteCallback(
          new RemoteCallback
            ...
            public final void onResult(Bundle bundle) {
              AmbientContextShellCommand.TestableCallbackInternal.this
  ...
  public final int runStartDetection() {
    ...
    ambientContextManagerService.startDetection(
        parseInt,
        ambientContextEventRequest,
        nextArgRequired,
        testableCallbackInternal.createAmbientContextObserver());
  ...
  public final int runStopDetection() {
    this.mService.stopAmbientContextEvent(
        Integer.parseInt(getNextArgRequired()), getNextArgRequired());
    return 0;
  }

  public final int runQueryServiceStatus() {
    this.mService.queryServiceStatus(
  ...
  public final int runQueryWearableServiceStatus() {
    this.mService.queryServiceStatus(
        Integer.parseInt(getNextArgRequired()),
        getNextArgRequired(),
        new int[] {FrameworkStatsLog.GPS_ENGINE_STATE_CHANGED},
        sTestableCallbackInternal.createRemoteStatusCallback());
    return 0;
  }
  ...
  public final int runQueryMixedServiceStatus() {
    this.mService.queryServiceStatus(
        Integer.parseInt(getNextArgRequired()),
        getNextArgRequired(),
        new int[] {1, FrameworkStatsLog.GPS_ENGINE_STATE_CHANGED},
        sTestableCallbackInternal.createRemoteStatusCallback());
    return 0;
  }
  ...
  public void onHelp() {
    ...
    outPrintWriter.println(
        "  start-detection-wearable USER_ID PACKAGE_NAME: Starts AmbientContextEvent detection for"
            + " wearable.");
    outPrintWriter.println(
        "  start-detection-mixed USER_ID PACKAGE_NAME:  Starts AmbientContextEvent detection for"
            + " mixed events.");
    outPrintWriter.println(
    ...
    outPrintWriter.println(
        "  query-wearable-service-status USER_ID PACKAGE_NAME: Prints the service status code for"
            + " wearable.");
    outPrintWriter.println(
        "  query-mixed-service-status USER_ID PACKAGE_NAME: Prints the service status code for"
            + " mixed events.");
    outPrintWriter.println(
  ...
  public final int getBoundPackageName() {
    ...
    ComponentName componentName =
        this.mService.getComponentName(
            Integer.parseInt(getNextArgRequired()),
