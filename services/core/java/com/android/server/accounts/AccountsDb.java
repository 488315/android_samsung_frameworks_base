...
public class AccountsDb implements AutoCloseable {
    ...
    public long findExtrasIdByAccountId(long j, String str) {
        Cursor query = this.mDeDatabase.getReadableDatabaseUserIsUnlocked().query("ceDb.extras", new String[]{KnoxCustomManagerService.f1773ID}, "accounts_id=" + j + " AND key=?", new String[]{str}, null, null, null);
        try {
            if (query.moveToNext()) {
    ...
    public long findCeAccountId(Account account) {
        Cursor query = this.mDeDatabase.getReadableDatabaseUserIsUnlocked().query("ceDb.accounts", new String[]{KnoxCustomManagerService.f1773ID}, "name=? AND type=?", new String[]{account.name, account.type}, null, null, null);
        try {
            if (!query.moveToNext()) {
                query.close();
                return -1L;
    ...
    public class DeDatabaseHelper extends SQLiteOpenHelper {
        ...
        public final void migratePreNDbToDe(File file) {
            ...
            writableDatabase.execSQL("INSERT INTO " + AccountsDb.TABLE_DEBUG + "(" + KnoxCustomManagerService.f1773ID + "," + AccountsDb.DEBUG_TABLE_ACTION_TYPE + "," + AccountsDb.DEBUG_TABLE_TIMESTAMP + "," + AccountsDb.DEBUG_TABLE_CALLER_UID + "," + AccountsDb.DEBUG_TABLE_TABLE_NAME + "," + AccountsDb.DEBUG_TABLE_KEY + ") SELECT " + KnoxCustomManagerService.f1773ID + "," + AccountsDb.DEBUG_TABLE_ACTION_TYPE + "," + AccountsDb.DEBUG_TABLE_TIMESTAMP + "," + AccountsDb.DEBUG_TABLE_CALLER_UID + "," + AccountsDb.DEBUG_TABLE_TABLE_NAME + "," + AccountsDb.DEBUG_TABLE_KEY + " FROM preNDb." + AccountsDb.TABLE_DEBUG);
    ...
    public long findSharedAccountId(Account account) {
        Cursor query = this.mDeDatabase.getReadableDatabase().query("shared_accounts", new String[]{KnoxCustomManagerService.f1773ID}, "name=? AND type=?", new String[]{account.name, account.type}, null, null, null);
        try {
            if (query.moveToNext()) {
                return query.getLong(0);
            }
    ...
    public long findDeAccountId(Account account) {
        Cursor query = this.mDeDatabase.getReadableDatabase().query("accounts", new String[]{KnoxCustomManagerService.f1773ID}, "name=? AND type=?", new String[]{account.name, account.type}, null, null, null);
        try {
            if (!query.moveToNext()) {
                query.close();
                return -1L;
    ...
    public Map findAllDeAccounts() {
        ...
        Cursor query = readableDatabase.query("accounts", new String[]{KnoxCustomManagerService.f1773ID, "type", "name"}, null, null, null, null, KnoxCustomManagerService.f1773ID);
        while (query.moveToNext()) {
            try {
                long j = query.getLong(0);
