...
public class AccountsDb implements AutoCloseable {
  ...
  public static String DEBUG_ACTION_CALLED_ACCOUNT_SESSION_FINISH =
  ...
  public class CeDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sQLiteDatabase) {
      ...
      sQLiteDatabase.execSQL(
          "CREATE TABLE accounts ( _id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, type"
              + " TEXT NOT NULL, password TEXT, UNIQUE(name,type))");
      sQLiteDatabase.execSQL(
          "CREATE TABLE authtokens (  _id INTEGER PRIMARY KEY AUTOINCREMENT,  accounts_id INTEGER"
              + " NOT NULL, type TEXT NOT NULL,  authtoken TEXT,  UNIQUE (accounts_id,type))");
      sQLiteDatabase.execSQL(
          "CREATE TABLE extras ( _id INTEGER PRIMARY KEY AUTOINCREMENT, accounts_id INTEGER, key"
              + " TEXT NOT NULL, value TEXT, UNIQUE(accounts_id,key))");
    ...
    public final void createAccountsDeletionTrigger(SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          " CREATE TRIGGER accountsDelete DELETE ON accounts BEGIN   DELETE FROM authtokens    "
              + " WHERE accounts_id=OLD._id ;   DELETE FROM extras     WHERE accounts_id=OLD._id ;"
              + " END");
    }
    ...
    public static CeDatabaseHelper create(Context context, File file, File file2) {
      ...
      boolean migratePreNDbToCe =
          (exists || !file.exists()) ? false : migratePreNDbToCe(file, file2);
  ...
  public Cursor findAuthtokenForAllAccounts(String str, String str2) {
    return this.mDeDatabase
        ...
        .rawQuery(
            "SELECT ceDb.authtokens._id, ceDb.accounts.name, ceDb.authtokens.type FROM"
                + " ceDb.accounts JOIN ceDb.authtokens ON ceDb.accounts._id ="
                + " ceDb.authtokens.accounts_id WHERE ceDb.authtokens.authtoken = ? AND"
                + " ceDb.accounts.type = ?",
            new String[] {str2, str});
  }
  ...
  public boolean deleteAuthToken(String str) {
    return this.mDeDatabase
            .getWritableDatabaseUserIsUnlocked()
            .delete("ceDb.authtokens", "_id= ?", new String[] {str})
        > 0;
  }
  ...
  public int updateCeAccountPassword(long j, String str) {
    ...
    return writableDatabaseUserIsUnlocked.update(
        "ceDb.accounts", contentValues, "_id=?", new String[] {String.valueOf(j)});
  }
  ...
  public boolean deleteAuthTokensByAccountId(long j) {
    return this.mDeDatabase
            .getWritableDatabaseUserIsUnlocked()
            .delete("ceDb.authtokens", "accounts_id=?", new String[] {String.valueOf(j)})
        > 0;
  }
  ...
  public long findExtrasIdByAccountId(long j, String str) {
    Cursor query =
        this.mDeDatabase
            ...
            .query(
                "ceDb.extras",
                new String[] {KnoxCustomManagerService.f1773ID},
  ...
  public boolean updateExtra(long j, String str) {
    ...
    return writableDatabaseUserIsUnlocked.update(
            "extras", contentValues, "_id=?", new String[] {String.valueOf(j)})
        == 1;
  }
  ...
  public class DeDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sQLiteDatabase) {
      ...
      sQLiteDatabase.execSQL(
          "CREATE TABLE accounts ( _id INTEGER PRIMARY KEY, name TEXT NOT NULL, type TEXT NOT NULL,"
    ...
    public final void createSharedAccountsTable(SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          "CREATE TABLE shared_accounts ( _id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT"
              + " NULL, type TEXT NOT NULL, UNIQUE(name,type))");
    }

    public final void createAccountsDeletionTrigger(SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          " CREATE TRIGGER accountsDelete DELETE ON accounts BEGIN   DELETE FROM grants     WHERE"
              + " accounts_id=OLD._id ; END");
    }

    public final void createGrantsTable(SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          "CREATE TABLE grants (  accounts_id INTEGER NOT NULL, auth_token_type STRING NOT NULL, "
    ...
    public final void createAccountsVisibilityTable(SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          "CREATE TABLE visibility ( accounts_id INTEGER NOT NULL, _package TEXT NOT NULL, value"
              + " INTEGER, PRIMARY KEY(accounts_id,_package))");
    }
    ...
    public static void createDebugTable(SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          "CREATE TABLE "
              ...
              + " INTEGER NOT NULL,"
              + AccountsDb.DEBUG_TABLE_TABLE_NAME
              + " TEXT NOT NULL,"
              + AccountsDb.DEBUG_TABLE_KEY
              + " INTEGER PRIMARY KEY)");
    ...
    public final void createAccountsDeletionVisibilityCleanupTrigger(
        SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          " CREATE TRIGGER accountsDeleteVisibility DELETE ON accounts BEGIN   DELETE FROM"
              + " visibility     WHERE accounts_id=OLD._id ; END");
    }
    ...
    public final void migratePreNDbToDe(File file) {
      ...
      writableDatabase.execSQL(
          "INSERT INTO accounts(_id,name,type, previous_name,"
              + " last_password_entry_time_millis_epoch) SELECT _id,name,type, previous_name,"
              + " last_password_entry_time_millis_epoch FROM preNDb.accounts");
      writableDatabase.execSQL(
  ...
  public boolean deleteSharedAccount(Account account) {
    return this.mDeDatabase
            .getWritableDatabase()
            .delete(
                "shared_accounts", "name=? AND type=?", new String[] {account.name, account.type})
        > 0;
  }
    ...
    try {
      cursor =
          readableDatabase.query(
              "shared_accounts", new String[] {"name", "type"}, null, null, null, null, null);
  ...
  public long findAccountLastAuthenticatedTime(Account account) {
    return DatabaseUtils.longForQuery(
        this.mDeDatabase.getReadableDatabase(),
        "SELECT last_password_entry_time_millis_epoch FROM accounts WHERE name=? AND type=?",
        new String[] {account.name, account.type});
  }
  ...
  public void dumpDeAccountsTable(PrintWriter printWriter) {
    Cursor query =
        this.mDeDatabase
            .getReadableDatabase()
            .query("accounts", ACCOUNT_TYPE_COUNT_PROJECTION, null, null, "type", null, null);
    while (query.moveToNext()) {
      try {
  ...
  public Map findAllDeAccounts() {
    ...
    Cursor query =
        readableDatabase.query(
            "accounts",
            new String[] {KnoxCustomManagerService.f1773ID, "type", "name"},
            null,
            null,
            null,
            null,
  ...
  public long insertDeAccount(Account account, long j) {
    SQLiteDatabase writableDatabase = this.mDeDatabase.getWritableDatabase();
    ContentValues contentValues = new ContentValues();
    contentValues.put(KnoxCustomManagerService.f1773ID, Long.valueOf(j));
    contentValues.put("name", account.name);
    contentValues.put("type", account.type);
    contentValues.put(
        "last_password_entry_time_millis_epoch", Long.valueOf(System.currentTimeMillis()));
  ...
  public boolean renameDeAccount(long j, String str, String str2) {
    ...
    return writableDatabase.update(
            "accounts", contentValues, "_id=?", new String[] {String.valueOf(j)})
        > 0;
  }
  ...
  public boolean deleteGrantsByAccountIdAuthTokenTypeAndUid(long j, String str, long j2) {
    return this.mDeDatabase
            ...
            .delete(
                "grants",
                "accounts_id=? AND auth_token_type=? AND uid=?",
                new String[] {String.valueOf(j), str, String.valueOf(j2)})
        > 0;
  }
  ...
  public List findAllUidGrants() {
    ...
    ArrayList arrayList = new ArrayList();
    Cursor query =
        readableDatabase.query("grants", new String[] {"uid"}, null, null, "uid", null, null);
    while (query.moveToNext()) {
      try {
  ...
  public long findMatchingGrantsCount(int i, String str, Account account) {
    return DatabaseUtils.longForQuery(
        this.mDeDatabase.getReadableDatabase(),
        "SELECT COUNT(*) FROM grants, accounts WHERE accounts_id=_id AND uid=? AND"
            + " auth_token_type=? AND name=? AND type=?",
        new String[] {String.valueOf(i), str, account.name, account.type});
  }

  public long findMatchingGrantsCountAnyToken(int i, Account account) {
    return DatabaseUtils.longForQuery(
        this.mDeDatabase.getReadableDatabase(),
        "SELECT COUNT(*) FROM grants, accounts WHERE accounts_id=_id AND uid=? AND name=? AND"
            + " type=?",
        new String[] {String.valueOf(i), account.name, account.type});
  }
  ...
  public boolean deleteGrantsByUid(int i) {
    return this.mDeDatabase
            .getWritableDatabase()
            .delete("grants", "uid=?", new String[] {Integer.toString(i)})
        > 0;
  }
  ...
  public boolean setAccountVisibility(long j, String str, int i) {
    ...
    return writableDatabase.replace(
  ...
  public Map findAllVisibilityValues() {
    ...
    Cursor rawQuery =
        readableDatabase.rawQuery(
            "SELECT visibility._package, visibility.value, accounts.name, accounts.type FROM"
                + " visibility JOIN accounts ON accounts._id = visibility.accounts_id",
            null);
  ...
  public boolean deleteMetaByAuthTypeAndUid(String str, int i) {
    ...
    return writableDatabase.delete(
            "meta", "key=? AND value=?", new String[] {sb.toString(), String.valueOf(i)})
        > 0;
  }
  ...
  public List findAllAccountGrants() {
    Cursor rawQuery =
        this.mDeDatabase
            .getReadableDatabase()
            .rawQuery("SELECT name, uid FROM accounts, grants WHERE accounts_id=_id", null);
    if (rawQuery != null) {
      try {
  ...
  public class PreNDatabaseHelper extends SQLiteOpenHelper {
    ...
    public final void addLastSuccessfullAuthenticatedTimeColumn(SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          "ALTER TABLE accounts ADD COLUMN last_password_entry_time_millis_epoch DEFAULT 0");
    }
    ...
    public final void createAccountsDeletionTrigger(SQLiteDatabase sQLiteDatabase) {
      sQLiteDatabase.execSQL(
          " CREATE TRIGGER accountsDelete DELETE ON accounts BEGIN   DELETE FROM authtokens    "
              + " WHERE accounts_id=OLD._id ;   DELETE FROM extras     WHERE accounts_id=OLD._id ; "
              + "  DELETE FROM grants     WHERE accounts_id=OLD._id ; END");
    }
    ...
    public final void populateMetaTableWithAuthTypeAndUID(SQLiteDatabase sQLiteDatabase, Map map) {
      for (Map.Entry entry : map.entrySet()) {
        insertMetaAuthTypeAndUid(
            sQLiteDatabase, (String) entry.getKey(), ((Integer) entry.getValue()).intValue());
      }
    }
    ...
    public void onUpgrade(SQLiteDatabase sQLiteDatabase, int i, int i2) {
      ...
      if (i == 8) {
        populateMetaTableWithAuthTypeAndUID(
            sQLiteDatabase,
            AccountManagerService.getAuthenticatorTypeAndUIDForUser(this.mContext, this.mUserId));
        i++;
      }
      if (i != i2) {
        Log.e("AccountsDb", "failed to upgrade version " + i + " to version " + i2);
  ...
  public List findCeAccountsNotInDe() {
    Cursor rawQuery =
        this.mDeDatabase
            .getReadableDatabaseUserIsUnlocked()
            .rawQuery(
                "SELECT name,type FROM ceDb.accounts WHERE NOT EXISTS  (SELECT _id FROM accounts"
                    + " WHERE _id=ceDb.accounts._id )",
                null);
    try {
      ...
      int longForQuery =
          (int)
              DatabaseUtils.longForQuery(
                  readableDatabase, "SELECT COUNT(*) FROM " + TABLE_DEBUG, null);
      if (longForQuery < 64) {
        return longForQuery;
      }
      return DatabaseUtils.longForQuery(
  ...
  public SQLiteStatement compileSqlStatementForLogging() {
    return this.mDeDatabase
        .getWritableDatabase()
        .compileStatement("INSERT OR REPLACE INTO " + TABLE_DEBUG + " VALUES (?,?,?,?,?,?)");
  }
  ...
  public void dumpDebugTable(PrintWriter printWriter) {
    Cursor query =
        this.mDeDatabase
            .getReadableDatabase()
            .query(TABLE_DEBUG, null, null, null, null, null, DEBUG_TABLE_TIMESTAMP);
  ...
  public static void resetDatabase(SQLiteDatabase sQLiteDatabase) {
    ...
    rawQuery.close();
    rawQuery =
        sQLiteDatabase.rawQuery("SELECT name FROM sqlite_master WHERE type ='trigger'", null);
    while (rawQuery.moveToNext()) {
      try {
