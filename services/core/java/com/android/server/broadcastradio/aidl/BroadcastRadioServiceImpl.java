...
public final class BroadcastRadioServiceImpl {
    ...
    public final AnonymousClass1 mServiceListener = new IServiceCallback.Stub() { // from class:
                    ...
                    synchronized (BroadcastRadioServiceImpl.this.mLock) {
                        try {
                            ...
                            RadioModule tryLoadingModule =
                                    RadioModule.tryLoadingModule(num.intValue(), iBinder, str);
                            if (tryLoadingModule == null) {
                                Slogf.w(
                                        "BcRadioAidlSrv",
                                        "No module %s with id %d (HAL AIDL)",
                                        str,
                                        num);
                                return;
                            }
                            ...
                            RadioModule radioModule =
                                    (RadioModule)
                                            BroadcastRadioServiceImpl.this.mModules.get(
                                                    num.intValue());
                            BroadcastRadioServiceImpl.this.mModules.put(
                                    num.intValue(), tryLoadingModule);
                            if (radioModule != null) {
                            ...
                            try {
                                ((IBroadcastRadio.Stub.Proxy) tryLoadingModule.mService)
                                        .mRemote.linkToDeath(
                                                BroadcastRadioServiceImpl.this
                                ...
                                Slogf.w(
                                        "BcRadioAidlSrv",
                                        "Service has already died, so remove its entry from"
                                            + " mModules.");
    ...
    public final class BroadcastRadioDeathRecipient implements IBinder.DeathRecipient {
        ...
        public final void binderDied() {
            Slogf.i(
                    "BcRadioAidlSrv",
                    "ServiceDied for module id %d",
                    Integer.valueOf(this.mModuleId));
            synchronized (BroadcastRadioServiceImpl.this.mLock) {
                try {
                    RadioModule radioModule =
                            (RadioModule)
                                    BroadcastRadioServiceImpl.this.mModules.removeReturnOld(
                                            this.mModuleId);
                    if (radioModule != null) {
                        radioModule.closeSessions();
                    }
                    Iterator it =
                            ((ArrayMap) BroadcastRadioServiceImpl.this.mServiceNameToModuleIdMap)
                                    .entrySet()
                                    .iterator();
                    while (true) {
                        if (!it.hasNext()) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            try {
                ServiceManager.registerForNotifications(
                        (String) arrayList.get(i), this.mServiceListener);
