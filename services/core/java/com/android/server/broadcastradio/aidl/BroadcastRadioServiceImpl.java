...
public final class BroadcastRadioServiceImpl {
            ...
            synchronized (BroadcastRadioServiceImpl.this.mLock) {
                ...
                try {
                    ...
                    if (BroadcastRadioServiceImpl.DEBUG) {
                        Slogf.m88d("BcRadioAidlSrv", "Loaded broadcast radio module %s with id %d (HAL AIDL)", str, num);
                    }
                    ...
                    try {
                        tryLoadingModule.getService().asBinder().linkToDeath(BroadcastRadioServiceImpl.this.new BroadcastRadioDeathRecipient(num.intValue()), num.intValue());
                    } catch (RemoteException unused) {
                        Slogf.m102w("BcRadioAidlSrv", "Service has already died, so remove its entry from mModules.");
            ...
            synchronized (BroadcastRadioServiceImpl.this.mLock) {
                ...
                for (Map.Entry entry : BroadcastRadioServiceImpl.this.mServiceNameToModuleIdMap.entrySet()) {
                    if (((Integer) entry.getValue()).intValue() == this.mModuleId) {
                        Slogf.m105w("BcRadioAidlSrv", "Service %s died, removed RadioModule with ID %d", entry.getKey(), Integer.valueOf(this.mModuleId));
                        return;
                    }
                }
            }
    ...
    public BroadcastRadioServiceImpl(ArrayList arrayList) {
        if (DEBUG) {
            Slogf.m88d("BcRadioAidlSrv", "Initializing BroadcastRadioServiceImpl %s", IBroadcastRadio.DESCRIPTOR);
        }
        for (int i = 0; i < arrayList.size(); i++) {
            try {
                ServiceManager.registerForNotifications((String) arrayList.get(i), this.mServiceListener);
            } catch (RemoteException e) {
                Slogf.m93e("BcRadioAidlSrv", e, "failed to register for service notifications for service %s", arrayList.get(i));
            }
        }
    }
    ...
    public ICloseHandle addAnnouncementListener(int[] iArr, IAnnouncementListener iAnnouncementListener) {
        boolean z;
        if (DEBUG) {
            Slogf.m88d("BcRadioAidlSrv", "Add AnnouncementListener with enable types %s", Arrays.toString(iArr));
        }
        ...
        synchronized (this.mLock) {
            ...
            for (int i = 0; i < this.mModules.size(); i++) {
                try {
                    ...
                    Slogf.m106w("BcRadioAidlSrv", e, "Announcements not supported for this module", new Object[0]);
                }
            }
        }
        if (!z) {
