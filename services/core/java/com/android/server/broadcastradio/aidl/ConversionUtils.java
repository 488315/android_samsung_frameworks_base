    ...
    public static VendorKeyValue[] vendorInfoToHalVendorKeyValues(Map map) {
        ...
        for (Map.Entry entry : map.entrySet()) {
            ...
            if (str2 == null || str == null) {
                Slogf.m105w("BcRadioAidlSrv.convert", "VendorKeyValue contains invalid entry: key = %s, value = %s", str2, str);
            } else {
    ...
    public static Map vendorInfoFromHalVendorKeyValues(VendorKeyValue[] vendorKeyValueArr) {
        ...
        for (VendorKeyValue vendorKeyValue : vendorKeyValueArr) {
            ...
            if (str2 == null || (str = vendorKeyValue.value) == null) {
                Slogf.m105w("BcRadioAidlSrv.convert", "VendorKeyValue contains invalid entry: key = %s, value = %s", str2, vendorKeyValue.value);
            } else {
    ...
    public static RadioManager.BandDescriptor[] amfmConfigToBands(AmFmRegionConfig amFmRegionConfig) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (band == Utils.FrequencyBand.UNKNOWN) {
                Slogf.m92e("BcRadioAidlSrv.convert", "Unknown frequency band at %d kHz", Integer.valueOf(amFmRegionConfig.ranges[i].lowerBound));
        ...
        for (int i = 0; i < metadataArr.length; i++) {
            switch (metadataArr[i].getTag()) {
                ...
                case 15:
                    ...
                    break;
                default:
                    Slogf.m105w("BcRadioAidlSrv.convert", "Ignored unknown metadata entry: %s", metadataArr[i]);
                    break;
            }
        }
    ...
    public static ProgramList.Chunk chunkFromHalProgramListChunk(ProgramListChunk programListChunk) {
        ...
        while (true) {
            ...
            if (programInfoFromHalProgramInfo == null) {
                Slogf.m105w("BcRadioAidlSrv.convert", "Program info %s in program list chunk is not valid", programListChunk.modified[i2]);
            } else {
