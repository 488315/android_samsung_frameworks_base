...
public final class TunerSession extends ITuner.Stub {
    ...
    public void setConfiguration(final RadioManager.BandConfig bandConfig) {
        if (!RadioServiceUserController.isCurrentOrSystemUser()) {
            Slogf.m102w("BcRadioAidlSrv.session", "Cannot set configuration for AIDL HAL client from non-current user");
            return;
        }
        synchronized (this.mLock) {
        ...
        Slogf.m94i("BcRadioAidlSrv.session", "Ignoring setConfiguration - not applicable for broadcastradio HAL AIDL");
    ...
    public void setMuted(boolean z) {
        synchronized (this.mLock) {
            ...
            this.mIsMuted = z;
            Slogf.m105w("BcRadioAidlSrv.session", "Mute %b via RadioService is not implemented - please handle it via app", Boolean.valueOf(z));
        }
    }
    ...
    public void cancel() {
        ...
        if (!RadioServiceUserController.isCurrentOrSystemUser()) {
            Slogf.m102w("BcRadioAidlSrv.session", "Cannot cancel on AIDL HAL client from non-current user");
            return;
        }
        synchronized (this.mLock) {
            checkNotClosedLocked();
    ...
    public void cancelAnnouncement() {
        Slogf.m102w("BcRadioAidlSrv.session", "Announcements control doesn't involve cancelling at the HAL level in AIDL");
    }
    ...
    public boolean startBackgroundScan() {
        Slogf.m102w("BcRadioAidlSrv.session", "Explicit background scan trigger is not supported with HAL AIDL");
        if (!RadioServiceUserController.isCurrentOrSystemUser()) {
            Slogf.m102w("BcRadioAidlSrv.session", "Cannot start background scan on AIDL HAL client from non-current user");
            return false;
        }
    ...
    public void startProgramListUpdates(ProgramList.Filter filter) {
        ...
        if (!RadioServiceUserController.isCurrentOrSystemUser()) {
            Slogf.m102w("BcRadioAidlSrv.session", "Cannot start program list updates on AIDL HAL client from non-current user");
            return;
        }
        if (filter == null) {
    ...
    public final void dispatchClientUpdateChunks(List list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            try {
                ...
                Slogf.m106w("BcRadioAidlSrv.session", e, "mCallback.onProgramListUpdated() failed", new Object[0]);
            }
        }
    }
    ...
    public void stopProgramListUpdates() {
        ...
        if (!RadioServiceUserController.isCurrentOrSystemUser()) {
            Slogf.m102w("BcRadioAidlSrv.session", "Cannot stop program list updates on AIDL HAL client from non-current user");
            return;
        }
        synchronized (this.mLock) {
            checkNotClosedLocked();
    ...
    public void setConfigFlag(int i, boolean z) {
        ...
        if (!RadioServiceUserController.isCurrentOrSystemUser()) {
            Slogf.m102w("BcRadioAidlSrv.session", "Cannot set config flag for AIDL HAL client from non-current user");
            return;
        }
        synchronized (this.mLock) {
            checkNotClosedLocked();
    ...
    public Map setParameters(Map map) {
        ...
        if (!RadioServiceUserController.isCurrentOrSystemUser()) {
            Slogf.m102w("BcRadioAidlSrv.session", "Cannot set parameters for AIDL HAL client from non-current user");
            return new ArrayMap();
        }
        synchronized (this.mLock) {
            checkNotClosedLocked();
