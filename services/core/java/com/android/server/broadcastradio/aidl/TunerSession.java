...
public final class TunerSession extends ITuner.Stub {
  ...
  public TunerSession(
      RadioModule radioModule, IBroadcastRadio iBroadcastRadio, ITunerCallback iTunerCallback) {
    ...
    synchronized (this.mLock) {
      ...
      if (num != null) {
        try {
          ...
          Slogf.m106w("BcRadioAidlSrv.session", e, "mCallback.onError(%s) failed", num);
        }
      }
  ...
  public void setConfiguration(final RadioManager.BandConfig bandConfig) {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(
          "BcRadioAidlSrv.session",
          "Cannot set configuration for AIDL HAL client from non-current user");
      return;
    }
    synchronized (this.mLock) {
    ...
    Slogf.m94i(
        "BcRadioAidlSrv.session",
        "Ignoring setConfiguration - not applicable for broadcastradio HAL AIDL");
    this.mModule.fanoutAidlCallback(
        new RadioModule
            .AidlCallbackRunnable() { // from class:
                                      // com.android.server.broadcastradio.aidl.TunerSession$$ExternalSyntheticLambda1
  ...
  public void setMuted(boolean z) {
    synchronized (this.mLock) {
      ...
      Slogf.m105w(
          "BcRadioAidlSrv.session",
          "Mute %b via RadioService is not implemented - please handle it via app",
          Boolean.valueOf(z));
    }
  }
  ...
  public void step(boolean z, boolean z2) {
    ...
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w("BcRadioAidlSrv.session", "Cannot step on AIDL HAL client from non-current user");
      return;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
  ...
  public void seek(boolean z, boolean z2) {
    ...
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w("BcRadioAidlSrv.session", "Cannot scan on AIDL HAL client from non-current user");
      return;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
  ...
  public void tune(ProgramSelector programSelector) {
    ...
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w("BcRadioAidlSrv.session", "Cannot tune on AIDL HAL client from non-current user");
      return;
    }
    android.hardware.broadcastradio.ProgramSelector programSelectorToHalProgramSelector =
        ConversionUtils.programSelectorToHalProgramSelector(programSelector);
  ...
  public void cancel() {
    Slogf.m94i("BcRadioAidlSrv.session", "Cancel");
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(
          "BcRadioAidlSrv.session", "Cannot cancel on AIDL HAL client from non-current user");
      return;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
      try {
        ...
        Slogf.m90e("BcRadioAidlSrv.session", "Failed to cancel tuner session");
  ...
  public void cancelAnnouncement() {
    Slogf.m102w(
        "BcRadioAidlSrv.session",
        "Announcements control doesn't involve cancelling at the HAL level in AIDL");
  }
  ...
  public boolean startBackgroundScan() {
    Slogf.m102w(
        "BcRadioAidlSrv.session",
        "Explicit background scan trigger is not supported with HAL AIDL");
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(
          "BcRadioAidlSrv.session",
          "Cannot start background scan on AIDL HAL client from non-current user");
      return false;
    }
    this.mModule.fanoutAidlCallback(
        new RadioModule
            .AidlCallbackRunnable() { // from class:
                                      // com.android.server.broadcastradio.aidl.TunerSession$$ExternalSyntheticLambda0
          @Override // com.android.server.broadcastradio.aidl.RadioModule.AidlCallbackRunnable
          public final void run(ITunerCallback iTunerCallback, int i) {
  ...
  public void updateProgramInfoFromHalCache(ProgramInfoCache programInfoCache) {
    synchronized (this.mLock) {
      ...
      dispatchClientUpdateChunks(
          programInfoCache2.filterAndUpdateFromInternal(programInfoCache, true));
    }
  }
    ...
    for (int i = 0; i < list.size(); i++) {
      try {
        if (!ConversionUtils.isAtLeastU(getUid())) {
          this.mCallback.onProgramListUpdated(
              ConversionUtils.convertChunkToTargetSdkVersion(
                  (ProgramList.Chunk) list.get(i), getUid()));
        } else {
          this.mCallback.onProgramListUpdated((ProgramList.Chunk) list.get(i));
        }
      } catch (RemoteException e) {
        Slogf.m106w(
            "BcRadioAidlSrv.session", e, "mCallback.onProgramListUpdated() failed", new Object[0]);
      }
    }
  }
  ...
  public void setConfigFlag(int i, boolean z) {
    this.mLogger.logRadioEvent(
        "set ConfigFlag %s to %b ", ConfigFlag$$.toString(i), Boolean.valueOf(z));
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(
          "BcRadioAidlSrv.session",
          "Cannot set config flag for AIDL HAL client from non-current user");
      return;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
  ...
  public Map setParameters(Map map) {
    ...
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(
          "BcRadioAidlSrv.session",
          "Cannot set parameters for AIDL HAL client from non-current user");
      return new ArrayMap();
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
      try {
        vendorInfoFromHalVendorKeyValues =
            ConversionUtils.vendorInfoFromHalVendorKeyValues(
                this.mService.setParameters(ConversionUtils.vendorInfoToHalVendorKeyValues(map)));
      } catch (RemoteException e) {
        throw e.rethrowFromSystemServer();
      }
    }
  ...
  public Map getParameters(List list) {
    ...
    synchronized (this.mLock) {
      ...
      try {
        vendorInfoFromHalVendorKeyValues =
            ConversionUtils.vendorInfoFromHalVendorKeyValues(
                this.mService.getParameters((String[]) list.toArray(new String[0])));
      } catch (RemoteException e) {
        throw e.rethrowFromSystemServer();
      }
    }
