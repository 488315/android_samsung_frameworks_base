...
class Tuner extends ITuner.Stub {
  ...
  public void setConfiguration(RadioManager.BandConfig bandConfig) {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot set configuration for HAL 1.x client from non-current user");
      return;
    }
    if (bandConfig == null) {
  ...
  public void step(boolean z, boolean z2) {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot step on HAL 1.x client from non-current user");
      return;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
  ...
  public void seek(boolean z, boolean z2) {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot seek on HAL 1.x client from non-current user");
      return;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
  ...
  public void tune(ProgramSelector programSelector) {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot tune on HAL 1.x client from non-current user");
      return;
    }
  ...
  public void cancel() {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot cancel on HAL 1.x client from non-current user");
      return;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
  ...
  public void cancelAnnouncement() {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot cancel announcement on HAL 1.x client from non-current user");
      return;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
  ...
  public boolean startBackgroundScan() {
    ...
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot start background scan on HAL 1.x client from non-current user");
      return false;
    }
    synchronized (this.mLock) {
      checkNotClosedLocked();
  ...
  public void startProgramListUpdates(ProgramList.Filter filter) {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot start program list updates on HAL 1.x client from non-current user");
    } else {
  ...
  public void stopProgramListUpdates() {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot stop program list updates on HAL 1.x client from non-current user");
    } else {
  ...
  public void setConfigFlag(int i, boolean z) {
    if (!RadioServiceUserController.isCurrentOrSystemUser()) {
      Slogf.m102w(TAG, "Cannot set config flag for HAL 1.x client from non-current user");
    } else {
      if (i == 2) {
        synchronized (this.mLock) {
          checkNotClosedLocked();
