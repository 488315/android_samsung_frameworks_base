...
public final class AppRestrictionController {
    ...
    public final class ConstantsObserver implements DeviceConfig.OnPropertiesChangedListener {
        ...
        public ConstantsObserver(Handler handler, Context context) {
            this.mDefaultBgPromptFgsWithNotiToBgRestricted = context.getResources().getBoolean(R.bool.config_batterySaver_full_disableOptionalSensors);
            this.mDefaultBgPromptAbusiveAppToBgRestricted = context.getResources().getBoolean(R.bool.config_batterySaver_full_disableLaunchBoost);
        }
        ...
        public void onPropertiesChanged(DeviceConfig.Properties properties) {
            ...
            while (it.hasNext() && (str = (String) it.next()) != null && str.startsWith("bg_")) {
                switch (str) {
                    case "bg_prompt_abusive_apps_to_bg_restricted":
                        updateBgPromptAbusiveAppToBgRestricted();
                        break;
                    case "bg_auto_restrict_abusive_apps":
                        updateBgAutoRestrictAbusiveApps();
                        break;
                    case "bg_prompt_fgs_on_long_running":
                        updateBgPromptFgsOnLongRunning();
                        break;
                    case "bg_auto_restricted_bucket_on_bg_restricted":
                        updateBgAutoRestrictedBucketChanged();
                        break;
                    case "bg_restriction_exempted_packages":
                        updateBgRestrictionExemptedPackages();
                        break;
                    case "bg_abusive_notification_minimal_interval":
                        updateBgAbusiveNotificationMinimalInterval();
                        break;
                    case "bg_long_fgs_notification_minimal_interval":
                        updateBgLongFgsNotificationMinimalInterval();
                        break;
                    case "bg_prompt_fgs_with_noti_on_long_running":
                        updateBgPromptFgsWithNotiOnLongRunning();
                        break;
                    case "bg_prompt_fgs_with_noti_to_bg_restricted":
    ...
    */
    public final boolean isSystemModule(String str) {
        boolean z;
        synchronized (this.mLock) {
            Boolean bool = (Boolean) this.mSystemModulesCache.get(str);
            if (bool != null) {
    ...
    public class NotificationHelper {
        ...
        public void postLongRunningFgsIfNecessary(String str, int i) {
            ...
            if (this.mBgController.mConstantsObserver.mBgPromptFgsOnLongRunning) {
                ...
                postNotificationIfNecessary(1, R.string.restr_pin_create_pin, R.string.relative_time, PendingIntent.getBroadcastAsUser(this.mContext, 0, intent, 201326592, UserHandle.SYSTEM), str, i, null);
            }
        }
    ...
    public int getPotentialSystemExemptionReason(int i, String str) {
        ...
        return appStandbyInternal.isActiveDeviceAdmin(str, userId) ? FrameworkStatsLog.f58x60da79b1 : (activityManagerService.mConstants.mFlagSystemExemptPowerRestrictionsEnabled && appOpsManager.checkOpNoThrow(128, i, str) == 0) ? FrameworkStatsLog.TIF_TUNE_CHANGED : MARsExemptionManager.getInstance().isFgExemptedFromMars(str) ? 350 : -1;
    }
    ...
    public int getPotentialUserAllowedExemptionReason(int i, String str) {
        ...
        if (isRoleHeldByUid("android.app.role.DIALER", i)) {
            return FrameworkStatsLog.f108xb8c45718;
        }
        if (isRoleHeldByUid("android.app.role.EMERGENCY", i)) {
            return FrameworkStatsLog.f109x9a09c896;
        }
