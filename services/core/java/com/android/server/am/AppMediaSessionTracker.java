...
public final class AppMediaSessionTracker extends BaseAppStateDurationsTracker {
  ...
  public BaseAppStateDurationsTracker.SimplePackageDurations createAppStateEvents(
      int i, String str) {
    return new BaseAppStateDurationsTracker.SimplePackageDurations(
        i, str, (BaseAppStateEvents.MaxTrackingDurationConfig) this.mInjector.getPolicy());
  }

  @Override // com.android.server.am.BaseAppStateEvents.Factory
  public BaseAppStateDurationsTracker.SimplePackageDurations createAppStateEvents(
      BaseAppStateDurationsTracker.SimplePackageDurations simplePackageDurations) {
    ...
    if (z) {
      this.mInjector
          ...
          .addOnActiveSessionsChangedListener(
              null, UserHandle.ALL, this.mHandlerExecutor, this.mSessionsChangedListener);
    } else {
      this.mInjector
          .getMediaSessionManager()
          .removeOnActiveSessionsChangedListener(this.mSessionsChangedListener);
    }
  }
  ...
  public final void handleMediaSessionChanged(List list) {
    ...
    if (list != null) {
      synchronized (this.mLock) {
        ...
        while (it.hasNext()) {
          ...
          BaseAppStateDurationsTracker.SimplePackageDurations simplePackageDurations =
              (BaseAppStateDurationsTracker.SimplePackageDurations)
                  this.mPkgEvents.get(uid, packageName);
        ...
        for (int size = map.size() - 1; size >= 0; size--) {
          ...
          while (size2 >= 0) {
            BaseAppStateDurationsTracker.SimplePackageDurations simplePackageDurations2 =
                (BaseAppStateDurationsTracker.SimplePackageDurations) arrayMap.valueAt(size2);
            if (simplePackageDurations2.isActive()
                && this.mTmpMediaControllers.get(
                        simplePackageDurations2.mPackageName, simplePackageDurations2.mUid)
                    == null) {
    ...
    synchronized (this.mLock) {
      ...
      for (int size3 = map2.size() - 1; size3 >= 0; size3--) {
        ...
        while (size4 >= 0) {
          BaseAppStateDurationsTracker.SimplePackageDurations simplePackageDurations3 =
              (BaseAppStateDurationsTracker.SimplePackageDurations) arrayMap2.valueAt(size4);
    ...
    trim(
        Math.max(
            ...
            SystemClock.elapsedRealtime()
                - ((AppMediaSessionPolicy) this.mInjector.getPolicy()).getMaxTrackingDuration()));
  }
  ...
  public final class AppMediaSessionPolicy
    ...
    public AppMediaSessionPolicy(
        BaseAppStateTracker.Injector injector, AppMediaSessionTracker appMediaSessionTracker) {
      super(
          injector,
    ...
    public void onMaxTrackingDurationChanged(long j) {
      ...
      final AppMediaSessionTracker appMediaSessionTracker =
          (AppMediaSessionTracker) baseAppStateTracker;
