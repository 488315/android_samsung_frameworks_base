...
public final /* synthetic */ class AppExitInfoTracker$$ExternalSyntheticLambda8
        ...
        switch (i) {
            case 0:
                ...
                for (int i2 = 0; i2 < size; i2++) {
                    AppExitInfoTracker.AppExitInfoContainer appExitInfoContainer =
                            (AppExitInfoTracker.AppExitInfoContainer) sparseArray.valueAt(i2);
                    ...
                    for (int i3 = 0; i3 < appExitInfoContainer.mInfos.size(); i3++) {
                        ((ApplicationExitInfo) appExitInfoContainer.mInfos.valueAt(i3))
                                .writeToProto(protoOutputStream, 2246267895810L);
                    }
                    for (int i4 = 0; i4 < appExitInfoContainer.mRecoverableCrashes.size(); i4++) {
                        ((ApplicationExitInfo) appExitInfoContainer.mRecoverableCrashes.valueAt(i4))
            ...
            default:
                ...
                for (int size2 = sparseArray2.size() - 1; size2 >= 0; size2--) {
                    AppExitInfoTracker.AppExitInfoContainer appExitInfoContainer2 =
                            (AppExitInfoTracker.AppExitInfoContainer) sparseArray2.valueAt(size2);
                    for (int size3 = appExitInfoContainer2.mInfos.size() - 1; size3 >= 0; size3--) {
                        appExitInfoContainer2.mInfos.keyAt(size3);
                        File traceFile =
                                ((ApplicationExitInfo) appExitInfoContainer2.mInfos.valueAt(size3))
                                        .getTraceFile();
                        if (traceFile != null) {
                            arraySet.remove(traceFile.getName());
                        }
                    }
                    for (int size4 = appExitInfoContainer2.mRecoverableCrashes.size() - 1;
                            size4 >= 0;
                            size4--) {
                        appExitInfoContainer2.mRecoverableCrashes.keyAt(size4);
                        File traceFile2 =
                                ((ApplicationExitInfo)
                                                appExitInfoContainer2.mRecoverableCrashes.valueAt(
                                                        size4))
                                        .getTraceFile();
                        if (traceFile2 != null) {
