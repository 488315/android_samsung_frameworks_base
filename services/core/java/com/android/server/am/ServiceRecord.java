...
public final class ServiceRecord extends Binder implements ComponentName.WithComponentName {
  ...
  */
  public void dumpDebug(ProtoOutputStream protoOutputStream, long j) {
    ...
    if (filterComparison != null) {
      ...
      filterComparison
          .getIntent()
          .dumpDebug(protoOutputStream, 1146756268036L, false, true, false, false);
    } else {
      j2 = 1133871366146L;
    }
    ...
    ProtoUtils.toDuration(
        protoOutputStream,
        1146756268034L,
        this.nextRestartTime - this.mRestartSchedulingTime,
        uptimeMillis);
    ...
    if (this.deliveredStarts.size() > 0) {
      ...
      for (int i = 0; i < size; i++) {
        ((StartItem) this.deliveredStarts.get(i))
            .dumpDebug(protoOutputStream, 2246267895831L, uptimeMillis);
      }
    }
    if (this.pendingStarts.size() > 0) {
      int size2 = this.pendingStarts.size();
      for (int i2 = 0; i2 < size2; i2++) {
        ((StartItem) this.pendingStarts.get(i2))
            .dumpDebug(protoOutputStream, 2246267895832L, uptimeMillis);
      }
    }
  ...
  */
  public void dump(PrintWriter printWriter, String str) {
    ...
    printWriter.println(
        PowerExemptionManager.reasonCodeToString(this.mAllowWhileInUsePermissionInFgsReason));
    printWriter.print(str);
    ...
    if (this.isForeground || this.foregroundId != 0) {
      ...
      if (isShortFgs() && this.mShortFgsInfo != null) {
        ...
        TimeUtils.formatDuration(
            this.mShortFgsInfo.getProcStateDemoteTime(), uptimeMillis, printWriter);
    ...
    TimeUtils.formatDuration(
        this.nextRestartTime - this.mRestartSchedulingTime, uptimeMillis, printWriter);
  ...
  public ServiceState getTracker() {
    ...
    if ((applicationInfo.flags & 8) == 0) {
      ServiceState serviceState2 =
          this.ams.mProcessStats.getServiceState(
  ...
  public void updateProcessStateOnRequest() {
    ...
    this.mProcessStateOnRequest =
        (processRecord == null || processRecord.getThread() == null || this.app.isKilled())
            ? 20
            : this.app.mState.getCurProcState();
  }
  ...
  public void addConnection(IBinder iBinder, ConnectionRecord connectionRecord) {
    ...
    ProcessRecord processRecord = this.app;
    if (processRecord != null) {
      processRecord.mServices.addBoundClientUid(
          connectionRecord.clientUid,
          connectionRecord.clientPackageName,
          connectionRecord.getFlags());
  ...
  public void allowBgActivityStartsOnServiceStart(
    ...
    setAllowedBgActivityStartsByStart(
        backgroundStartPrivileges.merge(this.mBackgroundStartPrivilegesByStartMerged));
    ProcessRecord processRecord = this.app;
    if (processRecord != null) {
    ...
    if (this.mCleanUpAllowBgActivityStartsByStartCallback == null) {
      this.mCleanUpAllowBgActivityStartsByStartCallback =
          new Runnable() { // from class:
                           // com.android.server.am.ServiceRecord$$ExternalSyntheticLambda0
    ...
    activityManagerService.mHandler.postDelayed(
        this.mCleanUpAllowBgActivityStartsByStartCallback,
        activityManagerService.mConstants.SERVICE_BG_ACTIVITY_START_TIMEOUT);
  }
    ...
    synchronized (activityManagerService) {
      try {
        ...
        if (!this.mBackgroundStartPrivilegesByStart.isEmpty()) {
          BackgroundStartPrivileges merge =
              BackgroundStartPrivileges.merge(this.mBackgroundStartPrivilegesByStart);
          ...
          if (merge.allowsAny()) {
            ...
            Slog.wtf(
                "ActivityManager",
                "Service callback to revoke bg activity starts by service start triggered but"
  ...
  public final void setAllowedBgActivityStartsByStart(
      BackgroundStartPrivileges backgroundStartPrivileges) {
  ...
  public final void updateParentProcessBgActivityStartsToken() {
    ...
    BackgroundStartPrivileges backgroundStartPrivilegesWithExclusiveToken =
        getBackgroundStartPrivilegesWithExclusiveToken();
    if (backgroundStartPrivilegesWithExclusiveToken.allowsAny()) {
      this.app.addOrUpdateBackgroundStartPrivileges(
          this, backgroundStartPrivilegesWithExclusiveToken);
    } else {
      this.app.removeBackgroundStartPrivileges(this);
    }
  ...
  public void updateKeepWarmLocked() {
    ...
    if (this.ams.mConstants.KEEP_WARMING_SERVICES.contains(this.name)) {
      ...
      int i = this.userId;
      if (currentUserId == i
          || this.ams.mUserController.isCurrentProfile(i)
          || this.ams.isSingleton(
              this.processName,
              this.appInfo,
              this.instanceName.getClassName(),
  ...
  public AppBindRecord retrieveAppBindingLocked(
      Intent intent, ProcessRecord processRecord, ProcessRecord processRecord2) {
    ...
    AppBindRecord appBindRecord2 =
        new AppBindRecord(this, intentBindRecord, processRecord, processRecord2);
  ...
  public final void updateFgsHasNotificationPermission() {
    ...
    this.ams.mHandler.post(
        new Runnable() { // from class: com.android.server.am.ServiceRecord.1
          @Override // java.lang.Runnable
          public void run() {
            ...
            ServiceRecord.this.mFgsHasNotificationPermission =
                notificationManagerInternal.areNotificationsEnabledForPackage(str, i);
          }
        });
  }
            ...
            try {
              if (notification2.getSmallIcon() == null) {
                ...
                CharSequence loadLabel =
                    serviceRecord.appInfo.loadLabel(serviceRecord.ams.mContext.getPackageManager());
                if (loadLabel == null) {
                ...
                try {
                  ...
                  Notification.Builder builder =
                      new Notification.Builder(
                          serviceRecord2.ams.mContext.createPackageContextAsUser(
                              serviceRecord2.appInfo.packageName,
                              0,
                              new UserHandle(ServiceRecord.this.userId)),
                          notification2.getChannelId());
                  builder.setSmallIcon(ServiceRecord.this.appInfo.icon);
                  builder.setFlag(64, true);
                  Intent intent = new Intent("android.settings.APPLICATION_DETAILS_SETTINGS");
                  intent.setData(
                      Uri.fromParts("package", ServiceRecord.this.appInfo.packageName, null));
                  ...
                  builder.setContentTitle(
                      ServiceRecord.this.ams.mContext.getString(
                          R.string.close_button_text, loadLabel));
                  builder.setContentText(
                      ServiceRecord.this.ams.mContext.getString(
                          R.string.clone_profile_label_badge, loadLabel));
                  builder.setContentIntent(activityAsUser);
                  notification2 = builder.build();
                } catch (PackageManager.NameNotFoundException unused) {
                }
              }
              if (notificationManagerInternal.getNotificationChannel(
                      str, i, notification2.getChannelId())
                  == null) {
                try {
                  ...
                  i3 =
                      packageManager.getApplicationInfoAsUser(
                              serviceRecord4.appInfo.packageName, 0, serviceRecord4.userId)
                          .targetSdkVersion;
              ...
              if (notification2.getSmallIcon() == null) {
                throw new RuntimeException(
                    "invalid service notification: " + ServiceRecord.this.foregroundNoti);
              }
              String str2 = str;
              notificationManagerInternal.enqueueNotification(
                  str2, str2, i, pid, null, i2, notification2, ServiceRecord.this.userId, z);
              ServiceRecord serviceRecord5 = ServiceRecord.this;
              serviceRecord5.foregroundNoti = notification2;
              serviceRecord5.signalForegroundServiceNotification(
                  serviceRecord5.packageName, serviceRecord5.appInfo.uid, i2, false);
    ...
    this.ams.mHandler.post(
        new Runnable() { // from class: com.android.server.am.ServiceRecord.3
          ...
          public void run() {
            ...
            try {
              String str2 = str;
              notificationManagerInternal.cancelNotification(
                  str2, str2, i2, pid, null, i, ServiceRecord.this.userId);
            } catch (RuntimeException e) {
              Slog.w("ActivityManager", "Error canceling notification for service", e);
            }
            ServiceRecord serviceRecord = ServiceRecord.this;
            serviceRecord.signalForegroundServiceNotification(
                serviceRecord.packageName, serviceRecord.appInfo.uid, i, true);
          }
        });
  }
      ...
      try {
        for (int size = this.ams.mForegroundServiceStateListeners.size() - 1; size >= 0; size--) {
          ((ActivityManagerInternal.ForegroundServiceStateListener)
                  this.ams.mForegroundServiceStateListeners.get(size))
              .onForegroundServiceNotificationUpdated(str, this.appInfo.uid, i2, z);
        }
      } catch (Throwable th) {
        ActivityManagerService.resetPriorityAfterLockedSection();
        throw th;
  ...
  public void stripForegroundServiceFlagFromNotification() {
    ...
    final String str = this.packageName;
    this.ams.mHandler.post(
        new Runnable() { // from class: com.android.server.am.ServiceRecord.4
          @Override // java.lang.Runnable
          public void run() {
            NotificationManagerInternal notificationManagerInternal =
                (NotificationManagerInternal)
  ...
  public boolean shouldTriggerShortFgsTimeout(long j) {
    ...
    return shouldTriggerShortFgsTimedEvent(
        shortFgsInfo == null ? 0L : shortFgsInfo.getTimeoutTime(), j);
  }

  public boolean shouldDemoteShortFgsProcState(long j) {
    ShortFgsInfo shortFgsInfo = this.mShortFgsInfo;
    return shouldTriggerShortFgsTimedEvent(
        shortFgsInfo == null ? 0L : shortFgsInfo.getProcStateDemoteTime(), j);
  }

  public boolean shouldTriggerShortFgsAnr(long j) {
    ShortFgsInfo shortFgsInfo = this.mShortFgsInfo;
    return shouldTriggerShortFgsTimedEvent(
        shortFgsInfo == null ? 0L : shortFgsInfo.getAnrTime(), j);
  }
