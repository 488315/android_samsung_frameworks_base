...
public final class PhantomProcessRecord {
  ...
  public Runnable mProcKillTimer =
          ...
          synchronized (PhantomProcessRecord.this.mLock) {
            Slog.w(
                ...
                "Process "
                    ...
                    + PhantomProcessRecord.this.mService.mConstants.mProcessKillTimeoutMs
                    + "ms");
  ...
  public void killLocked(String str, boolean z) {
    ...
    if (z || this.mUid == this.mService.mCurOomAdjUid) {
      this.mService.reportUidInfoMessageLocked(
          "ActivityManager", "Killing " + toString() + ": " + str, this.mUid);
    }
    if (this.mPid > 0) {
      EventLog.writeEvent(
          30023,
          Integer.valueOf(UserHandle.getUserId(this.mUid)),
          Integer.valueOf(this.mPid),
          this.mProcessName,
          Integer.valueOf(this.mAdj),
          str);
      if (!Process.supportsPidFd()) {
        onProcDied(false);
      } else {
