...
public final class ProcessServiceRecord {
  ...
  public boolean areAllShortForegroundServicesProcstateTimedOut(long j) {
    ...
    for (int size = this.mServices.size() - 1; size >= 0; size--) {
      ...
      if (serviceRecord.isShortFgs()
          && serviceRecord.hasShortFgsInfo()
          && serviceRecord.getShortFgsInfo().getProcStateDemoteTime() >= j) {
        return false;
      }
    }
    return true;
  ...
  public boolean hasTopStartedAlmostPerceptibleServices() {
    return this.mHasTopStartedAlmostPerceptibleServices
        || (this.mLastTopStartedAlmostPerceptibleBindRequestUptimeMs > 0
            && SystemClock.uptimeMillis() - this.mLastTopStartedAlmostPerceptibleBindRequestUptimeMs
                < this.mService.mConstants.mServiceBindAlmostPerceptibleTimeoutMs);
  }
  ...
  public int modifyRawOomAdj(int i) {
    ...
    if (i >= 100) {
      i2 = 200;
      if (i >= 200) {
        i2 = FrameworkStatsLog.f446x58c6c32;
        if (i >= 250) {
          i2 = 900;
          if (i >= 900) {
            return i < 999 ? i + 1 : i;
  ...
  public boolean incServiceCrashCountLocked(long j) {
    ...
    for (int numberOfRunningServices = numberOfRunningServices() - 1;
      ...
      if (runningServiceAt.crashCount < this.mService.mConstants.BOUND_SERVICE_MAX_CRASH_RETRY
          && (runningServiceAt.isForeground || z2)) {
        z = true;
      }
    }
    return z;
