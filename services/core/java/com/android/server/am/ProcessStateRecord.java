...
public final class ProcessStateRecord {
  ...
  public void setSetProcState(int i) {
    if (ActivityManager.isProcStateCached(this.mSetProcState)
        && !ActivityManager.isProcStateCached(i)) {
  ...
  public boolean getCachedIsHeavyWeight() {
    if (this.mCachedIsHeavyWeight == -1) {
      this.mCachedIsHeavyWeight =
          this.mApp.getWindowProcessController().isHeavyWeightProcess() ? 1 : 0;
    }
  ...
  public boolean getCachedHasVisibleActivities() {
    if (this.mCachedHasVisibleActivities == -1) {
      this.mCachedHasVisibleActivities =
          this.mApp.getWindowProcessController().hasVisibleActivities() ? 1 : 0;
    }
  ...
  public boolean getCachedIsReceivingBroadcast(int[] iArr) {
    if (this.mCachedIsReceivingBroadcast == -1) {
      boolean isReceivingBroadcastLocked =
          this.mService.isReceivingBroadcastLocked(this.mApp, iArr);
  ...
  public void computeOomAdjFromActivitiesIfNecessary(
      OomAdjuster.ComputeOomAdjWindowCallback computeOomAdjWindowCallback,
      int i,
      boolean z,
      boolean z2,
      int i2,
    ...
    int min =
        Math.min(
            ...
            this.mApp
                ...
                .computeOomAdjFromActivities(computeOomAdjWindowCallback));
  ...
  public void dump(PrintWriter printWriter, String str, long j) {
    if (this.mReportedInteraction || this.mFgInteractionTime != 0) {
      ...
      if (this.mInteractionEventTime != 0) {
        ...
        TimeUtils.formatDuration(
            this.mInteractionEventTime, SystemClock.elapsedRealtime(), printWriter);
      }
      if (this.mFgInteractionTime != 0) {
        printWriter.print(" fgInteractionTime=");
        TimeUtils.formatDuration(
            this.mFgInteractionTime, SystemClock.elapsedRealtime(), printWriter);
      }
      printWriter.println();
    }
    printWriter.print(str);
    ...
    if (j2 > 0 && j2 < Long.MAX_VALUE) {
      ...
      TimeUtils.dumpTimeWithDelta(
          printWriter,
          (currentTimeMillis - elapsedRealtime) + this.mLastInvisibleTime,
          currentTimeMillis);
      printWriter.println();
    }
