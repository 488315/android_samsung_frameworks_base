...
public final class ConnectionRecord {
  public static final int[] BIND_ORIG_ENUMS = {
    1,
    2,
    4,
    8388608,
  ...
  public static final int[] BIND_PROTO_ENUMS = {
  ...
  public void dump(PrintWriter printWriter, String str) {
    ...
    printWriter.println(
        str + "conn=" + this.conn.asBinder() + " flags=0x" + Long.toHexString(this.flags));
  }
  ...
  public void startAssociationIfNeeded() {
    if (this.association == null) {
      ...
      if (serviceRecord.app != null) {
        if (serviceRecord.appInfo.uid == this.clientUid
            && serviceRecord.processName.equals(this.clientProcessName)) {
          return;
        }
        ProcessStats.ProcessStateHolder processStateHolder =
            this.binding
                ...
                .app
                .getPkgList()
                .get(this.binding.service.instanceName.getPackageName());
        if (processStateHolder == null) {
          Slog.wtf(
              "ActivityManager",
              "No package in referenced service "
                  + this.binding.service.shortInstanceName
                  + ": proc="
                  + this.binding.service.app);
          return;
        }
        if (processStateHolder.pkg == null) {
          Slog.wtf(
              "ActivityManager",
              "Inactive holder in referenced service "
                  + this.binding.service.shortInstanceName
                  + ": proc="
                  + this.binding.service.app);
          return;
          ...
          this.association =
              processStateHolder
                  ...
                  .getAssociationStateLocked(
                      processStateHolder.state, this.binding.service.instanceName.getClassName())
                  .startSource(this.clientUid, this.clientProcessName, this.clientPackageName);
        }
      }
    }
  }
  ...
  public void dumpDebug(ProtoOutputStream protoOutputStream, long j) {
    ...
    ProtoUtils.writeBitWiseFlagsToProtoEnum(
        protoOutputStream, 2259152797699L, this.flags, BIND_ORIG_ENUMS, BIND_PROTO_ENUMS);
    if (this.serviceDead) {
