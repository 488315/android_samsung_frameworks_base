...
public final class CachedAppOptimizer {
  ...
  public abstract class AggregatedCompactionStats {
    ...
    public void dump(PrintWriter printWriter) {
      ...
      printWriter.println(
          "      Some: (" + this.mSomeCompactPerformed + "/" + this.mSomeCompactRequested + ")");
      printWriter.println(
          "      Full: (" + this.mFullCompactPerformed + "/" + this.mFullCompactRequested + ")");
      ...
      if (throttledSome > 0 || throttledFull > 0) {
        ...
        printWriter.println(
            "       NoPid: "
                ...
                + " Unaccounted: "
                + (((((j3 - this.mProcCompactionsNoPidThrottled)
                                - this.mProcCompactionsOomAdjThrottled)
                            - this.mProcCompactionsTimeThrottled)
                        - this.mProcCompactionsRSSThrottled)
                    - this.mProcCompactionsMiscThrottled));
      ...
      if (this.mFullCompactPerformed > 0) {
        ...
        printWriter.println(
            "    Avg Compaction Efficiency (Anon Freed/Anon RSS): "
                + (((double) this.mTotalAnonMemFreedKBs) / ((double) this.mSumOrigAnonRss)));
        ...
        printWriter.println(
            "    Avg Compression Ratio (1 - ZRAM Consumed/DeltaAnonRSS): "
                + (1.0d
                    - (((double) this.mTotalZramConsumedKBs)
                        / ((double) this.mTotalDeltaAnonRssKBs))));
        long j4 = this.mFullCompactPerformed;
        printWriter.println(
            "    Avg Anon Mem Freed/Compaction (KB) : "
                + (j4 > 0 ? this.mTotalAnonMemFreedKBs / j4 : 0L));
        printWriter.println(
  ...
  public CachedAppOptimizer(
    ...
    this.mOnFlagsChangedListener =
        new DeviceConfig
            .OnPropertiesChangedListener() { // from class:
                                             // com.android.server.am.CachedAppOptimizer.1
            ...
            synchronized (CachedAppOptimizer.this.mPhenotypeFlagLock) {
              for (String str : properties.getKeyset()) {
                if (CachedAppOptimizer.KEY_USE_COMPACTION.equals(str)) {
                  ...
                  if (!CachedAppOptimizer.KEY_COMPACT_THROTTLE_1.equals(str)
                      && !CachedAppOptimizer.KEY_COMPACT_THROTTLE_2.equals(str)
                      && !CachedAppOptimizer.KEY_COMPACT_THROTTLE_3.equals(str)
                      && !CachedAppOptimizer.KEY_COMPACT_THROTTLE_4.equals(str)
                      && !CachedAppOptimizer.KEY_COMPACT_THROTTLE_5.equals(str)
                      && !CachedAppOptimizer.KEY_COMPACT_THROTTLE_6.equals(str)) {
                    ...
                    } else if (CachedAppOptimizer.KEY_COMPACT_FULL_DELTA_RSS_THROTTLE_KB.equals(
                        str)) {
    ...
    this.mOnNativeBootFlagsChangedListener =
        new DeviceConfig
            .OnPropertiesChangedListener() { // from class:
                                             // com.android.server.am.CachedAppOptimizer.2
          public void onPropertiesChanged(DeviceConfig.Properties properties) {
            synchronized (CachedAppOptimizer.this.mPhenotypeFlagLock) {
              for (String str : properties.getKeyset()) {
                ...
                } else if (CachedAppOptimizer.KEY_FREEZER_BINDER_ENABLED.equals(str)
                    || CachedAppOptimizer.KEY_FREEZER_BINDER_DIVISOR.equals(str)
                    || CachedAppOptimizer.KEY_FREEZER_BINDER_THRESHOLD.equals(str)
                    || CachedAppOptimizer.KEY_FREEZER_BINDER_OFFSET.equals(str)) {
    ...
    this.mCompactionStatsHistory =
        new LinkedList() { // from class: com.android.server.am.CachedAppOptimizer.4
          @Override // java.util.LinkedList, java.util.AbstractList, java.util.AbstractCollection,
                    // java.util.Collection, java.util.List, java.util.Deque, java.util.Queue
          public boolean add(SingleCompactionStats singleCompactionStats) {
            if (size() >= 20) {
              remove();
            }
            return super.add((C05654) singleCompactionStats);
          }
        };
  ...
  public void init() {
    ...
    this.mAm
        .mContext
        .getContentResolver()
        .registerContentObserver(CACHED_APP_FREEZER_ENABLED_URI, false, this.mSettingsObserver);
  ...
  public void dump(PrintWriter printWriter) {
    ...
    synchronized (this.mPhenotypeFlagLock) {
      ...
      long j = 0;
      long j2 = 0;
      for (AggregatedProcessCompactionStats aggregatedProcessCompactionStats :
          this.mPerProcessCompactStats.values()) {
      ...
      for (AggregatedSourceCompactionStats aggregatedSourceCompactionStats :
          this.mPerSourceCompactStats.values()) {
      ...
      printWriter.println();
      printWriter.println(
          "Total Compactions Performed by profile: " + j + " some, " + j2 + " full");
      ...
      for (CancelCompactReason cancelCompactReason : this.mTotalCompactionsCancelled.keySet()) {
        printWriter.println(
            "    "
                + cancelCompactReason
                + ": "
                + this.mTotalCompactionsCancelled.get(cancelCompactReason));
      }
      printWriter.println();
      ...
      printWriter.println(
          "    Avg Mem Freed per Compact (KB): "
              + (this.mSystemCompactionsPerformed > 0 ? this.mSystemTotalMemFreed / r1 : 0.0d));
      printWriter.println();
      printWriter.println(
          "  Tracking last compaction stats for "
              + this.mLastCompactionStats.size()
              + " processes.");
          ...
          if (!this.mPendingCompactionProcesses.isEmpty()) {
            ...
            for (int i2 = 0; i2 < size2; i2++) {
              ProcessRecord processRecord2 =
                  (ProcessRecord) this.mPendingCompactionProcesses.get(i2);
              printWriter.println(
                  "    pid: "
                      ...
                      + ". hasPendingCompact: "
                      + processRecord2.mOptRecord.hasPendingCompact());
            }
          }
        } catch (Throwable th) {
  ...
  public boolean compactApp(
    ...
    AggregatedSourceCompactionStats perSourceAggregatedCompactStat =
        getPerSourceAggregatedCompactStat(compactSource);
    AggregatedProcessCompactionStats perProcessAggregatedCompactStat =
        getPerProcessAggregatedCompactStat(processRecord.processName);
    int i = AbstractC05665.f1630x5cb38e25[compactProfile.ordinal()];
    if (i == 1) {
    ...
    handler.sendMessage(
        handler.obtainMessage(
            1, processRecord.mState.getCurAdj(), processRecord.mState.getSetProcState()));
    return true;
  }

  /* renamed from: com.android.server.am.CachedAppOptimizer$5 */
  public abstract /* synthetic */ class AbstractC05665 {

    /* renamed from: $SwitchMap$com$android$server$am$CachedAppOptimizer$CompactProfile */
    public static final /* synthetic */ int[] f1630x5cb38e25;
    ...
    static {
      ...
      try {
        f1630x5cb38e25[CompactProfile.FULL.ordinal()] = 2;
  ...
  public final AggregatedProcessCompactionStats getPerProcessAggregatedCompactStat(String str) {
    ...
    AggregatedProcessCompactionStats aggregatedProcessCompactionStats2 =
        new AggregatedProcessCompactionStats(str);
    this.mPerProcessCompactStats.put(str, aggregatedProcessCompactionStats2);
    return aggregatedProcessCompactionStats2;
  }

  public final AggregatedSourceCompactionStats getPerSourceAggregatedCompactStat(
      CompactSource compactSource) {
    ...
    AggregatedSourceCompactionStats aggregatedSourceCompactionStats2 =
        new AggregatedSourceCompactionStats(compactSource);
    this.mPerSourceCompactStats.put(
        (EnumMap) compactSource, (CompactSource) aggregatedSourceCompactionStats2);
  ...
  */
  public void compactAllDelayedCompactionProcesses() {
    KernelMemoryProxy$ReclaimerLog.write("B|delayedCompaction", false);
    while (true) {
      ActivityManagerGlobalLock activityManagerGlobalLock = this.mProcLock;
      ActivityManagerService.boostPriorityForProcLockedSection();
      synchronized (activityManagerGlobalLock) {
        try {
          if (this.mDelayedCompactionProcesses.size() != 0) {
            ProcessRecord processRecord =
                (ProcessRecord) this.mDelayedCompactionProcesses.remove(0);
            if (processRecord != null) {
              ...
              if (pid != 0) {
                if (!processRecord.mOptRecord.hasPendingCompact()) {
                  KernelMemoryProxy$ReclaimerLog.write(
                      "delayedCompaction: trigger compaction for " + str + "(" + pid + ")");
                  processRecord.mOptRecord.setHasPendingCompact(true);
                  this.mPendingCompactionProcesses.add(processRecord);
                  Handler handler = this.mCompactionHandler;
                  handler.sendMessage(
                      handler.obtainMessage(
                          1,
                          processRecord.mState.getCurAdj(),
                          processRecord.mState.getSetProcState()));
                }
              }
            }
          } else {
            ActivityManagerService.resetPriorityAfterProcLockedSection();
            KernelMemoryProxy$ReclaimerLog.write("E|delayedCompaction", false);
            return;
          }
        } catch (Throwable th) {
          ActivityManagerService.resetPriorityAfterProcLockedSection();
  ...
  */
  ...
  public final void updateUseFreezer() {
    String string =
        Settings.Global.getString(this.mAm.mContext.getContentResolver(), "cached_apps_freezer");
  ...
  public final void updateCompactStatsdSampleRate() {
    this.mCompactStatsdSampleRate =
        DeviceConfig.getFloat(
            "activity_manager", KEY_COMPACT_STATSD_SAMPLE_RATE, DEFAULT_STATSD_SAMPLE_RATE);
    this.mCompactStatsdSampleRate =
        Math.min(
            1.0f,
            Math.max(DisplayPowerController2.RATE_FROM_DOZE_TO_ON, this.mCompactStatsdSampleRate));
  }

  public final void updateFreezerStatsdSampleRate() {
    this.mFreezerStatsdSampleRate =
        DeviceConfig.getFloat(
            "activity_manager", KEY_FREEZER_STATSD_SAMPLE_RATE, DEFAULT_STATSD_SAMPLE_RATE);
    this.mFreezerStatsdSampleRate =
        Math.min(
            1.0f,
            Math.max(DisplayPowerController2.RATE_FROM_DOZE_TO_ON, this.mFreezerStatsdSampleRate));
  }

  public final void updateFullRssThrottle() {
    this.mFullAnonRssThrottleKb =
  ...
  public final void updateMinOomAdjThrottle() {
    this.mCompactThrottleMinOomAdj =
        DeviceConfig.getLong(
            "activity_manager",
  ...
  public final void updateMaxOomAdjThrottle() {
    this.mCompactThrottleMaxOomAdj =
        DeviceConfig.getLong(
            "activity_manager",
  ...
  public final void updateFreezerDebounceTimeout() {
    this.mFreezerDebounceTimeout =
        DeviceConfig.getLong("activity_manager_native_boot", KEY_FREEZER_DEBOUNCE_TIMEOUT, 10000L);
  ...
  public final void updateFreezerExemptInstPkg() {
    this.mFreezerExemptInstPkg =
        DeviceConfig.getBoolean("activity_manager_native_boot", KEY_FREEZER_EXEMPT_INST_PKG, true);
    Slog.d("ActivityManager", "Freezer exemption set to " + this.mFreezerExemptInstPkg);
  }

  public final void updateFreezerBinderState() {
    this.mFreezerBinderEnabled =
        DeviceConfig.getBoolean("activity_manager_native_boot", KEY_FREEZER_BINDER_ENABLED, true);
    this.mFreezerBinderDivisor =
        DeviceConfig.getLong(
            "activity_manager_native_boot",
            KEY_FREEZER_BINDER_DIVISOR,
            DEFAULT_FREEZER_BINDER_DIVISOR);
    this.mFreezerBinderOffset =
        DeviceConfig.getInt("activity_manager_native_boot", KEY_FREEZER_BINDER_OFFSET, 500);
    this.mFreezerBinderThreshold =
        DeviceConfig.getLong("activity_manager_native_boot", KEY_FREEZER_BINDER_THRESHOLD, 1000L);
    Slog.d(
        "ActivityManager",
        "Freezer binder state set to enabled="
            + this.mFreezerBinderEnabled
            + ", divisor="
            + this.mFreezerBinderDivisor
            + ", offset="
  ...
  public final long updateEarliestFreezableTime(ProcessRecord processRecord, long j) {
    ...
    ProcessCachedOptimizerRecord processCachedOptimizerRecord = processRecord.mOptRecord;
    processCachedOptimizerRecord.setEarliestFreezableTime(
        Math.max(processCachedOptimizerRecord.getEarliestFreezableTime(), j + uptimeMillis));
  ...
  public void freezeAppAsyncLSP(ProcessRecord processRecord) {
    freezeAppAsyncLSP(
        processRecord, updateEarliestFreezableTime(processRecord, this.mFreezerDebounceTimeout));
  }
  ...
  public void freezeAppAsyncInternalLSP(ProcessRecord processRecord, long j, boolean z) {
    ...
    if (!processCachedOptimizerRecord.isFreezeSticky() || z) {
      if (!processCachedOptimizerRecord.isDoingTrim()
          && this.mAm.mConstants.USE_MODERN_TRIM
          && processRecord.mState.getSetAdj() >= 850
      ...
      if (FreecessController.getInstance()
          .freezeTargetProcess(
              processRecord.info != null ? processRecord.info.packageName : null,
              processRecord.uid)) {
  ...
  */
    ...
    if ((getBinderFreezeInfo(pid) & 1) != 0) {
      ...
      if (z3) {
        ...
        try {
          ...
          Slog.d(
              "ActivityManager",
              "sync unfroze " + pid + " " + processRecord.processName + " for " + i);
          Handler handler = this.mFreezeHandler;
          handler.sendMessage(
              handler.obtainMessage(
                  4,
                  ...
                  (int)
                      Math.min(
                          processCachedOptimizerRecord.getFreezeUnfreezeTime() - freezeUnfreezeTime,
                          2147483647L),
                  new Pair(processRecord.processName, Integer.valueOf(i))));
          return;
        } catch (RuntimeException unused2) {
          Slog.e(
              "ActivityManager",
      ...
      while (!this.mPendingCompactionProcesses.isEmpty()) {
        try {
          cancelCompactionForProcess(
              (ProcessRecord) this.mPendingCompactionProcesses.get(0), cancelCompactReason);
        } catch (Throwable th) {
          ActivityManagerService.resetPriorityAfterProcLockedSection();
          throw th;
        }
  ...
  public void cancelCompactionForProcess(
    ...
    if (z) {
      if (this.mTotalCompactionsCancelled.containsKey(cancelCompactReason)) {
        this.mTotalCompactionsCancelled.put(
            ...
            (CancelCompactReason)
                Integer.valueOf(
                    ((Integer) this.mTotalCompactionsCancelled.get(cancelCompactReason)).intValue()
                        + 1));
      } else {
    ...
    public void dump(PrintWriter printWriter) {
      printWriter.println(
          "    ("
              + this.mProcessName
              + ","
              + this.mSourceType.name()
              + ","
              + this.mDeltaAnonRssKBs
              + ","
              + this.mZramConsumedKBs
              + ","
              + this.mAnonMemFreedKBs
              + ","
              + getCompactEfficiency()
              + ","
              + getCompactCost()
              + ","
              + this.mProcState
              ...
              + this.mOomAdj
              + ","
              + OomAdjuster.oomAdjReasonToString(this.mOomAdjReason)
              + ")");
    }
  ...
  public final class MemCompactionHandler extends Handler {
    ...
    public final boolean shouldTimeThrottleCompaction(
        ProcessRecord processRecord,
        long j,
        CompactProfile compactProfile,
        CompactSource compactSource) {
      ...
      if (compactProfile == compactProfile2) {
        return (lastCompactProfile == compactProfile2
                && j - lastCompactTime < CachedAppOptimizer.this.mCompactThrottleSomeSome)
            || (lastCompactProfile == CompactProfile.FULL
                && j - lastCompactTime < CachedAppOptimizer.this.mCompactThrottleSomeFull);
      }
      CompactProfile compactProfile3 = CompactProfile.FULL;
      if (compactProfile == compactProfile3) {
        return (lastCompactProfile == compactProfile2
                && j - lastCompactTime < CachedAppOptimizer.this.mCompactThrottleFullSome)
            || (lastCompactProfile == compactProfile3
                && j - lastCompactTime < CachedAppOptimizer.this.mCompactThrottleFullFull);
      }
      return false;
    }
    ...
    public final boolean shouldRssThrottleCompaction(
        CompactProfile compactProfile, int i, String str, long[] jArr) {
      long j = jArr[2];
      SingleCompactionStats singleCompactionStats =
          (SingleCompactionStats)
              CachedAppOptimizer.this.mLastCompactionStats.get(Integer.valueOf(i));
      if (jArr[0] == 0 && jArr[1] == 0 && jArr[2] == 0 && jArr[3] == 0) {
        return true;
      }
      if (compactProfile == CompactProfile.FULL) {
        if (CachedAppOptimizer.this.mFullAnonRssThrottleKb > 0
            && j < CachedAppOptimizer.this.mFullAnonRssThrottleKb) {
          return true;
        }
        if (singleCompactionStats != null && CachedAppOptimizer.this.mFullDeltaRssThrottleKb > 0) {
          ...
          if (Math.abs(jArr[1] - rssAfterCompaction[1])
                  + Math.abs(jArr[2] - rssAfterCompaction[2])
                  + Math.abs(jArr[3] - rssAfterCompaction[3])
              <= CachedAppOptimizer.this.mFullDeltaRssThrottleKb) {
            return true;
          }
        }
      }
    ...
    public void handleMessage(Message message) {
      ...
      if (i3 != 1) {
        if (i3 == 5) {
          ...
          Slog.d(
              "ActivityManager",
              "Performing native compaction for pid=" + i4 + " type=" + compactProfile2.name());
          Trace.traceBegin(64L, "compactSystem");
          try {
            if (CoreRune.FAST_MADVISE_ENABLED) {
              CachedAppOptimizer.this.mProcessDependencies.performCompactionFast(
                  compactProfile2, i4);
            } else {
        ...
        if (CachedAppOptimizer.this.isProcessFrozen(i5)
            || FreecessController.getInstance().isProcessFrozen(i5)) {
          Slog.d("ActivityManager", "FCA:Reclamation for pid: " + i5);
          long[] rss2 = CachedAppOptimizer.this.mProcessDependencies.getRss(i5);
          long uptimeMillis = SystemClock.uptimeMillis();
          ...
          long j3 = rss3[0] - rss2[0];
          long j4 = rss3[1] - rss2[1];
          long j5 = rss3[2] - rss2[2];
          long j6 = rss3[3] - rss2[3];
          ...
          synchronized (activityManagerGlobalLock) {
            try {
              ProcessRecord processRecordFromPidLocked =
                  CachedAppOptimizer.this.mAm.getProcessRecordFromPidLocked(i5);
              if (processRecordFromPidLocked != null) {
                processRecordFromPidLocked.mRSSresiduePostFCA = compactProcess == 0 ? rss3[1] : 0L;
          ...
          EventLog.writeEvent(
              ...
              "",
              Integer.valueOf(compactProcess),
              0,
              0,
              0,
              0);
      ...
      synchronized (activityManagerGlobalLock2) {
        try {
          ...
          ProcessRecord processRecord2 =
              (ProcessRecord) CachedAppOptimizer.this.mPendingCompactionProcesses.remove(0);
          ...
          if (CachedAppOptimizer.this.mAm.getAppLaunchFlag()
              && !CachedAppOptimizer.this.isReclaimerControlEnabled()) {
            ActivityManagerService.resetPriorityAfterProcLockedSection();
            return;
          }
          ...
          ActivityManagerService.resetPriorityAfterProcLockedSection();
          AggregatedSourceCompactionStats perSourceAggregatedCompactStat =
              CachedAppOptimizer.this.getPerSourceAggregatedCompactStat(
                  processCachedOptimizerRecord2.getReqCompactSource());
          AggregatedProcessCompactionStats perProcessAggregatedCompactStat =
              CachedAppOptimizer.this.getPerProcessAggregatedCompactStat(str4);
          if (pid == 0) {
          ...
          if (!isForceCompact) {
            ...
            if (shouldTimeThrottleCompaction(
                processRecord2, uptimeMillis3, reqCompactProfile, reqCompactSource)) {
            ...
            if (shouldRssThrottleCompaction(compactProfile, pid, str4, rss)) {
              ...
              return;
            } else if (CachedAppOptimizer.this.mCachedAppOptimizerReclaimer
          ...
          CompactProfile resolveCompactionProfile =
              CachedAppOptimizer.this.resolveCompactionProfile(compactProfile);
          try {
            ...
            try {
              Trace.traceBegin(
                  64L,
                  "Compact "
                      + resolveCompactionProfile.name()
                      + ": "
                      + str4
                      + " lastOomAdjReason: "
                      + lastOomAdjChangeReason
                      + " source: "
                      + reqCompactSource.name());
              ...
              if (CoreRune.FAST_MADVISE_ENABLED) {
                CachedAppOptimizer.this.mProcessDependencies.performCompactionFast(
                    resolveCompactionProfile, pid);
              } else {
                CachedAppOptimizer.this.mProcessDependencies.performCompaction(
                    resolveCompactionProfile, pid);
              }
              long threadCpuTimeNs2 = CachedAppOptimizer.threadCpuTimeNs();
              long[] rss4 = CachedAppOptimizer.this.mProcessDependencies.getRss(pid);
              str = str4;
              try {
                long uptimeMillis4 = SystemClock.uptimeMillis();
                long j7 = uptimeMillis4 - uptimeMillis3;
                long j8 = threadCpuTimeNs2 - threadCpuTimeNs;
                long usedZramMemory2 = CachedAppOptimizer.getUsedZramMemory();
                long j9 = rss4[0] - rss[0];
                long j10 = rss4[1] - rss[1];
                long j11 = rss4[2] - rss[2];
                long j12 = rss4[3] - rss[3];
                CompactProfile compactProfile3 = compactProfile;
                StringBuilder sb = new StringBuilder();
                sb.append("E|Compact d_rss=");
                int i8 = i;
                sb.append(rss4[0] - rss[0]);
                sb.append("KB");
                KernelMemoryProxy$ReclaimerLog.write(sb.toString());
                int i9 =
                    AbstractC05665.f1630x5cb38e25[
                        processCachedOptimizerRecord.getReqCompactProfile().ordinal()];
                if (i9 == 1) {
                  ...
                  long j13 = -j11;
                  long j14 = usedZramMemory2 - usedZramMemory;
                  long j15 = j13 - j14;
                  long j16 = j8 / 1000000;
                  long j17 = rss[2];
                  if (j13 <= 0) {
                  ...
                  CachedAppOptimizer.this.mLastCompactionStats.put(
                      Integer.valueOf(pid), singleCompactionStats);
                ...
                EventLog.writeEvent(
                    ...
                    Long.valueOf(j12),
                    Long.valueOf(j7),
                    lastCompactProfile.name(),
                    Long.valueOf(lastCompactTime),
                    Integer.valueOf(i2),
                    Integer.valueOf(i8),
                    Long.valueOf(usedZramMemory),
                    Long.valueOf(usedZramMemory - usedZramMemory2));
                ActivityManagerGlobalLock activityManagerGlobalLock3 =
                    CachedAppOptimizer.this.mProcLock;
                ActivityManagerService.boostPriorityForProcLockedSection();
                synchronized (activityManagerGlobalLock3) {
                  ProcessCachedOptimizerRecord processCachedOptimizerRecord3 =
                ...
                Slog.d(
                    "ActivityManager",
                    "Exception occurred while compacting pid: "
                        + str
                        + ". Exception:"
                        + e.getMessage());
                j2 = 64;
  ...
  public final class FreezeHandler extends Handler
    ...
    public void handleMessage(Message message) {
      ...
      if (i == 6) {
        CachedAppOptimizer.this.reportOneUidFrozenStateChanged(
            ((Integer) message.obj).intValue(), message.arg1 == 1);
      } else {
        if (i != 7) {
          return;
        }
    ...
    public final void handleBinderFreezerFailure(final ProcessRecord processRecord, String str) {
      ...
      if (processRecord.mOptRecord.getLastUsedTimeout()
          <= CachedAppOptimizer.this.mFreezerBinderThreshold) {
        Slog.d(
            "ActivityManager",
            "Kill app due to repeated failure to freeze binder: "
                + processRecord.getPid()
                + " "
                + processRecord.processName);
        CachedAppOptimizer.this.mAm.mHandler.post(
            new Runnable() { // from class:
                             // com.android.server.am.CachedAppOptimizer$FreezeHandler$$ExternalSyntheticLambda2
              @Override // java.lang.Runnable
              public final void run() {
                CachedAppOptimizer.FreezeHandler.this.lambda$handleBinderFreezerFailure$0(
                    processRecord);
              }
            });
        return;
      ...
      long max =
          Math.max(
              (processRecord.mOptRecord.getLastUsedTimeout()
                      / CachedAppOptimizer.this.mFreezerBinderDivisor)
                  + (CachedAppOptimizer.this.mRandom.nextInt(
                          CachedAppOptimizer.this.mFreezerBinderOffset * 2)
                      - CachedAppOptimizer.this.mFreezerBinderOffset),
              CachedAppOptimizer.this.mFreezerBinderThreshold);
      Slog.d(
          "ActivityManager",
          "Reschedule freeze for process "
              + processRecord.getPid()
              + " "
              + processRecord.processName
              + " ("
            ...
            if (pid != 0 && !processCachedOptimizerRecord.isFrozen()) {
              ...
              try {
                ...
                CachedAppOptimizer.this.mFreezeHandler.post(
                    new Runnable() { // from class:
                                     // com.android.server.am.CachedAppOptimizer$FreezeHandler$$ExternalSyntheticLambda0
                      @Override // java.lang.Runnable
                      public final void run() {
              ...
              if (isFrozen) {
                ...
                if (CachedAppOptimizer.this.mRandom.nextFloat()
                  ...
                  FrameworkStatsLog.write(
                      FrameworkStatsLog.APP_FREEZE_CHANGED, 1, pid, str, freezeUnfreezeTime2, 0, 0);
                }
                try {
                  ...
                  CachedAppOptimizer.this.mFreezeHandler.post(
                      new Runnable() { // from class:
                                       // com.android.server.am.CachedAppOptimizer$FreezeHandler$$ExternalSyntheticLambda1
                        @Override // java.lang.Runnable
                        public final void run() {
                          CachedAppOptimizer.FreezeHandler.this.lambda$freezeProcess$2(
                              processRecord);
                        }
                      });
                  return;
