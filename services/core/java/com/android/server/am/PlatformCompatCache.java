  ...
  public final boolean isChangeEnabled(long j, ApplicationInfo applicationInfo, boolean z) {
    try {
      return this.mCacheEnabled
          ? ((CacheItem) this.mCaches.get(j)).isChangeEnabled(applicationInfo)
          : this.mIPlatformCompatProxy.isChangeEnabled(j, applicationInfo);
      ...
      synchronized (this.mLock) {
        ...
        if (indexOfKey >= 0) {
          ApplicationInfo applicationInfo =
              (ApplicationInfo)
                  ((WeakReference) ((Pair) this.mCache.valueAt(indexOfKey)).second).get();
