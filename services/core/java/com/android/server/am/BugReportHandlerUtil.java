...
public abstract class BugReportHandlerUtil {
  public static boolean isBugReportHandlerEnabled(Context context) {
    return context.getResources().getBoolean(R.bool.config_batterySaver_full_enableFirewall);
  }
  ...
  public static boolean launchBugReportHandlerApp(Context context) {
    ...
    } else if (getBugReportHandlerAppReceivers(
            context, customBugReportHandlerApp, customBugReportHandlerUser)
        .isEmpty()) {
      customBugReportHandlerApp = getDefaultBugReportHandlerApp(context);
      customBugReportHandlerUser = context.getUserId();
    ...
    if (isShellApp(customBugReportHandlerApp)
        || !isValidBugReportHandlerApp(customBugReportHandlerApp)
        || getBugReportHandlerAppReceivers(
                context, customBugReportHandlerApp, customBugReportHandlerUser)
            .isEmpty()) {
      return false;
    }
    if (getBugReportHandlerAppResponseReceivers(
            context, customBugReportHandlerApp, customBugReportHandlerUser)
        .isEmpty()) {
    ...
    Slog.i(
        "ActivityManager",
        "Getting response from bug report handler app: " + customBugReportHandlerApp);
    ...
    try {
      context.sendOrderedBroadcastAsUser(
          intent,
          UserHandle.of(customBugReportHandlerUser),
          "android.permission.DUMP",
          -1,
          null,
          new BugreportHandlerResponseBroadcastReceiver(
              customBugReportHandlerApp, customBugReportHandlerUser),
          null,
          0,
          null,
          null);
      ...
      Slog.e(
          "ActivityManager", "Error while trying to get response from bug report handler app.", e);
      return false;
    } finally {
  ...
  public static String getCustomBugReportHandlerApp(Context context) {
    return Settings.Secure.getStringForUser(
        context.getContentResolver(), "custom_bugreport_handler_app", context.getUserId());
  }

  public static int getCustomBugReportHandlerUser(Context context) {
    return Settings.Secure.getIntForUser(
        context.getContentResolver(), "custom_bugreport_handler_user", -10000, context.getUserId());
  }
  ...
  public static String getDefaultBugReportHandlerApp(Context context) {
    return context.getResources().getString(R.string.data_usage_rapid_body);
  }
  ...
  public static void resetCustomBugreportHandlerAppAndUser(Context context) {
    ...
    try {
      Settings.Secure.putString(
          ...
          "custom_bugreport_handler_app",
          getDefaultBugReportHandlerApp(context));
      Settings.Secure.putInt(
          context.getContentResolver(), "custom_bugreport_handler_user", context.getUserId());
    } finally {
      Binder.restoreCallingIdentity(clearCallingIdentity);
    }
  }
