...
public class MARsBigData {
  ...
  public BroadcastReceiver mIntentReceiver =
      new BroadcastReceiver() { // from class: com.android.server.am.mars.MARsBigData.1
  ...
  public MARsBigData(Context context) {
    ...
    context.registerReceiverAsUser(
        ...
        UserHandle.ALL,
        intentFilter,
        "com.samsung.android.permission.HQM_NOTIFICATION_PERMISSION",
        null);
  }
  ...
  public void sendFalconBigData(MARsPackageInfo mARsPackageInfo) {
    ...
    try {
      jSONObject
          .put("PKGN", mARsPackageInfo.getName())
          .put("UID", mARsPackageInfo.getUid())
          .put(
              "NUSD",
              (System.currentTimeMillis() - mARsPackageInfo.getDisableResetTime())
                  / ClipboardService.DEFAULT_CLIPBOARD_TIMEOUT_MILLIS)
          .put("BUSE", mARsPackageInfo.getBatteryUsage());
    } catch (JSONException e) {
      e.printStackTrace();
    }
    String jSONObject2 = jSONObject.toString();
  ...
  public void sendFGSTypeBigData() {
    ...
    for (ForegroundServiceRecord foregroundServiceRecord :
        ForegroundServiceMgr.getInstance().getMap().values()) {
      try {
        jSONObject
            .put("PKGN", foregroundServiceRecord.getPackageName())
            .put("UID", 0)
            .put("NUSD", foregroundServiceRecord.getFGSType())
            .put("BUSE", foregroundServiceRecord.getUsingFGSType());
      } catch (Exception e) {
        e.printStackTrace();
      }
      String jSONObject2 = jSONObject.toString();
