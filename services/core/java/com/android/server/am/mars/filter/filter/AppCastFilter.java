...
public class AppCastFilter implements IFilter {
  ...
  public static final Uri URI_APP_CAST_PACKAGE =
      Uri.parse("content://com.samsung.android.smartmirroring/app_cast_sent_top_package");
  ...
  public int filter(String str, int i, int i2, int i3) {
    String str2;
    return (this.mIsAppCastEnabled && (str2 = this.mAppCastPackage) != null && str2.equals(str))
        ? 24
        : 0;
  }
    ...
    if (this.mContext != null) {
      try {
        if (this.mAppCastEnabledObserver == null) {
          this.mAppCastEnabledObserver =
              new ContentObserver(
                  new Handler()) { // from class:
                                   // com.android.server.am.mars.filter.filter.AppCastFilter.1
                @Override // android.database.ContentObserver
                public void onChange(boolean z, Uri uri) {
                  String valueFromSmartMirroring =
                      AppCastFilter.this.getValueFromSmartMirroring(
                          AppCastFilter.URI_APP_CAST_ENABLED);
                  if (valueFromSmartMirroring != null) {
                    AppCastFilter.this.mIsAppCastEnabled =
                        Boolean.parseBoolean(valueFromSmartMirroring);
                  }
                }
              };
          this.mContext
              .getContentResolver()
              .registerContentObserver(URI_APP_CAST_ENABLED, false, this.mAppCastEnabledObserver);
        }
        if (this.mAppCastPackageObserver == null) {
          this.mAppCastPackageObserver =
              new ContentObserver(
                  new Handler()) { // from class:
                                   // com.android.server.am.mars.filter.filter.AppCastFilter.2
                @Override // android.database.ContentObserver
                public void onChange(boolean z, Uri uri) {
                  AppCastFilter appCastFilter = AppCastFilter.this;
                  appCastFilter.mAppCastPackage =
                      appCastFilter.getValueFromSmartMirroring(AppCastFilter.URI_APP_CAST_PACKAGE);
                }
              };
          this.mContext
              .getContentResolver()
              .registerContentObserver(URI_APP_CAST_PACKAGE, false, this.mAppCastPackageObserver);
        }
      } catch (Exception e) {
        e.printStackTrace();
      }
