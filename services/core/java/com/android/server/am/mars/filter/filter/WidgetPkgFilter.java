...
public class WidgetPkgFilter implements IFilter {
    ...
    if (MARsPolicyManager.getInstance().isFirstTimeTriggerAutorun() && i3 == 3) {
      synchronized (this.mBoundedWidgetPkgs) {
        if (this.mBoundedWidgetPkgs.size() > 0
            && (arrayList = (ArrayList) this.mBoundedWidgetPkgs.get(Integer.valueOf(i))) != null
  ...
  public void getBoundAppWidgetPackages() {
    ...
    for (UserHandle userHandle :
        ((UserManager) this.mContext.getSystemService(UserManager.class)).getUserProfiles()) {
      ...
      if (identifier == this.mContextUserId || isSubUser(identifier)) {
        ...
        for (int i = 0; i < installedProvidersForProfile.size(); i++) {
          String packageName =
              ((AppWidgetProviderInfo) installedProvidersForProfile.get(i))
                  .provider.getPackageName();
          if (appWidgetManager.isBoundWidgetPackage(packageName, identifier)) {
            synchronized (this.mBoundedWidgetPkgs) {
              ArrayList arrayList =
                  (ArrayList) this.mBoundedWidgetPkgs.get(Integer.valueOf(identifier));
              if (arrayList == null) {
              ...
              if (!arrayList.contains(packageName)) {
                ...
                MARsPolicyManager.getInstance()
                    .addFilterDebugInfoToHistory("FILTER 4 bound", packageName);
              }
