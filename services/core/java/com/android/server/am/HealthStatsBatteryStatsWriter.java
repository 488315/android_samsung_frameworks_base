...
public class HealthStatsBatteryStatsWriter {
  ...
  public void writeUid(
      HealthStatsWriter healthStatsWriter, BatteryStats batteryStats, BatteryStats.Uid uid) {
    healthStatsWriter.addMeasurement(
        10001, batteryStats.computeBatteryRealtime(this.mNowRealtimeMs * 1000, 0) / 1000);
    healthStatsWriter.addMeasurement(
        10002, batteryStats.computeBatteryUptime(this.mNowUptimeMs * 1000, 0) / 1000);
    healthStatsWriter.addMeasurement(
        10003, batteryStats.computeBatteryScreenOffRealtime(this.mNowRealtimeMs * 1000, 0) / 1000);
    healthStatsWriter.addMeasurement(
        10004, batteryStats.computeBatteryScreenOffUptime(this.mNowUptimeMs * 1000, 0) / 1000);
    for (Map.Entry entry : uid.getWakelockStats().entrySet()) {
      ...
      addTimers(
          healthStatsWriter, FrameworkStatsLog.SUBSYSTEM_SLEEP_STATE, str, wakelock.getWakeTime(1));
      addTimers(
          healthStatsWriter,
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (keyAt == -10000) {
        addTimer(
            ...
            ((BatteryStats.Uid.Sensor) sensorStats.valueAt(i)).getSensorTime());
      } else if (keyAt == -10001) {
        addTimer(
            healthStatsWriter,
            10065,
    ...
    for (int i2 = 0; i2 < size2; i2++) {
      ...
      healthStatsWriter.addStats(
          ...
          Integer.toString(pidStats.keyAt(i2)),
          healthStatsWriter2);
    }
    for (Map.Entry entry4 : uid.getProcessStats().entrySet()) {
      ...
      healthStatsWriter.addStats(
          FrameworkStatsLog.SYSTEM_ELAPSED_REALTIME, (String) entry4.getKey(), healthStatsWriter3);
    }
    for (Map.Entry entry5 : uid.getPackageStats().entrySet()) {
      ...
      healthStatsWriter.addStats(
          FrameworkStatsLog.SYSTEM_UPTIME, (String) entry5.getKey(), healthStatsWriter4);
    }
    ...
    if (modemControllerActivity != null) {
      ...
      healthStatsWriter.addMeasurement(
          FrameworkStatsLog.APP_SIZE, modemControllerActivity.getPowerCounter().getCountLocked(0));
    }
    healthStatsWriter.addMeasurement(
        FrameworkStatsLog.CATEGORY_SIZE,
        uid.getWifiRunningTime(this.mNowRealtimeMs * 1000, 0) / 1000);
    healthStatsWriter.addMeasurement(
        FrameworkStatsLog.PROC_STATS,
        uid.getFullWifiLockTime(this.mNowRealtimeMs * 1000, 0) / 1000);
    healthStatsWriter.addTimer(
        FrameworkStatsLog.BATTERY_VOLTAGE,
        uid.getWifiScanCount(0),
        uid.getWifiScanTime(this.mNowRealtimeMs * 1000, 0) / 1000);
    healthStatsWriter.addMeasurement(
        FrameworkStatsLog.NUM_FINGERPRINTS_ENROLLED,
        uid.getWifiMulticastTime(this.mNowRealtimeMs * 1000, 0) / 1000);
    ...
    addTimer(
        healthStatsWriter, FrameworkStatsLog.PROC_STATS_PKG_PROC, uid.getFlashlightTurnedOnTimer());
    ...
    addTimer(
        healthStatsWriter, FrameworkStatsLog.CPU_TIME_PER_THREAD_FREQ, uid.getBluetoothScanTimer());
    addTimer(
        healthStatsWriter,
    ...
    healthStatsWriter.addMeasurement(
        FrameworkStatsLog.DANGEROUS_PERMISSION_STATE, uid.getNetworkActivityBytes(2, 0));
    ...
    healthStatsWriter.addMeasurement(
        FrameworkStatsLog.APPS_ON_EXTERNAL_STORAGE_INFO, uid.getNetworkActivityPackets(3, 0));
    healthStatsWriter.addMeasurement(
