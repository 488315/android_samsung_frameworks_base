...
public final class AppErrorDialog extends BaseErrorDialog implements View.OnClickListener {
    ...
    public AppErrorDialog(Context context, ActivityManagerService activityManagerService, Data data) {
        ...
        if (processRecord.getPkgList().size() == 1 && (applicationLabel = context.getPackageManager().getApplicationLabel(processRecord.info)) != null) {
            setTitle(resources.getString(data.repeating ? R.string.biometric_face_not_recognized : R.string.biometric_error_user_canceled, bidiFormatter.unicodeWrap(applicationLabel.toString()), bidiFormatter.unicodeWrap(processRecord.info.processName)));
        } else {
            setTitle(resources.getString(data.repeating ? R.string.bluetooth_a2dp_audio_route_name : R.string.bluetooth_a2dp_audio_route_id, bidiFormatter.unicodeWrap(processRecord.processName.toString())));
        }
        setCancelable(true);
