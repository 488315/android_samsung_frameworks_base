...
public abstract class BaseAppStateDurationsTracker extends BaseAppStateEventsTracker {
    ...
    public final void dumpEventLocked(
                ...
                if (linkedList != null) {
                    linkedListArr2[i] =
                            BaseAppStateDurations.subtract(
                                    linkedList, uidStateDurations.mEvents[0]);
                }
                i++;
            }
        }
        baseAppStateDurations2.dump(
                printWriter,
                ConnectivityModuleConnector$$ExternalSyntheticOutline0.m("    ", str),
                j);
        printWriter.print("  " + str);
    ...
    public final long getTotalDurationsSince(int i, int i2, long j, long j2) {
        synchronized (this.mLock) {
            try {
                ...
                UidStateDurations uidStateDurations =
                        (UidStateDurations) this.mUidStateDurations.get(i);
                if (uidStateDurations != null && !uidStateDurations.isEmpty()) {
    ...
    public final void onUidProcStateChanged(int i, int i2) {
        synchronized (this.mLock) {
            try {
                ...
                uidStateDurations.addEvent(
                        i2 < 4,
                        new BaseAppStateTimeEvents$BaseTimeEvent(SystemClock.elapsedRealtime()),
                        0);
            } catch (Throwable th) {
                throw th;
            }
