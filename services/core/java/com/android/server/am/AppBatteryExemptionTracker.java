...
public final class AppBatteryExemptionTracker extends BaseAppStateDurationsTracker implements BaseAppStateTracker.StateListener {
        ...
        public LinkedList add(LinkedList linkedList, LinkedList linkedList2) {
            ...
            while (true) {
                ...
                if (timestamp == timestamp2) {
                    ...
                    if (z4 == (!z || z2)) {
                        UidStateEventWithBattery uidStateEventWithBattery5 = (UidStateEventWithBattery) uidStateEventWithBattery3.clone();
                        if (z4) {
                            UidStateEventWithBattery uidStateEventWithBattery6 = (UidStateEventWithBattery) linkedList3.peekLast();
                            long timestamp3 = uidStateEventWithBattery5.getTimestamp() - uidStateEventWithBattery6.getTimestamp();
                            it = it3;
                            it2 = it4;
                            long j6 = timestamp3 + j3;
                            if (j6 != 0) {
                                batteryUsage.scale((timestamp3 * 1.0d) / j6);
                                uidStateEventWithBattery5.update(uidStateEventWithBattery6, new AppBatteryTracker.ImmutableBatteryUsage(batteryUsage));
                            } else {
                                uidStateEventWithBattery5.update(uidStateEventWithBattery6, AppBatteryTracker.BATTERY_USAGE_NONE);
                            }
    ...
    public final class AppBatteryExemptionPolicy extends BaseAppStateEventsTracker.BaseAppStateEventsPolicy {
        public AppBatteryExemptionPolicy(BaseAppStateTracker.Injector injector, AppBatteryExemptionTracker appBatteryExemptionTracker) {
            super(injector, appBatteryExemptionTracker, "bg_battery_exemption_enabled", true, "bg_current_drain_window", appBatteryExemptionTracker.mContext.getResources().getInteger(R.integer.config_criticalBatteryWarningLevel));
        }
