...
public class PendingIntentController {
  ...
  public PendingIntentController(
    ...
    this.f1632mH = new Handler(looper);
  ...
  */
  public boolean removePendingIntentsForPackage(String str, int i, int i2, boolean z) {
    synchronized (this.mLock) {
      ...
      Iterator it = this.mIntentSenderRecords.values().iterator();
      while (it.hasNext()) {
          ...
          if (pendingIntentRecord == null) {
            ...
            if (pendingIntentRecord.key.userId == i) {
              ...
              if (pendingIntentRecord.key.activity != null) {
                this.f1632mH.sendMessage(
                    PooledLambda.obtainMessage(
                        new PendingIntentController$$ExternalSyntheticLambda0(),
                        this,
                        pendingIntentRecord.key.activity,
                        pendingIntentRecord.ref));
              }
            }
          } else if (UserHandle.getAppId(pendingIntentRecord.uid) == i2
              && (i == -1 || pendingIntentRecord.key.userId == i)) {
  ...
  public void cancelIntentSender(IIntentSender iIntentSender) {
    if (iIntentSender instanceof PendingIntentRecord) {
      synchronized (this.mLock) {
        ...
        try {
          if (!UserHandle.isSameApp(
              AppGlobals.getPackageManager()
                  .getPackageUid(
                      pendingIntentRecord.key.packageName,
                      268435456L,
                      UserHandle.getCallingUserId()),
              Binder.getCallingUid())) {
            String str =
                "Permission Denial: cancelIntentSender() from pid="
  ...
  public boolean registerIntentSenderCancelListener(
      IIntentSender iIntentSender, IResultReceiver iResultReceiver) {
    if (!(iIntentSender instanceof PendingIntentRecord)) {
      Slog.w(
          "ActivityManager",
          "registerIntentSenderCancelListener called on non-PendingIntentRecord");
      return true;
    }
    synchronized (this.mLock) {
      PendingIntentRecord pendingIntentRecord = (PendingIntentRecord) iIntentSender;
  ...
  public void setPendingIntentAllowlistDuration(
    ...
    if (!(iIntentSender instanceof PendingIntentRecord)) {
      Slog.w(
          "ActivityManager",
          "markAsSentFromNotification(): not a PendingIntentRecord: " + iIntentSender);
      return;
    }
    synchronized (this.mLock) {
  ...
  public final void makeIntentSenderCanceled(PendingIntentRecord pendingIntentRecord) {
    ...
    RemoteCallbackList detachCancelListenersLocked =
        pendingIntentRecord.detachCancelListenersLocked();
    if (detachCancelListenersLocked != null) {
      this.f1632mH.sendMessage(
          PooledLambda.obtainMessage(
              new BiConsumer() { // from class:
                                 // com.android.server.am.PendingIntentController$$ExternalSyntheticLambda1
                @Override // java.util.function.BiConsumer
                public final void accept(Object obj, Object obj2) {
                  ((PendingIntentController) obj)
                      .handlePendingIntentCancelled((RemoteCallbackList) obj2);
                }
              },
              this,
    ...
    ((AlarmManagerInternal) LocalServices.getService(AlarmManagerInternal.class))
        .remove(new PendingIntent(pendingIntentRecord));
  }
    ...
    synchronized (this.mLock) {
      if (this.mIntentSenderRecords.size() > 0) {
        ...
        for (WeakReference weakReference : this.mIntentSenderRecords.values()) {
          if (weakReference != null && weakReference.get() != null) {
            ...
            if (indexOfKey < 0) {
              ...
              sparseIntArray2.put(
                  pendingIntentRecord.uid,
                  pendingIntentRecord.key.requestIntent.getExtrasTotalSize());
            } else {
              sparseIntArray.put(pendingIntentRecord.uid, sparseIntArray.valueAt(indexOfKey) + 1);
              sparseIntArray2.put(
                  pendingIntentRecord.uid,
                  sparseIntArray2.valueAt(indexOfKey)
                      + pendingIntentRecord.key.requestIntent.getExtrasTotalSize());
            }
          }
        }
        ...
        for (int i = 0; i < size; i++) {
          arrayList.add(
              new PendingIntentStats(
                  sparseIntArray.keyAt(i),
                  sparseIntArray.valueAt(i),
                  sparseIntArray2.valueAt(i) / 1024));
        }
      }
    }
    return arrayList;
  ...
  public void incrementUidStatLocked(PendingIntentRecord pendingIntentRecord) {
    ...
    if (i == this.mConstants.PENDINGINTENT_WARNING_THRESHOLD) {
      Slog.wtf(
          "ActivityManager",
          "Too many PendingIntent created for uid "
              + i2
              + ", recent 10: "
              + Arrays.toString(ringBuffer.toArray()));
