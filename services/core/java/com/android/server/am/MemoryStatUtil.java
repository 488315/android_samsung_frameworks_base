...
public abstract class MemoryStatUtil {
  ...
  public static MemoryStat readMemoryStatFromMemcg(int i, int i2) {
    return parseMemoryStatFromMemcg(
        readFileContents(
            String.format(
                Locale.US,
                "/dev/memcg/apps/uid_%d/pid_%d/memory.stat",
                Integer.valueOf(i),
                Integer.valueOf(i2))));
  }

  public static MemoryStat readMemoryStatFromProcfs(int i) {
    return parseMemoryStatFromProcfs(
        readFileContents(String.format(Locale.US, "/proc/%d/stat", Integer.valueOf(i))));
  }
