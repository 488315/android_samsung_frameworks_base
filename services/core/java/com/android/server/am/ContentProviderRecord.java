...
public final class ContentProviderRecord implements ComponentName.WithComponentName {
  ...
  public ContentProviderHolder newHolder(
  ...
  public boolean canRunHere(ProcessRecord processRecord) {
    ...
    return (providerInfo.multiprocess || providerInfo.processName.equals(processRecord.processName))
  ...
  public boolean removeExternalProcessHandleLocked(IBinder iBinder) {
    ...
    if (hasExternalProcessHandles()) {
      ArrayMap arrayMap = this.externalProcessTokenToHandle;
      if (arrayMap == null
          || (externalProcessHandle = (ExternalProcessHandle) arrayMap.get(iBinder)) == null) {
        z = false;
      } else {
        ...
        if (!z) {
          if (this.launchingApp == null) {
            ...
            EventLogTags.writeAmProviderLostProcess(
                userId, applicationInfo.packageName, applicationInfo.uid, this.info.authority);
          } else {
            Slog.wtf(
                "ActivityManager",
  ...
  public class ExternalProcessHandle implements IBinder.DeathRecipient {
    ...
    public ExternalProcessHandle(IBinder iBinder, int i, String str) {
      ...
      try {
        ...
        Slog.e(
            "ExternalProcessHanldle", "Couldn't register for death for token: " + this.mToken, e);
      }
    }
    ...
    public void startAssociationIfNeeded(ContentProviderRecord contentProviderRecord) {
      ...
      ProcessStats.ProcessStateHolder processStateHolder =
          contentProviderRecord.proc.getPkgList().get(contentProviderRecord.name.getPackageName());
        ...
        this.mAssociation =
            processStateHolder
                ...
                .getAssociationStateLocked(
                    processStateHolder.state, contentProviderRecord.name.getClassName())
                .startSource(this.mOwningUid, this.mOwningProcessName, (String) null);
      }
    }
      ...
      synchronized (activityManagerService) {
        try {
          if (ContentProviderRecord.this.hasExternalProcessHandles()
              && ContentProviderRecord.this.externalProcessTokenToHandle.get(this.mToken) != null) {
