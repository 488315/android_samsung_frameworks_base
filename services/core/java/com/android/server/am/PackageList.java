...
public final class PackageList {
  ...
  public ProcessStats.ProcessStateHolder put(
      String str, ProcessStats.ProcessStateHolder processStateHolder) {
    ProcessStats.ProcessStateHolder processStateHolder2;
    synchronized (this) {
      this.mProcess.getWindowProcessController().addPackage(str);
      processStateHolder2 =
          (ProcessStats.ProcessStateHolder) this.mPkgList.put(str, processStateHolder);
    }
  ...
  public void forEachPackage(BiConsumer biConsumer) {
    synchronized (this) {
      ...
      for (int i = 0; i < size; i++) {
        biConsumer.accept(
            (String) this.mPkgList.keyAt(i),
            (ProcessStats.ProcessStateHolder) this.mPkgList.valueAt(i));
      }
    }
  }
      ...
      for (int i = 0; i < size; i++) {
        arrayList.add(
            new VersionedPackage(
                (String) this.mPkgList.keyAt(i),
                ((ProcessStats.ProcessStateHolder) this.mPkgList.valueAt(i)).appVersion));
      }
      return arrayList;
    }
  }
