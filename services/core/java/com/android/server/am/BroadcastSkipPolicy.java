...
public class BroadcastSkipPolicy {
  ...
  public final String shouldSkipMessage(BroadcastRecord broadcastRecord, ResolveInfo resolveInfo) {
    ...
    if (broadcastOptions != null
        && (resolveInfo.activityInfo.applicationInfo.targetSdkVersion
                < broadcastOptions.getMinManifestReceiverApiLevel()
            || resolveInfo.activityInfo.applicationInfo.targetSdkVersion
                > broadcastOptions.getMaxManifestReceiverApiLevel())) {
      return "Target SDK mismatch: receiver "
          ...
          + broadcastOptions.getMaxManifestReceiverApiLevel()
          + "] broadcasting "
          + broadcastDescription(broadcastRecord, componentName);
    }
    if (broadcastOptions != null
        && !broadcastOptions.testRequireCompatChange(
            resolveInfo.activityInfo.applicationInfo.uid)) {
      return "Compat change filtered: broadcasting "
          + broadcastDescription(broadcastRecord, componentName)
          + " to uid "
          + resolveInfo.activityInfo.applicationInfo.uid
          + " due to compat change "
          + broadcastRecord.options.getRequireCompatChangeId();
    }
    if (!broadcastSkipPolicy.mService.validateAssociationAllowedLocked(
    ...
    if (!broadcastSkipPolicy.mService.mIntentFirewall.checkBroadcast(
        broadcastRecord.intent,
        broadcastRecord.callingUid,
        broadcastRecord.callingPid,
        broadcastRecord.resolvedType,
        resolveInfo.activityInfo.applicationInfo.uid)) {
    ...
    if (ActivityManagerService.checkComponentPermission(
            activityInfo2.permission,
            broadcastRecord.callingPid,
            broadcastRecord.callingUid,
            activityInfo2.applicationInfo.uid,
            activityInfo2.exported)
        != 0) {
    ...
    if (str != null && (permissionToOpCode = AppOpsManager.permissionToOpCode(str)) != -1) {
      if (broadcastSkipPolicy
              ...
              .noteOpNoThrow(
                  ...
                  broadcastRecord.callingUid,
                  broadcastRecord.callerPackage,
                  broadcastRecord.callerFeatureId,
                  "Broadcast delivered to " + resolveInfo.activityInfo.name)
          != 0) {
        return "Appop Denial: broadcasting "
            + broadcastDescription(broadcastRecord, componentName)
            + " requires appop "
            + AppOpsManager.permissionToOp(resolveInfo.activityInfo.permission);
      }
    }
    ActivityInfo activityInfo3 = resolveInfo.activityInfo;
    if ((activityInfo3.flags & 1073741824) != 0
        && ActivityManager.checkUidPermission(
                "android.permission.INTERACT_ACROSS_USERS", activityInfo3.applicationInfo.uid)
            != 0) {
      return "Permission Denial: Receiver "
          + componentName.flattenToShortString()
          + " requests FLAG_SINGLE_USER, but app does not hold"
          + " android.permission.INTERACT_ACROSS_USERS";
    }
    if (resolveInfo.activityInfo.applicationInfo.isInstantApp()
        && broadcastRecord.callingUid != resolveInfo.activityInfo.applicationInfo.uid) {
    ...
    if (broadcastRecord.callerInstantApp) {
      ...
      if ((activityInfo4.flags & 1048576) == 0
          && broadcastRecord.callingUid != activityInfo4.applicationInfo.uid) {
        return "Instant App Denial: receiving "
            + broadcastRecord.intent
            + " to "
            + componentName.flattenToShortString()
            + " requires receiver have visibleToInstantApps set due to sender "
            + broadcastRecord.callerPackage
            + " (uid "
            + broadcastRecord.callingUid
            + ")";
    ...
    if (processRecord != null && processRecord.mErrorState.isCrashing()) {
      return "Skipping deliver ordered ["
          + broadcastRecord.queue.toString()
          + "] "
          + broadcastRecord
          + " to "
          + broadcastRecord.curApp
          + ": process crashing";
    }
    try {
      IPackageManager packageManager = AppGlobals.getPackageManager();
      ActivityInfo activityInfo5 = resolveInfo.activityInfo;
      if (!packageManager.isPackageAvailable(
          activityInfo5.packageName, UserHandle.getUserId(activityInfo5.applicationInfo.uid))) {
        return "Skipping delivery to "
      ...
      if (!broadcastSkipPolicy.requestStartTargetPermissionsReviewIfNeededLocked(
          broadcastRecord,
          activityInfo6.packageName,
          UserHandle.getUserId(activityInfo6.applicationInfo.uid))) {
      ...
      int appStartModeLOSP =
          activityManagerService.getAppStartModeLOSP(
              applicationInfo.uid,
              activityInfo7.packageName,
      ...
      if (appStartModeLOSP != 0) {
        ...
        if (disallowBackgroundStart(broadcastRecord)) {
          broadcastSkipPolicy.mService.addBackgroundCheckViolationLocked(
              broadcastRecord.intent.getAction(), componentName.getPackageName());
      ...
      if (!"android.intent.action.ACTION_SHUTDOWN".equals(broadcastRecord.intent.getAction())
          && !broadcastSkipPolicy.mService.mUserController.isUserRunning(
              UserHandle.getUserId(resolveInfo.activityInfo.applicationInfo.uid), 0)) {
        return "Skipping delivery to "
            + resolveInfo.activityInfo.packageName
            + " / "
            + resolveInfo.activityInfo.applicationInfo.uid
      ...
      if (strArr2 != null && strArr2.length > 0) {
        ...
        while (true) {
          ...
          try {
            ...
            i2 =
                packageManager2.checkPermission(
                    str2, applicationInfo2.packageName, UserHandle.getUserId(applicationInfo2.uid));
          ...
          if (permissionToOpCode2 != i3) {
            if (i2 == 0) {
              ...
              if (appOpsManager.checkOpNoThrow(
                      permissionToOpCode2,
                      activityInfo8.applicationInfo.uid,
                      activityInfo8.packageName)
                  == 0) {
                return "Skipping delivery to "
                    + resolveInfo.activityInfo.packageName
                    + " due to excluded permission "
                    + str2;
              }
            } else {
              continue;
      ...
      if (strArr4 != null
          && strArr4.length > 0
          && ArrayUtils.contains(strArr4, componentName.getPackageName())) {
      ...
      if (resolveInfo.activityInfo.applicationInfo.uid != 1000
          && (strArr = broadcastRecord.requiredPermissions) != null
          && strArr.length > 0) {
        int i5 = 0;
        while (true) {
          ...
          try {
            ...
            i =
                packageManager3.checkPermission(
                    str3, applicationInfo3.packageName, UserHandle.getUserId(applicationInfo3.uid));
          ...
          if (permissionToOpCode3 != -1 && permissionToOpCode3 != broadcastRecord.appOp) {
            if (!noteOpForManifestReceiver(
                permissionToOpCode3, broadcastRecord, resolveInfo, componentName)) {
              return "Skipping delivery to "
                  + resolveInfo.activityInfo.packageName
                  + " due to required appop "
                  + permissionToOpCode3;
            }
          }
          i5++;
  ...
  public boolean disallowBackgroundStart(BroadcastRecord broadcastRecord) {
    return (broadcastRecord.intent.getFlags() & 8388608) != 0
        || (broadcastRecord.intent.getComponent() == null
            && broadcastRecord.intent.getPackage() == null
            && (broadcastRecord.intent.getFlags() & 16777216) == 0
            && !isSignaturePerm(broadcastRecord.requiredPermissions));
  }

  public final String shouldSkipMessage(
      BroadcastRecord broadcastRecord, BroadcastFilter broadcastFilter) {
    String str;
    String str2;
    String str3;
    String str4;
    ...
    BroadcastOptions broadcastOptions = broadcastRecord.options;
    if (broadcastOptions != null
        && !broadcastOptions.testRequireCompatChange(broadcastFilter.owningUid)) {
      return "Compat change filtered: broadcasting "
          + broadcastRecord.intent.toString()
          + " to uid "
          + broadcastFilter.owningUid
          + " due to compat change "
          + broadcastRecord.options.getRequireCompatChangeId();
    }
    if (!broadcastSkipPolicy.mService.validateAssociationAllowedLocked(
    ...
    if (!broadcastSkipPolicy.mService.mIntentFirewall.checkBroadcast(
        ...
        broadcastRecord.callingPid,
        broadcastRecord.resolvedType,
        broadcastFilter.receiverList.uid)) {
      return "Firewall blocked: broadcasting "
          + broadcastRecord.intent.toString()
          + " from "
          + broadcastRecord.callerPackage
    ...
    if (str14 != null) {
      if (ActivityManagerService.checkComponentPermission(
              str14, broadcastRecord.callingPid, broadcastRecord.callingUid, -1, true)
          != 0) {
        return "Permission Denial: broadcasting "
            + broadcastRecord.intent.toString()
            + " from "
            ...
            + broadcastRecord.callingUid
            + ") requires "
            + broadcastFilter.requiredPermission
      ...
      if (permissionToOpCode != -1
        ...
        return "Appop Denial: broadcasting "
            ...
            + broadcastRecord.callingUid
            + ") requires appop "
            + AppOpsManager.permissionToOp(broadcastFilter.requiredPermission)
            + " due to registered receiver "
            + broadcastFilter;
      }
    }
    ...
    if (processRecord == null
        ...
        || broadcastFilter.receiverList.app.mErrorState.isCrashing()) {
      return "Skipping deliver ["
          + broadcastRecord.queue.toString()
          + "] "
          + broadcastRecord
          + " to "
          + broadcastFilter.receiverList
          + ": process gone or crashing";
    }
    if (!((broadcastRecord.intent.getFlags() & 2097152) != 0)
        && broadcastFilter.instantApp
        && broadcastFilter.receiverList.uid != broadcastRecord.callingUid) {
      return "Instant App Denial: receiving "
          + broadcastRecord.intent.toString()
          + " to "
          + broadcastFilter.receiverList.app
          + " (pid="
          + broadcastFilter.receiverList.pid
          + ", uid="
          + broadcastFilter.receiverList.uid
          + ") due to sender "
          + broadcastRecord.callerPackage
    ...
    if (!broadcastFilter.visibleToInstantApp
        && broadcastRecord.callerInstantApp
        && broadcastFilter.receiverList.uid != broadcastRecord.callingUid) {
      return "Instant App Denial: receiving "
          + broadcastRecord.intent.toString()
          + " to "
          ...
          + ", uid="
          + broadcastFilter.receiverList.uid
          + ") requires receiver be visible to instant apps due to sender "
          + broadcastRecord.callerPackage
          + " (uid "
          + broadcastRecord.callingUid
          + ")";
    ...
    if (strArr != null && strArr.length > 0) {
      ...
      while (true) {
        ...
        String str23 = str19;
        if (ActivityManagerService.checkComponentPermission(
                str21, receiverList.pid, receiverList.uid, -1, true)
            != 0) {
          return str18
              + broadcastRecord.intent.toString()
              + " to "
        ...
        if (permissionToOpCode2 == -1 || permissionToOpCode2 == broadcastRecord.appOp) {
          ...
          if (appOpsManager.noteOpNoThrow(permissionToOpCode2, i3, str24, str25, sb.toString())
              != 0) {
            return str23
                + broadcastRecord.intent.toString()
                + " to "
                ...
                + broadcastFilter.receiverList.uid
                + str20
                + AppOpsManager.permissionToOp(str21)
                + " due to sender "
                + broadcastRecord.callerPackage
                + " (uid "
                + broadcastRecord.callingUid
    ...
    if (strArr3 == null || strArr3.length == 0) {
      ...
      if (ActivityManagerService.checkComponentPermission(
              null, receiverList2.pid, receiverList2.uid, -1, true)
          != 0) {
    ...
    if (strArr4 != null && strArr4.length > 0) {
      ...
      while (true) {
        ...
        int checkComponentPermission =
            ActivityManagerService.checkComponentPermission(
                str26, receiverList3.pid, receiverList3.uid, -1, true);
        ...
        if (permissionToOpCode3 == -1) {
          ...
          if (this.mService
            ...
            return str3
                ...
                + broadcastFilter.receiverList.uid
                + ") excludes appop "
                + AppOpsManager.permissionToOp(str26)
                + " due to sender "
                + broadcastRecord.callerPackage
                + " (uid "
                + broadcastRecord.callingUid
    ...
    if (strArr6 != null
        && strArr6.length > 0
        && ArrayUtils.contains(strArr6, broadcastFilter.packageName)) {
      return "Skipping delivery of excluded package "
          + broadcastRecord.intent.toString()
          + " to "
          + broadcastFilter.receiverList.app
    ...
    if (broadcastRecord.appOp != -1) {
      if (this.mService
              ...
              .noteOpNoThrow(
                  ...
                  broadcastFilter.receiverList.uid,
                  broadcastFilter.packageName,
                  broadcastFilter.featureId,
                  str29 + broadcastFilter.receiverId)
          != 0) {
        return str3
            + broadcastRecord.intent.toString()
            + " to "
            ...
            + broadcastFilter.receiverList.uid
            + str28
            + AppOpsManager.opToName(broadcastRecord.appOp)
            + " due to sender "
            + broadcastRecord.callerPackage
            + " (uid "
            + broadcastRecord.callingUid
    ...
    int i5 =
        broadcastRecord.sticky
            ? broadcastRecord.originalStickyCallingUid
            : broadcastRecord.callingUid;
    boolean z = broadcastFilter.exported;
    if (!z
        && ActivityManagerService.checkComponentPermission(
                null, broadcastRecord.callingPid, i5, broadcastFilter.receiverList.uid, z)
            != 0) {
    ...
    if (requestStartTargetPermissionsReviewIfNeededLocked(
        broadcastRecord, broadcastFilter.packageName, broadcastFilter.owningUserId)) {
      return null;
    }
  ...
  public static String broadcastDescription(
      BroadcastRecord broadcastRecord, ComponentName componentName) {
    return broadcastRecord.intent.toString()
        + " from "
        + broadcastRecord.callerPackage
        + " (pid="
        + broadcastRecord.callingPid
  ...
  public final boolean noteOpForManifestReceiver(
      int i,
  ...
  public final boolean noteOpForManifestReceiverInner(
      int i,
      BroadcastRecord broadcastRecord,
      ResolveInfo resolveInfo,
      ComponentName componentName,
    ...
    Slog.w(
        "BroadcastQueue",
        "Appop Denial: receiving "
            + broadcastRecord.intent
            + " to "
            + componentName.flattenToShortString()
            + " requires appop "
            + AppOpsManager.opToName(i)
            + " due to sender "
            + broadcastRecord.callerPackage
            + " (uid "
            + broadcastRecord.callingUid
    ...
    for (int length = strArr.length - 1; length >= 0; length--) {
      try {
        PermissionInfo permissionInfo =
            permissionManager.getPermissionInfo(strArr[length], "android", 0);
  ...
  public final boolean requestStartTargetPermissionsReviewIfNeededLocked(
      BroadcastRecord broadcastRecord, String str, final int i) {
    boolean z = true;
    ...
    if (z && broadcastRecord.intent.getComponent() != null) {
      ...
      PendingIntentController pendingIntentController =
          activityManagerService.mPendingIntentController;
      ...
      PendingIntentRecord intentSender =
          pendingIntentController.getIntentSender(
              ...
              new Intent[] {intent},
              new String[] {
                intent.resolveType(activityManagerService.mContext.getContentResolver())
              },
              1409286144,
              null);
      ...
      this.mService.mHandler.post(
          new Runnable() { // from class: com.android.server.am.BroadcastSkipPolicy.1
            @Override // java.lang.Runnable
            public void run() {
              BroadcastSkipPolicy.this.mService.mContext.startActivityAsUser(
                  intent2, new UserHandle(i));
            }
          });
    } else {
      Slog.w(
          "BroadcastQueue",
          "u" + i + " Receiving a broadcast in package" + str + " requires a permissions review");
    }
    return false;
  }
}
