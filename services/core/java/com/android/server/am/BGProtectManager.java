...
public class BGProtectManager {
  ...
  public static int WEBVIEW_ADJ_THRESHOLD =
      BgAppPropManager.getSlmkPropertyInt("webview_adj_th", Integer.toString(920));
  public static boolean mCameraGuardEnable =
  ...
  public int forceKillHeavyProcessLimit =
      Math.min(BgAppPropManager.getSlmkPropertyInt("dha_cached_min", "4"), 4);
  ...
  static {
    ...
    DHA_STATICEXCEPT_PROC_ARRAY =
        new String[][] {
          ...
          new String[] {DynamicHiddenApp.decodeToStr("RElBTEVS"), exceptflag2.getString()},
          new String[] {DynamicHiddenApp.decodeToStr("SE9NRUhVQg=="), exceptflag2.getString()},
          new String[] {
          ...
          new String[] {DynamicHiddenApp.decodeToStr("QklYQlk="), exceptFlag.HOMEHUB.getString()}
        };
    PROVIDER_LIFEGUARD_ARRAY =
        new String[] {
          ...
          DynamicHiddenApp.decodeToStr("Y29tLnNhbXN1bmcuYW5kcm9pZC5nbG9iYWxwb3N0cHJvY21ncg==")
        };
    ...
    DHA_NEVERKILLEXCEPT_ARRAY_BY_KEY =
        new String[][] {
          ...
          new String[] {
            DynamicHiddenApp.decodeToStr("Y29tLmdvb2dsZS5hbmRyb2lkLmdtcy5wZXJzaXN0ZW50"),
            "1",
            DynamicHiddenApp.decodeToStr("Y29tLmdvb2dsZS5hbmRyb2lkLmdtcw=="),
            "priv"
          }
    ...
    forceKillHeavyProcess2 =
        DynamicHiddenApp.decodeToStr("Y29tLmF2YXN0LmFuZHJvaWQubW9iaWxlc2VjdXJpdHk=");
    forceKillHeavyProcess3 = DynamicHiddenApp.decodeToStr("Y29tLnNreXBlLnJhaWRlcg==");
    forceKillHeavyProcessList =
        new ArrayList(
            Arrays.asList(forceKillHeavyProcess1, forceKillHeavyProcess2, forceKillHeavyProcess3));
    ...
    switch (processRecord.mState.getCurProcState()) {
      ...
      default:
        ...
        if (processRecord2 != null) {
          if (processRecord2.getIpmLaunchtype() == 0
              && processRecord.getMlLaunchTime() > this.NapProcessSlotDefault.getMlLaunchTime()) {
            this.NapProcessSlotDefault.setMlLaunchTime(-1L);
            this.NapProcessSlotDefault.setIpmLaunchType(-1);
            if (this.NapProcessSlotDefault.mState.getCurProcState() >= 19
                && this.NapProcessSlotDefault.mState.getCurRawAdj() > 860) {
              this.NapProcessSlotDefault.killLocked(
                  "ML_Kill: over " + this.NapProcessSlotLimit + " slots kill a", 13, true);
            }
          }
          ...
          Log.e(
              ...
              "app launch time is " + processRecord.getMlLaunchTime() + " tmp time is ");
          this.NapProcessSlotDefault = processRecord;
        }
        if (processRecord.getIpmLaunchtype() == 0
            && processRecord.getMlLaunchTime() != -1
            && elapsedRealtime > 1800000
            && processRecord.mState.getCurProcState() >= 19
            && processRecord.mState.getCurRawAdj() > 860) {
          processRecord.killLocked(
              "ML_Kill: timeout " + elapsedRealtime + " slots kill a", 13, true);
  ...
  */
  public int checkKeptProcess(ProcessRecord processRecord) {
    ...
    boolean z;
    int i2;
    if (mAMSExceptionEnable && processRecord.isAMSException) {
      if (UserHandle.getUserId(processRecord.uid) < 100) {
        if (processRecord.AMSExceptionFlag > exceptFlag.NORMALANDKNOX.getValue()) {
          if (processRecord.AMSExceptionFlag == exceptFlag.HOMEHUB.getValue()) {}
        }
        i = 1;
        z = false;
      } else {
        if (this.mKnoxAMSExceptionEnable) {
          if (processRecord.AMSExceptionFlag < exceptFlag.NORMALANDKNOX.getValue()
              || processRecord.AMSExceptionFlag > exceptFlag.KNOXONLY.getValue()) {
            if (processRecord.AMSExceptionFlag == exceptFlag.NORMALANDKNOXPWHL.getValue()) {
              z = true;
              i = 0;
            }
          }
      ...
      if (processRecord.mState.getCurProcState() == 17
    ...
    if ((this.mDhaKeepEmptyEnable == 1 || this.mDhaKeepEmptyEnableKnox == 1)
        && (i2 = processRecord.dhaKeepEmptyFlag) > 0
        && i2 < 3) {
      z = true;
    }
    if (i != 1 && z) {
    ...
    int i3 = !processRecord.isActiveLaunch() ? i : 3;
    if (processRecord.getIpmLaunchtype() == 0) {
      i3 = 4;
    }
    if (processRecord.getIpmLaunchtype() == 1) {
      i3 = 5;
    }
    if (isForKeepingCheck(processRecord)) {
      i3 = 6;
    }
    if (processRecord.isNeverKillException) {
      i3 = 8;
    }
    if (this.doKillRestrict
        && IsForceKillHeavyProcess(processRecord.processName)
        && processRecord.mState.getCurAdj() >= 100
        && forceKillHeavyProcess2.equals(processRecord.processName)) {
      processRecord.killLocked("bg_restriction", 13, 2, true);
      this.doKillRestrict = false;
    }
    return i3;
  }
  ...
  public void resetKillExceptFlag(ProcessRecord processRecord) {
    ...
    if (isDhaKeepEmptyProcess != -1) {
      ...
      if (this.mDhaKeepEmptyEnableKnox == 1
  ...
  public void setLmkdProtectFlag(ProcessRecord processRecord) {
    ...
    if (appIsPickedProcess(processRecord.processName, processRecord.userId) != -1) {
      Slog.d(
          "DynamicHiddenApp_BGProtectManager",
          "[SecIpm] it's a ML_TYPE_EMPTYPROCESS protected process " + processRecord.processName);
  ...
  */
  public int setCustomADJAndGetProcState(ProcessRecord processRecord) {
    ...
    int i;
    if (processRecord == null || (processStateRecord = processRecord.mState) == null) {
      return -1;
    }
    int curProcState = processStateRecord.getCurProcState();
    if (processStateRecord.getCurAdj() != processStateRecord.getSetAdj()) {
      if (processStateRecord.getCurAdj() >= 850 && processStateRecord.getCurAdj() <= 999) {
        if (DynamicHiddenApp.LMK_ENABLE_USERSPACE_LMK && DynamicHiddenApp.LMK_ENABLE_REENTRY_LMK) {
          if (processRecord.hasActivities()
              && (processStateRecord.getCurProcState() == 10
                  || processStateRecord.getCurProcState() == 15)) {
            curProcState = 16;
          }
        } else if (IsProtected(processRecord)) {
          processStateRecord.setCurAdj(900);
          processStateRecord.setCurRawAdj(processStateRecord.getCurAdj());
        }
        if (DynamicHiddenApp.PICKED_ADJ_ENABLE) {
          if (processRecord.userId == 0 && IsProtected(processRecord)) {
            if (IsAllowListCleared()) {
              processStateRecord.setCurAdj(850);
              processStateRecord.setCurRawAdj(processStateRecord.getCurAdj());
            } else if (SystemClock.uptimeMillis() - processStateRecord.getLastStateTime()
                >= this.PICKED_ADJ_TIME_LIMIT) {
              if (!isContainPickedExceptList(processRecord.processName)
                  && isDhaKeepEmptyProcess(processRecord.processName) != -1) {
                processStateRecord.setCurAdj(900);
                processStateRecord.setCurRawAdj(processStateRecord.getCurAdj());
                addPickedExceptList(processRecord.processName);
                Slog.i(
                    "DynamicHiddenApp_BGProtectManager",
                    "Picked process timeout "
              ...
              processStateRecord.setCurAdj(850);
              processStateRecord.setCurRawAdj(processStateRecord.getCurAdj());
              removePickedExceptList(processRecord.processName);
            }
          }
          if (processRecord.getIpmLaunchtype() == 1) {
            processStateRecord.setCurAdj(850);
      ...
      int i2 = 92;
      int i3 = 99;
      if (processStateRecord.getCurAdj() >= 250 && processStateRecord.getCurAdj() <= 999) {
        if (processRecord.userId == 0
            && processRecord.AMSExceptionFlag == exceptFlag.CAMERAGUARD.getValue()) {
          curProcState = processStateRecord.getCurProcState() == 16 ? 91 : 90;
        }
        if (IsProtected(processRecord)) {
          if (this.AMSExceptionProviderUpgradeAdjEnable
              && ("android.process.acore".equals(processRecord.processName)
                  || "android.process.media".equals(processRecord.processName))) {
            processStateRecord.setCurAdj(200);
            processStateRecord.setCurRawAdj(processStateRecord.getCurAdj());
          }
          curProcState =
              processRecord.AMSExceptionFlag == exceptFlag.CAMERAMEDIA.getValue() ? 92 : 99;
        }
      }
      if (processStateRecord.getCurAdj() >= 800 && processStateRecord.getCurAdj() <= 999) {
        if (processRecord.AMSExceptionFlag == exceptFlag.SANDBOX.getValue()) {
          if (!processRecord.isAMSException) {
            curProcState = 98;
          } else if (!this.CLEANUP_WEBVIEW_ENABLE
              || processStateRecord.getCurAdj() <= WEBVIEW_ADJ_THRESHOLD) {
            curProcState = 99;
          }
        }
        if (!processRecord.isActiveLaunch() || processStateRecord.getCurProcState() != 19) {
          i =
              (processRecord.getIpmLaunchtype() == 0 && processStateRecord.getCurProcState() == 19)
                  ? 96
                  : 97;
          if (processRecord.AMSExceptionFlag == exceptFlag.HOMEHUB.getValue()
              && DynamicHiddenApp.sHH_AMSExceptionEnable) {
            curProcState = 99;
          }
        }
        curProcState = i;
      ...
      if (DynamicHiddenApp.BORA_POLICY_ENABLE) {
        if (processRecord.AMSExceptionFlag != exceptFlag.CAMERAMEDIA.getValue()
            || processStateRecord.getCurAdj() < 250) {
          if (processStateRecord.getCurAdj() == 700 && processRecord.hasActivities()) {
            i2 = 150;
          } else if (isRecentActivityProcess(processRecord)) {
            i2 = getIndexOfRecentActivityProcess(processRecord) + 180;
            if (i2 > 199) {
              i2 = 199;
            }
          }
        }
        if (processStateRecord.getCurAdj() < 200
            && processStateRecord.getCurAdj() >= 0
            && processRecord.AMSExceptionFlag == exceptFlag.BROWSERMAIN.getValue()) {
          i2 = 95;
        }
        if (!processRecord.isSDException && processRecord.isNeverKillException) {
          if (processRecord.userId != 0) {
            processStateRecord.setCurAdj(-700);
          } else if (processRecord.AMSExceptionFlag >= exceptFlag.NORMALANDKNOX.getValue()) {
            processStateRecord.setCurAdj(-700);
          }
          curProcState =
              (processStateRecord.getCurProcState() == 19 || !isBEKCondition(processRecord))
                  ? i3
                  : 93;
        ...
        i3 = i2;
        if (processStateRecord.getCurProcState() == 19) {}
        if (CoreRune.ALLIED_PROC_PROTECTION_LMKD) {}
      }
      i2 = curProcState;
      if (processStateRecord.getCurAdj() < 200) {
      ...
      if (!processRecord.isSDException) {
        if (processRecord.userId != 0) {}
        if (processStateRecord.getCurProcState() == 19) {}
        if (CoreRune.ALLIED_PROC_PROTECTION_LMKD) {}
      }
  ...
  public boolean IsProtected(ProcessRecord processRecord) {
    int i;
    int i2;
    return DynamicHiddenApp.sHH_AMSExceptionEnable
  ...
  public boolean isProtectedInChimera(ProcessRecord processRecord) {
    ...
    if (!DynamicHiddenApp.sHH_AMSExceptionEnable
        ? !(((i = processRecord.dhaKeepEmptyFlag) <= 0 || i >= 4)
            && (!processRecord.isAMSException
                || processRecord.AMSExceptionFlag == exceptFlag.BROWSERMAIN.getValue()))
  ...
  public static boolean isCameraGuardProcess(String str) {
    HashMap hashMap;
    return mCameraGuardEnable
        && (hashMap = dha_cameraguard_map) != null
  ...
  public static int isWebviewProcess(ProcessRecord processRecord) {
    if (processRecord.getHostingRecord() != null
        && processRecord.getHostingRecord().usesWebviewZygote()) {
      if (processRecord.getHostingRecord().getName() != null) {
        Slog.i(
            "DynamicHiddenApp_BGProtectManager",
            "check webview name : "
                + processRecord.processName
                + "check hostingname webview : "
  ...
  public boolean isForceKillHeavyCondition(ProcessRecord processRecord, int i) {
    return IsForceKillHeavyProcess(processRecord.processName)
        && (i > this.forceKillHeavyProcessLimit || processRecord.mState.getCurAdj() >= 920);
  }
  ...
  public void addRecentActivityProcess(ProcessRecord processRecord) {
    if (this.recentActivityProcessList.size() >= this.recentActivityProcessLimit
        || !isCachedOrPickedActivityProcess(processRecord)) {
      return;
    }
  ...
  public final boolean isRecentActivityProcess(ProcessRecord processRecord) {
    return this.recentActivityProcessList.contains(processRecord)
        && isCachedOrPickedActivityProcess(processRecord);
  }
  ...
  public final boolean isCachedOrPickedActivityProcess(ProcessRecord processRecord) {
    ...
    return (processRecord.hasActivities()
            && (processRecord.mState.getCurProcState() == 10
                || processRecord.mState.getCurProcState() == 15))
        || processRecord.mState.getCurProcState() == 16;
  }
  ...
  public final void dhaAddPackageName(HashMap hashMap, String str, int i, boolean z) {
    ...
    if (decodeToStr5.equals(str)) {
      hashMap.put(
          DynamicHiddenApp.decodeToStr("Y29tLnNhbXN1bmcuYW5kcm9pZC5ob21laHVi"), Integer.valueOf(i));
      return;
    }
    ...
    if (decodeToStr7.equals(str)
        && BgAppPropManager.getSemSystemPropertyInt("ro.debuggable", "0") == 1) {
      Slog.i(
          "DynamicHiddenApp_BGProtectManager", "it's debuggable binary!! add FACTORY in allowlist");
      ...
      hashMap.put(
          DynamicHiddenApp.decodeToStr("Y29tLnNhbXN1bmcuYW5kcm9pZC5haXJjb21tYW5kbWFuYWdlcg=="),
          Integer.valueOf(i));
      return;
    }
        ...
        hashMap.put(
            ...
            new Pair(
                ...
                new PackageValidationInfo(
                    DynamicHiddenApp.decodeToStr("Y29tLnNlYy5mYWNhdGZ1bmN0aW9u"), i3)));
        hashMap.put(
            DynamicHiddenApp.decodeToStr("Y29tLnNlYy5mYWN1aWZ1bmN0aW9u"),
            new Pair(
                Integer.valueOf(i),
                new PackageValidationInfo(
                    DynamicHiddenApp.decodeToStr("Y29tLnNlYy5mYWN1aWZ1bmN0aW9u"), i3)));
        hashMap.put(
            DynamicHiddenApp.decodeToStr("Y29tLnNhbXN1bmcuYW5kcm9pZC5haXJjb21tYW5kbWFuYWdlcg=="),
            new Pair(
                Integer.valueOf(i),
  ...
  public String getContactsPackageName(Context context) {
    ...
    String string =
        SemFloatingFeature.getInstance()
            .getString("SEC_FLOATING_FEATURE_CONTACTS_CONFIG_PACKAGE_NAME", decodeToStr);
    if (decodeToStr.equals(string)) {
      return decodeToStr;
    }
    try {
  ...
  public final String getInCallUIPackageName(Context context) {
    ...
    String string =
        SemFloatingFeature.getInstance()
            .getString("SEC_FLOATING_FEATURE_VOICECALL_CONFIG_INCALLUI_PACKAGE_NAME", decodeToStr);
    if (decodeToStr.equals(string)) {
      return decodeToStr;
    }
    try {
  ...
  public class PackageValidationInfo {
    ...
    public boolean validate(ProcessRecord processRecord) {
      if (processRecord != null && processRecord.info != null) {
        ...
        if ((i & i2) == i2
            && processRecord.info.packageName != null
            && processRecord.info.packageName.equals(this.packageName)) {
          return true;
        }
      }
      return false;
