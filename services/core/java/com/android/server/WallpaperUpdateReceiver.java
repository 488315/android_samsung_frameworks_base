...
public class WallpaperUpdateReceiver extends BroadcastReceiver {
  ...
  public void onReceive(Context context, Intent intent) {
    if (intent == null
        || !"android.intent.action.DEVICE_CUSTOMIZATION_READY".equals(intent.getAction())) {
      return;
    }
    AsyncTask.execute(
        new Runnable() { // from class:
          // com.android.server.WallpaperUpdateReceiver$$ExternalSyntheticLambda0
  ...
  public final void updateWallpaper() {
    try {
      ...
      if (isUserSetWallpaper(wallpaperManager, systemUiContext)) {
        ...
        wallpaperManager.setResource(R.drawable.chooser_action_button_bg);
      }
    } catch (Exception e) {
  ...
  public final boolean isUserSetWallpaper(WallpaperManager wallpaperManager, Context context) {
    ...
    if (wallpaperInfo == null) {
      return (wallpaperManager.getWallpaperFile(1) == null
              && wallpaperManager.getWallpaperFile(2) == null)
          ? false
          : true;
    }
    return !wallpaperInfo
        .getComponent()
        .equals(WallpaperManager.getDefaultWallpaperComponent(context));
  }
}
