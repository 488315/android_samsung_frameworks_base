...
public class RoamingPolicy extends IRoamingPolicy.Stub implements EnterpriseServiceCallback {
  ...
  public final BroadcastReceiver mSimFactoryIntentReceiver =
      new BroadcastReceiver() { // from class:
                                // com.android.server.enterprise.restriction.RoamingPolicy.1
    ...
    if (this.mTelMgr.getPhoneCount() > 1) {
      String semGetTelephonyProperty =
          TelephonyManager.semGetTelephonyProperty(
              ...
              "false");
      isNetworkRoaming =
          !TextUtils.isEmpty(semGetTelephonyProperty) && semGetTelephonyProperty.equals("true");
    }
    if (isNetworkRoaming) {
  ...
  public final ContextInfo enforceOwnerOnlyAndRoamingPermission(ContextInfo contextInfo) {
    return getEDM()
        .enforceOwnerOnlyAndActiveAdminPermission(
            contextInfo,
            new ArrayList(Arrays.asList("com.samsung.android.knox.permission.KNOX_ROAMING")));
  }

  public boolean setRoamingSync(ContextInfo contextInfo, boolean z) {
    ContextInfo enforceOwnerOnlyAndRoamingPermission =
        enforceOwnerOnlyAndRoamingPermission(contextInfo);
    boolean putBoolean =
        this.mEdmStorageProvider.putBoolean(
            enforceOwnerOnlyAndRoamingPermission.mCallerUid, "ROAMING", "roamingSyncEnabled", z);
  ...
  public boolean isRoamingSyncEnabled(ContextInfo contextInfo) {
    boolean z;
    Iterator it =
        this.mEdmStorageProvider.getBooleanList("ROAMING", "roamingSyncEnabled").iterator();
    while (true) {
      if (!it.hasNext()) {
        z = true;
        break;
  ...
  public boolean setRoamingPush(ContextInfo contextInfo, boolean z) {
    boolean putBoolean =
        this.mEdmStorageProvider.putBoolean(
            enforceOwnerOnlyAndRoamingPermission(contextInfo).mCallerUid,
            "ROAMING",
            "roamingPushEnabled",
            z);
  ...
  public boolean isRoamingPushEnabled(ContextInfo contextInfo) {
    boolean z;
    Iterator it =
        this.mEdmStorageProvider.getBooleanList("ROAMING", "roamingPushEnabled").iterator();
    while (true) {
      if (!it.hasNext()) {
        z = true;
        break;
  ...
  public boolean setRoamingData(ContextInfo contextInfo, boolean z) {
    ...
    boolean putBoolean =
        this.mEdmStorageProvider.putBoolean(
            enforceOwnerOnlyAndRoamingPermission.mCallerUid, "ROAMING", "roamingDataEnabled", z);
    int callingOrCurrentUserId =
        Utils.getCallingOrCurrentUserId(enforceOwnerOnlyAndRoamingPermission);
    if (!isRoamingDataEnabled(enforceOwnerOnlyAndRoamingPermission)) {
      Intent intent =
          new Intent("com.samsung.android.knox.intent.action.ROAMING_SETROAMINGDATA_INTERNAL");
    ...
    Log.d(
        "RoamingPolicy",
        "setDataRoaming : " + isRoamingDataEnabled(enforceOwnerOnlyAndRoamingPermission));
    setRoamingDataAllowedSystemUI(
        callingOrCurrentUserId, isRoamingDataEnabled(enforceOwnerOnlyAndRoamingPermission));
    updatePolicyState();
    return putBoolean;
  }
  ...
  public boolean isRoamingDataEnabled(ContextInfo contextInfo) {
    boolean z;
    Iterator it =
        this.mEdmStorageProvider.getBooleanList("ROAMING", "roamingDataEnabled").iterator();
    while (true) {
      if (!it.hasNext()) {
        z = true;
        break;
  ...
  public void onAdminRemoved(int i) {
    ...
    if (isRoamingSyncEnabled(null)) {
      Log.d(
          "RoamingPolicy",
          "Roaming Sync is not being applied, so de-register listener and restore sync setting");
      deregisterRoamingListener();
      restoreUserAutoSyncSetting();
    }
  ...
  public boolean isRoamingVoiceCallsEnabled(ContextInfo contextInfo) {
    Iterator it =
        this.mEdmStorageProvider.getBooleanList("ROAMING", "roamingVoiceCallsEnabled").iterator();
    while (it.hasNext()) {
