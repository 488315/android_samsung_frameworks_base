...
public class EdmStorageHelper extends SQLiteOpenHelper {
  ...
  public final void updateProfilePoicyTable(SQLiteDatabase sQLiteDatabase) {
    try {
      sQLiteDatabase.execSQL(
          "alter table ProfilePolicyPORestrictions rename to ProfilePolicyPORestrictions_tmp");
      sQLiteDatabase.execSQL(
          "CREATE TABLE ProfilePolicyPORestrictions (adminUid INTEGER,"
              + " ProfilePolicyPORestrictionsValue NUMERIC DEFAULT 1,"
              + " ProfilePolicyPORestrictionsProperty TEXT, PRIMARY KEY (adminUid,"
              + " ProfilePolicyPORestrictionsProperty) FOREIGN KEY (adminUid) REFERENCES"
              + " ADMIN(adminUid) ON DELETE CASCADE ON UPDATE CASCADE)");
  ...
  public final boolean createDomainFilterListTable(SQLiteDatabase sQLiteDatabase) {
    try {
      sQLiteDatabase.execSQL(
          "CREATE TABLE DomainFilterListTable (adminUid INTEGER, packageName TEXT,domain"
              + " TEXT,typeList TEXT, PRIMARY KEY (adminUid, packageName, domain) FOREIGN KEY"
              + " (adminUid) REFERENCES ADMIN(adminUid) ON DELETE CASCADE ON UPDATE CASCADE)");
      return true;
    } catch (Exception e) {
  ...
  public final boolean alterDomainFilterTable(SQLiteDatabase sQLiteDatabase) {
    try {
      ...
      sQLiteDatabase.execSQL(
          "CREATE TABLE DomainFilterTable (adminUid INTEGER, packageName TEXT, signature TEXT, dns1"
              + " TEXT, dns2 TEXT, networkId INT, PRIMARY KEY (adminUid, packageName) FOREIGN KEY"
              + " (adminUid) REFERENCES ADMIN(adminUid) ON DELETE CASCADE ON UPDATE CASCADE)");
      sQLiteDatabase.execSQL(
          "INSERT INTO DomainFilterTable SELECT adminUid, packageName, signature, dns1, dns2,"
              + " networkId from DomainFilterTable_tmp");
  ...
  public final boolean insertDomainListData(
  ...
  */
      ...
      try {
        cursor =
            sQLiteDatabase.rawQuery(
                "SELECT adminUid, packageName, blacklist, whitelist FROM DomainFilterTable", null);
        boolean z3 = true;
          ...
          try {
            ...
            z =
                TextUtils.isEmpty(string3)
                    || insertDomainListData(
                        sQLiteDatabase,
                        i,
                        string,
                        Arrays.asList(string3.split(KnoxVpnFirewallHelper.DELIMITER)),
                        "blacklist");
            ...
            z3 =
                (TextUtils.isEmpty(string2)
                        ...
                        || insertDomainListData(
                            ...
                            i,
                            string,
                            Arrays.asList(string2.split(KnoxVpnFirewallHelper.DELIMITER)),
                            "whitelist"))
                    ? z
                    : false;
  ...
  public static void createTable(SQLiteDatabase sQLiteDatabase, Table table) {
    ...
    String str2;
    String format =
        String.format("CREATE TABLE %s (%s", table.mTableName, table.buildTableColumns());
    ...
    if (str3 != null
      ...
      format =
          String.format(
              "%s FOREIGN KEY (%s) REFERENCES %s(%s) ON DELETE CASCADE ON UPDATE CASCADE",
              format, str2, str3, str);
    }
  ...
  */
  public void doTablesCreationOrUpdate(final SQLiteDatabase sQLiteDatabase) {
    ...
    try {
      try {
        ...
        new EnterpriseXmlParser(
                xmlResourceParser,
                new TableCallback() { // from class:
                                      // com.android.server.enterprise.storage.EdmStorageHelper.1
                  ...
                  public void onTableFound(Table table) {
                    ...
                    try {
                      if (!EdmStorageHelper.isTableExists(sQLiteDatabase, str)) {
                        ...
                        ArrayList missingColumns =
                            EdmStorageHelper.getMissingColumns(sQLiteDatabase, str, table);
                        Iterator it = missingColumns.iterator();
                        while (it.hasNext()) {
                          sQLiteDatabase.execSQL(
                              String.format(
                                  "ALTER TABLE %s ADD COLUMN %s;",
                                  str, ((Column) it.next()).getSQLDeclaration()));
                        }
                        Log.d(
                            "EdmStorageHelper",
                            String.format(
                                "onTableFound Altered Table %s with Columns %d",
                                str, Integer.valueOf(missingColumns.size())));
                      }
                    } catch (Exception e) {
  ...
  public void doCreationOrUpdatePostCommands(SQLiteDatabase sQLiteDatabase) {
    try {
      sQLiteDatabase.execSQL(
          "CREATE TRIGGER IF NOT EXISTS webFilterLoggingPolicy_TbSize  AFTER INSERT  ON"
              + " WebFilterLogTable WHEN (SELECT COUNT(*) FROM WebFilterLogTable) > 1000  BEGIN "
              + " DELETE FROM WebFilterLogTable WHERE id = (SELECT id FROM WebFilterLogTable ORDER"
              + " BY id LIMIT 1); END;  END;");
    } catch (Exception e) {
    ...
    try {
      sQLiteDatabase.execSQL(
          "CREATE TRIGGER IF NOT EXISTS bluetoothLoggingPolicy_TbSize  AFTER INSERT  ON"
              + " BluetoothLogTable WHEN (SELECT COUNT(*) FROM BluetoothLogTable) > 1000  BEGIN "
              + " DELETE FROM BluetoothLogTable WHERE id = (SELECT id FROM BluetoothLogTable ORDER"
              + " BY id LIMIT 1); END;  END;");
    } catch (Exception e2) {
    ...
    try {
      sQLiteDatabase.execSQL(
          "INSERT INTO EnumLSOItemTypes(Item_Type, Item_Description) VALUES (1, 'SpaceView');");
      sQLiteDatabase.execSQL(
          "INSERT INTO EnumLSOItemTypes(Item_Type, Item_Description) VALUES (2, 'TextView');");
      sQLiteDatabase.execSQL(
          "INSERT INTO EnumLSOItemTypes(Item_Type, Item_Description) VALUES (3, 'ImageView');");
      sQLiteDatabase.execSQL(
          "INSERT INTO EnumLSOItemTypes(Item_Type, Item_Description) VALUES (4, 'ContainerView');");
      sQLiteDatabase.execSQL(
          "INSERT INTO EnumLSOItemTypes(Item_Type, Item_Description) VALUES (5, 'CustomWidget');");
    ...
    try {
      ...
      sQLiteDatabase.execSQL(
          String.format(
              "INSERT INTO generic VALUES ('PlatformSoftwareVersion', '%s', %d);", str, 0));
    ...
    try {
      if (isTableExists(sQLiteDatabase, "APPLICATION_SIGNATURE")) {
        try {
          Cursor query =
              sQLiteDatabase.query("APPLICATION_SIGNATURE", null, null, null, null, null, null);
          try {
    ...
    try {
      sQLiteDatabase.execSQL(
          "CREATE TRIGGER IF NOT EXISTS  domainFilterReportTableCircularBuffer  AFTER INSERT  ON"
              + " DomainFilterReportTable WHEN (SELECT COUNT(*) FROM DomainFilterReportTable) >"
              + " 1000 BEGIN  DELETE FROM DomainFilterReportTable WHERE id = (SELECT id FROM"
  ...
  public static ArrayList getMissingColumns(
  ...
  public static void postAdminInfoTableCreate(SQLiteDatabase sQLiteDatabase) {
    try {
      ...
      if (isTableExists(sQLiteDatabase, "ADMIN")) {
        sQLiteDatabase.execSQL(
            "INSERT INTO ADMIN_INFO(adminUid,adminName,canRemove) SELECT * from ADMIN WHERE"
                + " adminUid!=1000"
                + KnoxVpnFirewallHelper.DELIMITER);
        Log.d(
            "EdmStorageHelper",
            "In postAdminInfoTableCreate - Start adding KnoxCustomManagerService.DB_UID to ADMIN"
                + " table...");
        sQLiteDatabase.execSQL("INSERT INTO ADMIN VALUES (1000,1000,0);");
        Log.d(
            "EdmStorageHelper",
            "In postAdminInfoTableCreate - Finished adding KnoxCustomManagerService.DB_UID to ADMIN"
                + " table");
      }
    } catch (Exception e) {
  ...
  public static void postAdminTableCreate(SQLiteDatabase sQLiteDatabase) {
    try {
      sQLiteDatabase.execSQL(
          "CREATE TRIGGER ADMIN_INFO_ONINSERT  AFTER INSERT  ON ADMIN_INFO BEGIN INSERT INTO ADMIN"
              + " VALUES (NEW.adminUid,NEW.adminUid,0, NEW.adminUid/100000); END;");
      if (getCount(sQLiteDatabase, "ADMIN_INFO", "adminUid!=0") > 0) {
        sQLiteDatabase.execSQL(
            "INSERT INTO ADMIN SELECT adminUid,adminUid,0,adminUid/100000 FROM ADMIN_INFO WHERE"
                + " adminUid!=0"
                + KnoxVpnFirewallHelper.DELIMITER);
      }
      sQLiteDatabase.execSQL(
          "CREATE TRIGGER ADMIN_INFO_ONUPDATE  UPDATE  OF adminUid ON ADMIN_INFO BEGIN UPDATE ADMIN"
              + " SET adminUid = NEW.adminUid WHERE adminUid = OLD.adminUid; END;");
      try {
    ...
    try {
      ...
      sQLiteDatabase.execSQL(
          "CREATE TRIGGER ADMIN_INFO_ONINSERT  AFTER INSERT  ON ADMIN_INFO BEGIN INSERT INTO ADMIN"
              + " VALUES (NEW.adminUid,NEW.adminUid,0, NEW.adminUid/100000); END;");
      sQLiteDatabase.execSQL(
          "CREATE TRIGGER IF NOT EXISTS ADMIN_INFO_ONUPDATE  UPDATE  OF adminUid ON ADMIN_INFO"
              + " BEGIN UPDATE ADMIN SET adminUid = NEW.adminUid WHERE adminUid = OLD.adminUid;"
              + " END;");
    } catch (Exception e) {
      Log.e(
          "EdmStorageHelper", "ADMIN_INFOUpdate of ADMIN_INFO_ONINSERT trigger has failed : " + e);
    }
  }
  ...
  public static boolean addISAPackageNameColumn(SQLiteDatabase sQLiteDatabase, Table table) {
    ...
    try {
      ...
      sQLiteDatabase.execSQL(
          "INSERT INTO "
              + table.mTableName
              + " (adminUid,fpBaseLined,packageName,fpCurrent,fpDirty,fpNewRow) SELECT"
              + " adminUid,fpBaseLined,packageName,fpCurrent,fpDirty,fpNewRow from "
              + sb2
              + KnoxVpnFirewallHelper.DELIMITER);
      StringBuilder sb4 = new StringBuilder();
      sb4.append("DROP TABLE ");
