...
public class ApplicationIconDb extends SQLiteOpenHelper {
    ...
    public boolean updateApplicationIcon(String str, byte[] bArr, int i) {
        ...
        try {
            Cursor query = writableDatabase.query("ApplicationIcon", new String[]{KnoxCustomManagerService.f1773ID, "nameowner"}, "pkgname = ? AND userid= ?", new String[]{str, String.valueOf(userId)}, null, null, null, "1");
            try {
                if (query == null) {
    ...
    */
    public boolean deleteApplicationIcon(String str, int i) {
        ...
        try {
            ...
            Cursor rawQuery = writableDatabase.rawQuery("SELECT * FROM ApplicationIcon WHERE pkgname = '" + str + "' AND nameowner = " + i, null);
            if (rawQuery != null) {
                try {
                    try {
                    } catch (Throwable th) {
                ...
                if (rawQuery.getCount() != 0) {
                    i2 = i;
                    str2 = "nameowner";
                    if (rawQuery.moveToFirst() && ((i3 = rawQuery.getInt(rawQuery.getColumnIndex("nameowner"))) == 0 || i3 == i)) {
            ...
            rawQuery = writableDatabase.rawQuery("SELECT * FROM ApplicationIcon WHERE pkgname = '" + str + "' AND userid = " + i2, null);
    ...
    */
    public boolean updateApplicationName(String str, String str2, int i) {
        ...
        try {
            ...
            Cursor rawQuery = writableDatabase.rawQuery("SELECT * FROM ApplicationIcon WHERE pkgname = '" + str + "' AND userid = " + userId, null);
            try {
    ...
    public ArrayList clearApplicationDataForUid(int i) {
        ...
        try {
            try {
                SQLiteDatabase writableDatabase = getWritableDatabase();
                if (writableDatabase != null) {
                    Cursor rawQuery = writableDatabase.rawQuery("SELECT * FROM ApplicationIcon WHERE nameowner = " + i, null);
                    try {
                        int columnIndex = rawQuery.getColumnIndex("pkgname");
                        if (rawQuery.moveToFirst()) {
                            do {
    ...
    public HashMap getApplicationIconChangedMap() {
        ...
        try {
            try {
                ...
                cursor = readableDatabase.rawQuery("SELECT * FROM ApplicationIcon WHERE imagedata IS NOT NULL", null);
                if (cursor != null && cursor.getCount() > 0) {
                    while (cursor.moveToNext()) {
                        if (cursor.getBlob(cursor.getColumnIndex("imagedata")) != null) {
                            ...
                            ((List) hashMap.get(Integer.valueOf(i))).add(cursor.getString(cursor.getColumnIndex("pkgname")));
                        }
                    }
                }
            } catch (Exception e) {
    ...
    public HashMap getApplicationNameChangedMap() {
        ...
        try {
            try {
                ...
                cursor = readableDatabase.rawQuery("SELECT * FROM ApplicationIcon WHERE newname IS NOT NULL", null);
                if (cursor != null && cursor.getCount() > 0) {
                    while (cursor.moveToNext()) {
    ...
    */
    public byte[] getApplicationIcon(String str, int i) {
        ...
        try {
            ...
            cursor = readableDatabase.rawQuery("SELECT * FROM ApplicationIcon WHERE pkgname = '" + str + "' AND userid = " + i, null);
            if (cursor != null) {
                try {
                    try {
