...
public class DexPolicy extends IDexPolicy.Stub implements EnterpriseServiceCallback {
    ...
    */
    public final void restoreNetworkStatus(ContextInfo contextInfo) {
        ...
        boolean z2 = false;
        try {
            z = this.mEdmStorageProvider.getBoolean(contextInfo.mCallerUid, "DEX_POLICY", "prevDataStatus");
        } catch (Exception e) {
            e = e;
            z = false;
        }
        try {
            try {
                z2 = this.mEdmStorageProvider.getBoolean(contextInfo.mCallerUid, "DEX_POLICY", "prevWifiStatus");
            } catch (Exception e2) {
                e = e2;
