...
public class MultiUserManagerService extends IMultiUserManager.Stub
  ...
  public int multipleUsersAllowed(ContextInfo contextInfo, boolean z) {
    ...
    int i =
        !this.mEdmStorageProvider
                .getBooleanList("MULTI_USER_MGMT", "multiUserAllowed")
                .contains(Boolean.FALSE)
            ? 1
            : 0;
    if (z && i == 0) {
      RestrictionToastManager.show(R.string.personal_apps_suspension_title);
    }
    return i;
  }
        ...
        if (createUser != null) {
          ...
          if (trim.length() <= 0) {
            this.mUserManager.setUserName(
                i2, this.mContext.getString(R.string.phoneTypeCallback, Integer.valueOf(i2)));
          }
          i = i2;
        }
      }
  ...
  public boolean isUserCreationAllowed(ContextInfo contextInfo, boolean z) {
    boolean z2 =
        !this.mEdmStorageProvider
            .getBooleanList("MULTI_USER_MGMT", "multiUserCreationAllowed")
            .contains(Boolean.FALSE);
    if (z && !z2) {
      RestrictionToastManager.show(R.string.phoneTypeAssistant);
    }
  ...
  public boolean isUserRemovalAllowed(ContextInfo contextInfo, boolean z) {
    boolean z2 =
        !this.mEdmStorageProvider
            .getBooleanList("MULTI_USER_MGMT", "multiUserRemovalAllowed")
            .contains(Boolean.FALSE);
    if (z && !z2) {
      RestrictionToastManager.show(R.string.phoneTypeCar);
    }
