...
public class PackageManagerAdapter implements IPackageManagerAdapter {
  ...
  public boolean clearUserData(String str, int i) {
    ...
    try {
      if (!ActivityManagerNative.getDefault()
          .clearApplicationUserData(validStr, false, clearUserDataObserver, i)) {
        Log.d(
            "PackageManagerAdapter",
            "Couldn't clear application user data for package: " + validStr);
        return false;
      }
      synchronized (clearUserDataObserver) {
        ...
        if (true != clearUserDataObserver.success
            || !clearUserDataObserver.packageName.equalsIgnoreCase(validStr)) {
          z = false;
        }
      }
      return z;
  ...
  public boolean setComponentEnabledSetting(
        ...
        if (i2 != i) {
          if (!isUserRunningAndUnlocked(i2)) {
            Log.v(
                "PackageManagerAdapter",
                String.format(
                    "cannot getAllProvidersForProfile for user %s because it is in locked state",
            ...
            Binder.withCleanCallingIdentity(
                new FunctionalUtils
                    .ThrowingSupplier() { // from class:
                                          // com.android.server.enterprise.adapterlayer.PackageManagerAdapter$$ExternalSyntheticLambda0
                  public final Object getOrThrow() {
                    Integer lambda$installExistingPackageAsUserForMDM$0;
                    lambda$installExistingPackageAsUserForMDM$0 =
                        PackageManagerAdapter.lambda$installExistingPackageAsUserForMDM$0(str, i);
  ...
  public static /* synthetic */ Integer lambda$installExistingPackageAsUserForMDM$0(
      String str, int i) {
    return Integer.valueOf(
        mIPackageManager.installExistingPackageAsUser(str, i, 4194304, 1, (List) null));
  }
  ...
  public static boolean getUserRestrictions(int i, String str, boolean z) {
    ...
    try {
      z2 =
          ((UserManager) mContext.getSystemService("user"))
              .getUserRestrictions(new UserHandle(i))
              .getBoolean(str, z);
    } catch (Exception e) {
  ...
  public final boolean isUserRunningAndUnlocked(int i) {
    return ((UserManager) mContext.getSystemService("user")).isUserRunning(i)
