...
public class KnoxMUMRCPPolicyService extends IRCPPolicy.Stub implements EnterpriseServiceCallback {
  ...
  public final String getDefaultRCPPolicy(int i, String str) {
    return ((SemPersonaManager) this.mContext.getSystemService("persona")).exists(i)
        ? "false"
        : "true";
  }
  ...
  public final String getPropertyValue(String str, int i, String str2, String str3) {
    ...
    if (str2 == null
        ...
        || str3.isEmpty()
        || i == 0
        || (adminLUidListAsUser = this.mEdmStorageProvider.getAdminLUidListAsUser(i)) == null
        || adminLUidListAsUser.isEmpty()) {
      return null;
    }
  ...
  public final List getListFromSyncPolicy(
      ContextInfo contextInfo, String str, String str2, String str3) {
    ContextInfo enforceSecurityPermission =
        enforceSecurityPermission(
            contextInfo,
            new ArrayList(Arrays.asList("com.samsung.android.knox.permission.KNOX_CONTAINER_RCP")));
    if (isEngMode) {
      Log.d(
          "KnoxMUMRCPPolicyService",
          "admin uid from context info: " + enforceSecurityPermission.mCallerUid);
    }
    if (str2 != null && !str2.isEmpty() && str3 != null && !str3.isEmpty()) {
      ...
      List valuesList =
          this.mEdmStorageProvider.getValuesList(str, new String[] {"name"}, contentValues);
      if (valuesList != null && valuesList.size() > 0) {
  ...
  public boolean setAllowChangeDataSyncPolicy(
      ContextInfo contextInfo, List list, String str, boolean z) {
  ...
  public boolean setNotificationSyncPolicy(
      ContextInfo contextInfo, List list, String str, String str2) {
  ...
  public List getPackagesFromNotificationSyncPolicy(
  ...
  public final int isMigrationStateSet(int i) {
    return Settings.System.getIntForUser(
        this.mContext.getContentResolver(), "rcp_profile_migration_completed", 0, i);
  }
  ...
  public boolean isMoveFilesToContainerAllowed(ContextInfo contextInfo) {
    ...
    if (isMigrationStateSet(contextInfo.mContainerId) == 0) {
      ...
      if (isEngMode) {
        Log.d(
            "KnoxMUMRCPPolicyService",
            "isMoveFilesToContainerAllowed: CONTAINER_ID - ADMIN_UID - CONTAINER_PROPERTY_NAME:"
                + contextInfo.mContainerId
                + " "
                + mUMContainerOwnerUid
                + " EnforceMoveFilesToContainer");
      }
      List valuesList =
          this.mEdmStorageProvider.getValuesList("CONTAINER_POLICY", strArr, contentValues);
      if (valuesList != null && valuesList.size() > 0) {
        Log.d(
            "KnoxMUMRCPPolicyService",
            "isMoveFilesToContainerAllowed: "
                + ((ContentValues) valuesList.get(0)).getAsBoolean("propertyValue"));
        String asString = ((ContentValues) valuesList.get(0)).getAsString("propertyValue");
        return asString != null && asString.equalsIgnoreCase("1");
      }
      Log.e(
          "KnoxMUMRCPPolicyService",
          "isMoveFilesToContainerAllowed failed to get value from DB > returning false");
      return false;
    }
    try {
      return getEDM()
          .getProfilePolicy()
          .getRestriction(contextInfo, "restriction_property_move_files_to_profile");
      ...
      Log.e(
          "KnoxMUMRCPPolicyService", "isMoveFilesToContainerAllowed : SecurityException occurred");
      return false;
    }
  }
  ...
  public boolean allowMoveFilesToContainer(ContextInfo contextInfo, boolean z) {
    ...
    boolean restriction =
        getEDM()
            .getProfilePolicy()
            .setRestriction(contextInfo, "restriction_property_move_files_to_profile", z);
    if (restriction) {
      long clearCallingIdentity = Binder.clearCallingIdentity();
      try {
        AuditLog.logAsUser(
  ...
  public boolean isMoveFilesToOwnerAllowed(ContextInfo contextInfo) {
    ...
    if (isMigrationStateSet(contextInfo.mContainerId) == 0) {
      ...
      if (z) {
        Log.d(
            "KnoxMUMRCPPolicyService",
            "isMoveFilesToOwnerAllowed: CONTAINER_ID - ADMIN_UID - CONTAINER_PROPERTY_NAME:"
                + contextInfo.mContainerId
                + " "
                + mUMContainerOwnerUid
                + " EnforceMoveFilesToOwner");
      }
      List valuesList =
          this.mEdmStorageProvider.getValuesList("CONTAINER_POLICY", strArr, contentValues);
      if (valuesList != null && valuesList.size() > 0) {
        if (z) {
          Log.d(
              "KnoxMUMRCPPolicyService",
              "isMoveFilesToOwnerAllowed: "
                  + ((ContentValues) valuesList.get(0)).getAsString("propertyValue"));
        }
        String asString = ((ContentValues) valuesList.get(0)).getAsString("propertyValue");
        return asString != null && asString.equalsIgnoreCase("1");
      }
      Log.e(
          "KnoxMUMRCPPolicyService",
          "isMoveFilesToOwnerAllowed failed to get value from DB > returning false");
      return false;
    }
    try {
      return getEDM()
          .getProfilePolicy()
          .getRestriction(contextInfo, "restriction_property_move_files_to_owner");
    } catch (NullPointerException unused) {
  ...
  public boolean allowMoveFilesToOwner(ContextInfo contextInfo, boolean z) {
    ...
    boolean restriction =
        getEDM()
            .getProfilePolicy()
            .setRestriction(contextInfo, "restriction_property_move_files_to_owner", z);
    if (restriction) {
      long clearCallingIdentity = Binder.clearCallingIdentity();
      try {
        AuditLog.logAsUser(
  ...
  public boolean isMoveAppsToContainerAllowed(ContextInfo contextInfo) {
    if (contextInfo != null) {
      ...
      if (z) {
        Log.d(
            "KnoxMUMRCPPolicyService",
            "getEnforceAuthForContainer: CONTAINER_ID - ADMIN_UID - CONTAINER_PROPERTY_NAME:"
                + contextInfo.mContainerId
                + " "
                + mUMContainerOwnerUid
                + " EnforceMoveAppsToContainer");
      }
      List valuesList =
          this.mEdmStorageProvider.getValuesList("CONTAINER_POLICY", strArr, contentValues);
      if (valuesList != null && valuesList.size() > 0) {
      ...
      Log.e(
          "KnoxMUMRCPPolicyService",
          "getEnforceAuthForContainer failed to get value from DB > returning true");
      return false;
    }
  ...
  public boolean allowMoveAppsToContainer(ContextInfo contextInfo, boolean z) {
    ...
    if (isEngMode) {
      Log.d(
          "KnoxMUMRCPPolicyService",
          "setEnforceAuthForContainer: CONTAINER_ID - ADMIN_UID - CONTAINER_PROPERTY_NAME:"
              + contextInfo.mContainerId
              + " "
    ...
    if (count > 0) {
      ...
      putValuesNoUpdate =
          this.mEdmStorageProvider.putValues("CONTAINER_POLICY", contentValues2, contentValues);
    } else {
      contentValues.put("propertyValue", Boolean.valueOf(z));
      putValuesNoUpdate =
          this.mEdmStorageProvider.putValuesNoUpdate("CONTAINER_POLICY", contentValues);
    }
    if (!putValuesNoUpdate) {
      Log.e(
          "KnoxMUMRCPPolicyService",
          "setEnforceAuthForContainer failed : result = " + putValuesNoUpdate);
      return false;
    }
  ...
  public boolean isShareClipboardDataToOwnerAllowed(ContextInfo contextInfo) {
    ...
    if (contextInfo != null
        && !userManager.hasUserRestriction(
            "no_cross_profile_copy_paste", new UserHandle(contextInfo.mContainerId))) {
      z = true;
    }
  ...
  public boolean allowShareClipboardDataToOwner(ContextInfo contextInfo, boolean z) {
    ...
    if (z) {
      sendPolicyUpdate =
          ContainerProxy.sendPolicyUpdate(
              "knox.container.proxy.POLICY_CLEAR_USER_RESTRICTION", bundle);
    } else {
      sendPolicyUpdate =
          ContainerProxy.sendPolicyUpdate(
    ...
    if (sendPolicyUpdate != null
        && sendPolicyUpdate.getInt("android.intent.extra.RETURN_RESULT", -1) == 0) {
  ...
  public void sendRCPPolicyChangeBroadcast(String str, String str2, int i) {
    Log.d(
        "KnoxMUMRCPPolicyService",
        "sendRCPPolicyChangedBroadcast , mAppName : "
            + str
            + " , mPolicyType : "
            + str2
            + " , mPersonaID : "
