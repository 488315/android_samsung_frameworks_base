...
public abstract class EnterpriseService {
  ...
  public static void addLazySystemService(
      final String str, EnterpriseServiceCallback enterpriseServiceCallback) {
    addPolicyService(str, enterpriseServiceCallback);
    sLazyServiceList.add(str);
    ServiceManager.addService(
        str,
        new IServiceCreator() { // from class:
                                // com.android.server.enterprise.EnterpriseService$$ExternalSyntheticLambda0
          public final IBinder createService(Context context) {
            IBinder lambda$addLazySystemService$0;
            lambda$addLazySystemService$0 =
                EnterpriseService.lambda$addLazySystemService$0(str, context);
  ...
  public static void wakeUpLazyServices() {
    for (String str : sLazyServiceList) {
      ...
      EnterpriseServiceCallback enterpriseServiceCallback =
          (EnterpriseServiceCallback) getPolicyService(str);
  ...
  public static void addSystemService(
      String str, EnterpriseServiceCallback enterpriseServiceCallback) {
  ...
  public static void invokeSystemReadyIfNeeded() {
    for (Map.Entry entry : getPolicyServices().entrySet()) {
      try {
        invokeSystemReadyIfNeeded(
            (EnterpriseServiceCallback) entry.getValue(), (String) entry.getKey());
      } catch (Exception e) {
        Log.e(
            "EnterpriseService",
            "invokeSystemReadyIfNeeded() failed in" + ((String) entry.getKey()),
            e);
      }
    }
  }
