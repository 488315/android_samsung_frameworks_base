...
public class HdmService extends IHdmManager.Stub implements EnterpriseServiceCallback {
  ...
  public final boolean isDeviceRootKeyValid(
  ...
  public final ContextInfo enforceHDMPermission(ContextInfo contextInfo) {
    return getEDM()
        .enforcePermissionByContext(contextInfo, "com.samsung.android.knox.permission.KNOX_HDM");
  }
    ...
    if (!z2 && bArr == null) {
      ...
      Log.i(
          ...
          "beforeSoftwareBlockResult: "
              + this.hdmSoftwareBlock.handleSoftwareBlockBefore(str.getBytes(defaultCharset)));
      hdmApplyPolicy =
          this.vendorController.hdmApplyPolicy(
              str.getBytes(defaultCharset), this.mSerialNumber, bytes, this.mHashImei, bytes2, z);
      Log.i(
          str2,
          "afterSoftwareBlockResult: "
              + this.hdmSoftwareBlock.handleSoftwareBlockAfter(hdmApplyPolicy));
      Binder.restoreCallingIdentity(clearCallingIdentity);
    }
      ...
      if (!z2 && bArr == null) {
        ...
        hdmGetId =
            this.vendorController.hdmGetId(
                str2.getBytes(Charset.defaultCharset()), str.getBytes(Charset.defaultCharset()), z);
        Binder.restoreCallingIdentity(clearCallingIdentity);
      }
      if (this.vendorController.hdmUnload() != 0) {
  ...
  public final synchronized String getHdmPolicy(String str, String str2) {
    ...
    byte[] hdmGetPolicy =
        this.vendorController.hdmGetPolicy(
            z, str.getBytes(Charset.defaultCharset()), str2.getBytes(Charset.defaultCharset()));
    if (this.vendorController.hdmUnload() != 0) {
      Log.d(str3, "hdm_unload failure");
    }
    if (!z) {
  ...
  public synchronized int syncSwBlockFromBoot() {
    ...
    return this.hdmSoftwareBlock.syncSwBlockFromBoot(
        getHdmPolicy(UUID.randomUUID().toString(), "HDMFW"));
  }

  public synchronized boolean isNFCBlockedByHDM(ContextInfo contextInfo) {
    Log.d(TAG, "isNFCBlockedByHDM() on HdmService.java: ");
    enforceHDMPermission(contextInfo);
    return this.hdmSoftwareBlock.isNFCBlockedByHDM(
        getHdmPolicy(UUID.randomUUID().toString(), "HDMFW"));
  }
