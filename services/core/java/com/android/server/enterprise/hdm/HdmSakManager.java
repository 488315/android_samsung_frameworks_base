...
public abstract class HdmSakManager {
  ...
  public static boolean isSupported(Context context) {
    try {
      boolean isSupportSecureKeyService =
          new SecureKeyGenerator().isSupportSecureKeyService(context);
  ...
  public static byte[] generateHdmKey() {
    try {
      ...
      Log.i(
          TAG,
          "serviceId: "
              + new String(generateKeyPair.getServiceID())
              + ", certLen: "
              + certificates.length
              + ", keyLen: "
              + serviceKey.length);
  ...
  public static byte[] getHashedUniqueNumber() {
    try {
      ...
      return hash(
          uniqueNumber != null ? uniqueNumber.getBytes(StandardCharsets.UTF_8) : null, null);
    } catch (Exception e) {
  ...
  public static byte[] getMacHash(Context context) {
    String factoryMacAddress =
        ((SemWifiManager) context.getSystemService("sem_wifi")).getFactoryMacAddress();
    return hash(
        factoryMacAddress != null ? factoryMacAddress.getBytes(StandardCharsets.UTF_8) : null,
        null);
  }
  ...
  public static String getUniqueNumber() {
    ...
    if (isExistFile("/sys/class/sec/ufs/un")) {
      ...
      read =
          isExistFile("/sys/block/mmcblk0/device/unique_number")
              ? read("/sys/block/mmcblk0/device/unique_number", "EMMC_UN")
              : null;
    }
    if (read != null) {
  ...
  public static boolean isWifiOnly() {
    return "wifi-only".equalsIgnoreCase(SystemProperties.get("ro.carrier", "").trim())
        || "yes".equalsIgnoreCase(SystemProperties.get("ro.radio.noril", "").trim());
  }
}
