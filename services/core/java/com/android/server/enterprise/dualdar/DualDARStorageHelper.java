...
public abstract class DualDARStorageHelper {
  public static int setDualDARProfile(Context context, Bundle bundle) {
    ...
    context.enforceCallingPermission(
        "com.samsung.android.knox.permission.KNOX_DUAL_DAR_INTERNAL", "dualdar storage denied");
    if (bundle == null) {
      return -1;
    }
    String string =
  ...
  public static Bundle getDualDARProfile(Context context) {
    ...
    List values =
        new EdmStorageProvider(context)
            .getValues(
                "DUAL_DAR_CONFIG",
                new String[] {
                  "configValue", "clientAppPackageName", "clientAppSignature", "clientAppLocation"
                },
                (ContentValues) null);
    ...
    Log.d(
        "DualDARStorageHelper",
        "getDualDARProfile() - isEnableDualDAR "
            + z
            + "package Name "
            + asString
            + " signature "
