...
public class Admin implements IObserver {
  ...
  public Admin(int i, Context context, String str) {
    ...
    try {
      this.mIsPseudoAdminOfOrganizationOwnedDevice =
          this.mEdmStorageProvider.checkPseudoAdminForUid(i);
      Log.d(
          "Admin",
    ...
    if (asInterface != null) {
      try {
        this.mIsProfileOwnerOfOrganizationOwnedDevice =
            asInterface.isProfileOwnerOfOrganizationOwnedDeviceMDM(UserHandle.getUserId(i));
        Log.d(
            "Admin",
        ...
        Log.e(
            "Admin",
            "Error on calling isProfileOwnerOfOrganizationOwnedDeviceMDM : " + e2.getMessage());
      }
    }
  }
  ...
  public void notifyReadyToDump(boolean z) {
    ArrayList arrayList;
    if (!z
        || (arrayList = (ArrayList) this.mLogWritter.getDumpFilesList()) == null
        || ((PartialFileNode) arrayList.get(0)).getFile() == null) {
      return;
    }
  ...
  public void notifyDumpFinished(boolean z, boolean z2) {
    ...
    if (((IPersonaManagerAdapter) AdapterRegistry.getAdapter(IPersonaManagerAdapter.class))
        .isLegacyContainer(UserHandle.getUserId(this.mUid))) {
    ...
    this.mContext.sendBroadcastAsUser(
        intent, new UserHandle(userId), "com.samsung.android.knox.permission.KNOX_AUDIT_LOG");
    try {
      ...
      this.mContext.sendBroadcastAsUser(
          intent2, new UserHandle(userId), "com.samsung.android.knox.permission.KNOX_AUDIT_LOG");
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
