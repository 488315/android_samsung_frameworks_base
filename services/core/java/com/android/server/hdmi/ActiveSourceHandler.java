...
public final class ActiveSourceHandler {
  ...
  public static ActiveSourceHandler create(
      HdmiCecLocalDeviceTv hdmiCecLocalDeviceTv, IHdmiControlCallback iHdmiControlCallback) {
  ...
  public void process(HdmiCecLocalDevice.ActiveSource activeSource, int i) {
    ...
    if (!hdmiCecLocalDeviceTv.isProhibitMode()) {
      HdmiCecLocalDevice.ActiveSource m29of =
          HdmiCecLocalDevice.ActiveSource.m29of(hdmiCecLocalDeviceTv.getActiveSource());
      hdmiCecLocalDeviceTv.updateActiveSource(activeSource, "ActiveSourceHandler");
      boolean z = this.mCallback == null;
      if (!m29of.equals(activeSource)) {
    ...
    if (activeSource2.logicalAddress == getSourceAddress()) {
      this.mService.sendCecCommand(
          HdmiCecMessageBuilder.buildActiveSource(
              activeSource2.logicalAddress, activeSource2.physicalAddress));
      hdmiCecLocalDeviceTv.updateActiveSource(activeSource2, "ActiveSourceHandler");
      invokeCallback(0);
      return;
    }
    hdmiCecLocalDeviceTv.startRoutingControl(
        activeSource.physicalAddress, activeSource2.physicalAddress, this.mCallback);
  }
