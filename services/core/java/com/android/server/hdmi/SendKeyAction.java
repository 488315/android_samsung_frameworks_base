...
public final class SendKeyAction extends HdmiCecFeatureAction {
    ...
    if (this.mTargetAddress == 5 && localDevice().getDeviceInfo().getLogicalAddress() != 0) {
      sendCommand(
          HdmiCecMessageBuilder.buildUserControlPressed(
              getSourceAddress(), this.mTargetAddress, androidKeyToCecKey),
          new HdmiControlService
              .SendMessageCallback() { // from class: com.android.server.hdmi.SendKeyAction.1
  ...
  public final void sendKeyUp() {
    if (HdmiCecKeycode.isVolumeKeycode(this.mLastKeycode)
        && localDevice().getService().isAbsoluteVolumeBehaviorEnabled()) {
      sendCommand(
          HdmiCecMessageBuilder.buildUserControlReleased(getSourceAddress(), this.mTargetAddress),
          new HdmiControlService
  ...
  public /* synthetic */ void lambda$sendKeyUp$0(int i) {
    sendCommand(
        HdmiCecMessageBuilder.buildGiveAudioStatus(
            getSourceAddress(), localDevice().findAudioReceiverAddress()));
  }
