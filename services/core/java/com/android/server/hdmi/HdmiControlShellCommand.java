...
public final class HdmiControlShellCommand extends ShellCommand {
  ...
  public IHdmiControlCallback.Stub mHdmiControlCallback =
      new IHdmiControlCallback
          .Stub() { // from class: com.android.server.hdmi.HdmiControlShellCommand.1
        public void onComplete(int i) {
          HdmiControlShellCommand.this
              .getOutPrintWriter()
              .println(" done (" + HdmiControlShellCommand.this.getResultString(i) + ")");
  ...
  public void onHelp() {
    ...
    outPrintWriter.println(
        "      Changes the number of messages that can be stored in dumpsys history to new_size");
  }
    ...
    if (nextArgRequired.equals("get")) {
      ...
      try {
        printWriter.println(
            nextArgRequired2
                + " = "
                + this.mBinderService.getCecSettingStringValue(nextArgRequired2));
      } catch (IllegalArgumentException unused) {
        printWriter.println(
            nextArgRequired2 + " = " + this.mBinderService.getCecSettingIntValue(nextArgRequired2));
      }
      return 0;
    }
  ...
  public final int historySize(PrintWriter printWriter) {
    ...
    if (nextArgRequired.equals("get")) {
      printWriter.println(
          "CEC dumpsys message history size = " + this.mBinderService.getMessageHistorySize());
      return 0;
    }
    if (nextArgRequired.equals("set")) {
      String nextArgRequired2 = getNextArgRequired();
      try {
        ...
        if (this.mBinderService.setMessageHistorySize(parseInt)) {
          ...
          printWriter.println(
              "Message history size not changed, was it lower than the minimum size?");
        }
        return 0;
        ...
        ? i != 1
            ? i != 2
                ? i != 3
                    ? i != 5
                        ? i != 6
                            ? i != 7 ? Integer.toString(i) : "Communication Failed"
                            : "Incorrect mode"
                        : "Exception"
