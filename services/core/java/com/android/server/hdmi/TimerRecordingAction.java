...
public class TimerRecordingAction extends HdmiCecFeatureAction {
  ...
  public final void sendTimerMessage() {
    ...
    if (i == 1) {
      buildSetDigitalTimer =
          HdmiCecMessageBuilder.buildSetDigitalTimer(
              getSourceAddress(), this.mRecorderAddress, this.mRecordSource);
    } else if (i == 2) {
      buildSetDigitalTimer =
          HdmiCecMessageBuilder.buildSetAnalogueTimer(
              getSourceAddress(), this.mRecorderAddress, this.mRecordSource);
    } else if (i == 3) {
      buildSetDigitalTimer =
          HdmiCecMessageBuilder.buildSetExternalTimer(
              getSourceAddress(), this.mRecorderAddress, this.mRecordSource);
    } else {
      m27tv().announceTimerRecordingResult(this.mRecorderAddress, 2);
      finish();
      return;
    }
    sendCommand(
        ...
        new HdmiControlService
            .SendMessageCallback() { // from class: com.android.server.hdmi.TimerRecordingAction.1
          @Override // com.android.server.hdmi.HdmiControlService.SendMessageCallback
          public void onSendCompleted(int i2) {
            if (i2 != 0) {
              TimerRecordingAction.this
                  .m27tv()
                  .announceTimerRecordingResult(TimerRecordingAction.this.mRecorderAddress, 1);
  ...
  public final boolean handleTimerStatus(HdmiCecMessage hdmiCecMessage) {
    ...
    if (params.length == 1 || params.length == 3) {
      m27tv().announceTimerRecordingResult(this.mRecorderAddress, bytesToInt(params));
  ...
  public final boolean handleFeatureAbort(HdmiCecMessage hdmiCecMessage) {
    ...
    m27tv().announceTimerRecordingResult(this.mRecorderAddress, 1);
    finish();
    return true;
  }
  ...
  public void handleTimerEvent(int i) {
    if (this.mState != i) {
      Slog.w(
          ...
          "Timeout in invalid state:[Expected:" + this.mState + ", Actual:" + i + "]");
      return;
    }
    m27tv().announceTimerRecordingResult(this.mRecorderAddress, 1);
    finish();
