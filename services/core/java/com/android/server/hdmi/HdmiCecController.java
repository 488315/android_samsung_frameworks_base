...
public final class HdmiCecController {
  ...
  public final Predicate mRemoteDeviceAddressPredicate =
      new Predicate() { // from class: com.android.server.hdmi.HdmiCecController.1
        @Override // java.util.function.Predicate
        public boolean test(Integer num) {
          return !HdmiCecController.this
              .mService
              .getHdmiCecNetwork()
              .isAllocatedLocalDeviceAddress(num.intValue());
        }
      };
  public final Predicate mSystemAudioAddressPredicate =
      new Predicate() { // from class: com.android.server.hdmi.HdmiCecController.2
        @Override // java.util.function.Predicate
        public boolean test(Integer num) {
  ...
  public ArrayBlockingQueue mMessageHistory =
      new ArrayBlockingQueue(FrameworkStatsLog.f446x58c6c32);
  ...
  public static HdmiCecController create(
      HdmiControlService hdmiControlService, HdmiCecAtomWriter hdmiCecAtomWriter) {
    Object[] objArr = 0;
    Object[] objArr2 = 0;
    HdmiCecController createWithNativeWrapper =
        createWithNativeWrapper(hdmiControlService, new NativeWrapperImplAidl(), hdmiCecAtomWriter);
    ...
    HdmiCecController createWithNativeWrapper2 =
        createWithNativeWrapper(hdmiControlService, new NativeWrapperImpl11(), hdmiCecAtomWriter);
  ...
  public void allocateLogicalAddress(
      final int i, final int i2, final AllocateAddressCallback allocateAddressCallback) {
  ...
  public final void handleAllocateLogicalAddress(
      final int i, int i2, final AllocateAddressCallback allocateAddressCallback) {
    final int i3;
    boolean z;
    assertRunOnIoThread();
    ...
    for (int i4 = 0; i4 < 16; i4++) {
      if (!arrayList.contains(Integer.valueOf(i4))
          && HdmiUtils.isEligibleAddressForDevice(i, i4)
          && HdmiUtils.isEligibleAddressForCecVersion(this.mService.getCecVersion(), i4)) {
    ...
    HdmiLogger.debug(
        "New logical address for device [%d]: [preferred:%d, assigned:%d]",
        Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(i3));
    if (allocateAddressCallback != null) {
      runOnServiceThread(
          new Runnable() { // from class: com.android.server.hdmi.HdmiCecController.4
            @Override // java.lang.Runnable
            public void run() {
  ...
  public void setHpdSignalType(int i, int i2) {
    assertRunOnServiceThread();
    HdmiLogger.debug(
        "setHpdSignalType: portId %b, signal %b", Integer.valueOf(i2), Integer.valueOf(i));
  ...
  public void pollDevices(
      HdmiControlService.DevicePollingCallback devicePollingCallback, int i, int i2, int i3) {
    assertRunOnServiceThread();
          ...
          new Runnable() { // from class: com.android.server.hdmi.HdmiCecController.5
            ...
            public void run() {
              ...
              HdmiCecController.this.runOnServiceThread(
                  new Runnable() { // from class: com.android.server.hdmi.HdmiCecController.5.1
                    @Override // java.lang.Runnable
                    public void run() {
                      RunnableC15435 runnableC15435 = RunnableC15435.this;
                      HdmiCecController.this.runDevicePolling(
                          i, list, i2, devicePollingCallback, list2);
                    }
                  });
            }
          });
    ...
    for (int i4 = 0; i4 < i3; i4++) {
      ...
      if (nativeSendCecCommand != 1) {
        HdmiLogger.warning(
            ...
            Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(nativeSendCecCommand));
      }
    }
    return false;
  }
  ...
  public void flush(final Runnable runnable) {
    assertRunOnServiceThread();
    runOnIoThread(
        new Runnable() { // from class: com.android.server.hdmi.HdmiCecController.6
          @Override // java.lang.Runnable
          public void run() {
  ...
  public void onReceiveCommand(HdmiCecMessage hdmiCecMessage) {
    ...
    if (!this.mService.isCecControlEnabled()
        && !HdmiCecMessage.isCecTransportMessage(hdmiCecMessage.getOpcode())) {
      HdmiLogger.warning(
          "Message " + hdmiCecMessage + " received when cec disabled", new Object[0]);
    }
    if (!this.mService.isAddressAllocated()
        || isAcceptableAddress(hdmiCecMessage.getDestination())) {
  ...
  public void sendCommand(
      ...
      final HdmiControlService.SendMessageCallback sendMessageCallback) {
    assertRunOnServiceThread();
    final ArrayList arrayList = new ArrayList();
    runOnIoThread(
        new Runnable() { // from class: com.android.server.hdmi.HdmiCecController.7
          @Override // java.lang.Runnable
          public void run() {
            ...
            byte[] buildBody =
                HdmiCecController.buildBody(hdmiCecMessage.getOpcode(), hdmiCecMessage.getParams());
            while (true) {
              nativeSendCecCommand =
                  HdmiCecController.this.mNativeWrapperImpl.nativeSendCecCommand(
                      hdmiCecMessage.getSource(), hdmiCecMessage.getDestination(), buildBody);
              if (nativeSendCecCommand == 0) {
                arrayList.add("ACK");
            ...
            if (nativeSendCecCommand != 0) {
              Slog.w(
                  "HdmiCecController",
                  "Failed to send " + hdmiCecMessage + " with errorCode=" + nativeSendCecCommand);
            }
            HdmiCecController.this.runOnServiceThread(
                new Runnable() { // from class: com.android.server.hdmi.HdmiCecController.7.1
                  ...
                  public void run() {
                    ...
                    RunnableC15457 runnableC15457 = RunnableC15457.this;
                    hdmiCecAtomWriter.messageReported(
                        hdmiCecMessage,
                        2,
                        HdmiCecController.this.getCallingUid(),
                        nativeSendCecCommand);
                    HdmiControlService.SendMessageCallback sendMessageCallback2 =
                        sendMessageCallback;
  ...
  public final void handleIncomingCecCommand(int i, int i2, byte[] bArr) {
    ...
    HdmiCecMessage build =
        HdmiCecMessage.build(i, i2, bArr[0], Arrays.copyOfRange(bArr, 1, bArr.length));
    ...
    this.mHdmiCecAtomWriter.messageReported(
        build, incomingMessageDirection(i, i2), getCallingUid());
  ...
  public final void handleHotplug(int i, boolean z) {
    assertRunOnServiceThread();
    HdmiLogger.debug(
        "Hotplug event:[port:%d, connected:%b]", Integer.valueOf(i), Boolean.valueOf(z));
  ...
  public final class NativeWrapperImplAidl implements NativeWrapper, IBinder.DeathRecipient {
    ...
    public boolean connectToHal() {
      IHdmiCec asInterface =
          IHdmiCec.Stub.asInterface(ServiceManager.getService(IHdmiCec.DESCRIPTOR + "/default"));
    ...
    public void nativeSetHpdSignalType(int i, int i2) {
      try {
        ...
        HdmiLogger.error(
            "Could not set HPD signal type for portId " + i2 + " to " + i + ". Error: ",
            Integer.valueOf(e.errorCode));
      } catch (RemoteException e2) {
        HdmiLogger.error(
            "Could not set HPD signal type for portId " + i2 + " to " + i + ". Exception: ",
            e2,
            new Object[0]);
      }
    }
    ...
    public int nativeGetHpdSignalType(int i) {
      try {
        ...
        HdmiLogger.error(
            "Could not get HPD signal type for portId " + i + ". Exception: ", e, new Object[0]);
        return 0;
      }
    }
  }
