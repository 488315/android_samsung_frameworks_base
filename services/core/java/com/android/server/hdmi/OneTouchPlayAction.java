...
public final class OneTouchPlayAction extends HdmiCecFeatureAction {
  ...
  public static OneTouchPlayAction create(
  ...
  public OneTouchPlayAction(
      HdmiCecLocalDevice hdmiCecLocalDevice, int i, IHdmiControlCallback iHdmiControlCallback) {
    this(
        hdmiCecLocalDevice,
        i,
        iHdmiControlCallback,
        hdmiCecLocalDevice.getDeviceInfo().getCecVersion() >= 6
            && getTargetCecVersion(hdmiCecLocalDevice, i) >= 6);
  }
  ...
  public boolean start() {
    ...
    boolean z =
        this.mIsCec20 && getTargetDevicePowerStatus(this.mSource, this.mTargetAddress, -1) == 0;
    setAndBroadcastActiveSource();
    if (shouldTurnOnConnectedAudioSystem()) {
      sendCommand(
          HdmiCecMessageBuilder.buildSystemAudioModeRequest(
              getSourceAddress(), 5, getSourcePath(), true));
    }
    if (!this.mIsCec20) {
      queryDevicePowerStatus();
    } else {
      int targetDevicePowerStatus =
          getTargetDevicePowerStatus(this.mSource, this.mTargetAddress, -1);
  ...
  public final void setAndBroadcastActiveSource() {
    this.mSource.mService.setAndBroadcastActiveSourceFromOneDeviceType(
        this.mTargetAddress, getSourcePath(), "OneTouchPlayAction#broadcastActiveSource()");
  ...
  public final void queryDevicePowerStatus() {
    sendCommand(
        HdmiCecMessageBuilder.buildGiveDevicePowerStatus(getSourceAddress(), this.mTargetAddress));
  }

  @Override // com.android.server.hdmi.HdmiCecFeatureAction
  public boolean processCommand(HdmiCecMessage hdmiCecMessage) {
    if (this.mState != 1
        || this.mTargetAddress != hdmiCecMessage.getSource()
        || hdmiCecMessage.getOpcode() != 144) {
      return false;
    }
  ...
  public final boolean shouldTurnOnConnectedAudioSystem() {
    ...
    return stringValue.equals("to_tv_and_audio_system")
        || stringValue.equals(INetd.IF_FLAG_BROADCAST);
  }

  public static int getTargetCecVersion(HdmiCecLocalDevice hdmiCecLocalDevice, int i) {
    HdmiDeviceInfo cecDeviceInfo =
        hdmiCecLocalDevice.mService.getHdmiCecNetwork().getCecDeviceInfo(i);
  ...
  public static int getTargetDevicePowerStatus(
      HdmiCecLocalDevice hdmiCecLocalDevice, int i, int i2) {
    HdmiDeviceInfo cecDeviceInfo =
        hdmiCecLocalDevice.mService.getHdmiCecNetwork().getCecDeviceInfo(i);
