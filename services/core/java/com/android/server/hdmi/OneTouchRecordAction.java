...
public class OneTouchRecordAction extends HdmiCecFeatureAction {
  ...
  public final void sendRecordOn() {
    sendCommand(
        HdmiCecMessageBuilder.buildRecordOn(
            getSourceAddress(), this.mRecorderAddress, this.mRecordSource),
        new HdmiControlService
            .SendMessageCallback() { // from class: com.android.server.hdmi.OneTouchRecordAction.1
          @Override // com.android.server.hdmi.HdmiControlService.SendMessageCallback
          public void onSendCompleted(int i) {
            if (i != 0) {
              OneTouchRecordAction.this
                  .m27tv()
                  .announceOneTouchRecordResult(OneTouchRecordAction.this.mRecorderAddress, 49);
  ...
  public boolean processCommand(HdmiCecMessage hdmiCecMessage) {
    if (this.mState == 1
        && this.mRecorderAddress == hdmiCecMessage.getSource()
        && hdmiCecMessage.getOpcode() == 10) {
  ...
  public final boolean handleRecordStatus(HdmiCecMessage hdmiCecMessage) {
    ...
    m27tv().announceOneTouchRecordResult(this.mRecorderAddress, b);
    Slog.i(
        "OneTouchRecordAction",
        "Got record status:" + ((int) b) + " from " + hdmiCecMessage.getSource());
    if (b == 1 || b == 2 || b == 3 || b == 4) {
      this.mState = 2;
  ...
  public void handleTimerEvent(int i) {
    ...
    m27tv().announceOneTouchRecordResult(this.mRecorderAddress, 49);
    finish();
  }
