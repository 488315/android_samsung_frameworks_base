...
public class SetAudioVolumeLevelDiscoveryAction extends HdmiCecFeatureAction {
  ...
  public SetAudioVolumeLevelDiscoveryAction(
  ...
  public boolean start() {
    sendCommand(
        SetAudioVolumeLevelMessage.build(getSourceAddress(), this.mTargetAddress, 127),
        new HdmiControlService
            .SendMessageCallback() { // from class:
                                     // com.android.server.hdmi.SetAudioVolumeLevelDiscoveryAction$$ExternalSyntheticLambda0
  ...
  public final boolean handleFeatureAbort(HdmiCecMessage hdmiCecMessage) {
    if (hdmiCecMessage.getParams().length < 2
        || (hdmiCecMessage.getParams()[0] & 255) != 115
        || hdmiCecMessage.getSource() != this.mTargetAddress) {
      return false;
    }
        ...
        cecDeviceInfo.toBuilder()
            .setDeviceFeatures(
                cecDeviceInfo.getDeviceFeatures().toBuilder()
                    .setSetAudioVolumeLevelSupport(i)
                    .build())
            .build());
    return true;
  }
