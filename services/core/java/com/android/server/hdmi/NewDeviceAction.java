...
public final class NewDeviceAction extends HdmiCecFeatureAction {
  ...
  public final void requestOsdName(boolean z) {
    ...
    sendCommand(
        HdmiCecMessageBuilder.buildGiveOsdNameCommand(
            getSourceAddress(), this.mDeviceLogicalAddress));
  ...
  public final void addDeviceInfo() {
    if (!localDevice()
        .mService
        .getHdmiCecNetwork()
        .isInDeviceList(this.mDeviceLogicalAddress, this.mDevicePhysicalAddress)) {
      Slog.w(
          "NewDeviceAction",
          String.format(
              "Device not found (%02x, %04x)",
              Integer.valueOf(this.mDeviceLogicalAddress),
              Integer.valueOf(this.mDevicePhysicalAddress)));
      return;
    }
    ...
    HdmiDeviceInfo build =
        HdmiDeviceInfo.cecDeviceBuilder()
            .setLogicalAddress(this.mDeviceLogicalAddress)
            .setPhysicalAddress(this.mDevicePhysicalAddress)
            .setPortId(m27tv().getPortId(this.mDevicePhysicalAddress))
    ...
    m27tv().processDelayedMessages(this.mDeviceLogicalAddress);
    if (HdmiUtils.isEligibleAddressForDevice(5, this.mDeviceLogicalAddress)) {
      m27tv().onNewAvrAdded(build);
    }
  }
  ...
  public boolean isActionOf(HdmiCecLocalDevice.ActiveSource activeSource) {
    return this.mDeviceLogicalAddress == activeSource.logicalAddress
        && this.mDevicePhysicalAddress == activeSource.physicalAddress;
  }
}
