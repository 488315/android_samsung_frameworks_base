...
public final class DeviceSelectActionFromTv extends HdmiCecFeatureAction {
  ...
  public DeviceSelectActionFromTv(
    ...
    this(
        ...
        hdmiCecLocalDeviceTv.getDeviceInfo().getCecVersion() >= 6
    ...
    sendCommand(
        ...
        new HdmiControlService
            .SendMessageCallback() { // from class:
                                     // com.android.server.hdmi.DeviceSelectActionFromTv.1
  ...
  public void handleTimerEvent(int i) {
    ...
    if (i2 == 1) {
      if (m27tv().isPowerStandbyOrTransient()) {
  ...
  public final void sendSetStreamPath() {
    m27tv().getActiveSource().invalidate();
    m27tv().setActivePath(this.mTarget.getPhysicalAddress());
    sendCommand(
        HdmiCecMessageBuilder.buildSetStreamPath(
            getSourceAddress(), this.mTarget.getPhysicalAddress()));
  }
}
