...
public final class SystemAudioAutoInitiationAction extends HdmiCecFeatureAction {
  ...
  public final void sendGiveSystemAudioModeStatus() {
    sendCommand(
        HdmiCecMessageBuilder.buildGiveSystemAudioModeStatus(getSourceAddress(), this.mAvrAddress),
        new HdmiControlService
            .SendMessageCallback() { // from class:
                                     // com.android.server.hdmi.SystemAudioAutoInitiationAction.1
  ...
  public boolean processCommand(HdmiCecMessage hdmiCecMessage) {
    if (this.mState != 1
        || this.mAvrAddress != hdmiCecMessage.getSource()
        || hdmiCecMessage.getOpcode() != 126) {
      return false;
    }
  ...
  public final void handleSystemAudioModeStatusMessage(boolean z) {
    ...
    boolean isSystemAudioControlFeatureEnabled = m27tv().isSystemAudioControlFeatureEnabled();
    if (z != isSystemAudioControlFeatureEnabled) {
      addAndStartAction(
          new SystemAudioActionFromTv(
              m27tv(), this.mAvrAddress, isSystemAudioControlFeatureEnabled, null));
    } else {
      m27tv().setSystemAudioMode(isSystemAudioControlFeatureEnabled);
    }
    finish();
  }
  ...
  public final void handleSystemAudioModeStatusTimeout() {
    if (!canChangeSystemAudio()) {
      ...
      addAndStartAction(
          new SystemAudioActionFromTv(
              m27tv(), this.mAvrAddress, m27tv().isSystemAudioControlFeatureEnabled(), null));
      finish();
    }
  }

  public final boolean canChangeSystemAudio() {
    return (m27tv().hasAction(SystemAudioActionFromTv.class)
            || m27tv().hasAction(SystemAudioActionFromAvr.class))
        ? false
        : true;
  }
}
