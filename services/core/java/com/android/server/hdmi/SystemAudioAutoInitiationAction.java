...
public final class SystemAudioAutoInitiationAction extends HdmiCecFeatureAction {
    ...
    public final void handleSystemAudioModeStatusMessage(boolean z) {
        ...
        if (z != isSystemAudioControlFeatureEnabled) {
            addAndStartAction(new SystemAudioActionFromTv(m27tv(), this.mAvrAddress, isSystemAudioControlFeatureEnabled, null));
        } else {
    ...
    public final void handleSystemAudioModeStatusTimeout() {
        if (!canChangeSystemAudio()) {
            ...
            addAndStartAction(new SystemAudioActionFromTv(m27tv(), this.mAvrAddress, m27tv().isSystemAudioControlFeatureEnabled(), null));
            finish();
        }
    }

    public final boolean canChangeSystemAudio() {
        return (m27tv().hasAction(SystemAudioActionFromTv.class) || m27tv().hasAction(SystemAudioActionFromAvr.class)) ? false : true;
    }
}
