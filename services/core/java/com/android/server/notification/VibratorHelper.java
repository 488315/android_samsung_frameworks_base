...
public final class VibratorHelper {
  ...
  public VibratorHelper(Context context) {
    ...
    this.mDefaultPattern =
        getLongArray(resources, R.array.config_serviceStateLocationAllowedPackages, 17, jArr);
    this.mFallbackPattern = getLongArray(context.getResources(), 17236258, 17, jArr);
    this.mDefaultPwlePattern =
        getFloatArray(context.getResources(), R.array.config_sfps_enroll_stage_thresholds);
  ...
  public static VibrationEffect createPwleWaveformVibration(float[] fArr, boolean z) {
    ...
    try {
      ...
      if (length != 0 && length % 3 == 0) {
        ...
        return z
            ? VibrationEffect.startComposition().repeatEffectIndefinitely(build).compose()
            : build;
      }
      return null;
    } catch (IllegalArgumentException unused) {
      Slog.e(
          "NotificationVibratorHelper",
          "Error creating vibration PWLE waveform with pattern: " + Arrays.toString(fArr));
      return null;
    }
  }
  ...
  public VibrationEffect createFallbackVibration(boolean z) {
    ...
    return (!this.mVibrator.hasFrequencyControl()
            || (createPwleWaveformVibration =
                    createPwleWaveformVibration(this.mFallbackPwlePattern, z))
                == null)
        ? createWaveformVibration(this.mFallbackPattern, z)
        : createPwleWaveformVibration;
  }
  ...
  public VibrationEffect createDefaultVibration(boolean z) {
    ...
    return (!this.mVibrator.hasFrequencyControl()
            || (createPwleWaveformVibration =
                    createPwleWaveformVibration(this.mDefaultPwlePattern, z))
                == null)
        ? createWaveformVibration(this.mDefaultPattern, z)
        : createPwleWaveformVibration;
  }
