...
public abstract class ZenLog {
  ...
  public static void traceSetRingerModeExternal(int i, int i2, String str, int i3, int i4) {
    append(
        ...
        str
            + ",e:"
            + ringerModeToString(i)
            + "->"
            + ringerModeToString(i2)
            + ",i:"
            + ringerModeToString(i3)
            + "->"
            + ringerModeToString(i4));
  }
  ...
  public static void traceSubscribe(
      Uri uri, IConditionProvider iConditionProvider, RemoteException remoteException) {
    append(9, uri + "," + subscribeResult(iConditionProvider, remoteException));
  }

  public static void traceUnsubscribe(
      Uri uri, IConditionProvider iConditionProvider, RemoteException remoteException) {
    append(10, uri + "," + subscribeResult(iConditionProvider, remoteException));
  }

  public static void traceConfig(
      String str, ZenModeConfig zenModeConfig, ZenModeConfig zenModeConfig2) {
  ...
  public static void traceEffectsSuppressorChanged(List list, List list2, long j) {
    append(
        ...
        "suppressed effects:"
            + j
            + ","
            + componentListToString(list)
            + "->"
            + componentListToString(list2));
  }
  ...
  public static String subscribeResult(
      IConditionProvider iConditionProvider, RemoteException remoteException) {
    return iConditionProvider == null
        ? "no provider"
  ...
  public static String hintsToString(int i) {
    return i != 0
        ? i != 1
            ? i != 2
                ? i != 4 ? Integer.toString(i) : "disable_call_effects"
                : "disable_notification_effects"
            : "disable_effects"
        : "none";
  }
