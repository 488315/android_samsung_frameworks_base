...
public class BadgeExtractor implements NotificationSignalExtractor {
  ...
  public RankingReconsideration process(NotificationRecord notificationRecord) {
    ...
    if (!badgingEnabled || !canShowBadge) {
      ...
      notificationRecord.setShowBadge(
          notificationRecord.getChannel().canShowBadge() && canShowBadge);
    } else {
      notificationRecord.setShowBadge(canShowBadge);
    }
    if (notificationRecord.isIntercepted()
        && (notificationRecord.getSuppressedVisualEffects() & 64) != 0) {
      notificationRecord.setShowBadge(false);
    }
    ...
    if (this.mConfig.isMediaNotificationFilteringEnabled()
        && notificationRecord.getNotification().isMediaNotification()) {
      notificationRecord.setShowBadge(false);
    }
    return null;
  }
