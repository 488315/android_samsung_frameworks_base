...
public class CalendarTracker {
  ...
  public final ContentObserver mObserver =
      new ContentObserver(null) { // from class: com.android.server.notification.CalendarTracker.1
    ...
    try {
      try {
        cursor2 =
            this.mUserContext
                .getContentResolver()
                .query(build, INSTANCE_PROJECTION, null, null, "begin ASC");
        try {
          try {
            ...
            while (cursor2 != null) {
              ...
              try {
                  ...
                  try {
                    try {
                      ...
                      Log.d(
                          "ConditionProviders.CT",
                          String.format(
                              "title=%s time=%s-%s vis=%s availability=%s eventId=%s name=%s"
                                  + " owner=%s calId=%s canAccessCal=%s",
                              string,
                              new Date(j3),
                              new Date(j4),
                              Boolean.valueOf(z3),
                              availabilityToString(i3),
                              Integer.valueOf(i2),
                              string2,
                              string3,
                              Long.valueOf(j5),
                ...
                if (z3 && contains) {
                  ...
                  if ((eventInfo2.calName == null && eventInfo2.calendarId == null)
                      || Objects.equals(eventInfo2.calendarId, Long.valueOf(j5))
                      || Objects.equals(eventInfo2.calName, string2)) {
                    z2 = true;
      ...
      try {
        ...
        if (query != null && query.getCount() != 0) {
          ...
          while (query.moveToNext()) {
            ...
            if (DEBUG) {
              Log.d(
                  ...
                  ""
                      + String.format(
                          "status=%s, meetsReply=%s",
                          attendeeStatusToString(i4), Boolean.valueOf(meetsReply)));
            }
          ...
          if (DEBUG) {
            Log.d(
                "ConditionProviders.CT",
                "meetsAttendee took " + (System.currentTimeMillis() - currentTimeMillis));
          }
          return z;
        }
        boolean z2 = DEBUG;
  ...
  public final void setRegistered(boolean z) {
    ...
    if (this.mRegistered) {
      ...
      contentResolver.registerContentObserver(
          CalendarContract.Instances.CONTENT_URI, true, this.mObserver, userId);
      contentResolver.registerContentObserver(
          CalendarContract.Events.CONTENT_URI, true, this.mObserver, userId);
      contentResolver.registerContentObserver(
          CalendarContract.Calendars.CONTENT_URI, true, this.mObserver, userId);
    }
  }
