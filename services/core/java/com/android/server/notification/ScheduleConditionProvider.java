...
public class ScheduleConditionProvider extends SystemConditionProviderService {
            ...
            synchronized (ScheduleConditionProvider.this.mSubscriptions) {
              ...
              while (it.hasNext()) {
                ScheduleCalendar scheduleCalendar =
                    (ScheduleCalendar)
                        ScheduleConditionProvider.this.mSubscriptions.get((Uri) it.next());
  ...
  public void dump(PrintWriter printWriter, NotificationManagerService.DumpFilter dumpFilter) {
    ...
    synchronized (this.mSubscriptions) {
      for (Uri uri : this.mSubscriptions.keySet()) {
        printWriter.print("        ");
        printWriter.print(
            meetsSchedule((ScheduleCalendar) this.mSubscriptions.get(uri), currentTimeMillis)
                ? "* "
                : "  ");
    ...
    printWriter.println(
        "      snoozed due to alarm: "
            + TextUtils.join(KnoxVpnFirewallHelper.DELIMITER, this.mSnoozedForAlarm));
    SystemConditionProviderService.dumpUpcomingTime(
        printWriter, "mNextAlarmTime", this.mNextAlarmTime, currentTimeMillis);
  }
      ...
      for (Uri uri : this.mSubscriptions.keySet()) {
        Condition evaluateSubscriptionLocked =
            evaluateSubscriptionLocked(
                uri, (ScheduleCalendar) this.mSubscriptions.get(uri), currentTimeMillis, nextAlarm);
  ...
  public PendingIntent getPendingIntent(long j) {
    return PendingIntent.getBroadcast(
        this.mContext,
        1,
        new Intent(ACTION_EVALUATE).setPackage("android").addFlags(268435456).putExtra("time", j),
        201326592);
  }
