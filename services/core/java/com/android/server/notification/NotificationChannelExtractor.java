...
public class NotificationChannelExtractor implements NotificationSignalExtractor {
  ...
  public RankingReconsideration process(NotificationRecord notificationRecord) {
    ...
    notificationRecord.updateNotificationChannel(
        rankingConfig.getConversationNotificationChannel(
            notificationRecord.getSbn().getPackageName(),
            notificationRecord.getSbn().getUid(),
            notificationRecord.getChannel().getId(),
            notificationRecord.getSbn().getShortcutId(),
            true,
            false));
    return null;
  }
