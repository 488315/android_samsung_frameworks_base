...
public class EventConditionProvider extends SystemConditionProviderService {
  ...
  public final CalendarTracker.Callback mTrackerCallback =
      new CalendarTracker
          .Callback() { // from class: com.android.server.notification.EventConditionProvider.2
        ...
        public void onChanged() {
          ...
          EventConditionProvider.this.mWorker.removeCallbacks(
              EventConditionProvider.this.mEvaluateSubscriptionsW);
          EventConditionProvider.this.mWorker.postDelayed(
              EventConditionProvider.this.mEvaluateSubscriptionsW, 2000L);
        }
      };
  public final BroadcastReceiver mReceiver =
      new BroadcastReceiver() { // from class:
                                // com.android.server.notification.EventConditionProvider.3
  ...
  public final Runnable mEvaluateSubscriptionsW =
  ...
  public void dump(PrintWriter printWriter, NotificationManagerService.DumpFilter dumpFilter) {
    ...
    SystemConditionProviderService.dumpUpcomingTime(
        printWriter, "mNextAlarmTime", this.mNextAlarmTime, System.currentTimeMillis());
  ...
  public final void reloadTrackers() {
    ...
    for (UserHandle userHandle : UserManager.get(this.mContext).getUserProfiles()) {
      Context contextForUser =
          userHandle.isSystem() ? this.mContext : getContextForUser(this.mContext, userHandle);
      if (contextForUser == null) {
        ...
        this.mTrackers.put(
            userHandle.getIdentifier(), new CalendarTracker(this.mContext, contextForUser));
      }
    }
    evaluateSubscriptions();
  }
  ...
  public final void evaluateSubscriptionsW() {
    ...
    synchronized (this.mSubscriptions) {
      ...
      for (int i2 = 0; i2 < this.mTrackers.size(); i2++) {
        ((CalendarTracker) this.mTrackers.valueAt(i2))
            .setCallback(this.mSubscriptions.isEmpty() ? null : this.mTrackerCallback);
      }
          ...
          if (tryParseEventConditionId.calName == null) {
            ...
            while (i3 < this.mTrackers.size()) {
              CalendarTracker.CheckEventResult checkEvent =
                  ((CalendarTracker) this.mTrackers.valueAt(i3))
                      .checkEvent(tryParseEventConditionId, currentTimeMillis);
              if (checkEventResult == null) {
                ...
                checkEventResult.recheckAt =
                    Math.min(checkEventResult.recheckAt, checkEvent.recheckAt);
              }
              i3++;
              it3 = it2;
            }
            ...
            it = it3;
            int resolveUserId =
                ZenModeConfig.EventInfo.resolveUserId(tryParseEventConditionId.userId);
            CalendarTracker calendarTracker = (CalendarTracker) this.mTrackers.get(resolveUserId);
            if (calendarTracker == null) {
              Slog.w(
                  "ConditionProviders.ECP", "No calendar tracker found for user " + resolveUserId);
              arrayList.add(createCondition(uri, 0));
            } else {
              checkEventResult =
                  calendarTracker.checkEvent(tryParseEventConditionId, currentTimeMillis);
            }
          }
    ...
    if (DEBUG) {
      Slog.d(
          "ConditionProviders.ECP",
          "evaluateSubscriptions took " + (System.currentTimeMillis() - currentTimeMillis));
    }
  }
  ...
  public final void rescheduleAlarm(long j, long j2) {
    ...
    if (DEBUG) {
      Slog.d(
          "ConditionProviders.ECP",
          String.format(
              "Scheduling evaluate for %s, in %s, now=%s",
              SystemConditionProviderService.m51ts(j2),
              SystemConditionProviderService.formatDuration(j2 - j),
              SystemConditionProviderService.m51ts(j)));
    }
  ...
  public PendingIntent getPendingIntent(long j) {
    return PendingIntent.getBroadcast(
        this.mContext,
        1,
        new Intent(ACTION_EVALUATE).addFlags(268435456).setPackage("android").putExtra("time", j),
        201326592);
  }
