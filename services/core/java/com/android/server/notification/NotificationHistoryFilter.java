...
public final class NotificationHistoryFilter {
  ...
  public boolean isFiltering() {
    return (getPackage() == null
            && getChannel() == null
            && this.mNotificationCount >= Integer.MAX_VALUE)
        ? false
        : true;
  }

  public boolean matchesPackageAndChannelFilter(
    ...
    if (getPackage().equals(historicalNotification.getPackage())) {
      return TextUtils.isEmpty(getChannel())
          || getChannel().equals(historicalNotification.getChannelId());
    }
    return false;
  }

  public boolean matchesPackageAndSbnKeyFilter(
      NotificationHistory.HistoricalNotification historicalNotification) {
    if (TextUtils.isEmpty(getPackage())) {
      return true;
    }
    if (getPackage().equals(historicalNotification.getPackage())) {
      return TextUtils.isEmpty(getSbnKey())
          || getSbnKey().equals(historicalNotification.getSbnKey());
    }
    return false;
  }
