...
public class RateEstimator {
  ...
  public float getRate(long j) {
    return this.mLastEventTime == null
        ? DisplayPowerController2.RATE_FROM_DOZE_TO_ON
        : (float) (1.0d / getInterarrivalEstimate(j));
  }

  public final double getInterarrivalEstimate(long j) {
    return (this.mInterarrivalTime * 0.8d)
        + (Math.max((j - this.mLastEventTime.longValue()) / 1000.0d, 5.0E-4d)
            * 0.19999999999999996d);
  }
}
