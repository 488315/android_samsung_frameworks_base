...
public class ShortcutHelper {
  ...
  public final LauncherApps.Callback mLauncherAppsCallback =
      new LauncherApps.Callback() { // from class: com.android.server.notification.ShortcutHelper.1
            ...
            for (String str2 : new HashSet(hashMap.keySet())) {
              ...
              if (!z) {
                ...
                if (hashMap.isEmpty()) {
                  ...
                  if (ShortcutHelper.this.mLauncherAppsCallbackRegistered
                      && ShortcutHelper.this.mActiveShortcutBubbles.isEmpty()) {
                    ShortcutHelper.this.mLauncherAppsService.unregisterCallback(
                        ShortcutHelper.this.mLauncherAppsCallback);
  ...
  public static boolean isConversationShortcut(
      ShortcutInfo shortcutInfo, ShortcutServiceInternal shortcutServiceInternal, int i) {
      ...
      if (str != null && str2 != null && userHandle != null) {
        try {
          ...
          if (isConversationShortcut(
              shortcutInfo, this.mShortcutServiceInternal, userHandle.getIdentifier())) {
            return shortcutInfo;
          }
          return null;
        } finally {
  ...
  public void cacheShortcut(ShortcutInfo shortcutInfo, UserHandle userHandle) {
    ...
    this.mShortcutServiceInternal.cacheShortcuts(
        ...
        Collections.singletonList(shortcutInfo.getId()),
        shortcutInfo.getUserId(),
        16384);
  }

  public void maybeListenForShortcutChangesForBubbles(
      NotificationRecord notificationRecord, boolean z, Handler handler) {
    String shortcutId =
        notificationRecord.getNotification().getBubbleMetadata() != null
            ? notificationRecord.getNotification().getBubbleMetadata().getShortcutId()
            : null;
    if (!z
        && !TextUtils.isEmpty(shortcutId)
        && notificationRecord.getShortcutInfo() != null
        && notificationRecord.getShortcutInfo().getId().equals(shortcutId)) {
      HashMap hashMap =
          (HashMap) this.mActiveShortcutBubbles.get(notificationRecord.getSbn().getPackageName());
      if (hashMap == null) {
        hashMap = new HashMap();
      }
