...
public class VisibilityExtractor implements NotificationSignalExtractor {
  ...
  public RankingReconsideration process(NotificationRecord notificationRecord) {
    ...
    if (userId == -1) {
      notificationRecord.setPackageVisibilityOverride(
          notificationRecord.getChannel().getLockscreenVisibility());
    } else {
      boolean canShowNotificationsOnLockscreen =
          this.mConfig.canShowNotificationsOnLockscreen(userId);
      ...
      if (!canShowNotificationsOnLockscreen || !adminAllowsKeyguardFeature || !z || !z2) {
        ...
        boolean canShowPrivateNotificationsOnLockScreen =
            this.mConfig.canShowPrivateNotificationsOnLockScreen(userId);
        ...
        boolean z4 =
            this.mPreferencesHelper.getLockScreenNotificationVisibilityForPackage(
                    notificationRecord.getSbn().getPackageName(), notificationRecord.getUid())
                != 0;
        if (!canShowPrivateNotificationsOnLockScreen
            || !adminAllowsKeyguardFeature2
            || !z3
            || !z4) {
