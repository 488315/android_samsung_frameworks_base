...
public class CountdownConditionProvider extends SystemConditionProviderService {
  ...
  public void onSubscribe(Uri uri) {
    ...
    if (this.mTime > 0) {
      ...
      if (z) {
        ...
        objArr[1] = ACTION;
        objArr[2] = SystemConditionProviderService.m51ts(j2);
        objArr[3] = Long.valueOf(this.mTime - currentTimeMillis);
        objArr[4] = relativeTimeSpanString;
        objArr[5] = SystemConditionProviderService.m51ts(currentTimeMillis);
        Slog.d(
            "ConditionProviders.CCP",
            String.format("%s %s for %s, %s in the future (%s), now=%s", objArr));
      }
    }
  }
  ...
  public PendingIntent getPendingIntent(Uri uri) {
    return PendingIntent.getBroadcast(
        this.mContext,
        100,
        new Intent(ACTION).setPackage("android").putExtra("condition_id", uri).setFlags(1073741824),
        201326592);
  }
      ...
      if (CountdownConditionProvider.ACTION.equals(intent.getAction())) {
        ...
        if (tryParseCountdownConditionId > 0) {
          CountdownConditionProvider.this.notifyCondition(
              CountdownConditionProvider.newCondition(
                  tryParseCountdownConditionId, isValidCountdownToAlarmConditionId, 0));
        }
      }
    }
  }
  ...
  public static String tryParseDescription(Uri uri) {
    ...
    return String.format(
        ...
        SystemConditionProviderService.m51ts(tryParseCountdownConditionId),
        Long.valueOf(tryParseCountdownConditionId - currentTimeMillis),
        DateUtils.getRelativeTimeSpanString(
            tryParseCountdownConditionId, currentTimeMillis, 60000L),
        SystemConditionProviderService.m51ts(currentTimeMillis));
  }
}
