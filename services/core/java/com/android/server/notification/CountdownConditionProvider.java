...
public class CountdownConditionProvider extends SystemConditionProviderService {
    ...
    public static String tryParseDescription(Uri uri) {
        ...
        long currentTimeMillis = System.currentTimeMillis();
        return String.format("Scheduled for %s, %s in the future (%s), now=%s", SystemConditionProviderService.m51ts(tryParseCountdownConditionId), Long.valueOf(tryParseCountdownConditionId - currentTimeMillis), DateUtils.getRelativeTimeSpanString(tryParseCountdownConditionId, currentTimeMillis, 60000L), SystemConditionProviderService.m51ts(currentTimeMillis));
    }
}
