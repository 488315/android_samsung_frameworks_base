...
public class EdgeLightingPolicyManager {
  ...
  public final String getDesktopSettingsValue(String str, String str2) {
    ...
    try {
      Bundle call =
          this.mContext
              .getContentResolver()
              .call(DESKTOP_SETTINGS_URI, "getSettings", (String) null, bundle);
      if (call != null) {
  ...
  public EdgeLightingPolicyManager(Context context) {
    ContentObserver contentObserver =
        new ContentObserver(
            this
                .mHandler) { // from class:
                             // com.android.server.notification.edgelighting.EdgeLightingPolicyManager.1
    ...
    ContentObserver contentObserver2 =
        new ContentObserver(
            this
                .mHandler) { // from class:
                             // com.android.server.notification.edgelighting.EdgeLightingPolicyManager.2
          @Override // android.database.ContentObserver
          public void onChange(boolean z, Uri uri) {
            ...
            edgeLightingPolicyManager.mIsTouchpadEnabled =
                "true"
                    .equals(
                        edgeLightingPolicyManager.getDesktopSettingsValue(
    ...
    this.mContext
        ...
        .registerContentObserver(
            Settings.Global.getUriFor("heads_up_notifications_enabled"), true, contentObserver);
    contentObserver2.onChange(true);
    this.mContext
        .getContentResolver()
        .registerContentObserver(DESKTOP_SETTINGS_URI, true, contentObserver2);
  ...
  public void updateEdgeLightingPolicyFromHost(EdgeLightingPolicy edgeLightingPolicy) {
    ...
    String stringForUser =
        Settings.Secure.getStringForUser(
            this.mContext.getContentResolver(), "edge_lighting_recommend_app_list", -2);
  ...
  public final boolean isValidApplication(String str, int i) {
    ...
    List queryIntentActivitiesAsUser =
        this.mPackageManager.queryIntentActivitiesAsUser(intent, 0, i);
  ...
  public EdgeLightingPolicyInfo getAcceptablePolicy(String str, int i) {
    ...
    if (this.mWhitePolicy.size() < 1
        || (policyInfoWithPackageName = this.mWhitePolicy.getPolicyInfoWithPackageName(str)) == null
        || (policyInfoWithPackageName.range & i) == 0) {
      return null;
    }
  ...
  public final EdgeLightingPolicyInfo getRejectablePolicy(String str, int i) {
    EdgeLightingPolicyInfo policyInfoWithPackageName;
    if (this.mBlackPolicy.size() < 1
        || (policyInfoWithPackageName = this.mBlackPolicy.getPolicyInfoWithPackageName(str)) == null
        || (policyInfoWithPackageName.range & i) == 0) {
      return null;
    }
    return policyInfoWithPackageName;
  ...
  public boolean isNotificationForEdgeLighting(StatusBarNotification statusBarNotification) {
    ...
    Slog.d(
        TAG,
        "isNotificationForEdgeLighting : small icon is null : "
  ...
  public Bundle putNotification(
    ...
    synchronized (this.mNotificationMap) {
      ...
      if (i2 == 0
          && (notificationData =
                  (NotificationData)
                      this.mNotificationMap.get(statusBarNotification.getPackageName()))
              != null) {
        ...
        if (iNotificationManager == null) {
          ...
          i3 =
              iNotificationManager.getLockScreenNotificationVisibilityForPackage(
                  statusBarNotification.getPackageName(), statusBarNotification.getUid());
        }
      } catch (RemoteException e) {
      ...
      if (statusBarNotification.getNotification().isGroupSummary()
          && (groupData = this.mNotificationGroup.getGroupData(statusBarNotification.getGroupKey()))
              != null
          && groupData.getLastChild() != null) {
        ...
        bundle.putCharSequence(
            "tickerText", groupData.getLastChild().mNotificationInfo.getCharSequence("tickerText"));
        sb2.append(bundle.getCharSequence("titleText"));
        sb2.append(" -> ");
        sb2.append(groupData.getLastChild().mNotificationInfo.getCharSequence("titleText"));
        sb2.append(" , ");
        bundle.putCharSequence(
            "titleText", groupData.getLastChild().mNotificationInfo.getCharSequence("titleText"));
        sb2.append(bundle.getCharSequence("text"));
        sb2.append(" -> ");
        sb2.append(groupData.getLastChild().mNotificationInfo.getCharSequence("text"));
        sb2.append(" , ");
        bundle.putCharSequence(
            "text", groupData.getLastChild().mNotificationInfo.getCharSequence("text"));
        sb2.append(bundle.getCharSequence("subText"));
        sb2.append(" -> ");
        sb2.append(groupData.getLastChild().mNotificationInfo.getCharSequence("subText"));
        bundle.putCharSequence(
            "subText", groupData.getLastChild().mNotificationInfo.getCharSequence("subText"));
        bundle.putString(
            "noti_key", groupData.getLastChild().mNotificationInfo.getString("noti_key"));
        bundle.putParcelable(
            "content_intent",
            groupData.getLastChild().mNotificationInfo.getParcelable("content_intent"));
        if (DEBUG) {
  ...
  public void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
    ...
    synchronized (this.mDisableRecords) {
      ...
      while (it.hasNext()) {
        ...
        printWriter.println(
            "  [DisableRecord] : " + disableRecord.packageName + " " + disableRecord.what);
      }
      printWriter.println("");
    }
    synchronized (this.mDisabledPackages) {
  ...
  public class NotificationGroup {
    ...
    public void remove(StatusBarNotification statusBarNotification) {
      synchronized (this.mGroupMap) {
        Slog.d(
            EdgeLightingPolicyManager.TAG + ":NotificationGroup",
            "remove : sbn : " + statusBarNotification);
        GroupNotificationData groupNotificationData =
            (GroupNotificationData) this.mGroupMap.get(getKey(statusBarNotification));
          ...
          if (groupNotificationData != null) {
            ...
            if (child != null) {
              notificationData.mNotificationInfo.putCharSequence(
                  "tickerText", child.mNotificationInfo.getCharSequence("tickerText"));
              notificationData.mNotificationInfo.putCharSequence(
                  "titleText", child.mNotificationInfo.getCharSequence("titleText"));
              notificationData.mNotificationInfo.putCharSequence(
                  "text", child.mNotificationInfo.getCharSequence("text"));
              notificationData.mNotificationInfo.putCharSequence(
                  "subText", child.mNotificationInfo.getCharSequence("subText"));
            }
          } else {
      ...
      synchronized (this.mGroupMap) {
        if (EdgeLightingPolicyManager.DEBUG) {
          for (Map.Entry entry : this.mGroupMap.entrySet()) {
            Slog.d(
                EdgeLightingPolicyManager.TAG + ".NotificationGroup",
                "group key = " + ((String) entry.getKey()) + " ,value= " + entry.getValue());
          }
        } else {
          Slog.d(
              EdgeLightingPolicyManager.TAG + ".NotificationGroup",
