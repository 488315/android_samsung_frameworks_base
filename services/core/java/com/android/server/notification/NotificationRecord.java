...
public final class NotificationRecord {
  ...
  public NotificationRecord(
    ...
    this.mTargetSdkVersion =
        ((PackageManagerInternal) LocalServices.getService(PackageManagerInternal.class))
            .getPackageTargetSdkVersion(statusBarNotification.getPackageName());
  ...
  public final boolean notificationSoundBlockedByMDM() {
    ...
    if (this.sbn.getPackageName() != null
        && (enterpriseDeviceManager = EnterpriseDeviceManager.getInstance(this.mContext)) != null
        && (applicationPolicy = enterpriseDeviceManager.getApplicationPolicy()) != null) {
      long clearCallingIdentity = Binder.clearCallingIdentity();
      try {
        if (!applicationPolicy.isStatusBarNotificationAllowedAsUser(
            this.sbn.getPackageName(), UserHandle.getUserId(this.sbn.getUid()))) {
          if (applicationPolicy.getApplicationNotificationModeAsUser(
                  UserHandle.getUserId(this.sbn.getUid()))
              == 4) {
  ...
  public final Light calculateLights() {
    int color =
        this.mContext
            .getResources()
            .getColor(R.color.datepicker_default_pressed_text_color_material_light);
    int integer =
        this.mContext
            .getResources()
            .getInteger(R.integer.config_displayWhiteBalanceColorTemperatureFilterHorizon);
    int integer2 =
        this.mContext
            .getResources()
            .getInteger(R.integer.config_displayWhiteBalanceColorTemperatureDefault);
    Light light =
        getChannel().shouldShowLights()
            ? new Light(
                getChannel().getLightColor() != 0 ? getChannel().getLightColor() : color,
                integer,
                integer2)
            : null;
    ...
    if ((notification.flags & 1) != 0) {
      return (notification.defaults & 4) != 0
          ? new Light(color, integer, integer2)
          : new Light(notification.ledARGB, notification.ledOnMS, notification.ledOffMS);
    }
    return null;
  }
  ...
  public final VibrationEffect calculateVibration() {
    ...
    if (getChannel().shouldVibrate()) {
      vibrationEffect =
          getChannel().getVibrationPattern() == null
              ? createDefaultVibration
              : VibratorHelper.createWaveformVibration(getChannel().getVibrationPattern(), z);
    } else {
  ...
  public final AudioAttributes calculateAttributes() {
    ...
    if (i >= 0 && i < AudioSystem.getNumStreamTypes()) {
      return new AudioAttributes.Builder()
          .setInternalLegacyStreamType(notification.audioStreamType)
          .build();
    }
  ...
  public final int calculateInitialImportance() {
    ...
    if (this.mPreChannelsNotification
        && (importance == -1000 || !getChannel().hasUserSetImportance())) {
  ...
  public void dump(PrintWriter printWriter, String str, Context context, boolean z) {
    ...
    printWriter.println(
        str2
            + "mSystemImportance="
            + NotificationListenerService.Ranking.importanceToString(this.mSystemImportance));
    printWriter.println(
        str2
            + "mAsstImportance="
            + NotificationListenerService.Ranking.importanceToString(this.mAssistantImportance));
    printWriter.println(
        str2
            + "mImportance="
    ...
    printWriter.println(
        str2
            + "mProposedImportance="
            + NotificationListenerService.Ranking.importanceToString(this.mProposedImportance));
    ...
    printWriter.println(
        str2
            ...
            + this.mRankingTimeMs
            + "("
            + simpleDateFormat.format(new Date(this.mRankingTimeMs))
            + ")");
    printWriter.println(
        str2
            + "mCreationTimeMs="
    ...
    if (this.mPreChannelsNotification) {
      printWriter.println(
          str2
              + String.format(
                  "defaults=0x%08x flags=0x%08x",
                  Integer.valueOf(notification.defaults), Integer.valueOf(notification.flags)));
    ...
    printWriter.println(
        str2 + "mSuggestionsGeneratedByAssistant=" + this.mSuggestionsGeneratedByAssistant);
    ...
    if (size2 != 0) {
      ...
      for (int i2 = 0; i2 < size2; i2++) {
        printWriter.println(
            String.format(
                "%s      [%d] %s",
                str2, Integer.valueOf(i2), ((CharSequence) this.mSmartReplies.get(i2)).toString()));
      }
    }
  }

  public final void dumpNotification(
    ...
    printWriter.println(sb2.toString());
    printWriter.println(
        str + "timeout=" + TimeUtils.formatForLogging(notification.getTimeoutAfter()));
    ...
    for (String str2 : notification.extras.keySet()) {
      ...
      if (obj == null) {
        ...
        if (z && (obj instanceof CharSequence) && shouldRedactStringExtra(str2)) {
          printWriter.print(
              String.format(" [length=%d]", Integer.valueOf(((CharSequence) obj).length())));
          ...
          if (!z) {
            for (int i2 = 0; i2 < length2; i2++) {
              ...
              printWriter.print(
                  String.format(
                      "%s      [%d] %s",
                      str, Integer.valueOf(i2), String.valueOf(Array.get(obj, i2))));
            }
          }
        } else {
  ...
  public final String toString() {
    return String.format(
        ...
        getSbn().getPackageName(),
        getSbn().getUser(),
        Integer.valueOf(getSbn().getId()),
        getSbn().getTag(),
      ...
      for (Adjustment adjustment : this.mAdjustments) {
        ...
        if (signals.containsKey("key_people")) {
          ...
          EventLogTags.writeNotificationAdjusted(
              getKey(), "key_people", stringArrayList.toString());
        }
        if (signals.containsKey("key_snooze_criteria")) {
          ArrayList parcelableArrayList =
              adjustment
                  .getSignals()
                  .getParcelableArrayList("key_snooze_criteria", SnoozeCriterion.class);
          setSnoozeCriteria(parcelableArrayList);
          EventLogTags.writeNotificationAdjusted(
              getKey(), "key_snooze_criteria", parcelableArrayList.toString());
        }
        ...
        if (signals.containsKey("key_user_sentiment")
            ...
            && (getChannel().getUserLockedFields() & 4) == 0) {
          setUserSentiment(adjustment.getSignals().getInt("key_user_sentiment", 0));
          EventLogTags.writeNotificationAdjusted(
              getKey(), "key_user_sentiment", Integer.toString(getUserSentiment()));
        }
        if (signals.containsKey("key_contextual_actions")) {
          setSystemGeneratedSmartActions(
              signals.getParcelableArrayList("key_contextual_actions", Notification.Action.class));
          EventLogTags.writeNotificationAdjusted(
              getKey(), "key_contextual_actions", getSystemGeneratedSmartActions().toString());
        }
        if (signals.containsKey("key_text_replies")) {
          setSmartReplies(signals.getCharSequenceArrayList("key_text_replies"));
          EventLogTags.writeNotificationAdjusted(
              getKey(), "key_text_replies", getSmartReplies().toString());
        }
  ...
  public void setVisibility(
    ...
    EventLogTags.writeNotificationVisibility(
        getKey(),
        z ? 1 : 0,
        getLifespanMs(currentTimeMillis),
        getFreshnessMs(currentTimeMillis),
        0,
        i);
      ...
      MetricsLogger.action(
          getLogMaker()
              ...
              .addTaggedData(
                  ...
                  Integer.valueOf(getInterruptionMs(currentTimeMillis))));
      MetricsLogger.histogram(
          this.mContext, "note_interruptive", getInterruptionMs(currentTimeMillis));
    }
  }
  ...
  public void calculateGrantableUris() {
    ...
    notification.visitUris(
        new Consumer() { // from class:
                         // com.android.server.notification.NotificationRecord$$ExternalSyntheticLambda0
      ...
      try {
        ...
        if (!z) {
          if (z2) {
            ...
            Log.w(
                "NotificationRecord", "Replacing " + uri + " from " + uid + ": " + e.getMessage());
          } else {
  ...
  public LogMaker getLogMaker(long j) {
    LogMaker addTaggedData =
        getSbn()
            ...
            .addTaggedData(793, Integer.valueOf(getLifespanMs(j)))
            .addTaggedData(795, Integer.valueOf(getFreshnessMs(j)))
            .addTaggedData(794, Integer.valueOf(getExposureMs(j)))
            .addTaggedData(NetworkConstants.ETHER_MTU, Integer.valueOf(getInterruptionMs(j)));
    int i = this.mImportanceExplanationCode;
    if (i != 0) {
  ...
  public boolean isConversation() {
    ...
    if (!notification.isStyle(Notification.MessagingStyle.class)) {
      return this.mPkgAllowedAsConvo
          && this.mTargetSdkVersion < 30
          && "msg".equals(getNotification().category);
    }
    if (this.mTargetSdkVersion >= 30
        && notification.isStyle(Notification.MessagingStyle.class)
        && ((shortcutInfo = this.mShortcutInfo) == null || isOnlyBots(shortcutInfo.getPersons()))) {
      return false;
    }
