    ...
    synchronized (this.mProxyByGroupTmp) {
      ...
      for (int i3 = 0; i3 < size; i3++) {
        ...
        NotificationRecord notificationRecord3 =
            (NotificationRecord) this.mProxyByGroupTmp.get(notificationRecord2.getGroupKey());
        ...
        objArr[1] =
            Character.valueOf(
                (!notificationRecord2.isRecentlyIntrusive()
                        || notificationRecord2.getImportance() <= 1)
                    ? '1'
                    : '0');
        ...
        notificationRecord2.setGlobalSortKey(
            TextUtils.formatSimple(
                "crtcl=0x%04x:intrsv=%c:grnk=0x%04x:gsmry=%c:%s:rnk=0x%04x", objArr));
      }
