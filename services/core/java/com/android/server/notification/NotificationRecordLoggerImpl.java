...
public class NotificationRecordLoggerImpl implements NotificationRecordLogger {
  ...
  public void logNotificationPosted(
      NotificationRecordLogger.NotificationReported notificationReported) {
    writeNotificationReportedAtom(notificationReported);
  }

  @Override // com.android.server.notification.NotificationRecordLogger
  public void logNotificationAdjusted(
      NotificationRecord notificationRecord, int i, int i2, InstanceId instanceId) {
    writeNotificationReportedAtom(
        new NotificationRecordLogger.NotificationReported(
            new NotificationRecordLogger.NotificationRecordPair(notificationRecord, null),
            NotificationRecordLogger.NotificationReportedEvent.NOTIFICATION_ADJUSTED,
            i,
            i2,
            instanceId));
  }

  public final void writeNotificationReportedAtom(
      NotificationRecordLogger.NotificationReported notificationReported) {
  ...
  public void log(UiEventLogger.UiEventEnum uiEventEnum, NotificationRecord notificationRecord) {
    ...
    this.mUiEventLogger.logWithInstanceId(
        uiEventEnum,
        notificationRecord.getUid(),
        notificationRecord.getSbn().getPackageName(),
        notificationRecord.getSbn().getInstanceId());
  }
