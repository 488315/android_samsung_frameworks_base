...
public class PulledStats {
  ...
  public ParcelFileDescriptor toParcelFileDescriptor(final int i) {
    final ParcelFileDescriptor[] createPipe = ParcelFileDescriptor.createPipe();
    if (i == 1) {
      new Thread(
          "NotificationManager pulled metric output") { // from class:
                                                        // com.android.server.notification.PulledStats.1
  ...
  public void dump(
      int i, PrintWriter printWriter, NotificationManagerService.DumpFilter dumpFilter) {
    if (i == 1) {
