...
public class CarrierMessagingClientServiceFinder extends AppServiceFinder {
  ...
  public CarrierMessagingClientServiceFinder(
      Context context, BiConsumer biConsumer, Handler handler) {
    super(context, biConsumer, handler);
    this.mRoleHolderChangedListener =
  ...
  public boolean isEnabled(AppBindingConstants appBindingConstants) {
    return appBindingConstants.SMS_SERVICE_ENABLED
        && this.mContext.getResources().getBoolean(R.bool.config_supportsBubble);
  }
  ...
  public String getTargetPackage(int i) {
    return (String)
        CollectionUtils.firstOrNull(
            this.mRoleManager.getRoleHoldersAsUser("android.app.role.SMS", UserHandle.of(i)));
  }

  @Override // com.android.server.appbinding.finders.AppServiceFinder
  public void startMonitoring() {
    this.mRoleManager.addOnRoleHoldersChangedListenerAsUser(
        BackgroundThread.getExecutor(), this.mRoleHolderChangedListener, UserHandle.ALL);
  }
