...
public class AppBindingService extends Binder {
  ...
  public AppBindingService(Injector injector, Context context) {
    ...
    this.mSettingsObserver =
        new ContentObserver(null) { // from class: com.android.server.appbinding.AppBindingService.1
    ...
    this.mPackageUserMonitor =
        new BroadcastReceiver() { // from class: com.android.server.appbinding.AppBindingService.2
          @Override // android.content.BroadcastReceiver
          public void onReceive(Context context2, Intent intent) {
            int intExtra = intent.getIntExtra("android.intent.extra.user_handle", -10000);
            if (intExtra == -10000) {
              Slog.w(
                  "AppBindingService", "Intent broadcast does not contain user handle: " + intent);
              return;
            }
            ...
            if (schemeSpecificPart == null) {
              Slog.w(
                  "AppBindingService", "Intent broadcast does not contain package name: " + intent);
              return;
            }
    ...
    arrayList.add(
        new CarrierMessagingClientServiceFinder(
            ...
            new BiConsumer() { // from class:
                               // com.android.server.appbinding.AppBindingService$$ExternalSyntheticLambda3
              @Override // java.util.function.BiConsumer
              public final void accept(Object obj, Object obj2) {
                AppBindingService.this.onAppChanged(
                    (AppServiceFinder) obj, ((Integer) obj2).intValue());
              }
            },
            handler));
  ...
  public final void onPhaseActivityManagerReady() {
    ...
    this.mContext.registerReceiverAsUser(
        this.mPackageUserMonitor, UserHandle.ALL, intentFilter, null, this.mHandler);
    IntentFilter intentFilter2 = new IntentFilter();
    intentFilter2.addAction("android.intent.action.USER_REMOVED");
    this.mContext.registerReceiverAsUser(
        this.mPackageUserMonitor, UserHandle.ALL, intentFilter2, null, this.mHandler);
    this.mContext
        .getContentResolver()
        .registerContentObserver(
            Settings.Global.getUriFor("app_binding_constants"), false, this.mSettingsObserver);
    refreshConstants();
  }

  public final void refreshConstants() {
    String globalSettingString =
        this.mInjector.getGlobalSettingString(
            this.mContext.getContentResolver(), "app_binding_constants");
    ...
    synchronized (this.mLock) {
      forAllAppsLocked(
          new Consumer() { // from class:
                           // com.android.server.appbinding.AppBindingService$$ExternalSyntheticLambda1
  ...
  public final void onUserRemoved(final int i) {
    synchronized (this.mLock) {
      forAllAppsLocked(
          new Consumer() { // from class:
                           // com.android.server.appbinding.AppBindingService$$ExternalSyntheticLambda2
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
  ...
  public final AppServiceConnection findConnectionLock(int i, AppServiceFinder appServiceFinder) {
    for (int i2 = 0; i2 < this.mConnections.size(); i2++) {
      ...
      if (appServiceConnection.getUserId() == i
          && appServiceConnection.getFinder() == appServiceFinder) {
    ...
    for (int i2 = 0; i2 < this.mApps.size(); i2++) {
      ...
      if (appServiceFinder == null || appServiceFinder == appServiceFinder2) {
        ...
        ServiceInfo findService =
            appServiceFinder2.findService(i, this.mIPackageManager, this.mConstants);
        if (findService != null) {
  ...
  public final void unbindServicesLocked(int i, AppServiceFinder appServiceFinder, String str) {
    for (int size = this.mConnections.size() - 1; size >= 0; size--) {
      ...
      if (appServiceConnection.getUserId() == i
          && (appServiceFinder == null || appServiceConnection.getFinder() == appServiceFinder)) {
  ...
  public class AppServiceConnection extends PersistentConnection {
    ...
    public AppServiceConnection(
        ...
        Handler handler,
        AppServiceFinder appServiceFinder,
