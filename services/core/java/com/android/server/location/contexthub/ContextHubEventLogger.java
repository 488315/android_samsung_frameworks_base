...
public class ContextHubEventLogger {
  ...
  public class ContextHubRestartEvent extends ContextHubEventBase {
    ...
    public String toString() {
      return ContextHubServiceUtil.formatDateFromTimestamp(this.timeStampInMs)
          + ": ContextHubRestartEvent[hubId = "
          + this.contextHubId
          + ']';
    }
  }
  ...
  public synchronized void logNanoappLoad(int i, long j, int i2, long j2, boolean z) {
    NanoappLoadEvent nanoappLoadEvent =
        new NanoappLoadEvent(System.currentTimeMillis(), i, j, i2, j2, z);
  ...
  public synchronized void logNanoappUnload(int i, long j, boolean z) {
    NanoappUnloadEvent nanoappUnloadEvent =
        new NanoappUnloadEvent(System.currentTimeMillis(), i, j, z);
    if (!this.mNanoappUnloadEventQueue.add(nanoappUnloadEvent)) {
      Log.e(
          "ContextHubEventLogger",
          "Unable to add nanoapp unload event to queue: " + nanoappUnloadEvent);
    }
  }
  ...
  public synchronized void logContextHubRestart(int i) {
    ContextHubRestartEvent contextHubRestartEvent =
        new ContextHubRestartEvent(System.currentTimeMillis(), i);
    if (!this.mContextHubRestartEventQueue.add(contextHubRestartEvent)) {
      Log.e(
          "ContextHubEventLogger",
          "Unable to add Context Hub restart event to queue: " + contextHubRestartEvent);
    }
  }
