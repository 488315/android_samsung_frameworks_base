...
public class LocationFudger {
  ...
  public LocationResult createCoarse(LocationResult locationResult) {
    synchronized (this) {
      if (locationResult != this.mCachedFineLocationResult
          && locationResult != this.mCachedCoarseLocationResult) {
        LocationResult map =
            locationResult.map(
      ...
      if (location != this.mCachedFineLocation && location != this.mCachedCoarseLocation) {
        ...
        double wrapLongitude =
            wrapLongitude(location2.getLongitude())
                + wrapLongitude(metersToDegreesLongitude(this.mLongitudeOffsetM, wrapLatitude));
        double wrapLatitude2 =
            wrapLatitude + wrapLatitude(metersToDegreesLatitude(this.mLatitudeOffsetM));
        double wrapLatitude3 =
            wrapLatitude(Math.round(wrapLatitude2 / r5) * metersToDegreesLatitude(this.mAccuracyM));
        double wrapLongitude2 =
            wrapLongitude(
                Math.round(wrapLongitude / r5)
                    * metersToDegreesLongitude(this.mAccuracyM, wrapLatitude3));
