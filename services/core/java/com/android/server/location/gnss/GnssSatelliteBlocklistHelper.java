...
public class GnssSatelliteBlocklistHelper {
  ...
  public GnssSatelliteBlocklistHelper(
      Context context,
      Looper looper,
      GnssSatelliteBlocklistCallback gnssSatelliteBlocklistCallback) {
  ...
  public void updateSatelliteBlocklist() {
    String string =
        Settings.Global.getString(this.mContext.getContentResolver(), "gnss_satellite_blocklist");
    if (string == null) {
      string = "";
    }
    ...
    try {
      ...
      if (parseSatelliteBlocklist.size() % 2 != 0) {
        Log.e(
            "GnssBlocklistHelper",
            "blocklist string has odd number of values.Aborting updateSatelliteBlocklist");
        return;
      }
