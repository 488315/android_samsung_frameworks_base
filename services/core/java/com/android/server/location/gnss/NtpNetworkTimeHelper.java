...
public class NtpNetworkTimeHelper extends NetworkTimeHelper {
  ...
  public NtpNetworkTimeHelper(
      Context context,
      Looper looper,
      NetworkTimeHelper.InjectTimeCallback injectTimeCallback,
      NtpTrustedTime ntpTrustedTime) {
    this.mDumpLog = new LocalLog(10, false);
    this.mNtpBackOff =
        new ExponentialBackOff(
            300000L, BackupManagerConstants.DEFAULT_KEY_VALUE_BACKUP_INTERVAL_MILLISECONDS);
    ...
    this.mWakeLock =
        ((PowerManager) context.getSystemService("power")).newWakeLock(1, "NtpTimeHelper");
  }

  public NtpNetworkTimeHelper(
      Context context, Looper looper, NetworkTimeHelper.InjectTimeCallback injectTimeCallback) {
  ...
  public final synchronized void lambda$blockingGetNtpTimeAndInject$0(String str) {
    ...
    new Thread(
            new Runnable() { // from class:
                             // com.android.server.location.gnss.NtpNetworkTimeHelper$$ExternalSyntheticLambda0
  ...
  public final void blockingGetNtpTimeAndInject() {
    ...
    boolean forceRefresh =
        (cachedTimeResult == null || cachedTimeResult.getAgeMillis() >= 86400000)
            ? this.mNtpTime.forceRefresh()
            : true;
    synchronized (this) {
      this.mInjectNtpTimeState = 2;
      if (maybeInjectCachedNtpTime(
          "blockingGetNtpTimeAndInject:, debugId="
              + elapsedRealtime
              + ", refreshSuccess="
              + forceRefresh)) {
      ...
      if (this.mPeriodicTimeInjection || !forceRefresh) {
        logDebug(
            "blockingGetNtpTimeAndInject: Scheduling later NTP retrieval, debugId="
                + elapsedRealtime
                + ", mPeriodicTimeInjection="
                + this.mPeriodicTimeInjection
                + ", refreshSuccess="
                + forceRefresh
                + ", delayMillis="
                + j);
        ...
        this.mHandler.postDelayed(
            new Runnable() { // from class:
                             // com.android.server.location.gnss.NtpNetworkTimeHelper$$ExternalSyntheticLambda1
              @Override // java.lang.Runnable
              public final void run() {
  ...
  public final synchronized boolean maybeInjectCachedNtpTime(String str) {
    ...
    if (cachedTimeResult != null && cachedTimeResult.getAgeMillis() < 86400000) {
      ...
      logDebug(
          "maybeInjectCachedNtpTime: Injecting latest NTP time, reason="
              + str
              + ", ntpResult="
              + cachedTimeResult
      ...
      this.mHandler.post(
          new Runnable() { // from class:
                           // com.android.server.location.gnss.NtpNetworkTimeHelper$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
              NtpNetworkTimeHelper.this.lambda$maybeInjectCachedNtpTime$1(
                  timeMillis, elapsedRealtimeMillis, uncertaintyMillis);
            }
          });
      return true;
    }
    logDebug(
        "maybeInjectCachedNtpTime: Not injecting latest NTP time, reason="
            + str
            + ", ntpResult="
            + cachedTimeResult);
    return false;
