...
public class GnssConfiguration {
    ...
    public static int getEsExtensionSecCSC() {
        ...
        int i2 =
                SemCarrierFeature.getInstance()
                        .getInt(i, "CarrierFeature_GPS_ConfigEsExtensionSec", 0, false);
        if (i2 > 300) {
    ...
    public static void setEsExtensionSec() {
        ...
        NetworkScorerAppManager$$ExternalSyntheticOutline0.m(
                esExtensionSecCSC, "Set ES extension seconds : ", "GnssConfiguration");
        HalInterfaceVersion native_get_gnss_configuration_version =
                native_get_gnss_configuration_version();
        if (native_get_gnss_configuration_version == null
                || native_get_gnss_configuration_version.mMajor < 2
                || native_set_es_extension_sec(esExtensionSecCSC)) {
            return;
        }
        ExtendedEthernetServiceImpl$1$$ExternalSyntheticOutline0.m(
                esExtensionSecCSC, "Unable to set ES_EXTENSION_SEC: ", "GnssConfiguration");
    }
        ...
        try {
            ...
            Log.e(
                    ...
                    XmlUtils$$ExternalSyntheticOutline0.m(
                            "Unable to parse config parameter ",
                            str,
                            " value: ",
                            property,
    ...
    public final void loadPropertiesFromCarrierConfig(int i, boolean z) {
        ...
        PersistableBundle configForSubId =
                SubscriptionManager.isValidSubscriptionId(i)
                        ? carrierConfigManager.getConfigForSubId(i)
    ...
    public final void reloadGpsProperties(int i, boolean z) {
        ...
        if (Flags.gnssConfigurationFromResource()) {
            ...
            for (String str :
                    context.getResources().getStringArray(R.array.resolver_target_actions_unpin)) {
                if (z2) {
                    DualAppManagerService$$ExternalSyntheticOutline0.m(
                            "GnssParamsResource: ", str, "GnssConfiguration");
                }
                ...
                if (indexOf <= 0 || (i2 = indexOf + 1) >= str.length()) {
                    ...
                    properties.setProperty(
                            str.substring(0, indexOf).trim().toUpperCase(Locale.ROOT),
                            str.substring(i2));
                }
            }
        }
        ...
        if (intConfig > 300) {
            Log.w(
                    "GnssConfiguration",
                    "ES_EXTENSION_SEC: " + intConfig + " too high, reset to 300");
            i3 = 300;
        } else if (intConfig < 0) {
            Log.w(
                    "GnssConfiguration",
                    "ES_EXTENSION_SEC: " + intConfig + " is negative, reset to zero.");
        } else {
            i3 = intConfig;
        }
