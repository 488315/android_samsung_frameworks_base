...
public final class TimeDetectorNetworkTimeHelper extends NetworkTimeHelper {
    ...
    public final class EnvironmentImpl {
        ...
        public final void requestImmediateTimeQueryCallback(
                final TimeDetectorNetworkTimeHelper timeDetectorNetworkTimeHelper,
                final String str) {
            synchronized (this) {
                this.mHandler.removeCallbacksAndMessages(this.mImmediateRunnableToken);
                this.mHandler.postDelayed(
                        new Runnable() { // from class:
                                         // com.android.server.location.gnss.TimeDetectorNetworkTimeHelper$EnvironmentImpl$$ExternalSyntheticLambda0
                            @Override // java.lang.Runnable
                            public final void run() {
                                TimeDetectorNetworkTimeHelper.this.queryAndInjectNetworkTime(str);
                            }
                        },
                        this.mImmediateRunnableToken,
                        0L);
            }
        }
    }

    public TimeDetectorNetworkTimeHelper(
            EnvironmentImpl environmentImpl, GnssLocationProviderSec gnssLocationProviderSec) {
        ...
        StateChangeListener stateChangeListener =
                new StateChangeListener() { // from class:
                                            // com.android.server.location.gnss.TimeDetectorNetworkTimeHelper$$ExternalSyntheticLambda0
                    @Override // com.android.server.timezonedetector.StateChangeListener
                    public final void onChange() {
                        TimeDetectorNetworkTimeHelper timeDetectorNetworkTimeHelper =
                                TimeDetectorNetworkTimeHelper.this;
                        timeDetectorNetworkTimeHelper.mEnvironment
                                .requestImmediateTimeQueryCallback(
                                        timeDetectorNetworkTimeHelper, "onNetworkTimeAvailable");
                    }
                };
        TimeDetectorInternalImpl timeDetectorInternalImpl =
                (TimeDetectorInternalImpl) environmentImpl.mTimeDetectorInternal;
        timeDetectorInternalImpl.getClass();
        TimeDetectorStrategyImpl timeDetectorStrategyImpl =
                (TimeDetectorStrategyImpl) timeDetectorInternalImpl.mTimeDetectorStrategy;
        synchronized (timeDetectorStrategyImpl) {
            timeDetectorStrategyImpl.mNetworkTimeUpdateListeners.add(stateChangeListener);
        }
    }

    public final void maybeInjectNetworkTime(
            NetworkTimeSuggestion networkTimeSuggestion, String str) {
        long elapsedRealtime;
        if (networkTimeSuggestion == null) {
            elapsedRealtime = Long.MAX_VALUE;
        } else {
            long elapsedRealtimeMillis =
                    networkTimeSuggestion.mUnixEpochTime.getElapsedRealtimeMillis();
        ...
        if (elapsedRealtime > BackupManagerConstants.DEFAULT_FULL_BACKUP_INTERVAL_MILLISECONDS) {
            String str2 =
                    "maybeInjectNetworkTime: Not injecting latest network time latestNetworkTime="
        ...
        String str3 =
                "maybeInjectNetworkTime: Injecting latest network time latestNetworkTime="
                        + networkTimeSuggestion
                        + " reason="
                        + str
                        + " System time offset millis="
                        + (unixEpochTimeMillis - System.currentTimeMillis());
        this.mDumpLog.log(str3);
        if (z) {
            Log.d("TDNetworkTimeHelper", str3);
        }
        ((GnssLocationProvider) this.mInjectTimeCallback)
                .mGnssNative.injectTime(
                        unixEpochTimeMillis,
                        unixEpochTime.getElapsedRealtimeMillis(),
                        networkTimeSuggestion.mUncertaintyMillis);
        this.mNetworkTimeInjected = true;
    }

    public final synchronized void queryAndInjectNetworkTime(String str) {
        NetworkTimeSuggestion latestNetworkSuggestion =
                ((TimeDetectorStrategyImpl)
                                ((TimeDetectorInternalImpl) this.mEnvironment.mTimeDetectorInternal)
        ...
        synchronized (environmentImpl) {
            environmentImpl.mHandler.removeCallbacksAndMessages(
                    environmentImpl.mScheduledRunnableToken);
        }
        if (this.mPeriodicTimeInjectionEnabled) {
            String str2 =
                    "queryAndInjectNtpTime: Scheduling periodic query reason="
                            + str
                            + " latestNetworkTime="
                            + latestNetworkSuggestion
                            + " maxDelayMillis=86400000";
            this.mDumpLog.log(str2);
            if (DEBUG) {
                Log.d("TDNetworkTimeHelper", str2);
            }
            ...
            synchronized (environmentImpl2) {
                try {
                    synchronized (environmentImpl2) {
                        environmentImpl2.mHandler.removeCallbacksAndMessages(
                                environmentImpl2.mScheduledRunnableToken);
                    }
                } finally {
                }
            }
            environmentImpl2.mHandler.postDelayed(
                    new Runnable() { // from class:
                                     // com.android.server.location.gnss.TimeDetectorNetworkTimeHelper$EnvironmentImpl$$ExternalSyntheticLambda1
                        @Override // java.lang.Runnable
                        public final void run() {
                            TimeDetectorNetworkTimeHelper.this.queryAndInjectNetworkTime(
                                    "delayedTimeQueryCallback");
                        }
                    },
                    environmentImpl2.mScheduledRunnableToken,
                    j);
