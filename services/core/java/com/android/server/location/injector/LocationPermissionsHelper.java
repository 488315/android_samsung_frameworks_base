...
public abstract class LocationPermissionsHelper {
  ...
  public LocationPermissionsHelper(AppOpsHelper appOpsHelper) {
    ...
    appOpsHelper.addListener(
        new AppOpsHelper
            .LocationAppOpListener() { // from class:
                                       // com.android.server.location.injector.LocationPermissionsHelper$$ExternalSyntheticLambda0
  ...
  public final boolean hasLocationPermissions(int i, CallerIdentity callerIdentity) {
    if (i == 0) {
      this.mPauseReasonByCaller.put(
          Integer.valueOf(callerIdentity.getUid()), LocationConstants.PAUSED_BY.PERMISSION_NONE);
      return false;
    }
    if (!hasPermission(LocationPermissions.asPermission(i), callerIdentity)) {
      this.mPauseReasonByCaller.put(
          Integer.valueOf(callerIdentity.getUid()), LocationConstants.PAUSED_BY.PERMISSION_CHECK);
      return false;
    }
    if (this.mFreezedUids.contains(Integer.valueOf(callerIdentity.getUid()))) {
      this.mPauseReasonByCaller.put(
          Integer.valueOf(callerIdentity.getUid()), LocationConstants.PAUSED_BY.FREEZE);
      return false;
    }
    boolean checkOpNoThrow =
        this.mAppOps.checkOpNoThrow(LocationPermissions.asAppOp(i), callerIdentity);
    if (checkOpNoThrow) {
      this.mPauseReasonByCaller.remove(Integer.valueOf(callerIdentity.getUid()));
    } else {
      this.mPauseReasonByCaller.put(
          Integer.valueOf(callerIdentity.getUid()), LocationConstants.PAUSED_BY.APP_OPS);
    }
  ...
  public LocationConstants.PAUSED_BY getReasonByCaller(int i) {
    return (LocationConstants.PAUSED_BY)
        this.mPauseReasonByCaller.getOrDefault(
            Integer.valueOf(i), LocationConstants.PAUSED_BY.UNKNOWN);
  }
