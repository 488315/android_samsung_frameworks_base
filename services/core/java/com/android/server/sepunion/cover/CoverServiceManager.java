                ...
                public void onServiceDisconnected(ComponentName componentName2) {
                    ...
                    synchronized (CoverServiceManager.this.mLock) {
                        ...
                        if (findActiveServiceByComponentLocked != null) {
                            findActiveServiceByComponentLocked.unbind();
                            CoverServiceManager.this.removeCoverServiceLocked(findActiveServiceByComponentLocked);
                            CoverServiceManager.this.updateBindHistoryLocked("unbound:cn=" + componentName2 + ",type=" + i);
                            if ((findActiveServiceByComponentLocked.type == 7 || findActiveServiceByComponentLocked.type == 14) && CoverServiceManager.IS_B6_DEVICE && CoverServiceManager.SUPPORT_FLIP_SUITCASE) {
                                if (CoverServiceManager.LED_SUIT.equals(findActiveServiceByComponentLocked.component)) {
                                    if (findActiveServiceByComponentLocked.disconnect()) {
                                        Log.d(CoverServiceManager.TAG, "onServiceDisconnected : retry to connect cover service, " + findActiveServiceByComponentLocked.type);
                                        CoverServiceManager.this.reconnectCoverService(findActiveServiceByComponentLocked);
                                    } else {
                                        Log.d(CoverServiceManager.TAG, "onServiceDisconnected : give up to connect cover service, " + findActiveServiceByComponentLocked.type);
                                    }
                                }
                            }
                        }
