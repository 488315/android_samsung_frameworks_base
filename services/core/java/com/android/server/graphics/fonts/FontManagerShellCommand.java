...
public class FontManagerShellCommand extends ShellCommand {
  ...
  public final void dumpSingleFontConfig(
  ...
  public final void dumpFontConfig(
  ...
  public final void dumpFallback(
      IndentingPrintWriter indentingPrintWriter, FontFamily[] fontFamilyArr) {
  ...
  public final void writeCommandResult(
      ShellCommand shellCommand, FontManagerService.SystemFontException systemFontException) {
    ...
    Slog.e(
        ...
        "Command failed: " + Arrays.toString(shellCommand.getAllArgs()),
        systemFontException);
  }

  public final int dump(ShellCommand shellCommand) {
    if (!DumpUtils.checkDumpPermission(
        this.mService.getContext(), "FontManagerShellCommand", shellCommand.getErrPrintWriter())) {
      return 1;
    }
    ...
    try {
      ...
      try {
        ...
        this.mService.update(
            -1, Collections.singletonList(new FontUpdateRequest(openFileForSystem, bArr)));
    ...
    try {
      ...
      try {
        List parseFontFamilyUpdateXml =
            parseFontFamilyUpdateXml(new FileInputStream(openFileForSystem.getFileDescriptor()));
        openFileForSystem.close();
      ...
      while (true) {
        ...
        if (next == 2) {
          ...
          if (depth == 1) {
            ...
            arrayList.add(
                new FontUpdateRequest(FontUpdateRequest.Family.readFromXml(resolvePullParser)));
          } else {
            throw new FontManagerService.SystemFontException(
    ...
    indentingPrintWriter.println(
        "Last Modified Date: "
            + LocalDateTime.ofEpochSecond(
                    systemFontConfig.getLastModifiedTimeMillis(), 0, ZoneOffset.UTC)
                .format(DateTimeFormatter.ISO_DATE_TIME));
    indentingPrintWriter.println(
        "Number of updated font files: " + this.mService.getFontFileMap().size());
    return 0;
  }
