...
public abstract class ExtconUEventObserver extends UEventObserver {
  ...
  public void startObserving(ExtconInfo extconInfo) {
    ...
    if (devicePath == null) {
      Slog.wtf(
          "ExtconUEventObserver",
          "Unable to start observing  "
              + extconInfo.getName()
              + " because the device path is null. This probably means the selinux policies need to"
              + " be changed.");
      return;
    }
  ...
  public final class ExtconInfo {
      ...
      if (listFiles == null) {
        Slog.w(
            ...
            file
                ...
                + " isDir "
                + file.isDirectory()
                + " but listFiles returns null.This probably means the selinux policies need to be"
                + " changed.");
      ...
      File[] listFilesOrEmpty =
          FileUtils.listFilesOrEmpty(
              ...
              new FilenameFilter() { // from class:
                // com.android.server.ExtconUEventObserver$ExtconInfo$$ExternalSyntheticLambda0
      ...
      for (File file : listFilesOrEmpty) {
        ...
        try {
          ...
          Slog.w(
              ...
              "Unable to read "
                  + str2
                  + "/name. This probably means the selinux policies need to be changed.",
              e);
        }
      }
    }
    ...
    public String getDevicePath() {
      try {
        File file =
            new File(TextUtils.formatSimple("/sys/class/extcon/%s", new Object[] {this.mName}));
        if (!file.exists()) {
          return null;
        }
