...
public class LocaleManagerShellCommand extends ShellCommand {
  ...
  public void onHelp() {
    ...
    outPrintWriter.println(
        "  set-app-locales <PACKAGE_NAME> [--user <USER_ID>] [--locales <LOCALE_INFO>][--delegate"
            + " <FROM_DELEGATE>]");
    ...
    outPrintWriter.println(
        "      --user <USER_ID>: apply for the given user, the current user is used when"
            + " unspecified.");
    outPrintWriter.println(
        "      --locales <LOCALE_INFO>: The language tags of locale to be included as a single"
            + " String separated by commas.");
    outPrintWriter.println("                 eg. en,en-US,hi ");
    outPrintWriter.println("                 Empty locale list is used when unspecified.");
    outPrintWriter.println(
        "      --delegate <FROM_DELEGATE>: The locales are set from a delegate, the value could be"
            + " true or false. false is the default when unspecified.");
      ...
      while (true) {
        ...
        if (nextOption != null) {
          ...
          try {
            ...
            getOutPrintWriter()
                .println("Unknown package " + nextArg + " for userId " + currentUser);
          }
          return 0;
        }
      }
    ...
    if (nextArg != null) {
      ...
      try {
        LocaleList applicationLocales =
            this.mBinderService.getApplicationLocales(nextArg, currentUser);
        getOutPrintWriter()
            .println(
                "Locales for "
                    + nextArg
    ...
    if (nextArg != null) {
      ...
      try {
        LocaleConfig overrideLocaleConfig =
            this.mBinderService.getOverrideLocaleConfig(nextArg, currentUser);
        if (overrideLocaleConfig == null) {
          getOutPrintWriter()
              .println("LocaleConfig for " + nextArg + " for user " + currentUser + " is null");
          return 0;
        }
