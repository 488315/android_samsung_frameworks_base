...
public final class DevicePolicyCacheImpl extends DevicePolicyCache {
    ...
    public final void dump(IndentingPrintWriter indentingPrintWriter) {
        synchronized (this.mLock) {
            ...
            indentingPrintWriter.println(
                    "Content protection policy: " + this.mContentProtectionPolicy);
            indentingPrintWriter.println(
                    "Admin can grant sensors permission: " + this.mCanGrantSensorsPermissions);
    ...
    public final boolean isScreenCaptureAllowed(int i) {
        ...
        synchronized (this.mLock) {
            try {
                z =
                        (((HashSet) this.mScreenCaptureDisallowedUsers).contains(Integer.valueOf(i))
                                        || ((HashSet) this.mScreenCaptureDisallowedUsers)
                                                .contains(-1))
                                ? false
                                : true;
            } finally {
