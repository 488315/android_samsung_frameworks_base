...
public final class KnoxPolicyHelper {
    ...
    public final void handlePasswordExpirationNotification(int i, ActiveAdmin activeAdmin) {
        ...
        if (devicePolicyManager == null
                || z
                || devicePolicyManager.isDeviceOwnerApp(activeAdmin.info.getPackageName())
                || this.mDPM.isProfileOwnerApp(activeAdmin.info.getPackageName())) {
            return;
        }
        Intent intent =
                new Intent(
    ...
    public final boolean isAdminRemovableInternal(ComponentName componentName, int i) {
        if (getIEDMService() == null) {
            Log.d(
                    "KnoxPolicyHelper",
                    "removeActiveAdmin() : passed EDMS.isAdminRemovableInternal() because edms is"
                        + " null");
            return true;
        }
        try {
            ...
            Log.e(
                    "KnoxPolicyHelper",
                    "removeActiveAdmin() : failed to call EDMS.isAdminRemovableInternal()",
                    e);
            return true;
        }
    }
    ...
    public final void removeActiveAdminFromDpm(ArrayList arrayList, int i) {
        if (getIEDMService() == null) {
            Log.i(
                    "KnoxPolicyHelper",
                    "removeUserData() : passed EDMS.removeActiveAdminFromDpm() because edms is"
                        + " null");
            return;
        }
        if (arrayList != null) {
            ...
            while (it.hasNext()) {
                try {
                    this.mEdmService.removeActiveAdminFromDpm(
                            ((ActiveAdmin) it.next()).info.getComponent(), i);
                } catch (RemoteException e) {
                    Slogf.e(
                            "KnoxPolicyHelper",
                            "failed to remove active admin from edm database " + e.getMessage());
                }
            }
        }
    }
