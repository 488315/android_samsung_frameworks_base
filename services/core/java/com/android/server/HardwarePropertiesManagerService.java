...
public class HardwarePropertiesManagerService extends IHardwarePropertiesManager.Stub {
  ...
  public void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
    if (DumpUtils.checkDumpPermission(
      ...
      printWriter.println(
          "Fan speed: "
              + Arrays.toString(getFanSpeeds(callingPackageName))
      ...
      for (int i = 0; i < cpuUsages.length; i++) {
        printWriter.println(
            "Cpu usage of core: "
                + i
                + ", active = "
                + cpuUsages[i].getActive()
                + ", total = "
                + cpuUsages[i].getTotal());
      }
  ...
  public final void enforceHardwarePropertiesRetrievalAllowed(String str) {
    ...
    if (((DevicePolicyManager) this.mContext.getSystemService(DevicePolicyManager.class))
            .isDeviceOwnerApp(str)
        || this.mContext.checkCallingOrSelfPermission("android.permission.DEVICE_POWER") == 0) {
      return;
    }
    if (vrManagerInternal == null || !vrManagerInternal.isCurrentVrListener(str, userId)) {
      throw new SecurityException(
          "The caller is neither a device owner, nor holding the DEVICE_POWER permission, nor the"
              + " current VrListener.");
    }
  }
}
