...
public final class GenieUnloadPolicyHandler extends PolicyHandler {
    ...
    public final int executePolicy(ChimeraCommonUtil.TriggerSource triggerSource, int i) {
        ...
        if (GenieConfigurations.GENAI_UNLOAD_MEMORY_PSI_LEVEL != i
                || ChimeraCommonUtil.TriggerSource.TRIGGER_SOURCE_LMKD_GENIE != triggerSource
                || availableMemoryKb > GenieConfigurations.GENAI_UNLOAD_THRESHOLD) {
            return 0;
        }
        ...
        if (systemRepository.killGenieProcess(
                GenieConfigurations.GENAI_UNLOAD_OOMADJ_THRESHOLD, str)) {
            SystemRepository.log(
                    "GenieUnloadPolicyHandler", "process killed, remove from LRU list");
