...
public class HeimdallReportManager {
  ...
  public void reportBigdata(HeimdallProcessData heimdallProcessData) {
    heimdallProcessData.firstAppVersionName =
        getPackageVersion(heimdallProcessData.firstAppPackageName);
  ...
  public final void reportDumpSysMemInfo(HeimdallProcessData heimdallProcessData) {
    writeMemInfo(
        heimdallProcessData,
        "/data/log/heimdalld_log/"
            + ("heimdall_dumpsys_meminfo_" + heimdallProcessData.firstAppPackageName));
  }

  public final void reportProcStatus(HeimdallProcessData heimdallProcessData) {
    writeShellCmdOutput(
        "cat /proc/" + heimdallProcessData.pid + "/status",
        "/data/log/heimdalld_log/"
            + ("heimdall_proc_status_" + heimdallProcessData.firstAppPackageName));
  }

  public final void reportDmaBufDump(HeimdallProcessData heimdallProcessData) {
    writeDmaBufSize(
        "/data/log/heimdalld_log/"
            + ("heimdall_dma_buf_" + heimdallProcessData.firstAppPackageName));
  }

  public final void compressReports(HeimdallProcessData heimdallProcessData) {
    String str =
        "tar -cvf /data/log/heimdalld_log_"
            + heimdallProcessData.firstAppPackageName
            + ".tar /data/log/heimdalld_log/heimdall_dumpsys_meminfo_"
            + heimdallProcessData.firstAppPackageName
            + " /data/log/heimdalld_log/heimdall_proc_status_"
            + heimdallProcessData.firstAppPackageName
            + " /data/log/heimdalld_log/heimdall_dma_buf_"
            + heimdallProcessData.firstAppPackageName;
    Heimdall.log("Execute Command: " + str);
    try {
      Runtime.getRuntime().exec(str).waitFor();
      String str2 =
          "gzip -1 -f /data/log/heimdalld_log_" + heimdallProcessData.firstAppPackageName + ".tar";
      Heimdall.log("Execute Command: " + str2);
      try {
        Runtime.getRuntime().exec(str2).waitFor();
        String str3 =
            "rm -rf /data/log/heimdalld_log/heimdall_dumpsys_meminfo_"
                + heimdallProcessData.firstAppPackageName
                + " /data/log/heimdalld_log/heimdall_proc_status_"
                + heimdallProcessData.firstAppPackageName
                + " /data/log/heimdalld_log/heimdall_dma_buf_"
        ...
        try {
          ...
          String str4 =
              "/data/log/heimdalld_log_" + heimdallProcessData.firstAppPackageName + ".tar.gz";
          try {
  ...
  public final void reportJavaHeapDump(HeimdallProcessData heimdallProcessData) {
    writeHeapDump(
        heimdallProcessData,
        "/data/log/core/"
            + ("heimdall_heapdump_" + heimdallProcessData.firstAppPackageName + ".prof"),
        true,
        false);
  }
      ...
      try {
        ...
        try {
          printWriter2.println(
              "\n** MEMINFO in pid "
                  + heimdallProcessData.pid
                  + " ["
                  + heimdallProcessData.processName
                  + "] **");
  ...
  public final void writeHeapDump(
      HeimdallProcessData heimdallProcessData, String str, boolean z, boolean z2) {
            ...
            new RemoteCallback(
                new RemoteCallback
                    .OnResultListener() { // from class:
                                          // com.android.server.chimera.heimdall.HeimdallReportManager.1
  ...
  public class BigdataManager {
    ...
    public BigdataManager() {
      this.mSemHqmManager =
          (SemHqmManager) HeimdallReportManager.this.mContext.getSystemService("HqmManagerService");
        ...
        if (file.exists()) {
          try {
            FileInputStream fileInputStream =
                new FileInputStream("/data/misc/pageboost/heimdall_last_bigdata_string");
            try {
              InputStreamReader inputStreamReader =
              ...
              try {
                ...
                while (true) {
                  try {
                    ...
                    BigdataInfo bigdataInfo = HeimdallReportManager.this.new BigdataInfo(readLine);
      ...
      try {
        ...
        try {
          FileOutputStream fileOutputStream =
              new FileOutputStream("/data/misc/pageboost/heimdall_last_bigdata_string");
          try {
    ...
    public final void sendBigdataInfo(BigdataInfo bigdataInfo) {
      ...
      if (this.mSemHqmManager.sendHWParamToHQM(
          0, "AP", "HDMM", "sm", "0.0", "sec", "", jsonData, "")) {
