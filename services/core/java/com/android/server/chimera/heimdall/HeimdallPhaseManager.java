...
public class HeimdallPhaseManager {
  ...
  public final void scanProcess(HeimdallProcessData heimdallProcessData) {
    ...
    if (isValidMemorySizeLong(j)) {
      ...
      if (Heimdall.SCAN_VERSION < 2
          || (readMemtrackMemory = Heimdall.readMemtrackMemory(heimdallProcessData.pid)) == null
  ...
  public final boolean isPidChanged(HeimdallProcessData heimdallProcessData) {
    for (ActivityManager.RunningAppProcessInfo runningAppProcessInfo :
        this.mService.getRunningAppProcesses()) {
      if (runningAppProcessInfo.processName.equals(heimdallProcessData.processName)
          && runningAppProcessInfo.pid == heimdallProcessData.pid) {
        return false;
      }
    }
