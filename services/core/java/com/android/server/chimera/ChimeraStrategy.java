    ...
    public class DynamicFreeMem {
        ...
        */
        public long updateFreeMem(long j) {
            if (this.mReentryCounter.isReentryEnabled() && this.mReentryCounter.getReentry() != DisplayPowerController2.RATE_FROM_DOZE_TO_ON) {
                ...
                if (longValue > 0) {
                    ...
                    int i = 2;
                    int i2 = longValue < this.PSI_MEM_SOME_THRESHOLD_MIN ? 0 : longValue < this.PSI_MEN_SOME_THRESHOLD_MAX ? 1 : 2;
                    if (i2 == 0) {
                        this.mKeepCnt = 0;
                    ...
                    if (ChimeraStrategy.this.mMemFreeTarget > ((ChimeraCommonUtil.getRamSizeGb() * 1024) * 1024) / 3) {
                        ChimeraStrategy.this.mSystemRepository.logDebug("ChimeraStrategy", "over max free target");
                        ChimeraStrategy.this.mMemFreeTarget = ((ChimeraCommonUtil.getRamSizeGb() * 1024) * 1024) / 3;
                    } else if (ChimeraStrategy.this.mMemFreeTarget < ((ChimeraCommonUtil.getRamSizeGb() * 1024) * 1024) / 8) {
                        ChimeraStrategy.this.mSystemRepository.logDebug("ChimeraStrategy", "less min free target");
                        ChimeraStrategy.this.mMemFreeTarget = ((ChimeraCommonUtil.getRamSizeGb() * 1024) * 1024) / 8;
                    }
                    ChimeraStrategy.this.mSystemRepository.logDebug("ChimeraStrategy", "reentry: " + reentry + " memDelta: " + i + " psiLevel: " + i2 + "  avgPsiSomeTotal: " + longValue + " full Total:" + recentAppInfo.get(3) + " New free target memory: " + ChimeraStrategy.this.mMemFreeTarget);
                }
            }
