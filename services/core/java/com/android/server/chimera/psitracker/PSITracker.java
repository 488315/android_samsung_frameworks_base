...
public class PSITracker implements SystemEventListener.TimeOrTimeZoneChangedListener {
    ...
    */
    public final void record240MinutesAvailMem() {
        ...
        try {
            j = 0;
            for (SystemRepository.RunningAppProcessInfo runningAppProcessInfo : this.mSystemRepository.getRunningAppProcesses()) {
                try {
        ...
        if (DEBUG) {
            Log.d("PSITracker", " getAvailable memInfo.totalMem = " + memoryInfo.totalMem + "  available = " + j52 + " runningSize = " + j42 + " cachedSize = " + j32);
        }
    ...
    */
    public List getAvailableMemInfo(long j, long j2) {
        ...
        if (j != 0 || j2 != 0) {
            str = "SELECT id,availMem,running,cached,checkTime FROM " + this.f1669db.mAvailMemTable + " WHERE checkTime <= " + j2 + " AND checkTime > " + j + " ORDER BY id";
        } else {
            str = "SELECT id,availMem,running,cached,checkTime FROM " + this.f1669db.mAvailMemTable + " ORDER BY id";
        }
        Cursor cursor = null;
        try {
            try {
