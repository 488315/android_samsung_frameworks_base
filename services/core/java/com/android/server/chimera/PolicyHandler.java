...
public abstract class PolicyHandler
    implements SystemEventListener.BottleNeckHintListener,
        SystemEventListener.PmmCriticalListener,
        SystemEventListener.PmmStateChangeListener,
        SystemEventListener.LmkdEventListener,
  ...
  public PolicyHandler(
    ...
    int quickReclaimDefaultThreshold =
        this.mChimeraStrategy.getQuickReclaimDefaultThreshold() * 1024;
  ...
  public final void readSystemProperties() {
    this.mPkgKillIntervalMs =
        Integer.parseInt(
            this.mSystemRepository.getSystemProperty(
                "persist.sys.chimera_pkg_kill_interval_ms",
                String.valueOf(this.mPkgKillIntervalDefault)));
    this.mCemPkgKillIntervalMs =
        Integer.parseInt(
            this.mSystemRepository.getSystemProperty(
                "ro.slmk.chimera_cem_pkg_kill_interval_ms", CEM_PKG_KILL_INTERVAL_DEFAULT));
    this.mPkgProtectedParameters =
        new int[][] {
          new int[] {200, 850, this.mPkgKillIntervalMs},
          new int[] {100, 850, this.mPkgKillIntervalHeavy}
        };
  }
  ...
  public void dumpCommonInfo(PrintWriter printWriter) {
    ...
    while (i < 3) {
      ...
      printWriter.println(
          String.format(
              "     G%d: %d", Integer.valueOf(i3), Integer.valueOf(this.mKillCntByGrp[i])));
      i = i3;
    }
    ...
    printWriter.println(
        "mIsQuickReclaimEnabled: "
            + this.mSettingRepository.isQuickReclaimEnable()
            + ", heavy apps : "
            + getHeavyAppsToString());
    StringBuilder sb4 = new StringBuilder();
    sb4.append("QuickReclaimKillCnt: ");
    sb4.append(this.mQuickReclaimKillCnt);
    printWriter.println(sb4.toString());
    printWriter.println(
        "QuickReclaimDynamicThreshold: " + this.mDynamicQuickReclaimAdditionalMemory);
    printWriter.println(
        "Protected AccessibilityPackges: "
            + String.join(", ", this.mSystemRepository.getAccessibilityServicePackages()));
    printWriter.println(
        "App File Cache Reclaim Enable: " + this.mSettingRepository.isAppCacheReclaimEnable());
    ...
    if (ChimeraCommonUtil.isQuotaEnable()) {
      printWriter.println(
          "AlwaysRunningQuotaPPN Trigger Cnt: "
              + PerProcessNandswap.mAlwaysRunningQuotaPPNTriggerCnt);
      printWriter.println(
          "AlwaysRunningQuotaPPN Cnt: " + PerProcessNandswap.mAlwaysRunningQuotaPPNCnt);
  ...
  public void dumpAdjInfo(PrintWriter printWriter) {
    ...
    for (int i = 0; i < killCntByAdj.length; i++) {
      printWriter.println(
          "kills at or below oom_adj " + ChimeraCommonUtil.ADJ_LEVELS[i] + ": " + killCntByAdj[i]);
    }
    printWriter.println();
  ...
  public void updateKillStatistics(
  ...
  public ChimeraDataInfo getChimeraStat() {
    ...
    chimeraDataInfo.setTargetAvaMem(
        this.mChimeraStrategy.getFreeMemTarget(
  ...
  public void onLmkdEventTriggered(int i, int i2, int i3) {
    this.mSystemRepository.logDebug(
        "PolicyHandler", "onLmkdKillTriggered() - lmkdLevel: " + i + " type: " + i3);
    if (i3 != 1) {
      return;
    }
  ...
  public void onAppLaunchIntent(String str) {
    ...
    if (currentTimeMillis - this.mQuickReclaimLastFilterTime
    ...
    boolean z =
        this.mSystemRepository.getCameraApps().contains(str)
            || (isQuickReclaimBigGameEnable()
                && this.mSystemRepository.getBigGameApps().contains(str));
    if (!equals && !z) {
      clearQuickReclaimFields();
    } else {
  ...
  public void quickKill(List list, String str, long j, boolean z) {
    ...
    if (j2 < 0) {
      clearQuickReclaimFields();
      this.mSystemRepository.log(
          "PolicyHandler",
          "available memory: " + availableMemoryKb + ", quit chimera quick reclaim");
      return;
    }
    ...
    while (it2.hasNext()) {
      ...
      if (z2 || str.equals(chimeraAppInfo.packageName)) {
        ...
        this.mSystemRepository.log(
            "PolicyHandler",
            "quick reclaim skip "
                + chimeraAppInfo.getPidList()
                + ": "
                + chimeraAppInfo.packageName
                + " for process changed");
      } else {
        it = it2;
        if (chimeraAppInfo.procList.stream()
            .anyMatch(
                new Predicate() { // from class:
                                  // com.android.server.chimera.PolicyHandler$$ExternalSyntheticLambda0
                  @Override // java.util.function.Predicate
                  public final boolean test(Object obj) {
                    boolean lambda$quickKill$0;
                    lambda$quickKill$0 =
                        PolicyHandler.this.lambda$quickKill$0(
                            chimeraAppInfo, (ChimeraAppInfo.ProcessInfo) obj);
          ...
          this.mSystemRepository.log(
              ...
              "quick reclaim skip "
                  ...
                  + ": "
                  + chimeraAppInfo.packageName
                  + " for hasConnectionProvider");
        } else {
          this.mLastKilledTimeMap.put(
              chimeraAppInfo.packageName, Long.valueOf(System.currentTimeMillis()));
            ...
            if (!supportsPidFd) {
              ...
              this.mThreadPoolExecutor.execute(
                  new Runnable() { // from class:
                                   // com.android.server.chimera.PolicyHandler$$ExternalSyntheticLambda1
    ...
    this.mSystemRepository.logDebug(
        "PolicyHandler",
        "quick reclaim kill before "
            + str
            + " complete: killed "
            + i2
            + " apps, freed "
            + i3
            + " KB, before kill relTarget: "
            + j2
            + " KB and Processing time(ms): "
            + (System.currentTimeMillis() - j));
  }

  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ boolean lambda$quickKill$0(
      ChimeraAppInfo chimeraAppInfo, ChimeraAppInfo.ProcessInfo processInfo) {
    return this.mSystemRepository.hasConnectionProvider(
        processInfo.processName, chimeraAppInfo.uid);
  }
  ...
  public /* synthetic */ void lambda$quickKill$1(ChimeraAppInfo.ProcessInfo processInfo) {
    ...
    try {
      if (Process.killProcessWithMrelease(processInfo.pidFd.getInt$())) {
        this.mSystemRepository.log(
            "PolicyHandler",
            "finish quick reclaim for "
                + processInfo.pid
                + ": "
                + processInfo.processName
                + " and Processing time(ms)"
                + (System.currentTimeMillis() - currentTimeMillis));
      } else {
        this.mSystemRepository.log(
            "PolicyHandler",
            "quick reclaim failed "
                + processInfo.pid
                + ": "
                + processInfo.processName
                + " by killProcessWithMrelease");
        this.mQuickReclaimKillCnt.decrementAndGet();
      }
    } catch (Exception e) {
      this.mSystemRepository.log(
          "PolicyHandler", "quick reclaim kill with exception " + e.getMessage());
      this.mQuickReclaimKillCnt.decrementAndGet();
    }
  }

  public final boolean isPidDeadOrReused(ChimeraAppInfo chimeraAppInfo) {
    if (!chimeraAppInfo.procList.isEmpty()
        && chimeraAppInfo.packageName.equals(
            this.mSystemRepository.getPackageNameByPid(
                ((ChimeraAppInfo.ProcessInfo) chimeraAppInfo.procList.get(0)).pid))) {
      return chimeraAppInfo.procList.stream()
          .anyMatch(
              new Predicate() { // from class:
                                // com.android.server.chimera.PolicyHandler$$ExternalSyntheticLambda5
                @Override // java.util.function.Predicate
                public final boolean test(Object obj) {
                  boolean lambda$isPidDeadOrReused$2;
                  lambda$isPidDeadOrReused$2 =
                      PolicyHandler.this.lambda$isPidDeadOrReused$2(
                          (ChimeraAppInfo.ProcessInfo) obj);
  ...
  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ boolean lambda$isPidDeadOrReused$2(
      ChimeraAppInfo.ProcessInfo processInfo) {
  ...
  public void filterAppListForQuickReclaim() {
    if (this.mSettingRepository.isQuickReclaimEnable()) {
      long availableMemoryKb = ChimeraCommonUtil.getAvailableMemoryKb(this.mSystemRepository);
      if ((this.mChimeraStrategy.getFreeMemTarget(availableMemoryKb) - availableMemoryKb)
              + this.mDynamicQuickReclaimAdditionalMemory
          <= 0) {
        return;
      }
      List appsToKill =
          this.mAppManager.getAppsToKill(
              this.mSkipReasonLogger,
              this.mChimeraStrategy.getProtectedCountOnHomeTrigger() >> 1,
              ChimeraCommonUtil.TriggerSource.TRIGGER_SOURCE_HOME_IDLE);
      ...
      appsToKill.sort(
          new Comparator() { // from class:
                             // com.android.server.chimera.PolicyHandler$$ExternalSyntheticLambda4
            @Override // java.util.Comparator
            public final int compare(Object obj, Object obj2) {
              int lambda$filterAppListForQuickReclaim$3;
              lambda$filterAppListForQuickReclaim$3 =
                  PolicyHandler.lambda$filterAppListForQuickReclaim$3(
                      (ChimeraAppInfo) obj, (ChimeraAppInfo) obj2);
  ...
  public static /* synthetic */ int lambda$filterAppListForQuickReclaim$3(
      ChimeraAppInfo chimeraAppInfo, ChimeraAppInfo chimeraAppInfo2) {
  ...
  public void filterPreKillApp(List list) {
    ...
    while (it.hasNext()) {
      ...
      if (chimeraAppInfo.group >= 4) {
        ...
        return;
      } else if (isAppKillable(chimeraAppInfo)
          && (!Process.supportsPidFd() || openPidFds(chimeraAppInfo))) {
  ...
  public final boolean openPidFds(ChimeraAppInfo chimeraAppInfo) {
    ...
    for (ChimeraAppInfo.ProcessInfo processInfo : chimeraAppInfo.procList) {
      try {
        ...
        this.mSystemRepository.logDebug(
            "PolicyHandler",
            "open pid " + processInfo.pid + ": " + processInfo.processName + " failed!");
        return false;
      }
    }
    return true;
  ...
  public boolean isExpiredKillInterval(ChimeraAppInfo chimeraAppInfo) {
    long currentTimeMillis =
        System.currentTimeMillis()
            - ((Long) this.mLastKilledTimeMap.getOrDefault(chimeraAppInfo.packageName, 0L))
                .longValue();
    long j =
        (chimeraAppInfo.statsAndOomScores.isAllCachedEmptyProcess() || isService(chimeraAppInfo))
            ? this.mCemPkgKillIntervalMs
            : this.mPkgProtectedParameters[this.mCurProtectLevel.ordinal()][2];
    ...
    this.mSystemRepository.logDebug(
        ...
        "Skipped by interval: "
            + chimeraAppInfo.packageName
            + ", elapsed: "
            + toHHmmss(currentTimeMillis)
            + ", interval: "
            + toHHmmss(j)
            + toAppInfoDescription(chimeraAppInfo));
    return false;
  }
  ...
  public boolean isAppKillable(ChimeraAppInfo chimeraAppInfo) {
    ...
    ProcessStatsAndOomScores create =
        ProcessStatsAndOomScores.create(chimeraAppInfo, this.mSystemRepository);
    ...
    if ((chimeraAppInfo.appType & IInstalld.FLAG_CLEAR_APP_DATA_KEEP_ART_PROFILES) == 131072) {
      ...
      if ((this.mCurProtectLevel == ProtectLevel.NORMAL
              && hasProtectedServices > ProtectedReason.NONE.ordinal())
          || (this.mCurProtectLevel == ProtectLevel.HEAVY
              && hasProtectedServices > ProtectedReason.NONE.ordinal()
              && protectedReason != ProtectedReason.ACTIVITY_TIME)) {
        this.mSkipReasonLogger.mark(chimeraAppInfo, SkipReasonLogger.Reason.SERVICE);
        this.mSystemRepository.logDebug(
            "PolicyHandler",
            "Skipped by Normal Service condition: "
                + chimeraAppInfo.packageName
                + toAppInfoDescription(chimeraAppInfo)
                + " reason: "
                + protectedReason);
    ...
    while (true) {
      ...
      if (!this.mSystemRepository.isThreadGroupLeader(processInfo.pid)) {
        this.mSystemRepository.log(
            "PolicyHandler",
            "Skipped by Thread Group Leader condition: "
                + chimeraAppInfo.packageName
                + " pid: "
                + processInfo.pid);
        z = true;
  ...
  public int hasProtectedServices(ChimeraAppInfo chimeraAppInfo) {
    ...
    if (chimeraAppInfo != null && chimeraAppInfo.statsAndOomScores != null) {
      ...
      while (true) {
        ...
        if (processStatsAndOomScores.mScores[i]
                <= this.mPkgProtectedParameters[this.mCurProtectLevel.ordinal()][1]
            && chimeraAppInfo.statsAndOomScores.mScores[i] >= -1000
            && (hasChimeraProtectedProc =
                    this.mSystemRepository.hasChimeraProtectedProc(
                        ((ChimeraAppInfo.ProcessInfo) chimeraAppInfo.procList.get(i)).processName,
                        chimeraAppInfo.uid))
                > 0) {
  ...
  public final void handleDeviceIdle(int i) {
    List<ChimeraAppInfo> appsToDeviceIdle =
        this.mAppManager.getAppsToDeviceIdle(
            i,
            this.mChimeraStrategy.getDeviceIdleAppThreshold() * 1024,
            this.mChimeraStrategy.getDeviceIdleNativeThreshold() * 1024);
    ...
    for (ChimeraAppInfo chimeraAppInfo : appsToDeviceIdle) {
      this.mSystemRepository.logDebug(
          "PolicyHandler", "killHandleDiveceIdleApps " + chimeraAppInfo.packageName);
      chimeraAppInfo.statsAndOomScores =
          ProcessStatsAndOomScores.create(chimeraAppInfo, this.mSystemRepository);
      int i3 = i2;
      long j2 = j;
      long j3 = 0;
      for (ChimeraAppInfo.ProcessInfo processInfo : chimeraAppInfo.procList) {
        ...
        killProcessForDeviceIdle(
            chimeraAppInfo.packageName,
            chimeraAppInfo.uid,
            processInfo.processName,
            processInfo.pid,
            procPss,
            chimeraAppInfo.idleKillAdj);
        j3 += procPss;
        i3++;
        j2 += procPss;
      }
      updateKillStatistics(
          chimeraAppInfo, ChimeraCommonUtil.TriggerSource.TRIGGER_SOURCE_DEVICE_IDLE);
      reportIdleKill(chimeraAppInfo.packageName, (int) j3, chimeraAppInfo.idleKillAdj);
      i2 = i3;
      j = j2;
    }
    this.mSystemRepository.log(
        "PolicyHandler", "handleDiveceIdleApps kill end. killed count:" + i2 + " released:" + j);
  }
  ...
  public void killProcessForDeviceIdle(String str, int i, String str2, int i2, long j, int i3) {
    ...
    this.mSystemRepository.log(
        "PolicyHandler",
        "killProcessForDeviceIdle - killed:" + i + "/" + str + " pid:" + i2 + " pss:" + j);
  }
  ...
  /* renamed from: com.android.server.chimera.PolicyHandler$1 */
  public abstract /* synthetic */ class AbstractC09801 {

    /* renamed from: $SwitchMap$com$android$server$chimera$ChimeraCommonUtil$TriggerSource */
    public static final /* synthetic */ int[] f1668x30947b05;
    ...
    static {
      ...
      try {
        f1668x30947b05[ChimeraCommonUtil.TriggerSource.TRIGGER_SOURCE_HOME_IDLE.ordinal()] = 2;
  ...
  public boolean prepareForTrigger(ChimeraCommonUtil.TriggerSource triggerSource) {
    int i = AbstractC09801.f1668x30947b05[triggerSource.ordinal()];
  ...
  public void handlePMMCriticalOtherApps(List list) {
    ...
    if (list != null && list.size() > 0) {
      ...
      if (i3 > 0) {
        ...
        Pair processStatesAndOomScoresForPIDs =
            this.mSystemRepository.getProcessStatesAndOomScoresForPIDs(iArr2);
        if (processStatesAndOomScoresForPIDs != null
            && (iArr = (int[]) processStatesAndOomScoresForPIDs.second) != null) {
          int i6 = 0;
          int i7 = 0;
          long j2 = 0;
          while (i6 < i3) {
            ...
            if (i8 < 100
                || i8 >= 300
                || this.mAppManager.getAppStandbyBucket(strArr3[i6], iArr4[i6]) == 5) {
              i2 = i6;
              strArr = strArr3;
            } else {
              i2 = i6;
              strArr = strArr3;
              killProcessForPMMCritical(
                  strArr3[i6], iArr4[i6], strArr2[i6], iArr2[i6], iArr[i6], jArr[i6], false);
              i7++;
              j2 += jArr[i2];
            }
            i6 = i2 + 1;
          ...
          this.mSystemRepository.log(
              "PolicyHandler",
              "handlePMMCriticalOtherApps end. killed count:" + i + " released:" + j);
        }
      }
    }
    i = 0;
    ...
    this.mPMMKillCnt += i;
    this.mSystemRepository.log(
        "PolicyHandler", "handlePMMCriticalOtherApps end. killed count:" + i + " released:" + j);
  }
  ...
  public void handlePMMCriticalLockScreenApps(List list) {
    ...
    if (list != null && list.size() > 0) {
      ...
      while (it.hasNext()) {
        ChimeraAppInfo chimeraAppInfo = (ChimeraAppInfo) it.next();
        this.mSystemRepository.logDebug(
            "PolicyHandler", "handlePMMCriticalLockScreenApps " + chimeraAppInfo.packageName);
        int i2 = i;
        long j3 = j2;
        for (ChimeraAppInfo.ProcessInfo processInfo : chimeraAppInfo.procList) {
          if (processInfo.initialIdlePss > 0) {
            ...
            this.mSystemRepository.logDebug(
                "PolicyHandler",
                "handlePMMCriticalLockScreenApps : pid:"
                    + processInfo.pid
                    + " "
                    + procPss
                    + "/"
                    + processInfo.initialIdlePss);
            if (procPss - processInfo.initialIdlePss > 102400 && procPss > ((int) (r1 * 1.2f))) {
              killProcessForPMMCritical(
                  chimeraAppInfo.packageName,
                  chimeraAppInfo.uid,
                  processInfo.processName,
    ...
    this.mSystemRepository.log(
        "PolicyHandler",
        "handlePMMCriticalLockScreenApps end. killed count:" + i + " released:" + j);
  }

  public void killProcessForPMMCritical(
      String str, int i, String str2, int i2, int i3, long j, boolean z) {
    addRescheduleExceptionPackage(str);
  ...
  public void onQuotaExceed(ChimeraQuotaMonitor.QuotaReclaimTarget quotaReclaimTarget) {
    ...
    if (list != null
        && !list.isEmpty()
        && (appsForCritical = this.mAppManager.getAppsForCritical(list)) != null
        && (obj = appsForCritical.second) != null) {
      List<ChimeraAppInfo> list2 = (List) obj;
      if (!list2.isEmpty()) {
        list2.sort(
            new Comparator() { // from class:
                               // com.android.server.chimera.PolicyHandler$$ExternalSyntheticLambda2
              @Override // java.util.Comparator
              public final int compare(Object obj2, Object obj3) {
                int lambda$onQuotaExceed$4;
                lambda$onQuotaExceed$4 =
                    PolicyHandler.this.lambda$onQuotaExceed$4(
                        (ChimeraAppInfo) obj2, (ChimeraAppInfo) obj3);
        ...
        for (ChimeraAppInfo chimeraAppInfo : list2) {
          if (isVisibleOrPerceptible(chimeraAppInfo)
              && this.mAppManager.getAppStandbyBucket(
                      chimeraAppInfo.packageName, chimeraAppInfo.uid)
                  != 5) {
            Iterator it = chimeraAppInfo.procList.iterator();
            long j2 = 0;
            while (it.hasNext()) {
        ...
        if (j3 != 0) {
          updateActionStatistics(triggerSource);
          this.mSystemRepository.logDebug(
              "PolicyHandler", "Quota kill complete: killed " + i + " apps, freed " + j3 + " KB");
        } else {
    ...
    this.mSystemRepository.logDebug(
        "PolicyHandler", "Quota is still not enough, need PPN do more " + j);
    PerProcessNandswap.getInstance().handleAlwaysRunningQuotaPPN(j);
  }

  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ int lambda$onQuotaExceed$4(
      ChimeraAppInfo chimeraAppInfo, ChimeraAppInfo chimeraAppInfo2) {
    int i = chimeraAppInfo.group - chimeraAppInfo2.group;
    return i != 0
        ? i
        : Long.compare(getDRAMUsedByApp(chimeraAppInfo2), getDRAMUsedByApp(chimeraAppInfo));
  }

  public void addKillInfoToHistory(
      ChimeraAppInfo chimeraAppInfo, ChimeraCommonUtil.TriggerSource triggerSource) {
  ...
  public void dumpHistoryBuffer(PrintWriter printWriter) {
    if (this.mKillHistoryBuffer.size() > 0) {
      printWriter.println(
          "Date Time PackageName AppType StandbyBucket PSS PIDs OomScores TriggerSource:");
  ...
  public void calcAppScores(List list) {
    ...
    while (it2.hasNext()) {
      ...
      float f =
          (((this.mWeightLru * chimeraAppInfo2.lruIdx) / i)
                  + ((this.mWeightStandbyBucket * chimeraAppInfo2.cacStandbyBucket) / 50.0f)
                  + ((this.mWeightMem * chimeraAppInfo2.reclaimGain) / j))
              * 100.0f;
  ...
  public void calcAppScoresIncludeSwap(List list) {
    ...
    while (it2.hasNext()) {
      ChimeraAppInfo chimeraAppInfo2 = (ChimeraAppInfo) it2.next();
      float f =
          (((chimeraAppInfo2.lruIdx * 0.5f) / i)
                  + ((chimeraAppInfo2.cacStandbyBucket * 0.1f) / 50.0f)
                  + (((chimeraAppInfo2.reclaimGain - chimeraAppInfo2.swapPss) * 0.4f) / j))
              * 100.0f;
      chimeraAppInfo2.score = f;
      chimeraAppInfo2.finalScore = ((4 - chimeraAppInfo2.group) * 100.0f) + f;
    }
  ...
  public boolean isGcReclaimEnabled() {
    return this.mSettingRepository.isGcEnabled()
        || this.mSettingRepository.isReclaimPageCacheEnabled();
  }
  ...
  public boolean isReusedPid(ChimeraAppInfo chimeraAppInfo) {
    return chimeraAppInfo.procList.stream()
        .anyMatch(
            new Predicate() { // from class:
                              // com.android.server.chimera.PolicyHandler$$ExternalSyntheticLambda7
              @Override // java.util.function.Predicate
              public final boolean test(Object obj) {
                boolean lambda$isReusedPid$5;
                lambda$isReusedPid$5 =
                    PolicyHandler.this.lambda$isReusedPid$5((ChimeraAppInfo.ProcessInfo) obj);
  ...
  public boolean isSystemPid(ChimeraAppInfo chimeraAppInfo) {
    ...
    return chimeraAppInfo.procList.stream()
        .anyMatch(
            new Predicate() { // from class:
                              // com.android.server.chimera.PolicyHandler$$ExternalSyntheticLambda6
              @Override // java.util.function.Predicate
              public final boolean test(Object obj) {
                boolean lambda$isSystemPid$6;
                lambda$isSystemPid$6 =
                    PolicyHandler.lambda$isSystemPid$6(systemPid, (ChimeraAppInfo.ProcessInfo) obj);
                return lambda$isSystemPid$6;
              }
            });
  }

  public static /* synthetic */ boolean lambda$isSystemPid$6(
      int i, ChimeraAppInfo.ProcessInfo processInfo) {
  ...
  public void markSkipReason(
      ChimeraAppInfo chimeraAppInfo, SkipReasonLogger.Reason reason, String str) {
    this.mSkipReasonLogger.mark(chimeraAppInfo, reason);
    this.mSystemRepository.logDebug(
        "PolicyHandler",
        "Skipped by "
  ...
  public final boolean isCameraPidChanged() {
    ...
    for (SystemRepository.CameraProcInfo cameraProcInfo : this.mCameraRelateInfos) {
      ...
      if (!Process.readProcFile(
              "/proc/" + cameraProcInfo.pid + "/cmdline", CMDLINE_OUT, strArr, null, null)
          || (str = strArr[0]) == null
          || !str.contains(cameraProcInfo.name)) {
        return true;
      }
    }
    return false;
  ...
  public final void getCameraRelatedPids() {
    ...
    for (Integer num : getPidsFromProcName(list)) {
      if (num.intValue() >= 100) {
        if (Process.readProcFile("/proc/" + num + "/cmdline", CMDLINE_OUT, strArr, null, null)
            && (str = strArr[0]) != null) {
          ...
          this.mSystemRepository.log(
              "PolicyHandler",
              "get camera process's pid: " + cameraProcInfo.pid + " name: " + cameraProcInfo.name);
    ...
    this.mSystemRepository.log(
        "PolicyHandler",
        "get camera process's pid success and Processing time(ms): "
            + (System.currentTimeMillis() - currentTimeMillis));
  }
  ...
  public static /* synthetic */ boolean lambda$isCameraPidInit$7(
      SystemRepository.CameraProcInfo cameraProcInfo) {
    return cameraProcInfo.pid != 0;
  }

  public final boolean isCameraPidInit() {
    return this.mCameraRelateInfos.stream()
        .allMatch(
            new Predicate() { // from class:
                              // com.android.server.chimera.PolicyHandler$$ExternalSyntheticLambda3
              @Override // java.util.function.Predicate
              public final boolean test(Object obj) {
                boolean lambda$isCameraPidInit$7;
                lambda$isCameraPidInit$7 =
                    PolicyHandler.lambda$isCameraPidInit$7((SystemRepository.CameraProcInfo) obj);
  ...
  public void onCameraClose() {
    ...
    this.mSystemRepository.logDebug(
        "PolicyHandler",
        "get dynamic quick reclaim additional memory successfully " + min + " (MB)");
  }
  ...
  public void onForegroundActivitiesChanged(
    ...
    if (ChimeraCommonUtil.isColdLaunch(str) || ChimeraCommonUtil.isWarmLaunch(str)) {
      ...
      this.mSystemRepository.logDebug(
          "PolicyHandler", "Chimera AppFileCacheReclaim do reclaimAppCaches skip: " + str);
    }
  }

  public final void reclaimAppCaches(String str) {
    List<String> appFilePathsByPackageName =
        this.mSystemRepository.getAppFilePathsByPackageName(str);
    if (appFilePathsByPackageName != null) {
      for (String str2 : appFilePathsByPackageName) {
        if (ChimeraCommonUtil.doReclaimPageCacheByFilePath(str2)) {
          String[] split = str2.split("/");
          String str3 = split[split.length - 1];
          str3.hashCode();
          switch (str3) {
            case "base.odex":
              long[] jArr = this.mAppFileCacheRecliamCnt;
              jArr[1] = jArr[1] + 1;
              break;
            case "base.vdex":
              long[] jArr2 = this.mAppFileCacheRecliamCnt;
              jArr2[2] = jArr2[2] + 1;
              break;
            case "base.apk":
              long[] jArr3 = this.mAppFileCacheRecliamCnt;
              jArr3[0] = jArr3[0] + 1;
              break;
            default:
              long[] jArr4 = this.mAppFileCacheRecliamCnt;
              jArr4[3] = jArr4[3] + 1;
              break;
          }
        } else {
          this.mSystemRepository.logDebug(
              "PolicyHandler", "Chimera AppFileCacheReclaim doReclaimPageCacheByFilePath failed.");
        }
      }
    }
  }
  ...
  public boolean handleQuotaKill(
      ChimeraAppInfo chimeraAppInfo, ChimeraCommonUtil.TriggerSource triggerSource, long j) {
    String str;
    int i = chimeraAppInfo.group;
    if (i == 1) {
      if (chimeraAppInfo.hasRestartService(this.mSystemRepository)) {
        SystemRepository systemRepository = this.mSystemRepository;
        systemRepository.forceStop(
            chimeraAppInfo.packageName, systemRepository.getUserId(chimeraAppInfo.uid));
        str = "Force-stop";
      } else {
        Iterator it = chimeraAppInfo.procList.iterator();
        while (it.hasNext()) {
          this.mSystemRepository.killProcessForChimera(
              ((ChimeraAppInfo.ProcessInfo) it.next()).processName,
              chimeraAppInfo.uid,
              "Chimera Quota Kill");
        }
        str = "Killed";
      ...
      while (it2.hasNext()) {
        this.mSystemRepository.killProcessForChimera(
            ((ChimeraAppInfo.ProcessInfo) it2.next()).processName,
            chimeraAppInfo.uid,
            "Chimera Quota Kill");
      }
