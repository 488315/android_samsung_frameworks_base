...
public class ChimeraManager implements SystemEventListener.MediaScanFinishedListener {
  ...
  public ChimeraManager(Context context, ActivityManagerService activityManagerService) {
    ...
    this.mSystemRepository =
        RepositoryFactory.getInstance().getSystemRepository(context, activityManagerService);
    this.mSettingRepository =
        RepositoryFactory.getInstance().getSettingRepository(this.mSystemRepository);
    ...
    this.mChimeraStrategy =
        new ChimeraStrategy(
            new ChimeraRecentAppManager(
                this.mSystemRepository, this.mSettingRepository, this.mHandlerThread.getLooper()),
            this.mSystemRepository,
            this.mSettingRepository);
    this.mAppManager = new ChimeraAppManager(this.mSystemRepository);
    this.mContext = context;
    this.mSystemEventListener =
        new SystemEventListener(context, this.mHandlerThread.getLooper(), this.mSystemRepository);
  ...
  public final void registerSystemEventListener() {
    ...
    if (this.mSettingRepository.isAppsIdleKillEnabled()
        || this.mSettingRepository.isNativeProcessesIdleKillEnabled()) {
  ...
  public void createPSITracker() {
    ...
    PSITracker pSITracker2 =
        new PSITracker(this.mSystemRepository, this.mContext, this.mHandlerThread.getLooper());
  ...
  public ChimeraDataInfo getChimeraStat() {
    return (ChimeraDataInfo)
        Optional.ofNullable(this.mPolicyHandler)
            .map(
                new Function() { // from class:
                                 // com.android.server.chimera.ChimeraManager$$ExternalSyntheticLambda0
  ...
  public void onMediaScanFinished() {
    ...
    if (ChimeraCommonUtil.isQuotaEnable()) {
      ChimeraQuotaMonitor.getInstance()
          .schedule(
              this.mSystemRepository,
              this.mAppManager,
              this.mSystemEventListener,
              this.mChimeraStrategy.getAlwaysRunningProcQuota());
    }
  }
