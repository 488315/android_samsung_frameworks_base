...
public class DualModeChanger extends ModeChanger {
    ...
    public ModeChanger.ModeToModeChangeInfo setDesktopMode(State state, boolean z, ModeChanger.ModeToModeChangeInfo modeToModeChangeInfo) {
        if (DesktopModeFeature.DEBUG) {
            Log.m7d(TAG, "setDesktopMode(), enter=" + z + ", isModeChangeLocked=" + state.isModeChangeLocked());
        }
    ...
    public void onDesktopDisplayConfigured(final boolean z) {
        if (DesktopModeFeature.DEBUG) {
            Log.m11v(TAG, "onDesktopDisplayConfigured(), added=" + z + ", mDesktopDisplayId=" + this.mDesktopDisplayId);
        }
    ...
    public final void lambda$onDesktopDisplayConfigured$1(boolean z) {
        ...
        if (desktopModeState.state == 30 && desktopModeState.getDisplayType() == 102) {
            ...
            if (DesktopModeFeature.DEBUG) {
                Log.m7d(TAG, "handleDesktopDisplayConfigured(), added=" + z + ", desktopModeState=" + desktopModeState);
            }
    ...
    public final void scheduleStopLoadingScreenIfPossible(boolean z) {
        ...
        if (DesktopModeFeature.DEBUG) {
            Log.m7d(TAG, "scheduleStopLoadingScreenIfPossible(), enter=" + z + ", mWallpaperShown=" + this.mWallpaperShown + ", mLoadingScreenAnimationComplete=" + this.mLoadingScreenAnimationComplete + ", desktopModeState=" + desktopModeState);
        }
    ...
    public final void handleStopLoadingScreen(boolean z, int i) {
        ...
        if (state.getDesktopModeState().state != 50) {
            if (DesktopModeFeature.DEBUG) {
                Log.m12w(TAG, "Failed to stop loading screen. onDesktopDisplayConfigured() is not called! enter=" + z + ", desktopModeState=" + state.getDesktopModeState());
                return;
            }
            return;
        }
        ...
        if (!z) {
            ...
            if (this.mDesktopDisplayContext != null && DesktopModeSettings.getSettings(this.mResolver, "auto_open_last_app", false)) {
                ToastManager.showToast(this.mDesktopDisplayContext, R.string.foreground_service_tap_for_details);
            }
            showDexMirroringTipsNotification(this.mContext.getString(R.string.fp_power_button_bp_title), this.mContext.getString(R.string.fp_power_button_bp_positive_button));
    ...
    public boolean canShowTouchpadNotification(State state) {
        ...
        Log.m7d(TAG, "canShowTouchpadNotification navBarGestureEnabled=" + isNavBarGestureEnabled + ", hasNavigationBar=" + hasNavigationBar + ", isTouchpadAvailable=" + isTouchpadAvailable + ", isTouchpadEnabled=" + isTouchpadEnabled);
    ...
    public class ModeChangeReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("android.intent.action.TIME_TICK".equals(action)) {
                ...
                if (freeMemory < 419430400) {
                    ...
                    if (DualModeChanger.this.mDesktopDisplayContext != null) {
                        ToastManager.showToast(DualModeChanger.this.mDesktopDisplayContext, R.string.global_action_restart);
                        return;
                    }
                    return;
                }
            ...
            if ("com.samsung.android.desktopmode.action.SPEN_NOTIFICATION_CHANGE_MODE_PRESSED".equals(action)) {
                ...
                if (DesktopModeFeature.DEBUG) {
                    Log.m7d(DualModeChanger.TAG, "SPen notification change mode clicked, displayID=" + intExtra3);
                }
