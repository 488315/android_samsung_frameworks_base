...
public class DockManager {
  ...
  public DockManager(Context context, ServiceThread serviceThread, IStateManager iStateManager) {
    ...
    StateManager.StateListener stateListener =
        new StateManager
            .StateListener() { // from class: com.android.server.desktopmode.DockManager.1
          ...
          public void onDockStateChanged(State state) {
            if (DesktopModeFeature.DEBUG) {
              Log.m7d(
                  DockManager.TAG,
                  "onDockStateChanged n="
                      + state.getDockState()
                      + ", p="
                      + state.getPreviousDockState());
            }
            if (state.getDockState().isUndocked()
                && (state.getPreviousDockState().isDexStation()
                    || state.getPreviousDockState().isDexPad())) {
    ...
    this.mLastDockControlLibMsgState =
        new DockMsgState(
            200,
            210,
            FrameworkStatsLog.CAMERA_SHOT_LATENCY_REPORTED__MODE__CONTROL_DS_MODE_ASTRO,
            230,
            240);
    ...
    context.registerReceiverAsUser(
        new BroadcastReceiver() { // from class: com.android.server.desktopmode.DockManager.2
          ...
          public void onReceive(Context context2, Intent intent) {
            ...
            if ("android.intent.action.ACTION_POWER_CONNECTED".equals(action)
              ...
              if (DesktopModeFeature.DEBUG) {
                Log.m7d(DockManager.TAG, "onReceive(), action=" + action);
              }
  ...
  public final void requestUpdateDockLibStatus(String str) {
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "requestUpdateDockLibStatus, reason - " + str);
    }
  ...
  public final void updateDockLibStatus(State state) {
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(
          TAG,
          "updateDockLibStatus(), isDesktopDockConnected="
              + state.getDockState().isDexStation()
              + ", isDexPadConnected="
              + state.getDockState().isDexPad()
              + ", mDockState="
              + state.getDockState().getType()
              + ", mPrevDockState="
              + state.getPreviousDockState().getType()
              + ", mDockChargerPower="
              + this.mDockChargerPower
              + ", mDockChargerSupport="
              + this.mDockChargerSupport);
    }
  ...
  public final void updateDockVersionToSettings(boolean z) {
    if (z) {
      this.mDockVersion =
          Utils.readFile("/sys/class/dp_sec/dex_ver", "")
              + "."
              + Utils.readFile("/sys/class/sec/ccic/acc_device_version", "");
    ...
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "updateDockVersionToSettings(), mDockVersion = " + this.mDockVersion);
    }
  ...
  public final void createDockControlLib(int i) {
    if (i == 40992 || i == 41001) {
      if (this.mControlResponseListener == null) {
        this.mControlResponseListener =
            new ISDCDeviceController.ControlResponseListener() { // from class:
              // com.android.server.desktopmode.DockManager.3
      ...
      if (this.mISDCDeviceController == null) {
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(TAG, "createDockControlLib dockType=" + i);
        }
        this.mLastDockControlLibMsgState.setLast(100, 201, null);
        this.mISDCDeviceController =
            new SDCDeviceController(this.mContext, i, this.mControlResponseListener);
        if (i == 41001) {
    ...
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "createDockControlLib is not supported");
    }
  }
  ...
  public final void destroyDockControlLib(int i) {
    ...
    if (this.mISDCDeviceController != null) {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(TAG, "destroyDockControlLib");
      }
  ...
  public final void initialize() {
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "DockManager initialize()");
    }
    this.mDockVersion = null;
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter) {
    ...
    for (int i = 0; i < this.mLastDockControlLibMsgState.mMsgState.size(); i++) {
      indentingPrintWriter.println(
          "mLastDockControlLibMsgState=" + this.mLastDockControlLibMsgState.toString(i));
    }
  ...
  public final class DockHandler extends Handler {
    ...
    public void handleMessage(Message message) {
      ...
      if (i == 150) {
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(DockManager.TAG, "MSG_DOCK_REQUEST_UPDATE_DOCK_LIB_STATUS");
        }
      ...
      if (i == 211) {
        ...
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(
              DockManager.TAG,
              "MSG_DOCK_RESPONSE_POWER_INFO onConnectedPowerChargerInfoUpdated, power="
                  + i3
                  + " previous power="
                  + DockManager.this.mDockChargerPower
                  + " support="
                  + i4);
        }
        ...
        if (i3 == 0 || i3 != DockManager.this.mDockChargerPower) {
          if (i3 < 15) {
            if (DockManager.this.mDockChargerPower == 0
                || DockManager.this.mDockChargerPower >= 15) {
              if (DesktopModeFeature.DEBUG) {
                Log.m7d(DockManager.TAG, "updateDockLowChargerPower true");
              }
            ...
            if (DesktopModeFeature.DEBUG) {
              Log.m7d(DockManager.TAG, "updateDockLowChargerPower false");
            }
      ...
      if (i == 221) {
        ...
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(
              DockManager.TAG,
              "MSG_DOCK_ERROR onError, error=" + DockManager.dockControlErrorToString(intValue));
        }
        DockManager.this.mLastDockControlLibMsgState.setLast(
            102, message.what, DockManager.dockControlErrorToString(intValue));
        DockManager.this.mDockControlLibError.put(intValue, true);
        StringBuilder sb = new StringBuilder();
        while (i2 < DockManager.this.mDockControlLibError.size()) {
          sb.append(
              DockManager.dockControlErrorToString(
                  DockManager.this.mDockControlLibError.keyAt(i2)));
          sb.append(" = ");
        ...
        DockManager.this.mLastDockControlLibMsgState.setLast(
            104, FrameworkStatsLog.f434xe7bba16c, sb.toString());
        return;
      }
      if (i != 231) {
        switch (i) {
          case 201:
            ...
            if (DesktopModeFeature.DEBUG) {
              Log.m7d(DockManager.TAG, "MSG_DOCK_REQUEST_CREATE dockType=" + intValue2);
            }
          ...
          case 202:
            if (DesktopModeFeature.DEBUG) {
              Log.m7d(
                  DockManager.TAG,
                  "MSG_DOCK_REQUEST_POWER_INFO requestConnectedPowerChargerInfoUpdate");
            }
          ...
          case 203:
            ...
            if (DesktopModeFeature.DEBUG) {
              Log.m7d(DockManager.TAG, "MSG_DOCK_REQUEST_DESTROY dockType=" + intValue3);
            }
      ...
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(
            DockManager.TAG,
            "MSG_DOCK_VERSION_DSVERSION_UPDATED onDSVersionUpdated, version=" + str);
      }
  ...
  public class DockMsgState {
    ...
    public String toString(int i) {
      return msgTypeToString(((Integer) this.mMsgState.keyAt(i)).intValue())
          + ", "
          + ((String) this.mMsgState.valueAt(i));
    }
