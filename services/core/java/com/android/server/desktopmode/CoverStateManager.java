...
public class CoverStateManager {
  ...
  public final CoverManager.StateListener mCoverStateListener =
      new CoverManager
          .StateListener() { // from class: com.android.server.desktopmode.CoverStateManager.1
        public void onCoverStateChanged(CoverState coverState) {
          if (DesktopModeFeature.DEBUG) {
            Log.m7d(CoverStateManager.TAG, "mCoverState=" + coverState);
          }
          synchronized (CoverStateManager.this.mLock) {
            if (CoverStateManager.this.mCoverManagerDisabled
                && coverState.attached == CoverStateManager.this.mCoverState.attached) {
              if (DesktopModeFeature.DEBUG) {
                Log.m7d(
                    CoverStateManager.TAG,
                    "onCoverStateChanged - mCoverManagerDisabled && state.attached =="
                        + " mCoverState.attached");
              }
            } else {
  ...
  public final StateManager.StateListener mStateListener =
      new StateManager
          .StateListener() { // from class: com.android.server.desktopmode.CoverStateManager.2
        ...
        public void onDockStateChanged(State state) {
          synchronized (CoverStateManager.this.mLock) {
            if (CoverStateManager.this.mDesktopDockConnected
                != state.getDockState().isDexStation()) {
  ...
  public final SemDesktopModeManager.DesktopModeBlocker mBlocker =
      new SemDesktopModeManager
          .DesktopModeBlocker() { // from class: com.android.server.desktopmode.CoverStateManager.3
        public String onBlocked() {
          if (!CoverStateManager.this.mAuthComplete) {
            Log.m10i(CoverStateManager.TAG, "onBlocked(), !mAuthComplete");
            return null;
          }
          Log.m12w(CoverStateManager.TAG, "onBlocked(), Unknown reason");
          return null;
        }
      };
  ...
  public class Authenticator extends BroadcastReceiver {
    ...
    public void initialize() {
      if (isAuthNeeded()) {
        CoverStateManager.this.mContext.registerReceiverAsUser(
            this,
            UserHandle.ALL,
            new IntentFilter("com.samsung.android.intent.action.ACCESSORY_AUTHENTICATION_STOPPED"),
            null,
            null);
        if (CoverStateManager.this.mInputManagerService != null) {
          CoverStateManager.this.mInputManagerService.setDesktopModeServiceCallbacks(
              new InputManagerService.DesktopModeServiceCallbacks() { // from class:
                // com.android.server.desktopmode.CoverStateManager.Authenticator.1
                ...
                public void notifyUnverifiedCoverAttachChanged(long j, boolean z) {
                  if (DesktopModeFeature.DEBUG) {
                    Log.m7d(
                        CoverStateManager.TAG, "notifyUnverifiedCoverAttachChanged, attached=" + z);
                  }
                  if (z) {
                    return;
                  }
        ...
        CoverStateManager.this.mHandler.removeCallbacksAndMessages(null);
        CoverStateManager.this.mHandler.postDelayed(
            new Runnable() { // from class:
              // com.android.server.desktopmode.CoverStateManager$Authenticator$$ExternalSyntheticLambda0
    ...
    public /* synthetic */ void lambda$initialize$0() {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(
            CoverStateManager.TAG,
            "Cover auth timeout, mAuthComplete=" + CoverStateManager.this.mAuthComplete);
      }
      if (CoverStateManager.this.mAuthComplete) {
        return;
      }
    ...
    public boolean isAuthNeeded() {
      return CoverStateManager.this.mIsNfcAuthSystemFeatureEnabled
          && CoverStateManager.this.mInputManagerService != null
          && CoverStateManager.this.mInputManagerService.getSwitchState(-1, -256, 27) == 1;
    }
    ...
    public void onReceive(Context context, Intent intent) {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(CoverStateManager.TAG, "onReceive(), action=" + intent.getAction());
      }
      CoverStateManager.this.mHandler.removeCallbacksAndMessages(null);
      setAuthCompleteAndResetCallbacksReceiver();
    }
  ...
  public CoverStateManager(
    ...
    this.mIsNfcAuthSystemFeatureEnabled =
        context.getPackageManager().hasSystemFeature("com.sec.feature.nfc_authentication_cover");
  }
  ...
  public final boolean updateCoverSupportStateLocked() {
    ...
    if (this.mAuthComplete) {
      i =
          (!this.mCoverState.attached
                  || this.mStateManager.getState().isDexOnPcOrWirelessDexConnected()
                  || DesktopModeFeature.IS_TABLET
                  || !this.mDesktopDockConnected
    ...
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(
          TAG,
          "updateCoverSupportState(), mCoverSupportState="
              + coverSupportStateToString(this.mCoverSupportState)
              + ", mCoverState="
              + this.mCoverState
              + ", mAuthComplete="
              + this.mAuthComplete
              + ", mDesktopDockConnected="
              + this.mDesktopDockConnected);
    }
    return z;
  }
  ...
  public final void setMirroringSwitchDisabled(boolean z) {
    ...
    DesktopModeSettings.setSettings(
        this.mContext.getContentResolver(), "mirroring_switch_disabled", z);
  }

  public final boolean getSettingMirroringSwitchDisabled() {
    return DesktopModeSettings.getSettings(
        this.mContext.getContentResolver(), "mirroring_switch_disabled", false);
  }
  ...
  public void disableCoverManager(boolean z) {
    synchronized (this.mLock) {
      if (this.mCoverManagerDisabled != z) {
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(TAG, "disableCoverManager()=" + this.mCoverManagerDisabled + " -> " + z);
        }
  ...
  public boolean goToSleepIfFlipTypeCoverClosed() {
    ...
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "goToSleepIfFlipTypeCoverClosed(), coverState=" + coverState);
    }
    if (!coverState.attached
        || !isFlipTypeCover(coverState)
        || isCoverUiTypeCover(coverState)
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter) {
    ...
    synchronized (this.mLock) {
      ...
      indentingPrintWriter.println(
          "mCoverSupportState=" + coverSupportStateToString(this.mCoverSupportState));
      ...
      indentingPrintWriter.println(
          "mIsNfcAuthSystemFeatureEnabled=" + this.mIsNfcAuthSystemFeatureEnabled);
    }
