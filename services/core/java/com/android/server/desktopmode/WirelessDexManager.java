...
public class WirelessDexManager {
            ...
            if (list != null) {
                ...
                while (it.hasNext()) {
                    ...
                    if (DesktopModeFeature.DEBUG) {
                        Log.m7d(WirelessDexManager.TAG, "onParametersChanged (Key : " + key + ", Value : " + value + ")");
                    }
    ...
    public class Receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("com.samsung.intent.action.WIFI_DISPLAY_SOURCE_STATE".equals(action)) {
                ...
                if (DesktopModeFeature.DEBUG) {
                    Log.m7d(WirelessDexManager.TAG, "WifiDisplay Connection state=" + intExtra + ", disconnectedByUser=" + booleanExtra + ", isWirelessDex=" + state.isWirelessDexConnected() + ", mIsWirelessDexEntered=" + WirelessDexManager.this.mIsWirelessDexEntered);
                }
    ...
    public final void showToastWeakNetwork() {
        ...
        if (state.isWirelessDexConnected() && state.getDesktopModeState().compareTo(4, 0)) {
            ToastManager.showToast(Utils.getDisplayContext(this.mContext, state.getDesktopDisplayId()), String.format(this.mContext.getString(R.string.gnss_service), this.mContext.getString(17042504)));
        }
    }
    ...
    public final void showToastToNotifyNetworkDisconnection() {
        ...
        if (DesktopModeFeature.DEBUG) {
            Log.m7d(TAG, "[" + this.mConnectedDeviceName + "] has been disconnected by something other than the user request.");
        }
        Context context = this.mContext;
        ToastManager.showToast(context, String.format(context.getString(R.string.forward_intent_to_owner), this.mConnectedDeviceName));
    }
