...
public class SettingsHelper {
  ...
  public SettingsHelper(
    ...
    StateManager.StateListener stateListener =
        new StateManager
            .StateListener() { // from class: com.android.server.desktopmode.SettingsHelper.1
          ...
          public void onUserChanged(State state) {
            ...
            settingsHelper.backupOrRestoreSettings(
                i == 3 || i == 4, state, state.getCurrentUserId());
            if (DesktopModeFeature.SUPPORT_SFC) {
              SettingsHelper.this.backupOrRestoreSuperFastCharging(
                  state.getDockState().isDexPad(), state.getCurrentUserId());
            }
          }
    ...
    contentResolver.registerContentObserver(
        DesktopModeSettings.getUri(),
        true,
        new SettingsObserver(new Handler(serviceThread.getLooper())),
        -1);
  }
  ...
  public void backupOrRestoreSettings(boolean z, State state, int i) {
    ...
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(
          TAG,
          "backupOrRestoreSettings(), enter="
              + z
              + ", displayType="
              + SemDesktopModeState.displayTypeToString(displayType)
              + ", userId="
              + i);
    }
    if (!z && displayType == 0) {
  ...
  public void clearSettingsByLauncherDataCleared(State state, int i) {
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "clearSettingsByLauncherDataCleared()");
    }
    ...
    if (desktopModeState.enabled == 4) {
      if (desktopModeState.getDisplayType() == 101) {
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(TAG, "set Dex default on virtual keyboard in standalone");
        }
        ...
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(
              TAG,
              "set the setting for touchkeyboard, isDexStationCoverAttached="
                  + state.isDexStationConnectedWithFlipCover());
        }
        DesktopModeSettings.setSettingsAsUser(
            this.mResolver, "touch_keyboard", !state.isDexStationConnectedWithFlipCover(), i);
  ...
  public void setDefaultSettingsInRetailMode(State state, int i) {
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "setDefaultSettingsInRetailMode()");
    }
  ...
  public final void setVirtualKeyboard(boolean z, State state, int i) {
    if (z) {
      if (state.isDexOnPcConnected()) {
        int intForUser =
            Settings.Secure.getIntForUser(this.mResolver, "show_ime_with_hard_keyboard", 1, i);
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(TAG, "setVirtualKeyboard(), current normalModeValue=" + intForUser);
        }
        if (intForUser == 0) {
          DesktopModeSettings.setSettingsAsUser(this.mResolver, "keyboard_backup", intForUser, i);
          setNormalModeVirtualKeyboard(1, i);
          if (DesktopModeFeature.DEBUG) {
            Log.m7d(TAG, "setVirtualKeyboard(), Enabling to show onScreenKeyboard");
            return;
          }
          return;
        }
    ...
    int settingsAsUser =
        DesktopModeSettings.getSettingsAsUser(this.mResolver, "keyboard_backup", 1, i);
    if (settingsAsUser == 0) {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(TAG, "setVirtualKeyboard(), Restoring backed up value=" + settingsAsUser);
      }
      setNormalModeVirtualKeyboard(0, i);
  ...
  public final void setTouchKeyboard(boolean z, State state, int i) {
    if (z) {
      if (state.isDexStationConnectedWithFlipCover()) {
        boolean settingsAsUser =
            DesktopModeSettings.getSettingsAsUser(this.mResolver, "touch_keyboard", true, i);
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(TAG, "setTouchKeyboard(), currentValue=" + settingsAsUser);
        }
        if (settingsAsUser) {
          DesktopModeSettings.setSettingsAsUser(
              this.mResolver, "touch_keyboard_backup", settingsAsUser, i);
          DesktopModeSettings.setSettingsAsUser(this.mResolver, "touch_keyboard", false, i);
          if (DesktopModeFeature.DEBUG) {
            Log.m7d(TAG, "setTouchKeyboard(), Enabling to show on dex display");
            return;
          }
          return;
        }
    ...
    boolean settingsAsUser2 =
        DesktopModeSettings.getSettingsAsUser(this.mResolver, "touch_keyboard_backup", false, i);
    if (settingsAsUser2) {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(TAG, "setTouchKeyboard(), Restoring backed up value=" + settingsAsUser2);
      }
  ...
  public final void setPsmSpeedLimit(boolean z, int i) {
    if (isLowPowerMode()) {
      if (z) {
        ...
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(TAG, "setPsmSpeedLimit(), current value=" + psmValue);
        }
        if (psmValue == 1) {
          ...
          if (DesktopModeFeature.DEBUG) {
            Log.m7d(TAG, "setPsmSpeedLimit(), Disabling speed limit");
            return;
          }
          return;
        }
      ...
      int settingsAsUser =
          DesktopModeSettings.getSettingsAsUser(this.mResolver, "speed_limit_backup", 0, i);
      if (settingsAsUser == 1) {
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(TAG, "setPsmSpeedLimit(), Restoring backed up value=" + settingsAsUser);
        }
  ...
  public final int getPsmValue(String str) {
    ...
    try {
      ...
      Log.m9e(TAG, "NumberFormatException in getPsmValue", e);
      return -1;
    }
  }
  ...
  */
  public final void setPsmValue(String str, int i) {
    int i2;
    int indexOf;
    String string = Settings.Global.getString(this.mResolver, str);
    if (string != null && (indexOf = string.indexOf(44)) > 0) {
      try {
        i2 = Integer.parseInt(string.substring(indexOf + 1).trim());
      } catch (NumberFormatException e) {
        Log.m9e(TAG, "NumberFormatException in setPsmValue", e);
      }
      Settings.Global.putString(this.mResolver, str, i + ", " + i2);
      if (DesktopModeFeature.DEBUG) {
        return;
      }
      Log.m7d(TAG, "setPsmValue = " + i + ", " + i2);
      return;
    }
    i2 = 0;
    Settings.Global.putString(this.mResolver, str, i + ", " + i2);
  ...
  public final void setScreensaver(boolean z, int i) {
    ...
    int settingsAsUser =
        DesktopModeSettings.getSettingsAsUser(this.mResolver, "screensaver_backup", 0, i);
    if (settingsAsUser == 1) {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(TAG, "setScreensaver(), Restoring backed up value=" + settingsAsUser);
      }
  ...
  public void backupOrRestoreSuperFastCharging(boolean z, int i) {
    ...
    if (z) {
      ...
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(
            TAG, "backupOrRestoreFastCharging(), current normalSfcValue=" + superFastChargingValue);
      }
      if (superFastChargingValue == 0) {
        DesktopModeSettings.setSettingsAsUser(
            this.mResolver, "super_fast_charging_backup", superFastChargingValue, i);
        setSuperFastChargingValue(1, i);
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(TAG, "backupOrRestoreFastCharging(), Enabling super fast charging");
        }
      }
    } else {
      int settingsAsUser =
          DesktopModeSettings.getSettingsAsUser(this.mResolver, "super_fast_charging_backup", 1, i);
      if (settingsAsUser == 0) {
        if (DesktopModeFeature.DEBUG) {
          Log.m7d(
              TAG, "backupOrRestoreFastCharging(), Restoring backed up value=" + settingsAsUser);
        }
  ...
  public void dump(
      IndentingPrintWriter indentingPrintWriter, ContentResolver contentResolver, int i) {
    ...
    public void onChange(boolean z, Uri uri, int i) {
      ...
      String settingsAsUser =
          DesktopModeSettings.getSettingsAsUser(
              SettingsHelper.this.mResolver, lastPathSegment, (String) null, i);
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(
            SettingsHelper.TAG,
            "Setting " + lastPathSegment + " for user " + i + " changed to=" + settingsAsUser);
      }
      if (i != 0 && DesktopModeSettings.isGlobalKey(lastPathSegment)) {
        DesktopModeSettings.setSettingsAsUser(
            SettingsHelper.this.mResolver, lastPathSegment, settingsAsUser, 0);
      }
