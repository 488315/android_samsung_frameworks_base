...
public class PackageStateManager extends PackageMonitor {
  ...
  public final boolean isPackageAvailable(String str) {
    ...
    try {
      ApplicationInfo applicationInfoAsUser =
          this.mContext
              .getPackageManager()
              .getApplicationInfoAsUser(str, 0, this.mStateManager.getState().getCurrentUserId());
      if (applicationInfoAsUser != null) {
        boolean z = applicationInfoAsUser.enabled;
        return (!z || (enterpriseDeviceManager = this.mEnterpriseDeviceManager) == null)
            ? z
            : !enterpriseDeviceManager
                .getApplicationPolicy()
                .isApplicationStartDisabledAsUser(str, r0);
      }
    } catch (PackageManager.NameNotFoundException unused) {
      Log.m12w(TAG, "isPackageAvailable failed: unknown package " + str);
    }
    return false;
  }
  ...
  public final boolean updatePackageState(String str) {
    synchronized (this.mLock) {
      ...
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(
            TAG, "updatePackageState(), packageName=" + str + ", available=" + isPackageAvailable);
      }
      return true;
    }
  }
  ...
  public final void setSettingsComponent(State state) {
    ...
    setComponentFromList(state, R.array.config_default_vm_number, (i == 3 || i == 4) ? 1 : 0);
  }
  ...
  public final void setComponentFromList(State state, int i, int i2) {
    for (String str : this.mContext.getResources().getStringArray(i)) {
      ...
      if (unflattenFromString != null) {
        try {
          if (this.mPackageManager.isPackageAvailable(
              unflattenFromString.getPackageName(), state.getCurrentUserId())) {
            this.mPackageManager.setComponentEnabledSetting(
                unflattenFromString, i2, 1, state.getCurrentUserId(), "");
          }
        } catch (RemoteException | IllegalArgumentException e) {
          Log.m9e(TAG, "Failed to enable/disable components", e);
        }
      }
    }
  }
  ...
  public final void setComponent(String str, boolean z) {
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "setComponent(component= " + str + ", enabled=" + z + ")");
    }
    try {
      this.mPackageManager.setComponentEnabledSetting(
          ComponentName.createRelative("com.sec.android.desktopmode.uiservice", str),
          z ? 1 : 2,
          1,
          this.mStateManager.getState().getCurrentUserId(),
          "");
    } catch (RemoteException | IllegalArgumentException unused) {
      Log.m12w(TAG, "Failed to setComponent");
    }
  }
  ...
  public final void register(Handler handler) {
    ...
    intentFilter2.addAction(
        "com.samsung.android.knox.intent.action.ADD_PACKAGE_PREVENT_START_BLACKLIST_CHANGED_INTERNAL");
    this.mContext.registerReceiverAsUser(this, UserHandle.ALL, intentFilter2, null, handler);
  }

  public void onReceive(Context context, Intent intent) {
    if ("com.samsung.android.knox.intent.action.ADD_PACKAGE_PREVENT_START_BLACKLIST_CHANGED_INTERNAL"
        .equals(intent.getAction())) {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(
            TAG,
            "onReceive(),"
                + " action=com.samsung.android.knox.intent.action.ADD_PACKAGE_PREVENT_START_BLACKLIST_CHANGED_INTERNAL");
      }
  ...
  public void onSomePackagesChanged() {
    if (DesktopModeFeature.DEBUG) {
      Log.m7d(TAG, "onSomePackagesChanged()");
    }
    updatePackageState();
  }
  ...
  public void onPackageUpdateFinished(String str, int i) {
    if ("com.sec.android.app.desktoplauncher".equals(str)) {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(TAG, "onPackageUpdateFinished(), packageName=" + str);
      }
  ...
  public void onPackageDataCleared(String str, int i) {
    if ("com.sec.android.app.desktoplauncher".equals(str)) {
      if (DesktopModeFeature.DEBUG) {
        Log.m7d(TAG, "onPackageDataCleared(), packageName=" + str);
      }
      setSettingsComponent(this.mStateManager.getState());
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter) {
    indentingPrintWriter.println(
        "Current " + PackageStateManager.class.getSimpleName() + " state:");
  ...
  public class StateListener extends StateManager.StateListener {
    ...
    public void onUserChanged(State state) {
      PackageStateManager.this.updatePackageState();
      PackageStateManager.this.setComponent(
          "com.sec.android.desktopmode.uiservice.DesktopModeTile", true);
      PackageStateManager.this.setComponent(
          "com.sec.android.desktopmode.uiservice.DesktopDisplayDesktopModeTile", true);
      PackageStateManager.this.setComponent(
          "com.sec.android.desktopmode.uiservice.activity.touchpad.TouchpadActivity", false);
    ...
    public void onDualModeSetDesktopModeInternal(boolean z) {
      ...
      packageStateManager.setSettingsComponent(packageStateManager.mStateManager.getState());
      PackageStateManager.this.setComponent(
          "com.sec.android.desktopmode.uiservice.activity.touchpad.TouchpadActivity", z);
    }
  }
}
