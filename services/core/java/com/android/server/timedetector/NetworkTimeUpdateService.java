...
public class NetworkTimeUpdateService extends Binder {
  ...
  interface Engine {
    ...
    void refreshAndRescheduleIfRequired(
        Network network, String str, RefreshCallbacks refreshCallbacks);
  ...
  public NetworkTimeUpdateService(Context context) {
    ...
    this.mEngine =
        new EngineImpl(
            new Supplier() { // from class:
                             // com.android.server.timedetector.NetworkTimeUpdateService$$ExternalSyntheticLambda0
              @Override // java.util.function.Supplier
              public final Object get() {
                return Long.valueOf(SystemClock.elapsedRealtime());
              }
            },
            context.getResources().getInteger(R.integer.config_powerStatsAggregationPeriod),
            context
                .getResources()
                .getInteger(R.integer.config_previousVibrationsDumpAggregationTimeMillisLimit),
            context.getResources().getInteger(R.integer.config_previousVibrationsDumpSizeLimit),
            ntpTrustedTime);
    final AlarmManager alarmManager = (AlarmManager) context.getSystemService(AlarmManager.class);
    final TimeDetectorInternal timeDetectorInternal =
    ...
    this.mRefreshCallbacks =
        new Engine
            .RefreshCallbacks() { // from class:
                                  // com.android.server.timedetector.NetworkTimeUpdateService.1
  ...
  public void systemRunning() {
    this.mContext.registerReceiver(
        new ScheduledRefreshBroadcastReceiver(),
        new IntentFilter("com.android.server.timedetector.NetworkTimeUpdateService.action.POLL"));
    registerMDMReceiver();
  ...
  public boolean forceRefreshForTests() {
    Network network;
    this.mContext.enforceCallingPermission(
        "android.permission.SET_TIME", "force network time refresh");
    long clearCallingIdentity = Binder.clearCallingIdentity();
    try {
  ...
  public class MDMReceiver extends BroadcastReceiver implements Runnable {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if ("com.samsung.android.knox.intent.action.UPDATE_NTP_PARAMETERS_INTERNAL".equals(action)) {
        NetworkTimeUpdateService.this.mEngine.updateNTPParametersMDM(
            NetworkTimeUpdateService.this.mContext);
        NetworkTimeUpdateService.this.mHandler.post(this);
  ...
  class EngineImpl implements Engine {
    ...
    public boolean forceRefreshForTests(Network network, Engine.RefreshCallbacks refreshCallbacks) {
      ...
      if (tryRefresh) {
        makeNetworkTimeSuggestion(
            this.mNtpTrustedTime.getCachedTimeResult(),
    ...
    */
    public void refreshAndRescheduleIfRequired(
        Network network, String str, Engine.RefreshCallbacks refreshCallbacks) {
      boolean z;
      boolean tryRefresh;
      long j;
      long longValue;
      if (network == null) {
        logToDebugAndDumpsys(
            "refreshIfRequiredAndReschedule: reason="
                + str
                + ": No default network available. No refresh attempted and no next attempt"
                + " scheduled.");
        return;
      }
      ...
      synchronized (this) {
        ...
        z =
            calculateTimeResultAgeMillis(cachedTimeResult, longValue2)
                    >= ((long) this.mNormalPollingIntervalMillis)
                && isRefreshAllowed(longValue2);
      }
      if (z || str.equalsIgnoreCase("NTP_PARAMETERS_UPDATED")) {
        if (mNtpSetByMDM) {
          mForceNtpSetByMDM = true;
        }
        tryRefresh = tryRefresh(network);
      } else {
        tryRefresh = false;
      }
      synchronized (this) {
        NtpTrustedTime.TimeResult cachedTimeResult2 = this.mNtpTrustedTime.getCachedTimeResult();
        long longValue3 = ((Long) this.mElapsedRealtimeMillisSupplier.get()).longValue();
        long calculateTimeResultAgeMillis =
            calculateTimeResultAgeMillis(cachedTimeResult2, longValue3);
        if (z) {
          if (tryRefresh) {
            ...
            if (i < 0) {
              ...
              if (i2 > i) {
                this.mTryAgainCounter = 0;
                if (NetworkTimeUpdateService.mBootCompleted) {
                  NetworkTimeUpdateService.mMDMContext.sendBroadcast(
                      new Intent(
        ...
        long j2 =
            this.mTryAgainCounter > 0
                ? this.mShortPollingIntervalMillis
                : this.mNormalPollingIntervalMillis;
        if (calculateTimeResultAgeMillis < j2) {
          ...
          if (l != null) {
            ...
            Log.w(
                "NetworkTimeUpdateService",
                "mLastRefreshAttemptElapsedRealtimeMillis unexpectedly missing. Scheduling using"
            ...
            if (j <= longValue3) {
              Log.w(
                  "NetworkTimeUpdateService",
                  "nextRefreshElapsedRealtimeMillis is a time in the past. Scheduling using"
                      + " currentElapsedRealtimeMillis instead");
              logToDebugAndDumpsys(
                  "nextRefreshElapsedRealtimeMillis is a time in the past. Scheduling using"
                      + " currentElapsedRealtimeMillis instead");
              j = longValue3 + j2;
            ...
            logToDebugAndDumpsys(
                "refreshIfRequiredAndReschedule: network="
                    + network
                    + ", reason="
                    + str
                    + ", initialTimeResult="
                    ...
                    + tryRefresh
                    + ", currentElapsedRealtimeMillis="
                    + formatElapsedRealtimeMillis(longValue3)
                    + ", latestTimeResult="
                    + cachedTimeResult2
                    + ", mTryAgainCounter="
    ...
    public final void makeNetworkTimeSuggestion(
        NtpTrustedTime.TimeResult timeResult,
        String str,
        Engine.RefreshCallbacks refreshCallbacks) {
      NetworkTimeSuggestion networkTimeSuggestion =
          new NetworkTimeSuggestion(
              new UnixEpochTime(timeResult.getElapsedRealtimeMillis(), timeResult.getTimeMillis()),
    ...
    public void dump(PrintWriter printWriter) {
      ...
      indentingPrintWriter.println(
          "mNormalPollingIntervalMillis=" + this.mNormalPollingIntervalMillis);
      indentingPrintWriter.println(
          "mShortPollingIntervalMillis=" + this.mShortPollingIntervalMillis);
      indentingPrintWriter.println("mTryAgainTimesMax=" + this.mTryAgainTimesMax);
      synchronized (this) {
        Long l = this.mLastRefreshAttemptElapsedRealtimeMillis;
        indentingPrintWriter.println(
            "mLastRefreshAttemptElapsedRealtimeMillis="
                + (l == null ? "null" : formatElapsedRealtimeMillis(l.longValue())));
