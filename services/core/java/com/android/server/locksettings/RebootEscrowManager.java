...
public class RebootEscrowManager {
  ...
  public class Injector {
    ...
    public final RebootEscrowProviderInterface createRebootEscrowProvider() {
      ...
      if (serverBasedResumeOnReboot()) {
        ...
        rebootEscrowProviderHalImpl =
            new RebootEscrowProviderServerBasedImpl(this.mContext, this.mStorage);
      } else {
    ...
    public PowerManager.WakeLock getWakeLock() {
      return ((PowerManager) this.mContext.getSystemService(PowerManager.class))
          .newWakeLock(1, "RebootEscrowManager");
    }
    ...
    public void reportMetric(boolean z, int i, int i2, int i3, int i4, int i5, int i6) {
      FrameworkStatsLog.write(
          FrameworkStatsLog.REBOOT_ESCROW_RECOVERY_REPORTED, z, i, i2, i3, i4, i5, i6);
    }
    ...
    if (this.mInjector.waitForInternet()) {
      this.mInjector.post(
          ...
          new Runnable() { // from class:
                           // com.android.server.locksettings.RebootEscrowManager$$ExternalSyntheticLambda5
  ...
  public void loadRebootEscrowDataIfAvailable(final Handler handler) {
    ...
    if (arrayList.isEmpty()) {
      Slog.i(
          "RebootEscrowManager",
          "No reboot escrow data found for users, skipping loading escrow data");
      clearMetricsStorage();
      return;
    }
    ...
    if (this.mInjector.waitForInternet()) {
      this.mInjector.postDelayed(
          handler,
          new Runnable() { // from class:
                           // com.android.server.locksettings.RebootEscrowManager$$ExternalSyntheticLambda1
            @Override // java.lang.Runnable
            public final void run() {
      ...
      this.mInjector.post(
          handler,
          new Runnable() { // from class:
                           // com.android.server.locksettings.RebootEscrowManager$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
              RebootEscrowManager.this.lambda$loadRebootEscrowDataIfAvailable$2(
                  handler, users, arrayList);
            }
          });
    } else {
      this.mInjector.post(
          handler,
          new Runnable() { // from class:
                           // com.android.server.locksettings.RebootEscrowManager$$ExternalSyntheticLambda3
            @Override // java.lang.Runnable
            public final void run() {
              RebootEscrowManager.this.lambda$loadRebootEscrowDataIfAvailable$3(
                  handler, users, arrayList);
            }
          });
    }
  ...
  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ void lambda$loadRebootEscrowDataIfAvailable$3(
      Handler handler, List list, List list2) {
    lambda$scheduleLoadRebootEscrowDataOrFail$4(handler, 0, list, list2);
  }

  public void scheduleLoadRebootEscrowDataOrFail(
      final Handler handler, final int i, final List list, final List list2) {
    ...
    if (i < loadEscrowDataRetryLimit && !this.mRebootEscrowTimedOut) {
      ...
      this.mInjector.postDelayed(
          ...
          new Runnable() { // from class:
            ...
            @Override // java.lang.Runnable
            public final void run() {
              RebootEscrowManager.this.lambda$scheduleLoadRebootEscrowDataOrFail$4(
                  handler, i, list, list2);
            }
          },
  ...
  public void lambda$loadRebootEscrowDataIfAvailable$2(
      final Handler handler, final List list, final List list2) {
    ...
    ConnectivityManager.NetworkCallback networkCallback =
        new ConnectivityManager
            .NetworkCallback() { // from class:
                                 // com.android.server.locksettings.RebootEscrowManager.1
  ...
  public void lambda$scheduleLoadRebootEscrowDataOrFail$4(
      Handler handler, int i, List list, List list2) {
    ...
    try {
      RebootEscrowKey andClearRebootEscrowKey =
          getAndClearRebootEscrowKey(keyStoreEncryptionKey, handler);
      if (andClearRebootEscrowKey == null) {
        if (this.mLoadEscrowDataErrorCode == 0) {
          if (this.mInjector.serverBasedResumeOnReboot()
              != this.mStorage.getInt("reboot_escrow_key_provider", -1, 0)) {
      ...
      while (it.hasNext()) {
        z &=
            restoreRebootEscrowForUser(
                ((UserInfo) it.next()).id, andClearRebootEscrowKey, keyStoreEncryptionKey);
      }
      if (!z) {
  ...
  public final void reportMetricOnRestoreComplete(boolean z, int i, Handler handler) {
    ...
    Slog.i(
        ...
        "Reporting RoR recovery metrics, success: "
            ...
            + i2
            + ", error code: "
            + this.mLoadEscrowDataErrorCode);
    this.mInjector.reportMetric(
        z, this.mLoadEscrowDataErrorCode, i2, i, i3, vbmetaDigestStatusOnRestoreComplete, -1);
  ...
  public final RebootEscrowKey getAndClearRebootEscrowKey(SecretKey secretKey, Handler handler) {
    RebootEscrowProviderInterface createRebootEscrowProviderIfNeeded =
        this.mInjector.createRebootEscrowProviderIfNeeded();
    ...
    RebootEscrowKey andClearRebootEscrowKey =
        createRebootEscrowProviderIfNeeded.getAndClearRebootEscrowKey(secretKey);
    if (andClearRebootEscrowKey != null) {
  ...
  public final boolean restoreRebootEscrowForUser(
      int i, RebootEscrowKey rebootEscrowKey, SecretKey secretKey) {
    ...
    try {
      ...
      RebootEscrowData fromEncryptedData =
          RebootEscrowData.fromEncryptedData(rebootEscrowKey, readRebootEscrow, secretKey);
      this.mCallbacks.onRebootEscrowRestored(
          fromEncryptedData.getSpVersion(), fromEncryptedData.getSyntheticPassword(), i);
  ...
  public void callToRebootEscrowIfNeeded(int i, byte b, byte[] bArr) {
    if (this.mRebootEscrowWanted) {
      if (this.mInjector.createRebootEscrowProviderIfNeeded() == null) {
        Slog.w(
            "RebootEscrowManager", "Not storing escrow data, RebootEscrowProvider is unavailable");
        return;
      }
      ...
      SecretKey generateKeyStoreEncryptionKeyIfNeeded =
          this.mKeyStoreManager.generateKeyStoreEncryptionKeyIfNeeded();
      if (generateKeyStoreEncryptionKeyIfNeeded == null) {
        Slog.e("RebootEscrowManager", "Failed to generate encryption key from keystore.");
        return;
      }
      try {
        this.mStorage.writeRebootEscrow(
            i,
            RebootEscrowData.fromSyntheticPassword(
                    generateEscrowKeyIfNeeded, b, bArr, generateKeyStoreEncryptionKeyIfNeeded)
                .getBlob());
  ...
  public int armRebootEscrowIfNeeded() {
    ...
    this.mStorage.setLong(
        "reboot_escrow_key_stored_timestamp", this.mInjector.getCurrentTimeMillis(), 0);
    this.mStorage.setString(
        "reboot_escrow_key_vbmeta_digest", this.mInjector.getVbmetaDigest(false), 0);
    this.mStorage.setString(
        "reboot_escrow_key_other_vbmeta_digest", this.mInjector.getVbmetaDigest(true), 0);
  ...
  public boolean prepareRebootEscrow() {
    ...
    if (this.mInjector.createRebootEscrowProviderIfNeeded() == null) {
      Slog.w(
          "RebootEscrowManager",
          "No reboot escrow provider, skipping resume on reboot preparation.");
      return false;
    }
  ...
  public class RebootEscrowEventLog {
    ...
    public void dump(IndentingPrintWriter indentingPrintWriter) {
      ...
      while (true) {
        ...
        RebootEscrowEvent rebootEscrowEvent =
            rebootEscrowEventArr[(this.mNextIndex + i) % rebootEscrowEventArr.length];
        if (rebootEscrowEvent != null) {
          indentingPrintWriter.print("Event #");
          indentingPrintWriter.println(i);
          indentingPrintWriter.println(
              " time="
                  + simpleDateFormat.format(new Date(rebootEscrowEvent.mWallTime))
                  + " (timestamp="
                  + rebootEscrowEvent.mTimestamp
                  + ")");
          indentingPrintWriter.print(" event=");
  ...
  public void dump(IndentingPrintWriter indentingPrintWriter) {
    ...
    indentingPrintWriter.print(
        "RebootEscrowProvider type is "
            + (rebootEscrowProvider == null
                ? "null"
                : String.valueOf(rebootEscrowProvider.getType())));
    indentingPrintWriter.println();
