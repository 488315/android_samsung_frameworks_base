...
public class RebootEscrowProviderServerBasedImpl implements RebootEscrowProviderInterface {
  ...
  public class Injector {
    ...
    public Injector(Context context) {
      ...
      ResumeOnRebootServiceProvider.ResumeOnRebootServiceConnection serviceConnection =
    ...
    public final ResumeOnRebootServiceProvider.ResumeOnRebootServiceConnection
  ...
  public RebootEscrowProviderServerBasedImpl(
      Context context, LockSettingsStorage lockSettingsStorage) {
  ...
  public final byte[] unwrapServerBlob(byte[] bArr, SecretKey secretKey) {
    ResumeOnRebootServiceProvider.ResumeOnRebootServiceConnection serviceConnection =
        this.mInjector.getServiceConnection();
    if (serviceConnection == null) {
      Slog.w(
          "RebootEscrowProviderServerBased",
          "Had reboot escrow data for users, but resume on reboot server service is unavailable");
      return null;
    }
  ...
  public final byte[] wrapEscrowKey(byte[] bArr, SecretKey secretKey) {
    ...
    serviceConnection.bindToService(this.mInjector.getServiceTimeoutInSeconds());
    byte[] wrapBlob =
        serviceConnection.wrapBlob(
            bArr,
            this.mInjector.getServerBlobLifetimeInMillis(),
            this.mInjector.getServiceTimeoutInSeconds());
    serviceConnection.unbindService();
    if (wrapBlob == null) {
      Slog.w(
          "RebootEscrowProviderServerBased", "Server encrypted reboot escrow key cannot be null");
      return null;
    }
