...
public final class PlatformKeyManager {
    ...
    public static void ensureDecryptionKeyIsValid(
            int i, PlatformDecryptionKey platformDecryptionKey) {
        try {
            Cipher.getInstance("AES/GCM/NoPadding")
                    .init(
                            4,
                            platformDecryptionKey.mKey,
    ...
    public static String getDecryptAlias(int i, int i2) {
        return DualAppManagerService$$ExternalSyntheticOutline0.m(
                i,
                i2,
    ...
    public static PlatformKeyManager getInstance(
        ...
        try {
            ...
            return new PlatformKeyManager(
                    applicationContext, new KeyStoreProxyImpl(keyStore), recoverableKeyStoreDb);
    ...
    public final void generateAndLoadKey(int i, int i2) {
        ...
        try {
            ((KeyStoreProxyImpl) keyStoreProxy)
                    .mKeyStore.setEntry(
                            decryptAlias,
                            new KeyStore.SecretKeyEntry(generateKey),
                            encryptionPaddings.build());
            ((KeyStoreProxyImpl) keyStoreProxy)
                    .mKeyStore.setEntry(
                            encryptAlias,
                            new KeyStore.SecretKeyEntry(generateKey),
            ...
            Pageboost$PageboostFileDBHelper$$ExternalSyntheticOutline0.m(
                    i, contentValues, "user_id", i2, "platform_key_generation_id");
            ...
            writableDatabase2.update(
                    "keys", contentValues2, "user_id = ?", new String[] {String.valueOf(i)});
    ...
    public final PlatformDecryptionKey getDecryptKeyInternal(int i) {
        ...
        if (isKeyLoaded(i, platformKeyGenerationId)) {
            return new PlatformDecryptionKey(
                    ...
                    (SecretKey)
                            ((KeyStoreProxyImpl) this.mKeyStore)
                                    .mKeyStore.getKey(decryptAlias, null));
        }
        throw new UnrecoverableKeyException(
                ConnectivityModuleConnector$$ExternalSyntheticOutline0.m(
                        "KeyStore doesn't contain key ", decryptAlias));
    }
    ...
    public final PlatformEncryptionKey getEncryptKey(int i) {
        ...
        try {
            ...
            Log.i(
                    "PlatformKeyManager",
                    "Regenerating permanently invalid Platform key for user " + i + ".");
            this.regenerate(i);
    ...
    public final PlatformEncryptionKey getEncryptKeyInternal(int i) {
        ...
        if (isKeyLoaded(i, platformKeyGenerationId)) {
            return new PlatformEncryptionKey(
                    ...
                    (SecretKey)
                            ((KeyStoreProxyImpl) this.mKeyStore)
                                    .mKeyStore.getKey(encryptAlias, null));
        }
        throw new UnrecoverableKeyException(
                ConnectivityModuleConnector$$ExternalSyntheticOutline0.m(
                        "KeyStore doesn't contain key ", encryptAlias));
    }
    ...
    public final void init(int i) {
        ...
        if (isKeyLoaded(i, platformKeyGenerationId)) {
            ...
            Log.i(
                    "PlatformKeyManager",
                    "Platform key generation " + platformKeyGenerationId + " exists already.");
            return;
        }
        int i2 = 1;
        if (platformKeyGenerationId == -1) {
            ...
            Log.w(
                    "PlatformKeyManager",
                    "Platform generation ID was "
                            + platformKeyGenerationId
                            + " but no entry was present in AndroidKeyStore. Generating fresh"
                            + " key.");
