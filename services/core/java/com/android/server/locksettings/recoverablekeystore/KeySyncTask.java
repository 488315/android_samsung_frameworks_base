...
public class KeySyncTask implements Runnable {
  ...
  public final void syncKeys() {
    ...
    if (isCustomLockScreen()) {
      Log.w(
          "KeySyncTask",
          "Unsupported credential type " + this.mCredentialType + " for user " + this.mUserId);
      if (generationId < 1001000) {
  ...
  public final void syncKeysForAgent(int i) {
    ...
    if (shouldCreateSnapshot(i)) {
      ...
      z =
          this.mRecoverableKeyStoreDb.getSnapshotVersion(this.mUserId, i) != null
              && this.mRecoverySnapshotStorage.get(i) == null;
      if (z) {
    ...
    String defaultCertificateAliasIfEmpty =
        this.mTestOnlyInsecureCertificateHelper.getDefaultCertificateAliasIfEmpty(
            this.mRecoverableKeyStoreDb.getActiveRootOfTrust(this.mUserId, i));
    CertPath recoveryServiceCertPath =
        this.mRecoverableKeyStoreDb.getRecoveryServiceCertPath(
            this.mUserId, i, defaultCertificateAliasIfEmpty);
    if (recoveryServiceCertPath != null) {
      ...
      recoveryServicePublicKey =
          this.mRecoverableKeyStoreDb.getRecoveryServicePublicKey(this.mUserId, i);
    }
    ...
    if (this.mTestOnlyInsecureCertificateHelper.isTestOnlyCertificateAlias(
        defaultCertificateAliasIfEmpty)) {
      Log.w("KeySyncTask", "Insecure root certificate is used by recovery agent " + i);
      if (this.mTestOnlyInsecureCertificateHelper.doesCredentialSupportInsecureMode(
          this.mCredentialType, this.mCredential)) {
        Log.w(
            "KeySyncTask",
            "Whitelisted credential is used to generate snapshot by recovery agent " + i);
      } else {
        Log.w(
            "KeySyncTask",
            "Non whitelisted credential is used to generate recovery snapshot by "
                + i
                + " - ignore attempt.");
        return;
      }
    ...
    try {
      ...
      if (this.mTestOnlyInsecureCertificateHelper.isTestOnlyCertificateAlias(
          defaultCertificateAliasIfEmpty)) {
        keysToSync =
            this.mTestOnlyInsecureCertificateHelper.keepOnlyWhitelistedInsecureKeys(keysToSync);
      }
      try {
        SecretKey generateRecoveryKey = generateRecoveryKey();
        try {
          Map encryptKeysWithRecoveryKey =
              KeySyncUtils.encryptKeysWithRecoveryKey(generateRecoveryKey, keysToSync);
          ...
          try {
            byte[] thmEncryptRecoveryKey =
                KeySyncUtils.thmEncryptRecoveryKey(
                    recoveryServicePublicKey,
                    hashCredentialsBySaltedSha256,
                    KeySyncUtils.packVaultParams(
                        recoveryServicePublicKey, counterId.longValue(), 10, serverParams),
                    generateRecoveryKey);
            if (shouldUseScryptToHashCredential) {
            ...
            KeyChainProtectionParams build =
                new KeyChainProtectionParams.Builder()
                    .setUserSecretType(100)
                    .setLockScreenUiFormat(getUiFormat(this.mCredentialType))
                    .setKeyDerivationParams(createSha256Params)
                    .setSecret(new byte[0])
                    .build();
            ArrayList arrayList = new ArrayList();
            arrayList.add(build);
            KeyChainSnapshot.Builder encryptedRecoveryKeyBlob =
                new KeyChainSnapshot.Builder()
                    .setSnapshotVersion(getSnapshotVersion(i, z))
                    .setMaxAttempts(10)
                    .setCounterId(counterId.longValue())
                    .setServerParams(serverParams)
                    .setKeyChainProtectionParams(arrayList)
                    .setWrappedApplicationKeys(
                        createApplicationKeyEntries(encryptKeysWithRecoveryKey, keysToSync))
                    .setEncryptedRecoveryKeyBlob(thmEncryptRecoveryKey);
            try {
              ...
              Log.wtf(
                  "KeySyncTask",
                  "Cannot serialize CertPath when calling setTrustedHardwareCertPath",
                  e);
            }
      ...
      Log.e(
          "KeySyncTask",
          "Loaded keys for same generation ID as platform key, so BadPlatformKeyException should be"
              + " impossible.",
          e6);
    } catch (InsecureUserException e7) {
      Log.e(
          "KeySyncTask",
          "A screen unlock triggered the key sync flow, so user must have lock screen. This should"
              + " be impossible.",
          e7);
  ...
  public final Map getKeysToSync(int i) {
    ...
    return WrappedKey.unwrapKeys(
        decryptKey,
        this.mRecoverableKeyStoreDb.getAllKeys(this.mUserId, i, decryptKey.getGenerationId()));
  }

  public final boolean shouldCreateSnapshot(int i) {
    if (!ArrayUtils.contains(
        this.mRecoverableKeyStoreDb.getRecoverySecretTypes(this.mUserId, i), 100)) {
      return false;
    }
    if (this.mCredentialUpdated
        && this.mRecoverableKeyStoreDb.getSnapshotVersion(this.mUserId, i) != null) {
    ...
    for (String str : map.keySet()) {
      arrayList.add(
          new WrappedApplicationKey.Builder()
              .setAlias(str)
              .setEncryptedKeyMaterial((byte[]) map.get(str))
              .setMetadata((byte[]) ((Pair) map2.get(str)).second)
              .build());
    }
    return arrayList;
  }
