    ...
    try {
      ...
      try {
        ...
        throw new RuntimeException(
            "IllegalBlockSizeException should not be thrown by AES/GCM/NoPadding mode.", e);
      }
  ...
  public static Map unwrapKeys(PlatformDecryptionKey platformDecryptionKey, Map map) {
    ...
    for (String str : map.keySet()) {
      ...
      if (wrappedKey.getPlatformKeyGenerationId() != generationId) {
        throw new BadPlatformKeyException(
            String.format(
                Locale.US,
                "WrappedKey with alias '%s' was wrapped with platform key %d, not platform key %d",
                str,
                Integer.valueOf(wrappedKey.getPlatformKeyGenerationId()),
                Integer.valueOf(platformDecryptionKey.getGenerationId())));
      }
      cipher.init(
          4, platformDecryptionKey.getKey(), new GCMParameterSpec(128, wrappedKey.getNonce()));
      try {
        hashMap.put(
            str,
            Pair.create(
                (SecretKey) cipher.unwrap(wrappedKey.getKeyMaterial(), "AES", 3),
                wrappedKey.getKeyMetadata()));
      } catch (InvalidKeyException | NoSuchAlgorithmException e) {
        Log.e(
            "WrappedKey",
            String.format(Locale.US, "Error unwrapping recoverable key with alias '%s'", str),
            e);
      }
    }
    return hashMap;
