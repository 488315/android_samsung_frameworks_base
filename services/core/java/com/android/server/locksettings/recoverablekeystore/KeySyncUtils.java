...
public abstract class KeySyncUtils {
  ...
  public static byte[] thmEncryptRecoveryKey(
      PublicKey publicKey, byte[] bArr, byte[] bArr2, SecretKey secretKey) {
    return SecureBox.encrypt(
        publicKey,
        calculateThmKfHash(bArr),
        ArrayUtils.concat(new byte[][] {THM_ENCRYPTED_RECOVERY_KEY_HEADER, bArr2}),
        locallyEncryptRecoveryKey(bArr, secretKey));
  }
  ...
  public static byte[] locallyEncryptRecoveryKey(byte[] bArr, SecretKey secretKey) {
    return SecureBox.encrypt(
        null, bArr, LOCALLY_ENCRYPTED_RECOVERY_KEY_HEADER, secretKey.getEncoded());
  }
  ...
  public static Map encryptKeysWithRecoveryKey(SecretKey secretKey, Map map) {
    ...
    for (String str : map.keySet()) {
      ...
      hashMap.put(
          str, SecureBox.encrypt(null, secretKey.getEncoded(), concat, secretKey2.getEncoded()));
    }
    return hashMap;
  }
  ...
  public static byte[] encryptRecoveryClaim(
      PublicKey publicKey, byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4) {
    return SecureBox.encrypt(
        publicKey,
        null,
        ArrayUtils.concat(new byte[][] {RECOVERY_CLAIM_HEADER, bArr, bArr2}),
        ArrayUtils.concat(new byte[][] {bArr3, bArr4}));
  }

  public static byte[] decryptRecoveryClaimResponse(byte[] bArr, byte[] bArr2, byte[] bArr3) {
    return SecureBox.decrypt(
        null, bArr, ArrayUtils.concat(new byte[][] {RECOVERY_RESPONSE_HEADER, bArr2}), bArr3);
  }
  ...
  public static byte[] packVaultParams(PublicKey publicKey, long j, int i, byte[] bArr) {
    return ByteBuffer.allocate(bArr.length + 77)
        .order(ByteOrder.LITTLE_ENDIAN)
        .put(SecureBox.encodePublicKey(publicKey))
        .putLong(j)
        .putInt(i)
        .put(bArr)
        .array();
