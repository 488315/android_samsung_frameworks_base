...
public final class ZramWriteback extends JobService {
  ...
  private int getWrittenPageCount() {
    ...
    try {
      String wbStats = FileUtils.readTextFile(new File(wbStatsPath), WB_STATS_MAX_FILE_SIZE, "");
  ...
  private static boolean isWritebackEnabled() {
    try {
      String backingDev =
          FileUtils.readTextFile(new File(String.format(BDEV_SYS, sZramDeviceId)), 128, "");
  ...
  private static void schedNextWriteback(Context context) {
    ...
    js.schedule(
        new JobInfo.Builder(WRITEBACK_IDLE_JOB_ID, sZramWriteback)
