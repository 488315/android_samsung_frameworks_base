...
public final class GlobalKeyManager {
  ...
  public boolean handleGlobalKey(Context context, int i, KeyEvent keyEvent) {
    ...
    if (this.mKeyMapping.size() <= 0
        || (globalKeyAction = (GlobalKeyAction) this.mKeyMapping.get(i)) == null) {
      return false;
    }
    context.sendBroadcastAsUser(
        new GlobalKeyIntent(globalKeyAction.mComponentName, keyEvent, this.mBeganFromNonInteractive)
            .getIntent(),
        UserHandle.CURRENT,
        null);
              ...
              if (attributeValue != null && attributeValue2 != null) {
                ...
                if (keyCodeFromString != 0) {
                  this.mKeyMapping.put(
                      keyCodeFromString, new GlobalKeyAction(attributeValue2, attributeValue3));
                } else {
                  Log.wtf(
                      "GlobalKeyManager",
                      "Global keys entry does not map to a valid key code: " + attributeValue);
                }
              }
  ...
  public void dump(String str, PrintWriter printWriter) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      printWriter.print("=");
      printWriter.print(
          ((GlobalKeyAction) this.mKeyMapping.valueAt(i)).mComponentName.flattenToString());
      printWriter.print(",dispatchWhenNonInteractive=");
      printWriter.println(
          ((GlobalKeyAction) this.mKeyMapping.valueAt(i)).mDispatchWhenNonInteractive);
    }
    printWriter.print(str);
    printWriter.println("}");
  }
