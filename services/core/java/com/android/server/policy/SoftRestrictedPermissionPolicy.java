...
public abstract class SoftRestrictedPermissionPolicy {
  public static final SoftRestrictedPermissionPolicy DUMMY_POLICY =
      new SoftRestrictedPermissionPolicy() { // from class:
                                             // com.android.server.policy.SoftRestrictedPermissionPolicy.1
  ...
  public static final HashSet sForcedScopedStorageAppWhitelist =
      new HashSet(Arrays.asList(getForcedScopedStorageAppWhitelist()));
        ...
        if (!str.equals(applicationInfo.packageName)) {
          try {
            i =
                Integer.min(
                    i,
                    packageManager.getApplicationInfoAsUser(str, 0, userHandle).targetSdkVersion);
  ...
  public static SoftRestrictedPermissionPolicy forPermission(
      ...
      if (str.equals("android.permission.WRITE_EXTERNAL_STORAGE")) {
        if (applicationInfo != null) {
          boolean z9 =
              (context
                          .getPackageManager()
                          .getPermissionFlags(str, applicationInfo.packageName, userHandle)
                      & 14336)
                  != 0;
    ...
    if (applicationInfo != null) {
      ...
      boolean hasUidRequestedLegacyExternalStorage =
          hasUidRequestedLegacyExternalStorage(applicationInfo.uid, context);
      boolean hasWriteMediaStorageGrantedForUid =
          hasWriteMediaStorageGrantedForUid(applicationInfo.uid, context);
  ...
  public static boolean hasUidRequestedLegacyExternalStorage(int i, Context context) {
    ...
    for (String str : packagesForUid) {
      if (packageManager
          .getApplicationInfoAsUser(str, 0, userHandleForUid)
  ...
  public static String[] getForcedScopedStorageAppWhitelist() {
    String string =
        DeviceConfig.getString("storage_native_boot", "forced_scoped_storage_whitelist", "");
