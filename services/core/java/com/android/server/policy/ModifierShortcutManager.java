...
public final class ModifierShortcutManager {
    ...
    public ModifierShortcutManager(
            Context context,
        ...
        roleManager.addOnRoleHoldersChangedListenerAsUser(
                ...
                new OnRoleHoldersChangedListener() { // from class:
                                                     // com.android.server.policy.ModifierShortcutManager$$ExternalSyntheticLambda1
                ...
                if (xml.getEventType() != 1 && "bookmark".equals(xml.getName())) {
                    ...
                    if (TextUtils.isEmpty(attributeValue3)) {
                        Log.w(
                                ...
                                "Shortcut required for bookmark with category="
                                        + attributeValue4
                                        + " packageName="
                                        + attributeValue
                                        + " className="
                        ...
                        if (attributeValue == null || attributeValue2 == null) {
                            if (attributeValue4 != null) {
                                if (attributeValue6 != null) {
                                    Log.w(
                                            "ModifierShortcutManager",
                                            "Cannot specify role bookmark when category is present"
                                                + " for bookmark shortcutChar="
                                                    + ((int) charAt)
                                                    + " category= "
                                                    + attributeValue4);
                                } else {
                                    intent =
                                            Intent.makeMainSelectorActivity(
                                                    "android.intent.action.MAIN", attributeValue4);
                                }
                                ...
                                Log.w(
                                        ...
                                        "Unable to add bookmark for shortcut "
                                                + attributeValue3
                                                + ": missing package/class, category or role"
                                                + " attributes");
                            } else if (z) {
                            ...
                            ComponentName componentName =
                                    new ComponentName(attributeValue, attributeValue2);
                            try {
                            ...
                            Log.w(
                                    "ModifierShortcutManager",
                                    "Cannot specify role or category when package and class are"
                                        + " present for bookmark packageName="
                                            + attributeValue
                                            + " className="
                                            + attributeValue2
                                            + " shortcutChar="
    ...
    public final Intent getRoleLaunchIntent(String str) {
        ...
        Intent launchIntentForPackage =
                this.mPackageManager.getLaunchIntentForPackage(defaultApplication);
        launchIntentForPackage.putExtra(
                "com.android.server.policy.ModifierShortcutManager.EXTRA_ROLE", str);
