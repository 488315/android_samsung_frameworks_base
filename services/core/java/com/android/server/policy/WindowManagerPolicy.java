...
public interface WindowManagerPolicy extends WindowManagerPolicyConstants {
  ...
  WindowManagerPolicyExt createPolicyExtension(
      Context context, WindowManagerPolicyExt.WindowManagerFuncs windowManagerFuncs);
  ...
  public interface WindowManagerFuncs {
    ...
    static String lidStateToString(int i) {
      return i != -1
          ? i != 0 ? i != 1 ? Integer.toString(i) : "LID_OPEN" : "LID_CLOSED"
          : "LID_ABSENT";
    }

    static String cameraLensStateToString(int i) {
      return i != -1
          ? i != 0 ? i != 1 ? Integer.toString(i) : "CAMERA_LENS_COVERED" : "CAMERA_LENS_UNCOVERED"
  ...
  default int getWindowLayerLw(WindowState windowState) {
    return getWindowLayerFromTypeLw(
        windowState.getBaseType(), windowState.canAddInternalSystemWindow());
  }
    ...
    switch (i) {
      ...
      case 2005:
        if (!WmCoverState.isEnabled()
            || (windowLayerFromTypeLw = WmCoverState.getInstance().getWindowLayerFromTypeLw(i))
                == -1) {
          return 21;
        }
