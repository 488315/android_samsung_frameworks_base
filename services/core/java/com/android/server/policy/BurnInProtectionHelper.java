...
public class BurnInProtectionHelper
  ...
  public BroadcastReceiver mBurnInProtectionReceiver =
      new BroadcastReceiver() { // from class: com.android.server.policy.BurnInProtectionHelper.1
  ...
  public BurnInProtectionHelper(Context context, int i, int i2, int i3, int i4, int i5) {
    ...
    context.registerReceiver(
        this.mBurnInProtectionReceiver,
    ...
    ValueAnimator ofFloat =
        ValueAnimator.ofFloat(1.0f, DisplayPowerController2.RATE_FROM_DOZE_TO_ON);
  ...
  public final void updateBurnInProtection() {
    ...
    if (this.mBurnInProtectionActive) {
      ...
      this.mAlarmManager.setExact(
          3,
          elapsedRealtime + (((j2 - (j2 % j)) + j) - currentTimeMillis),
  ...
  public void dump(String str, PrintWriter printWriter) {
    ...
    printWriter.println(
        str2
            ...
            + this.mMinHorizontalBurnInOffset
            + ", "
            + this.mMaxHorizontalBurnInOffset
            + ")");
    printWriter.println(
        str2
            + "mVerticalBurnInOffsetsBounds=("
            + this.mMinVerticalBurnInOffset
            + ", "
            + this.mMaxVerticalBurnInOffset
            + ")");
    ...
    printWriter.println(
        str2
            + "mLastBurnInOffset=("
            + this.mLastBurnInXOffset
            + ", "
            + this.mLastBurnInYOffset
            + ")");
    printWriter.println(
        str2
            + "mOfsetChangeDirections=("
            + this.mXOffsetDirection
            + ", "
            + this.mYOffsetDirection
            + ")");
  }
