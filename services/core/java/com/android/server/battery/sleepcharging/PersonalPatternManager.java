...
public final class PersonalPatternManager {
    ...
    try {
      Bundle call =
          this.mContext
              .getContentResolver()
              .call(
                  RunestoneStateContract.CONTENT_URI,
                  "getRubinState",
                  "android_com.android.server.BatteryService",
                  (Bundle) null);
      if (call != null) {
        ...
        message =
            "currentRubinState:"
                + call.getString("currentRubinState")
                + " ,isDeviceRubinSupported:"
                + z
                + " ,isDeviceRubinWorkable:"
                + z2
  ...
  public Map getSleepPatterns() {
    ...
    try {
      Cursor query =
          this.mContext
              .getContentResolver()
              .query(SleepPatternContract.SleepPatternInfo.CONTENT_URI, null, bundle, null);
      try {
        if (query == null) {
  ...
  public class SleepPattern {
    ...
    public String getInfo() {
      ...
      sb.append(
          " ,time:"
              + convertMillisToLocalTime(this.bedTimeMillis)
              + " ~ "
              + convertMillisToLocalTime(this.wakeupTimeMillis));
