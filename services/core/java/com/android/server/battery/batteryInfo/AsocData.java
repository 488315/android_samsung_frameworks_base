...
public final class AsocData extends BaseData {
    public final void updateAndSet() {
        long readNodeAsLong$1 =
                BattUtils.readNodeAsLong$1("/sys/class/power_supply/battery/fg_asoc");
        String m =
                DeviceIdleController$$ExternalSyntheticOutline0.m(
                        readNodeAsLong$1, "[updateAndSet]asocFromDriver:");
        String str = this.TAG;
        Slog.d(str, m);
        for (int i = 0; i < this.mBatteryCount; i++) {
            ((Long[]) this.mCurrentValues)[i] =
                    Long.valueOf(BattUtils.readNodeAsLong$1((String) this.efsPaths.get(i)));
            ...
            if (((Long[]) this.mCurrentValues)[i].longValue() < 0) {
                ...
                if (!this.authPaths.isEmpty()) {
                    if (this.mAuthentificationResults[i]) {
                        ...
                        DeviceIdleController$$ExternalSyntheticOutline0.m(
                                i,
                                "[updateAndSet]Authentification false => skip writing asoc_",
                                str);
                    }
                }
            }
            if (readNodeAsLong$1 >= 0
                    && readNodeAsLong$1 < ((Long[]) this.mCurrentValues)[i].longValue()
                    && ((Long[]) this.mCurrentValues)[i].longValue() - readNodeAsLong$1 < 10) {
