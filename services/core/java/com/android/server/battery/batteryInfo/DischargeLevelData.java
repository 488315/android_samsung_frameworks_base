...
public final class DischargeLevelData extends BaseCycleData {
    public final void setCycle(FullStatusUsageData fullStatusUsageData) {
        String format;
        long asLong =
                Arrays.stream((Long[]) this.mCurrentValues)
                        .mapToLong(new AsocData$$ExternalSyntheticLambda1())
                        .max()
                        .getAsLong();
        String str =
                "[setCycle]mCurrentValues:"
        ...
        if (fullStatusUsageData == null) {
            ...
            long asLong2 =
                    Arrays.stream((Long[]) fullStatusUsageData.mCurrentValues)
                            .mapToLong(new AsocData$$ExternalSyntheticLambda1())
                            .max()
                            .getAsLong();
            Slog.d(
                    str2,
                    "[setCycle]mCurrentValues:"
                            + Arrays.toString(fullStatusUsageData.mCurrentValues)
                            + "=> maxFullStatusUsage:"
                            + asLong2);
            format =
                    String.format("%d %d", Long.valueOf(asLong / 100), Long.valueOf(asLong2 / 720));
        }
    ...
    public final void updateEfsFromSBP(long j) {
        String m =
                DeviceIdleController$$ExternalSyntheticOutline0.m(
                        j, "[updateEfsFromSBP]addtionalValue:");
        String str = this.TAG;
        Slog.d(str, m);
        for (int i = 0; i < this.mBatteryCount; i++) {
            if (this.mAuthentificationResults[i]) {
                ((Long[]) this.mCurrentValues)[i] =
                        Long.valueOf(
                                BattUtils.readNodeAsLong((String) this.authPaths.get(i)) * 100);
            } else {
                DeviceIdleController$$ExternalSyntheticOutline0.m(
                        i, "[updateEfsFromSBP]Authentification false => use only efs_", str);
                ((Long[]) this.mCurrentValues)[i] =
