...
public abstract class ProtoStreamUtils {
  ...
  public abstract class PowerEntityUtils {
    ...
    public static void dumpsys(PowerEntity[] powerEntityArr, PrintWriter printWriter) {
      ...
      for (int i = 0; i < powerEntityArr.length; i++) {
        printWriter.println(
            "PowerEntityId: "
                + powerEntityArr[i].f8id
                + ", PowerEntityName: "
                + powerEntityArr[i].name);
        if (powerEntityArr[i].states != null) {
          for (int i2 = 0; i2 < powerEntityArr[i].states.length; i2++) {
            printWriter.println(
                "  StateId: "
                    + powerEntityArr[i].states[i2].f9id
                    + ", StateName: "
                    + powerEntityArr[i].states[i2].name);
          }
        }
      }
    }
  ...
  public abstract class StateResidencyResultUtils {
    public static void adjustTimeSinceBootToEpoch(
        StateResidencyResult[] stateResidencyResultArr, long j) {
  ...
  public abstract class EnergyMeasurementUtils {
    public static void adjustTimeSinceBootToEpoch(
        EnergyMeasurement[] energyMeasurementArr, long j) {
      ...
      while (true) {
        try {
          ...
          if (nextField == 2) {
            ...
            Slog.e(
                ProtoStreamUtils.TAG,
                "Unhandled field in proto: " + ProtoUtils.currentFieldToString(protoInputStream));
          }
        } catch (WireTypeMismatchException unused) {
          Slog.e(
              ProtoStreamUtils.TAG,
              "Wire Type mismatch in proto: " + ProtoUtils.currentFieldToString(protoInputStream));
        }
      }
    }
  ...
  public abstract class EnergyConsumerUtils {
    ...
    public static void dumpsys(EnergyConsumer[] energyConsumerArr, PrintWriter printWriter) {
      ...
      for (int i = 0; i < energyConsumerArr.length; i++) {
        printWriter.println(
            "EnergyConsumerId: "
                + energyConsumerArr[i].f5id
                + ", Ordinal: "
                + energyConsumerArr[i].ordinal
                + ", Type: "
                + ((int) energyConsumerArr[i].type)
                + ", Name: "
                + energyConsumerArr[i].name);
      }
    }
  }
  ...
  public abstract class EnergyConsumerResultUtils {
    public static void adjustTimeSinceBootToEpoch(
        EnergyConsumerResult[] energyConsumerResultArr, long j) {
