...
public final class PowerStatsLogger extends Handler {
  ...
  public void handleMessage(Message message) {
    ...
    if (i == 0) {
      StateResidencyResult[] stateResidency =
          this.mPowerStatsHALWrapper.getStateResidency(new int[0]);
      ProtoStreamUtils.StateResidencyResultUtils.adjustTimeSinceBootToEpoch(
          stateResidency, this.mStartWallTime);
      this.mPowerStatsResidencyStorage.write(
          ProtoStreamUtils.StateResidencyResultUtils.getProtoBytes(stateResidency));
    } else if (i == 1) {
      EnergyConsumerResult[] energyConsumed =
          this.mPowerStatsHALWrapper.getEnergyConsumed(new int[0]);
      ProtoStreamUtils.EnergyConsumerResultUtils.adjustTimeSinceBootToEpoch(
          energyConsumed, this.mStartWallTime);
      this.mPowerStatsModelStorage.write(
          ProtoStreamUtils.EnergyConsumerResultUtils.getProtoBytes(energyConsumed, true));
    } else {
      if (i != 2) {
        return;
      }
      EnergyMeasurement[] readEnergyMeter = this.mPowerStatsHALWrapper.readEnergyMeter(new int[0]);
      ProtoStreamUtils.EnergyMeasurementUtils.adjustTimeSinceBootToEpoch(
          readEnergyMeter, this.mStartWallTime);
      this.mPowerStatsMeterStorage.write(
          ProtoStreamUtils.EnergyMeasurementUtils.getProtoBytes(readEnergyMeter));
      EnergyConsumerResult[] energyConsumed2 =
          this.mPowerStatsHALWrapper.getEnergyConsumed(new int[0]);
      ProtoStreamUtils.EnergyConsumerResultUtils.adjustTimeSinceBootToEpoch(
          energyConsumed2, this.mStartWallTime);
      this.mPowerStatsModelStorage.write(
          ProtoStreamUtils.EnergyConsumerResultUtils.getProtoBytes(energyConsumed2, false));
    }
  }
  ...
  public void writeMeterDataToFile(FileDescriptor fileDescriptor) {
    ...
    try {
      ProtoStreamUtils.ChannelUtils.packProtoMessage(
          this.mPowerStatsHALWrapper.getEnergyMeterInfo(), protoOutputStream);
      this.mPowerStatsMeterStorage.read(
          new PowerStatsDataStorage
              .DataElementReadCallback() { // from class:
                                           // com.android.server.powerstats.PowerStatsLogger.1
            ...
            public void onReadDataElement(byte[] bArr) {
              try {
                ...
                ProtoStreamUtils.EnergyMeasurementUtils.packProtoMessage(
                    ProtoStreamUtils.EnergyMeasurementUtils.unpackProtoMessage(bArr),
                    protoOutputStream);
              } catch (IOException e) {
                Slog.e(
                    PowerStatsLogger.TAG,
  ...
  public void writeModelDataToFile(FileDescriptor fileDescriptor) {
    final ProtoOutputStream protoOutputStream = new ProtoOutputStream(fileDescriptor);
    try {
      ProtoStreamUtils.EnergyConsumerUtils.packProtoMessage(
          this.mPowerStatsHALWrapper.getEnergyConsumerInfo(), protoOutputStream);
      this.mPowerStatsModelStorage.read(
          new PowerStatsDataStorage
              .DataElementReadCallback() { // from class:
                                           // com.android.server.powerstats.PowerStatsLogger.2
            @Override // com.android.server.powerstats.PowerStatsDataStorage.DataElementReadCallback
            public void onReadDataElement(byte[] bArr) {
              try {
                new ProtoInputStream(new ByteArrayInputStream(bArr));
                ProtoStreamUtils.EnergyConsumerResultUtils.packProtoMessage(
                    ProtoStreamUtils.EnergyConsumerResultUtils.unpackProtoMessage(bArr),
                    protoOutputStream,
                    true);
              } catch (IOException e) {
                Slog.e(
  ...
  public void writeResidencyDataToFile(FileDescriptor fileDescriptor) {
    final ProtoOutputStream protoOutputStream = new ProtoOutputStream(fileDescriptor);
    try {
      ProtoStreamUtils.PowerEntityUtils.packProtoMessage(
          this.mPowerStatsHALWrapper.getPowerEntityInfo(), protoOutputStream);
      this.mPowerStatsResidencyStorage.read(
          new PowerStatsDataStorage
              .DataElementReadCallback() { // from class:
                                           // com.android.server.powerstats.PowerStatsLogger.3
            @Override // com.android.server.powerstats.PowerStatsDataStorage.DataElementReadCallback
            public void onReadDataElement(byte[] bArr) {
              try {
                new ProtoInputStream(new ByteArrayInputStream(bArr));
                ProtoStreamUtils.StateResidencyResultUtils.packProtoMessage(
                    ProtoStreamUtils.StateResidencyResultUtils.unpackProtoMessage(bArr),
                    protoOutputStream);
              } catch (IOException e) {
                Slog.e(
                    PowerStatsLogger.TAG, "Failed to write residency data to incident report.", e);
              }
            }
          });
    } catch (IOException e) {
  ...
  public PowerStatsLogger(
    ...
    PowerStatsDataStorage powerStatsDataStorage =
        new PowerStatsDataStorage(context, this.mDataStoragePath, str);
    this.mPowerStatsMeterStorage = powerStatsDataStorage;
    PowerStatsDataStorage powerStatsDataStorage2 =
        new PowerStatsDataStorage(context, this.mDataStoragePath, str3);
    this.mPowerStatsModelStorage = powerStatsDataStorage2;
    PowerStatsDataStorage powerStatsDataStorage3 =
        new PowerStatsDataStorage(context, this.mDataStoragePath, str5);
    this.mPowerStatsResidencyStorage = powerStatsDataStorage3;
    byte[] protoBytes =
        ProtoStreamUtils.ChannelUtils.getProtoBytes(iPowerStatsHALWrapper.getEnergyMeterInfo());
    ...
    byte[] protoBytes2 =
        ProtoStreamUtils.EnergyConsumerUtils.getProtoBytes(
            iPowerStatsHALWrapper.getEnergyConsumerInfo());
    ...
    byte[] protoBytes3 =
        ProtoStreamUtils.PowerEntityUtils.getProtoBytes(iPowerStatsHALWrapper.getPowerEntityInfo());
