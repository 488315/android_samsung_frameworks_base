        ...
        try {
            ...
            for (EnergyMeasurement energyMeasurement : energyMeasurementArr) {
                if (energyMeasurement.durationMs == energyMeasurement.timestampMs) {
                    list.add(FrameworkStatsLog.buildStatsEvent(i, ((Channel) this.mChannels.get(Integer.valueOf(energyMeasurement.f7id))).subsystem, ((Channel) this.mChannels.get(Integer.valueOf(energyMeasurement.f7id))).name, energyMeasurement.durationMs, energyMeasurement.energyUWs));
                }
            }
            return 0;
        } catch (Exception e) {
            ...
            for (StateResidencyResult stateResidencyResult : stateResidencyResultArr) {
                ...
                while (true) {
                    ...
                    if (i2 < stateResidencyArr.length) {
                        ...
                        list.add(FrameworkStatsLog.buildStatsEvent(i, (String) this.mEntityNames.get(Integer.valueOf(stateResidencyResult.f11id)), (String) ((Map) this.mStateNames.get(Integer.valueOf(stateResidencyResult.f11id))).get(Integer.valueOf(stateResidency.f10id)), stateResidency.totalStateEntryCount, stateResidency.totalTimeInStateMs));
                        i2++;
                    }
                }
            }
