...
public class PowerStatsDataStorage {
  ...
  public PowerStatsDataStorage(Context context, File file, String str) {
    ...
    for (int i = 0; i < listFiles.length; i++) {
      if (listFiles[i]
              .getName()
              .startsWith(
                  this.mDataStorageFilename.substring(0, this.mDataStorageFilename.lastIndexOf(46)))
  ...
  public void write(byte[] bArr) {
    ...
    try {
      try {
        ...
        this.mFileRotator.rewriteActive(
            new DataRewriter(new DataElement(bArr).toByteArray()), currentTimeMillis);
  ...
  public void read(DataElementReadCallback dataElementReadCallback) {
    this.mLock.lock();
    try {
      this.mFileRotator.readMatching(
          new DataReader(dataElementReadCallback), Long.MIN_VALUE, Long.MAX_VALUE);
    } finally {
      this.mLock.unlock();
    }
  }
