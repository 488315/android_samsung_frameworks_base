...
public class AudioSystemAdapter
        ...
        for (Pair pair : list) {
          ArrayList devicesForAttributes =
              getDevicesForAttributes(
                  (AudioAttributes) pair.first, ((Boolean) pair.second).booleanValue());
          if (!this.mLastDevicesForAttr.containsKey(pair)
              || !sameDeviceList(devicesForAttributes, (List) this.mLastDevicesForAttr.get(pair))) {
            try {
  ...
  public void addOnDevicesForAttributesChangedListener(
    ...
    synchronized (this.mRegisteredAttributesMap) {
      List list =
          (List) this.mRegisteredAttributesMap.get(iDevicesForAttributesCallback.asBinder());
      if (list == null) {
        list = new ArrayList();
  ...
  public static void setVolRangeInitReqListener(
      OnVolRangeInitRequestListener onVolRangeInitRequestListener) {
    synchronized (sVolRangeInitReqListenerLock) {
    ...
    synchronized (sDeviceCacheLock) {
      printWriter.println(
          " last cache clear time: "
              + withZone.format(Instant.ofEpochMilli(this.mDevicesForAttributesCacheClearTimeMs)));
