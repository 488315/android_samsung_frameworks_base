...
public class FocusRequester {
  ...
  public void dump(PrintWriter printWriter) {
    printWriter.println(
        "  source:"
            ...
            + focusGainToString()
            + " -- flags: "
            + flagsToString(this.mGrantFlags)
            + " -- loss: "
            + focusLossToString()
            + " -- notified: "
            + this.mFocusLossWasNotified
            + " -- limbo"
            + this.mFocusLossFadeLimbo
            + " -- uid: "
            + this.mCallingUid
            + " -- attr: "
            + this.mAttributes
            + " -- device: "
            + Integer.toHexString(this.mDevice)
            + " -- sdk:"
            + this.mSdkTarget);
  }
  ...
  */
  ...
  public boolean handleFocusLossFromGain(int i, FocusRequester focusRequester, boolean z) {
    ...
    if (focusLossForGainRequest == -1
        && this.mFocusController.isDelayLossApp(this.mPackageName)
        && focusRequester != null
        && !"com.android.bluetooth".equals(focusRequester.getPackageName())
        && !"com.samsung.android.mcfds".equals(focusRequester.getPackageName())) {
  ...
  public void handleFocusLoss(int i, FocusRequester focusRequester, boolean z) {
    try {
      if (i != this.mFocusLossReceived) {
        ...
        if (!this.mFocusController.mustNotifyFocusOwnerOnDuck()
            && this.mFocusLossReceived == -3
  ...
  public int dispatchFocusChange(int i) {
    ...
    if ((i == 3 || i == 4 || i == 2 || i == 1) && this.mFocusGainRequest != i) {
      Log.w(
          "MediaFocusControl",
          "focus gain was requested with " + this.mFocusGainRequest + ", dispatching " + i);
    } else if (i == -3 || i == -2 || i == -1) {
      this.mFocusLossReceived = i;
    }
    try {
      ...
      Log.e(
          "MediaFocusControl",
          "dispatchFocusChange: error talking to focus listener " + this.mClientId,
          e);
      return 0;
    }
  }
  ...
  public void dispatchFocusResultFromExtPolicy(int i) {
    ...
    try {
      ...
      Log.e(
          "MediaFocusControl",
          "dispatchFocusResultFromExtPolicy: error talking to focus listener" + this.mClientId,
          e);
    }
  }
