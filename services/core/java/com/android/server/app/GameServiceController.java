...
public final class GameServiceController {
  ...
  public GameServiceController(
      Context context,
      Executor executor,
      GameServiceProviderSelector gameServiceProviderSelector,
      GameServiceProviderInstanceFactory gameServiceProviderInstanceFactory) {
  ...
  public void notifyUserUnlocking(SystemService.TargetUser targetUser) {
    if (this.mCurrentForegroundUser != null
        && this.mCurrentForegroundUser.getUserIdentifier() == targetUser.getUserIdentifier()) {
      this.mBackgroundExecutor.execute(new GameServiceController$$ExternalSyntheticLambda0(this));
    }
  }
    ...
    if (this.mHasBootCompleted) {
      synchronized (this.mLock) {
        GameServiceConfiguration gameServiceConfiguration =
            this.mGameServiceProviderSelector.get(
                this.mCurrentForegroundUser, this.mGameServiceProviderOverride);
        String packageName =
            gameServiceConfiguration == null ? null : gameServiceConfiguration.getPackageName();
        GameServiceConfiguration.GameServiceComponentConfiguration
            gameServiceComponentConfiguration =
                gameServiceConfiguration == null
                    ? null
                    : gameServiceConfiguration.getGameServiceComponentConfiguration();
        evaluateGameServiceProviderPackageChangedListenerLocked(packageName);
        if (!Objects.equals(
            gameServiceComponentConfiguration, this.mActiveGameServiceComponentConfiguration)) {
          if (this.mGameServiceProviderInstance != null) {
            Slog.i(
                "GameServiceController",
                "Stopping Game Service provider: " + this.mActiveGameServiceComponentConfiguration);
          ...
          Slog.i(
              "GameServiceController",
              "Starting Game Service provider: " + this.mActiveGameServiceComponentConfiguration);
          this.mGameServiceProviderInstance =
              this.mGameServiceProviderInstanceFactory.create(
                  this.mActiveGameServiceComponentConfiguration);
      ...
      if (TextUtils.equals(intent.getData().getSchemeSpecificPart(), this.mPackageName)) {
        ...
        executor.execute(
            new Runnable() { // from class:
              // com.android.server.app.GameServiceController$PackageChangedBroadcastReceiver$$ExternalSyntheticLambda0
