...
public final class GameServiceProviderSelectorImpl implements GameServiceProviderSelector {
  ...
  public GameServiceConfiguration get(SystemService.TargetUser targetUser, String str) {
    ...
    if (!(targetUser.isFull() && !targetUser.isManagedProfile())) {
      Slog.i(
          "GameServiceProviderSelector",
          "Game Service not supported for user: " + targetUser.getUserIdentifier());
      return null;
    }
    if (TextUtils.isEmpty(str)) {
      str = this.mResources.getString(R.string.ext_media_status_checking);
      i = 1048576;
    }
    if (TextUtils.isEmpty(str)) {
    ...
    List queryIntentServicesAsUser =
        this.mPackageManager.queryIntentServicesAsUser(
            new Intent("android.service.games.action.GAME_SERVICE").setPackage(str),
            i | 128,
            userIdentifier);
    if (queryIntentServicesAsUser == null || queryIntentServicesAsUser.isEmpty()) {
      Slog.w(
          "GameServiceProviderSelector",
          "No available game service found for user id: " + userIdentifier);
    ...
    while (true) {
      ...
      if (serviceInfo != null
          && (determineGameSessionServiceFromGameService =
                  determineGameSessionServiceFromGameService(serviceInfo))
              != null) {
        gameServiceConfiguration =
            new GameServiceConfiguration(
                str,
                new GameServiceConfiguration.GameServiceComponentConfiguration(
                    new UserHandle(userIdentifier),
    ...
    Slog.w(
        "GameServiceProviderSelector",
        "No valid game service found for user id: " + userIdentifier);
    return new GameServiceConfiguration(str, null);
  }
  ...
  public final ComponentName determineGameSessionServiceFromGameService(ServiceInfo serviceInfo) {
    ...
    try {
      XmlResourceParser loadXmlMetaData =
          serviceInfo.loadXmlMetaData(this.mPackageManager, "android.game_service");
      try {
        if (loadXmlMetaData == null) {
          Slog.w(
              "GameServiceProviderSelector",
              "No android.game_service meta-data found for " + serviceInfo.getComponentName());
          if (loadXmlMetaData != null) {
        ...
        Resources resourcesForApplication =
            this.mPackageManager.getResourcesForApplication(serviceInfo.packageName);
        ...
        TypedArray obtainAttributes =
            resourcesForApplication.obtainAttributes(
                asAttributeSet, com.android.internal.R.styleable.GameService);
        ...
        try {
          ...
          Slog.w(
              "GameServiceProviderSelector", "GameSessionService does not exist: " + componentName);
          return null;
        }
      } catch (Throwable th) {
        if (loadXmlMetaData != null) {
