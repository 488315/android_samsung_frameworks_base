...
public final class TvRemoteProviderWatcher {
  ...
  public TvRemoteProviderWatcher(Context context, Object obj, Handler handler) {
    ...
    this.mScanPackagesReceiver =
        new BroadcastReceiver() { // from class: com.android.server.tv.TvRemoteProviderWatcher.1
          ...
          public void onReceive(Context context2, Intent intent) {
            ...
            TvRemoteProviderWatcher.this.mHandler.post(
                TvRemoteProviderWatcher.this.mScanPackagesRunnable);
          }
        };
    ...
    simpleStringSplitter.setString(context.getString(R.string.ext_media_status_removed));
      ...
      Iterator it =
          this.mPackageManager
              .queryIntentServicesAsUser(
                  new Intent("com.android.media.tv.remoteprovider.TvRemoteProvider"),
                  0,
                  this.mUserId)
              .iterator();
      while (it.hasNext()) {
        ...
        if (serviceInfo != null && verifyServiceTrusted(serviceInfo)) {
          ...
          if (findProvider < 0) {
            ...
            TvRemoteProviderProxy tvRemoteProviderProxy2 =
                (TvRemoteProviderProxy) this.mProviderProxies.get(findProvider);
      ...
      if (i2 < this.mProviderProxies.size()) {
        for (int size = this.mProviderProxies.size() - 1; size >= i2; size--) {
          TvRemoteProviderProxy tvRemoteProviderProxy3 =
              (TvRemoteProviderProxy) this.mProviderProxies.get(size);
  ...
  public boolean verifyServiceTrusted(ServiceInfo serviceInfo) {
    ...
    if (!this.mUnbundledServicePackages.contains(serviceInfo.packageName)) {
      Slog.w(
          "TvRemoteProviderWatcher",
          "Ignoring atv remote provider service because the package has not been set and/or"
              + " whitelisted: "
              + serviceInfo.packageName
              + "/"
              + serviceInfo.name);
  ...
  public final boolean hasNecessaryPermissions(String str) {
    return this.mPackageManager.checkPermission(
            "android.permission.TV_VIRTUAL_REMOTE_CONTROLLER", str)
        == 0;
  }
