...
public final class TvRemoteProviderWatcher {
    ...
    public TvRemoteProviderWatcher(Context context, Object obj) {
        ...
        this.mScanPackagesReceiver =
                new BroadcastReceiver() { // from class:
                                          // com.android.server.tv.TvRemoteProviderWatcher.1
                    ...
                    public final void onReceive(Context context2, Intent intent) {
                        ...
                        TvRemoteProviderWatcher tvRemoteProviderWatcher =
                                TvRemoteProviderWatcher.this;
                        tvRemoteProviderWatcher.mHandler.post(
                                tvRemoteProviderWatcher.mScanPackagesRunnable);
                    }
                };
        this.mScanPackagesRunnable =
                new Runnable() { // from class: com.android.server.tv.TvRemoteProviderWatcher.2
                    ...
                    public final void run() {
                        ...
                        if (tvRemoteProviderWatcher.mRunning) {
                            ...
                            Iterator it =
                                    packageManager
                                            .queryIntentServicesAsUser(intent, 0, i)
                                            .iterator();
                            int i2 = 0;
                            while (it.hasNext()) {
                                ServiceInfo serviceInfo = ((ResolveInfo) it.next()).serviceInfo;
                                if (serviceInfo != null
                                        && tvRemoteProviderWatcher.verifyServiceTrusted(
                                                serviceInfo)) {
                                    ...
                                    while (true) {
                                        ...
                                        TvRemoteProviderProxy tvRemoteProviderProxy =
                                                (TvRemoteProviderProxy)
                                                        tvRemoteProviderWatcher.mProviderProxies
                                                                .get(i3);
                                        if (tvRemoteProviderProxy
                                                        .mComponentName
                                                        .getPackageName()
                                                        .equals(str)
                                                && tvRemoteProviderProxy
                                    ...
                                    if (i3 < 0) {
                                        ...
                                        TvRemoteProviderProxy tvRemoteProviderProxy2 =
                                                new TvRemoteProviderProxy(
                                                        context2,
                                                        tvRemoteProviderWatcher.mLock,
                                                        componentName,
                                                        i);
                                        tvRemoteProviderProxy2.start();
                                        tvRemoteProviderWatcher.mProviderProxies.add(
                                                i2, tvRemoteProviderProxy2);
                                        i2++;
                                    } else if (i3 >= i2) {
                                        TvRemoteProviderProxy tvRemoteProviderProxy3 =
                                                (TvRemoteProviderProxy)
                                                        tvRemoteProviderWatcher.mProviderProxies
                                                                .get(i3);
                                        tvRemoteProviderProxy3.start();
                                        if (tvRemoteProviderProxy3.mRunning
                                                && !tvRemoteProviderProxy3.mConnected) {
                                            tvRemoteProviderProxy3.unbind();
                                            tvRemoteProviderProxy3.bind();
                                        }
                                        Collections.swap(
                                                tvRemoteProviderWatcher.mProviderProxies, i3, i2);
                                        i2++;
                                    }
                                }
                            }
                            if (z) {
                                NetworkScorerAppManager$$ExternalSyntheticOutline0.m(
                                        i2,
                                        "scanPackages() targetIndex ",
                                        "TvRemoteProviderWatcher");
                            }
                            if (i2 < tvRemoteProviderWatcher.mProviderProxies.size()) {
                                for (int size2 =
                                                tvRemoteProviderWatcher.mProviderProxies.size() - 1;
                                        size2 >= i2;
                                        size2--) {
                                    TvRemoteProviderProxy tvRemoteProviderProxy4 =
                                            (TvRemoteProviderProxy)
                                                    tvRemoteProviderWatcher.mProviderProxies.get(
                                                            size2);
                                    tvRemoteProviderWatcher.mProviderProxies.remove(
                                            tvRemoteProviderProxy4);
                                    if (tvRemoteProviderProxy4.mRunning) {
                                        if (TvRemoteProviderProxy.DEBUG) {
                                            Slog.d(
                                                    "TvRemoteProviderProxy",
                                                    tvRemoteProviderProxy4 + ": Stopping");
                                        }
        ...
        simpleStringSplitter.setString(
                context.getString(R.string.face_authenticated_no_confirmation_required));
        simpleStringSplitter.forEach(
                new Consumer() { // from class:
                                 // com.android.server.tv.TvRemoteProviderWatcher$$ExternalSyntheticLambda0
    ...
    public boolean verifyServiceTrusted(ServiceInfo serviceInfo) {
        ...
        if (str == null || !str.equals("android.permission.BIND_TV_REMOTE_SERVICE")) {
            StringBuilder sb =
                    new StringBuilder(
                            "Ignoring atv remote provider service because it did not require the"
                                + " BIND_TV_REMOTE_SERVICE permission in its manifest: ");
            sb.append(serviceInfo.packageName);
            sb.append("/");
            BinaryTransparencyService$$ExternalSyntheticOutline0.m(
                    sb, serviceInfo.name, "TvRemoteProviderWatcher");
            return false;
        }
        if (((HashSet) this.mUnbundledServicePackages).contains(serviceInfo.packageName)) {
            if (this.mPackageManager.checkPermission(
            ...
            BinaryTransparencyService$$ExternalSyntheticOutline0.m(
                    new StringBuilder(
                            "Ignoring atv remote provider service because its package does not have"
                                + " TV_VIRTUAL_REMOTE_CONTROLLER permission: "),
                    serviceInfo.packageName,
                    "TvRemoteProviderWatcher");
            return false;
        }
        ...
        sb2.append("/");
        BinaryTransparencyService$$ExternalSyntheticOutline0.m(
                sb2, serviceInfo.name, "TvRemoteProviderWatcher");
        return false;
    }
}
