...
public class ConfigUpdateInstallReceiver extends BroadcastReceiver {
    ...
    new Thread() { // from class: com.android.server.updates.ConfigUpdateInstallReceiver.1
      ...
      public void run() {
        try {
          ...
          String requiredHashFromIntent =
              ConfigUpdateInstallReceiver.this.getRequiredHashFromIntent(intent);
          int currentVersion = ConfigUpdateInstallReceiver.this.getCurrentVersion();
          String currentHash =
              ConfigUpdateInstallReceiver.getCurrentHash(
                  ConfigUpdateInstallReceiver.this.getCurrentContent());
          if (!ConfigUpdateInstallReceiver.this.verifyVersion(currentVersion, versionFromIntent)) {
            Slog.i(
                "ConfigUpdateInstallReceiver", "Not installing, new version is <= current version");
            return;
          }
          if (!ConfigUpdateInstallReceiver.this.verifyPreviousHash(
              currentHash, requiredHashFromIntent)) {
          ...
          BufferedInputStream altContent =
              ConfigUpdateInstallReceiver.this.getAltContent(context, intent);
          try {
  ...
  public final int getCurrentVersion() {
    try {
      return Integer.parseInt(
          IoUtils.readFileAsString(this.updateVersion.getCanonicalPath()).trim());
    } catch (IOException unused) {
      Slog.i(
          "ConfigUpdateInstallReceiver", "Couldn't find current metadata, assuming first update");
      return 0;
    }
  }
  ...
  public final BufferedInputStream getAltContent(Context context, Intent intent) {
    ...
    try {
      return new BufferedInputStream(
          context.getContentResolver().openInputStream(contentFromIntent));
    } finally {
  ...
  public final byte[] getCurrentContent() {
    try {
      ...
      Slog.i(
          "ConfigUpdateInstallReceiver", "Failed to read current content, assuming first update!");
      return null;
    }
  }
  ...
  public void install(InputStream inputStream, int i) {
    ...
    writeUpdate(
        this.updateDir, this.updateVersion, new ByteArrayInputStream(Long.toString(i).getBytes()));
  }
}
