  ...
  public class KNOXReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      if ("android.intent.action.BOOT_COMPLETED".equals(intent.getAction())) {
        Log.secI(
            "SemClipboardService",
            "ACTION_BOOT_COMPLETED. Check and Start to migrate semclipboard to keyboard.");
    ...
    try {
      synchronized (this.mClipboardEventListeners) {
        this.mClipboardEventListeners.register(
            iOnClipboardEventListener, new ClipboardEventListenerInfo(Binder.getCallingUid(), str));
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
        ...
        for (int i2 = 0; i2 < beginBroadcast; i2++) {
          try {
            ClipboardEventListenerInfo clipboardEventListenerInfo =
                (ClipboardEventListenerInfo) this.mClipboardEventListeners.getBroadcastCookie(i2);
            if (this.mAppOps.checkOpNoThrow(
                    29, clipboardEventListenerInfo.mUid, clipboardEventListenerInfo.mPackageName)
                == 0) {
    ...
    try {
      ...
      if (extras != null) {
        contentValues.put(
            ...
            Boolean.valueOf(extras.getBoolean("android.content.extra.IS_SENSITIVE")));
        contentValues.put(
            "com.microsoft.appmanager",
            Boolean.valueOf(extras.containsKey("com.microsoft.appmanager")));
      ...
      if (uri != null) {
        grantUriPermission(
            Uri.parse(uri), i, KnoxCustomManagerService.SAMSUNG_HONEYBOARD_PKG_NAME, 0);
      }
      ...
      Log.e(
          "SemClipboardService", "Exception occurs in insertContentUri because " + e.getMessage());
      return null;
    } finally {
      Binder.restoreCallingIdentity(clearCallingIdentity);
    }
  ...
  public final ClipData convertSemClipToClip(SemClipData semClipData) {
    ...
    if (clipType == 2) {
      ...
      return ClipData.newUri(
          this.mContext.getContentResolver(), "SemImageClipData", semImageClipData.getContentUri());
    }
    if (clipType == 4) {
      SemHtmlClipData semHtmlClipData = (SemHtmlClipData) semClipData;
      return ClipData.newHtmlText(
          semHtmlClipData.getLabel(), semHtmlClipData.getPlainText(), semHtmlClipData.getHtml());
    }
    if (clipType == 8) {
    ...
    if (clipType == 16) {
      ...
      return ClipData.newUri(
          this.mContext.getContentResolver(), semUriClipData.getLabel(), semUriClipData.getUri());
    }
    if (clipType != 32) {
      return null;
    }
          ...
          try {
            ...
            if (this.mAppOps.checkOpNoThrow(
              ...
              this.mClipboardEventListeners
                  .getBroadcastItem(i2)
                  .onClipboardEvent(i, (SemClipData) null);
            }
          } catch (RemoteException unused) {
          } catch (Throwable th) {
            this.mClipboardEventListeners.finishBroadcast();
  ...
  public final void insertToClipboardForMigration(SemClipData semClipData, int i) {
    if (semClipData != null) {
      try {
        ...
        Log.e(
            "SemClipboardService",
            "Exception occurs in insertContentUri because " + e.getMessage());
      }
    }
  }
}
