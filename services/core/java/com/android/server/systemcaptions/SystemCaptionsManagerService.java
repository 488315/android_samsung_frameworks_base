...
public final class SystemCaptionsManagerService extends AbstractMasterSystemService {
    public SystemCaptionsManagerService(Context context) {
        super(
                ...
                new FrameworkResourcesServiceNameResolver(
                        context, R.string.deprecated_target_sdk_message),
                null,
                68);
    }
    ...
    public final AbstractPerUserSystemService newServiceLocked(int i, boolean z) {
        ...
        SystemCaptionsManagerPerUserService systemCaptionsManagerPerUserService =
                new SystemCaptionsManagerPerUserService(this, this.mLock, i);
        if (this.verbose) {
            Slog.v("SystemCaptionsManagerPerUserService", "initialize()");
        }
        if (systemCaptionsManagerPerUserService.mRemoteService == null) {
            String componentNameLocked =
                    systemCaptionsManagerPerUserService.getComponentNameLocked();
            if (componentNameLocked == null) {
                if (this.verbose) {
                    Slog.v(
                            "SystemCaptionsManagerPerUserService",
                            "getRemoteServiceLocked(): Not set");
                }
                remoteSystemCaptionsManagerService = null;
                if (remoteSystemCaptionsManagerService == null && this.verbose) {
                    Slog.v(
                            "SystemCaptionsManagerPerUserService",
                            "initialize(): Failed to init remote server");
                }
                return systemCaptionsManagerPerUserService;
            }
            systemCaptionsManagerPerUserService.mRemoteService =
                    new RemoteSystemCaptionsManagerService(
                            getContext(),
            ...
            if (this.verbose) {
                ProxyManager$$ExternalSyntheticOutline0.m(
                        i,
                        "getRemoteServiceLocked(): initialize for user ",
                        "SystemCaptionsManagerPerUserService");
            }
            RemoteSystemCaptionsManagerService remoteSystemCaptionsManagerService2 =
                    systemCaptionsManagerPerUserService.mRemoteService;
            ...
            if (!handler.hasMessages(1)) {
                handler.sendMessage(
                        PooledLambda.obtainMessage(
                                        new RemoteSystemCaptionsManagerService$$ExternalSyntheticLambda0(
                                                0),
                                        remoteSystemCaptionsManagerService2)
                                .setWhat(1));
            } else if (z2) {
    ...
    public final void onServiceRemoved(
        ...
        SystemCaptionsManagerPerUserService systemCaptionsManagerPerUserService =
                (SystemCaptionsManagerPerUserService) abstractPerUserSystemService;
        synchronized (this.mLock) {
            if (((SystemCaptionsManagerService) systemCaptionsManagerPerUserService.mMaster)
                    .verbose) {
            ...
            RemoteSystemCaptionsManagerService remoteSystemCaptionsManagerService =
                    systemCaptionsManagerPerUserService.mRemoteService;
            if (remoteSystemCaptionsManagerService != null) {
                remoteSystemCaptionsManagerService.mHandler.sendMessage(
                        PooledLambda.obtainMessage(
                                new RemoteSystemCaptionsManagerService$$ExternalSyntheticLambda0(1),
