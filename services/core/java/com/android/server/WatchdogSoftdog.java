...
public class WatchdogSoftdog {
  ...
  public void softdogInitialize() {
    if ("off".equals(SystemProperties.get("persist.vendor.softdog"))) {
      Slog.i(
          "Watchdog:WatchdogSoftdog", "!@persist.vendor.softdog is off, so do not turn on softdog");
  ...
  public void softdogKick(int i) {
    ...
    try {
      String readTextFile =
          FileUtils.readTextFile(new File("/sys/class/power_supply/battery/temp"), 1024, null);
