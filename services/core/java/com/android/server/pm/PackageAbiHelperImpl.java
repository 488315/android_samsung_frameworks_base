...
public final class PackageAbiHelperImpl implements PackageAbiHelper {
  ...
  public PackageAbiHelper.NativeLibraryPaths deriveNativeLibraryPaths(
      AndroidPackage androidPackage, boolean z, boolean z2, File file) {
    return deriveNativeLibraryPaths(
        new PackageAbiHelper.Abis(
            AndroidPackageUtils.getRawPrimaryCpuAbi(androidPackage),
            AndroidPackageUtils.getRawSecondaryCpuAbi(androidPackage)),
        file,
  ...
  public static PackageAbiHelper.NativeLibraryPaths deriveNativeLibraryPaths(
      PackageAbiHelper.Abis abis, File file, String str, String str2, boolean z, boolean z2) {
    String absolutePath;
    ...
    if (ApkLiteParseUtils.isApkFile(file2)) {
      if (z4) {
        ...
        boolean is64BitInstructionSet =
            VMRuntime.is64BitInstructionSet(InstructionSets.getPrimaryInstructionSet(abis));
        String deriveCodePathName = deriveCodePathName(str);
        absolutePath =
            Environment.buildPath(
                    new File(calculateBundledApkRoot),
                    new String[] {is64BitInstructionSet ? "lib64" : "lib", deriveCodePathName})
                .getAbsolutePath();
        if (abis.secondary != null) {
          str3 =
              Environment.buildPath(
                      new File(calculateBundledApkRoot),
                      new String[] {is64BitInstructionSet ? "lib" : "lib64", deriveCodePathName})
                  .getAbsolutePath();
        }
      } else {
      ...
      absolutePath2 =
          new File(absolutePath, InstructionSets.getPrimaryInstructionSet(abis)).getAbsolutePath();
      if (abis.secondary != null) {
        str3 =
            new File(absolutePath, VMRuntime.getInstructionSet(abis.secondary)).getAbsolutePath();
      }
    }
  ...
  public PackageAbiHelper.Abis getBundledAppAbis(AndroidPackage androidPackage) {
    return getBundledAppAbi(
        androidPackage,
        calculateBundledApkRoot(androidPackage.getBaseApkPath()),
        deriveCodePathName(androidPackage.getPath()));
  }

  public final PackageAbiHelper.Abis getBundledAppAbi(
      AndroidPackage androidPackage, String str, String str2) {
    boolean exists;
    boolean exists2;
    String str3;
    String str4;
    ...
    if (ApkLiteParseUtils.isApkFile(file)) {
      ...
      exists =
          (ArrayUtils.isEmpty(strArr) || TextUtils.isEmpty(strArr[0]))
              ? false
              : new File(file2, VMRuntime.getInstructionSet(strArr[0])).exists();
      String[] strArr2 = Build.SUPPORTED_32_BIT_ABIS;
      exists2 =
          (ArrayUtils.isEmpty(strArr2) || TextUtils.isEmpty(strArr2[0]))
              ? false
              : new File(file2, VMRuntime.getInstructionSet(strArr2[0])).exists();
    }
    String str5 = null;
    if (exists && !exists2) {
      ...
      if (exists2 && exists) {
        if (!androidPackage.isMultiArch()) {
          Slog.e(
              "PackageManager",
              "Package " + androidPackage + " has multiple bundled libs, but is not multiarch.");
        }
  ...
  */
  public Pair derivePackageAbi(
      AndroidPackage androidPackage, boolean z, boolean z2, String str, File file) {
    ...
    PackageAbiHelper.NativeLibraryPaths deriveNativeLibraryPaths =
    ...
    if (androidPackage.isMultiArch()) {
        ...
        if (shouldExtractLibs) {
          ...
          i2 =
              NativeLibraryHelper.copyNativeBinariesForSupportedAbi(
                  autoCloseable2, file2, strArr, z4, isIncrementalPath);
        } else {
      ...
      if (i2 >= 0 && AndroidPackageUtils.isLibrary(androidPackage) && shouldExtractLibs) {
        throw new PackageManagerException(
            -110, "Shared library native lib extraction not supported");
      }
      maybeThrowExceptionForMultiArchCopy(
          "Error unpackaging 32 bit native libs for multiarch app.", i2);
      ...
      if (strArr2.length > 0) {
        if (shouldExtractLibs) {
          ...
          findSupportedAbi2 =
              NativeLibraryHelper.copyNativeBinariesForSupportedAbi(
                  autoCloseable2, file2, strArr2, z4, isIncrementalPath);
        } else {
          Trace.traceBegin(262144L, "findSupportedAbi");
      ...
      maybeThrowExceptionForMultiArchCopy(
          "Error unpackaging 64 bit native libs for multiarch app.", i);
      if (i >= 0) {
      ...
      if (i2 >= 0) {
        try {
          ...
          return new Pair(
              abis,
              deriveNativeLibraryPaths(
                  abis, file, androidPackage.getPath(), androidPackage.getBaseApkPath(), z, z2));
        }
        if (i >= 0) {
          ...
          return new Pair(
              abis2,
              deriveNativeLibraryPaths(
                  abis2, file, androidPackage.getPath(), androidPackage.getBaseApkPath(), z, z2));
        }
      } else {
        str3 = str2;
      }
      ...
      if (Build.SUPPORTED_64_BIT_ABIS.length > 0
          && str == null
          && NativeLibraryHelper.hasRenderscriptBitcode(autoCloseable2)) {
        strArr3 = Build.SUPPORTED_32_BIT_ABIS;
        if (strArr3.length <= 0) {
          throw new PackageManagerException(
              -16,
              "Apps that contain RenderScript with target API level < 21 are not supported on"
      ...
      if (shouldExtractLibs) {
        ...
        findSupportedAbi =
            NativeLibraryHelper.copyNativeBinariesForSupportedAbi(
                autoCloseable2, file2, strArr3, z4, isIncrementalPath);
      } else {
        Trace.traceBegin(262144L, "findSupportedAbi");
      ...
      if (findSupportedAbi < 0 && findSupportedAbi != -114) {
        throw new PackageManagerException(
            -110, "Error unpackaging native libs for app, errorCode=" + findSupportedAbi);
      }
    ...
    return new Pair(
        abis22,
        deriveNativeLibraryPaths(
            abis22, file, androidPackage.getPath(), androidPackage.getBaseApkPath(), z, z2));
  }

  public final boolean shouldExtractLibs(AndroidPackage androidPackage, boolean z, boolean z2) {
    boolean z3 =
        !AndroidPackageUtils.isLibrary(androidPackage)
  ...
  public String getAdjustedAbiForSharedUser(ArraySet arraySet, AndroidPackage androidPackage) {
    ...
    String instructionSet =
        (androidPackage == null
                || (rawPrimaryCpuAbi = AndroidPackageUtils.getRawPrimaryCpuAbi(androidPackage))
                    == null)
            ? null
            : VMRuntime.getInstructionSet(rawPrimaryCpuAbi);
    ...
    while (it.hasNext()) {
      ...
      if (androidPackage == null
          || !androidPackage.getPackageName().equals(packageStateInternal2.getPackageName())) {
        if (packageStateInternal2.getPrimaryCpuAbiLegacy() != null) {
          String instructionSet2 =
              VMRuntime.getInstructionSet(packageStateInternal2.getPrimaryCpuAbiLegacy());
