...
public final class UserVisibilityMediator implements Dumpable {
    ...
    public UserVisibilityMediator(boolean z, boolean z2, Handler handler) {
        ...
        if (z3) {
            Slogf.m94i(TAG, "UserVisibilityMediator created with DBG on");
        }
    }
    ...
    */
    public int assignUserToDisplayOnStart(int i, int i2, int i3, int i4) {
        ...
        boolean z = DBG;
        if (z) {
            Slogf.m88d(TAG, "assignUserToDisplayOnStart(%d, %d, %s, %d): actualProfileGroupId=%d", Integer.valueOf(i), Integer.valueOf(i2), UserManagerInternal.userStartModeToString(i3), Integer.valueOf(i4), Integer.valueOf(i5));
        }
        synchronized (this.mLock) {
            int userVisibilityOnStartLocked = getUserVisibilityOnStartLocked(i, i5, i3, i4);
            if (z) {
                Slogf.m88d(TAG, "result of getUserVisibilityOnStartLocked(%s)", UserManagerInternal.userAssignmentResultToString(userVisibilityOnStartLocked));
            }
            if (userVisibilityOnStartLocked != -1 && userVisibilityOnStartLocked != 3) {
                ...
                if (z) {
                    Slogf.m88d(TAG, "mapping result: %s", secondaryDisplayMappingStatusToString(canAssignUserToDisplayLocked));
                }
                ...
                if (i3 == 1) {
                    ...
                    if (i3 != 2) {
                        ...
                        Slogf.m88d(TAG, "adding user %d to list of invisible profiles", Integer.valueOf(i));
                        this.mStartedInvisibleProfileUserIds.add(Integer.valueOf(i));
                    }
                    if (canAssignUserToDisplayLocked != 1) {
                        if (z) {
                            Slogf.m88d(TAG, "adding user / display mapping (%d -> %d)", Integer.valueOf(i), Integer.valueOf(i4));
                        }
                        ...
                        Slogf.m86d(TAG, "don't need to update mUsersOnSecondaryDisplays");
                    }
                    dispatchVisibilityChanged(visibleUsers, getVisibleUsers());
                    if (z) {
                        Slogf.m88d(TAG, "returning %s", UserManagerInternal.userAssignmentResultToString(userVisibilityOnStartLocked));
                    }
                    return userVisibilityOnStartLocked;
                }
                if (z) {
                    Slogf.m88d(TAG, "adding visible user / profile group id mapping (%d -> %d)", Integer.valueOf(i), Integer.valueOf(i5));
                }
    ...
    public final int getUserVisibilityOnStartLocked(int i, int i2, int i3, int i4) {
        ...
        if (i4 != 0) {
            if (z2) {
                Slogf.m105w(TAG, "getUserVisibilityOnStartLocked(%d, %d, %s, %d) failed: cannot start foreground user on secondary display", Integer.valueOf(i), Integer.valueOf(i2), UserManagerInternal.userStartModeToString(i3), Integer.valueOf(i4));
                return -1;
            }
            if (!this.mVisibleBackgroundUsersEnabled) {
                Slogf.m105w(TAG, "getUserVisibilityOnStartLocked(%d, %d, %s, %d) failed: called on device that doesn't support multiple users on multiple displays", Integer.valueOf(i), Integer.valueOf(i2), UserManagerInternal.userStartModeToString(i3), Integer.valueOf(i4));
                return -1;
            }
        }
        if (isProfile(i, i2)) {
            if (i4 != 0) {
                Slogf.m105w(TAG, "canStartUserLocked(%d, %d, %s, %d) failed: cannot start profile user on secondary display", Integer.valueOf(i), Integer.valueOf(i2), UserManagerInternal.userStartModeToString(i3), Integer.valueOf(i4));
                return -1;
            }
            if (i3 == 1) {
                Slogf.m105w(TAG, "startUser(%d, %d, %s, %d) failed: cannot start profile user in foreground", Integer.valueOf(i), Integer.valueOf(i2), UserManagerInternal.userStartModeToString(i3), Integer.valueOf(i4));
                return -1;
            }
            if (i3 == 2) {
                return 2;
            ...
            if (i3 == 3) {
                ...
                Slogf.m105w(TAG, "getUserVisibilityOnStartLocked(%d, %d, %s, %d) failed: cannot start profile user visible when its parent is not visible in that display", Integer.valueOf(i), Integer.valueOf(i2), UserManagerInternal.userStartModeToString(i3), Integer.valueOf(i4));
                return -1;
            }
        } else if (this.mUsersAssignedToDisplayOnStart != null && isUserAssignedToDisplayOnStartLocked(i, i4)) {
            if (DBG) {
                Slogf.m88d(TAG, "full user %d is already visible on display %d", Integer.valueOf(i), Integer.valueOf(i4));
            }
            return 3;
        }
    ...
    public final int canAssignUserToDisplayLocked(int i, int i2, int i3, int i4) {
        ...
        if (i4 == 0) {
            if (this.mVisibleBackgroundUserOnDefaultDisplayEnabled && i3 == 3) {
                ...
                if (userStartedOnDisplay != -10000 && userStartedOnDisplay != i2) {
                    Slogf.m105w(TAG, "canAssignUserToDisplayLocked(): cannot start user %d visible on default display because user %d already did so", Integer.valueOf(i), Integer.valueOf(userStartedOnDisplay));
                    return -1;
                }
                z = true;
            } else {
            ...
            if (!z) {
                if (DBG) {
                    Slogf.m88d(TAG, "Ignoring mapping for default display for user %d starting as %s", Integer.valueOf(i), UserManagerInternal.userStartModeToString(i3));
                }
                return 2;
            }
        }
        if (i == 0) {
            Slogf.m105w(TAG, "Cannot assign system user to secondary display (%d)", Integer.valueOf(i4));
            return -1;
        }
        if (i4 == -1) {
            Slogf.m105w(TAG, "Cannot assign to INVALID_DISPLAY (%d)", Integer.valueOf(i4));
            return -1;
        }
        if (i == this.mCurrentUserId) {
            Slogf.m105w(TAG, "Cannot assign current user (%d) to other displays", Integer.valueOf(i));
            return -1;
        }
        if (isProfile(i, i2)) {
            if (i4 != 0) {
                Slogf.m102w(TAG, "Profile user can only be started in the default display");
                return -1;
            }
            if (DBG) {
                Slogf.m88d(TAG, "Don't need to map profile user %d to default display", Integer.valueOf(i));
            }
            return 2;
        }
        ...
        for (int i5 = 0; i5 < this.mUsersAssignedToDisplayOnStart.size(); i5++) {
            ...
            if (DBG) {
                Slogf.m88d(TAG, "%d: assignedUserId=%d, assignedDisplayId=%d", Integer.valueOf(i5), Integer.valueOf(keyAt), Integer.valueOf(valueAt));
            }
            if (i4 == valueAt) {
                Slogf.m105w(TAG, "Cannot assign user %d to display %d because such display is already assigned to user %d", Integer.valueOf(i), Integer.valueOf(i4), Integer.valueOf(keyAt));
                return -1;
            }
            if (i == keyAt) {
                Slogf.m105w(TAG, "Cannot assign user %d to display %d because such user is as already assigned to display %d", Integer.valueOf(i), Integer.valueOf(i4), Integer.valueOf(keyAt));
                return -1;
            }
        }
        return 1;
    ...
    public void unassignUserFromDisplayOnStop(int i) {
        ...
        if (DBG) {
            Slogf.m88d(TAG, "unassignUserFromDisplayOnStop(%d)", Integer.valueOf(i));
        }
        synchronized (this.mLock) {
    ...
    public final void unassignUserFromAllDisplaysOnStopLocked(int i) {
        boolean z = DBG;
        if (z) {
            Slogf.m88d(TAG, "Removing %d from mStartedVisibleProfileGroupIds (%s)", Integer.valueOf(i), this.mStartedVisibleProfileGroupIds);
        }
        this.mStartedVisibleProfileGroupIds.delete(i);
        if (this.mStartedInvisibleProfileUserIds != null) {
            Slogf.m88d(TAG, "Removing %d from list of invisible profiles", Integer.valueOf(i));
            this.mStartedInvisibleProfileUserIds.remove(Integer.valueOf(i));
        }
        if (this.mVisibleBackgroundUsersEnabled) {
            if (z) {
                Slogf.m88d(TAG, "Removing user %d from mUsersOnDisplaysMap (%s)", Integer.valueOf(i), this.mUsersAssignedToDisplayOnStart);
            }
            ...
            for (int size = this.mExtraDisplaysAssignedToUsers.size() - 1; size >= 0; size--) {
                if (this.mExtraDisplaysAssignedToUsers.valueAt(size) == i) {
                    if (DBG) {
                        Slogf.m88d(TAG, "Removing display %d from mExtraDisplaysAssignedToUsers (%s)", Integer.valueOf(this.mExtraDisplaysAssignedToUsers.keyAt(size)), this.mExtraDisplaysAssignedToUsers);
                    }
    ...
    public boolean isUserVisible(int i, int i2) {
        ...
        if (!this.mVisibleBackgroundUsersEnabled) {
            if (DBG) {
                Slogf.m88d(TAG, "isUserVisible(%d, %d): returning false as device does not support visible background users", Integer.valueOf(i), Integer.valueOf(i2));
            }
            return false;
        }
        synchronized (this.mLock) {
        ...
        if (isCurrentUserOrRunningProfileOfCurrentUser(i)) {
            if (this.mVisibleBackgroundUserOnDefaultDisplayEnabled) {
                ...
                if (userStartedOnDisplay != -10000) {
                    if (DBG) {
                        Slogf.m88d(TAG, "getMainDisplayAssignedToUser(%d): returning INVALID_DISPLAY for current user user %d was started on DEFAULT_DISPLAY", Integer.valueOf(i), Integer.valueOf(userStartedOnDisplay));
                    }
                    return -1;
                }
            }
        ...
        synchronized (this.mLock) {
            for (int i2 = 0; i2 < this.mUsersAssignedToDisplayOnStart.size(); i2++) {
                if (this.mUsersAssignedToDisplayOnStart.valueAt(i2) == i) {
                    ...
                    if (DBG) {
                        Slogf.m88d(TAG, "getUserAssignedToDisplay(%d): skipping user %d because it's a profile", Integer.valueOf(i), Integer.valueOf(keyAt));
                    }
                }
            }
            if (!z) {
                ...
                Slogf.m88d(TAG, "getUserAssignedToDisplay(%d): no user assigned to display, returning USER_NULL instead", Integer.valueOf(i));
                return -10000;
            }
            int currentUserId = getCurrentUserId();
            if (DBG) {
                Slogf.m88d(TAG, "getUserAssignedToDisplay(%d): no user assigned to display, returning current user (%d) instead", Integer.valueOf(i), Integer.valueOf(currentUserId));
            }
    ...
    public void addListener(UserManagerInternal.UserVisibilityListener userVisibilityListener) {
        if (DBG) {
            Slogf.m88d(TAG, "adding listener %s", userVisibilityListener);
        }
        synchronized (this.mLock) {
    ...
    public final void dispatchVisibilityChanged(IntArray intArray, IntArray intArray2) {
        if (intArray == null) {
            if (DBG) {
                Slogf.m86d(TAG, "dispatchVisibilityChanged(): ignoring, no listeners");
                return;
            }
            return;
        }
        ...
        if (DBG) {
            Slogf.m88d(TAG, "dispatchVisibilityChanged(): visibleUsersBefore=%s, visibleUsersAfter=%s, %d listeners (%s)", intArray, intArray2, Integer.valueOf(copyOnWriteArrayList.size()), copyOnWriteArrayList);
        }
    ...
    public final void dispatchVisibilityChanged(CopyOnWriteArrayList copyOnWriteArrayList, final int i, final boolean z) {
        ...
        if (DBG) {
            Slogf.m88d(TAG, "dispatchVisibilityChanged(%d -> %b): sending to %d listeners", Integer.valueOf(i), Boolean.valueOf(z), Integer.valueOf(copyOnWriteArrayList.size()));
        }
