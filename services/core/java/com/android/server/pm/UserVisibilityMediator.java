  ...
  */
  public int assignUserToDisplayOnStart(int i, int i2, int i3, int i4) {
    Preconditions.checkArgument(
        !isSpecialUserId(i), "user id cannot be generic: %d", new Object[] {Integer.valueOf(i)});
    ...
    if (z) {
      Slogf.m88d(
          TAG,
          "assignUserToDisplayOnStart(%d, %d, %s, %d): actualProfileGroupId=%d",
          Integer.valueOf(i),
          Integer.valueOf(i2),
    ...
    synchronized (this.mLock) {
      ...
      if (z) {
        Slogf.m88d(
            TAG,
            "result of getUserVisibilityOnStartLocked(%s)",
            UserManagerInternal.userAssignmentResultToString(userVisibilityOnStartLocked));
      }
      if (userVisibilityOnStartLocked != -1 && userVisibilityOnStartLocked != 3) {
        ...
        if (z) {
          Slogf.m88d(
              TAG,
              "mapping result: %s",
              secondaryDisplayMappingStatusToString(canAssignUserToDisplayLocked));
        }
        ...
        if (i3 == 1) {
          ...
          if (i3 != 2) {
            if (i3 != 3) {
              Slogf.wtf(
                  TAG,
                  "invalid userStartMode passed to assignUserToDisplayOnStart: %d",
                  Integer.valueOf(i3));
            }
          ...
          if (canAssignUserToDisplayLocked != 1) {
            if (z) {
              Slogf.m88d(
                  TAG,
                  "adding user / display mapping (%d -> %d)",
                  Integer.valueOf(i),
                  Integer.valueOf(i4));
            }
            ...
            Slogf.wtf(
                TAG,
                "invalid resut from canAssignUserToDisplayLocked: %d",
                Integer.valueOf(canAssignUserToDisplayLocked));
          } else if (z) {
        ...
        if (z) {
          Slogf.m88d(
              TAG,
              "adding visible user / profile group id mapping (%d -> %d)",
              Integer.valueOf(i),
              Integer.valueOf(i5));
        }
  ...
  public final int getUserVisibilityOnStartLocked(int i, int i2, int i3, int i4) {
    if (i3 == 2 && i4 != 0) {
      Slogf.wtf(
          TAG,
          "cannot start user (%d) as BACKGROUND_USER on secondary display (%d) (it should be"
    ...
    if (i4 == 0 && z) {
      if (this.mVisibleBackgroundUserOnDefaultDisplayEnabled && isCurrentUserLocked(i)) {
        Slogf.wtf(
            TAG,
            "trying to start current user (%d) visible in background on default display",
            Integer.valueOf(i));
        return 3;
      }
      if (!this.mVisibleBackgroundUserOnDefaultDisplayEnabled && !isProfile(i, i2)) {
        Slogf.wtf(
            TAG, "cannot start full user (%d) visible on default display", Integer.valueOf(i));
        return -1;
      }
    }
    boolean z2 = i3 == 1;
    if (i4 != 0) {
      if (z2) {
        Slogf.m105w(
            TAG,
            "getUserVisibilityOnStartLocked(%d, %d, %s, %d) failed: cannot start foreground user on"
                + " secondary display",
            Integer.valueOf(i),
            Integer.valueOf(i2),
            UserManagerInternal.userStartModeToString(i3),
      ...
      if (!this.mVisibleBackgroundUsersEnabled) {
        Slogf.m105w(
            TAG,
            "getUserVisibilityOnStartLocked(%d, %d, %s, %d) failed: called on device that doesn't"
                + " support multiple users on multiple displays",
            Integer.valueOf(i),
            Integer.valueOf(i2),
            UserManagerInternal.userStartModeToString(i3),
            Integer.valueOf(i4));
    ...
    if (isProfile(i, i2)) {
      if (i4 != 0) {
        Slogf.m105w(
            TAG,
            "canStartUserLocked(%d, %d, %s, %d) failed: cannot start profile user on secondary"
                + " display",
            Integer.valueOf(i),
            Integer.valueOf(i2),
            UserManagerInternal.userStartModeToString(i3),
      ...
      if (i3 == 3) {
        ...
        Slogf.m105w(
            TAG,
            "getUserVisibilityOnStartLocked(%d, %d, %s, %d) failed: cannot start profile user"
                + " visible when its parent is not visible in that display",
            Integer.valueOf(i),
            Integer.valueOf(i2),
            UserManagerInternal.userStartModeToString(i3),
            Integer.valueOf(i4));
        ...
        && isUserAssignedToDisplayOnStartLocked(i, i4)) {
      if (DBG) {
        Slogf.m88d(
            TAG,
  ...
  public final int canAssignUserToDisplayLocked(int i, int i2, int i3, int i4) {
    ...
    if (i4 == 0) {
      if (this.mVisibleBackgroundUserOnDefaultDisplayEnabled && i3 == 3) {
        ...
        if (userStartedOnDisplay != -10000 && userStartedOnDisplay != i2) {
          Slogf.m105w(
              TAG,
              "canAssignUserToDisplayLocked(): cannot start user %d visible on default display"
                  + " because user %d already did so",
              Integer.valueOf(i),
              Integer.valueOf(userStartedOnDisplay));
          return -1;
        }
        z = true;
      } else {
    ...
    for (int i5 = 0; i5 < this.mUsersAssignedToDisplayOnStart.size(); i5++) {
      ...
      if (i4 == valueAt) {
        Slogf.m105w(
            TAG,
            "Cannot assign user %d to display %d because such display is already assigned to user"
                + " %d",
            Integer.valueOf(i),
            Integer.valueOf(i4),
            Integer.valueOf(keyAt));
      ...
      if (i == keyAt) {
        Slogf.m105w(
            TAG,
            "Cannot assign user %d to display %d because such user is as already assigned to"
                + " display %d",
            Integer.valueOf(i),
            Integer.valueOf(i4),
            Integer.valueOf(keyAt));
  ...
  public final boolean isUserAssignedToDisplayOnStartLocked(int i, int i2) {
    ...
    Slogf.wtf(
        TAG,
        "isUserAssignedToDisplayOnStartLocked(%d, %d): called when mUsersAssignedToDisplayOnStart"
            + " is null",
        Integer.valueOf(i),
        Integer.valueOf(i2));
    return false;
  ...
  public boolean isUserVisible(int i, int i2) {
    ...
    if (!this.mVisibleBackgroundUsersEnabled) {
      if (DBG) {
        Slogf.m88d(
            TAG,
            "isUserVisible(%d, %d): returning false as device does not support visible background"
                + " users",
            Integer.valueOf(i),
            Integer.valueOf(i2));
      }
  ...
  public final boolean isFullUserVisibleOnBackgroundLocked(int i, int i2) {
    return this.mUsersAssignedToDisplayOnStart.get(i, -1) == i2
        || this.mExtraDisplaysAssignedToUsers.get(i2, -10000) == i;
  }
      ...
      if (this.mVisibleBackgroundUserOnDefaultDisplayEnabled) {
        ...
        if (userStartedOnDisplay != -10000) {
          if (DBG) {
            Slogf.m88d(
                TAG,
                "getMainDisplayAssignedToUser(%d): returning INVALID_DISPLAY for current user user"
                    + " %d was started on DEFAULT_DISPLAY",
                Integer.valueOf(i),
                Integer.valueOf(userStartedOnDisplay));
          }
          return -1;
  ...
  public final int getUserAssignedToDisplay(int i, boolean z) {
    ...
    synchronized (this.mLock) {
      ...
      if (!z) {
        ...
        Slogf.m88d(
            TAG,
            "getUserAssignedToDisplay(%d): no user assigned to display, returning USER_NULL"
                + " instead",
            Integer.valueOf(i));
        return -10000;
      }
      ...
      if (DBG) {
        Slogf.m88d(
            TAG,
            "getUserAssignedToDisplay(%d): no user assigned to display, returning current user (%d)"
                + " instead",
            Integer.valueOf(i),
  ...
  public final void dispatchVisibilityChanged(IntArray intArray, IntArray intArray2) {
    ...
    if (DBG) {
      Slogf.m88d(
          TAG,
          "dispatchVisibilityChanged(): visibleUsersBefore=%s, visibleUsersAfter=%s, %d listeners"
              + " (%s)",
          intArray,
          intArray2,
  ...
  public final void dump(IndentingPrintWriter indentingPrintWriter) {
    ...
    synchronized (this.mLock) {
      ...
      dumpSparseIntArray(
          indentingPrintWriter, this.mUsersAssignedToDisplayOnStart, "user / display", "u", "d");
      dumpSparseIntArray(
          indentingPrintWriter,
          this.mExtraDisplaysAssignedToUsers,
          "extra display / user",
  ...
  public static String secondaryDisplayMappingStatusToString(int i) {
    return DebugUtils.constantToString(
        UserVisibilityMediator.class, "SECONDARY_DISPLAY_MAPPING_", i);
  }
}
