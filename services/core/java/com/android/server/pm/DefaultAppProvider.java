...
public class DefaultAppProvider {
  ...
  public static /* synthetic */ void lambda$setDefaultBrowser$0(
      AndroidFuture androidFuture, Boolean bool) {
  ...
  public String getDefaultHome(int i) {
    return getRoleHolder(
        ...
        ((UserManagerInternal) this.mUserManagerInternalSupplier.get()).getProfileParentId(i));
  }
  ...
  public boolean setDefaultHome(String str, int i, Executor executor, Consumer consumer) {
    ...
    try {
      roleManager.addRoleHolderAsUser(
          "android.app.role.HOME", str, 0, UserHandle.of(i), executor, consumer);
      Binder.restoreCallingIdentity(clearCallingIdentity);
      return true;
    } catch (Throwable th) {
      Binder.restoreCallingIdentity(clearCallingIdentity);
  ...
  public final String getRoleHolder(String str, int i) {
    ...
    try {
      return (String)
          CollectionUtils.firstOrNull(roleManager.getRoleHoldersAsUser(str, UserHandle.of(i)));
    } finally {
      Binder.restoreCallingIdentity(clearCallingIdentity);
    }
  }
