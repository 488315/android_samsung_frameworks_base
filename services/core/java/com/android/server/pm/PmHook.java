...
public abstract class PmHook {
  ...
  public static final void beginInstallLog(AndroidPackage androidPackage, int i) {
    AuditLog.logAsUser(
        ...
        String.format(
            ...
            androidPackage.getPackageName(), Long.valueOf(androidPackage.getLongVersionCode())),
        "",
        i);
  }
  ...
  public static final void uninstallLog(String str, boolean z, int i) {
    AuditLog.logAsUser(
        ...
        "PackageManagerService",
        String.format(
            z ? "Uninstall application %s succeeded" : "Uninstall application %s failed", str),
        "",
        i);
  }
  ...
  public static final void installSuccesLog(String str, int i) {
    AuditLog.logAsUser(
        ...
        Process.myPid(),
        "PackageManagerService",
        String.format("Install application %s succeeded", str),
        "",
        i);
  }
  ...
  public static final void auditLogInstallFail(String str, int i, boolean z) {
    AuditLog.logAsUser(
        ...
        Process.myPid(),
        "PackageManagerService",
        String.format("Install application %s failed", str),
        z ? "" : null,
        i);
  }
    ...
    try {
      if (str2 == null) {
        if (i != 1 && i != 0) {
          if (i == 2 || i == 3) {
            AuditLog.logAsUser(
                ...
                Process.myPid(),
                "PackageManagerService",
                String.format("Application %s has been disabled.", str),
                "",
                i2);
          }
        }
        AuditLog.logAsUser(
            ...
            Process.myPid(),
            "PackageManagerService",
            String.format("Application %s has been enabled.", str),
            "",
            i2);
      }
      if (i != 1 && i != 0) {
        if (i == 2 || i == 3) {
          AuditLog.logAsUser(
              ...
              Process.myPid(),
              "PackageManagerService",
              String.format("Component %s/%s has been disabled.", str, str2),
              "",
              i2);
        }
      }
      AuditLog.logAsUser(
          ...
          Process.myPid(),
          "PackageManagerService",
          String.format("Component %s/%s has been enabled.", str, str2),
          "",
          i2);
    } finally {
  ...
  */
  public static boolean isUpdateAllowedByMdm(
      PackageManager.ComponentEnabledSetting componentEnabledSetting, int i, Context context) {
    ...
    try {
      if (asInterface == null) {
        ...
        if (!asInterface.getApplicationStateEnabledAsUser(packageName, false, i)) {
          Log.w(
              "PM_HOOK",
              "This app can not be enabled due to EDM policy. packageName = " + packageName);
          return false;
        }
        if (componentEnabledSetting.isComponent()) {
          ComponentName componentName = componentEnabledSetting.getComponentName();
          if (!asInterface.getApplicationComponentState(
              new ContextInfo(Binder.getCallingUid()), componentName)) {
            Log.w(
                "PM_HOOK",
                "This component can not be enabled due to EDM policy. componentName = "
                    + componentName);
            return false;
          }
        }
    ...
    try {
      if (SemPersonaManager.isKnoxId(i)
          && enabledState == 1
          && (componentNameForUid =
                  edmStorageProvider.getComponentNameForUid(
                      (mUMContainerOwnerUid =
                          (edmStorageProvider = new EdmStorageProvider(context))
                              .getMUMContainerOwnerUid(i))))
              != null
          && componentNameForUid.getPackageName().equals(packageName)
          && UserHandle.getUserId(mUMContainerOwnerUid) != i) {
        ...
        if (devicePolicyManager != null) {
          ...
          if (profileOwnerAsUser != null
              && profileOwnerAsUser.getPackageName() != null
              && packageName.equals(profileOwnerAsUser.getPackageName())) {
