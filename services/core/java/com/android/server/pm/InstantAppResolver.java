...
public abstract class InstantAppResolver {
  ...
  public static Intent sanitizeIntent(Intent intent) {
    ...
    intent2.setDataAndType(
        intent.getData() == null ? null : Uri.fromParts(intent.getScheme(), "", ""),
        intent.getType());
  ...
  */
  public static AuxiliaryResolveInfo doInstantAppResolutionPhaseOne(
      ...
      InstantAppRequest instantAppRequest) {
    Intent intent;
    String str;
    int i;
    int i2;
    ...
    try {
      List instantAppResolveInfoList =
          instantAppResolverConnection.getInstantAppResolveInfoList(
              buildRequestInfo(instantAppRequest));
        ...
        try {
          auxiliaryResolveInfo =
              filterInstantAppIntent(
                  computer,
                  userManagerService,
                  instantAppResolveInfoList,
                  intent2,
                  instantAppRequest.resolvedType,
                  instantAppRequest.userId,
                  intent2.getPackage(),
                  str2,
    ...
    return (auxiliaryResolveInfo == null
            || (intent.getFlags() & IInstalld.FLAG_FREE_CACHE_DEFY_TARGET_FREE_BYTES) == 0)
        ? auxiliaryResolveInfo
        : new AuxiliaryResolveInfo(
            str2,
            false,
            createFailureIntent(intent, str2),
            (List) null,
            instantAppRequest.hostDigestPrefixSecure);
  }

  public static void doInstantAppResolutionPhaseTwo(
      final Context context,
      final Computer computer,
      final UserManagerService userManagerService,
      InstantAppResolverConnection instantAppResolverConnection,
      final InstantAppRequest instantAppRequest,
    ...
    try {
      instantAppResolverConnection.getInstantAppIntentFilterList(
          buildRequestInfo(instantAppRequest),
          new InstantAppResolverConnection
              .PhaseTwoCallback() { // from class: com.android.server.pm.InstantAppResolver.1
            ...
            public void onPhaseTwoResolved(List list, long j) {
              ...
              if (list != null && list.size() > 0) {
                ...
                AuxiliaryResolveInfo filterInstantAppIntent =
              ...
              Intent buildEphemeralInstallerIntent =
                  InstantAppResolver.buildEphemeralInstallerIntent(
                      intent5,
                      intent6,
                      intent4,
                      str2,
                      ...
                      str4,
                      i,
                      auxiliaryResolveInfo.installFailureActivity,
                      str,
                      false,
                      auxiliaryResolveInfo.filters);
              ...
              InstantAppResolver.logMetrics(
                  900, j, str, instantAppRequest.responseObj.filters != null ? 0 : 1);
          ...
          try {
            ...
            if (list != null && list.size() == 1) {
              intent4.putExtra(
                  ...
                  ((AuxiliaryResolveInfo.AuxiliaryFilter) list.get(0)).splitName);
            }
      ...
      if (list != null) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
          ...
          AuxiliaryResolveInfo.AuxiliaryFilter auxiliaryFilter =
              (AuxiliaryResolveInfo.AuxiliaryFilter) list.get(i2);
          InstantAppResolveInfo instantAppResolveInfo = auxiliaryFilter.resolveInfo;
          bundle2.putBoolean(
              "android.intent.extra.UNKNOWN_INSTANT_APP",
              instantAppResolveInfo != null && instantAppResolveInfo.shouldLetInstallerDecide());
          ...
          if (i2 == 0) {
            ...
            intent5.putExtra(
                "android.intent.extra.VERSION_CODE", (int) auxiliaryFilter.versionCode);
          }
        }
  ...
  public static InstantAppRequestInfo buildRequestInfo(InstantAppRequest instantAppRequest) {
    return new InstantAppRequestInfo(
        sanitizeIntent(instantAppRequest.origIntent),
        instantAppRequest.hostDigestPrefixSecure,
  ...
  public static AuxiliaryResolveInfo filterInstantAppIntent(
      Computer computer,
      UserManagerService userManagerService,
      List list,
      Intent intent,
    ...
    while (it.hasNext()) {
      ...
      if (z2 && instantAppResolveInfo.shouldLetInstallerDecide()) {
        Slog.d(
            "PackageManager",
            "InstantAppResolveInfo with mShouldLetInstallerDecide=true when digest required;"
                + " ignoring");
      } else {
        ...
        List computeResolveFilters =
            computeResolveFilters(
                computer, userManagerService, intent, str, i, str2, str3, instantAppResolveInfo);
  ...
  public static List computeResolveFilters(
      Computer computer,
      UserManagerService userManagerService,
      Intent intent,
      String str,
    ...
    if (instantAppResolveInfo.shouldLetInstallerDecide()) {
      return Collections.singletonList(
          new AuxiliaryResolveInfo.AuxiliaryFilter(
              instantAppResolveInfo, (String) null, instantAppResolveInfo.getExtras()));
    }
    ...
    ComponentResolver.InstantAppIntentResolver instantAppIntentResolver =
        new ComponentResolver.InstantAppIntentResolver(userManagerService);
    for (int size = intentFilters.size() - 1; size >= 0; size--) {
      InstantAppIntentFilter instantAppIntentFilter =
          (InstantAppIntentFilter) intentFilters.get(size);
        ...
        for (int size2 = filters.size() - 1; size2 >= 0; size2--) {
          ...
          if ((authoritiesIterator != null && authoritiesIterator.hasNext())
              || ((!intentFilter.hasDataScheme("http") && !intentFilter.hasDataScheme("https"))
                  ...
                  || !intentFilter.hasCategory("android.intent.category.BROWSABLE"))) {
            instantAppIntentResolver.addFilter(
                computer,
                new AuxiliaryResolveInfo.AuxiliaryFilter(
                    intentFilter,
  ...
  public static void logMetrics(int i, long j, String str, int i2) {
    getLogger()
        .write(
            new LogMaker(i)
                .setType(4)
                .addTaggedData(901, new Long(System.currentTimeMillis() - j))
                .addTaggedData(903, str)
                .addTaggedData(902, new Integer(i2)));
  }
}
