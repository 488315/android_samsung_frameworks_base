...
public class EmergencyModePackageHandler {
  ...
  public EmergencyModePackageHandler(
      Context context,
  ...
  public class EMPackageHandler extends Handler {
    ...
    public EMPackageHandler(Looper looper) {
      ...
      this.emPreviousID = -1;
      this.TIMEOUT = 60000L;
      this.emReceiverBroadcastNext =
          new IIntentReceiver.Stub() { // from class:
            // com.android.server.pm.EmergencyModePackageHandler.EMPackageHandler.1
            public void performReceive(
              ...
              if (EMPackageHandler.this.emID == i3) {
                Elog.d(
                    "EMPkgHandler",
                    "performReceive sending EM_SEND_PENDING_BROADCAST id[" + i3 + "]");
                EmergencyModePackageHandler.this.mEMPackageHander.sendEmptyMessage(2);
                return;
              }
              Elog.v(
                  "EMPkgHandler",
                  "performReceive canceled emID["
                      + EMPackageHandler.this.emID
                      + "]  thisID["
                      + i3
                      + "]");
            }
    ...
    public void doHandleMessage(Message message) {
      ...
      if (i2 == 1) {
        ...
        PendingPackageBroadcastsWithList pendingPackageBroadcastsWithList =
            EmergencyModePackageHandler.this.mPendingBroadcastsForBurst;
        if (pendingPackageBroadcastsWithList != null) {
          ...
          if (size <= 0) {
            Elog.v(
                "EMPkgHandler",
                "pending size [" + this.emSize + "] EM_MAKE_PENDING_BROADCAST Cancel");
            this.emProgression = 0;
          ...
          int handlePendingBroadcastsForBurst =
              EmergencyModePackageHandler.this.handlePendingBroadcastsForBurst(
                  strArr, arrayListArr, iArr, zArr, iArr2, size);
      ...
      if (i2 != 2) {
        ...
        if (i2 == 4) {
          ...
          Intent intent =
              new Intent("com.samsung.intent.action.EMERGENCY_FINISHED_SENDING_PACKAGE_CHANGED");
      ...
      if (this.emCurrentPosition >= this.emTotSize) {
        ...
        Elog.v(
            "EMPkgHandler",
            "SKIP EMERGENCY_FINISHED_SENDING_PACKAGE_CHANGED emPrevioudID["
                + this.emPreviousID
                + "] emID["
                + this.emID
                + "]");
      ...
      while (i3 < 10 && (i = this.emCurrentPosition) < this.emTotSize) {
        PackageStateInternal packageStateInternal =
            snapshotComputer.getPackageStateInternal(this.emPackages[i]);
        int enabledState =
            packageStateInternal != null
                ? packageStateInternal.getUserStateOrDefault(this.emUserId).getEnabledState()
                : -1;
        ...
        if (enabledState == iArr3[i4]) {
          ...
          if (z) {
            ...
            EmergencyModePackageHandler emergencyModePackageHandler =
                EmergencyModePackageHandler.this;
            String[] strArr2 = this.emPackages;
            int i5 = this.emCurrentPosition;
            emergencyModePackageHandler.sendPackageChangedBroadcastWithReceiver(
                strArr2[i5],
                this.emDontKillFlags[i5],
                this.emComponents[i5],
                this.emUids[i5],
                this.emReceiverBroadcastNext,
                this.emID);
          } else {
            EmergencyModePackageHandler.this.sendPackageChangedBroadcastWithReceiver(
                this.emPackages[i4],
                this.emDontKillFlags[i4],
                this.emComponents[i4],
                this.emUids[i4],
                null,
                this.emID);
          }
          ...
          Elog.v(
              "EMPkgHandler",
              "SKIP EM_SEND_PENDING_BROADCAST ["
                  + this.emCurrentPosition
                  + "] / ["
                  + (this.emTotSize - 1)
                  + "] name["
                  + this.emPackages[this.emCurrentPosition]
                  + "] curr["
                  + enabledState
                  + "]  now["
                  + this.emNewState[this.emCurrentPosition]
                  + "]  emUserId["
                  + this.emUserId
                  + "]");
  ...
  */
  public final void sendPackageChangedBroadcastWithReceiver(
      String str, boolean z, ArrayList arrayList, int i, IIntentReceiver iIntentReceiver, int i2) {
    boolean z2;
    ...
    try {
      try {
        ...
        this.mPkgMgrSvc.sendPackageBroadcast(
            ...
            null,
            iIntentReceiver,
            new int[] {UserHandle.getUserId(i)},
            iArr,
            null,
            null);
      }
  ...
  */
  public void setApplicationEnabledSettingWithList(
    ...
    Elog.d(
        "EMPkgHandler",
        "setApplicationEnabledSettingWithList usePending = " + z + "  Start now = " + z2);
      ...
      if (isKnox(i8)) {
        ...
        while (it.hasNext()) {
          ...
          if (this.mProtectedPackages.isPackageStateProtected(i8, str2)) {
            Elog.d(
                "EMPkgHandler",
                "setApplicationEnabledSettingWithList- skip ProtectedPackage = " + str2);
            it = it;
          } else {
            arrayList2.add(str2);
          }
    ...
    if (z) {
      ...
      PendingPackageBroadcastsWithList pendingPackageBroadcastsWithList =
          this.mPendingBroadcastsForBurst;
      if (pendingPackageBroadcastsWithList != null) {
        int size = pendingPackageBroadcastsWithList.size();
        if (size <= 0) {
          return;
        ...
        handlePendingBroadcastsForBurst =
            handlePendingBroadcastsForBurst(strArr, arrayListArr, iArr, zArr, new int[size], size);
        if (z) {
  ...
  public final int handlePendingBroadcastsForBurst(
      String[] strArr, ArrayList[] arrayListArr, int[] iArr, boolean[] zArr, int[] iArr2, int i) {
    ...
    for (int i3 = 0; i3 < this.mPendingBroadcastsForBurst.userIdCount(); i3++) {
      ...
      Iterator it =
          this.mPendingBroadcastsForBurst.packagesForUserId(userIdAt).entrySet().iterator();
      LinkedHashMap packagesNewStateForUserId =
          this.mPendingBroadcastsForBurst.packagesNewStateForUserId(userIdAt);
      if (packagesNewStateForUserId != null) {
        while (it.hasNext() && i2 < i) {
          Map.Entry entry = (Map.Entry) it.next();
          PackageStateInternal packageStateInternal =
              snapshotComputer.getPackageStateInternal((String) entry.getKey());
  ...
  public void sendPackageChangedBroadcastEMPkgHndlr(
      String str, boolean z, ArrayList arrayList, int i) {
    this.mPkgMgrSvc.sendPackageChangedBroadcast(
        this.mPkgMgrSvc.snapshotComputer(), str, z, arrayList, i, "EmergencyMode");
  }
}
