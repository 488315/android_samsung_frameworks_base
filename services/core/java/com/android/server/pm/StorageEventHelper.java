...
public final class StorageEventHelper extends StorageEventListener {
  ...
  public void onVolumeForgotten(String str) {
    ...
    synchronized (this.mPm.mLock) {
      for (PackageStateInternal packageStateInternal :
          this.mPm.mSettings.getVolumePackagesLPr(str)) {
        Slog.d(
            "PackageManager",
            "Destroying "
        ...
        this.mPm.deletePackageVersioned(
            new VersionedPackage(packageStateInternal.getPackageName(), -1),
            new PackageManager.LegacyPackageDeleteObserver((IPackageDeleteObserver) null)
                .getBinder(),
            0,
            2);
  ...
  public final void loadPrivatePackages(final VolumeInfo volumeInfo) {
    this.mPm.mHandler.post(
        new Runnable() { // from class:
          // com.android.server.pm.StorageEventHelper$$ExternalSyntheticLambda0
  ...
  */
  public final void lambda$loadPrivatePackages$0(VolumeInfo volumeInfo) {
    ...
    List<PackageStateInternal> volumePackagesLPr;
    int i;
    ...
    for (PackageStateInternal packageStateInternal : volumePackagesLPr) {
      arrayList.add(
          this.mPm.freezePackage(
              packageStateInternal.getPackageName(), -1, "loadPrivatePackagesInner", 13));
      synchronized (this.mPm.mInstallLock) {
        try {
          arrayList2.add(
              installPackageHelper.initPackageTracedLI(
                  packageStateInternal.getPath(), defParseFlags, 512));
        } catch (PackageManagerException e) {
          Slog.w(
              "PackageManager",
              "Failed to scan " + packageStateInternal.getPath() + ": " + e.getMessage());
        }
    ...
    StorageManagerInternal storageManagerInternal =
        (StorageManagerInternal) this.mPm.mInjector.getLocalService(StorageManagerInternal.class);
      ...
      if (z) {
        PackageManagerServiceUtils.logCriticalInfo(
            ...
            "Partitions fingerprint changed from "
                + findOrCreateVersion.fingerprint
                + " to "
                + PackagePartitions.FINGERPRINT
                + "; regranting permissions for "
                + str);
      }
  ...
  public final void unloadPrivatePackages(final VolumeInfo volumeInfo) {
    this.mPm.mHandler.post(
        new Runnable() { // from class:
          // com.android.server.pm.StorageEventHelper$$ExternalSyntheticLambda1
          @Override // java.lang.Runnable
          public final void run() {
          ...
          if (packageStateInternal.getPkg() != null) {
            ...
            PackageFreezer freezePackageForDelete =
                this.mPm.freezePackageForDelete(
                    packageStateInternal.getPackageName(), -1, 1, "unloadPrivatePackagesInner", 13);
            try {
              if (this.mDeletePackageHelper.deletePackageLIF(
                  packageStateInternal.getPackageName(),
                  null,
                  false,
                  userIds,
  ...
  public void reconcileApps(Computer computer, String str) {
    ...
    for (File file : FileUtils.listFilesOrEmpty(Environment.getDataAppDirectory(str))) {
      boolean z = true;
      if ((ApkLiteParseUtils.isApkFile(file) || file.isDirectory())
