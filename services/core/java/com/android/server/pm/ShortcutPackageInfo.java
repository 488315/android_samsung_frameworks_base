...
public class ShortcutPackageInfo {
  ...
  public int canRestoreTo(ShortcutService shortcutService, PackageInfo packageInfo, boolean z) {
    ...
    if (!shortcutService.isSmartSwitchBackupAllowed()
        && (!ShortcutService.shouldBackupApp(packageInfo) || !this.mBackupSourceBackupAllowed)) {
    ...
    Slog.w(
        ...
        String.format(
            ...
            Long.valueOf(this.mBackupSourceVersionCode)));
    return 100;
  }
  ...
  public static ShortcutPackageInfo generateForInstalledPackageForTest(
    ...
    ShortcutPackageInfo shortcutPackageInfo =
        new ShortcutPackageInfo(
            packageInfoWithSignatures.getLongVersionCode(),
            packageInfoWithSignatures.lastUpdateTime,
            BackupUtils.hashSignatureArray(signingInfo.getApkContentsSigners()),
            false);
    shortcutPackageInfo.mBackupSourceBackupAllowed =
        ShortcutService.shouldBackupApp(packageInfoWithSignatures);
  ...
  public void refreshSignature(
      ShortcutService shortcutService, ShortcutPackageItem shortcutPackageItem) {
    if (this.mIsShadow) {
      shortcutService.wtf(
          "Attempted to refresh package info for shadow package "
              + shortcutPackageItem.getPackageName()
              + ", user="
              + shortcutPackageItem.getOwnerUserId());
      return;
    }
    PackageInfo packageInfoWithSignatures =
        shortcutService.getPackageInfoWithSignatures(
  ...
  public void saveToXml(
      ShortcutService shortcutService, TypedXmlSerializer typedXmlSerializer, boolean z) {
    ...
    ShortcutService.writeAttr(
        typedXmlSerializer, "allow-backup-initialized", this.mBackupAllowedInitialized);
    ShortcutService.writeAttr(typedXmlSerializer, "bk_src_version", this.mBackupSourceVersionCode);
    ShortcutService.writeAttr(
        typedXmlSerializer, "bk_src_backup-allowed", this.mBackupSourceBackupAllowed);
    for (int i = 0; i < this.mSigHashes.size(); i++) {
      typedXmlSerializer.startTag((String) null, "signature");
      ShortcutService.writeAttr(
          typedXmlSerializer,
          "hash",
          Base64.getEncoder().encodeToString((byte[]) this.mSigHashes.get(i)));
  ...
  public void loadFromXml(TypedXmlPullParser typedXmlPullParser, boolean z) {
    long parseLongAttribute =
        ShortcutService.parseLongAttribute(typedXmlPullParser, "version", -1L);
    long parseLongAttribute2 =
        ShortcutService.parseLongAttribute(typedXmlPullParser, "last_udpate_time");
    int i = 1;
    boolean z2 = z || ShortcutService.parseBooleanAttribute(typedXmlPullParser, "shadow");
    long parseLongAttribute3 =
        ShortcutService.parseLongAttribute(typedXmlPullParser, "bk_src_version", -1L);
    boolean parseBooleanAttribute =
        ShortcutService.parseBooleanAttribute(typedXmlPullParser, "allow-backup", true);
    boolean parseBooleanAttribute2 =
        ShortcutService.parseBooleanAttribute(typedXmlPullParser, "bk_src_backup-allowed", true);
      ...
      if (next == 2) {
        ...
        if (depth2 == depth + 1) {
          ...
          if (name.equals("signature")) {
            arrayList.add(
                Base64.getDecoder()
                    .decode(ShortcutService.parseStringAttribute(typedXmlPullParser, "hash")));
            i = 1;
          }
        }
