  ...
  public static void syncDataFromArtService(DynamicCodeLogger dynamicCodeLogger) {
    ...
    try {
      for (String str : withUnfilteredSnapshot.getPackageStates().keySet()) {
        for (DexContainerFileUseInfo dexContainerFileUseInfo :
            dexUseManagerLocal.getSecondaryDexContainerFileUseInfo(str)) {
          Iterator it = dexContainerFileUseInfo.getLoadingPackages().iterator();
          while (it.hasNext()) {
            dynamicCodeLogger.recordDex(
                dexContainerFileUseInfo.getUserHandle().getIdentifier(),
                dexContainerFileUseInfo.getDexContainerFile(),
                str,
                (String) it.next());
          }
        }
      ...
      try {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
          if (DynamicCodeLoggingService.this.mAuditWatchingStopRequested) {
            Log.w(
                DynamicCodeLoggingService.TAG,
                "Stopping AuditWatchingJob run at scheduler request");
            return false;
          }
