  ...
  public void updateDefaultCrossProfileIntentFilter() {
    ...
    for (UserInfo userInfo : this.mUserManagerInternal.getUsers(false)) {
      ...
      if (userProperties != null
          ...
          && (profileParentId = this.mUserManagerInternal.getProfileParentId(userInfo.id))
              != (i = userInfo.id)) {
        clearCrossProfileIntentFilters(
            i, this.mContext.getOpPackageName(), Integer.valueOf(profileParentId));
        clearCrossProfileIntentFilters(
            profileParentId, this.mContext.getOpPackageName(), Integer.valueOf(userInfo.id));
    ...
    synchronized (this.mLock) {
      ...
      while (it.hasNext()) {
        ...
        if (crossProfileIntentFilter.getOwnerPackage().equals(str)
            && (num == null || crossProfileIntentFilter.mTargetUserId == num.intValue())) {
