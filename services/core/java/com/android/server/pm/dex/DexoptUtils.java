...
public abstract class DexoptUtils {
  ...
  public static String[] getClassLoaderContexts(
      AndroidPackage androidPackage, List list, boolean[] zArr) {
    String encodeSharedLibraries = list != null ? encodeSharedLibraries(list) : "";
    String encodeClassLoader =
        encodeClassLoader("", androidPackage.getClassLoaderName(), encodeSharedLibraries);
    ...
    if (!androidPackage.isIsolatedSplitLoading()
        || splitDependencies == null
        || splitDependencies.size() == 0) {
      while (i < length) {
        if (zArr[i]) {
          strArr[i] =
              encodeClassLoader(name, androidPackage.getClassLoaderName(), encodeSharedLibraries);
        } else {
          strArr[i] = null;
        }
      ...
      for (int i2 = 0; i2 < splitRelativeCodePaths.length; i2++) {
        strArr2[i2] =
            encodeClassLoader(
                splitRelativeCodePaths[i2], androidPackage.getSplitClassLoaderNames()[i2]);
      }
      ...
      while (i < length) {
        String encodeClassLoader3 =
            encodeClassLoader("", androidPackage.getSplitClassLoaderNames()[i - 1]);
        if (zArr[i]) {
          if (strArr[i] != null) {
            encodeClassLoader3 =
                encodeClassLoaderChain(encodeClassLoader3, strArr[i]) + encodeSharedLibraries;
          }
  ...
  public static String encodeSharedLibrary(SharedLibraryInfo sharedLibraryInfo) {
    ...
    String encodeClassLoader =
        encodeClassLoader(
            encodeClasspath((String[]) allCodePaths.toArray(new String[allCodePaths.size()])),
  ...
  public static String[] getSplitRelativeCodePaths(AndroidPackage androidPackage) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (!parent2.equals(parent)) {
        Slog.wtf(
            "DexoptUtils", "Split paths have different base paths: " + parent2 + " and " + parent);
      }
    }
    return strArr;
  }
