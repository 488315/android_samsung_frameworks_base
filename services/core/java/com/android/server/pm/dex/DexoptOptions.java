  ...
  public DexoptParams convertToDexoptParams(int i) {
    if (this.mSplitName != null) {
      throw new UnsupportedOperationException(
          "Request to optimize only split " + this.mSplitName + " for " + this.mPackageName);
    }
    ...
    int i4 = this.mFlags;
    return new DexoptParams.Builder(convertToArtServiceDexoptReason(this.mCompilationReason), i3)
        .setCompilerFilter(this.mCompilerFilter)
        .setPriorityClass(
            (i4 & 4) != 0
                ? (i4 & IInstalld.FLAG_FREE_CACHE_DEFY_TARGET_FREE_BYTES) != 0
                    ? 80
                    : (i4 & 512) != 0 ? 40 : 60
                : 100)
        .build();
