...
public abstract class ApkChecksums {
  ...
  public static void getChecksums(
    ...
    processRequiredChecksums(
        list, arrayList, i2, iOnChecksumsReadyListener, injector, SystemClock.uptimeMillis());
  }
                    ...
                    new Runnable() { // from class:
                      ...
                      public final void run() {
                        ApkChecksums.processRequiredChecksums(
                            list, list2, i, iOnChecksumsReadyListener, injector, j);
                      }
                    },
                    1000L);
            return;
  ...
  public static void getAvailableApkChecksums(
    ...
    if (isRequired(1, i, map)
        && (extractHashFromFS = extractHashFromFS(str, absolutePath)) != null) {
      map.put(Integer.valueOf(extractHashFromFS.getType()), extractHashFromFS);
    }
    if ((isRequired(32, i, map) || isRequired(64, i, map))
        && (extractHashFromV2V3Signature = extractHashFromV2V3Signature(str, absolutePath, i))
            != null) {
  ...
  public static void getInstallerChecksums(
      String str,
      File file,
      int i,
      String str2,
    ...
    if ((certificateArr == null || certificateArr.length != 0)
        && (findDigestsForFile = findDigestsForFile(file)) != null) {
      ...
      try {
        ...
        if (findSignatureForDigests != null) {
          Certificate[] verifySignature =
              verifySignature(readChecksums, Files.readAllBytes(findSignatureForDigests.toPath()));
        ...
        if (signatures != null && signatures.length != 0 && (signature = signatures[0]) != null) {
          ...
          for (Checksum checksum : readChecksums) {
            ...
            if (apkChecksum != null
                && !Arrays.equals(apkChecksum.getValue(), checksum.getValue())) {
                ...
                verityHashForFile(
                    ...
                    VerityBuilder.generateFsVerityRootHash(
                        ...
                        new ByteBufferFactory() { // from class:
                          // com.android.server.pm.ApkChecksums.1
  ...
  public static ApkChecksum extractHashFromFS(String str, String str2) {
    byte[] fsverityDigest;
    if (!containsFile(Environment.getProductDirectory(), str2)
        && (fsverityDigest = VerityUtils.getFsverityDigest(str2)) != null) {
      return new ApkChecksum(str, 1, fsverityDigest);
    }
    try {
      byte[] bArr =
          (byte[])
              ApkSignatureSchemeV4Verifier.extractCertificates(str2)
                  .contentDigests
                  .getOrDefault(3, null);
      if (bArr != null) {
  ...
  public static Map extractHashFromV2V3Signature(String str, String str2, int i) {
    ...
    ParseResult verifySignaturesInternal =
        ApkSignatureVerifier.verifySignaturesInternal(
            ParseTypeImpl.forDefaultParsing(), str2, 2, false);
    if (verifySignaturesInternal.isError()) {
      ...
      map =
          ((ApkSignatureVerifier.SigningDetailsWithDigests) verifySignaturesInternal.getResult())
              .contentDigests;
    }
    if (map == null) {
      return null;
  ...
  public static void calculateChecksumIfRequested(Map map, String str, File file, int i, int i2) {
    byte[] apkChecksum;
    if ((i & i2) == 0
        || map.containsKey(Integer.valueOf(i2))
        || (apkChecksum = getApkChecksum(file, i2)) == null) {
      return;
    }
    ...
    try {
      ...
      try {
        ...
        MessageDigest messageDigest =
            MessageDigest.getInstance(getMessageDigestAlgoForChecksumKind(i));
        while (true) {
      ...
      try {
        ...
        byte[][] computeContentDigestsPer1MbChunk =
            ApkSigningBlockUtils.computeContentDigestsPer1MbChunk(
                contentDigestAlgos, randomAccessFile.getFD(), signatureInfo);
        int length = contentDigestAlgos.length;
        for (int i2 = 0; i2 < length; i2++) {
          int checksumKindForContentDigestAlgo =
              getChecksumKindForContentDigestAlgo(contentDigestAlgos[i2]);
          if (checksumKindForContentDigestAlgo != -1) {
            map.put(
                Integer.valueOf(checksumKindForContentDigestAlgo),
                new ApkChecksum(
                    str, checksumKindForContentDigestAlgo, computeContentDigestsPer1MbChunk[i2]));
          }
        }
        randomAccessFile.close();
