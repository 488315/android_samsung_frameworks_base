  ...
  public void dump(PrintWriter printWriter, String str) {
    ...
    if (isSystem()) {
      ...
      try {
        ...
        for (String str3 :
            Resources.getSystem()
                .getStringArray(R.array.config_secondaryBuiltInDisplayWaterfallCutout)) {
  ...
  public final class Builder {
    ...
    public UserTypeDetails createUserTypeDetails() {
      Preconditions.checkArgument(
          this.mName != null, "Cannot create a UserTypeDetails with no name.");
      Preconditions.checkArgument(
          hasValidBaseType(),
          "UserTypeDetails " + this.mName + " has invalid baseType: " + this.mBaseType);
      Preconditions.checkArgument(
          hasValidPropertyFlags(),
          "UserTypeDetails "
              + this.mName
              + " has invalid flags: "
              + Integer.toHexString(this.mDefaultUserInfoPropertyFlags));
      checkSystemAndMainUserPreconditions();
      if (hasBadge()) {
        int[] iArr = this.mBadgeLabels;
        Preconditions.checkArgument(
            (iArr == null || iArr.length == 0) ? false : true,
            "UserTypeDetails " + this.mName + " has badge but no badgeLabels.");
        int[] iArr2 = this.mBadgeColors;
        Preconditions.checkArgument(
            (iArr2 == null || iArr2.length == 0) ? false : true,
            "UserTypeDetails " + this.mName + " has badge but no badgeColors.");
      }
      if (!isProfile()) {
        List list = this.mDefaultCrossProfileIntentFilters;
        Preconditions.checkArgument(
            list == null || list.isEmpty(),
            "UserTypeDetails %s has a non empty defaultCrossProfileIntentFilters",
      ...
      return new UserTypeDetails(
          ...
          iArr3,
          iArr4,
          iArr5 == null ? iArr4 : iArr5,
          this.mDefaultRestrictions,
          this.mDefaultSystemSettings,
    ...
    public final void checkSystemAndMainUserPreconditions() {
      Preconditions.checkArgument(
          ((this.mBaseType & IInstalld.FLAG_FREE_CACHE_DEFY_TARGET_FREE_BYTES) != 0)
              == ((this.mDefaultUserInfoPropertyFlags & 1) != 0),
          "UserTypeDetails " + this.mName + " cannot be SYSTEM xor PRIMARY.");
      Preconditions.checkArgument(
          (this.mDefaultUserInfoPropertyFlags & 16384) == 0 || this.mMaxAllowed == 1,
          "UserTypeDetails " + this.mName + " must not sanction more than one MainUser.");
    }
  }
