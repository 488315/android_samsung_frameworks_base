...
public abstract class VerificationUtils {
  ...
  public static long getVerificationTimeoutSamsung(Context context, boolean z) {
    ...
    return Math.min(
        Settings.Global.getLong(context.getContentResolver(), "verifier_timeout_samsung", 0L),
        30000L);
  }

  public static long getDefaultVerificationTimeout(Context context) {
    return Math.max(
        Settings.Global.getLong(context.getContentResolver(), "verifier_timeout", 10000L), 10000L);
  }

  public static long getDefaultStreamingVerificationTimeout(Context context) {
    return Math.max(
        Settings.Global.getLong(context.getContentResolver(), "streaming_verifier_timeout", 3000L),
        3000L);
  }
  ...
  public static void broadcastPackageVerified(
    ...
    context.sendBroadcastAsUser(
        intent, userHandle, "android.permission.PACKAGE_VERIFICATION_AGENT");
  }

  public static void processVerificationResponseOnTimeout(
      int i,
      PackageVerificationState packageVerificationState,
      PackageVerificationResponse packageVerificationResponse,
      PackageManagerService packageManagerService) {
    packageVerificationState.setVerifierResponseOnTimeout(
        packageVerificationResponse.callerUid, packageVerificationResponse.code);
    processVerificationResponse(
        i,
        packageVerificationState,
        packageVerificationResponse.code,
        "Verification timed out",
        packageManagerService);
  }

  public static void processVerificationResponse(
      int i,
      PackageVerificationState packageVerificationState,
      PackageVerificationResponse packageVerificationResponse,
      PackageManagerService packageManagerService) {
    packageVerificationState.setVerifierResponse(
        packageVerificationResponse.callerUid, packageVerificationResponse.code);
    processVerificationResponse(
        i,
        packageVerificationState,
    ...
    if (packageVerificationState.isVerificationComplete()) {
      ...
      Uri fromFile =
          verifyingSession != null
              ? Uri.fromFile(verifyingSession.mOriginInfo.mResolvedFile)
              : null;
      ...
      if (packageManagerService != null && verifyingSession != null) {
        broadcastPackageVerified(
            i,
            fromFile,
            i3,
            null,
            verifyingSession.getDataLoaderType(),
            verifyingSession.getUser(),
