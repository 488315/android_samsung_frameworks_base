...
public class AppDataHelper {
  ...
  public void prepareAppDataPostCommitLIF(final AndroidPackage androidPackage, int i) {
    ...
    if (!shouldHaveAppStorage(androidPackage)) {
      Slog.w(
          "PackageManager", "Skipping preparing app data for " + androidPackage.getPackageName());
      return;
    }
    ...
    for (final UserInfo userInfo : userManagerInternal.getUsers(false)) {
      if (StorageManager.isUserKeyUnlocked(userInfo.id)
          && storageManagerInternal.isCeStoragePrepared(userInfo.id)) {
        i2 = 3;
      ...
      if (packageLPr.getInstalled(userInfo.id)) {
        prepareAppData(batch, androidPackage, i, userInfo.id, i3)
            .thenRun(
                new Runnable() { // from class:
                  // com.android.server.pm.AppDataHelper$$ExternalSyntheticLambda0
                  @Override // java.lang.Runnable
                  public final void run() {
                    AppDataHelper.lambda$prepareAppDataPostCommitLIF$0(
                        UserManagerInternal.this, userInfo, androidPackage, storageManagerInternal);
                  }
                });
      }
    }
  ...
  public static /* synthetic */ void lambda$prepareAppDataPostCommitLIF$0(
      UserManagerInternal userManagerInternal,
      UserInfo userInfo,
      AndroidPackage androidPackage,
      StorageManagerInternal storageManagerInternal) {
    if (userManagerInternal.isUserUnlockingOrUnlocked(userInfo.id)) {
      storageManagerInternal.prepareAppDataAfterInstall(
          androidPackage.getPackageName(),
          UserHandle.getUid(userInfo.id, UserHandle.getAppId(androidPackage.getUid())));
    }
  }
  ...
  public final CompletableFuture prepareAppData(
      Installer.Batch batch, AndroidPackage androidPackage, int i, int i2, int i3) {
  ...
  public final void prepareAppDataAndMigrate(
      Installer.Batch batch,
    ...
    prepareAppData(batch, androidPackage, -1, i, i2)
        .thenRun(
            new Runnable() { // from class:
              // com.android.server.pm.AppDataHelper$$ExternalSyntheticLambda2
              @Override // java.lang.Runnable
              public final void run() {
                AppDataHelper.this.lambda$prepareAppDataAndMigrate$1(
                    z, packageState, androidPackage, i, i2);
              }
            });
  }

  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ void lambda$prepareAppDataAndMigrate$1(
      boolean z, PackageState packageState, AndroidPackage androidPackage, int i, int i2) {
  ...
  public final CompletableFuture prepareAppDataLeaf(
      Installer.Batch batch,
      final AndroidPackage androidPackage,
      int i,
      final int i2,
    ...
    final CreateAppDataArgs buildCreateAppDataArgs =
        Installer.buildCreateAppDataArgs(
            ...
            !androidPackage.getUsesSdkLibraries().isEmpty());
    buildCreateAppDataArgs.previousAppId = i;
    return batch
        .createAppData(buildCreateAppDataArgs)
        .whenComplete(
            new BiConsumer() { // from class:
              // com.android.server.pm.AppDataHelper$$ExternalSyntheticLambda1
              @Override // java.util.function.BiConsumer
              public final void accept(Object obj, Object obj2) {
                AppDataHelper.this.lambda$prepareAppDataLeaf$2(
                    packageName,
                    androidPackage,
                    i2,
                    i3,
                    buildCreateAppDataArgs,
                    packageLPr,
                    (Long) obj,
                    (Throwable) obj2);
              }
  ...
  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ void lambda$prepareAppDataLeaf$2(
      String str,
      AndroidPackage androidPackage,
      int i,
      int i2,
    ...
    if (th != null) {
      PackageManagerServiceUtils.logCriticalInfo(
          5, "Failed to create app data for " + str + ", but trying to recover: " + th);
    ...
    if (!DexOptHelper.useArtService()
        && (this.mPm.isDeviceUpgrading() || this.mPm.isFirstBoot() || i != 0)) {
      try {
  ...
  public void prepareAppDataContentsLIF(
      AndroidPackage androidPackage, PackageStateInternal packageStateInternal, int i, int i2) {
    if (androidPackage == null) {
      Slog.wtf("PackageManager", "Package was null!", new Throwable());
    } else {
  ...
  public final void prepareAppDataContentsLeafLIF(
      AndroidPackage androidPackage, PackageStateInternal packageStateInternal, int i, int i2) {
    String volumeUuid = androidPackage.getVolumeUuid();
    ...
    if ((i2 & 2) != 0) {
      String rawPrimaryCpuAbi =
          packageStateInternal == null
              ? AndroidPackageUtils.getRawPrimaryCpuAbi(androidPackage)
  ...
  public final boolean maybeMigrateAppDataLIF(
      PackageState packageState, AndroidPackage androidPackage, int i) {
    if (!packageState.isSystem() || StorageManager.isFileEncrypted()) {
      return false;
    }
    try {
      this.mInstaller.migrateAppData(
          androidPackage.getVolumeUuid(),
          androidPackage.getPackageName(),
          i,
          androidPackage.isDefaultToDeviceProtectedStorage() ? 1 : 2);
    } catch (Installer.InstallerException e) {
      PackageManagerServiceUtils.logCriticalInfo(
          5, "Failed to migrate " + androidPackage.getPackageName() + ": " + e.getMessage());
    }
    return true;
  }
      ...
      if (packageStateInternal.getPkg() == null) {
        ...
        if (z2 && !packageStateInternal.getPkg().isCoreApp()) {
          ...
          if (packageStateInternal.getUserStateOrDefault(i).isInstalled()) {
            ...
            prepareAppDataAndMigrate(
                batch, packageStateInternal, packageStateInternal.getPkg(), i, i2, z);
            i10++;
          } else {
            arrayList2 = arrayList6;
          }
  ...
  public final void assertPackageStorageValid(Computer computer, String str, String str2, int i) {
    ...
    if (!packageStateInternal.getUserStateOrDefault(i).isInstalled()) {
      throw PackageManagerException.ofInternalError(
          "Package " + str2 + " not installed for user " + i, -9);
    }
    if (packageStateInternal.getPkg() == null
        || shouldHaveAppStorage(packageStateInternal.getPkg())) {
      return;
    }
    throw PackageManagerException.ofInternalError(
        "Package " + str2 + " shouldn't have storage", -10);
  }

  public Future fixAppsDataOnBoot() {
    final int i = StorageManager.isFileEncrypted() ? 1 : 3;
    final List reconcileAppsDataLI =
        reconcileAppsDataLI(StorageManager.UUID_PRIVATE_INTERNAL, 0, i, true, true);
    return SystemServerInitThreadPool.submit(
        new Runnable() { // from class:
          // com.android.server.pm.AppDataHelper$$ExternalSyntheticLambda3
          @Override // java.lang.Runnable
          public final void run() {
  ...
  public /* synthetic */ void lambda$fixAppsDataOnBoot$3(List list, int i) {
    ...
    while (it.hasNext()) {
      PackageStateInternal packageStateInternal =
          this.mPm.snapshotComputer().getPackageStateInternal((String) it.next());
      if (packageStateInternal != null
          && packageStateInternal.getUserStateOrDefault(0).isInstalled()) {
        prepareAppDataAndMigrate(
            batch, packageStateInternal, packageStateInternal.getPkg(), 0, i, true);
        i2++;
      }
    }
    synchronized (this.mPm.mInstallLock) {
  ...
  public final void clearAppDataLeafLIF(AndroidPackage androidPackage, int i, int i2) {
    PackageStateInternal packageStateInternal =
        this.mPm.snapshotComputer().getPackageStateInternal(androidPackage.getPackageName());
    for (int i3 : this.mPm.resolveUserIds(i)) {
      try {
        this.mInstaller.clearAppData(
            ...
            i2,
            packageStateInternal != null
                ? packageStateInternal.getUserStateOrDefault(i3).getCeDataInode()
                : 0L);
      } catch (Installer.InstallerException e) {
  ...
  public void destroyAppDataLeafLIF(AndroidPackage androidPackage, int i, int i2) {
    ...
    for (int i3 : this.mPm.resolveUserIds(i)) {
      try {
        this.mInstaller.destroyAppData(
            androidPackage.getVolumeUuid(),
            androidPackage.getPackageName(),
            i3,
            i2,
      ...
      this.mPm
          .getDynamicCodeLogger()
          .notifyPackageDataDestroyed(androidPackage.getPackageName(), i);
    }
  }
      ...
      try {
        try {
          DexOptHelper.getArtManagerLocal()
              .clearAppProfiles(withFilteredSnapshot, androidPackage.getPackageName());
