...
public final class FrozenPackageInterceptor {
  ...
  public final ActivityInterceptorCallback mActivityInterceptorCallback =
      new ActivityInterceptorCallback() { // from class:
        ...
        public ActivityInterceptorCallback.ActivityInterceptResult onInterceptActivityLaunch(
          ...
          if (FrozenPackageInterceptor.this.mPMInternal.isPackageFrozen(
                  str, activityInterceptorInfo.getCallingUid(), activityInterceptorInfo.getUserId())
              && FrozenPackageInterceptor.this.isPackageBeingInstalled(str)) {
            return new ActivityInterceptorCallback.ActivityInterceptResult(
                FrozenAppActivity.createIntent(activityInterceptorInfo.getUserId(), str),
  ...
  public void registerListeners() {
    ((ActivityTaskManagerInternal) LocalServices.getService(ActivityTaskManagerInternal.class))
        .registerActivityStartInterceptor(6, this.mActivityInterceptorCallback);
  }

  public final boolean isPackageBeingInstalled(String str) {
    for (PackageInstaller.SessionInfo sessionInfo :
        this.mContext.getPackageManager().getPackageInstaller().getAllSessions()) {
