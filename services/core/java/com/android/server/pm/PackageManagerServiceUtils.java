...
public abstract class PackageManagerServiceUtils {
  ...
  public static final Predicate REMOVE_IF_APEX_PKG = new Predicate() { // from class:
        // com.android.server.pm.PackageManagerServiceUtils$$ExternalSyntheticLambda0
  ...
  public static final Predicate REMOVE_IF_NULL_PKG = new Predicate() { // from class:
        // com.android.server.pm.PackageManagerServiceUtils$$ExternalSyntheticLambda1
        @Override // java.util.function.Predicate
        public final boolean test(Object obj) {
  ...
  public static final ThreadLocal DISABLE_ENFORCE_INTENTS_TO_MATCH_INTENT_FILTERS =
      ThreadLocal.withInitial(
          new Supplier() { // from class:
            // com.android.server.pm.PackageManagerServiceUtils$$ExternalSyntheticLambda2
  ...
  public static boolean isUnusedSinceTimeInMillis(
      long j, long j2, long j3, PackageDexUsage.PackageUseInfo packageUseInfo, long j4, long j5) {
    boolean z = false;
    if (j2 - j < j3) {
      return false;
    }
  ...
  public static long getLastModifiedTime(AndroidPackage androidPackage) {
    ...
    for (int length = androidPackage.getSplitCodePaths().length - 1; length >= 0; length--) {
      lastModified =
          Math.max(
              lastModified, new File(androidPackage.getSplitCodePaths()[length]).lastModified());
    }
    return lastModified;
  }
  ...
  public static void logCriticalInfo(int i, String str) {
    ...
    try {
      ...
      FastPrintWriter fastPrintWriter =
          new FastPrintWriter(new FileOutputStream(settingsProblemFile, true));
      fastPrintWriter.println(
          new SimpleDateFormat().format(new Date(System.currentTimeMillis())) + ": " + str);
  ...
  public static void enforceShellRestriction(
      UserManagerInternal userManagerInternal, String str, int i, int i2) {
    if (i == 2000) {
      ...
      if (i2 < 0) {
        Slog.e(
            "PackageManager",
            "Unable to check shell permission for user " + i2 + "\n\t" + Debug.getCallers(3));
      }
    }
  }
  ...
  public static boolean comparePackageSignatures(
      PackageSetting packageSetting, Signature[] signatureArr) {
    SigningDetails signingDetails = packageSetting.getSigningDetails();
    return signingDetails == SigningDetails.UNKNOWN
        || compareSignatures(signingDetails.getSignatures(), signatureArr) == 0;
  }

  public static boolean matchSignaturesCompat(
      String str, PackageSignatures packageSignatures, SigningDetails signingDetails) {
    ArraySet arraySet = new ArraySet();
    ...
    if (signingDetails.hasPastSigningCertificates()) {
      logCriticalInfo(
          ...
          "Existing package "
              + str
              + " has flattened signing certificate chain. Unable to install newer version with"
              + " rotated signing certificate.");
    }
    return false;
  }

  public static boolean matchSignaturesRecover(
      String str, SigningDetails signingDetails, SigningDetails signingDetails2, int i) {
    String message;
    try {
  ...
  public static boolean matchSignatureInSystem(
      String str, SigningDetails signingDetails, PackageSetting packageSetting) {
    if (signingDetails.checkCapability(packageSetting.getSigningDetails(), 1)
        || packageSetting.getSigningDetails().checkCapability(signingDetails, 8)) {
      return true;
    }
  ...
  public static boolean verifySignatures(
    ...
    if (packageSetting.getSigningDetails().getSignatures() != null) {
      boolean z6 =
          signingDetails.checkCapability(packageSetting.getSigningDetails(), 1)
              || packageSetting.getSigningDetails().checkCapability(signingDetails, 8);
      if (z6 || !z) {
        z4 = false;
      } else {
      ...
      if (!z6 && z2) {
        z6 =
            matchSignaturesRecover(
                    packageName, packageSetting.getSigningDetails(), signingDetails, 1)
                || matchSignaturesRecover(
                    packageName, signingDetails, packageSetting.getSigningDetails(), 8);
      }
      if (!z6 && isApkVerificationForced(packageSetting2)) {
        z6 =
            matchSignatureInSystem(
                packageName, packageSetting.getSigningDetails(), packageSetting2);
      }
      if (!z6 && z3) {
    ...
    if (sharedUserSetting != null
      ...
      boolean canJoinSharedUserId =
          canJoinSharedUserId(
              packageName,
              signingDetails,
              sharedUserSetting,
              packageSetting.getSigningDetails().getSignatures() != null ? 1 : 0);
      if (!canJoinSharedUserId && z) {
        canJoinSharedUserId =
            matchSignaturesCompat(packageName, sharedUserSetting.signatures, signingDetails);
      }
      if (!canJoinSharedUserId && z2) {
        if (!matchSignaturesRecover(
                packageName, sharedUserSetting.signatures.mSigningDetails, signingDetails, 2)
            && !matchSignaturesRecover(
                packageName, signingDetails, sharedUserSetting.signatures.mSigningDetails, 2)) {
          z5 = false;
        }
        z4 |= z5;
      ...
      if (!signingDetails.hasCommonAncestor(sharedUserSetting.signatures.mSigningDetails)) {
        throw new PackageManagerException(
            ...
            "Package "
                + packageName
                + " has a signing lineage that diverges from the lineage of the sharedUserId");
      }
    }
    return z4;
  }

  public static boolean canJoinSharedUserId(
      String str, SigningDetails signingDetails, SharedUserSetting sharedUserSetting, int i) {
    SigningDetails signingDetails2 = sharedUserSetting.getSigningDetails();
    boolean z =
        signingDetails.checkCapability(signingDetails2, 2)
            || signingDetails2.checkCapability(signingDetails, 2);
    if (z && i != 0) {
      return true;
    }
    ...
    if (signingDetails.hasPastSigningCertificates()) {
      ...
      while (it.hasNext()) {
        ...
        if (signingDetails.hasAncestor(signingDetails3)
            && !signingDetails.checkCapability(signingDetails3, 2)) {
          Slog.d(
              "PackageManager",
              "Package "
                  + str
                  + " revoked the sharedUserId capability from the signing key used to sign "
  ...
  public static int extractNativeBinaries(File file, String str) {
    ...
    try {
      int copyNativeBinariesWithOverride =
          NativeLibraryHelper.copyNativeBinariesWithOverride(create, file2, (String) null, false);
  ...
  public static int decompressFile(File file, File file2) {
    if (PackageManagerService.DEBUG_COMPRESSION) {
      Slog.i(
          "PackageManager",
          "Decompress file; src: " + file.getAbsolutePath() + ", dst: " + file2.getAbsolutePath());
    }
  ...
  public static File[] getCompressedFiles(String str) {
    ...
    File[] listFiles =
        new File(parentFile, name.substring(0, lastIndexOf))
            .listFiles(
                new FilenameFilter() { // from class:
                  // com.android.server.pm.PackageManagerServiceUtils.1
  ...
  public static PackageInfoLite getMinimalPackageInfo(
      Context context, PackageLite packageLite, String str, int i, String str2) {
    ...
    try {
      ...
      try {
        int resolveInstallLocation =
            InstallLocationUtils.resolveInstallLocation(context, sessionParams);
    ...
    try {
      ParseResult parsePackageLite =
          ApkLiteParseUtils.parsePackageLite(
              ParseTypeImpl.forDefaultParsing().reset(), new File(str), 0);
      if (parsePackageLite.isError()) {
        throw new PackageManagerException(
            parsePackageLite.getErrorCode(),
            parsePackageLite.getErrorMessage(),
            parsePackageLite.getException());
      }
      return InstallLocationUtils.calculateInstalledSize(
          (PackageLite) parsePackageLite.getResult(), str2);
  ...
  public static int copyPackage(String str, File file) {
    ...
    try {
      ...
      for (int i = 0; i < packageLite.getSplitNames().length; i++) {
        copyFile(
            packageLite.getSplitApkPaths()[i],
            file,
            "split_" + packageLite.getSplitNames()[i] + ".apk");
      }
      return 1;
  ...
  public static void applyEnforceIntentFilterMatching(
    ...
    for (int size = list.size() - 1; size >= 0; size--) {
      ...
      if (ActivityManager.checkComponentPermission(
              null, i, componentInfo.applicationInfo.uid, false)
          != 0) {
        ...
        if (service != null && !service.getIntents().isEmpty()) {
          boolean isChangeEnabledInternal =
              platformCompat.isChangeEnabledInternal(161252188L, componentInfo.applicationInfo);
          ...
          while (true) {
            ...
            if (IntentResolver.intentMatchesFilter(
                ((ParsedIntentInfo) service.getIntents().get(i2)).getIntentFilter(), intent, str)) {
              z2 = true;
              break;
            }
            i2++;
          ...
          if (!z2) {
            ...
            if (isChangeEnabledInternal) {
              Slog.w(
                  "PackageManager", "Intent does not match component's intent filter: " + intent);
  ...
  public static boolean hasAnyDomainApproval(
      ...
      long j,
      int i) {
    return domainVerificationManagerInternal.approvalLevelForDomain(
            packageStateInternal, intent, j, i)
        > 0;
  }
  ...
  public static boolean isAdoptedShell(int i, Context context) {
    return i != 1000
        && context.checkCallingOrSelfPermission("com.android.permission.USE_SYSTEM_DATA_LOADERS")
            == 0;
  }
  ...
  public static File preparePackageParserCache(boolean z, boolean z2, String str) {
    ...
    if (createDir == null) {
      Slog.wtf(
          "PackageManager",
          "Cache directory cannot be created - wiping base dir " + packageCacheDirectory);
    ...
    if (createDir.lastModified()
        >= new File(Environment.getRootDirectory(), "framework").lastModified()) {
      return createDir;
    }
  ...
  public static void checkDowngrade(
      AndroidPackage androidPackage, PackageInfoLite packageInfoLite) {
    if (packageInfoLite.getLongVersionCode() < androidPackage.getLongVersionCode()) {
      throw new PackageManagerException(
          -25,
          "Update version code "
              + packageInfoLite.versionCode
    ...
    if (packageInfoLite.baseRevisionCode < androidPackage.getBaseRevisionCode()) {
      throw new PackageManagerException(
          -25,
          "Update base revision code "
              + packageInfoLite.baseRevisionCode
              + " is older than current "
    ...
    while (true) {
      ...
      if (indexOf != -1
          && packageInfoLite.splitRevisionCodes[i]
              < androidPackage.getSplitRevisionCodes()[indexOf]) {
        throw new PackageManagerException(
            -25,
            "Update split "
                + str
