...
public final class InstallRequest {
  ...
  public InstallRequest(
      int i, int i2, AndroidPackage androidPackage, int[] iArr, Runnable runnable) {
    this.mAppId = -1;
    this.mNeedToMove = false;
    this.mUserId = i;
  ...
  public InstallRequest(
      ParsedPackage parsedPackage, int i, int i2, UserHandle userHandle, ScanResult scanResult) {
    this.mAppId = -1;
    this.mNeedToMove = false;
    if (userHandle != null) {
  ...
  public boolean isUpdate() {
    PackageRemovedInfo packageRemovedInfo = this.mRemovedInfo;
    return (packageRemovedInfo == null || packageRemovedInfo.mRemovedPackage == null)
        ? false
        : true;
  }
  ...
  public PackageSetting getRealPackageSetting() {
    PackageSetting scanRequestPackageSetting =
        isExistingSettingCopied() ? getScanRequestPackageSetting() : getScannedPackageSetting();
    return scanRequestPackageSetting == null
        ? getScannedPackageSetting()
        : scanRequestPackageSetting;
  }
