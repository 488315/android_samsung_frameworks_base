...
public class ShortcutLauncher extends ShortcutPackageItem {
    ...
    if (size == 0) {
      ...
      for (int i2 = 0; i2 < size; i2++) {
        ...
        if (findShortcutById != null
            && (findShortcutById.isDynamic()
                || findShortcutById.isLongLived()
                || findShortcutById.isManifestShortcut()
                || ((arraySet != null && arraySet.contains(str2)) || z))) {
          arraySet2.add(str2);
        }
      }
  ...
  public boolean hasPinned(ShortcutInfo shortcutInfo) {
    ArraySet pinnedShortcutIds =
        getPinnedShortcutIds(shortcutInfo.getPackage(), shortcutInfo.getUserId());
  ...
  public void ensurePackageInfo() {
    PackageInfo packageInfoWithSignatures =
        this.mShortcutUser.mService.getPackageInfoWithSignatures(
  ...
  public void saveToXml(TypedXmlSerializer typedXmlSerializer, boolean z) {
    int size;
    if ((this.mShortcutUser.mService.isSmartSwitchBackupAllowed()
            || !z
            || getPackageInfo().isBackupAllowed())
        && (size = this.mPinnedShortcuts.size()) != 0) {
  ...
  public static ShortcutLauncher loadFromXml(
    ...
    ShortcutLauncher shortcutLauncher =
        new ShortcutLauncher(
            shortcutUser,
            i,
            ShortcutService.parseStringAttribute(typedXmlPullParser, "package-name"),
            z ? i : ShortcutService.parseIntAttribute(typedXmlPullParser, "launcher-user", i));
        ...
        if (depth2 == depth + 1) {
          ...
          if (name.equals("package-info")) {
            ...
            int parseIntAttribute =
                z ? i : ShortcutService.parseIntAttribute(typedXmlPullParser, "package-user", i);
            ArraySet arraySet2 = new ArraySet();
            shortcutLauncher.mPinnedShortcuts.put(
                UserPackage.of(parseIntAttribute, parseStringAttribute), arraySet2);
            arraySet = arraySet2;
          }
        }
        if (depth2 == depth + 2) {
  ...
  public File getShortcutPackageItemFile() {
    ...
    return new File(
        new File(shortcutUser.mService.injectUserDataPath(shortcutUser.getUserId()), "launchers"),
        getPackageName() + getPackageUserId() + ".xml");
  }
}
