...
public final class Permission {
  ...
  public Permission(
      PermissionInfo permissionInfo, int i, boolean z, int i2, int[] iArr, boolean z2) {
  ...
  public boolean isPermission(ParsedPermission parsedPermission) {
    ...
    return permissionInfo != null
        && Objects.equals(permissionInfo.packageName, parsedPermission.getPackageName())
        && Objects.equals(this.mPermissionInfo.name, parsedPermission.getName());
  }
  ...
  public boolean isSetup() {
    return (this.mPermissionInfo.protectionLevel & IInstalld.FLAG_FREE_CACHE_DEFY_TARGET_FREE_BYTES)
        != 0;
  }
  ...
  public boolean addToTree(int i, PermissionInfo permissionInfo, Permission permission) {
    ...
    boolean z =
        (permissionInfo2.protectionLevel == i
                ...
                && comparePermissionInfos(this.mPermissionInfo, permissionInfo))
            ? false
            : true;
  ...
  public void updateDynamicPermission(Collection collection) {
    ...
    if (this.mType != 2
        || (findPermissionTree = findPermissionTree(collection, this.mPermissionInfo.name))
            == null) {
      return;
    }
  ...
  public static boolean isOverridingSystemPermission(
      Permission permission,
      PermissionInfo permissionInfo,
      PackageManagerInternal packageManagerInternal) {
    PackageStateInternal packageStateInternal;
    if (permission == null
        || Objects.equals(permission.mPermissionInfo.packageName, permissionInfo.packageName)
        || !permission.mReconciled
  ...
  */
  public static Permission createOrUpdate(
      Permission permission,
      PermissionInfo permissionInfo,
      ...
      if (permission.mType == 1 && !permission.mReconciled) {
        ...
        Slog.w(
            ...
            "New decl "
                ...
                + " of permission  "
                + permissionInfo.name
                + " is system; overriding "
                + permission.mPermissionInfo.packageName);
        permission = null;
        z2 = true;
        boolean z3 =
            (permission != null || permission.mType == 1 || permission.isInternal()) ? false : true;
        boolean z4 =
            (permission != null || permission.mType == 1 || permission.isRuntime()) ? false : true;
        if (permission == null) {
        ...
        if (!permission.mReconciled) {
          ...
          if (str == null || str.equals(permissionInfo.packageName)) {
            ...
            if (findPermissionTree == null
                || findPermissionTree.mPermissionInfo.packageName.equals(
              ...
              Slog.w(
                  ...
                  "Permission "
                      ...
                      + permissionInfo.packageName
                      + " ignored: base tree "
                      + findPermissionTree.mPermissionInfo.name
                      + " is from package "
                      + findPermissionTree.mPermissionInfo.packageName);
            }
          } else {
            Slog.w(
                "Permission",
  ...
  public static Permission enforcePermissionTree(Collection collection, String str, int i) {
    Permission findPermissionTree;
    if (str != null
        && (findPermissionTree = findPermissionTree(collection, str)) != null
        && findPermissionTree.getUid() == UserHandle.getAppId(i)) {
      return findPermissionTree;
    }
    throw new SecurityException(
        "Calling uid " + i + " is not allowed to add to or remove from the permission tree");
  }
  ...
  public static boolean comparePermissionInfos(
      PermissionInfo permissionInfo, PermissionInfo permissionInfo2) {
    return permissionInfo.icon == permissionInfo2.icon
        && permissionInfo.logo == permissionInfo2.logo
        && permissionInfo.protectionLevel == permissionInfo2.protectionLevel
        && Objects.equals(permissionInfo.name, permissionInfo2.name)
        && Objects.equals(permissionInfo.nonLocalizedLabel, permissionInfo2.nonLocalizedLabel)
        && Objects.equals(permissionInfo.packageName, permissionInfo2.packageName);
  }
}
