...
public abstract class PackageInfoUtils {
  ...
  public static PackageInfo generate(
  ...
  public static PackageInfo generateWithComponents(
      AndroidPackage androidPackage,
      int[] iArr,
      long j,
      long j2,
      ...
      if (size9 > 0) {
        ...
        for (int i5 = 0; i5 < size9; i5++) {
          ParsedPermission parsedPermission =
              (ParsedPermission) androidPackage.getPermissions().get(i5);
      ...
      if (size10 > 0) {
        ...
        for (int i6 = 0; i6 < size10; i6++) {
          ParsedUsesPermission parsedUsesPermission =
              (ParsedUsesPermission) usesPermissions.get(i6);
          ...
          if (androidPackage
              ...
              .contains(packageInfo.requestedPermissions[i6])) {
    ...
    if ((2147483648L & j) != 0) {
      ...
      if (size11 > 0) {
        ...
        for (int i7 = 0; i7 < size11; i7++) {
          ParsedAttribution parsedAttribution =
              (ParsedAttribution) androidPackage.getAttributions().get(i7);
          if (parsedAttribution != null) {
            packageInfo.attributions[i7] =
                new Attribution(parsedAttribution.getTag(), parsedAttribution.getLabel());
          }
        }
      }
    ...
    if ((64 & j) != 0) {
      if (signingDetails.hasPastSigningCertificates()) {
        packageInfo.signatures = new Signature[] {signingDetails.getPastSigningCertificates()[0]};
    ...
    if ((1 & j) != 0 && (size5 = androidPackage.getActivities().size()) > 0) {
      ...
      while (i9 < size5) {
        ...
        if (!ComponentParseUtils.isMatch(
                packageUserStateInternal,
                packageStateInternal.isSystem(),
                androidPackage.isEnabled(),
                parsedActivity,
                j)
            || PackageManager.APP_DETAILS_ACTIVITY_CLASS_NAME.equals(parsedActivity.getName())) {
          i2 = i9;
    ...
    if ((16 & j) != 0 && (size = androidPackage.getInstrumentations().size()) > 0) {
      ...
      for (int i16 = 0; i16 < size; i16++) {
        packageInfo.instrumentation[i16] =
            generateInstrumentationInfo(
                (ParsedInstrumentation) androidPackage.getInstrumentations().get(i16),
                androidPackage,
                j,
                packageUserStateInternal,
                i,
  ...
  public static void updateApplicationInfo(
    ...
    applicationInfo.flags |=
        flag(packageUserState.isStopped(), 2097152)
            | flag(packageUserState.isInstalled(), 8388608)
            | flag(packageUserState.isSuspended(), 1073741824);
    boolean z = true;
    applicationInfo.privateFlags |=
        flag(packageUserState.isInstantApp(), 128)
            | flag(packageUserState.isVirtualPreload(), 65536)
            | flag(packageUserState.isHidden(), 1);
  ...
  public static ApplicationInfo generateDelegateApplicationInfo(
      ApplicationInfo applicationInfo, long j, PackageUserState packageUserState, int i) {
    int i2;
    if (applicationInfo == null
        || !checkUseInstalledOrHidden(j, packageUserState, applicationInfo)) {
      return null;
    }
  ...
  public static ApplicationInfo generateApplicationInfo(
    ...
    if (androidPackage == null
        || !checkUseInstalledOrHidden(
            androidPackage, packageStateInternal, packageUserStateInternal, j)
        || !AndroidPackageUtils.isMatchForSystemOnly(packageStateInternal, j)) {
      return null;
    }
    ApplicationInfo generateAppInfoWithoutState =
    ...
    generateAppInfoWithoutState.sharedLibraryFiles =
        usesLibraryFiles.isEmpty() ? null : (String[]) usesLibraryFiles.toArray(new String[0]);
    ...
    generateAppInfoWithoutState.privateFlagsExt =
        i5 | appInfoPrivateFlagsExt(i5, packageStateInternal);
  ...
  public static ActivityInfo generateActivityInfo(
    ...
    return generateActivityInfo(
        androidPackage, parsedActivity, j, packageUserStateInternal, null, i, packageStateInternal);
  }
  ...
  public static ActivityInfo generateDelegateActivityInfo(
      ActivityInfo activityInfo, long j, PackageUserState packageUserState, int i) {
    if (activityInfo == null
        || !checkUseInstalledOrHidden(j, packageUserState, activityInfo.applicationInfo)) {
      return null;
    }
    ActivityInfo activityInfo2 = new ActivityInfo(activityInfo);
    activityInfo2.applicationInfo =
        generateDelegateApplicationInfo(activityInfo2.applicationInfo, j, packageUserState, i);
  ...
  public static ServiceInfo generateServiceInfo(
    ...
    return generateServiceInfo(
        androidPackage, parsedService, j, packageUserStateInternal, null, i, packageStateInternal);
  }
  ...
  public static ProviderInfo generateProviderInfo(
    ...
    providerInfo.uriPermissionPatterns =
        (PatternMatcher[]) parsedProvider.getUriPermissionPatterns().toArray(new PatternMatcher[0]);
    providerInfo.pathPermissions =
        (PathPermission[]) parsedProvider.getPathPermissions().toArray(new PathPermission[0]);
    if ((2048 & j) == 0) {
  ...
  public static InstrumentationInfo generateInstrumentationInfo(
    ...
    instrumentationInfo.splitSourceDirs =
        androidPackage.getSplitCodePaths().length == 0 ? null : androidPackage.getSplitCodePaths();
    ...
    assignFieldsPackageItemInfoParsedComponent(
        instrumentationInfo, parsedInstrumentation, packageStateInternal, i);
    if ((j & 128) == 0) {
  ...
  public static boolean checkUseInstalledOrHidden(
      long j, PackageUserState packageUserState, ApplicationInfo applicationInfo) {
    long j2 = 536870912 & j;
    if (j2 == 0
  ...
  public static void assignFieldsComponentInfoParsedMainComponent(
      ComponentInfo componentInfo, ParsedMainComponent parsedMainComponent) {
  ...
  public static void assignFieldsPackageItemInfoParsedComponent(
    ...
    Pair nonLocalizedLabelAndIcon =
        ParsedComponentStateUtils.getNonLocalizedLabelAndIcon(
            parsedComponent, packageStateInternal, i);
  ...
  public static int appInfoFlags(
    ...
    return appInfoFlags(
        flag(androidPackage.isFactoryTest(), 16)
            | flag(androidPackage.isExternalStorage(), 262144)
            | flag(androidPackage.isHardwareAccelerated(), 536870912)
            | flag(androidPackage.isBackupAllowed(), 32768)
            | flag(androidPackage.isKillAfterRestoreAllowed(), 65536)
            | flag(
                androidPackage.isRestoreAnyVersion(),
                IInstalld.FLAG_CLEAR_APP_DATA_KEEP_ART_PROFILES)
            | flag(androidPackage.isFullBackupOnly(), 67108864)
            | flag(androidPackage.isPersistent(), 8)
            | flag(androidPackage.isDebuggable(), 2)
            | flag(androidPackage.isVmSafeMode(), 16384)
            | flag(androidPackage.isDeclaredHavingCode(), 4)
            | flag(androidPackage.isTaskReparentingAllowed(), 32)
            | flag(androidPackage.isClearUserDataAllowed(), 64)
            | flag(androidPackage.isLargeHeap(), 1048576)
            | flag(androidPackage.isCleartextTrafficAllowed(), 134217728)
            | flag(androidPackage.isRtlSupported(), 4194304)
            | flag(androidPackage.isTestOnly(), 256)
            | flag(androidPackage.isMultiArch(), Integer.MIN_VALUE)
            | flag(androidPackage.isExtractNativeLibrariesRequested(), 268435456)
            | flag(androidPackage.isGame(), 33554432)
            | flag(androidPackage.isSmallScreensSupported(), 512)
            | flag(androidPackage.isNormalScreensSupported(), 1024)
            | flag(
                androidPackage.isLargeScreensSupported(),
                IInstalld.FLAG_FREE_CACHE_DEFY_TARGET_FREE_BYTES)
            | flag(androidPackage.isExtraLargeScreensSupported(), 524288)
            | flag(androidPackage.isResizeable(), IInstalld.FLAG_USE_QUOTA)
            | flag(androidPackage.isAnyDensity(), IInstalld.FLAG_FORCE)
            | flag(AndroidPackageUtils.isSystem(androidPackage), 1),
        packageStateInternal);
  }

  public static int appInfoFlags(int i, PackageStateInternal packageStateInternal) {
    return packageStateInternal != null
        ? i | flag(packageStateInternal.isUpdatedSystemApp(), 128)
        : i;
  }

  public static int appInfoPrivateFlags(
      AndroidPackage androidPackage, PackageStateInternal packageStateInternal) {
    int flag =
        flag(androidPackage.isStaticSharedLibrary(), 16384)
            | flag(androidPackage.isResourceOverlay(), 268435456)
            | flag(androidPackage.isIsolatedSplitLoading(), 32768)
            | flag(androidPackage.isHasDomainUrls(), 16)
            | flag(androidPackage.isProfileableByShell(), 8388608)
            | flag(androidPackage.isBackupInForeground(), IInstalld.FLAG_FORCE)
            | flag(androidPackage.isUseEmbeddedDex(), 33554432)
            | flag(androidPackage.isDefaultToDeviceProtectedStorage(), 32)
            | flag(androidPackage.isDirectBootAware(), 64)
            | flag(androidPackage.isPartiallyDirectBootAware(), 256)
            | flag(androidPackage.isClearUserDataOnFailedRestoreAllowed(), 67108864)
            | flag(androidPackage.isAllowAudioPlaybackCapture(), 134217728)
            | flag(androidPackage.isRequestLegacyExternalStorage(), 536870912)
            | flag(androidPackage.isNonSdkApiRequested(), 4194304)
            | flag(androidPackage.isUserDataFragile(), 16777216)
            | flag(androidPackage.isSaveStateDisallowed(), 2)
            | flag(androidPackage.isResizeableActivityViaSdkVersion(), IInstalld.FLAG_USE_QUOTA)
            | flag(androidPackage.isAllowNativeHeapPointerTagging(), Integer.MIN_VALUE)
            | flag(AndroidPackageUtils.isSystemExt(androidPackage), 2097152)
            | flag(AndroidPackageUtils.isPrivileged(androidPackage), 8)
            | flag(
                AndroidPackageUtils.isOem(androidPackage),
                IInstalld.FLAG_CLEAR_APP_DATA_KEEP_ART_PROFILES)
            | flag(AndroidPackageUtils.isVendor(androidPackage), 262144)
            | flag(AndroidPackageUtils.isProduct(androidPackage), 524288)
            | flag(AndroidPackageUtils.isOdm(androidPackage), 1073741824)
            | flag(androidPackage.isSignedWithPlatformKey(), 1048576);
    Boolean resizeableActivity = androidPackage.getResizeableActivity();
    if (resizeableActivity != null) {
      flag =
          resizeableActivity.booleanValue()
              ? flag | 1024
              : flag | IInstalld.FLAG_FREE_CACHE_DEFY_TARGET_FREE_BYTES;
    }
    return appInfoPrivateFlags(flag, packageStateInternal);
  }

  public static int appInfoPrivateFlagsExt(
      AndroidPackage androidPackage, PackageStateInternal packageStateInternal) {
    boolean contains =
        SystemConfig.getInstance()
            .getHiddenApiWhitelistedApps()
            .contains(androidPackage.getPackageName());
    return appInfoPrivateFlagsExt(
        flag(androidPackage.isOnBackInvokedCallbackEnabled(), 8)
            | flag(androidPackage.isProfileable(), 1)
            | flag(androidPackage.hasRequestForegroundServiceExemption(), 2)
            | flag(androidPackage.isAttributionsUserVisible(), 4)
            | flag(contains, 16),
        packageStateInternal);
  }
  ...
  public static void initForUser(
      ApplicationInfo applicationInfo, AndroidPackage androidPackage, int i) {
    ...
    if (i == 0) {
      applicationInfo.credentialProtectedDataDir =
          packageImpl.getBaseAppDataCredentialProtectedDirForSystemUser() + packageName;
      ...
      StringBuilder replace =
          new StringBuilder(packageImpl.getBaseAppDataCredentialProtectedDirForSystemUser())
      ...
      StringBuilder replace2 =
          new StringBuilder(packageImpl.getBaseAppDataDeviceProtectedDirForSystemUser())
  ...
  public static void initForUser(
      InstrumentationInfo instrumentationInfo, AndroidPackage androidPackage, int i) {
    PackageImpl packageImpl = (PackageImpl) androidPackage;
    String packageName = androidPackage.getPackageName();
    if ("android".equals(packageName)) {
  ...
  public static File getDataDir(AndroidPackage androidPackage, int i) {
    ...
    if (androidPackage.isDefaultToDeviceProtectedStorage()) {
      return Environment.getDataUserDePackageDirectory(
          androidPackage.getVolumeUuid(), i, androidPackage.getPackageName());
    }
    return Environment.getDataUserCePackageDirectory(
        androidPackage.getVolumeUuid(), i, androidPackage.getPackageName());
  }
  ...
  public class CachedApplicationInfoGenerator {
    ...
    public ApplicationInfo generate(
        AndroidPackage androidPackage,
        long j,
        PackageUserStateInternal packageUserStateInternal,
        int i,
        PackageStateInternal packageStateInternal) {
      ApplicationInfo applicationInfo =
          (ApplicationInfo) this.mCache.get(androidPackage.getPackageName());
      if (applicationInfo != null) {
