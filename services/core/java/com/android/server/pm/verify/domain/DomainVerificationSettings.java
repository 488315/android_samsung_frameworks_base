...
public class DomainVerificationSettings {
  ...
  public void writeSettings(
    ...
    synchronized (this.mLock) {
      DomainVerificationPersistence.writeToXml(
          ...
          this.mPendingPkgStates,
          this.mRestoredPkgStates,
          i,
          function);
    }
  }
    ...
    synchronized (this.mLock) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        DomainVerificationPkgState domainVerificationPkgState2 =
            (DomainVerificationPkgState) domainVerificationStateMap.get(packageName);
      ...
      for (int i2 = 0; i2 < size2; i2++) {
        ...
        this.mRestoredPkgStates.put(
            domainVerificationPkgState3.getPackageName(), domainVerificationPkgState3);
      }
    }
  }
  ...
  public void mergePkgState(
    ...
    Set emptySet =
        pkg == null ? Collections.emptySet() : this.mCollector.collectValidAutoVerifyDomains(pkg);
    ArrayMap stateMap = domainVerificationPkgState.getStateMap();
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (emptySet.contains(str)
          && (((num = (Integer) stateMap.get(str)) == null || num.intValue() == 0)
    ...
    for (int i2 = 0; i2 < size2; i2++) {
      ...
      DomainVerificationInternalUserState domainVerificationInternalUserState =
          (DomainVerificationInternalUserState) userStates2.valueAt(i2);
      if (domainVerificationInternalUserState != null) {
        ...
        DomainVerificationInternalUserState domainVerificationInternalUserState2 =
            (DomainVerificationInternalUserState) userStates.get(keyAt);
        ...
        if (domainVerificationInternalUserState2 == null) {
          userStates.put(
              keyAt,
              new DomainVerificationInternalUserState(keyAt, enabledHosts, isLinkHandlingAllowed));
        } else {
          domainVerificationInternalUserState2
              .addHosts(enabledHosts)
              .setLinkHandlingAllowed(isLinkHandlingAllowed);
        }
      }
    }
  }
