...
public class DomainVerificationLegacySettings {
  ...
  public void writeSettings(TypedXmlSerializer typedXmlSerializer) {
    ...
    try {
      SettingsXml.WriteSection startSection =
          serializer.startSection("domain-verifications-legacy");
      try {
        synchronized (this.mLock) {
          ...
          for (int i = 0; i < size; i++) {
            ...
            if (userStates != null) {
              SettingsXml.WriteSection attribute =
                  serializer
                      .startSection("user-states")
                      .attribute("packageName", (String) this.mStates.keyAt(i));
              try {
                int size2 = userStates.size();
                for (int i2 = 0; i2 < size2; i2++) {
                  attribute
                      .startSection("user-state")
                      .attribute("userId", userStates.keyAt(i2))
                      .attribute(
                          LauncherConfigurationInternal.KEY_STATE_BOOLEAN, userStates.valueAt(i2))
                      .finish();
                }
                if (attribute != null) {
                  attribute.close();
  ...
  public final void readUserState(SettingsXml.ReadSection readSection, LegacyState legacyState) {
    legacyState.addUserState(
        readSection.getInt("userId"),
        readSection.getInt(LauncherConfigurationInternal.KEY_STATE_BOOLEAN));
  }
