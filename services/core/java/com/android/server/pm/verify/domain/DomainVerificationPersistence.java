...
public abstract class DomainVerificationPersistence {
  ...
  public static void writePackageStates(
      SettingsXml.WriteSection writeSection, Collection collection, int i, Function function) {
  ...
  public static DomainVerificationPkgState createPkgStateFromXml(
      SettingsXml.ReadSection readSection) {
  ...
  public static void readDomainStates(SettingsXml.ReadSection readSection, ArrayMap arrayMap) {
    ...
    while (children.moveToNext("domain")) {
      arrayMap.put(
          children.getString("name"),
          Integer.valueOf(children.getInt(LauncherConfigurationInternal.KEY_STATE_BOOLEAN, 0)));
    }
  }

  public static void writePkgStateToXml(
      SettingsXml.WriteSection writeSection,
    ...
    SettingsXml.WriteSection attribute =
        writeSection
            .startSection("package-state")
            .attribute("packageName", packageName)
            .attribute("id", domainVerificationPkgState.getId().toString())
            .attribute("hasAutoVerifyDomains", domainVerificationPkgState.isHasAutoVerifyDomains())
            .attribute("signature", str);
    try {
  ...
  public static void writeUserStates(
      SettingsXml.WriteSection writeSection, int i, SparseArray sparseArray) {
    ...
    try {
      if (i == -1) {
        for (int i2 = 0; i2 < size; i2++) {
          writeUserStateToXml(
              startSection, (DomainVerificationInternalUserState) sparseArray.valueAt(i2));
        }
      } else {
  ...
  public static void writeStateMap(SettingsXml.WriteSection writeSection, ArrayMap arrayMap) {
    ...
    SettingsXml.WriteSection startSection =
        writeSection.startSection(LauncherConfigurationInternal.KEY_STATE_BOOLEAN);
    try {
      int size = arrayMap.size();
      for (int i = 0; i < size; i++) {
        startSection
            .startSection("domain")
            .attribute("name", (String) arrayMap.keyAt(i))
            .attribute(
  ...
  public static void writeUserStateToXml(
    ...
    SettingsXml.WriteSection attribute =
        writeSection
            .startSection("user-state")
            .attribute("userId", domainVerificationInternalUserState.getUserId())
            .attribute(
                "allowLinkHandling", domainVerificationInternalUserState.isLinkHandlingAllowed());
    try {
      ...
      if (!enabledHosts.isEmpty()) {
        ...
        try {
          ...
          for (int i = 0; i < size; i++) {
            startSection
                .startSection("host")
                .attribute("name", (String) enabledHosts.valueAt(i))
                .finish();
          }
          if (startSection != null) {
            startSection.close();
