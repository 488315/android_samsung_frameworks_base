...
public final class PreferredActivityHelper {
  ...
  public final ResolveInfo findPreferredActivityNotLocked(
      Computer computer,
      Intent intent,
      String str,
      long j,
  ...
  public ResolveInfo findPreferredActivityNotLocked(
    ...
    if (Thread.holdsLock(this.mPm.mLock)) {
      Slog.wtf(
          "PackageManager",
          "Calling thread " + Thread.currentThread().getName() + " is holding mLock",
          new Throwable());
    }
    ...
    boolean z5 =
        Settings.Global.getInt(this.mPm.mContext.getContentResolver(), "device_provisioned", 0)
            == 1;
    synchronized (this.mPm.mLock) {
      findPreferredActivityInternal2 =
          this.mPm
              .snapshotComputer()
              .findPreferredActivityInternal2(intent, str, j, list, z, z2, z3, i, z4, z5);
    }
  ...
  public boolean updateDefaultHomeNotLocked(Computer computer, final int i) {
    ...
    ResolveInfo findPreferredActivityNotLocked =
        findPreferredActivityNotLocked(
            ...
            null,
            0L,
            computer.queryIntentActivitiesInternal(homeIntent, null, 786432L, i),
            true,
            false,
            false,
            i);
    String str =
        (findPreferredActivityNotLocked == null
                || (activityInfo = findPreferredActivityNotLocked.activityInfo) == null)
            ? null
    ...
    if ((packagesForUid == null
            || !ArrayUtils.contains(packagesForUid, this.mPm.mRequiredPermissionControllerPackage))
        && str != null) {
      return this.mPm.setActiveLauncherPackage(
          str,
          i,
          new Consumer() { // from class:
            // com.android.server.pm.PreferredActivityHelper$$ExternalSyntheticLambda1
  ...
  public void addPreferredActivity(
    ...
    if (this.mPm.mContext.checkCallingOrSelfPermission(
      ...
      this.mPm.mContext.enforceCallingOrSelfPermission(
    ...
    synchronized (this.mPm.mLock) {
      PreferredIntentResolver editPreferredActivitiesLPw =
          this.mPm.mSettings.editPreferredActivitiesLPw(i2);
      ...
      PreferredActivityLog.logPreferenceChange(
          new PreferredActivity(watchedIntentFilter, i, componentNameArr, componentName, z), str);
      editPreferredActivitiesLPw.addFilter(
          (PackageDataSnapshot) this.mPm.snapshotComputer(),
          (WatchedIntentFilter)
              new PreferredActivity(watchedIntentFilter, i, componentNameArr, componentName, z));
      this.mPm.scheduleWritePackageRestrictions(i2);
    }
    if (isHomeFilter(watchedIntentFilter)
        && updateDefaultHomeNotLocked(this.mPm.snapshotComputer(), i2)) {
      return;
    }
  ...
  */
  public void replacePreferredActivity(
    ...
    if (watchedIntentFilter.countDataAuthorities() != 0
        || watchedIntentFilter.countDataPaths() != 0
        || watchedIntentFilter.countDataSchemes() > 1
        || watchedIntentFilter.countDataTypes() != 0) {
      throw new IllegalArgumentException(
          "replacePreferredActivity expects filter to have no data authorities, paths, or types;"
              + " and at most one scheme.");
    }
    if (DesktopModeUtils.isHomeFilter(watchedIntentFilter)
        && DesktopModeUtils.isReplacingPreferredBlockedByDesktopMode(
            watchedIntentFilter,
            componentName,
            DesktopModeFeature.isDesktopMode(this.mPm.mContext),
    ...
    if (this.mPm.mContext.checkCallingOrSelfPermission(
      ...
      synchronized (this.mPm.mLock) {
        if (this.mPm.snapshotComputer().getUidTargetSdkVersion(callingUid) < 8) {
          Slog.w(
              "PackageManager",
              "Ignoring replacePreferredActivity() from uid " + Binder.getCallingUid());
          return;
        }
        this.mPm.mContext.enforceCallingOrSelfPermission(
            "android.permission.SET_PREFERRED_APPLICATIONS", null);
    ...
    synchronized (this.mPm.mLock) {
      ...
      if (preferredActivities != null) {
        ArrayList findFilters = preferredActivities.findFilters(watchedIntentFilter);
        if (findFilters != null && findFilters.size() == 1) {
          PreferredActivity preferredActivity = (PreferredActivity) findFilters.get(0);
          PreferredComponent preferredComponent = preferredActivity.mPref;
          if (preferredComponent.mAlways && preferredComponent.mComponent.equals(componentName)) {
            ...
            if (preferredComponent2.mMatch == (i & 268369920)) {
              ...
              if (findFilters != null) {
                PmLog.logDebugInfoAndLogcat("Removing prefs while replacing");
  ...
  public void clearPackagePreferredActivities(Computer computer, String str) {
    ...
    if ((packageStateInternal == null || !computer.isCallerSameApp(str, callingUid))
        && this.mPm.mContext.checkCallingOrSelfPermission(
                "android.permission.SET_PREFERRED_APPLICATIONS")
            != 0) {
      if (computer.getUidTargetSdkVersion(callingUid) < 8) {
        Slog.w(
            "PackageManager", "Ignoring clearPackagePreferredActivities() from uid " + callingUid);
        return;
      }
      this.mPm.mContext.enforceCallingOrSelfPermission(
          "android.permission.SET_PREFERRED_APPLICATIONS", null);
    ...
    if (packageStateInternal == null
      ...
      try {
        ...
        if (asInterface != null
            && str.equals(asInterface.getKioskHomePackageAsUser(callingUserId))
            && asInterface.isKioskModeEnabledAsUser(callingUserId)) {
          Intent intent =
              new Intent("com.samsung.android.knox.intent.action.TERMINATE_KIOSK_INTERNAL");
        ...
        Log.w(
            "PackageManager",
            " clearPackagePreferredActivities in kioskMode exception: " + e.toString());
      }
  ...
  public void setHomeActivity(Computer computer, ComponentName componentName, int i) {
    ...
    if (!z) {
      throw new IllegalArgumentException(
          "Component " + componentName + " cannot be home on user " + i);
    }
    replacePreferredActivity(
        computer, getHomeFilter(), 1048576, componentNameArr, componentName, i);
  }
  ...
  public void addPersistentPreferredActivity(
      WatchedIntentFilter watchedIntentFilter, ComponentName componentName, int i) {
    int callingUid = Binder.getCallingUid();
    if (callingUid != 1000) {
    ...
    synchronized (this.mPm.mLock) {
      this.mPm
          ...
          .addFilter(
              (PackageDataSnapshot) this.mPm.snapshotComputer(),
              (WatchedIntentFilter)
                  new PersistentPreferredActivity(watchedIntentFilter, componentName, true));
      this.mPm.scheduleWritePackageRestrictions(i);
    }
    if (isHomeFilter(watchedIntentFilter)) {
  ...
  public void clearPackagePersistentPreferredActivities(String str, int i) {
    ...
    synchronized (this.mPm.mLock) {
      clearPackagePersistentPreferredActivities =
          this.mPm.mSettings.clearPackagePersistentPreferredActivities(str, i);
    }
  ...
  public void clearPersistentPreferredActivity(IntentFilter intentFilter, int i) {
    ...
    synchronized (this.mPm.mLock) {
      clearPersistentPreferredActivity =
          this.mPm.mSettings.clearPersistentPreferredActivity(intentFilter, i);
    }
  ...
  public final boolean isHomeFilter(WatchedIntentFilter watchedIntentFilter) {
    return watchedIntentFilter.hasAction("android.intent.action.MAIN")
        && watchedIntentFilter.hasCategory("android.intent.category.HOME")
        && watchedIntentFilter.hasCategory("android.intent.category.DEFAULT");
  }

  public final void restoreFromXml(
      TypedXmlPullParser typedXmlPullParser, int i, String str, BlobXmlRestorer blobXmlRestorer) {
    int next;
    do {
  ...
  public void restorePreferredActivities(byte[] bArr, int i) {
    ...
    try {
      ...
      restoreFromXml(
          ...
          i,
          "pa",
          new BlobXmlRestorer() { // from class:
            // com.android.server.pm.PreferredActivityHelper$$ExternalSyntheticLambda0
            ...
            public final void apply(TypedXmlPullParser typedXmlPullParser, int i2) {
              PreferredActivityHelper.this.lambda$restorePreferredActivities$1(
                  typedXmlPullParser, i2);
            }
          });
    } catch (Exception unused) {
    }
  ...
  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ void lambda$restorePreferredActivities$1(
  ...
  public void restoreDefaultApps(byte[] bArr, int i) {
    ...
    try {
      ...
      restoreFromXml(
          ...
          "da",
          new BlobXmlRestorer() { // from class:
            // com.android.server.pm.PreferredActivityHelper$$ExternalSyntheticLambda2
            @Override // com.android.server.pm.PreferredActivityHelper.BlobXmlRestorer
            public final void apply(TypedXmlPullParser typedXmlPullParser, int i2) {
  ...
  /* JADX INFO: Access modifiers changed from: private */
  public /* synthetic */ void lambda$restoreDefaultApps$2(
      TypedXmlPullParser typedXmlPullParser, int i) {
  ...
  public final void resetNetworkPolicies(int i) {
    ((NetworkPolicyManagerInternal)
            this.mPm.mInjector.getLocalService(NetworkPolicyManagerInternal.class))
        .resetUserState(i);
  }
  ...
  public int getPreferredActivities(Computer computer, List list, List list2, String str) {
    ...
    int preferredActivitiesInternal =
        getPreferredActivitiesInternal(computer, watchedIntentFilterList, list2, str);
    list.clear();
  ...
  public final int getPreferredActivitiesInternal(
      Computer computer, List list, List list2, String str) {
    int callingUid = Binder.getCallingUid();
    if (computer.getInstantAppPackageName(callingUid) != null) {
      return 0;
    }
    ...
    if (preferredActivities != null) {
      ...
      while (filterIterator.hasNext()) {
        ...
        if (str == null || (packageName.equals(str) && preferredActivity.mPref.mAlways)) {
          if (!computer.shouldFilterApplication(
              computer.getPackageStateInternal(packageName), callingUid, callingUserId)) {
            if (list != null) {
    ...
    if (asInterface != null) {
      try {
        for (DefaultAppConfiguration defaultAppConfiguration :
            asInterface.getAllDefaultApplicationsInternal(callingUserId)) {
          if (str == null
              || defaultAppConfiguration.getComponentName().getPackageName().equals(str)) {
            if (list != null) {
              list.add(
                  new WatchedIntentFilter(
                      asInterface.createIntentFilter(defaultAppConfiguration.getTaskType())));
            }
            if (list2 != null) {
  ...
  public ResolveInfo findPersistentPreferredActivity(Computer computer, Intent intent, int i) {
    ...
    String resolveTypeIfNeeded =
        updateIntentForResolve.resolveTypeIfNeeded(this.mPm.mContext.getContentResolver());
    long updateFlagsForResolve =
        computer.updateFlagsForResolve(
            ...
            computer.isImplicitImageCaptureIntentAndNotSetByDpc(
                updateIntentForResolve, i, resolveTypeIfNeeded, 0L));
    return computer.findPersistentPreferredActivity(
        updateIntentForResolve,
        resolveTypeIfNeeded,
        updateFlagsForResolve,
        computer.queryIntentActivitiesInternal(
            updateIntentForResolve, resolveTypeIfNeeded, updateFlagsForResolve, i),
        false,
        i);
  }
  ...
  public void setLastChosenActivity(
    ...
    if (PersonaServiceHelper.isSpfKnoxSupported()
        && PersonaServiceHelper.isLastChosenActivity(
            computer, intent, str, i, watchedIntentFilter, i2, componentName, this.mPm, this)) {
      return;
    }
    long j = i;
    findPreferredActivityNotLocked(
        ...
        str,
        j,
        computer.queryIntentActivitiesInternal(intent, str, j, callingUserId),
        false,
        true,
        false,
        callingUserId);
  ...
  public ResolveInfo getLastChosenActivity(Computer computer, Intent intent, String str, int i) {
    ...
    int callingUserId = UserHandle.getCallingUserId();
    if (PersonaServiceHelper.isSpfKnoxSupported()) {
      Pair lastChosenActivity =
          PersonaServiceHelper.getLastChosenActivity(computer, intent, str, i, this.mPm, this);
