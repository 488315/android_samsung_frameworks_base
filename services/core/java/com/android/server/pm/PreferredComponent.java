...
public class PreferredComponent {
          ...
          && ((packageUserState =
                      (PackageUserState)
                          packageManagerInternal
                              .getPackageStateInternal(activityInfo.packageName)
                              .getUserStates()
                              .get(i))
                  == null
              || packageUserState.getInstallReason() != 3)) {
        int i4 = 0;
        while (true) {
          if (i4 >= length) {
            z2 = false;
          ...
          if (this.mSetPackages[i4].equals(activityInfo.packageName)
              && this.mSetClasses[i4].equals(activityInfo.name)) {
            i2++;
            z2 = true;
            break;
          }
  ...
  public boolean sameSet(ComponentName[] componentNameArr) {
    ...
    while (true) {
      ...
      while (true) {
        ...
        if (this.mSetPackages[i3].equals(componentName.getPackageName())
            && this.mSetClasses[i3].equals(componentName.getClassName())) {
          i2++;
          break;
        }
        i3++;
  ...
  public boolean sameSet(PreferredComponent preferredComponent) {
    if (this.mSetPackages == null
        ...
        || preferredComponent.mSetPackages == null
        || !sameComponent(preferredComponent.mComponent)) {
      return false;
    }
    ...
    for (int i = 0; i < length2; i++) {
      if (!this.mSetPackages[i].equals(preferredComponent.mSetPackages[i])
          || !this.mSetClasses[i].equals(preferredComponent.mSetClasses[i])) {
        return false;
      }
    }
    return true;
  ...
  public final boolean sameComponent(ComponentName componentName) {
    ...
    return componentName2 != null
        && componentName != null
        && componentName2.getPackageName().equals(componentName.getPackageName())
        && this.mComponent.getClassName().equals(componentName.getClassName());
  }
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (!z || !activityInfo.packageName.equals(setupWizardPackageName)) {
        ...
        while (true) {
          ...
          if (this.mSetPackages[i2].equals(activityInfo.packageName)
              && this.mSetClasses[i2].equals(activityInfo.name)) {
            z2 = true;
            break;
          }
          i2++;
