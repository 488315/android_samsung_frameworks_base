...
public class UpdateOwnershipHelper {
  ...
  public static boolean hasValidOwnershipDenyList(PackageSetting packageSetting) {
    ...
    return pkg != null
        ...
        && pkg.getProperties().containsKey("android.app.PROPERTY_LEGACY_UPDATE_OWNERSHIP_DENYLIST")
        && usesAnyPermission(
            pkg,
    ...
    try {
      int resourceId =
          ((PackageManager.Property)
                  pkg.getProperties().get("android.app.PROPERTY_LEGACY_UPDATE_OWNERSHIP_DENYLIST"))
              .getResourceId();
      ...
      XmlResourceParser xml =
          ResourcesManager.getInstance()
              .getResources(
                  (IBinder) null,
                  generateAppInfoWithoutState.sourceDir,
                  generateAppInfoWithoutState.splitSourceDirs,
                  generateAppInfoWithoutState.resourceDirs,
                  generateAppInfoWithoutState.overlayPaths,
      ...
      Slog.e(
          "PackageManager",
          "Failed to parse update owner list for " + packageSetting.getPackageName(),
          e);
      return null;
    }
  }
  ...
  public void addToUpdateOwnerDenyList(String str, ArraySet arraySet) {
    synchronized (this.mLock) {
      for (int i = 0; i < arraySet.size(); i++) {
        ArraySet arraySet2 =
            (ArraySet)
                this.mUpdateOwnerOptOutsToOwners.putIfAbsent(
                    (String) arraySet.valueAt(i), new ArraySet(new String[] {str}));
        if (arraySet2 != null) {
          arraySet2.add(str);
        }
      }
