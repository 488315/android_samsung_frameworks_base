...
public class BackgroundInstallControlService extends SystemService {
  ...
  public BackgroundInstallControlService(Injector injector) {
    ...
    usageStatsManagerInternal.registerListener(
        new UsageStatsManagerInternal.UsageEventListener() { // from class:
          // com.android.server.pm.BackgroundInstallControlService$$ExternalSyntheticLambda0
  ...
  public ParceledListSlice getBackgroundInstalledPackages(long j, int i) {
    List installedPackagesAsUser =
        this.mPackageManager.getInstalledPackagesAsUser(PackageManager.PackageInfoFlags.of(j), i);
  ...
  public ParceledListSlice getMockBackgroundInstalledPackages(String str) {
    ...
    for (String str2 : split) {
      try {
        arrayList.add(
            this.mPackageManager.getPackageInfo(str2, PackageManager.PackageInfoFlags.of(131072L)));
    ...
    try {
      ApplicationInfo applicationInfoAsUser =
          this.mPackageManager.getApplicationInfoAsUser(
              str, PackageManager.ApplicationInfoFlags.of(0L), i);
      try {
        ...
        if (this.mPermissionManager.checkPermission(
                installingPackageName, "android.permission.INSTALL_PACKAGES", i)
            != 0) {
          return;
        }
        long currentTimeMillis =
            System.currentTimeMillis()
                - (SystemClock.uptimeMillis() - applicationInfoAsUser.createTimestamp);
        if (installedByAdb(initiatingPackageName)
            || wasForegroundInstallation(installingPackageName, i, currentTimeMillis)) {
          return;
        }
        initBackgroundInstalledPackages();
  ...
  public final boolean wasForegroundInstallation(String str, int i, long j) {
    ...
    while (it.hasNext()) {
      ...
      if (foregroundTimeFrame.startTimeStampMillis <= j
          && (!foregroundTimeFrame.isDone() || j <= foregroundTimeFrame.endTimeStampMillis)) {
        return true;
      }
    }
    return false;
  ...
  public final boolean isInstaller(String str, int i) {
    return this.mInstallerForegroundTimeFrames.contains(i, str)
        || this.mPermissionManager.checkPermission(str, "android.permission.INSTALL_PACKAGES", i)
            == 0;
  }
  ...
  public void onStart(boolean z) {
    ...
    this.mPackageManagerInternal.getPackageList(
        new PackageManagerInternal.PackageListObserver() { // from class:
          // com.android.server.pm.BackgroundInstallControlService.1
          @Override // android.content.pm.PackageManagerInternal.PackageListObserver
          public void onPackageAdded(String str, int i) {
            BackgroundInstallControlService.this
                .mHandler
                .obtainMessage(1, UserHandle.getUserId(i), 0, str)
                .sendToTarget();
          }
          ...
          public void onPackageRemoved(String str, int i) {
            BackgroundInstallControlService.this
                .mHandler
                .obtainMessage(2, UserHandle.getUserId(i), 0, str)
                .sendToTarget();
          }
        });
  }
  ...
  public final class ForegroundTimeFrame implements Comparable {
    ...
    public int compareTo(ForegroundTimeFrame foregroundTimeFrame) {
      int compare =
          Long.compare(this.startTimeStampMillis, foregroundTimeFrame.startTimeStampMillis);
