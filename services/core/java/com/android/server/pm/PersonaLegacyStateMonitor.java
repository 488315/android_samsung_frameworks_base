...
public class PersonaLegacyStateMonitor {
  ...
  public ContainerStateReceiver receiver = new ContainerStateReceiver() { // from class:
        ...
        public void onContainerRunning(Context context, int i, Bundle bundle) {
          ...
          if ((userInfo.getAttributes() & 8) > 0) {
            PersonaLegacyStateMonitor.this.notifyStateChange(
                userInfo.id, SemPersonaState.ADMIN_LOCKED);
            return;
          }
          if ((userInfo.getAttributes() & 16) > 0) {
            PersonaLegacyStateMonitor.this.notifyStateChange(
                userInfo.id, SemPersonaState.LICENSE_LOCKED);
          } else if ((userInfo.getAttributes() & 4) > 0) {
            PersonaLegacyStateMonitor.this.notifyStateChange(
                userInfo.id, SemPersonaState.TIMA_COMPROMISED);
          } else {
        ...
        public void onContainerRemoved(Context context, int i, Bundle bundle) {
          ...
          int mUMContainerOwnerUid =
              PersonaLegacyStateMonitor.this.mEdmStorageProvider.getMUMContainerOwnerUid(i);
          ...
          if (packagesForUid != null) {
            for (String str : packagesForUid) {
              ...
              Log.i(
                  ...
                  "Sending container removed intent to MDM for user " + i + " Package is " + str);
            }
            return;
          }
          ...
          Log.i(
              "PersonaManagerService::LegacyStateMonitor",
              "Sending container removed intent to MDM for user " + i);
        }
      };
  ...
  */
  public final void notifyStateChange(int i, SemPersonaState semPersonaState) {
    ...
    intent.putExtra(
        "com.sec.knox.container.extra.observer.previousstate", semPersonaState4.toString());
    ...
    if (translateStatus != translateStatus2) {
      String[] packagesForUid2 =
          this.mContext.getPackageManager().getPackagesForUid(mUMContainerOwnerUid);
      ...
      if (packagesForUid2 != null) {
        ...
        Log.i(
            "PersonaManagerService::LegacyStateMonitor",
            "Sending admin lock intent to MDM for user " + i);
        ...
        if (semPersonaState == semPersonaState2 && semPersonaState4 != semPersonaState2) {
          ...
          if (packagesForUid == null) {
            for (String str4 : packagesForUid) {
              ...
              Log.i(
                  "PersonaManagerService::LegacyStateMonitor",
                  "Sending admin lock change intent to MDM for user " + i + " Package is " + str4);
            }
          } else {
  ...
  public static int translateStatus(SemPersonaState semPersonaState) {
    ...
    switch (AbstractC21642.$SwitchMap$com$samsung$android$knox$SemPersonaState[
