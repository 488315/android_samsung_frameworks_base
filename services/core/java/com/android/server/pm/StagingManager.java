...
public final class StagingManager {
    ...
    public final class Lifecycle extends SystemService {
        ...
        public final void onBootPhase(int i) {
            ...
            synchronized (stagingManager.mSuccessfulStagedSessionIds) {
                for (int i2 = 0;
                        i2 < ((ArrayList) stagingManager.mSuccessfulStagedSessionIds).size();
                        i2++) {
                    try {
                        stagingManager.mApexManager.markStagedSessionSuccessful(
                                ((Integer)
                                                ((ArrayList)
                                                                stagingManager
            ...
            ApexManager.ApexManagerImpl apexManagerImpl =
                    (ApexManager.ApexManagerImpl) sStagingManager.mApexManager;
        ...
        if (PackageInstallerSession.this.params.isMultiPackage) {
            ...
            while (it.hasNext()) {
                PackageInstallerSession.StagedSession stagedSession3 =
                        (PackageInstallerSession.StagedSession) ((StagedSession) it.next());
                ...
                if (z2) {
                    try {
                        BufferedWriter bufferedWriter =
                                new BufferedWriter(new FileWriter(this.mFailureReasonFile));
                        try {
    ...
    public Map getStagedApexInfos(StagedSession stagedSession) {
        ...
        Preconditions.checkArgument(
                stagedSession2.containsApexSession(),
                packageInstallerSession.sessionId + " session does not contain apex");
        ...
        try {
            apexInfoArr =
                    apexManagerImpl.waitForApexService().getStagedApexInfos(apexSessionParams);
        } catch (RemoteException e) {
            ...
            MagnificationConnectionManager$$ExternalSyntheticOutline0.m(
                    e2, new StringBuilder("Failed to collect staged apex infos"), "ApexManager");
                ...
                try {
                    PackageInstallerSession.StagedSession stagedSession =
                            (PackageInstallerSession.StagedSession)
                                    ((StagedSession) this.mStagedSessions.valueAt(i));
                    if (stagedSession.isSessionReady()
                            && !PackageInstallerSession.this.isDestroyed()
        ...
        synchronized (this.mStagedApexObservers) {
            ...
            while (it.hasNext()) {
                ...
                apexStagedEvent.stagedApexModuleNames =
                        (String[]) ((ArrayList) getStagedApexModuleNames()).toArray(new String[0]);
                try {
    ...
    public void onBootCompletedBroadcastReceived() {
        ...
        BackgroundThread.getExecutor()
                .execute(new StagingManager$$ExternalSyntheticLambda0(0, this));
    }

    public final void onInstallationFailure(
        ...
        if (stagedSession2.containsApexSession()) {
            if (!this.mApexManager.revertActiveSessions()) {
                ...
                Slog.e(
                        "StagingManager",
                        "Successfully aborted apexd session. Rebooting device in order to revert to"
                            + " the previous state of APEXd.");
                this.mPowerManager.reboot(null);
            }
        }
    }
    ...
    public final void resumeSession(StagedSession stagedSession, boolean z, boolean z2) {
        ...
        if (z && !z2) {
            String m =
                    AmFmBandRange$$ExternalSyntheticOutline0.m(
                            PackageInstallerSession.this.sessionId,
                            new StringBuilder("Reverting back to safe state. Marking "),
                            " as failed.");
            ...
            if (!TextUtils.isEmpty(reasonForRevert)) {
                m =
                        AnyMotionDetector$$ExternalSyntheticOutline0.m(
                                m, " Reason for revert: ", reasonForRevert);
            }
        ...
        if (containsApexSession) {
                ...
                while (it.hasNext()) {
                    ...
                    synchronized (apexManagerImpl.mLock) {
                        try {
                            ...
                            String str2 =
                                    (String)
                                            apexManagerImpl.mPackageNameToApexModuleName.get(
                                                    packageName);
                            str =
                                    str2 == null
                                            ? null
                                            : (String)
                                                    ((ArrayMap) apexManagerImpl.mErrorWithApkInApex)
                                                            .get(str2);
                        } finally {
                        }
                    }
                    if (str != null) {
                        throw new PackageManagerException(
                                ...
                                BootReceiver$$ExternalSyntheticOutline0.m(
                                        "Failed to install apk-in-apex of ",
                                        packageName,
                                        " : ",
                                        str));
                    }
            ...
            if (PackageInstallerSession.this.params.isMultiPackage) {
                ...
                while (it2.hasNext()) {
                    PackageInstallerSession.StagedSession stagedSession3 =
                            (PackageInstallerSession.StagedSession) ((StagedSession) it2.next());
                    if (!stagedSession3.isApexSession()) {
                ...
                while (it3.hasNext()) {
                    PackageInstallerSession.StagedSession stagedSession4 =
                            (PackageInstallerSession.StagedSession) ((StagedSession) it3.next());
                        ...
                        if (!arraySet.add(str3)) {
                            StringBuilder m2 =
                                    DumpUtils$$ExternalSyntheticOutline0.m(
                                            "Package: ", packageName2, " in session: ");
                        ...
                        while (i3 < size2) {
                            ...
                            if (packageManagerInternal.getPackage(str4) == null) {
                                ...
                                PackageStateInternal packageStateInternal =
                                        packageManagerInternal.getPackageStateInternal(str4);
                                if (packageStateInternal != null) {
                                    ...
                                    int[] queryInstalledUsers =
                                            PackageStateUtils.queryInstalledUsers(
                                                    packageStateInternal, userIds);
                                    String seInfo = packageSetting.getSeInfo();
                                    List userHandles2 =
                                            UserHandle.toUserHandles(queryInstalledUsers);
                                    rollbackManagerServiceImpl.assertNotInWorkerThread();
                                    i = i3;
                                    rollbackManagerServiceImpl.snapshotAndRestoreUserData(
                                            str4,
                                            UserHandle.fromUserHandles(userHandles2),
                                            i4,
                                            ceDataInode,
                                            seInfo,
                                            0);
            ...
            CustomizedBinderCallsStatsInternal$$ExternalSyntheticOutline0.m(
                    new StringBuilder("APEX packages in session "),
                    PackageInstallerSession.this.sessionId,
        ...
        Slog.d(
                "StagingManager",
                "Installing APK packages in session " + PackageInstallerSession.this.sessionId);
            ...
            if (containsApexSession) {
                if (z) {
                    synchronized (this.mSuccessfulStagedSessionIds) {
                        ((ArrayList) this.mSuccessfulStagedSessionIds)
                                .add(Integer.valueOf(PackageInstallerSession.this.sessionId));
                    }
                } else {
