...
public class StagingManager {
    ...
    if (iStagedApexObserver.asBinder() != null) {
      try {
        iStagedApexObserver
            .asBinder()
            .linkToDeath(
                new IBinder.DeathRecipient() { // from class: com.android.server.pm.StagingManager.1
  ...
  public final void abortCheckpoint(String str, boolean z, boolean z2) {
    ...
    if (z) {
      try {
        if (z2) {
          try {
            bufferedWriter = new BufferedWriter(new FileWriter(this.mFailureReasonFile));
          } catch (Exception e) {
    ...
    while (it.hasNext()) {
      ...
      if (apkInApexInstallError != null) {
        throw new PackageManagerException(
            -128,
            "Failed to install apk-in-apex of " + packageName + " : " + apkInApexInstallError);
      }
    }
  }
  ...
  public final void snapshotAndRestoreForApexSession(StagedSession stagedSession) {
    if ((stagedSession.sessionParams().installFlags & 262144) != 0
        || stagedSession.sessionParams().installReason == 5) {
      List extractApexSessions = extractApexSessions(stagedSession);
      if (extractApexSessions.isEmpty()) {
        return;
      }
      ...
      RollbackManagerInternal rollbackManagerInternal =
      ...
      for (int i = 0; i < size; i++) {
        ...
        for (int i2 = 0; i2 < size2; i2++) {
          snapshotAndRestoreApkInApexUserData(
              (String) apksInApex.get(i2), userIds, rollbackManagerInternal);
        }
      }
    }
  }

  public final void snapshotAndRestoreApexUserData(
      String str, int[] iArr, RollbackManagerInternal rollbackManagerInternal) {
    rollbackManagerInternal.snapshotAndRestoreUserData(
        str, UserHandle.toUserHandles(iArr), 0, 0L, null, 0);
  }

  public final void snapshotAndRestoreApkInApexUserData(
      String str, int[] iArr, RollbackManagerInternal rollbackManagerInternal) {
    ...
    if (packageManagerInternal.getPackage(str) == null) {
      Slog.e(
          "StagingManager",
          "Could not find package: " + str + "for snapshotting/restoring user data.");
      return;
    }
    ...
    if (packageStateInternal != null) {
      rollbackManagerInternal.snapshotAndRestoreUserData(
          str,
          UserHandle.toUserHandles(
              PackageStateUtils.queryInstalledUsers(packageStateInternal, iArr, true)),
          packageStateInternal.getAppId(),
          packageStateInternal.getUserStateOrDefault(0).getCeDataInode(),
  ...
  public final void resumeSession(StagedSession stagedSession, boolean z, boolean z2) {
    ...
    if (z && !z2) {
      String str =
          "Reverting back to safe state. Marking " + stagedSession.sessionId() + " as failed.";
  ...
  public void onInstallationFailure(
    ...
    if (stagedSession.containsApexSession()) {
      if (!this.mApexManager.revertActiveSessions()) {
        ...
        Slog.e(
            "StagingManager",
            "Successfully aborted apexd session. Rebooting device in order to revert to the"
                + " previous state of APEXd.");
        this.mPowerManager.reboot(null);
      }
    }
  }
  ...
  public void abortCommittedSession(StagedSession stagedSession) {
    ...
    if (!stagedSession.isDestroyed()) {
      throw new IllegalStateException(
          "Committed session must be destroyed before aborting it from StagingManager");
    }
  ...
  public final boolean ensureActiveApexSessionIsAborted(StagedSession stagedSession) {
    ...
    if (!stagedSession.containsApexSession()
        || (stagedSessionInfo = this.mApexManager.getStagedSessionInfo(stagedSession.sessionId()))
            == null
  ...
  public static boolean isApexSessionFailed(ApexSessionInfo apexSessionInfo) {
    return apexSessionInfo.isActivationFailed
        || apexSessionInfo.isUnknown
        || apexSessionInfo.isReverted
        || apexSessionInfo.isRevertInProgress
  ...
  public final void handleNonReadyAndDestroyedSessions(List list) {
    ...
    while (i < size) {
      ...
      if (stagedSession.isDestroyed()) {
        ...
        this.mBootCompleted.thenRun(
            new Runnable() { // from class:
              // com.android.server.pm.StagingManager$$ExternalSyntheticLambda1
  ...
  public void restoreSessions(List list, boolean z) {
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      Preconditions.checkArgument(
          !stagedSession.hasParentSessionId(), stagedSession.sessionId() + " is a child session");
      Preconditions.checkArgument(
          stagedSession.isCommitted(), stagedSession.sessionId() + " is not committed");
      Preconditions.checkArgument(
          true ^ stagedSession.isInTerminalState(),
          stagedSession.sessionId() + " is in terminal state");
      createSession(stagedSession);
    }
    if (z) {
    ...
    try {
      ...
      for (int i3 = 0; i3 < list.size(); i3++) {
        ...
        if (stagedSession2.containsApexSession()) {
          ApexSessionInfo apexSessionInfo =
              (ApexSessionInfo) sessions.get(stagedSession2.sessionId());
          if (apexSessionInfo == null || apexSessionInfo.isUnknown) {
            stagedSession2.setSessionFailed(
                -128,
                "apexd did not know anything about a staged session supposed to be activated");
            ...
            stagedSession2.setSessionFailed(
                ...
                "Staged session "
                    ...
                    + " at boot didn't activate nor fail. Marking it as failed anyway.");
          } else {
            Slog.w(
                "StagingManager",
                "Apex session " + stagedSession2.sessionId() + " is in impossible state");
      ...
      if (z2 && z3) {
        abortCheckpoint(
            "Found both applied and failed apex sessions", supportsCheckpoint, needsCheckpoint);
        return;
      }
      if (z3) {
        ...
        try {
          ...
          onInstallationFailure(
              ...
              new PackageManagerException(
                  -110, "Staged install failed due to unhandled exception: " + e2),
              supportsCheckpoint,
              needsCheckpoint);
        }
      }
    ...
    synchronized (this.mFailedPackageNames) {
      if (!this.mFailedPackageNames.isEmpty()) {
        WatchdogRollbackLogger.logApexdRevert(
            this.mContext, this.mFailedPackageNames, this.mNativeFailureReason);
      }
    }
  }
  ...
  public final void markStagedSessionsAsSuccessful() {
    synchronized (this.mSuccessfulStagedSessionIds) {
      for (int i = 0; i < this.mSuccessfulStagedSessionIds.size(); i++) {
        this.mApexManager.markStagedSessionSuccessful(
            ((Integer) this.mSuccessfulStagedSessionIds.get(i)).intValue());
      }
  ...
  public void systemReady() {
    ...
    this.mContext.registerReceiver(
        new BroadcastReceiver() { // from class: com.android.server.pm.StagingManager.2
  ...
  public void onBootCompletedBroadcastReceived() {
    ...
    BackgroundThread.getExecutor()
        .execute(
            new Runnable() { // from class:
              // com.android.server.pm.StagingManager$$ExternalSyntheticLambda0
              @Override // java.lang.Runnable
              public final void run() {
  ...
  public Map getStagedApexInfos(StagedSession stagedSession) {
    ...
    Preconditions.checkArgument(
        true ^ stagedSession.hasParentSessionId(),
        stagedSession.sessionId() + " session has parent session");
    Preconditions.checkArgument(
        stagedSession.containsApexSession(),
        stagedSession.sessionId() + " session does not contain apex");
    ...
    synchronized (this.mStagedSessions) {
      for (int i = 0; i < this.mStagedSessions.size(); i++) {
        ...
        if (stagedSession.isSessionReady()
            ...
            && !stagedSession.hasParentSessionId()
            && stagedSession.containsApexSession()
            && (apexInfo = (ApexInfo) getStagedApexInfos(stagedSession).get(str)) != null) {
  ...
  public final void notifyStagedApexObservers() {
    synchronized (this.mStagedApexObservers) {
      for (IStagedApexObserver iStagedApexObserver : this.mStagedApexObservers) {
        ...
        apexStagedEvent.stagedApexModuleNames =
            (String[]) getStagedApexModuleNames().toArray(new String[0]);
        try {
