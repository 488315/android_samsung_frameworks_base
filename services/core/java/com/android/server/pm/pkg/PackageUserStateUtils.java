    ...
    public static boolean isEnabled(PackageUserState packageUserState, boolean z, boolean z2, String str, long j) {
        ...
        if (enabledState != 0) {
            if (enabledState != 2 && enabledState != 3) {
                if (enabledState == 4) {
                    if ((536870912 & j) != 0 && packageUserState.getLastDisableAppCaller() != null && "auto_disabler".equals(packageUserState.getLastDisableAppCaller()) && str != null) {
                        if (packageUserState.getEnabledComponents() != null && packageUserState.getEnabledComponents().contains(str)) {
                            return true;
                        }
                        if (packageUserState.getDisabledComponents() == null || !packageUserState.getDisabledComponents().contains(str)) {
                            return z2;
                        }
                        return false;
                    }
