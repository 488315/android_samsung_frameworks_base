...
public abstract class ParsedProcessUtils {
  public static ParseResult parseDenyPermission(
      Set set, Resources resources, XmlResourceParser xmlResourceParser, ParseInput parseInput) {
    TypedArray obtainAttributes =
        resources.obtainAttributes(xmlResourceParser, R.styleable.AndroidManifestDenyPermission);
    try {
      String nonConfigurationString = obtainAttributes.getNonConfigurationString(0, 0);
      if (nonConfigurationString != null
          && nonConfigurationString.equals("android.permission.INTERNET")) {
  ...
  public static ParseResult parseAllowPermission(
      Set set, Resources resources, XmlResourceParser xmlResourceParser, ParseInput parseInput) {
    TypedArray obtainAttributes =
        resources.obtainAttributes(xmlResourceParser, R.styleable.AndroidManifestAllowPermission);
    try {
      String nonConfigurationString = obtainAttributes.getNonConfigurationString(0, 0);
      if (nonConfigurationString != null
          && nonConfigurationString.equals("android.permission.INTERNET")) {
  ...
  public static ParseResult parseProcess(
      Set set,
      String[] strArr,
    ...
    TypedArray obtainAttributes =
        resources.obtainAttributes(xmlResourceParser, R.styleable.AndroidManifestProcess);
    if (set != null) {
      try {
    ...
    ParseResult buildProcessName =
        ComponentParseUtils.buildProcessName(
            ...
            parsingPackage.getPackageName(),
            obtainAttributes.getNonConfigurationString(1, 0),
            i,
            strArr,
            parseInput);
    ...
    String buildClassName =
        ParsingUtils.buildClassName(packageName, obtainAttributes.getNonConfigurationString(0, 0));
      ...
      if (next != 3 && next != 4) {
        ...
        if (name.equals("allow-permission")) {
          parseAllowPermission =
              parseAllowPermission(
                  parsedProcessImpl.getDeniedPermissions(),
                  resources,
                  xmlResourceParser,
                  parseInput);
          ...
          parseAllowPermission =
              ParsingUtils.unknownTag("<process>", parsingPackage, xmlResourceParser, parseInput);
        }
      ...
      if (next != 1 && (next != 3 || xmlResourceParser.getDepth() > depth)) {
        if (next != 3 && next != 4) {
          ...
          switch (c) {
            case 0:
              parseAllowPermission =
                  parseAllowPermission(set, resources, xmlResourceParser, parseInput);
              if (parseAllowPermission.isSuccess()) {
            ...
            case 1:
              parseAllowPermission =
                  parseProcess(
                      set, strArr, parsingPackage, resources, xmlResourceParser, i, parseInput);
              if (parseAllowPermission.isSuccess()) {
                ...
                if (arrayMap.put(parsedProcess.getName(), parsedProcess) != null) {
                  parseAllowPermission =
                      parseInput.error(
                          "<process> specified existing name '" + parsedProcess.getName() + "'");
                  break;
                }
              }
              break;
            case 2:
              parseAllowPermission =
                  parseDenyPermission(set, resources, xmlResourceParser, parseInput);
              if (parseAllowPermission.isSuccess()) {
                set = (Set) parseAllowPermission.getResult();
                break;
              }
            ...
            default:
              parseAllowPermission =
                  ParsingUtils.unknownTag(
                      "<processes>", parsingPackage, xmlResourceParser, parseInput);
              break;
          }
          if (parseAllowPermission.isError()) {
            return parseInput.error(parseAllowPermission);
