  ...
  */
  public static ParseResult parsePermission(
    ...
    TypedArray obtainAttributes =
        resources.obtainAttributes(xmlResourceParser, R.styleable.AndroidManifestPermission);
    try {
      ParseResult parseComponent =
          ParsedComponentUtils.parseComponent(
              parsedPermissionImpl2,
              str,
              parsingPackage,
              obtainAttributes,
              z,
      ...
      try {
        ...
        if (i != -1 && i < Build.VERSION.SDK_INT) {
          ParseResult success = parseInput.success((Object) null);
          typedArray.close();
          return success;
        }
        if (typedArray.hasValue(12)) {
          if ("android".equals(packageName)) {
            ...
            Slog.w(
                "PackageParsing",
                packageName
                    + " defines a background permission. Only the 'android' package can do that.");
          }
        }
        parsedPermissionImpl2
            .setGroup(typedArray.getNonResourceString(4))
            .setRequestRes(typedArray.getResourceId(13, 0))
            .setProtectionLevel(typedArray.getInt(3, 0))
            .setFlags(typedArray.getInt(8, 0));
        ...
        if (resourceId != 0) {
          ...
          if (parsedPermissionImpl.getKnownCerts().isEmpty()) {
            Slog.w(
                "PackageParsing",
                packageName
                    + " defines a knownSigner permission but the provided knownCerts resource is"
                    + " null");
          }
        } else {
          parsedPermissionImpl = parsedPermissionImpl2;
        ...
        if (isRuntime(parsedPermissionImpl)
            && "android".equals(parsedPermissionImpl.getPackageName())) {
          if ((parsedPermissionImpl.getFlags() & 4) != 0
              && (parsedPermissionImpl.getFlags() & 8) != 0) {
            throw new IllegalStateException(
                "Permission cannot be both soft and hard restricted: "
                    + parsedPermissionImpl.getName());
          }
          typedArray.close();
          parsedPermissionImpl.setProtectionLevel(
              PermissionInfo.fixProtectionLevel(parsedPermissionImpl.getProtectionLevel()));
          if ((getProtectionFlags(parsedPermissionImpl) & (-12353)) == 0
              && getProtection(parsedPermissionImpl) != 2
              && getProtection(parsedPermissionImpl) != 4) {
            return parseInput.error(
                "<permission> protectionLevel specifies a non-instant, non-appop, non-runtimeOnly"
                    + " flag but is not based on signature or internal type");
          }
          parseAllMetaData =
  ...
  public static ParseResult parsePermissionTree(
    ...
    String str = "<" + xmlResourceParser.getName() + ">";
    TypedArray obtainAttributes =
        resources.obtainAttributes(xmlResourceParser, R.styleable.AndroidManifestPermissionTree);
    try {
      ParseResult parseComponent =
          ParsedComponentUtils.parseComponent(
              parsedPermissionImpl,
      ...
      if (indexOf < 0) {
        return parseInput.error(
            "<permission-tree> name has less than three segments: "
                + parsedPermissionImpl.getName());
      }
      parsedPermissionImpl.setProtectionLevel(0).setTree(true);
      ParseResult parseAllMetaData =
          ComponentParseUtils.parseAllMetaData(
              parsingPackage, resources, xmlResourceParser, str, parsedPermissionImpl, parseInput);
      if (parseAllMetaData.isError()) {
        return parseInput.error(parseAllMetaData);
      }
      return parseInput.success((ParsedPermission) parseAllMetaData.getResult());
  ...
  public static ParseResult parsePermissionGroup(
    ...
    String str = "<" + xmlResourceParser.getName() + ">";
    TypedArray obtainAttributes =
        resources.obtainAttributes(xmlResourceParser, R.styleable.AndroidManifestPermissionGroup);
    try {
      ParseResult parseComponent =
          ParsedComponentUtils.parseComponent(
      ...
      try {
        parsedPermissionGroupImpl
            .setRequestDetailRes(typedArray.getResourceId(12, 0))
            .setBackgroundRequestRes(typedArray.getResourceId(9, 0))
            .setBackgroundRequestDetailRes(typedArray.getResourceId(10, 0))
            .setRequestRes(typedArray.getResourceId(11, 0))
            .setPriority(typedArray.getInt(3, 0))
            .setFlags(typedArray.getInt(6, 0));
        typedArray.recycle();
        ParseResult parseAllMetaData =
            ComponentParseUtils.parseAllMetaData(
                parsingPackage,
  ...
  public static boolean isMalformedDuplicate(
      ParsedPermission parsedPermission, ParsedPermission parsedPermission2) {
    return (parsedPermission == null
            || parsedPermission2 == null
            || parsedPermission.isTree()
            || parsedPermission2.isTree()
            || (parsedPermission.getProtectionLevel() == parsedPermission2.getProtectionLevel()
                && Objects.equals(parsedPermission.getGroup(), parsedPermission2.getGroup())))
        ? false
        : true;
  }
