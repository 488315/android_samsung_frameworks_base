...
public abstract class InstallConstraintsTagParser {
  public static ParseResult parseInstallConstraints(
    ...
    if (!SystemConfig.getInstance()
        ...
        .contains(parsingPackage.getPackageName())) {
      return parseInput.skip("install-constraints cannot be used by this package");
    }
    ParseResult parseFingerprintPrefixes =
        parseFingerprintPrefixes(parseInput, resources, xmlResourceParser);
    if (parseFingerprintPrefixes.isSuccess()) {
      if (validateFingerprintPrefixes((Set) parseFingerprintPrefixes.getResult())) {
        return parseInput.success(parsingPackage);
      }
      return parseInput.skip(
          "Install of this package is restricted on this device; device fingerprint does not start"
              + " with one of the allowed prefixes");
    }
    return parseInput.skip(parseFingerprintPrefixes.getErrorMessage());
  }

  public static ParseResult parseFingerprintPrefixes(
    ...
    while (true) {
      ...
      if (next == 2) {
        if (xmlResourceParser.getName().equals("fingerprint-prefix")) {
          ParseResult readFingerprintPrefixValue =
              readFingerprintPrefixValue(parseInput, resources, xmlResourceParser);
  ...
  public static ParseResult readFingerprintPrefixValue(
      ParseInput parseInput, Resources resources, XmlResourceParser xmlResourceParser) {
    TypedArray obtainAttributes =
        resources.obtainAttributes(
            xmlResourceParser, R.styleable.AndroidManifestInstallConstraintsFingerprintPrefix);
    try {
