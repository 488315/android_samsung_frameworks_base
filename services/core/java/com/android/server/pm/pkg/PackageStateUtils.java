...
public abstract class PackageStateUtils {
  ...
  public static int[] queryInstalledUsers(
      PackageStateInternal packageStateInternal, int[] iArr, boolean z) {
    int i = 0;
    for (int i2 : iArr) {
  ...
  public static boolean isEnabledAndMatches(
      PackageStateInternal packageStateInternal, ComponentInfo componentInfo, long j, int i) {
    if (packageStateInternal == null) {
      return false;
    }
    return PackageUserStateUtils.isMatch(
        packageStateInternal.getUserStateOrDefault(i), componentInfo, j);
  }
