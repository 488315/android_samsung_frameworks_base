...
public final class PackageManagerNative extends IPackageManagerNative.Stub {
    ...
    public final int getLocationFlags(String str) {
        ApplicationInfo applicationInfo =
                this.mPm
                        .snapshotComputer()
                        .getApplicationInfo(str, 0L, UserHandle.getUserId(Binder.getCallingUid()));
        if (applicationInfo == null) {
            throw new RemoteException(
                    ConnectivityModuleConnector$$ExternalSyntheticOutline0.m(
                            "Couldn't get ApplicationInfo for package ", str));
        }
        return (applicationInfo.isSystemApp() ? 1 : 0)
                | (applicationInfo.isVendor() ? 2 : 0)
                | (applicationInfo.isProduct() ? 4 : 0);
    }
                ...
                try {
                    PackageInstallerSession.StagedSession stagedSession =
                            (PackageInstallerSession.StagedSession)
                                    ((StagingManager.StagedSession)
                                            stagingManager.mStagedSessions.valueAt(i));
                    if (stagedSession.isSessionReady()
                            ...
                            && (apexInfo =
                                            (ApexInfo)
                                                    stagingManager
                                                            .getStagedApexInfos(stagedSession)
                                                            .get(str))
                                    != null) {
    ...
    public final String[] getStagedApexModuleNames() {
        return (String[])
                ((ArrayList) this.mPm.mInstallerService.mStagingManager.getStagedApexModuleNames())
    ...
    public final int getTargetSdkVersionForPackage(String str) {
        ...
        throw new RemoteException(
                ConnectivityModuleConnector$$ExternalSyntheticOutline0.m(
                        "Couldn't get targetSdkVersion for package ", str));
    }
    ...
    public final long getVersionCodeForPackage(String str) {
        try {
            PackageInfo packageInfo =
                    this.mPm
                            .snapshotComputer()
                            .getPackageInfo(str, 0L, UserHandle.getUserId(Binder.getCallingUid()));
            if (packageInfo != null) {
    ...
    public final boolean[] isAudioPlaybackCaptureAllowed(String[] strArr) {
        ...
        for (int i = length - 1; i >= 0; i--) {
            ApplicationInfo applicationInfo =
                    snapshotComputer.getApplicationInfo(strArr[i], 0L, userId);
    ...
    public final boolean isPackageDebuggable(String str) {
        ApplicationInfo applicationInfo =
                this.mPm
                        .snapshotComputer()
                        .getApplicationInfo(str, 0L, UserHandle.getCallingUserId());
        if (applicationInfo != null) {
            return (applicationInfo.flags & 2) != 0;
        }
        throw new RemoteException(
                ConnectivityModuleConnector$$ExternalSyntheticOutline0.m(
                        "Couldn't get debug flag for package ", str));
    }
                        ...
                        .linkToDeath(
                                new IBinder
                                        .DeathRecipient() { // from class:
                                                            // com.android.server.pm.StagingManager.1
                                    public final /* synthetic */ IStagedApexObserver val$observer;

                                    public AnonymousClass1(
                                            IStagedApexObserver iStagedApexObserver2) {
                                    ...
                                    public final void binderDied() {
                                        synchronized (StagingManager.this.mStagedApexObservers) {
                                            ((ArrayList) StagingManager.this.mStagedApexObservers)
                                                    .remove(r2);
                                        }
                                    }
                                },
