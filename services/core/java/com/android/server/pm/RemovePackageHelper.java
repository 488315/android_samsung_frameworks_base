...
public final class RemovePackageHelper {
  ...
  public RemovePackageHelper(
      PackageManagerService packageManagerService, AppDataHelper appDataHelper) {
    ...
    synchronized (this.mPm.mLock) {
      ...
      if (androidPackage != null) {
        cleanPackageDataStructuresLILPw(
            androidPackage, AndroidPackageUtils.isSystem(androidPackage), z);
      }
    }
  }
    ...
    if (z) {
      ...
      for (int i2 = 0; i2 < size2; i2++) {
        this.mSharedLibraries.removeSharedLibrary(
            (String) androidPackage.getLibraryNames().get(i2), 0L);
      }
    }
  ...
  public void removePackageDataLIF(
      PackageSetting packageSetting,
      int[] iArr,
      PackageRemovedInfo packageRemovedInfo,
      int i,
    ...
    if (packageRemovedInfo != null) {
      ...
      packageRemovedInfo.mIsStaticSharedLib =
          (pkg == null || pkg.getStaticSharedLibraryName() == null) ? false : true;
      packageRemovedInfo.populateUsers(
          packageSetting.queryInstalledUsers(this.mUserManagerInternal.getUserIds(), true),
          packageSetting);
    ...
    if (i4 == 0) {
      AndroidPackage buildFakeForDeletion =
          pkg != null
              ? pkg
              : PackageImpl.buildFakeForDeletion(
                  packageSetting.getPackageName(), packageSetting.getVolumeUuid());
    ...
    if (i4 == 0) {
      ...
      synchronized (this.mPm.mLock) {
        ...
        this.mPm.mAppsFilter.removePackage(
            snapshotComputer, snapshotComputer.getPackageStateInternal(packageName));
        ...
        if (this.mPm.mSettings.isDisabledSystemPackageLPr(packageName)) {
          ...
          SharedUserSetting sharedUserSettingLPr =
              this.mPm.mSettings.getSharedUserSettingLPr(packageSetting);
          i3 = removePackageLPw;
        ...
        this.mPm.clearPackagePreferredActivitiesLPw(
            packageSetting.getPackageName(), sparseBooleanArray, -1);
        this.mPm.mSettings.removeRenamedPackageLPw(packageSetting.getRealName());
      }
      if (sparseBooleanArray.size() > 0) {
        new PreferredActivityHelper(this.mPm)
  ...
  public final void cleanUpResourcesLI(File file, String[] strArr) {
    ...
    if (file != null && file.exists()) {
      ParseResult parsePackageLite =
          ApkLiteParseUtils.parsePackageLite(ParseTypeImpl.forDefaultParsing().reset(), file, 0);
