...
public class PersonaServiceHelper {
  ...
  public static Map packagesForPOP =
  ...
  public static Map packagesForSF =
  ...
  public static Map packagesForDual =
  ...
  public static ArrayList DTI_PACKAGES =
      new ArrayList(
          Arrays.asList(
              "com.dti.samsung",
              "com.smg.rt",
  ...
  public static boolean isForKnoxNFC() {
    return SemPersonaManager.isKnoxVersionSupported(
            SemPersonaManager.KnoxContainerVersion.KNOX_CONTAINER_VERSION_2_4_0)
        && Binder.getCallingUid() == 1027;
  }

  public static ResolveInfo getPreferredInfoForUser(
      Computer computer,
      Intent intent,
      String str,
      long j,
    ...
    return preferredActivityHelper.findPreferredActivityNotLocked(
        computer,
        intent,
        str,
        j,
  ...
  public static boolean setLastChosenActivityForKnox(
      Computer computer,
      Intent intent,
      String str,
      long j,
    ...
    if (list.size() == 1) {
      ...
      if (userInfo != null) {
        ...
        for (UserInfo userInfo2 : getUserManager().getUsers(true)) {
          if ((userInfo2.isManagedProfile() && userInfo2.profileGroupId == i2)
              || userInfo2.id == i2) {
            Log.d(
                "PersonaServiceHelper",
                "setLastChosenActivityForKnox remove pref for " + userInfo2.id);
            try {
              preferredActivityHelper.findPreferredActivityNotLocked(
                  computer,
                  intent,
                  str,
                  j,
                  getIPackageManager()
                      .queryIntentActivities(intent, str, j, userInfo2.id)
                      .getList(),
                  false,
                  true,
                  false,
        ...
        Log.d(
            "PersonaServiceHelper",
            "setLastChosenActivityForKnox setting preferred activity for " + intValue);
        preferredActivityHelper.addPreferredActivity(
            computer,
            watchedIntentFilter,
            i,
            null,
      ...
      Log.e(
          "PersonaServiceHelper",
          "setLastChosenActivityForKnox invalid size for user list: " + list.size());
    }
    return true;
  }

  public static boolean isLastChosenActivity(
      Computer computer,
      Intent intent,
      String str,
      int i,
    ...
    if (!isForKnoxNFC()
        || (integerArrayListExtra =
                intent.getIntegerArrayListExtra("com.samsung.sec.knox.EXTRA_KNOX_ARRAY"))
            == null) {
      return false;
    }
    Log.d("PersonaServiceHelper", "Set last chosen activity with user list.");
    if (!setLastChosenActivityForKnox(
        computer,
        intent,
        str,
        i,
  ...
  public static Pair getLastChosenActivity(
      Computer computer,
      Intent intent,
      String str,
      long j,
      ...
      if (integerArrayListExtra != null) {
        for (Integer num : integerArrayListExtra) {
          ...
          try {
            list =
                getIPackageManager()
                    .queryIntentActivities(intent, str, j, num.intValue())
                    .getList();
          } catch (RemoteException e) {
            e.printStackTrace();
            list = null;
          ...
          if (list != null) {
            ...
            Log.d(
                "PersonaServiceHelper",
                "pms.getLastChosenActivity " + findPreferredActivityNotLocked + " for " + num);
  ...
  public static void trimPersonaFromInstallation(
        ...
        for (UserInfo userInfo : list) {
          if (userInfo.isManagedProfile()
            ...
            if (containsPackage(packageSetting.getPackageName(), true, userInfo)) {
              ...
              Log.d(
                  "PersonaServiceHelper",
                  "" + packageSetting.getPackageName() + " is installed in " + userInfo.id);
              z = true;
            } else {
              z = false;
            }
            if (containsPackage(packageSetting.getPackageName(), false, userInfo)) {
              ...
              Log.d(
                  "PersonaServiceHelper",
                  "" + packageSetting.getPackageName() + " is uninstalled in " + userInfo.id);
              z = true;
            }
            if (parsedPackage != null
                && parsedPackage.getPackageName() != null
                && parsedPackage.isOverlayIsStatic()
                && "android".equals(parsedPackage.getOverlayTarget())) {
              Log.d(
                  "PersonaServiceHelper",
                  ""
                      + packageSetting.getPackageName()
                      + " is (system overlay package) skip trim in "
                      + userInfo.id);
              z = true;
            }
            if (!z) {
              if (userHandle == null || !UserHandle.ALL.equals(userHandle)) {
                if (z2 && userHandle == null) {
                  Log.d(
                      "PersonaServiceHelper",
                      "New installation & target user = null or all\n set false for persona "
                          + userInfo.id);
                  packageSetting.setInstalled(false, userInfo.id);
                } else if (z3
                    && (userHandle == null
                        || UserHandle.ALL.equals(userHandle)
                        || userInfo.isDualAppProfile())) {
                  if (packageSetting2 == null) {
                    Log.d(
                        "PersonaServiceHelper",
                        "If no info about old packages trim it\n set false for persona "
                            + userInfo.id);
                    packageSetting.setInstalled(false, userInfo.id);
                  } else if (!packageSetting2.getInstalled(userInfo.id)) {
                    Log.d(
                        "PersonaServiceHelper",
                        "if already not installed, then trim it\n set false for persona "
                            + userInfo.id);
                    packageSetting.setInstalled(false, userInfo.id);
                  }
                }
                ...
                Log.d(
                    "PersonaServiceHelper",
                    "if already installed, then don't trim it\n set true for persona "
                        + userInfo.id);
                packageSetting.setInstalled(true, userInfo.id);
              } else {
                Log.d(
          ...
          if (!packageSetting.getInstalled(userInfo.id)
              && SemPersonaManager.isKnoxId(userInfo.id)
              && isRequiredAppForKnox(packageSetting, userInfo.id)) {
            packageSetting.setInstalled(true, userInfo.id);
          }
          if (!packageSetting.getInstalled(userInfo.id)
              && packageSetting2 != null
              && SemPersonaManager.isKnoxId(userInfo.id)
              && isRequiredAppForKnox(packageSetting2, userInfo.id)) {
            packageSetting.setInstalled(true, userInfo.id);
          }
          if (packageSetting2 != null
              && packageSetting2.getInstalled(userInfo.id)
              && (SemPersonaManager.isKnoxId(userInfo.id)
  ...
  public static boolean isRequiredAppForKnox(Bundle bundle, int i, String str) {
    ...
    try {
      if (!SemPersonaManager.isKnoxId(i)
          || !PmServerUtils.isMetaDataInBundle(
              bundle, "com.samsung.android.multiuser.required_managed_profile")) {
        return false;
      }
      Log.d(
          "PersonaServiceHelper",
  ...
  public static boolean isDisallowedAppForKnox(Bundle bundle, int i, String str) {
    ...
    try {
      if (SemPersonaManager.isKnoxId(i)
          && PmServerUtils.isMetaDataInBundle(
              bundle, "com.samsung.android.multiuser.disallowed_managed_profile")) {
        Log.d(
            "PersonaServiceHelper",
      ...
      if (SemPersonaManager.isDoEnabled(i)) {
        try {
          if (!SystemProperties.getBoolean("persist.sys.knox.leave_all_system_apps_enabled", false)
              && (iPackageManager = getIPackageManager()) != null
              && (installerPackageName = iPackageManager.getInstallerPackageName(str)) != null
  ...
  public static void deletePackageAsUserAndPersona(
      final String str,
      final IPackageDeleteObserver iPackageDeleteObserver,
      final int i,
      final int i2,
    ...
    if (z) {
      ...
      sb.append(
          iPackageDeleteObserver != null
              ? Integer.valueOf(iPackageDeleteObserver.hashCode())
              : "null");
      sb.append("}\npkg{");
      sb.append(str);
      sb.append("}\nuser{");
    ...
    if (UserHandle.getUserId(callingUid) != i) {
      context.enforceCallingOrSelfPermission(
          "android.permission.INTERACT_ACROSS_USERS_FULL", "deletePackage for user " + i);
    }
    ...
    handler.post(
        new Runnable() { // from class: com.android.server.pm.PersonaServiceHelper.4
          @Override // java.lang.Runnable
          public void run() {
            int i3;
            handler.removeCallbacks(this);
            try {
              if (PersonaServiceHelper.getApplicationPolicyService() != null
                  && !PersonaServiceHelper.getApplicationPolicyService()
                      .getApplicationUninstallationEnabled(
                          new ContextInfo(Binder.getCallingUid()), str)) {
            ...
            if (iPackageDeleteObserver != null) {
              try {
                Log.d(
                    "PersonaServiceHelper",
                    "return delete result to caller: " + iPackageDeleteObserver.hashCode());
        ...
        KnoxContainerManager knoxContainerManager =
            EnterpriseKnoxManager.getInstance()
                .getKnoxContainerManager(
                    context,
                    new ContextInfo(
                        new EdmStorageProvider(context).getMUMContainerOwnerUid(callingUserId),
                        callingUserId));
        if (knoxContainerManager != null
            && (containerConfigurationPolicy =
                    knoxContainerManager.getContainerConfigurationPolicy())
                != null
            && !(z = containerConfigurationPolicy.isUsbAccessEnabled())) {
          Slog.d(
              "PersonaServiceHelper",
              "isUsbHostModeEnabledForContainer: Usb Host Mode sdcard is disabled for User-"
                  + callingUserId
                  + ". Ignoring USB device access request...");
        }
      } catch (Exception e) {
  ...
  public static boolean shouldBlockUsbHostMode(Context context) {
    return SemPersonaManager.isKnoxId(UserHandle.getCallingUserId())
        && !isUsbHostModeEnabledForContainer(context);
  }
  ...
  */
  public static boolean shouldBlockBluetooth(Context context, int i) {
    boolean z;
    KnoxContainerManager knoxContainerManager;
    ...
    try {
      knoxContainerManager =
          EnterpriseKnoxManager.getInstance().getKnoxContainerManager(context, i);
    } catch (SecurityException e) {
      Slog.w("PersonaServiceHelper", "isKnoxBluetoothEnabled(): SecurityException: " + e);
    }
    if (knoxContainerManager != null) {
      z = knoxContainerManager.getContainerConfigurationPolicy().isBluetoothEnabled();
      Slog.d("PersonaServiceHelper", "BMS::isKnoxBluetoothEnabled = " + z);
      return z;
    }
    z = true;
    Slog.d("PersonaServiceHelper", "BMS::isKnoxBluetoothEnabled = " + z);
  ...
  /*
      Code decompiled incorrectly, please refer to instructions dump.
  */
  public static boolean shouldBlockBluetoothHeadset(Context context, String str, int i, int i2) {
    boolean z;
    KnoxContainerManager knoxContainerManager;
    if (ALLOWED_BLUETOOTH_TARGET.contains(str) && SemPersonaManager.isKnoxId(i) && i2 == 0) {
      try {
        knoxContainerManager =
            EnterpriseKnoxManager.getInstance().getKnoxContainerManager(context, i);
      } catch (SecurityException e) {
        Slog.w("PersonaServiceHelper", "retrieveServiceLocked(): SecurityException: " + e);
      }
      if (knoxContainerManager != null) {
        z = knoxContainerManager.getContainerConfigurationPolicy().isBluetoothEnabled();
        return z;
            ...
            getDualDARPolicyService()
                .map(
                    new Function() { // from class:
                      // com.android.server.pm.PersonaServiceHelper$$ExternalSyntheticLambda4
                      @Override // java.util.function.Function
                      public final Object apply(Object obj) {
                        Boolean lambda$isDERestrictionEnforced$0;
                        lambda$isDERestrictionEnforced$0 =
                            PersonaServiceHelper.lambda$isDERestrictionEnforced$0(
                                i, (IDualDARPolicy) obj);
  ...
  public static /* synthetic */ Boolean lambda$isDERestrictionEnforced$0(
      int i, IDualDARPolicy iDualDARPolicy) {
    Bundle bundle;
    try {
        ...
        && (bundle =
                (Bundle)
                    getDualDARPolicyService()
                        .map(
                            new Function() { // from class:
                              // com.android.server.pm.PersonaServiceHelper$$ExternalSyntheticLambda2
                              @Override // java.util.function.Function
                              public final Object apply(Object obj) {
                                Bundle lambda$isDataLockTimeoutEnabled$1;
                                lambda$isDataLockTimeoutEnabled$1 =
                                    PersonaServiceHelper.lambda$isDataLockTimeoutEnabled$1(
                                        i, (IDualDARPolicy) obj);
                        ...
                        .orElseGet(
                            new Supplier() { // from class:
                              // com.android.server.pm.PersonaServiceHelper$$ExternalSyntheticLambda3
                              @Override // java.util.function.Supplier
                              public final Object get() {
                                Bundle lambda$isDataLockTimeoutEnabled$2;
                                lambda$isDataLockTimeoutEnabled$2 =
                                    PersonaServiceHelper.lambda$isDataLockTimeoutEnabled$2(i);
                                return lambda$isDataLockTimeoutEnabled$2;
                              }
                            }))
            != null
        && bundle.getLong("dualdar-config-datalock-timeout")
            >= DualDARPolicy.getMinDataLockTimeoutManagedDevice();
  }

  public static /* synthetic */ Bundle lambda$isDataLockTimeoutEnabled$1(
      int i, IDualDARPolicy iDualDARPolicy) {
    try {
  ...
  public static /* synthetic */ Bundle lambda$isDataLockTimeoutEnabled$2(int i) {
    Log.e(
        "PersonaServiceHelper",
        "checkPackageStartable failed to acquire dualDARPolicy for user: " + i);
    return null;
  }
  ...
  public static boolean isPackageAllowlistedForDualDAR(Context context, String str, final int i) {
    ...
    Bundle bundle =
        (Bundle)
            getDualDARPolicyService()
                .map(
                    new Function() { // from class:
                      // com.android.server.pm.PersonaServiceHelper$$ExternalSyntheticLambda0
                      @Override // java.util.function.Function
                      public final Object apply(Object obj) {
                        Bundle lambda$isPackageAllowlistedForDualDAR$3;
                        lambda$isPackageAllowlistedForDualDAR$3 =
                            PersonaServiceHelper.lambda$isPackageAllowlistedForDualDAR$3(
                                i, (IDualDARPolicy) obj);
                ...
                .orElseGet(
                    new Supplier() { // from class:
                      // com.android.server.pm.PersonaServiceHelper$$ExternalSyntheticLambda1
                      @Override // java.util.function.Supplier
                      public final Object get() {
                        Bundle lambda$isPackageAllowlistedForDualDAR$4;
                        lambda$isPackageAllowlistedForDualDAR$4 =
                            PersonaServiceHelper.lambda$isPackageAllowlistedForDualDAR$4(i);
    ...
    if (bundle != null) {
      ...
      if (applicationInfo != null
          && ((applicationInfo.isDirectBootAware() || applicationInfo.isPartiallyDirectBootAware())
              && verifyPackageForDualDAR(context, i, str, arrayList))) {
        Log.d(
            "PersonaServiceHelper",
            "checkPackageStartable direct boot aware and whitelisted allowed: " + str);
        return true;
      }
    }
    return false;
  ...
  public static /* synthetic */ Bundle lambda$isPackageAllowlistedForDualDAR$3(
      int i, IDualDARPolicy iDualDARPolicy) {
    try {
      return iDualDARPolicy.getConfig(new ContextInfo(Binder.getCallingUid(), i));
    } catch (RemoteException e) {
  ...
  public static boolean isPackageAllowlistedForDEAccessForDualDAR(
      Context context, String str, final int i) {
    if (isSystemApp(i, str)) {
      Log.d("PersonaServiceHelper", "system app allowed - " + str);
      return true;
    }
    Bundle bundle =
        (Bundle)
            getDualDARPolicyService()
                .map(
                    new Function() { // from class:
                      // com.android.server.pm.PersonaServiceHelper$$ExternalSyntheticLambda5
                      @Override // java.util.function.Function
                      public final Object apply(Object obj) {
                        Bundle lambda$isPackageAllowlistedForDEAccessForDualDAR$5;
                        lambda$isPackageAllowlistedForDEAccessForDualDAR$5 =
                            PersonaServiceHelper.lambda$isPackageAllowlistedForDEAccessForDualDAR$5(
                                i, (IDualDARPolicy) obj);
                ...
                .orElseGet(
                    new Supplier() { // from class:
                      // com.android.server.pm.PersonaServiceHelper$$ExternalSyntheticLambda6
                      @Override // java.util.function.Supplier
                      public final Object get() {
                        Bundle lambda$isPackageAllowlistedForDEAccessForDualDAR$6;
                        lambda$isPackageAllowlistedForDEAccessForDualDAR$6 =
                            PersonaServiceHelper.lambda$isPackageAllowlistedForDEAccessForDualDAR$6(
                                i);
  ...
  public static /* synthetic */ Bundle lambda$isPackageAllowlistedForDEAccessForDualDAR$5(
      int i, IDualDARPolicy iDualDARPolicy) {
    try {
      return iDualDARPolicy.getConfig(new ContextInfo(Binder.getCallingUid(), i));
    } catch (RemoteException e) {
  ...
  public static boolean verifyPackageForDualDAR(Context context, int i, String str, List list) {
    ...
    return comparePackageSignature(
        context, appIdentity.getPackageName(), appIdentity.getSignature(), i);
  }
    ...
    try {
      ...
      if (packageInfo != null && (signingInfo = packageInfo.signingInfo) != null) {
        for (Signature signature : signingInfo.getApkContentsSigners()) {
          if (signature != null
              && (str2.equals(bytesToHex(computeSha256DigestBytes(signature.toByteArray())))
                  || str2.equals(signature.toCharsString()))) {
            return true;
          }
        }
      }
