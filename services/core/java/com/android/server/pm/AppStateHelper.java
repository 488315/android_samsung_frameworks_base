    ...
    public final List getDependencyPackages(List list) {
        ...
        for (ActivityManager.RunningAppProcessInfo runningAppProcessInfo :
            ...
            while (it.hasNext()) {
                ...
                if (ArrayUtils.contains(runningAppProcessInfo.pkgList, str)
            ...
            packageManagerInternal.forEachPackageState(
                    new Consumer() { // from class:
                        ...
                        public final void accept(Object obj) {
                            ...
                            if (AppStateHelper.containsAny(pkg.getUsesLibrariesSorted(), arrayList4)
                                    || AppStateHelper.containsAny(
                                            pkg.getUsesOptionalLibrariesSorted(), arrayList4)
                                    || AppStateHelper.containsAny(
                                            pkg.getUsesStaticLibrariesSorted(), arrayList5)
                                    || AppStateHelper.containsAny(
                                            pkg.getUsesSdkLibrariesSorted(), arrayList6)) {
        ...
        try {
            NetworkStats querySummary =
                    networkStatsManager.querySummary(
                            i,
                            null,
                            currentTimeMillis - ACTIVE_NETWORK_DURATION_MILLIS,
                            currentTimeMillis);
            try {
                NetworkStats.Bucket bucket = new NetworkStats.Bucket();
                while (querySummary.hasNextBucket()) {
                    querySummary.getNextBucket(bucket);
                    if (((ArrayList) list).contains(packageManager.getNameForUid(bucket.getUid()))
