...
public class SemPatternVibration extends SemVibration {
  ...
  public final HalVibration getHapticEngineVibration(int[] iArr) {
    ...
    if (this.mRepeat <= -1 || isExecutablePattern(colorfulPattern)) {
      return new HalVibration(
          this.mToken,
          this.mEffect,
          this.mVibratorHelper.getIndexDuration(this.mIndex) + 100,
          colorfulPattern,
          this.mMagnitude,
          -1,
          iArr,
  ...
  public final HalVibration getPatternVibration() {
    ...
    return new HalVibration(
        ...
        this.mVibratorHelper.getIndexDuration(this.mIndex),
        patternByIndex,
        this.mMagnitude,
  ...
  public final boolean patternValidation(long[] jArr) {
    ...
    Slog.d(
        ...
        "semPatternVibrate() - pattern.length = " + jArr.length + ", repeat = " + this.mRepeat);
  ...
  public final boolean isExecutablePattern(long[] jArr) {
    ...
    if (i >= length) {
      Slog.w(
          "VibratorManagerService",
          "isExecutablePattern() - length = " + length + ", repeat = " + this.mRepeat);
      return false;
    }
    if (jArr[i] > 0) {
      return true;
    }
    if ((((i / 2) * 2 == i && (length / 2) * 2 == length)
            || ((i / 2) * 2 != i && (length / 2) * 2 != length))
        && jArr[length - 1] > 0) {
      return true;
    }
