...
public abstract class Vibration {
  ...
  public Vibration(IBinder iBinder, CallerInfo callerInfo) {
    ...
    this.f1733id = sNextVibrationId.getAndIncrement();
  ...
  public final class CallerInfo {
    ...
    public boolean equals(Object obj) {
      ...
      return Objects.equals(this.attrs, callerInfo.attrs)
          ...
          && this.displayId == callerInfo.displayId
    ...
    public int hashCode() {
      return Objects.hash(
          ...
          Integer.valueOf(this.displayId),
          this.opPkg,
          this.reason);
    }

    public String toString() {
      return "CallerInfo{ attrs="
          + this.attrs
          + ", uid="
          + this.uid
          + ", displayId="
  ...
  public final class DebugInfo {
    ...
    public String toString() {
      ...
      sb.append(", endTime: ");
      sb.append(
          this.mEndTime == 0 ? null : Vibration.DEBUG_DATE_FORMAT.format(new Date(this.mEndTime)));
    ...
    public final void dumpEffect(
      ...
      for (int i = 0; i < sequential.getEffects().size(); i++) {
        ...
        if (combinedVibration instanceof CombinedVibration.Mono) {
          ...
          dumpEffect(
              protoOutputStream, 2246267895809L, (CombinedVibration.Stereo) combinedVibration);
        }
        protoOutputStream.write(
            2220498092034L, ((Integer) sequential.getDelays().get(i)).intValue());
      }
      protoOutputStream.end(start);
    }

    public final void dumpEffect(
        ProtoOutputStream protoOutputStream, long j, CombinedVibration.Mono mono) {
      long start = protoOutputStream.start(j);
    ...
    public final void dumpEffect(
        ProtoOutputStream protoOutputStream, long j, CombinedVibration.Stereo stereo) {
      long start = protoOutputStream.start(j);
      for (int i = 0; i < stereo.getEffects().size(); i++) {
        protoOutputStream.write(2220498092034L, stereo.getEffects().keyAt(i));
        dumpEffect(
            protoOutputStream, 2246267895809L, (VibrationEffect) stereo.getEffects().valueAt(i));
      }
      protoOutputStream.end(start);
    }

    public final void dumpEffect(
        ProtoOutputStream protoOutputStream, long j, VibrationEffect vibrationEffect) {
      long start = protoOutputStream.start(j);
    ...
    public final void dumpEffect(
        ProtoOutputStream protoOutputStream, long j, StepSegment stepSegment) {
      long start = protoOutputStream.start(j);
    ...
    public final void dumpEffect(
        ProtoOutputStream protoOutputStream, long j, RampSegment rampSegment) {
      long start = protoOutputStream.start(j);
    ...
    public final void dumpEffect(
        ProtoOutputStream protoOutputStream, long j, PrebakedSegment prebakedSegment) {
      long start = protoOutputStream.start(j);
    ...
    public final void dumpEffect(
        ProtoOutputStream protoOutputStream, long j, PrimitiveSegment primitiveSegment) {
      long start = protoOutputStream.start(j);
