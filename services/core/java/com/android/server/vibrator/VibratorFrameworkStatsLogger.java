...
public class VibratorFrameworkStatsLogger {
  ...
  public void writeVibratorStateOnAsync(final int i, final long j) {
    this.mHandler.post(
        new Runnable() { // from class:
          // com.android.server.vibrator.VibratorFrameworkStatsLogger$$ExternalSyntheticLambda2
          @Override // java.lang.Runnable
          public final void run() {
  ...
  public void writeVibrationReportedAsync(VibrationStats.StatsInfo statsInfo) {
    ...
    synchronized (this.mLock) {
      ...
      max =
          Math.max(
              0L,
              (this.mLastVibrationReportedLogUptime + this.mVibrationReportedLogIntervalMillis)
  ...
  public final void lambda$new$0() {
    ...
    if (statsInfo == null) {
      Slog.w(
          "VibratorFrameworkStatsLogger",
          "Unexpected vibration metric flush with empty queue. Ignoring.");
    } else {
      statsInfo.writeVibrationReported();
    }
    if (z) {
      this.mHandler.postDelayed(
          this.mConsumeVibrationStatsQueueRunnable, this.mVibrationReportedLogIntervalMillis);
    }
  }
}
