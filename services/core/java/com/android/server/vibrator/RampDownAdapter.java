...
public final class RampDownAdapter implements VibrationEffectAdapters.SegmentsAdapter {
  ...
  public int apply(List list, int i, VibratorInfo vibratorInfo) {
    return this.mRampDownDuration <= 0
        ? i
        : addRampDownToLoop(list, addRampDownToZeroAmplitudeSegments(list, i));
  }
  ...
  public final int addRampDownToZeroAmplitudeSegments(List list, int i) {
    ...
    while (i2 < size) {
      ...
      if (isOffSegment((VibrationEffectSegment) list.get(i2))
          && endsWithNonZeroAmplitude(stepSegment)) {
        ...
        if (stepSegment instanceof StepSegment) {
          ...
          list2 =
              createStepsDown(stepSegment2.getAmplitude(), stepSegment2.getFrequencyHz(), duration);
          ...
          if (duration <= i3) {
            ...
            list2 =
                Arrays.asList(
                    createRampDown(endAmplitude, endFrequencyHz, i3),
                    createRampDown(
                        DisplayPowerController2.RATE_FROM_DOZE_TO_ON,
                        endFrequencyHz,
                        duration - this.mRampDownDuration));
          }
        } else {
          list2 = null;
        }
  ...
  public final int addRampDownToLoop(List list, int i) {
    ...
    if (endsWithNonZeroAmplitude((VibrationEffectSegment) list.get(size))
        && isOffSegment((VibrationEffectSegment) list.get(i))) {
      ...
      if (stepSegment instanceof StepSegment) {
        ...
        list.addAll(
            createStepsDown(
                stepSegment2.getAmplitude(),
                stepSegment2.getFrequencyHz(),
                Math.min(duration, this.mRampDownDuration)));
      } else if (stepSegment instanceof RampSegment) {
        RampSegment rampSegment = (RampSegment) stepSegment;
        list.add(
            createRampDown(
                rampSegment.getEndAmplitude(),
                rampSegment.getEndFrequencyHz(),
                Math.min(duration, this.mRampDownDuration)));
      }
    }
    return i;
  ...
  public final List createStepsDown(float f, float f2, long j) {
    ...
    arrayList.add(
        new StepSegment(
            DisplayPowerController2.RATE_FROM_DOZE_TO_ON,
            f2,
            ((int) j) - (this.mStepDuration * (min - 1))));
    return arrayList;
  }
  ...
  public static VibrationEffectSegment updateDuration(
      VibrationEffectSegment vibrationEffectSegment, long j) {
    if (vibrationEffectSegment instanceof RampSegment) {
      ...
      return new RampSegment(
          ...
          rampSegment.getEndAmplitude(),
          rampSegment.getStartFrequencyHz(),
          rampSegment.getEndFrequencyHz(),
          (int) j);
    }
  ...
  public static boolean isOffSegment(VibrationEffectSegment vibrationEffectSegment) {
    if (vibrationEffectSegment instanceof StepSegment) {
      return ((StepSegment) vibrationEffectSegment).getAmplitude()
          == DisplayPowerController2.RATE_FROM_DOZE_TO_ON;
    }
    if (!(vibrationEffectSegment instanceof RampSegment)) {
      return false;
    ...
    RampSegment rampSegment = (RampSegment) vibrationEffectSegment;
    return rampSegment.getStartAmplitude() == DisplayPowerController2.RATE_FROM_DOZE_TO_ON
        && rampSegment.getEndAmplitude() == DisplayPowerController2.RATE_FROM_DOZE_TO_ON;
  }
  ...
  public static boolean endsWithNonZeroAmplitude(VibrationEffectSegment vibrationEffectSegment) {
    return vibrationEffectSegment instanceof StepSegment
        ? ((StepSegment) vibrationEffectSegment).getAmplitude()
            != DisplayPowerController2.RATE_FROM_DOZE_TO_ON
        : (vibrationEffectSegment instanceof RampSegment)
            && ((RampSegment) vibrationEffectSegment).getEndAmplitude()
                != DisplayPowerController2.RATE_FROM_DOZE_TO_ON;
  }
}
