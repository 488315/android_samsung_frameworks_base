...
public class SemCustomVibration extends SemVibration {
  ...
  */
  public HalVibration getVibration() {
      ...
      try {
        query =
            this.mContext
                .getContentResolver()
                .query(
                    Uri.parse(this.mSemHapticSegment.getCategoryPath()),
                    null,
                    "vibration_pattern=?",
                    new String[] {Integer.toString(this.mIndex)},
                    null);
      } finally {
          ...
          if (columnIndex >= 0) {
            ...
            vibrationEffect =
                VibrationEffect.createWaveform(
                    getCustomPatternData(string),
                    getCustomAmplitudeData(string, this.mMagnitude),
                    this.mRepeat);
          }
          if (query != null) {
            query.close();
    ...
    vibrationEffect =
        VibrationEffect.semCreateHaptic(this.mSemHapticSegment.getDefaultSepIndex(), this.mRepeat);
    if (query != null) {}
    if (vibrationEffect != null) {}
    return new HalVibration(
        this.mToken,
