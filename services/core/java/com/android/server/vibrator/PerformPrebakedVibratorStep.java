...
public final class PerformPrebakedVibratorStep extends AbstractVibratorStep {
  public PerformPrebakedVibratorStep(
  ...
  public List play() {
    ...
    try {
      ...
      if (!(prebakedSegment instanceof PrebakedSegment)) {
        Slog.w(
            "VibrationThread",
            "Ignoring wrong segment for a PerformPrebakedVibratorStep: " + prebakedSegment);
        return nextSteps(1);
      }
      ...
      long m108on = this.controller.m108on(prebakedSegment2, getVibration().f1733id);
      handleVibratorOnResult(m108on);
      getVibration().stats.reportPerformEffect(m108on, prebakedSegment2);
      if (m108on == 0
