...
public final class VirtualVibSoundHelper {
    ...
    public final void playVirtualSoundIfNeeded(
            String str, VibrationAttributes vibrationAttributes, boolean z) {
        if (z) {
            if (vibrationAttributes.hasTag("VIRTUAL_VIB_SOUND")
                    || this.mAudioSettingsHelper.checkAppCategory(
                            str, "virtual_vibration_sound_allowance")) {
                            ...
                            if (audioManager != null) {
                                if (audioManager.getRingerModeInternal() == 1) {
                                    ...
                                    for (AudioDeviceInfo audioDeviceInfo :
                                            vibrationCustomSettings.mAudioManager.getDevices(2)) {
                                        ...
                                        Log.d(
                                                ...
                                                "skip virtual sound: connected A2DP or"
                                                    + " USB_HEADSET");
                                    }
                                    Log.d(
                                            "VibratorManagerService",
                                            "sco Devices:"
                                                    + i
                                                    + ", only watch connected:"
                                                    + vibrationCustomSettings.mOnlyWatchConnected);
                                    if (i == 0
                                            || (i == 1
                ...
                if (!z2) {
                    Slog.d(
                            "VibratorManagerService",
                            "skip virtual sound: shouldSoundForRingerMode false");
                    return;
                }
                ...
                float loadSystemSetting =
                        r10.loadSystemSetting(r11, "VIB_RECVCALL_MAGNITUDE")
                                / (this.mSettings.mCustomSettings.LEVEL_TO_MAGNITUDE.length - 1);
                if (loadSystemSetting <= FullScreenMagnificationGestureHandler.MAX_SCALE) {
                    return;
                }
                Slog.d(
                        "VibratorManagerService",
                        "play virtual sound: " + loadSystemSetting + " volume");
