...
public final class VibrationStats {
  ...
  public void reportComposePwle(long j, RampSegment[] rampSegmentArr) {
    ...
    if (j > 0) {
      for (RampSegment rampSegment : rampSegmentArr) {
        if (rampSegment.getStartAmplitude() == DisplayPowerController2.RATE_FROM_DOZE_TO_ON
  ...
  public final class StatsInfo {
    ...
    public StatsInfo(
        int i, int i2, int i3, Vibration.Status status, VibrationStats vibrationStats, long j) {
      this.uid = i;
      ...
      if (vibrationStats.hasStarted()) {
        this.startLatencyMillis =
            (int)
                Math.max(
                    0L, vibrationStats.mStartUptimeMillis - vibrationStats.mCreateUptimeMillis);
    ...
    public void writeVibrationReported() {
      if (this.mIsWritten) {
        Slog.wtf(
            "VibrationStats", "Writing same vibration stats multiple times for uid=" + this.uid);
      }
      ...
      FrameworkStatsLog.write_non_chained(
          ...
          this.endedBySameUid,
          this.endedByUsage,
          this.interruptedUsage,
          this.repeatCount,
          this.totalDurationMillis,
          this.vibratorOnMillis,
          this.startLatencyMillis,
          this.endLatencyMillis,
          this.halComposeCount,
          this.halComposePwleCount,
          this.halOnCount,
          this.halOffCount,
          this.halPerformCount,
          this.halSetAmplitudeCount,
          this.halSetExternalControlCount,
          this.halSupportedCompositionPrimitivesUsed,
          this.halSupportedEffectsUsed,
          this.halUnsupportedCompositionPrimitivesUsed,
          this.halUnsupportedEffectsUsed,
          this.halCompositionSize,
          this.halPwleSize);
    }
