...
public final class StepToRampAdapter implements VibrationSegmentsAdapter {
    public static RampSegment convertStepToRamp(
            VibratorInfo vibratorInfo, StepSegment stepSegment) {
        float frequencyHz = stepSegment.getFrequencyHz();
        if (!Float.isNaN(vibratorInfo.getResonantFrequencyHz())
                && frequencyHz == FullScreenMagnificationGestureHandler.MAX_SCALE) {
    ...
    public final int adaptToVibrator(VibratorInfo vibratorInfo, List list, int i) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            ...
            if (stepSegment instanceof StepSegment) {
                ...
                if (stepSegment2.getFrequencyHz()
                ...
                for (int i4 = i3 - 1;
                        i4 >= 0
                                && (((VibrationEffectSegment) arrayList.get(i4))
                                        instanceof StepSegment);
                        i4--) {
                    arrayList.set(
                            i4, convertStepToRamp(vibratorInfo, (StepSegment) arrayList.get(i4)));
                }
                for (int i5 = i3 + 1;
                        i5 < size
                                && (((VibrationEffectSegment) arrayList.get(i5))
                                        instanceof StepSegment);
                        i5++) {
                    arrayList.set(
                            i5, convertStepToRamp(vibratorInfo, (StepSegment) arrayList.get(i5)));
                }
            }
        }
        return i;
