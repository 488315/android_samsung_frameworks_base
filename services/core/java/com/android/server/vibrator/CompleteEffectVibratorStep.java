  ...
  public List cancel() {
    if (this.mCancelled) {
      return Arrays.asList(
          new TurnOffVibratorStep(this.conductor, SystemClock.uptimeMillis(), this.controller));
    }
    return super.cancel();
  }
    ...
    try {
      ...
      long min =
          Math.min(
              ...
              this.conductor.vibrationSettings.getRampDownDuration());
      ...
      if (currentAmplitude >= 0.001f && min > rampStepDuration) {
        ...
        return Arrays.asList(
            new RampOffVibratorStep(
                ...
                f,
                this.controller,
                this.mCancelled ? uptimeMillis + min : this.mPendingVibratorOffDeadline));
      }
      if (!this.mCancelled) {
        return Arrays.asList(
            new TurnOffVibratorStep(
                this.conductor, this.mPendingVibratorOffDeadline, this.controller));
      }
      stopVibrating();
      return VibrationStepConductor.EMPTY_STEP_LIST;
    } finally {
