...
public abstract class UserspaceRebootLogger {
  public static void noteUserspaceRebootWasRequested() {
    if (!PowerManager.isRebootingUserspaceSupportedImpl()) {
      Slog.wtf(
          "UserspaceRebootLogger",
          "noteUserspaceRebootWasRequested: Userspace reboot is not supported.");
    } else {
      SystemProperties.set("persist.sys.userspace_reboot.log.should_log", "1");
      SystemProperties.set(
          "sys.userspace_reboot.log.last_started", String.valueOf(SystemClock.elapsedRealtime()));
    }
  }
  ...
  public static void noteUserspaceRebootSuccess() {
    if (!PowerManager.isRebootingUserspaceSupportedImpl()) {
      Slog.wtf(
          "UserspaceRebootLogger",
          "noteUserspaceRebootSuccess: Userspace reboot is not supported.");
    } else {
      SystemProperties.set(
          "sys.userspace_reboot.log.last_finished", String.valueOf(SystemClock.elapsedRealtime()));
    }
  }
  ...
  public static void logEventAsync(boolean z, Executor executor) {
    ...
    final long j =
        computeOutcome == 1
            ? SystemProperties.getLong("sys.userspace_reboot.log.last_finished", 0L)
                - SystemProperties.getLong("sys.userspace_reboot.log.last_started", 0L)
            : 0L;
    final int i = z ? 1 : 2;
    executor.execute(
        new Runnable() { // from class:
          // com.android.server.UserspaceRebootLogger$$ExternalSyntheticLambda0
  ...
  public static /* synthetic */ void lambda$logEventAsync$0(int i, long j, int i2) {
    Slog.i(
        "UserspaceRebootLogger",
        "Logging UserspaceRebootReported atom: { outcome: "
            + i
            + " durationMillis: "
            + j
            + " encryptionState: "
  ...
  public static int computeOutcome() {
    ...
    if (emptyIfNull.startsWith("userspace_failed,watchdog_fork")
        || emptyIfNull.startsWith("userspace_failed,shutdown_aborted")) {
      return 2;
    }
    if (emptyIfNull.startsWith("mount_userdata_failed")
        || emptyIfNull.startsWith("userspace_failed,init_user0")
        || emptyIfNull.startsWith("userspace_failed,enablefilecrypto")) {
      return 3;
    }
