...
public class SoundTriggerMiddlewarePermission implements ISoundTriggerMiddlewareInternal, Dumpable {
  ...
  public SoundTriggerMiddlewarePermission(
      ISoundTriggerMiddlewareInternal iSoundTriggerMiddlewareInternal, Context context) {
  ...
  public SoundTriggerModuleDescriptor[] listModules() {
    enforcePermissionForPreflight(
        this.mContext, getIdentity(), "android.permission.CAPTURE_AUDIO_HOTWORD");
  ...
  public final void enforcePermissionsForPreflight(Identity identity) {
    ...
    enforcePermissionForPreflight(
        this.mContext, identity, "android.permission.CAPTURE_AUDIO_HOTWORD");
  }
  ...
  public void enforcePermissionsForDataDelivery(Identity identity, String str) {
    ...
    enforcePermissionForDataDelivery(
        this.mContext, identity, "android.permission.CAPTURE_AUDIO_HOTWORD", str);
  }
  ...
  public static void enforcePermissionForDataDelivery(
    ...
    if (PermissionUtil.checkPermissionForDataDelivery(context, identity, str, str2) != 0) {
      throw new SecurityException(
          String.format(
              "Failed to obtain permission %s for identity %s",
              str, ObjectPrinter.print(identity, 16)));
    }
  }

  public static void enforceSoundTriggerRecordAudioPermissionForDataDelivery(
      Identity identity, String str) {
    if (((LegacyPermissionManagerInternal)
                LocalServices.getService(LegacyPermissionManagerInternal.class))
            .checkSoundTriggerRecordAudioPermissionForDataDelivery(
                identity.uid, identity.packageName, identity.attributionTag, str)
        != 0) {
      throw new SecurityException(
          String.format(
              "Failed to obtain permission RECORD_AUDIO for identity %s",
  ...
  public static void enforcePermissionForPreflight(Context context, Identity identity, String str) {
    int checkPermissionForPreflight =
        PermissionUtil.checkPermissionForPreflight(context, identity, str);
  ...
  public class ModuleWrapper extends ISoundTriggerModule.Stub {
    ...
    public ModuleWrapper(
        Identity identity, ISoundTriggerCallback iSoundTriggerCallback, boolean z) {
    ...
    public final void enforcePermissions() {
      SoundTriggerMiddlewarePermission.this.enforcePermissionsForPreflight(
          this.mOriginatorIdentity);
    }
    ...
    public class CallbackWrapper implements ISoundTriggerCallback {
      ...
      public final void enforcePermissions(String str) {
        if (ModuleWrapper.this.mIsTrusted) {
          ...
          SoundTriggerMiddlewarePermission.this.enforcePermissionsForPreflight(
              moduleWrapper.mOriginatorIdentity);
        } else {
          ModuleWrapper moduleWrapper2 = ModuleWrapper.this;
          SoundTriggerMiddlewarePermission.this.enforcePermissionsForDataDelivery(
              moduleWrapper2.mOriginatorIdentity, str);
        }
      }
    }
  }
