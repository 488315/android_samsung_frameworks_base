...
public class CustomizedBinderCallsStatsInternal {
  ...
  public void reportCpuUsage(final int i) {
    if (!isPassedInterval(this.mLastStoreTime, 55000L)) {
      Slog.i(
          "CustomizedBinderCallsStatsInternal", "1 minute has NOT pass since last binder stats.");
      return;
    }
    ...
    BackgroundThread.getHandler()
        .post(
            new Runnable() { // from class:
              // com.android.server.CustomizedBinderCallsStatsInternal$$ExternalSyntheticLambda0
  ...
  public final void leaveAtraceLog(int i) {
    if (System.currentTimeMillis() - SystemProperties.getLong("ro.runtime.firstboot", 0L) < 600000
        || !isPackageInstalled("com.salab.issuetracker")
        || i < 10
        || !isPassedInterval(
            this.mLastAtraceTime,
  ...
  public final void detectHeaviestApp(int i) {
    ...
    if (i < 20
        || (heaviestApplicationUid = this.mBinderCallsStats.getHeaviestApplicationUid(90))
            == null) {
      return;
    }
    Slog.i(
        ...
        "heavy binder caller : "
            + heaviestApplicationUid.mPackageName
            + "("
            + heaviestApplicationUid.mUid
            + ")");
    if (isPassedInterval(
        this.mLastNotifyTime, BackupManagerConstants.DEFAULT_FULL_BACKUP_INTERVAL_MILLISECONDS)) {
  ...
  public final void sendAnomalyAppToHWParam(
      BinderCallsStats.HeavyBinderCallerInfo heavyBinderCallerInfo, int i) {
    ...
    if (semHqmManager != null) {
      ...
      sb.append(
          "\"RATIO\":\""
              + String.format("%.2f", Float.valueOf(heavyBinderCallerInfo.mRatio))
              + "\",");
      int length = sb.length() + heavyBinderCallerInfo.mExtraInfo.length() + 10;
      if (length > 1000) {
        sb.append(
            "\"EXTRA\":\""
                + heavyBinderCallerInfo.mExtraInfo.substring(
                    0, heavyBinderCallerInfo.mExtraInfo.length() - (length - 1000))
                + "\"");
      } else {
      ...
      Slog.e(
          "CustomizedBinderCallsStatsInternal",
          "Failed to send anomaly application info. to HWParam");
      return;
    }
