...
public final class MediaButtonReceiverHolder {
  ...
  public static MediaButtonReceiverHolder unflattenFromString(Context context, String str) {
    ...
    if (TextUtils.isEmpty(str)
        ...
        || ((split.length != 2 && split.length != 3)
            || (unflattenFromString = ComponentName.unflattenFromString(split[0])) == null)) {
      return null;
    }
  ...
  public boolean send(
    ...
    makeBasic.setTemporaryAppAllowlist(j, 0, FrameworkStatsLog.f89x74364b3b, "");
    makeBasic.setBackgroundActivityStartsAllowed(true);
    if (this.mPendingIntent != null) {
      Log.d(
          "PendingIntentHolder",
          "Sending " + keyEvent + " to the last known PendingIntent " + this.mPendingIntent);
      try {
        this.mPendingIntent.send(
            context, i, intent, onFinished, handler, null, makeBasic.toBundle());
      } catch (PendingIntent.CanceledException e) {
        Log.w(
            "PendingIntentHolder",
            "Error sending key event to media button receiver " + this.mPendingIntent,
            e);
        return false;
      }
      ...
      try {
        ...
        Log.w(
            "PendingIntentHolder",
            "Error sending media button to the restored intent "
                + this.mComponentName
                + ", type="
                + this.mComponentType,
            e2);
      ...
      Log.e(
          "PendingIntentHolder",
          "Shouldn't be happen -- pending intent or component name must be set");
      return false;
    }
    return true;
  }
  ...
  public String flattenToString() {
    ...
    return componentName == null
        ...
        : String.join(
            ...
            String.valueOf(this.mComponentType));
  }
  ...
  public static ComponentName getComponentName(PendingIntent pendingIntent, int i) {
    ...
    while (it.hasNext()) {
      ...
      if (componentInfo != null
          && TextUtils.equals(componentInfo.packageName, pendingIntent.getCreatorPackage())
