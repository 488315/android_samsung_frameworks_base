...
public class MediaShellCommand extends ShellCommand {
  ...
  public int onCommand(String str) {
    ...
    if (sThread == null) {
      ...
      sMediaSessionManager =
          (MediaSessionManager)
              currentActivityThread.getSystemContext().getSystemService("media_session");
    }
  ...
  public class ControllerMonitor {
    ...
    public void run() {
      ...
      boolean z;
      printUsageMessage();
      HandlerThread handlerThread = new HandlerThread("MediaCb") { // from class:
            // com.android.server.media.MediaShellCommand.ControllerMonitor.1
            @Override // android.os.HandlerThread
            public void onLooperPrepared() {
              try {
                ControllerMonitor.this.mController.registerCallback(
                    ControllerMonitor.this.mControllerCallback);
              } catch (RuntimeException unused) {
        ...
        try {
          try {
            BufferedReader bufferedReader =
                new BufferedReader(new InputStreamReader(MediaShellCommand.this.mInput));
            while (true) {
