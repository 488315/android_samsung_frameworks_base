...
public class MediaSession2Record implements MediaSessionRecordImpl {
  ...
  public MediaSession2Record(
      Session2Token session2Token, MediaSessionService mediaSessionService, Looper looper, int i) {
    ...
    synchronized (obj) {
      ...
      this.mController =
          new MediaController2.Builder(mediaSessionService.getContext(), session2Token)
              .setControllerCallback(handlerExecutor, new Controller2Callback())
              .build();
      this.mPolicies = i;
    }
  }
  ...
  public class Controller2Callback extends MediaController2.ControllerCallback {
    ...
    public void onPlaybackActiveChanged(MediaController2 mediaController2, boolean z) {
      ...
      if (MediaSession2Record.DEBUG) {
        Log.d(
            "MediaSession2Record",
            "playback active changed, " + MediaSession2Record.this.mSessionToken + ", active=" + z);
      }
      synchronized (MediaSession2Record.this.mLock) {
        mediaSessionService = MediaSession2Record.this.mService;
      }
