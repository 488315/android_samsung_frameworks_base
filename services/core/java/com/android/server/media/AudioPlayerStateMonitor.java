...
public class AudioPlayerStateMonitor {
  ...
  public void cleanUpAudioPlaybackUids(int i) {
    synchronized (this.mLock) {
      ...
      for (int size = this.mSortedAudioPlaybackClientUids.size() - 1;
          size >= 0 && ((Integer) this.mSortedAudioPlaybackClientUids.get(size)).intValue() != i;
          size--) {
        int intValue = ((Integer) this.mSortedAudioPlaybackClientUids.get(size)).intValue();
        if (identifier == UserHandle.getUserHandleForUid(intValue).getIdentifier()
            && !isPlaybackActive(intValue)) {
  ...
  public final void sendAudioPlayerActiveStateChangedMessageLocked(
      AudioPlaybackConfiguration audioPlaybackConfiguration, boolean z) {
    Iterator it = this.mListenerMap.values().iterator();
    while (it.hasNext()) {
      ((MessageHandler) it.next())
          .sendAudioPlayerActiveStateChangedMessage(audioPlaybackConfiguration, z);
    }
  }
      ...
      synchronized (AudioPlayerStateMonitor.this.mLock) {
        ...
        while (it.hasNext()) {
          ...
          if (audioPlaybackConfiguration.isActive()) {
            AudioPlayerStateMonitor.this.mActiveAudioUids.add(
        ...
        for (int i2 = 0; i2 < arrayMap.size(); i2++) {
          ...
          if (!AudioPlayerStateMonitor.this.mPrevActiveAudioPlaybackConfigs.containsKey(
              Integer.valueOf(audioPlaybackConfiguration2.getPlayerInterfaceId()))) {
            Log.d(
                AudioPlayerStateMonitor.TAG,
                "Found a new active media playback. " + audioPlaybackConfiguration2);
            int indexOf =
                AudioPlayerStateMonitor.this.mSortedAudioPlaybackClientUids.indexOf(
                    Integer.valueOf(clientUid));
            if (indexOf != 0) {
              if (indexOf > 0) {
                AudioPlayerStateMonitor.this.mSortedAudioPlaybackClientUids.remove(indexOf);
              }
              AudioPlayerStateMonitor.this.mSortedAudioPlaybackClientUids.add(
                  0, Integer.valueOf(clientUid));
            }
          }
        }
        if (AudioPlayerStateMonitor.this.mActiveAudioUids.size() > 0) {
          ...
          if (!audioPlayerStateMonitor.mActiveAudioUids.contains(
              audioPlayerStateMonitor.mSortedAudioPlaybackClientUids.get(0))) {
            int i3 = 1;
            while (true) {
              if (i3 >= AudioPlayerStateMonitor.this.mSortedAudioPlaybackClientUids.size()) {
                ...
                i =
                    ((Integer) AudioPlayerStateMonitor.this.mSortedAudioPlaybackClientUids.get(i3))
                        .intValue();
        ...
        while (it2.hasNext()) {
          ...
          if ((AudioPlayerStateMonitor.this.mPrevActiveAudioPlaybackConfigs.remove(
                      Integer.valueOf(audioPlaybackConfiguration3.getPlayerInterfaceId()))
                  != null)
              != audioPlaybackConfiguration3.isActive()) {
            AudioPlayerStateMonitor.this.sendAudioPlayerActiveStateChangedMessageLocked(
                audioPlaybackConfiguration3, false);
          }
        }
        Iterator it3 =
            AudioPlayerStateMonitor.this.mPrevActiveAudioPlaybackConfigs.values().iterator();
        while (it3.hasNext()) {
          AudioPlayerStateMonitor.this.sendAudioPlayerActiveStateChangedMessageLocked(
              (AudioPlaybackConfiguration) it3.next(), true);
        }
