...
public final class RemoteDisplayProviderWatcher {
  ...
  public final BroadcastReceiver mScanPackagesReceiver = new BroadcastReceiver() { // from class:
      ...
      Iterator it =
          this.mPackageManager
              .queryIntentServicesAsUser(
                  new Intent("com.android.media.remotedisplay.RemoteDisplayProvider"),
                  0,
                  this.mUserId)
              .iterator();
      while (it.hasNext()) {
        ...
        if (serviceInfo != null && verifyServiceTrusted(serviceInfo)) {
          ...
          if (findProvider < 0) {
            ...
            RemoteDisplayProviderProxy remoteDisplayProviderProxy2 =
                (RemoteDisplayProviderProxy) this.mProviders.get(findProvider);
      ...
      if (i2 < this.mProviders.size()) {
        for (int size = this.mProviders.size() - 1; size >= i2; size--) {
          RemoteDisplayProviderProxy remoteDisplayProviderProxy3 =
              (RemoteDisplayProviderProxy) this.mProviders.get(size);
  ...
  public final boolean verifyServiceTrusted(ServiceInfo serviceInfo) {
    ...
    if (str == null || !str.equals("android.permission.BIND_REMOTE_DISPLAY")) {
      Slog.w(
          ...
          "Ignoring remote display provider service because it did not require the"
              + " BIND_REMOTE_DISPLAY permission in its manifest: "
    ...
    if (this.mPackageManager.checkPermission(
            "android.permission.REMOTE_DISPLAY_PROVIDER", serviceInfo.packageName)
        == 0) {
      return true;
    }
    Slog.w(
        "RemoteDisplayProvider",
        "Ignoring remote display provider service because it does not have the"
