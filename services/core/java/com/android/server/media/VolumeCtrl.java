        ...
        if (c3 == 2) {
          ...
          switch (c) {
            ...
            case 2:
              break;
            default:
              mediaShellCommand.showError(
                  "Error: no valid volume adjustment, was " + str2 + ", expected lower|same|raise");
              return;
          }
          ...
          if (c3 == 1
              && (i3 > asInterface.getStreamMaxVolume(i2)
                  || i3 < asInterface.getStreamMinVolume(i2))) {
            mediaShellCommand.showError(
                String.format(
                    "Error: invalid volume index %d for stream %d (should be in [%d..%d])",
                    Integer.valueOf(i3),
                    Integer.valueOf(i2),
                    Integer.valueOf(asInterface.getStreamMinVolume(i2)),
                    Integer.valueOf(asInterface.getStreamMaxVolume(i2))));
            return;
          }
          ...
          if (c3 != 1) {
            ...
            str = "[V]";
            asInterface.setStreamVolumeForDeviceWithAttribution(
                i2, i3, i5, name, (String) null, i4);
          } else {
            str = "[V]";
          ...
          if (z) {
            if (i4 != 0) {
              mediaShellCommand.log(
                  str,
                  "device : "
                      + AudioSystem.getOutputDeviceName(i4)
                      + " stream("
                      + streamName(i2)
                      + ") volume is "
                      + AudioSystem.getStreamVolumeIndex(i2, i4));
            } else {
              mediaShellCommand.log(
                  str,
                  "volume is "
                      + asInterface.getStreamVolume(i2)
                      + " in range ["
                      + asInterface.getStreamMinVolume(i2)
                      + ".."
                      + asInterface.getStreamMaxVolume(i2)
                      + "]");
            }
          }
          if (z3) {
            ...
            if (i3 > asInterface.getStreamMaxVolume(3) * 10 || i3 < 0) {
              mediaShellCommand.showError(
                  String.format(
                      "Error: invalid volume index %d(should be in [0..%d])",
                      Integer.valueOf(i3),
                      Integer.valueOf(asInterface.getStreamMaxVolume(3) * 10)));
              return;
            } else {
