        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() { // from class: com.android.server.media.projection.MediaProjectionManagerService.3
            ...
            public void run() {
                ...
                if (SystemProperties.get("ro.build.characteristics", "phone").contains("tablet")) {
                    Toast.makeText(new ContextThemeWrapper(MediaProjectionManagerService.this.mContext, R.style.Theme.DeviceDefault.Light), MediaProjectionManagerService.this.mContext.getString(17043288, activeDisplay.getDeviceName()), 1).show();
                } else {
                    Toast.makeText(new ContextThemeWrapper(MediaProjectionManagerService.this.mContext, R.style.Theme.DeviceDefault.Light), MediaProjectionManagerService.this.mContext.getString(17043287, activeDisplay.getDeviceName()), 1).show();
                }
            }
        });
    }
