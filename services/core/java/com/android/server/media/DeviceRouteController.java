...
public interface DeviceRouteController {
  ...
  static DeviceRouteController createInstance(
      Context context, OnDeviceRouteChangedListener onDeviceRouteChangedListener) {
    AudioManager audioManager = (AudioManager) context.getSystemService(AudioManager.class);
    IAudioService asInterface = IAudioService.Stub.asInterface(ServiceManager.getService("audio"));
    if (MediaFeatureFlagManager.getInstance()
        .getBoolean("BluetoothRouteController__enable_legacy_bluetooth_routes_controller", true)) {
      return new LegacyDeviceRouteController(
          context, audioManager, asInterface, onDeviceRouteChangedListener);
    }
