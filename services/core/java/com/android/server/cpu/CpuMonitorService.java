...
public final class CpuMonitorService extends SystemService {
  ...
  static {
    ...
    CACHE_DURATION_MILLISECONDS =
        (Build.IS_USERDEBUG || Build.IS_ENG) ? timeUnit2.toMillis(30L) : timeUnit2.toMillis(10L);
  ...
  */
  ...
  public CpuMonitorService(
    ...
    this.mLocalService =
        new CpuMonitorInternal() { // from class: com.android.server.cpu.CpuMonitorService.1
        };
  ...
  public void onStart() {
    if (!this.mCpuInfoReader.init() || this.mCpuInfoReader.readCpuInfos() == null) {
      Slogf.wtf(
          TAG,
          "Failed to initialize CPU info reader. This happens when the CPU frequency stats are not"
              + " available or the sysfs interface has changed in the Kernel. Cannot monitor CPU"
              + " without these stats. Terminating CPU monitor service");
      return;
    }
    ...
    synchronized (this.mLock) {
      if (this.mShouldDebugMonitor && !this.mHandler.hasCallbacks(this.mMonitorCpuStats)) {
        ...
        Slogf.m94i(TAG, "Starting debug monitoring");
  ...
  public final void doDump(final IndentingPrintWriter indentingPrintWriter) {
    ...
    synchronized (this.mLock) {
      ...
      if (hasClientCallbacksLocked()) {
        ...
        indentingPrintWriter.increaseIndent();
        this.mAvailabilityCallbackInfosByCallbacksByCpuset.forEach(
  ...
  public static /* synthetic */ void lambda$doDump$0(
      IndentingPrintWriter indentingPrintWriter,
      CpuAvailabilityCallbackInfo cpuAvailabilityCallbackInfo) {
  ...
  public final void monitorCpuStats() {
    ...
    synchronized (this.mLock) {
      ...
      for (int i3 = 0; i3 < this.mCpusetInfosByCpuset.size(); i3++) {
        ...
        cpusetInfo.populateLatestCpuAvailabilityInfo(
            uptimeMillis, this.mLatestAvailabilityDurationMillis);
        checkClientThresholdsAndNotifyLocked(cpusetInfo);
      }
      if (this.mCurrentMonitoringIntervalMillis > 0
          && (hasClientCallbacksLocked() || this.mShouldDebugMonitor)) {
        this.mHandler.postAtTime(
            this.mMonitorCpuStats, uptimeMillis + this.mCurrentMonitoringIntervalMillis);
      } else {
        stopMonitoringCpuStatsLocked();
      }
    }
  ...
  public final void checkClientThresholdsAndNotifyLocked(CpusetInfo cpusetInfo) {
    ...
    if (cpusetInfo.getLatestCpuAvailabilityInfo() == null
        || prevCpuAvailabilityPercent < 0
        || this.mAvailabilityCallbackInfosByCallbacksByCpuset.numElementsForKey(cpusetInfo.cpuset)
            == 0) {
      return;
    }
          ...
          new PriorityDump
            ...
            public void dumpCritical(
              ...
              if (!DumpUtils.checkDumpAndUsageStatsPermission(
                      CpuMonitorService.this.mContext, CpuMonitorService.TAG, printWriter)
                  || z) {
                return;
              }
  ...
  public final class CpusetInfo {
      ...
      if (j3 != j) {
        if (CpuMonitorService.DEBUG) {
          Slogf.m88d(
              ...
              CpuAvailabilityMonitoringConfig.toCpusetString(this.cpuset));
        }
        this.mLatestCpuAvailabilityInfo = null;
      } else {
        ...
        if (cpuAvailabilityInfo == null || cpuAvailabilityInfo.dataTimestampUptimeMillis != j3) {
          this.mLatestCpuAvailabilityInfo =
              new CpuAvailabilityInfo(
                  this.cpuset,
                  j3,
                  snapshot.getAverageAvailableCpuFreqPercent(),
                  getCumulativeAvgAvailabilityPercent(j - j2),
                  j2);
        }
      }
    }
    ...
    public int getPrevCpuAvailabilityPercent() {
      ...
      return ((Snapshot) this.mSnapshotsByUptime.valueAt(size - 2))
    ...
    public String toString() {
      return "CpusetInfo{cpuset = "
          + CpuAvailabilityMonitoringConfig.toCpusetString(this.cpuset)
          + ", mSnapshotsByUptime = "
          + this.mSnapshotsByUptime
