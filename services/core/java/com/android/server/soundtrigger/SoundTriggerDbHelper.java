    ...
    synchronized (this) {
      ...
      Cursor rawQuery =
          readableDatabase.rawQuery(
              "SELECT  * FROM st_sound_model WHERE model_uuid= '" + uuid + "'", null);
      try {
        if (!rawQuery.moveToFirst()) {
          return null;
        }
        return new SoundTrigger.GenericSoundModel(
            uuid,
            UUID.fromString(rawQuery.getString(rawQuery.getColumnIndex("vendor_uuid"))),
