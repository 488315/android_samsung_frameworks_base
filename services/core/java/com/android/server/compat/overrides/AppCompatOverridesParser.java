...
public final class AppCompatOverridesParser {
    ...
    if (str.equals("*")) {
      if (set.isEmpty()) {
        Slog.w(
            ...
            "Wildcard can't be used in 'remove_overrides' flag with an empty owned_change_ids'"
                + " flag");
        return Collections.emptyMap();
      }
      Iterator<ApplicationInfo> it =
          this.mPackageManager.getInstalledApplications(4194304).iterator();
      while (it.hasNext()) {
              ...
              ((Set)
                      arrayMap.computeIfAbsent(
                          ...
                          new Function() { // from class:
                                           // com.android.server.compat.overrides.AppCompatOverridesParser$$ExternalSyntheticLambda0
                            @Override // java.util.function.Function
                            public final Object apply(Object obj) {
                              Set lambda$parseRemoveOverrides$0;
                              lambda$parseRemoveOverrides$0 =
                                  AppCompatOverridesParser.lambda$parseRemoveOverrides$0(
                                      (String) obj);
  ...
  public static Set parseOwnedChangeIds(String str) {
    ...
    for (String str2 : str.split(",")) {
      try {
        ...
        Slog.w(
            "AppCompatOverridesParser", "Invalid change ID in 'owned_change_ids' flag: " + str2, e);
      }
    }
    return arraySet;
  }
        ...
        try {
          ...
          if (!set.contains(Long.valueOf(parseLong))) {
            ...
            if (BOOLEAN_PATTERN.matcher(str8).matches()) {
              PackageOverride.Builder enabled =
                  new PackageOverride.Builder().setEnabled(Boolean.parseBoolean(str8));
              try {
                if (!str6.isEmpty()) {
                ...
                try {
                  ...
                  if (!arrayMap.containsKey(Long.valueOf(parseLong))
                      || packageOverrideComparator.compare(
                              build, (PackageOverride) arrayMap.get(Long.valueOf(parseLong)))
                          < 0) {
                ...
                Slog.w(
                    "AppCompatOverridesParser",
                    "Invalid min/max version code in override entry: " + str5,
                    e2);
              }
            } else {
              Slog.w(
                  "AppCompatOverridesParser", "Invalid enabled string in override entry: " + str5);
            }
          }
  ...
  public static Pair extractSignatureFromConfig(String str) {
    ...
    if (asList.size() > 2) {
      Slog.w(
          "AppCompatOverridesParser", "Only one signature per config is supported. Config: " + str);
      return null;
    }
  ...
  */
  public final boolean verifySignature(String str, String str2) {
    try {
      boolean z = true;
      if (!str2.isEmpty()
          && !this.mPackageManager.hasSigningCertificate(str, HexEncoding.decode(str2), 1)) {
        z = false;
      }
      return z;
    } catch (IllegalArgumentException e) {
  ...
  public final class PackageOverrideComparator implements Comparator {
    ...
    public int compare(PackageOverride packageOverride, PackageOverride packageOverride2) {
      ...
      return Long.compare(
          getVersionProximity(packageOverride, this.mVersionCode),
          getVersionProximity(packageOverride2, this.mVersionCode));
    }
