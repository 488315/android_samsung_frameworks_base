...
public final class CompatChange extends CompatibilityChangeInfo {
  ...
  public CompatChange(Change change) {
    this(
        ...
        change.getEnableSinceTargetSdk(),
        change.getDisabled(),
        change.getLoggingOnly(),
        change.getDescription(),
        change.getOverridable());
  }
  ...
  public synchronized void addPackageOverride(
      String str,
  ...
  public synchronized boolean recheckOverride(
  ...
  public synchronized boolean removePackageOverride(
      String str, OverrideAllowedState overrideAllowedState, Long l) {
  ...
  public boolean isEnabled(
    ...
    int min =
        Math.min(applicationInfo.targetSdkVersion, androidBuildClassifier.platformTargetSdk());
    ...
    if (changeOverrides.getDeferred() != null) {
      for (OverrideValue overrideValue : changeOverrides.getDeferred().getOverrideValue()) {
        this.mRawOverrides.put(
            overrideValue.getPackageName(),
            new PackageOverride.Builder().setEnabled(overrideValue.getEnabled()).build());
      }
    }
    if (changeOverrides.getValidated() != null) {
      for (OverrideValue overrideValue2 : changeOverrides.getValidated().getOverrideValue()) {
        this.mEvaluatedOverrides.put(
            overrideValue2.getPackageName(), Boolean.valueOf(overrideValue2.getEnabled()));
        this.mRawOverrides.put(
            overrideValue2.getPackageName(),
            new PackageOverride.Builder().setEnabled(overrideValue2.getEnabled()).build());
      }
    }
    if (changeOverrides.getRaw() != null) {
      for (RawOverrideValue rawOverrideValue : changeOverrides.getRaw().getRawOverrideValue()) {
        this.mRawOverrides.put(
            rawOverrideValue.getPackageName(),
            new PackageOverride.Builder()
                .setMinVersionCode(rawOverrideValue.getMinVersionCode())
                .setMaxVersionCode(rawOverrideValue.getMaxVersionCode())
                .setEnabled(rawOverrideValue.getEnabled())
                .build());
      }
    }
  }
