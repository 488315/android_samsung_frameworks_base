...
class SensorPrivacyService$SensorPrivacyServiceImpl$$ExternalSyntheticLambda13
    ...
    public final void accept(Object obj, Object obj2) {
        ...
        SensorPrivacyService.SensorPrivacyServiceImpl.SensorUseReminderDialogInfo
        ...
        ArraySet arraySet =
                (ArraySet)
                        sensorPrivacyServiceImpl.mQueuedSensorUseReminderDialogs.get(
                                sensorUseReminderDialogInfo);
        sensorPrivacyServiceImpl.mQueuedSensorUseReminderDialogs.remove(
                sensorUseReminderDialogInfo);
        if (arraySet == null) {
            Log.e(
                    "SensorPrivacyService",
                    "Unable to show sensor use dialog because sensor set is null. Was the dialog"
                        + " queue modified from outside the handler thread?");
            return;
        }
        Intent intent = new Intent();
        intent.setComponent(
                ComponentName.unflattenFromString(
                        sensorPrivacyServiceImpl.getSensorUseActivityName(arraySet)));
        ActivityOptions makeBasic = ActivityOptions.makeBasic();
        int i = sensorUseReminderDialogInfo.mTaskId;
        if (i == -1) {
            SensorPrivacyService sensorPrivacyService = SensorPrivacyService.this;
            makeBasic.setLaunchDisplayId(
                    (sensorPrivacyService.mIsFolded && sensorPrivacyService.mIsLargeCoverScreen)
                            ? 1
                            : 0);
        } else {
