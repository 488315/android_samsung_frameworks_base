...
public class NetworkScorerAppManager {
      ...
      if (hasPermissions(serviceInfo.applicationInfo.uid, serviceInfo.packageName)) {
        if (VERBOSE) {
          Log.v(
              ...
              serviceInfo.packageName + " is a valid scorer/recommender.");
        }
        arrayList.add(
            new NetworkScorerAppData(
                serviceInfo.applicationInfo.uid,
                new ComponentName(serviceInfo.packageName, serviceInfo.name),
                getRecommendationServiceLabel(serviceInfo, packageManager),
                findUseOpenWifiNetworksActivity(serviceInfo),
                getNetworkAvailableNotificationChannelId(serviceInfo)));
      } else if (VERBOSE) {
        Log.v(
            "NetworkScorerAppManager",
            serviceInfo.packageName + " is NOT a valid scorer/recommender.");
      }
    }
    return arrayList;
  }

  public final String getRecommendationServiceLabel(
  ...
  public final ComponentName findUseOpenWifiNetworksActivity(ServiceInfo serviceInfo) {
    ...
    if (TextUtils.isEmpty(string)) {
      if (DEBUG) {
        Log.d(
            "NetworkScorerAppManager",
            "No use_open_wifi_package metadata found on " + serviceInfo.getComponentName());
      }
      return null;
    }
  ...
  public final boolean hasScoreNetworksPermission(String str) {
    return this.mContext
            .getPackageManager()
            .checkPermission("android.permission.SCORE_NETWORKS", str)
        == 0;
  }
  ...
  public final boolean canAccessLocation(int i, String str) {
    return isLocationModeEnabled()
        && PermissionChecker.checkPermissionForPreflight(
                this.mContext, "android.permission.ACCESS_COARSE_LOCATION", -1, i, str)
            == 0;
  }
  ...
  public boolean setActiveScorer(String str) {
    ...
    if (TextUtils.isEmpty(str)) {
      Log.i(
          "NetworkScorerAppManager",
          "Network scorer forced off, was: " + networkRecommendationsPackage);
    ...
    if (getScorer(str) != null) {
      Log.i(
          "NetworkScorerAppManager",
          "Changing network scorer from " + networkRecommendationsPackage + " to " + str);
  ...
  public void updateState() {
    ...
    if (TextUtils.equals(networkRecommendationsPackage, defaultPackageSetting)
        || getScorer(defaultPackageSetting) == null) {
      i = 0;
    } else {
      if (DEBUG) {
        Log.d(
            "NetworkScorerAppManager",
            "Defaulting the network recommendations app to: " + defaultPackageSetting);
      }
  ...
  public void migrateNetworkScorerAppSettingIfNeeded() {
    ...
    if (z) {
      Log.d(
          "NetworkScorerAppManager",
          "Migrating Settings.Global.NETWORK_SCORER_APP (" + string + ")...");
    }
    ComponentName enableUseOpenWifiActivity = activeScorer.getEnableUseOpenWifiActivity();
    if (TextUtils.isEmpty(this.mSettingsFacade.getString(this.mContext, "use_open_wifi_package"))
      ...
      if (z) {
        Log.d(
            "NetworkScorerAppManager",
            "Settings.Global.USE_OPEN_WIFI_PACKAGE set to '" + string + "'.");
      }
    }
    ...
    if (z) {
      ...
      Log.d(
          ...
          "Settings.Global.USE_OPEN_WIFI_PACKAGE is: '"
              + this.mSettingsFacade.getString(this.mContext, "use_open_wifi_package")
              + "'.");
    }
  }

  public final String getDefaultPackageSetting() {
    return this.mContext.getResources().getString(R.string.date_picker_month_typeface);
  }
