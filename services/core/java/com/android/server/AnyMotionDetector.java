...
public class AnyMotionDetector {
  ...
  public AnyMotionDetector(
          ...
          public void onSensorChanged(SensorEvent sensorEvent) {
            ...
            synchronized (AnyMotionDetector.this.mLock) {
              ...
              AnyMotionDetector.this.mRunningStats.accumulate(
                  new Vector3(elapsedRealtime, fArr[0], fArr[1], fArr[2]));
              stopOrientationMeasurementLocked =
                  AnyMotionDetector.this.mRunningStats.getSampleCount()
                          >= AnyMotionDetector.this.mNumSufficientSamples
                      ? AnyMotionDetector.this.stopOrientationMeasurementLocked()
                      : -1;
            }
            if (stopOrientationMeasurementLocked != -1) {
              AnyMotionDetector.this.mHandler.removeCallbacks(
                  AnyMotionDetector.this.mWakelockTimeout);
    ...
    this.mSensorRestart =
        new Runnable() { // from class: com.android.server.AnyMotionDetector.2
    ...
    this.mMeasurementTimeout =
        new Runnable() { // from class: com.android.server.AnyMotionDetector.3
          @Override // java.lang.Runnable
          public void run() {
            int i;
            synchronized (AnyMotionDetector.this.mLock) {
    ...
    this.mWakelockTimeout =
        new Runnable() { // from class: com.android.server.AnyMotionDetector.4
          @Override // java.lang.Runnable
          public void run() {
            synchronized (AnyMotionDetector.this.mLock) {
    ...
    synchronized (this.mLock) {
      if (this.mState != 1) {
        ...
        this.mHandler.sendMessageDelayed(
            Message.obtain(this.mHandler, this.mWakelockTimeout), 30000L);
  ...
  public final void startOrientationMeasurementLocked() {
    ...
    this.mHandler.sendMessageDelayed(
        Message.obtain(this.mHandler, this.mMeasurementTimeout), 3000L);
  ...
  public final int getStationaryStatusLocked() {
    ...
    if (angleBetween >= this.mThresholdAngle || this.mRunningStats.getEnergy() >= 5.0f) {
      return (!Float.isNaN(angleBetween)
              && this.mCurrentGravityVector.timeMillisSinceBoot
                      - this.mPreviousGravityVector.timeMillisSinceBoot
                  > 120000)
          ? -1
          : 1;
    }
  ...
  public final class Vector3 {
    ...
    public Vector3 normalized() {
      float norm = norm();
      return new Vector3(
          this.timeMillisSinceBoot, this.f1612x / norm, this.f1613y / norm, this.f1614z / norm);
    }

    public float angleBetween(Vector3 vector3) {
      float abs =
          Math.abs((float) Math.toDegrees(Math.atan2(cross(vector3).norm(), dotProduct(vector3))));
      Slog.d(
          "AnyMotionDetector",
          "angleBetween: this = "
              + toString()
    ...
    public Vector3 cross(Vector3 vector3) {
      ...
      float f = this.f1613y;
      float f2 = vector3.f1614z;
      float f3 = this.f1614z;
      float f4 = vector3.f1613y;
      float f5 = vector3.f1612x;
      float f6 = this.f1612x;
      return new Vector3(j, (f * f2) - (f3 * f4), (f3 * f5) - (f2 * f6), (f6 * f4) - (f * f5));
    }

    public String toString() {
      return ((("timeMillisSinceBoot=" + this.timeMillisSinceBoot) + " | x=" + this.f1612x)
              + ", y="
              + this.f1613y)
          + ", z="
          + this.f1614z;
    ...
    public float dotProduct(Vector3 vector3) {
      return (this.f1612x * vector3.f1612x)
          + (this.f1613y * vector3.f1613y)
          + (this.f1614z * vector3.f1614z);
    }

    public Vector3 times(float f) {
      return new Vector3(
          this.timeMillisSinceBoot, this.f1612x * f, this.f1613y * f, this.f1614z * f);
    }

    public Vector3 plus(Vector3 vector3) {
      return new Vector3(
          vector3.timeMillisSinceBoot,
          vector3.f1612x + this.f1612x,
          vector3.f1613y + this.f1613y,
          this.f1614z + vector3.f1614z);
    }
  ...
  public class RunningSignalStats {
    ...
    public void accumulate(Vector3 vector3) {
      ...
      if (vector32 != null) {
        ...
        float f = minus.f1612x;
        float f2 = minus.f1613y;
        float f3 = minus.f1614z;
    ...
    public String toString() {
      ...
      return ((("previousVector = " + (vector33 != null ? vector33.toString() : "null"))
                  + ", currentVector = "
                  + vector32)
              + ", sampleCount = "
              + this.sampleCount)
