  ...
  */
  public final void generateCropInternal(WallpaperData wallpaperData) {
    ...
    WallpaperDisplayHelper.DisplayData displayDataOrCreate =
        this.mWallpaperDisplayHelper.getDisplayDataOrCreate(displayId);
    DisplayInfo displayInfo = this.mWallpaperDisplayHelper.getDisplayInfo(displayId);
    Rect rect = new Rect(wallpaperData.cropHint);
    if ((Rune.isTablet() || Rune.isFolder())
        && wallpaperData.getOrientation() != 0
        && this.mSemService.getOrientation() != 0
        && wallpaperData.getOrientation() != this.mSemService.getOrientation()) {
    ...
    Log.m129d(
        ...
        "generateCrop  info=("
            ...
            + options.outHeight
            + ") preload="
            + wallpaperData.mSemWallpaperData.getIsPreloaded()
            + " isQmg="
            + isQmgImage);
    Log.addLogString(
        str,
      ...
      try {
        if (rect.isEmpty()) {
          ...
          if (options.outHeight > rect.height() || options.outWidth > rect.width()) {
            z = true;
            z2 =
                rect.height() <= displayDataOrCreate.mHeight
                    || rect.height() > GLHelper.getMaxTextureSize()
                    || rect.width() > GLHelper.getMaxTextureSize();
            if (z2) {
              if (wallpaperData.mSemWallpaperData.getOrientation() == 2) {
                if (Rune.isTablet()
                    || (Rune.SUPPORT_SUB_DISPLAY_MODE
                        && !WhichChecker.isSubDisplay(
              ...
              if (((int) (rect.width() * (max / rect.height()))) < min) {
                ...
                if (Rune.SUPPORT_SUB_DISPLAY_MODE
                  ...
                  Log.m129d(
                      str,
                      "generateCrop: [before] cropHint.top = "
                          + rect.top
                          + ", cropHint.bottom = "
                          + rect.bottom);
                  rect.top = height;
                  ...
                  Log.m129d(
                      str,
                      "generateCrop: [after] cropHint.top = "
                          + rect.top
                          + ", cropHint.bottom = "
                          + rect.bottom);
                }
            ...
            Slog.v(
                str, "dims: w=" + displayDataOrCreate.mWidth + " h=" + displayDataOrCreate.mHeight);
            ...
            if ((z && !z2) || wallpaperData.mSemWallpaperData.getIsPreloaded() || isQmgImage) {
              ...
              File file =
                  new File(
                      WallpaperUtils.getWallpaperDir(wallpaperData.userId),
                      !wallpaperData.wallpaperFile.getName().equals("wallpaper_orig")
                          ? "decode_record"
                          : "decode_lock_record");
              file.createNewFile();
              ...
              decodeBitmap =
                  ImageDecoder.decodeBitmap(
                      ImageDecoder.createSource(wallpaperData.wallpaperFile),
                      new ImageDecoder
                          .OnHeaderDecodedListener() { // from class:
                                                       // com.android.server.wallpaper.WallpaperCropper$$ExternalSyntheticLambda0
                        @Override // android.graphics.ImageDecoder.OnHeaderDecodedListener
                        public final void onHeaderDecoded(
                            ImageDecoder imageDecoder,
                            ImageDecoder.ImageInfo imageInfo,
                            ImageDecoder.Source source) {
                          WallpaperCropper.lambda$generateCropInternal$0(
                              i, rect2, imageDecoder, imageInfo, source);
                        }
                      });
              file.delete();
              if (decodeBitmap != null) {
                ...
                Bitmap createScaledBitmap =
                    Bitmap.createScaledBitmap(decodeBitmap, width2, height5, true);
                ...
                try {
                  ...
                  if (!TextUtils.isEmpty(semGetLastCallingPackage)
                      && ("android.app.cts.wallpapers".equals(semGetLastCallingPackage)
                          || "com.android.wallpaperbackup".equals(semGetLastCallingPackage))) {
                    createScaledBitmap.compress(
                        Bitmap.CompressFormat.PNG, 100, bufferedOutputStream);
                  } else {
                    createScaledBitmap.compress(
            ...
            ImageDecoder.decodeBitmap(
                ...
                new ImageDecoder
                  ...
                  public final void onHeaderDecoded(
                      ...
                      ImageDecoder.Source source) {
                    WallpaperCropper.lambda$generateCropInternal$0(
                        i, rect22, imageDecoder, imageInfo, source);
                  }
                });
        file2.delete();
        if (decodeBitmap != null) {}
  ...
  public static /* synthetic */ void lambda$generateCropInternal$0(
      int i,
      Rect rect,
      ImageDecoder imageDecoder,
      ImageDecoder.ImageInfo imageInfo,
