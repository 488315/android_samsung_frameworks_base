...
public class WallpaperData implements Cloneable {
  ...
  public WallpaperData(int i, int i2) {
    this(
        ...
        WhichChecker.isPhone(i2)
            ? WhichChecker.isLock(i2) ? "wallpaper_lock_orig" : "wallpaper_orig"
            : null,
        WhichChecker.isPhone(i2) ? WhichChecker.isLock(i2) ? "wallpaper_lock" : "wallpaper" : null);
  ...
  public WallpaperData(WallpaperData wallpaperData) {
    ...
    this.mSemWallpaperData = wallpaperData.mSemWallpaperData.m14847clone();
  ...
  public static /* synthetic */ void lambda$toString$0(
      StringBuilder sb, WallpaperManagerService.DisplayConnector displayConnector) {
  ...
  public String semGetLastCallingPackage(boolean z) {
    return this.mSemWallpaperData.getLastCallingPackage(z);
  }
  ...
  public boolean isSameWallpaperData(WallpaperData wallpaperData) {
    if (this.mSemWallpaperData.getWidth() == wallpaperData.mSemWallpaperData.getWidth()
        && this.mSemWallpaperData.getHeight() == wallpaperData.mSemWallpaperData.getHeight()
        && this.name.equals(wallpaperData.name)) {
      ComponentName componentName = this.wallpaperComponent;
      if (componentName == null) {
        if (new ComponentName("com.android.systemui", "com.android.systemui.ImageWallpaper")
  ...
  public void setCallingPackage(String str) {
    if (TextUtils.isEmpty(str) || str.startsWith("null")) {
      ...
      this.mSemWallpaperData.setLastClearCallstackWithNullPackage(
    ...
    semWallpaperData.setWallpaperHistory(
        str, null, semWallpaperData.getWallpaperTypeString(), getPackageName());
  }
  ...
  public void cleanUp() {
    ...
    if (Rune.SUPPORT_VIDEO_WALLPAPER && wpType != 8) {
      ...
      if (this.mSemWallpaperData.getVideoFirstFrameFile() != null
          && this.mSemWallpaperData.getVideoFirstFrameFile().exists()) {
        Slog.d(
            "WallpaperData",
            "delete video thumbnail file path : "
                + this.mSemWallpaperData.getVideoFirstFrameFile().getPath());
    ...
    if (wpType != 0) {
      Slog.d(
          "WallpaperData",
          "delete wallpaper and crop file. wpType=" + this.mSemWallpaperData.getWpType());
      File file = this.wallpaperFile;
    ...
    if (wpType != 4) {
      ...
      if (this.mSemWallpaperData.getAnimatedBackground() != null
          && this.mSemWallpaperData.getAnimatedBackground().exists()) {
        Slog.d(
            "WallpaperData",
            "delete animated background file path : "
                + this.mSemWallpaperData.getAnimatedBackground().getPath());
    ...
    if (wpType != 1) {
      ...
      if (this.mSemWallpaperData.getMotionBackground() != null
          && this.mSemWallpaperData.getMotionBackground().exists()) {
        Slog.d(
            "WallpaperData",
            "delete motion background file path : "
                + this.mSemWallpaperData.getMotionBackground().getPath());
    ...
    if (Rune.SUPPORT_PREVIEW_LOCK_ONLY_LIVE_WALLPAPER
        && WhichChecker.isLock(this.mSemWallpaperData.getWhich())
        && wpType != 7) {
  ...
  public WallpaperData m12812clone() {
    try {
      WallpaperData wallpaperData = (WallpaperData) super.clone();
      wallpaperData.cropHint = new Rect(this.cropHint);
      wallpaperData.mSemWallpaperData = this.mSemWallpaperData.m14847clone();
  ...
  public void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
    ...
    try {
      ...
      printWriter.println(
          "\t\t mSemWallpaperData.getWhich() = " + this.mSemWallpaperData.getWhich());
      ...
      if (this.mSemWallpaperData.getWpType() == 4) {
        printWriter.println(
            "\t\t mAnimatedPkgName = " + this.mSemWallpaperData.getAnimatedPkgName());
      }
      if (this.mSemWallpaperData.getWpType() == 8) {
        ...
        printWriter.println(
            "\t\t mVideoDefaultHasBeenUsed = "
                + this.mSemWallpaperData.getVideoDefaultHasBeenUsed());
      }
