...
public final class DockObserver extends SystemService {
  ...
  public final class ExtconStateProvider {
    ...
    public static ExtconStateProvider fromFile(String str) {
      ...
      try {
        ...
        try {
          ExtconStateProvider fromString =
              fromString(new String(cArr, 0, fileReader.read(cArr, 0, 1024)).trim());
          fileReader.close();
          return fromString;
        } finally {
        }
  ...
  public DockObserver(Context context) {
    ...
    ExtconUEventObserver extconUEventObserver =
        new ExtconUEventObserver() { // from class: com.android.server.DockObserver.2
          ...
          public void onUEvent(
            ...
            synchronized (DockObserver.this.mLock) {
              ...
              if (str != null) {
                DockObserver.this.setDockStateFromProviderLocked(
                    ExtconStateProvider.fromString(str));
              } else {
    ...
    this.mCcicObserver =
        new UEventObserver() { // from class: com.android.server.DockObserver.3
          public void onUEvent(UEventObserver.UEvent uEvent) {
            ...
            try {
              synchronized (DockObserver.this.mLock) {
                ...
                DockObserver.this.setActualDockStateLocked(
    ...
    this.mAllowTheaterModeWakeFromDock =
        context.getResources().getBoolean(R.bool.config_allowTheaterModeWakeFromDock);
    this.mKeepDreamingWhenUnplugging =
    ...
    List extconInfoForTypes =
        ExtconUEventObserver.ExtconInfo.getExtconInfoForTypes(new String[] {"DOCK"});
    if (!extconInfoForTypes.isEmpty()) {
      ExtconUEventObserver.ExtconInfo extconInfo =
          (ExtconUEventObserver.ExtconInfo) extconInfoForTypes.get(0);
      Slog.i(
          "DockObserver",
          "Found extcon info devPath: "
              + extconInfo.getDevicePath()
              + ", statePath: "
              + extconInfo.getStatePath());
        ...
        try {
          ...
          try {
            setActualDockStateLocked(
                Integer.valueOf(new String(cArr2, 0, fileReader.read(cArr2, 0, 1024)).trim())
                    .intValue());
            this.mPreviousDockState = this.mActualDockState;
            fileReader.close();
          } finally {
  ...
  public final boolean allowWakeFromDock() {
    ...
    return this.mAllowTheaterModeWakeFromDock
        || Settings.Global.getInt(getContext().getContentResolver(), "theater_mode_on", 0) == 0;
  }
  ...
  public final void handleDockStateChange() {
    ...
    synchronized (this.mLock) {
      Slog.i(
          "DockObserver",
          "Dock state changed from " + this.mPreviousDockState + " to " + this.mReportedDockState);
      ...
      if (this.mReportedDockState == 200
          && (str = this.mReportedUsbpdIds) != null
          && !str.equals("")) {
  ...
  public final void setDockStateFromProviderLocked(ExtconStateProvider extconStateProvider) {
    setActualDockStateLocked(
        "1".equals(extconStateProvider.getValue("DOCK"))
            ? getDockedStateExtraValue(extconStateProvider)
            : 0);
  }
  ...
  public final class BinderService extends Binder {
    ...
    public void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
      if (DumpUtils.checkDumpPermission(
          DockObserver.this.getContext(), "DockObserver", printWriter)) {
          ...
          synchronized (DockObserver.this.mLock) {
            ...
            if (DockObserver.this.mLogRecent != null) {
              ...
              for (int i = 0; i < 10; i++) {
                printWriter.println(
                    "   Time: "
                        + DockObserver.this.mLogRecent.uEventTime[i]
                        + " State: "
                        + DockObserver.this.mLogRecent.uEventType[i]
                        + " UsbpdIds: "
                        + DockObserver.this.mLogRecent.uEventUsbpdIds[i]);
              }
              printWriter.println(" == Latest states to Apps ==");
              for (int i2 = 0; i2 < 10; i2++) {
                printWriter.println(
                    "  Time: "
                        + DockObserver.this.mLogRecent.reportTime[i2]
                        + " State: "
                        + DockObserver.this.mLogRecent.report[i2]
                        + " UsbpdIds: "
                        + DockObserver.this.mLogRecent.reportUsbpdIds[i2]);
              }
            } else {
  ...
  public final class DeviceProvisionedObserver extends ContentObserver {
    ...
    public final void updateRegistration() {
      ...
      if (z) {
        contentResolver.registerContentObserver(
            Settings.Global.getUriFor("device_provisioned"), false, this);
      } else {
    ...
    public boolean isDeviceProvisioned() {
      return Settings.Global.getInt(
              DockObserver.this.getContext().getContentResolver(), "device_provisioned", 0)
          != 0;
    }
  }
}
