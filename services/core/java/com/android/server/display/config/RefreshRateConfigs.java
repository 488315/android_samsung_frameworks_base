    ...
    while (true) {
      ...
      if (xmlPullParser.getEventType() == 2) {
        ...
        if (name.equals("defaultRefreshRate")) {
          refreshRateConfigs.setDefaultRefreshRate(
              new BigInteger(XmlParser.readText(xmlPullParser)));
        } else if (name.equals("defaultPeakRefreshRate")) {
          refreshRateConfigs.setDefaultPeakRefreshRate(
              new BigInteger(XmlParser.readText(xmlPullParser)));
        } else if (name.equals("refreshRateZoneProfiles")) {
          refreshRateConfigs.setRefreshRateZoneProfiles(
              RefreshRateZoneProfiles.read(xmlPullParser));
        } else if (name.equals("defaultRefreshRateInHbmHdr")) {
          refreshRateConfigs.setDefaultRefreshRateInHbmHdr(
              new BigInteger(XmlParser.readText(xmlPullParser)));
        } else if (name.equals("defaultRefreshRateInHbmSunlight")) {
          refreshRateConfigs.setDefaultRefreshRateInHbmSunlight(
              new BigInteger(XmlParser.readText(xmlPullParser)));
