    ...
    while (true) {
      ...
      if (xmlPullParser.getEventType() == 2) {
        ...
        if (name.equals("transitionPoint")) {
          highBrightnessMode.setTransitionPoint_all(
          ...
          highBrightnessMode.setAllowInLowPowerMode_all(
              Boolean.parseBoolean(XmlParser.readText(xmlPullParser)));
        } else if (name.equals("minimumHdrPercentOfScreen")) {
          highBrightnessMode.setMinimumHdrPercentOfScreen_all(
              new BigDecimal(XmlParser.readText(xmlPullParser)));
