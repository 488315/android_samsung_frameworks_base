...
public final class AdaptiveBrightnessWeightStats implements Parcelable {
  public static final Parcelable.Creator CREATOR =
      new Parcelable
          .Creator() { // from class: com.android.server.display.AdaptiveBrightnessWeightStats.1
  ...
  public AdaptiveBrightnessWeightStats(
    ...
    Preconditions.checkArrayElementsInRange(
        fArr, DisplayPowerController2.RATE_FROM_DOZE_TO_ON, Float.MAX_VALUE, "bucketBoundaries");
    if (fArr.length < 1) {
    ...
    if (brightnessWeightsArr == null) {
      ...
      while (i < fArr.length) {
        brightnessWeightsArr[i] =
            new BrightnessWeights(
                fArr[i],
                brightnessMappingStrategy.convertToNits(
                    brightnessMappingStrategy.getBrightness(fArr[i])),
                60.0f,
                0L);
        i++;
      }
      ...
      while (i < fArr.length) {
        brightnessWeightsArr[i].setBrightness(
            brightnessMappingStrategy.convertToNits(
                brightnessMappingStrategy.getBrightness(fArr[i])));
        i++;
      }
    ...
    this.mContinuityStatsCollector =
        new ContinuityStatsCollector(
            fArr, brightnessMappingStrategy, timeStatsCollector, this.mStats);
  ...
  public AdaptiveBrightnessWeightStats(Parcel parcel) {
    ...
    while (true) {
      ...
      if (i < brightnessWeightsArr.length) {
        ...
        Slog.d(
            "AdaptiveBrightnessWeightStats",
            "AdaptiveBrightnessWeightStats (parcel): mBrightnessMapper: " + ((Object) null));
  ...
  public boolean equals(Object obj) {
    ...
    return Arrays.equals(this.mBucketBoundaries, adaptiveBrightnessWeightStats.mBucketBoundaries)
  ...
  public void log(
    ...
    this.mContinuityStatsCollector.addUserBrightnessStat(
        f, f4, f4 - brightnessChangeEvent.lastBrightness, spline2);
  ...
  public static float getDistributionRatio(float f, float f2, float f3, float f4) {
    ...
    Slog.e(
        "AdaptiveBrightnessWeightStats",
        "getDistributionRatio: wrong boundary " + f + ": " + f2 + " < " + f3 + " < " + f4);
    return Float.NaN;
  }

  public static float getBrightnessForSpline(
      float f, Spline spline, BrightnessMappingStrategy brightnessMappingStrategy) {
    if (brightnessMappingStrategy != null) {
      return brightnessMappingStrategy.convertToNits(
          brightnessMappingStrategy.getBrightnessForSpline(f, spline));
    }
    return Float.NaN;
  }
  ...
  public void summarizeStats() {
    ...
    printCurrentStats(
        "summarizeStats(): lastStats[]:", brightnessWeightsArr, this.mBucketBoundaries);
    for (int i = 0; i < this.mStats.length; i++) {
      float weight = stats2[i].getWeight() + stats[i].getWeight();
      float brightness =
          Float.compare(weight, DisplayPowerController2.RATE_FROM_DOZE_TO_ON) == 0
              ? 0.0f
              : ((stats2[i].getBrightness() * stats2[i].getWeight())
                      + (stats[i].getBrightness() * stats[i].getWeight()))
                  / weight;
      ...
      if (weight <= 28800.0f) {
        ...
        constrain =
            Float.compare(f, DisplayPowerController2.RATE_FROM_DOZE_TO_ON) == 0
                ? 0.0f
                : ((brightness2 * weight2) + (constrain * weight)) / f;
      }
  ...
  public class WeightStat {
    ...
    public String toString() {
      ...
      return String.format(
          Integer.toString((int) this.mBrightness) + XmlUtils.STRING_ARRAY_SEPARATOR + format,
          new Object[0]);
    }
  }
  ...
  public class TimeStatsCollector {
    ...
    public void updateTransientStats(float f, float f2, float f3, Spline spline) {
      ...
      int bucketIndex =
          AdaptiveBrightnessWeightStats.getBucketIndex(f, timeStatsCollector.mBucketBoundaries);
      ...
      int constrain =
          MathUtils.constrain(
              AdaptiveBrightnessWeightStats.getBucketIndex(
                  lowerBoundary, timeStatsCollector.mBucketBoundaries),
              0,
              timeStatsCollector.mBucketBoundaries.length - 1);
      int constrain2 =
          MathUtils.constrain(
              AdaptiveBrightnessWeightStats.getBucketIndex(
                  upperBoundary, timeStatsCollector.mBucketBoundaries),
              0,
              timeStatsCollector.mBucketBoundaries.length - 1);
      ...
      while (constrain <= constrain2) {
        ...
        if (brightnessMappingStrategy != null) {
          ...
          float brightnessForSpline =
              AdaptiveBrightnessWeightStats.getBrightnessForSpline(
                  f5, spline, brightnessMappingStrategy);
          ...
          float distributionRatio =
              AdaptiveBrightnessWeightStats.getDistributionRatio(
                  f5, lowerBoundary, f4, upperBoundary);
          int findBrightnessIndex =
              AdaptiveBrightnessWeightStats.findBrightnessIndex(brightnessForSpline, arrayList);
  ...
  public class ContinuityStatsCollector {
    ...
    public final void addUserBrightnessStat(
      ...
      UserBrightnessStat userBrightnessStat =
          new UserBrightnessStat(f, f2, f3, spline, z ? getTimeDurationForBucketLux(f) : Float.NaN);
    ...
    public final void updateContinuityStats() {
      ...
      while (it.hasNext()) {
        ...
        WeightStat calculateContinuity =
            calculateContinuity(userBrightnessStat, this.mPrevUserBrightnessStats);
    ...
    public final void updateTimeDurationPrevUserBrightnessStats() {
      ...
      while (it.hasNext()) {
        ...
        Slog.d(
            "AdaptiveBrightnessWeightStats",
            "updateTimeDurationPrevUserBrightnessStats: lux: "
                + lux
                + " timeDuration: "
                + timeDurationForBucketLux);
      }
    }
    ...
    public UserBrightnessStat getPrevUserBrightnessStatForContinuity(
        UserBrightnessStat userBrightnessStat, ArrayList arrayList) {
      float lux = userBrightnessStat.getLux();
    ...
    public final void getElapsedTimeSinceFirstShortTermReset(
        UserBrightnessStat userBrightnessStat,
        UserBrightnessStat userBrightnessStat2,
        ContinuityCalculator continuityCalculator) {
      float lux = userBrightnessStat.getLux();
      ...
      continuityCalculator.mSb.append(
          String.format(
              " cl:%.1f pl:%.1f t(%d + %d)",
              Float.valueOf(lux),
              Float.valueOf(userBrightnessStat2.getLux()),
              Integer.valueOf((int) postTimeDuration),
              Integer.valueOf((int) preTimeDuration)));
    }

    public final void getSimilarity(
        UserBrightnessStat userBrightnessStat,
        UserBrightnessStat userBrightnessStat2,
        ContinuityCalculator continuityCalculator) {
      ...
      float f2 = Float.NaN;
      float f3 =
          (brightness <= DisplayPowerController2.RATE_FROM_DOZE_TO_ON
                  || brightness2 <= DisplayPowerController2.RATE_FROM_DOZE_TO_ON)
              ? Float.NaN
              : brightness > brightness2 ? brightness2 / brightness : brightness / brightness2;
      if (adjustment > DisplayPowerController2.RATE_FROM_DOZE_TO_ON
          && adjustment2 > DisplayPowerController2.RATE_FROM_DOZE_TO_ON) {
      ...
      continuityCalculator.mSb.append(
          String.format(
              " cb:%.1f pb:%.1f (%s,%d,%.2f,%.2f)",
              Float.valueOf(brightness),
              Float.valueOf(brightness2),
              Boolean.valueOf(z),
    ...
    public final WeightStat calculateContinuity(
        UserBrightnessStat userBrightnessStat, ArrayList arrayList) {
      UserBrightnessStat prevUserBrightnessStatForContinuity =
          getPrevUserBrightnessStatForContinuity(userBrightnessStat, arrayList);
      ...
      getElapsedTimeSinceFirstShortTermReset(
          userBrightnessStat, prevUserBrightnessStatForContinuity, continuityCalculator);
    ...
    public final void distributeToBucketBoundaries(
        UserBrightnessStat userBrightnessStat, WeightStat weightStat, WeightStat[] weightStatArr) {
      int bucketIndex =
          AdaptiveBrightnessWeightStats.getBucketIndex(
              userBrightnessStat.getLux(), this.mBucketBoundaries);
      ...
      int constrain =
          MathUtils.constrain(
              AdaptiveBrightnessWeightStats.getBucketIndex(lowerBoundary, this.mBucketBoundaries),
              0,
              this.mBucketBoundaries.length - 1);
      int constrain2 =
          MathUtils.constrain(
              AdaptiveBrightnessWeightStats.getBucketIndex(upperBoundary, this.mBucketBoundaries),
              0,
              this.mBucketBoundaries.length - 1);
      float[] fArr = this.mBucketBoundaries;
      if (fArr[constrain] < lowerBoundary && constrain <= fArr.length - 2) {
      ...
      while (constrain <= constrain2) {
        if (this.mBrightnessMapper != null) {
          ...
          float brightnessForSpline =
              AdaptiveBrightnessWeightStats.getBrightnessForSpline(
                  f2, userBrightnessStat.getSpline(), this.mBrightnessMapper);
          if (bucketIndex == constrain) {
            Slog.d(
                "AdaptiveBrightnessWeightStats",
                "distributeToBucketBoundaries: continuityStat: b: "
                    + AdaptiveBrightnessWeightStats.getBrightnessForSpline(
                        f2, userBrightnessStat.getSpline(), this.mBrightnessMapper)
                    + weightStat
                    + " ambientLux:"
                    + userBrightnessStat.getLux()
                    + " currentBucketLux:"
                    + f
                    + " mBrightnessMapper: "
                    + brightnessForSpline);
          ...
          float distributionRatio =
              AdaptiveBrightnessWeightStats.getDistributionRatio(
                  f2, lowerBoundary, f, upperBoundary);
          WeightStat weightStat2 = weightStatArr[constrain];
          if (weightStat2 != null) {
            float weight = weightStat2.getWeight() + (weightStat.getWeight() * distributionRatio);
            weightStatArr[constrain].set(
                ((weightStatArr[constrain].getBrightness() * weightStatArr[constrain].getWeight())
                        + ((brightnessForSpline * weightStat.getWeight()) * distributionRatio))
                    / weight,
                weight);
          } else {
            weightStatArr[constrain] =
                new WeightStat(brightnessForSpline, weightStat.getWeight() * distributionRatio);
          }
        }
        constrain++;
      }
      AdaptiveBrightnessWeightStats.printCurrentStats(
          "distributeToBucketBoundaries() rawContinuityStats: ",
          weightStatArr,
          this.mBucketBoundaries);
    }
    ...
    public final void addCurrentUserBrightnessStatsToPrevious() {
      ...
      while (it.hasNext()) {
        UserBrightnessStat userBrightnessStat = (UserBrightnessStat) it.next();
        addUserBrightnessStat(
            userBrightnessStat.getLux(),
            userBrightnessStat.getBrightness(),
            userBrightnessStat.getAdjustment(),
            userBrightnessStat.getSpline(),
            this.mPrevUserBrightnessStats,
            false);
      }
      for (int i = 0; i < this.mPrevUserBrightnessStats.size(); i++) {
        Slog.d(
            ...
            "addCurrentUserBrightnessStatsToPrevious: ["
                + i
                + "] "
                + this.mPrevUserBrightnessStats.get(i));
      }
      this.mCurrentUserBrightnessStats.clear();
    }

    public static void removeRedundantUserBrightnessStats(
        UserBrightnessStat userBrightnessStat, ArrayList arrayList) {
      Iterator it = arrayList.iterator();
      while (it.hasNext()) {
    ...
    public class ContinuityCalculator {
      ...
      public void calculate() {
        ...
        if (Float.compare(f, DisplayPowerController2.RATE_FROM_DOZE_TO_ON) == 0
            || Float.isNaN(this.mLux)
            || Float.isNaN(this.mBrightness)) {
          return;
        }
        ...
        this.mContinuity =
            Math.min(
                ((float) Math.sqrt(this.mSimilarity * f2)) * 14400.0f,
                ContinuityStatsCollector.this
                        .mTotalStats[
                        AdaptiveBrightnessWeightStats.getBucketIndex(
                            this.mLux, ContinuityStatsCollector.this.mBucketBoundaries)]
                        .getWeight()
                    * 3.0f);
        Slog.d(
            "AdaptiveBrightnessWeightStats",
            "ContinuityCalculator.calculate(): mContinuity: "
                + this.mContinuity
                + " mLux: "
                + this.mLux
                + " mBrightness: "
    ...
    public class UserBrightnessStat {
      ...
      public String toString() {
        return String.format(
            ...
            Float.valueOf(this.mBrightness - this.mAdjustment),
            Float.valueOf(this.mBrightness),
            Float.valueOf(this.mAdjustment),
