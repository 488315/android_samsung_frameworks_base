...
public final class DisplayBrightnessController {
    ...
    public DisplayBrightnessController(Context context, Injector injector, int i, float f, BrightnessSetting brightnessSetting, Runnable runnable, HandlerExecutor handlerExecutor) {
        ...
        this.mPersistBrightnessNitsForDefaultDisplay = context.getResources().getBoolean(R.bool.config_magnification_area);
    ...
    public final float getScreenBrightnessSettingOnBootPhase(Context context) {
        ...
        if (this.mDisplayId == 0) {
            ...
            f = BrightnessSynchronizer.brightnessIntToFloat(Settings.System.getIntForUser(context.getContentResolver(), "sub_screen_brightness", context.getResources().getInteger(R.integer.config_triplePressOnStemPrimaryBehavior), -2));
            if (Float.isNaN(f)) {
                f = this.mScreenBrightnessDefault;
            }
        } else {
