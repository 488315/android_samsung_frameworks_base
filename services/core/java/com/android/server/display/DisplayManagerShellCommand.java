...
public class DisplayManagerShellCommand extends ShellCommand {
  public static String RANDOM_STR_1KBYTE =
      "";
  ...
  public int onCommand(String str) {
    ...
    switch (str) {
      ...
      case "get-displays":
        return getDisplays();
      case "ab-add-brightness-weights":
        ...
        if (str.startsWith("bufferFill")) {
          ...
          if (strArr == null || strArr.length < 2) {
            outPrintWriter.println(
                "Cannot parse bufferFill-[size in kbytes] from input ["
                    + str
            ...
            try {
              ...
              if (parseInt <= 0) {
                outPrintWriter.println(
                    "Got a size <= 0 from input [" + str + "], will use default size.");
              } else {
                i = parseInt;
              }
            } catch (Exception unused2) {
              outPrintWriter.println(
                  "Exception occured while parsing bufferFill-[size in kbytes] from input ["
                      + str
                      + "], will use default size.");
            }
          }
          if (!"1".equals(SystemProperties.get("persist.sys.testlab", "0"))) {
            outPrintWriter.println(
                "Error: bufferFill command can only be operated when persist.sys.testlab=1");
            return 0;
          }
  ...
  public void onHelp() {
    ...
    outPrintWriter.println(
        "  ab-add-brightness-weights [lux]:[brightness(0.0f~1.0f)]:[durationSec]:[continuity]");
    ...
    outPrintWriter.println(
        "    Sets the user preferred display mode which has fields WIDTH, HEIGHT and REFRESH-RATE."
            + " If DISPLAY_ID is passed, the mode change is applied to displaywith id = DISPLAY_ID,"
            + " else mode change is applied globally.");
    outPrintWriter.println("  clear-user-preferred-display-mode DISPLAY_ID (optional)");
    outPrintWriter.println(
        "    Clears the user preferred display mode. If DISPLAY_ID is passed, the mode is cleared"
            + " for  display with id = DISPLAY_ID, else mode is cleared globally.");
    ...
    outPrintWriter.println(
        "    Returns the user preferred display mode or null if no mode is set by user.If"
            + " DISPLAY_ID is passed, the mode for display with id = DISPLAY_ID is returned, else"
            + " global display mode is returned.");
    ...
    outPrintWriter.println(
        "    Debug operation: Fill [size] KBytes to async binder buffer by making testing async"
            + " binder call.");
    outPrintWriter.println();
  ...
  public final int getDisplays() {
    Display[] displays =
        ((DisplayManager) this.mService.getContext().getSystemService(DisplayManager.class))
            .getDisplays(getNextArg());
    PrintWriter outPrintWriter = getOutPrintWriter();
  ...
  public final int injectLux(PrintWriter printWriter) {
    ...
    if (nextArg != null) {
      ...
      SensorEvent sensorEvent =
          new SensorEvent(null, 0, 0L, new float[] {parseFloat, parseFloat2, f});
      Slog.d(
  ...
  public final int addBrightnessWeights(PrintWriter printWriter) {
    ...
    try {
      ...
      this.mService.addBrightnessWeights(
  ...
  public final int setUserPreferredDisplayMode() {
    ...
    try {
      ...
      if ((parseInt2 < 0 || parseInt3 < 0)
          && parseFloat <= DisplayPowerController2.RATE_FROM_DOZE_TO_ON) {
        getErrPrintWriter()
            .println("Error: invalid value of resolution (width, height) and refresh rate");
        return 1;
      }
      ...
      this.mService.setUserPreferredDisplayModeInternal(
          parseInt, new Display.Mode(parseInt2, parseInt3, parseFloat));
      return 0;
  ...
  public final int getUserPreferredDisplayMode() {
    ...
    Display.Mode userPreferredDisplayModeInternal =
        this.mService.getUserPreferredDisplayModeInternal(parseInt);
    ...
    getOutPrintWriter()
        .println(
            "User preferred display mode: "
                + userPreferredDisplayModeInternal.getPhysicalWidth()
                + " "
                + userPreferredDisplayModeInternal.getPhysicalHeight()
                + " "
                + userPreferredDisplayModeInternal.getRefreshRate());
    return 0;
  }
  ...
  public final int getActiveDisplayModeAtStart() {
    ...
    try {
      Display.Mode activeDisplayModeAtStart =
          this.mService.getActiveDisplayModeAtStart(Integer.parseInt(nextArg));
      ...
      getOutPrintWriter()
          .println(
              "Boot display mode: "
                  + activeDisplayModeAtStart.getPhysicalWidth()
                  + " "
                  + activeDisplayModeAtStart.getPhysicalHeight()
                  + " "
                  + activeDisplayModeAtStart.getRefreshRate());
      return 0;
    } catch (NumberFormatException unused) {
  ...
  public final int setMatchContentFrameRateUserPreference() {
    ...
    try {
      ...
      ((DisplayManager) this.mService.getContext().getSystemService(DisplayManager.class))
          .setRefreshRateSwitchingType(toRefreshRateSwitchingType(parseInt));
      return 0;
    } catch (NumberFormatException unused) {
  ...
  public final int getUserDisabledHdrTypes() {
    ...
    getOutPrintWriter()
        .println("User disabled HDR types: " + Arrays.toString(userDisabledHdrTypes));
    return 0;
  }
  ...
  public final int toRefreshRateSwitchingType(int i) {
    ...
    Slog.e(
        "DisplayManagerShellCommand", i + " is not a valid value of matchContentFrameRate type.");
    return -1;
  }
