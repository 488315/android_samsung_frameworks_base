...
public class AmbientBrightnessStatsTracker {
    ...
    this.mTimer =
        new Timer(
            new Clock() { // from class:
                          // com.android.server.display.AmbientBrightnessStatsTracker$$ExternalSyntheticLambda0
  ...
  public synchronized void stop() {
    if (this.mTimer.isRunning()) {
      this.mAmbientBrightnessStats.log(
          this.mCurrentUserId,
          this.mInjector.getLocalDate(),
          this.mCurrentAmbientBrightness,
          this.mTimer.totalDurationSec());
    }
    this.mTimer.reset();
    this.mCurrentAmbientBrightness = -1.0f;
  }
  ...
  public class AmbientBrightnessStats {
    ...
    public void writeToXML(OutputStream outputStream) {
      ...
      LocalDate minusDays =
          AmbientBrightnessStatsTracker.this.mInjector.getLocalDate().minusDays(7L);
      ...
      for (Map.Entry entry : this.mStats.entrySet()) {
        for (AmbientBrightnessDayStats ambientBrightnessDayStats : (Deque) entry.getValue()) {
          int userSerialNumber =
              AmbientBrightnessStatsTracker.this.mInjector.getUserSerialNumber(
                  AmbientBrightnessStatsTracker.this.mUserManager,
                  ((Integer) entry.getKey()).intValue());
          if (userSerialNumber != -1
              && ambientBrightnessDayStats.getLocalDate().isAfter(minusDays)) {
            ...
            resolveSerializer.attribute(
                (String) null, "local-date", ambientBrightnessDayStats.getLocalDate().toString());
    ...
    */
      ...
      try {
        ...
        if (!"ambient-brightness-stats".equals(name)) {
          throw new XmlPullParserException(
              "Ambient brightness stats not found in tracker file " + name);
        }
        LocalDate minusDays =
            AmbientBrightnessStatsTracker.this.mInjector.getLocalDate().minusDays(7L);
        ...
        while (true) {
          ...
          if (next2 != 3
              && next2 != 4
              && "ambient-brightness-day-stats".equals(resolvePullParser.getName())) {
            int attributeInt = resolvePullParser.getAttributeInt((String) null, "user");
            LocalDate parse =
                LocalDate.parse(resolvePullParser.getAttributeValue((String) null, "local-date"));
            String[] split =
                resolvePullParser.getAttributeValue((String) null, "bucket-boundaries").split(",");
            String[] split2 =
                resolvePullParser.getAttributeValue((String) null, "bucket-stats").split(",");
            ...
            int userId =
                AmbientBrightnessStatsTracker.this.mInjector.getUserId(
                    AmbientBrightnessStatsTracker.this.mUserManager, attributeInt);
            if (userId != -1 && parse.isAfter(minusDays)) {
              getOrCreateUserStats(hashMap, userId)
                  .offer(new AmbientBrightnessDayStats(parse, fArr, fArr2));
            }
          }
        }
        this.mStats = hashMap;
    ...
    public final AmbientBrightnessDayStats getOrCreateDayStats(Deque deque, LocalDate localDate) {
      AmbientBrightnessDayStats ambientBrightnessDayStats =
          (AmbientBrightnessDayStats) deque.peekLast();
      if (ambientBrightnessDayStats != null
          && ambientBrightnessDayStats.getLocalDate().equals(localDate)) {
        return ambientBrightnessDayStats;
      }
      if (ambientBrightnessDayStats != null) {
        FrameworkStatsLog.write(
            FrameworkStatsLog.AMBIENT_BRIGHTNESS_STATS_REPORTED,
            ambientBrightnessDayStats.getStats(),
            ambientBrightnessDayStats.getBucketBoundaries());
      }
      AmbientBrightnessDayStats ambientBrightnessDayStats2 =
          new AmbientBrightnessDayStats(
              localDate, AmbientBrightnessStatsTracker.BUCKET_BOUNDARIES_FOR_NEW_STATS);
      if (deque.size() == 7) {
        deque.poll();
      }
  ...
  class Timer {
    ...
    public float totalDurationSec() {
      return this.started
          ? (float) ((this.clock.elapsedTimeMillis() - this.startTimeMillis) / 1000.0d)
          : DisplayPowerController2.RATE_FROM_DOZE_TO_ON;
    }
  }
