...
public class BrightnessSetting {
  ...
  public final Handler mHandler =
      new Handler(
          Looper.getMainLooper()) { // from class: com.android.server.display.BrightnessSetting.1
  ...
  public BrightnessSetting(
    ...
    this.mBrightness =
        persistentDataStore.getBrightness(logicalDisplay.getPrimaryDisplayDeviceLocked());
    ...
    synchronized (this.mSyncRoot) {
      if (f != this.mBrightness) {
        this.mPersistentDataStore.setBrightness(
            this.mLogicalDisplay.getPrimaryDisplayDeviceLocked(), f);
      }
      this.mBrightness = f;
