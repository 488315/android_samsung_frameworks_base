  ...
  public DeviceStateToLayoutMap(DisplayIdProducer displayIdProducer) {
    this(
        ...
        Environment.buildPath(
            ...
            new String[] {"etc/displayconfig/display_layout_configuration.xml"}));
  }
  ...
  public void dumpLocked(IndentingPrintWriter indentingPrintWriter) {
    ...
    for (int i = 0; i < this.mLayoutMap.size(); i++) {
      indentingPrintWriter.println(
          "state(" + this.mLayoutMap.keyAt(i) + "): " + this.mLayoutMap.valueAt(i));
    }
  }
  ...
  public void loadLayoutsFromConfig(File file) {
    ...
    try {
        ...
        for (com.android.server.display.config.layout.Layout layout : read.getLayout()) {
          ...
          for (Display display : layout.getDisplay()) {
            createLayout.createDisplayLocked(
                DisplayAddress.fromPhysicalDisplayId(display.getAddress().longValue()),
                display.isDefaultDisplay(),
                display.isEnabled(),
                display.getDisplayGroup(),
                this.mIdProducer,
                getPosition(display.getPosition()),
                0,
                display.getBrightnessThrottlingMapId(),
                display.getRefreshRateZoneId(),
                display.getRefreshRateThermalThrottlingMapId());
          }
        }
        bufferedInputStream.close();
      } finally {
      ...
      Slog.e(
          ...
          "Encountered an error while reading/parsing display layout config file: " + file,
          e);
    }
  }
