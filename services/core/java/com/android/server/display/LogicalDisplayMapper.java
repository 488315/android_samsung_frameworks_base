...
public class LogicalDisplayMapper implements DisplayDeviceRepository.Listener {
  ...
  public LogicalDisplayMapper(
    ...
    this(
        ...
        new DeviceStateToLayoutMap(
            new DisplayIdProducer() { // from class:
                                      // com.android.server.display.LogicalDisplayMapper$$ExternalSyntheticLambda2
  ...
  public LogicalDisplayMapper(
    ...
    this.mIdProducer =
        new DisplayIdProducer() { // from class:
                                  // com.android.server.display.LogicalDisplayMapper$$ExternalSyntheticLambda3
          @Override // com.android.server.display.layout.DisplayIdProducer
          public final int getId(boolean z) {
            int lambda$new$0;
    ...
    this.mSupportsConcurrentInternalDisplays =
        context.getResources().getBoolean(R.bool.config_showPercentageTextDuringRebootToUpdate);
    this.mDeviceStatesOnWhichToWakeUp =
        toSparseBooleanArray(context.getResources().getIntArray(R.array.config_statusBarIcons));
    this.mDeviceStatesOnWhichToSleep =
        toSparseBooleanArray(
            context.getResources().getIntArray(R.array.config_sms_enabled_single_shift_tables));
  ...
  public int[] getDisplayIdsLocked(int i, boolean z) {
    ...
    for (int i3 = 0; i3 < size; i3++) {
      ...
      if ((logicalDisplay.isEnabledLocked() || z)
          && logicalDisplay.getDisplayInfoLocked().hasAccess(i)) {
  ...
  public DisplayInfo getDisplayInfoForStateLocked(int i, int i2) {
    ...
    if (byAddressLocked == null) {
      Slog.w(
          ...
          "The display device ("
              + byId.getAddress()
              + "), is not available for the display state "
              + this.mDeviceState);
      return null;
    }
    ...
    if (displayLocked == null) {
      Slog.w(
          "LogicalDisplayMapper",
          "The logical display associated with address ("
              + byId.getAddress()
              + "), is not available for the display state "
              + this.mDeviceState);
      return null;
  ...
  public void setDeviceStateLocked(int i, boolean z) {
    ...
    Slog.i(
        "LogicalDisplayMapper",
        "Requesting Transition to state: "
            + i
            + ", from state="
            + this.mDeviceState
            + ", interactive="
    ...
    boolean shouldDeviceBePutToSleep =
        shouldDeviceBePutToSleep(
            this.mPendingDeviceState, this.mDeviceState, z, this.mInteractive, this.mBootCompleted);
  ...
  public boolean shouldDeviceBePutToSleep(int i, int i2, boolean z, boolean z2, boolean z3) {
    return i2 != -1
        && this.mDeviceStatesOnWhichToSleep.get(i)
        && !this.mDeviceStatesOnWhichToSleep.get(i2)
        && !z
        && z2
        && z3;
  }
  ...
  public final boolean areAllTransitioningDisplaysOffLocked() {
    ...
    for (int i = 0; i < size; i++) {
      LogicalDisplay logicalDisplay = (LogicalDisplay) this.mLogicalDisplays.valueAt(i);
      if (logicalDisplay.isInTransitionLocked()
          && (primaryDisplayDeviceLocked = logicalDisplay.getPrimaryDisplayDeviceLocked()) != null
          && primaryDisplayDeviceLocked.getDisplayDeviceInfoLocked().state != 1) {
        return false;
      }
    }
    return true;
  ...
  public final void finishStateTransitionLocked(boolean z) {
    ...
    boolean z3 =
        this.mDeviceStatesOnWhichToWakeUp.get(i)
            ...
            && this.mBootCompleted;
    boolean z4 =
        this.mDeviceStatesOnWhichToSleep.get(this.mPendingDeviceState)
            && !this.mDeviceStatesOnWhichToSleep.get(this.mDeviceState)
            && this.mInteractive
            && this.mBootCompleted;
  ...
  public final void handleDisplayDeviceAddedLocked(DisplayDevice displayDevice) {
    ...
    if ((1048576 & i) != 0) {
      ...
      LogicalDisplay createNewLogicalDisplayLocked =
  ...
  public final void handleDisplayDeviceRemovedLocked(DisplayDevice displayDevice) {
    ...
    if (byId.getAddress().equals(displayDeviceInfoLocked.address)) {
      ...
      for (int i = 0; i < this.mLogicalDisplays.size(); i++) {
        DisplayDevice primaryDisplayDeviceLocked =
            ((LogicalDisplay) this.mLogicalDisplays.valueAt(i)).getPrimaryDisplayDeviceLocked();
        if (primaryDisplayDeviceLocked != null) {
          ...
          if ((displayDeviceInfoLocked2.flags & 1) != 0
              && !displayDeviceInfoLocked2.address.equals(displayDeviceInfoLocked.address)) {
  ...
  */
    ...
    while (true) {
      ...
      if (!logicalDisplay.isValidLocked()) {
        ...
        DisplayGroup displayGroupLocked =
            getDisplayGroupLocked(getDisplayGroupIdFromDisplayIdLocked(keyAt));
        if (displayGroupLocked != null) {
          Slog.d(
              "LogicalDisplayMapper",
              "Removing display #" + keyAt + " from group #" + displayGroupLocked.getGroupId());
        ...
        if (z) {
          ...
          Slog.d("LogicalDisplayMapper", "Removed display: " + keyAt);
        }
      } else {
        if (!z) {
          ...
          Slog.i(
              "LogicalDisplayMapper", "Swapping new display: " + keyAt + ": " + displayInfoLocked);
          this.mLogicalDisplaysToUpdate.put(keyAt, 4);
        } else if (!isDirtyLocked && this.mTempDisplayInfo.equals(displayInfoLocked)) {
          if (keyAt == 0
              && displayInfoLocked.state == 2
              && logicalDisplay.getPrimaryDisplayDeviceLocked().getCurrentLayerStackLocked() < 0) {
            Slog.i(
                "LogicalDisplayMapper",
                "Sync with displaystate and layerstack of display: " + keyAt);
          ...
          if (canMoveToDisplayGroupLocked(logicalDisplay)) {
            moveToNewDisplayGroupLocked(logicalDisplay);
          }
  ...
  public final void assignDisplayGroupLocked(LogicalDisplay logicalDisplay) {
    if (logicalDisplay.isValidLocked()) {
      ...
      Integer num =
          (Integer) this.mVirtualDeviceDisplayMapping.get(primaryDisplayDeviceLocked.getUniqueId());
      ...
      boolean z = false;
      boolean z2 =
          ((displayDeviceInfoLocked.flags & 16384) == 0
                  && TextUtils.isEmpty(displayGroupNameLocked)
      ...
      if (displayGroupIdFromDisplayIdLocked == -1 || z3 != z2 || z != z4) {
        if (displayIdLocked == 2) {
          ...
          displayGroupIdFromDisplayIdLocked =
              (!CoreRune.CARLIFE_DISPLAY_GROUP
                      || (displayDeviceInfoLocked.flags & Integer.MIN_VALUE) == 0)
                  ? assignDisplayGroupIdLocked(
                      z2, logicalDisplay.getDisplayGroupNameLocked(), z4, num)
                  : 4;
        }
      }
      ...
      if (displayGroupLocked != displayGroupLocked2) {
        ...
        sb.append(
            displayGroupLocked != null ? Integer.valueOf(displayGroupLocked.getGroupId()) : "null");
  ...
  public final void resetLayoutLocked(int i, int i2, boolean z) {
    ...
    for (int i3 = 0; i3 < size; i3++) {
      ...
      if (primaryDisplayDeviceLocked != null) {
        ...
        if (logicalDisplay.isInTransitionLocked()
            || (byAddress == null || byAddress.isEnabled())
                != (byAddress2 == null || byAddress2.isEnabled())
            || (byAddress != null
                && byAddress2 != null
                && byAddress.getLogicalDisplayId() != byAddress2.getLogicalDisplayId())
            || ((byAddress == null) != (byAddress2 == null))) {
          if (z != logicalDisplay.isInTransitionLocked()) {
            Slog.i(
                "LogicalDisplayMapper",
                "Set isInTransition on display " + displayIdLocked + ": " + z);
          }
  ...
  public final void applyLayoutLocked() {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (byAddressLocked == null) {
        ...
        displayLocked.updateLayoutLimitedRefreshRateLocked(
            displayDeviceConfig.getRefreshRange(at.getRefreshRateZoneId()));
        displayLocked.updateThermalRefreshRateThrottling(
            displayDeviceConfig.getThermalRefreshRateThrottlingData(
                at.getRefreshRateThermalThrottlingMapId()));
        setEnabledLocked(displayLocked, at.isEnabled());
        displayLocked.setThermalBrightnessThrottlingDataIdLocked(
            at.getThermalBrightnessThrottlingMapId() == null
                ? "default"
                : at.getThermalBrightnessThrottlingMapId());
  ...
  public final LogicalDisplay createNewLogicalDisplayLocked(DisplayDevice displayDevice, int i) {
    ...
    if (logicalDisplay.getDisplayInfoLocked().type == 1
        && this.mDeviceStateToLayoutMap.size() > 1) {
  ...
  */
  public final void setEnabledLocked(LogicalDisplay logicalDisplay, boolean z) {
    ...
    if (z && z2) {
      Slog.i(
          "LogicalDisplayMapper",
          "Not creating a logical display for a secondary display because single display demo mode"
              + " is enabled: "
  ...
  public final void initializeDefaultDisplayDeviceLocked(DisplayDevice displayDevice) {
    ...
    layout.createDefaultDisplayLocked(
        displayDevice.getDisplayDeviceInfoLocked().address, this.mIdProducer);
  }
  ...
  public final boolean canMoveToDisplayGroupLocked(LogicalDisplay logicalDisplay) {
    ...
    if (displayIdLocked == 0
        || displayIdLocked == 2
        || logicalDisplay.getPrimaryDisplayDeviceLocked() == null
        || getDisplayGroupIdFromDisplayIdLocked(2) == -1) {
      return false;
    }
    DisplayInfo displayInfoLocked = logicalDisplay.getDisplayInfoLocked();
    DisplayDevice primaryDisplayDeviceLocked = logicalDisplay.getPrimaryDisplayDeviceLocked();
    ...
    return (displayInfoLocked.type == 5
            && !logicalDisplay.hasContentLocked()
            && primaryDisplayDeviceLocked.getDisplayIdToMirrorLocked() == 2)
        || this.mDisplayDeviceRepo.isExternalDisplayDeviceForDexLocked(displayDeviceInfoLocked);
  }
  ...
  public final void moveToNewDisplayGroupLocked(LogicalDisplay logicalDisplay) {
    ...
    int displayGroupIdFromDisplayIdLocked = getDisplayGroupIdFromDisplayIdLocked(displayIdLocked);
    DisplayGroup displayGroupLocked = getDisplayGroupLocked(displayGroupIdFromDisplayIdLocked);
    boolean isDexDisplayDeviceEnabledLocked =
