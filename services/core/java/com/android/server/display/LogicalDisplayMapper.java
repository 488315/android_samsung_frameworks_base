...
public class LogicalDisplayMapper implements DisplayDeviceRepository.Listener {
    ...
    public LogicalDisplayMapper(Context context, DisplayDeviceRepository displayDeviceRepository, Listener listener, DisplayManagerService.SyncRoot syncRoot, Handler handler, DeviceStateToLayoutMap deviceStateToLayoutMap) {
        ...
        this.mSupportsConcurrentInternalDisplays = context.getResources().getBoolean(R.bool.config_showPercentageTextDuringRebootToUpdate);
        this.mDeviceStatesOnWhichToWakeUp = toSparseBooleanArray(context.getResources().getIntArray(R.array.config_statusBarIcons));
        this.mDeviceStatesOnWhichToSleep = toSparseBooleanArray(context.getResources().getIntArray(R.array.config_sms_enabled_single_shift_tables));
        ...
        while (true) {
            ...
            if (!logicalDisplay.isValidLocked()) {
                ...
                if (displayGroupLocked != null) {
                    Slog.d("LogicalDisplayMapper", "Removing display #" + keyAt + " from group #" + displayGroupLocked.getGroupId());
                ...
                if (!z) {
                    ...
                    Slog.i("LogicalDisplayMapper", "Swapping new display: " + keyAt + ": " + displayInfoLocked);
                    this.mLogicalDisplaysToUpdate.put(keyAt, 4);
                } else if (!isDirtyLocked && this.mTempDisplayInfo.equals(displayInfoLocked)) {
                    if (keyAt == 0 && displayInfoLocked.state == 2 && logicalDisplay.getPrimaryDisplayDeviceLocked().getCurrentLayerStackLocked() < 0) {
                        Slog.i("LogicalDisplayMapper", "Sync with displaystate and layerstack of display: " + keyAt);
