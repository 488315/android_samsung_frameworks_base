...
public final class SkinThermalStatusObserver extends IThermalEventListener.Stub
  ...
  public SkinThermalStatusObserver(
      DisplayModeDirector.Injector injector, VotesStorage votesStorage) {
    ...
    synchronized (this.mThermalObserverLock) {
      ...
      this.mHandler.post(
          new Runnable() { // from class:
                           // com.android.server.display.mode.SkinThermalStatusObserver$$ExternalSyntheticLambda2
      ...
      if (this.mLoggingEnabled) {
        Slog.d(
            ...
            "New thermal throttling status , current thermal status = " + status);
      }
    }
  }
  ...
  public void onDisplayRemoved(final int i) {
    synchronized (this.mThermalObserverLock) {
      ...
      this.mHandler.post(
          new Runnable() { // from class:
                           // com.android.server.display.mode.SkinThermalStatusObserver$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
  ...
  public final void lambda$updateThermalRefreshRateThrottling$1(int i) {
    ...
    if (this.mLoggingEnabled) {
      Slog.d(
          "SkinThermalStatusObserver",
          "Updating votes for status=" + i2 + ", display =" + i + ", map=" + sparseArray);
    }
  ...
  public final void reportThrottlingIfNeeded(int i, int i2, SparseArray sparseArray) {
    ...
    Vote forRenderFrameRates =
        findBestMatchingRefreshRateRange != null
            ? Vote.forRenderFrameRates(
                findBestMatchingRefreshRateRange.min, findBestMatchingRefreshRateRange.max)
            : null;
  ...
  public final SurfaceControl.RefreshRateRange findBestMatchingRefreshRateRange(
  ...
  public final void fallbackReportThrottlingIfNeeded(int i, int i2) {
    Vote forRenderFrameRates =
        i2 >= 4
            ? Vote.forRenderFrameRates(DisplayPowerController2.RATE_FROM_DOZE_TO_ON, 60.0f)
            : null;
    this.mVotesStorage.updateVote(i, 16, forRenderFrameRates);
    if (this.mLoggingEnabled) {
      Slog.d(
          "SkinThermalStatusObserver",
          "Voted(fallback): vote=" + forRenderFrameRates + ", display =" + i);
    }
  }
