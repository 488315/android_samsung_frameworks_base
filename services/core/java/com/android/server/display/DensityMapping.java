...
public class DensityMapping {
    ...
    Arrays.sort(
        ...
        Comparator.comparingInt(
            new ToIntFunction() { // from class:
                                  // com.android.server.display.DensityMapping$$ExternalSyntheticLambda0
  ...
  public static void verifyDensityMapping(Entry[] entryArr) {
    for (int i = 1; i < entryArr.length; i++) {
      ...
      if (entry.density > entry2.density) {
        throw new IllegalStateException(
            "Found two entries in the density mapping with increasing diagonal but decreasing"
                + " density: "
                + entry
                + ", "
                + entry2);
  ...
  public class Entry {
    ...
    public String toString() {
      return "DensityMappingEntry{squaredDiagonal="
          + this.squaredDiagonal
          + ", density="
          + this.density
          + '}';
    }
