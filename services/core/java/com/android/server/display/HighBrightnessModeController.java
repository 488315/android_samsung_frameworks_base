        ...
        */
        public void onHdrInfoChanged(IBinder iBinder, final int i, final int i2, final int i3, int i4, final float f) {
            ...
            if (!HighBrightnessModeController.this.mSupportHdrSolution || i <= 0) {
                ...
                int max = Math.max(HighBrightnessModeController.this.mWidth, HighBrightnessModeController.this.mHeight);
                int min = Math.min(HighBrightnessModeController.this.mWidth, HighBrightnessModeController.this.mHeight);
                int max2 = Math.max(i2, i3);
                int min2 = Math.min(i2, i3);
                if (min > 0 && min2 > 0) {
                    int i5 = i2 * i3;
                    int i6 = HighBrightnessModeController.this.mWidth * HighBrightnessModeController.this.mHeight;
                    float f2 = max;
                    float f3 = min;
                    float f4 = f2 / f3;
                    float f5 = max2 / min2;
                    ...
                    if (HighBrightnessModeController.this.mLogLevel > 1) {
                        ...
                        if (HighBrightnessModeController.this.mStaticVRR && this.mIsBrightnessScaledUp && this.mRefreshRateToken == null) {
                            try {
                                this.mRefreshRateToken = this.mDisplayManager.acquireRefreshRateMinLimitToken(this.mToken, 120, str);
            ...
            if (HighBrightnessModeController.this.mSDRDimming > 0 || HighBrightnessModeController.this.mSupportPhotoHdr) {
                ...
                Slog.d(str, "mSupportPhotoHdr: " + HighBrightnessModeController.this.mSupportPhotoHdr + ", SDR Dimming: " + HighBrightnessModeController.this.mSDRDimming + ", mIsHdrLayerPresent: " + HighBrightnessModeController.this.mIsHdrLayerPresent + ", numberOfHdrLayers: " + i + ", HdrLayerSize: " + i2 + "x" + i3 + str4 + HighBrightnessModeController.this.mWidth + "x" + HighBrightnessModeController.this.mHeight + "mThresRatio: " + HighBrightnessModeController.this.mThresRatio);
                return;
            }
            HighBrightnessModeController.this.mHandler.post(new Runnable() { // from class: com.android.server.display.HighBrightnessModeController$HdrListener$$ExternalSyntheticLambda1
                @Override // java.lang.Runnable
                public final void run() {
                    HighBrightnessModeController.HdrListener.this.lambda$onHdrInfoChanged$1();
                }
            });
            Slog.d(str, "mSupportPhotoHdr: " + HighBrightnessModeController.this.mSupportPhotoHdr + ", SDR Dimming: " + HighBrightnessModeController.this.mSDRDimming + "mIsHdrLayerPresent: " + HighBrightnessModeController.this.mIsHdrLayerPresent);
        }
