...
public class AutomaticBrightnessController {
        ...
        this.mIntentReceiver = new BroadcastReceiver() { // from class: com.android.server.display.AutomaticBrightnessController.1
            ...
            public void onReceive(Context context2, Intent intent) {
                ...
                if (floatExtra < DisplayPowerController2.RATE_FROM_DOZE_TO_ON) {
                    Slog.m72d("AutomaticBrightnessController", "[CAB] lux value is not received lux: " + floatExtra + "  action: " + action);
                    return;
                }
                if (PowerManagerUtil.SEC_FEATURE_USE_PMS_LOG || Build.IS_ENG) {
                    Slog.m72d("AutomaticBrightnessController", "[CAB] onReceive lux: " + floatExtra + "  action: " + action);
                }
    ...
    public final boolean setDisplayPolicy(int i) {
        ...
        if (this.mLoggingEnabled) {
            Slog.m72d("AutomaticBrightnessController", "Display policy transitioning from " + i2 + " to " + i);
        }
    ...
    public final boolean setScreenBrightnessByUser(float f) {
        ...
        if (!isValidUserLux(calculateAmbientLux)) {
            if (PowerManagerUtil.SEC_FEATURE_SUPPORT_HBM && this.mAmbientLux >= PowerManagerUtil.HBM_LUX) {
                Slog.m72d("AutomaticBrightnessController", "setScreenBrightnessByUser: cancel currentLux: " + calculateAmbientLux);
                return false;
            }
        } else {
    ...
    public final void adjustLightSensorRate(int i) {
        if (i != this.mCurrentLightSensorRate) {
            if (this.mLoggingEnabled) {
                Slog.m72d("AutomaticBrightnessController", "adjustLightSensorRate: previousRate=" + this.mCurrentLightSensorRate + ", currentRate=" + i);
            }
    ...
    public final void setAmbientLux(float f) {
        ...
        if (f < DisplayPowerController2.RATE_FROM_DOZE_TO_ON) {
            Slog.m79w("AutomaticBrightnessController", "Ambient lux was negative, ignoring and setting to 0");
            f = 0.0f;
        }
        sendAmbientLux(f);
        this.mAmbientLux = f;
    ...
    public final float calculateAmbientLux(long j, long j2) {
        ...
        if (size == 0) {
            Slog.m74e("AutomaticBrightnessController", "calculateAmbientLux: No ambient light readings available");
            return -1.0f;
        }
        if (PowerManagerUtil.CAMERA_ADAPTIVE_BRIGHTNESS_SUPPORTED || this.mIsCameraLightTypeSupported) {
        ...
        if (this.mLoggingEnabled) {
            Slog.m72d("AutomaticBrightnessController", "calculateAmbientLux: selected endIndex=" + i3 + ", point=(" + this.mAmbientLightRingBuffer.getTime(i3) + ", " + this.mAmbientLightRingBuffer.getLux(i3) + ")");
        }
        ...
        if (this.mLoggingEnabled) {
            Slog.m72d("AutomaticBrightnessController", "calculateAmbientLux: totalWeight=" + f + ", newAmbientLux=" + (f2 / f));
        }
        return f2 / f;
    }
    ...
    */
    public final void updateAmbientLux(long j) {
        ...
        if (!this.mAmbientLuxValid || this.mShouldApplyEarlyWakeUp) {
            ...
            if (j < j2) {
                if (this.mLoggingEnabled) {
                    Slog.m72d("AutomaticBrightnessController", "updateAmbientLux: Sensor not ready yet: time=" + j + ", timeWhenSensorWarmedUp=" + j2);
                }
            ...
            this.mAmbientLuxValid = true;
            if (this.mLoggingEnabled) {
                Slog.m72d("AutomaticBrightnessController", "updateAmbientLux: Initializing: mAmbientLightRingBuffer=" + this.mAmbientLightRingBuffer + ", mAmbientLux=" + this.mAmbientLux);
            }
        ...
        if (this.mUseLightSensorBlockingPrevention && this.mProximitySensor.getType() == 65640 && this.mIsLightSensorPositionUpdated) {
        ...
        if (this.mUseLightSensorBlockingPrevention) {
            if (this.mProximitySensor.getType() == 65640) {
                if (this.mIsLightSensorPositionUpdated) {
                    ...
                    if (this.mIsRearLightSensor) {
                        ...
                        Slog.m72d("AutomaticBrightnessController", "Last front lux: " + this.mLastAmbientLuxOfFrontLightSensor + ", Current rear lux: " + this.mFastAmbientLux);
                        i = (Math.max(this.mLastAmbientLuxOfFrontLightSensor, 1.0f) / Math.max(this.mFastAmbientLux, 1.0f) > 5.0f || this.mFastAmbientLux < 50.0f) ? 2 : 1;
                    } else {
                        i = 0;
                    }
                    if (this.mBrightnessThresoldsType != i) {
                        ...
                        if ((f4 <= this.mAmbientDarkeningThreshold && !this.mIsRearLightSensor) || f4 >= this.mAmbientBrighteningThreshold) {
                            z = true;
                        }
            ...
            if (this.mLoggingEnabled) {
                Slog.m72d("AutomaticBrightnessController", "updateAmbientLux: Scheduling ambient lux update for " + min + TimeUtils.formatUptime(min));
            }
    ...
    public final void updateAutoBrightness(boolean z, boolean z2) {
        if (this.mAmbientLuxValid) {
            ...
            if (this.mLoggingEnabled) {
                Slog.m72d("AutomaticBrightnessController", "updateAutoBrightness: mScreenAutoBrightness=" + this.mScreenAutoBrightness + ", newScreenAutoBrightness=" + brightnessIntToFloat);
            }
        ...
        if (this.mBrightnessAdjustmentSamplePending) {
            ...
            if (this.mAmbientLuxValid) {
                ...
                if (f >= DisplayPowerController2.RATE_FROM_DOZE_TO_ON || f == DisplayPowerController2.RATE_FROM_DOZE_TO_ON) {
                    if (this.mLoggingEnabled) {
                        Slog.m72d("AutomaticBrightnessController", "Auto-brightness adjustment changed by user: lux=" + this.mAmbientLux + ", brightness=" + this.mScreenAutoBrightness + ", ring=" + this.mAmbientLightRingBuffer);
                    }
    ...
    public final void updateForegroundAppSync() {
        if (this.mLoggingEnabled) {
            Slog.m72d("AutomaticBrightnessController", "Updating foreground app: packageName=" + this.mPendingForegroundAppPackageName + ", category=" + this.mPendingForegroundAppCategory);
        }
    ...
    public void switchToIdleMode() {
        ...
        Slog.m76i("AutomaticBrightnessController", "mPreviousShortTermModel" + this.mPausedShortTermModel);
    ...
    public void switchToInteractiveScreenBrightnessMode() {
        if (this.mCurrentBrightnessMapper.isForIdleMode()) {
            ...
            this.mHandler.sendEmptyMessageAtTime(7, this.mClock.uptimeMillis() + this.mCurrentBrightnessMapper.getShortTermModelTimeout());
            Slog.m76i("AutomaticBrightnessController", "mPreviousShortTermModel" + this.mPausedShortTermModel.toString());
    ...
    public final void lambda$injectLux$0(SensorEvent sensorEvent) {
        boolean z = false;
        if (!this.mAmbientLuxValid) {
            Slog.m72d("AutomaticBrightnessController", "onSensorChanged: 1st lux : " + sensorEvent.values[0]);
        }
        if (this.mLightSensorEnabled) {
            long uptimeMillis = this.mClock.uptimeMillis();
            ...
            if (this.mIsUnderDisplayLightSensor) {
                if (fArr[1] > DisplayPowerController2.RATE_FROM_DOZE_TO_ON) {
                    ...
                    this.mRecentLightSamples = 0;
                    this.mAmbientLightRingBuffer.clear();
                    Slog.m72d("AutomaticBrightnessController", "onSensorChanged: min lux : " + sensorEvent.values[0]);
                }
