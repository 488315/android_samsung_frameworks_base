...
public class DeviceStorageMonitorService extends SystemService {
  ...
  public class State {
    ...
    public static String levelToString(int i) {
      return i != -1
          ? i != 0
              ? i != 1
                  ? i != 2 ? i != 3 ? i != 4 ? Integer.toString(i) : "FULL" : "LOW" : "WARNING"
                  : "CAUTION"
              : "NORMAL"
          : "UNKNOWN";
    }
  ...
  public class FileNodeState {
    ...
    public static String levelToString(int i) {
      return i != -1
          ? i != 0 ? i != 1 ? i != 2 ? Integer.toString(i) : "FN_FULL" : "FN_LOW" : "FN_NORMAL"
          : "FN_UNKNOWN";
    }
  }
  ...
  */
    ...
    for (VolumeInfo volumeInfo : storageManager2.getWritablePrivateVolumes()) {
      ...
      if (path.getUsableSpace() < (3 * bytes) / 2) {
        Slog.w(
            ...
            "check(" + volumeInfo.path + ") freeStorage = " + path.getUsableSpace());
        try {
          str = "check(";
          storageManager = storageManager2;
        } catch (IOException e) {
        ...
        try {
          ((PackageManagerInternal) LocalServices.getService(PackageManagerInternal.class))
              .freeStorage(volumeInfo.getFsUuid(), bytes * 2, 0);
      ...
      if (usableSpace > j2 && !State.isExhaustion) {
        ...
        if (this.mForceLevel == -1) {
          ...
          i3 = 3;
          if (j > bytes
              && (!StorageManager.UUID_DEFAULT.equals(convert2)
                  || j >= BOOT_IMAGE_STORAGE_REQUIREMENT)) {
  ...
  public final void checkHigh() {
    ...
    for (VolumeInfo volumeInfo : storageManager.getWritablePrivateVolumes()) {
      ...
      if (path.getUsableSpace() < (path.getTotalSpace() * i) / 100) {
        try {
          ((PackageManagerInternal) LocalServices.getService(PackageManagerInternal.class))
              .freeAllAppCacheAboveQuota(volumeInfo.getFsUuid());
        } catch (IOException e) {
          Slog.w("DeviceStorageMonitorService", e);
        }
      }
  ...
  public DeviceStorageMonitorService(Context context) {
    ...
    this.mLocalService =
        new DeviceStorageMonitorInternal() { // from class:
                                             // com.android.server.storage.DeviceStorageMonitorService.2
          ...
          public long getMemoryLowThreshold() {
            return ((StorageManager)
                    DeviceStorageMonitorService.this
                        .getContext()
    ...
    this.mRemoteService =
        new Binder() { // from class: com.android.server.storage.DeviceStorageMonitorService.3
          @Override // android.os.Binder
          public void dump(
              FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
            if (DumpUtils.checkDumpPermission(
                DeviceStorageMonitorService.this.getContext(),
                "DeviceStorageMonitorService",
                printWriter)) {
          ...
          public void onShellCommand(
            ...
            DeviceStorageMonitorService.this.new Shell()
                .exec(
                    this,
                    fileDescriptor,
                    fileDescriptor2,
  ...
  public void onStart() {
    ...
    if (context.getPackageManager().hasSystemFeature("android.software.leanback")) {
      this.mNotifManager.createNotificationChannel(
          new NotificationChannel(
              "devicestoragemonitor.tv",
              context.getString(R.string.foreground_service_apps_in_background),
              4));
    }
  ...
  public final void updateExhaustionBroadcasts(VolumeInfo volumeInfo, boolean z, int i) {
    Slog.w(
        "DeviceStorageMonitorService",
        "updateExhaustionBroadcasts(" + volumeInfo.path + ") seq:" + i);
    Intent putExtra =
        new Intent("com.samsung.intent.action.DEVICE_STORAGE_EXHAUSTION")
            .addFlags(67108864)
            .addFlags(16777216)
            .putExtra("seq", i);
    Intent putExtra2 =
        new Intent("com.samsung.intent.action.DEVICE_STORAGE_NOT_EXHAUSTION")
            .addFlags(67108864)
            .putExtra("seq", i);
    if (z) {
      loge(
          "updateExhaustionBroadcasts("
              + volumeInfo.path
              + ") sending intent : ACTION_DEVICE_STORAGE_EXHAUSTION");
    ...
    loge(
        "updateExhaustionBroadcasts("
            + volumeInfo.path
            + ") sending intent : ACTION_DEVICE_STORAGE_NOT_EXHAUSTION");
    loge("mUsableBytes : " + mUsableBytes + ", mUsableFileNode : " + mUsableFileNode);
  ...
  */
  public final void updateBroadcasts(VolumeInfo volumeInfo, int i, int i2, int i3) {
    ...
    Intent putExtra =
        new Intent("com.samsung.intent.action.DEVICE_STORAGE_CAUTION_ON")
            .addFlags(67108864)
            .addFlags(16777216)
            .putExtra("seq", i3);
    Intent putExtra2 =
        new Intent("com.samsung.intent.action.DEVICE_STORAGE_CAUTION_OFF")
            .addFlags(67108864)
            .putExtra("seq", i3);
    if (State.isEntering(1, i, i2)) {
      loge(
          "updateBroadcasts("
              + volumeInfo.path
              + ") sending intent : ACTION_DEVICE_STORAGE_CAUTION_ON");
      loge("mUsableBytes : " + mUsableBytes + ", mUsableFileNode : " + mUsableFileNode);
      getContext().sendStickyBroadcastAsUser(putExtra, UserHandle.ALL);
      ...
      loge(
          "updateBroadcasts("
              + volumeInfo.path
              + ") sending intent : ACTION_DEVICE_STORAGE_CAUTION_OFF");
      ...
      String str2 = str;
      Intent putExtra3 =
          new Intent("com.samsung.intent.action.DEVICE_STORAGE_WARNING_ON")
              .addFlags(67108864)
              .addFlags(16777216)
              .putExtra(str2, i3);
      Intent putExtra4 =
          new Intent("com.samsung.intent.action.DEVICE_STORAGE_WARNING_OFF")
              .addFlags(67108864)
              .putExtra(str2, i3);
      if (!State.isEntering(2, i, i2)) {
        loge(
            "updateBroadcasts("
                + volumeInfo.path
                + ") sending intent : ACTION_DEVICE_STORAGE_WARNING_ON");
        loge("mUsableBytes : " + mUsableBytes + ", mUsableFileNode : " + mUsableFileNode);
        ...
        loge(
            "updateBroadcasts("
                + volumeInfo.path
                + ") sending intent : ACTION_DEVICE_STORAGE_WARNING_OFF");
        loge("mUsableBytes : " + mUsableBytes + ", mUsableFileNode : " + mUsableFileNode);
        getContext().removeStickyBroadcastAsUser(putExtra3, UserHandle.ALL);
        getContext().sendBroadcastAsUser(putExtra4, UserHandle.ALL);
      }
      Intent putExtra5 =
          new Intent("android.intent.action.DEVICE_STORAGE_LOW")
              .addFlags(85983232)
              .putExtra(str2, i3);
      Intent putExtra6 =
          new Intent("android.intent.action.DEVICE_STORAGE_OK")
              .addFlags(85983232)
              .putExtra(str2, i3);
      if (!State.isEntering(3, i, i2)) {
        loge(
            "updateBroadcasts(" + volumeInfo.path + ") sending intent : ACTION_DEVICE_STORAGE_LOW");
        loge("mUsableBytes : " + mUsableBytes + ", mUsableFileNode : " + mUsableFileNode);
      ...
      Intent putExtra7 =
          new Intent("android.intent.action.DEVICE_STORAGE_FULL")
              .addFlags(67108864)
              .putExtra(str2, i3);
      Intent putExtra8 =
          new Intent("android.intent.action.DEVICE_STORAGE_NOT_FULL")
              .addFlags(67108864)
              .putExtra(str2, i3);
      Intent putExtra9 =
          new Intent("com.samsung.intent.action.DEVICE_STORAGE_FULL")
              .addFlags(67108864)
              .putExtra(str2, i3);
      Intent putExtra10 =
          new Intent("com.samsung.intent.action.DEVICE_STORAGE_NOT_FULL")
              .addFlags(67108864)
              .putExtra(str2, i3);
      if (!State.isEntering(4, i, i2)) {
        loge(
            "updateBroadcasts("
                + volumeInfo.path
                + ") sending intent : ACTION_DEVICE_STORAGE_FULL");
        loge("mUsableBytes : " + mUsableBytes + ", mUsableFileNode : " + mUsableFileNode);
        getContext().sendStickyBroadcastAsUser(putExtra7, UserHandle.ALL);
        putExtra9.setPackage(
  ...
  public final void updateBroadcasts_filenode(VolumeInfo volumeInfo, int i, int i2, int i3) {
    ...
    if (Objects.equals(StorageManager.UUID_PRIVATE_INTERNAL, volumeInfo.getFsUuid())) {
      new StatFs(volumeInfo.path);
      Intent putExtra =
          new Intent("com.samsung.intent.action.DEVICE_FILENODE_FULL")
              .addFlags(85983232)
              .putExtra("seq", i3);
      Intent putExtra2 =
          new Intent("com.samsung.intent.action.DEVICE_FILENODE_NOT_FULL ")
              .addFlags(85983232)
              .putExtra("seq", i3);
      if (FileNodeState.isEntering(1, i, i2)) {
        loge(
            "updateBroadcasts_filenode("
                + volumeInfo.path
                + ") sending intent : ACTION_DEVICE_FILENODE_FULL");
        loge("mUsableBytes : " + mUsableBytes + ", mUsableFileNode : " + mUsableFileNode);
        getContext().sendStickyBroadcastAsUser(putExtra, UserHandle.ALL);
        return;
      }
      if (FileNodeState.isLeaving(1, i, i2)) {
        loge(
            "updateBroadcasts_filenode("
                + volumeInfo.path
                + ") sending intent : ACTION_DEVICE_FILENODE_NOT_FULL");
        loge("mUsableBytes : " + mUsableBytes + ", mUsableFileNode : " + mUsableFileNode);
        getContext().removeStickyBroadcastAsUser(putExtra, UserHandle.ALL);
        getContext().sendBroadcastAsUser(putExtra2, UserHandle.ALL);
      }
