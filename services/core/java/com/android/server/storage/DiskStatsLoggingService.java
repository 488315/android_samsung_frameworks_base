...
public class DiskStatsLoggingService extends JobService {
  public static ComponentName sDiskStatsLoggingService =
  ...
  public boolean onStartJob(JobParameters jobParameters) {
    ...
    logRunnable.setDownloadsDirectory(
  ...
  public static void schedule(Context context) {
    ((JobScheduler) context.getSystemService("jobscheduler"))
        .schedule(
            new JobInfo.Builder(1145656139, sDiskStatsLoggingService)
  ...
  class LogRunnable implements Runnable {
    ...
    public void run() {
      boolean z = true;
      try {
        FileCollector.MeasurementResult measurementResult =
            FileCollector.getMeasurementResult(this.mContext);
        FileCollector.MeasurementResult measurementResult2 =
            FileCollector.getMeasurementResult(this.mDownloadsDirectory);
    ...
    public final void logToFile(
        ...
        List list,
        long j) {
      DiskStatsFileLogger diskStatsFileLogger =
          new DiskStatsFileLogger(measurementResult, measurementResult2, list, j);
      try {
