...
public final class StorageSessionController {
  ...
  public final String createVolumeInfoStrForPulbicVolume(VolumeInfo volumeInfo) {
    ...
    return ((((("VolumeInfo" + String.format("{%s}:", volumeInfo.id))
                        + KnoxVpnFirewallHelper.DELIMITER_IP_RESTORE)
                    + String.format("type=%s diskId=%s partGuid=%s", valueToString, diskId, str))
                + String.format(
                    " mountFlags=%s mountUserId=%d state=%s",
                    flagsToString, Integer.valueOf(volumeInfo.getMountUserId()), valueToString2))
            + KnoxVpnFirewallHelper.DELIMITER_IP_RESTORE)
        + String.format(
            " fsType=%s fsUuid=%s path=%s internalPath=%s",
            volumeInfo.fsType, volumeInfo.fsUuid, volumeInfo.path, volumeInfo.internalPath);
  }
  ...
  public int getConnectionUserIdForVolume(VolumeInfo volumeInfo) {
    boolean isMediaSharedWithParent =
        ((UserManager)
                this.mContext
                    .createContextAsUser(UserHandle.of(volumeInfo.mountUserId), 0)
      ...
      synchronized (this.mLock) {
        ...
        if (storageUserConnection == null) {
          ...
          storageUserConnection =
              new StorageUserConnection(this.mContext, connectionUserIdForVolume, this);
        ...
        storageUserConnection.startSession(
            id,
            parcelFileDescriptor,
            volumeInfo.getPath().getPath(),
            volumeInfo.getInternalPath().getPath());
      }
    }
  }
      ...
      synchronized (this.mLock) {
        ...
        if (storageUserConnection != null) {
          ...
          storageUserConnection.notifyVolumeStateChanged(
              id, volumeInfo.buildStorageVolume(this.mContext, volumeInfo.getMountUserId(), false));
        } else {
          Slog.w(
              "StorageSessionController",
              "No available storage user connection for userId : " + connectionUserIdForVolume);
        }
      }
    }
  }
  ...
  public void freeCache(String str, long j) {
    synchronized (this.mLock) {
      ...
      for (int i = 0; i < size; i++) {
        StorageUserConnection storageUserConnection =
            (StorageUserConnection) this.mConnections.get(this.mConnections.keyAt(i));
        if (storageUserConnection != null) {
  ...
  public void onVolumeUnmount(VolumeInfo volumeInfo) {
    ...
    if (volumeInfo.type == 0) {
      Slog.i(
          "StorageSessionController",
          "On volume unmount " + createVolumeInfoStrForPulbicVolume(volumeInfo));
    } else {
    ...
    if (shouldHandle(null)) {
      ...
      synchronized (this.mLock) {
        ...
        for (int i = 0; i < this.mConnections.size(); i++) {
          sparseArray.put(
              this.mConnections.keyAt(i), (StorageUserConnection) this.mConnections.valueAt(i));
        }
      }
      for (int i2 = 0; i2 < sparseArray.size(); i2++) {
        StorageUserConnection storageUserConnection =
            (StorageUserConnection) sparseArray.valueAt(i2);
        for (String str : storageUserConnection.getAllSessionIds()) {
          ...
          try {
            ...
            Slog.e(
                "StorageSessionController",
                "Failed to exit session: " + str + ". Killing MediaProvider...",
                e2);
  ...
  public final void initExternalStorageServiceComponent() {
    ...
    ComponentName componentName =
        new ComponentName(
            resolveExternalStorageServiceAsUser.packageName,
            resolveExternalStorageServiceAsUser.name);
    if (!"android.permission.BIND_EXTERNAL_STORAGE_SERVICE"
        .equals(resolveExternalStorageServiceAsUser.permission)) {
      throw new ExternalStorageServiceException(
          componentName.flattenToShortString()
              + " does not require permission android.permission.BIND_EXTERNAL_STORAGE_SERVICE");
    }
    ...
    try {
      ActivityManager.getService()
          .killApplication(
              ...
              this.mExternalStorageServiceAppId,
              i,
              "storage_session_controller reset",
              13);
  ...
  public final ServiceInfo resolveExternalStorageServiceAsUser(int i) {
    ...
    ResolveInfo resolveServiceAsUser =
        this.mContext.getPackageManager().resolveServiceAsUser(intent, 786564, i);
