...
public abstract class ProviderSession implements RemoteCredentialService.ProviderCallbacks {
  ...
  public abstract ProviderData mo4615prepareUiData();
    ...
    this.mProviderSessionMetric =
        new ProviderSessionMetric(
            ((RequestSession) providerInternalCallback)
                .mRequestSessionMetric.getSessionIdTrackTwo());
  }
  ...
  public void updateStatusAndInvokeCallback(Status status, CredentialsSource credentialsSource) {
    ...
    this.mProviderSessionMetric.collectCandidateMetricUpdate(
        isTerminatingStatus(status) || isStatusWaitingForRemoteResponse(status),
        isCompletionStatus(status) || isUiInvokingStatus(status),
        this.mProviderSessionUid,
        credentialsSource == CredentialsSource.AUTH_ENTRY,
        credentialProviderInfo != null && credentialProviderInfo.isPrimary());
    this.mCallbacks.onProviderStatusChanged(status, this.mComponentName, credentialsSource);
  }

  public void startCandidateMetrics() {
    this.mProviderSessionMetric.collectCandidateMetricSetupViaInitialMetric(
        ((RequestSession) this.mCallbacks).mRequestSessionMetric.getInitialPhaseMetric());
  }

  public Boolean isProviderResponseSet() {
    return Boolean.valueOf(
        this.mProviderResponse != null || this.mProviderResponseSet.booleanValue());
  }
    ...
    try {
      ...
      if (applicationInfo != null) {
        if (this.mContext.checkPermission(
                "android.permission.PROVIDE_REMOTE_CREDENTIALS", -1, applicationInfo.uid)
            == 0) {
          return true;
        }
      }
      ...
      Slog.e(
          "ProviderSession", "Error getting info for " + this.mComponentName.flattenToString(), e);
      return false;
    }
  }
}
