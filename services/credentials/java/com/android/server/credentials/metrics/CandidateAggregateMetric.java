...
public class CandidateAggregateMetric {
    ...
    while (it.hasNext()) {
      ...
      if (candidatePhasePerProviderMetric.getCandidateUid() == -1) {
        ...
        if (this.mServiceBeganTimeNanoseconds == -1) {
          this.mServiceBeganTimeNanoseconds =
              candidatePhasePerProviderMetric.getServiceBeganTimeNanoseconds();
        }
        this.mQueryReturned =
            this.mQueryReturned || candidatePhasePerProviderMetric.isQueryReturned();
        ResponseCollective responseCollective =
            candidatePhasePerProviderMetric.getResponseCollective();
        ResponseCollective.combineTypeCountMaps(
            linkedHashMap, responseCollective.getResponseCountsMap());
        ResponseCollective.combineTypeCountMaps(
            linkedHashMap2, responseCollective.getEntryCountsMap());
        ...
        if (!candidatePhasePerProviderMetric.getFrameworkException().isEmpty()) {
          this.mExceptionCountQuery.put(
              candidatePhasePerProviderMetric.getFrameworkException(),
              Integer.valueOf(
                  ((Integer)
                              this.mExceptionCountQuery.getOrDefault(
                                  candidatePhasePerProviderMetric.getFrameworkException(), 0))
                          .intValue()
                      + 1));
        }
      }
  ...
  public final void collectAuthAggregates(Map map) {
    ...
    Iterator it = map.values().iterator();
    while (it.hasNext()) {
      for (BrowsedAuthenticationMetric browsedAuthenticationMetric :
          ((ProviderSession) it.next())
              .getProviderSessionMetric()
        ...
        if (browsedAuthenticationMetric.getProviderUid() != -1) {
          ...
          ResponseCollective authEntryCollective =
              browsedAuthenticationMetric.getAuthEntryCollective();
          ResponseCollective.combineTypeCountMaps(
              linkedHashMap, authEntryCollective.getResponseCountsMap());
          ResponseCollective.combineTypeCountMaps(
              linkedHashMap2, authEntryCollective.getEntryCountsMap());
          ...
          if (!browsedAuthenticationMetric.getFrameworkException().isEmpty()) {
            this.mExceptionCountQuery.put(
                ...
                Integer.valueOf(
                    ((Integer)
                                this.mExceptionCountQuery.getOrDefault(
                                    browsedAuthenticationMetric.getFrameworkException(), 0))
                            .intValue()
                        + 1));
          }
        }
  ...
  public int[] getUniqueExceptionCountsQuery() {
    return this.mExceptionCountQuery.values().stream()
  ...
  public int[] getUniqueExceptionCountsAuth() {
    return this.mExceptionCountAuth.values().stream()
        .mapToInt(new AudioService$$ExternalSyntheticLambda0())
        .toArray();
  }
