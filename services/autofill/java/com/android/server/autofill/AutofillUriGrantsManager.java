...
public final class AutofillUriGrantsManager {
  ...
  public void grantUriPermissions(
      ComponentName componentName, IBinder iBinder, int i, ClipData clipData) {
    String packageName = componentName.getPackageName();
    IBinder uriPermissionOwnerForActivity =
        this.mActivityTaskMgrInternal.getUriPermissionOwnerForActivity(iBinder);
    if (uriPermissionOwnerForActivity == null) {
      Slog.w(
          TAG,
          "Can't grant URI permissions, because the target activity token is invalid: clip="
              + clipData
              + ", targetActivity="
              + componentName
              + ", targetUserId="
    ...
    try {
      ...
      try {
        this.mUgm.grantUriPermissionFromOwner(
            iBinder, this.mSourceUid, str, uriWithoutUserId, 1, userIdFromUri, i);
      } catch (RemoteException e2) {
        e = e2;
        Slog.e(
            TAG,
            "Granting URI permissions failed: uri="
                + uri
                + str5
                + this.mSourceUid
                + str2
