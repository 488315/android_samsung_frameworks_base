...
public class CompanionDevicePresenceMonitor
    implements AssociationStore.OnChangeListener,
        BluetoothCompanionDeviceConnectionListener.Callback,
        BleCompanionDeviceScanner.Callback {
  ...
  public final SimulatedDevicePresenceSchedulerHelper mSchedulerHelper =
  ...
  public CompanionDevicePresenceMonitor(
      UserManager userManager, AssociationStore associationStore, Callback callback) {
    this.mAssociationStore = associationStore;
    this.mCallback = callback;
    this.mBtConnectionListener =
        new BluetoothCompanionDeviceConnectionListener(userManager, associationStore, this);
  ...
  public boolean isDevicePresent(int i) {
    return this.mReportedSelfManagedDevices.contains(Integer.valueOf(i))
        || this.mConnectedBtDevices.contains(Integer.valueOf(i))
        || this.mNearbyBleDevices.contains(Integer.valueOf(i))
        || this.mSimulated.contains(Integer.valueOf(i));
  }
  ...
  public void onBluetoothCompanionDeviceDisconnected(int i) {
    if (this.mNearbyBleDevices.remove(Integer.valueOf(i))) {
      Log.d(
          "CDM_CompanionDevicePresenceMonitor",
          "Bluetooth device disconnect was detected. Pre-emptively marking the BLE device as"
              + " lost.");
    }
  ...
  public final void onDevicePresent(Set set, int i, String str) {
    ...
    if (!set.add(Integer.valueOf(i))) {
      Slog.i(
          ...
          "Association with id "
              + i
              + " is ALREADY reported as present by this source ("
              + str
              + ")");
    }
  ...
  public final void onDeviceGone(Set set, int i, String str) {
    ...
    if (!set.remove(Integer.valueOf(i))) {
      Slog.w(
          "CDM_CompanionDevicePresenceMonitor",
          "Association with id " + i + " was NOT reported as present by this source (" + str + ")");
      return;
    }
    if (isDevicePresent(i)) {
  ...
  public void dump(PrintWriter printWriter) {
    ...
    if (this.mConnectedBtDevices.isEmpty()
        && this.mNearbyBleDevices.isEmpty()
        && this.mReportedSelfManagedDevices.isEmpty()) {
            ...
            .append(
                (CharSequence)
                    this.mAssociationStore
                        .getAssociationById(((Integer) it.next()).intValue())
                        .toShortString())
            .append('\n');
      }
    }
      ...
      while (it2.hasNext()) {
        printWriter
            ...
            .append(
                (CharSequence)
                    this.mAssociationStore
                        .getAssociationById(((Integer) it2.next()).intValue())
                        .toShortString())
            .append('\n');
      }
    }
    ...
    while (it3.hasNext()) {
      printWriter
          ...
          .append(
              (CharSequence)
                  this.mAssociationStore
                      .getAssociationById(((Integer) it3.next()).intValue())
                      .toShortString())
          .append('\n');
    }
  }
    ...
    public void handleMessage(Message message) {
      ...
      if (CompanionDevicePresenceMonitor.this.mSimulated.contains(Integer.valueOf(i))) {
        CompanionDevicePresenceMonitor companionDevicePresenceMonitor =
            CompanionDevicePresenceMonitor.this;
        companionDevicePresenceMonitor.onDeviceGone(
            companionDevicePresenceMonitor.mSimulated, i, "simulated");
      }
    }
  }
}
