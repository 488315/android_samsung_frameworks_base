...
public class BleCompanionDeviceScanner implements AssociationStore.OnChangeListener {
  public static final ScanSettings SCAN_SETTINGS =
      new ScanSettings.Builder()
          ...
          .setScanMode(0)
          .setMatchMode(101)
          .semSetScanFilterRssiThreshold(-75)
          .build();
  ...
  public final ScanCallback mScanCallback = new ScanCallback() { // from class:
        // com.android.server.companion.presence.BleCompanionDeviceScanner.2
        @Override // android.bluetooth.le.ScanCallback
        public void onScanResult(int i, ScanResult scanResult) {
          BluetoothDevice device = scanResult.getDevice();
          if (i == 2) {
            if (BleCompanionDeviceScanner.this.mMainThreadHandler.hasNotifyDeviceLostMessages(
                device)) {
              BleCompanionDeviceScanner.this.mMainThreadHandler.removeNotifyDeviceLostMessages(
                  device);
              Log.d(
                  "CDM_BleCompanionDeviceScanner",
                  "FIRST_MATCH received during pending MATCH_LOST: return");
              return;
            } else {
          ...
          if (i == 4) {
            BleCompanionDeviceScanner.this.mMainThreadHandler.sendNotifyDeviceLostDelayedMessage(
                device);
            return;
          }
          Slog.wtf(
              "CDM_BleCompanionDeviceScanner",
              "Unexpected callback " + BleCompanionDeviceScanner.nameForBleScanCallbackType(i));
        }
    ...
    if (Looper.getMainLooper().isCurrentThread()) {
      ...
      this.mMainThreadHandler.post(
          new Runnable() { // from class:
            // com.android.server.companion.presence.BleCompanionDeviceScanner$$ExternalSyntheticLambda0
  ...
  public final void startScan() {
    ...
    for (AssociationInfo associationInfo : this.mAssociationStore.getAssociations()) {
      if (associationInfo.isNotifyOnDeviceNearby()
          && (deviceMacAddressAsString = associationInfo.getDeviceMacAddressAsString()) != null) {
  ...
  public final void notifyDeviceFound(BluetoothDevice bluetoothDevice) {
    Iterator it =
        this.mAssociationStore.getAssociationsByAddress(bluetoothDevice.getAddress()).iterator();
    while (it.hasNext()) {
  ...
  public static String nameForBleScanCallbackType(int i) {
    return (i != 1 ? i != 2 ? i != 4 ? "Unknown" : "MATCH_LOST" : "FIRST_MATCH" : "ALL_MATCHES")
        + "("
        + i
        + ")";
  }
