...
public abstract class PermissionsUtils {
  ...
  public static void enforcePermissionsForAssociation(
      Context context, AssociationRequest associationRequest, int i) {
  ...
  public static void enforceRequestDeviceProfilePermissions(Context context, String str, int i) {
    ...
    throw new SecurityException(
        "Application must hold " + str2 + " to associate with a device with " + str + " profile.");
  }

  public static void enforceRequestSelfManagedPermission(Context context, int i) {
    if (context.checkPermission(
            "android.permission.REQUEST_COMPANION_SELF_MANAGED", Binder.getCallingPid(), i)
        != 0) {
      throw new SecurityException(
          "Application does not hold android.permission.REQUEST_COMPANION_SELF_MANAGED");
    }
  }

  public static boolean checkCallerCanInteractWithUserId(Context context, int i) {
    return UserHandle.getCallingUserId() == i
        || context.checkCallingPermission("android.permission.INTERACT_ACROSS_USERS") == 0;
  }
  ...
  public static void enforceCallerIsSystemOr(int i, String str) {
    ...
    if (UserHandle.getCallingUserId() != i) {
      throw new SecurityException(
          "Calling UserId (" + callingUserId + ") does not match the expected UserId (" + i + ")");
    }
  ...
  public static boolean checkCallerCanManageCompanionDevice(Context context) {
    return Binder.getCallingUid() == 1000
        || context.checkCallingPermission("android.permission.MANAGE_COMPANION_DEVICES") == 0;
  }
  ...
  public static void enforceCallerCanManageAssociationsForPackage(
      Context context, int i, String str, String str2) {
  ...
  public static boolean checkCallerCanManageAssociationsForPackage(
  ...
  public static AssociationInfo sanitizeWithCallerChecks(
      Context context, AssociationInfo associationInfo) {
    if (associationInfo != null
        && checkCallerCanManageAssociationsForPackage(
            context, associationInfo.getUserId(), associationInfo.getPackageName())) {
