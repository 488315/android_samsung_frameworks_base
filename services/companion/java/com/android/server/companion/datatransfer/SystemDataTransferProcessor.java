...
public class SystemDataTransferProcessor {
  public static final ComponentName SYSTEM_DATA_TRANSFER_REQUEST_APPROVAL_ACTIVITY =
      ComponentName.createRelative(
          "com.android.companiondevicemanager", ".CompanionDeviceDataTransferActivity");
  public final AssociationStore mAssociationStore;
  public final Context mContext;
  public final ExecutorService mExecutor;
  public final ResultReceiver mOnSystemDataTransferRequestConfirmationReceiver =
      new ResultReceiver(Handler.getMain()) { // from class:
        // com.android.server.companion.datatransfer.SystemDataTransferProcessor.2
        @Override // android.os.ResultReceiver
        public void onReceiveResult(int i, Bundle bundle) {
          Slog.d(
              "CDM_SystemDataTransferProcessor",
              "onReceiveResult() code=" + i + ", data=" + bundle);
          if (i == 0 || i == 1) {
            SystemDataTransferRequest systemDataTransferRequest =
                (PermissionSyncRequest)
                    bundle.getParcelable("permission_sync_request", PermissionSyncRequest.class);
            if (systemDataTransferRequest != null) {
              systemDataTransferRequest.setUserConsented(i == 0);
              Slog.i(
                  "CDM_SystemDataTransferProcessor",
                  "Recording request: " + systemDataTransferRequest);
              SystemDataTransferProcessor.this.mSystemDataTransferRequestStore.writeRequest(
                  systemDataTransferRequest.getUserId(), systemDataTransferRequest);
              return;
            }
            return;
          }
  ...
  public SystemDataTransferProcessor(
    ...
    companionTransportManager.addListener(
        1669491075,
        new IOnMessageReceivedListener() { // from class:
          // com.android.server.companion.datatransfer.SystemDataTransferProcessor.1
    ...
    this.mPermissionControllerManager =
        (PermissionControllerManager) context.getSystemService(PermissionControllerManager.class);
  ...
  public final AssociationInfo resolveAssociation(String str, int i, int i2) {
    AssociationInfo sanitizeWithCallerChecks =
        PermissionsUtils.sanitizeWithCallerChecks(
            this.mContext, this.mAssociationStore.getAssociationById(i2));
    if (sanitizeWithCallerChecks != null) {
      return sanitizeWithCallerChecks;
    }
    throw new DeviceNotAssociatedException(
        "Association " + i2 + " is not associated with the app " + str + " for user " + i);
  }
  ...
  public PendingIntent buildPermissionTransferUserConsentIntent(String str, int i, int i2) {
    ...
    Slog.i(
        "CDM_SystemDataTransferProcessor",
        "Creating permission sync intent for userId [" + i + "] associationId [" + i2 + "]");
    ...
    bundle.putParcelable(
        ...
        Utils.prepareForIpc(this.mOnSystemDataTransferRequestConfirmationReceiver));
    ...
    try {
      return PendingIntent.getActivityAsUser(
          this.mContext, i2, intent, 1409286144, null, UserHandle.CURRENT);
    } finally {
  ...
  public void startSystemDataTransfer(
    ...
    Iterator it =
        this.mSystemDataTransferRequestStore.readRequestsByAssociationId(i, i2).iterator();
    while (true) {
      if (!it.hasNext()) {
        z = false;
        break;
      ...
      if ((systemDataTransferRequest instanceof PermissionSyncRequest)
          && systemDataTransferRequest.isUserConsented()) {
        z = true;
        break;
      }
    }
    ...
    long clearCallingIdentity = Binder.clearCallingIdentity();
    try {
      this.mPermissionControllerManager.getRuntimePermissionBackup(
          UserHandle.of(i),
          this.mExecutor,
          new Consumer() { // from class:
            // com.android.server.companion.datatransfer.SystemDataTransferProcessor$$ExternalSyntheticLambda0
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
              SystemDataTransferProcessor.this.lambda$startSystemDataTransfer$0(
                  i2, iSystemDataTransferCallback, (byte[]) obj);
            }
          });
    } finally {
      Binder.restoreCallingIdentity(clearCallingIdentity);
  ...
  public /* synthetic */ void lambda$startSystemDataTransfer$0(
      int i, ISystemDataTransferCallback iSystemDataTransferCallback, byte[] bArr) {
    translateFutureToCallback(
        this.mTransportManager.requestPermissionRestore(i, bArr), iSystemDataTransferCallback);
  }
  ...
  public static void translateFutureToCallback(
      Future future, ISystemDataTransferCallback iSystemDataTransferCallback) {
    try {
      try {
