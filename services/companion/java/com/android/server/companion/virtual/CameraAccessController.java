...
public class CameraAccessController extends CameraManager.AvailabilityCallback
  ...
  public CameraAccessController(
      ...
      CameraAccessBlockedCallback cameraAccessBlockedCallback) {
  ...
  public void close() {
    synchronized (this.mObserverLock) {
      ...
      if (i < 0) {
        Slog.wtf(
            "CameraAccessController", "Unexpected negative mObserverCount: " + this.mObserverCount);
      } else if (i > 0) {
        Slog.w(
    ...
    synchronized (this.mLock) {
      ...
      while (it.hasNext()) {
        int queryUidFromPackageName =
            queryUidFromPackageName(((UserInfo) it.next()).getUserHandle().getIdentifier(), str2);
        if (this.mVirtualDeviceManagerInternal.isAppRunningOnAnyVirtualDevice(
            queryUidFromPackageName)) {
      ...
      CameraInjectionSession cameraInjectionSession =
          injectionSessionData != null
              ? (CameraInjectionSession) injectionSessionData.cameraIdToSession.get(str)
              : null;
  ...
  public final void startBlocking(final String str, final String str2) {
    try {
      Slog.d(
          "CameraAccessController", "startBlocking() cameraId: " + str2 + " packageName: " + str);
  ...
  public final void onInjectionSucceeded(
      String str, String str2, CameraInjectionSession cameraInjectionSession) {
    synchronized (this.mLock) {
      InjectionSessionData injectionSessionData =
          (InjectionSessionData) this.mPackageToSessionData.get(str2);
      if (injectionSessionData == null) {
        Slog.e(
            "CameraAccessController",
            "onInjectionSucceeded didn't find expected entry for package " + str2);
        cameraInjectionSession.close();
        return;
      }
      CameraInjectionSession cameraInjectionSession2 =
          (CameraInjectionSession)
              injectionSessionData.cameraIdToSession.put(str, cameraInjectionSession);
      if (cameraInjectionSession2 != null) {
        Slog.e(
            "CameraAccessController",
            "onInjectionSucceeded found unexpected existing session for camera " + str);
  ...
  public final void onInjectionError(String str, String str2, int i) {
    if (i != 2) {
      Slog.e(
          "CameraAccessController",
          "Unexpected injection error code:" + i + " for camera:" + str + " and package:" + str2);
      return;
    }
    synchronized (this.mLock) {
      InjectionSessionData injectionSessionData =
