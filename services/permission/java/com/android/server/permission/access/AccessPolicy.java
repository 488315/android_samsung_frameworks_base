...
public final class AccessPolicy {
  ...
  */
  ...
  public static final SchemePolicy _init_$lambda$1$addPolicy(
      ArrayMap arrayMap, SchemePolicy schemePolicy) {
  ...
  public final SchemePolicy getSchemePolicy(String str, String str2) {
    ...
    throw new IllegalStateException(
        ("Scheme policy for " + str + " and " + str2 + " does not exist").toString());
  }
  ...
  public final void parseSystemState(
    ...
    while (true) {
      ...
      if (Intrinsics.areEqual(binaryXmlPullParser.getName(), "access")) {
          ...
          if (eventType4 == 2) {
            ...
            for (int i3 = 0; i3 < size; i3++) {
              ...
              for (int i4 = 0; i4 < size2; i4++) {
                ((SchemePolicy) arrayMap2.valueAt(i4))
                    .parseSystemState(binaryXmlPullParser, accessState);
              }
            }
            int depth3 = binaryXmlPullParser.getDepth();
            if (depth3 != depth2) {
              throw new XmlPullParserException(
                  "Unexpected post-block depth " + depth3 + ", expected " + depth2);
            }
            while (true) {
        ...
        Log.w(
            LOG_TAG,
            "Ignoring unknown tag " + binaryXmlPullParser.getName() + " when parsing system state");
      }
      int depth4 = binaryXmlPullParser.getDepth();
      if (depth4 != depth) {
        throw new XmlPullParserException(
            "Unexpected post-block depth " + depth4 + ", expected " + depth);
      }
      while (true) {
  ...
  */
      ...
      if (Intrinsics.areEqual(binaryXmlPullParser.getName(), "access")) {
          ...
          if (eventType4 == i4) {
            ...
            for (int i5 = 0; i5 < size; i5++) {
              ...
              for (int i6 = 0; i6 < size2; i6++) {
                ((SchemePolicy) arrayMap2.valueAt(i6))
                    .parseUserState(binaryXmlPullParser, accessState, i);
              }
            }
            int depth3 = binaryXmlPullParser.getDepth();
            if (depth3 != depth2) {
        ...
        Log.w(
            ...
            "Ignoring unknown tag "
                ...
                + " when parsing user state for user "
                + i);
      }
      int depth4 = binaryXmlPullParser.getDepth();
      if (depth4 != depth) {
  ...
  public final int getDecision(
      GetStateScope getStateScope, AccessUri accessUri, AccessUri accessUri2) {
  ...
  public final void serializeSystemState(
    ...
    for (int i = 0; i < size; i++) {
      ...
      for (int i2 = 0; i2 < size2; i2++) {
        ((SchemePolicy) arrayMap2.valueAt(i2))
  ...
  public final void serializeUserState(
    ...
    for (int i2 = 0; i2 < size; i2++) {
      ...
      for (int i3 = 0; i3 < size2; i3++) {
        ((SchemePolicy) arrayMap2.valueAt(i3))
  ...
  public final void onPackageAdded(
      MutateStateScope mutateStateScope, Map map, Map map2, SparseArray sparseArray, String str) {
    boolean z;
    PackageState packageState = (PackageState) map.get(str);
    if (packageState == null) {
      throw new IllegalStateException(
          ("Added package " + str + " isn't found in packageStates in onPackageAdded()")
              .toString());
    }
    int appId = packageState.getAppId();
    SystemState systemState = mutateStateScope.getNewState().getSystemState();
  ...
  */
  public final void onPackageRemoved(
    ...
    if (!(!map.containsKey(str))) {
      throw new IllegalStateException(
          ("Removed package " + str + " is still in packageStates in onPackageRemoved()")
              .toString());
    }
    SystemState systemState = mutateStateScope.getNewState().getSystemState();
    systemState.setPackageStates(map);
  ...
  public final void onPackageInstalled(
    ...
    if (packageState == null) {
      throw new IllegalStateException(
          ("Installed package " + str + " isn't found in packageStates in onPackageInstalled()")
              .toString());
    }
    ArrayMap arrayMap = this.schemePolicies;
    int size = arrayMap.size();
