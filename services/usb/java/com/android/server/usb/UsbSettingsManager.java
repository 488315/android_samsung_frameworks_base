  ...
  public void dump(DualDumpOutputStream dualDumpOutputStream, String str, long j) {
    ...
    synchronized (this.mSettingsByUser) {
      ...
      for (int i2 = 0; i2 < size; i2++) {
        getSettingsForUser(((UserInfo) users.get(i2)).id)
            .dump(dualDumpOutputStream, "user_settings", 2246267895809L);
      }
    }
    synchronized (this.mSettingsByProfileGroup) {
      ...
      for (i = 0; i < size2; i++) {
        ((UsbProfileGroupSettingsManager) this.mSettingsByProfileGroup.valueAt(i))
            .dump(dualDumpOutputStream, "profile_group_settings", 2246267895810L);
      }
    }
