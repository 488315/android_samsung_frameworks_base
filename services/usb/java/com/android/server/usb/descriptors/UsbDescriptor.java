...
public abstract class UsbDescriptor {
  ...
  public static String[] sStatusStrings = {
  ...
  public void postParse(ByteStream byteStream) {
    ...
    if (readCount < i) {
      ...
      Log.w(
          "UsbDescriptor",
          "UNDERRUN t:0x"
              + Integer.toHexString(this.mType)
              + " r: "
              + readCount
              + " < l: "
              + this.mLength);
  ...
  public final void reportParseStatus(ReportCanvas reportCanvas) {
    ...
    if (status == 0 || status == 2 || status == 3) {
      reportCanvas.writeParagraph(
          "status: " + getStatusString() + " [" + getOverUnderRunCount() + "]", true);
    }
  }

  public void report(ReportCanvas reportCanvas) {
    String str =
        UsbStrings.getDescriptorName(getType())
            + ": "
