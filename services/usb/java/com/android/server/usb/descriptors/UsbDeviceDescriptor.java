...
public final class UsbDeviceDescriptor extends UsbDescriptor {
  ...
  public String getDeviceReleaseString() {
    ...
    return String.format(
        "%d.%d%d",
        Integer.valueOf((((i & 61440) >> 12) * 10) + ((i & 3840) >> 8)),
        Integer.valueOf((i & 240) >> 4),
  ...
  public UsbDevice.Builder toAndroid(UsbDescriptorParser usbDescriptorParser) {
    ...
    for (int i = 0; i < this.mConfigDescriptors.size(); i++) {
      usbConfigurationArr[i] =
          ((UsbConfigDescriptor) this.mConfigDescriptors.get(i)).toAndroid(usbDescriptorParser);
    }
    return new UsbDevice.Builder(
        usbDescriptorParser.getDeviceAddr(),
        this.mVendorID,
        this.mProductID,
        this.mDevClass,
        this.mDevSubClass,
        ...
        usbConfigurationArr,
        serialString,
        usbDescriptorParser.hasAudioPlayback(),
        usbDescriptorParser.hasAudioCapture(),
        usbDescriptorParser.hasMIDIInterface(),
        usbDescriptorParser.hasVideoPlayback(),
        usbDescriptorParser.hasVideoCapture());
  }
  ...
  public void report(ReportCanvas reportCanvas) {
    ...
    reportCanvas.writeListItem(
        "Class "
            ...
            + devSubClass
            + ": "
            + UsbStrings.getClassName(devSubClass));
    ...
    reportCanvas.writeListItem(
        "Manufacturer "
            ...
            + ((int) productIndex)
            + ": "
            + parser.getDescriptorString(productIndex));
