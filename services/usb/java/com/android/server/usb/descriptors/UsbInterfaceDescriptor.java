...
public class UsbInterfaceDescriptor extends UsbDescriptor {
  ...
  public UsbInterface toAndroid(UsbDescriptorParser usbDescriptorParser) {
    ...
    for (int i = 0; i < this.mEndpointDescriptors.size(); i++) {
      usbEndpointArr[i] =
          ((UsbEndpointDescriptor) this.mEndpointDescriptors.get(i)).toAndroid(usbDescriptorParser);
    }
  ...
  public void report(ReportCanvas reportCanvas) {
    ...
    reportCanvas.writeListItem(
        "Subclass: " + ReportCanvas.getHexString(usbSubclass) + ": " + audioSubclassName);
    reportCanvas.writeListItem(
        "Protocol: " + protocol + ": " + ReportCanvas.getHexString(protocol));
