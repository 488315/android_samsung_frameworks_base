...
public final class UsbAlsaDevice {
  public static final String[] DIRECTION_STR = {
    KnoxVpnFirewallHelper.INPUT_CHAIN, KnoxVpnFirewallHelper.OUTPUT_CHAIN
  };
  ...
  public UsbAlsaDevice(
      ...
      boolean z4,
      boolean z5) {
    this.mHasDevice = new boolean[] {z2, z};
  ...
  public String getAlsaCardDeviceString() {
    ...
    if (i2 < 0 || (i = this.mDeviceNum) < 0) {
      Slog.e(
          "UsbAlsaDevice",
          "Invalid alsa card or device alsaCard: "
              + this.mCardNum
              + " alsaDevice: "
              + this.mDeviceNum);
      return null;
  ...
  public final boolean updateWiredDeviceConnectionState(int i, boolean z) {
    ...
    if (this.mDeviceType[i] == 0) {
      Slog.d(
          "UsbAlsaDevice",
          "Unable to set device connection state as " + DIRECTION_STR[i] + " device type is none");
      return false;
    }
    if (this.mAlsaCardDeviceString == null) {
      Slog.w(
          ...
          "Failed to update "
              + DIRECTION_STR[i]
              + " device connection state failed as alsa card device string is null");
      return false;
    }
    Slog.d(
        "UsbAlsaDevice",
        "pre-call device:0x"
            + Integer.toHexString(this.mDeviceType[i])
            + " addr:"
            + this.mAlsaCardDeviceString
            + " name:"
            + this.mDeviceName);
    ...
    if (i2 != iArr[i]) {
      iArr[i] = i2;
      try {
        this.mAudioService.setWiredDeviceConnectionState(
            new AudioDeviceAttributes(
                this.mDeviceType[i], this.mAlsaCardDeviceString, this.mDeviceName),
            i2,
            "UsbAlsaDevice");
      } catch (RemoteException unused) {
        Slog.e(
            "UsbAlsaDevice",
            "RemoteException in setWiredDeviceConnectionState for " + DIRECTION_STR[i]);
        return false;
      }
    }
    return true;
  ...
  public synchronized void updateWiredDeviceConnectionStateByBundle(boolean z) {
    ...
    try {
      if (this.mHasDevice[0]) {
        ...
        this.mAudioService.setWiredDeviceConnectionState(
            new AudioDeviceAttributes(-2113929216, alsaCardDeviceString, this.mDeviceName),
            z ? 1 : 0,
            "UsbAlsaDevice");
      }
      if (this.mHasDevice[1]) {
        Slog.d(
            "UsbAlsaDevice", "OUTPUT addr:" + alsaCardDeviceString + " name:" + this.mDeviceName);
        this.mAudioService.setWiredDeviceConnectionState(
            new AudioDeviceAttributes(67108864, alsaCardDeviceString, this.mDeviceName),
            z ? 1 : 0,
            "UsbAlsaDevice");
      }
    } catch (RemoteException unused) {
  ...
  public boolean equals(Object obj) {
    ...
    return this.mCardNum == usbAlsaDevice.mCardNum
        && this.mDeviceNum == usbAlsaDevice.mDeviceNum
        && Arrays.equals(this.mHasDevice, usbAlsaDevice.mHasDevice)
        && Arrays.equals(this.mIsHeadset, usbAlsaDevice.mIsHeadset)
        && this.mIsDock == usbAlsaDevice.mIsDock;
  }
  ...
  public int hashCode() {
    ...
    return ((((i2 + (!zArr2[0] ? 1 : 0)) * 31) + (!zArr2[1] ? 1 : 0)) * 31)
        + (!this.mIsDock ? 1 : 0);
  }
}
