...
public final class UsbPortAidl implements UsbPortHal {
      ...
      try {
        ...
        this.mBinder.linkToDeath(
            new IBinder.DeathRecipient() { // from class:
              // com.android.server.usb.hal.port.UsbPortAidl$$ExternalSyntheticLambda0
        ...
        UsbPortManager.logAndPrintException(
            indentingPrintWriter, "connectToProxy: usb hal service not responding", e);
  ...
  public static boolean isServicePresent(IndentingPrintWriter indentingPrintWriter) {
    try {
      ...
      UsbPortManager.logAndPrintException(
          indentingPrintWriter, "connectToProxy: usb Aidl hal service not found.", e);
      return false;
    }
  }
  ...
  public void enableContaminantPresenceDetection(String str, boolean z, long j) {
    synchronized (this.mLock) {
      ...
      try {
        ...
        UsbPortManager.logAndPrintException(
            this.mPw, "Failed to set contaminant detection. opID:" + j, e);
      }
    }
  }
  ...
  public void queryPortStatus(long j) {
    synchronized (this.mLock) {
      ...
      try {
        ...
        UsbPortManager.logAndPrintException(
            null, "ServiceStart: Failed to query port status. opID:" + j, e);
      }
    }
  }
      ...
      try {
        ...
        while (true) {
          try {
            ...
            UsbPortManager.logAndPrintException(
                this.mPw,
                "resetUsbPort: Failed to call onOperationComplete portID=" + str + "opID:" + j,
                e);
        ...
        if (j2 != j) {
          UsbPortManager.logAndPrint(
              4, this.mPw, "resetUsbPort: operationID exists ! opID:" + j + " key:" + j2);
        }
        try {
          ...
          UsbPortManager.logAndPrintException(
              this.mPw, "resetUsbPort: Failed to resetUsbPort: portID=" + str + "opId:" + j, e2);
          iUsbOperationInternal.onOperationComplete(1);
          sCallbacks.remove(j2);
        }
    ...
    synchronized (this.mLock) {
      try {
        try {
          if (this.mProxy == null) {
            UsbPortManager.logAndPrint(
                6, this.mPw, "enableUsbData: Proxy is null. Retry !opID:" + j);
            iUsbOperationInternal.onOperationComplete(1);
            return false;
          }
          j2 = j;
          while (true) {
            try {
              ...
              UsbPortManager.logAndPrintException(
                  this.mPw,
                  "enableUsbData: Failed to call onOperationComplete portID=" + str + "opID:" + j,
                  e);
              sCallbacks.remove(j2);
              return false;
            }
          ...
          if (j2 != j) {
            UsbPortManager.logAndPrint(
                4, this.mPw, "enableUsbData: operationID exists ! opID:" + j + " key:" + j2);
          }
          try {
            longSparseArray.put(j2, iUsbOperationInternal);
            ...
            UsbPortManager.logAndPrintException(
                this.mPw,
                "enableUsbData: Failed to invoke enableUsbData: portID=" + str + "opID:" + j,
                e2);
            iUsbOperationInternal.onOperationComplete(1);
            sCallbacks.remove(j2);
            return false;
  ...
  public void enableLimitPowerTransfer(
    ...
    synchronized (this.mLock) {
      ...
      if (this.mProxy == null) {
        UsbPortManager.logAndPrint(
            6, this.mPw, "enableLimitPowerTransfer: Proxy is null. Retry !opID:" + j);
        iUsbOperationInternal.onOperationComplete(1);
        return;
      }
      long j2 = j;
      ...
      try {
        ...
        UsbPortManager.logAndPrintException(
            this.mPw,
            "enableLimitPowerTransfer: Failed while invoking AIDL HAL portID=" + str + " opID:" + j,
            e2);
  ...
  public void enableUsbDataWhileDocked(
    ...
    synchronized (this.mLock) {
      ...
      if (this.mProxy == null) {
        UsbPortManager.logAndPrint(
            6, this.mPw, "enableUsbDataWhileDocked: Proxy is null. Retry !opID:" + j);
        iUsbOperationInternal.onOperationComplete(1);
        return;
      }
      long j2 = j;
      ...
      if (j2 != j) {
        UsbPortManager.logAndPrint(
            4, this.mPw, "enableUsbDataWhileDocked: operationID exists ! opID:" + j + " key:" + j2);
      }
      try {
        longSparseArray.put(j2, iUsbOperationInternal);
        ...
        UsbPortManager.logAndPrintException(
            this.mPw,
            "enableUsbDataWhileDocked: error while invoking halportID=" + str + " opID:" + j,
            e2);
        if (iUsbOperationInternal != null) {
          iUsbOperationInternal.onOperationComplete(1);
        }
  ...
  public class HALCallback extends IUsbCallback.Stub {
    ...
    public final int toContaminantProtectionStatus(byte b) {
      ...
      if (b != 1) {
        ...
        if (b != 2) {
          ...
          UsbPortManager.logAndPrint(
              6, this.mPw, "Unrecognized aidlContaminantProtection:" + ((int) b));
          return 0;
        }
      }
      return i;
      ...
      for (AltModeData altModeData : altModeDataArr) {
        if (altModeData.getTag() == 0) {
          ...
          return new DisplayPortAltModeInfo(
              displayPortAltModeData.partnerSinkStatus,
              displayPortAltModeData.cableStatus,
              toDisplayPortAltModeNumLanesInt(displayPortAltModeData.pinAssignment),
              displayPortAltModeData.hpd,
              displayPortAltModeData.linkTrainingStatus);
        }
      }
      return null;
    }
      ...
      if (this.mUsbPortAidl.mSystemReady) {
        ...
        while (i3 < length) {
          ...
          arrayList.add(
              new RawPortInfo(
                  portStatus.portName,
                  toSupportedModes(portStatus.supportedModes),
                  toSupportedContaminantProtectionModes(
                      portStatus.supportedContaminantProtectionModes),
                  toPortMode(portStatus.currentMode),
                  portStatus.canChangeMode,
                  portStatus.currentPowerRole,
                  portStatus.canChangePowerRole,
                  portStatus.currentDataRole,
                  portStatus.canChangeDataRole,
                  portStatus.supportsEnableContaminantPresenceProtection,
                  toContaminantProtectionStatus(portStatus.contaminantProtectionStatus),
                  portStatus.supportsEnableContaminantPresenceDetection,
                  portStatus.contaminantDetectionStatus,
                  toUsbDataStatusInt(portStatus.usbDataStatus),
                  portStatus.powerTransferLimited,
                  portStatus.powerBrickStatus,
                  portStatus.supportsComplianceWarnings,
                  formatComplianceWarnings(portStatus.complianceWarnings),
                  portStatus.plugOrientation,
                  toSupportedAltModesInt(portStatus.supportedAltModes),
                  formatDisplayPortAltModeInfo(portStatus.supportedAltModes)));
    ...
    public void notifyEnableUsbDataStatus(String str, boolean z, int i, long j) {
      if (i == 0) {
        UsbPortManager.logAndPrint(
            4, this.mPw, "notifyEnableUsbDataStatus:" + str + ": opID:" + j + " enable:" + z);
      } else {
        UsbPortManager.logAndPrint(
            6, this.mPw, str + "notifyEnableUsbDataStatus: opID:" + j + " failed. err:" + i);
      }
      try {
        ((IUsbOperationInternal) UsbPortAidl.sCallbacks.get(j)).onOperationComplete(i == 0 ? 0 : 1);
      } catch (RemoteException e) {
        UsbPortManager.logAndPrintException(
            this.mPw, "notifyEnableUsbDataStatus: Failed to call onOperationComplete", e);
      }
    }
    ...
    public void notifyContaminantEnabledStatus(String str, boolean z, int i, long j) {
      if (i == 0) {
        UsbPortManager.logAndPrint(
            4, this.mPw, "notifyContaminantEnabledStatus:" + str + ": opID:" + j + " enable:" + z);
        return;
      }
      UsbPortManager.logAndPrint(
          6, this.mPw, str + "notifyContaminantEnabledStatus: opID:" + j + " failed. err:" + i);
    }
    ...
    public void notifyLimitPowerTransferStatus(String str, boolean z, int i, long j) {
      if (i == 0) {
        ...
        UsbPortManager.logAndPrint(
            6, this.mPw, str + "notifyLimitPowerTransferStatus: opID:" + j + " failed. err:" + i);
      }
      try {
        if (((IUsbOperationInternal) UsbPortAidl.sCallbacks.get(j)) != null) {
          ((IUsbOperationInternal) UsbPortAidl.sCallbacks.get(j))
              .onOperationComplete(i == 0 ? 0 : 1);
        }
      } catch (RemoteException e) {
        UsbPortManager.logAndPrintException(
            this.mPw, "enableLimitPowerTransfer: Failed to call onOperationComplete", e);
      }
    }
    ...
    public void notifyEnableUsbDataWhileDockedStatus(String str, int i, long j) {
      if (i == 0) {
        ...
        UsbPortManager.logAndPrint(
            6,
            this.mPw,
            str + "notifyEnableUsbDataWhileDockedStatus: opID:" + j + " failed. err:" + i);
      }
      try {
        if (((IUsbOperationInternal) UsbPortAidl.sCallbacks.get(j)) != null) {
          ((IUsbOperationInternal) UsbPortAidl.sCallbacks.get(j))
    ...
    public void notifyResetUsbPortStatus(String str, int i, long j) {
      ...
      try {
        ...
        UsbPortManager.logAndPrintException(
            this.mPw, "notifyResetUsbPortStatus: Failed to call onOperationComplete", e);
      }
    }
  }
}
