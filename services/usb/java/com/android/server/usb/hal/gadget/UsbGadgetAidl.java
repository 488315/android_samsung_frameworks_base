...
public final class UsbGadgetAidl implements UsbGadgetHal {
  ...
  public final void connectToProxy(IndentingPrintWriter indentingPrintWriter) {
    synchronized (this.mGadgetProxyLock) {
      ...
      try {
        this.mGadgetProxy =
            IUsbGadget.Stub.asInterface(ServiceManager.waitForService(USB_GADGET_AIDL_SERVICE));
  ...
  public static boolean isServicePresent(IndentingPrintWriter indentingPrintWriter) {
    try {
      ...
      UsbDeviceManager.logAndPrintException(
          indentingPrintWriter, "connectToProxy: usb gadget Aidl hal service not found.", e);
      return false;
    }
  }

  public UsbGadgetAidl(
    ...
    synchronized (this.mGadgetProxyLock) {
      try {
        try {
          ...
          UsbDeviceManager.logAndPrintException(
              this.mPw, "RemoteException while calling getCurrentUsbFunctions, opID:" + j, e);
        }
      } catch (Throwable th) {
        throw th;
      }
  ...
  public void getUsbSpeed(long j) {
    try {
      ...
      UsbDeviceManager.logAndPrintException(
          this.mPw, "RemoteException while calling getUsbSpeed, opID:" + j, e);
    }
  }
  ...
  public class UsbGadgetCallback extends IUsbGadgetCallback.Stub {
    ...
    public void setCurrentUsbFunctionsCb(long j, int i, long j2) {
      if (i == 0) {
        UsbDeviceManager.logAndPrint(
            4,
            this.mPw,
            "Usb setCurrentUsbFunctionsCb ,functions:"
                + j
                + " ,status:"
                + i
                + " ,transactionId:"
        ...
        UsbDeviceManager.logAndPrint(
            6,
            this.mPw,
            "Usb setCurrentUsbFunctionsCb failed ,functions:"
                + j
                + " ,status:"
                + i
                + " ,transactionId:"
      ...
      UsbGadgetAidl.this.mDeviceManager.setCurrentUsbFunctionsCb(
    ...
    public void getCurrentUsbFunctionsCb(long j, int i, long j2) {
      if (i == 0) {
        UsbDeviceManager.logAndPrint(
            4,
            this.mPw,
            "Usb getCurrentUsbFunctionsCb ,functions:"
                + j
                + " ,status:"
                + i
                + " ,transactionId:"
        ...
        UsbDeviceManager.logAndPrint(
            6,
            this.mPw,
            "Usb getCurrentUsbFunctionsCb failed ,functions:"
                + j
                + " ,status:"
                + i
                + " ,transactionId:"
    ...
    public void getUsbSpeedCb(int i, long j) {
      UsbDeviceManager.logAndPrint(
          4, this.mPw, "getUsbSpeedCb speed:" + i + " ,transactionId:" + j);
    ...
    public void resetCb(int i, long j) {
      if (i == 0) {
        UsbDeviceManager.logAndPrint(
            4, this.mPw, "Usb resetCb status:" + i + " ,transactionId:" + j);
      } else {
        UsbDeviceManager.logAndPrint(
            6, this.mPw, "Usb resetCb status" + i + " ,transactionId:" + j);
      }
