...
public class SemUsbBackend {
  ...
  public SemUsbBackend(
      Context context, UsbPortManager usbPortManager, UsbDeviceManager usbDeviceManager) {
    this.mContext = context;
    this.mIsUnlocked =
        context
            .registerReceiver(null, new IntentFilter("android.hardware.usb.action.USB_STATE"))
  ...
  public final void setUsbFunction(int i) {
    ...
    Slog.d(
        "SemUsbBackend",
        "USB connection setUsbFunction : " + i + ", operationId : " + incrementAndGet);
    if (i == 2) {
      Slog.d("SemUsbBackend", "USB connection setUsbFunction : + UsbManager.USB_FUNCTION_MTP");
      this.mDeviceManager.setCurrentFunctions(
          UsbManager.usbFunctionsFromString("mtp"), incrementAndGet);
      return;
    }
    if (i == 4) {
      Slog.d("SemUsbBackend", "USB connection setUsbFunction :  + UsbManager.USB_FUNCTION_PTP");
      this.mDeviceManager.setCurrentFunctions(
          UsbManager.usbFunctionsFromString("ptp"), incrementAndGet);
      return;
    }
    if (i == 6) {
      Slog.d("SemUsbBackend", "USB connection setUsbFunction :  + UsbManager.USB_FUNCTION_MIDI");
      this.mDeviceManager.setCurrentFunctions(
          UsbManager.usbFunctionsFromString("midi"), incrementAndGet);
    } else if (i == 8) {
      Slog.d(
          "SemUsbBackend",
          "USB connection setUsbFunction :  + UsbManager.USB_FUNCTION_MASS_STORAGE");
      this.mDeviceManager.setCurrentFunctions(
          UsbManager.usbFunctionsFromString("mass_storage"), incrementAndGet);
    } else if (i == 10) {
      Slog.d(
          "SemUsbBackend", "USB connection setUsbFunction :  + UsbManager.USB_FUNCTION_MTP_GADGET");
      this.mDeviceManager.setCurrentFunctions(
          UsbManager.usbFunctionsFromString("mtp,conn_gadget"), incrementAndGet);
    } else {
      Slog.d(
          "SemUsbBackend",
          "USB connection setUsbFunction :  + UsbManager.USB_FUNCTION_SEC_CHARGING");
      this.mDeviceManager.setCurrentFunctions(
          UsbManager.usbFunctionsFromString("sec_charging"), incrementAndGet);
    }
  }
