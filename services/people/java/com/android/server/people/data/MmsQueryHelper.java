...
public class MmsQueryHelper {
  ...
  public boolean querySince(long j) {
    String[] strArr = {KnoxCustomManagerService.f1773ID, "date", "msg_box"};
    ...
    try {
      Cursor query =
          this.mContext
              .getContentResolver()
              .query(Telephony.Mms.CONTENT_URI, strArr, "date > ?", strArr2, null);
      boolean z = false;
      try {
        if (query == null) {
        ...
        while (query.moveToNext()) {
          String string = query.getString(query.getColumnIndex(KnoxCustomManagerService.f1773ID));
