...
public abstract class SharesheetModelScorer {
  ...
  public static void computeScore(List list, int i, long j) {
    ...
    while (it.hasNext()) {
      ...
      if (shareTarget.getEventHistory() != null) {
        ...
        if (!activeTimeSlots.isEmpty()) {
          ...
          while (it2.hasNext()) {
            shareTargetRankingScore.incrementFrequencyScore(
                getFreqDecayedOnElapsedTime(
                    j - ((Long) ((Range) it2.next()).getLower()).longValue()));
          }
        ...
        if (!activeTimeSlots2.isEmpty()) {
          ...
          while (it3.hasNext()) {
            shareTargetRankingScore.incrementMimeFrequencyScore(
                getFreqDecayedOnElapsedTime(
                    j - ((Long) ((Range) it3.next()).getLower()).longValue()));
          }
          f2 += shareTargetRankingScore.getMimeFrequencyScore();
          i3++;
        }
        Range mostRecentActiveTimeSlot =
            shareTarget
                .getEventHistory()
                .getEventIndex(Event.SHARE_EVENT_TYPES)
                .getMostRecentActiveTimeSlot();
        if (mostRecentActiveTimeSlot != null) {
          int size = priorityQueue.size();
          Integer num = RECENCY_SCORE_COUNT;
          if (size < num.intValue()
              || ((Long) mostRecentActiveTimeSlot.getUpper()).longValue()
                  > ((Long) ((Range) ((Pair) priorityQueue.peek()).second).getUpper())
                      .longValue()) {
    ...
    while (!priorityQueue.isEmpty()) {
      ((ShareTargetRankingScore) ((Pair) priorityQueue.poll()).first)
          .setRecencyScore(
              priorityQueue.size() > 1 ? 0.35f - ((priorityQueue.size() - 2) * 0.02f) : 0.4f);
    }
    Float valueOf = Float.valueOf(i2 != 0 ? f / i2 : 0.0f);
    Float valueOf2 = Float.valueOf(i3 != 0 ? f2 / i3 : 0.0f);
    for (int i4 = 0; i4 < arrayList.size(); i4++) {
      ShareTargetPredictor.ShareTarget shareTarget2 =
          (ShareTargetPredictor.ShareTarget) list.get(i4);
      ShareTargetRankingScore shareTargetRankingScore2 =
          (ShareTargetRankingScore) arrayList.get(i4);
      double d = 0.0d;
      shareTargetRankingScore2.setFrequencyScore(
          normalizeFreqScore(
              valueOf.equals(Float.valueOf(DisplayPowerController2.RATE_FROM_DOZE_TO_ON))
                  ? 0.0d
                  : shareTargetRankingScore2.getFrequencyScore() / valueOf.floatValue()));
      ...
      shareTargetRankingScore2.setTotalScore(
          probOR(
              probOR(
                  shareTargetRankingScore2.getRecencyScore(),
                  shareTargetRankingScore2.getFrequencyScore()),
              shareTargetRankingScore2.getMimeFrequencyScore()));
  ...
  public static void computeScoreForAppShare(
      List list, int i, int i2, long j, DataManager dataManager, int i3) {
    ...
    while (it.hasNext()) {
      ...
      arrayMap.computeIfAbsent(
          ...
          new Function() { // from class:
            // com.android.server.people.prediction.SharesheetModelScorer$$ExternalSyntheticLambda1
      ...
      int i3 = 0;
      while (i3 < list2.size()
          && shareTarget.getScore()
              <= ((ShareTargetPredictor.ShareTarget) list2.get(i3)).getScore()) {
        i3++;
      }
  ...
  public static void promoteMostChosenAndFrequentlyUsedApps(
      Map map, int i, DataManager dataManager, int i2) {
    ...
    Map queryAppUsageStats =
        dataManager.queryAppUsageStats(
            i2, currentTimeMillis - ONE_MONTH_WINDOW, currentTimeMillis, map.keySet());
    float promoteApp =
        promoteApp(
            map,
            queryAppUsageStats,
            new Function() { // from class:
              // com.android.server.people.prediction.SharesheetModelScorer$$ExternalSyntheticLambda2
              @Override // java.util.function.Function
              public final Object apply(Object obj) {
    ...
    promoteApp(
        ...
        queryAppUsageStats,
        new Function() { // from class:
          // com.android.server.people.prediction.SharesheetModelScorer$$ExternalSyntheticLambda3
          @Override // java.util.function.Function
          public final Object apply(Object obj) {
      ...
      for (Map.Entry entry : map2.entrySet()) {
        if (map.containsKey(entry.getKey())) {
          ShareTargetPredictor.ShareTarget shareTarget =
              (ShareTargetPredictor.ShareTarget) ((List) map.get(entry.getKey())).get(0);
          if (shareTarget.getScore() <= DisplayPowerController2.RATE_FROM_DOZE_TO_ON) {
            float intValue =
                (((Integer) function.apply((AppUsageStatsData) entry.getValue())).intValue() * f)
                    / i;
  ...
  public static void promoteForegroundApp(Map map, DataManager dataManager, int i) {
    ...
    if (findSharingForegroundApp != null) {
      ShareTargetPredictor.ShareTarget shareTarget =
          (ShareTargetPredictor.ShareTarget) ((List) map.get(findSharingForegroundApp)).get(0);
      shareTarget.setScore(
          probOR(shareTarget.getScore(), DisplayPowerController2.RATE_FROM_DOZE_TO_ON));
    }
  }

  public static String findSharingForegroundApp(Map map, DataManager dataManager, int i) {
    long currentTimeMillis = System.currentTimeMillis();
    List queryAppMovingToForegroundEvents =
        dataManager.queryAppMovingToForegroundEvents(
            i, currentTimeMillis - FOREGROUND_APP_PROMO_TIME_WINDOW, currentTimeMillis);
    String str = null;
    for (int size = queryAppMovingToForegroundEvents.size() - 1; size >= 0; size--) {
      String className =
          ((UsageEvents.Event) queryAppMovingToForegroundEvents.get(size)).getClassName();
      String packageName =
          ((UsageEvents.Event) queryAppMovingToForegroundEvents.get(size)).getPackageName();
      if (packageName != null
          && ((className == null || !className.contains(CHOOSER_ACTIVITY))
