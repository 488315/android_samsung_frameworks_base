...
public class ShareTargetPredictor extends AppTargetPredictor {
  ...
  public ShareTargetPredictor(
    ...
    this.mIntentFilter =
        (IntentFilter)
            appPredictionContext.getExtras().getParcelable("intent_filter", IntentFilter.class);
  ...
  public void predictTargets() {
    ...
    SharesheetModelScorer.computeScore(
        directShareTargets, getShareEventType(this.mIntentFilter), System.currentTimeMillis());
    Collections.sort(
        directShareTargets,
        Comparator.comparing(
            ...
            .thenComparing(
                new Function() { // from class:
                  // com.android.server.people.prediction.ShareTargetPredictor$$ExternalSyntheticLambda1
                  @Override // java.util.function.Function
                  public final Object apply(Object obj) {
                    Integer lambda$predictTargets$0;
                    lambda$predictTargets$0 =
                        ShareTargetPredictor.lambda$predictTargets$0(
    ...
    for (int i = 0;
        i < Math.min(getPredictionContext().getPredictedTargetCount(), directShareTargets.size());
        i++) {
  ...
  public void lambda$onSortAppTargets$1(List list, Consumer consumer) {
    ...
    SharesheetModelScorer.computeScoreForAppShare(
        appShareTargets,
        getShareEventType(this.mIntentFilter),
    ...
    Collections.sort(
        ...
        new Comparator() { // from class:
          // com.android.server.people.prediction.ShareTargetPredictor$$ExternalSyntheticLambda2
          @Override // java.util.Comparator
          public final int compare(Object obj, Object obj2) {
            int lambda$sortTargets$1;
            lambda$sortTargets$1 =
                ShareTargetPredictor.lambda$sortTargets$1(
                    (ShareTargetPredictor.ShareTarget) obj,
                    (ShareTargetPredictor.ShareTarget) obj2);
    ...
    for (ShareTarget shareTarget : appShareTargets) {
      ...
      arrayList.add(
          new AppTarget.Builder(appTarget.getId(), appTarget.getPackageName(), appTarget.getUser())
              .setClassName(appTarget.getClassName())
              .setRank(
                  shareTarget.getScore() > DisplayPowerController2.RATE_FROM_DOZE_TO_ON
                      ? (int) (shareTarget.getScore() * 1000.0f)
                      : 0)
              .build());
    }
  ...
  public static /* synthetic */ int lambda$sortTargets$1(
      ShareTarget shareTarget, ShareTarget shareTarget2) {
  ...
  public final List getDirectShareTargets() {
    ...
    ArrayList arrayList = new ArrayList();
    for (ShortcutManager.ShareShortcutInfo shareShortcutInfo :
        getDataManager().getShareShortcuts(this.mIntentFilter, this.mCallingUserId)) {
      ...
      PackageData packageData =
          getDataManager().getPackage(shortcutInfo.getPackage(), shortcutInfo.getUserId());
  ...
  public final List getAppShareTargets(List list) {
    ...
    while (it.hasNext()) {
      ...
      PackageData packageData =
          getDataManager()
              .getPackage(appTarget.getPackageName(), appTarget.getUser().getIdentifier());
      arrayList.add(
          new ShareTarget(
              appTarget,
              packageData == null
                  ? null
                  : packageData.getClassLevelEventHistory(appTarget.getClassName()),
              null));
    }
    return arrayList;
  }
  ...
  public final int getShareEventType(IntentFilter intentFilter) {
    return getDataManager()
        .mimeTypeToShareEventType(intentFilter != null ? intentFilter.getDataType(0) : null);
  }
