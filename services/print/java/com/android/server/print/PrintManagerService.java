  ...
  public class PrintManagerImpl extends IPrintManager.Stub {
    ...
    public Bundle print(
      ...
      if (!isPrintingEnabled()) {
        ...
        try {
          CharSequence printingDisabledReasonForUser =
              devicePolicyManagerInternal.getPrintingDisabledReasonForUser(callingUserId);
          if (printingDisabledReasonForUser != null) {
            Toast.makeText(this.mContext, Looper.getMainLooper(), printingDisabledReasonForUser, 1)
                .show();
          }
          try {
      ...
      synchronized (this.mLock) {
        ...
        String resolveCallingPackageNameEnforcingSecurity =
            resolveCallingPackageNameEnforcingSecurity(str4);
        UserState orCreateUserStateLocked =
            getOrCreateUserStateLocked(resolveCallingUserEnforcingPermissions, false);
        clearCallingIdentity = Binder.clearCallingIdentity();
        try {
          return orCreateUserStateLocked.print(
              ...
              printAttributes,
              resolveCallingPackageNameEnforcingSecurity,
              resolveCallingAppEnforcingPermissions);
        } finally {
        }
      }
    }
    ...
    public PrintJobInfo getPrintJobInfo(PrintJobId printJobId, int i, int i2) {
      ...
      synchronized (this.mLock) {
        ...
        long clearCallingIdentity = Binder.clearCallingIdentity();
        try {
          return orCreateUserStateLocked.getPrintJobInfo(
              printJobId, resolveCallingAppEnforcingPermissions);
        } finally {
          Binder.restoreCallingIdentity(clearCallingIdentity);
        }
      }
    ...
    public final Icon validateIconUserBoundary(Icon icon) {
      ...
      if (icon != null
          && ((icon.getType() == 4 || icon.getType() == 6)
              && (encodedUserInfo = icon.getUri().getEncodedUserInfo()) != null)) {
        int resolveCallingUserEnforcingPermissions =
            resolveCallingUserEnforcingPermissions(Integer.parseInt(encodedUserInfo));
        synchronized (this.mLock) {
          if (resolveCallingProfileParentLocked(resolveCallingUserEnforcingPermissions)
              != getCurrentUserId()) {
            return null;
    ...
    public void restartPrintJob(PrintJobId printJobId, int i, int i2) {
      ...
      synchronized (this.mLock) {
        ...
        long clearCallingIdentity = Binder.clearCallingIdentity();
        try {
          orCreateUserStateLocked.restartPrintJob(
              printJobId, resolveCallingAppEnforcingPermissions);
        } finally {
          Binder.restoreCallingIdentity(clearCallingIdentity);
        }
          ...
          if (appId
              != UserHandle.getAppId(
                  this.mContext
                      .getPackageManager()
                      .getPackageUidAsUser(
                          "com.android.printspooler", resolveCallingUserEnforcingPermissions))) {
    ...
    public boolean isPrintServiceEnabled(ComponentName componentName, int i) {
      ...
      synchronized (this.mLock) {
        ...
        return getOrCreateUserStateLocked(resolveCallingUserEnforcingPermissions, false)
    ...
    public void addPrintJobStateChangeListener(
      ...
      synchronized (this.mLock) {
        ...
        long clearCallingIdentity = Binder.clearCallingIdentity();
        try {
          orCreateUserStateLocked.addPrintJobStateChangeListener(
              iPrintJobStateChangeListener, resolveCallingAppEnforcingPermissions);
        } finally {
          Binder.restoreCallingIdentity(clearCallingIdentity);
        }
      }
    ...
    public final void registerContentObservers() {
      ...
      this.mContext
          ...
          .registerContentObserver(
              uriFor,
              false,
              new ContentObserver(BackgroundThread.getHandler()) { // from class:
                    ...
                    synchronized (PrintManagerImpl.this.mLock) {
                      ...
                      for (int i2 = 0; i2 < size; i2++) {
                        if (i == -1 || i == PrintManagerImpl.this.mUserStates.keyAt(i2)) {
                          ((UserState) PrintManagerImpl.this.mUserStates.valueAt(i2))
      ...
      new PackageMonitor() { // from class:
        ...
        public final boolean hasPrintService(String str) {
          ...
          List queryIntentServicesAsUser =
              PrintManagerImpl.this
                  .mContext
                  .getPackageManager()
                  .queryIntentServicesAsUser(intent, 276824068, getChangingUserId());
          return (queryIntentServicesAsUser == null || queryIntentServicesAsUser.isEmpty())
              ? false
              : true;
        }
        ...
        public void onPackageModified(String str) {
          if (PrintManagerImpl.this.mUserManager.isUserUnlockingOrUnlocked(getChangingUserId())) {
            boolean z = false;
            UserState orCreateUserStateLocked =
                PrintManagerImpl.this.getOrCreateUserStateLocked(getChangingUserId(), false, false);
            synchronized (PrintManagerImpl.this.mLock) {
        ...
        public boolean onHandleForceStop(Intent intent, String[] strArr, int i, boolean z) {
          ...
          synchronized (PrintManagerImpl.this.mLock) {
            ...
            while (it.hasNext()) {
              String packageName =
                  ((PrintServiceInfo) it.next()).getComponentName().getPackageName();
          ...
          if (PrintManagerImpl.this.mUserManager.isUserUnlockingOrUnlocked(getChangingUserId())) {
            synchronized (PrintManagerImpl.this.mLock) {
              if (hasPrintService(str)) {
                PrintManagerImpl.this
                    .getOrCreateUserStateLocked(getChangingUserId(), false, false)
                    .updateIfNeededLocked();
              }
            }
          }
    ...
    public final UserState getOrCreateUserStateLocked(int i, boolean z, boolean z2, boolean z3) {
      if (z2 && !this.mUserManager.isUserUnlockingOrUnlocked(i)) {
        throw new IllegalStateException(
            "User " + i + " must be unlocked for printing to be available");
      }
      ...
      BackgroundThread.getHandler()
          .post(
              new Runnable() { // from class:
                // com.android.server.print.PrintManagerService.PrintManagerImpl.3
                  ...
                  if (PrintManagerImpl.this.mUserManager.isUserUnlockingOrUnlocked(i)) {
                    synchronized (PrintManagerImpl.this.mLock) {
                      orCreateUserStateLocked =
                          PrintManagerImpl.this.getOrCreateUserStateLocked(i, true, false, true);
                    }
    ...
    public final void handleUserStopped(final int i) {
      BackgroundThread.getHandler()
          .post(
              new Runnable() { // from class:
                // com.android.server.print.PrintManagerService.PrintManagerImpl.4
                @Override // java.lang.Runnable
                public void run() {
                  synchronized (PrintManagerImpl.this.mLock) {
    ...
    public final int resolveCallingAppEnforcingPermissions(int i) {
      ...
      throw new SecurityException(
          "Call from app "
              ...
              + " as app "
              + i
              + " without com.android.printspooler.permission.ACCESS_ALL_PRINT_JOBS");
    }
