...
public class Canvas extends BaseCanvas {
    ...
    public void concat(Matrix matrix) {
        if (matrix != null) {
            nConcat(this.mNativeCanvasWrapper, matrix.m16ni());
        }
    }

    public void setMatrix(Matrix matrix) {
        nSetMatrix(this.mNativeCanvasWrapper, matrix == null ? 0L : matrix.m16ni());
    }

    @Deprecated
    public void getMatrix(Matrix ctm) {
        nGetMatrix(this.mNativeCanvasWrapper, ctm.m16ni());
    }
    ...
    private static void checkValidClipOp(Region.EnumC0836Op op) {
        if (sCompatiblityVersion >= 28 && op != Region.EnumC0836Op.INTERSECT && op != Region.EnumC0836Op.DIFFERENCE) {
            throw new IllegalArgumentException("Invalid Region.Op - only INTERSECT and DIFFERENCE are allowed");
        }
    }

    @Deprecated
    public boolean clipRect(RectF rect, Region.EnumC0836Op op) {
        checkValidClipOp(op);
        return nClipRect(this.mNativeCanvasWrapper, rect.left, rect.top, rect.right, rect.bottom, op.nativeInt);
    }

    @Deprecated
    public boolean clipRect(Rect rect, Region.EnumC0836Op op) {
        checkValidClipOp(op);
        return nClipRect(this.mNativeCanvasWrapper, rect.left, rect.top, rect.right, rect.bottom, op.nativeInt);
    }

    public boolean clipRectUnion(Rect rect) {
        return nClipRect(this.mNativeCanvasWrapper, rect.left, rect.top, rect.right, rect.bottom, Region.EnumC0836Op.UNION.nativeInt);
    }

    public boolean clipRect(RectF rect) {
        return nClipRect(this.mNativeCanvasWrapper, rect.left, rect.top, rect.right, rect.bottom, Region.EnumC0836Op.INTERSECT.nativeInt);
    }

    public boolean clipOutRect(RectF rect) {
        return nClipRect(this.mNativeCanvasWrapper, rect.left, rect.top, rect.right, rect.bottom, Region.EnumC0836Op.DIFFERENCE.nativeInt);
    }
    ...
    @Deprecated
    public boolean clipRect(float left, float top, float right, float bottom, Region.EnumC0836Op op) {
        checkValidClipOp(op);
        return nClipRect(this.mNativeCanvasWrapper, left, top, right, bottom, op.nativeInt);
    }

    public boolean clipRect(float left, float top, float right, float bottom) {
        return nClipRect(this.mNativeCanvasWrapper, left, top, right, bottom, Region.EnumC0836Op.INTERSECT.nativeInt);
    }

    public boolean clipOutRect(float left, float top, float right, float bottom) {
        return nClipRect(this.mNativeCanvasWrapper, left, top, right, bottom, Region.EnumC0836Op.DIFFERENCE.nativeInt);
    }
    ...
    @Deprecated
    public boolean clipPath(Path path, Region.EnumC0836Op op) {
        checkValidClipOp(op);
        return nClipPath(this.mNativeCanvasWrapper, path.readOnlyNI(), op.nativeInt);
    }

    public boolean clipPath(Path path) {
        return clipPath(path, Region.EnumC0836Op.INTERSECT);
    }

    public boolean clipOutPath(Path path) {
        return clipPath(path, Region.EnumC0836Op.DIFFERENCE);
    }

    @Deprecated
    public boolean clipRegion(Region region, Region.EnumC0836Op op) {
        return false;
    }
