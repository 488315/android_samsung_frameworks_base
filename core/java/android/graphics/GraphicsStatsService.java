...
public class GraphicsStatsService extends IGraphicsStats.Stub {
  ...
  public GraphicsStatsService(Context context) {
    ...
    if (!this.mGraphicsStatsDir.exists()) {
      throw new IllegalStateException(
          "Graphics stats directory does not exist: " + this.mGraphicsStatsDir.getAbsolutePath());
    }
    ...
    this.mWriteOutHandler =
        new Handler(
            bgthread.getLooper(),
            new Handler.Callback() { // from class: android.graphics.GraphicsStatsService.1
            ...
            try {
              ParcelFileDescriptor pfd =
                  requestBufferForProcessLocked(
                      token, uid, pid, packageName, info.getLongVersionCode());
              return pfd;
            } catch (Throwable th) {
              th = th;
              throw th;
  ...
  public void saveBuffer(HistoricalBuffer buffer) {
    ...
    synchronized (this.mFileAccessLock) {
      ...
      parent.mkdirs();
      if (!parent.exists()) {
        Log.m102w(TAG, "Unable to create path: '" + parent.getAbsolutePath() + "'");
      } else {
        nSaveBuffer(
  ...
  private void deleteRecursiveLocked(File file) {
    ...
    if (!file.delete()) {
      Log.m102w(TAG, "Failed to delete '" + file.getAbsolutePath() + "'!");
    }
  }
  ...
  */
  private ActiveBuffer fetchActiveBuffersLocked(
      IGraphicsStatsCallback token, int uid, int pid, String packageName, long versionCode)
      throws RemoteException {
  ...
  private final class ActiveBuffer implements IBinder.DeathRecipient {
    ...
    ActiveBuffer(
      ...
      try {
        SharedMemory create =
            SharedMemory.create("GFXStats-" + pid, GraphicsStatsService.this.mAshmemSize);
