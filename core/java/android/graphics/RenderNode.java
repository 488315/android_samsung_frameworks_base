...
public final class RenderNode {
    ...
    public CompositePositionUpdateListener with(PositionUpdateListener listener) {
      return new CompositePositionUpdateListener(
          (PositionUpdateListener[])
              ArrayUtils.appendElement(PositionUpdateListener.class, this.mListeners, listener));
    }

    public CompositePositionUpdateListener without(PositionUpdateListener listener) {
      return new CompositePositionUpdateListener(
          (PositionUpdateListener[])
              ArrayUtils.removeElement(PositionUpdateListener.class, this.mListeners, listener));
    }
  ...
  public void getMatrix(Matrix outMatrix) {
    nGetTransformMatrix(this.mNativeRenderNode, outMatrix.m16ni());
  }

  public void getInverseMatrix(Matrix outMatrix) {
    nGetInverseTransformMatrix(this.mNativeRenderNode, outMatrix.m16ni());
  }
  ...
  public boolean setUseCompositingLayer(boolean forceToLayer, Paint paint) {
    ...
    return didChange
        | nSetLayerPaint(this.mNativeRenderNode, paint != null ? paint.getNativeInstance() : 0L);
  }
  ...
  public boolean setStaticMatrix(Matrix matrix) {
    return nSetStaticMatrix(this.mNativeRenderNode, matrix.m16ni());
  }

  public boolean setAnimationMatrix(Matrix matrix) {
    return nSetAnimationMatrix(this.mNativeRenderNode, matrix != null ? matrix.m16ni() : 0L);
  }

  public Matrix getAnimationMatrix() {
    Matrix output = new Matrix();
    if (nGetAnimationMatrix(this.mNativeRenderNode, output.m16ni())) {
      return output;
    }
    return null;
  }
  ...
  public boolean setRenderEffect(RenderEffect renderEffect) {
    return nSetRenderEffect(
        this.mNativeRenderNode, renderEffect != null ? renderEffect.getNativeInstance() : 0L);
  }
  ...
  public boolean setPosition(Rect position) {
    return nSetLeftTopRightBottom(
        this.mNativeRenderNode, position.left, position.top, position.right, position.bottom);
  }
