...
public class Paint {
  ...
  private static native void nGetFontMetricsIntForText(
      long j, String str, int i, int i2, int i3, int i4, boolean z, FontMetricsInt fontMetricsInt);

  private static native void nGetFontMetricsIntForText(
      long j, char[] cArr, int i, int i2, int i3, int i4, boolean z, FontMetricsInt fontMetricsInt);
  ...
  private static native float nGetRunCharacterAdvance(
      long j, char[] cArr, int i, int i2, int i3, int i4, boolean z, int i5, float[] fArr, int i6);
  ...
  public boolean setFontVariationSettings(String fontVariationSettings) {
    ...
    if (settings == null || settings.length() == 0) {
      this.mFontVariationSettings = null;
      setTypeface(
          Typeface.createFromTypefaceWithVariation(this.mTypeface, Collections.emptyList()));
      return true;
    }
  ...
  public void getFontMetricsInt(
    ...
    try {
      ...
      nGetFontMetricsIntForText(
          this.mNativePaint, buf, start - contextStart, count, 0, contextCount, isRtl, outMetrics);
    } finally {
  ...
  public void getFontMetricsInt(
    ...
    if (count == 0) {
      ...
      nGetFontMetricsIntForText(
          this.mNativePaint, text, start, count, contextStart, contextCount, isRtl, outMetrics);
    }
  }
  ...
  public float measureText(String text, int start, int end) {
    ...
    float w =
        nGetTextAdvances(
            this.mNativePaint, text, start, end, start, end, this.mBidiFlags, (float[]) null, 0);
    setTextSize(oldSize);
    return (float) Math.ceil(this.mInvCompatScaling * w);
  }
  ...
  public int breakText(char[] text, int index, int count, float maxWidth, float[] measuredWidth) {
    ...
    if (!this.mHasCompatScaling) {
      return nBreakText(
          this.mNativePaint, text, index, count, maxWidth, this.mBidiFlags, measuredWidth);
    }
    float oldSize = getTextSize();
    setTextSize(this.mCompatScaling * oldSize);
    int res =
  ...
  public int breakText(
    ...
    if (!this.mHasCompatScaling) {
      return nBreakText(
          this.mNativePaint, text, measureForwards, maxWidth, this.mBidiFlags, measuredWidth);
    }
    float oldSize = getTextSize();
    setTextSize(this.mCompatScaling * oldSize);
    int res =
  ...
  public int getTextWidths(char[] text, int index, int count, float[] widths) {
    ...
    if (!this.mHasCompatScaling) {
      nGetTextAdvances(
          this.mNativePaint, text, index, count, index, count, this.mBidiFlags, widths, 0);
      return count;
    }
    float oldSize = getTextSize();
    setTextSize(this.mCompatScaling * oldSize);
