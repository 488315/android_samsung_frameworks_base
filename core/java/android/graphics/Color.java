...
public class Color {
  ...
  public int toArgb() {
    if (this.mColorSpace.isSrgb()) {
      float[] fArr = this.mComponents;
      return ((int) ((fArr[2] * 255.0f) + 0.5f))
          | (((int) ((fArr[3] * 255.0f) + 0.5f)) << 24)
          | (((int) ((fArr[0] * 255.0f) + 0.5f)) << 16)
          | (((int) ((fArr[1] * 255.0f) + 0.5f)) << 8);
    }
  ...
  public static Color valueOf(float r, float g, float b, float a, ColorSpace colorSpace) {
    if (colorSpace.getComponentCount() > 3) {
      throw new IllegalArgumentException(
          "The specified color space must use a color model with at most 3 color components");
    }
