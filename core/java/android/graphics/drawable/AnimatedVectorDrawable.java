...
public class AnimatedVectorDrawable extends Drawable implements Animatable2 {
  ...
  private AnimatedVectorDrawable(AnimatedVectorDrawableState state, Resources res) {
    ...
    Drawable.Callback callback =
        new Drawable.Callback() { // from class: android.graphics.drawable.AnimatedVectorDrawable.1
  ...
  public Drawable mutate() {
    if (!this.mMutated && super.mutate() == this) {
      this.mAnimatedVectorState =
          new AnimatedVectorDrawableState(this.mAnimatedVectorState, this.mCallback, this.mRes);
      this.mMutated = true;
    }
    return this;
  }
  ...
  public void draw(Canvas canvas) {
    if (!canvas.isHardwareAccelerated()) {
      ...
      if ((vectorDrawableAnimator instanceof VectorDrawableAnimatorRT)
          && !vectorDrawableAnimator.isRunning()
          && ((VectorDrawableAnimatorRT) this.mAnimatorSet).mPendingAnimationActions.size() > 0) {
        fallbackOntoUI();
      }
    }
    ...
    while (true) {
      ...
      if (eventType == 2) {
        ...
        if (ANIMATED_VECTOR.equals(tagName)) {
          TypedArray a =
              obtainAttributes(res, theme, attrs, C4337R.styleable.AnimatedVectorDrawable);
          ...
          TypedArray a2 =
              obtainAttributes(res, theme, attrs, C4337R.styleable.AnimatedVectorDrawableTarget);
          String target = a2.getString(0);
          int animResId = a2.getResourceId(1, 0);
          if (animResId != 0) {
            if (theme != null) {
              Animator animator =
                  AnimatorInflater.loadAnimator(res, theme, animResId, pathErrorScale);
              updateAnimatorProperty(
                  animator, target, state.mVectorDrawable, state.mShouldIgnoreInvalidAnim);
    ...
    if (vectorDrawableAnimator instanceof VectorDrawableAnimatorRT) {
      ...
      if (animator.isRunning()) {
        throw new UnsupportedOperationException(
            "Cannot force Animated Vector Drawable to run on UI thread when the animation has"
                + " started on RenderThread.");
      }
      fallbackOntoUI();
    }
  }
  ...
  public boolean canApplyTheme() {
    ...
    return (animatedVectorDrawableState != null && animatedVectorDrawableState.canApplyTheme())
        || super.canApplyTheme();
  }
  ...
  private static class AnimatedVectorDrawableState extends Drawable.ConstantState {
    ...
    public boolean canApplyTheme() {
      ...
      return (vectorDrawable != null && vectorDrawable.canApplyTheme())
      ...
      if (this.mPendingAnims != null) {
        if (res != null) {
          ...
          Log.m96e(
              ...
              "Failed to load animators. Either the AnimatedVectorDrawable must be created using a"
                  + " Resources object or applyTheme() must be called with a non-null Theme"
                  + " object.");
        }
        this.mPendingAnims = null;
      }
    ...
    private Animator prepareLocalAnimator(int index) {
      ...
      Animator localAnimator = animator.mo547clone();
  ...
  public void reverse() {
    ensureAnimatorSet();
    if (!canReverse()) {
      Log.m102w(LOGTAG, "AnimatedVectorDrawable can't reverse()");
    } else {
          ...
          new AnimatorListenerAdapter() { // from class:
            ...
            public void onAnimationStart(Animator animation) {
              ArrayList<Animatable2.AnimationCallback> tmpCallbacks =
                  new ArrayList<>(AnimatedVectorDrawable.this.mAnimationCallbacks);
  ...
  private static class VectorDrawableAnimatorUI implements VectorDrawableAnimator {
    ...
    public void init(AnimatorSet set) {
      ...
      AnimatorSet mo547clone = set.mo547clone();
      this.mSet = mo547clone;
      long totalDuration = mo547clone.getTotalDuration();
        ...
        if (propertyId < 0) {
          if (this.mDrawable.mAnimatedVectorState.mShouldIgnoreInvalidAnim) {
            ...
            throw new IllegalArgumentException(
                "Property: " + this.mTmpValues.propertyName + " is not supported for FullPath");
          }
        } else {
          propertyPtr =
