...
public class RippleDrawable extends LayerDrawable {
  ...
  public void setBackgroundActive(
      boolean hovered, boolean focused, boolean pressed, boolean windowFocused) {
    ...
    if (this.mState.mRippleStyle == 0) {
      ...
      if (Looper.myLooper() == null) {
        Log.m102w(TAG, "Thread doesn't have a looper. Skipping animation.");
        return;
      }
      if (this.mBackground == null && (hovered || focused)) {
        RippleBackground rippleBackground =
            new RippleBackground(this, this.mHotspotBounds, isBounded());
  ...
  public void inflate(Resources r, XmlPullParser parser, AttributeSet attrs, Resources.Theme theme)
    ...
    TypedArray a = obtainAttributes(r, theme, attrs, C4337R.styleable.RippleDrawable);
    setPaddingMode(1);
    ...
    if (this.mState.mColor == null) {
      if (this.mState.mTouchThemeAttrs == null || this.mState.mTouchThemeAttrs[0] == 0) {
        throw new XmlPullParserException(
            a.getPositionDescription() + ": <ripple> requires a valid color attribute");
      }
    }
  }
  ...
  public void applyTheme(Resources.Theme t) {
    ...
    if (state.mTouchThemeAttrs != null) {
      TypedArray a = t.resolveAttributes(state.mTouchThemeAttrs, C4337R.styleable.RippleDrawable);
      try {
        try {
          updateStateFromTypedArray(a);
          verifyRequiredAttributes(a);
  ...
  private void onHotspotBoundsChanged() {
    ...
    for (int i2 = 0; i2 < this.mRunningAnimations.size(); i2++) {
      ...
      s.getProperties()
          .getShader()
          .setResolution(this.mHotspotBounds.width(), this.mHotspotBounds.height());
  ...
  private void enterPatternedBackgroundAnimation(
      boolean focused, boolean hovered, boolean windowFocused) {
    float f = 0.0f;
  ...
  private void startBackgroundAnimation() {
    ...
    ValueAnimator ofFloat =
        ValueAnimator.ofFloat(this.mBackgroundOpacity, this.mTargetBackgroundOpacity);
    this.mBackgroundAnimation = ofFloat;
    ofFloat.setInterpolator(LINEAR_INTERPOLATOR);
    this.mBackgroundAnimation.setDuration(80L);
    this.mBackgroundAnimation.addUpdateListener(
        new ValueAnimator
  ...
  private void drawPatterned(Canvas canvas) {
    ...
    if (!addRipple || this.mRunningAnimations.size() > 10) {
      ...
      this.mRunningAnimations.add(
          new RippleAnimationSession(properties, !useCanvasProps)
              .setOnAnimationUpdated(
              ...
              .setForceSoftwareAnimation(useCanvasProps ? false : true)
              .enter(canvas));
    }
    if (shouldExit) {
      for (int i = 0; i < this.mRunningAnimations.size(); i++) {
    ...
    while (true) {
      ...
      if (!canvas.isHardwareAccelerated()) {
        Log.m96e(
            TAG,
            "The RippleDrawable.STYLE_PATTERNED animation is not supported for a non-hardware"
                + " accelerated Canvas. Skipping animation.");
        break;
      }
      if (useCanvasProps) {
        RippleAnimationSession.AnimationProperties<CanvasProperty<Float>, CanvasProperty<Paint>> p =
            s.getCanvasProperties();
  ...
  private RippleAnimationSession.AnimationProperties<Float, Paint> createAnimationProperties(
    ...
    RippleAnimationSession.AnimationProperties<Float, Paint> properties =
        new RippleAnimationSession.AnimationProperties<>(
            Float.valueOf(cx),
            Float.valueOf(cy),
            Float.valueOf(radius),
            Float.valueOf(noisePhase),
            p,
            Float.valueOf(0.0f),
            color2,
            shader);
  ...
  private void updateMaskShaderIfNeeded() {
    ...
    if (bitmap2 == null
        || bitmap2.getWidth() != bounds.width()
        || this.mMaskBuffer.getHeight() != bounds.height()) {
      ...
      this.mMaskShader =
          new BitmapShader(createBitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
  ...
  private int getMaskType() {
    ...
    if (this.mRipple == null
        && this.mExitingRipplesCount <= 0
        && (((rippleBackground = this.mBackground) == null || !rippleBackground.isVisible())
            && this.mState.mRippleStyle == 0)) {
      return -1;
    }
  ...
  Paint updateRipplePaint() {
    ...
    if (this.mMaskColorFilter != null) {
      ...
      if (this.mMaskColorFilter.getColor() != maskColor) {
        this.mMaskColorFilter =
            new PorterDuffColorFilter(maskColor, this.mMaskColorFilter.getMode());
        this.mFocusColorFilter =
            new PorterDuffColorFilter(color | (-16777216), this.mFocusColorFilter.getMode());
      }
  ...
  static class RippleState extends LayerDrawable.LayerState {
    ...
    public boolean canApplyTheme() {
      ...
      return this.mTouchThemeAttrs != null
          || ((colorStateList = this.mColor) != null && colorStateList.canApplyTheme())
          || super.canApplyTheme();
    }
    ...
    public int getChangingConfigurations() {
      ...
      return changingConfigurations
          | (colorStateList != null ? colorStateList.getChangingConfigurations() : 0);
    }
  }
