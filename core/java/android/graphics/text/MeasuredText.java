...
public class MeasuredText {
  ...
  public float getWidth(int start, int end) {
    Preconditions.checkArgument(
        start >= 0 && start <= this.mChars.length,
        "start(%d) must be 0 <= start <= %d",
        Integer.valueOf(start),
        Integer.valueOf(this.mChars.length));
    Preconditions.checkArgument(
        end >= 0 && end <= this.mChars.length,
        "end(%d) must be 0 <= end <= %d",
        Integer.valueOf(end),
        Integer.valueOf(this.mChars.length));
    Preconditions.checkArgument(
        start <= end,
        "start(%d) is larger than end(%d)",
        Integer.valueOf(start),
        Integer.valueOf(end));
  ...
  public float getCharWidthAt(int offset) {
    Preconditions.checkArgument(
        offset >= 0 && offset < this.mChars.length,
        "offset(%d) is larger than text length %d" + offset,
        Integer.valueOf(this.mChars.length));
  ...
  public static final class Builder {
    ...
    private static native void nAddStyleRun(
        long j, long j2, int i, int i2, int i3, int i4, boolean z);

    private static native long nBuildMeasuredText(
        long j, long j2, char[] cArr, boolean z, boolean z2, boolean z3);
    ...
    public Builder setComputeHyphenation(int mode) {
      switch (mode) {
        ...
        case 2:
          ...
          return this;
        default:
          Log.m96e(MeasuredText.TAG, "Unknown hyphenation mode: " + mode);
          this.mComputeHyphenation = false;
          this.mFastHyphenation = false;
          return this;
      }
