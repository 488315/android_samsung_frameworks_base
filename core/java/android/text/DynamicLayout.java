...
public class DynamicLayout extends Layout {
  ...
  public void reflow(CharSequence s, int where, int before, int after) {
    ...
    if (reflowed == null) {
      ...
      StaticLayout.Builder b3 =
          StaticLayout.Builder.obtain(text, where2, where2 + after3, getPaint(), getWidth());
      reflowed2 = reflowed3;
      b2 = b3;
    } else {
      reflowed2 = reflowed;
    ...
    int n2 =
        (where2 + after3 != len && reflowed2.getLineStart(n - 1) == where2 + after3) ? n - 1 : n;
  ...
  public void updateBlocks(int startLine, int endLine, int newLineCount) {
    ...
    if (newNumberOfBlocks > iArr.length) {
      int[] blockEndLines =
          ArrayUtils.newUnpaddedIntArray(Math.max(iArr.length * 2, newNumberOfBlocks));
      ...
      if (numAddedBlocks + numRemovedBlocks != 0) {
        System.arraycopy(
            iArr, lastBlock + 1, iArr, firstBlock + numAddedBlocks, (i - lastBlock) - 1);
