    ...
    if (actualWidth < availableWidth) {
      if (a == Layout.Alignment.ALIGN_CENTER) {
        x2 = left - ((availableWidth - actualWidth) / 2);
          ...
          || ((!ltr && a == Layout.Alignment.ALIGN_NORMAL) || a == Layout.Alignment.ALIGN_RIGHT)) {
  ...
  public static boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
    ...
    switch (event.getActionMasked()) {
      case 0:
        ...
        buffer.setSpan(
            new DragState(event.getX(), event.getY(), widget.getScrollX(), widget.getScrollY()),
            0,
            0,
            17);
        return true;
      ...
      case 2:
        ...
        if (ds2.length > 0) {
          if (!ds2[0].mFarEnough) {
            ...
            if (Math.abs(event.getX() - ds2[0].f524mX) >= slop
                || Math.abs(event.getY() - ds2[0].f525mY) >= slop) {
          ...
          if (ds2[0].mFarEnough) {
            ...
            boolean cap =
                ((event.getMetaState() & 1) == 0
                    ...
                    : true;
            if (cap) {
              dx = event.getX() - ds2[0].f524mX;
              dy = event.getY() - ds2[0].f525mY;
            } else {
              dx = ds2[0].f524mX - event.getX();
              dy = ds2[0].f525mY - event.getY();
            }
            ds2[0].f524mX = event.getX();
            ds2[0].f525mY = event.getY();
            int nx = widget.getScrollX() + ((int) dx);
            int ny = widget.getScrollY() + ((int) dy);
            int padding = widget.getTotalPaddingTop() + widget.getTotalPaddingBottom();
            Layout layout = widget.getLayout();
            int ny2 =
                Math.max(Math.min(ny, layout.getHeight() - (widget.getHeight() - padding)), 0);
