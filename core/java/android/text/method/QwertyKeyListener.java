...
public class QwertyKeyListener extends BaseKeyListener {
  ...
  */
  public boolean onKeyDown(View view, Editable content, int keyCode, KeyEvent event) {
    ...
    int activeStart2 = content.getSpanStart(TextKeyListener.ACTIVE);
    int activeEnd3 = content.getSpanEnd(TextKeyListener.ACTIVE);
    int i3 = event.getUnicodeChar(getMetaState(content, event));
    if (this.mFullKeyboard) {
      i = i3;
      activeEnd = activeEnd3;
    ...
    if (i2 != 0) {
      ...
      if (activeStart == selStart && activeEnd == selEnd) {
        ...
        if ((selEnd - selStart) - 1 == 0) {
          ...
          if (i2 == composed && event.getRepeatCount() > 0) {
            return true;
          }
          if (composed != 0) {
            i2 = composed;
      ...
      if ((pref & 1) != 0
          && Character.isLowerCase(i2)
          && TextKeyListener.shouldCap(this.mAutoCap, content, selStart)) {
      ...
      if ((pref & 2) != 0 && this.mAutoText) {
        if (i2 != 32
          ...
          if (rep == null) {
            ...
            for (Replaced replaced :
                (Replaced[]) content.getSpans(0, content.length(), Replaced.class)) {
      ...
      if ((pref & 4) != 0 && this.mAutoText) {
        ...
        if (selEnd5 - 3 >= 0
            && content.charAt(selEnd5 - 1) == ' '
            && content.charAt(selEnd5 - 2) == ' ') {
          ...
          if (Character.isLetter(c3) || Character.isDigit(c3)) {
            content.replace(selEnd5 - 2, selEnd5 - 1, MediaMetrics.SEPARATOR);
          }
        }
      }
      return true;
    }
    int activeStart3 = activeStart;
    if (keyCode == 67) {
      if (event.hasNoModifiers()) {
        activeEnd2 = activeEnd;
      } else {
        activeEnd2 = activeEnd;
      }
      if (selStart == selEnd) {
        ...
        Replaced[] repl =
            (Replaced[]) content.getSpans(selStart - consider, selStart, Replaced.class);
        if (repl.length > 0) {
