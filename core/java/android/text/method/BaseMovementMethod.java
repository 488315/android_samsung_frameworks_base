...
public class BaseMovementMethod implements MovementMethod {
  ...
  public boolean onKeyOther(TextView widget, Spannable text, KeyEvent event) {
    ...
    if (keyCode != 0 && event.getAction() == 2) {
      ...
      for (int i = 0;
          i < repeat && handleMovementKey(widget, text, keyCode, movementMetaState, event);
          i++) {
        handled = true;
      }
      if (handled) {
  ...
  /* renamed from: up */
  protected boolean mo89up(TextView widget, Spannable buffer) {
    return false;
  }
  ...
  protected boolean scrollDown(TextView widget, Spannable buffer, int amount) {
    ...
    if (bottomLine <= limit) {
      Touch.scrollTo(
          ...
          layout,
          widget.getScrollX(),
          layout.getLineTop(Math.min((bottomLine + amount) - 1, limit) + 1) - innerHeight);
      return true;
    }
    return false;
  }
  ...
  protected boolean scrollPageDown(TextView widget, Spannable buffer) {
    ...
    if (bottomLine <= layout.getLineCount() - 1) {
      Touch.scrollTo(
          widget, layout, widget.getScrollX(), layout.getLineTop(bottomLine + 1) - innerHeight);
      return true;
    }
    return false;
  }
