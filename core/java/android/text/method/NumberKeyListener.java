...
public abstract class NumberKeyListener extends BaseKeyListener implements InputFilter {
  ...
  public CharSequence filter(
    ...
    int i = start;
    while (i < end && m90ok(accept, source.charAt(i))) {
      i++;
    }
    if (i == end) {
      return null;
    ...
    for (int j = end2 - 1; j >= i2; j--) {
      if (!m90ok(accept, source.charAt(j))) {
  ...
  /* renamed from: ok */
  protected static boolean m90ok(char[] accept, char c) {
  ...
  static boolean addFormatCharsFromSkeleton(
      Collection<Character> collection, Locale locale, String skeleton, String symbolsToIgnore) {
    if (locale == null) {
      return false;
    }
  ...
  static boolean addFormatCharsFromSkeletons(
      Collection<Character> collection, Locale locale, String[] skeletons, String symbolsToIgnore) {
