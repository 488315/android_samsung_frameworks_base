...
public class TextLine {
          ...
          if (j3 == runLimit || charAt(j3) == '\t') {
            ...
            horizontal2 +=
                shapeRun(
                    ...
                    runIsRtl,
                    f,
                    (runIndex == runCount + (-1) && j3 == this.mLen) ? false : true);
            if (j != runLimit) {
              int i = this.mDir;
  ...
  public float measure(int offset, boolean trailing, Paint.FontMetricsInt fmi) {
    ...
    while (runIndex < this.mDirections.getRunCount()
        && (runStart = this.mDirections.getRunStart(runIndex)) <= this.mLen) {
      int runLimit = Math.min(this.mDirections.getRunLength(runIndex) + runStart, this.mLen);
      boolean runIsRtl = this.mDirections.isRunRtl(runIndex);
      float h2 = h;
      int segStart = runStart;
  ...
  public void measureAllBounds(float[] bounds, float[] advances) {
    ...
    int runIndex = 0;
    while (runIndex < this.mDirections.getRunCount()
        && (j = this.mDirections.getRunStart(runIndex)) <= this.mLen) {
  ...
  private float drawRun(
    ...
    if ((this.mDir == 1) == runIsRtl) {
      ...
      handleRun(
          start, limit, limit, runIsRtl, c, null, x + w, top, y, bottom, null, false, null, 0);
      return w;
    }
    return handleRun(
        start, limit, limit, runIsRtl, c, null, x, top, y, bottom, null, needWidth, null, 0);
  }
  ...
  private float shapeRun(
    ...
    if ((this.mDir == 1) == runIsRtl) {
      float w = -measureRun(start, limit, limit, runIsRtl, null, null, 0);
      handleRun(
          start, limit, limit, runIsRtl, null, consumer, x + w, 0, 0, 0, null, false, null, 0);
      return w;
    }
    return handleRun(
        start, limit, limit, runIsRtl, null, consumer, x, 0, 0, 0, null, needWidth, null, 0);
  }
    ...
    if (cursor == 0) {
      ...
      if (runLimit2 == runLevel3) {
        ...
        if (cursor == (advance ? runLimit : runStart) && advance == trailing) {
          ...
          runs = runs2;
          int newCaret3 =
              getOffsetBeforeAfter(runLimit2, runStart, runLimit, runIsRtl, cursor, advance);
      ...
      if (newCaret == -1) {
        ...
        newCaret =
            getOffsetBeforeAfter(
                ...
                otherRunLimit,
                otherRunIsRtl,
                advance3 ? otherRunStart : otherRunLimit,
                advance3);
  ...
  */
  private int getOffsetBeforeAfter(
        ...
        if (this.mSpanned != null && runStart != runLimit) {
          ...
          while (true) {
            int nextSpanTransition =
                this.mSpanned.nextSpanTransition(
                    this.mStart + spanStart2, limit, MetricAffectingSpan.class);
            i = this.mStart;
          ...
          MetricAffectingSpan[] spans =
              (MetricAffectingSpan[])
                  TextUtils.removeEmptySpans(
                      (MetricAffectingSpan[])
                          this.mSpanned.getSpans(
                              i + spanStart2, i + spanLimit2, MetricAffectingSpan.class),
                      this.mSpanned,
          ...
          if (!this.mCharsValid) {
            return wp.getTextRunCursor(
                this.mChars, spanStart, spanLimit - spanStart, runIsRtl, offset, cursorOpt);
          }
          CharSequence charSequence = this.mText;
          int i2 = this.mStart;
          return wp.getTextRunCursor(
                  charSequence, i2 + spanStart, i2 + spanLimit, runIsRtl, i2 + offset, cursorOpt)
              - this.mStart;
        }
        spanStart = spanStart2;
        spanLimit = runLimit;
    ...
    return after
        ? TextUtils.getOffsetAfter(this.mText, this.mStart + offset) - this.mStart
        : TextUtils.getOffsetBefore(this.mText, this.mStart + offset) - this.mStart;
  }
    ...
    if (this.mCharsValid) {
      ...
      if (precomputedText == null) {
        ...
        wp.getFontMetricsInt(
            charSequence, i + start, count, i + contextStart, contextCount, runIsRtl, fmi);
      } else {
        int i2 = this.mStart;
  ...
  private float getRunAdvance(
      ...
      int advancesIndex) {
    if (this.mCharsValid) {
      return wp.getRunCharacterAdvance(
          this.mChars,
          start,
          end,
          contextStart,
  ...
  private float handleText(
    ...
    if (needWidth
      ...
      totalWidth3 =
          getRunAdvance(
              wp, start, end, contextStart, contextEnd, runIsRtl, offset, advances, advancesIndex);
    } else {
      numDecorations = numDecorations2;
    }
    ...
    if (c != null) {
      ...
      drawTextRun(
          c, wp, start, end, contextStart, contextEnd, runIsRtl, leftX, y + wp.baselineShift);
      if (numDecorations != 0) {
        int i = 0;
        while (i < numDecorations) {
          ...
          float decorationStartAdvance =
              getRunAdvance(
                  wp, start, end, contextStart, contextEnd, runIsRtl, decorationStart, null, 0);
          float decorationEndAdvance =
              getRunAdvance(
                  wp, start, end, contextStart, contextEnd, runIsRtl, decorationEnd, null, 0);
          if (runIsRtl) {
          ...
          if (info2.underlineColor != 0) {
            ...
            drawStroke(
                wp,
                c,
                info2.underlineColor,
                wp.getUnderlinePosition(),
                info2.underlineThickness,
                decorationXLeft,
                decorationXRight,
                y);
          } else {
  ...
  private float handleRun(
    ...
    if (advances != null && advances.length - advancesIndex < measureLimit - start) {
      throw new IndexOutOfBoundsException(
          "advances doesn't have enough space to receive the result");
    }
    ...
    if (spanned == null) {
      ...
      needsSpanMeasurement =
          (this.mMetricAffectingSpanSpanSet.numberOfSpans == 0
                  && this.mCharacterStyleSpanSet.numberOfSpans == 0)
              ? false
              : true;
    }
        ...
        if (this.mMetricAffectingSpanSpanSet.spanStarts[j2] < this.mStart + mlimit3) {
          ...
          if (i7 > i8 + i5) {
            boolean insideEllipsis =
                i8 + this.mEllipsisStart <= this.mMetricAffectingSpanSpanSet.spanStarts[j2]
                    && this.mMetricAffectingSpanSpanSet.spanEnds[j2]
                        <= this.mStart + this.mEllipsisEnd;
      ...
      if (replacement != null) {
        ...
        float width =
            handleReplacement(
                ...
                bottom,
                fmi,
                needWidth || mlimit3 < measureLimit);
        x2 += width;
        if (advances != null) {
        ...
        while (j4 < mlimit4) {
          ...
          for (int k = 0; k < textLine2.mCharacterStyleSpanSet.numberOfSpans; k++) {
            if (textLine2.mCharacterStyleSpanSet.spanStarts[k] < textLine2.mStart + offset
                && textLine2.mCharacterStyleSpanSet.spanEnds[k] > textLine2.mStart + j4) {
          ...
          if (j4 == i5) {
            ...
            activePaint2.setStartHyphenEdit(
                textLine2.adjustStartHyphenEdit(
                    activeStart3, textLine2.mPaint.getStartHyphenEdit()));
            activePaint2.setEndHyphenEdit(
                textLine2.adjustEndHyphenEdit(activeEnd, textLine2.mPaint.getEndHyphenEdit()));
            if (needWidth) {
              i2 = measureLimit;
              mlimit2 = mlimit4;
            } else {
              ...
              if (activeEnd >= i2) {
                ...
                x3 +=
                    handleText(
                        ...
                        fmi,
                        z2,
                        Math.min(activeEnd, mlimit2),
                        textLine2.mDecorations,
                        advances,
                        (advancesIndex + activeStart3) - i9);
                wp = wp6;
                activePaint = activePaint2;
                activePaint.set(wp);
                textLine = this;
        ...
        activePaint3.setStartHyphenEdit(
            textLine3.adjustStartHyphenEdit(activeStart4, textLine3.mPaint.getStartHyphenEdit()));
        activePaint3.setEndHyphenEdit(
            textLine3.adjustEndHyphenEdit(activeEnd2, textLine3.mPaint.getEndHyphenEdit()));
          ...
          x2 =
              x3
                  + handleText(
                      ...
                      fmi,
                      z,
                      Math.min(activeEnd2, mlimit5),
                      textLine3.mDecorations,
                      advances,
                      (advancesIndex + activeStart4) - start);
        }
        z = true;
        x2 =
            x3
    ...
    if (this.mCharsValid) {
      glyphs =
          TextRunShaper.shapeTextRun(
              this.mChars, start, count, contextStart, contextCount, x, 0.0f, runIsRtl, paint);
    } else {
      CharSequence charSequence = this.mText;
      int i = this.mStart;
      glyphs =
