  ...
  public static int getOffsetBefore(CharSequence text, int offset) {
    ...
    if (text instanceof Spanned) {
      ReplacementSpan[] spans =
          (ReplacementSpan[]) ((Spanned) text).getSpans(offset2, offset2, ReplacementSpan.class);
  ...
  public static CharSequence toUpperCase(Locale locale, CharSequence source, boolean copySpans) {
    ...
    if (!copySpans) {
      return edits.hasChanges()
          ? (StringBuilder) CaseMap.toUpper().apply(locale, source, new StringBuilder(), edits)
          : source;
    }
    SpannableStringBuilder result =
        (SpannableStringBuilder)
            CaseMap.toUpper().apply(locale, source, new SpannableStringBuilder(), edits);
    if (!edits.hasChanges()) {
      return source;
    }
    ...
    for (Object span : spans) {
      ...
      int destStart =
          sourceStart == sourceLength ? result.length() : toUpperMapToDest(iterator, sourceStart);
      int destEnd =
          sourceEnd == sourceLength ? result.length() : toUpperMapToDest(iterator, sourceEnd);
    ...
    for (int i2 = totalLen - 1; i2 >= 0; i2--) {
      ...
      if (remainingElements > 0) {
        if (res == null) {
          ...
          morePiece =
              res.getQuantityString(moreId, remainingElements, Integer.valueOf(remainingElements));
        }
  ...
  */
    ...
    try {
      ...
      while (i3 < len) {
        ...
        if (buf[i3] == c) {
          ...
          i = i3;
          tempMt =
              MeasuredParagraph.buildForMeasurement(p, format, 0, format.length(), textDir, tempMt);
  ...
  public static CharSequence makeSafeForPresentation(
    ...
    Preconditions.checkArgument(
        z,
        "Cannot set SAFE_STRING_FLAG_SINGLE_LINE and SAFE_STRING_FLAG_FIRST_LINE at thesame time");
    String shortString =
  ...
  public static boolean isHalant(char c) {
    return c == 2381 || c == 2509 || c == 2637 || c == 2765 || c == 3021 || c == 3149 || c == 3277
        || c == 3405 || c == 3551 || c == 2893;
  }
  ...
  public static int indexOfWordPrefix(CharSequence text, char[] prefix) {
    ...
    for (int i = 0; i < textLength && i + prefixLength <= textLength; i++) {
      ...
      while (j < prefixLength
          && Character.toUpperCase(text.charAt(i + j)) == Character.toUpperCase(prefix[j])) {
        j++;
      }
      if (j == prefixLength) {
        return i;
