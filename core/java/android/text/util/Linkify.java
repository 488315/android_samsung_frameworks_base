...
public class Linkify {
  ...
  private static boolean addLinks(
    ...
    if ((mask & 8192) != 0) {
      ...
      while (j < links.size()) {
        ...
        if (linkedText2.endsWith(".ht")
            ...
            && "tp".equalsIgnoreCase(text.toString().substring(link2.end, link2.end + 2))) {
          if (links.size() > j + 1) {
            LinkSpec linkNext = links.get(j + 1);
            if ((linkNext.start == link2.end + 6
                    && text.length() > link2.end + 5
                    && "tp://"
                        .equalsIgnoreCase(text.toString().substring(link2.end, link2.end + 5)))
                || (linkNext.start == link2.end + 7
                    && text.length() > link2.end + 6
                    && "tps://"
                        .equalsIgnoreCase(text.toString().substring(link2.end, link2.end + 6)))) {
    ...
    if (j2 != 0) {
      gatherLinks(
          links, text, Patterns.AUTOLINK_EMAIL_ADDRESS, new String[] {"mailto:"}, null, null);
    }
    if ((49156 & mask) == 0) {
      spannable = text;
    } else {
  ...
  public static boolean containsUnsupportedCharacters(String text) {
    if (text.contains("\u202c")) {
      Log.m96e(LOG_TAG, "Unsupported character for applying links: u202C");
      return true;
    }
    if (text.contains("\u202d")) {
      Log.m96e(LOG_TAG, "Unsupported character for applying links: u202D");
      return true;
    }
    if (text.contains("\u202e")) {
      Log.m96e(LOG_TAG, "Unsupported character for applying links: u202E");
      return true;
    }
    return false;
  }
  ...
  private static void applyLink(
      String url, int start, int end, Spannable text, Function<String, URLSpan> urlSpanFactory) {
  ...
  private static void gatherTelLinks(ArrayList<LinkSpec> links, Spannable s, Context context) {
    ...
    String regionCode =
        ctx != null
            ? ((TelephonyManager) ctx.getSystemService(TelephonyManager.class))
                .getSimCountryIso()
                .toUpperCase(Locale.US)
            : Locale.getDefault().getCountry();
    Iterable<PhoneNumberMatch> matches =
        phoneUtil.findNumbers(
            s.toString(), regionCode, PhoneNumberUtil.Leniency.POSSIBLE, Long.MAX_VALUE);
    for (PhoneNumberMatch match : matches) {
      ...
      if ((rawString.charAt(0) == '['
              && !rawString.contains(NavigationBarInflaterView.SIZE_MOD_END))
          || (rawString.charAt(0) == '('
              && !rawString.contains(NavigationBarInflaterView.KEY_CODE_END))) {
        spec.start++;
      } else if (rawString.charAt(0) == '+' && rawString.charAt(1) == '+') {
        spec.start++;
      }
      if (!"KR".equals(regionCode)
          || needToAddLink(s.toString(), match.rawString(), spec.start, spec.end)) {
        links.add(spec);
      }
    }
  }
  ...
  private static boolean needToAddLink(String rawStr, String matchStr, int start, int end) {
    ...
    if (charAfterMatch1 == 50896
        ...
        || (charAfterMatch1 == ' ' && (charAfterMatch2 == 50896 || charAfterMatch2 == 8361))) {
      return false;
    }
