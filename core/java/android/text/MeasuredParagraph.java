...
public class MeasuredParagraph {
  ...
  public Layout.Directions getDirections(int start, int end) {
    ...
    return AndroidBidi.directions(
        this.mParaDir, this.mLevels.getRawArray(), start, this.mCopiedBuffer, start, length);
  }
    ...
    if (mt.mSpanned == null) {
      ...
      while (spanStart < end) {
        ...
        mt.applyMetricsAffectingSpan(
            ...
            null,
            (MetricAffectingSpan[])
                TextUtils.removeEmptySpans(spans, mt.mSpanned, MetricAffectingSpan.class),
            spanStart,
            spanEnd,
            null);
        spanStart = spanEnd;
  ...
  public static MeasuredParagraph buildForStaticLayout(
    ...
    if (mt2.mSpanned == null) {
      ...
      while (spanStart < end) {
        ...
        MetricAffectingSpan[] spans =
            (MetricAffectingSpan[])
                mt2.mSpanned.getSpans(spanStart, spanEnd, MetricAffectingSpan.class);
        MeasuredParagraph mt3 = mt2;
        mt2.applyMetricsAffectingSpan(
            paint,
            lineBreakConfig,
            (MetricAffectingSpan[])
                TextUtils.removeEmptySpans(spans, mt2.mSpanned, MetricAffectingSpan.class),
            spanStart,
            spanEnd,
            builder);
  ...
  private void applyStyleRun(
      int start, int end, TextPaint paint, LineBreakConfig config, MeasuredText.Builder builder) {
    int levelEnd;
    boolean z = false;
    if (this.mLtrWithoutBidi) {
      if (builder == null) {
  ...
  private void applyMetricsAffectingSpan(
    ...
    if (replacement != null) {
      applyReplacementRun(
          replacement, startInCopiedBuffer, endInCopiedBuffer, this.mCachedPaint, builder);
    } else {
      applyStyleRun(
          startInCopiedBuffer, endInCopiedBuffer, this.mCachedPaint, lineBreakConfig, builder);
    }
    if (needFontMetrics) {
