...
public class SpannableStringBuilder
  ...
  private static final int[][] sCachedIntBuffer =
      (int[][]) Array.newInstance((Class<?>) Integer.TYPE, 6, 0);
  ...
  public SpannableStringBuilder(CharSequence text, int start, int end) {
    ...
    char[] newUnpaddedCharArray =
        ArrayUtils.newUnpaddedCharArray(GrowingArrayUtils.growSize(srclen));
      ...
      for (int i2 = 0; i2 < this.mSpanCount; i2++) {
        ...
        if (start > where) {
          start += this.mGapLength;
            ...
            && ((flag = (this.mSpanFlags[i2] & 240) >> 4) == 2 || (atEnd && flag == 3))) {
          start += this.mGapLength;
        }
        if (end > i3) {
          end -= this.mGapLength;
        ...
        if (end > where) {
            ...
            && ((flag2 = this.mSpanFlags[i2] & 15) == 2 || (atEnd && flag2 == 3))) {
          end += this.mGapLength;
        }
        iArr[i2] = start;
        iArr2[i2] = end;
  ...
  private boolean removeSpansForChange(int start, int end, boolean textIsRemoved, int i) {
    ...
    int i3;
    if ((i & 1) != 0
        && resolveGap(this.mSpanMax[i]) >= start
    ...
    if ((this.mSpanFlags[i] & 33) == 33 && (i2 = this.mSpanStarts[i]) >= start) {
      ...
      if (i2 < i4 + i5
          && (i3 = this.mSpanEnds[i]) >= start
          && i3 < i5 + i4
          && (textIsRemoved || i2 > start || i3 < i4)) {
        this.mIndexOfSpan.remove(this.mSpans[i]);
        removeSpan(i, 0);
        return true;
      }
    }
    return resolveGap(this.mSpanStarts[i]) <= end
        && (i & 1) != 0
  ...
  private void change(int start, int end, CharSequence cs, int csStart, int csEnd) {
    ...
    if (replacedLength > 0) {
      ...
      while (i9 < this.mSpanCount) {
        ...
        iArr2[i9] =
            updatedIntervalBound(iArr2[i9], start, nbNewChars, startFlag, atEnd, textIsRemoved);
        int endFlag = this.mSpanFlags[i9] & 15;
        int[] iArr3 = this.mSpanEnds;
        int i10 = i9;
        boolean textIsRemoved2 = textIsRemoved;
        iArr3[i10] =
            updatedIntervalBound(iArr3[i9], start, nbNewChars, endFlag, atEnd, textIsRemoved2);
        i9 = i10 + 1;
        i3 = i3;
        i4 = i4;
  ...
  private int updatedIntervalBound(
      int offset, int start, int nbNewChars, int flag, boolean atEnd, boolean textIsRemoved) {
    if (offset >= start) {
      int i = this.mGapStart;
  ...
  */
    ...
    for (int i = 0; i < this.mSpanCount; i++) {
      ...
      if ((spanFlags & 2048) == 0) {
        ...
        if (spanStart2 > newReplaceEnd) {
          if (nbNewChars != 0) {
            ...
            if (spanEnd2 <= newReplaceEnd) {
              if (nbNewChars != 0) {
                ...
                if (spanChanged) {
                  sendSpanChanged(
                      this.mSpans[i], previousSpanStart, previousSpanEnd, spanStart2, spanEnd2);
                }
                int[] iArr = this.mSpanFlags;
                iArr[i] = iArr[i] & (-61441);
              }
              ...
              if (spanEnd2 >= replaceStart
                  && ((spanEnd2 != replaceStart || (spanFlags & 16384) != 16384)
                      && (spanEnd2 != newReplaceEnd || (spanFlags & 32768) != 32768))) {
                spanChanged = true;
                previousSpanEnd = spanEnd2;
                if (spanChanged) {}
          ...
          if (spanStart2 >= replaceStart
              && ((spanStart2 != replaceStart || (spanFlags & 4096) != 4096)
                  && (spanStart2 != newReplaceEnd || (spanFlags & 8192) != 8192))) {
            spanChanged2 = true;
            previousSpanStart = spanStart2;
            if (spanEnd2 <= newReplaceEnd) {}
          }
  ...
  private void setSpan(
      boolean send, Object what, int start, int end, int flags, boolean enforceParagraph) {
    int start2;
    int end2;
    Integer index;
    int ostart;
    ...
    if (flagsStart == 2 && flagsEnd == 1 && start == end) {
      if (send) {
        Log.m96e(TAG, "SPAN_EXCLUSIVE_EXCLUSIVE spans cannot have a zero length");
        return;
      }
      return;
    }
    ...
    if (identityHashMap == null || (index = identityHashMap.get(what)) == null) {
      ...
      this.mSpanOrder =
          GrowingArrayUtils.append(this.mSpanOrder, this.mSpanCount, this.mSpanInsertCount);
      ...
      if (spanStart <= queryEnd) {
        ...
        if (spanEnd >= queryStart
            && ((spanStart == spanEnd
                ...
                && (Object.class == kind || kind.isInstance(this.mSpans[i])))) {
          count++;
        }
        if ((i & 1) != 0) {
  ...
  */
  private <T> int getSpansRec(
    ...
    if ((i & 1) != 0) {
      ...
      if (spanMax >= queryStart) {
        count2 =
            getSpansRec(
                queryStart, queryEnd, kind, left, tArr, priority, insertionOrder, count, sort);
        if (i < this.mSpanCount) {
          return count2;
        }
        ...
        if (spanStart <= queryEnd) {
          ...
          if (spanEnd >= queryStart
            ...
            if (sort) {
              ...
              int j = 0;
              while (j < count2) {
                int p = getSpanFlags(tArr[j]) & Spanned.SPAN_PRIORITY;
                if (spanPriority > p) {
                  break;
                }
                j++;
            ...
            tArr[target] = this.mSpans[i];
            count3 = count2 + 1;
          } else {
            count3 = count2;
          }
          if (count3 >= tArr.length || (i & 1) == 0) {
            return count3;
          }
          return getSpansRec(
              queryStart,
  ...
  private final <T> void siftDown(
      int index, T[] array, int size, int[] priority, int[] insertionOrder) {
    int left = (index * 2) + 1;
    while (left < size) {
  ...
  private void sendSpanChanged(Object what, int oldStart, int oldEnd, int start, int end) {
    SpanWatcher[] spanWatchers =
        (SpanWatcher[])
            getSpans(
                Math.min(oldStart, start),
                Math.min(Math.max(oldEnd, end), length()),
                SpanWatcher.class);
  ...
  public float getTextRunAdvances(
    ...
    if (end <= i) {
      float ret =
          p.getTextRunAdvances(
              this.mText, start, len, contextStart, contextLen, isRtl, advances, advancesPos);
      return ret;
    }
    if (start >= i) {
      char[] cArr = this.mText;
      ...
      float ret2 =
          p.getTextRunAdvances(
              cArr, start + i2, len, contextStart + i2, contextLen, isRtl, advances, advancesPos);
      return ret2;
    }
    char[] buf = TextUtils.obtain(contextLen);
    getChars(contextStart, contextEnd, buf, 0);
    float ret3 =
        p.getTextRunAdvances(
            buf, start - contextStart, len, 0, contextLen, isRtl, advances, advancesPos);
    TextUtils.recycle(buf);
    return ret3;
  }
  ...
  public int getTextRunCursor(
    ...
    if (contextStart >= ret) {
      ...
      return p.getTextRunCursor(cArr, contextStart + i, contextLen, isRtl, offset + i, cursorOpt)
          - this.mGapLength;
    }
    char[] buf = TextUtils.obtain(contextLen);
    getChars(contextStart, contextEnd, buf, 0);
    int ret2 =
        p.getTextRunCursor(buf, 0, contextLen, isRtl, offset - contextStart, cursorOpt)
            + contextStart;
    TextUtils.recycle(buf);
    return ret2;
  }
