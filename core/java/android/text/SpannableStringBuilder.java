...
public class SpannableStringBuilder implements CharSequence, GetChars, Spannable, Editable, Appendable, GraphicsOperations {
    ...
    */
    ...
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    private <T> int getSpansRec(int queryStart, int queryEnd, Class<T> kind, int i, T[] tArr, int[] priority, int[] insertionOrder, int count, boolean sort) {
        int count2;
        int spanStart;
        int spanEnd;
        int count3;
        ...
        if ((i & 1) != 0) {
            ...
            if (spanMax >= queryStart) {
                count2 = getSpansRec(queryStart, queryEnd, kind, left, tArr, priority, insertionOrder, count, sort);
                if (i < this.mSpanCount) {
                    return count2;
                }
                ...
                if (spanStart <= queryEnd) {
                    ...
                    return getSpansRec(queryStart, queryEnd, kind, rightChild(i), tArr, priority, insertionOrder, count3, sort);
                }
                return count2;
            }
        }
