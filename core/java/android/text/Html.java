...
public class Html {
    ...
    public static Spanned fromHtml(
            String source, int flags, ImageGetter imageGetter, TagHandler tagHandler) {
        Parser parser = new Parser();
        try {
            parser.setProperty(
                    "http://www.ccil.org/~cowan/tagsoup/properties/schema", HtmlParser.schema);
    ...
    private static void encodeTextAlignmentByDiv(StringBuilder out, Spanned text, int option) {
        ...
        while (i < len) {
            ...
            ParagraphStyle[] style =
                    (ParagraphStyle[]) text.getSpans(i, next, ParagraphStyle.class);
            String elements = " ";
    ...
    private static String getTextStyles(
            Spanned text,
            int start,
            int end,
        ...
        if (includeTextAlign) {
            AlignmentSpan[] alignmentSpans =
                    (AlignmentSpan[]) text.getSpans(start, end, AlignmentSpan.class);
    ...
    private static void withinBlockquote(
            StringBuilder out, Spanned text, int start, int end, int option) {
        if ((option & 1) == 0) {
    ...
    private static void withinBlockquoteIndividual(
    ...
    private static void withinBlockquoteConsecutive(
            StringBuilder out, Spanned text, int start, int end) {
    ...
    private static void withinParagraph(StringBuilder out, Spanned text, int start, int end) {
        ...
        while (i < end) {
            ...
            CharacterStyle[] style =
                    (CharacterStyle[]) text.getSpans(i, next, CharacterStyle.class);
            for (int j = 0; j < style.length; j++) {
                ...
                if ((style[j] instanceof TypefaceSpan)
                        && "monospace".equals(((TypefaceSpan) style[j]).getFamily())) {
                    out.append("<tt>");
                }
                ...
                if (style[j] instanceof AbsoluteSizeSpan) {
                    ...
                    out.append(
                            String.format(
                                    "<span style=\"font-size:%.0fpx\";>", Float.valueOf(sizeDip)));
                }
                if (style[j] instanceof RelativeSizeSpan) {
                    float sizeEm = ((RelativeSizeSpan) style[j]).getSizeChange();
                    out.append(
                            String.format(
                                    "<span style=\"font-size:%.2fem;\">", Float.valueOf(sizeEm)));
                }
                if (style[j] instanceof ForegroundColorSpan) {
                    ...
                    out.append(
                            String.format(
                                    ...
                                    Integer.valueOf(color & 16777215)));
                }
                if (style[j] instanceof BackgroundColorSpan) {
                    ...
                    out.append(
                            String.format(
                                    ...
                                    Integer.valueOf(16777215 & color2)));
                }
            }
            ...
            for (int j2 = style.length - 1; j2 >= 0; j2--) {
                ...
                if ((style[j2] instanceof TypefaceSpan)
                        && ((TypefaceSpan) style[j2]).getFamily().equals("monospace")) {
                    out.append("</tt>");
                }
        ...
        while (i < end) {
            ...
            if (c == '<') {
                ...
                if (c < 56320 && i + 1 < end && (d = text.charAt(i + 1)) >= 56320 && d <= 57343) {
                    ...
                    out.append("&#")
                            .append(codepoint)
                            .append(NavigationBarInflaterView.GRAVITY_SEPARATOR);
                }
            } else if (c > '~' || c < ' ') {
                out.append("&#")
                        .append((int) c)
