...
public class Html {
  ...
  private static String getTextStyles(
      Spanned text, int start, int end, boolean forceNoVerticalMargin, boolean includeTextAlign) {
    String margin = null;
    ...
    while (i < end) {
      ...
      for (int j = 0; j < style.length; j++) {
        ...
        if (style[j] instanceof ForegroundColorSpan) {
          ...
          out.append(
              String.format("<span style=\"color:#%06X;\">", Integer.valueOf(color & 16777215)));
        }
        if (style[j] instanceof BackgroundColorSpan) {
          int color2 = ((BackgroundColorSpan) style[j]).getBackgroundColor();
          out.append(
              String.format(
                  "<span style=\"background-color:#%06X;\">", Integer.valueOf(16777215 & color2)));
        }
      }
