...
public final class Formatter {
  ...
  private static String getByteSuffixOverride(Resources res) {
    return res.getString(C4337R.string.byteShort);
  }
  ...
  public static String formatShortElapsedTime(Context context, long millis) {
    ...
    if (minutes >= 2 || (minutes > 0 && seconds == 0)) {
      return measureFormat.format(
          new Measure(Integer.valueOf(minutes + ((seconds + 30) / 60)), MeasureUnit.MINUTE));
    }
    if (minutes > 0) {
      return measureFormat.formatMeasures(
          new Measure(Integer.valueOf(minutes), MeasureUnit.MINUTE),
