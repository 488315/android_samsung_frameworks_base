...
public class GestureOverlayView extends FrameLayout {
  ...
  public GestureOverlayView(Context context, AttributeSet attrs) {
    this(context, attrs, C4337R.attr.gestureOverlayViewStyle);
  }
  ...
  public GestureOverlayView(
    ...
    TypedArray a =
        context.obtainStyledAttributes(
            attrs, C4337R.styleable.GestureOverlayView, defStyleAttr, defStyleRes);
  ...
  public boolean dispatchTouchEvent(MotionEvent event) {
    ...
    if (isEnabled()) {
      boolean cancelDispatch =
          (this.mIsGesturing
                  || ((gesture = this.mCurrentGesture) != null
  ...
  private Rect touchMove(MotionEvent event) {
    ...
    float x = event.getX();
    float y = event.getY();
    float previousX = this.f65mX;
    float previousY = this.f66mY;
    ...
    areaToRefresh2.set(
        ((int) f) - border, ((int) f2) - border, ((int) f) + border, ((int) f2) + border);
      ...
      if (GestureOverlayView.this.mIsFadingOut) {
        ...
        if (duration > GestureOverlayView.this.mFadeDuration) {
          ...
          float interpolatedTime =
              Math.max(0.0f, Math.min(1.0f, duration / GestureOverlayView.this.mFadeDuration));
          GestureOverlayView gestureOverlayView = GestureOverlayView.this;
          gestureOverlayView.mFadingAlpha =
              1.0f - gestureOverlayView.mInterpolator.getInterpolation(interpolatedTime);
