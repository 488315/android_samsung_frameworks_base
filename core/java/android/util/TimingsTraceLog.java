...
public class TimingsTraceLog {
    ...
    public void traceBegin(String name) {
        ...
        if (i + 1 >= this.mMaxNestedCalls) {
            Slog.m121w(this.mTag, "not tracing duration of '" + name + "' because already reached " + this.mMaxNestedCalls + " levels");
            return;
        }
        int i2 = i + 1;
    ...
    public void logDuration(String name, long timeMs) {
        ...
        Slog.m113d(this.mTag, "!@Boot_SystemServer: " + timeMs + "ms : " + name.substring(0, Math.min(50, name.length())));
        Slog.m117i(this.mTag, "!@Boot_EBS:   Took " + timeMs + "ms by '" + name.substring(0, Math.min(50, name.length())) + "'");
    }
