...
public final class StrictJarFile {
  ...
  /* renamed from: fd */
  private final FileDescriptor f539fd;
  ...
  public void close() throws IOException {
    if (!this.closed) {
      ...
      nativeClose(this.nativeHandle);
      IoUtils.closeQuietly(this.f539fd);
      this.closed = true;
    }
  }
  ...
  private InputStream getZipInputStream(ZipEntry ze) {
    if (ze.getMethod() == 0) {
      return new FDStream(this.f539fd, ze.getDataOffset(), ze.getDataOffset() + ze.getSize());
    }
    FDStream wrapped =
        new FDStream(this.f539fd, ze.getDataOffset(), ze.getDataOffset() + ze.getCompressedSize());
      ...
      try {
        ...
        if (i != -1) {
          ...
          throw new IOException(
              "Size mismatch on inflated file: " + this.bytesRead + " vs " + this.entry.getSize());
        }
        return i;
      } catch (IOException e) {
        throw new IOException(
            "Error reading data for " + this.entry.getName() + " near offset " + this.bytesRead, e);
      }
    }
  ...
  public static class FDStream extends InputStream {
    ...
    /* renamed from: fd */
    private final FileDescriptor f540fd;
    private long offset;
    ...
    public int read(byte[] buffer, int byteOffset, int byteCount) throws IOException {
      synchronized (this.f540fd) {
        ...
        try {
          Os.lseek(this.f540fd, j2, OsConstants.SEEK_SET);
          int count = IoBridge.read(this.f540fd, buffer, byteOffset, byteCount);
          if (count <= 0) {
            return -1;
          }
          this.offset += count;
