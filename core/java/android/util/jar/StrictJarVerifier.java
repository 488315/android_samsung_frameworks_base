  ...
  static Certificate[] verifyBytes(byte[] blockBytes, byte[] sfBytes)
    ...
    try {
      try {
        ...
        return (Certificate[])
            verifiedSignerCertChain.toArray(new X509Certificate[verifiedSignerCertChain.size()]);
      } catch (IOException e) {
    ...
    try {
      ...
      try {
          ...
          if (v2SignatureGenerated) {
            throw new SecurityException(
                signatureFile
                    + " indicates "
                    + this.jarName
                    + " is signed using APK Signature Scheme v2, but no such signature was found."
                    + " Signature stripped?");
          }
          if (v3SignatureGenerated) {
            throw new SecurityException(
                signatureFile
                    + " indicates "
                    + this.jarName
                    + " is signed using APK Signature Scheme v3, but no such signature was found."
                    + " Signature stripped?");
          }
        }
        ...
        if (i <= 0 || createdBySigntool) {
          ...
          entries = entries2;
          if (!verify(
              attributes, "-Digest-Manifest-Main-Attributes", manifestBytes, 0, i, false, true)) {
            throw failedVerification(this.jarName, signatureFile);
          }
        }
        String digestAttribute = createdBySigntool ? "-Digest" : "-Digest-Manifest";
        if (!verify(
            attributes, digestAttribute, manifestBytes, 0, manifestBytes.length, false, false)) {
