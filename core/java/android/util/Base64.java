...
public class Base64 {
  ...
  public static byte[] decode(byte[] input, int offset, int len, int flags) {
    ...
    if (decoder.f529op == decoder.output.length) {
      return decoder.output;
    }
    byte[] temp = new byte[decoder.f529op];
    System.arraycopy(decoder.output, 0, temp, 0, decoder.f529op);
    return temp;
  }
    ...
    public boolean process(byte[] input, int offset, int len, boolean finish) {
      ...
      while (p < len2) {
        if (state == 0) {
          while (p + 4 <= len2) {
            int i =
                (alphabet[input[p] & 255] << 18)
                    | (alphabet[input[p + 1] & 255] << 12)
                    | (alphabet[input[p + 2] & 255] << 6)
