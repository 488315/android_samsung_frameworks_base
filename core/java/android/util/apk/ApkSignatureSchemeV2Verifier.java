...
public class ApkSignatureSchemeV2Verifier {
    ...
    try {
      ...
      try {
        ...
        return new VerifiedSigner(
            (X509Certificate[][]) signerCerts.toArray(new X509Certificate[signerCerts.size()][]),
            verityRootHash,
            contentDigests);
    ...
    while (attrs.hasRemaining()) {
      ...
      switch (id) {
        case STRIPPING_PROTECTION_ATTR_ID /* -1091571699 */:
          if (attr.remaining() < 4) {
            throw new IOException(
                "V2 Signature Scheme Stripping Protection Attribute  value too small.  Expected 4"
                    + " bytes, but found "
                    + attr.remaining());
          }
          ...
          if (vers == 3) {
            throw new SecurityException(
                "V2 signature indicates APK is signed using APK Signature Scheme v3, but none was"
  ...
  public static class VerifiedSigner {
    ...
    public VerifiedSigner(
        X509Certificate[][] certs, byte[] verityRootHash, Map<Integer, byte[]> contentDigests) {
      this.certs = certs;
