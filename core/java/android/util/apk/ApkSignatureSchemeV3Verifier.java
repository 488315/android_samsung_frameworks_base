...
public class ApkSignatureSchemeV3Verifier {
    ...
    private static VerifiedSigner verify(RandomAccessFile apk, boolean verifyIntegrity)
            throws SignatureNotFoundException, SecurityException, IOException {
        ApkSignatureSchemeV3Verifier verifier =
                new ApkSignatureSchemeV3Verifier(apk, verifyIntegrity);
        try {
    ...
    private VerifiedSigner verify(SignatureInfo signatureInfo, int blockId)
        ...
        try {
            ...
            try {
                ...
                while (signers.hasRemaining()) {
                    try {
                        ...
                        throw new SecurityException(
                                "Failed to parse/verify signer #" + signerCount + " block", e2);
                    }
                }
                ...
                if (signerCount != 1) {
                    throw new SecurityException(
                            "APK Signature Scheme V3 only supports one signer: multiple signers"
                                + " found.");
                }
                ...
                if (!contentDigests.containsKey(3)) {
                    ...
                    byte[] verityRootHash2 =
                            ApkSigningBlockUtils.parseVerityDigestAndVerifySourceLength(
                                    verityDigest, this.mApk.getChannel().size(), signatureInfo);
                    verityRootHash = verityRootHash2;
                }
                return new VerifiedSigner(
                        result.first, result.second, verityRootHash, contentDigests, blockId);
    ...
    private Pair<X509Certificate[], ApkSigningBlockUtils.VerifiedProofOfRotation> verifySigner(
        ...
        if (Build.VERSION.SDK_INT >= minSdkVersion && Build.VERSION.SDK_INT <= maxSdkVersion) {
            ...
            try {
                try {
                    ...
                    if (!sigVerified) {
                        throw new SecurityException(
                                jcaSignatureAlgorithm + " signature did not verify");
                    }
                    signedData.clear();
                    ...
                    while (digests.hasRemaining()) {
                        ...
                        try {
                            ByteBuffer digest =
                                    ApkSigningBlockUtils.getLengthPrefixedSlice(digests);
                            byte[] bestSigAlgorithmSignatureBytes2 = bestSigAlgorithmSignatureBytes;
                            try {
                                AlgorithmParameterSpec jcaSignatureAlgorithmParams2 =
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        bestSigAlgorithmSignatureBytes =
                                                bestSigAlgorithmSignatureBytes2;
                        ...
                        throw new IOException(
                                "Failed to parse digest record #" + signatureCount3, e);
                    }
                    if (!signaturesSigAlgorithms.equals(digestsSigAlgorithms)) {
                        throw new SecurityException(
                                "Signature algorithms don't match between digests and signatures"
                                    + " records");
                    }
                    ...
                    byte[] previousSignerDigest =
                            contentDigests.put(Integer.valueOf(certificateCount), contentDigest);
                    ...
                    for (ByteBuffer certificates =
                        ...
                        try {
                            ...
                            throw new SecurityException(
                                    "Failed to decode certificate #" + digestAlgorithm2, e7);
                        }
                    }
                    if (certs.isEmpty()) {
                    ...
                    if (signedMinSDK != minSdkVersion) {
                        throw new SecurityException(
                                "minSdkVersion mismatch between signed and unsigned in v3 signer"
                                    + " block.");
                    }
                    ...
                    if (signedMaxSDK != maxSdkVersion) {
                        throw new SecurityException(
                                "maxSdkVersion mismatch between signed and unsigned in v3 signer"
                                    + " block.");
                    }
                    ByteBuffer additionalAttrs =
                            ApkSigningBlockUtils.getLengthPrefixedSlice(signedData);
                    ...
                    | NoSuchAlgorithmException
                    | SignatureException
                    | InvalidKeySpecException e9) {
                e = e9;
            }
        }
        if (this.mBlockId == APK_SIGNATURE_SCHEME_V31_BLOCK_ID
                && (!this.mOptionalRotationMinSdkVersion.isPresent()
                        || this.mOptionalRotationMinSdkVersion.getAsInt() > minSdkVersion)) {
            this.mOptionalRotationMinSdkVersion = OptionalInt.of(minSdkVersion);
        }
        throw new PlatformNotSupportedException(
                "Signer not supported by this platform version. This platform: "
                        + Build.VERSION.SDK_INT
                        + ", signer minSdkVersion: "
                        + minSdkVersion
                        + ", maxSdkVersion: "
                        + maxSdkVersion);
    }
        ...
        while (attrs.hasRemaining()) {
            ...
            switch (id) {
                case ROTATION_ON_DEV_RELEASE_ATTR_ID /* -1029262406 */:
                    if (this.mBlockId == APK_SIGNATURE_SCHEME_V31_BLOCK_ID
                            && Build.VERSION.SDK_INT == this.mSignerMinSdkVersion
                            && "REL".equals(Build.VERSION.CODENAME)) {
                        this.mOptionalRotationMinSdkVersion =
                                OptionalInt.of(this.mSignerMinSdkVersion);
                        throw new PlatformNotSupportedException(
                                "The device is running a release version of "
                                        + this.mSignerMinSdkVersion
                ...
                case PROOF_OF_ROTATION_ATTR_ID /* 1000370060 */:
                    if (por != null) {
                        throw new SecurityException(
                                "Encountered multiple Proof-of-rotation records when verifying APK"
                                    + " Signature Scheme v3 signature");
                    }
                    por = ApkSigningBlockUtils.verifyProofOfRotationStruct(attr, certFactory);
                    try {
                        if (por.certs.size() > 0
                                && !Arrays.equals(
                                        por.certs.get(por.certs.size() - 1).getEncoded(),
                                        certChain[0].getEncoded())) {
                            throw new SecurityException(
                                    "Terminal certificate in Proof-of-rotation record does not"
                                        + " match APK signing certificate");
                        }
                    } catch (CertificateEncodingException e) {
                        throw new SecurityException(
                                "Failed to encode certificate when comparing Proof-of-rotation"
                                    + " record and signing certificate",
                                e);
                    }
                    break;
