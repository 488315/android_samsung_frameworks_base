...
public abstract class SourceStampVerifier {
    ...
    for (Map.Entry<Integer, byte[]> signatureSchemeDigest : signatureSchemeDigests.entrySet()) {
      if (!signedSignatureSchemeData.containsKey(signatureSchemeDigest.getKey())) {
        throw new SecurityException(
            String.format(
                "No signatures found for signature scheme %d", signatureSchemeDigest.getKey()));
      }
  ...
  private static X509Certificate verifySourceStampCertificate(
      ByteBuffer sourceStampBlockData, byte[] sourceStampCertificateDigest) throws IOException {
    try {
  ...
  private static Map<Integer, byte[]> getApkContentDigestsFromSignatureBlock(
    ...
    while (signers.hasRemaining()) {
      ...
      while (digests.hasRemaining()) {
        ...
        int digestAlgorithm =
            ApkSigningBlockUtils.getSignatureAlgorithmContentDigestAlgorithm(sigAlgorithm);
  ...
  private static ApkSigningBlockUtils.VerifiedProofOfRotation verifySourceStampAttributes(
      ByteBuffer stampAttributeData, X509Certificate sourceStampCertificate) throws IOException {
    try {
      CertificateFactory certFactory = CertificateFactory.getInstance("X.509");
      ...
      while (stampAttributes.hasRemaining()) {
        ...
        if (id == PROOF_OF_ROTATION_ATTR_ID) {
          if (verifiedProofOfRotation != null) {
            throw new SecurityException(
                "Encountered multiple Proof-of-rotation records when verifying source stamp"
                    + " signature");
          }
          verifiedProofOfRotation =
              ApkSigningBlockUtils.verifyProofOfRotationStruct(attribute, certFactory);
          try {
            if (verifiedProofOfRotation.certs.size() > 0
              ...
              throw new SecurityException(
                  "Terminal certificate in Proof-of-rotation record does not match source stamp"
                      + " certificate");
            }
          } catch (CertificateEncodingException e) {
            throw new SecurityException(
                "Failed to encode certificate when comparing Proof-of-rotation record and source"
                    + " stamp certificate",
                e);
          }
        }
  ...
  private static void closeApkJar(StrictJarFile apkJar) {
    ...
    try {
      ...
      Slog.m116e(TAG, "Could not close APK jar", e);
    }
  }
}
