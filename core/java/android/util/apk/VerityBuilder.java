...
public abstract class VerityBuilder {
  ...
  private static void generateApkVerityDigestAtLeafLevel(
    ...
    BufferedDigester digester = new BufferedDigester(salt, output);
    consumeByChunk(
        digester, DataSource.create(apk.getFD(), 0L, signatureInfo.apkSigningBlockOffset), 1048576);
  ...
  private static byte[] generateFsVerityTreeInternal(
    ...
    generateFsVerityDigestAtLeafLevel(
        apk,
        salt,
        slice(output, levelOffset[levelOffset.length - 2], levelOffset[levelOffset.length - 1]));
  ...
  private static int[] calculateVerityLevelOffset(long fileSize) {
    ...
    for (int i = 0; i < levelSize.size(); i++) {
      levelOffset[i + 1] =
          levelOffset[i] + Math.toIntExact(levelSize.get((levelSize.size() - i) - 1).longValue());
    }
    return levelOffset;
  }
