...
public class TimeUtils {
  ...
  private static android.icu.util.TimeZone getIcuTimeZone(
    ...
    if (countryTimeZones == null
        || (offsetResult =
                countryTimeZones.lookupByOffsetWithBias(whenMillis, bias, offsetMillis, isDst))
            == null) {
      return null;
    }
  ...
  private static int formatDurationLocked(long duration, int fieldLen) {
    ...
    if (fieldLen != 0) {
      ...
      for (int myLen4 =
              r5
                  + accumField(
                      millis, 2, true, myLen3 + accumField(seconds, 1, myLen3 > 0, 2) > 0 ? 3 : 0)
                  + 1;
          myLen4 < fieldLen;
          myLen4++) {
        formatStr[pos2] = ' ';
    ...
    int pos6 =
        printFieldLocked(formatStr, hours, DateFormat.HOUR, pos5, pos5 != pos4, zeropad ? 2 : 0);
    int pos7 =
        printFieldLocked(
            formatStr, minutes, DateFormat.MINUTE, pos6, pos6 != pos4, zeropad ? 2 : 0);
