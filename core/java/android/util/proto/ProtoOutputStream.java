...
public final class ProtoOutputStream extends ProtoStream {
  ...
  public static int checkFieldId(long fieldId, long expectedFlags) {
    ...
    if (((int) fieldId) == 0) {
      throw new IllegalArgumentException(
          "Invalid proto field " + ((int) fieldId) + " fieldId=" + Long.toHexString(fieldId));
    }
    if (fieldType != expectedType
        || (fieldCount != expectedCount
            && (fieldCount != ProtoStream.FIELD_COUNT_PACKED || expectedCount != 2199023255552L))) {
            ...
            if (childRawSize >= 0) {
              if (childEncodedSize != childRawSize) {
                throw new RuntimeException(
                    "Pre-computed size where the precomputed size and the raw size in the buffer"
                        + " don't match! childRawSize="
                        + childRawSize
                        + " childEncodedSize="
                        + childEncodedSize
                        + " childEncodedSizePos="
    ...
    while (true) {
      ...
      if (tagPos < objectEnd) {
        ...
        switch (wireType) {
          ...
          case 2:
            EncodedBuffer encodedBuffer = this.mBuffer;
            encodedBuffer.writeFromThisBuffer(
                this.mCopyBegin, encodedBuffer.getReadPos() - this.mCopyBegin);
            int childRawSize = this.mBuffer.readRawFixed32();
            int childEncodedSize = this.mBuffer.readRawFixed32();
  ...
  public void dump(String tag) {
    Log.m94d(tag, this.mBuffer.getDebugString());
