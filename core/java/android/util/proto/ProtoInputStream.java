...
public final class ProtoInputStream extends ProtoStream {
  ...
  public int nextField() throws IOException {
    ...
    if (this.mDepth >= 0
        && getOffset() == getOffsetFromToken(this.mExpectedObjectTokenStack.get(this.mDepth))) {
      this.mFieldNumber = -1;
    } else {
      readTag();
    }
  ...
  public long start(long fieldId) throws IOException {
    ...
    int i3 = this.mDepth;
    if (i3 > 0
        && getOffsetFromToken(this.mExpectedObjectTokenStack.get(i3))
  ...
  private long readVarint() throws IOException {
    ...
    while (true) {
      ...
      if (fragment < 0) {
        throw new ProtoParseException(
            "Incomplete varint at offset 0x" + Integer.toHexString(getOffset()) + dumpDebugData());
      }
  ...
  private int readFixed32() throws IOException {
    ...
    while (bytesLeft > 0) {
      ...
      if (fragment < 0) {
        throw new ProtoParseException(
            "Incomplete fixed32 at offset 0x" + Integer.toHexString(getOffset()) + dumpDebugData());
      }
      incOffset(fragment);
      bytesLeft -= fragment;
      while (fragment > 0) {
  ...
  private long readFixed64() throws IOException {
    ...
    while (bytesLeft > 0) {
      ...
      if (fragment < 0) {
        throw new ProtoParseException(
            "Incomplete fixed64 at offset 0x" + Integer.toHexString(getOffset()) + dumpDebugData());
      }
      incOffset(fragment);
      bytesLeft -= fragment;
      while (fragment > 0) {
