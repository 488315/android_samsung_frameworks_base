...
public abstract class PrinterDiscoverySession {
  ...
  void setObserver(IPrintServiceClient observer) {
    ...
    if (!this.mPrinters.isEmpty()) {
      try {
        ...
        Log.m97e(LOG_TAG, "Error sending added printers", re);
      }
    }
  }
  ...
  public final void addPrinters(List<PrinterInfo> printers) {
    PrintService.throwIfNotCalledOnMainThread();
    if (this.mIsDestroyed) {
      Log.m102w(LOG_TAG, "Not adding printers - session destroyed.");
      return;
    }
  ...
  public final void removePrinters(List<PrinterId> printerIds) {
    PrintService.throwIfNotCalledOnMainThread();
    if (this.mIsDestroyed) {
      Log.m102w(LOG_TAG, "Not removing printers - session destroyed.");
      return;
    }
    if (this.mIsDiscoveryStarted) {
      ...
      if (!removedPrinterIds.isEmpty()) {
        try {
          ...
          Log.m97e(LOG_TAG, "Error sending removed printers", re);
          return;
        }
      }
      return;
  ...
  private void sendOutOfDiscoveryPeriodPrinterChanges() {
    ...
    if (removedPrinterIds != null) {
      try {
        ...
        Log.m97e(LOG_TAG, "Error sending removed printers", re2);
      }
    }
    this.mLastSentPrinters = null;
  }
  ...
  void requestCustomPrinterIcon(PrinterId printerId) {
    ...
    if (!this.mIsDestroyed && (iPrintServiceClient = this.mObserver) != null) {
      CustomPrinterIconCallback callback =
          new CustomPrinterIconCallback(printerId, iPrintServiceClient);
