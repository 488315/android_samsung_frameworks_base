    ...
    while (true) {
        ...
        try {
          cursor2 =
              contentProviderClient.query(
                  MtpDatabase.getObjectPropertiesUri(mtpObject.getFormat(), str),
                  this.mColumns,
                  PATH_WHERE,
                  new String[] {path},
                  null,
      ...
      switch (property.code) {
        ...
        case MtpConstants.PROPERTY_DATE_ADDED /* 56398 */:
          ...
          break;
        case MtpConstants.PROPERTY_PARENT_OBJECT /* 56331 */:
          mtpPropertyList.append(
              id,
              property.code,
              property.type,
              mtpObject.getParent().isRoot() ? 0L : mtpObject.getParent().getId());
          break;
        case MtpConstants.PROPERTY_HIDDEN /* 56333 */:
          mtpPropertyList.append(
              id,
              property.code,
              4,
              mtpObject.getName().startsWith(MediaMetrics.SEPARATOR) ? 1L : 0L);
          break;
        case MtpConstants.PROPERTY_PERSISTENT_UID /* 56385 */:
          mtpPropertyList.append(
              id,
              property.code,
              property.type,
              (mtpObject.getPath().toString().hashCode() << 32) + mtpObject.getModifiedTime());
          break;
        case MtpConstants.PROPERTY_TRACK /* 56459 */:
          if (cursor == null) {
            i = 0;
