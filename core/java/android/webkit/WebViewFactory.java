      ...
      try {
        try {
          ...
          Method staticFactory =
              providerClass.getMethod(CHROMIUM_WEBVIEW_FACTORY_METHOD, WebViewDelegate.class);
          Trace.traceBegin(16L, "WebViewFactoryProvider invocation");
          try {
            sProviderInstance =
                (WebViewFactoryProvider) staticFactory.invoke(null, new WebViewDelegate());
            if (!isSetDataDirectorySuffix) {
              isSetDataDirectorySuffix = false;
              String needRecord =
                  SystemProperties.get("persist.sys.app_webview_preload_need", "false");
              if (needRecord.startsWith("launching")) {
                ...
                SecIpmManager mSecIpmManager =
                    (SecIpmManager)
                        AppGlobals.getInitialApplication().getSystemService("PkgPredictorService");
      ...
      try {
        ...
        try {
          try {
            ...
            try {
              Class<WebViewFactoryProvider> webViewProviderClass =
                  getWebViewProviderClass(clazzLoader);
