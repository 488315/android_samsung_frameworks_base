...
public class WebView extends AbsoluteLayout
  ...
  private void checkThread() {
    if (this.mWebViewThread != null && Looper.myLooper() != this.mWebViewThread) {
      Throwable throwable =
          new Throwable(
              "A WebView method was called on thread '"
                  ...
                  + "'. All WebView methods must be called on the same thread. (Expected Looper "
                  + this.mWebViewThread
                  + " called on "
                  + Looper.myLooper()
                  + ", FYI main Looper is "
      ...
      Log.m102w(LOGTAG, Log.getStackTraceString(throwable));
  ...
  public void onCreateVirtualViewTranslationRequests(
    ...
    this.mProvider
        .getViewDelegate()
        .onCreateVirtualViewTranslationRequests(virtualIds, supportedFormats, requestsCollector);
  }
  ...
  public void dispatchCreateViewTranslationRequest(
    ...
    this.mProvider
        .getViewDelegate()
        .dispatchCreateViewTranslationRequest(viewIds, supportedFormats, capability, requests);
  }
