    ...
    private static final class SpellCheckerSessionListenerImpl extends ISpellCheckerSessionListener.Stub {
                ...
                switch (scp.mWhat) {
                    case 1:
                        try {
                            ...
                            Log.m96e(SpellCheckerSession.TAG, "Failed to cancel " + e);
                            break;
                        }
                    case 2:
                        try {
                            ...
                            Log.m96e(SpellCheckerSession.TAG, "Failed to get suggestions " + e2);
                            break;
                        }
                    case 3:
                        try {
                            ...
                            Log.m96e(SpellCheckerSession.TAG, "Failed to close " + e3);
                            break;
                        }
                    case 4:
                        try {
                            ...
                            Log.m96e(SpellCheckerSession.TAG, "Failed to get suggestions " + e4);
                            break;
                        }
                }
            } else {
        ...
        private void processCloseLocked() {
            ...
            switch (this.mState) {
                ...
                case 1:
                    ...
                    break;
                default:
                    Log.m96e(SpellCheckerSession.TAG, "processCloseLocked is called unexpectedly. mState=" + stateToString(this.mState));
                    break;
            }
        }
        ...
        public void onServiceConnected(ISpellCheckerSession session) {
            synchronized (this) {
                switch (this.mState) {
                    case 0:
                        if (session == null) {
                            Log.m96e(SpellCheckerSession.TAG, "ignoring onServiceConnected due to session=null");
                            return;
                        }
                    ...
                    case 3:
                        return;
                    default:
                        Log.m96e(SpellCheckerSession.TAG, "ignoring onServiceConnected due to unexpected mState=" + stateToString(this.mState));
                        return;
                }
            }
        }
        ...
        private void processOrEnqueueTask(SpellCheckerParams scp) {
            ...
            synchronized (this) {
                ...
                int i2 = this.mState;
                if (i2 != 0 && i2 != 1) {
                    Log.m96e(SpellCheckerSession.TAG, "ignoring processOrEnqueueTask due to unexpected mState=" + stateToString(this.mState) + " scp.mWhat=" + taskToString(scp.mWhat));
                    return;
                }
                if (i2 == 0) {
                    if (scp.mWhat == 3) {
