...
public class InsetsAnimationControlImpl
  ...
  public InsetsAnimationControlImpl(
    ...
    WindowInsetsAnimation windowInsetsAnimation =
        new WindowInsetsAnimation(types, interpolator, durationMs);
  ...
  private boolean calculatePerceptible(Insets currentInsets, float currentAlpha) {
    return currentInsets.left * 100 >= (this.mShownInsets.left - this.mHiddenInsets.left) * 5
        ...
        && currentInsets.right * 100 >= (this.mShownInsets.right - this.mHiddenInsets.right) * 5
        && currentInsets.bottom * 100 >= (this.mShownInsets.bottom - this.mHiddenInsets.bottom) * 5
        && currentAlpha >= 0.5f;
  }
  ...
  public void updateSurfacePosition(SparseArray<InsetsSourceControl> controls) {
    for (int i = controls.size() - 1; i >= 0; i--) {
      ...
      if (c != null) {
        ...
        c.setSurfacePosition(position.f85x, position.f86y);
      }
    }
  }
  ...
  public boolean applyChangeInsets(InsetsState outState) {
    ...
    updateLeashesForSide(
        0, offset.left, this.mPendingInsets.left, params, outState, this.mPendingAlpha);
    updateLeashesForSide(
        1, offset.top, this.mPendingInsets.top, params, outState, this.mPendingAlpha);
    updateLeashesForSide(
        2, offset.right, this.mPendingInsets.right, params, outState, this.mPendingAlpha);
    updateLeashesForSide(
        3, offset.bottom, this.mPendingInsets.bottom, params, outState, this.mPendingAlpha);
    this.mController.applySurfaceParams(
        (SyncRtSurfaceTransactionApplier.SurfaceParams[])
            params.toArray(new SyncRtSurfaceTransactionApplier.SurfaceParams[params.size()]));
  ...
  private Insets getInsetsFromState(InsetsState state, Rect frame, SparseIntArray idSideMap) {
    return state
        .calculateInsets(frame, null, false, 16, 0, 0, 2, 0, idSideMap)
  ...
  private void updateLeashesForSide(
    ...
    for (int i = controls.size() - 1; i >= 0; i--) {
      ...
      this.mTmpMatrix.setTranslate(
          control.getSurfacePosition().f85x, control.getSurfacePosition().f86y);
      if (source != null) {
  ...
  private void addTranslationToMatrix(int side, int offset, Matrix m, Rect frame) {
    ...
    float surfaceOffset =
        translator != null ? translator.translateLengthInAppWindowToScreen(offset) : offset;
    switch (side) {
      case 0:
  ...
  private static void buildSideControlsMap(
    ...
    for (int i = idSideMap.size() - 1; i >= 0; i--) {
      ...
      if (control != null) {
        if (side == 4) {
          Log.m98i(TAG, "ISIDE_FLOATING detected: " + control);
        }
