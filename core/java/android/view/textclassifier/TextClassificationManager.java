...
public final class TextClassificationManager {
    ...
    private TextClassifier getSystemTextClassifier(int type) {
        synchronized (this.mLock) {
            if (getSettings().isSystemTextClassifierEnabled()) {
                try {
                    Log.m166d("androidtc", "Initializing SystemTextClassifier, type = " + TextClassifier.typeToString(type));
    ...
    private TextClassifier getLocalTextClassifier() {
        Log.m166d("androidtc", "Local text-classifier not supported. Returning a no-op text-classifier.");
        return TextClassifier.NO_OP;
    }
