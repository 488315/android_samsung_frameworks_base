...
public class TwoFingerSwipeGestureDetector implements WindowManagerPolicyConstants.PointerEventListener {
    ...
    /* renamed from: UP */
    public static final int f556UP = 2;
    ...
    public void setGestureExclusionRegion(Region region) {
        if (this.DEBUG) {
            Slog.m113d(this.TAG, "setGestureExclusionRegion. " + region);
        }
    ...
    public void onInputEvent(InputEvent event) {
        if (event instanceof MotionEvent) {
            ...
            if (this.DEBUG && this.DEBUG_NOISE) {
                Slog.m113d(this.TAG, actionToString(action) + " pointerCount=" + pointerCount);
            }
            if (action == 0) {
                detecting();
            ...
            if (!isValidPointerCount(pointerCount)) {
                if (this.DEBUG) {
                    Slog.m113d(this.TAG, "invalid pointer count=" + me.getPointerCount());
                }
                cancel();
                return;
            }
            ...
            if (5 == action) {
                if (this.mState == 1) {
                    if (isOverTouchTime(this.mPivotTime, me.getEventTime())) {
                        if (this.DEBUG) {
                            Slog.m113d(this.TAG, String.format("prev=%s cur=%s diff=%s timeThreshold=%f", Long.valueOf(this.mPivotTime), Long.valueOf(me.getEventTime()), Long.valueOf(me.getEventTime() - this.mPivotTime), Float.valueOf(this.mTimeThreshold)));
                        }
                        cancel();
                        return;
                    }
                    ...
                    if (pivotIdx < 0) {
                        if (this.DEBUG) {
                            Slog.m113d(this.TAG, "missing first touch.");
                        }
                        cancel();
                        return;
                    }
                    if (allMatch(me, new BiFunction() { // from class: android.view.TwoFingerSwipeGestureDetector$$ExternalSyntheticLambda1
                        ...
                        if (this.DEBUG) {
                            Slog.m113d(this.TAG, String.format("ACTION_POINTER_DOWN. any pointer doesn't in thresholds. %s %s %s", getXYString(Float.valueOf(me.getX(0)), Float.valueOf(me.getY(0))), getXYString(Float.valueOf(me.getX(1)), Float.valueOf(me.getY(1))), Arrays.toString(this.mThresholdsForOneFinger)));
                        }
                        cancel();
                        return;
                    }
                    ...
                    if (allMatch(me, new BiFunction() { // from class: android.view.TwoFingerSwipeGestureDetector$$ExternalSyntheticLambda4
                        ...
                        if (this.DEBUG) {
                            Slog.m113d(this.TAG, "started on gesture exclude region.");
                        }
                        cancel();
                        return;
                    }
                    ...
                    if (position == 0) {
                        if (this.DEBUG) {
                            Slog.m113d(this.TAG, "position invalid. " + getXYString(Float.valueOf(this.mStartCenterX), Float.valueOf(this.mStartCenterY)));
                        }
                        cancel();
                        return;
                    }
                    ...
                    detected();
                    if (this.DEBUG) {
                        Slog.m113d(this.TAG, String.format("detected reason. events=%s, display=%s side=%b down=%b threshold=%s", me, this.mDisplayBounds, Boolean.valueOf(this.mSideEnabled), Boolean.valueOf(this.mDownEnabled), Arrays.toString(this.mThresholds)));
                        return;
                    }
                    return;
                }
            ...
            if (6 == action) {
                if (this.mState == 2) {
                    ...
                    if (!this.mUseThreeFinger && isTwoFingerDistanceFartherThanBefore(this.mInitialDistance, distance)) {
                        if (this.DEBUG) {
                            Slog.m113d(this.TAG, String.format("ACTION_UP. Distance between finger is farther than before. distance ratio=%f touch slop ratio=%f ", Float.valueOf(distance / this.mInitialDistance), Float.valueOf((distance - this.mInitialDistance) / this.mTouchSlopSquare)));
                        }
                        cancel();
                        return;
                    }
            ...
            if (1 == action) {
                if (this.mState == 2) {
                    if (isOverTouchTime(this.mPivotTime, me.getEventTime())) {
                        if (this.DEBUG) {
                            Slog.m113d(this.TAG, String.format("ACTION_UP. prev=%s cur=%s timeThreshold=%f", Long.valueOf(this.mPivotTime), Long.valueOf(me.getEventTime()), Float.valueOf(this.mTimeThreshold)));
                        }
                        cancel();
                        return;
                    }
                    ...
                    if (!isOverThreshold(curCenterX2, curCenterY2, this.mThresholds, this.mStartPosition).booleanValue()) {
                        if (this.DEBUG) {
                            Slog.m113d(this.TAG, "ACTION_UP. didn't over threshold. sp=" + this.mStartPosition + " cur=" + getXYString(Float.valueOf(curCenterX2), Float.valueOf(curCenterY2)) + " thresholds=" + Arrays.toString(this.mThresholds));
                        }
                        cancel();
                        return;
                    } else {
    ...
    protected boolean commitIfPossible(float curCenterX, float curCenterY) {
        ...
        if (gestureFrom == -1) {
            if (this.DEBUG) {
                Slog.m113d(this.TAG, "ActionMOVE: gestureFrom not found.");
                return false;
            }
            return false;
        }
        ...
        for (int i = 0; i < pointerCount; i++) {
            if (i != upIndex) {
                ...
                if (dot < 0.0f) {
                    ...
                    if (this.DEBUG) {
                        Slog.m113d(this.TAG, "dot product is negative. id1=(" + x1 + "," + y1 + ") id2=(" + x2 + "," + y2 + NavigationBarInflaterView.KEY_CODE_END);
                        return false;
                    }
                    return false;
                }
    ...
    private void committed(final int gestureFrom) {
        if (this.DEBUG) {
            Slog.m113d(this.TAG, "committed " + Debug.getCallers(2));
        }
        this.mState = 3;
    ...
    public void cancel() {
        ...
        if (this.DEBUG) {
            Slog.m113d(this.TAG, "canceled from " + Debug.getCaller());
        }
        this.mState = 4;
    ...
    public int getPosition(int x, int y, int[] thresholds) {
        int result = 0;
        if (this.mDisplayBounds.isEmpty()) {
            Slog.m115e(this.TAG, "display bounds is empty.");
            return 0;
        }
    ...
    protected void build() {
        if (this.DEBUG) {
            Slog.m113d(this.TAG, "updateDipResources. density=" + this.mDensity);
        }
    ...
    public void setDisplayBounds(Rect displayBounds) {
        if (this.DEBUG) {
            Slog.m113d(this.TAG, "setDisplayBounds. displayBounds=" + displayBounds);
        }
