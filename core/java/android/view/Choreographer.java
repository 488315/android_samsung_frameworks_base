                            ...
                            try {
                                if (frameIntervalNanos == 0) {
                                    ...
                                    try {
                                        ...
                                        if (skippedFrames >= j5) {
                                            try {
                                                Log.m98i(TAG, "Skipped " + skippedFrames + " frames!  The application may be doing too much work on its main thread.");
                                            } catch (Throwable th2) {
                                                th = th2;
                                                obj = obj2;
                                                j = 8;
                                ...
                                try {
                                    ...
                                    timeline = this.mFrameData.update(frameTimeNanos3, this.mDisplayEventReceiver, jitterNanos);
                                } catch (Throwable th4) {
                                    th = th4;
                                    obj = obj2;
                                    j = 8;
                                                    ...
                                                    if (resynced) {
                                                        try {
                                                            if (Trace.isTagEnabled(j2)) {
                                                                String message = String.format("Choreographer#doFrame - resynced to %d in %.1fms", Long.valueOf(timeline.mVsyncId), Float.valueOf((timeline.mDeadlineNanos - startNanos) * 1.0E-6f));
                                                                j = j2;
                                                                try {
                                                                    ...
                                                                    if (!this.mIsFg && this.mIsFirstBBA) {
                                                                        ...
                                                                        Log.m94d(TAG, "CoreRune.SYSPERF_ACTIVE_APP_BBA_ENABLE : stop animation in background states");
                                                                    }
        ...
        public void onVsync(long timestampNanos, long physicalDisplayId, int frame, DisplayEventReceiver.VsyncEventData vsyncEventData) {
            try {
                ...
                if (timestampNanos > now) {
                    Log.m102w(Choreographer.TAG, "Frame time is " + ((timestampNanos - now) * 1.0E-6f) + " ms in the future!  Check that graphics HAL is generating vsync timestamps using the correct timebase.");
                    timestampNanos = now;
                }
                if (this.mHavePendingVsync) {
                    Log.m102w(Choreographer.TAG, "Already have a pending vsync event.  There should only be one at a time.");
                } else {
