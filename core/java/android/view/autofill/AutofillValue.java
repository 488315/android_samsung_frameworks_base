...
public final class AutofillValue implements Parcelable {
  public static final Parcelable.Creator<AutofillValue> CREATOR =
      new Parcelable.Creator<AutofillValue>() { // from class: android.view.autofill.AutofillValue.1
  ...
  public boolean getToggleValue() {
    Preconditions.checkState(
        isToggle(), "value must be a toggle value, not type=%d", Integer.valueOf(this.mType));
  ...
  public static AutofillValue forText(CharSequence value) {
    if (Helper.sVerbose && !Looper.getMainLooper().isCurrentThread()) {
      Log.m100v(TAG, "forText() not called on main thread: " + Thread.currentThread());
    }
    if (value == null) {
      return null;
    }
