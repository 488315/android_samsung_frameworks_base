...
public final class AutofillClientController implements AutofillManager.AutofillClient, Dumpable {
  ...
  private AutofillManager getAutofillManager() {
    if (this.mAutofillManager == null) {
      this.mAutofillManager =
          (AutofillManager) this.mActivity.getSystemService(AutofillManager.class);
    }
  ...
  public void onActivityResumed() {
    ...
    if (this.mAutoFillResetNeeded
        && !this.mAutoFillIgnoreFirstResumePause
        && (focus = this.mActivity.getCurrentFocus()) != null
  ...
  public void onActivityPerformResume(boolean followedByPause) {
    if (this.mAutoFillResetNeeded) {
      ...
      if (followedByPause && DEBUG) {
        Slog.m119v(TAG, "autofill will ignore first pause when relaunching " + this);
      }
    }
  }
  ...
  public void onActivityPaused() {
    if (this.mAutoFillResetNeeded) {
      if (!this.mAutoFillIgnoreFirstResumePause) {
        if (DEBUG) {
          Log.m100v(TAG, "autofill notifyViewExited " + this);
        }
      ...
      if (DEBUG) {
        Log.m100v(TAG, "autofill got first pause " + this);
      }
  ...
  public void onDispatchActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    getAutofillManager()
        .onAuthenticationResult(requestCode, resultData, this.mActivity.getCurrentFocus());
  }
  ...
  public void onStartActivity(Intent startIntent, Intent cachedIntent) {
    if (cachedIntent != null
        ...
        && TextUtils.equals(
            ...
            startIntent.resolveActivity(this.mActivity.getPackageManager()).getPackageName())) {
  ...
  public void enableAutofillCompatibilityIfNeeded() {
    AutofillManager afm;
    if (this.mActivity.isAutofillCompatibilityEnabled()
        && (afm = (AutofillManager) this.mActivity.getSystemService(AutofillManager.class))
            != null) {
  ...
  public boolean[] autofillClientGetViewVisibility(AutofillId[] autofillIds) {
    ...
    if (Helper.sVerbose) {
      Log.m100v(TAG, "autofillClientGetViewVisibility(): " + Arrays.toString(visible));
    }
    return visible;
  }
  ...
  public View autofillClientFindViewByAccessibilityIdTraversal(int viewId, int windowId) {
    ...
    for (int rootNum = 0; rootNum < roots.size(); rootNum++) {
      ...
      if (rootView != null
          && rootView.getAccessibilityWindowId() == windowId
          && (view = rootView.findViewByAccessibilityIdTraversal(viewId)) != null) {
        return view;
      }
    }
    return null;
  ...
  public View autofillClientFindViewByAutofillIdTraversal(AutofillId autofillId) {
    ...
    for (int rootNum = 0; rootNum < roots.size(); rootNum++) {
      View rootView = roots.get(rootNum).getView();
      if (rootView != null
          && (view = rootView.findViewByAutofillIdTraversal(autofillId.getViewId())) != null) {
        return view;
      }
    }
    return null;
      ...
      if (rootView != null) {
        ...
        for (int viewNum = 0; viewNum < viewCount; viewNum++) {
          if (autofillIds[viewNum] != null && views[viewNum] == null) {
            views[viewNum] =
                rootView.findViewByAutofillIdTraversal(autofillIds[viewNum].getViewId());
          }
        }
      }
    }
  ...
  @Override // android.view.autofill.AutofillManager.AutofillClient
  public boolean autofillClientRequestShowFillUi(
      View anchor, int width, int height, Rect anchorBounds, IAutofillWindowPresenter presenter) {
    boolean wasShowing;
    AutofillPopupWindow autofillPopupWindow = this.mAutofillPopupWindow;
    if (autofillPopupWindow == null) {
      wasShowing = false;
  ...
  @Override // android.view.autofill.AutofillManager.AutofillClient
  public void autofillClientAuthenticate(
      int authenticationId, IntentSender intent, Intent fillInIntent, boolean authenticateInline) {
    try {
      ActivityOptions activityOptions =
          ActivityOptions.makeBasic().setPendingIntentBackgroundActivityStartMode(1);
      this.mActivity.startIntentSenderForResult(
          intent,
          AUTO_FILL_AUTH_WHO_PREFIX,
          authenticationId,
          fillInIntent,
          0,
          0,
      ...
      Log.m97e(TAG, "authenticate() failed for intent:" + intent, e);
    }
  }
}
