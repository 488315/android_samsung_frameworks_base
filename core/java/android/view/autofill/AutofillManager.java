...
public final class AutofillManager {
  ...
  public interface AutofillClient {
    ...
    boolean autofillClientRequestShowFillUi(
        View view, int i, int i2, Rect rect, IAutofillWindowPresenter iAutofillWindowPresenter);
  ...
  private boolean isPassingImeActionCheck(EditText editText) {
    ...
    if (this.mNonAutofillableImeActionIdSet.contains(String.valueOf(actionId))) {
      Log.m94d("AutofillManager", "view not autofillable - not passing ime action check");
      return false;
    }
    return true;
  }
  ...
  private boolean isPassingMultilineCheck(EditText editText) {
    ...
    Log.m94d("AutofillManager", "view not autofillable - has multiline input type");
    return false;
  }

  private boolean isPackageFullyAllowedOrDeniedForAutofill(String listString, String packageName) {
    return listString.indexOf(new StringBuilder().append(packageName).append(":;").toString())
        != -1;
  }

  private boolean isPackagePartiallyDeniedOrAllowedForAutofill(
      String listString, String packageName) {
    return listString.indexOf(new StringBuilder().append(packageName).append(":").toString()) != -1;
  }

  /* renamed from: shouldIncludeAllChildrenViewsWithAutofillTypeNotNoneInAssistStructure */
  public boolean m164x4bd520f() {
  ...
  private Set<String> getDeniedOrAllowedActivitySetFromString(
    ...
    int firstNextSemicolonIndex =
        listString.indexOf(NavigationBarInflaterView.GRAVITY_SEPARATOR, packageInStringIndex);
    ...
    if (activityStringStartIndex >= firstNextSemicolonIndex) {
      Log.m96e(
          "AutofillManager",
          "Failed to get denied activity names from list because it's wrongly formatted");
      return new ArraySet();
    }
    String activitySubstring =
  ...
  public boolean isAutofillable(View view) {
    ...
    if (!view.isImportantForAutofill() && isActivityDeniedForAutofill()) {
      Log.m94d("AutofillManager", "view is not autofillable - activity denied for autofill");
      return false;
    }
    if (isActivityAllowedForAutofill()) {
      Log.m94d("AutofillManager", "view is autofillable - activity allowed for autofill");
      return true;
    }
    ...
    if (view.isImportantForAutofill()
        || this.mShouldEnableAutofillOnAllViewTypes
    ...
    Log.m94d(
        "AutofillManager",
        "view is not autofillable - not important and filtered by view type check");
    return false;
  }
  ...
  public void enableCompatibilityMode() {
    synchronized (this.mLock) {
      if (Helper.sDebug) {
        Slog.m113d("AutofillManager", "creating CompatibilityBridge for " + this.mContext);
      }
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    synchronized (this.mLock) {
      this.mLastAutofilledData =
          (ParcelableMap)
              savedInstanceState.getParcelable(LAST_AUTOFILLED_DATA_TAG, ParcelableMap.class);
      if (isActiveLocked()) {
        Log.m102w("AutofillManager", "New session was started before onCreate()");
        return;
      }
      ...
      if (this.mSessionId != Integer.MAX_VALUE) {
        ...
        if (client != null) {
          ...
          try {
            if (clientAdded) {
              this.mService.restoreSession(
                  this.mSessionId,
                  client.autofillClientGetActivityToken(),
                  this.mServiceClient.asBinder(),
                  receiver);
              boolean z = true;
              ...
              Log.m102w("AutofillManager", "No service client for session " + this.mSessionId);
            }
            if (!sessionWasRestored) {
              Log.m102w("AutofillManager", "Session " + this.mSessionId + " could not be restored");
              this.mSessionId = Integer.MAX_VALUE;
              this.mState = 0;
            } else {
              if (Helper.sDebug) {
                Log.m94d("AutofillManager", "session " + this.mSessionId + " was restored");
              }
              client.autofillClientResetableStateAvailable();
            }
          } catch (RemoteException e) {
            Log.m97e("AutofillManager", "Could not figure out if there was an autofill session", e);
          } catch (SyncResultReceiver.TimeoutException e2) {
            Log.m96e("AutofillManager", "Fail to get session restore status: " + e2);
          }
        }
      }
    }
  ...
  public FillEventHistory getFillEventHistory() {
    try {
      ...
      Log.m96e("AutofillManager", "Fail to get fill event history: " + e2);
      return null;
    }
  }
  ...
  public void notifyVirtualViewsReady(View view, SparseArray<VirtualViewFillInfo> infos) {
    ...
    if (view.isCredential() && this.mIsFillAndSaveDialogDisabledForCredentialManager) {
      if (Helper.sDebug) {
        Log.m94d(
            "AutofillManager",
            "Ignoring Fill Dialog request since important for credMan:"
      ...
      for (int i = 0; i < infos.size(); i++) {
        ...
        notifyViewReadyInner(
            getAutofillId(view, virtualId), info == null ? null : info.getAutofillHints());
      }
    }
  }
  ...
  public void notifyViewEnteredForFillDialog(View v) {
    if (Helper.sDebug) {
      Log.m94d("AutofillManager", "notifyViewEnteredForFillDialog:" + v.getAutofillId());
    }
    if (v.isCredential() && this.mIsFillAndSaveDialogDisabledForCredentialManager) {
      if (Helper.sDebug) {
        Log.m94d(
            "AutofillManager",
            "Ignoring Fill Dialog request since important for credMan:" + v.getAutofillId());
      }
      this.mHasCredentialField = true;
      return;
    }
        ...
        synchronized (this.mLock) {
          if (!isActiveLocked()) {
            ...
            if (clientAdded) {
              ...
              Log.m100v("AutofillManager", "not starting session: no service client");
            }
          }
        }
      }
      if (this.mIsFillDialogEnabled
          || ArrayUtils.containsAny(autofillHints, this.mFillDialogEnabledHints)) {
        if (Helper.sDebug) {
          Log.m94d("AutofillManager", "Triggering pre-emptive request for fill dialog.");
        }
        int flags = 64 | 16;
        synchronized (this.mLock) {
  ...
  private int getImeStateFlag(View v) {
    ...
    if (v == null
        || (rootWindowInsets = v.getRootWindowInsets()) == null
  ...
  private boolean shouldIgnoreViewEnteredLocked(AutofillId id, int flags) {
    ...
    if (isDisabledByServiceLocked()) {
      if (Helper.sVerbose) {
        Log.m100v(
            "AutofillManager",
            "ignoring notifyViewEntered(flags="
                + flags
                + ", view="
                + id
                + ") on state "
                + getStateAsStringLocked()
                + " because disabled by svc");
      }
      return true;
    }
    if (isFinishedLocked()
        && (flags & 1) == 0
        && (arraySet = this.mEnteredIds) != null
        && arraySet.contains(id)) {
      if (Helper.sVerbose) {
        Log.m100v(
            "AutofillManager",
            "ignoring notifyViewEntered(flags="
                ...
                + ") on state "
                + getStateAsStringLocked()
                + " because view was already entered: "
                + this.mEnteredIds);
      }
      return true;
    }
  ...
  private void notifyViewEntered(View view, int flags) {
    ...
    synchronized (this.mLock) {
      callback =
          notifyViewEnteredLocked(view, view.getAutofillId(), null, view.getAutofillValue(), flags);
    }
    if (callback != null) {
  ...
  void notifyViewExitedLocked(View view) {
    ...
    if (clientAdded) {
      if ((this.mEnabled || this.mEnabledForAugmentedAutofillOnly)
          && isActiveLocked()
          && !isClientDisablingEnterExitEvent()) {
  ...
  private void notifyViewVisibilityChangedInternal(
      View view, int virtualId, boolean isVisible, boolean virtual) {
    ArraySet<AutofillId> arraySet;
    synchronized (this.mLock) {
      if (this.mForAugmentedAutofillOnly) {
        if (Helper.sVerbose) {
          Log.m100v(
              "AutofillManager", "notifyViewVisibilityChanged(): ignoring on augmented only mode");
        }
        return;
      }
      if (this.mEnabled && isActiveLocked()) {
        ...
        if (Helper.sVerbose) {
          Log.m100v("AutofillManager", "visibility changed for " + id + ": " + isVisible);
        }
        if (!isVisible && (arraySet = this.mFillableIds) != null && arraySet.contains(id)) {
          if (Helper.sDebug) {
            Log.m94d("AutofillManager", "Hidding UI when view " + id + " became invisible");
          }
        ...
        if (trackedViews != null) {
          ...
          Log.m100v(
              "AutofillManager", "Ignoring visibility change on " + id + ": no tracked views");
        }
  ...
  private AutofillCallback notifyViewEnteredLocked(
      View view, AutofillId id, Rect bounds, AutofillValue value, int flags) {
    ...
    if (!clientAdded) {
      if (Helper.sVerbose) {
        Log.m100v("AutofillManager", "ignoring notifyViewEntered(" + id + "): no service client");
      }
      return null;
    }
    if (!this.mEnabled && !this.mEnabledForAugmentedAutofillOnly) {
      if (Helper.sVerbose) {
        Log.m100v("AutofillManager", "ignoring notifyViewEntered(" + id + "): disabled");
      }
    ...
    if (!isClientDisablingEnterExitEvent()) {
      ...
      if (AutofillFeatureFlags.isFillAndSaveDialogDisabledForCredentialManager()
          && this.mHasCredentialField) {
        flags |= 1024;
        if (Helper.sVerbose) {
          Log.m100v("AutofillManager", "updating session with flag screen has credman view");
        }
      }
        ...
        if (this.mForAugmentedAutofillOnly && (flags2 & 1) != 0) {
          if (Helper.sDebug) {
            Log.m94d(
                ...
                "notifyViewEntered("
                    + id
                    + "): resetting mForAugmentedAutofillOnly on manual request");
          }
  ...
  public void notifyViewExited(View view, int virtualId) {
    if (Helper.sVerbose) {
      Log.m100v("AutofillManager", "notifyViewExited(" + view.getAutofillId() + ", " + virtualId);
    }
    if (!hasAutofillFeature()) {
      return;
    }
  ...
  private void notifyViewClicked(AutofillId id) {
    ...
    if (Helper.sVerbose) {
      Log.m100v(
          "AutofillManager", "notifyViewClicked(): id=" + id + ", trigger=" + this.mSaveTriggerId);
    }
    synchronized (this.mLock) {
      if (this.mEnabled && isActiveLocked()) {
        ...
        if (autofillId != null && autofillId.equals(id)) {
          if (Helper.sDebug) {
            Log.m94d("AutofillManager", "triggering commit by click of " + id);
          }
          commitLocked(3);
          this.mMetricsLogger.write(
              newLog(MetricsProto.MetricsEvent.AUTOFILL_SAVE_EXPLICITLY_TRIGGERED));
        }
      }
    }
  }
  ...
  public void onActivityFinishing() {
    ...
    synchronized (this.mLock) {
      if (this.mSaveOnFinish) {
        if (Helper.sDebug) {
          Log.m94d("AutofillManager", "onActivityFinishing(): calling commitLocked()");
        }
        commitLocked(1);
      } else {
        if (Helper.sDebug) {
          Log.m94d("AutofillManager", "onActivityFinishing(): calling cancelLocked()");
        }
        cancelLocked();
      }
    }
  ...
  public void commit() {
    ...
    if (Helper.sVerbose) {
      Log.m100v("AutofillManager", "commit() called by app");
    }
    synchronized (this.mLock) {
      commitLocked(2);
    }
  ...
  public void cancel() {
    if (Helper.sVerbose) {
      Log.m100v("AutofillManager", "cancel() called by app or augmented autofill service");
    }
    if (!hasAutofillFeature()) {
      return;
    }
  ...
  public boolean hasEnabledAutofillServices() {
    ...
    SyncResultReceiver receiver = new SyncResultReceiver(5000);
    try {
      this.mService.isServiceEnabled(
          this.mContext.getUserId(), this.mContext.getPackageName(), receiver);
  ...
  public void onAuthenticationResult(int authenticationId, Intent data, View focusView) {
    ...
    if (Helper.sDebug) {
      Log.m94d(
          "AutofillManager",
          "onAuthenticationResult(): id= " + authenticationId + ", data=" + data);
    }
    synchronized (this.mLock) {
      if (isActiveLocked()) {
        if (!this.mOnInvisibleCalled
            && focusView != null
            && focusView.canNotifyAutofillEnterExitEvent()) {
          notifyViewExitedLocked(focusView);
          notifyViewEnteredLocked(
              focusView, focusView.getAutofillId(), null, focusView.getAutofillValue(), 0);
        }
        if (data == null) {
          return;
        }
        ...
        if (data.hasExtra(EXTRA_AUTHENTICATION_RESULT_EPHEMERAL_DATASET)) {
          responseData.putBoolean(
              ...
              data.getBooleanExtra(EXTRA_AUTHENTICATION_RESULT_EPHEMERAL_DATASET, false));
        }
        try {
          this.mService.setAuthenticationResult(
              responseData, this.mSessionId, authenticationId, this.mContext.getUserId());
        } catch (RemoteException e) {
          Log.m97e("AutofillManager", "Error delivering authentication result", e);
        }
      }
    }
  }
  ...
  public AutofillId getNextAutofillId() {
    ...
    if (id == null && Helper.sDebug) {
      Log.m94d("AutofillManager", "getNextAutofillId(): client " + client + " returned null");
    }
    return id;
  }
  ...
  */
  private void startSessionLocked(AutofillId id, Rect bounds, AutofillValue value, int flags) {
    ...
    if ((set == null || !set.contains(id)) && !this.mEnabledForAugmentedAutofillOnly) {
      ...
      if (Helper.sVerbose) {
        Log.m100v("AutofillManager", "Starting session for augmented autofill on " + id);
      }
      flags2 = flags | 8;
    }
    if (Helper.sVerbose) {
      Log.m100v(
          ...
          "startSessionLocked(): id="
              ...
              + ", augmentedOnly="
              + this.mForAugmentedAutofillOnly
              + ", enabledAugmentedOnly="
              + this.mEnabledForAugmentedAutofillOnly
              + ", enteredIds="
              + this.mEnteredIds);
    }
    if (this.mForAugmentedAutofillOnly
      ...
      if (Helper.sVerbose) {
        Log.m100v(
            "AutofillManager", "resetting mForAugmentedAutofillOnly on manual autofill request");
      }
      this.mForAugmentedAutofillOnly = false;
    }
    if (this.mState != 0 && !isFinishedLocked() && (flags2 & 1) == 0) {
      if (Helper.sVerbose) {
        Log.m100v(
            "AutofillManager",
            "not automatically starting session for "
                + id
                + " on state "
                + getStateAsStringLocked()
                + " and flags "
    ...
    try {
      ...
      ComponentName clientActivity = client.autofillClientGetComponentName();
      try {
        if (!this.mEnabledForAugmentedAutofillOnly
            && (autofillOptions = this.mOptions) != null
            && autofillOptions.isAutofillDisabledLocked(clientActivity)) {
          if (this.mOptions.isAugmentedAutofillEnabled(this.mContext)) {
            if (Helper.sDebug) {
              Log.m94d(
                  ...
                  "startSession("
                      + clientActivity
                      + "): disabled by service but allowlisted for augmented autofill");
              flags3 = flags2 | 8;
              ...
              if ((extraFlags & 1) != 0) {
                if (Helper.sDebug) {
                  Log.m94d(
                      "AutofillManager",
                      "startSession(" + clientActivity + "): for augmented only");
                }
            ...
            if (Helper.sDebug) {
              Log.m94d(
                  ...
                  "startSession("
                      + clientActivity
                      + "): ignored because disabled by service and not allowlisted for augmented"
                      + " autofill");
            }
        ...
        e = e2;
        flags2 = flags3;
        Log.m102w("AutofillManager", "Exception getting result from SyncResultReceiver: " + e);
        return;
      }
      flags3 = flags2;
  ...
  public void finishSessionLocked(int commitReason) {
    if (Helper.sVerbose) {
      Log.m100v("AutofillManager", "finishSessionLocked(): " + getStateAsStringLocked());
    }
    if (isActiveLocked()) {
      try {
  ...
  private void cancelSessionLocked() {
    if (Helper.sVerbose) {
      Log.m100v("AutofillManager", "cancelSessionLocked(): " + getStateAsStringLocked());
    }
    if (isActiveLocked()) {
      try {
  ...
  private void updateSessionLocked(
      AutofillId id, Rect bounds, AutofillValue value, int action, int flags) {
    if (Helper.sVerbose) {
      Log.m100v(
          "AutofillManager",
          "updateSessionLocked(): id="
              + id
              + ", bounds="
              + bounds
              + ", value="
    ...
    try {
      this.mService.updateSession(
          this.mSessionId, id, bounds, value, action, flags, this.mContext.getUserId());
    } catch (RemoteException e) {
      throw e.rethrowFromSystemServer();
    }
  }
  ...
  private boolean tryAddServiceClientIfNeededLocked() {
    ...
    if (this.mService == null) {
      Log.m102w("AutofillManager", "Autofill service is null!");
      return false;
    }
    if (this.mServiceClient == null) {
      ...
      try {
        ...
        SyncResultReceiver receiver = new SyncResultReceiver(5000);
        this.mService.addClient(
            this.mServiceClient, client.autofillClientGetComponentName(), userId, receiver);
        try {
          ...
          Log.m102w("AutofillManager", "Failed to initialize autofill: " + e);
  ...
  private boolean startAutofillIfNeededLocked(View view) {
    if (this.mState == 0
      ...
      if (Helper.sVerbose) {
        Log.m100v(
            "AutofillManager",
            "startAutofillIfNeededLocked(): enabled="
                + this.mEnabled
                + " mServiceClient="
                + this.mServiceClient);
      }
  ...
  public void setAugmentedAutofillWhitelist(Set<String> packages, Set<ComponentName> activities) {
    ...
    try {
      this.mService.setAugmentedAutofillWhitelist(
          Helper.toList(packages), Helper.toList(activities), resultReceiver);
      ...
      switch (resultCode) {
        ...
        default:
          Log.wtf(
              "AutofillManager",
              "setAugmentedAutofillWhitelist(): received invalid result: " + resultCode);
          return;
      }
    } catch (RemoteException e) {
      throw e.rethrowFromSystemServer();
      ...
      Log.m96e(
          "AutofillManager", "Fail to get the result of set AugmentedAutofill whitelist. " + e2);
    }
  }
      ...
      try {
        try {
          if (this.mSessionId == sessionId
              ...
              && client.autofillClientRequestShowFillUi(
                  anchor, width, height, anchorBounds, presenter)) {
  ...
  public void authenticate(
    ...
    synchronized (this.mLock) {
      if (sessionId == this.mSessionId && (client = getClient()) != null) {
        ...
        client.autofillClientAuthenticate(
            authenticationId, intent, fillInIntent, authenticateInline);
      }
    }
  }
    ...
    if (Helper.sVerbose) {
      Log.m100v(
          ...
          "setState("
              + flags
              + ": "
              + DebugUtils.flagsToString(AutofillManager.class, "SET_STATE_FLAG_", flags)
  ...
  private void setAutofilledIfValuesIs(
      View view, AutofillValue targetValue, boolean hideHighlight) {
  ...
  public void autofill(
      int sessionId, List<AutofillId> ids, List<AutofillValue> values, boolean hideHighlight) {
    AutofillId id;
    AutofillClient client;
    synchronized (this.mLock) {
      try {
        ...
        while (numApplied2 < itemCount) {
          ...
          try {
            ...
            if (view == null) {
              client = client2;
              Log.m94d("AutofillManager", "autofill(): no View with id " + id);
              if (failedIds == null) {
        ...
        if (failedIds != null) {
          if (Helper.sVerbose) {
            Log.m100v("AutofillManager", "autofill(): total failed views: " + failedIds);
          }
          try {
        ...
        this.mMetricsLogger.write(
            newLog(MetricsProto.MetricsEvent.AUTOFILL_DATASET_APPLIED)
                .addTaggedData(
                    MetricsProto.MetricsEvent.FIELD_AUTOFILL_NUM_VALUES, Integer.valueOf(itemCount))
                .addTaggedData(
                    MetricsProto.MetricsEvent.FIELD_AUTOFILL_NUM_VIEWS_FILLED,
                    Integer.valueOf(numApplied)));
      } catch (Throwable th4) {
        th = th4;
      }
    }
  ...
  public void autofillContent(int sessionId, AutofillId id, ClipData clip) {
    synchronized (this.mLock) {
      ...
      if (view == null) {
        Log.m94d("AutofillManager", "autofillContent(): no view with id " + id);
      ...
      if (result != null) {
        Log.m102w(
            "AutofillManager",
            "autofillContent(): receiver could not insert content: id="
                + id
                + ", view="
                + view
                + ", clip="
        ...
        this.mMetricsLogger.write(
            newLog(MetricsProto.MetricsEvent.AUTOFILL_DATASET_APPLIED)
                .addTaggedData(MetricsProto.MetricsEvent.FIELD_AUTOFILL_NUM_VALUES, 1)
                .addTaggedData(MetricsProto.MetricsEvent.FIELD_AUTOFILL_NUM_VIEWS_FILLED, 1));
      }
    }
  }
  ...
  private void reportAutofillContentFailure(AutofillId id) {
    try {
      this.mService.setAutofillFailure(
          this.mSessionId, Collections.singletonList(id), this.mContext.getUserId());
    } catch (RemoteException e) {
      throw e.rethrowFromSystemServer();
    }
  }
    ...
    LogMaker log =
        new LogMaker(category)
            .addTaggedData(
                ...
                Integer.valueOf(this.mSessionId));
  ...
  private void setNotifyOnClickLocked(AutofillId id, boolean notify) {
    ...
    if (view == null) {
      Log.m102w("AutofillManager", "setNotifyOnClick(): invalid id: " + id);
    } else {
  ...
  public void setSaveUiState(int sessionId, boolean shown) {
    if (Helper.sDebug) {
      Log.m94d("AutofillManager", "setSaveUiState(" + sessionId + "): " + shown);
    }
    synchronized (this.mLock) {
      if (this.mSessionId != Integer.MAX_VALUE) {
        Log.m102w(
  ...
  public void getAugmentedAutofillClient(IResultReceiver result) {
    synchronized (this.mLock) {
      ...
      resultData.putBinder(
          EXTRA_AUGMENTED_AUTOFILL_CLIENT, this.mAugmentedAutofillServiceClient.asBinder());
      try {
        result.send(0, resultData);
      } catch (RemoteException e) {
        Log.m102w("AutofillManager", "Could not send AugmentedAutofillClient back: " + e);
      }
    }
  }
  ...
  public void requestShowSoftInput(AutofillId id) {
    if (Helper.sVerbose) {
      Log.m100v(
          "AutofillManager", "requestShowSoftInput(" + id + NavigationBarInflaterView.KEY_CODE_END);
    }
    AutofillClient client = getClient();
    if (client == null) {
      return;
    ...
    if (view == null) {
      if (Helper.sVerbose) {
        Log.m100v("AutofillManager", "View is not found");
        return;
      }
      return;
    }
    ...
    if (handler == null) {
      if (Helper.sVerbose) {
        Log.m100v("AutofillManager", "Ignoring requestShowSoftInput due to no handler in view");
      }
    } else {
      if (handler.getLooper() != Looper.myLooper()) {
        if (Helper.sVerbose) {
          Log.m100v("AutofillManager", "Scheduling showSoftInput() on the view UI thread");
        }
        handler.post(
            new Runnable() { // from class:
  ...
  public static void requestShowSoftInputInViewThread(View view) {
    if (!view.isFocused()) {
      Log.m102w("AutofillManager", "Ignoring requestShowSoftInput() due to non-focused view");
      return;
    }
    InputMethodManager inputMethodManager =
        (InputMethodManager) view.getContext().getSystemService(InputMethodManager.class);
    boolean ret = inputMethodManager.showSoftInput(view, 0);
    if (Helper.sVerbose) {
      Log.m100v("AutofillManager", " InputMethodManager.showSoftInput returns " + ret);
    }
  }
  ...
  public void requestHideFillUi(AutofillId id, boolean force) {
    ...
    if (Helper.sVerbose) {
      Log.m100v("AutofillManager", "requestHideFillUi(" + id + "): anchor = " + anchor);
    }
    if (anchor == null) {
  ...
  public void notifyDisableAutofill(long disableDuration, ComponentName componentName) {
    synchronized (this.mLock) {
      ...
      if (componentName != null) {
        ...
        this.mOptions.disabledActivities.put(
            componentName.flattenToString(), Long.valueOf(expiration));
      } else {
  ...
  public void notifyNoFillUi(int sessionId, AutofillId id, int sessionFinishedState) {
    if (Helper.sVerbose) {
      Log.m100v(
          "AutofillManager", "notifyNoFillUi(): sessionFinishedState=" + sessionFinishedState);
    }
    View anchor = findView(id);
    if (anchor == null) {
      return;
  ...
  public void notifyCallback(int sessionId, AutofillId id, int event) {
    if (Helper.sVerbose) {
      Log.m100v(
          "AutofillManager",
          "notifyCallback(): sessionId=" + sessionId + ", autofillId=" + id + ", event=" + event);
    }
    View anchor = findView(id);
    if (anchor == null) {
      return;
  ...
  public void onPendingSaveUi(int operation, IBinder token) {
    if (Helper.sVerbose) {
      Log.m100v("AutofillManager", "onPendingSaveUi(" + operation + "): " + token);
    }
    synchronized (this.mLock) {
      try {
        this.mService.onPendingSaveUi(operation, token);
      } catch (RemoteException e) {
        Log.m97e("AutofillManager", "Error in onPendingSaveUi: ", e);
      }
    }
  }
  ...
  public void dump(String outerPrefix, PrintWriter pw) {
    ...
    synchronized (this.mLock) {
      ...
      if (this.mCompatibilityBridge != null) {
        ...
        pw.print("virtualId: ");
        pw.println(
            AccessibilityNodeInfo.getVirtualDescendantId(this.mCompatibilityBridge.mFocusedNodeId));
        pw.print(pfx22);
  ...
  public void post(Runnable runnable) {
    ...
    if (client == null) {
      if (Helper.sVerbose) {
        Log.m100v("AutofillManager", "ignoring post() because client is null");
        return;
      }
      return;
    }
  ...
  private boolean shouldShowAutofillDialog(View view, AutofillId id) {
    if (!hasFillDialogUiFeature()
        || this.mShowAutofillDialogCalled
        || this.mFillDialogTriggerIds == null
        || this.mHasCredentialField
        || getImeStateFlag(view) == 128) {
      return false;
    }
  ...
  private final class CompatibilityBridge implements AccessibilityManager.AccessibilityPolicy {
    ...
    private AccessibilityServiceInfo getCompatServiceInfo() {
      synchronized (AutofillManager.this.mLock) {
        ...
        ResolveInfo resolveInfo =
            AutofillManager.this.mContext.getPackageManager().resolveService(intent, 1048704);
        try {
          AccessibilityServiceInfo accessibilityServiceInfo2 =
              new AccessibilityServiceInfo(resolveInfo, AutofillManager.this.mContext);
          this.mCompatServiceInfo = accessibilityServiceInfo2;
          return accessibilityServiceInfo2;
        } catch (IOException | XmlPullParserException e) {
          Log.m96e("AutofillManager", "Cannot find compat autofill service:" + intent);
    ...
    @Override // android.view.accessibility.AccessibilityManager.AccessibilityPolicy
    public AccessibilityEvent onAccessibilityEvent(
      ...
      if (Helper.sVerbose) {
        Log.m100v(
            ...
            "onAccessibilityEvent("
                ...
                + ", client="
                + AutofillManager.this.getClient());
      }
      switch (type) {
        case 1:
          synchronized (AutofillManager.this.mLock) {
        ...
        case 8:
          synchronized (AutofillManager.this.mLock) {
            if (this.mFocusedWindowId != event.getWindowId()
                || this.mFocusedNodeId != event.getSourceNodeId()) {
              if (this.mFocusedWindowId != -1
    ...
    private boolean notifyViewEntered(int windowId, long nodeId, Rect focusedBounds) {
      ...
      if (!isVirtualNode(virtualId)
          || (view = findViewByAccessibilityId(windowId, nodeId)) == null
          || (node = findVirtualNodeByAccessibilityId(view, virtualId)) == null
          || !node.isEditable()) {
        return false;
      }
    ...
    private void notifyValueChanged(int windowId, long nodeId) {
      ...
      AutofillManager.this.notifyValueChanged(
          view, virtualId, AutofillValue.forText(node.getText()));
    }
  ...
  private class TrackedViews {
    ...
    private void initialTrackedViews(
        AutofillId[] trackedIds,
        ArraySet<AutofillId> visibleSet,
        ArraySet<AutofillId> invisibleSet) {
      boolean[] isVisible;
      AutofillClient client = AutofillManager.this.getClient();
      ...
      if (client.autofillClientIsVisibleForAutofill()) {
        if (Helper.sVerbose) {
          Log.m100v("AutofillManager", "client is visible, check tracked ids");
        }
    ...
    void notifyViewVisibilityChangedLocked(AutofillId id, boolean isVisible) {
      if (Helper.sDebug) {
        Log.m94d(
            "AutofillManager",
            "notifyViewVisibilityChangedLocked(): id=" + id + " isVisible=" + isVisible);
      }
      ...
      if (this.mIsTrackedSaveView && this.mVisibleTrackedIds.isEmpty()) {
        if (Helper.sVerbose) {
          Log.m100v(
              "AutofillManager", "No more visible ids. Invisible = " + this.mInvisibleTrackedIds);
        }
        AutofillManager.this.finishSessionLocked(4);
      }
      if (this.mVisibleDialogTrackedIds.isEmpty()) {
        if (Helper.sVerbose) {
          Log.m100v(
              "AutofillManager",
              "No more visible ids. Invisible = " + this.mInvisibleDialogTrackedIds);
        }
    ...
    void onVisibleForAutofillChangedLocked() {
      ...
      if (client != null) {
        if (Helper.sVerbose) {
          Log.m100v(
              "AutofillManager",
              "onVisibleForAutofillChangedLocked(): inv= "
                  + this.mInvisibleTrackedIds
                  + " vis="
                  + this.mVisibleTrackedIds);
        }
        onVisibleForAutofillChangedInternalLocked(
            this.mVisibleTrackedIds, this.mInvisibleTrackedIds);
        onVisibleForAutofillChangedInternalLocked(
            this.mVisibleDialogTrackedIds, this.mInvisibleDialogTrackedIds);
      }
      if (this.mIsTrackedSaveView && this.mVisibleTrackedIds.isEmpty()) {
        if (Helper.sVerbose) {
          Log.m100v("AutofillManager", "onVisibleForAutofillChangedLocked(): no more visible ids");
        }
        AutofillManager.this.finishSessionLocked(4);
      }
      if (this.mVisibleDialogTrackedIds.isEmpty()) {
    ...
    void onVisibleForAutofillChangedInternalLocked(
        ArraySet<AutofillId> visibleSet, ArraySet<AutofillId> invisibleSet) {
      if (Helper.sVerbose) {
        Log.m100v(
            "AutofillManager",
            "onVisibleForAutofillChangedLocked(): inv= " + invisibleSet + " vis=" + visibleSet);
      }
  ...
  static final class AutofillManagerClient extends IAutoFillManagerClient.Stub {
    ...
    public void autofill(
        final int sessionId,
        final List<AutofillId> ids,
        final List<AutofillValue> values,
        final boolean hideHighlight) {
      final AutofillManager afm = this.mAfm.get();
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda2
              @Override // java.lang.Runnable
              public final void run() {
    ...
    public void authenticate(
        ...
        final boolean authenticateInline) {
      final AutofillManager afm = this.mAfm.get();
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda3
              @Override // java.lang.Runnable
              public final void run() {
                AutofillManager.this.authenticate(
                    sessionId, authenticationId, intent, fillInIntent, authenticateInline);
              }
            });
      }
    }
    ...
    public void requestShowFillUi(
        ...
        final IAutofillWindowPresenter presenter) {
      final AutofillManager afm = this.mAfm.get();
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
              ...
              @Override // java.lang.Runnable
              public final void run() {
                AutofillManager.this.requestShowFillUi(
                    sessionId, id, width, height, anchorBounds, presenter);
              }
            });
      }
    }
    ...
    @Override // android.view.autofill.IAutoFillManagerClient
    public void notifyNoFillUi(
        final int sessionId, final AutofillId id, final int sessionFinishedState) {
      final AutofillManager afm = this.mAfm.get();
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda12
              @Override // java.lang.Runnable
              public final void run() {
    ...
    public void notifyFillUiShown(final int sessionId, final AutofillId id) {
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda17
              @Override // java.lang.Runnable
              public final void run() {
    ...
    public void notifyFillUiHidden(final int sessionId, final AutofillId id) {
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda15
              @Override // java.lang.Runnable
              public final void run() {
    ...
    @Override // android.view.autofill.IAutoFillManagerClient
    public void notifyDisableAutofill(final long disableDuration, final ComponentName componentName)
    ...
    @Override // android.view.autofill.IAutoFillManagerClient
    public void dispatchUnhandledKey(
        final int sessionId, final AutofillId id, final KeyEvent fullScreen) {
      final AutofillManager afm = this.mAfm.get();
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda6
              @Override // java.lang.Runnable
              public final void run() {
    ...
    public void startIntentSender(final IntentSender intentSender, final Intent intent) {
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda0
              @Override // java.lang.Runnable
              public final void run() {
                AutofillManager.AutofillManagerClient.lambda$startIntentSender$11(
                    AutofillManager.this, intentSender, intent);
              }
            });
      }
    }

    static /* synthetic */ void lambda$startIntentSender$11(
        AutofillManager afm, IntentSender intentSender, Intent intent) {
      try {
        afm.mContext.startIntentSender(intentSender, intent, 0, 0, 0);
      } catch (IntentSender.SendIntentException e) {
        Log.m97e("AutofillManager", "startIntentSender() failed for intent:" + intentSender, e);
      }
    }
    ...
    public void setTrackedViews(
        final int sessionId,
        final AutofillId[] ids,
        final boolean saveOnAllViewsInvisible,
        final boolean saveOnFinish,
        final AutofillId[] fillableIds,
        final AutofillId saveTriggerId) {
      final AutofillManager afm = this.mAfm.get();
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda5
              @Override // java.lang.Runnable
              public final void run() {
                AutofillManager.this.setTrackedViews(
                    sessionId,
                    ids,
                    saveOnAllViewsInvisible,
                    saveOnFinish,
    ...
    public void setSessionFinished(final int newState, final List<AutofillId> autofillableIds) {
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda7
              @Override // java.lang.Runnable
              public final void run() {
    ...
    public void requestShowSoftInput(final AutofillId id) {
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda1
              @Override // java.lang.Runnable
              public final void run() {
    ...
    public void notifyFillDialogTriggerIds(final List<AutofillId> ids) {
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AutofillManagerClient$$ExternalSyntheticLambda4
              @Override // java.lang.Runnable
              public final void run() {
  ...
  static final class AugmentedAutofillManagerClient extends IAugmentedAutofillManagerClient.Stub {
    ...
    public AssistStructure.ViewNodeParcelable getViewNodeParcelable(AutofillId id) {
      ...
      if (view == null) {
        Log.m102w("AutofillManager", "getViewNodeParcelable(" + id + "): could not find view");
        return null;
      }
    ...
    public Rect getViewCoordinates(AutofillId id) {
      ...
      Rect rect =
          new Rect(
              location[0],
              location[1] - windowVisibleDisplayFrame.top,
              location[0] + view.getWidth(),
              (location[1] - windowVisibleDisplayFrame.top) + view.getHeight());
      if (Helper.sVerbose) {
        Log.m100v("AutofillManager", "Coordinates for " + id + ": " + rect);
      }
      return rect;
    }
    ...
    public void autofill(
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AugmentedAutofillManagerClient$$ExternalSyntheticLambda0
              @Override // java.lang.Runnable
              public final void run() {
                AutofillManager.this.autofill(sessionId, ids, values, hideHighlight);
              }
    ...
    public void requestShowFillUi(
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AugmentedAutofillManagerClient$$ExternalSyntheticLambda3
              @Override // java.lang.Runnable
              public final void run() {
                AutofillManager.this.requestShowFillUi(
                    sessionId, id, width, height, anchorBounds, presenter);
    ...
    public void requestHideFillUi(int sessionId, final AutofillId id) {
      ...
      if (afm != null) {
        afm.post(
            new Runnable() { // from class:
                             // android.view.autofill.AutofillManager$AugmentedAutofillManagerClient$$ExternalSyntheticLambda2
              @Override // java.lang.Runnable
              public final void run() {
                AutofillManager.this.requestHideFillUi(id, false);
              }
    ...
    public boolean requestAutofill(int sessionId, AutofillId id) {
      ...
      if (afm == null || afm.mSessionId != sessionId) {
        if (Helper.sDebug) {
          Slog.m113d("AutofillManager", "Autofill not available or sessionId doesn't match");
        }
        return false;
      }
      ...
      if (view == null || !view.isFocused()) {
        if (Helper.sDebug) {
          Slog.m113d("AutofillManager", "View not available or is not on focus");
        }
        return false;
      }
      if (Helper.sVerbose) {
        Log.m100v("AutofillManager", "requestAutofill() by AugmentedAutofillService.");
      }
      afm.post(
          new Runnable() { // from class:
    ...
    private View getView(AutofillManager afm, AutofillId id) {
      ...
      if (client == null) {
        Log.m102w("AutofillManager", "getView(" + id + "): no autofill client");
        return null;
      }
      View view = client.autofillClientFindViewByAutofillIdTraversal(id);
      if (view == null) {
        Log.m102w("AutofillManager", "getView(" + id + "): could not find view");
      }
      return view;
    }
  }
