...
public final class AutofillManager {
    ...
    public AutofillManager(Context context, IAutoFillManager service) {
        ...
        if (Helper.sDebug) {
            Log.m94d("AutofillManager", "Fill dialog is enabled:" + isFillDialogEnabled + ", hints=" + Arrays.toString(fillDialogEnabledHints));
        }
    ...
    private Set<String> getDeniedOrAllowedActivitySetFromString(String listString, String packageName) {
        ...
        if (activityStringStartIndex >= firstNextSemicolonIndex) {
            Log.m96e("AutofillManager", "Failed to get denied activity names from list because it's wrongly formatted");
            return new ArraySet();
        }
    ...
    public boolean isAutofillable(View view) {
        ...
        Log.m94d("AutofillManager", "view is not autofillable - not important and filtered by view type check");
        return false;
    }
    ...
    public void notifyVirtualViewsReady(View view, SparseArray<VirtualViewFillInfo> infos) {
        ...
        if (view.isCredential() && this.mIsFillAndSaveDialogDisabledForCredentialManager) {
            if (Helper.sDebug) {
                Log.m94d("AutofillManager", "Ignoring Fill Dialog request since important for credMan:" + view.getAutofillId().toString());
            }
    ...
    public void notifyViewEnteredForFillDialog(View v) {
        ...
        if (v.isCredential() && this.mIsFillAndSaveDialogDisabledForCredentialManager) {
            if (Helper.sDebug) {
                Log.m94d("AutofillManager", "Ignoring Fill Dialog request since important for credMan:" + v.getAutofillId());
            }
            this.mHasCredentialField = true;
            return;
        }
    ...
    private boolean shouldIgnoreViewEnteredLocked(AutofillId id, int flags) {
        ...
        if (isDisabledByServiceLocked()) {
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "ignoring notifyViewEntered(flags=" + flags + ", view=" + id + ") on state " + getStateAsStringLocked() + " because disabled by svc");
            }
            return true;
        }
        if (isFinishedLocked() && (flags & 1) == 0 && (arraySet = this.mEnteredIds) != null && arraySet.contains(id)) {
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "ignoring notifyViewEntered(flags=" + flags + ", view=" + id + ") on state " + getStateAsStringLocked() + " because view was already entered: " + this.mEnteredIds);
            }
            return true;
        }
        return false;
    ...
    private void notifyViewVisibilityChangedInternal(View view, int virtualId, boolean isVisible, boolean virtual) {
        ...
        synchronized (this.mLock) {
            if (this.mForAugmentedAutofillOnly) {
                if (Helper.sVerbose) {
                    Log.m100v("AutofillManager", "notifyViewVisibilityChanged(): ignoring on augmented only mode");
                }
                return;
            }
            if (this.mEnabled && isActiveLocked()) {
                ...
                if (trackedViews != null) {
                    ...
                    Log.m100v("AutofillManager", "Ignoring visibility change on " + id + ": no tracked views");
                }
        ...
        if (!isClientDisablingEnterExitEvent()) {
            ...
            if (!isActiveLocked()) {
                ...
                if (this.mForAugmentedAutofillOnly && (flags2 & 1) != 0) {
                    if (Helper.sDebug) {
                        Log.m94d("AutofillManager", "notifyViewEntered(" + id + "): resetting mForAugmentedAutofillOnly on manual request");
                    }
    ...
    public void notifyValueChanged(View view) {
        ...
        synchronized (this.mLock) {
            ...
            if (!startAutofillIfNeededLocked(view) && Helper.sVerbose) {
                Log.m100v("AutofillManager", "notifyValueChanged(" + view.getAutofillId() + "): ignoring on state " + getStateAsStringLocked());
            }
        }
    }
    ...
    public void notifyValueChanged(View view, int virtualId, AutofillValue value) {
        ...
        synchronized (this.mLock) {
            ...
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "notifyValueChanged(" + view.getAutofillId() + ":" + virtualId + "): ignoring on state " + getStateAsStringLocked());
            }
        }
    }
    ...
    private void notifyViewClicked(AutofillId id) {
        ...
        if (Helper.sVerbose) {
            Log.m100v("AutofillManager", "notifyViewClicked(): id=" + id + ", trigger=" + this.mSaveTriggerId);
        }
        synchronized (this.mLock) {
            if (this.mEnabled && isActiveLocked()) {
    ...
    public AutofillClient getClient() {
        ...
        if (client == null && Helper.sVerbose) {
            Log.m100v("AutofillManager", "No AutofillClient for " + this.mContext.getPackageName() + " on context " + this.mContext);
        }
        return client;
    }
    ...
    public void onAuthenticationResult(int authenticationId, Intent data, View focusView) {
        ...
        if (Helper.sDebug) {
            Log.m94d("AutofillManager", "onAuthenticationResult(): id= " + authenticationId + ", data=" + data);
        }
        synchronized (this.mLock) {
            if (isActiveLocked()) {
    ...
    */
    private void startSessionLocked(AutofillId id, Rect bounds, AutofillValue value, int flags) {
        ...
        if (Helper.sVerbose) {
            Log.m100v("AutofillManager", "startSessionLocked(): id=" + id + ", bounds=" + bounds + ", value=" + value + ", flags=" + flags2 + ", state=" + getStateAsStringLocked() + ", compatMode=" + isCompatibilityModeEnabledLocked() + ", augmentedOnly=" + this.mForAugmentedAutofillOnly + ", enabledAugmentedOnly=" + this.mEnabledForAugmentedAutofillOnly + ", enteredIds=" + this.mEnteredIds);
        }
        if (this.mForAugmentedAutofillOnly && !this.mEnabledForAugmentedAutofillOnly && (flags2 & 1) != 0) {
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "resetting mForAugmentedAutofillOnly on manual autofill request");
            }
            this.mForAugmentedAutofillOnly = false;
        }
        if (this.mState != 0 && !isFinishedLocked() && (flags2 & 1) == 0) {
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "not automatically starting session for " + id + " on state " + getStateAsStringLocked() + " and flags " + flags2);
                return;
            }
            return;
        }
            ...
            try {
                if (!this.mEnabledForAugmentedAutofillOnly && (autofillOptions = this.mOptions) != null && autofillOptions.isAutofillDisabledLocked(clientActivity)) {
                    if (this.mOptions.isAugmentedAutofillEnabled(this.mContext)) {
                        if (Helper.sDebug) {
                            Log.m94d("AutofillManager", "startSession(" + clientActivity + "): disabled by service but allowlisted for augmented autofill");
                            flags3 = flags2 | 8;
                            ...
                            if ((extraFlags & 1) != 0) {
                                if (Helper.sDebug) {
                                    Log.m94d("AutofillManager", "startSession(" + clientActivity + "): for augmented only");
                                }
                        ...
                        if (Helper.sDebug) {
                            Log.m94d("AutofillManager", "startSession(" + clientActivity + "): ignored because disabled by service and not allowlisted for augmented autofill");
                        }
    ...
    private void updateSessionLocked(AutofillId id, Rect bounds, AutofillValue value, int action, int flags) {
        if (Helper.sVerbose) {
            Log.m100v("AutofillManager", "updateSessionLocked(): id=" + id + ", bounds=" + bounds + ", value=" + value + ", action=" + action + ", flags=" + flags);
        }
        try {
        ...
        if (this.mServiceClient == null) {
            ...
            try {
                ...
                try {
                    ...
                    if (Helper.sVerbose) {
                        Log.m100v("AutofillManager", "receiver results: flags=" + flags + " enabled=" + this.mEnabled + ", enabledForAugmentedOnly: " + this.mEnabledForAugmentedAutofillOnly);
                    }
    ...
    private boolean startAutofillIfNeededLocked(View view) {
        if (this.mState == 0 && this.mSessionId == Integer.MAX_VALUE && (view instanceof EditText) && !TextUtils.isEmpty(((EditText) view).getText()) && !view.isFocused() && view.isImportantForAutofill() && view.isLaidOut() && view.isVisibleToUser()) {
            boolean clientAdded = tryAddServiceClientIfNeededLocked();
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "startAutofillIfNeededLocked(): enabled=" + this.mEnabled + " mServiceClient=" + this.mServiceClient);
            }
    ...
    public void setAugmentedAutofillWhitelist(Set<String> packages, Set<ComponentName> activities) {
        ...
        try {
            ...
            Log.m96e("AutofillManager", "Fail to get the result of set AugmentedAutofill whitelist. " + e2);
        }
    }
    ...
    public void setState(int flags) {
        if (Helper.sVerbose) {
            Log.m100v("AutofillManager", "setState(" + flags + ": " + DebugUtils.flagsToString(AutofillManager.class, "SET_STATE_FLAG_", flags) + NavigationBarInflaterView.KEY_CODE_END);
        }
        synchronized (this.mLock) {
            try {
                if ((flags & 32) != 0) {
    ...
    public void autofillContent(int sessionId, AutofillId id, ClipData clip) {
        synchronized (this.mLock) {
            ...
            if (result != null) {
                Log.m102w("AutofillManager", "autofillContent(): receiver could not insert content: id=" + id + ", view=" + view + ", clip=" + clip);
                reportAutofillContentFailure(id);
            } else {
    ...
    public void setSaveUiState(int sessionId, boolean shown) {
        ...
        synchronized (this.mLock) {
            if (this.mSessionId != Integer.MAX_VALUE) {
                Log.m102w("AutofillManager", "setSaveUiState(" + sessionId + ", " + shown + ") called on existing session " + this.mSessionId + "; cancelling it");
                cancelSessionLocked();
            }
            if (shown) {
    ...
    public void setSessionFinished(int newState, List<AutofillId> autofillableIds) {
        ...
        synchronized (this.mLock) {
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "setSessionFinished(): from " + getStateAsStringLocked() + " to " + getStateAsString(newState) + "; autofillableIds=" + autofillableIds);
            }
            if (autofillableIds != null) {
    ...
    public void requestShowSoftInput(AutofillId id) {
        if (Helper.sVerbose) {
            Log.m100v("AutofillManager", "requestShowSoftInput(" + id + NavigationBarInflaterView.KEY_CODE_END);
        }
        AutofillClient client = getClient();
        if (client == null) {
            return;
    ...
    public void notifyNoFillUi(int sessionId, AutofillId id, int sessionFinishedState) {
        if (Helper.sVerbose) {
            Log.m100v("AutofillManager", "notifyNoFillUi(): sessionFinishedState=" + sessionFinishedState);
        }
        View anchor = findView(id);
        if (anchor == null) {
            return;
    ...
    public void notifyCallback(int sessionId, AutofillId id, int event) {
        if (Helper.sVerbose) {
            Log.m100v("AutofillManager", "notifyCallback(): sessionId=" + sessionId + ", autofillId=" + id + ", event=" + event);
        }
        View anchor = findView(id);
        if (anchor == null) {
            return;
    ...
    private final class CompatibilityBridge implements AccessibilityManager.AccessibilityPolicy {
        ...
        public AccessibilityEvent onAccessibilityEvent(AccessibilityEvent event, boolean accessibilityEnabled, int relevantEventTypes) {
            ...
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "onAccessibilityEvent(" + AccessibilityEvent.eventTypeToString(type) + "): virtualId=" + AccessibilityNodeInfo.getVirtualDescendantId(event.getSourceNodeId()) + ", client=" + AutofillManager.this.getClient());
            }
            switch (type) {
                case 1:
                    synchronized (AutofillManager.this.mLock) {
    ...
    private class TrackedViews {
        ...
        TrackedViews(AutofillId[] trackedIds, AutofillId[] allTrackedIds) {
            ...
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "TrackedViews(trackedIds=" + Arrays.toString(trackedIds) + "):  mVisibleTrackedIds=" + arraySet + " mInvisibleTrackedIds=" + arraySet2 + " allTrackedIds=" + Arrays.toString(allTrackedIds) + " mVisibleDialogTrackedIds=" + arraySet3 + " mInvisibleDialogTrackedIds=" + arraySet4);
            }
        ...
        void notifyViewVisibilityChangedLocked(AutofillId id, boolean isVisible) {
            if (Helper.sDebug) {
                Log.m94d("AutofillManager", "notifyViewVisibilityChangedLocked(): id=" + id + " isVisible=" + isVisible);
            }
            ...
            if (this.mIsTrackedSaveView && this.mVisibleTrackedIds.isEmpty()) {
                if (Helper.sVerbose) {
                    Log.m100v("AutofillManager", "No more visible ids. Invisible = " + this.mInvisibleTrackedIds);
                }
                AutofillManager.this.finishSessionLocked(4);
            }
            if (this.mVisibleDialogTrackedIds.isEmpty()) {
                if (Helper.sVerbose) {
                    Log.m100v("AutofillManager", "No more visible ids. Invisible = " + this.mInvisibleDialogTrackedIds);
                }
        ...
        void onVisibleForAutofillChangedLocked() {
            ...
            if (client != null) {
                if (Helper.sVerbose) {
                    Log.m100v("AutofillManager", "onVisibleForAutofillChangedLocked(): inv= " + this.mInvisibleTrackedIds + " vis=" + this.mVisibleTrackedIds);
                }
        ...
        void onVisibleForAutofillChangedInternalLocked(ArraySet<AutofillId> visibleSet, ArraySet<AutofillId> invisibleSet) {
            if (Helper.sVerbose) {
                Log.m100v("AutofillManager", "onVisibleForAutofillChangedLocked(): inv= " + invisibleSet + " vis=" + visibleSet);
            }
    ...
    static final class AutofillManagerClient extends IAutoFillManagerClient.Stub {
        ...
        static /* synthetic */ void lambda$startIntentSender$11(AutofillManager afm, IntentSender intentSender, Intent intent) {
            try {
