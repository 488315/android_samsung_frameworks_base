...
public final class MotionEvent extends InputEvent implements Parcelable {
    ...
    public static MotionEvent obtain(long downTime, long eventTime, int action, int pointerCount, PointerProperties[] pointerProperties, PointerCoords[] pointerCoords, int metaState, int buttonState, float xPrecision, float yPrecision, int deviceId, int edgeFlags, int source, int displayId, int flags, int classification) {
        ...
        if (!success) {
            Log.m96e(TAG, "Could not initialize MotionEvent");
            ev.recycle();
            return null;
        }
        return ev;
                ...
                try {
                    ...
                    for (int i = 0; i < pointerCount; i++) {
                        try {
                            ...
                            try {
                                pc[i].f544x = clamp(pc[i].f544x, left, right);
                                try {
                                    pc[i].f545y = clamp(pc[i].f545y, top, bottom);
                                } catch (Throwable th) {
                                    th = th;
                                    throw th;
                                }
    ...
    */
    public final MotionEvent split(int idBits) {
        ...
        int oldAction2;
        MotionEvent ev2;
        MotionEvent ev3 = this;
        MotionEvent ev4 = obtain();
        synchronized (gSharedTempLock) {
            try {
                try {
                    oldPointerCount = nativeGetPointerCount(ev3.mNativePtr);
                    ensureSharedTempPointerCapacity(oldPointerCount);
                    pp = gSharedTempPointerProperties;
                    pc = gSharedTempPointerCoords;
                    map = gSharedTempPointerIndexMap;
                    oldAction = nativeGetAction(ev3.mNativePtr);
            ...
            while (true) {
                ...
                try {
                    nativeGetPointerProperties(ev3.mNativePtr, i, pp[newPointerCount]);
                    int idBit = 1 << pp[newPointerCount].f546id;
                    if ((idBit & idBits) != 0) {
            ...
            try {
                if (oldActionMasked != 5 && oldActionMasked != 6) {
                    newAction = oldAction;
                    historySize = nativeGetHistorySize(ev3.mNativePtr);
                    h = 0;
                    while (h <= historySize) {
                        int historyPos = h == historySize ? Integer.MIN_VALUE : h;
                        for (int i2 = 0; i2 < newPointerCount; i2++) {
                            nativeGetPointerCoords(ev3.mNativePtr, map[i2], historyPos, pc[i2]);
                        }
                        try {
                            long eventTimeNanos = nativeGetEventTimeNanos(ev3.mNativePtr, historyPos);
                            if (h == 0) {
                                int nativeGetDeviceId = nativeGetDeviceId(ev3.mNativePtr);
                                int nativeGetSource = nativeGetSource(ev3.mNativePtr);
                                int nativeGetDisplayId = nativeGetDisplayId(ev3.mNativePtr);
                                int nativeGetFlags = nativeGetFlags(ev3.mNativePtr);
                                int h3 = h;
                                int nativeGetEdgeFlags = nativeGetEdgeFlags(ev3.mNativePtr);
                                int historySize3 = historySize;
                                int nativeGetMetaState = nativeGetMetaState(ev3.mNativePtr);
                                int newPointerCount3 = newPointerCount;
                                int nativeGetButtonState = nativeGetButtonState(ev3.mNativePtr);
                                int oldActionPointerIndex3 = oldActionPointerIndex;
                                int nativeGetClassification = nativeGetClassification(ev3.mNativePtr);
                                oldActionMasked2 = oldActionMasked;
                                float nativeGetXOffset = nativeGetXOffset(ev3.mNativePtr);
                                oldAction2 = oldAction;
                                float nativeGetYOffset = nativeGetYOffset(ev3.mNativePtr);
                                float nativeGetXPrecision = nativeGetXPrecision(ev3.mNativePtr);
                                float nativeGetYPrecision = nativeGetYPrecision(ev3.mNativePtr);
                                long nativeGetDownTimeNanos = nativeGetDownTimeNanos(ev3.mNativePtr);
                                h2 = h3;
                                ...
                                oldAction2 = oldAction;
                                ev2 = ev4;
                                nativeAddBatch(ev2.mNativePtr, eventTimeNanos, pc, 0);
                            }
                            h = h2 + 1;
                            ev4 = ev2;
                    ...
                    if (CoreRune.MT_SUPPORT_COMPAT_SANDBOX) {
                        if (shouldApplyCompatSandbox()) {
                            ev = ev5;
                            ev.setCompatSandboxScale(this.mCompatSandboxXOffset, this.mCompatSandboxYOffset, this.mCompatSandboxScale);
                            return ev;
                        }
                    }
                    ev = ev5;
    ...
    public static final class PointerCoords {
        ...
        public void copyFrom(PointerCoords other) {
            ...
            this.f544x = other.f544x;
            this.f545y = other.f545y;
    ...
    public static final class PointerProperties {
        ...
        public boolean equals(PointerProperties other) {
            return other != null && this.f546id == other.f546id && this.toolType == other.toolType;
        }

        public int hashCode() {
            return this.f546id | (this.toolType << 8);
        }
    }
}
