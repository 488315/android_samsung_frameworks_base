...
public class PathInterpolator extends BaseInterpolator implements NativeInterpolator {
  ...
  /* renamed from: mX */
  private float[] f576mX;

  /* renamed from: mY */
  private float[] f577mY;
  ...
  public PathInterpolator(Resources res, Resources.Theme theme, AttributeSet attrs) {
    TypedArray a;
    if (theme != null) {
      a = theme.obtainStyledAttributes(attrs, C4337R.styleable.PathInterpolator, 0, 0);
    } else {
      a = res.obtainAttributes(attrs, C4337R.styleable.PathInterpolator);
    }
  ...
  private void initPath(Path path) {
    ...
    this.f576mX = new float[numPoints];
    this.f577mY = new float[numPoints];
    float prevX = 0.0f;
  ...
  public float getInterpolation(float t) {
    ...
    int startIndex = 0;
    int endIndex = this.f576mX.length - 1;
    while (endIndex - startIndex > 1) {
      int midIndex = (startIndex + endIndex) / 2;
      if (t < this.f576mX[midIndex]) {
        endIndex = midIndex;
      } else {
        startIndex = midIndex;
      }
    }
    float[] fArr = this.f576mX;
    float f = fArr[endIndex];
    float f2 = fArr[startIndex];
    float xRange = f - f2;
    if (xRange == 0.0f) {
      return this.f577mY[startIndex];
    }
    float tInRange = t - f2;
    float fraction = tInRange / xRange;
    float[] fArr2 = this.f577mY;
  ...
  public long createNativeInterpolator() {
    return NativeInterpolatorFactory.createPathInterpolator(this.f576mX, this.f577mY);
  }
}
