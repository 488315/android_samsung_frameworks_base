...
public final class PointerIcon implements Parcelable {
    ...
    if (type >= 10000 || (CoreRune.FW_DIRECT_WRITING && type == 1022)) {
      a =
          context.obtainStyledAttributes(
              null,
              C4337R.styleable.DeviceDefault_Pointer,
              C4337R.attr.zzz_DeviceDefaultPointerStyle,
              0);
    } else {
      int defStyleRes = sUseLargeIcons ? C4337R.style.LargePointer : C4337R.style.Pointer;
      a = context.obtainStyledAttributes(null, C4337R.styleable.Pointer, 0, defStyleRes);
    }
    int resourceId = a.getResourceId(typeIndex, -1);
    a.recycle();
    if (resourceId == -1) {
      Log.m102w(TAG, "Missing theme resources for pointer icon type " + type);
  ...
  /* JADX INFO: Access modifiers changed from: private */
  public static PointerIcon createDefaultIcon(
  ...
  public PointerIcon load(Context context) {
    ...
    try {
      ...
      result.loadResource(
          displayContext, displayContext.getResources(), this.mSystemIconResourceId);
      return result;
  ...
  public boolean equals(Object other) {
    ...
    if (this.mType != otherIcon.mType
        || (i = this.mSystemIconResourceId) != otherIcon.mSystemIconResourceId) {
      return false;
    }
    if (i != 0
        || ((this.mBitmap == null
  ...
  private void loadResource(Context context, Resources resources, int resourceId) {
    ...
    try {
      try {
        ...
        TypedArray a = resources.obtainAttributes(parser, C4337R.styleable.PointerIcon);
        ...
        if (drawable instanceof AnimationDrawable) {
          ...
          if (frames != 1) {
            ...
            for (int i = 1; i < frames; i++) {
              ...
              if (!(drawableFrame instanceof BitmapDrawable)) {
                throw new IllegalArgumentException(
                    "Frame of an animated pointer icon must refer to a bitmap drawable.");
              }
              if (drawableFrame.getIntrinsicWidth() != width
                ...
                throw new IllegalArgumentException(
                    "The bitmap size of "
                        + i
                        + "-th frame is different. All frames should have the exact same size and"
            ...
            Log.m102w(
                TAG,
                "Animation icon with single frame -- simply treating the first frame as a normal"
                    + " bitmap icon.");
            drawable2 = drawable3;
          }
          drawable = drawable2;
        ...
        boolean changeColor =
            (sUseLargeIcons || sPointerIconColor == this.mPointerIconColor || this.mType >= 20000)
                ? false
                : true;
        if (drawable instanceof BitmapDrawable) {
          ...
          if (changeColor) {
            ...
            paint.setColorFilter(
                new PorterDuffColorFilter(this.mPointerIconColor, PorterDuff.Mode.MULTIPLY));
            Canvas canvas = new Canvas(bitmap);
          ...
          throw new IllegalArgumentException(
              "<pointer-icon> bitmap attribute must refer to a bitmap or vector drawable.");
        }
        float f = this.mPointerIconSizeScale;
  ...
  public String toString() {
    return "PointerIcon{type="
        ...
        + ", hotspotY="
        + this.mHotSpotY
        + ", systemIconResourceId="
        + this.mSystemIconResourceId
        + "}";
  }
  ...
  private static void registerDisplayListener(Context context) {
    sDisplayListener =
        new DisplayManager.DisplayListener() { // from class: android.view.PointerIcon.2
  ...
  public static void setDefaultPointerIconInternal(int toolType, PointerIcon icon, boolean forced) {
    try {
      if (getInputManagerService() == null) {
        Log.m94d(TAG, "setDefaultPointerIconInternal failed to get IMS");
        return;
      }
