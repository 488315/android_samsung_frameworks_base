...
public class AccessibilityCache {
    ...
    public void addWindow(AccessibilityWindowInfo window) {
        synchronized (this.mLock) {
            if (!this.mEnabled) {
                ...
                if (DEBUG) {
                    Log.m98i(LOG_TAG, "Caching window: " + window.getId() + " at display Id [ " + window.getDisplayId() + " ]");
                }
    ...
    */
    ...
    private void clearNodesForWindowLocked(int windowId) {
        if (DEBUG) {
            Log.m98i(LOG_TAG, "clearNodesForWindowLocked(" + windowId + NavigationBarInflaterView.KEY_CODE_END);
        }
        LongSparseArray<AccessibilityNodeInfo> nodes = this.mNodeCache.get(windowId);
        if (nodes == null) {
            return;
                        ...
                        while (j2 < nodeCount) {
                            ...
                            if (!seen.add(node)) {
                                ...
                                if (node.isAccessibilityFocused()) {
                                    if (accessFocus != null) {
                                        Log.m96e(LOG_TAG, "Duplicate accessibility focus:" + node + " in window:" + windowId);
                                    } else {
                                        accessFocus = node;
                                    }
                                }
                                ...
                                if (nodeParent == null) {
                                    ...
                                    if (k == 0) {
                                        Log.m96e(LOG_TAG, "Invalid parent-child relation between parent: " + nodeParent + " and child: " + node);
                                    }
                                }
                                ...
                                while (k3 < childCount3) {
                                    ...
                                    if (child == null) {
                                        ...
                                        if (parent == node) {
                                            ...
                                            childCount = childCount3;
                                            Log.m96e(LOG_TAG, "Invalid child-parent relation between child: " + node + " and parent: " + nodeParent);
                                        }
                                    }
                                    k3++;
