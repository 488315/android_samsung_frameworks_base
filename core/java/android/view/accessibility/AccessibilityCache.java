...
public class AccessibilityCache {
  ...
  public void setWindowsOnAllDisplays(
      SparseArray<List<AccessibilityWindowInfo>> windowsOnAllDisplays, long populationTimeStamp) {
    synchronized (this.mLock) {
      if (!this.mEnabled) {
        if (DEBUG) {
          Log.m98i(LOG_TAG, "Cache is disabled");
        }
        return;
      }
      if (DEBUG) {
        Log.m98i(LOG_TAG, "Set windows");
      }
  ...
  */
  public void onAccessibilityEvent(AccessibilityEvent event) {
    ...
    synchronized (this.mLock) {
      ...
      boolean z = DEBUG;
      if (z) {
        Log.m98i(LOG_TAG, "onAccessibilityEvent(" + event + NavigationBarInflaterView.KEY_CODE_END);
      }
      ...
      switch (eventType) {
        ...
        case 8192:
          ...
          if (nodeToRefresh != null) {
            if (z) {
              Log.m98i(LOG_TAG, "Refreshing and re-adding cached node.");
            }
  ...
  private AccessibilityNodeInfo removeCachedNodeLocked(int windowId, long sourceId) {
    ...
    if (DEBUG) {
      Log.m98i(LOG_TAG, "Removing cached node.");
    }
  ...
  public AccessibilityNodeInfo getNode(int windowId, long accessibilityNodeId) {
    synchronized (this.mLock) {
      ...
      if (VERBOSE) {
        Log.m98i(LOG_TAG, "get(0x" + Long.toHexString(accessibilityNodeId) + ") = " + info);
      }
      return info;
    }
  }
  ...
  public SparseArray<List<AccessibilityWindowInfo>> getWindowsOnAllDisplays() {
    ...
    synchronized (this.mLock) {
      ...
      for (int i = 0; i < displayCounts; i++) {
        ...
        SparseArray<AccessibilityWindowInfo> windowsOfDisplay =
            this.mWindowCacheByDisplay.valueAt(i);
  ...
  public void add(AccessibilityNodeInfo info) {
    synchronized (this.mLock) {
      ...
      if (VERBOSE) {
        Log.m98i(LOG_TAG, "add(" + info + NavigationBarInflaterView.KEY_CODE_END);
      }
      int windowId = info.getWindowId();
      LongSparseArray<AccessibilityNodeInfo> nodes = this.mNodeCache.get(windowId);
      if (nodes == null) {
  ...
  private void clearWindowCacheLocked() {
    if (DEBUG) {
      Log.m98i(LOG_TAG, "clearWindowCacheLocked");
    }
    int displayCounts = this.mWindowCacheByDisplay.size();
    if (displayCounts > 0) {
  ...
  public AccessibilityNodeInfo getFocus(int focusType, long initialNodeId, int windowId) {
    ...
    synchronized (this.mLock) {
      ...
      if (initialNodeId != currentFocusId) {
        ...
        if (!isCachedNodeOrDescendantLocked(
            currentFocusedNode3.getParentNodeId(), initialNodeId, nodes)) {
          if (VERBOSE) {
            ...
            Log.m98i(LOG_TAG, append.append(str2).toString());
          }
          return null;
        }
      } else {
      ...
      if (!VERBOSE) {
        ...
        StringBuilder append2 =
            new StringBuilder()
                .append("getFocus(0x")
                .append(Long.toHexString(currentFocusId))
                .append(") = ")
                .append(currentFocusedNode2)
                .append(" with type: ");
        if (focusType == 2) {
        ...
        Log.m98i(LOG_TAG, append2.append(str).toString());
      }
      return new AccessibilityNodeInfo(currentFocusedNode2);
    }
  }

  private boolean isCachedNodeOrDescendantLocked(
      long nodeId, long ancestorId, LongSparseArray<AccessibilityNodeInfo> nodes) {
  ...
  private void clearNodesForWindowLocked(int windowId) {
    if (DEBUG) {
      Log.m98i(
          LOG_TAG,
          "clearNodesForWindowLocked(" + windowId + NavigationBarInflaterView.KEY_CODE_END);
    }
    LongSparseArray<AccessibilityNodeInfo> nodes = this.mNodeCache.get(windowId);
    if (nodes == null) {
      return;
  ...
  private void clearSubTreeLocked(int windowId, long rootNodeId) {
    if (DEBUG) {
      Log.m98i(LOG_TAG, "Clearing cached subtree.");
    }
    LongSparseArray<AccessibilityNodeInfo> nodes = this.mNodeCache.get(windowId);
    if (nodes != null) {
  ...
  private boolean clearSubTreeRecursiveLocked(
      LongSparseArray<AccessibilityNodeInfo> nodes, long rootNodeId) {
  ...
  public void checkIntegrity() {
    ...
    synchronized (accessibilityCache.mLock) {
      if (accessibilityCache.mWindowCacheByDisplay.size() > 0
          || accessibilityCache.mNodeCache.size() != 0) {
        ...
        for (int i = 0; i < displayCounts3; i++) {
          SparseArray<AccessibilityWindowInfo> windowsOfDisplay =
              accessibilityCache.mWindowCacheByDisplay.valueAt(i);
          if (windowsOfDisplay != null) {
            int windowCount = windowsOfDisplay.size();
            for (int j = 0; j < windowCount; j++) {
              AccessibilityWindowInfo window = windowsOfDisplay.valueAt(j);
              if (window.isActive()) {
                if (activeWindow3 != null) {
                  Log.m96e(LOG_TAG, "Duplicate active window:" + window);
                } else {
              ...
              if (window.isFocused()) {
                if (focusedWindow3 != null) {
                  Log.m96e(LOG_TAG, "Duplicate focused window:" + window);
                } else {
          ...
          if (nodes.size() <= 0) {
            ...
            while (j2 < nodeCount) {
              ...
              if (!seen.add(node)) {
                ...
                Log.m96e(LOG_TAG, "Duplicate node: " + node + " in window:" + windowId);
                ...
                if (node.isAccessibilityFocused()) {
                  if (accessFocus != null) {
                    Log.m96e(
                        LOG_TAG,
                        "Duplicate accessibility focus:" + node + " in window:" + windowId);
                  } else {
                    accessFocus = node;
                  }
                }
                if (node.isFocused()) {
                  if (inputFocus2 != null) {
                    Log.m96e(LOG_TAG, "Duplicate input focus: " + node + " in window:" + windowId);
                  } else {
                    inputFocus2 = node;
                  }
                }
                ...
                if (nodeParent == null) {
                  ...
                  if (k == 0) {
                    Log.m96e(
                        LOG_TAG,
                        "Invalid parent-child relation between parent: "
                            + nodeParent
                            + " and child: "
                            + node);
                  }
                ...
                while (k3 < childCount3) {
                  ...
                  if (child == null) {
                    ...
                    if (parent == node) {
                      ...
                      Log.m96e(
                          LOG_TAG,
                          "Invalid child-parent relation between child: "
                              + node
                              + " and parent: "
                              + nodeParent);
                    }
