...
public class NotificationHeaderView extends RelativeLayout {
  ...
  public NotificationHeaderView(
    ...
    this.mTouchableHeight =
        res.getDimensionPixelSize(C4337R.dimen.notification_header_touchable_height);
    this.mEntireHeaderClickable =
        res.getBoolean(C4337R.bool.config_notificationHeaderClickableForExpand);
  }
  ...
  protected void onFinishInflate() {
    ...
    this.mTopLineView = (NotificationTopLineView) findViewById(C4337R.id.notification_top_line);
    this.mExpandButton = (NotificationExpandButton) findViewById(C4337R.id.expand_button);
    this.mAltExpandTarget = findViewById(C4337R.id.alternate_expand_target);
  ...
  public void styleTextAsTitle(boolean styleTextAsTitle) {
    int styleResId;
    if (styleTextAsTitle) {
      styleResId = C4337R.style.TextAppearance_DeviceDefault_Notification_Title;
    } else {
      styleResId = C4337R.style.TextAppearance_DeviceDefault_Notification_Info;
    }
    View headerText = findViewById(C4337R.id.header_text);
    if (headerText instanceof TextView) {
      ((TextView) headerText).setTextAppearance(styleResId);
    }
    View appNameText = findViewById(C4337R.id.app_name_text);
  ...
  public class HeaderTouchListener implements View.OnTouchListener {
    ...
    public void bindTouchRects() {
      ...
      addWidthRect();
      this.mTouchSlop =
          ViewConfiguration.get(NotificationHeaderView.this.getContext()).getScaledTouchSlop();
    }
        ...
        case 1:
          if (this.mTrackGesture) {
            ...
            if (!NotificationHeaderView.this.mTopLineView.onTouchUp(
                x - topLineX, y - topLineY, this.mDownX - topLineX, this.mDownY - topLineY)) {
