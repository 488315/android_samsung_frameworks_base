    ...
    public void addOnDeviceTranslationCapabilityUpdateListener(Executor executor, Consumer<TranslationCapability> capabilityListener) {
        ...
        synchronized (this.mLock) {
            if (this.mCapabilityCallbacks.containsKey(capabilityListener)) {
                Log.m102w(TAG, "addOnDeviceTranslationCapabilityUpdateListener: the listener for " + capabilityListener + " already registered; ignoring.");
                return;
            }
    ...
    public void removeOnDeviceTranslationCapabilityUpdateListener(Consumer<TranslationCapability> capabilityListener) {
        ...
        synchronized (this.mLock) {
            ...
            if (remoteCallback == null) {
                Log.m102w(TAG, "removeOnDeviceTranslationCapabilityUpdateListener: the capability listener not found; ignoring.");
                return;
            }
            try {
        ...
        synchronized (this.mLock) {
            ...
            if (this.mTranslationCapabilityUpdateListeners.containsKey(formatPair)) {
                ...
                if (intents.contains(pendingIntent)) {
                    ...
                    Log.m102w(TAG, "pending intent=" + pendingIntent + " does not exist in mTranslationCapabilityUpdateListeners");
                }
            } else {
                Log.m102w(TAG, "format pair=" + formatPair + " does not exist in mTranslationCapabilityUpdateListeners");
            }
        }
    }
