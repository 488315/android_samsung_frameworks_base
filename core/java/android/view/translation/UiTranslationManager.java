  ...
  public void registerUiTranslationStateCallback(
    ...
    synchronized (this.mCallbacks) {
      if (this.mCallbacks.containsKey(callback)) {
        Log.m102w(
            TAG, "registerUiTranslationStateCallback: callback already registered; ignoring.");
        return;
      }
  ...
  public void unregisterUiTranslationStateCallback(UiTranslationStateCallback callback) {
    ...
    synchronized (this.mCallbacks) {
      ...
      if (remoteCallback == null) {
        Log.m102w(TAG, "unregisterUiTranslationStateCallback: callback not found; ignoring.");
        return;
      }
      try {
  ...
  public void onTranslationFinished(
    ...
    try {
      this.mService.onTranslationFinished(
          activityDestroyed, activityId.getToken(), componentName, this.mContext.getUserId());
    } catch (RemoteException e) {
      throw e.rethrowFromSystemServer();
    }
  }
      ...
      this.mExecutor.execute(
          new Runnable() { // from class:
            ...
            public final void run() {
              UiTranslationManager.UiTranslationStateRemoteCallback.this.lambda$sendResult$0(
                  bundle);
            }
          });
    }
      ...
      Binder.withCleanCallingIdentity(
          new FunctionalUtils
            ...
            public final void runOrThrow() {
              UiTranslationManager.UiTranslationStateRemoteCallback.this.lambda$sendResult$1(
                  bundle);
            }
          });
    }
    ...
    public void lambda$sendResult$0(Bundle bundle) {
      ...
      switch (state) {
        case 0:
          this.mSourceLocale =
              (ULocale)
                  bundle.getSerializable(UiTranslationManager.EXTRA_SOURCE_LOCALE, ULocale.class);
          ULocale uLocale =
              (ULocale)
                  bundle.getSerializable(UiTranslationManager.EXTRA_TARGET_LOCALE, ULocale.class);
