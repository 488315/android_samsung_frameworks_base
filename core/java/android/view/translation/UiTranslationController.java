...
public class UiTranslationController implements Dumpable {
    ...
    public void updateUiTranslationState(int state, TranslationSpec sourceSpec, TranslationSpec targetSpec, List<AutofillId> views, UiTranslationSpec uiTranslationSpec) {
        if (this.mActivity.isDestroyed()) {
            Log.m98i("UiTranslationController", "Cannot update " + stateToString(state) + " for destroyed " + this.mActivity);
            return;
        }
        boolean isLoggable = Log.isLoggable(UiTranslationManager.LOG_TAG, 3);
        Log.m98i("UiTranslationController", "updateUiTranslationState state: " + stateToString(state) + (isLoggable ? ", views: " + views + ", spec: " + uiTranslationSpec : ""));
        ...
        switch (state) {
            ...
            case 3:
                ...
                return;
            default:
                Log.m102w("UiTranslationController", "onAutoTranslationStateChange(): unknown state: " + state);
                return;
        }
    }
    ...
    public void onActivityDestroyed() {
        synchronized (this.mLock) {
            Log.m98i("UiTranslationController", "onActivityDestroyed(): mCurrentState is " + stateToString(this.mCurrentState));
    ...
    private void onVirtualViewTranslationCompleted(SparseArray<LongSparseArray<ViewTranslationResponse>> translatedResult) {
        ...
        if (this.mActivity.isDestroyed()) {
            Log.m100v("UiTranslationController", "onTranslationCompleted:" + this.mActivity + "is destroyed.");
            return;
        }
        synchronized (this.mLock) {
            if (this.mCurrentState == 3) {
                Log.m102w("UiTranslationController", "onTranslationCompleted: the translation state is finished now. Skip to show the translated text.");
                return;
            }
            for (int i = 0; i < translatedResult.size(); i++) {
                ...
                if (viewRef != null) {
                    ...
                    if (view == null) {
                        Log.m102w("UiTranslationController", "onTranslationCompleted: the view for autofill id " + autofillId + " may be gone.");
                    } else {
                        LongSparseArray<ViewTranslationResponse> virtualChildResponse = translatedResult.valueAt(i);
                        if (isLoggable) {
                            Log.m100v("UiTranslationController", "onVirtualViewTranslationCompleted: received response for AutofillId " + autofillId);
                        }
    ...
    static /* synthetic */ void lambda$onVirtualViewTranslationCompleted$3(View view, boolean isLoggable) {
        if (view.getViewTranslationCallback() == null) {
            if (isLoggable) {
                Log.m94d("UiTranslationController", view + " doesn't support showing translation because of null ViewTranslationCallback.");
            }
    ...
    private void onTranslationCompleted(SparseArray<ViewTranslationResponse> translatedResult) {
        ...
        if (uiTranslationController.mActivity.isDestroyed()) {
            Log.m100v("UiTranslationController", "onTranslationCompleted:" + uiTranslationController.mActivity + "is destroyed.");
            return;
        }
        int resultCount = translatedResult.size();
        if (isLoggable2) {
            Log.m100v("UiTranslationController", "onTranslationCompleted: receive " + resultCount + " responses.");
        }
                    ...
                    if (uiTranslationController.mCurrentState != 3) {
                        ...
                        while (i < resultCount) {
                            try {
                                ...
                                if (isLoggable2) {
                                    try {
                                        Log.m100v("UiTranslationController", "onTranslationCompleted: " + sanitizedViewTranslationResponse(response));
                                    } catch (Throwable th) {
                                        th = th;
                                    }
                                }
                                ...
                                if (autofillId == null) {
                                    Log.m102w("UiTranslationController", "No AutofillId is set in ViewTranslationResponse");
    ...
    public /* synthetic */ void lambda$onTranslationCompleted$4(View view, ViewTranslationResponse response, boolean isLoggable, AutofillId autofillId, int currentState) {
        ...
        if (view.getViewTranslationResponse() != null && view.getViewTranslationResponse().equals(response) && (callback instanceof TextViewTranslationCallback)) {
            ...
            if (textViewCallback.isShowingTranslation() || textViewCallback.isAnimationRunning()) {
                if (isLoggable) {
                    Log.m94d("UiTranslationController", "Duplicate ViewTranslationResponse for " + autofillId + ". Ignoring.");
                    return;
                }
                return;
            }
    ...
    public void createTranslatorAndStart(TranslationSpec sourceSpec, TranslationSpec targetSpec, List<AutofillId> views) {
        ...
        if (translator == null) {
            Log.m102w("UiTranslationController", "Can not create Translator for sourceSpec:" + sourceSpec + " targetSpec:" + targetSpec);
        } else {
    ...
    */
    static /* synthetic */ void lambda$runForEachView$7(ArrayMap views, boolean isLoggable, BiConsumer action) {
        try {
            ...
            while (i < viewCounts) {
                ...
                if (isLoggable) {
                    Log.m94d("UiTranslationController", "runForEachView for autofillId = " + (view != null ? view.getAutofillId() : " null"));
                }
    ...
    private Translator createTranslatorIfNeeded(TranslationSpec sourceSpec, TranslationSpec targetSpec) {
        ...
        if (tm == null) {
            Log.m96e("UiTranslationController", "Can not find TranslationManager when trying to create translator.");
            return null;
        }
