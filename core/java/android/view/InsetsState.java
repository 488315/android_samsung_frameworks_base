...
public class InsetsState implements Parcelable {
    ...
    while (i4 >= 0) {
      ...
      if (type == WindowInsets.Type.ime()) {
        ...
        if (ignoringVisibilitySource == null) {
          ...
          i = legacyWindowFlags;
          processSource(
              ignoringVisibilitySource, relativeFrameMax, true, typeMaxInsetsMap, null, null);
        }
      }
      i4 = i2 - 1;
      insetsState = ignoringVisibilityState;
  ...
  public boolean equals(
      Object o, boolean excludingCaptionInsets, boolean excludeInvisibleImeFrames) {
    if (this == o) {
      return true;
    }
    ...
    if (!this.mDisplayFrame.equals(state.mDisplayFrame)
        || !this.mDisplayCutout.equals(state.mDisplayCutout)
        || !this.mRoundedCorners.equals(state.mRoundedCorners)
        || !this.mRoundedCornerFrame.equals(state.mRoundedCornerFrame)
        || !this.mPrivacyIndicatorBounds.equals(state.mPrivacyIndicatorBounds)
        || !this.mDisplayShape.equals(state.mDisplayShape)) {
      return false;
    }
    ...
    while (thisIndex < thisSize && thatIndex < thatSize) {
      ...
      while (true) {
        ...
        if (thisSource == null
            || !((excludingCaptionInsets && thisSource.getType() == WindowInsets.Type.captionBar())
                || (excludeInvisibleImeFrames
                    && thisSource.getType() == WindowInsets.Type.ime()
      ...
      while (thatSource != null
          && ((excludingCaptionInsets && thatSource.getType() == WindowInsets.Type.captionBar())
              || (excludeInvisibleImeFrames
                  && thatSource.getType() == WindowInsets.Type.ime()
