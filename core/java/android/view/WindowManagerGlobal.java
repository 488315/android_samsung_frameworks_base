...
public final class WindowManagerGlobal {
    ...
    public void addView(View view, ViewGroup.LayoutParams params, Display display, Window parentWindow, int userId) {
        ...
        if (this.mViews.size() >= 300) {
            synchronized (this.mLock) {
                ...
                for (int j = 0; i2 >= 0 && j < 50; j++) {
                    Log.m94d(TAG, "addedView(" + i2 + NavigationBarInflaterView.KEY_CODE_END + this.mViews.get(i2));
                    if (this.mParams.get(i2) != null) {
                        Log.m94d(TAG, "addedParams(" + i2 + NavigationBarInflaterView.KEY_CODE_END + this.mParams.get(i2) + " / Title: " + ((Object) this.mParams.get(i2).getTitle()));
                    }
                    i2--;
                }
            }
        ...
        if (wparams.type != 1 && wparams.type != 3) {
            Log.m98i(TAG, "WindowManagerGlobal#addView, ty=" + wparams.type + ", view=" + view + ", caller=" + Debug.getCallers(3));
        }
    ...
    private void removeViewLocked(int index, boolean immediate) {
        ...
        if (root != null && (type = root.mWindowAttributes.type) != 1 && type != 3) {
            Log.m98i(TAG, "WindowManagerGlobal#removeView, ty=" + type + ", view=" + view + ", caller=" + Debug.getCallers(3));
        }
        if (root != null) {
