...
public final class WindowManagerGlobal {
          ...
          sWindowSession =
              windowManager.openSession(
                  new IWindowSessionCallback
                      .Stub() { // from class: android.view.WindowManagerGlobal.1
  ...
  public void addView(
      View view, ViewGroup.LayoutParams params, Display display, Window parentWindow, int userId) {
    ViewRootImpl root;
    View decorView;
    if (view == null) {
    ...
    if (this.mViews.size() >= 300) {
      synchronized (this.mLock) {
        ...
        for (int j = 0; i2 >= 0 && j < 50; j++) {
          Log.m94d(
              TAG,
              "addedView(" + i2 + NavigationBarInflaterView.KEY_CODE_END + this.mViews.get(i2));
          if (this.mParams.get(i2) != null) {
            Log.m94d(
                ...
                "addedParams("
                    ...
                    + this.mParams.get(i2)
                    + " / Title: "
                    + ((Object) this.mParams.get(i2).getTitle()));
          }
          i2--;
        }
      }
    ...
    synchronized (this.mLock) {
      ...
      if (windowlessSession == null) {
        ...
        root =
            new ViewRootImpl(
                view.getContext(), display, windowlessSession, new WindowlessWindowLayout());
      }
  ...
  public void removeView(View view, boolean immediate) {
    ...
    synchronized (this.mLock) {
      ...
      if (curView != view) {
        throw new IllegalStateException(
            "Calling with view " + view + " but the ViewAncestor is attached to " + curView);
      }
    }
  }
      ...
      synchronized (this.mLock) {
        try {
          ...
          while (i2 < count) {
            ...
            objArr[2] = Float.valueOf(info.renderNodeMemoryUsage / 1024.0f);
  ...
  public void registerProposedRotationListener(
    ...
    synchronized (this.mLock) {
      ...
      if (delegate.add(executor, listener)) {
        ...
        try {
          int currentRotation =
              getWindowManagerService().registerProposedRotationListener(contextToken, delegate);
