    ...
    synchronized (sLock) {
      ...
      if (this.mInternalListenerAttached && this.mListeners.size() == 0) {
        try {
          ...
          Log.m94d(TAG, "Could not unregister ICrossWindowBlurEnabledListener");
        }
      }
    }
  }
  ...
  private void attachInternalListenerIfNeededLocked() {
    if (!this.mInternalListenerAttached) {
      try {
        ...
        Log.m94d(TAG, "Could not register ICrossWindowBlurEnabledListener");
      }
    }
  }
      ...
      synchronized (CrossWindowBlurListeners.sLock) {
        ...
        for (int i = 0; i < CrossWindowBlurListeners.this.mListeners.size(); i++) {
          try {
            ...
            crossWindowBlurListeners.notifyListener(
                (Consumer) crossWindowBlurListeners.mListeners.keyAt(i),
