  ...
  private static boolean isRTLText(String sCurStr, int newCursorPosition, View targetView) {
    ...
    if (sCurStr != null && sCurStr.length() > 0) {
      ...
      if (nDirection == -1) {
        ...
        if (nDirection == -1) {
          for (int nIndex4 = newCursorPosition;
              nIndex4 < sCurStr.length() && (ch = sCurStr.charAt(nIndex4)) != '\n';
              nIndex4++) {
        ...
        if (nDirection == -1
            && targetView != null
            && targetView.getLayoutDirection() == 1
            && isRtlLanguage()) {
          nDirection = 1;
        }
      }
