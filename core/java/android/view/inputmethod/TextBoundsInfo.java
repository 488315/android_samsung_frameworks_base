...
public final class TextBoundsInfo implements Parcelable {
      ...
      if (graphemeStart2 >= lineStart) {
        float cursorPosition =
            getCursorHorizontalPosition(
                graphemeStart2, lineStart, lineEnd, lineBounds.left, lineBounds.right);
  ...
  public int[] getRangeForRect(
      RectF area, SegmentFinder segmentFinder, Layout.TextInclusionStrategy inclusionStrategy) {
    int lineStart;
    ...
    while (lineStart2 != -1 && start == -1) {
      start =
          getStartForRectWithinLine(lineStart2, lineEnd, area, segmentFinder, inclusionStrategy);
  ...
  private int getStartForRectWithinLine(
    ...
    for (int index = lineStart; index < lineEnd; index++) {
      ...
      if (level != runLevel) {
        int start =
            getStartForRectWithinRun(runStart, index, area, segmentFinder, inclusionStrategy);
        if (start != -1) {
          return start;
        }
        int runStart2 = index;
  ...
  public void writeToParcel(Parcel dest, int flags) {
    ...
    encodeSegmentFinder(
        encodedFlags, 134217728, 67108864, this.mStart, this.mEnd, this.mGraphemeSegmentFinder);
    encodeSegmentFinder(
        encodedFlags, 536870912, 268435456, this.mStart, this.mEnd, this.mWordSegmentFinder);
    encodeSegmentFinder(
        encodedFlags,
        Integer.MIN_VALUE,
        1073741824,
  ...
  private TextBoundsInfo(Parcel source) {
    ...
    this.mGraphemeSegmentFinder =
        decodeSegmentFinder(encodedFlags, 134217728, 67108864, readInt, readInt2);
    this.mWordSegmentFinder =
        decodeSegmentFinder(encodedFlags, 536870912, 268435456, readInt, readInt2);
    this.mLineSegmentFinder =
        decodeSegmentFinder(encodedFlags, Integer.MIN_VALUE, 1073741824, readInt, readInt2);
    ...
    public TextBoundsInfo build() {
      ...
      if (fArr.length != (i - i2) * 4) {
        throw new IllegalStateException(
            "The length of characterBounds doesn't match the length of the given start and end."
                + " Expected length: "
      ...
      if (iArr.length != i - i2) {
        throw new IllegalStateException(
            "The length of characterFlags doesn't match the length of the given start and end."
                + " Expected length: "
      ...
      if (iArr2.length != i - i2) {
        throw new IllegalStateException(
            "The length of characterBidiLevels doesn't match the length of the given start and end."
                + " Expected length: "
                + (this.mEnd - this.mStart)
                + " characterFlags length: "
      ...
      if (!TextBoundsInfo.isLineDirectionFlagConsistent(iArr, segmentFinder, i2, i)) {
        throw new IllegalStateException(
            "characters in the same line must have the same FLAG_LINE_IS_RTL flag value.");
      }
  ...
  private static void encodeSegmentFinder(
    ...
    if ((end - start) + 1 != flags.length) {
      throw new IllegalStateException(
          "The given flags array must have the same length as the given range. flags length: "
              + flags.length
              + " range: ["
              + start
              + ", "
