...
public class BaseInputConnection implements InputConnection {
  ...
  public BaseInputConnection(View targetView, boolean fullEditor) {
    this.mIMM =
        (InputMethodManager) targetView.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
  ...
  public boolean deleteSurroundingTextInCodePoints(int beforeLength, int afterLength) {
    ...
    if (a >= 0
        && b >= 0
        && (start = findIndexBackward(content, a, Math.max(beforeLength, 0))) != INVALID_INDEX
        && (end = findIndexForward(content, b, Math.max(afterLength, 0))) != INVALID_INDEX) {
      int numDeleteBefore = a - start;
  ...
  public boolean commitContent(InputContentInfo inputContentInfo, int flags, Bundle opts) {
    ...
    ClipData clip =
        new ClipData(
            inputContentInfo.getDescription(), new ClipData.Item(inputContentInfo.getContentUri()));
    ContentInfo payload =
