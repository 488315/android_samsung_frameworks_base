...
public class RoundedCorners implements Parcelable {
  ...
  public static final RoundedCorners NO_ROUNDED_CORNERS =
      new RoundedCorners(
          new RoundedCorner(0), new RoundedCorner(1), new RoundedCorner(2), new RoundedCorner(3));
  ...
  public RoundedCorners(
    ...
    this.mRoundedCorners = new RoundedCorner[] {topLeft, topRight, bottomRight, bottomLeft};
  }
  ...
  public static RoundedCorners fromResources(
      ...
      int displayHeight) {
    return fromRadii(
        loadRoundedCornerRadii(res, displayUniqueId),
  ...
  public static RoundedCorners fromRadii(
      Pair<Integer, Integer> radii, int displayWidth, int displayHeight) {
    return fromRadii(radii, displayWidth, displayHeight, displayWidth, displayHeight);
  }

  private static RoundedCorners fromRadii(
      Pair<Integer, Integer> radii,
      int physicalDisplayWidth,
      int physicalDisplayHeight,
      int displayWidth,
      int displayHeight) {
    ...
    synchronized (CACHE_LOCK) {
      ...
      while (i < 4) {
        roundedCorners[i] =
            createRoundedCorner(i, i <= 1 ? topRadius : bottomRadius, displayWidth, displayHeight);
        i++;
      }
  ...
  private static Pair<Integer, Integer> loadRoundedCornerRadii(
    ...
    Pair<Integer, Integer> radii =
        new Pair<>(
            Integer.valueOf(radiusTop > 0 ? radiusTop : radiusDefault),
            Integer.valueOf(radiusBottom > 0 ? radiusBottom : radiusDefault));
    return radii;
  }
  ...
  public static int getRoundedCornerRadius(Resources res, String displayUniqueId) {
    ...
    TypedArray array = res.obtainTypedArray(C4337R.array.config_roundedCornerRadiusArray);
    if (index >= 0 && index < array.length()) {
      radius = array.getDimensionPixelSize(index, 0);
    } else {
      radius = res.getDimensionPixelSize(C4337R.dimen.rounded_corner_radius);
    }
    array.recycle();
    return radius;
  }
  ...
  public static int getRoundedCornerTopRadius(Resources res, String displayUniqueId) {
    int radius;
    int index = DisplayUtils.getDisplayUniqueIdConfigIndex(res, displayUniqueId);
    TypedArray array = res.obtainTypedArray(C4337R.array.config_roundedCornerTopRadiusArray);
    if (index >= 0 && index < array.length()) {
      radius = array.getDimensionPixelSize(index, 0);
    } else {
      radius = res.getDimensionPixelSize(C4337R.dimen.rounded_corner_radius_top);
    }
    array.recycle();
    return radius;
  }
  ...
  public static int getRoundedCornerBottomRadius(Resources res, String displayUniqueId) {
    int radius;
    int index = DisplayUtils.getDisplayUniqueIdConfigIndex(res, displayUniqueId);
    TypedArray array = res.obtainTypedArray(C4337R.array.config_roundedCornerBottomRadiusArray);
    if (index >= 0 && index < array.length()) {
      radius = array.getDimensionPixelSize(index, 0);
    } else {
      radius = res.getDimensionPixelSize(C4337R.dimen.rounded_corner_radius_bottom);
    }
    array.recycle();
    return radius;
  }
  ...
  public static int getRoundedCornerRadiusAdjustment(Resources res, String displayUniqueId) {
    int radius;
    int index = DisplayUtils.getDisplayUniqueIdConfigIndex(res, displayUniqueId);
    TypedArray array = res.obtainTypedArray(C4337R.array.config_roundedCornerRadiusAdjustmentArray);
    if (index >= 0 && index < array.length()) {
      radius = array.getDimensionPixelSize(index, 0);
    } else {
      radius = res.getDimensionPixelSize(C4337R.dimen.rounded_corner_radius_adjustment);
    }
    array.recycle();
    return radius;
  }
  ...
  public static int getRoundedCornerRadiusTopAdjustment(Resources res, String displayUniqueId) {
    ...
    int index = DisplayUtils.getDisplayUniqueIdConfigIndex(res, displayUniqueId);
    TypedArray array =
        res.obtainTypedArray(C4337R.array.config_roundedCornerTopRadiusAdjustmentArray);
    if (index >= 0 && index < array.length()) {
      radius = array.getDimensionPixelSize(index, 0);
    } else {
      radius = res.getDimensionPixelSize(C4337R.dimen.rounded_corner_radius_top_adjustment);
    }
    array.recycle();
    return radius;
  }
  ...
  public static int getRoundedCornerRadiusBottomAdjustment(Resources res, String displayUniqueId) {
    ...
    int index = DisplayUtils.getDisplayUniqueIdConfigIndex(res, displayUniqueId);
    TypedArray array =
        res.obtainTypedArray(C4337R.array.config_roundedCornerBottomRadiusAdjustmentArray);
    if (index >= 0 && index < array.length()) {
      radius = array.getDimensionPixelSize(index, 0);
    } else {
      radius = res.getDimensionPixelSize(C4337R.dimen.rounded_corner_radius_bottom_adjustment);
    }
    array.recycle();
    return radius;
  }
  ...
  public static boolean getBuiltInDisplayIsRound(Resources res, String displayUniqueId) {
    boolean isRound;
    int index = DisplayUtils.getDisplayUniqueIdConfigIndex(res, displayUniqueId);
    TypedArray array = res.obtainTypedArray(C4337R.array.config_builtInDisplayIsRoundArray);
    if (index >= 0 && index < array.length()) {
      isRound = array.getBoolean(index, false);
    } else {
      isRound = res.getBoolean(C4337R.bool.config_mainBuiltInDisplayIsRound);
    }
    array.recycle();
    return isRound;
  }
  ...
  public RoundedCorners insetWithFrame(Rect frame, Rect roundedCornerFrame) {
    ...
    while (i4 < i3) {
      if (roundedCorners2.mRoundedCorners[i4].isEmpty()) {
        ...
        switch (i4) {
          ...
          default:
            throw new IllegalArgumentException(
                "The position is not one of the RoundedCornerPosition =" + i4);
        }
        i = i4;
        roundedCorners = roundedCorners3;
        i2 = i3;
        roundedCorners[i] =
            insetRoundedCorner(
                i4, radius, centerX, centerY, insetLeft, insetTop, insetRight, insetBottom);
      }
      i4 = i + 1;
  ...
  public RoundedCorners inset(int insetLeft, int insetTop, int insetRight, int insetBottom) {
    ...
    for (int i = 0; i < 4; i++) {
      roundedCorners[i] =
          insetRoundedCorner(
              i,
              this.mRoundedCorners[i].getRadius(),
              this.mRoundedCorners[i].getCenter().f85x,
              this.mRoundedCorners[i].getCenter().f86y,
              insetLeft,
              insetTop,
              insetRight,
              insetBottom);
  ...
  public void setRoundedCorner(int position, RoundedCorner roundedCorner) {
    this.mRoundedCorners[position] =
        roundedCorner == null ? new RoundedCorner(position) : roundedCorner;
  }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      roundedCorners[i] =
          new RoundedCorner(
              ...
              (int) (roundedCorner.getCenter().f85x * scale),
              (int) (roundedCorner.getCenter().f86y * scale));
    }
    return new RoundedCorners(roundedCorners);
  }
  ...
  private static RoundedCorner createRoundedCorner(
      int position, int radius, int displayWidth, int displayHeight) {
    switch (position) {
      case 0:
        return new RoundedCorner(0, radius, radius > 0 ? radius : 0, radius > 0 ? radius : 0);
      case 1:
        return new RoundedCorner(
            1, radius, radius > 0 ? displayWidth - radius : 0, radius > 0 ? radius : 0);
      case 2:
        return new RoundedCorner(
            2,
            radius,
      ...
      case 3:
        return new RoundedCorner(
            3, radius, radius > 0 ? radius : 0, radius > 0 ? displayHeight - radius : 0);
      default:
        throw new IllegalArgumentException(
            "The position is not one of the RoundedCornerPosition =" + position);
    }
