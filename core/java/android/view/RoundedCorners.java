...
public class RoundedCorners implements Parcelable {
    ...
    public static int getRoundedCornerRadiusTopAdjustment(Resources res, String displayUniqueId) {
        int radius;
        int index = DisplayUtils.getDisplayUniqueIdConfigIndex(res, displayUniqueId);
        TypedArray array = res.obtainTypedArray(C4337R.array.config_roundedCornerTopRadiusAdjustmentArray);
        if (index >= 0 && index < array.length()) {
            radius = array.getDimensionPixelSize(index, 0);
        } else {
    ...
    public static int getRoundedCornerRadiusBottomAdjustment(Resources res, String displayUniqueId) {
        int radius;
        int index = DisplayUtils.getDisplayUniqueIdConfigIndex(res, displayUniqueId);
        TypedArray array = res.obtainTypedArray(C4337R.array.config_roundedCornerBottomRadiusAdjustmentArray);
        if (index >= 0 && index < array.length()) {
            radius = array.getDimensionPixelSize(index, 0);
        } else {
    ...
    public RoundedCorners inset(int insetLeft, int insetTop, int insetRight, int insetBottom) {
        RoundedCorner[] roundedCorners = new RoundedCorner[4];
        for (int i = 0; i < 4; i++) {
            roundedCorners[i] = insetRoundedCorner(i, this.mRoundedCorners[i].getRadius(), this.mRoundedCorners[i].getCenter().f85x, this.mRoundedCorners[i].getCenter().f86y, insetLeft, insetTop, insetRight, insetBottom);
        }
        return new RoundedCorners(roundedCorners);
    }
    ...
    public RoundedCorners scale(float scale) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            roundedCorners[i] = new RoundedCorner(i, (int) (roundedCorner.getRadius() * scale), (int) (roundedCorner.getCenter().f85x * scale), (int) (roundedCorner.getCenter().f86y * scale));
        }
        return new RoundedCorners(roundedCorners);
    }
