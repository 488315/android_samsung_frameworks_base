...
public class CutoutSpecification {
  ...
  public static class Parser {
    ...
    private void computeBoundsRectAndAddToRegion(Path p, Region inoutRegion, Rect inoutRect) {
      ...
      inoutRegion.m21op(inoutRect, Region.EnumC0836Op.UNION);
    }
    ...
    private void parseSvgPathSpec(Region region, String spec) {
      if (TextUtils.length(spec) < CutoutSpecification.MINIMAL_ACCEPTABLE_PATH_LENGTH) {
        Log.m96e(CutoutSpecification.TAG, "According to SVG definition, it shouldn't happen");
        return;
      }
      translateMatrix();
    ...
    public CutoutSpecification parse(String originalSpec) {
      ...
      if (dpIndex != -1) {
        spec =
            originalSpec.substring(0, dpIndex)
                + originalSpec.substring(CutoutSpecification.DP_MARKER.length() + dpIndex);
      } else {
        spec = originalSpec;
      }
      ...
      this.mInsets =
          Insets.m14of(
              this.mSafeInsetLeft, this.mSafeInsetTop, this.mSafeInsetRight, this.mSafeInsetBottom);
