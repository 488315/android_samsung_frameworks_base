...
public class Surface implements Parcelable {
      ...
      new Parcelable.Creator<Surface>() { // from class: android.view.Surface.1
        ...
        public Surface createFromParcel(Parcel source) {
          try {
            ...
            Log.m97e(Surface.TAG, "Exception creating surface from parcel", e);
            return null;
          }
        }
    ...
    synchronized (this.mLock) {
      ...
      lockCanvas =
          this.mHwuiContext.lockCanvas(
              nativeGetWidth(this.mNativeObject), nativeGetHeight(this.mNativeObject));
    }
    return lockCanvas;
  }
  ...
  public void attachAndQueueBufferWithColorSpace(HardwareBuffer buffer, ColorSpace colorSpace) {
    synchronized (this.mLock) {
      ...
      int err =
          nativeAttachAndQueueBufferWithColorSpace(this.mNativeObject, buffer, colorSpace.getId());
      if (err != 0) {
        throw new RuntimeException(
            "Failed to attach and queue buffer to Surface (bad object?), native error: " + err);
      }
    }
  }
  ...
  public void setFrameRate(float frameRate, int compatibility, int changeFrameRateStrategy) {
    synchronized (this.mLock) {
      checkNotReleasedLocked();
      int error =
          nativeSetFrameRate(this.mNativeObject, frameRate, compatibility, changeFrameRateStrategy);
  ...
  private final class CompatibleCanvas extends Canvas {
    ...
    public void setMatrix(Matrix matrix) {
      ...
      if (Surface.this.mCompatibleMatrix == null
          || (matrix2 = this.mOrigMatrix) == null
          || matrix2.equals(matrix)) {
