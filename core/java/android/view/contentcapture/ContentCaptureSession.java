...
public abstract class ContentCaptureSession implements AutoCloseable {
    ...
    public final ContentCaptureSession createContentCaptureSession(ContentCaptureContext context) {
        ...
        if (ContentCaptureHelper.sDebug) {
            Log.m94d(TAG, "createContentCaptureSession(" + context + ": parent=" + this.mId + ", child=" + child.mId);
        }
    ...
    public final void destroy() {
        synchronized (this.mLock) {
            if (this.mDestroyed) {
                if (ContentCaptureHelper.sDebug) {
                    Log.m94d(TAG, "destroy(" + this.mId + "): already destroyed");
                }
                return;
            }
            this.mDestroyed = true;
            if (ContentCaptureHelper.sVerbose) {
                Log.m100v(TAG, "destroy(): state=" + getStateAsString(this.mState) + ", mId=" + this.mId);
            }
            ArrayList<ContentCaptureSession> arrayList = this.mChildren;
            if (arrayList != null) {
                int numberChildren = arrayList.size();
                if (ContentCaptureHelper.sVerbose) {
                    Log.m100v(TAG, "Destroying " + numberChildren + " children first");
                }
                for (int i = 0; i < numberChildren; i++) {
                    ...
                    try {
                        ...
                        Log.m102w(TAG, "exception destroying child session #" + i + ": " + e);
                    }
                }
            }
            onDestroy();
