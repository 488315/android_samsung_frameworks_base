...
public final class InputEventConsistencyVerifier {
    ...
    try {
      ...
      if ((source & 4) != 0) {
        switch (action) {
          ...
          default:
            problem(
                "Invalid action " + MotionEvent.actionToString(action) + " for trackball event.");
            break;
        }
  ...
  public void onTouchEvent(MotionEvent event, int nestingLevel) {
    ...
    try {
      ...
      int source = event.getSource();
      if (!newStream
          && (i = this.mTouchEventStreamDeviceId) != -1
          && (i != deviceId || this.mTouchEventStreamSource != source)) {
        problem(
            "Touch event stream contains events from multiple sources: previous device id "
        ...
        switch (action) {
          case 0:
            if (this.mTouchEventStreamPointers != 0) {
              problem(
                  "ACTION_DOWN but pointers are already down.  Probably missing ACTION_UP from"
                      + " previous gesture.");
            }
            ensureHistorySizeIsZeroForThisAction(event);
            ensurePointerCountIsOneForThisAction(event);
          ...
          default:
            ...
            if (actionMasked == 5) {
              ...
              if (actionIndex >= 0 && actionIndex < pointerCount) {
                ...
                if ((i2 & idBit) != 0) {
                  problem(
                      "ACTION_POINTER_DOWN specified pointer id " + id + " which is already down.");
                  this.mTouchEventStreamIsTainted = true;
                } else {
        ...
        switch (action) {
          ...
          case 11:
            ...
            if ((this.mButtonsPressed & actionButton) != 0) {
              problem(
                  "Action button for ACTION_BUTTON_PRESS event is "
                      + actionButton
                      + ", but it has already been pressed and has yet to be released.");
            }
            ...
            if (this.mButtonsPressed != buttonState) {
              problem(
                  String.format(
                      "Reported button state differs from expected button state based on press and"
                          + " release events. Is 0x%08x but expected 0x%08x.",
                      Integer.valueOf(buttonState), Integer.valueOf(this.mButtonsPressed)));
              break;
            }
            break;
          case 12:
            ...
            if ((this.mButtonsPressed & actionButton) != actionButton) {
              problem(
                  "Action button for ACTION_BUTTON_RELEASE event is "
                      + actionButton
                      + ", but it was either never pressed or has already been released.");
            }
  ...
  private void finishEvent() {
    ...
    if (sb != null && sb.length() != 0) {
      if (!this.mCurrentEvent.isTainted()) {
        ...
        Log.m94d(this.mLogTag, this.mViolationMessage.toString());
