...
public class WindowlessWindowManager implements IWindowSession {
                    ...
                    try {
                        ...
                        try {
                            if (iWindowSession instanceof IWindowSession.Stub) {
                                try {
                                    iWindowSession.grantInputChannel(displayId, new SurfaceControl(sc, "WindowlessWindowManager.addToDisplay"), window, this.mHostInputToken, attrs.flags, attrs.privateFlags, attrs.inputFeatures, attrs.type, attrs.token, state.mFocusGrantToken, attrs.getTitle().toString(), outInputChannel);
                                } catch (RemoteException e) {
                                    e = e;
                                ...
                                this.mRealWm.grantInputChannelWithTaskToken(displayId, sc, window, this.mHostInputToken, attrs.flags, attrs.privateFlags, attrs.inputFeatures, attrs.type, attrs.token, state.mFocusGrantToken, attrs.getTitle().toString(), outInputChannel, attrs.surfaceInsets.left, this.mTaskToken);
                            } else {
                                this.mRealWm.grantInputChannel(displayId, sc, window, this.mHostInputToken, attrs.flags, attrs.privateFlags, attrs.inputFeatures, attrs.type, attrs.token, state.mFocusGrantToken, attrs.getTitle().toString(), outInputChannel);
                            }
    ...
    */
    public int relayout(IWindow window, WindowManager.LayoutParams inAttrs, int requestedWidth, int requestedHeight, int viewFlags, int flags, int seq, int lastSyncSeqId, ClientWindowFrames outFrames, MergedConfiguration outMergedConfiguration, SurfaceControl outSurfaceControl, InsetsState outInsetsState, InsetsSourceControl.Array outActiveControls, Bundle outSyncSeqIdBundle) {
        ...
        if ((attrChanges & 65540) != 0 && state.mInputChannelToken != null) {
            try {
                ...
                if (iWindowSession instanceof IWindowSession.Stub) {
                    try {
                        iWindowSession.updateInputChannel(state.mInputChannelToken, state.mDisplayId, new SurfaceControl(sc, "WindowlessWindowManager.relayout"), attrs.flags, attrs.privateFlags, attrs.inputFeatures, state.mInputRegion);
                    } catch (RemoteException e) {
                        e = e;
                        Log.m97e(TAG, "Failed to update surface input channel: ", e);
                    ...
                    try {
                        iWindowSession.updateInputChannel(state.mInputChannelToken, state.mDisplayId, sc, attrs.flags, attrs.privateFlags, attrs.inputFeatures, state.mInputRegion);
                    } catch (RemoteException e2) {
                        e = e2;
                        Log.m97e(TAG, "Failed to update surface input channel: ", e);
    ...
    public void finishDrawing(IWindow window, SurfaceControl.Transaction postDrawTransaction, int seqId) {
        ...
        synchronized (this) {
            ...
            if (CoreRune.SAFE_DEBUG && (state = this.mStateForWindow.get(window)) != null) {
                Log.m94d(TAG, "finishDrawing, seqId=" + seqId + ", sc=" + state.mSurfaceControl + ", hasCallback=" + (c != null));
            }
            if (c == null) {
    ...
    public boolean transferEmbeddedTouchFocusToHost(IWindow window) {
        Log.m96e(TAG, "Received request to transferEmbeddedTouch focus on WindowlessWindowManager we shouldn't get here!");
        return false;
    }
