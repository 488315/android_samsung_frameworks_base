...
public class GestureDetector {
    ...
    public void handleMessage(Message msg) {
      switch (msg.what) {
        case 1:
          ...
          return;
        case 2:
          Log.m98i(GestureDetector.TAG, "handleMessage LONG_PRESS");
          ...
          if (CoreRune.SAFE_DEBUG) {
            ...
            Log.m94d(GestureDetector.TAG, "onLP " + (deep ? 1 : 0));
            return;
          }
          return;
        case 3:
          if (GestureDetector.this.mDoubleTapListener != null) {
            if (!GestureDetector.this.mStillDown) {
              Log.m98i(GestureDetector.TAG, "handleMessage TAP");
              GestureDetector.this.recordGestureClassification(1);
              GestureDetector.this.mDoubleTapListener.onSingleTapConfirmed(
                  GestureDetector.this.mCurrentDownEvent);
              if (CoreRune.SAFE_DEBUG) {
                Log.m94d(GestureDetector.TAG, "onSTC#1");
                return;
              }
              return;
            }
  ...
  */
  public boolean onTouchEvent(MotionEvent ev) {
    ...
    if (this.mCheckLog) {
      Log.m98i(TAG, "obtain mCurrentMotionEventRaw. action: " + action + " id: " + ev.getId());
      this.mCheckLog = false;
    }
    MotionEvent obtain = MotionEvent.obtain(ev);
    this.mCurrentMotionEvent = obtain;
    this.mCurrentMotionEventRawX = obtain.getRawX();
      ...
      case 0:
        if (this.mDoubleTapListener != null) {
          ...
          if (motionEvent4 != null
              && (motionEvent2 = this.mPreviousUpEvent) != null
              && hadTapMessage
              && isConsideredDoubleTap(motionEvent4, motionEvent2, ev)) {
            ...
            boolean handled6 = CoreRune.SAFE_DEBUG;
            if (handled6) {
              Log.m94d(TAG, "onDT");
            }
            ...
            Log.m98i(
                TAG,
                "obtain mCurrentDownEvent. id: " + ev.getId() + " caller: " + Debug.getCallers(3));
            MotionEvent obtain2 = MotionEvent.obtain(ev);
            this.mCurrentDownEvent = obtain2;
            this.mCurrentDownEventRawX = obtain2.getRawX();
            ...
            if (this.mIsLongpressEnabled) {
              ...
              handler.sendMessageAtTime(
                  handler.obtainMessage(2, 3, 0),
                  this.mCurrentDownEvent.getDownTime() + ViewConfiguration.getLongPressTimeout());
            }
            this.mHandler.sendEmptyMessageAtTime(
                1, this.mCurrentDownEvent.getDownTime() + TAP_TIMEOUT);
        ...
        if (motionEvent != null) {}
        Log.m98i(
            TAG, "obtain mCurrentDownEvent. id: " + ev.getId() + " caller: " + Debug.getCallers(3));
        MotionEvent obtain22 = MotionEvent.obtain(ev);
        this.mCurrentDownEvent = obtain22;
        this.mCurrentDownEventRawX = obtain22.getRawX();
        this.mCurrentDownEventRawY = this.mCurrentDownEvent.getRawY();
        this.mCheckLog = true;
        this.mAlwaysInTapRegion = true;
        this.mAlwaysInBiggerTapRegion = true;
        ...
        if (this.mIsDoubleTapping) {
          ...
          if (this.mInLongPress) {
            ...
            if (this.mDeferConfirmSingleTap
                && (onDoubleTapListener = this.mDoubleTapListener) != null) {
              onDoubleTapListener.onSingleTapConfirmed(ev);
              if (CoreRune.SAFE_DEBUG) {
                Log.m94d(TAG, "onSTC#2");
              }
            }
            ...
            if (this.mIsDoubleTapping) {
              ...
              if (!shouldInhibitDefaultAction) {
                ...
                if (deltaY2 > slopSquare) {
                  ...
                  hasPendingLongPress = hasPendingLongPress2;
                  handled4 = false;
                  handler2.sendMessageAtTime(
                      handler2.obtainMessage(2, 3, 0),
                      ev.getDownTime()
                          + ((long) (longPressTimeout * this.mAmbiguousGestureMultiplier)));
                } else {
                  motionClassification = motionClassification2;
                  hasPendingLongPress = hasPendingLongPress2;
                  handled4 = false;
              ...
              if (Math.abs(scrollX) >= 1.0f || Math.abs(scrollY) >= 1.0f) {
                recordGestureClassification(5);
                boolean handled7 =
                    this.mListener.onScroll(this.mCurrentDownEvent, ev, scrollX, scrollY);
                this.mLastFocusX = focusX;
                this.mLastFocusY = focusY;
                handled2 = handled7;
              } else {
    ...
    if (!handled2
        && (inputEventConsistencyVerifier = this.mInputEventConsistencyVerifier) != null) {
    ...
    switch (ev.getActionMasked()) {
      case 11:
        ...
        if (onContextClickListener != null
            && !this.mInContextClick
            && !this.mInLongPress
            && ((actionButton == 32 || actionButton == 2)
                && onContextClickListener.onContextClick(ev))) {
  ...
  public void recordGestureClassification(int classification) {
    ...
    if (this.mCurrentDownEvent == null || this.mCurrentMotionEvent == null) {
      ...
      FrameworkStatsLog.write(
          ...
          getClass().getName(),
          classification,
          (int) (SystemClock.uptimeMillis() - this.mCurrentMotionEvent.getDownTime()),
          (float)
              Math.hypot(
                  this.mCurrentMotionEventRawX - this.mCurrentDownEventRawX,
                  this.mCurrentMotionEventRawY - this.mCurrentDownEventRawY));
      this.mHasRecordedClassification = true;
    }
  }
}
