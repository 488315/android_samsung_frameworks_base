...
public class ScrollCaptureConnection extends IScrollCaptureConnection.Stub
  ...
  public ICancellationSignal startCapture(Surface surface, IScrollCaptureCallbacks remote)
    ...
    IScrollCaptureCallbacks iScrollCaptureCallbacks =
        (IScrollCaptureCallbacks) Objects.requireNonNull(remote, "<callbacks> must non-null");
  ...
  public void binderDied() {
    ...
    Log.m96e(TAG, "Controlling process just died.");
    close();
  }
  ...
  public synchronized void close() {
    ...
    if (this.mActive) {
      Log.m102w(TAG, "close(): capture session still active! Ending now.");
      cancelPendingAction();
  ...
  private void cancelPendingAction() {
    if (this.mCancellation != null) {
      ...
      Log.m102w(TAG, "cancelling pending operation.");
