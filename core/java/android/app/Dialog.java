...
public class Dialog implements DialogInterface, Window.Callback, KeyEvent.Callback, View.OnCreateContextMenuListener, Window.OnWindowDismissedCallback {
    ...
    Dialog(Context context, int themeResId, boolean createContextThemeWrapper) {
        ...
        Log.m98i(TAG, "mIsDeviceDefault = " + this.mIsDeviceDefault + ", mIsSamsungBasicInteraction = " + this.mIsSamsungBasicInteraction + ", isMetaDataInActivity = " + isMetaDataInActivity);
    ...
    public void show() {
        ...
        if (this.mIsDeviceDefault) {
            ...
            if (isReduceTransparency2 && ((ProgressDialog) this).getCurrentProgressStyle() == 1000) {
                int dialogBaseSize = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.tw_progress_circle_dialog_size);
                l.height = dialogBaseSize;
                l.width = dialogBaseSize;
            }
            l.dimDuration = this.mContext.getResources().getInteger(C4337R.integer.sem_dialog_dim_duration);
        }
        ...
        if (this.mIsDeviceDefault && l.width > 0) {
            Drawable drawable = this.mContext.getResources().getDrawable(C4337R.drawable.tw_dialog_background_material, this.mContext.getTheme());
            View view = this.mDecor;
            if (view != null && view.getBackground() != null && drawable.getConstantState() != null && drawable.getConstantState().equals(this.mDecor.getBackground().getConstantState())) {
                int bottomInset = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_dialog_background_inset_vertical);
    ...
    private void alignToAnchor() {
        ...
        if (this.mAnchorType == 1) {
            int actionButtonWidth = res.getDimensionPixelSize(C4337R.dimen.sem_action_button_min_width_overflow);
        ...
        params.f574y = ((screenPos[1] + height) - statusBarHeight) + res.getDimensionPixelSize(C4337R.dimen.sem_dialog_window_margin_in_large);
        this.mWindow.setAttributes(params);
    ...
    public void semSetAnchor(int x, int y) {
        ...
        if (isSupportAnchor()) {
            ...
            int dialogWidth = semGetDialogWidth();
            int statusBarHeight = res.getConfiguration().windowConfiguration.isPopOver() ? 0 : res.getDimensionPixelSize(C4337R.dimen.status_bar_height);
            WindowManager.LayoutParams l = this.mWindow.getAttributes();
            l.gravity = 51;
            l.f573x = x - (dialogWidth / 2);
            l.f574y = (y - statusBarHeight) + res.getDimensionPixelSize(C4337R.dimen.sem_dialog_window_margin_in_large);
    ...
    private boolean isSupportAnchor() {
        if (isEmbedActivityMode() || this.mContext.getResources().getConfiguration().windowConfiguration.isPopOver() || this.mContext.getResources().getBoolean(C4337R.bool.sem_config_dialogLargeScreen)) {
            return true;
        }
