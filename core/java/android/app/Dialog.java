...
public class Dialog
  ...
  Dialog(Context context, int themeResId, boolean createContextThemeWrapper) {
    ...
    this.mDismissAction =
        new Runnable() { // from class: android.app.Dialog$$ExternalSyntheticLambda1
    ...
    this.mOnLayoutChangeListener =
        new View
            .OnLayoutChangeListener() { // from class: android.app.Dialog$$ExternalSyntheticLambda2
    ...
    w.setOnWindowSwipeDismissedCallback(
        new Window
            .OnWindowSwipeDismissedCallback() { // from class:
                                                // android.app.Dialog$$ExternalSyntheticLambda3
    ...
    Log.m98i(
        ...
        "mIsDeviceDefault = "
            ...
            + ", mIsSamsungBasicInteraction = "
            + this.mIsSamsungBasicInteraction
            + ", isMetaDataInActivity = "
            + isMetaDataInActivity);
  ...
  public void show() {
    ...
    if (this.mIsDeviceDefault) {
      ...
      if (isReduceTransparency2 && ((ProgressDialog) this).getCurrentProgressStyle() == 1000) {
        int dialogBaseSize =
            this.mContext
                .getResources()
                .getDimensionPixelSize(C4337R.dimen.tw_progress_circle_dialog_size);
        l.height = dialogBaseSize;
        l.width = dialogBaseSize;
      }
      l.dimDuration =
          this.mContext.getResources().getInteger(C4337R.integer.sem_dialog_dim_duration);
    }
    ...
    if (this.mIsDeviceDefault && l.width > 0) {
      Drawable drawable =
          this.mContext
              .getResources()
              .getDrawable(C4337R.drawable.tw_dialog_background_material, this.mContext.getTheme());
      View view = this.mDecor;
      if (view != null
          && view.getBackground() != null
          && drawable.getConstantState() != null
          && drawable.getConstantState().equals(this.mDecor.getBackground().getConstantState())) {
        int bottomInset =
            this.mContext
                .getResources()
                .getDimensionPixelSize(C4337R.dimen.sem_dialog_background_inset_vertical);
  ...
  public void semSetAnchor(View anchor, int anchorType) {
    if (isSupportAnchor()) {
      Log.m98i(TAG, "semSetAnchor anchorView = " + anchor + " , anchorType : " + anchorType);
      ...
      if (rootView != null) {
        ...
        this.mRemoveOnLayoutChangeListnerRunnable =
            new Runnable() { // from class: android.app.Dialog$$ExternalSyntheticLambda0
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private boolean isNeedToUpdateAttributes(View rootView) {
    ...
    int smallestScreenWidthDp =
        rootView.getContext().getResources().getConfiguration().smallestScreenWidthDp;
    ...
    if (this.mReconsiderForAlignToAnchor) {
      needToUpdate = true;
      this.mReconsiderForAlignToAnchor = false;
      Log.m98i(TAG, "Reconsidered to update LayoutParams");
    }
    if (needToUpdate) {
      Log.m98i(TAG, "Dialog LayoutParams update is needed");
    }
    return needToUpdate;
  }
  ...
  private void alignToAnchor() {
    ...
    if (!isSupportAnchor() || (view = this.mAnchorView) == null) {
      Log.m96e(TAG, "AnchorView is null state or not on Large Screen");
      params.gravity = 81;
      params.f573x = 0;
      params.f574y = 0;
    ...
    if (this.mAnchorView.getVisibility() == 0
        && screenPos[0] <= 0
        && windowPos[0] <= 0
        && screenPos[1] <= 0
        && windowPos[1] <= 0) {
      this.mReconsiderForAlignToAnchor = true;
      Log.m96e(TAG, "AnchorView position is invalid, so do not update position");
      return;
    }
    boolean isMultiWindowMode =
        res.getConfiguration().windowConfiguration.getWindowingMode() == 5
            || res.getConfiguration().windowConfiguration.getWindowingMode() == 6;
    int statusBarHeight = res.getDimensionPixelSize(C4337R.dimen.status_bar_height);
    View appView =
        WindowManagerGlobal.getInstance()
            .getWindowView(this.mAnchorView.getApplicationWindowToken());
    if (appView == null) {
      ViewRootImpl viewRootImpl = this.mAnchorView.getViewRootImpl();
      if (viewRootImpl != null) {
        appView = viewRootImpl.getView();
      } else {
        Log.m96e(TAG, "Cannot find app view");
      }
    }
    if (appView != null && (insets = appView.getRootWindowInsets()) != null) {
      statusBarHeight = insets.getSystemWindowInsetTop();
      Log.m98i(TAG, "top inset = " + statusBarHeight);
    }
    ...
    if (this.mAnchorType == 1) {
      int actionButtonWidth =
          res.getDimensionPixelSize(C4337R.dimen.sem_action_button_min_width_overflow);
    ...
    params.gravity = 51;
    params.f573x = screenPos[0] - xOffset;
    params.f574y =
        ((screenPos[1] + height) - statusBarHeight)
            + res.getDimensionPixelSize(C4337R.dimen.sem_dialog_window_margin_in_large);
    this.mWindow.setAttributes(params);
  ...
  public void semSetAnchor(int x, int y) {
    Resources res = this.mContext.getResources();
    if (isSupportAnchor()) {
      Log.m98i(TAG, "semSetAnchor set x : " + x + ", y : " + y);
      int dialogWidth = semGetDialogWidth();
      int statusBarHeight =
          res.getConfiguration().windowConfiguration.isPopOver()
              ? 0
              : res.getDimensionPixelSize(C4337R.dimen.status_bar_height);
      WindowManager.LayoutParams l = this.mWindow.getAttributes();
      l.gravity = 51;
      l.f573x = x - (dialogWidth / 2);
      l.f574y =
          (y - statusBarHeight)
              + res.getDimensionPixelSize(C4337R.dimen.sem_dialog_window_margin_in_large);
  ...
  private int semGetDialogWidth() {
    Resources res = this.mContext.getResources();
    float dlgWidth =
        TypedValue.applyDimension(1, res.getConfiguration().screenWidthDp, res.getDisplayMetrics());
    ...
    if (min == 0) {
      res.getValue(C4337R.dimen.sem_config_prefDialogWidth, minWidth, true);
      if (minWidth.type == 5) {
  ...
  void dismissDialog() {
    ...
    if (this.mWindow.isDestroyed()) {
      Log.m96e(TAG, "Tried to dismissDialog() but the Dialog's window was already destroyed!");
      return;
    }
    try {
  ...
  protected void onStart() {
    ...
    if (context != null && WindowOnBackInvokedDispatcher.isOnBackInvokedCallbackEnabled(context)) {
      this.mDefaultBackCallback =
          new OnBackInvokedCallback() { // from class: android.app.Dialog$$ExternalSyntheticLambda4
  ...
  public boolean onKeyUp(int keyCode, KeyEvent event) {
    if ((keyCode == 4 || keyCode == 111)
        && event.isTracking()
        && !event.isCanceled()
        && !WindowOnBackInvokedDispatcher.isOnBackInvokedCallbackEnabled(this.mContext)) {
      onBackPressed();
      return true;
    }
    return false;
  ...
  public boolean onTouchEvent(MotionEvent event) {
    if (this.mCancelable
        && this.mShowing
        && this.mWindow.shouldCloseOnTouch(this.mContext, event)) {
      cancel();
      return true;
    }
    return false;
  ...
  public boolean dispatchKeyEvent(KeyEvent event) {
    ...
    if ((onKeyListener != null && onKeyListener.onKey(this, event.getKeyCode(), event))
        || this.mWindow.superDispatchKeyEvent(event)) {
      return true;
    }
    View view = this.mDecor;
  ...
  private boolean isSupportAnchor() {
    if (isEmbedActivityMode()
        || this.mContext.getResources().getConfiguration().windowConfiguration.isPopOver()
        || this.mContext.getResources().getBoolean(C4337R.bool.sem_config_dialogLargeScreen)) {
      return true;
    }
    Log.m98i(TAG, "semSetAnchor isn't supported");
    return false;
  }
