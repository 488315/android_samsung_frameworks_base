...
public final class PeopleManager {
  ...
  public PeopleManager(Context context) throws ServiceManager.ServiceNotFoundException {
    ...
    this.mService =
        IPeopleManager.Stub.asInterface(ServiceManager.getServiceOrThrow(Context.PEOPLE_SERVICE));
  }
  ...
  public void addOrUpdateStatus(String conversationId, ConversationStatus status) {
    ...
    try {
      this.mService.addOrUpdateStatus(
          this.mContext.getPackageName(), this.mContext.getUserId(), conversationId, status);
    } catch (RemoteException e) {
      throw e.rethrowFromSystemServer();
    }
  }
  ...
  public void clearStatus(String conversationId, String statusId) {
    ...
    try {
      this.mService.clearStatus(
          this.mContext.getPackageName(), this.mContext.getUserId(), conversationId, statusId);
    } catch (RemoteException e) {
      throw e.rethrowFromSystemServer();
    }
  }
  ...
  static class ConversationListenerProxy extends IConversationListener.Stub {
    ...
    public void onConversationUpdate(final ConversationChannel conversation) {
      ...
      if (this.mListener == null || (executor = this.mExecutor) == null) {
        Slog.m115e(PeopleManager.LOG_TAG, "Binder is dead");
      } else {
        executor.execute(
            new Runnable() { // from class:
              ...
              public final void run() {
                PeopleManager.ConversationListenerProxy.this.lambda$onConversationUpdate$0(
                    conversation);
              }
            });
      }
