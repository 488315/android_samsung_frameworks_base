...
public class AssistStructure implements Parcelable {
    ...
    static final class ParcelTransferWriter extends Binder {
        ...
        void writeToParcel(AssistStructure as, Parcel out) {
            ...
            Log.m98i(AssistStructure.TAG, "Flattened " + (more ? Slice.HINT_PARTIAL : "final") + " assist data: " + (out.dataPosition() - start) + " bytes, containing " + this.mNumWrittenWindows + " windows, " + this.mNumWrittenViews + " views");
        }
            ...
            public HtmlInfoNode createFromParcel(Parcel parcel) {
                ...
                if (names != null && values != null) {
                    if (names.length != values.length) {
                        Log.m102w(AssistStructure.TAG, "HtmlInfo attributes mismatch: names=" + names.length + ", values=" + values.length);
                    } else {
    ...
    public void dump(boolean showSensitive) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            Log.m98i(TAG, "Window #" + i + " [" + node.getLeft() + "," + node.getTop() + " " + node.getWidth() + "x" + node.getHeight() + "] " + ((Object) node.getTitle()));
    ...
    void dump(String prefix, ViewNode node, boolean showSensitive) {
        String safeText;
        Log.m98i(TAG, prefix + "View [" + node.getLeft() + "," + node.getTop() + " " + node.getWidth() + "x" + node.getHeight() + "] " + node.getClassName());
        int id = node.getId();
        if (id != 0) {
        ...
        if (text != null) {
            ...
            Log.m98i(TAG, prefix + "  Text (sel " + node.getTextSelectionStart() + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + node.getTextSelectionEnd() + "): " + safeText);
            Log.m98i(TAG, prefix + "  Text size: " + node.getTextSize() + " , style: #" + node.getTextStyle());
            Log.m98i(TAG, prefix + "  Text color fg: #" + Integer.toHexString(node.getTextColor()) + ", bg: #" + Integer.toHexString(node.getTextBackgroundColor()));
        ...
        if (htmlInfo != null) {
            Log.m98i(TAG, prefix + "  HtmlInfo: tag=" + htmlInfo.getTag() + ", attr=" + htmlInfo.getAttributes());
        }
        ...
        if (autofillId == null) {
            ...
            Log.m98i(TAG, prefix + "  Autofill info: id= " + autofillId + ", type=" + node.getAutofillType() + ", options=" + Arrays.toString(node.getAutofillOptions()) + ", hints=" + Arrays.toString(node.getAutofillHints()) + ", value=" + node.getAutofillValue() + ", sanitized=" + node.isSanitized() + ", important=" + node.getImportantForAutofill());
        }
    ...
    boolean waitForReady() {
        ...
        synchronized (this) {
            ...
            if (this.mPendingAsyncChildren.size() > 0) {
                Log.m102w(TAG, "Skipping assist structure, waiting too long for async children (have " + this.mPendingAsyncChildren.size() + " remaining");
                skipStructure = true;
            }
        }
