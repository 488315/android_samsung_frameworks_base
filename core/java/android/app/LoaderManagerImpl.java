...
class LoaderManagerImpl extends LoaderManager {
    ...
    final class LoaderInfo implements Loader.OnLoadCompleteListener<Object>, Loader.OnLoadCanceledListener<Object> {
        ...
        void start() {
            ...
            if (LoaderManagerImpl.DEBUG) {
                Log.m100v(LoaderManagerImpl.TAG, "  Starting: " + this);
            }
        ...
        void retain() {
            if (LoaderManagerImpl.DEBUG) {
                Log.m100v(LoaderManagerImpl.TAG, "  Retaining: " + this);
            }
        ...
        void finishRetain() {
            if (this.mRetaining) {
                if (LoaderManagerImpl.DEBUG) {
                    Log.m100v(LoaderManagerImpl.TAG, "  Finished Retaining: " + this);
                }
        ...
        void stop() {
            ...
            if (LoaderManagerImpl.DEBUG) {
                Log.m100v(LoaderManagerImpl.TAG, "  Stopping: " + this);
            }
            this.mStarted = false;
        ...
        boolean cancel() {
            Loader<Object> loader;
            if (LoaderManagerImpl.DEBUG) {
                Log.m100v(LoaderManagerImpl.TAG, "  Canceling: " + this);
            }
        ...
        void destroy() {
            if (LoaderManagerImpl.DEBUG) {
                Log.m100v(LoaderManagerImpl.TAG, "  Destroying: " + this);
            }
            ...
            if (this.mCallbacks != null && this.mLoader != null && this.mHaveData && needReset) {
                if (LoaderManagerImpl.DEBUG) {
                    Log.m100v(LoaderManagerImpl.TAG, "  Reseting: " + this);
                }
        ...
        public void onLoadCanceled(Loader<Object> loader) {
            if (LoaderManagerImpl.DEBUG) {
                Log.m100v(LoaderManagerImpl.TAG, "onLoadCanceled: " + this);
            }
            if (this.mDestroyed) {
                if (LoaderManagerImpl.DEBUG) {
                    Log.m100v(LoaderManagerImpl.TAG, "  Ignoring load canceled -- destroyed");
                }
            } else {
                if (LoaderManagerImpl.this.mLoaders.get(this.mId) != this) {
                    if (LoaderManagerImpl.DEBUG) {
                        Log.m100v(LoaderManagerImpl.TAG, "  Ignoring load canceled -- not active");
                        return;
                    }
                    return;
                }
                ...
                if (pending != null) {
                    if (LoaderManagerImpl.DEBUG) {
                        Log.m100v(LoaderManagerImpl.TAG, "  Switching to pending loader: " + pending);
                    }
        ...
        public void onLoadComplete(Loader<Object> loader, Object data) {
            if (LoaderManagerImpl.DEBUG) {
                Log.m100v(LoaderManagerImpl.TAG, "onLoadComplete: " + this);
            }
            if (this.mDestroyed) {
                if (LoaderManagerImpl.DEBUG) {
                    Log.m100v(LoaderManagerImpl.TAG, "  Ignoring load complete -- destroyed");
                    return;
                }
                return;
            }
            if (LoaderManagerImpl.this.mLoaders.get(this.mId) != this) {
                if (LoaderManagerImpl.DEBUG) {
                    Log.m100v(LoaderManagerImpl.TAG, "  Ignoring load complete -- not active");
                    return;
                }
                return;
            }
        ...
        void callOnLoadFinished(Loader<Object> loader, Object data) {
            if (this.mCallbacks != null) {
                ...
                try {
                    if (LoaderManagerImpl.DEBUG) {
                        Log.m100v(LoaderManagerImpl.TAG, "  onLoadFinished in " + loader + ": " + loader.dataToString(data));
                    }
    ...
    public <D> Loader<D> initLoader(int i, Bundle bundle, LoaderManager.LoaderCallbacks<D> loaderCallbacks) {
        ...
        if (DEBUG) {
            Log.m100v(TAG, "initLoader in " + this + ": args=" + bundle);
        }
        if (loaderInfo == null) {
            loaderInfo = createAndInstallLoader(i, bundle, loaderCallbacks);
            if (DEBUG) {
                Log.m100v(TAG, "  Created new loader " + loaderInfo);
            }
        } else {
            if (DEBUG) {
                Log.m100v(TAG, "  Re-using existing loader " + loaderInfo);
            }
    ...
    public <D> Loader<D> restartLoader(int i, Bundle bundle, LoaderManager.LoaderCallbacks<D> loaderCallbacks) {
        ...
        LoaderInfo loaderInfo = this.mLoaders.get(i);
        if (DEBUG) {
            Log.m100v(TAG, "restartLoader in " + this + ": args=" + bundle);
        }
        if (loaderInfo != null) {
            ...
            if (loaderInfo2 != null) {
                if (loaderInfo.mHaveData) {
                    if (DEBUG) {
                        Log.m100v(TAG, "  Removing last inactive loader: " + loaderInfo);
                    }
                    ...
                    if (DEBUG) {
                        Log.m100v(TAG, "  Current loader is stopped; replacing");
                    }
                    this.mLoaders.put(i, null);
                    loaderInfo.destroy();
                } else {
                    if (DEBUG) {
                        Log.m100v(TAG, "  Current loader is running; configuring pending loader");
                    }
                    if (loaderInfo.mPendingLoader != null) {
                        if (DEBUG) {
                            Log.m100v(TAG, "  Removing pending loader: " + loaderInfo.mPendingLoader);
                        }
                        loaderInfo.mPendingLoader.destroy();
                        loaderInfo.mPendingLoader = null;
                    }
                    if (DEBUG) {
                        Log.m100v(TAG, "  Enqueuing as new pending loader");
                    }
                ...
                if (DEBUG) {
                    Log.m100v(TAG, "  Making last loader inactive: " + loaderInfo);
                }
                loaderInfo.mLoader.abandon();
                this.mInactiveLoaders.put(i, loaderInfo);
            }
    ...
    public void destroyLoader(int id) {
        ...
        if (DEBUG) {
            Log.m100v(TAG, "destroyLoader in " + this + " of " + id);
        }
    ...
    void doStart() {
        if (DEBUG) {
            Log.m100v(TAG, "Starting in " + this);
        }
        if (this.mStarted) {
    ...
    void doStop() {
        if (DEBUG) {
            Log.m100v(TAG, "Stopping in " + this);
        }
        if (!this.mStarted) {
            RuntimeException e = new RuntimeException("here");
            e.fillInStackTrace();
    ...
    void doRetain() {
        if (DEBUG) {
            Log.m100v(TAG, "Retaining in " + this);
        }
        if (!this.mStarted) {
            RuntimeException e = new RuntimeException("here");
            e.fillInStackTrace();
    ...
    void finishRetain() {
        if (this.mRetaining) {
            if (DEBUG) {
                Log.m100v(TAG, "Finished Retaining in " + this);
            }
            this.mRetaining = false;
            for (int i = this.mLoaders.size() - 1; i >= 0; i--) {
    ...
    void doDestroy() {
        if (!this.mRetaining) {
            if (DEBUG) {
                Log.m100v(TAG, "Destroying Active in " + this);
            }
            for (int i = this.mLoaders.size() - 1; i >= 0; i--) {
        ...
        if (DEBUG) {
            Log.m100v(TAG, "Destroying Inactive in " + this);
        }
