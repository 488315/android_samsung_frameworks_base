...
public class Instrumentation {
    ...
    public void resetInTouchMode() {
        boolean defaultInTouchMode = getContext().getResources().getBoolean(C4337R.bool.config_defaultInTouchMode);
    ...
    private void setDisplayIfNeeded(KeyEvent event) {
        ...
        if (eventDisplayId != -1) {
            if (VERBOSE) {
                Log.m100v(TAG, "setDisplayIfNeeded(" + event + "): not changing display id as it's explicitly set to " + eventDisplayId);
                return;
            }
            return;
        }
        ...
        if (VERBOSE) {
            Log.m100v(TAG, "setDisplayIfNeeded(" + event + "): eventDisplayId=" + eventDisplayId + ", user=" + this.mInstrContext.getUser() + ", userDisplayId=" + userDisplayId);
        }
        if (userDisplayId == -1) {
            Log.m96e(TAG, "setDisplayIfNeeded(" + event + "): UserManager returned INVALID_DISPLAY as display assigned to user " + this.mInstrContext.getUser());
        } else {
    ...
    private AppComponentFactory getFactory(String pkg) {
        ...
        if (activityThread == null) {
            Log.m97e(TAG, "Uninitialized ActivityThread, likely app-created Instrumentation, disabling AppComponentFactory", new Throwable());
            return AppComponentFactory.DEFAULT;
        }
    ...
    public static void checkStartActivityResult(int res, Object intent) {
        ...
        switch (res) {
            ...
            case -102:
                return;
            case -101:
                Log.m94d(TAG, "checkStartActivityResult() : mdm admin has blocked start activity " + intent);
                return;
            case -100:
