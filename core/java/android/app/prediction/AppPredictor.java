...
public final class AppPredictor {
  ...
  AppPredictor(Context context, AppPredictionContext predictionContext) {
    ...
    try {
      ...
      Log.m97e(TAG, "Failed to create predictor", e);
  ...
  public void notifyAppTargetEvent(AppTargetEvent event) {
    ...
    try {
      ...
      Log.m97e(TAG, "Failed to notify app target event", e);
      e.rethrowAsRuntimeException();
    }
  }
  ...
  public void notifyLaunchLocationShown(String launchLocation, List<AppTargetId> targetIds) {
    ...
    try {
      this.mPredictionManager.notifyLaunchLocationShown(
          this.mSessionId, launchLocation, new ParceledListSlice(targetIds));
    } catch (RemoteException e) {
      Log.m97e(TAG, "Failed to notify location shown event", e);
      e.rethrowAsRuntimeException();
    }
  }
  ...
  private void registerPredictionUpdatesLocked(Executor callbackExecutor, final Callback callback) {
    ...
    try {
      ...
      Log.m97e(TAG, "Failed to register for prediction updates", e);
      e.rethrowAsRuntimeException();
    }
  }
  ...
  private void unregisterPredictionUpdatesLocked(Callback callback) {
    ...
    try {
      ...
      Log.m97e(TAG, "Failed to unregister for prediction updates", e);
      e.rethrowAsRuntimeException();
    }
  }
  ...
  public void requestPredictionUpdate() {
    ...
    try {
      ...
      Log.m97e(TAG, "Failed to request prediction update", e);
      e.rethrowAsRuntimeException();
    }
  }

  public void sortTargets(
      List<AppTarget> targets, Executor callbackExecutor, Consumer<List<AppTarget>> callback) {
    if (this.mIsClosed.get()) {
      throw new IllegalStateException("This client has already been destroyed.");
    }
    try {
      this.mPredictionManager.sortAppTargets(
          this.mSessionId,
          new ParceledListSlice(targets),
          new CallbackWrapper(callbackExecutor, callback));
    } catch (RemoteException e) {
      Log.m97e(TAG, "Failed to sort targets", e);
      e.rethrowAsRuntimeException();
    }
  }
