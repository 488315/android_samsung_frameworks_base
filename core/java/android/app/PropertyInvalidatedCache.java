...
public class PropertyInvalidatedCache<Query, Result> {
  ...
  private static void setNonce(String name, long val) {
    ...
    for (int attempt = 0; attempt < 5; attempt++) {
      try {
        ...
        if (attempt > 0) {
          Log.m102w(TAG, "Nonce set after " + attempt + " tries");
          return;
        }
        return;
      ...
      synchronized (this.mLock) {
        ...
        if (!alreadyQueued) {
          ...
          synchronized (PropertyInvalidatedCache.sCorkLock) {
            long count =
                ((Long)
                        PropertyInvalidatedCache.sCorkedInvalidates.getOrDefault(
                            this.mPropertyName, 0L))
                    .longValue();
  ...
  public static void disableForTestMode() {
    Log.m94d(TAG, "disabling all caches in the process");
    sEnabled = false;
  }
    ...
    synchronized (obj2) {
      try {
        ...
        try {
          ...
          pw.println(
              TextUtils.formatSimple("    Hits: %d, Misses: %d, Skips: %d, Clears: %d", objArr));
          pw.println(
          ...
          pw.println(
              TextUtils.formatSimple(
                  "    Nonce: 0x%016x, Invalidates: %d, CorkedInvalidates: %d", objArr2));
          pw.println(
              TextUtils.formatSimple(
  ...
  private static void dumpCorkInfo(PrintWriter pw) {
    ...
    if (activeCorks.size() > 0) {
      ...
      for (int i = 0; i < activeCorks.size(); i++) {
        ...
        pw.println(
            TextUtils.formatSimple(
                "    Property Name: %s Count: %d", entry.getKey(), entry.getValue()));
      }
    }
  }
  ...
  public static void dumpCacheInfo(ParcelFileDescriptor pfd, String[] args) {
    ...
    try {
      ...
      Log.m96e(TAG, "Failed to dump PropertyInvalidatedCache instances");
    }
  }
