...
public class DownloadManager {
    ...
    public Request setDestinationInExternalPublicDir(String dirType, String subPath) {
      ...
      if (context.getApplicationInfo().targetSdkVersion >= 29
        ...
        try {
          ...
          try {
            if (client == null) {
              Log.m98i("DownloadManager", "client is null maybe due to download provider disabled");
              if (client != null) {
                client.close();
              }
              return null;
  ...
  public int secmarkRowDeleted(long... ids) {
    ...
    return this.mResolver.update(
        ContentUris.withAppendedId(Downloads.Impl.CONTENT_CDURI, ids[0]), values, null, null);
  }
    ...
    try {
      return Long.valueOf(
          Settings.Global.getLong(
              context.getContentResolver(), Settings.Global.DOWNLOAD_MAX_BYTES_OVER_MOBILE));
  ...
  public boolean rename(Context context, long id, String displayName) {
    ...
    try {
      ...
      if (cursor.moveToFirst()) {
        ...
        if (status != 8) {
          throw new IllegalStateException(
              "Download is not completed yet: " + DatabaseUtils.dumpCurrentRowToString(cursor));
        }
  ...
  public long addCompletedDownload(
    ...
    ContentValues contentValues =
        request.toContentValues(
            this.mPackageName.contains(SBROWSER_PACKAGE_NAME) ? this.mPackageName : null);
  ...
  public long secAddCompletedDownload(
    ...
    values.put(
        Downloads.Impl.COLUMN_MEDIA_SCANNED, Integer.valueOf(isMediaScannerScannable ? 0 : 2));
        ...
        case 1:
          if (!this.mAccessFilename) {
            throw new SecurityException(
                "COLUMN_LOCAL_FILENAME is deprecated; use ContentResolver.openFileDescriptor()"
                    + " instead");
          }
          break;
      }
  ...
  private static class SecCursorTranslator extends CursorWrapper {
    ...
    public String getColumnName(int columnIndex) {
      ...
      if (columnIndex < 0 || columnIndex >= numColumns) {
        throw new IllegalArgumentException(
            "Invalid column index " + columnIndex + ", " + numColumns + " columns exist");
      }
    ...
    public String[] getColumnNames() {
      ...
      System.arraycopy(
          DownloadManager.SEC_COLUMNS, 0, returnColumns, 0, DownloadManager.SEC_COLUMNS.length);
