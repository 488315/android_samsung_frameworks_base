...
public class TransactionExecutor {
  ...
  public void execute(ClientTransaction transaction) {
    ...
    if (token != null
        && (destroyItem =
                (activitiesToBeDestroyed = this.mTransactionHandler.getActivitiesToBeDestroyed())
                    .get(token))
            != null) {
      ...
      if (this.mTransactionHandler.getActivityClient(token) == null) {
        Slog.m121w(
            TAG,
            TransactionExecutorHelper.tId(transaction)
                + "Skip pre-destroyed transaction:\n"
    ...
    while (i < size) {
      ...
      if (item.shouldHaveDefinedPreExecutionState()
          && (closestPreExecutionState =
                  this.mHelper.getClosestPreExecutionState(r, item.getPostExecutionState()))
              != -1) {
  ...
  private void executeLifecycleState(ClientTransaction transaction) {
    ...
    if (lifecycleItem == null
        || (r =
                this.mTransactionHandler.getActivityClient(
                    (token = transaction.getActivityToken())))
            == null) {
      return;
    }
      ...
      switch (state) {
        ...
        case 4:
          this.mTransactionHandler.handlePauseActivity(
              r, false, false, 0, false, this.mPendingActions, "LIFECYCLER_PAUSE_ACTIVITY");
          break;
        case 5:
          this.mTransactionHandler.handleStopActivity(
              r, 0, this.mPendingActions, false, "LIFECYCLER_STOP_ACTIVITY");
          break;
        case 6:
          this.mTransactionHandler.handleDestroyActivity(
              r, false, 0, false, "performLifecycleSequence. cycling to:" + path.get(size - 1));
          break;
        case 7:
