...
public class ApplicationLoaders {
          ...
          if (loader == null) {
            ...
            try {
              ...
              GraphicsEnvironment.getInstance()
                  .setLayerPaths(classloader, librarySearchPath, libraryPermittedPath);
              Trace.traceEnd(64L);
              if (cacheKey != null) {
  ...
  private void createAndCacheNonBootclasspathSystemClassLoader(SharedLibraryInfo lib) {
      ...
      for (SharedLibraryInfo dependency : dependencies) {
        ...
        if (cached == null) {
          throw new IllegalStateException(
              "Failed to find dependency " + dependencyPath + " of cachedlibrary " + path);
        }
    ...
    Log.m94d(TAG, "Created zygote-cached class loader: " + path);
  ...
  public ClassLoader getCachedNonBootclasspathSystemLib(
      String zip, ClassLoader parent, String classLoaderName, List<ClassLoader> sharedLibraries) {
    CachedClassLoader cached;
    Map<String, CachedClassLoader> map = this.mSystemLibsCacheMap;
    if (map == null
        || parent != null
        || classLoaderName != null
        || (cached = map.get(zip)) == null) {
      return null;
    }
    ...
    Log.m94d(TAG, "Returning zygote-cached class loader: " + zip);
