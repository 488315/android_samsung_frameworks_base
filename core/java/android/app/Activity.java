...
public class Activity extends ContextThemeWrapper implements LayoutInflater.Factory2, Window.Callback, KeyEvent.Callback, View.OnCreateContextMenuListener, ComponentCallbacks2, Window.OnWindowDismissedCallback, ContentCaptureManager.ContentCaptureClient {
    ...
    private void notifyVoiceInteractionManagerServiceActivityEvent(int type) {
        if (this.mVoiceInteractionManagerService == null) {
            ...
            if (asInterface == null) {
                Log.m102w(TAG, "notifyVoiceInteractionManagerServiceActivityEvent: Can not get VoiceInteractionManagerService");
                return;
            }
        }
        try {
    ...
    public final void requestShowKeyboardShortcuts() {
        ComponentName sysuiComponent = ComponentName.unflattenFromString(getResources().getString(C4337R.string.config_systemUIServiceComponent));
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if (sdkVersion >= 5) {
            if (keyCode == 4) {
                ...
                Slog.m113d(TAG, "onKeyUp(KEYCODE_BACK) isTracking()=" + event.isTracking() + " isCanceled()=" + event.isCanceled() + " hasCallback=" + hasCallback);
            }
    ...
    public Uri getReferrer() {
        ...
        if (intent != null) {
            try {
                ...
                Log.m102w(TAG, "Cannot read referrer from intent; intent extras contain unknown custom Parcelable objects");
            }
        }
    ...
    public void semConvertFromTranslucent(boolean skipSetWindowOpaque) {
        Log.m94d(TAG, "semConvertFromTranslucent, activity=" + this + ", caller=" + Debug.getCallers(3));
        this.mTranslucentCallback = null;
    ...
    public Intent getParentActivityIntent() {
        ...
        try {
            ...
            Log.m96e(TAG, "getParentActivityIntent: bad parentActivityName '" + parentName + "' in manifest");
            return null;
        }
    }
    ...
    */
    final void attach(Context context, ActivityThread aThread, Instrumentation instr, IBinder token, int ident, Application application, Intent intent, ActivityInfo info, CharSequence title, Activity parent, String id, NonConfigurationInstances lastNonConfigurationInstances, Configuration config, String referrer, IVoiceInteractor voiceInteractor, Window window, ViewRootImpl.ActivityConfigCallback activityConfigCallback, IBinder assistToken, IBinder shareableActivityToken) {
        ...
        if (!CoreRune.SYSFW_APP_SPEG) {
            ...
            str = referrer;
            if (SPEG_PACKAGE_NAME.equals(str) || ((activity2 = this.mParent) != null && SPEG_PACKAGE_NAME.equals(activity2.mReferrer))) {
                ...
                if (!CoreRune.SAFE_DEBUG) {
                    Log.m98i(TAG, "attach mDexTaskDocking=" + WindowConfiguration.dexTaskDockingStateToString(this.mDexTaskDocking));
                    return;
                }
                return;
            }
    ...
    final void performCreate(Bundle icicle, PersistableBundle persistentState) {
        ...
        this.mShouldDockBigOverlays = getResources().getBoolean(C4337R.bool.config_dockBigOverlayWindows);
    ...
    void onDexTaskDockingChanged(int state) {
        if (CoreRune.SAFE_DEBUG || CoreRune.IS_DEBUG_LEVEL_MID) {
            Log.m98i(TAG, "onDexTaskDockingChanged=" + WindowConfiguration.dexTaskDockingStateToString(state) + "   mDecor=" + this.mDecor + " state in number?" + state);
        }
        View view = this.mDecor;
        if (view != null) {
