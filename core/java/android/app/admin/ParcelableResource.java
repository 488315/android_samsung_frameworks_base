...
public final class ParcelableResource implements Parcelable {
  ...
  public Drawable getDrawable(
    ...
    try {
      ...
      Slog.m116e(TAG, "Unable to load drawable resource " + this.mResourceName, e);
  ...
  public String getString(Context context, Supplier<String> defaultStringLoader) {
    try {
      ...
      Slog.m116e(TAG, "Unable to load string resource " + this.mResourceName, e);
  ...
  public String getString(
    ...
    try {
      ...
      return String.format(
          context.getResources().getConfiguration().getLocales().get(0), rawString, formatArgs);
    } catch (PackageManager.NameNotFoundException | RuntimeException e) {
      Slog.m116e(TAG, "Unable to load string resource " + this.mResourceName, e);
      return loadDefaultString(defaultStringLoader);
    }
  ...
  private void verifyResourceName(Resources resources) throws IllegalStateException {
    ...
    if (!this.mResourceName.equals(name)) {
      throw new IllegalStateException(
          String.format(
              "Current resource name %s for resource id %d has changed from the previously stored"
                  + " resource name %s.",
