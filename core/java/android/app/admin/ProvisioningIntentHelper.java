...
final class ProvisioningIntentHelper {
  ...
  public static Intent createProvisioningIntentFromNfcIntent(Intent nfcIntent) {
    ...
    if (!NfcAdapter.ACTION_NDEF_DISCOVERED.equals(nfcIntent.getAction())) {
      Log.m96e(TAG, "Wrong Nfc action: " + nfcIntent.getAction());
      return null;
    }
  ...
  private static Intent createProvisioningIntentFromNdefRecord(NdefRecord firstRecord) {
    ...
    if (properties == null) {
      Log.m96e(TAG, "Failed to load NdefRecord properties.");
      return null;
    }
    Bundle bundle = createBundleFromProperties(properties);
    if (!containsRequiredProvisioningExtras(bundle)) {
      Log.m96e(TAG, "Bundle does not contain the required provisioning extras.");
      return null;
    }
  ...
  private static Properties loadPropertiesFromPayload(byte[] payload) {
    ...
    try {
      ...
      Log.m96e(TAG, "NFC Intent properties loading failed.");
      return null;
    }
  }
  ...
  private static void addPropertyToBundle(
    ...
    if (map.get(propertyName) == PersistableBundle.class) {
      try {
        ...
        Log.m97e(TAG, "Failed to parse " + propertyName + MediaMetrics.SEPARATOR, e);
        return;
      }
    }
  ...
  private static PersistableBundle deserializeExtrasBundle(Properties properties, String extraName)
      throws IOException {
  ...
  private static boolean containsRequiredProvisioningExtras(Bundle bundle) {
    return bundle.containsKey(DevicePolicyManager.EXTRA_PROVISIONING_DEVICE_ADMIN_PACKAGE_NAME)
        || bundle.containsKey(DevicePolicyManager.EXTRA_PROVISIONING_DEVICE_ADMIN_COMPONENT_NAME);
  }
  ...
  private static NdefRecord getFirstNdefRecord(Intent nfcIntent) {
    ...
    if (ndefMessages == null) {
      Log.m98i(TAG, "No EXTRA_NDEF_MESSAGES from nfcIntent");
      return null;
    }
    ...
    Log.m98i(TAG, "No compatible records found on nfcIntent");
    return null;
  }
