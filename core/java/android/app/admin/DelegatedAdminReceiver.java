...
public class DelegatedAdminReceiver extends BroadcastReceiver {
    ...
    public final void onReceive(Context context, Intent intent) {
        ...
        if (DeviceAdminReceiver.ACTION_CHOOSE_PRIVATE_KEY_ALIAS.equals(action)) {
            int uid =
                    intent.getIntExtra(DeviceAdminReceiver.EXTRA_CHOOSE_PRIVATE_KEY_SENDER_UID, -1);
            Uri uri =
                    (Uri)
                            intent.getParcelableExtra(
                                    DeviceAdminReceiver.EXTRA_CHOOSE_PRIVATE_KEY_URI, Uri.class);
            String alias =
                    intent.getStringExtra(DeviceAdminReceiver.EXTRA_CHOOSE_PRIVATE_KEY_ALIAS);
        ...
        if (DeviceAdminReceiver.ACTION_NETWORK_LOGS_AVAILABLE.equals(action)) {
            long batchToken =
                    intent.getLongExtra(DeviceAdminReceiver.EXTRA_NETWORK_LOGS_TOKEN, -1L);
            int networkLogsCount =
                    intent.getIntExtra(DeviceAdminReceiver.EXTRA_NETWORK_LOGS_COUNT, 0);
