...
public class DeviceAdminReceiver extends BroadcastReceiver {
  ...
  public static final String ACTION_RECOVERY_PASSWORD_REQUESTED =
      "android.app.action.ACTION_RECOVERY_PASSWORD_REQUESTED";
  ...
  private void onOperationSafetyStateChanged(Context context, Intent intent) {
    ...
    Log.m102w(TAG, "Igoring intent that's missing required extras");
  }
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    if (ACTION_PASSWORD_CHANGED.equals(action)) {
      onPasswordChanged(
          context,
          intent,
          (UserHandle) intent.getParcelableExtra(Intent.EXTRA_USER, UserHandle.class));
      return;
    }
    ...
    if (ACTION_TRANSFER_OWNERSHIP_COMPLETE.equals(action)) {
      PersistableBundle bundle =
          (PersistableBundle)
              intent.getParcelableExtra(
                  EXTRA_TRANSFER_OWNERSHIP_ADMIN_EXTRAS_BUNDLE, PersistableBundle.class);
      ...
      onTransferAffiliatedProfileOwnershipComplete(
          context, (UserHandle) intent.getParcelableExtra(Intent.EXTRA_USER, UserHandle.class));
