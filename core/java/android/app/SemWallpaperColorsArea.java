...
public class SemWallpaperColorsArea implements Cloneable {
    ...
    public SemWallpaperColorsArea(Context context, int which, int rotation, WallpaperColorOverrideAreas baseOverrideColorArea) {
        ...
        Log.m94d(TAG, "SemWallpaperColorsArea [" + width + ", " + height + "] which: " + which + " rotation: " + rotation + " has Base: " + (baseOverrideColorArea != null));
        ...
        if ((Rune.SUPPORT_SUB_DISPLAY_MODE && Rune.SUPPORT_COVER_DISPLAY_WATCHFACE && (this.mWhich & 16) == 16) || (Rune.VIRTUAL_DISPLAY_WALLPAPER && (this.mWhich & 32) == 32)) {
            if (Rune.SUPPORT_LARGE_FRONT_SUB_DISPLAY) {
                ...
                statusBarTopMargin = resource.getDimensionPixelSize(C4337R.dimen.status_bar_camera_top_margin);
            }
        } else if (rotation != 0) {
            if (!isLandscapeMode) {
                this.mWidth = height;
        ...
        Log.m94d(TAG, "SemWallpaperColorsArea which = " + this.mWhich + ", mDensity : " + this.mDensity + ", " + this.mWidth + "x" + this.mHeight + "," + this.mDpWidth + "x" + this.mDpHeight + "," + this.mDpStatusBarHeight + "," + this.mDpNavigationBarHeight + ", " + this.mDpStatusBarTopMargin);
    }
    ...
    public Rect get(int area, int width, int height) {
        ...
        if (this.mWallpaperColorOverrideAreas != null) {
            ...
            Log.m98i(TAG, "Get custom area. display type = " + displayType + ", rotation = " + rotationType + ", area = " + area + " areaFlag = " + safeAreaFlag + " rect = " + rect);
    ...
    private Point getDisplaySize(int which) {
        ...
        Log.m94d(TAG, "getDisplaySize() which:" + which + ", displayId: " + displayId + ", size: " + size);
        ...
        if ((isSubDisplay && ratio < 2.0f) || (!isSubDisplay && ratio > 2.0f)) {
            ...
            Log.m94d(TAG, "getDisplaySize() wrong displayInfo, changed to displayId: " + displayId2 + ", size: " + size);
        }
        return size;
    }
