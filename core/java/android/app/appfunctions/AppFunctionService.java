...
public abstract class AppFunctionService extends Service {
    ...
    private final Binder mBinder =
            createBinder(
                    this,
                    new OnExecuteFunction() { // from class:
                        // android.app.appfunctions.AppFunctionService$$ExternalSyntheticLambda0
    ...
    public interface OnExecuteFunction {
        void perform(
                ...
                String str,
                CancellationSignal cancellationSignal,
                OutcomeReceiver<ExecuteAppFunctionResponse, AppFunctionException> outcomeReceiver);
    }
    ...
    public static Binder createBinder(
            final Context context, final OnExecuteFunction onExecuteFunction) {
        return new IAppFunctionService
                .Stub() { // from class: android.app.appfunctions.AppFunctionService.1
            ...
            public void executeAppFunction(
                ...
                if (Context.this.checkCallingPermission(
                                Manifest.permission.BIND_APP_FUNCTION_SERVICE)
                        == -1) {
                    throw new SecurityException("Can only be called by the system server.");
                }
                final SafeOneTimeExecuteAppFunctionCallback safeCallback =
                        new SafeOneTimeExecuteAppFunctionCallback(callback);
                try {
                    onExecuteFunction.perform(
                            request,
                            callingPackage,
                            AppFunctionService.buildCancellationSignal(cancellationCallback),
                            new OutcomeReceiver<
                                    ExecuteAppFunctionResponse,
                                    AppFunctionException>() { // from class:
                                // android.app.appfunctions.AppFunctionService.1.1
                    ...
                    safeCallback.onError(
                            new AppFunctionException(
                                    AppFunctionService.toErrorCode(ex), ex.getMessage()));
                }
            }
        };
    }
