...
public abstract class BlobBackupHelper implements BackupHelper {
  ...
  private ArrayMap<String, Long> readOldState(ParcelFileDescriptor oldStateFd) {
    ...
    try {
      ...
      if (version > this.mCurrentBlobVersion) {
        Log.m102w(TAG, "Prior state from unrecognized version " + version);
      } else {
      ...
      Log.m96e(TAG, "Error examining prior backup state " + e2.getMessage());
      state.clear();
    }
    return state;
  }
  ...
  private void writeBackupState(ArrayMap<String, Long> state, ParcelFileDescriptor stateFile) {
    try {
      ...
      Log.m97e(TAG, "Unable to write updated state", e);
    }
  }
  ...
  private byte[] deflate(byte[] data) {
    ...
    try {
      ...
      Log.m102w(TAG, "Unable to process payload: " + e.getMessage());
      return null;
    }
  }
  ...
  private byte[] inflate(byte[] compressedData) {
    ...
    try {
      ...
      if (version > this.mCurrentBlobVersion) {
        Log.m102w(TAG, "Saved payload from unrecognized version " + version);
        return null;
      }
      ...
      Log.m102w(TAG, "Unable to process restored payload: " + e.getMessage());
      return null;
    }
  }
  ...
  public void performBackup(
      ParcelFileDescriptor oldStateFd, BackupDataOutput data, ParcelFileDescriptor newStateFd) {
    ...
    try {
      try {
        ...
        Log.m102w(TAG, "Unable to record notification state: " + e.getMessage());
        newState.clear();
      }
    } finally {
  ...
  public void restoreEntity(BackupDataInputStream data) {
    ...
    while (true) {
      try {
        ...
        Log.m96e(TAG, "Exception restoring entity " + key + " : " + e.getMessage());
        return;
      }
    }
    if (which >= this.mKeys.length) {
      Log.m96e(TAG, "Unrecognized key " + key + ", ignoring");
      return;
    }
