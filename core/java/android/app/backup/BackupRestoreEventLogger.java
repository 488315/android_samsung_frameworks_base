...
public final class BackupRestoreEventLogger {
    ...
    public BackupRestoreEventLogger(int operationType) {
        ...
        try {
            ...
            Slog.m123w("Couldn't create MessageDigest for hash computation", e);
        }
    ...
    private DataTypeResult getDataTypeResult(int operationType, String dataType) {
        if (operationType != this.mOperationType) {
            Slog.m113d(TAG, "Operation type mismatch: logger created for " + this.mOperationType + ", trying to log for " + operationType);
            return null;
        }
        if (!this.mResults.containsKey(dataType)) {
            if (this.mResults.keySet().size() == 15) {
                Slog.m113d(TAG, "Logger is full, ignoring new data type");
                return null;
            }
