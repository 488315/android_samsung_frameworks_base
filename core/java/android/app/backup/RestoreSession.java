...
public class RestoreSession {
  ...
  public int getAvailableRestoreSets(RestoreObserver observer, BackupManagerMonitor monitor) {
    ...
    try {
      ...
      Log.m94d(TAG, "Can't contact server to get available sets");
      return -1;
    }
  }
  ...
  public int restoreAll(long token, RestoreObserver observer, BackupManagerMonitor monitor) {
    ...
    if (this.mObserver != null) {
      Log.m94d(TAG, "restoreAll() called during active restore");
      return -1;
    }
    ...
    try {
      ...
      Log.m94d(TAG, "Can't contact server to restore");
      return -1;
    }
  }
  ...
  public int restorePackages(
      long token, RestoreObserver observer, Set<String> packages, BackupManagerMonitor monitor) {
    BackupManagerMonitorWrapper monitorWrapper;
    if (this.mObserver != null) {
      Log.m94d(TAG, "restoreAll() called during active restore");
      return -1;
    ...
    try {
      int err =
          this.mBinder.restorePackages(
              token, this.mObserver, (String[]) packages.toArray(new String[0]), monitorWrapper);
      return err;
    } catch (RemoteException e) {
      Log.m94d(TAG, "Can't contact server to restore packages");
      return -1;
    }
  }
  ...
  public int restorePackage(
    ...
    BackupManagerMonitorWrapper monitorWrapper;
    if (this.mObserver != null) {
      Log.m94d(TAG, "restorePackage() called during active restore");
      return -1;
    }
    this.mObserver = new RestoreObserverWrapper(this.mContext, observer);
    if (monitor == null) {
    ...
    try {
      ...
      Log.m94d(TAG, "Can't contact server to restore package");
      return -1;
    }
  }
