...
public class BackupHelperDispatcher {
  ...
  public void performBackup(
    ...
    if (oldState != null) {
      ...
      while (true) {
        ...
        if (err == 0) {
          ...
          Log.m94d(TAG, "handling existing helper '" + header.keyPrefix + "' " + helper);
          if (helper != null) {
    ...
    for (Map.Entry<String, BackupHelper> entry : helpers.entrySet()) {
      ...
      Log.m94d(TAG, "handling new helper '" + header.keyPrefix + "'");
  ...
  private void doOneBackup(
    ...
    if (pos < 0) {
      throw new IOException(
          "allocateHeader_native failed (error " + pos + NavigationBarInflaterView.KEY_CODE_END);
    }
    ...
    if (err != 0) {
      throw new IOException(
          "writeHeader_native failed (error " + err + NavigationBarInflaterView.KEY_CODE_END);
    }
  }

  public void performRestore(
      BackupDataInput input, int appVersionCode, ParcelFileDescriptor newState) throws IOException {
    ...
    while (input.readNextHeader()) {
      ...
      if (pos > 0) {
        ...
        if (helper != null) {
          ...
          Log.m102w(TAG, "Couldn't find helper for: '" + rawKey + "'");
          alreadyComplained = true;
        }
      } else if (!alreadyComplained) {
        Log.m102w(TAG, "Entity with no prefix: '" + rawKey + "'");
        alreadyComplained = true;
      }
