...
public class SemWallpaperResourcesUtils {
  ...
  public static File getOMCWallpaperFile(Context context, int flag, String color) {
    ...
    if (omcResourcePath != null) {
      ...
      if (omcFile != null) {
        Log.m94d(TAG, "omc wallpaper return: " + omcFile.getAbsolutePath());
      }
    }
    return omcFile;
  }
  ...
  public static File getCSCWallpaperFile(Context context, int flag, String color) {
    ...
    if ((flag & 3) == 1) {
      ...
      cscFile =
          getOperatorFile(MULTI_CSC_WALLPAPER_DIR_PATH, getOperatorFileName(context, flag, color));
      if (cscFile == null) {
        cscFile =
            getOperatorFile(LOCK_CSC_WALLPAPER_DIR_PATH, getOperatorFileName(context, flag, color));
      }
    }
    if (cscFile != null) {
      Log.m94d(TAG, "csc wallpaper return: " + cscFile.getAbsolutePath());
    }
    return cscFile;
  }
  ...
  private static File getCSCWallpaperFile(Context context, String color) {
    ...
    if (cscFile == null
        && (cscFile =
                getOperatorFile(
                    MULTI_CSC_WALLPAPER_DIR_PATH, getOperatorFileName(context, 1, color)))
            == null) {
      cscFile =
          getOperatorFile(HOME_CSC_WALLPAPER_DIR_PATH, getOperatorFileName(context, 1, color));
    }
    if (cscFile != null) {
      Log.m94d(TAG, "csc wallpaper return: " + cscFile.getAbsolutePath());
    }
