...
final class BackStackRecord extends FragmentTransaction
  ...
  void bumpBackStackNesting(int amt) {
    ...
    if (FragmentManagerImpl.DEBUG) {
      Log.m100v(TAG, "Bump nesting in " + this + " by " + amt);
    }
    int numOps = this.mOps.size();
    for (int opNum = 0; opNum < numOps; opNum++) {
      C0150Op op = this.mOps.get(opNum);
      if (op.fragment != null) {
        ...
        if (FragmentManagerImpl.DEBUG) {
          Log.m100v(TAG, "Bump nesting of " + op.fragment + " to " + op.fragment.mBackStackNesting);
        }
      }
    }
  }
  ...
  void executePopOps(boolean moveToState) {
    for (int opNum = this.mOps.size() - 1; opNum >= 0; opNum--) {
      ...
      if (f != null) {
        f.setNextTransition(
            FragmentManagerImpl.reverseTransit(this.mTransition), this.mTransitionStyle);
      }
      switch (op.cmd) {
        case 1:
