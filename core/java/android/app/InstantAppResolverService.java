...
public abstract class InstantAppResolverService extends Service {
    ...
    public void onGetInstantAppIntentFilter(Intent sanitizedIntent, int[] hostDigestPrefix, String token, InstantAppResolutionCallback callback) {
        Log.m96e(TAG, "New onGetInstantAppIntentFilter is not overridden");
        if (sanitizedIntent.isWebIntent()) {
        ...
        return new IInstantAppResolver.Stub() { // from class: android.app.InstantAppResolverService.1
            ...
            public void getInstantAppResolveInfoList(InstantAppRequestInfo request, int sequence, IRemoteCallback callback) {
                if (InstantAppResolverService.DEBUG_INSTANT) {
                    Slog.m119v(InstantAppResolverService.TAG, NavigationBarInflaterView.SIZE_MOD_START + request.getToken() + "] Phase1 called; posting");
                }
            ...
            public void getInstantAppIntentFilterList(InstantAppRequestInfo request, IRemoteCallback callback) {
                if (InstantAppResolverService.DEBUG_INSTANT) {
                    Slog.m119v(InstantAppResolverService.TAG, NavigationBarInflaterView.SIZE_MOD_START + request.getToken() + "] Phase2 called; posting");
                }
                SomeArgs args = SomeArgs.obtain();
                args.arg1 = request;
                args.arg2 = callback;
        ...
        public void handleMessage(Message message) {
            ...
            switch (action) {
                case 1:
                    ...
                    if (InstantAppResolverService.DEBUG_INSTANT) {
                        Slog.m113d(InstantAppResolverService.TAG, NavigationBarInflaterView.SIZE_MOD_START + request.getToken() + "] Phase1 request; prefix: " + Arrays.toString(request.getHostDigestPrefix()) + ", userId: " + request.getUserHandle().getIdentifier());
                    }
                ...
                case 2:
                    ...
                    args2.recycle();
                    if (InstantAppResolverService.DEBUG_INSTANT) {
                        Slog.m113d(InstantAppResolverService.TAG, NavigationBarInflaterView.SIZE_MOD_START + request2.getToken() + "] Phase2 request; prefix: " + Arrays.toString(request2.getHostDigestPrefix()) + ", userId: " + request2.getUserHandle().getIdentifier());
                    }
