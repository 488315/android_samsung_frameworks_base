...
public class AlarmManager {
    ...
    public void run() {
      try {
        ...
        try {
          ...
          Log.m97e(AlarmManager.TAG, "Unable to report completion to Alarm Manager!", e);
        }
      } catch (Throwable th) {
        try {
          this.mCompletion.alarmComplete(this);
        } catch (Exception e2) {
          Log.m97e(AlarmManager.TAG, "Unable to report completion to Alarm Manager!", e2);
        }
        throw th;
      }
    }
  ...
  public void set(
      int type, long triggerAtMillis, String tag, OnAlarmListener listener, Handler targetHandler) {
    setImpl(
        type,
        triggerAtMillis,
        legacyExactLength(),
  ...
  public void cancel(PendingIntent operation) {
    if (operation == null) {
      ...
      Log.m96e(TAG, "cancel() called with a null PendingIntent");
    } else {
      try {
  ...
  public void cancel(OnAlarmListener listener) {
    ...
    if (wrapper == null) {
      Log.m102w(TAG, "Unrecognized alarm listener " + listener);
    } else {
      wrapper.cancel();
    }
  }
