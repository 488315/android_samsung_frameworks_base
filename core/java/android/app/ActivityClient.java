...
public class ActivityClient {
  ...
  boolean navigateUpTo(
      IBinder token, Intent destIntent, String resolvedType, int resultCode, Intent resultData) {
    try {
  ...
  void overridePendingTransition(
    ...
    try {
      getActivityClientController()
          .overridePendingTransition(token, packageName, enterAnim, exitAnim, backgroundColor);
    } catch (RemoteException e) {
      e.rethrowFromSystemServer();
    }
  }
  ...
  void requestCompatCameraControl(
    ...
    if (!res.getBoolean(C4337R.bool.config_isCameraCompatControlForStretchedIssuesEnabled)) {
      return;
    }
    try {
      getActivityClientController()
          .requestCompatCameraControl(token, showControl, transformationApplied, callback);
    } catch (RemoteException e) {
      e.rethrowFromSystemServer();
    }
  }
