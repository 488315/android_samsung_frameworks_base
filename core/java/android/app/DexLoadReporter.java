...
class DexLoadReporter implements BaseDexClassLoader.Reporter {
  ...
  private void notifyPackageManager(Map<String, String> classLoaderContextMap) {
    ...
    try {
      ActivityThread.getPackageManager()
          .notifyDexLoad(
              packageName, classLoaderContextMap, VMRuntime.getRuntime().vmInstructionSet());
    } catch (RemoteException re) {
      Slog.m116e(TAG, "Failed to notify PM about dex load for package " + packageName, re);
    }
  }
  ...
  private void registerSecondaryDexForProfiling(String dexPath, String[] dataDirs) {
    ...
    if (!secondaryProfileDir.exists() && !secondaryProfileDir.mkdir()) {
      Slog.m115e(TAG, "Could not create the profile directory: " + secondaryCurProfile);
      return;
    }
    try {
      ...
      Slog.m115e(
          TAG, "Failed to create profile for secondary dex " + dexPath + ":" + ex.getMessage());
    }
  }
