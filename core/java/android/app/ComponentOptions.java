  ...
  ComponentOptions(Bundle opts) {
    ...
    if (pendingIntentBalAllowedIsSetExplicitly) {
      this.mPendingIntentBalAllowed =
          Boolean.valueOf(opts.getBoolean(KEY_PENDING_INTENT_BACKGROUND_ACTIVITY_ALLOWED));
    }
    setPendingIntentBackgroundActivityLaunchAllowedByPermission(
        opts.getBoolean(KEY_PENDING_INTENT_BACKGROUND_ACTIVITY_ALLOWED_BY_PERMISSION, false));
  }
