...
final class FragmentManagerImpl extends FragmentManager implements LayoutInflater.Factory2 {
  ...
  final CopyOnWriteArrayList<Pair<FragmentManager.FragmentLifecycleCallbacks, Boolean>>
  ...
  private void throwException(RuntimeException ex) {
    Log.m96e(TAG, ex.getMessage());
    LogWriter logw = new LogWriter(6, TAG);
    PrintWriter pw = new FastPrintWriter((Writer) logw, false, 1024);
    if (this.mHost != null) {
      Log.m96e(TAG, "Activity state:");
      try {
        this.mHost.onDump("  ", null, pw, new String[0]);
      } catch (Exception e) {
        pw.flush();
        Log.m97e(TAG, "Failed dumping state", e);
      }
    } else {
      Log.m96e(TAG, "Fragment manager state:");
      try {
        dump("  ", null, pw, new String[0]);
      } catch (Exception e2) {
        pw.flush();
        Log.m97e(TAG, "Failed dumping state", e2);
      }
    }
    pw.flush();
    throw ex;
  ...
  private boolean popBackStackImmediate(String name, int id, int flags) {
    ...
    if (fragment != null
        && id < 0
        && name == null
        && (childManager = fragment.mChildFragmentManager) != null
  ...
  public void removeOnBackStackChangedListener(
    ...
    ArrayList<FragmentManager.OnBackStackChangedListener> arrayList =
        this.mBackStackChangeListeners;
    if (arrayList != null) {
  ...
  public Fragment getFragment(Bundle bundle, String key) {
    ...
    if (f == null) {
      throwException(
          new IllegalStateException(
              "Fragment no longer exists for key " + key + ": index " + index));
    }
    return f;
  }
  ...
  Animator loadAnimator(Fragment fragment, int transit, boolean enter, int transitionStyle) {
    ...
    if (fragment.getNextAnim() != 0
        && (anim = AnimatorInflater.loadAnimator(this.mHost.getContext(), fragment.getNextAnim()))
            != null) {
      return anim;
    }
    ...
    TypedArray attrs =
        this.mHost
            .getContext()
            .obtainStyledAttributes(transitionStyle, C4337R.styleable.FragmentAnimation);
  ...
  */
  void moveToState(
      final Fragment f, int newState, int transit, int transitionStyle, boolean keepActive) {
    int newState2;
    Animator anim;
    int newState3;
    String resName;
    ...
    if (f.mState <= newState2) {
      ...
      switch (f.mState) {
        case 0:
          if (newState2 > 0) {
            if (DEBUG) {
              Log.m100v(TAG, "moveto CREATED: " + f);
            }
            if (f.mSavedFragmentState != null) {
              ...
              if (f.mTarget != null) {
                f.mTargetRequestCode =
                    f.mSavedFragmentState.getInt(TARGET_REQUEST_CODE_STATE_TAG, 0);
              }
            ...
            if (!f.mCalled) {
              throw new SuperNotCalledException(
                  str + f + " did not call through to super.onAttach()");
            }
            ...
            if (newState3 > 1) {
              if (DEBUG) {
                Log.m100v(TAG, "moveto ACTIVITY_CREATED: " + f);
              }
              if (!f.mFromLayout) {
            ...
            if (newState2 > 3) {
              if (DEBUG) {
                Log.m100v(TAG, "moveto STARTED: " + f);
              }
              f.performStart();
            ...
            if (newState2 > 4) {
              if (DEBUG) {
                Log.m100v(TAG, "moveto RESUMED: " + f);
              }
              f.performResume();
      ...
      switch (f.mState) {
        case 1:
          if (newState2 < 1) {
            ...
            if (anim3 != null) {
              ...
              if (DEBUG) {
                Log.m100v(TAG, "movefrom CREATED: " + f);
              }
              if (!f.mRetaining) {
                f.performDestroy();
        ...
        case 5:
          if (newState2 < 5) {
            if (DEBUG) {
              Log.m100v(TAG, "movefrom RESUMED: " + f);
            }
            f.performPause();
        ...
        case 4:
          if (newState2 < 4) {
            if (DEBUG) {
              Log.m100v(TAG, "movefrom STARTED: " + f);
            }
            f.performStop();
        ...
        case 3:
          if (newState2 < 2) {
            if (DEBUG) {
              Log.m100v(TAG, "movefrom ACTIVITY_CREATED: " + f);
            }
            if (f.mView != null
                && this.mHost.onShouldSaveFragmentState(f)
                && f.mSavedViewState == null) {
                        ...
                        new AnimatorListenerAdapter() { // from class:
                          ...
                          public void onAnimationEnd(Animator anim5) {
                            ...
                            if (container3.indexOfChild(view) == -1 && animator != null) {
                              ...
                              fragmentManagerImpl2.moveToState(
                                  fragment2, fragment2.getStateAfterAnimating(), 0, 0, false);
                            }
                          }
                        });
      ...
      if (anim != null) {
        ...
        if (fragment.mHidden) {
          if (fragment.isHideReplaced()) {
            ...
            anim.addListener(
                new AnimatorListenerAdapter() { // from class: android.app.FragmentManagerImpl.3
                  @Override // android.animation.AnimatorListenerAdapter,
                            // android.animation.Animator.AnimatorListener
                  public void onAnimationEnd(Animator animation) {
  ...
  void moveToState(int newState, boolean always) {
    ...
    if (this.mActive != null) {
      ...
      if (this.mNeedMenuInvalidate
          && (fragmentHostCallback = this.mHost) != null
          && this.mCurState == 5) {
  ...
  void makeActive(Fragment f) {
    ...
    if (DEBUG) {
      Log.m100v(TAG, "Allocated fragment index " + f);
    }
  }
  ...
  void makeInactive(Fragment f) {
    ...
    if (DEBUG) {
      Log.m100v(TAG, "Freeing fragment index " + f);
    }
  ...
  public void removeFragment(Fragment fragment) {
    if (DEBUG) {
      Log.m100v(TAG, "remove: " + fragment + " nesting=" + fragment.mBackStackNesting);
    }
  ...
  public void detachFragment(Fragment fragment) {
    if (DEBUG) {
      Log.m100v(TAG, "detach: " + fragment);
    }
    if (!fragment.mDetached) {
      fragment.mDetached = true;
      if (fragment.mAdded) {
        if (DEBUG) {
          Log.m100v(TAG, "remove from detach: " + fragment);
        }
        synchronized (this.mAdded) {
          this.mAdded.remove(fragment);
        }
  ...
  public void attachFragment(Fragment fragment) {
    ...
    if (fragment.mDetached) {
      ...
      if (!fragment.mAdded) {
        ...
        if (DEBUG) {
          Log.m100v(TAG, "add from attach: " + fragment);
        }
        synchronized (this.mAdded) {
          this.mAdded.add(fragment);
        }
  ...
  private void checkStateLoss() {
    ...
    if (this.mNoTransactionsBecause != null) {
      throw new IllegalStateException(
          "Can not perform this action inside of " + this.mNoTransactionsBecause);
    }
  }
  ...
  */
  ...
  public int allocBackStackIndex(BackStackRecord bse) {
    synchronized (this) {
      ...
      if (arrayList != null && arrayList.size() > 0) {
        ...
        if (DEBUG) {
          Log.m100v(TAG, "Adding back stack index " + index + " with " + bse);
        }
      ...
      if (DEBUG) {
        Log.m100v(TAG, "Setting back stack index " + index2 + " to " + bse);
      }
  ...
  public void setBackStackIndex(int index, BackStackRecord bse) {
    synchronized (this) {
      ...
      if (index < N) {
        if (DEBUG) {
          Log.m100v(TAG, "Setting back stack index " + index + " to " + bse);
        }
        this.mBackStackIndices.set(index, bse);
      } else {
        while (N < index) {
          ...
          if (DEBUG) {
            Log.m100v(TAG, "Adding available back stack index " + N);
          }
  ...
  public void freeBackStackIndex(int index) {
    synchronized (this) {
      ...
      if (DEBUG) {
        Log.m100v(TAG, "Freeing back stack index " + index);
      }
  ...
  private void executePostponedTransaction(
    ...
    while (i < numPostponed) {
      ...
      if (records != null
          && !listener.mIsBack
          && (index2 = records.indexOf(listener.mRecord)) != -1
          && isRecordPop.get(index2).booleanValue()) {
        listener.cancelTransaction();
      } else if (listener.isReady()
        ...
        if (records != null
            ...
            && isRecordPop.get(index).booleanValue()) {
          listener.cancelTransaction();
        } else {
      ...
      if (!canReorder) {
        ...
        if (isRecordPop.get(recordNum).booleanValue()) {
          while (reorderingEnd < numRecords
              && isRecordPop.get(reorderingEnd).booleanValue()
      ...
      for (int i = 0; i < this.mActive.size(); i++) {
        ...
        if (f != null) {
          if (f.mRetainInstance) {
            ...
            if (DEBUG) {
              Log.m100v(TAG, "retainNonConfig: keeping retained " + f);
            }
          }
  ...
  Parcelable saveAllState() {
    ...
    for (int i = 0; i < N2; i++) {
      ...
      if (f != null) {
        ...
        if (DEBUG) {
          Log.m100v(TAG, "Saved state of " + f + ": " + fs.mSavedFragmentState);
        }
      }
    }
    if (!haveFragments) {
      if (DEBUG) {
        Log.m100v(TAG, "saveAllState: no fragments!");
      }
      return null;
    }
    int[] added = null;
    ...
    if (N3 > 0) {
      ...
      for (int i2 = 0; i2 < N3; i2++) {
        ...
        if (DEBUG) {
          Log.m100v(TAG, "saveAllState: adding fragment #" + i2 + ": " + this.mAdded.get(i2));
        }
      }
    }
    ArrayList<BackStackRecord> arrayList = this.mBackStack;
    if (arrayList != null && (N = arrayList.size()) > 0) {
      ...
      for (int i3 = 0; i3 < N; i3++) {
        ...
        if (DEBUG) {
          Log.m100v(TAG, "saveAllState: adding back stack #" + i3 + ": " + this.mBackStack.get(i3));
        }
      }
    }
  ...
  void restoreAllState(Parcelable state, FragmentManagerNonConfig nonConfig) {
    ...
    if (nonConfig != null) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (DEBUG) {
          Log.m100v(TAG, "restoreAllState: re-attaching retained " + f);
        }
        int index = 0;
        ...
        if (index == fms.mActive.length) {
          throwException(
              new IllegalStateException("Could not find active fragment with index " + f.mIndex));
        }
    ...
    for (int i2 = 0; i2 < fms.mActive.length; i2++) {
      ...
      if (fs2 != null) {
        ...
        if (DEBUG) {
          Log.m100v(TAG, "restoreAllState: active #" + i2 + ": " + f2);
        }
    ...
    if (fms.mAdded != null) {
      for (int i4 = 0; i4 < fms.mAdded.length; i4++) {
        ...
        if (f4 == null) {
          throwException(
              new IllegalStateException("No instantiated fragment for index #" + fms.mAdded[i4]));
        }
        f4.mAdded = true;
        if (DEBUG) {
          Log.m100v(TAG, "restoreAllState: added #" + i4 + ": " + f4);
        }
    ...
    if (fms.mBackStack != null) {
      ...
      for (int i5 = 0; i5 < fms.mBackStack.length; i5++) {
        ...
        if (DEBUG) {
          Log.m100v(
              TAG, "restoreAllState: back stack #" + i5 + " (index " + bse.mIndex + "): " + bse);
  ...
  public void attachController(
      FragmentHostCallback<?> host, FragmentContainer container, Fragment parent) {
    if (this.mHost != null) {
  ...
  int getTargetSdk() {
    ...
    if (fragmentHostCallback != null
        && (context = fragmentHostCallback.getContext()) != null
        && (info = context.getApplicationInfo()) != null) {
  ...
  public void registerFragmentLifecycleCallbacks(
      FragmentManager.FragmentLifecycleCallbacks cb, boolean recursive) {
  ...
  void dispatchOnFragmentPreAttached(Fragment f, Context context, boolean onlyRecursive) {
    ...
    Iterator<Pair<FragmentManager.FragmentLifecycleCallbacks, Boolean>> it =
        this.mLifecycleCallbacks.iterator();
    while (it.hasNext()) {
  ...
  void dispatchOnFragmentActivityCreated(
      Fragment f, Bundle savedInstanceState, boolean onlyRecursive) {
    Fragment fragment = this.mParent;
    if (fragment != null) {
      FragmentManager parentManager = fragment.getFragmentManager();
      if (parentManager instanceof FragmentManagerImpl) {
  ...
  void dispatchOnFragmentViewCreated(
      Fragment f, View v, Bundle savedInstanceState, boolean onlyRecursive) {
    Fragment fragment = this.mParent;
    if (fragment != null) {
      FragmentManager parentManager = fragment.getFragmentManager();
      if (parentManager instanceof FragmentManagerImpl) {
  ...
  public View onCreateView(View parent, String name, Context context, AttributeSet attrs) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.Fragment);
    if (fname2 != null) {
      fname = fname2;
    } else {
      fname = a.getString(0);
  ...
  private class PopBackStackState implements OpGenerator {
    ...
    public boolean generateOps(ArrayList<BackStackRecord> records, ArrayList<Boolean> isRecordPop) {
      FragmentManager childManager;
      if (FragmentManagerImpl.this.mPrimaryNav != null
          && this.mId < 0
          && this.mName == null
          && (childManager = FragmentManagerImpl.this.mPrimaryNav.mChildFragmentManager) != null
          && childManager.popBackStackImmediate()) {
        return false;
      }
      return FragmentManagerImpl.this.popBackStackState(
          records, isRecordPop, this.mName, this.mId, this.mFlags);
    }
  }
  ...
  static class StartEnterTransitionListener implements Fragment.OnStartEnterTransitionListener {
    ...
    public void completeTransaction() {
      ...
      this.mRecord.mManager.completeExecute(
          this.mRecord, this.mIsBack, canceled ? false : true, true);
    }
