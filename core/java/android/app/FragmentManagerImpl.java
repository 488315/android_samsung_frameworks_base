...
final class FragmentManagerImpl extends FragmentManager implements LayoutInflater.Factory2 {
    ...
    Animator loadAnimator(Fragment fragment, int transit, boolean enter, int transitionStyle) {
        ...
        TypedArray attrs = this.mHost.getContext().obtainStyledAttributes(transitionStyle, C4337R.styleable.FragmentAnimation);
    ...
    */
    void moveToState(final Fragment f, int newState, int transit, int transitionStyle, boolean keepActive) {
        ...
        if (f.mState != newState2) {
            Log.m102w(TAG, "moveToState: Fragment state for " + f + " not updated inline; expected state " + newState2 + " found " + f.mState);
            f.mState = newState2;
        }
    }
    ...
    */
    ...
    void restoreAllState(Parcelable state, FragmentManagerNonConfig nonConfig) {
        ...
        if (nonConfig != null) {
            ...
            for (int i3 = 0; i3 < count2; i3++) {
                ...
                if (f3.mTargetIndex >= 0) {
                    ...
                    if (f3.mTarget == null) {
                        Log.m102w(TAG, "Re-attaching retained fragment " + f3 + " target no longer exists: " + f3.mTargetIndex);
                        f3.mTarget = null;
                    }
                }
            }
        ...
        if (fms.mBackStack != null) {
            ...
            for (int i5 = 0; i5 < fms.mBackStack.length; i5++) {
                ...
                if (DEBUG) {
                    Log.m100v(TAG, "restoreAllState: back stack #" + i5 + " (index " + bse.mIndex + "): " + bse);
    ...
    public View onCreateView(View parent, String name, Context context, AttributeSet attrs) {
        ...
        if (DEBUG) {
            Log.m100v(TAG, "onCreateView: id=0x" + Integer.toHexString(id) + " fname=" + fname + " existing=" + fragment2);
        }
        if (fragment2 == null) {
