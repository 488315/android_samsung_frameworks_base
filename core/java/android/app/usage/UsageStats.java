...
public final class UsageStats implements Parcelable {
  ...
  public UsageStats(UsageStats stats) {
    ...
    this.mActivities = stats.mActivities.m5443clone();
  ...
  public long getLastTimePackageUsed() {
    return Math.max(
        ...
        Math.max(
            ...
            Math.max(this.mLastTimeForegroundServiceUsed, this.mLastTimeComponentUsed)));
  }
  ...
  private void mergeEventMap(ArrayMap<String, Integer> left, ArrayMap<String, Integer> right) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (left.containsKey(className)) {
        left.put(
            className, Integer.valueOf(Math.max(left.get(className).intValue(), event.intValue())));
      } else {
  ...
  public void add(UsageStats right) {
    ...
    if (right.mBeginTimeStamp > this.mBeginTimeStamp) {
      ...
      this.mLastTimeForegroundServiceUsed =
          Math.max(this.mLastTimeForegroundServiceUsed, right.mLastTimeForegroundServiceUsed);
    }
