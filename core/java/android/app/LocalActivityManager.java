...
public class LocalActivityManager {
  ...
  private void moveToState(LocalActivityRecord r, int desiredState) {
    ...
    if (r.curState != 1) {
      ...
      if (clientRecord == null) {
        Log.m102w(TAG, "Can't get activity record for " + r.f19id);
        return;
      }
      switch (r.curState) {
        case 2:
    ...
    if (lastNonConfigurationInstances == null) {
      ...
      Object instanceObj2 = lastNonConfigurationInstances.get(r.f19id);
    ...
    r.activity =
        this.mActivityThread.startActivityNow(
            this.mParent, r.f19id, r.intent, r.activityInfo, r, r.instanceState, instance, r, r);
    if (r.activity == null) {
      return;
    }
  ...
  private Window performDestroy(LocalActivityRecord r, boolean finish) {
    ...
    if (clientRecord != null) {
      this.mActivityThread.performDestroyActivity(
          clientRecord, finish, 0, false, "LocalActivityManager::performDestroy");
    }
    r.activity = null;
    r.window = null;
    if (finish) {
  ...
  public String getCurrentId() {
    LocalActivityRecord localActivityRecord = this.mResumed;
    if (localActivityRecord != null) {
      return localActivityRecord.f19id;
    }
    return null;
  }
    ...
    if (state != null) {
      for (String id : state.keySet()) {
        try {
          ...
          Log.m97e(TAG, "Exception thrown when restoring LocalActivityManager state", e);
        }
      }
    }
    this.mCurState = 2;
  ...
  public Bundle saveInstanceState() {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (r.instanceState != null) {
        state.putBundle(r.f19id, r.instanceState);
      }
    }
    return state;
  }
  ...
  public HashMap<String, Object> dispatchRetainNonConfigurationInstance() {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (r != null
        ...
        instanceMap.put(r.f19id, instance);
      }
    }
    return instanceMap;
  }
  ...
  public void dispatchDestroy(boolean finishing) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (clientRecord != null) {
        this.mActivityThread.performDestroyActivity(
            clientRecord, finishing, 0, false, "LocalActivityManager::dispatchDestroy");
      }
    }
