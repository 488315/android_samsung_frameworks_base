...
public class ConfirmationPrompt {
  ...
  private ConfirmationPrompt(Context context, CharSequence promptText, byte[] extraData) {
    ...
    this.mConfirmationCallback =
        new IConfirmationCallback.Stub() { // from class: android.security.ConfirmationPrompt.1
          @Override // android.security.apc.IConfirmationCallback
          public void onCompleted(final int result, final byte[] dataThatWasConfirmed)
            ...
            if (ConfirmationPrompt.this.mCallback != null) {
              ...
              if (executor == null) {
                ...
                executor.execute(
                    new Runnable() { // from class: android.security.ConfirmationPrompt.1.1
  ...
  private static boolean isAccessibilityServiceRunning(Context context) {
    try {
      ...
      Log.m102w(TAG, "Unexpected SettingNotFoundException");
      e.printStackTrace();
      return false;
    }
  }
  ...
  public void presentPrompt(Executor executor, ConfirmationCallback callback)
    ...
    int responseCode =
        getService()
            .presentConfirmationPrompt(
                this.mConfirmationCallback,
                this.mPromptText.toString(),
                this.mExtraData,
                locale,
                uiOptionsAsFlags);
    switch (responseCode) {
      ...
      default:
        Log.m102w(
            TAG,
            "Unexpected responseCode=" + responseCode + " from presentConfirmationPrompt() call.");
  ...
  public void cancelPrompt() {
    ...
    Log.m102w(
        TAG, "Unexpected responseCode=" + responseCode + " from cancelConfirmationPrompt() call.");
    throw new IllegalStateException();
  }
