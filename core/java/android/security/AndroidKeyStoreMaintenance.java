        ...
        try {
            try {
                try {
                    ...
                    int userId2 = domain == 0 ? UserHandle.getUserId((int) namespace) : UserHandle.getUserId(Binder.getCallingUid());
                    if (!KeyStoreAuditLog.isAuditLogEnabledAsUser(userId2)) {
                        return 0;
                    }
                    KeyStoreAuditLog.AuditLogParams params = KeyStoreAuditLog.AuditLogParams.init(null, namespace, domain, 1, TAG, 1);
                    params.setUserId(userId2);
                    KeyStoreAuditLog.auditLogPrivilegedAsUser(params);
                    return 0;
                } catch (Exception e) {
                    Log.m97e(TAG, "Can not connect to keystore", e);
                    int userId3 = domain == 0 ? UserHandle.getUserId((int) namespace) : UserHandle.getUserId(Binder.getCallingUid());
                    if (!KeyStoreAuditLog.isAuditLogEnabledAsUser(userId3)) {
                        return 4;
                    }
                    KeyStoreAuditLog.AuditLogParams params2 = KeyStoreAuditLog.AuditLogParams.init(null, namespace, domain, 1, TAG, 4);
                ...
                try {
                    ...
                    int userId4 = domain == 0 ? UserHandle.getUserId((int) namespace) : UserHandle.getUserId(Binder.getCallingUid());
                    if (KeyStoreAuditLog.isAuditLogEnabledAsUser(userId4)) {
                        KeyStoreAuditLog.AuditLogParams params3 = KeyStoreAuditLog.AuditLogParams.init(null, namespace, domain, 1, TAG, errorCode2);
                    ...
                    userId = domain != 0 ? UserHandle.getUserId((int) namespace) : UserHandle.getUserId(Binder.getCallingUid());
            ...
            userId = domain != 0 ? UserHandle.getUserId((int) namespace) : UserHandle.getUserId(Binder.getCallingUid());
            if (KeyStoreAuditLog.isAuditLogEnabledAsUser(userId)) {
                KeyStoreAuditLog.AuditLogParams params4 = KeyStoreAuditLog.AuditLogParams.init(null, namespace, domain, 1, TAG, errorCode);
