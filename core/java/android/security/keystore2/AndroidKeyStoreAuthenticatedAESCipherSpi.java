...
abstract class AndroidKeyStoreAuthenticatedAESCipherSpi extends AndroidKeyStoreCipherSpiBase {
  ...
  abstract static class GCM extends AndroidKeyStoreAuthenticatedAESCipherSpi {
    ...
    protected final void initAlgorithmSpecificParameters() throws InvalidKeyException {
      if (!isEncrypting()) {
        throw new InvalidKeyException(
            "IV required when decrypting. Use IvParameterSpec or AlgorithmParameters to provide"
                + " it.");
      }
    }
    ...
    protected final void initAlgorithmSpecificParameters(AlgorithmParameterSpec params)
      ...
      if (iv.length != 12) {
        throw new InvalidAlgorithmParameterException(
            "Unsupported IV length: " + iv.length + " bytes. Only 12 bytes long IV supported");
      }
    ...
    protected final void initAlgorithmSpecificParameters(AlgorithmParameters params)
      ...
      if (params == null) {
        if (!isEncrypting()) {
          throw new InvalidAlgorithmParameterException(
              "IV required when decrypting. Use GCMParameterSpec or GCM AlgorithmParameters to"
                  + " provide it.");
        }
      } else {
        ...
        try {
          ...
          if (!isEncrypting()) {
            throw new InvalidAlgorithmParameterException(
                "IV and tag length required when decrypting, but not found in parameters: "
                    + params,
                e);
          }
          setIv(null);
  ...
  protected final void initKey(int opmode, Key key) throws InvalidKeyException {
    if (!(key instanceof AndroidKeyStoreSecretKey)) {
      throw new InvalidKeyException(
          "Unsupported key: "
              + (key != null ? key.getClass().getName() : SemCapabilities.FEATURE_TAG_NULL));
    }
