...
public class AndroidKeyStoreKeyFactorySpi extends KeyFactorySpi {
    ...
    protected <T extends KeySpec> T engineGetKeySpec(Key key, Class<T> keySpecClass)
        ...
        if (KeyInfo.class.equals(keySpecClass)) {
            if (!(key instanceof AndroidKeyStorePrivateKey)) {
                throw new InvalidKeySpecException(
                        "Unsupported key type: "
                                + key.getClass().getName()
                                + ". KeyInfo can be obtained only for Android Keystore private"
                                + " keys");
            }
        ...
        if (X509EncodedKeySpec.class.equals(keySpecClass)) {
            if (!(key instanceof AndroidKeyStorePublicKey)) {
                throw new InvalidKeySpecException(
                        "Unsupported key type: "
                                + key.getClass().getName()
                                + ". X509EncodedKeySpec can be obtained only for Android Keystore"
                                + " public keys");
            }
        ...
        if (PKCS8EncodedKeySpec.class.equals(keySpecClass)) {
            ...
            throw new InvalidKeySpecException(
                    "Cannot export key material of public key in PKCS#8 format. Only X.509 format"
                        + " (X509EncodedKeySpec) supported for public keys.");
        }
        ...
        if (equals) {
            ...
            StringBuilder append =
                    new StringBuilder()
                            .append("Obtaining RSAPublicKeySpec not supported for ")
        ...
        if (ECPublicKeySpec.class.equals(keySpecClass)) {
            ...
            StringBuilder append2 =
                    new StringBuilder()
                            .append("Obtaining ECPublicKeySpec not supported for ")
                            .append(key.getAlgorithm())
                            .append(" ");
            if (!(key instanceof AndroidKeyStorePrivateKey)) {
                str = "public";
