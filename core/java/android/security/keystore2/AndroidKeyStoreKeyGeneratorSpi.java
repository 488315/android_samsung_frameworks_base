...
public abstract class AndroidKeyStoreKeyGeneratorSpi extends KeyGeneratorSpi {
    ...
    public static class AES extends AndroidKeyStoreKeyGeneratorSpi {
        ...
        @Override // android.security.keystore2.AndroidKeyStoreKeyGeneratorSpi,
    ...
    protected abstract static class HmacBase extends AndroidKeyStoreKeyGeneratorSpi {
            ...
            try {
                if (params instanceof KeyGenParameterSpec) {
                    ...
                    if (spec.getKeystoreAlias() == null) {
                        throw new InvalidAlgorithmParameterException(
                                "KeyStore entry alias not provided");
                    }
                    this.mRng = random;
                    this.mSpec = spec;
                    this.mKeySizeBits =
                            spec.getKeySize() != -1 ? spec.getKeySize() : this.mDefaultKeySizeBits;
                    if (this.mKeySizeBits <= 0) {
                        throw new InvalidAlgorithmParameterException(
                                "Key size must be positive: " + this.mKeySizeBits);
                    }
                    if (this.mKeySizeBits % 8 != 0) {
                        throw new InvalidAlgorithmParameterException(
                                "Key size must be a multiple of 8: " + this.mKeySizeBits);
                    }
                    try {
                        this.mKeymasterPurposes =
                                KeyProperties.Purpose.allToKeymaster(spec.getPurposes());
                        this.mKeymasterPaddings =
                                KeyProperties.EncryptionPadding.allToKeymaster(
                                        spec.getEncryptionPaddings());
                        if (spec.getSignaturePaddings().length > 0) {
                            throw new InvalidAlgorithmParameterException(
                                    "Signature paddings not supported for symmetric key"
                                        + " algorithms");
                        }
                        this.mKeymasterBlockModes =
                                KeyProperties.BlockMode.allToKeymaster(spec.getBlockModes());
                        if ((spec.getPurposes() & 1) != 0
                                && spec.isRandomizedEncryptionRequired()) {
                            for (int keymasterBlockMode : this.mKeymasterBlockModes) {
                                if (!KeymasterUtils
                                        .isKeymasterBlockModeIndCpaCompatibleWithSymmetricCrypto(
                                                keymasterBlockMode)) {
                                    throw new InvalidAlgorithmParameterException(
                                            "Randomized encryption (IND-CPA) required but may be"
                                                + " violated by block mode: "
                        ...
                        if (this.mKeymasterAlgorithm == 128) {
                            ...
                            if (spec.isDigestsSpecified()) {
                                ...
                                if (keymasterDigestsFromSpec.length != 1
                                        || keymasterDigestsFromSpec[0] != this.mKeymasterDigest) {
                                    throw new InvalidAlgorithmParameterException(
                                            "Unsupported digests specification: "
                                                    + Arrays.asList(spec.getDigests())
                                                    + ". Only "
                                                    + KeyProperties.Digest.fromKeymaster(
                                                            this.mKeymasterDigest)
    ...
    protected SecretKey engineGenerateKey() {
        ...
        ArrayUtils.forEach(
                ...
                new Consumer() { // from class:
                                 // android.security.keystore2.AndroidKeyStoreKeyGeneratorSpi$$ExternalSyntheticLambda0
        ...
        ArrayUtils.forEach(
                this.mKeymasterBlockModes,
                new Consumer() { // from class:
                                 // android.security.keystore2.AndroidKeyStoreKeyGeneratorSpi$$ExternalSyntheticLambda1
                    @Override // java.util.function.Consumer
                    public final void accept(Object obj) {
                        AndroidKeyStoreKeyGeneratorSpi.this.lambda$engineGenerateKey$1(
                                params, (Integer) obj);
                    }
                });
        ArrayUtils.forEach(
                this.mKeymasterPaddings,
                new Consumer() { // from class:
                                 // android.security.keystore2.AndroidKeyStoreKeyGeneratorSpi$$ExternalSyntheticLambda2
                    @Override // java.util.function.Consumer
                    public final void accept(Object obj) {
                        params.add(
                                KeyStore2ParameterUtils.makeEnum(
        ...
        ArrayUtils.forEach(
                this.mKeymasterDigests,
                new Consumer() { // from class:
                                 // android.security.keystore2.AndroidKeyStoreKeyGeneratorSpi$$ExternalSyntheticLambda3
                    @Override // java.util.function.Consumer
                    public final void accept(Object obj) {
                        params.add(
                                KeyStore2ParameterUtils.makeEnum(
        ...
        if (this.mKeymasterAlgorithm == 128 && this.mKeymasterDigests.length != 0) {
            int digestOutputSizeBits =
                    KeymasterUtils.getDigestOutputSizeBits(this.mKeymasterDigests[0]);
            if (digestOutputSizeBits == -1) {
                throw new ProviderException(
                        "HMAC key authorized for unsupported digest: "
                                + KeyProperties.Digest.fromKeymaster(this.mKeymasterDigests[0]));
            }
            ...
            try {
                KeyMetadata metadata =
                        iSecurityLevel.generateKey(
                                descriptor, null, params, flags, additionalEntropy);
                try {
                    String keyAlgorithmJCA =
                            KeyProperties.KeyAlgorithm.fromKeymasterSecretKeyAlgorithm(
                                    this.mKeymasterAlgorithm, this.mKeymasterDigest);
                    SecretKey result =
                            new AndroidKeyStoreSecretKey(
                                    descriptor, metadata, keyAlgorithmJCA, iSecurityLevel);
                    return result;
                    ...
                    try {
                        ...
                        Log.e(
                                TAG,
                                "Failed to delete key after generating successfully but failed to"
                                    + " get the algorithm string.",
                                kse);
                    }
                    throw new ProviderException(
                            "Failed to obtain JCA secret key algorithm name", e2);
                }
