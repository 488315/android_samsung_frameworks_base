...
public final class KeyChain {
  ...
  public static Intent createManageCredentialsIntent(AppUriAuthenticationPolicy policy) {
    ...
    intent.setComponent(
        ComponentName.createRelative("com.android.settings", ".security.RequestManageCredentials"));
    ...
    try {
      ...
      try {
        boolean isCredentialManagementApp =
            keyChainConnection.getService().isCredentialManagementApp(context.getPackageName());
      ...
      throw new RuntimeException(
          "Interrupted while checking whether the caller is the credential management app.", e2);
  ...
  public static KeyPair getKeyPair(Context context, String alias)
    ...
    if (isUcmKeyChainUriAndProvider(alias)) {
      if (isAndroidProvider(alias)) {
        ...
        Log.m94d(
            LOG, "Provider is ANDROID_SOURCE, flow default sequence with alias : " + originalAlias);
        alias = originalAlias;
      } else {
        ...
        if (lService != null) {
          try {
            ...
            List<X509Certificate> chain =
                store.getCertificateChain(toCertificate(certificateBytes));
            return (X509Certificate[]) chain.toArray(new X509Certificate[chain.size()]);
          } catch (RemoteException re) {
            Log.m96e(LOG, "Remote Exception " + re);
  ...
  public static String getWifiKeyGrantAsUser(Context context, UserHandle user, String alias) {
    try {
      try {
        ...
        Thread.currentThread().interrupt();
        Log.m99i(LOG, "Interrupted while getting grant for wifi", e);
        return null;
      }
    } catch (RemoteException | RuntimeException e2) {
      Log.m99i(LOG, "Couldn't get grant for wifi", e2);
      return null;
    }
  }
  ...
  public static boolean hasWifiKeyGrantAsUser(Context context, UserHandle user, String alias) {
    try {
      try {
        ...
        Log.m99i(LOG, "Couldn't query grant for wifi", e);
        return false;
      }
    } catch (InterruptedException e2) {
      Thread.currentThread().interrupt();
      Log.m99i(LOG, "Interrupted while querying grant for wifi", e2);
      return false;
    }
  }
  ...
  private static boolean isKeyChainUri(String uri) {
    ...
    Log.m94d(LOG, "isKeyChainUri: " + uri);
  ...
  private static String getSource(String uri) {
    ...
    if (paths == null || paths.size() < 3) {
      Log.m94d(LOG, "getSource: null or wrong");
      return null;
    }
    try {
  ...
  private static PrivateKey getUCMPrivateKey(String alias) {
    IEDMProxy lService = EnterpriseDeviceManager.EDMProxyServiceHelper.getService();
    if (lService == null) {
      Log.m96e(LOG, "getUCMPrivateKey. lService is null");
      return null;
    }
    try {
      byte[] certificateBytes = lService.ucmGetCertificateChain(alias);
      if (certificateBytes == null) {
        Log.m96e(LOG, "getUCMPrivateKey. certificateBytes is null");
        return null;
      }
  ...
  private static boolean isUcmKeyChainUriAndProvider(String alias) {
    ...
    Log.m94d(LOG, "Provider : " + provider);
