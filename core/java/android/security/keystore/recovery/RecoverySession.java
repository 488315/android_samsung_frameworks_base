...
public class RecoverySession implements AutoCloseable {
  ...
  public Map<String, Key> recoverKeyChainSnapshot(
    ...
    try {
      Map grantAliases =
          this.mRecoveryController
              .getBinder()
              .recoverKeyChainSnapshot(this.mSessionId, recoveryKeyBlob, applicationKeys);
    ...
    for (String alias : grantAliases.keySet()) {
      ...
      try {
        ...
        throw new InternalRecoveryServiceException(
            String.format(Locale.US, "Failed to get key '%s' from grant '%s'", alias, grantAlias),
            e);
      }
    }
    return keysByAlias;
  ...
  public void close() {
    try {
      ...
      Log.m97e(TAG, "Unexpected error trying to close session", e);
    }
  }
}
