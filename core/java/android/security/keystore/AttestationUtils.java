...
public abstract class AttestationUtils {
  ...
  public static X509Certificate[] parseCertificateChain(KeymasterCertificateChain kmChain)
      throws KeyAttestationException {
    Collection<byte[]> rawChain = kmChain.getCertificates();
    if (rawChain.size() < 2) {
      throw new KeyAttestationException(
    ...
    try {
      ...
      return (X509Certificate[])
          CertificateFactory.getInstance("X.509")
              .generateCertificates(new ByteArrayInputStream(concatenatedRawChain.toByteArray()))
  ...
  public static X509Certificate[] attestDeviceIds(
      Context context, int[] idTypes, byte[] attestationChallenge)
    ...
    KeyGenParameterSpec.Builder builder =
        new KeyGenParameterSpec.Builder(keystoreAlias, 4)
    ...
    try {
      KeyPairGenerator keyPairGenerator =
          KeyPairGenerator.getInstance(KeyProperties.KEY_ALGORITHM_EC, AndroidKeyStoreSpi.NAME);
      ...
      X509Certificate[] certificateChain =
          (X509Certificate[]) Arrays.copyOf(certs, certs.length, X509Certificate[].class);
