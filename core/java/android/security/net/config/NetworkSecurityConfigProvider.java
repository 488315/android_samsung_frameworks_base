...
public final class NetworkSecurityConfigProvider extends Provider {
  ...
  public static void install(Context context) {
    ...
    if (pos != 1) {
      throw new RuntimeException(
          "Failed to install provider as highest priority provider. Provider was installed at"
              + " position "
              + pos);
    }
  ...
  public static void handleNewApplication(Context context) {
    ...
    if (defaultConfig != null
        && defaultConfig.isCleartextTrafficPermitted() != config.isCleartextTrafficPermitted()) {
      Log.m102w(LOG_TAG, mProcessName + ": New config does not match the previously set config.");
