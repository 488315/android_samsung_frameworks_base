...
public abstract class RecognitionService extends Service {
  ...
  */
    ...
    try {
      if (sessionState == null) {
        if (this.mSessions.size() >= getMaxConcurrentSessionsCount()) {
          ...
          Log.m98i(
              TAG,
              "#startListening received when the service's capacity is full - ignoring this call.");
          return;
        }
        if (!intent.hasExtra(RecognizerIntent.EXTRA_AUDIO_SOURCE)
          ...
          if (preflightPermissionCheckPassed
            ...
            Log.m98i(
                TAG,
                "#startListening received from a caller without permission"
      ...
      Log.m98i(
          TAG,
          "#startListening received for a listener which is already in session - ignoring this"
              + " call.");
    } catch (RemoteException e) {
      Log.m94d(TAG, "#onError call from #startListening failed.");
    }
  }
  ...
  public void dispatchStopListening(IRecognitionListener listener) {
    ...
    if (sessionState == null) {
      try {
        ...
        Log.m94d(TAG, "#onError call from #stopListening failed.");
      }
      Log.m102w(
          TAG,
          "#stopListening received for a listener which has not started a session - ignoring this"
              + " call.");
      return;
    }
  ...
  public void dispatchCheckRecognitionSupport(
      Intent intent, IRecognitionSupportCallback callback, AttributionSource attributionSource) {
    onCheckRecognitionSupport(intent, attributionSource, new SupportCallback(callback));
  }

  /* JADX INFO: Access modifiers changed from: private */
  public void dispatchTriggerModelDownload(
      Intent intent, AttributionSource attributionSource, final IModelDownloadListener listener) {
    if (listener == null) {
  ...
  private static class ModelDownloadArgs {
    ...
    private ModelDownloadArgs(
        Intent intent, AttributionSource attributionSource, IModelDownloadListener listener) {
      this.mIntent = intent;
      this.mAttributionSource = attributionSource;
      this.mListener = listener;
    }
      ...
      if (service != null) {
        service.mHandler.sendMessage(
            Message.obtain(
                service.mHandler,
                1,
                new StartListeningArgs(recognizerIntent, listener, attributionSource)));
      }
    }
      ...
      if (service != null) {
        service.mHandler.sendMessage(
            Message.obtain(
                service.mHandler,
                5,
                new CheckRecognitionSupportArgs(recognizerIntent, callback, attributionSource)));
      }
    }
      ...
      if (service != null) {
        service.mHandler.sendMessage(
            Message.obtain(
                service.mHandler,
                6,
                new ModelDownloadArgs(recognizerIntent, attributionSource, listener)));
      }
    }
  ...
  void finishDataDelivery(SessionState sessionState) {
    if (sessionState.mStartedDataDelivery) {
      ...
      PermissionChecker.finishDataDelivery(
          this, op, sessionState.mCallback.getAttributionContextForCaller().getAttributionSource());
    }
  }
