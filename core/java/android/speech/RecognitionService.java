    ...
    */
        ...
        try {
            if (sessionState == null) {
                if (this.mSessions.size() >= getMaxConcurrentSessionsCount()) {
                    listener.onError(8);
                    Log.m98i(TAG, "#startListening received when the service's capacity is full - ignoring this call.");
                    return;
                }
                if (!intent.hasExtra(RecognizerIntent.EXTRA_AUDIO_SOURCE) && !checkPermissionForPreflightNotHardDenied(attributionSource)) {
                    ...
                    if (preflightPermissionCheckPassed || !checkPermissionAndStartDataDelivery(sessionState)) {
                        ...
                        Log.m98i(TAG, "#startListening received from a caller without permission android.permission.RECORD_AUDIO.");
                    }
                    return;
                }
            ...
            listener.onError(5);
            Log.m98i(TAG, "#startListening received for a listener which is already in session - ignoring this call.");
    ...
    public void dispatchStopListening(IRecognitionListener listener) {
        ...
        if (sessionState == null) {
            ...
            Log.m102w(TAG, "#stopListening received for a listener which has not started a session - ignoring this call.");
            return;
        }
    ...
    public void dispatchCancel(IRecognitionListener listener) {
        SessionState sessionState = this.mSessions.get(listener.asBinder());
        if (sessionState == null) {
            Log.m102w(TAG, "#cancel received for a listener which has not started a session - ignoring this call.");
        } else {
