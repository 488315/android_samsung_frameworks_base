...
class PlaybackSynthesisCallback extends AbstractSynthesisCallback {
    ...
    void stop() {
        synchronized (this.mStateLock) {
            ...
            if (this.mStatusCode == -2) {
                Log.m102w(TAG, "stop() called twice");
                return;
            }
    ...
    public int start(int sampleRateInHz, int audioFormat, int channelCount) {
        if (audioFormat != 3 && audioFormat != 2 && audioFormat != 4) {
            Log.m102w(TAG, "Audio format encoding " + audioFormat + " not supported. Please use one of AudioFormat.ENCODING_PCM_8BIT, AudioFormat.ENCODING_PCM_16BIT or AudioFormat.ENCODING_PCM_FLOAT");
        }
        this.mDispatcher.dispatchOnBeginSynthesis(sampleRateInHz, audioFormat, channelCount);
        int channelConfig = BlockingAudioTrack.getChannelConfig(channelCount);
        synchronized (this.mStateLock) {
            if (channelConfig == 0) {
                Log.m96e(TAG, "Unsupported number of channels :" + channelCount);
                this.mStatusCode = -5;
                return -1;
            }
    ...
    public int done() {
        synchronized (this.mStateLock) {
            ...
            SynthesisPlaybackQueueItem item = this.mItem;
            if (item == null) {
                Log.m102w(TAG, "done() was called before start() call");
                int i = this.mStatusCode;
                if (i == 0) {
