...
public class TextToSpeech {
            ...
            try {
                ...
                if (result >= 0) {
                    ...
                    if (TextUtils.isEmpty(voiceName)) {
                        Log.m102w(TAG, "Couldn't find the default voice for " + language + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + country + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + variant);
                        return -2;
                    }
                    if (service.loadVoice(getCallerIdentity(), voiceName) == -1) {
                        Log.m102w(TAG, "The service claimed " + language + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + country + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + variant + " was available with voice name " + voiceName + " but loadVoice returned ERROR");
                        return -2;
                    }
                    Voice voice = getVoice(service, voiceName);
                    if (voice == null) {
                        Log.m102w(TAG, "getDefaultVoiceNameFor returned " + voiceName + " for locale " + language + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + country + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + variant + " but getVoice returns null");
                        return -2;
                    }
    ...
    private static boolean verifyIntegerBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key) && !(bundle.get(key) instanceof Integer) && !(bundle.get(key) instanceof Long)) {
            bundle.remove(key);
            Log.m102w(TAG, "Synthesis request paramter " + key + " containst value  with invalid type. Should be an Integer or a Long");
            return false;
        }
        return true;
    }
    ...
    private static boolean verifyStringBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key) && !(bundle.get(key) instanceof String)) {
            bundle.remove(key);
            Log.m102w(TAG, "Synthesis request paramter " + key + " containst value  with invalid type. Should be a String");
            return false;
        }
        return true;
    }
    ...
    private static boolean verifyBooleanBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key) && !(bundle.get(key) instanceof Boolean) && !(bundle.get(key) instanceof String)) {
            bundle.remove(key);
            Log.m102w(TAG, "Synthesis request paramter " + key + " containst value  with invalid type. Should be a Boolean or String");
            return false;
        }
        return true;
    }
    ...
    private static boolean verifyFloatBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key) && !(bundle.get(key) instanceof Float) && !(bundle.get(key) instanceof Double)) {
            bundle.remove(key);
            Log.m102w(TAG, "Synthesis request paramter " + key + " containst value  with invalid type. Should be a Float or a Double");
            return false;
        }
        return true;
    }
                ...
                try {
                    try {
                        ...
                        if (onlyEstablishedConnection && !isEstablished()) {
                            Log.m102w(TextToSpeech.TAG, method + " failed: TTS engine connection not fully set up");
                            return errorResult;
                        }
