...
class FileSynthesisCallback extends AbstractSynthesisCallback {
    ...
    public int start(int sampleRateInHz, int audioFormat, int channelCount) {
        if (audioFormat != 3 && audioFormat != 2 && audioFormat != 4) {
            Log.m96e(TAG, "Audio format encoding " + audioFormat + " not supported. Please use one of AudioFormat.ENCODING_PCM_8BIT, AudioFormat.ENCODING_PCM_16BIT or AudioFormat.ENCODING_PCM_FLOAT");
        }
        ...
        synchronized (this.mStateLock) {
            ...
            if (this.mStarted) {
                Log.m96e(TAG, "Start called twice");
                return -1;
            }
            this.mStarted = true;
            ...
            try {
                ...
                Log.m97e(TAG, "Failed to write wav header to output file descriptor", ex);
                synchronized (this.mStateLock) {
                    cleanUp();
                    this.mStatusCode = -5;
                    return -1;
    ...
    public int audioAvailable(byte[] buffer, int offset, int length) {
        synchronized (this.mStateLock) {
            ...
            FileChannel fileChannel = this.mFileChannel;
            if (fileChannel == null) {
                Log.m96e(TAG, "File not open");
                this.mStatusCode = -5;
                return -1;
            }
            if (!this.mStarted) {
                Log.m96e(TAG, "Start method was not called");
                return -1;
            }
            ...
            try {
                ...
                Log.m97e(TAG, "Failed to write to output file descriptor", ex);
                synchronized (this.mStateLock) {
                    cleanUp();
                    this.mStatusCode = -5;
                    return -1;
    ...
    public int done() {
        synchronized (this.mStateLock) {
            if (this.mDone) {
                Log.m102w(TAG, "Duplicate call to done()");
                return -1;
            }
            int i = this.mStatusCode;
            if (i == -2) {
