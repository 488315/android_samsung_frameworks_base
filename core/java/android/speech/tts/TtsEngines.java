...
public class TtsEngines {
        ...
        try {
            try {
                try {
                    ...
                    if (parser2 == null) {
                        Log.m102w(TAG, "No meta-data found for :" + si);
                        if (parser2 != null) {
                            parser2.close();
                        }
                        return null;
                    ...
                    if (!XML_TAG_NAME.equals(parser2.getName())) {
                        Log.m102w(TAG, "Package " + si + " uses unknown tag :" + parser2.getName());
                        if (parser2 != null) {
                            parser2.close();
                        }
                        return null;
                    ...
                    TypedArray array = res.obtainAttributes(attrs, C4337R.styleable.TextToSpeechEngine);
                    ...
                    Log.m102w(TAG, "Could not load resources for : " + si);
                    if (0 != 0) {
                        parser.close();
                    }
                    return null;
                ...
                Log.m102w(TAG, "Error parsing metadata for " + si + ":" + e2);
                if (0 != 0) {
                    parser.close();
                }
                return null;
            } catch (XmlPullParserException e3) {
                Log.m102w(TAG, "Error parsing metadata for " + si + ":" + e3);
                if (0 != 0) {
                    parser.close();
                }
                return null;
    ...
    public Locale getLocalePrefForEngine(String engineName, String prefValue) {
        ...
        if (result == null) {
            Log.m102w(TAG, "Failed to parse locale " + localeString + ", returning en_US instead");
            return Locale.US;
        }
        return result;
    }
    ...
    public Locale parseLocaleString(String localeString) {
        ...
        if (!TextUtils.isEmpty(localeString)) {
            ...
            if (split.length == 0) {
                Log.m102w(TAG, "Failed to convert " + localeString + " to a valid Locale object. Only separators");
                return null;
            }
            if (split.length > 3) {
                Log.m102w(TAG, "Failed to convert " + localeString + " to a valid Locale object. Too many separators");
                return null;
            }
        ...
        try {
            ...
            Log.m102w(TAG, "Failed to convert " + localeString + " to a valid Locale object.");
            return null;
        }
    }
