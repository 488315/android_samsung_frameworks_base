...
final class SynthesisPlaybackQueueItem extends PlaybackQueueItem
  ...
  public void onMarkerReached(AudioTrack track) {
    ...
    if (marker == null) {
      Log.m96e(TAG, "onMarkerReached reached called but no marker in queue");
    } else {
  ...
  void put(byte[] buffer) throws InterruptedException {
    try {
      this.mListLock.lock();
      while (this.mAudioTrack.getAudioLengthMs(this.mUnconsumedBytes) > MAX_UNCONSUMED_AUDIO_MS
          && !this.mStopped) {
        this.mNotFull.await();
      }
      if (this.mStopped) {
