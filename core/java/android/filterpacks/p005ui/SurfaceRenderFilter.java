package android.filterpacks.p005ui;
...
public class SurfaceRenderFilter extends Filter implements SurfaceHolder.Callback {
  ...
  public void prepare(FilterContext context) {
    ...
    MutableFrameFormat screenFormat =
        ImageFormat.create(this.mSurfaceView.getWidth(), this.mSurfaceView.getHeight(), 3, 3);
  ...
  public void process(FilterContext context) {
    Frame gpuFrame;
    if (!this.mIsBound) {
      Log.m102w(TAG, this + ": Ignoring frame as there is no surface to render to!");
      return;
    }
    if (this.mLogVerbose) {
      Log.m100v(TAG, "Starting frame processing");
    }
    ...
    if (currentAspectRatio != this.mAspectRatio) {
      if (this.mLogVerbose) {
        Log.m100v(
            TAG, "New aspect ratio: " + currentAspectRatio + ", previously: " + this.mAspectRatio);
      }
      this.mAspectRatio = currentAspectRatio;
      updateTargetRect();
    }
    if (this.mLogVerbose) {
      Log.m100v(TAG, "Got input format: " + input.getFormat());
    }
      ...
      switch (this.mRenderMode) {
        ...
        case 1:
          if (relativeAspectRatio > 1.0f) {
            shaderProgram.setTargetRect(
                0.5f - (0.5f / relativeAspectRatio), 0.0f, 1.0f / relativeAspectRatio, 1.0f);
            break;
          } else {
            shaderProgram.setTargetRect(
                0.0f, 0.5f - (relativeAspectRatio * 0.5f), 1.0f, relativeAspectRatio);
            break;
          }
        case 2:
          if (relativeAspectRatio > 1.0f) {
