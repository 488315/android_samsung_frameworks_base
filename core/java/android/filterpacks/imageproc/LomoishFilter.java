...
public class LomoishFilter extends Filter {
    ...
    public LomoishFilter(String name) {
        ...
        this.mLomoishShader =
                "precision mediump float;\n"
                    ...
                    + "  vec3 bw_color = clamp(xform + (temp - xform) * (dither - 0.5), 0.0,"
                    + " 1.0);\n"
                    + "  coord = v_texcoord - vec2(0.5, 0.5);\n"
                    + "  float dist = length(coord * scale);\n"
                    + "  float lumen = 0.85 / (1.0 + exp((dist * inv_max_dist - 0.73) * 20.0)) +"
                    + " 0.15;\n"
                ...
                ShaderProgram shaderProgram =
                        new ShaderProgram(
                                ...
                                "precision mediump float;\n"
                                    ...
                                    + "  c_color.b = s_color.b * 0.5 + 0.25;\n"
                                    + "  float dither = rand(v_texcoord + seed);\n"
                                    + "  vec3 xform = clamp((c_color.rgb - 0.15) * 1.53846, 0.0,"
                                    + " 1.0);\n"
                                    + "  vec3 temp = clamp((color.rgb + stepsize - 0.15) * 1.53846,"
                                    + " 0.0, 1.0);\n"
                                    + "  vec3 bw_color = clamp(xform + (temp - xform) * (dither -"
                                    + " 0.5), 0.0, 1.0);\n"
                                    + "  coord = v_texcoord - vec2(0.5, 0.5);\n"
                                    + "  float dist = length(coord * scale);\n"
                                    + "  float lumen = 0.85 / (1.0 + exp((dist * inv_max_dist -"
                                    + " 0.73) * 20.0)) + 0.15;\n"
                                    + "  gl_FragColor = vec4(bw_color * lumen, color.a);\n"
                                    + "}\n");
    ...
    private void initParameters() {
        if (this.mProgram != null) {
            ...
            float max_dist =
                    ((float) Math.sqrt((scale[0] * scale[0]) + (scale[1] * scale[1]))) * 0.5f;
