...
public class GrainFilter extends Filter {
    ...
    public GrainFilter(String name) {
        ...
        this.mGrainShader =
                "precision mediump float;\n"
                    ...
                    + "varying vec2 v_texcoord;\n"
                    + "void main() {\n"
                    + "  float noise = texture2D(tex_sampler_1, v_texcoord + vec2(-stepX,"
                    + " -stepY)).r * 0.224;\n"
                    + "  noise += texture2D(tex_sampler_1, v_texcoord + vec2(-stepX, stepY)).r *"
                    + " 0.224;\n"
                    + "  noise += texture2D(tex_sampler_1, v_texcoord + vec2(stepX, -stepY)).r *"
                    + " 0.224;\n"
                    + "  noise += texture2D(tex_sampler_1, v_texcoord + vec2(stepX, stepY)).r *"
                    ...
                    + "  float energy = 0.33333 * color.r + 0.33333 * color.g + 0.33333 *"
                    + " color.b;\n"
                ...
                ShaderProgram shaderProgram =
                        new ShaderProgram(
                                ...
                                "precision mediump float;\n"
                                    ...
                                    + "}\n"
                                    + "void main() {\n"
                                    + "  gl_FragColor = vec4(rand(v_texcoord + seed), 0.0, 0.0,"
                                    + " 1.0);\n"
                                    + "}\n");
                ...
                ShaderProgram shaderProgram2 =
                        new ShaderProgram(
                                ...
                                "precision mediump float;\n"
                                    ...
                                    + "varying vec2 v_texcoord;\n"
                                    + "void main() {\n"
                                    + "  float noise = texture2D(tex_sampler_1, v_texcoord +"
                                    + " vec2(-stepX, -stepY)).r * 0.224;\n"
                                    + "  noise += texture2D(tex_sampler_1, v_texcoord +"
                                    + " vec2(-stepX, stepY)).r * 0.224;\n"
                                    + "  noise += texture2D(tex_sampler_1, v_texcoord + vec2(stepX,"
                                    + " -stepY)).r * 0.224;\n"
                                    + "  noise += texture2D(tex_sampler_1, v_texcoord + vec2(stepX,"
                                    + " stepY)).r * 0.224;\n"
                                    + "  noise += 0.4448;\n"
                                    + "  noise *= scale;\n"
                                    + "  vec4 color = texture2D(tex_sampler_0, v_texcoord);\n"
                                    + "  float energy = 0.33333 * color.r + 0.33333 * color.g +"
                                    + " 0.33333 * color.b;\n"
                                    + "  float mask = (1.0 - sqrt(energy));\n"
                                    + "  float weight = 1.0 - 1.333 * mask * noise;\n"
                                    + "  gl_FragColor = vec4(color.rgb * weight, color.a);\n"
    ...
    public void process(FilterContext context) {
        ...
        if (this.mNoiseProgram == null
                || this.mGrainProgram == null
                || inputFormat.getTarget() != this.mTarget) {
