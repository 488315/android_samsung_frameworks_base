...
public class AutoFixFilter extends Filter {
    private static final int[] normal_cdf = {
        ...
        446,
        446,
        WearSettingsEnums.ACCESSIBILITY_VIBRATION_WATCH_SWITCH,
        WearSettingsEnums.ACCESSIBILITY_VIBRATION_WATCH_SWITCH,
        WearSettingsEnums.ACCESSIBILITY_VIBRATION_WATCH_SWITCH,
        448,
        448,
        ...
        498,
        498,
        WearSettingsEnums.CONNECTIVITY_UWB,
        WearSettingsEnums.CONNECTIVITY_UWB,
        WearSettingsEnums.CONNECTIVITY_UWB,
        500,
        501,
                ...
                ShaderProgram shaderProgram =
                        new ShaderProgram(
                                ...
                                "precision mediump float;\n"
                                    ...
                                    + "  float alpha;\n"
                                    + "  if (mask_value > 0.0) {\n"
                                    + "    alpha = (pow(2.0 * mask_value, 1.5) - 1.0) * scale +"
                                    + " 1.0;\n"
                                    + "  } else { \n"
                                    + "    alpha = (pow(2.0 * mask_value, 2.0) - 1.0) * scale +"
                                    + " 1.0;\n"
                                    + "  }\n"
                                    + "  float index = energy * hist_scale + hist_offset;\n"
                                    + "  vec4 temp = texture2D(tex_sampler_1, vec2(index, 0.5));\n"
                                    ...
                                    + "  temp = texture2D(tex_sampler_2, vec2(index, 0.5));\n"
                                    + "  value = temp.g + temp.r * shift_scale;\n"
                                    + "  float dst_energy = energy * alpha + value * (1.0 -"
                                    + " alpha);\n"
                                    + "  float max_energy = energy / max(color.r, max(color.g,"
                                    + " color.b));\n"
                                    + "  if (dst_energy > max_energy) {\n"
                                    + "    dst_energy = max_energy;\n"
                                    + "  }\n"
                                    ...
                                    + "    gl_FragColor = color;\n"
                                    + "  } else {\n"
                                    + "    gl_FragColor = vec4(color.rgb * dst_energy / energy,"
                                    + " color.a);\n"
                                    + "  }\n"
                                    + "}\n");
