...
public class StraightenFilter extends Filter {
  ...
  private void updateParameters() {
    ...
    int i6 = this.mHeight;
    Point p2 =
        new Point(((-cosTheta) * i5) - (i6 * sinTheta), ((-sinTheta) * i5) + (i6 * cosTheta));
    int i7 = this.mWidth;
    int i8 = this.mHeight;
    Point p3 = new Point((i7 * cosTheta) - (i8 * sinTheta), (i7 * sinTheta) + (i8 * cosTheta));
    float maxWidth = Math.max(Math.abs(p0.f59x), Math.abs(p1.f59x));
    float maxHeight = Math.max(Math.abs(p0.f60y), Math.abs(p1.f60y));
    float scale = Math.min(this.mWidth / maxWidth, this.mHeight / maxHeight) * 0.5f;
    p0.set(((p0.f59x * scale) / this.mWidth) + 0.5f, ((p0.f60y * scale) / this.mHeight) + 0.5f);
    p1.set(((p1.f59x * scale) / this.mWidth) + 0.5f, ((p1.f60y * scale) / this.mHeight) + 0.5f);
    p2.set(((p2.f59x * scale) / this.mWidth) + 0.5f, ((p2.f60y * scale) / this.mHeight) + 0.5f);
    p3.set(((p3.f59x * scale) / this.mWidth) + 0.5f, ((p3.f60y * scale) / this.mHeight) + 0.5f);
