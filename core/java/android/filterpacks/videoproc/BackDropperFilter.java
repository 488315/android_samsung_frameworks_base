...
public class BackDropperFilter extends Filter {
    ...
    private static final String mBgMaskShader =
            "uniform sampler2D tex_sampler_0;\n"
                ...
                + "  float alpha = 0.5 * is_fg_sml + 0.3 * float(is_fg_mid) + 0.2 *"
                + " float(is_fg_lrg);\n"
    ...
    private static final String mBgSubtractForceShader =
            "  vec4 ghost_rgb = (fg_adjusted * 0.7 + vec4(0.3,0.3,0.4,0.))*0.65 + \n"
                ...
                + "                                     (mask.a - glow_start) / (glow_max -"
                + " glow_start) ) : \n"
                + "                 mask.a < mask_blend_fg ? mix(vec4(0.9,0.9,1.0,1.0), ghost_rgb,"
                + " \n"
                + "                                    (mask.a - glow_max) / (mask_blend_fg -"
                + " glow_max) ) : \n"
                + "                 ghost_rgb;\n"
                + "}\n";
    ...
    private static String mSharedUtilShader =
            "precision mediump float;\n"
                ...
                + "const mat4 coeff_yuv = mat4(0.299, -0.168736,  0.5,      0.000, 0.587,"
                + " -0.331264, -0.418688, 0.000, 0.114,  0.5,      -0.081312, 0.000, 0.000,  0.5,  "
                + "     0.5,      1.000 );\n"
    ...
    private void allocateFrames(FrameFormat inputFormat, FilterContext context) {
        ...
        this.mBgMaskProgram.setHostValue(
                "accept_variance", Float.valueOf(this.mAcceptStddev * this.mAcceptStddev));
        ...
        if (this.mUseTheForce) {
            this.mBgSubtractProgram =
                    new ShaderProgram(
                            context,
                            mSharedUtilShader + mBgSubtractShader + mBgSubtractForceShader);
        } else {
            this.mBgSubtractProgram =
        ...
        this.mBgSubtractProgram.setHostValue(
                "exposure_change", Float.valueOf(this.mExposureChange));
        this.mBgSubtractProgram.setHostValue(
    ...
    public void process(FilterContext filterContext) {
        ...
        this.mBgDistProgram.process(
                new Frame[] {this.mVideoInput, this.mBgMean[i], this.mBgVariance[i]},
                this.mDistance);
        ...
        if (this.mFrameCount <= this.mLearningDuration) {
            ...
            if (this.mFrameCount == this.mLearningDuration - this.mLearningVerifyDuration) {
                ...
                this.mMaskVerifyProgram.process(
                        new Frame[] {this.mMaskVerify[i], this.mMask}, this.mMaskVerify[z ? 1 : 0]);
            ...
            if (this.mFrameCount == this.mLearningDuration) {
                ...
                if (this.mLogVerbose) {
                    Log.v(
                            TAG,
                            String.format(
                                    "Mask_average is %d, threshold is %d",
                                    Integer.valueOf(i2), 20));
                }
                if (i2 >= 20) {
                    this.mStartLearning = true;
                } else {
        ...
        if (this.mFrameCount < this.mLearningDuration - this.mLearningVerifyDuration
            ...
            this.mBgUpdateMeanProgram.process(
                    new Frame[] {this.mVideoInput, this.mBgMean[i], this.mMask},
                    this.mBgMean[z ? 1 : 0]);
            this.mBgMean[z ? 1 : 0].generateMipMap();
            this.mBgMean[z ? 1 : 0].setTextureParameter(10241, 9985);
            this.mBgUpdateVarianceProgram.process(
                    new Frame[] {
                        this.mVideoInput, this.mBgMean[i], this.mBgVariance[i], this.mMask
                    },
                    this.mBgVariance[z ? 1 : 0]);
            ...
            if (this.startTime == -1) {
                ...
                Log.v(
                        ...
                        "Avg. frame duration: "
                                + String.format(
                                        "%.2f",
                                        Double.valueOf((elapsedRealtime - this.startTime) / 30.0d))
                                + " ms. Avg. fps: "
                                + String.format(
                                        "%.2f",
                                        Double.valueOf(
                                                1000.0d
                                                        / ((elapsedRealtime - this.startTime)
                                                                / 30.0d))));
    ...
    private void updateBgScaling(Frame video, Frame background, boolean fitModeChanged) {
        ...
        float backgroundAspect =
                background.getFormat().getWidth() / background.getFormat().getHeight();
