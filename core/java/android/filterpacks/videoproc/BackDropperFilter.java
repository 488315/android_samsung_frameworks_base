...
public class BackDropperFilter extends Filter {
  ...
  private static final String RGB_TO_YUV_MATRIX =
      "0.299, -0.168736,  0.5,      0.000, 0.587, -0.331264, -0.418688, 0.000, 0.114,  0.5,     "
          + " -0.081312, 0.000, 0.000,  0.5,       0.5,      1.000 ";
  ...
  private static final String mAutomaticWhiteBalance =
      "uniform sampler2D tex_sampler_0;\n"
          ...
          + "                         mean_bg.b / mean_video.b * green_normalizer, 1.) *"
          + " auto_wb_scale; \n"
  ...
  private static final String mBgSubtractShader =
      "uniform mat3 bg_fit_transform;\n"
          ...
          + "  vec4 wb_auto_scale = texture2D(tex_sampler_3, v_texcoord) * exposure_change /"
          + " auto_wb_scale;\n"
          + "  vec4 wb_manual_scale = vec4(1. + whitebalancered_change, 1., 1. +"
          + " whitebalanceblue_change, 1.);\n"
  ...
  private static String mSharedUtilShader =
      "precision mediump float;\n"
          ...
          + "const mat4 coeff_yuv = mat4(0.299, -0.168736,  0.5,      0.000, 0.587, -0.331264,"
          + " -0.418688, 0.000, 0.114,  0.5,      -0.081312, 0.000, 0.000,  0.5,       0.5,     "
          + " 1.000 );\n"
  ...
  public BackDropperFilter(String name) {
    ...
    if (adjStr.length() > 0) {
      try {
        ...
        if (isLoggable) {
          Log.m100v(TAG, "Adjusting accept threshold by " + adjStr + ", now " + this.mAcceptStddev);
        }
      } catch (NumberFormatException e) {
        Log.m96e(TAG, "Badly formatted property ro.media.effect.bgdropper.adj: " + adjStr);
      }
    }
  }
  ...
  private boolean createMemoryFormat(FrameFormat inputFormat) {
    ...
    if (this.mLogVerbose) {
      Log.m100v(TAG, "Mask frames size " + maskWidth + " x " + maskHeight);
      Log.m100v(TAG, "Pyramid levels " + widthExp + " x " + heightExp);
      Log.m100v(TAG, "Memory frames size " + memWidth + " x " + memHeight);
    }
  ...
  public void prepare(FilterContext context) {
    if (this.mLogVerbose) {
      Log.m100v(TAG, "Preparing BackDropperFilter!");
    }
  ...
  private void allocateFrames(FrameFormat inputFormat, FilterContext context) {
    ...
    if (this.mLogVerbose) {
      Log.m100v(TAG, "Allocating BackDropperFilter frames");
    }
    ...
    if (this.mLogVerbose) {
      Log.m100v(TAG, "Done allocating texture for Mean and Variance objects!");
    }
    ...
    this.mBgMaskProgram.setHostValue(
        "exp_lrg", Float.valueOf(this.mSubsampleLevel + this.mHierarchyLrgExp));
    this.mBgMaskProgram.setHostValue(
        "exp_mid", Float.valueOf(this.mSubsampleLevel + this.mHierarchyMidExp));
    this.mBgMaskProgram.setHostValue(
        "exp_sml", Float.valueOf(this.mSubsampleLevel + this.mHierarchySmlExp));
    if (this.mUseTheForce) {
      this.mBgSubtractProgram =
          new ShaderProgram(
              context, mSharedUtilShader + mBgSubtractShader + mBgSubtractForceShader);
    } else {
      this.mBgSubtractProgram =
          new ShaderProgram(context, mSharedUtilShader + mBgSubtractShader + "}\n");
    }
    ...
    this.mBgSubtractProgram.setHostValue(
        "whitebalanceblue_change", Float.valueOf(this.mWhiteBalanceBlueChange));
    this.mBgSubtractProgram.setHostValue(
        "whitebalancered_change", Float.valueOf(this.mWhiteBalanceRedChange));
    ShaderProgram shaderProgram3 =
        new ShaderProgram(context, mSharedUtilShader + mUpdateBgModelMeanShader);
    ...
    ShaderProgram shaderProgram4 =
        new ShaderProgram(context, mSharedUtilShader + mUpdateBgModelVarianceShader);
    ...
    ShaderProgram shaderProgram5 =
        new ShaderProgram(context, mSharedUtilShader + mAutomaticWhiteBalance);
    ...
    this.mAutomaticWhiteBalanceProgram.setHostValue(
        "autowb_toggle", Integer.valueOf(this.mAutoWBToggle));
    ShaderProgram shaderProgram6 =
        new ShaderProgram(context, mSharedUtilShader + mMaskVerifyShader);
    this.mMaskVerifyProgram = shaderProgram6;
    shaderProgram6.setHostValue("verify_rate", Float.valueOf(this.mVerifyRate));
    if (this.mLogVerbose) {
      Log.m100v(TAG, "Shader width set to " + this.mMemoryFormat.getWidth());
    }
  ...
  public void process(FilterContext filterContext) {
    ...
    if (this.mStartLearning) {
      if (this.mLogVerbose) {
        Log.m100v(TAG, "Starting learning");
      }
      this.mBgUpdateMeanProgram.setHostValue(
          "bg_adapt_rate", Float.valueOf(this.mAdaptRateLearning));
      this.mBgUpdateMeanProgram.setHostValue(
          "fg_adapt_rate", Float.valueOf(this.mAdaptRateLearning));
      this.mBgUpdateVarianceProgram.setHostValue(
          "bg_adapt_rate", Float.valueOf(this.mAdaptRateLearning));
      this.mBgUpdateVarianceProgram.setHostValue(
          "fg_adapt_rate", Float.valueOf(this.mAdaptRateLearning));
      this.mFrameCount = 0;
    ...
    this.mBgDistProgram.process(
        new Frame[] {this.mVideoInput, this.mBgMean[i], this.mBgVariance[i]}, this.mDistance);
    ...
    this.mAutomaticWhiteBalanceProgram.process(
        new Frame[] {this.mVideoInput, this.mBgInput}, this.mAutoWB);
    if (this.mFrameCount <= this.mLearningDuration) {
      ...
      if (i2 == i3 - i4) {
        ...
        this.mBgUpdateVarianceProgram.setHostValue(
            "bg_adapt_rate", Float.valueOf(this.mAdaptRateBg));
        this.mBgUpdateVarianceProgram.setHostValue(
            "fg_adapt_rate", Float.valueOf(this.mAdaptRateFg));
      } else if (i2 > i3 - i4) {
        GLFrame[] gLFrameArr = this.mMaskVerify;
        this.mMaskVerifyProgram.process(
            new Frame[] {gLFrameArr[i], this.mMask}, gLFrameArr[z ? 1 : 0]);
      ...
      if (this.mFrameCount == this.mLearningDuration) {
        ...
        if (this.mLogVerbose) {
          Log.m100v(
              TAG, String.format("Mask_average is %d, threshold is %d", Integer.valueOf(i5), 20));
        }
        if (i5 >= 20) {
          this.mStartLearning = true;
        } else {
          if (this.mLogVerbose) {
            Log.m100v(TAG, "Learning done");
          }
      ...
      this.mBgSubtractProgram.process(
          new Frame[] {pullInput, pullInput2, this.mMask, this.mAutoWB}, newFrame);
      pushOutput("video", newFrame);
      newFrame.release();
    }
    if (this.mFrameCount < this.mLearningDuration - this.mLearningVerifyDuration
        || this.mAdaptRateBg > SContextConstants.ENVIRONMENT_VALUE_UNKNOWN
        || this.mAdaptRateFg > SContextConstants.ENVIRONMENT_VALUE_UNKNOWN) {
      GLFrame[] gLFrameArr2 = this.mBgMean;
      this.mBgUpdateMeanProgram.process(
          new Frame[] {this.mVideoInput, gLFrameArr2[i], this.mMask}, gLFrameArr2[z ? 1 : 0]);
      this.mBgMean[z ? 1 : 0].generateMipMap();
      this.mBgMean[z ? 1 : 0].setTextureParameter(10241, 9985);
      GLFrame[] gLFrameArr3 = this.mBgVariance;
      this.mBgUpdateVarianceProgram.process(
          new Frame[] {this.mVideoInput, this.mBgMean[i], gLFrameArr3[i], this.mMask},
          gLFrameArr3[z ? 1 : 0]);
    ...
    if (this.mLogVerbose && i6 % 30 == 0) {
      if (this.startTime == -1) {
        ...
        Log.m100v(
            TAG,
            "Avg. frame duration: "
                + String.format("%.2f", Double.valueOf((elapsedRealtime - this.startTime) / 30.0d))
                + " ms. Avg. fps: "
                + String.format(
                    "%.2f",
                    Double.valueOf(1000.0d / ((elapsedRealtime - this.startTime) / 30.0d))));
  ...
  private void updateBgScaling(Frame video, Frame background, boolean fitModeChanged) {
    ...
    if (currentRelativeAspect != this.mRelativeAspect || fitModeChanged) {
      ...
      if (this.mMirrorBg) {
        if (this.mLogVerbose) {
          Log.m100v(TAG, "Mirroring the background!");
        }
      ...
      if (this.mLogVerbose) {
        Log.m100v(
            TAG,
            "bgTransform: xMin, yMin, xWidth, yWidth : "
                + xMin
                + ", "
                + yMin
                + ", "
