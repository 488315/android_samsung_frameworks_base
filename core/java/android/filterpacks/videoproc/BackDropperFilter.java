...
public class BackDropperFilter extends Filter {
    ...
    public void process(FilterContext filterContext) {
        ...
        if (this.mFrameCount <= this.mLearningDuration) {
            ...
            if (this.mFrameCount == this.mLearningDuration) {
                ...
                if (this.mLogVerbose) {
                    Log.m100v(TAG, String.format("Mask_average is %d, threshold is %d", Integer.valueOf(i5), 20));
                }
                if (i5 >= 20) {
                    this.mStartLearning = true;
                } else {
        ...
        if (this.mLogVerbose && i6 % 30 == 0) {
            if (this.startTime == -1) {
                ...
                Log.m100v(TAG, "Avg. frame duration: " + String.format("%.2f", Double.valueOf((elapsedRealtime - this.startTime) / 30.0d)) + " ms. Avg. fps: " + String.format("%.2f", Double.valueOf(1000.0d / ((elapsedRealtime - this.startTime) / 30.0d))));
    ...
    private void updateBgScaling(Frame video, Frame background, boolean fitModeChanged) {
        ...
        if (currentRelativeAspect != this.mRelativeAspect || fitModeChanged) {
            ...
            if (this.mLogVerbose) {
                Log.m100v(TAG, "bgTransform: xMin, yMin, xWidth, yWidth : " + xMin + ", " + yMin + ", " + xWidth + ", " + yWidth + ", mRelAspRatio = " + this.mRelativeAspect);
            }
