...
public class SurfaceTextureSource extends Filter {
  ...
  protected void prepare(FilterContext context) {
    if (mLogVerbose) {
      Log.m100v(TAG, "Preparing SurfaceTextureSource");
    }
    createFormats();
    this.mMediaFrame =
  ...
  public void open(FilterContext context) {
    if (mLogVerbose) {
      Log.m100v(TAG, "Opening SurfaceTextureSource");
    }
    ...
    if (this.mWaitForNewFrame || this.mFirstFrame) {
      ...
      if (i != 0) {
        ...
        if (!gotNewFrame) {
          ...
          if (z) {
            Log.m100v(TAG, "Timeout waiting for a new frame. Closing.");
          }
  ...
  public void close(FilterContext context) {
    if (mLogVerbose) {
      Log.m100v(TAG, "SurfaceTextureSource closed");
    }
