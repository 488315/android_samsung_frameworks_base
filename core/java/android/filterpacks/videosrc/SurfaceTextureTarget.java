...
public class SurfaceTextureTarget extends Filter {
    ...
    public void updateRenderMode() {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "updateRenderMode. Thread: " + Thread.currentThread());
        }
    ...
    public void prepare(FilterContext context) {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Prepare. Thread: " + Thread.currentThread());
        }
    ...
    public synchronized void open(FilterContext context) {
        if (this.mSurfaceTexture == null) {
            Log.m96e(TAG, "SurfaceTexture is null!!");
    ...
    public synchronized void disconnect(FilterContext context) {
        if (this.mLogVerbose) {
            Log.m100v(TAG, MediaMetrics.Value.DISCONNECT);
        }
        if (this.mSurfaceTexture == null) {
            Log.m94d(TAG, "SurfaceTexture is already null. Nothing to disconnect.");
            return;
        }
    ...
    public synchronized void process(FilterContext context) {
        ...
        if (currentAspectRatio != this.mAspectRatio) {
            if (this.mLogVerbose) {
                Log.m100v(TAG, "Process. New aspect ratio: " + currentAspectRatio + ", previously: " + this.mAspectRatio + ". Thread: " + Thread.currentThread());
            }
    ...
    public void fieldPortValueUpdated(String name, FilterContext context) {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "FPVU. Thread: " + Thread.currentThread());
        }
        updateRenderMode();
    }
    ...
    private void updateTargetRect() {
        int i;
        if (this.mLogVerbose) {
            Log.m100v(TAG, "updateTargetRect. Thread: " + Thread.currentThread());
        }
        ...
        if (i2 > 0 && (i = this.mScreenHeight) > 0 && this.mProgram != null) {
            ...
            if (this.mLogVerbose) {
                Log.m100v(TAG, "UTR. screen w = " + this.mScreenWidth + " x screen h = " + this.mScreenHeight + " Screen AR: " + screenAspectRatio + ", frame AR: " + this.mAspectRatio + ", relative AR: " + relativeAspectRatio);
            }
            ...
            switch (this.mRenderMode) {
                case 0:
                    this.mTargetQuad.f61p0.set(0.0f, 0.0f);
                    this.mTargetQuad.f62p1.set(1.0f, 0.0f);
                    this.mTargetQuad.f63p2.set(0.0f, 1.0f);
                    this.mTargetQuad.f64p3.set(1.0f, 1.0f);
                    this.mProgram.setClearsOutput(false);
                    break;
                case 1:
                    if (relativeAspectRatio > 1.0f) {
                        this.mTargetQuad.f61p0.set(0.5f - (0.5f / relativeAspectRatio), 0.0f);
                        this.mTargetQuad.f62p1.set((0.5f / relativeAspectRatio) + 0.5f, 0.0f);
                        this.mTargetQuad.f63p2.set(0.5f - (0.5f / relativeAspectRatio), 1.0f);
                        this.mTargetQuad.f64p3.set((0.5f / relativeAspectRatio) + 0.5f, 1.0f);
                    } else {
                        this.mTargetQuad.f61p0.set(0.0f, 0.5f - (relativeAspectRatio * 0.5f));
                        this.mTargetQuad.f62p1.set(1.0f, 0.5f - (relativeAspectRatio * 0.5f));
                        this.mTargetQuad.f63p2.set(0.0f, (relativeAspectRatio * 0.5f) + 0.5f);
                        this.mTargetQuad.f64p3.set(1.0f, (relativeAspectRatio * 0.5f) + 0.5f);
                    }
            ...
            if (this.mLogVerbose) {
                Log.m100v(TAG, "UTR. quad: " + this.mTargetQuad);
            }
