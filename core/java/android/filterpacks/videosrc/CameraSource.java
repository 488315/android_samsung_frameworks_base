...
public class CameraSource extends Filter {
    ...
    private int[] findClosestSize(int width, int height, Camera.Parameters parameters) {
        ...
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Requested resolution: (" + width + ", " + height + "). Closest match: (" + closestWidth + ", " + closestHeight + ").");
        }
    ...
    private int[] findClosestFpsRange(int fps, Camera.Parameters params) {
        ...
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Requested fps: " + fps + ".Closest frame rate range: [" + (closestRange[0] / 1000.0d) + "," + (closestRange[1] / 1000.0d) + NavigationBarInflaterView.SIZE_MOD_END);
        }
        return closestRange;
    }
}
