  ...
  public NfcFServiceInfo(PackageManager pm, ResolveInfo info)
    ...
    try {
      try {
        ...
        TypedArray sa = res.obtainAttributes(attrs, C4337R.styleable.HostNfcFService);
        this.mService = info;
        ...
        while (true) {
          ...
          if (eventType2 == 2 && "system-code-filter".equals(tagName) && systemCode == null) {
            TypedArray a = res.obtainAttributes(attrs, C4337R.styleable.SystemCodeFilter);
            String systemCode2 = a.getString(0).toUpperCase();
            if (!NfcFCardEmulation.isValidSystemCode(systemCode2)
                && !systemCode2.equalsIgnoreCase("NULL")) {
              Log.m96e(TAG, "Invalid System Code: " + systemCode2);
              systemCode = null;
            } else {
            ...
            TypedArray a2 = res.obtainAttributes(attrs, C4337R.styleable.Nfcid2Filter);
            String nfcid22 = a2.getString(0).toUpperCase();
            if (!nfcid22.equalsIgnoreCase("RANDOM")
                && !nfcid22.equalsIgnoreCase("NULL")
                && !NfcFCardEmulation.isValidNfcid2(nfcid22)) {
              Log.m96e(TAG, "Invalid NFCID2: " + nfcid22);
              nfcid2 = null;
            } else {
              nfcid2 = nfcid22;
            }
            ...
            TypedArray a3 = res.obtainAttributes(attrs, C4337R.styleable.T3tPmmFilter);
