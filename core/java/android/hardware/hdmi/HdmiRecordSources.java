...
public final class HdmiRecordSources {
  ...
  public static DigitalServiceSource ofDigitalChannelId(
    ...
    switch (broadcastSystem) {
      ...
      default:
        Log.m102w(TAG, "Invalid broadcast type:" + broadcastSystem);
  ...
  public static DigitalServiceSource ofArib(int aribType, AribData data) {
    ...
    switch (aribType) {
      ...
      default:
        Log.m102w(TAG, "Invalid ARIB type:" + aribType);
  ...
  public static DigitalServiceSource ofAtsc(int atscType, AtscData data) {
    ...
    switch (atscType) {
      ...
      default:
        Log.m102w(TAG, "Invalid ATSC type:" + atscType);
  ...
  public static DigitalServiceSource ofDvb(int dvbType, DvbData data) {
    ...
    switch (dvbType) {
      ...
      default:
        Log.m102w(TAG, "Invalid DVB type:" + dvbType);
  ...
  public static AnalogueServiceSource ofAnalogue(
    ...
    if (broadcastType < 0 || broadcastType > 2) {
      Log.m102w(TAG, "Invalid Broadcast type:" + broadcastType);
      throw new IllegalArgumentException("Invalid Broadcast type:" + broadcastType);
    }
    if (frequency < 0 || frequency > 65535) {
      Log.m102w(TAG, "Invalid frequency value[0x0000-0xFFFF]:" + frequency);
    ...
    if (broadcastSystem < 0 || broadcastSystem > 31) {
      Log.m102w(TAG, "Invalid Broadcast system:" + broadcastSystem);
  ...
  public static ExternalPlugData ofExternalPlug(int plugNumber) {
    if (plugNumber < 1 || plugNumber > 255) {
      Log.m102w(TAG, "Invalid plug number[1-255]" + plugNumber);
  ...
  public static ExternalPhysicalAddress ofExternalPhysicalAddress(int physicalAddress) {
    if (((-65536) & physicalAddress) != 0) {
      Log.m102w(TAG, "Invalid physical address:" + physicalAddress);
