...
public class SystemSensorManager extends SensorManager {
    ...
    protected boolean registerListenerImpl(SensorEventListener listener, Sensor sensor, int delayUs, Handler handler, int maxBatchReportLatencyUs, int reservedFlags) {
        ...
        if (sensor.getName().contains("Camera Light Sensor")) {
            ...
            if (handler != null) {
                Log.m94d("SensorManager", "[CameraLightSensor] Use handler looper= " + handler.getLooper() + " mainLooper= " + this.mMainLooper);
            } else {
                Log.m94d("SensorManager", "[CameraLightSensor] mainLooper= " + this.mMainLooper + ", Use CameraMangerThread looper.");
            }
        ...
        synchronized (this.mSensorListeners) {
            ...
            if (queue == null) {
                ...
                if (queue2.addSensor(sensor, delayUs2, maxBatchReportLatencyUs)) {
                    ...
                    queue2.dispose();
                    Log.m94d("SensorManager", "registerListener fail (1) :: " + sensor.getHandle() + ", " + sensor.getName() + ", " + delayUs2 + ", " + maxBatchReportLatencyUs + ", " + strlistener);
                    return false;
                }
            } else if (!queue.addSensor(sensor, delayUs2, maxBatchReportLatencyUs)) {
                Log.m94d("SensorManager", "registerListener fail (2) :: " + sensor.getHandle() + ", " + sensor.getName() + ", " + delayUs2 + ", " + maxBatchReportLatencyUs + ", " + strlistener);
                return false;
            }
            Log.m94d("SensorManager", "registerListener :: " + sensor.getHandle() + ", " + sensor.getName() + ", " + delayUs2 + ", " + maxBatchReportLatencyUs + ", " + strlistener);
            if (sensor.getName().contains("Camera Light Sensor")) {
                ...
                if (result && !queue.hasSensors()) {
                    ...
                    if (uid < 10000) {
                        if (sensor != null && listener != null) {
                            Log.m94d("SensorManager", "unregisterListener :: " + sensor.getHandle() + ", " + sensor.getName() + ", " + listener.toString());
                        } else {
        ...
        synchronized (this.mTriggerListeners) {
            ...
            if (queue == null) {
                ...
                if (!queue2.addSensor(sensor, 0, 0)) {
                    queue2.dispose();
                    Log.m94d("SensorManager", "requestTrigger :: fail (1) :: " + sensor.getHandle() + ", " + sensor.getName() + ", " + listener.toString());
                    return false;
                }
                this.mTriggerListeners.put(listener, queue2);
            } else if (!queue.addSensor(sensor, 0, 0)) {
                Log.m94d("SensorManager", "requestTrigger :: fail (2) :: " + sensor.getHandle() + ", " + sensor.getName() + ", " + listener.toString());
                return false;
            }
            Log.m94d("SensorManager", "requestTrigger :: " + sensor.getHandle() + ", " + sensor.getName() + ", " + listener.toString());
            return true;
        }
    }
    ...
    protected boolean cancelTriggerSensorImpl(TriggerEventListener listener, Sensor sensor, boolean disable) {
        ...
        synchronized (this.mTriggerListeners) {
            ...
            if (result && !queue.hasSensors()) {
                ...
                queue.dispose();
                if (sensor != null) {
                    Log.m94d("SensorManager", "cancelTrigger :: " + sensor.getHandle() + ", " + sensor.getName() + ", " + listener.toString());
                } else {
    ...
    protected boolean injectSensorDataImpl(Sensor sensor, float[] values, int accuracy, long timestamp) {
        synchronized (sLock) {
            InjectEventQueue injectEventQueue = sInjectEventQueue;
            if (injectEventQueue == null) {
                Log.m96e("SensorManager", "Data injection mode not activated before calling injectSensorData");
                return false;
            }
    ...
    public void cleanupSensorConnection(Sensor sensor) {
        ...
        if (sensor.getReportingMode() == 2) {
            synchronized (this.mTriggerListeners) {
                ...
                for (TriggerEventListener l : triggerListeners.keySet()) {
                    Log.m98i("SensorManager", "removed trigger listener" + l.toString() + " due to sensor disconnection");
        ...
        synchronized (this.mSensorListeners) {
            ...
            for (SensorEventListener l2 : sensorListeners.keySet()) {
                Log.m98i("SensorManager", "removed event listener" + l2.toString() + " due to sensor disconnection");
