...
public class SystemSensorManager extends SensorManager {
  ...
  protected boolean registerListenerImpl(
    ...
    if (listener == null || sensor == null) {
      Log.m96e("SensorManager", "sensor or listener is null");
      return false;
    }
    if (sensor.getReportingMode() == 2) {
      Log.m96e("SensorManager", "Trigger Sensors should use the requestTriggerSensor.");
      return false;
    }
    if (maxBatchReportLatencyUs < 0 || delayUs < 0) {
      Log.m96e("SensorManager", "maxBatchReportLatencyUs and delayUs should be non-negative");
      return false;
    }
  ...
  protected void unregisterListenerImpl(SensorEventListener listener, Sensor sensor) {
    ...
    if (listener != null
        && (cameraLightSensorManager = this.mCameraLightManager) != null
        && cameraLightSensorManager.isAllowListedListener(listener.toString())) {
        ...
        if (result && !queue.hasSensors()) {
          ...
          if (uid < 10000) {
            if (sensor != null && listener != null) {
              ...
              Log.m94d("SensorManager", "unregisterListener :: " + listener.toString());
            }
          } else {
            Log.m94d("SensorManager", "unregisterListener");
          }
        }
      }
    }
  ...
  protected boolean requestTriggerSensorImpl(TriggerEventListener listener, Sensor sensor) {
    ...
    if (this.mTriggerListeners.size() >= 128) {
      throw new IllegalStateException(
          "request failed, the trigger listeners size has exceeded the maximum limit 128");
    }
    ...
    synchronized (this.mTriggerListeners) {
      ...
      if (result && !queue.hasSensors()) {
        ...
        if (sensor != null) {
          ...
          Log.m94d("SensorManager", "cancelTrigger :: " + listener.toString());
        }
      }
      return result;
    }
    ...
    synchronized (sLock) {
      ...
      if (enable) {
        ...
        if (!isDataInjectionModeEnabled) {
          Log.m96e("SensorManager", "Data Injection mode not enabled");
          return false;
        }
        if (sInjectEventQueue == null) {
          try {
            sInjectEventQueue =
                new InjectEventQueue(this.mMainLooper, this, this.mContext.getPackageName());
          } catch (RuntimeException e) {
            Log.m96e("SensorManager", "Cannot create InjectEventQueue: " + e);
          }
        }
        if (sInjectEventQueue == null) {
          z = false;
  ...
  protected boolean injectSensorDataImpl(
    ...
    synchronized (sLock) {
      ...
      if (injectEventQueue == null) {
        Log.m96e(
            "SensorManager", "Data injection mode not activated before calling injectSensorData");
        return false;
      }
  ...
  private boolean requestCameraLightSensor(
    ...
    if (enable) {
      ...
      if (cameraLightSensorManager != null
          && cameraLightSensorManager.registerCameraLightSensor(cameraLooper, listener, sensor)) {
        Log.m98i("SensorManager", "[CameraLight] registerListener : " + listener.toString());
        return true;
      }
      Log.m96e("SensorManager", "[CameraLight] registerListener fail : " + listener.toString());
      return false;
    }
    CameraLightSensorManager cameraLightSensorManager2 = this.mCameraLightManager;
    if (cameraLightSensorManager2 == null
        || !cameraLightSensorManager2.unRegisterCameraLightSensor(listener)) {
      return false;
    }
    Log.m98i("SensorManager", "[CameraLight] unregisteListener : " + listener.toString());
    return true;
  }
    ...
    if (sensor.getReportingMode() == 2) {
      synchronized (this.mTriggerListeners) {
        ...
        for (TriggerEventListener l : triggerListeners.keySet()) {
          Log.m98i(
              "SensorManager",
              "removed trigger listener" + l.toString() + " due to sensor disconnection");
      ...
      if (this.mDynamicSensorListDirty) {
        ...
        boolean changed =
            diffSortedSensorList(
                this.mFullDynamicSensorsList, list, updatedList, addedList, removedList);
        if (changed) {
          Log.m98i("SensorManager", "DYNS dynamic sensor list cached should be updated");
          ...
          for (Map.Entry<SensorManager.DynamicSensorCallback, Handler> entry :
            ...
            handler.post(
                new Runnable() { // from class: android.hardware.SystemSensorManager.1
            ...
            public void onReceive(Context context, Intent intent) {
              if (intent.getAction().equals(VirtualDeviceManager.ACTION_VIRTUAL_DEVICE_REMOVED)) {
                synchronized (SystemSensorManager.this.mFullRuntimeSensorListByDevice) {
                  int deviceId =
                      intent.getIntExtra(VirtualDeviceManager.EXTRA_VIRTUAL_DEVICE_ID, 0);
                  List<Sensor> removedSensors =
                      (List)
                          SystemSensorManager.this.mFullRuntimeSensorListByDevice.removeReturnOld(
                              deviceId);
  ...
  private void setupDynamicSensorBroadcastReceiver() {
    if (this.mDynamicSensorBroadcastReceiver == null) {
      this.mDynamicSensorBroadcastReceiver =
          new BroadcastReceiver() { // from class: android.hardware.SystemSensorManager.3
            @Override // android.content.BroadcastReceiver
            public void onReceive(Context context, Intent intent) {
              if (intent.getAction().equals(Intent.ACTION_DYNAMIC_SENSOR_CHANGED)) {
                Log.m98i("SensorManager", "DYNS received DYNAMIC_SENSOR_CHANED broadcast");
  ...
  protected void registerDynamicSensorCallbackImpl(
    ...
    Log.m98i("SensorManager", "DYNS Register dynamic sensor callback");
    if (callback == null) {
  ...
  protected void unregisterDynamicSensorCallbackImpl(SensorManager.DynamicSensorCallback callback) {
    Log.m98i("SensorManager", "Removing dynamic sensor listerner");
  ...
  private static boolean diffSortedSensorList(
    ...
    while (true) {
      if (j < oldList.size()
          && (i >= newList.size() || newList.get(i).getHandle() > oldList.get(j).getHandle())) {
        changed = true;
        if (removed != null) {
        ...
        j++;
          ...
          && (j >= oldList.size() || newList.get(i).getHandle() < oldList.get(j).getHandle())) {
        changed = true;
        if (added != null) {
  ...
  private abstract static class BaseEventQueue {
    ...
    private int enableSensor(Sensor sensor, int rateUs, int maxBatchReportLatencyUs) {
      ...
      return nativeEnableSensor(
          this.mNativeSensorEventQueue, sensor.getHandle(), rateUs, maxBatchReportLatencyUs);
    }
  ...
  static final class SensorEventQueue extends BaseEventQueue {
    ...
    public void addSensorEvent(Sensor sensor) {
      SensorEvent t =
          new SensorEvent(Sensor.getMaxLengthValuesArray(sensor, this.mManager.mTargetSdkLevel));
    ...
    protected void dispatchFlushCompleteEvent(int handle) {
      ...
      if (!(this.mListener instanceof SensorEventListener2)
          || (sensor = (Sensor) this.mManager.mHandleToSensor.get(Integer.valueOf(handle)))
              == null) {
        return;
      }
  ...
  static final class TriggerEventQueue extends BaseEventQueue {
    ...
    public void addSensorEvent(Sensor sensor) {
      TriggerEvent t =
          new TriggerEvent(Sensor.getMaxLengthValuesArray(sensor, this.mManager.mTargetSdkLevel));
    ...
    protected void dispatchSensorEvent(int handle, float[] values, int accuracy, long timestamp) {
      ...
      if (t == null) {
        Log.m96e("SensorManager", "Error: Trigger Event is null for Sensor: " + sensor);
        return;
      }
      System.arraycopy(values, 0, t.values, 0, t.values.length);
      t.timestamp = timestamp;
