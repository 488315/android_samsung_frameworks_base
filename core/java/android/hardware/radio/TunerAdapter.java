...
final class TunerAdapter extends RadioTuner {
    ...
    public void close() {
        synchronized (this.mLock) {
            if (this.mIsClosed) {
                Log.m100v(TAG, "Tuner is already closed");
                return;
            }
            this.mIsClosed = true;
            ...
            try {
                ...
                Log.m97e(TAG, "Exception trying to close tuner", e);
            }
        }
    }
    ...
    public int setConfiguration(RadioManager.BandConfig config) {
        ...
        try {
            ...
            Log.m97e(TAG, "Service died", e);
            return -32;
        } catch (IllegalArgumentException e2) {
            Log.m97e(TAG, "Can't set configuration", e2);
            return -22;
        }
    }
    ...
    public int setMute(boolean mute) {
        try {
            ...
            Log.m97e(TAG, "Can't set muted", e2);
    ...
    public int tune(int channel, int subChannel) {
        ...
        try {
            ...
            Log.m97e(TAG, "Can't tune", e2);
            return -22;
        } catch (IllegalStateException e3) {
            Log.m97e(TAG, "Can't tune", e3);
            return -38;
        }
    }
    ...
    public int getProgramInformation(RadioManager.ProgramInfo[] info) {
        if (info == null || info.length != 1) {
            Log.m96e(TAG, "The argument must be an array of length 1");
            return -22;
        }
        RadioManager.ProgramInfo current = this.mCallback.getCurrentProgramInformation();
        if (current == null) {
            Log.m102w(TAG, "Didn't get program info yet");
            return -38;
        }
        info[0] = current;
        return 0;
    ...
    public List<RadioManager.ProgramInfo> getProgramList(Map<String, String> vendorFilter) {
        synchronized (this.mLock) {
            if (this.mLegacyListProxy == null || !Objects.equals(this.mLegacyListFilter, vendorFilter)) {
                Log.m98i(TAG, "Program list filter has changed, requesting new list");
                ...
                this.mCallback.setProgramListObserver(this.mLegacyListProxy, new ProgramList.OnCloseListener() { // from class: android.hardware.radio.TunerAdapter$$ExternalSyntheticLambda0
                    ...
                    public final void onClose() {
                        Log.m98i(TunerAdapter.TAG, "Empty closeListener in programListObserver");
                    }
                });
            }
        }
    ...
    public ProgramList getDynamicProgramList(ProgramList.Filter filter) {
        ...
        try {
            ...
            Log.m98i(TAG, "Program list is not supported with this hardware");
            return null;
        }
    }
    ...
    public /* synthetic */ void lambda$getDynamicProgramList$1() {
        try {
            ...
            Log.m97e(TAG, "Couldn't stop program list updates", ex);
        } catch (IllegalStateException ex2) {
            Log.m97e(TAG, "Tuner may already be closed", ex2);
        }
    }
