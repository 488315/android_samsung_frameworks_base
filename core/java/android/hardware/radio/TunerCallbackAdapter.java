...
final class TunerCallbackAdapter extends ITunerCallback.Stub {
    ...
    void setProgramListObserver(final ProgramList programList, final ProgramList.OnCloseListener closeListener) {
        ...
        synchronized (this.mLock) {
            if (this.mProgramList != null) {
                Log.m102w(TAG, "Previous program list observer wasn't properly closed, closing it...");
    ...
    public /* synthetic */ void lambda$setProgramListObserver$1(ProgramList programList) {
        synchronized (this.mLock) {
            ...
            if (this.mDelayedCompleteCallback) {
                Log.m94d(TAG, "Sending delayed onBackgroundScanComplete callback");
    ...
    public void onTuneFailed(final int status, final ProgramSelector selector) {
        ...
        switch (status) {
            ...
            case 4:
            case 7:
                Log.m98i(TAG, "Got an error with no mapping to the legacy API (" + status + "), doing a best-effort conversion to ERROR_SCAN_TIMEOUT");
                errorCode = 3;
                break;
            case -32:
            case -1:
    ...
    public void onCurrentProgramInfoChanged(final RadioManager.ProgramInfo info) {
        if (info == null) {
            Log.m96e(TAG, "ProgramInfo must not be null");
            return;
        }
        synchronized (this.mLock) {
    ...
    public void onBackgroundScanComplete() {
        synchronized (this.mLock) {
            if (this.mLastCompleteList == null) {
                Log.m98i(TAG, "Got onBackgroundScanComplete callback, but the program list didn't get through yet. Delaying it...");
