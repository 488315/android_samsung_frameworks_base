...
public final class HardwareConfig {
    ...
    public final boolean equals(Object otherObject) {
        ...
        if (this.type == other.type
                && HidlSupport.deepEquals(this.uuid, other.uuid)
                && this.state == other.state
                && HidlSupport.deepEquals(this.modem, other.modem)
                && HidlSupport.deepEquals(this.sim, other.sim)) {
            return true;
        }
        return false;
    }
    ...
    public final int hashCode() {
        return Objects.hash(
                ...
                Integer.valueOf(HidlSupport.deepHashCode(this.modem)),
                Integer.valueOf(HidlSupport.deepHashCode(this.sim)));
    }

    public final String toString() {
        return "{.type = "
                + HardwareConfigType.toString(this.type)
                + ", .uuid = "
                + this.uuid
                + ", .state = "
                + HardwareConfigState.toString(this.state)
                + ", .modem = "
                + this.modem
                + ", .sim = "
                + this.sim
    ...
    public final void readEmbeddedFromParcel(
        ...
        parcel.readEmbeddedBuffer(
                this.uuid.getBytes().length + 1, _hidl_blob.handle(), _hidl_offset + 8 + 0, false);
        this.state = _hidl_blob.getInt32(_hidl_offset + 24);
        int _hidl_vec_size = _hidl_blob.getInt32(_hidl_offset + 32 + 8);
        HwBlob childBlob =
                parcel.readEmbeddedBuffer(
                        _hidl_vec_size * 20, _hidl_blob.handle(), _hidl_offset + 32 + 0, true);
        this.modem.clear();
        for (int _hidl_index_0 = 0; _hidl_index_0 < _hidl_vec_size; _hidl_index_0++) {
        ...
        HwBlob childBlob2 =
                parcel.readEmbeddedBuffer(
                        _hidl_vec_size2 * 16, _hidl_blob.handle(), _hidl_offset + 48 + 0, true);
        this.sim.clear();
    ...
    public static final void writeVectorToParcel(
            HwParcel parcel, ArrayList<HardwareConfig> _hidl_vec) {
