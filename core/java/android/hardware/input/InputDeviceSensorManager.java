...
public class InputDeviceSensorManager implements InputManager.InputDeviceListener {
  ...
  public void onInputDeviceAdded(int deviceId) {
    synchronized (this.mInputSensorLock) {
      if (!this.mSensors.containsKey(Integer.valueOf(deviceId))) {
        ...
        Slog.m115e(
            TAG,
            "Received 'device added' notification for device "
                + deviceId
  ...
  public void onInputSensorChanged(
      int deviceId, int sensorType, int accuracy, long timestamp, float[] values) {
    synchronized (this.mInputSensorLock) {
  ...
  private static final class InputSensorEventListenerDelegate extends Handler {
    ...
    InputSensorEventListenerDelegate(
    ...
    public void addSensor(Sensor sensor) {
      for (Sensor s : this.mSensors) {
        if (InputDeviceSensorManager.sensorEquals(s, sensor)) {
          Slog.m121w(InputDeviceSensorManager.TAG, "Adding sensor " + sensor + " already exist!");
          return;
        }
      }
  ...
  /* JADX INFO: Access modifiers changed from: private */
  public boolean registerListenerInternal(
      SensorEventListener listener,
      Sensor sensor,
      int delayUs,
      int maxBatchReportLatencyUs,
      Handler handler) {
    if (listener == null) {
      Slog.m115e(TAG, "listener is null");
      return false;
    }
    if (sensor == null) {
      Slog.m115e(TAG, "sensor is null");
      return false;
    }
    if (sensor.getReportingMode() == 2) {
      Slog.m115e(TAG, "Trigger Sensors should use the requestTriggerSensor.");
      return false;
    }
    if (maxBatchReportLatencyUs < 0 || delayUs < 0) {
      Slog.m115e(TAG, "maxBatchReportLatencyUs and delayUs should be non-negative");
      return false;
    }
    if (getSensorForInputDevice(sensor.getId(), sensor.getType()) != null) {
      synchronized (this.mInputSensorLock) {
        ...
        if (!inputDevice.hasSensor()) {
          Slog.m115e(TAG, "The device doesn't have the sensor:" + sensor);
          return false;
        }
        if (!this.mGlobal.enableSensor(
            deviceId, sensor.getType(), delayUs, maxBatchReportLatencyUs)) {
          Slog.m115e(TAG, "Can't enable the sensor:" + sensor);
          return false;
        }
      }
    }
    synchronized (this.mInputSensorLock) {
      if (this.mInputServiceSensorListener == null) {
        ...
        if (!this.mGlobal.registerSensorListener(inputSensorEventListener)) {
          Slog.m115e(TAG, "Failed registering the sensor listener");
          return false;
        }
      }
      ...
      if (idx < 0) {
        InputSensorEventListenerDelegate d =
            new InputSensorEventListenerDelegate(
                listener,
                sensor,
                delayUs,
                maxBatchReportLatencyUs,
                handler == null ? this.mSensorHandler : handler);
  ...
  public void unregisterListenerInternal(SensorEventListener listener, Sensor sensor) {
    ...
    synchronized (this.mInputSensorLock) {
      ...
      if (idx >= 0) {
        ...
        if (this.mInputServiceSensorListener != null
            && this.mInputSensorEventListeners.size() == 0) {
        ...
        for (Sensor s : sensorsRegistered) {
          ...
          while (true) {
            ...
            if (!this.mInputSensorEventListeners.get(i).hasSensorRegistered(deviceId, sensorType)) {
              ...
              enableSensor = true;
              Slog.m121w(TAG, "device " + deviceId + " still uses sensor " + sensorType);
              break;
            }
          }
          if (!enableSensor) {
      ...
      Slog.m115e(TAG, "Listener is not registered");
    }
  }
  ...
  public class InputSensorManager extends SensorManager {
    ...
    protected boolean registerListenerImpl(
        ...
        int maxBatchReportLatencyUs,
        int reservedFlags) {
      return InputDeviceSensorManager.this.registerListenerInternal(
          listener, sensor, delayUs, maxBatchReportLatencyUs, handler);
    }
