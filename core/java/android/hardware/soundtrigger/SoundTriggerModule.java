...
public class SoundTriggerModule {
  ...
  public SoundTriggerModule(
    ...
    try {
      ...
      try {
        this.mService =
            service.attachAsOriginator(moduleId, originatorIdentity, this.mEventHandlerDelegate);
        if (ignored != null) {
          ignored.close();
        }
    ...
    try {
      if (model instanceof SoundTrigger.GenericSoundModel) {
        SoundModel aidlModel =
            ConversionUtil.api2aidlGenericSoundModel((SoundTrigger.GenericSoundModel) model);
        try {
          ...
          if (aidlModel.data != null) {
            try {
              ...
              Log.m97e(TAG, "Failed to close file", e);
            }
          }
        }
      }
      if (model instanceof SoundTrigger.KeyphraseSoundModel) {
        PhraseSoundModel aidlModel2 =
            ConversionUtil.api2aidlPhraseSoundModel((SoundTrigger.KeyphraseSoundModel) model);
        try {
          ...
          if (aidlModel2.common.data != null) {
            try {
              ...
              Log.m97e(TAG, "Failed to close file", e2);
            }
          }
        }
      }
            ...
            public void handleMessage(Message msg) {
              switch (msg.what) {
                ...
                case 4:
                  ...
                  break;
                default:
                  Log.m96e(SoundTriggerModule.TAG, "Unknown message: " + msg.toString());
                  break;
              }
            }
          };
    ...
    public synchronized void onRecognition(
      ...
      Message m =
          this.mHandler.obtainMessage(
              1, ConversionUtil.aidl2apiRecognitionEvent(handle, captureSession, event));
      this.mHandler.sendMessage(m);
    }

    @Override // android.media.soundtrigger_middleware.ISoundTriggerCallback
    public synchronized void onPhraseRecognition(
        int handle, PhraseRecognitionEventSys event, int captureSession) throws RemoteException {
      Message m =
          this.mHandler.obtainMessage(
              1, ConversionUtil.aidl2apiPhraseRecognitionEvent(handle, captureSession, event));
      this.mHandler.sendMessage(m);
    }
