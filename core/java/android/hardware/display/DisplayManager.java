...
public final class DisplayManager {
    ...
    public boolean supportsSeamlessRefreshRateSwitching() {
        return this.mContext.getResources().getBoolean(C4337R.bool.config_supportsSeamlessRefreshRateSwitching);
    }
    ...
    public int getHiddenDisplayId(String callerPackageName) {
        for (Display display : getDisplays()) {
            if (display.getType() == 5 && (display.getFlags() & 32768) != 0 && callerPackageName.equals(display.getOwnerPackageName())) {
                ...
                Slog.m117i(TAG_SPEG, "Display #" + id + " (" + SPEG_DISPLAY_NAME + "), owner: " + callerPackageName);
                return id;
            }
        }
        return -1;
    ...
    public int getHiddenDisplayId(int uid) {
        for (Display display : getDisplays()) {
            if (display.getType() == 5 && (display.getFlags() & 32768) != 0 && display.getOwnerUid() == uid) {
                int id = display.getDisplayId();
                Slog.m117i(TAG_SPEG, "Display #" + id + " (" + SPEG_DISPLAY_NAME + "), owner: " + display.getOwnerPackageName() + ":" + uid);
                return id;
            }
        }
        return -1;
    ...
    public void semStartScanWifiDisplays(int scanChannel) {
        Log.m94d(TAG, "semStartScanWifiDisplays, scanChannel = " + scanChannel + Log.getStackTraceString(new Throwable()));
        this.mGlobal.startWifiDisplayScan(scanChannel);
    }

    public void semStartScanWifiDisplays(int scanChannel, int interval) {
        Log.m94d(TAG, "semStartScanWifiDisplays, scanChannel = " + scanChannel + ", interval = " + interval + Log.getStackTraceString(new Throwable()));
    ...
    public boolean semRequestWifiDisplayParameter(String parameterGroup, SemWifiDisplayParameter parameter) {
        Log.m94d(TAG, "semRequestWifiDisplayParameter, parameterGroup : " + parameterGroup + ", parameter : " + parameter);
    ...
    */
    public void semEnableWifiDisplay(String ipAddr, String port, int deviceType, String options, String deviceName, String remoteP2pMacAddr, boolean isPendingRequest) {
        int mode;
        String str;
        String str2;
        ...
        if (deviceType == 14) {
            ...
            mode = (deviceType == 19 || deviceType == 16) ? 3 : deviceType == 17 ? 2 : deviceType == 10 ? 0 : 0;
        }
            ...
            if (!options.isEmpty()) {
                ...
                try {
                    ...
                    if (jsonObject.has(SemWifiDisplayParameter.KEY_SCAMBLE_SUPPORT)) {
                        initParameters.add(new SemWifiDisplayParameter(SemWifiDisplayParameter.KEY_SCAMBLE_SUPPORT, jsonObject.getString(SemWifiDisplayParameter.KEY_SCAMBLE_SUPPORT)));
                    }
                } catch (JSONException e6) {
                    str = str3;
                }
                if (connectionType != 3) {
                    SemWifiDisplayConfig wifidisplayConfig = new SemWifiDisplayConfig.Builder().setUsbConnection(ipAddr, port, deviceName, remoteP2pMacAddr).setMode(mode).addFlags(flags).addParameters(SemWifiDisplayParameter.INIT_PARAMETER, initParameters).addParameters(str2, getParameters).addParameters(str, setParameters).build();
                    Log.m94d(TAG, "semEnableWifiDisplay : deviceType = " + deviceType + ", config = " + wifidisplayConfig.toString() + ", option = " + options);
                    this.mGlobal.connectWifiDisplay(wifidisplayConfig, null, null);
                    return;
                } else {
                    SemWifiDisplayConfig wifidisplayConfig2 = new SemWifiDisplayConfig.Builder().setApConnection(ipAddr, port, deviceName, remoteP2pMacAddr).setMode(mode).addFlags(flags).addParameters(SemWifiDisplayParameter.INIT_PARAMETER, initParameters).addParameters(str2, getParameters).addParameters(str, setParameters).build();
                    Log.m94d(TAG, "semEnableWifiDisplay : deviceType = " + deviceType + ", config = " + wifidisplayConfig2.toString() + ", option = " + options);
