...
public class ActivityRecognitionHardware extends IActivityRecognitionHardware.Stub {
  ...
  private ActivityRecognitionHardware(Context context) {
    ...
    this.mSupportedActivitiesEnabledEvents =
        (int[][]) Array.newInstance((Class<?>) Integer.TYPE, length, 3);
  }
  ...
  private void onActivityChanged(Event[] events) {
    if (events == null || events.length == 0) {
      if (DEBUG) {
        Log.m94d(TAG, "No events to broadcast for onActivityChanged.");
        return;
      }
      return;
    }
    ...
    for (int i2 = 0; i2 < size; i2++) {
      ...
      try {
        ...
        Log.m97e(TAG, "Error delivering activity changed event.", e);
      }
    }
  ...
  private String getActivityName(int activityType) {
    ...
    String message =
        String.format(
            ...
            Integer.valueOf(activityType), Integer.valueOf(this.mSupportedActivities.length));
    Log.m96e(TAG, message);
    return null;
  }
  ...
  private class SinkList extends RemoteCallbackList<IActivityRecognitionHardwareSink> {
    ...
    public void onCallbackDied(IActivityRecognitionHardwareSink callback) {
      ...
      if (ActivityRecognitionHardware.DEBUG) {
        Log.m94d(ActivityRecognitionHardware.TAG, "RegisteredCallbackCount: " + callbackCount);
      }
      if (callbackCount != 0) {
        return;
      }
    ...
    private void disableActivityEventIfEnabled(int activityType, int eventType) {
      ...
      int result =
          ActivityRecognitionHardware.this.nativeDisableActivityEvent(activityType, eventType);
      ActivityRecognitionHardware.this.mSupportedActivitiesEnabledEvents[activityType][eventType] =
          0;
      String message =
          String.format(
              "DisableActivityEvent: activityType=%d, eventType=%d, result=%d",
              Integer.valueOf(activityType), Integer.valueOf(eventType), Integer.valueOf(result));
      Log.m96e(ActivityRecognitionHardware.TAG, message);
    }
  }
}
