...
public final class NanoAppBinary implements Parcelable {
    ...
    private void parseBinaryHeader() {
        ...
        try {
            ...
            if (i != 1) {
                Log.m96e(TAG, "Unexpected header version " + this.mHeaderVersion + " while parsing header (expected 1" + NavigationBarInflaterView.KEY_CODE_END);
                return;
            }
            ...
            if (this.mMagic != EXPECTED_MAGIC_VALUE) {
                Log.m96e(TAG, "Unexpected magic value " + String.format("0x%08X", Integer.valueOf(this.mMagic)) + "while parsing header (expected " + String.format("0x%08X", Integer.valueOf(EXPECTED_MAGIC_VALUE)) + NavigationBarInflaterView.KEY_CODE_END);
            } else {
                this.mHasValidHeader = true;
            }
        } catch (BufferUnderflowException e) {
            Log.m96e(TAG, "Not enough contents in nanoapp header");
        }
    }
