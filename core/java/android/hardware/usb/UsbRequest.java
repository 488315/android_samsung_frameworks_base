...
public class UsbRequest {
    ...
    */
    boolean queueIfConnectionOpen(ByteBuffer buffer) {
        boolean z;
        boolean wasQueued;
        UsbDeviceConnection connection = this.mConnection;
        if (connection == null || !connection.isOpen()) {
            ...
            if (this.mIsUsingNewQueue) {
                ...
                if (bytesTransferred >= 0) {
                    ...
                    try {
                        ...
                        Log.m97e(TAG, "Buffer " + this.mBuffer + " does not have enough space to read " + bytesToStore + " bytes", e);
    ...
    boolean cancelIfOpen() {
        ...
        if (this.mNativeContext == 0 || (connection != null && !connection.isOpen())) {
            Log.m102w(TAG, "Detected attempt to cancel a request on a connection which isn't open");
            return false;
        }
