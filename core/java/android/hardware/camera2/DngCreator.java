...
public final class DngCreator implements AutoCloseable {
  ...
  private synchronized native void nativeWriteInputStream(
      OutputStream outputStream, InputStream inputStream, int i, int i2, long j) throws IOException;
  ...
  public DngCreator(CameraCharacteristics characteristics, CaptureResult metadata) {
    ...
    if (timestampSource == 1) {
      ...
      Log.m102w(TAG, "Sensor timestamp source is unexpected: " + timestampSource);
      timeOffset = currentTime - SystemClock.uptimeMillis();
    }
