...
public final class CameraExtensionUtils {
  ...
  public static final class HandlerExecutor implements Executor {
    ...
    public void execute(Runnable runCmd) {
      try {
        ...
        Log.m102w(CameraExtensionUtils.TAG, "Handler thread unavailable, skipping message!");
      }
    }
  }
  ...
  public static Surface getBurstCaptureSurface(
      List<OutputConfiguration> outputConfigs, HashMap<Integer, List<Size>> supportedCaptureSizes) {
    ...
    for (OutputConfiguration config : outputConfigs) {
      ...
      if (surfaceInfo.mFormat == 34
        ...
        if (supportedPreviewSizes == null
          ...
          throw new IllegalArgumentException(
              "Repeating request surface size " + repeatingRequestSurfaceSize + " not supported!");
        }
        return config.getSurface();
      }
    }
