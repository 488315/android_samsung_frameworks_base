    ...
    public int capture(CaptureRequest request, Executor executor, CameraExtensionSession.ExtensionCaptureCallback listener) throws CameraAccessException {
        ...
        if (surface != null && request.containsTarget(surface)) {
            ...
            try {
                ...
                Log.m96e(TAG, "Failed to initialize internal burst request! Extension service does not respond!");
                throw new CameraAccessException(3);
            }
        }
        ...
        public void onConfigured(CameraCaptureSession session) {
            synchronized (CameraExtensionSessionImpl.this.mInterfaceLock) {
                ...
                try {
                    ...
                    Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to initialize session! Extension service does not respond!");
                    CameraExtensionSessionImpl.this.notifyConfigurationFailure();
                }
            }
        }
                ...
                public void run() {
                    ...
                    if (!initialRequestList.isEmpty()) {
                        try {
                            ...
                            Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to initialize the initial capture request!");
                            status = false;
                        }
                    } else {
                        try {
                            ...
                            Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to initialize internal repeating request!");
                            status = false;
                        }
                    }
                    if (!status) {
        ...
        public void onFailure() {
            CameraExtensionSessionImpl.this.mHandler.post(new Runnable() { // from class: android.hardware.camera2.impl.CameraExtensionSessionImpl.InitializeSessionHandler.2
                ...
                public void run() {
                    ...
                    Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to initialize proxy service session! This can happen when trying to configure multiple concurrent extension sessions!");
                    CameraExtensionSessionImpl.this.notifyConfigurationFailure();
                }
            });
        }
        ...
        public void checkAndFireBurstProcessing() {
            if (this.mCaptureRequestMap.size() == this.mCaptureStageMap.size()) {
                ...
                try {
                    ...
                    Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to process multi-frame request! Extension service does not respond!");
                    processStatus = false;
                }
            ...
            public void onImageAvailable(ImageReader reader, Image img) {
                ...
                if (BurstRequestHandler.this.mCapturePendingMap.indexOfKey(timestamp) >= 0) {
                    ...
                    if (captureStage != null) {
                        ...
                        Log.m96e(CameraExtensionSessionImpl.TAG, "Capture stage: " + ((Pair) BurstRequestHandler.this.mCapturePendingMap.get(timestamp)).second + " is absent!");
                        return;
                    }
                }
        ...
        public void onCaptureSequenceCompleted(CameraCaptureSession session, int sequenceId, long frameNumber) {
            ...
            synchronized (CameraExtensionSessionImpl.this.mInterfaceLock) {
                try {
                    ...
                    Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to start the internal repeating request!");
                    status = false;
                }
            }
            if (!status) {
                    ...
                    try {
                        ...
                        if (this.mImageListenerMap.containsKey(timestamp)) {
                            ...
                            if (entry.second != null) {
                                ...
                                Log.m102w(CameraExtensionSessionImpl.TAG, "Invalid image listener, dropping frame!");
                                img.close();
                            }
                        } else {
                        ...
                        Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to acquire image, too many images pending!");
            ...
            synchronized (CameraExtensionSessionImpl.this.mInterfaceLock) {
                if (this.mImageListenerMap.containsKey(timestamp)) {
                    ...
                    if (entry.first != null) {
                        ...
                        Log.m102w(CameraExtensionSessionImpl.TAG, "No valid image for listener with ts: " + timestamp.longValue());
                    }
                } else {
        ...
        */
                ...
                if (timestamp != null) {
                    ...
                    if (this.mSingleCapture || CameraExtensionSessionImpl.this.mPreviewProcessorType != 0) {
                        if (CameraExtensionSessionImpl.this.mPreviewProcessorType == 1) {
                            ...
                            if (idx >= 0 && this.mPendingResultMap.get(timestamp.longValue()).first == null) {
                                ...
                                try {
                                    ...
                                    processStatus = false;
                                    Log.m96e(CameraExtensionSessionImpl.TAG, "Extension service does not respond during processing, dropping frame!");
                                    parcelImage.buffer.close();
                                    image = this.mPendingResultMap.get(timestamp.longValue()).first;
                                } catch (RuntimeException e2) {
                                    processStatus = false;
                                    Log.m96e(CameraExtensionSessionImpl.TAG, "Runtime exception encountered during buffer processing, dropping frame!");
                                    parcelImage.buffer.close();
                                    image = this.mPendingResultMap.get(timestamp.longValue()).first;
                                }
                                image.close();
                        ...
                        try {
                            ...
                            Log.m96e(CameraExtensionSessionImpl.TAG, "Extension service does not respond during processing!");
                        }
                        if (captureStage != null) {
                            try {
                                ...
                                Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to update repeating request settings!");
        ...
        private void resumeInternalRepeatingRequest(boolean internal) {
            try {
                ...
                Log.m96e(CameraExtensionSessionImpl.TAG, "Failed to resume internal repeating request, extension service fails to respond!");
        ...
        public void discardPendingRepeatingResults(int idx, LongSparseArray<Pair<Image, TotalCaptureResult>> previewMap, boolean notifyCurrentIndex) {
            ...
            for (int i = idx; i >= 0; i--) {
                if (previewMap.valueAt(i).first != null) {
                    ...
                    Log.m102w(CameraExtensionSessionImpl.TAG, "Preview frame drop with timestamp: " + previewMap.keyAt(i));
                    long ident = Binder.clearCallingIdentity();
                    try {
        ...
        private class ImageForwardCallback implements OnImageAvailableListener {
            ...
            public void onImageAvailable(ImageReader reader, Image img) {
                ...
                try {
                    ...
                    Log.m102w(CameraExtensionSessionImpl.TAG, "Output surface likely abandoned, dropping buffer!");
                    img.close();
                } catch (RuntimeException e2) {
