...
public class CameraInjectionSessionImpl extends CameraInjectionSession
  ...
  public void binderDied() {
    synchronized (this.mInterfaceLock) {
      Log.m102w(TAG, "CameraInjectionSessionImpl died unexpectedly");
      if (this.mInjectionSession == null) {
        return;
      }
      Runnable r =
          new Runnable() { // from class: android.hardware.camera2.impl.CameraInjectionSessionImpl.1
  ...
  public void setRemoteInjectionSession(ICameraInjectionSession injectionSession) {
    synchronized (this.mInterfaceLock) {
      if (injectionSession == null) {
        Log.m96e(TAG, "The camera injection session has encountered a serious error");
      ...
      try {
        ...
        this.mExecutor.execute(
            new Runnable() { // from class:
              ...
              @Override // java.lang.Runnable
              public void run() {
                CameraInjectionSessionImpl.this.mInjectionStatusCallback.onInjectionSucceeded(
                    CameraInjectionSessionImpl.this);
              }
            });
      } catch (RemoteException e) {
        scheduleNotifyError(0);
  ...
  public void onInjectionError(int errorCode) {
    Log.m100v(
        TAG,
        String.format("Injection session error received, code %d", Integer.valueOf(errorCode)));
    synchronized (this.mInterfaceLock) {
      if (this.mInjectionSession == null) {
        return;
      }
      switch (errorCode) {
        ...
        case 2:
          ...
          break;
        default:
          Log.m96e(TAG, "Unknown error from injection session: " + errorCode);
          scheduleNotifyError(1);
          break;
      }
    }
