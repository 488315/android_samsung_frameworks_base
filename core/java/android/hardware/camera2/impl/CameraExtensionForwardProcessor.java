...
public class CameraExtensionForwardProcessor {
  ...
  public void onOutputSurface(Surface surface, int format) {
    ...
    try {
      ...
      Log.m96e(TAG, "Failed to initialize forward processor, extension service does not respond!");
    }
  }
  ...
  public void process(
      ParcelImage image, TotalCaptureResult totalCaptureResult, IProcessResultImpl resultCallback)
    ...
    public void onImageAvailable(ImageReader reader) {
      try {
        ...
        if (processedImage == null) {
          Log.m96e(CameraExtensionForwardProcessor.TAG, "Invalid image");
          return;
        }
        if (CameraExtensionForwardProcessor.this.mOutputSurface != null
          ...
          if (CameraExtensionForwardProcessor.this.mOutputWriter == null) {
            CameraExtensionForwardProcessor cameraExtensionForwardProcessor =
                CameraExtensionForwardProcessor.this;
            cameraExtensionForwardProcessor.mOutputWriter =
                ImageWriter.newInstance(
                    cameraExtensionForwardProcessor.mOutputSurface, 3, processedImage.getFormat());
          }
          try {
            CameraExtensionForwardProcessor.this.mOutputWriter.queueInputImage(processedImage);
            return;
          } catch (IllegalStateException e) {
            Log.m96e(CameraExtensionForwardProcessor.TAG, "Failed to queue processed buffer!");
        ...
        Log.m96e(CameraExtensionForwardProcessor.TAG, "Failed to acquire processed image!");
      }
    }
  }
}
