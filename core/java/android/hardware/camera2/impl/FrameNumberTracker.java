      ...
      if (this.mPendingFrameNumbers[requestType].isEmpty()) {
        ...
        while (true) {
          ...
          if (this.mPendingFrameNumbersWithOtherType[otherType].isEmpty()
              || errorFrameNumber
                  != this.mPendingFrameNumbersWithOtherType[otherType].element().longValue()) {
            i++;
          } else {
  ...
  private void updateCompletedFrameNumber(long frameNumber, int requestType)
    ...
    if (frameNumber < maxOtherFrameNumberSeen) {
      if (!this.mPendingFrameNumbers[requestType].isEmpty()) {
        ...
        int index1 =
            this.mPendingFrameNumbersWithOtherType[otherType1].indexOf(Long.valueOf(frameNumber));
        int index2 =
            this.mPendingFrameNumbersWithOtherType[otherType2].indexOf(Long.valueOf(frameNumber));
