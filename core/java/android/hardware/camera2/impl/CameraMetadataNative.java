...
public class CameraMetadataNative implements Parcelable {
    ...
    static {
        ...
        hashMap.put(CameraCharacteristics.f114x783e32d9.getNativeKey(), new GetCommand() { // from class: android.hardware.camera2.impl.CameraMetadataNative.12
            /* JADX WARN: Multi-variable type inference failed */
            @Override // android.hardware.camera2.impl.GetCommand
            public <T> T getValue(CameraMetadataNative cameraMetadataNative, Key<T> key) {
    ...
    public Face[] getFaces() {
        ...
        if (faceScores.length != faceRectangles.length) {
            Log.m102w(TAG, String.format("Face score size(%d) doesn match face rectangle size(%d)!", Integer.valueOf(faceScores.length), Integer.valueOf(faceRectangles.length)));
        }
        int numFaces = Math.min(faceScores.length, faceRectangles.length);
        if (faceDetectMode.intValue() == 2) {
            if (faceIds == null || faceLandmarks == null) {
                Log.m102w(TAG, "Expect face ids and landmarks to be non-null for FULL mode,fallback to SIMPLE mode");
                faceDetectMode = 1;
            } else {
                if (faceIds.length != numFaces || faceLandmarks.length != numFaces * 6) {
                    Log.m102w(TAG, String.format("Face id size(%d), or face landmark size(%d) don'tmatch face number(%d)!", Integer.valueOf(faceIds.length), Integer.valueOf(faceLandmarks.length * 6), Integer.valueOf(numFaces)));
                }
            ...
            if (faceIds == null) {
                ...
                if (faceIds.length != numFaces) {
                    Log.m102w(TAG, String.format("Face id size(%d) don't match face number(%d)!", Integer.valueOf(faceIds.length), Integer.valueOf(numFaces)));
                }
    ...
    public boolean setGpsLocation(Location l) {
        ...
        if (processMethod == null) {
            Log.m102w(TAG, "setGpsLocation - No process method, Location is not from a GPS or NETWORKprovider");
        } else {
    ...
    public StreamConfigurationMap getStreamConfigurationMapMaximumResolution() {
        ...
        if (configurations != null && minFrameDurations != null && stallDurations != null) {
            ...
            StreamConfiguration[] dynamicDepthConfigurations = (StreamConfiguration[]) getBase(CameraCharacteristics.f113x6f2fa73c);
            StreamConfigurationDuration[] dynamicDepthMinFrameDurations = (StreamConfigurationDuration[]) getBase(CameraCharacteristics.f112x51945058);
            ...
            HighSpeedVideoConfiguration[] highSpeedVideoConfigurations = (HighSpeedVideoConfiguration[]) getBase(CameraCharacteristics.f111xa18493cc);
