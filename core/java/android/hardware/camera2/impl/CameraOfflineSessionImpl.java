...
public class CameraOfflineSessionImpl extends CameraOfflineSession implements IBinder.DeathRecipient {
    ...
    public class CameraDeviceCallbacks extends ICameraDeviceCallbacks.Stub {
        ...
        public void onRepeatingRequestError(long lastFrameNumber, int repeatingRequestId) {
            Log.m96e(CameraOfflineSessionImpl.TAG, "Unexpected repeating request error received. Last frame number is " + lastFrameNumber);
        }
        ...
        public void onDeviceIdle() {
            synchronized (CameraOfflineSessionImpl.this.mInterfaceLock) {
                if (CameraOfflineSessionImpl.this.mRemoteSession == null) {
                    Log.m100v(CameraOfflineSessionImpl.TAG, "Ignoring idle state notifications during offline switches");
                    return;
                }
        ...
        private void onCaptureErrorLocked(int errorCode, CaptureResultExtras resultExtras) {
            ...
            final CaptureCallbackHolder holder = (CaptureCallbackHolder) CameraOfflineSessionImpl.this.mCaptureCallbackMap.get(requestId);
            if (holder == null) {
                Log.m96e(CameraOfflineSessionImpl.TAG, String.format("Receive capture error on unknown request ID %d", Integer.valueOf(requestId)));
                return;
            }
            ...
            if (errorCode == 5) {
                ...
                if (config == null) {
                    Log.m100v(CameraOfflineSessionImpl.TAG, String.format("Stream %d has been removed. Skipping buffer lost callback", Integer.valueOf(resultExtras.getErrorStreamId())));
                    return;
                }
