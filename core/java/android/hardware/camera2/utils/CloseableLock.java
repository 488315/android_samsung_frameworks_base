...
public class CloseableLock implements AutoCloseable {
  ...
  public CloseableLock() {
    ...
    this.mLockCount =
        new ThreadLocal<Integer>() { // from class: android.hardware.camera2.utils.CloseableLock.1
  ...
  public void close() {
    ...
    if (this.mLockCount.get().intValue() != 1) {
      throw new IllegalStateException(
          "Cannot close while one or more acquired locks are being held by this thread; release all"
              + " other locks first");
    }
    try {
      this.mLock.lock();
  ...
  private void log(String what) {
    Log.m100v("CloseableLock[" + this.mName + NavigationBarInflaterView.SIZE_MOD_END, what);
  }
}
