...
public final class CaptureRequest extends CameraMetadata<Key<?>> implements Parcelable {
  ...
  public static final Key<Range<Integer>> CONTROL_AE_TARGET_FPS_RANGE =
      new Key<>(
          ...
          new TypeReference<
              Range<Integer>>() { // from class: android.hardware.camera2.CaptureRequest.2
          });
    ...
    if (this.mRequestType == -1) {
      if (this.mIsReprocess) {
        ...
        if (enableZsl != null && enableZsl.booleanValue()) {
          int captureIntent =
              ((Integer) this.mLogicalCameraSettings.get(CONTROL_CAPTURE_INTENT)).intValue();
          if (captureIntent == 2) {
            isZslStill = true;
          }
        }
  ...
  private CaptureRequest(CaptureRequest source) {
    ...
    CameraMetadataNative cameraMetadataNative =
        this.mPhysicalCameraSettings.get(this.mLogicalCameraId);
  ...
  private CaptureRequest(
    ...
    if (physicalCameraIdSet != null && isReprocess) {
      throw new IllegalArgumentException(
          "Create a reprocess capture request with with more than one physical camera is not"
              + " supported!");
    }
  ...
  public int getReprocessableSessionId() {
    ...
    if (!this.mIsReprocess || (i = this.mReprocessableSessionId) == -1) {
      throw new IllegalStateException(
          "Getting the reprocessable session ID for a non-reprocess capture request is illegal.");
    }
    return i;
  }
  ...
  public void readFromParcel(Parcel in) {
    ...
    synchronized (this.mSurfacesLock) {
      ...
      Parcelable[] parcelableArray =
          (Parcelable[]) in.readParcelableArray(Surface.class.getClassLoader(), Surface.class);
  ...
  public void convertSurfaceToStreamId(SparseArray<OutputConfiguration> configuredOutputs) {
    synchronized (this.mSurfacesLock) {
      ...
      Log.m100v("CaptureRequest-JV", "Cannot convert already converted surfaces!");
    }
  }
  ...
  public static final class Builder {
    ...
    public Builder(
      ...
      this.mRequest =
          new CaptureRequest(
              template, reprocess, reprocessableSessionId, logicalCameraId, physicalCameraIdSet);
    }
