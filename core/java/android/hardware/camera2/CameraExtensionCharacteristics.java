...
public final class CameraExtensionCharacteristics {
  ...
  public CameraExtensionCharacteristics(
      Context context, String cameraId, Map<String, CameraCharacteristics> characteristicsMap) {
    ...
    private void connectToProxyLocked(Context ctx) {
      if (this.mConnection == null) {
        ...
        if (!vendorProxyPackage.isEmpty() && !vendorProxyService.isEmpty()) {
          Log.m100v(
              TAG, "Choosing the vendor camera extensions proxy package: " + vendorProxyPackage);
          Log.m100v(
              TAG, "Choosing the vendor camera extensions proxy service: " + vendorProxyService);
        ...
        this.mConnection =
            new ServiceConnection() { // from class:
              ...
              public void onServiceConnected(ComponentName component, IBinder binder) {
                CameraExtensionManagerGlobal.this.mProxy =
                ...
                try {
                  CameraExtensionManagerGlobal cameraExtensionManagerGlobal =
                      CameraExtensionManagerGlobal.this;
                  cameraExtensionManagerGlobal.mSupportsAdvancedExtensions =
                      cameraExtensionManagerGlobal.mProxy.advancedExtensionsSupported();
                } catch (RemoteException e) {
                  Log.m96e(CameraExtensionManagerGlobal.TAG, "Remote IPC failed!");
                }
        ...
        try {
          ...
          Log.m96e(TAG, "Timed out while initializing proxy service!");
        }
      }
    }
    ...
    public boolean registerClient(Context ctx, IBinder token) {
      synchronized (this.mLock) {
        ...
        try {
          ...
          Log.m96e(TAG, "Failed to initialize extension! Extension service does  not respond!");
        }
        if (!ret) {
          this.mConnectionCount--;
        }
        ...
        try {
          if (iCameraExtensionsProxyService != null) {
            try {
              ...
              Log.m96e(
                  TAG, "Failed to de-initialize extension! Extension service does not respond!");
      ...
      synchronized (this.mLock) {
        ...
        if (iCameraExtensionsProxyService != null) {
          try {
            ...
            Log.m96e(TAG, "Failed to release session! Extension service does not respond!");
          }
        }
      }
    }
  ...
  public static boolean isExtensionSupported(
      String cameraId, int extensionType, Map<String, CameraMetadataNative> characteristicsMap) {
    if (areAdvancedExtensionsSupported()) {
      try {
        ...
        Log.m96e(
            TAG, "Failed to query extension availability! Extension service does not respond!");
        return false;
      }
    }
    try {
  ...
  public List<Integer> getSupportedExtensions() {
    ...
    IBinder token =
        new Binder("CameraExtensionCharacteristics#getSupportedExtensions:" + this.mCameraId);
  ...
  public boolean isPostviewAvailable(int extension) {
    ...
    try {
      ...
      try {
        ...
        Log.m96e(
            TAG,
            "Failed to query the extension for postview availability! Extension service does not"
                + " respond!");
        unregisterClient(this.mContext, token);
        return false;
      }
  ...
  public List<Size> getPostviewSupportedSizes(int extension, Size captureSize, int format) {
    IBinder token =
        new Binder("CameraExtensionCharacteristics#getPostviewSupportedSizes:" + this.mCameraId);
    boolean success = registerClient(this.mContext, token);
    try {
      if (!success) {
        throw new IllegalArgumentException("Unsupported extensions");
        ...
        switch (format) {
          ...
          case 256:
            ...
            return generateSupportedSizes(
                extender.getSupportedPostviewResolutions(sz), Integer.valueOf(format), streamMap);
          default:
      ...
      if (format == 256) {
        return generateJpegSupportedSizes(
            extenders.second.getSupportedPostviewResolutions(sz), streamMap);
      }
      throw new IllegalArgumentException("Unsupported format: " + format);
    } catch (RemoteException e) {
      Log.m96e(
          TAG,
          "Failed to query the extension postview supported sizes! Extension service does not"
              + " respond!");
  ...
  public <T> List<Size> getExtensionSupportedSizes(int extension, Class<T> klass) {
    ...
    IBinder token =
        new Binder("CameraExtensionCharacteristics#getExtensionSupportedSizes:" + this.mCameraId);
    boolean success = registerClient(this.mContext, token);
    try {
      if (!success) {
        throw new IllegalArgumentException("Unsupported extensions");
      ...
      if (areAdvancedExtensionsSupported()) {
        ...
        extender.init(this.mCameraId, this.mCharacteristicsMapNative);
        return generateSupportedSizes(
            extender.getSupportedPreviewOutputResolutions(this.mCameraId), 34, streamMap);
      }
      Pair<IPreviewExtenderImpl, IImageCaptureExtenderImpl> extenders =
          initializeExtension(extension);
      ...
      Log.m96e(
          TAG,
          "Failed to query the extension supported sizes! Extension service does not respond!");
      return new ArrayList();
    } finally {
      unregisterClient(this.mContext, token);
    }
    ...
    try {
      ...
      try {
        ...
        if (format == 35) {
          ...
          return extenders.second.getCaptureProcessor() == null
              ...
              : generateSupportedSizes(
                  extenders.second.getSupportedResolutions(), Integer.valueOf(format), streamMap);
        }
        if (format != 256) {
          throw new IllegalArgumentException("Unsupported format: " + format);
        }
        ...
        return extenders2.second.getCaptureProcessor() != null
            ? generateJpegSupportedSizes(extenders2.second.getSupportedResolutions(), streamMap)
      ...
      case 256:
        ...
        try {
          try {
            ...
            Log.m96e(
                TAG,
                "Failed to query the extension capture latency! Extension service does not"
                    + " respond!");
          }
          if (!isExtensionSupported(this.mCameraId, extension, this.mCharacteristicsMapNative)) {
            throw new IllegalArgumentException("Unsupported extension");
          ...
          if (areAdvancedExtensionsSupported()) {
            ...
            LatencyRange latencyRange =
                extender.getEstimatedCaptureLatencyRange(this.mCameraId, sz, format);
  ...
  public boolean isCaptureProcessProgressAvailable(int extension) {
    IBinder token =
        new Binder(
            "CameraExtensionCharacteristics#isCaptureProcessProgressAvailable:" + this.mCameraId);
    boolean success = registerClient(this.mContext, token);
    try {
      if (!success) {
        throw new IllegalArgumentException("Unsupported extensions");
      ...
      try {
        ...
        Log.m96e(
            TAG,
            "Failed to query the extension progress callbacks! Extension service does not"
                + " respond!");
        unregisterClient(this.mContext, token);
        return false;
      }
  ...
  public Set<CaptureRequest.Key> getAvailableCaptureRequestKeys(int extension) {
    ...
    IBinder token =
        new Binder(
            "CameraExtensionCharacteristics#getAvailableCaptureRequestKeys:" + this.mCameraId);
    boolean success = registerClient(this.mContext, token);
    if (!success) {
      throw new IllegalArgumentException("Unsupported extensions");
    }
    ...
    try {
      try {
        ...
        if (captureRequestMeta != null) {
          int[] requestKeys =
              (int[]) captureRequestMeta.get(CameraCharacteristics.REQUEST_AVAILABLE_REQUEST_KEYS);
          if (requestKeys == null) {
            throw new AssertionError(
                "android.request.availableRequestKeys must be non-null in the characteristics");
          }
  ...
  public Set<CaptureResult.Key> getAvailableCaptureResultKeys(int extension) {
    ...
    IBinder token =
        new Binder(
            "CameraExtensionCharacteristics#getAvailableCaptureResultKeys:" + this.mCameraId);
    boolean success = registerClient(this.mContext, token);
    if (!success) {
      throw new IllegalArgumentException("Unsupported extensions");
    }
    ...
    try {
      try {
        ...
        if (captureResultMeta != null) {
          int[] resultKeys =
              (int[]) captureResultMeta.get(CameraCharacteristics.REQUEST_AVAILABLE_RESULT_KEYS);
          if (resultKeys == null) {
            throw new AssertionError(
                "android.request.availableResultKeys must be non-null in the characteristics");
          }
          CameraCharacteristics resultChars = new CameraCharacteristics(captureResultMeta);
          ret.addAll(
              resultChars.getAvailableKeyList(CaptureResult.class, crKeyTyped, resultKeys, true));
