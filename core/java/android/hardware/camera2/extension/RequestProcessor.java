...
public final class RequestProcessor {
    ...
    public interface RequestCallback {
        ...
        void onCaptureFailed(
                Request request, android.hardware.camera2.CaptureFailure captureFailure);
    ...
    public static final class Request {
        ...
        public Request(
                List<Integer> outputConfigIds,
                List<Pair<CaptureRequest.Key, Object>> parameters,
                int templateId) {
        ...
        static CameraMetadataNative getParametersMetadata(
                long vendorId, List<Pair<CaptureRequest.Key, Object>> parameters) {
            ...
            for (Pair<CaptureRequest.Key, Object> pair : parameters) {
                ret.set(
                        (CaptureRequest.Key<CaptureRequest.Key>) pair.first,
                        (CaptureRequest.Key) pair.second);
            }
            return ret;
        }

        static List<android.hardware.camera2.extension.Request> initializeParcelable(
                long vendorId, List<Request> requests) {
    ...
    public int submit(Request request, Executor executor, RequestCallback callback)
            throws CameraAccessException {
        ArrayList<Request> requests = new ArrayList<>(1);
        requests.add(0, request);
        List<android.hardware.camera2.extension.Request> parcelableRequests =
                Request.initializeParcelable(this.mVendorId, requests);
        try {
            int ret =
                    this.mRequestProcessor.submit(
                            parcelableRequests.get(0),
    ...
    public int submitBurst(List<Request> requests, Executor executor, RequestCallback callback)
            throws CameraAccessException {
        List<android.hardware.camera2.extension.Request> parcelableRequests =
                Request.initializeParcelable(this.mVendorId, requests);
        try {
            int ret =
                    this.mRequestProcessor.submitBurst(
                            parcelableRequests,
                            new RequestCallbackImpl(requests, callback, executor));
            if (ret != -1) {
                return ret;
    ...
    public int setRepeating(Request request, Executor executor, RequestCallback callback)
            throws CameraAccessException {
        ArrayList<Request> requests = new ArrayList<>(1);
        requests.add(0, request);
        List<android.hardware.camera2.extension.Request> parcelableRequests =
        ...
        try {
            int ret =
                    this.mRequestProcessor.setRepeating(
                            parcelableRequests.get(0),
                            new RequestCallbackImpl(requests, callback, executor));
            if (ret == -1) {
    ...
    static class RequestCallbackImpl extends IRequestCallback.Stub {
        ...
        public RequestCallbackImpl(
                List<Request> requests, RequestCallback callback, Executor executor) {
        ...
        public void onCaptureStarted(
                final int requestId, final long frameNumber, final long timestamp) {
            if (this.mRequests.get(requestId) != null) {
                long ident = Binder.clearCallingIdentity();
                try {
                    this.mExecutor.execute(
                            new Runnable() { // from class:
                                // android.hardware.camera2.extension.RequestProcessor$RequestCallbackImpl$$ExternalSyntheticLambda2
                                @Override // java.lang.Runnable
                                public final void run() {
                                    RequestProcessor.RequestCallbackImpl.this
                                            .lambda$onCaptureStarted$0(
                                                    requestId, frameNumber, timestamp);
                                }
                            });
                    return;
                } finally {
        ...
        public void onCaptureProgressed(final int requestId, ParcelCaptureResult partialResult) {
            if (this.mRequests.get(requestId) != null) {
                final CaptureResult result =
                        new CaptureResult(
                                partialResult.cameraId,
                                partialResult.results,
                                partialResult.parent,
                                partialResult.sequenceId,
                                partialResult.frameNumber);
                long ident = Binder.clearCallingIdentity();
                try {
                    this.mExecutor.execute(
                            new Runnable() { // from class:
                                // android.hardware.camera2.extension.RequestProcessor$RequestCallbackImpl$$ExternalSyntheticLambda3
                                @Override // java.lang.Runnable
                                public final void run() {
                                    RequestProcessor.RequestCallbackImpl.this
                                            .lambda$onCaptureProgressed$1(requestId, result);
                                }
                            });
                    return;
                } finally {
        ...
        public void onCaptureCompleted(
                final int requestId, ParcelTotalCaptureResult totalCaptureResult) {
            if (this.mRequests.get(requestId) != null) {
                ...
                if (totalCaptureResult.physicalResult != null
                    ...
                    physicalResults =
                            (PhysicalCaptureResultInfo[])
                                    totalCaptureResult.physicalResult.toArray(
                                            new PhysicalCaptureResultInfo[count]);
                }
                ArrayList<CaptureResult> partials =
                        new ArrayList<>(totalCaptureResult.partials.size());
                for (ParcelCaptureResult parcelResult : totalCaptureResult.partials) {
                    partials.add(
                            new CaptureResult(
                                    parcelResult.cameraId,
                                    parcelResult.results,
                                    parcelResult.parent,
                                    parcelResult.sequenceId,
                                    parcelResult.frameNumber));
                }
                final TotalCaptureResult result =
                        new TotalCaptureResult(
                                totalCaptureResult.logicalCameraId,
                                totalCaptureResult.results,
                                totalCaptureResult.parent,
                                totalCaptureResult.sequenceId,
                                totalCaptureResult.frameNumber,
                                partials,
                                totalCaptureResult.sessionId,
                                physicalResults);
                long ident = Binder.clearCallingIdentity();
                try {
                    this.mExecutor.execute(
                            new Runnable() { // from class:
                                // android.hardware.camera2.extension.RequestProcessor$RequestCallbackImpl$$ExternalSyntheticLambda5
                                @Override // java.lang.Runnable
                                public final void run() {
                                    RequestProcessor.RequestCallbackImpl.this
                                            .lambda$onCaptureCompleted$2(requestId, result);
                                }
                            });
                    return;
                } finally {
        ...
        public void onCaptureFailed(final int requestId, CaptureFailure captureFailure) {
            if (this.mRequests.get(requestId) != null) {
                final android.hardware.camera2.CaptureFailure failure =
                        new android.hardware.camera2.CaptureFailure(
                                captureFailure.request,
                                captureFailure.reason,
                                captureFailure.dropped,
                                captureFailure.sequenceId,
                                captureFailure.frameNumber,
                                captureFailure.errorPhysicalCameraId);
                long ident = Binder.clearCallingIdentity();
                try {
                    this.mExecutor.execute(
                            new Runnable() { // from class:
                                // android.hardware.camera2.extension.RequestProcessor$RequestCallbackImpl$$ExternalSyntheticLambda0
                                @Override // java.lang.Runnable
                                public final void run() {
                                    RequestProcessor.RequestCallbackImpl.this
                                            .lambda$onCaptureFailed$3(requestId, failure);
                                }
                            });
                    return;
                } finally {
        ...
        public void onCaptureBufferLost(
                final int requestId, final long frameNumber, final int outputStreamId) {
            if (this.mRequests.get(requestId) != null) {
                long ident = Binder.clearCallingIdentity();
                try {
                    this.mExecutor.execute(
                            new Runnable() { // from class:
                                // android.hardware.camera2.extension.RequestProcessor$RequestCallbackImpl$$ExternalSyntheticLambda4
                                @Override // java.lang.Runnable
                                public final void run() {
                                    RequestProcessor.RequestCallbackImpl.this
                                            .lambda$onCaptureBufferLost$4(
                                                    requestId, frameNumber, outputStreamId);
                                }
                            });
                    return;
                } finally {
        ...
        public /* synthetic */ void lambda$onCaptureBufferLost$4(
            ...
            this.mCallback.onCaptureBufferLost(
                    this.mRequests.get(requestId), frameNumber, outputStreamId);
        }
        ...
        public void onCaptureSequenceCompleted(final int sequenceId, final long frameNumber) {
            ...
            try {
                this.mExecutor.execute(
                        new Runnable() { // from class:
                            // android.hardware.camera2.extension.RequestProcessor$RequestCallbackImpl$$ExternalSyntheticLambda1
                            @Override // java.lang.Runnable
                            public final void run() {
                                RequestProcessor.RequestCallbackImpl.this
                                        .lambda$onCaptureSequenceCompleted$5(
        ...
        public void onCaptureSequenceAborted(final int sequenceId) {
            ...
            try {
                this.mExecutor.execute(
                        new Runnable() { // from class:
                            // android.hardware.camera2.extension.RequestProcessor$RequestCallbackImpl$$ExternalSyntheticLambda6
                            @Override // java.lang.Runnable
                            public final void run() {
                                RequestProcessor.RequestCallbackImpl.this
                                        .lambda$onCaptureSequenceAborted$6(sequenceId);
                            }
                        });
            } finally {
                Binder.restoreCallingIdentity(ident);
