...
public class MarshalQueryableEnum<T extends Enum<T>> implements MarshalQueryable<T> {
    ...
    if ((nativeType == 1 || nativeType == 0) && (managedType.getType() instanceof Class)) {
      ...
      if (typeClass.isEnum()) {
        try {
          ...
          Log.m96e(TAG, "Can't marshal class " + typeClass + "; no default constructor");
        } catch (SecurityException e2) {
          Log.m96e(TAG, "Can't marshal class " + typeClass + "; not accessible");
        }
      }
    }
    return false;
  ...
  public static <T extends Enum<T>> T getEnumFromValue(Class<T> enumType, int value) {
    ...
    if (ordinal < 0 || ordinal >= values.length) {
      ...
      throw new IllegalArgumentException(
          String.format(
              "Argument 'value' (%d) was not a valid enum value for type %s (registered? %b)",
              objArr));
    }
    return values[ordinal];
  }
