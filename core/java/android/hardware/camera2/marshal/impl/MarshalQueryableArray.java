...
public class MarshalQueryableArray<T> implements MarshalQueryable<T> {
    ...
    private interface PrimitiveArrayFiller {
        ...
        static PrimitiveArrayFiller getPrimitiveArrayFiller(Class<?> componentType) {
            if (componentType == Integer.TYPE) {
                return new PrimitiveArrayFiller() { // from class:
                    // android.hardware.camera2.marshal.impl.MarshalQueryableArray.PrimitiveArrayFiller.1
                    ...
                    public void fillArray(Object arr, int size, ByteBuffer buffer) {
                        IntBuffer ib =
                                buffer.asIntBuffer().get((int[]) int[].class.cast(arr), 0, size);
            ...
            if (componentType == Float.TYPE) {
                return new PrimitiveArrayFiller() { // from class:
                    // android.hardware.camera2.marshal.impl.MarshalQueryableArray.PrimitiveArrayFiller.2
                    @Override // android.hardware.camera2.marshal.impl.MarshalQueryableArray.PrimitiveArrayFiller
                    public void fillArray(Object arr, int size, ByteBuffer buffer) {
                        FloatBuffer fb =
                                buffer.asFloatBuffer()
                                        .get((float[]) float[].class.cast(arr), 0, size);
            ...
            if (componentType == Long.TYPE) {
                return new PrimitiveArrayFiller() { // from class:
                    // android.hardware.camera2.marshal.impl.MarshalQueryableArray.PrimitiveArrayFiller.3
                    @Override // android.hardware.camera2.marshal.impl.MarshalQueryableArray.PrimitiveArrayFiller
                    public void fillArray(Object arr, int size, ByteBuffer buffer) {
                        LongBuffer lb =
                                buffer.asLongBuffer().get((long[]) long[].class.cast(arr), 0, size);
            ...
            if (componentType == Double.TYPE) {
                return new PrimitiveArrayFiller() { // from class:
                    // android.hardware.camera2.marshal.impl.MarshalQueryableArray.PrimitiveArrayFiller.4
                    @Override // android.hardware.camera2.marshal.impl.MarshalQueryableArray.PrimitiveArrayFiller
                    public void fillArray(Object arr, int size, ByteBuffer buffer) {
                        DoubleBuffer db =
                                buffer.asDoubleBuffer()
                                        .get((double[]) double[].class.cast(arr), 0, size);
            ...
            throw new UnsupportedOperationException(
                    "PrimitiveArrayFiller of type " + componentType.getName() + " not supported");
        }
    }
    ...
    private class MarshalerArray extends Marshaler<T> {
        ...
        protected MarshalerArray(TypeReference<T> typeReference, int i) {
            ...
            this.mComponentMarshaler =
                    MarshalRegistry.getMarshaler(componentType, this.mNativeType);
            ...
            if (elementSize != Marshaler.NATIVE_SIZE_DYNAMIC) {
                ...
                if (remaining % elementSize != 0) {
                    throw new UnsupportedOperationException(
                            "Arrays for "
                                    + this.mTypeReference
                                    + " must be packed tighly into a multiple of "
                                    + elementSize
                                    + "; but there are "
                ...
                if (MarshalHelpers.isUnwrappedPrimitiveClass(this.mComponentClass)
                        && this.mComponentClass
                                == MarshalHelpers.getPrimitiveTypeClass(this.mNativeType)) {
                    PrimitiveArrayFiller.getPrimitiveArrayFiller(this.mComponentClass)
                            .fillArray(array, arraySize, buffer);
                } else {
                ...
                array =
                        copyListToArray(
                                arrayList,
                                Array.newInstance(this.mComponentClass, arrayList.size()));
            }
            if (buffer.remaining() != 0) {
                Log.e(
                        MarshalQueryableArray.TAG,
                        "Trailing bytes ("
                                + buffer.remaining()
                                + ") left over after unpacking "
                                + this.mClass);
            }
