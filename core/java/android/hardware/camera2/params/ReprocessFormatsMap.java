    ...
    while (i < entry.length) {
      ...
      if (left2 < 1) {
        throw new IllegalArgumentException(
            String.format(
                "Input %x had no output format length listed", Integer.valueOf(inputFormat)));
      }
      int length = entry[i2];
      left = left2 - 1;
      i = i2 + 1;
      ...
      if (length > 0) {
        if (left < length) {
          throw new IllegalArgumentException(
              String.format(
                  "Input %x had too few output formats listed (actual: %d, expected: %d)",
                  Integer.valueOf(inputFormat), Integer.valueOf(left), Integer.valueOf(length)));
        }
        i += length;
        left -= length;
      }
      ...
      if (i < iArr.length) {
        ...
        if (left2 < 1) {
          throw new AssertionError(
              String.format(
                  "Input %x had no output format length listed", Integer.valueOf(format)));
        }
        int length = iArr[i2];
        left = left2 - 1;
        i = i2 + 1;
        if (length > 0) {
          if (left < length) {
            throw new AssertionError(
                String.format(
                    "Input %x had too few output formats listed (actual: %d, expected: %d)",
                    Integer.valueOf(format), Integer.valueOf(left), Integer.valueOf(length)));
          }
          i += length;
          left -= length;
        }
      ...
      if (i < iArr.length) {
        ...
        if (length > 0 && left3 < length) {
          throw new AssertionError(
              String.format(
                  "Input %x had too few output formats listed (actual: %d, expected: %d)",
                  Integer.valueOf(format), Integer.valueOf(left3), Integer.valueOf(length)));
        }
        ...
        throw new IllegalArgumentException(
            String.format("Input format %x was not one in #getInputs", Integer.valueOf(format)));
      }
    }
  }
