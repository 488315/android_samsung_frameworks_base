    ...
    public static final class Builder {
        ...
        private List<MandatoryStreamCombination> getAvailableMandatoryStreamCombinationsInternal(StreamCombinationTemplate[] chosenStreamCombinations, boolean s10Bit) {
            ...
            while (i < length) {
                ...
                while (i2 < length2) {
                    ...
                    if (s10Bit && template.mFormat == 54) {
                        ...
                        if (!sdrYuvSizes.equals(new HashSet(sizes))) {
                            Log.m96e(MandatoryStreamCombination.TAG, "The supported 10-bit YUV sizes are different from the supported 8-bit YUV sizes!");
                            return null;
                        }
                    }
                    ...
                    try {
                        ...
                        Log.m96e(MandatoryStreamCombination.TAG, "No available sizes found for format: " + template.mFormat + " size threshold: " + template.mSizeThreshold + " combination: " + combTemplate.mDescription);
                        return null;
                    }
                }
                int i5 = length;
                try {
                    ...
                    Log.m96e(MandatoryStreamCombination.TAG, "No stream information for mandatory combination: " + combTemplate.mDescription);
                    return null;
                }
            }
        ...
        public List<MandatoryStreamCombination> getAvailableMandatoryConcurrentStreamCombinations() {
            ...
            if (isColorOutputSupported()) {
                ...
                Log.m100v(MandatoryStreamCombination.TAG, "Device is not backward compatible, depth streams are mandatory!");
        ...
        public List<MandatoryStreamCombination> getAvailableMandatoryMaximumResolutionStreamCombinations() {
            if (!isColorOutputSupported()) {
                Log.m100v(MandatoryStreamCombination.TAG, "Device is not backward compatible!, no mandatory maximum res streams");
                return null;
            }
        ...
        private Size getMaxCameraRecordingSize() {
            ...
            for (Size sz2 : videoSizes) {
                ...
                if (minFrameDuration < 3.344481605351171E7d) {
                    Log.m98i(MandatoryStreamCombination.TAG, "External camera " + this.mCameraId + " has max video size:" + sz2);
                    return sz2;
                }
            }
            Log.m102w(MandatoryStreamCombination.TAG, "Camera " + this.mCameraId + " does not support any 30fps video output");
            return FULLHD;
        }
        ...
        private Size getMaxPreviewSize(List<Size> orderedPreviewSizes) {
            ...
            Log.m102w(MandatoryStreamCombination.TAG, "Camera " + this.mCameraId + " maximum preview size search failed with display size " + this.mDisplaySize);
