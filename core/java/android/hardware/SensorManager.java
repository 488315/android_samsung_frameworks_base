...
public abstract class SensorManager {
  ...
  public Sensor getDefaultSensor(int type) {
    ...
    if (type == 8) {
      ...
      for (Sensor sensor2 : getSensorList(8)) {
        if (sensor2.isWakeUpSensor() == wakeUpSensor) {
          Log.m98i(TAG, "get Physical proximity");
          return sensor2;
        }
      }
    }
    if (type == 65601) {
      ...
      for (Sensor sensor3 : getSensorList(5)) {
        if (sensor3.isWakeUpSensor() == wakeUpSensor) {
          Log.m98i(TAG, "get light sensor instead of autobrightness");
          return sensor3;
        }
      }
    }
    if (type > 65536) {
      ...
      if (p.size() > 0) {
        ...
        Log.m98i(TAG, "Get Sensor type = " + type);
        return sensor4;
      }
      return null;
    }
  ...
  public boolean registerListener(
      SensorEventListener listener, Sensor sensor, int samplingPeriodUs, int maxReportLatencyUs) {
  ...
  private LegacySensorManager getLegacySensorManager() {
    ...
    synchronized (this.mSensorListByType) {
      if (this.mLegacySensorManager == null) {
        Log.m98i(
            TAG,
            "This application is using deprecated SensorManager API which will be removed someday. "
                + " Please consider switching to the new API.");
  ...
  public void setActiveScreenParameter(int screenID) {
    ...
    if (it.hasNext()) {
      ...
      Log.m94d(TAG, "set parameter active screen = " + screenID + ", " + data[1]);
      if (!setOperationParameterImpl(info)) {
        Log.m94d(TAG, "set parameter active screen failed ");
      }
    }
  }
  ...
  public void setBrightnessHysteresisParameter(float[] values) {
    ...
    for (Sensor sensor : l) {
      if ((sensor.getVersion() & 2) != 0) {
        Log.m94d(TAG, "Inject hysteresis info");
        try {
          SensorAdditionalInfo info =
              SensorAdditionalInfo.createCustomInfo(sensor, 268435458, values);
          if (!setOperationParameterImpl(info)) {
            Log.m94d(TAG, "Inject hysteresis info failed ");
            return;
          }
          return;
        } catch (IllegalArgumentException e) {
          Log.m96e(TAG, "IllegalArgumentException" + e);
          return;
        }
      }
    }
