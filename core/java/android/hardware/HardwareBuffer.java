...
public final class HardwareBuffer implements Parcelable, AutoCloseable {
  ...
  public static final Parcelable.Creator<HardwareBuffer> CREATOR =
      new Parcelable.Creator<HardwareBuffer>() { // from class: android.hardware.HardwareBuffer.1
  ...
  public static HardwareBuffer create(int width, int height, int format, int layers, long usage) {
    ...
    if (nativeObject == 0) {
      throw new IllegalArgumentException(
          "Unable to create a HardwareBuffer, either the dimensions passed were too large, too many"
              + " image layers were requested, or an invalid set of usage flags or invalid format"
              + " was passed");
    }
    return new HardwareBuffer(nativeObject);
  }
  ...
  private HardwareBuffer(long nativeObject) {
    ...
    NativeAllocationRegistry registry =
        new NativeAllocationRegistry(loader, nGetNativeFinalizer(), bufferSize);
  ...
  private void checkClosed(String name) {
    if (isClosed()) {
      throw new IllegalStateException(
          "This HardwareBuffer has been closed and its " + name + " cannot be obtained.");
    }
  }
  ...
  public void writeToParcel(Parcel dest, int flags) {
    if (isClosed()) {
      throw new IllegalStateException(
          "This HardwareBuffer has been closed and cannot be written to a parcel.");
    }
