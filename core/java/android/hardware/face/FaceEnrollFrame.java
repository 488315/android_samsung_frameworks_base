...
public final class FaceEnrollFrame implements Parcelable {
  ...
  private FaceEnrollFrame(Parcel source) {
    this.mCell =
        (FaceEnrollCell)
            source.readParcelable(FaceEnrollCell.class.getClassLoader(), FaceEnrollCell.class);
    this.mStage = source.readInt();
    this.mData =
        (FaceDataFrame)
            source.readParcelable(FaceDataFrame.class.getClassLoader(), FaceDataFrame.class);
  }
