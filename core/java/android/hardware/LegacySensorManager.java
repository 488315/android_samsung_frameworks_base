...
final class LegacySensorManager {
    ...
    synchronized (SensorManager.class) {
      if (!sInitialized) {
        ...
        if (asInterface != null) {
          try {
            sRotation =
                asInterface.watchRotation(
                    new IRotationWatcher
                        .Stub() { // from class: android.hardware.LegacySensorManager.1
  ...
  private static final class LmsFilter {
    ...
    /* renamed from: mV */
    private float[] f107mV = new float[24];

    /* renamed from: mT */
    private long[] f106mT = new long[24];
    private int mIndex = 12;

    public float filter(long time, float in) {
      float v = in;
      float[] fArr = this.f107mV;
      int i = this.mIndex;
      float v1 = fArr[i];
      if (v - v1 > 180.0f) {
        v -= 360.0f;
      ...
      for (int i4 = 0; i4 < 11; i4++) {
        ...
        float Z = this.f107mV[j];
        long j2 = this.f106mT[j];
        float T = (((j2 / 2) + (r14[j + 1] / 2)) - time) * 1.0E-9f;
        float dT = (j2 - r14[j + 1]) * 1.0E-9f;
        float dT2 = dT * dT;
        A += Z * dT2;
        B += T * dT2 * T;
        C += T * dT2;
