    ...
    private void initLocked(Looper cameraLooper) {
        if (this.mSystemHandler == null) {
            if (cameraLooper == null) {
                ...
                Log.m94d("CameraLightSensor_Manager", "Using CameraManagerThread looper " + thread.getLooper());
            } else {
        ...
        if (this.mCameraLightSensorConnection == null) {
            ...
            this.mCameraLightSensorConnection = new ServiceConnection() { // from class: android.hardware.CameraLightSensorManager.1
                ...
                public void onServiceConnected(ComponentName name, IBinder service) {
                    Log.m94d("CameraLightSensor_Manager", " Service is connected, cmp name = " + name + " service = " + service);
                ...
                public void onServiceDisconnected(ComponentName name) {
                    ...
                    synchronized (CameraLightSensorManager.this.mLockIPC) {
                        ...
                        if (CameraLightSensorManager.this.mCameraLightSensorConnection != null) {
                            Log.m94d("CameraLightSensor_Manager", "Unbinding and removing connection for service.");
        ...
        if (!this.mProcessBinded) {
            try {
                ...
                Log.m94d("CameraLightSensor_Manager", "Binding Package status= " + this.mProcessBinded + " Component= " + in.getComponent() + " UserHandle= " + UserHandle.CURRENT_OR_SELF);
    ...
    public boolean registerCameraLightSensor(Looper cameraLooper, SensorEventListener sensorListener, Sensor sensor) {
        synchronized (this.mLockIPC) {
            if (this.mCameraLightListener != null) {
                if (isAllowListedListener(sensorListener.toString())) {
                    ...
                    if (sensorListener.toString().contains("com.android.server.display.AutomaticBrightnessController")) {
                        try {
                            ...
                            if (this.mkeyguard != null) {
                                Log.m94d("Debug_Camera_Manager", "lock screen, mkeyguard= " + this.mkeyguard.isDeviceLocked() + " isFaceUnlockActive= " + isFaceUnlockActive);
                            }
                ...
                Log.m96e("CameraLightSensor_Manager", sensorListener + " not in allowlisted packages of Camera Light Sensor.");
            }
            ...
            switch (msg.what) {
                case 1:
                    synchronized (CameraLightSensorManager.this.mLockIPC) {
                        ...
                        if (CameraLightSensorManager.this.mSystemIPC != null) {
                            try {
                                Log.m94d("CameraLightSensor_Manager", "IPC_SERVICE_START_CAMERA -> MSG_SERVICE_START_CAMERA");
                            ...
                            Log.m94d("CameraLightSensor_Manager", "Binding Package status= " + CameraLightSensorManager.this.mProcessBinded + " Component= " + in.getComponent() + " UserHandle= " + UserHandle.CURRENT_OR_SELF);
                            Log.m94d("CameraLightSensor_Manager", "IPC_SERVICE_START_CAMERA Service not started yet,retry binding service = " + this.retyrBinding);
                            ...
                            Log.m94d("CameraLightSensor_Manager", "IPC_SERVICE_START_CAMERA Service not started yet,retry= " + this.retry);
                            this.retry--;
                            CameraLightSensorManager.this.mSystemHandler.sendEmptyMessageDelayed(1, 500L);
                        } else if (this.retry == 0) {
                            Log.m94d("CameraLightSensor_Manager", "IPC_SERVICE_START_CAMERA Service not started yet, will try it again after service connected");
                ...
                case 3:
                    synchronized (CameraLightSensorManager.this.mLockIPC) {
                        ...
                        if (CameraLightSensorManager.this.mSystemIPC != null) {
                            try {
                                Log.m94d("CameraLightSensor_Manager", "IPC_SERVICE_UNREGISTER_CONNECTION -> MSG_SERVICE_STOP_CAMERA");
                                CameraLightSensorManager.this.mSystemIPC.send(msg3);
                            } catch (Exception e2) {
                                Log.m94d("CameraLightSensor_Manager", "IPC_SERVICE_UNREGISTER_CONNECTION -> Dead Object");
                                e2.printStackTrace();
                            }
                        }
                    }
                ...
                case 6:
                    synchronized (CameraLightSensorManager.this.mLockIPC) {
                        ...
                        if (CameraLightSensorManager.this.mCameraLightListener != null && CameraLightSensorManager.this.mCameraLightListener.size() > 0) {
                            ...
                            Log.m94d("CameraLightSensor_Manager", " Exposure= " + this.camera_ev_bv[0] + " Brightness= " + this.camera_ev_bv[1] + " Total listeners= " + CameraLightSensorManager.this.mCameraLightListener.size());
                        }
                        return;
                    }
                case 8:
                            ...
                            synchronized (CameraLightSensorManager.this.mCameraLightListener) {
                                if (CameraLightSensorManager.this.mCameraLightListener.size() > 0) {
                                        ...
                                        for (SensorEventListener evL : CameraLightSensorManager.this.mCameraLightListener) {
                                            if (evL != null && this.camera_ev_bv != null) {
                                                ...
                                                if (cvtLux[0] != this.old_lux) {
                                                    Log.m94d("Debug_Camera_Listener", "" + evL.toString() + " lux= " + cvtLux[0] + " exposure= " + cvtLux[1] + " bv= " + cvtLux[2]);
                                                }
                                    ...
                                    if (CameraLightSensorManager.this.mSystemHandler != null) {
                                        ...
                                        Log.m94d("CameraLightSensor_Manager", "MSG_MANAGER_LISTENER_LOOP mSystemHandler is null");
                                    }
                                } else {
                                    Log.m94d("CameraLightSensor_Manager", "MSG_MANAGER_LISTENER_LOOP array is " + CameraLightSensorManager.this.mCameraLightListener.size());
                                    if (CameraLightSensorManager.this.mSystemHandler != null) {
                                        CameraLightSensorManager.this.mSystemHandler.removeMessages(8);
                                    }
                                }
