    ...
    public GeomagneticField(float gdLatitudeDeg, float gdLongitudeDeg, float altitudeMeters, long timeMillis) {
        ...
        for (int n = 1; n < MAX_N; n++) {
            ...
            while (m2 <= n) {
                ...
                gcX += f * fArr[n][m2];
                gcY += relativeRadiusPower[n + 2] * m2 * ((sinMLon[m2] * g) - (cosMLon[m2] * h)) * legendre.f104mP[n][m2] * fArr[n][m2] * inverseCosLatitude;
                gcZ -= ((((n + 1) * relativeRadiusPower[n + 2]) * ((cosMLon[m2] * g) + (sinMLon[m2] * h))) * legendre.f104mP[n][m2]) * fArr[n][m2];
                m2++;
                MAX_N = MAX_N2;
            }
        }
