...
class DayPickerViewPager extends ViewPager {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int maxHeight2 =
        Math.max(maxHeight + getPaddingTop() + getPaddingBottom(), getSuggestedMinimumHeight());
    ...
    if (count2 > 1) {
      ...
      while (i4 < count2) {
        ...
        if (lp2.width == -1) {
          ...
          childWidthMeasureSpec =
              getChildMeasureSpec(
                  widthMeasureSpec, getPaddingLeft() + getPaddingRight(), lp2.width);
        }
        if (lp2.height == -1) {
          childHeightMeasureSpec =
              View.MeasureSpec.makeMeasureSpec(
                  (getMeasuredHeight() - getPaddingTop()) - getPaddingBottom(), i);
        } else {
          childHeightMeasureSpec =
              getChildMeasureSpec(
                  heightMeasureSpec, getPaddingTop() + getPaddingBottom(), lp2.height);
        }
