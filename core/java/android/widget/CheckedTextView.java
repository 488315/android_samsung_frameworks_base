...
public class CheckedTextView extends TextView implements Checkable {
    ...
    public CheckedTextView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.CheckedTextView, defStyleAttr, defStyleRes);
        saveAttributeDataForStyleable(context, C4337R.styleable.CheckedTextView, attrs, a, defStyleAttr, defStyleRes);
        Drawable d = a.getDrawable(1);
        TypedValue outValue = new TypedValue();
        getContext().getTheme().resolveAttribute(C4337R.attr.parentIsDeviceDefault, outValue, true);
        if (outValue.data != 0) {
            this.mIsDeviceDefault = true;
        }
        this.mDrawablePadding = getContext().getResources().getDimensionPixelSize(C4337R.dimen.tw_checkedtextview_padding);
        if (d != null) {
