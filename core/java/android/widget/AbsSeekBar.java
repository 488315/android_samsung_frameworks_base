...
public abstract class AbsSeekBar extends ProgressBar {
  ...
  public AbsSeekBar(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    this.mSetDualColorMode = false;
    TypedArray a =
        context.obtainStyledAttributes(attrs, C4337R.styleable.SeekBar, defStyleAttr, defStyleRes);
    saveAttributeDataForStyleable(
        context, C4337R.styleable.SeekBar, attrs, a, defStyleAttr, defStyleRes);
    ...
    a.recycle();
    if (useDisabledAlpha) {
      TypedArray ta = context.obtainStyledAttributes(attrs, C4337R.styleable.Theme, 0, 0);
    ...
    this.mThumbExclusionMaxSize =
        getResources().getDimensionPixelSize(C4337R.dimen.seekbar_thumb_exclusion_max_size);
    Resources res = context.getResources();
    this.mTrackMinWidth = res.getDimensionPixelSize(C4337R.dimen.sem_seekbar_track_height);
    this.mTrackMaxWidth = res.getDimensionPixelSize(C4337R.dimen.sem_seekbar_track_height_expand);
    this.mModeExpandTrackMinWidth =
        res.getDimensionPixelSize(C4337R.dimen.sem_seekbar_mode_expand_track_height);
    this.mModeExpandTrackMaxWidth =
        res.getDimensionPixelSize(C4337R.dimen.sem_seekbar_mode_expand_track_height_expand);
    this.mThumbRadius = res.getDimensionPixelSize(C4337R.dimen.sem_seekbar_thumb_radius);
    this.mModeExpandThumbRadius =
        res.getDimensionPixelSize(C4337R.dimen.sem_seekbar_mode_expand_thumb_radius);
    if (this.mIsDeviceDefaultDark) {
      i = C4337R.color.tw_seekbar_color_control_normal_dark;
    } else {
      i = C4337R.color.tw_seekbar_color_control_normal_light;
    }
    this.mDefaultNormalProgressColor = colorToColorStateList(res.getColor(i, null));
    this.mDefaultSecondaryProgressColor =
        colorToColorStateList(res.getColor(C4337R.color.tw_seekbar_color_control_secondary, null));
    ...
    if (semGetProgressTintList == null) {
      if (this.mIsDeviceDefaultDark) {
        i8 = C4337R.color.tw_seekbar_color_control_activated_dark;
      } else {
        i8 = C4337R.color.tw_seekbar_color_control_activated_light;
      }
      this.mDefaultActivatedProgressColor = colorToColorStateList(res.getColor(i8, null));
    }
    if (this.mIsDeviceDefaultDark) {
      i2 = C4337R.color.tw_seekbar_color_overlap_normal_dark;
    } else {
      i2 = C4337R.color.tw_seekbar_color_overlap_normal_light;
    }
    this.mOverlapNormalProgressColor = colorToColorStateList(res.getColor(i2, null));
    if (this.mIsDeviceDefaultDark) {
      i3 = C4337R.color.tw_seekbar_color_overlap_activated_dark;
    } else {
      i3 = 17171641;
    }
    ...
    if (this.mIsDeviceDefaultDark) {
      i4 = C4337R.color.tw_seekbar_color_overlap_activated_dark;
    } else {
      i4 = 17171641;
    }
    ...
    if (colorStateList == null) {
      ...
      if (this.mIsDeviceDefaultDark) {
        i6 = C4337R.color.tw_thumb_color_control_activated_dark;
      } else {
        i6 = C4337R.color.tw_thumb_color_control_activated_light;
      }
    ...
    boolean z = res.getBoolean(C4337R.bool.tw_seekbar_sliding_animation);
  ...
  protected void drawableStateChanged() {
    ...
    if (this.mSetDualColorMode
        ...
        && (drawable = this.mOverlapBackground) != null) {
  ...
  void drawTrack(Canvas canvas) {
    ...
    if (thumbDrawable != null && this.mSplitTrack) {
      ...
      canvas.clipRect(tempRect, Region.EnumC0836Op.DIFFERENCE);
  ...
  protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    setMeasuredDimension(
        ...
        resolveSizeAndState(dh + this.mPaddingTop + this.mPaddingBottom, heightMeasureSpec, 0));
  }
    ...
    switch (event.getAction()) {
      case 0:
        if (this.mThumb != null) {
          ...
          float progress =
              ((getProgress() - getMin()) / (getMax() - getMin()))
                  - ((event.getX() - this.mPaddingLeft) / availableWidth);
      ...
      case 2:
        ...
        if (this.mIsDragging) {
          ...
          float x = event.getX();
          float y = event.getY();
          if ((this.mCurrentMode != 3 && Math.abs(x - this.mTouchDownX) > this.mScaledTouchSlop)
              || (this.mCurrentMode == 3
                  && Math.abs(y - this.mTouchDownY) > this.mScaledTouchSlop)) {
            startDrag(event);
            break;
          }
        }
  ...
  private void trackTouchEvent(MotionEvent event) {
    ...
    if (isLayoutRtl() && this.mMirrorForRtl) {
      if (x > width - this.mPaddingRight) {
        ...
        scale =
            (((availableWidth - x) + this.mPaddingLeft) / availableWidth) + this.mTouchThumbOffset;
  ...
  */
        ...
        switch (keyCode) {
          ...
          case 81:
            if (setProgressInternal(
                getProgress() + (isLayoutRtl() ? -increment : increment), true, true)) {
              onKeyChange();
              break;
            }
        }
  ...
  public void semSetMode(int mode) {
    ...
    switch (mode) {
      ...
      case 3:
        Drawable thumb =
            this.mContext.getDrawable(C4337R.drawable.tw_scrubber_control_vertical_material_anim);
        setThumb(thumb);
        break;
      case 4:
        this.mSplitProgress =
            this.mContext.getDrawable(C4337R.drawable.tw_split_seekbar_primary_progress_material);
        this.mDivider =
            this.mContext.getDrawable(C4337R.drawable.tw_split_seekbar_vertical_bar_material);
        updateSplitProgress();
        break;
      case 5:
  ...
  private void initializeExpandMode() {
    ...
    Drawable thumbDrawable =
        new ThumbDrawable(this.mModeExpandThumbRadius, this.mDefaultActivatedThumbColor, false);
    Drawable[] drawables = {
      background,
    ...
    setBackgroundResource(C4337R.drawable.sem_seekbar_background_borderless);
  ...
  private void initMuteAnimation() {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      ValueAnimator progressZeroAnimation =
          isGoingDirection ? ValueAnimator.ofInt(0, distance) : ValueAnimator.ofInt(distance, 0);
      ...
      progressZeroAnimation.addUpdateListener(
          new ValueAnimator.AnimatorUpdateListener() { // from class: android.widget.AbsSeekBar.1
  ...
  public void setDefaultColorForVolumePanel(boolean isClearCoverOpened) {
    ...
    if (this.mIsDeviceDefaultDark) {
      i2 = C4337R.color.tw_seekbar_color_control_activated_dark;
    } else {
      i2 = C4337R.color.tw_seekbar_color_control_activated_light;
    }
    this.mDefaultActivatedProgressColor = colorToColorStateList(res.getColor(i2, null));
    if (this.mIsDeviceDefaultDark) {
      i3 = C4337R.color.tw_thumb_color_control_activated_dark;
    } else {
      i3 = C4337R.color.tw_thumb_color_control_activated_light;
    }
    this.mDefaultActivatedThumbColor = colorToColorStateList(res.getColor(i3, null));
    this.mOverlapNormalProgressColor =
        colorToColorStateList(
            res.getColor(C4337R.color.tw_seekbar_color_overlap_normal_light, null));
    boolean z = this.mIsDeviceDefaultDark;
    int i5 = C4337R.color.tw_seekbar_color_overlap_activated_light;
    if (z) {
      i4 = 17171641;
    } else {
      i4 = 17171640;
  ...
  private class SliderDrawable extends Drawable {
    ...
    private void initAnimator() {
      ...
      this.mPressedAnimator.addUpdateListener(
          new ValueAnimator
              .AnimatorUpdateListener() { // from class: android.widget.AbsSeekBar.SliderDrawable.1
            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
      ...
      this.mReleasedAnimator.addUpdateListener(
          new ValueAnimator
              .AnimatorUpdateListener() { // from class: android.widget.AbsSeekBar.SliderDrawable.2
            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
              float value = ((Float) valueAnimator.getAnimatedValue()).floatValue();
              SliderDrawable.this.invalidateTrack(value);
  ...
  private class ThumbDrawable extends Drawable {
    ...
    public ThumbDrawable(int radius, ColorStateList color, boolean isVertical) {
      ...
      paint.setStrokeWidth(
          AbsSeekBar.this
              .mContext
              .getResources()
              .getDimension(C4337R.dimen.sem_seekbar_thumb_stroke));
      paint2.setColor(
          AbsSeekBar.this
              .mContext
              .getResources()
              .getColor(C4337R.color.tw_thumb_control_fill_color_activated));
      this.mIsVertical = isVertical;
      initAnimation();
    }
    ...
    void initAnimation() {
      ...
      this.mThumbPressed.addUpdateListener(
          new ValueAnimator
              .AnimatorUpdateListener() { // from class: android.widget.AbsSeekBar.ThumbDrawable.1
            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
            public void onAnimationUpdate(ValueAnimator animation) {
      ...
      this.mThumbReleased.addUpdateListener(
          new ValueAnimator
              .AnimatorUpdateListener() { // from class: android.widget.AbsSeekBar.ThumbDrawable.2
            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
            public void onAnimationUpdate(ValueAnimator animation) {
              float value = ((Float) animation.getAnimatedValue()).floatValue();
              ThumbDrawable.this.setRadius((int) value);
