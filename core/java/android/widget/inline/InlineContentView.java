...
public class InlineContentView extends ViewGroup {
  ...
  public InlineContentView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    this.mSurfaceCallback =
        new SurfaceHolder.Callback() { // from class: android.widget.inline.InlineContentView.1
    ...
    this.mOnReparentListener =
        new SurfaceControl
            .OnReparentListener() { // from class: android.widget.inline.InlineContentView.2
        ...
        new SurfaceView(
          ...
          protected void onSetSurfacePositionAndScale(
            ...
            if (InlineContentView.this.mParentPosition != null) {
              positionLeft =
                  (int)
                      ((positionLeft - InlineContentView.this.mParentPosition[0])
                          / InlineContentView.this.mParentScale.f87x);
              positionTop =
                  (int)
                      ((positionTop - InlineContentView.this.mParentPosition[1])
                          / InlineContentView.this.mParentScale.f88y);
            }
            ...
            super.onSetSurfacePositionAndScale(
                transaction, surface, positionLeft, positionTop, postScaleX2, postScaleY2);
          }
        };
  ...
  protected void onAttachedToWindow() {
    ...
    if (surfacePackageUpdater != null) {
      surfacePackageUpdater.getSurfacePackage(
  ...
  public void computeParentPositionAndScale() {
    ...
    if (parentSurfaceOwnerView != null) {
      ...
      float oldParentScaleX = this.mParentScale.f87x;
      if (lastParentSurfaceWidth > 0.0f) {
        this.mParentScale.f87x = lastParentSurfaceWidth / parentSurfaceOwnerView.getWidth();
      } else {
        this.mParentScale.f87x = 1.0f;
      }
      if (!contentPositionOrScaleChanged
          && Float.compare(oldParentScaleX, this.mParentScale.f87x) != 0) {
        contentPositionOrScaleChanged = true;
      }
      float lastParentSurfaceHeight = parentSurfaceOwnerView.getSurfaceRenderPosition().height();
      float oldParentScaleY = this.mParentScale.f88y;
      if (lastParentSurfaceHeight > 0.0f) {
        this.mParentScale.f88y = lastParentSurfaceHeight / parentSurfaceOwnerView.getHeight();
      } else {
        this.mParentScale.f88y = 1.0f;
      }
      if (!contentPositionOrScaleChanged
          && Float.compare(oldParentScaleY, this.mParentScale.f88y) != 0) {
        contentPositionOrScaleChanged = true;
      }
