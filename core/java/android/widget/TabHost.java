...
public class TabHost extends FrameLayout implements ViewTreeObserver.OnTouchModeChangeListener {
  ...
  public TabHost(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    if (this.mTabLayoutId == 0) {
      this.mTabLayoutId = C4337R.layout.tab_indicator_holo;
    }
    initTabHost();
  }
  ...
  public boolean dispatchKeyEvent(KeyEvent event) {
    ...
    if (!handled
        && event.getAction() == 0
        && (view = this.mCurrentView) != null
        && view.isRootNamespace()
      ...
      if (event.getKeyCode() == keyCodeShouldChangeFocus
          && this.mCurrentView.findFocus().focusSearch(directionShouldChangeFocus) == null) {
  ...
  private class LabelIndicatorStrategy implements IndicatorStrategy {
    ...
    public View createIndicatorView() {
      ...
      View tabIndicator =
          inflater.inflate(TabHost.this.mTabLayoutId, (ViewGroup) TabHost.this.mTabWidget, false);
      ...
      if (context.getApplicationInfo().targetSdkVersion <= 4) {
        tabIndicator.setBackgroundResource(C4337R.drawable.tab_indicator_v4);
        tv.setTextColor(context.getColorStateList(C4337R.color.tab_indicator_text_v4));
      }
      return tabIndicator;
    }
  }
