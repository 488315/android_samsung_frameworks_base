...
public class ToastPresenter {
  ...
  public static View getTextToastView(Context context, CharSequence text) {
    ...
    View view =
        isDeviceDefault
            ? LayoutInflater.from(context).inflate(17367422, (ViewGroup) null)
            : LayoutInflater.from(context).inflate(17367467, (ViewGroup) null);
    TextView textView = (TextView) view.findViewById(16908299);
    textView.setText(text);
    semCheckMaxFontScale(
        context,
        textView,
        context.getResources().getDimensionPixelSize(C4337R.dimen.sem_toast_text_size));
    return view;
  }
  ...
  public ToastPresenter(
    ...
    this.mWindowManager =
        new WeakReference<>((WindowManager) context.getSystemService(WindowManager.class));
  ...
  private int semGetNavigationBarHeight() {
    boolean hasNavigationBar =
        this.mView.getContext().getResources().getBoolean(C4337R.bool.config_showNavigationBar);
    if (!hasNavigationBar) {
      return 0;
    }
    int navigationBarHeight =
        this.mView
            .getContext()
            .getResources()
            .getDimensionPixelSize(C4337R.dimen.navigation_bar_height);
  ...
  private int semGetAdjustedYoffset(int gravity, int yOffset) {
    ...
    int mFingerOffsetY =
        this.mContext
            .getResources()
            .getDimensionPixelSize(C4337R.dimen.sem_toast_fingerPrint_y_offset);
    if (fpm != null) {
    ...
    if (logView instanceof TextView) {
      ...
      if (text.length() > 0) {
        ...
        if (text.length() > 3) {
          ...
          Log.m100v(
              TAG,
              "Text: " + firstChar + ((Object) text.subSequence(1, text.length())) + " in " + this);
        }
      }
    }
  }
