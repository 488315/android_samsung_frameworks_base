...
public final class Magnifier {
  ...
  private Magnifier(Builder params) {
    ...
    this.mDefaultHorizontalSourceToMagnifierOffset =
    ...
    this.mRamp =
        (int)
            TypedValue.applyDimension(
                1, FISHEYE_RAMP_WIDTH, view.getContext().getResources().getDisplayMetrics());
  }
  ...
  public void show(float sourceCenterX, float sourceCenterY) {
    show(
        sourceCenterX,
        sourceCenterY,
        this.mDefaultHorizontalSourceToMagnifierOffset + sourceCenterX,
        this.mDefaultVerticalSourceToMagnifierOffset + sourceCenterY);
  }
  ...
  public void show(
      float sourceCenterX, float sourceCenterY, float magnifierCenterX, float magnifierCenterY) {
    ...
    int startX2 = this.mClampedCenterZoomCoords.f85x - (this.mSourceWidth / 2);
    int startY = this.mClampedCenterZoomCoords.f86y - (this.mSourceHeight / 2);
    if (this.mIsFishEyeStyle) {
      float magnifierCenterX5 =
          this.mClampedCenterZoomCoords.f85x - this.mViewCoordinatesInSurface[0];
      magnifierCenterY2 = this.mClampedCenterZoomCoords.f86y - this.mViewCoordinatesInSurface[1];
      ...
      int rightBound2 =
          Math.min(this.mViewCoordinatesInSurface[0] + rightBound, this.mContentCopySurface.mWidth);
    ...
    if (sourceCenterX != this.mPrevShowSourceCoords.f87x
        || sourceCenterY != this.mPrevShowSourceCoords.f88y
        || this.mDirtyState) {
    ...
    } else if (magnifierCenterX2 == this.mPrevShowWindowCoords.f87x
        && magnifierCenterY2 == this.mPrevShowWindowCoords.f88y) {
      magnifierCenterX3 = magnifierCenterX2;
      magnifierCenterY3 = magnifierCenterY2;
    } else {
      final Point windowCoords = getCurrentClampedWindowCoordinates();
      final InternalPopupWindow currentWindowInstance = this.mWindow;
      sPixelCopyHandlerThread
          .getThreadHandler()
          .post(
              new Runnable() { // from class: android.widget.Magnifier$$ExternalSyntheticLambda2
    ...
    this.mPrevShowSourceCoords.f87x = sourceCenterX;
    this.mPrevShowSourceCoords.f88y = sourceCenterY;
    this.mPrevShowWindowCoords.f87x = magnifierCenterX3;
    this.mPrevShowWindowCoords.f88y = magnifierCenterY3;
  }
  ...
  public /* synthetic */ void lambda$show$0(
    ...
    synchronized (this.mLock) {
      ...
      internalPopupWindow.setContentPositionForNextDraw(windowCoords.f85x, windowCoords.f86y);
    }
  }
  ...
  public void dismiss() {
    if (this.mWindow != null) {
      ...
      this.mPrevShowSourceCoords.f87x = -1.0f;
      this.mPrevShowSourceCoords.f88y = -1.0f;
      this.mPrevShowWindowCoords.f87x = -1.0f;
      this.mPrevShowWindowCoords.f88y = -1.0f;
      this.mPrevStartCoordsInSurface.f85x = -1;
      this.mPrevStartCoordsInSurface.f86y = -1;
    }
  }
  ...
  public void update() {
    if (this.mWindow != null) {
      ...
      if (!this.mDirtyState) {
        performPixelCopy(
            this.mPrevStartCoordsInSurface.f85x, this.mPrevStartCoordsInSurface.f86y, false);
      } else {
        show(
            this.mPrevShowSourceCoords.f87x,
            this.mPrevShowSourceCoords.f88y,
            this.mPrevShowWindowCoords.f87x,
            this.mPrevShowWindowCoords.f88y);
      }
    }
  }
  ...
  public void setZoom(float zoom) {
    ...
    this.mZoom = zoom;
    this.mSourceWidth =
        this.mIsFishEyeStyle ? this.mWindowWidth : Math.round(this.mWindowWidth / zoom);
    this.mSourceHeight = Math.round(this.mWindowHeight / this.mZoom);
  ...
  private void obtainSurfaces() {
    ...
    if (this.mView.getViewRootImpl() != null
        && (mainWindowSurface = (viewRootImpl = this.mView.getViewRootImpl()).mSurface) != null
        && mainWindowSurface.isValid()) {
    ...
    if (view instanceof SurfaceView) {
      ...
      if (sc != null && sc.isValid()) {
        ...
        validSurfaceViewSurface =
            new SurfaceInfo(
                sc, surfaceViewSurface, surfaceFrame.right, surfaceFrame.bottom, new Rect(), false);
      }
    }
    this.mParentSurface =
        validMainWindowSurface != SurfaceInfo.NULL
            ? validMainWindowSurface
            : validSurfaceViewSurface;
    this.mContentCopySurface =
        this.mView instanceof SurfaceView ? validSurfaceViewSurface : validMainWindowSurface;
  }
  ...
  private void obtainContentCoordinates(float xPosInView, float yPosInView) {
    ...
    Rect surfaceBounds =
        new Rect(0, 0, this.mContentCopySurface.mWidth, this.mContentCopySurface.mHeight);
    ...
    if (resolvedLeft2 < 0 || resolvedTop2 < 0) {
      Log.m96e(
          TAG,
          "Magnifier's content is copied from a surface smaller thanthe content requested size. The"
              + " magnifier will be dismissed.");
    }
    ...
    point2.f86y =
        Math.max((i6 / 2) + resolvedTop2, Math.min(zoomCenterY, resolvedBottom2 - (i6 / 2)));
  }
  ...
  private void obtainWindowCoordinates(float xWindowPos, float yWindowPos) {
    ...
    this.mWindowCoords.f85x = windowCenterX - (this.mWindowWidth / 2);
    this.mWindowCoords.f86y = windowCenterY - (this.mWindowHeight / 2);
    if (this.mParentSurface != this.mContentCopySurface) {
      this.mWindowCoords.f85x += this.mViewCoordinatesInSurface[0];
      this.mWindowCoords.f86y += this.mViewCoordinatesInSurface[1];
    }
  }
  ...
  private void maybeDrawCursor(Canvas canvas) {
    if (this.mDrawCursorEnabled) {
      ...
      if (drawable != null) {
        ...
        canvas.drawRect(
            new Rect((r3 / 2) - 1, 0, (this.mSourceWidth / 2) + 1, this.mSourceHeight), paint);
      }
    }
  }

  private void performPixelCopy(
      int startXInSurface, int startYInSurface, final boolean updateWindowPosition) {
    if (this.mContentCopySurface.mSurface == null || !this.mContentCopySurface.mSurface.isValid()) {
      onPixelCopyFailed();
      return;
    }
    final Point windowCoords = getCurrentClampedWindowCoordinates();
    this.mPixelCopyRequestRect.set(
        startXInSurface,
        startYInSurface,
        ((this.mSourceWidth + startXInSurface) - this.mLeftCutWidth) - this.mRightCutWidth,
        this.mSourceHeight + startYInSurface);
    this.mPrevStartCoordsInSurface.f85x = startXInSurface;
    this.mPrevStartCoordsInSurface.f86y = startYInSurface;
    this.mDirtyState = false;
    final InternalPopupWindow currentWindowInstance = this.mWindow;
    if (this.mPixelCopyRequestRect.width() == 0) {
      this.mWindow.updateContent(
          Bitmap.createBitmap(this.mSourceWidth, this.mSourceHeight, Bitmap.Config.ALPHA_8));
    } else {
      final Bitmap bitmap =
          Bitmap.createBitmap(
              (this.mSourceWidth - this.mLeftCutWidth) - this.mRightCutWidth,
              this.mSourceHeight,
              Bitmap.Config.ARGB_8888);
      PixelCopy.request(
          this.mContentCopySurface.mSurface,
          this.mPixelCopyRequestRect,
          bitmap,
          new PixelCopy
              .OnPixelCopyFinishedListener() { // from class:
                                               // android.widget.Magnifier$$ExternalSyntheticLambda1
            @Override // android.view.PixelCopy.OnPixelCopyFinishedListener
            public final void onPixelCopyFinished(int i) {
              Magnifier.this.lambda$performPixelCopy$1(
                  currentWindowInstance, updateWindowPosition, windowCoords, bitmap, i);
            }
          },
  ...
  public /* synthetic */ void lambda$performPixelCopy$1(
    ...
    synchronized (this.mLock) {
      ...
      if (width < i) {
        ...
        Rect dstRect =
            new Rect(
                this.mLeftCutWidth, 0, this.mSourceWidth - this.mRightCutWidth, bitmap.getHeight());
  ...
  private void onPixelCopyFailed() {
    Log.m96e(TAG, "Magnifier failed to copy content from the view Surface. It will be dismissed.");
    Handler.getMain()
        .postAtFrontOfQueue(
            new Runnable() { // from class: android.widget.Magnifier$$ExternalSyntheticLambda0
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private Point getCurrentClampedWindowCoordinates() {
    ...
    if (this.mParentSurface.mIsMainWindowSurface) {
      ...
      windowBounds =
          new Rect(
              systemInsets.left + this.mParentSurface.mInsets.left,
              systemInsets.top + this.mParentSurface.mInsets.top,
              (this.mParentSurface.mWidth - systemInsets.right) - this.mParentSurface.mInsets.right,
              (this.mParentSurface.mHeight - systemInsets.bottom)
    ...
    int windowCoordsX =
        Math.max(
            windowBounds.left,
            Math.min(windowBounds.right - this.mWindowWidth, this.mWindowCoords.f85x));
    int windowCoordsY =
        Math.max(
            windowBounds.top,
            Math.min(windowBounds.bottom - this.mWindowHeight, this.mWindowCoords.f86y));
  ...
  static class InternalPopupWindow {
    ...
    InternalPopupWindow(
      ...
      BLASTBufferQueue bLASTBufferQueue =
          new BLASTBufferQueue("magnifier surface", build2, surfaceWidth, surfaceHeight, -3);
      this.mBBQ = bLASTBufferQueue;
      Surface createSurface = bLASTBufferQueue.createSurface();
      this.mSurface = createSurface;
      ThreadedRenderer.SimpleRenderer simpleRenderer =
          new ThreadedRenderer.SimpleRenderer(context, "magnifier renderer", createSurface);
      this.mRenderer = simpleRenderer;
      RenderNode createRenderNodeForBitmap =
          createRenderNodeForBitmap("magnifier content", elevation, cornerRadius);
      this.mBitmapRenderNode = createRenderNodeForBitmap;
      RenderNode createRenderNodeForOverlay =
          createRenderNodeForOverlay("magnifier overlay", cornerRadius);
      ...
      try {
        ...
        if (this.mCallback != null) {
          this.mCurrentContent =
              Bitmap.createBitmap(width, this.mContentHeight, Bitmap.Config.ARGB_8888);
          updateCurrentContentForTesting();
        }
        this.mHandler = handler;
        this.mMagnifierUpdater =
            new Runnable() { // from class:
                             // android.widget.Magnifier$InternalPopupWindow$$ExternalSyntheticLambda1
              @Override // java.lang.Runnable
              public final void run() {
    ...
    public void updateContentFactors(int contentHeight, float zoom) {
      ...
      if (i < contentHeight) {
        ...
        RecordingCanvas canvas =
            this.mRenderer.getRootNode().beginRecording(this.mContentWidth, contentHeight);
        try {
          canvas.enableZ();
    ...
    private RenderNode createRenderNodeForBitmap(String name, float elevation, float cornerRadius) {
      ...
      bitmapRenderNode.setLeftTopRightBottom(
          i, i2, this.mContentWidth + i, this.mContentHeight + i2);
      ...
      RecordingCanvas canvas =
          bitmapRenderNode.beginRecording(this.mContentWidth, this.mContentHeight);
      try {
    ...
    private RenderNode createRenderNodeForOverlay(String name, float cornerRadius) {
      ...
      int i = this.mOffsetX;
      int i2 = this.mOffsetY;
      overlayRenderNode.setLeftTopRightBottom(
          i, i2, this.mContentWidth + i, this.mContentHeight + i2);
      Outline outline = new Outline();
      outline.setRoundRect(0, 0, this.mContentWidth, this.mContentHeight, cornerRadius);
      outline.setAlpha(1.0f);
    ...
    private void setupOverlay() {
      drawOverlay();
      this.mOverlay.setCallback(
          new Drawable.Callback() { // from class: android.widget.Magnifier.InternalPopupWindow.1
    ...
    public void destroy() {
      ...
      new SurfaceControl.Transaction()
          ...
          .remove(this.mBbqSurfaceControl)
          .apply();
    ...
    */
    public void doDraw() {
      ...
      synchronized (this.mLock) {
        ...
        RecordingCanvas canvas =
            this.mBitmapRenderNode.beginRecording(this.mContentWidth, this.mContentHeight);
        try {
          ...
          if (this.mIsFishEyeStyle) {
            try {
              int margin =
                  (int) ((this.mContentWidth - ((r3 - (this.mRamp * 2)) / this.mZoom)) / 2.0f);
              Rect srcRect = new Rect(margin, 0, w - margin, h);
              int i = this.mRamp;
              Rect dstRect = new Rect(i, 0, this.mContentWidth - i, this.mContentHeight);
              canvas.drawBitmap(this.mBitmap, srcRect, dstRect, paint);
              canvas.drawBitmapMesh(
                  Bitmap.createBitmap(this.mBitmap, 0, 0, margin, h),
                  this.mMeshWidth,
                  this.mMeshHeight,
                  this.mMeshLeft,
                  0,
                  ...
                  paint);
              canvas.drawBitmapMesh(
                  Bitmap.createBitmap(this.mBitmap, w - margin, 0, margin, h),
                  this.mMeshWidth,
                  this.mMeshHeight,
                  this.mMeshRight,
                  0,
          ...
          z = false;
          HardwareRenderer.FrameDrawingCallback callback2 =
              new HardwareRenderer
                  .FrameDrawingCallback() { // from class:
                                            // android.widget.Magnifier$InternalPopupWindow$$ExternalSyntheticLambda0
                @Override // android.graphics.HardwareRenderer.FrameDrawingCallback
                public final void onFrameDraw(long j) {
                  Magnifier.InternalPopupWindow.this.lambda$doDraw$0(
                      updateWindowPosition, pendingX, pendingY, firstDraw, j);
                }
              };
          if (!this.mIsFishEyeStyle) {
    ...
    public /* synthetic */ void lambda$doDraw$0(
        boolean updateWindowPosition, int pendingX, int pendingY, boolean firstDraw, long frame) {
      if (!this.mSurface.isValid()) {
        return;
      }
      if (updateWindowPosition) {
  ...
  public static final class Builder {
    ...
    private void applyDefaults() {
      ...
      this.mWidth = resources.getDimensionPixelSize(C4337R.dimen.default_magnifier_width);
      this.mHeight = resources.getDimensionPixelSize(C4337R.dimen.default_magnifier_height);
      this.mElevation = resources.getDimension(C4337R.dimen.default_magnifier_elevation);
      this.mCornerRadius = resources.getDimension(C4337R.dimen.default_magnifier_corner_radius);
      this.mZoom = resources.getFloat(C4337R.dimen.default_magnifier_zoom);
      this.mHorizontalDefaultSourceToMagnifierOffset =
          resources.getDimensionPixelSize(C4337R.dimen.default_magnifier_horizontal_offset);
      this.mVerticalDefaultSourceToMagnifierOffset =
          resources.getDimensionPixelSize(C4337R.dimen.default_magnifier_vertical_offset);
      this.mOverlay =
          new ColorDrawable(resources.getColor(C4337R.color.default_magnifier_color_overlay, null));
  ...
  public static PointF getMagnifierDefaultSize() {
    ...
    size.f87x = resources.getDimension(C4337R.dimen.default_magnifier_width) / density;
    size.f88y = resources.getDimension(C4337R.dimen.default_magnifier_height) / density;
    return size;
  }
}
