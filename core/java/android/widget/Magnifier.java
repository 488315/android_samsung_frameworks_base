...
public final class Magnifier {
    ...
    public void show(float sourceCenterX, float sourceCenterY, float magnifierCenterX, float magnifierCenterY) {
        ...
        if (this.mIsFishEyeStyle) {
            float magnifierCenterX5 = this.mClampedCenterZoomCoords.f85x - this.mViewCoordinatesInSurface[0];
        ...
        if (sourceCenterX != this.mPrevShowSourceCoords.f87x || sourceCenterY != this.mPrevShowSourceCoords.f88y || this.mDirtyState) {
        ...
        } else if (magnifierCenterX2 == this.mPrevShowWindowCoords.f87x && magnifierCenterY2 == this.mPrevShowWindowCoords.f88y) {
            magnifierCenterX3 = magnifierCenterX2;
            magnifierCenterY3 = magnifierCenterY2;
        } else {
    ...
    public void update() {
        if (this.mWindow != null) {
            obtainSurfaces();
            if (!this.mDirtyState) {
                performPixelCopy(this.mPrevStartCoordsInSurface.f85x, this.mPrevStartCoordsInSurface.f86y, false);
            } else {
                show(this.mPrevShowSourceCoords.f87x, this.mPrevShowSourceCoords.f88y, this.mPrevShowWindowCoords.f87x, this.mPrevShowWindowCoords.f88y);
            }
        }
    }
    ...
    private void obtainContentCoordinates(float xPosInView, float yPosInView) {
        ...
        if (resolvedLeft2 < 0 || resolvedTop2 < 0) {
            Log.m96e(TAG, "Magnifier's content is copied from a surface smaller thanthe content requested size. The magnifier will be dismissed.");
        }
        ...
        point2.f86y = Math.max((i6 / 2) + resolvedTop2, Math.min(zoomCenterY, resolvedBottom2 - (i6 / 2)));
    }
    ...
    private Point getCurrentClampedWindowCoordinates() {
        ...
        int windowCoordsX = Math.max(windowBounds.left, Math.min(windowBounds.right - this.mWindowWidth, this.mWindowCoords.f85x));
        int windowCoordsY = Math.max(windowBounds.top, Math.min(windowBounds.bottom - this.mWindowHeight, this.mWindowCoords.f86y));
        ...
        public void doDraw() {
            ...
            synchronized (this.mLock) {
                ...
                try {
                    ...
                    if (this.mIsFishEyeStyle) {
                        try {
                            int margin = (int) ((this.mContentWidth - ((r3 - (this.mRamp * 2)) / this.mZoom)) / 2.0f);
    ...
    public static final class Builder {
        ...
        private void applyDefaults() {
            ...
            this.mHorizontalDefaultSourceToMagnifierOffset = resources.getDimensionPixelSize(C4337R.dimen.default_magnifier_horizontal_offset);
            this.mVerticalDefaultSourceToMagnifierOffset = resources.getDimensionPixelSize(C4337R.dimen.default_magnifier_vertical_offset);
            this.mOverlay = new ColorDrawable(resources.getColor(C4337R.color.default_magnifier_color_overlay, null));
