...
public class RadioGroup extends LinearLayout {
  ...
  public RadioGroup(Context context, AttributeSet attrs) {
    ...
    TypedArray attributes =
        context.obtainStyledAttributes(attrs, C4337R.styleable.RadioGroup, 16842878, 0);
    saveAttributeDataForStyleable(
        context, C4337R.styleable.RadioGroup, attrs, attributes, 16842878, 0);
  ...
  private class PassThroughHierarchyChangeListener implements ViewGroup.OnHierarchyChangeListener {
    ...
    public void onChildViewAdded(View parent, View child) {
      if (parent == RadioGroup.this && (child instanceof RadioButton)) {
        ...
        ((RadioButton) child)
            .setOnCheckedChangeWidgetListener(RadioGroup.this.mChildOnCheckedChangeListener);
      }
      ViewGroup.OnHierarchyChangeListener onHierarchyChangeListener =
          this.mOnHierarchyChangeListener;
  ...
  public void autofill(AutofillValue value) {
    if (isEnabled()) {
      if (!value.isList()) {
        Log.m102w(LOG_TAG, value + " could not be autofilled into " + this);
        return;
      }
      int index = value.getListValue();
      View child = getChildAt(index);
      if (child == null) {
        Log.m102w("View", "RadioGroup.autoFill(): no child with index " + index);
      } else {
        check(child.getId());
      }
    }
  ...
  public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
    ...
    if (getOrientation() == 0) {
      info.setCollectionInfo(
          AccessibilityNodeInfo.CollectionInfo.obtain(1, getVisibleChildWithTextCount(), false, 1));
    } else {
      info.setCollectionInfo(
          AccessibilityNodeInfo.CollectionInfo.obtain(getVisibleChildWithTextCount(), 1, false, 1));
    }
  }
