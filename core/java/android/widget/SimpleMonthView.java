...
class SimpleMonthView extends View {
  ...
  public SimpleMonthView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    this.mDesiredMonthHeight = res.getDimensionPixelSize(C4337R.dimen.date_picker_month_height);
    this.mDesiredDayOfWeekHeight =
        res.getDimensionPixelSize(C4337R.dimen.date_picker_day_of_week_height);
    this.mDesiredDayHeight = res.getDimensionPixelSize(C4337R.dimen.date_picker_day_height);
    this.mDesiredCellWidth = res.getDimensionPixelSize(C4337R.dimen.date_picker_day_width);
    this.mDesiredDaySelectorRadius =
        res.getDimensionPixelSize(C4337R.dimen.date_picker_day_selector_radius);
  ...
  private ColorStateList applyTextAppearance(Paint p, int resId) {
    TypedArray ta =
        this.mContext.obtainStyledAttributes(null, C4337R.styleable.TextAppearance, 0, resId);
  ...
  private void initPaints(Resources res) {
    String monthTypeface = res.getString(C4337R.string.date_picker_month_typeface);
    String dayOfWeekTypeface = res.getString(C4337R.string.date_picker_day_of_week_typeface);
    String dayTypeface = res.getString(C4337R.string.date_picker_day_typeface);
    int monthTextSize = res.getDimensionPixelSize(C4337R.dimen.date_picker_month_text_size);
    int dayOfWeekTextSize =
        res.getDimensionPixelSize(C4337R.dimen.date_picker_day_of_week_text_size);
    int dayTextSize = res.getDimensionPixelSize(C4337R.dimen.date_picker_day_text_size);
  ...
  */
  ...
  private boolean moveOneDay(boolean positive) {
    ...
    if (positive) {
      if (isLastDayOfWeek(this.mHighlightedDay)
          || (i2 = this.mHighlightedDay) >= this.mDaysInMonth) {
        return false;
      }
      ...
      if (isDayActivated) {
        ...
        if (isDayHighlighted) {
          ...
          if (isDayEnabled) {
            ...
            canvas.drawCircle(
                colCenterRtl, rowCenter, this.mDaySelectorRadius, this.mDayHighlightPaint);
          } else {
            stateMask = stateMask5;
          }
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    int preferredHeight =
        (this.mDesiredDayHeight * 6)
            + this.mDesiredDayOfWeekHeight
            + this.mDesiredMonthHeight
            + getPaddingTop()
            + getPaddingBottom();
  ...
  public int getDayAtLocation(int x, int y) {
    ...
    if (paddedX < 0
        || paddedX >= this.mPaddedWidth
        || (paddedY = y - getPaddingTop())
            < (headerHeight = this.mMonthHeight + this.mDayOfWeekHeight)
        || paddedY >= this.mPaddedHeight) {
      return -1;
    }
    if (isLayoutRtl()) {
