...
public class ScrollBarDrawable extends Drawable implements Drawable.Callback {
  ...
  public ScrollBarDrawable(View parent) {
    ...
    TypedArray ta = parent.getContext().obtainStyledAttributes(C4337R.styleable.Theme);
    ta.recycle();
  }
  ...
  public void draw(Canvas canvas) {
    ...
    if (drawThumb) {
      ...
      int thumbOffset =
          ScrollBarUtils.getThumbOffset(scrollBarLength, thumbLength, extent, range, this.mOffset);
  ...
  public boolean isStateful() {
    ...
    return (drawable4 != null && drawable4.isStateful())
        || ((drawable = this.mVerticalThumb) != null && drawable.isStateful())
        || (((drawable2 = this.mHorizontalTrack) != null && drawable2.isStateful())
            || (((drawable3 = this.mHorizontalThumb) != null && drawable3.isStateful())
                || super.isStateful()));
  }
  ...
  private void drawThumb(Canvas canvas, Rect bounds, int offset, int length, boolean vertical) {
    ...
    if (vertical) {
      if (this.mVerticalThumb != null) {
        ...
        if (changed) {
          thumb.setBounds(
              bounds.left, bounds.top + offset, bounds.right, bounds.top + offset + length);
    ...
    if (this.mHorizontalThumb != null) {
      ...
      if (changed) {
        thumb2.setBounds(
            bounds.left + offset, bounds.top, bounds.left + offset + length, bounds.bottom);
