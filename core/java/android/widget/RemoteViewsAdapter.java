      ...
      switch (msg.what) {
        case 1:
          ...
          try {
              ...
              if (adapter != null) {
                this.mBindRequested =
                    mgr.bindRemoteViewsService(
                        this.mContext, appWidgetId, intent, sd, InputDevice.SOURCE_HDMI);
              } else {
                Log.m96e(RemoteViewsAdapter.TAG, "bind: adapter was null");
              }
              return;
            }
            this.mBindRequested =
            ...
            Log.m96e(
                RemoteViewsAdapter.TAG, "Failed to bind remoteViewsService: " + e.getMessage());
            return;
          }
        case 2:
          enqueueDeferredUnbindServiceMessage();
    ...
    private boolean sendNotifyDataSetChange(boolean always) {
      if (!always) {
        try {
          ...
          Log.m96e(
              RemoteViewsAdapter.TAG, "Error in updateNotifyDataSetChanged(): " + e.getMessage());
          return false;
        }
      }
      ...
      TextView loadingTextView =
          (TextView)
              LayoutInflater.from(getContext())
                  .inflate(
                      C4337R.layout.remote_views_adapter_default_loading_view,
                      (ViewGroup) this,
                      false);
  ...
  static class FixedSizeRemoteViewsCache {
    ...
    public void insert(int position, RemoteViews v, long itemId, int[] visibleWindow) {
      ...
      while (getRemoteViewsBitmapMemoryUsage() >= 2097152
          && (trimIndex = getFarthestPositionFrom(pruneFromPosition, visibleWindow)) >= 0) {
  ...
  */
  public RemoteViewsAdapter(
    ...
    this.mServiceHandler =
        new RemoteServiceHandler(handlerThread.getLooper(), this, context.getApplicationContext());
    ...
    RemoteViewsCacheKey remoteViewsCacheKey =
        new RemoteViewsCacheKey(new Intent.FilterComparison(intent), intExtra);
    HashMap<RemoteViewsCacheKey, FixedSizeRemoteViewsCache> hashMap = sCachedRemoteViewsCaches;
    synchronized (hashMap) {
      FixedSizeRemoteViewsCache fixedSizeRemoteViewsCache = hashMap.get(remoteViewsCacheKey);
      Configuration configuration = context.getResources().getConfiguration();
      if (fixedSizeRemoteViewsCache != null
          && (fixedSizeRemoteViewsCache.mConfiguration.diff(configuration)
                  & CACHE_RESET_CONFIG_FLAGS)
              == 0) {
        FixedSizeRemoteViewsCache fixedSizeRemoteViewsCache2 = hashMap.get(remoteViewsCacheKey);
  ...
  public void saveRemoteViewsCache() {
    ...
    final RemoteViewsCacheKey key =
        new RemoteViewsCacheKey(new Intent.FilterComparison(this.mIntent), this.mAppWidgetId);
    HashMap<RemoteViewsCacheKey, FixedSizeRemoteViewsCache> hashMap = sCachedRemoteViewsCaches;
    synchronized (hashMap) {
  ...
  public void updateTemporaryMetaData(IRemoteViewsFactory factory) {
    ...
    try {
      ...
      Log.m96e(TAG, "Error in updateMetaData: " + e.getMessage());
    ...
    try {
      ...
      synchronized (this.mCache) {
        if (viewTypeInRange) {
          ...
          Log.m96e(
              TAG,
              "Error: widget's RemoteViewsFactory returns more view types than  indicated by"
                  + " getViewTypeCount() ");
        }
      }
    } catch (RemoteException | RuntimeException e) {
      Log.m96e(TAG, "Error in updateRemoteViews(" + position + "): " + e.getMessage());
    }
  }
