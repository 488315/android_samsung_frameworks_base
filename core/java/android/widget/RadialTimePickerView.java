...
public class RadialTimePickerView extends View {
  ...
  /* renamed from: AM */
  private static final int f603AM = 0;
  ...
  /* renamed from: PM */
  private static final int f604PM = 1;
  ...
  public RadialTimePickerView(
    ...
    super(context, attrs);
    this.HOURS_TO_MINUTES =
        new FloatProperty<RadialTimePickerView>(
            "hoursToMinutes") { // from class: android.widget.RadialTimePickerView.1
    ...
    Paint[] paintArr2 = {r1, r1, r1};
    this.mPaintSelector = paintArr2;
    Paint paint2 = new Paint();
    this.mPaintBackground = paint2;
    this.mTextColor = new ColorStateList[3];
    this.mTextSize =
        new int[] {
          res.getDimensionPixelSize(C4337R.dimen.timepicker_text_size_normal),
          res.getDimensionPixelSize(C4337R.dimen.timepicker_text_size_normal),
          res.getDimensionPixelSize(C4337R.dimen.timepicker_text_size_inner)
        };
    this.mTextInset =
        new int[] {
          res.getDimensionPixelSize(C4337R.dimen.timepicker_text_inset_normal),
          res.getDimensionPixelSize(C4337R.dimen.timepicker_text_inset_normal),
          res.getDimensionPixelSize(C4337R.dimen.timepicker_text_inset_inner)
        };
    ...
    this.mSelectorRadius = res.getDimensionPixelSize(C4337R.dimen.timepicker_selector_radius);
    this.mSelectorStroke = res.getDimensionPixelSize(C4337R.dimen.timepicker_selector_stroke);
    this.mSelectorDotRadius =
        res.getDimensionPixelSize(C4337R.dimen.timepicker_selector_dot_radius);
    this.mCenterDotRadius = res.getDimensionPixelSize(C4337R.dimen.timepicker_center_dot_radius);
  ...
  void applyAttributes(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    TypedArray a =
        getContext()
            .obtainStyledAttributes(attrs, C4337R.styleable.TimePicker, defStyleAttr, defStyleRes);
    saveAttributeDataForStyleable(
        context, C4337R.styleable.TimePicker, attrs, a, defStyleAttr, defStyleRes);
    ColorStateList numbersTextColor = a.getColorStateList(3);
    ColorStateList numbersInnerTextColor = a.getColorStateList(9);
    this.mTextColor[0] =
        numbersTextColor == null ? ColorStateList.valueOf(-65281) : numbersTextColor;
    this.mTextColor[2] =
        numbersInnerTextColor == null ? ColorStateList.valueOf(-65281) : numbersInnerTextColor;
    ...
    this.mPaintBackground.setColor(
        a.getColor(
            4,
            context.getColor(C4337R.color.timepicker_default_numbers_background_color_material)));
    a.recycle();
  }
  ...
  public void setCurrentItemShowing(int item, boolean animate) {
    switch (item) {
      ...
      case 1:
        ...
        break;
      default:
        Log.m96e(TAG, "ClockView does not support showing item " + item);
        break;
    }
  }
  ...
  private void drawHours(Canvas canvas, Path selectorPath, float alphaMod) {
    ...
    if (hoursAlpha > 0) {
      canvas.save(2);
      canvas.clipPath(selectorPath, Region.EnumC0836Op.DIFFERENCE);
      drawHoursClipped(canvas, hoursAlpha, false);
      canvas.restore();
      canvas.save(2);
      canvas.clipPath(selectorPath, Region.EnumC0836Op.INTERSECT);
  ...
  private void drawHoursClipped(Canvas canvas, int hoursAlpha, boolean showActivated) {
    ...
    drawTextElements(
        ...
        this.mTypeface,
        this.mTextColor[0],
        this.mOuterTextHours,
        this.mOuterTextX[0],
        this.mOuterTextY[0],
        this.mPaint[0],
        hoursAlpha,
        showActivated && !this.mIsOnInnerCircle,
        this.mSelectionDegrees[0],
        showActivated);
  ...
  private void drawSelector(Canvas canvas, Path selectorPath) {
    ...
    float selLength =
        this.mCircleRadius - MathUtils.lerp(hoursInset, minutesInset, this.mHoursToMinutes);
    double selAngleRad =
        Math.toRadians(MathUtils.lerpDeg(hoursAngleDeg, minutesAngleDeg2, this.mHoursToMinutes));
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    if (action == 2 || action == 1 || action == 0) {
      ...
      this.mChangedDuringTouch |=
          handleTouchInput(event.getX(), event.getY(), forceSelection, autoAdvance);
    }
    return true;
  }
  ...
  private boolean handleTouchInput(float x, float y, boolean forceSelection, boolean autoAdvance) {
    ...
    if (this.mShowHours) {
      ...
      valueChanged =
          (this.mIsOnInnerCircle == isOnInnerCircle && this.mSelectionDegrees[0] == snapDegrees2)
              ? false
              : true;
      this.mIsOnInnerCircle = isOnInnerCircle;
  ...
  private class RadialPickerTouchHelper extends ExploreByTouchHelper {
    ...
    protected boolean onPerformActionForVirtualView(
      ...
      if (action == 16) {
        ...
        if (type == 1) {
          int hour =
              RadialTimePickerView.this.mIs24HourMode
                  ? value
                  : hour12To24(value, RadialTimePickerView.this.mAmOrPm);
    ...
    private void getBoundsForVirtualView(int virtualViewId, Rect bounds) {
      ...
      if (type != 1) {
        if (type == 2) {
          centerRadius =
              RadialTimePickerView.this.mCircleRadius - RadialTimePickerView.this.mTextInset[1];
        ...
        if (innerCircle) {
          centerRadius =
              RadialTimePickerView.this.mCircleRadius - RadialTimePickerView.this.mTextInset[2];
          radius = RadialTimePickerView.this.mSelectorRadius;
        } else {
          centerRadius =
              RadialTimePickerView.this.mCircleRadius - RadialTimePickerView.this.mTextInset[0];
          radius = RadialTimePickerView.this.mSelectorRadius;
        }
      ...
      float xCenter =
          RadialTimePickerView.this.mXCenter + (((float) Math.sin(radians)) * centerRadius);
      float yCenter =
          RadialTimePickerView.this.mYCenter - (((float) Math.cos(radians)) * centerRadius);
      bounds.set(
          (int) (xCenter - radius),
          (int) (yCenter - radius),
          (int) (xCenter + radius),
    ...
    private boolean isVirtualViewSelected(int type, int value) {
      return type == 1
          ? RadialTimePickerView.this.getCurrentHour() == value
          : type == 2 && RadialTimePickerView.this.getCurrentMinute() == value;
    }
