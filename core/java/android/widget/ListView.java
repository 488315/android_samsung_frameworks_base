...
public class ListView extends AbsListView {
  ...
  public final class InspectionCompanion
      implements android.view.inspector.InspectionCompanion<ListView> {
  ...
  public void setDndListAnimator(SemDragAndDropListAnimator animator) {
    ...
    this.mDndListAnimator.setAutoScrollListener(
        new SemAbsDragAndDropAnimator
            .SemDragAutoScrollListener() { // from class: android.widget.ListView.1
  ...
  public ListView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(attrs, C4337R.styleable.ListView, defStyleAttr, defStyleRes);
    saveAttributeDataForStyleable(
        context, C4337R.styleable.ListView, attrs, a, defStyleAttr, defStyleRes);
  ...
  public void setAdapter(ListAdapter adapter) {
    ...
    if (this.mHeaderViewInfos.size() > 0 || this.mFooterViewInfos.size() > 0) {
      this.mAdapter =
          wrapHeaderListAdapterInternal(this.mHeaderViewInfos, this.mFooterViewInfos, adapter);
    } else {
  ...
  public boolean requestChildRectangleOnScreen(View child, Rect rect, boolean immediate) {
    ...
    if (showingBottomFadingEdge()
        && (this.mSelectedPosition < this.mItemCount - 1
    ...
    if (rect.bottom > listUnfadedBottom && rect.top > listUnfadedTop) {
      ...
      scrollYDelta =
          Math.min(
              rect.height() > height
                  ? 0 + (rect.top - listUnfadedTop)
                  : 0 + (rect.bottom - listUnfadedBottom),
              distanceToBottom);
    } else if (rect.top < listUnfadedTop && rect.bottom < listUnfadedBottom) {
      int scrollYDelta2 =
          rect.height() > height
              ? 0 - (listUnfadedBottom - rect.bottom)
              : 0 - (listUnfadedTop - rect.top);
  ...
  void fillGap(boolean down) {
    ...
    if (down) {
      ...
      int startOffset =
          count > 0 ? getChildAt(count - 1).getBottom() + this.mDividerHeight : paddingTop;
    ...
    int startOffset2 =
        count > 0 ? getChildAt(0).getTop() - this.mDividerHeight : getHeight() - paddingBottom;
  ...
  private int getBottomSelectionPixel(
      int childrenBottom, int fadingEdgeLength, int selectedPosition) {
  ...
  private View moveSelection(
      View oldSel, View newSel, int delta, int childrenTop, int childrenBottom) {
    View sel;
    int fadingEdgeLength = getVerticalFadingEdgeLength();
    int selectedPosition = this.mSelectedPosition;
    int topSelectionPixel = getTopSelectionPixel(childrenTop, fadingEdgeLength, selectedPosition);
    int bottomSelectionPixel =
        getBottomSelectionPixel(childrenTop, fadingEdgeLength, selectedPosition);
    if (delta > 0) {
      View oldSel2 =
          makeAndAddView(
              selectedPosition - 1, oldSel.getTop(), true, this.mListPadding.left, false);
      int dividerHeight = this.mDividerHeight;
      sel =
          makeAndAddView(
              selectedPosition,
              oldSel2.getBottom() + dividerHeight,
              true,
              this.mListPadding.left,
              true);
      if (sel.getBottom() > bottomSelectionPixel) {
        int spaceAbove = sel.getTop() - topSelectionPixel;
        int spaceBelow = sel.getBottom() - bottomSelectionPixel;
      ...
      sel =
          newSel != null
              ? makeAndAddView(
                  selectedPosition, newSel.getTop(), true, this.mListPadding.left, true)
              : makeAndAddView(
                  selectedPosition, oldSel.getTop(), false, this.mListPadding.left, true);
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (this.mItemCount > 0
        && ((widthMode == 0 || heightMode == 0)
            && (child = obtainView(0, this.mIsScrap)) != null)) {
      ...
      if (recycleOnMeasure()
          && this.mRecycler.shouldRecycleViewType(
              ((AbsListView.LayoutParams) child.getLayoutParams()).viewType)) {
    ...
    if (heightMode != 0) {
      ...
      heightSize =
          this.mListPadding.top
              + this.mListPadding.bottom
              + childHeight2
  ...
  private void measureScrapChild(View child, int position, int widthMeasureSpec, int heightHint) {
    ...
    int childWidthSpec =
        ViewGroup.getChildMeasureSpec(
            widthMeasureSpec, this.mListPadding.left + this.mListPadding.right, p.width);
  ...
  final int measureHeightOfChildren(
    ...
    for (int i = startPosition; i <= endPosition2; i++) {
      ...
      if (recyle
          && recycleBin.shouldRecycleViewType(
              ((AbsListView.LayoutParams) child.getLayoutParams()).viewType)) {
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
    ...
    if (semDragAndDropListAnimator != null
        && semDragAndDropListAnimator.onInterceptTouchEvent(ev)) {
      return true;
    }
    if (this.mSweepListAnimator != null
        && this.mSweepListAnimator.isSweepAnimatorEnabled()
        && this.mSweepListAnimator.onInterceptTouchEvent(ev)) {
      return true;
    }
  ...
  public boolean onTouchEvent(MotionEvent ev) {
    ...
    if (this.mSweepListAnimator != null
        && this.mSweepListAnimator.isSweepAnimatorEnabled()
        && this.mSweepListAnimator.onTouchEvent(ev)) {
      return true;
    }
  ...
  protected int getChildDrawingOrder(int childCount, int i) {
    SemDragAndDropListAnimator semDragAndDropListAnimator = this.mDndListAnimator;
    return semDragAndDropListAnimator != null
        ? semDragAndDropListAnimator.getChildDrawingOrder(childCount, i)
  ...
  */
  protected void layoutChildren() {
    ...
    boolean focusFirstItemTemporarily;
    View focusLayoutRestoreDirectChild2;
    AbsListView.RecycleBin recycleBin;
    View focusLayoutRestoreDirectChild3;
    int delta2;
    View sel;
    ...
    try {
      ...
      switch (this.mLayoutMode) {
        ...
        case 5:
          ...
          break;
        case 2:
          int index2 = this.mNextSelectedPosition - this.mFirstPosition;
          if (index2 >= 0 && index2 < childCount) {
            View newSel2 = getChildAt(index2);
            index = index2;
            delta = 0;
            oldSel = null;
            oldFirst = null;
        ...
        default:
          int index3 = this.mSelectedPosition - this.mFirstPosition;
          if (index3 >= 0 && index3 < childCount) {
            oldSel2 = getChildAt(index3);
          }
          ...
          int delta3 =
              this.mNextSelectedPosition >= 0
                  ...
                  : 0;
          View newSel3 = getChildAt(index3 + delta3);
          index = index3;
          delta = delta3;
          oldSel = oldSel2;
          oldFirst = oldFirst2;
      ...
      if (this.mItemCount != this.mAdapter.getCount()) {
        throw new IllegalStateException(
            "The content of the adapter has changed but ListView did not receive a notification."
                ...
                + " but only from the UI thread. Make sure your adapter calls"
                + " notifyDataSetChanged() when its content changes. [in ListView("
                + getId()
                + ", "
                + getClass()
                + ") with Adapter("
      ...
      if (viewRootImpl == null
        ...
        boolean mInsideViewAccFocused2 = focusHost != focusChild;
        if (focusChild != null) {
          if (!dataChanged
              || isDirectChildHeaderOrFooter(focusChild)
              || focusChild.hasTransientState()
              || this.mAdapterHasStableIds) {
            accessibilityFocusLayoutRestoreView2 = focusHost;
            accessibilityFocusLayoutRestoreNode2 =
                viewRootImpl.getAccessibilityFocusedVirtualView();
          }
          int accessibilityFocusPosition2 = getPositionForView(focusChild);
          ...
          mInsideViewAccFocused = mInsideViewAccFocused2;
        } else {
          accessibilityFocusLayoutRestoreNode = null;
          accessibilityFocusLayoutRestoreView = null;
          accessibilityFocusPosition = -1;
      ...
      View focusLayoutRestoreDirectChild4 = null;
      View focusLayoutRestoreView2 = null;
      boolean focusFirstItemTemporarily2 = false;
      View focusedChild = getFocusedChild();
      if (focusedChild != null) {
        if (!dataChanged
            || isDirectChildHeaderOrFooter(focusedChild)
            || focusedChild.hasTransientState()
            || this.mAdapterHasStableIds) {
          focusLayoutRestoreDirectChild4 = focusedChild;
          ...
          if (focusLayoutRestoreView2 != null) {
            ...
            if (this.mSelectedPosition == 0) {
              focusFirstItemTemporarily2 = true;
            }
          }
        }
        if (!hasFocus()) {
        ...
        focusLayoutRestoreDirectChild = focusLayoutRestoreDirectChild4;
        focusLayoutRestoreView = focusLayoutRestoreView2;
        focusFirstItemTemporarily = focusFirstItemTemporarily2;
      } else {
        focusLayoutRestoreDirectChild = null;
        focusLayoutRestoreView = null;
        focusFirstItemTemporarily = false;
      }
      ...
      if (dataChanged) {
        ...
        while (i < childCount) {
          ...
          i++;
          focusLayoutRestoreDirectChild = focusLayoutRestoreDirectChild;
        }
      ...
      switch (this.mLayoutMode) {
        case 1:
          ...
          focusLayoutRestoreDirectChild3 = focusLayoutRestoreDirectChild2;
          delta2 = accessibilityFocusPosition;
          this.mFirstPosition = 0;
        ...
        case 4:
          ...
          if (sel2 == null
              && (focusSelector = this.mFocusSelector) != null
              && (focusRunnable = focusSelector.setupFocusIfValid(selectedPosition)) != null) {
            post(focusRunnable);
          }
          if (this.mAppWidgetSnapScroll
              && this.mNeedLayoutSpecificDone
              && !this.mIsLayoutSpecificDone) {
        ...
        case 6:
          ...
          int i2 = delta;
          delta2 = accessibilityFocusPosition;
          sel = moveSelection(oldSel, newSel, i2, childrenTop, childrenBottom);
          break;
        default:
          recycleBin = recycleBin2;
          focusLayoutRestoreDirectChild3 = focusLayoutRestoreDirectChild2;
          ...
          if (childCount != 0) {
            if (this.mSelectedPosition >= 0 && this.mSelectedPosition < this.mItemCount) {
              sel =
                  fillSpecific(
                      this.mSelectedPosition, oldSel == null ? childrenTop : oldSel.getTop());
              break;
              ...
              sel =
                  fillSpecific(
                      this.mFirstPosition, oldFirst == null ? childrenTop : oldFirst.getTop());
              break;
            }
      ...
      if (sel != null) {
        if (this.mItemsCanFocus && hasFocus() && (!sel.hasFocus() || focusFirstItemTemporarily)) {
          boolean focusWasTaken =
              (sel == focusLayoutRestoreDirectChild3
                      && focusLayoutRestoreView != null
        ...
        if (this.mTouchMode != 1 && this.mTouchMode != 2) {
          ...
          if (!inTouchMode) {
            ...
            if (child != null) {
              ...
              && this.mSelectorPosition < this.mItemCount
      ...
      if (viewRootImpl != null) {
        ...
        if (newAccessibilityFocusedView == null) {
          if (accessibilityFocusLayoutRestoreView != null
            ...
            View restoreView =
                getChildAt(
                    MathUtils.constrain(delta2 - this.mFirstPosition, 0, getChildCount() - 1));
            if (restoreView != null) {
              restoreView.requestAccessibilityFocus();
            }
          }
        } else if (delta2 != -1) {
          int position = MathUtils.constrain(delta2 - this.mFirstPosition, 0, getChildCount() - 1);
          View restoreView2 =
              mInsideViewAccFocused
                  ? getChildAt(position).findViewById(newAccessibilityFocusedView.getId())
                  : getChildAt(position);
          if (newAccessibilityFocusedView.isAccessibilityFocused()
              && newAccessibilityFocusedView != restoreView2) {
      ...
      if ((this.mDataChanged || this.mSemAdapterChanged) && this.mSemEnableFillOut) {
        ...
        if (lastVisiblePos != this.mItemCount - 1 || lastVisiblePos < 0) {
          this.mSemFillOutEmptyArea = -1;
        } else {
          View lastView = getChildAt(getChildCount() - 1);
          this.mSemFillOutEmptyArea = lastView == null ? -1 : lastView.getBottom();
        }
      }
  ...
  private View makeAndAddView(
      int position, int y, boolean flow, int childrenLeft, boolean selected) {
    View activeView;
  ...
  private void setupChild(
    ...
    if ((isAttachedToWindow && !p.forceAdd) || (p.recycledHeaderFooter && p.viewType == -2)) {
      ...
      if (isAttachedToWindow
          && ((AbsListView.LayoutParams) view.getLayoutParams()).scrappedFromPosition != position) {
    ...
    if (needToMeasure) {
      int childWidthSpec =
          ViewGroup.getChildMeasureSpec(
              this.mWidthMeasureSpec, this.mListPadding.left + this.mListPadding.right, p.width);
      int lpHeight = p.height;
      if (lpHeight > 0) {
        childHeightSpec = View.MeasureSpec.makeMeasureSpec(lpHeight, 1073741824);
      } else {
  ...
  */
  private boolean commonKey(int keyCode, int count, KeyEvent event) {
    ...
    if (this.mAppWidgetInnerFocus && !KeyEvent.isConfirmKey(keyCode) && event.getAction() != 1) {
    ...
    if (KeyEvent.isConfirmKey(keyCode)
        ...
        && action != 1
        && !(handled2 = resurrectSelectionIfNeeded())
    ...
    if (this.mIsHoveredByMouse) {
      ...
      Log.m94d(TAG, "mIsHoveredByMouse false");
    }
    if (!handled2 && action != 1) {
      switch (keyCode) {
        case 19:
          if (event.hasNoModifiers() || event.hasModifiers(1)) {
            ...
            handled2 = resurrectSelectionIfNeeded() || fullScroll(33);
            count2 = count;
          }
          if (!handled2) {
            if (!sendToTextFilter(keyCode, count2, event)) {
              switch (action) {
              }
            }
          } else if (this.mAppWidgetInnerFocus && !KeyEvent.isConfirmKey(keyCode)) {
            this.mNextClickable = null;
            this.mClickableViewStates.clear();
            break;
          }
        ...
        case 20:
          if (event.hasNoModifiers() || event.hasModifiers(1)) {
            ...
            handled2 = resurrectSelectionIfNeeded() || fullScroll(130);
            count2 = count;
          }
          if (!handled2) {}
          break;
        case 21:
          if (event.hasNoModifiers() || event.hasModifiers(1)) {
            this.mSemCurrentFocusPosition = this.mSelectedPosition;
            handled2 = handleHorizontalFocusWithinListItem(17);
            count2 = count;
            if (!handled2) {}
          }
          break;
        case 22:
          if (event.hasNoModifiers() || event.hasModifiers(1)) {
            this.mSemCurrentFocusPosition = this.mSelectedPosition;
            handled2 = handleHorizontalFocusWithinListItem(66);
            if (!handled2) {
              handled2 = resurrectSelectionIfNeeded();
            }
            count2 = count;
          ...
          break;
        case 61:
          boolean needArrowScroll = false;
          if (this.mAppWidgetSnapScroll) {
            View deepestFocusedView = getDeepestFocusedChild();
            if (getPositionForView(deepestFocusedView) != this.mFirstPosition) {
              deepestFocusedView = this;
              clearFocus();
            }
            View viewFocusedNext =
                FocusFinder.getInstance().findNextFocus(this, deepestFocusedView, 2);
            int bottom = getHeight() - this.mListPadding.bottom;
            if (viewFocusedNext != null) {
              int[] rootLoc = new int[2];
              int[] childLoc = new int[2];
              getLocationOnScreen(rootLoc);
              viewFocusedNext.getLocationOnScreen(childLoc);
              int rootBottom = rootLoc[1] + bottom;
              int childBottom = childLoc[1] + viewFocusedNext.getHeight();
              View firstVisibleView = getChildAt(0);
              if (rootBottom < childBottom) {
                needArrowScroll = true;
                if (childLoc[1] > rootBottom) {
                  setSelectionFromTop(
                      this.mSelectedPosition, firstVisibleView.getTop() - childLoc[1]);
                }
              } else if (childLoc[1] < 0 && firstVisibleView != null) {
                int childRelativeTop2 = Math.abs(firstVisibleView.getTop() - childLoc[1]);
                if (childRelativeTop2 >= bottom / 3) {
                  childRelativeTop =
                      childRelativeTop2 - ((getHeight() - this.mListPadding.bottom) / 3);
                  handled2 = false;
                } else {
                  childRelativeTop = 0;
                  handled2 = true;
                }
                setSelectionFromTop(this.mSelectedPosition, -childRelativeTop);
                count2 = count;
                if (!handled2) {}
              }
            }
          ...
          if (this.mSelectZeroPositionOnKeyTab && getSelectedItemPosition() == getCount() - i) {
            if (!this.mAppWidgetInnerFocus) {
              i2 = 0;
            } else {
              i2 = 0;
            ...
            if (event.hasNoModifiers()) {
              handled2 = resurrectSelectionIfNeeded() || arrowScroll(130);
            } else if (event.hasModifiers(1)) {
              handled2 = resurrectSelectionIfNeeded() || arrowScroll(33);
            }
            handled2 = needArrowScroll ? false : handled2;
            count2 = count;
            if (!handled2) {}
          }
          break;
        case 92:
          if (event.hasNoModifiers()) {
            handled2 = resurrectSelectionIfNeeded() || pageScroll(33);
            count2 = count;
          } else if (event.hasModifiers(2)) {
            handled2 = resurrectSelectionIfNeeded() || fullScroll(33);
            count2 = count;
        ...
        case 93:
          if (event.hasNoModifiers()) {
            handled2 = resurrectSelectionIfNeeded() || pageScroll(130);
            count2 = count;
          } else if (event.hasModifiers(2)) {
            handled2 = resurrectSelectionIfNeeded() || fullScroll(130);
            count2 = count;
  ...
  boolean pageScroll(int direction) {
    ...
    if (nextPage < 0
        || (position = lookForSelectablePositionAfter(this.mSelectedPosition, nextPage, down))
            < 0) {
      return false;
    }
    this.mLayoutMode = 4;
  ...
  private boolean handleHorizontalFocusWithinListItem(int direction) {
    ...
    if (direction != 17 && direction != 66) {
      throw new IllegalArgumentException(
          "direction must be one of {View.FOCUS_LEFT, View.FOCUS_RIGHT}");
    }
    ...
    if (this.mItemsCanFocus
      ...
      View nextFocus =
          FocusFinder.getInstance()
              .findNextFocus((ViewGroup) selectedView, currentFocus, direction);
      if (nextFocus != null) {
      ...
      View globalNextFocus =
          FocusFinder.getInstance()
              .findNextFocus((ViewGroup) getRootView(), currentFocus, direction);
  ...
  private boolean arrowScrollImpl(int direction) {
    ...
    if (this.mAppWidgetSnapScroll) {
      ...
      if (direction == 33 && selectedPos > 0 && amountToScroll > 0) {
        if (selectedView.getHeight() > listBottom - this.mListPadding.top
            && selectedView.getTop() != 0) {
          ...
          && selectedPos < this.mAdapter.getCount() - 1
          && amountToScroll > 0
          && selectedView.getHeight() == listBottom - this.mListPadding.top) {
        nextSelectedPosition = selectedPos + 1;
    ...
    if (this.mItemsCanFocus
        ...
        && selectedView != null
        && selectedView.hasFocus()
        && (focused = selectedView.findFocus()) != null
  ...
  private void handleNewSelectionChange(
      View selectedView, int direction, int newSelectedPosition, boolean newFocusAssigned) {
    int topViewIndex;
    int bottomViewIndex;
    View topView;
    View bottomView;
  ...
  private int amountToScroll(int direction, int nextSelectedPosition) {
    ...
    if (nextSelectedPosition != -1
        && viewToMakeVisible2.getBottom() - goalTop >= getMaxScrollAmount()) {
      return 0;
    }
  ...
  private ArrowScrollFocusResult arrowScrollFocused(int direction) {
    ...
    if (selectedView == null || !selectedView.hasFocus()) {
      if (direction != 130) {
        ...
        int listTop =
            this.mListPadding.top + (topFadingEdgeShowing ? getArrowScrollPreviewLength() : 0);
        if (selectedView != null && selectedView.getTop() > listTop) {
    ...
    if (oldFocus != null) {
      ...
      if (this.mSelectedPosition != -1
          && positionOfNewFocus != this.mSelectedPosition
          && (selectablePosition = lookForSelectablePositionOnScreen(direction)) != -1
          && ((direction == 130 && selectablePosition < positionOfNewFocus)
              || (direction == 33 && selectablePosition > positionOfNewFocus))) {
        return null;
      }
  ...
  private void scrollListItemsBy(int amount) {
    ...
    if (amount < 0) {
      ...
      while (last.getBottom() < listBottom
          && (this.mFirstPosition + numChildren) - 1 < this.mItemCount - 1) {
  ...
  private View addViewBelow(View theView, int position) {
    ...
    setupChild(
        view, belowPosition, edgeOfNewChild, true, this.mListPadding.left, false, this.mIsScrap[0]);
    return view;
  }
  ...
  public boolean isOpaque() {
    boolean retValue =
        (this.mCachingActive
                && this.mIsCacheColorOpaque
                && this.mDividerIsOpaque
                && hasOpaqueScrollbars())
            || super.isOpaque();
    if (retValue) {
      ...
      int listBottom =
          getHeight()
              - (this.mListPadding != null ? this.mListPadding.bottom : this.mPaddingBottom);
  ...
  protected void dispatchDraw(Canvas canvas) {
            ...
            if (!drawDividers2 || scrollY >= listBottom2) {
              ...
              if (adapter2.isEnabled(itemIndex)) {
                if (headerDividers2 || (!isHeader && listBottom3 >= headerCount)) {
                  if (isLastItem
                      || (adapter2.isEnabled(listBottom3)
                          && (footerDividers || (!isFooter && listBottom3 < footerLimit)))) {
                    bounds.top = scrollY;
              ...
              if (top <= effectivePaddingTop5) {
                ...
                if (!adapter4.isEnabled(itemIndex2)) {
                  ...
                  headerDividers = headerDividers2;
                    ...
                    || (adapter4.isEnabled(start3)
                        && (footerDividers || (!isFooter2 && start3 < footerLimit)))) {
                  headerDividers = headerDividers2;
    ...
    if (this.mSemEnableFillOut && this.mSemFillOutEmptyArea != -1) {
      canvas.drawRect(
          0.0f, this.mSemFillOutEmptyArea, this.mRight, this.mBottom, this.mSemFillOutPaint);
    }
  ...
  protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
    SemDragAndDropListAnimator semDragAndDropListAnimator = this.mDndListAnimator;
    if (semDragAndDropListAnimator != null
        && !semDragAndDropListAnimator.preDrawChild(canvas, child, drawingTime)) {
      return false;
    }
  ...
  protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
    ...
    if (gainFocus && this.mDndListAnimator != null) {
      post(
          new Runnable() { // from class: android.widget.ListView.2
            @Override // java.lang.Runnable
            public void run() {
  ...
  View findViewByPredicateInHeadersOrFooters(
      ArrayList<FixedViewInfo> where, Predicate<View> predicate, View childToSkip) {
    View v;
    if (where != null) {
      int len = where.size();
      for (int i = 0; i < len; i++) {
        ...
        if (v2 != childToSkip
            && !v2.isRootNamespace()
            && (v = v2.findViewByPredicate(predicate)) != null) {
          return v;
        }
      }
      return null;
  ...
  public void onInitializeAccessibilityNodeInfoInternal(AccessibilityNodeInfo info) {
    ...
    AccessibilityNodeInfo.CollectionInfo collectionInfo =
        AccessibilityNodeInfo.CollectionInfo.obtain(-1, -1, false, selectionMode);
  ...
  public void onInitializeAccessibilityNodeInfoForItem(
    ...
    AccessibilityNodeInfo.CollectionItemInfo itemInfo =
        AccessibilityNodeInfo.CollectionItemInfo.obtain(position, 1, 0, 1, isHeading, isSelected);
  ...
  protected void wrapHeaderListAdapterInternal() {
    this.mAdapter =
        wrapHeaderListAdapterInternal(this.mHeaderViewInfos, this.mFooterViewInfos, this.mAdapter);
  }
  ...
  public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
    ...
    if (this.mAppWidgetSnapScroll
        && direction == 2
        && (firstVisibleView = getChildAt(this.mSelectedPosition - this.mFirstPosition)) != null) {
      ...
      if (cententH == childH) {
        ...
        if (getChildAt(1) != null) {
          getViewsToRemove(
              viewsToRemove,
              (ViewGroup) getChildAt(this.mSelectedPosition - this.mFirstPosition == 1 ? 0 : 1));
        }
    ...
    if (this.mClickableViewStates.isEmpty() && (view instanceof ViewGroup)) {
      ...
      for (int i = 0; i < parent.getChildCount(); i++) {
        ...
        if (child != null
          ...
          this.mClickableViewStates.put(
              Integer.valueOf(this.mClickableViewStates.size()),
              new AbsListView.ClickableViewState(child, false));
        }
        addClickables(child);
      }
    }
  ...
  private boolean consumeClickables() {
    ...
    for (Map.Entry<Integer, AbsListView.ClickableViewState> entry :
        this.mClickableViewStates.entrySet()) {
  ...
  private final int nextFocusedPositionForDirection(
      View focusedView, int selectedPos, int direction) {
    int i;
    int nextFocused;
    int i2;
    if (direction == 130) {
  ...
  private boolean pointerScroll(int direction) {
    ...
    if (amountToScroll > 0) {
      ...
      if (direction == 33
          && focusedView.getHeight() > listBottom - this.mListPadding.top
          && focusedView.getTop() != 0) {
        handled = false;
