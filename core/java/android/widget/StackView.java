...
public class StackView extends AdapterViewAnimator {
  ...
  public StackView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(
            attrs, C4337R.styleable.StackView, defStyleAttr, defStyleRes);
    saveAttributeDataForStyleable(
        context, C4337R.styleable.StackView, attrs, a, defStyleAttr, defStyleRes);
    ...
    if (fromIndex == -1 && toIndex == getNumActiveViews() - 1) {
      ...
      if (animate) {
        ...
        ObjectAnimator slideOut =
            ObjectAnimator.ofPropertyValuesHolder(animationSlider2, slideOutX, slideOutY);
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
    ...
    switch (action & 255) {
      ...
      case 2:
        ...
        if (pointerIndex == -1) {
          Log.m94d("StackView", "Error: No data for our primary pointer.");
          return false;
        }
  ...
  private void beginGestureIfNeeded(float deltaY) {
    ...
    if (((int) Math.abs(deltaY)) > this.mTouchSlop && this.mSwipeGestureType == 0) {
      ...
      boolean endOfStack =
          this.mLoopViews
              && adapterCount == 1
              && (((i2 = this.mStackMode) == 0 && swipeGestureType == 1)
                  || (i2 == 1 && swipeGestureType == 2));
      boolean beginningOfStack =
          this.mLoopViews
              && adapterCount == 1
              && (((i = this.mStackMode) == 1 && swipeGestureType == 1)
                  || (i == 0 && swipeGestureType == 2));
  ...
  private void handlePointerUp(MotionEvent ev) {
    ...
    if (deltaY > this.mSwipeThreshold
      ...
      if (i == 1) {
        ...
        ObjectAnimator pa =
            ObjectAnimator.ofPropertyValuesHolder(animationSlider, snapBackX, snapBackY);
        ...
        ObjectAnimator pa2 =
            ObjectAnimator.ofPropertyValuesHolder(animationSlider2, snapBackX2, snapBackY2);
  ...
  private class StackSlider {
    ...
    private float getDuration(boolean invert, float velocity) {
      ...
      float maxd =
          (float) Math.hypot(StackView.this.mSlideAmount, StackView.this.mSlideAmount * 0.4f);
      if (d > maxd) {
        d = maxd;
      }
      if (velocity == 0.0f) {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    boolean haveChildRefSize =
        (this.mReferenceChildWidth == -1 || this.mReferenceChildHeight == -1) ? false : true;
    if (heightSpecMode == 0) {
      if (haveChildRefSize) {
        i =
