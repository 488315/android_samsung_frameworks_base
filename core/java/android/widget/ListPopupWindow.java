...
public class ListPopupWindow implements ShowableListMenu {
  ...
  public ListPopupWindow(Context context, AttributeSet attributeSet, int i, int i2) {
    ...
    context.getTheme().resolveAttribute(C4337R.attr.parentIsDeviceDefault, typedValue, false);
  ...
  public void setWidth(int width) {
    if (this.mIsDexMode && width < -2) {
      this.mDropDownWidth = -2;
      Log.m94d(TAG, "An abnormal value has been passed. width: " + width);
    } else {
    ...
    if (height < 0 && -2 != height && -1 != height) {
      if (this.mContext.getApplicationInfo().targetSdkVersion < 26) {
        Log.m96e(
            ...
            "Negative value "
                + height
                + " passed to ListPopupWindow#setHeight produces undefined results");
      } else {
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    if (isShowing()
        && keyCode != 62
        && (this.mDropDownList.getSelectedItemPosition() >= 0 || !KeyEvent.isConfirmKey(keyCode))) {
      ...
      if (adapter != null) {
        ...
        lastItem =
            allEnabled
                ? adapter.getCount() - 1
                : this.mDropDownList.lookForSelectablePosition(adapter.getCount() - 1, false);
      }
  ...
  private int buildDropDown() {
    ...
    if (this.mDropDownList == null) {
      ...
      this.mDropDownList.setOnItemSelectedListener(
          new AdapterView.OnItemSelectedListener() { // from class: android.widget.ListPopupWindow.3
      ...
      if (hintView != null) {
        ...
        switch (this.mPromptPosition) {
          ...
          case 1:
            ...
            break;
          default:
            Log.m96e(TAG, "Invalid hint position " + this.mPromptPosition);
            break;
        }
        ...
        otherHeights =
            hintView.getMeasuredHeight() + hintParams2.topMargin + hintParams2.bottomMargin;
      }
    ...
    switch (i) {
      ...
      default:
        childWidthSpec =
            View.MeasureSpec.makeMeasureSpec(
                i - (this.mTempRect.left + this.mTempRect.right), 1073741824);
        break;
    }
    int listContent =
  ...
  private void setBlurEffect() {
    ...
    if (context != null && this.mIsDeviceDefault) {
      ...
      if (!isThemeApplied && !isReduceTransparency && this.mPopup.semIsAvailableBlurBackground()) {
        ...
        this.mContext
            .getTheme()
            .resolveAttribute(C4337R.attr.parentIsDeviceDefaultDark, colorValue, true);
        if (colorValue.data != 0) {
          blurBackgroundColorId = C4337R.color.sem_popup_menu_blur_background_dark;
        } else {
          blurBackgroundColorId = C4337R.color.sem_popup_menu_blur_background;
        }
        builder.setRadius(120);
        builder.setBackgroundColor(
            this.mContext.getResources().getColor(blurBackgroundColorId, this.mContext.getTheme()));
        builder.setBackgroundCornerRadius(
            this.mContext
                .getResources()
                .getDimensionPixelSize(C4337R.dimen.sem_popup_menu_corner_radius));
