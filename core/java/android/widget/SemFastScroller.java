...
class SemFastScroller {
    ...
    public SemFastScroller(AbsListView listView, int styleResId) {
        ...
        this.mPreviewMarginEnd = this.mContext.getResources().getDimensionPixelOffset(C4337R.dimen.fastscroll_preview_margin_end);
        this.mThumbMarginEnd = this.mContext.getResources().getDimensionPixelOffset(C4337R.dimen.fastscroll_thumb_margin_end);
        this.mAdditionalTouchArea = this.mContext.getResources().getDimension(C4337R.dimen.tw_fluid_scroller_additional_touch_area);
        this.mTrackPadding = this.mContext.getResources().getDimensionPixelOffset(C4337R.dimen.sem_fast_scroller_track_padding);
        this.mAdditionalBottomPadding = this.mContext.getResources().getDimensionPixelOffset(C4337R.dimen.sem_fast_scroller_additional_bottom_padding);
    ...
    private void updateAppearance() {
        ...
        this.mThumbBackgroundColor = this.mUseOpenThemeResources ? this.mContext.getResources().getColor(C4337R.color.sem_fast_scrollbar_bg_color) : this.mContext.getResources().getColor(C4337R.color.sem_fast_scrollbar_bg_color_app_widget);
    ...
    public void setStyle(int resId) {
        TypedArray ta = this.mContext.obtainStyledAttributes(null, C4337R.styleable.FastScroll, 16843767, resId);
    ...
    public void setEnabled(boolean enabled) {
        Log.m94d(TAG, "setEnabled() enabled = " + enabled);
    ...
    private void transitionToHidden() {
        Log.m94d(TAG, "transitionToHidden() mState = " + this.mState);
        if (this.mState != 2) {
    ...
    private void transitionToVisible() {
        Log.m94d(TAG, "transitionToVisible()");
        AnimatorSet animatorSet = this.mDecorAnimation;
        if (animatorSet != null) {
            animatorSet.cancel();
        }
    ...
    private void transitionToDragging() {
        Log.m94d(TAG, "transitionToDragging()");
        AnimatorSet animatorSet = this.mDecorAnimation;
        if (animatorSet != null) {
            animatorSet.cancel();
        }
    ...
    private void scrollTo(float position) {
        ...
        Log.m94d(TAG, "scrollTo() called transitionPreviewLayout() sectionIndex =" + sectionIndex + ", position = " + position);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            case 0:
                Log.m94d(TAG, "onInterceptTouchEvent() ACTION_DOWN ev.getY() = " + ev.getY());
            ...
            case 2:
                if (!isPointInside(ev.getX(), ev.getY())) {
                    ...
                    if (j >= 0 && j <= SystemClock.uptimeMillis()) {
                        ...
                        scrollTo(pos);
                        Log.m94d(TAG, "onInterceptTouchEvent() ACTION_MOVE pendingdrag open()");
        ...
        switch (me.getActionMasked()) {
            case 0:
                if (isPointInside(me.getX(), me.getY()) && !this.mList.isInScrollingContainer()) {
                    beginDrag();
                    this.mEffectState = 1;
                    Log.m94d(TAG, "onTouchEvent() ACTION_DOWN.. open() called with posY " + me.getY());
                    return true;
                }
                return false;
            case 1:
                if (this.mPendingDrag >= 0) {
                    ...
                    scrollTo(pos);
                    this.mEffectState = 1;
                    Log.m94d(TAG, "onTouchEvent() ACTION_UP.. open() called with posY " + me.getY());
                }
                if (this.mState == 2) {
                ...
                return false;
            case 2:
                Log.m94d(TAG, "onTouchEvent() ACTION_MOVE.. mState= " + this.mState + ", mInitialTouchY=" + this.mInitialTouchY);
                if (this.mPendingDrag >= 0 && Math.abs(me.getY() - this.mInitialTouchY) > this.mScaledTouchSlop) {
                    beginDrag();
                    float f = this.mScrollY;
                    if (f > top && f < bottom) {
                        Log.m94d(TAG, "onTouchEvent() ACTION_MOVE 1 mScrollY=" + this.mScrollY + ", min=" + min + ", max=" + max);
                ...
                if (this.mState == 2) {
                    ...
                    if (f3 > top && f3 < bottom) {
                        Log.m94d(TAG, "onTouchEvent() ACTION_MOVE 2 mScrollY=" + this.mScrollY + ", min=" + min + ", max=" + max);
