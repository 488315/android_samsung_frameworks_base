...
public class OverScroller {
    ...
    */
    public void fling(int startX, int startY, int velocityX, int velocityY, int minX, int maxX, int minY, int maxY, int overX, int overY, boolean isSkipMove, float frameLatency) {
        int velocityY2;
        int velocityX2 = velocityX;
        if (this.mFlywheel && !isFinished()) {
            float oldVelocityX = this.mScrollerX.mCurrVelocity;
            ...
            if (Math.signum(velocityX2) == Math.signum(oldVelocityX) && Math.signum(velocityY) == Math.signum(oldVelocityY)) {
                ...
                velocityY2 = (int) (velocityY + oldVelocityY);
                if (!isSkipMove) {
                    sIntervalTime = (long) (frameLatency < 0.0f ? 0.0f : frameLatency > FRAME_LATENCY_LIMIT ? FRAME_LATENCY_LIMIT : frameLatency);
                } else {
                    sIntervalTime = 0L;
                }
                this.mMode = 1;
