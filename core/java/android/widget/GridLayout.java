...
public class GridLayout extends ViewGroup {
  ...
  static final Alignment UNDEFINED_ALIGNMENT =
      new Alignment() { // from class: android.widget.GridLayout.2
  ...
  public final class InspectionCompanion
      implements android.view.inspector.InspectionCompanion<GridLayout> {
    ...
    public void mapProperties(PropertyMapper propertyMapper) {
      ...
      this.mAlignmentModeId =
          propertyMapper.mapIntEnum(
              "alignmentMode",
              16843642,
              new View$InspectionCompanion$$ExternalSyntheticLambda0(alignmentModeEnumMapping));
  ...
  static {
    ...
    CENTER =
        new Alignment() { // from class: android.widget.GridLayout.6
          @Override // android.widget.GridLayout.Alignment
          int getGravityOffset(View view, int cellDelta) {
            return cellDelta >> 1;
          }
    ...
    BASELINE =
        new Alignment() { // from class: android.widget.GridLayout.7
          @Override // android.widget.GridLayout.Alignment
          int getGravityOffset(View view, int cellDelta) {
            return 0;
          }
    ...
    FILL =
        new Alignment() { // from class: android.widget.GridLayout.8
          @Override // android.widget.GridLayout.Alignment
          int getGravityOffset(View view, int cellDelta) {
            return 0;
          }
  ...
  public GridLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    this.mDefaultGap = context.getResources().getDimensionPixelOffset(C4337R.dimen.default_gap);
    TypedArray a =
        context.obtainStyledAttributes(
            attrs, C4337R.styleable.GridLayout, defStyleAttr, defStyleRes);
    saveAttributeDataForStyleable(
        context, C4337R.styleable.GridLayout, attrs, a, defStyleAttr, defStyleRes);
    try {
  ...
  static <T> T[] append(T[] tArr, T[] tArr2) {
    T[] tArr3 =
        (T[])
            ((Object[])
                Array.newInstance(tArr.getClass().getComponentType(), tArr.length + tArr2.length));
  ...
  private void checkLayoutParams(LayoutParams lp, boolean horizontal) {
    ...
    if (count != Integer.MIN_VALUE) {
      if (span.max > count) {
        handleInvalidParams(
            groupName + " indices (start + span) mustn't exceed the " + groupName + " count");
      }
      if (span.size() > count) {
  ...
  private void consistencyCheck() {
    ...
    if (i == 0) {
      ...
      this.mPrinter.println(
          "The fields of some layout parameters were modified in between layout operations. Check"
              + " the javadoc for GridLayout.LayoutParams#rowSpec.");
      invalidateStructure();
      consistencyCheck();
    }
  }

  private void measureChildWithMargins2(
      View child, int parentWidthSpec, int parentHeightSpec, int childWidth, int childHeight) {
    int childWidthSpec =
        getChildMeasureSpec(parentWidthSpec, getTotalMargin(child, true), childWidth);
    int childHeightSpec =
        getChildMeasureSpec(parentHeightSpec, getTotalMargin(child, false), childHeight);
  ...
  static int adjust(int measureSpec, int delta) {
    return View.MeasureSpec.makeMeasureSpec(
        View.MeasureSpec.getSize(measureSpec + delta), View.MeasureSpec.getMode(measureSpec));
  }
  ...
  final class Axis {
    ...
    public void setCount(int count) {
      if (count != Integer.MIN_VALUE && count < getMaxIndex()) {
        GridLayout.handleInvalidParams(
            (this.horizontal ? "column" : "row")
                + "Count must be greater than or equal to the maximum of all grid indices (and"
                + " spans) defined in the LayoutParams of each child");
      }
    ...
    private PackedMap<Spec, Bounds> createGroupBounds() {
      Assoc<Spec, Bounds> assoc = Assoc.m170of(Spec.class, Bounds.class);
      int N = GridLayout.this.getChildCount();
      for (int i = 0; i < N; i++) {
        View c = GridLayout.this.getChildAt(i);
    ...
    private void computeGroupBounds() {
      ...
      for (int i = 0; i < N; i++) {
        ...
        Spec spec = this.horizontal ? lp.columnSpec : lp.rowSpec;
        int size =
            GridLayout.this.getMeasurementIncludingMargin(c, this.horizontal)
                + (spec.weight == 0.0f ? 0 : getDeltas()[i]);
    ...
    private PackedMap<Interval, MutableInt> createLinks(boolean min) {
      Assoc<Interval, MutableInt> result = Assoc.m170of(Interval.class, MutableInt.class);
    ...
    private void include(
        List<Arc> arcs, Interval key, MutableInt size, boolean ignoreIfAlreadyPresent) {
      if (key.size() == 0) {
        return;
      }
    ...
    private String arcsToString(List<Arc> arcs) {
      ...
      for (Arc arc : arcs) {
        ...
        result.append(
            src < dst
                ? var + dst + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + var + src + ">=" + value
                : var + src + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + var + dst + "<=" + (-value));
      }
    ...
    private void logError(String axisName, Arc[] arcs, boolean[] culprits0) {
      ...
      GridLayout.this.mPrinter.println(
          axisName
              + " constraints: "
              + arcsToString(culprits)
    ...
    private void computeMargins(boolean leading) {
      ...
      for (int i = 0; i < N; i++) {
        ...
        if (c.getVisibility() != 8) {
          ...
          margins[index] =
              Math.max(margins[index], GridLayout.this.getMargin1(c, this.horizontal, leading));
        }
      }
    }
  ...
  public static class LayoutParams extends ViewGroup.MarginLayoutParams {
    ...
    private void reInitSuper(Context context, AttributeSet attrs) {
      TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.ViewGroup_MarginLayout);
      try {
    ...
    private void init(Context context, AttributeSet attrs) {
      TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.GridLayout_Layout);
      try {
        ...
        this.columnSpec =
            GridLayout.spec(column, colSpan, GridLayout.getAlignment(gravity, true), colWeight);
        int row = a.getInt(2, Integer.MIN_VALUE);
        int rowSpan = a.getInt(3, i);
        float rowWeight = a.getFloat(5, 0.0f);
        this.rowSpec =
            GridLayout.spec(row, rowSpan, GridLayout.getAlignment(gravity, false), rowWeight);
      } finally {
        a.recycle();
      }
    }
  ...
  static final class Arc {
    ...
    public String toString() {
      return this.span
          + " "
          + (!this.valid ? "+>" : Session.SUBSESSION_SEPARATION_CHAR)
          + " "
          + this.value;
    }
  }
  ...
  static final class Assoc<K, V> extends ArrayList<Pair<K, V>> {
    ...
    /* renamed from: of */
    public static <K, V> Assoc<K, V> m170of(Class<K> keyType, Class<V> valueType) {
    ...
    private static <K> K[] compact(K[] kArr, int[] iArr) {
      ...
      K[] kArr2 =
          (K[])
              ((Object[])
                  Array.newInstance(
                      kArr.getClass().getComponentType(), GridLayout.max2(iArr, -1) + 1));
