...
public abstract class SemHorizontalAbsListView extends AdapterView<ListAdapter> implements TextWatcher, ViewTreeObserver.OnGlobalLayoutListener, Filter.FilterListener, ViewTreeObserver.OnTouchModeChangeListener, RemoteViewsAdapter.RemoteAdapterConnectionCallback {
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof SavedState) {
            ...
            Log.m96e(TAG, "SemHorizontalAbsListView.onRestoreInstanceState() is of neither SavedState type nor Bundle type, but of " + state.getClass().toString() + " type");
    ...
    boolean performLongPress(View child, int longPressPosition, long longPressId) {
        ...
        if (handled) {
            if (semGetEnableVibrationAtLongPress()) {
                ...
                Log.m94d(TAG, " does not need vibration");
            }
        }
        return handled;
    }
                ...
                if (child != null && !child.hasExplicitFocusable() && SemHorizontalAbsListView.this.getAdapter() != null && SemHorizontalAbsListView.this.mMotionPosition >= 0 && SemHorizontalAbsListView.this.getAdapter().isEnabled(SemHorizontalAbsListView.this.mMotionPosition)) {
                    ...
                    if (!SemHorizontalAbsListView.this.mDataChanged) {
                        ...
                        if (SemHorizontalAbsListView.this.mSelector != null) {
                            ...
                            SemHorizontalAbsListView.this.mSelector.setHotspot(this.f618x, this.f619y);
                        }
                        if (longClickable) {
    ...
    */
    protected boolean dispatchHoverEvent(MotionEvent ev) {
        ...
        this.mIsNeedPenSelectIconSet = !semIsTextViewHovered && this.mOldTextViewHoverState && this.mIsDragBlockEnabled && ev.getButtonState() == 2;
        ...
        if (action == 9 || this.mHoverScrollStateChanged) {
            ...
            if (this.mNeedsHoverScroll && toolType == 2) {
                ...
                if (isHoveringOn && this.mIsDragBlockEnabled && !this.mIsPenSelectPointerSetted && toolType == 2 && ev.getButtonState() == 2) {
                    showPointerIcon(ev, 20021);
                    this.mIsPenSelectPointerSetted = true;
                }
            }
            if (this.mNeedsHoverScroll && toolType == 3 && (0 == 0 || 0 == 0)) {
                this.mNeedsHoverScroll = false;
            }
        } else if (action == 7) {
            if ((this.mIsDragBlockEnabled && !this.mIsPenSelectPointerSetted && ev.getToolType(0) == 2 && ev.getButtonState() == 2) || this.mIsNeedPenSelectIconSet) {
                showPointerIcon(ev, 20021);
                this.mIsPenSelectPointerSetted = true;
            } else if (this.mIsDragBlockEnabled && this.mIsPenSelectPointerSetted && ev.getButtonState() != 2) {
        ...
        if (this.mIsEnabledPaddingInHoverScroll) {
            ...
            contentLeft = this.mExtraPaddingInLeftHoverArea;
            contentRight = getWidth() - this.mExtraPaddingInRightHoverArea;
        }
        if (!this.mIsRTL) {
            boolean canScrollRight2 = this.mFirstPosition + count < this.mItemCount;
            if (!canScrollRight2 && count > 0) {
            ...
            canScrollLeft = this.mFirstPosition > 0;
            if (!canScrollLeft && count > 0) {
            ...
            if (!canScrollRight && count > 0) {
                ...
                canScrollRight = child2.getRight() > this.mRight - this.mListPadding.right || child2.getWidth() > getWidth() - this.mListPadding.right;
            }
            canScrollLeft = this.mFirstPosition + count < this.mItemCount;
            if (!canScrollLeft && count > 0) {
                canScrollLeft = getChildAt(count + (-1)).getLeft() < this.mListPadding.left;
            }
        }
        boolean isPossibleTooltype = ev.getToolType(0) == 2;
        if (x <= this.mHoverLeftAreaWidth + contentLeft || x >= contentRight - this.mHoverRightAreaWidth) {
            if (y > 0 && y <= getBottom()) {
                if ((canScrollLeft || canScrollRight) && ((x < contentLeft || x > this.mHoverLeftAreaWidth + contentLeft || canScrollLeft || !this.mIsHoverOverscrolled) && (x < contentRight - this.mHoverRightAreaWidth || x > contentRight || canScrollRight || !this.mIsHoverOverscrolled))) {
                    if (isPossibleTooltype && ev.getButtonState() == 2) {
                        i = 1;
                        ...
                        if (!isLockScreenMode()) {
                            ...
                            switch (action) {
                                case 7:
                                    if (!this.mHoverAreaEnter) {
                                        ...
                                        if (!this.mHoverHandler.hasMessages(1)) {
                                            ...
                                            this.mHoverHandler.sendEmptyMessage(r4);
                                            break;
                                        }
                                    } else {
                                        if (this.mHoverHandler.hasMessages(1)) {
                                ...
                                case 9:
                                    this.mHoverAreaEnter = true;
                                    if ((b1 || b2) && !this.mHoverHandler.hasMessages(1)) {
                                        this.mHoverRecognitionStartTime = System.currentTimeMillis();
                                        if (b1) {
                                            showPointerIcon(ev, 20017);
                                            this.mHoverScrollDirection = 2;
                                        ...
                                        this.mHoverHandler.sendEmptyMessage(r42);
                                        break;
                                    }
                                    break;
                                case 10:
            ...
            if ((x > this.mHoverLeftAreaWidth + contentLeft && x < contentRight - this.mHoverRightAreaWidth) || y <= 0 || y > getBottom()) {
                this.mIsHoverOverscrolled = false;
            }
        ...
        if (x > this.mHoverLeftAreaWidth + contentLeft) {
            this.mIsHoverOverscrolled = false;
            if (!this.mHoverAreaEnter) {
            }
            showPointerIcon(ev, 20001);
    ...
    */
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        SemHorizontalFastScroller semHorizontalFastScroller = this.mFastScroll;
        if (semHorizontalFastScroller != null && semHorizontalFastScroller.onInterceptTouchEvent(ev)) {
            return true;
        }
        switch (actionMasked) {
            case 0:
            ...
            case 2:
                switch (this.mTouchMode) {
                    case 0:
                        ...
                        initVelocityTrackerIfNotExists();
                        this.mVelocityTracker.addMovement(ev);
                        if (startScrollIfNeeded(x2, (int) ev.getY(pointerIndex), null)) {
                            return true;
                        }
                }
            case 6:
    ...
    class PositionScroller extends AbsPositionScroller implements Runnable {
        ...
        void scrollToVisible(int targetPos, int boundPos, int duration) {
            ...
            if (targetPos < firstPos || targetPos > lastPos) {
                Log.m102w(SemHorizontalAbsListView.TAG, "scrollToVisible called with targetPos " + targetPos + " not visible [" + firstPos + ", " + lastPos + NavigationBarInflaterView.SIZE_MOD_END);
            }
