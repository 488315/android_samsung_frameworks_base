...
public class SpellChecker implements SpellCheckerSession.SpellCheckerSessionListener {
  ...
  void resetSession() {
    ...
    if (this.mCurrentLocale == null
        || textServicesManagerForUser == null
    ...
    while (true) {
      ...
      if (i < i2) {
        ...
        this.mSpellCheckSpans =
            (SpellCheckSpan[])
                GrowingArrayUtils.append(this.mSpellCheckSpans, this.mLength, new SpellCheckSpan());
  ...
  public void spellCheck(int start, int end, boolean forceCheckWhenEditingWord) {
    ...
    if (locale2 == null || (locale = this.mCurrentLocale) == null || !locale.equals(locale2)) {
      ...
      boolean spellCheckerActivated =
          textServicesManager != null && textServicesManager.isSpellCheckerEnabled();
      ...
      if (this.mIds[i] >= 0 && !spellCheckSpan.isSpellCheckInProgress()) {
        ...
        if (selectionStart == end + 1
            && WordIterator.isMidWordPunctuation(
                this.mCurrentLocale, Character.codePointBefore(editable, end + 1))) {
          isNotEditing = false;
  ...
  private static class SentenceIteratorWrapper {
    ...
    public int following(int offset) {
      ...
      if (offset <= this.mEndOffset
          && (result = this.mSentenceIterator.following(offset - this.mStartOffset)) != -1) {
        return this.mStartOffset + result;
      }
      return -1;
    }
  ...
  private class SpellParser {
    ...
    public void parse(int start, int end, boolean forceCheckWhenEditingWord) {
      ...
      if (end > max) {
        Log.m102w(SpellChecker.TAG, "Parse invalid region, from " + start + " to " + end);
        parseEnd = max;
      } else {
        parseEnd = end;
      }
    ...
    public void parse() {
      ...
      Range<Integer> sentenceBoundary =
          SpellChecker.this.detectSentenceBoundary(editable, textChangeStart, textChangeEnd);
      ...
      if (spellCheckEnd <= spellCheckStart) {
        Log.m102w(
            SpellChecker.TAG,
            "Trying to spellcheck invalid region, from " + sentenceStart + " to " + spellCheckEnd);
  ...
  public Range<Integer> detectSentenceBoundary(
    ...
    int iteratorWindowStart =
        findSeparator(
            sequence, Math.max(0, textChangeStart - 350), Math.max(0, textChangeStart - 700));
    ...
    return new Range<>(
        Integer.valueOf(sentenceStart), Integer.valueOf(Math.max(sentenceStart, sentenceEnd)));
  }
