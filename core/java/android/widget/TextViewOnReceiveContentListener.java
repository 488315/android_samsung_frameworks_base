...
public final class TextViewOnReceiveContentListener implements OnReceiveContentListener {
  ...
  public ContentInfo onReceiveContent(View view, ContentInfo payload) {
    ...
    if (Log.isLoggable(LOG_TAG, 3)) {
      Log.m94d(LOG_TAG, "onReceive: " + payload);
    }
  ...
  private void onReceiveForAutofill(TextView view, ContentInfo payload) {
    ...
    if (isUsageOfImeCommitContentEnabled(view)
      ...
      if (Log.isLoggable(LOG_TAG, 2)) {
        Log.m100v(LOG_TAG, "onReceive: Handled via IME");
      }
    } else {
  ...
  private static boolean isUsageOfImeCommitContentEnabled(View view) {
    return view.getReceiveContentMimeTypes() == null
        && !Compatibility.isChangeEnabled(AUTOFILL_NON_TEXT_REQUIRES_ON_RECEIVE_CONTENT_LISTENER);
  }
  ...
  private ClipData handleNonTextViaImeCommitContent(ClipData clip) {
    ...
    if (containsUri(clip) && !containsOnlyText(clip)) {
      ...
      if (inputConnection == null) {
        if (Log.isLoggable(LOG_TAG, 3)) {
          Log.m94d(LOG_TAG, "onReceive: No usable EditorInfo/InputConnection");
        }
        return clip;
      }
      ...
      if (!isClipMimeTypeSupported(editorInfoContentMimeTypes, clip.getDescription())) {
        if (Log.isLoggable(LOG_TAG, 3)) {
          Log.m94d(
              LOG_TAG, "onReceive: MIME type is not supported by the app's commitContent impl");
        }
        return clip;
      }
      if (Log.isLoggable(LOG_TAG, 2)) {
        Log.m100v(LOG_TAG, "onReceive: Trying to insert via IME: " + description);
      }
      int i = 0;
      ...
      while (i2 < clip.getItemCount()) {
        ...
        if (uri == null || !"content".equals(uri.getScheme())) {
          if (Log.isLoggable(LOG_TAG, 2)) {
            Log.m100v(LOG_TAG, "onReceive: No content URI in item: uri=" + uri);
          }
          remainingItems.add(item);
        } else {
          if (Log.isLoggable(LOG_TAG, 2)) {
            Log.m100v(LOG_TAG, "onReceive: Calling commitContent: uri=" + uri);
          }
          InputContentInfo contentInfo = new InputContentInfo(uri, description);
          if (!inputConnection.commitContent(contentInfo, i, null)) {
            if (Log.isLoggable(LOG_TAG, 2)) {
              Log.m100v(LOG_TAG, "onReceive: Call to commitContent returned false: uri=" + uri);
            }
            remainingItems.add(item);
          }
        }
    ...
    if (Log.isLoggable(LOG_TAG, 2)) {
      Log.m100v(LOG_TAG, "onReceive: Clip doesn't contain any non-text URIs: " + description);
    }
    return clip;
  }
