...
public class TextViewTranslationCallback implements ViewTranslationCallback {
  ...
  private void clearTranslationTransformation() {
    if (DEBUG) {
      Log.m100v(TAG, "clearTranslationTransformation: " + this.mTranslationTransformation);
    }
  ...
  public boolean onShowTranslation(View view) {
    int newTextLength = 0;
    if (view != null
        && view.getViewTranslationResponse() != null
        && view.getViewTranslationResponse().getValue(ViewTranslationRequest.ID_TEXT) != null) {
      TranslationResponseValue value =
          view.getViewTranslationResponse().getValue(ViewTranslationRequest.ID_TEXT);
    ...
    if (this.mIsShowingTranslation && this.mTranslatedTextLength == newTextLength) {
      if (DEBUG) {
        Log.m94d(TAG, view + " is already showing translated text.");
      }
      return false;
    }
    ...
    if (response == null) {
      Log.m96e(TAG, "onShowTranslation() shouldn't be called before onViewTranslationResponse().");
      return false;
    }
    TextView theTextView = (TextView) view;
    TranslationTransformationMethod translationTransformationMethod =
        this.mTranslationTransformation;
    if (translationTransformationMethod == null
        || !response.equals(translationTransformationMethod.getViewTranslationResponse())) {
    ...
    runChangeTextWithAnimationIfNeeded(
        ...
        new Runnable() { // from class:
                         // android.widget.TextViewTranslationCallback$$ExternalSyntheticLambda2
          @Override // java.lang.Runnable
          public final void run() {
            TextViewTranslationCallback.this.lambda$onShowTranslation$0(
                textViewRef, originalTranslationMethod2);
          }
        });
  ...
  public boolean onHideTranslation(View view) {
    if (view.getViewTranslationResponse() == null) {
      Log.m96e(TAG, "onHideTranslation() shouldn't be called before onViewTranslationResponse().");
      return false;
    }
    TranslationTransformationMethod translationTransformationMethod =
        this.mTranslationTransformation;
    if (translationTransformationMethod != null) {
      final TransformationMethod transformation =
          translationTransformationMethod.getOriginalTransformationMethod();
      TextView theTextView = (TextView) view;
      final WeakReference<TextView> textViewRef = new WeakReference<>(theTextView);
      runChangeTextWithAnimationIfNeeded(
          theTextView,
          new Runnable() { // from class:
                           // android.widget.TextViewTranslationCallback$$ExternalSyntheticLambda1
            @Override // java.lang.Runnable
            public final void run() {
              TextViewTranslationCallback.this.lambda$onHideTranslation$1(
                  textViewRef, transformation);
            }
          });
    ...
    if (DEBUG) {
      Log.m102w(TAG, "onHideTranslation(): no translated text.");
    }
    return false;
  }
  ...
  public boolean onClearTranslation(View view) {
    ...
    if (DEBUG) {
      Log.m102w(TAG, "onClearTranslation(): no translated text.");
    }
    return false;
  }
  ...
  private void runChangeTextWithAnimationIfNeeded(
      final TextView view, final Runnable changeTextRunnable) {
    ...
    ofArgb.addUpdateListener(
        new ValueAnimator
            .AnimatorUpdateListener() { // from class:
                                        // android.widget.TextViewTranslationCallback$$ExternalSyntheticLambda0
    ...
    this.mAnimator.addListener(
        new Animator
            .AnimatorListener() { // from class: android.widget.TextViewTranslationCallback.1
