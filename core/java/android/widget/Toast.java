...
public class Toast {
  ...
  public Toast(Context context, Looper looper) {
    ...
    ITransientNotificationStubC4181TN iTransientNotificationStubC4181TN =
        new ITransientNotificationStubC4181TN(
            context, context.getPackageName(), binder, arrayList, looper2);
    this.mTN = iTransientNotificationStubC4181TN;
    iTransientNotificationStubC4181TN.f625mY =
        context.getResources().getDimensionPixelSize(C4337R.dimen.toast_y_offset);
    iTransientNotificationStubC4181TN.mGravity =
        context.getResources().getInteger(C4337R.integer.config_toastDefaultGravity);
  }
  ...
  private Looper getLooper(Looper looper) {
    ...
    return (Looper)
        Preconditions.checkNotNull(
            Looper.myLooper(), "Can't toast on a thread that has not called Looper.prepare()");
  }
  ...
  */
  public void show() {
    ...
    if (knoxCustomManager != null && knoxCustomManager.getToastShowPackageNameState()) {
      ...
      if (view2 != null) {
        ...
        if (tv != null && appName != null) {
          ...
          && (charSequence = this.mText) != null
          && !charSequence.toString().startsWith(appName)) {
        try {
          ...
          this.mText =
              Html.fromHtml(
                  String.format("%1s: %2s", appName, oldText2.substring(idx12 + 1, idx22)));
        } catch (Exception e3) {
    ...
    if (!this.mTN.mIsCustomOffset) {
      ...
      if (context3 == null) {
        this.mTN.f625mY =
            this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.toast_y_offset);
      } else {
        this.mTN.f625mY =
            context3.getResources().getDimensionPixelSize(C4337R.dimen.toast_y_offset);
      }
    }
    service = getService();
    pkg = this.mContext.getOpPackageName();
      ...
      try {
        if (Compatibility.isChangeEnabled(CHANGE_TEXT_TOASTS_IN_THE_SYSTEM)) {
          ...
          service.enqueueTextToastForDex(
              ...
              displayId,
              callback,
              charSequence2 != null ? charSequence2.toString() : "",
              uid);
          return;
        }
        service.enqueueToastForDex(
  ...
  public float getHorizontalMargin() {
    if (isSystemRenderedTextToast()) {
      Log.m96e(
          TAG,
          "getHorizontalMargin() shouldn't be called on text toasts, the result may not reflect"
              + " actual values.");
    }
  ...
  public float getVerticalMargin() {
    if (isSystemRenderedTextToast()) {
      Log.m96e(
          TAG,
          "getVerticalMargin() shouldn't be called on text toasts, the result may not reflect"
              + " actual values.");
    }
  ...
  public int getGravity() {
    if (isSystemRenderedTextToast()) {
      Log.m96e(
          TAG,
          "getGravity() shouldn't be called on text toasts, the result may not reflect actual"
              + " values.");
    }
  ...
  public int getXOffset() {
    if (isSystemRenderedTextToast()) {
      Log.m96e(
          TAG,
          "getXOffset() shouldn't be called on text toasts, the result may not reflect actual"
              + " values.");
    }
  ...
  public int getYOffset() {
    if (isSystemRenderedTextToast()) {
      Log.m96e(
          TAG,
          "getYOffset() shouldn't be called on text toasts, the result may not reflect actual"
              + " values.");
    }
    ...
    if (Compatibility.isChangeEnabled(CHANGE_TEXT_TOASTS_IN_THE_SYSTEM)) {
      if (this.mNextView != null) {
        throw new IllegalStateException(
            "Text provided for custom toast, remove previous setView() calls if you want a text"
                + " toast instead.");
      }
      this.mText = s;
      return;
  ...
  private static class ITransientNotificationStubC4181TN extends ITransientNotification.Stub {
    ...
    ITransientNotificationStubC4181TN(
        Context context,
        String packageName,
        Binder token,
      ...
      ToastPresenter toastPresenter =
          new ToastPresenter(context, accessibilityManager, Toast.getService(), packageName);
              ...
              switch (msg.what) {
                ...
                case 2:
                  ...
                  try {
                    Toast.getService()
                        .cancelToast(
                            ITransientNotificationStubC4181TN.this.mPackageName,
                            ITransientNotificationStubC4181TN.this.mToken);
                    break;
                  } catch (RemoteException e) {
                    return;
                  }
    ...
    public void handleShow(IBinder windowToken) {
      ...
      Log.m100v(
          Toast.TAG,
          "HANDLE SHOW: " + this + " mView=" + this.mView + " mNextView=" + this.mNextView);
      if (!this.mHandler.hasMessages(2)
          && !this.mHandler.hasMessages(1)
          && (weakReference = this.mNextView) != null
  ...
  private boolean checkGameHomeAllowList() {
    ...
    int gameNoInterruption =
        Settings.System.getInt(this.mContext.getContentResolver(), "game_no_interruption", 0);
