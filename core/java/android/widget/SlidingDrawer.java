...
public class SlidingDrawer extends ViewGroup {
  ...
  public SlidingDrawer(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(
            attrs, C0000R.styleable.SlidingDrawer, defStyleAttr, defStyleRes);
    saveAttributeDataForStyleable(
        context, C0000R.styleable.SlidingDrawer, attrs, a, defStyleAttr, defStyleRes);
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    if (this.mVertical) {
      ...
      content.layout(
          i + childWidth,
          0,
          i + childWidth + content.getMeasuredWidth(),
          ...
          if (Math.abs(velocity) < this.mMaximumTapVelocity) {
            ...
            if (!vertical
                ? !((!z || left >= this.mTapThreshold + this.mTopOffset)
                    && (z
                        || left
                            <= (((this.mBottomOffset + this.mRight) - this.mLeft)
                                    - this.mHandleWidth)
                                - this.mTapThreshold))
                : !((!z || top >= this.mTapThreshold + this.mTopOffset)
                    && (z
                        || top
                            <= (((this.mBottomOffset + this.mBottom) - this.mTop)
                                    - this.mHandleHeight)
                                - this.mTapThreshold))) {
  ...
  */
  private void performFling(
      int position, float velocity, boolean always, boolean notifyScrollListener) {
    ...
    if (this.mExpanded) {
      if (!always) {
        if (velocity <= this.mMaximumMajorVelocity) {
          if (position
                  <= this.mTopOffset + (this.mVertical ? this.mHandleHeight : this.mHandleWidth)
              || velocity <= (-r0)) {
  ...
  private void moveHandle(int position) {
    ...
    if (this.mVertical) {
      ...
      if (position < i) {
        deltaY = i - top;
          ...
          > (((this.mBottomOffset + this.mBottom) - this.mTop) - this.mHandleHeight) - top) {
    ...
    region2.union(
        frame2.right - deltaX, 0, (frame2.right - deltaX) + this.mContent.getWidth(), getHeight());
    invalidate(region2);
  }
