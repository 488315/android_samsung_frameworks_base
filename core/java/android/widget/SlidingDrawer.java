...
public class SlidingDrawer extends ViewGroup {
    ...
    public SlidingDrawer(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        saveAttributeDataForStyleable(context, C0000R.styleable.SlidingDrawer, attrs, a, defStyleAttr, defStyleRes);
        ...
        if (this.mExpanded) {
            if (!always) {
                if (velocity <= this.mMaximumMajorVelocity) {
                    if (position <= this.mTopOffset + (this.mVertical ? this.mHandleHeight : this.mHandleWidth) || velocity <= (-r0)) {
