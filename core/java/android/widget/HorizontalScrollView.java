...
public class HorizontalScrollView extends FrameLayout {
  ...
  public final class InspectionCompanion
      implements android.view.inspector.InspectionCompanion<HorizontalScrollView> {
  ...
  public HorizontalScrollView(
    ...
    TypedArray a =
        context.obtainStyledAttributes(
            attrs, C0000R.styleable.HorizontalScrollView, defStyleAttr, defStyleRes);
    saveAttributeDataForStyleable(
        context, C0000R.styleable.HorizontalScrollView, attrs, a, defStyleAttr, defStyleRes);
    ...
    String themePackageName =
        Settings.System.getString(context.getContentResolver(), "current_sec_active_themepackage");
    boolean z2 =
    ...
    if (z2) {
      ...
      if (typedArray.getResourceId(13, 0) == 17303730) {
        setBackground(context.getDrawable(C4337R.drawable.tw_action_bar_background_stacked));
      }
      typedArray.recycle();
    }
    a.recycle();
    ...
    switch (action & 255) {
      ...
      case 2:
        ...
        if (index != -1) {
          ...
          if (pointerIndex == -1) {
            Log.m96e(TAG, "Invalid pointerId=" + index + " in onInterceptTouchEvent");
            break;
          } else {
  ...
  protected boolean dispatchHoverEvent(MotionEvent ev) {
    ...
    if ((x > i && x < contentRightSide - i)
        || range == 0
        || ((x >= 0 && x <= i && this.mScrollX <= 0 && this.mIsHoverOverscrolled)
            || ((x >= contentRightSide - this.mHoverAreaWidth
                    && x <= contentRightSide
                    && this.mScrollX >= range
    ...
    switch (action) {
      case 7:
        ...
        } else if (x >= contentRightSide - this.mHoverAreaWidth
            && x <= contentRightSide
    ...
    switch (action & 255) {
          ...
          if (getChildCount() > 0) {
            if (Math.abs(initialVelocity) > this.mMinimumVelocity) {
              fling(-initialVelocity);
                ...
                this.mScrollX, this.mScrollY, 0, getScrollRange(), 0, 0)) {
              postInvalidateOnAnimation();
            }
          }
          this.mActivePointerId = -1;
      ...
      case 2:
        ...
        if (activePointerIndex == -1) {
          this.mActivePointerId = ev.getPointerId(0);
          Log.m96e(TAG, "Invalid pointerId=" + this.mActivePointerId + " in onTouchEvent");
          return true;
        }
          ...
          if (!canOverscroll) {
            ...
            if (deltaX2 < 0 && this.mEdgeGlowRight.getDistance() != 0.0f) {
              consumed =
                  Math.round(
                      getWidth()
                          * this.mEdgeGlowRight.onPullDistance(deltaX2 / getWidth(), displacement));
            } else if (deltaX2 > 0 && this.mEdgeGlowLeft.getDistance() != 0.0f) {
              consumed =
                  Math.round(
                      (-getWidth())
                          * this.mEdgeGlowLeft.onPullDistance(
                              (-deltaX2) / getWidth(), 1.0f - displacement));
            }
  ...
  private int getScrollRange() {
    ...
    View child = getChildAt(0);
    int scrollRange =
        Math.max(0, child.getWidth() - ((getWidth() - this.mPaddingLeft) - this.mPaddingRight));
    return scrollRange;
  }
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (left < viewRight && viewLeft < right) {
        ...
        if (focusCandidate == null) {
          ...
          if ((leftFocus && viewLeft < focusCandidate.getLeft())
  ...
  private boolean isWithinDeltaOfScreen(View descendant, int delta) {
    ...
    return this.mTempRect.right + delta >= getScrollX()
        && this.mTempRect.left - delta <= getScrollX() + getWidth();
  }
  ...
  public final void smoothScrollBy(int dx, int dy) {
    ...
    if (duration > 250) {
      ...
      this.mScroller.startScroll(
          scrollX, this.mScrollY, Math.max(0, Math.min(scrollX + dx, maxX)) - scrollX, 0);
      postInvalidateOnAnimation();
    } else {
      if (!this.mScroller.isFinished()) {
        this.mScroller.abortAnimation();
  ...
  protected void measureChild(View child, int parentWidthMeasureSpec, int parentHeightMeasureSpec) {
    ...
    int childWidthMeasureSpec =
        View.MeasureSpec.makeSafeMeasureSpec(
            Math.max(0, View.MeasureSpec.getSize(parentWidthMeasureSpec) - horizontalPadding), 0);
    int childHeightMeasureSpec =
        getChildMeasureSpec(
  ...
  protected void measureChildWithMargins(
    ...
    int childWidthMeasureSpec =
        View.MeasureSpec.makeSafeMeasureSpec(
            Math.max(0, View.MeasureSpec.getSize(parentWidthMeasureSpec) - usedTotal), 0);
    child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
  }
  ...
  private int consumeFlingInStretch(int unconsumed) {
    ...
    if (unconsumed > 0
        && (edgeEffect2 = this.mEdgeGlowLeft) != null
      ...
      int consumed =
          Math.round(
              ((-size) / FLING_DESTRETCH_FACTOR)
                  * this.mEdgeGlowLeft.onPullDistance(deltaDistance, 0.5f));
    ...
    if (unconsumed < 0
        && (edgeEffect = this.mEdgeGlowRight) != null
      ...
      int consumed2 =
          Math.round(
              (size2 / FLING_DESTRETCH_FACTOR)
                  * this.mEdgeGlowRight.onPullDistance(deltaDistance2, 0.5f));
  ...
  protected int computeScrollDeltaToGetChildRectOnScreen(Rect rect) {
    ...
    if (rect.right > screenRight && rect.left > screenLeft) {
      int scrollXDelta =
          rect.width() > width ? 0 + (rect.left - screenLeft) : 0 + (rect.right - screenRight);
    ...
    int scrollXDelta2 =
        rect.width() > width ? 0 - (screenRight - rect.right) : 0 - (screenLeft - rect.left);
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    if (getChildCount() <= 0) {
      ...
      FrameLayout.LayoutParams childParams =
          (FrameLayout.LayoutParams) getChildAt(0).getLayoutParams();
    ...
    int childWidth3 = r - l;
    int available =
        ((childWidth3 - getPaddingLeftWithForeground()) - getPaddingRightWithForeground())
            - childMargins;
    ...
    if (!isLaidOut()) {
      int scrollRange =
          Math.max(0, childWidth - (((r - l) - this.mPaddingLeft) - this.mPaddingRight));
    ...
    if (getChildCount() > 0) {
      ...
      if (shouldFling) {
        this.mScroller.fling(
            this.mScrollX, this.mScrollY, velocityX, 0, 0, maxScroll, 0, 0, width / 2, 0);
        boolean movingRight = velocityX > 0;
        View currentFocused = findFocus();
        View newFocused =
            findFocusableViewInMyBounds(movingRight, this.mScroller.getFinalX(), currentFocused);
        if (newFocused == null) {
          newFocused = this;
        }
  ...
  static class SavedState extends View.BaseSavedState {
    public static final Parcelable.Creator<SavedState> CREATOR =
        new Parcelable.Creator<
            SavedState>() { // from class: android.widget.HorizontalScrollView.SavedState.1
    ...
    switch (msg.what) {
      case 1:
        ...
        if (currentTimeMillis - this.mHoverScrollStartTime >= this.mHoverScrollTimeInterval) {
          ...
          int offset =
              this.mHoverScrollDirection == 1 ? -this.mHoverScrollSpeed : this.mHoverScrollSpeed;
          ...
          if ((offset < 0 && this.mScrollX > 0) || (offset > 0 && this.mScrollX < range)) {
            ...
            if (canOverscroll && !this.mIsHoverOverscrolled) {
              ...
              if (edgeEffect != null
                  && (!edgeEffect.isFinished() || !this.mEdgeGlowRight.isFinished())) {
                invalidate();
              }
