...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher, ViewTreeObserver.OnGlobalLayoutListener, Filter.FilterListener, ViewTreeObserver.OnTouchModeChangeListener, RemoteViewsAdapter.RemoteAdapterConnectionCallback {
    ...
    public AbsListView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        initAbsListView();
        this.mOwnerThread = Thread.currentThread();
        TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.AbsListView, defStyleAttr, defStyleRes);
        saveAttributeDataForStyleable(context, C4337R.styleable.AbsListView, attrs, a, defStyleAttr, defStyleRes);
    ...
    private void initAbsListView() {
        ...
        boolean resolved = this.mContext.getTheme().resolveAttribute(C4337R.attr.twListMultiSelectBackground, value, true);
        if (resolved) {
            this.mMultiFocusImage = this.mContext.getResources().getDrawable(value.resourceId);
        }
        boolean resolved2 = this.mContext.getTheme().resolveAttribute(C4337R.attr.twDragBlockImage, value, true);
        if (resolved2) {
            this.mSemDragBlockImage = this.mContext.getResources().getDrawable(value.resourceId);
        }
        boolean resolved3 = this.mContext.getTheme().resolveAttribute(C4337R.attr.semGoToTopStyle, value, true);
        if (resolved3) {
        ...
        if (sIsSamsungBasicInteraction) {
            ...
            this.mSemFillOutPaint.setColor(getResources().getColor(C4337R.color.sem_round_and_bgcolor_dark, null));
        }
    }
    ...
    public boolean performAccessibilityActionInternal(int action, Bundle arguments) {
        ...
        switch (action) {
            ...
            case 67108864:
                ...
                return true;
            case 268435456:
                Log.m94d(TAG, "SEM_ACTION_AUTOSCROLL_SPEED_UP, current duration = " + this.mAutoscrollDuration);
                if (!canScrollDown()) {
                    return false;
                }
                ...
                return true;
            case 536870912:
                Log.m94d(TAG, "SEM_ACTION_AUTOSCROLL_SPEED_DOWN, current duration = " + this.mAutoscrollDuration);
                if (!canScrollDown()) {
                    return false;
                }
        ...
        if (child == null) {
            ...
            if (child == null) {
                ...
                if (this.mAdapter instanceof HeaderViewListAdapter) {
                    Log.m94d(TAG, "HeaderCount = " + ((HeaderViewListAdapter) this.mAdapter).getHeadersCount());
                    Log.m94d(TAG, "FooterCount = " + ((HeaderViewListAdapter) this.mAdapter).getFootersCount());
                }
                return null;
            }
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled()) {
            Log.m94d(TAG, "onTouchEvent() mIsLongPressMultiSelection : " + this.mIsLongPressMultiSelection);
        ...
        if (this.mIsDetaching || !isAttachedToWindow()) {
            Log.m94d(TAG, "onTouchEvent() mIsDetaching : " + this.mIsDetaching + ", isAttachedToWindow() : " + isAttachedToWindow());
            return false;
        }
        startNestedScroll(2);
        FastScroller fastScroller = this.mFastScroll;
    ...
    */
    boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
        ...
        if (childCount3 != 0) {
            ...
            boolean cannotScrollDown2 = firstPosition == 0 && firstTop >= listPadding.top && incrementalDeltaY2 >= 0;
                ...
                if (!cannotScrollUp) {
                    ...
                    if (this.mIsMouseHoverScroll && this.mSelectorPosition != -1) {
                        ...
                        if (this.mSelectorPosition != -1) {
                            if (childIndex3 >= 0 && childIndex3 < getChildCount() && this.mAdapter.isEnabled(this.mSelectorPosition)) {
                                positionSelector(-1, getChildAt(childIndex3));
                            } else if (childIndex3 >= 0 && childIndex3 < getChildCount() && !this.mAdapter.isEnabled(this.mSelectorPosition)) {
                                this.mSelectorRect.setEmpty();
                            }
                        }
                    } else {
                        int childIndex4 = this.mSelectorPosition;
                        if (childIndex4 != -1 && (childIndex = childIndex4 - this.mFirstPosition) >= 0 && childIndex < getChildCount()) {
    ...
    public EditText getTextFilterInput() {
        if (this.mTextFilter == null) {
            ...
            EditText editText = (EditText) layoutInflater.inflate(C4337R.layout.typing_filter, (ViewGroup) null);
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        @ViewDebug.ExportedProperty(category = Slice.HINT_LIST, mapping = {@ViewDebug.IntToString(from = -1, m158to = "ITEM_VIEW_TYPE_IGNORE"), @ViewDebug.IntToString(from = -2, m158to = "ITEM_VIEW_TYPE_HEADER_OR_FOOTER")})
        int viewType;
    ...
    class PositionScroller extends AbsPositionScroller implements Runnable {
        ...
        private void scrollToVisible(int targetPos, int boundPos, int duration) {
            ...
            if (targetPos < firstPos || targetPos > lastPos) {
                Log.m102w(AbsListView.TAG, "scrollToVisible called with targetPos " + targetPos + " not visible [" + firstPos + ", " + lastPos + NavigationBarInflaterView.SIZE_MOD_END);
            }
    ...
    public void semSetupGoToTop(int where) {
        ...
        if (!semIsTalkBackIsRunning() && this.mSemEnableGoToTop) {
            ...
            switch (where2) {
                ...
                case 2:
                    removeCallbacks(this.mSemGoToToFadeOutRunnable);
                    int btnW = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_go_to_top_scrollableview_size);
                    int btnH = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_go_to_top_scrollableview_size);
                    if (this.mAppWidgetGoToTop) {
                        int gapH2 = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_go_to_top_scrollableview_gap_appwidget);
                        gapH = gapH2 + this.mAppWidgetGoToTopOffset;
                    } else {
                        gapH = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_go_to_top_scrollableview_gap);
                    }
    ...
    public void semSetGoToTopEnabled(boolean enabled, int buttonStyle) {
        if (this.mAppWidgetGoToTop) {
            ...
            this.mSemGoToTopImage = isNight ? getResources().getDrawable(C4337R.drawable.sem_list_go_to_top_dark_appwidget) : getResources().getDrawable(C4337R.drawable.sem_list_go_to_top_light_appwidget);
        } else {
            this.mSemGoToTopImage = buttonStyle == 0 ? this.mSemGoToTopLightImage : this.mContext.getResources().getDrawable(C4337R.drawable.sem_list_go_to_top_dark);
        }
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    if (AbsListView.this.mAppWidgetGoToTop) {
                        ...
                        if (rect.left == 0 || rect.top == 0) {
                            Log.m102w(AbsListView.TAG, "Hide GotoTop immediatley left: " + rect.left + " top" + rect.top);
                            value = 0;
    ...
    */
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case 0:
                this.mGoToToping = false;
                if (this.mSemGoToTopState != 2 && semIsSupportGotoTop() && this.mSemGoToTopRect.contains(x, y)) {
                    semSetupGoToTop(2);
                    ...
                    if (this.mIsCtrlkeyPressed && ev.getToolType(0) == 3) {
                        ...
                        this.mTouchdownX = x;
                        this.mTouchdownY = y;
                        boolean isCarModeOn3 = Settings.System.getIntForUser(this.mContext.getContentResolver(), "car_mode_on", 0, -3) == 1;
                        if (isCarModeOn3) {
                            this.mIsNeedPenSelection = false;
                        }
                ...
                if (!this.mIsCtrlMultiSelection) {
                    ...
                    if (this.mSemGoToTopState != 2 && semIsSupportGotoTop()) {
                        if (canScrollUp()) {
                            ...
                            post(new Runnable() { // from class: android.widget.AbsListView.12
                                @Override // java.lang.Runnable
                                public void run() {
    ...
    */
    protected boolean dispatchHoverEvent(MotionEvent ev) {
        ...
        if (!semIsTextViewHovered && this.mOldTextViewHoverState && this.mIsDragBlockEnabled && (ev.getButtonState() == 32 || ev.getButtonState() == 2)) {
        ...
        if (y2 <= this.mHoverTopAreaHeight + contentTop || y2 >= contentBottom - this.mHoverBottomAreaHeight) {
            if (x > 0 && x <= getRight()) {
                if ((canScrollUp || canScrollDown) && ((y2 < contentTop || y2 > this.mHoverTopAreaHeight + contentTop || canScrollUp || !this.mIsHoverOverscrolled) && (y2 < contentBottom - this.mHoverBottomAreaHeight || y2 > contentBottom || canScrollDown || !this.mIsHoverOverscrolled))) {
                    if (isPossibleToolType && (ev.getButtonState() == 32 || ev.getButtonState() == 2)) {
                        ...
                        if (!this.mSemEnableGoToTop || this.mSemGoToTopState == 0 || !this.mSemGoToTopRect.contains(x, y2)) {
                            if (!this.mHoverAreaEnter) {
                                this.mHoverScrollStartTime = System.currentTimeMillis();
                            }
                            switch (action) {
                                ...
                                case 9:
                                    ...
                                    } else if (y2 >= contentBottom - this.mHoverBottomAreaHeight && y2 <= contentBottom && !this.mHoverHandler.hasMessages(1)) {
                                        this.mHoverRecognitionStartTime = System.currentTimeMillis();
                                        showPointerIcon(ev, 20015);
                                        this.mHoverScrollDirection = 1;
                                        this.mHoverHandler.sendEmptyMessage(1);
    ...
    private void initIndicator() {
        this.mIndicatorItemCnt = 0;
        this.mIndicatorAnimatedSize = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_indicator_radius_focused);
        this.mIndicatorRectSize = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_indicator_rect_size);
        if (this.mIndicatorMarginHorizontal == 0) {
            this.mIndicatorMarginHorizontal = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_indicator_rect_size);
        }
        this.mIndicatorFocusedSize = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_indicator_radius_focused);
        this.mIndicatorDefaultSize = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_indicator_radius);
        if (this.mAdapter != null) {
