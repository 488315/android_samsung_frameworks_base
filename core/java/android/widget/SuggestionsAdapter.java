...
class SuggestionsAdapter extends ResourceCursorAdapter implements View.OnClickListener {
  ...
  public void changeCursor(Cursor c) {
    if (this.mClosed) {
      Log.m102w(LOG_TAG, "Tried to change cursor after adapter was closed.");
      if (c != null) {
        c.close();
        return;
      }
    ...
    try {
      ...
      Log.m97e(LOG_TAG, "error changing cursor and caching columns", e);
    }
  }
  ...
  public View newView(Context context, Cursor cursor, ViewGroup parent) {
    ...
    ImageView iconRefine = (ImageView) v.findViewById(C4337R.id.edit_query);
  ...
  private static final class ChildViewCache {
    ...
    public ChildViewCache(View v) {
      ...
      this.mIconRefine = (ImageView) v.findViewById(C4337R.id.edit_query);
    }
  }
  ...
  private CharSequence formatUrl(Context context, CharSequence url) {
    if (this.mUrlColor == null) {
      ...
      context.getTheme().resolveAttribute(C4337R.attr.textColorSearchUrl, colorValue, true);
  ...
  public CharSequence convertToString(Cursor cursor) {
    ...
    if (this.mSearchable.shouldRewriteQueryFromData()
        && (data = getColumnString(cursor, SearchManager.SUGGEST_COLUMN_INTENT_DATA)) != null) {
      return data;
    }
  ...
  private Drawable getDrawableFromResourceValue(String drawableId) {
    ...
    try {
      ...
      String drawableUri =
          "android.resource://" + this.mProviderContext.getPackageName() + "/" + resourceId;
      ...
      Log.m102w(LOG_TAG, "Icon resource not found: " + drawableId);
      return null;
  ...
  private Drawable getDrawable(Uri uri) {
    try {
      ...
      if (ContentResolver.SCHEME_ANDROID_RESOURCE.equals(scheme)) {
        ...
        try {
          return r.f46r.getDrawable(r.f45id, this.mProviderContext.getTheme());
        } catch (Resources.NotFoundException e) {
      ...
      try {
        ...
        try {
          ...
          Log.m97e(LOG_TAG, "Error closing icon stream for " + uri, ex);
        }
      }
    } catch (FileNotFoundException fnfe) {
      Log.m102w(LOG_TAG, "Icon not found: " + uri + ", " + fnfe.getMessage());
      return null;
    }
    Log.m102w(LOG_TAG, "Icon not found: " + uri + ", " + fnfe.getMessage());
    return null;
  ...
  private Drawable getActivityIcon(ComponentName component) {
    ...
    try {
      ...
      if (drawable == null) {
        Log.m102w(
            LOG_TAG,
            "Invalid icon resource " + iconId + " for " + component.flattenToShortString());
        return null;
      }
      return drawable;
    } catch (PackageManager.NameNotFoundException ex) {
      Log.m102w(LOG_TAG, ex.toString());
      return null;
    }
  }
