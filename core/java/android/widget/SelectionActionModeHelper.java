...
public class SelectionActionModeHelper {
    ...
    public static <T> PointF movePointInsideNearestRectangle(PointF point, List<T> list, Function<T, RectF> extractor) {
        ...
        for (int index = 0; index < elementCount; index++) {
            ...
            double candidateDistance = Math.pow(point.f87x - candidateX, 2.0d) + Math.pow(point.f88y - candidateY, 2.0d);
    ...
    private static final class SelectionTracker {
        ...
        private final class LogAbandonRunnable implements Runnable {
            ...
            void schedule(int delayMillis) {
                if (this.mIsPending) {
                    Log.m96e(SelectionActionModeHelper.LOG_TAG, "Force flushing abandon due to new scheduling request");
                    flush();
                }
