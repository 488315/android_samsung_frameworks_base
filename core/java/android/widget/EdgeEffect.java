...
public class EdgeEffect {
  ...
  public EdgeEffect(Context context, AttributeSet attrs) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.EdgeEffect);
  ...
  public void onPull(float deltaDistance, float displacement) {
    ...
    int i = this.mState;
    if (i == 4 && now - this.mStartTime < this.mDuration && this.mEdgeEffectType == 0) {
      return;
    }
    if (i != 1) {
          ...
          (float)
              (Math.max(
                      ...
                      (1.0d
                              - (1.0d
                                  / Math.sqrt(
                                      Math.abs(this.mPullDistance) * this.mBounds.height())))
                          - 0.3d)
                  / 0.7d);
    ...
    if (edgeEffectBehavior == 1) {
      ...
      if (edgeEffectBehavior == 0) {
        ...
        this.mGlowScaleYFinish =
            Math.min(((((velocity2 / 100) * velocity2) * 1.5E-4f) / 2.0f) + 0.025f, 1.0f);
        this.mGlowAlphaFinish =
            Math.max(this.mGlowAlphaStart, Math.min(velocity2 * 6 * 1.0E-5f, MAX_ALPHA));
        this.mTargetDisplacement = 0.5f;
        return;
      }
      finish();
        ...
        ((-24.657d) * distance * DAMPING_RATIO)
            + (Math.pow(2.718281828459045d, deltaT * (-24.16386d))
                * (((-mDampedFreq) * cosCoeff * Math.sin(deltaT * mDampedFreq))
                    + (mDampedFreq * sinCoeff * Math.cos(deltaT * mDampedFreq))));
