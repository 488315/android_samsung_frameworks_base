  ...
  public void semSetCacheSize(int cacheSizeByte) {
    if (cacheSizeByte < 0 || cacheSizeByte > 8388608) {
      throw new IllegalArgumentException(
          "The cache size should not be negative value. Also, it should be less than soft heap size"
              + " (8M). Now: "
              + cacheSizeByte);
    }
    synchronized (this) {
    ...
    try {
      ...
      if (db != null) {
        ...
        if (str == null) {
          ...
          try {
            ...
            Log.m97e(TAG, "Couldn't open " + this.mName + " for writing (will try read-only):", ex);
            db =
                SQLiteDatabase.openDatabase(
                    filePath.getPath(), params.toBuilder().addOpenFlags(1).build(), this.mContext);
          }
        }
      }
      onConfigure(db);
      ...
      if (version != this.mNewVersion) {
        ...
        if (version > 0 && version < this.mMinimumSupportedVersion) {
          ...
          throw new IllegalStateException(
              "Unable to delete obsolete database " + this.mName + " with version " + version);
        }
        db.beginTransaction();
        try {
          if (version == 0) {
            ...
            Log.m98i(TAG, "DB version downgrading from " + version + " to " + this.mNewVersion);
            onDowngrade(db, version, this.mNewVersion);
          } else {
            Log.m98i(TAG, "DB version upgrading from " + version + " to " + this.mNewVersion);
      ...
      onOpen(db);
      if (db.isReadOnly()) {
        Log.m102w(TAG, "Opened " + this.mName + " in read-only mode");
      }
      this.mDatabase = db;
      this.mIsInitializing = false;
      return db;
