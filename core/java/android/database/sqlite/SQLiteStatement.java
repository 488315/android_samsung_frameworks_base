...
public final class SQLiteStatement extends SQLiteProgram {
  ...
  public int executeUpdateDelete() {
    ...
    try {
      try {
        return getSession()
            .executeForChangedRowCount(getSql(), getBindArgs(), getConnectionFlags(), null);
      } catch (SQLiteDatabaseCorruptException ex) {
        onCorruption(ex.getCorruptCode());
        throw ex;
      }
  ...
  public long executeInsert() {
    ...
    try {
      try {
        return getSession()
            .executeForLastInsertedRowId(getSql(), getBindArgs(), getConnectionFlags(), null);
      } catch (SQLiteDatabaseCorruptException ex) {
        onCorruption(ex.getCorruptCode());
        throw ex;
      }
  ...
  public ParcelFileDescriptor simpleQueryForBlobFileDescriptor() {
    ...
    try {
      try {
        return getSession()
            .executeForBlobFileDescriptor(getSql(), getBindArgs(), getConnectionFlags(), null);
      } catch (SQLiteDatabaseCorruptException ex) {
        onCorruption(ex.getCorruptCode());
        throw ex;
      }
