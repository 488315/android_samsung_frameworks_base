...
public class SQLiteCompatibilityWalFlags {
    ...
    private static void initIfNeeded() {
        ...
        String flags = null;
        if (app == null) {
            Log.m102w(TAG, "Cannot read global setting sqlite_compatibility_wal_flags - Application state not available");
        } else {
            try {
    ...
    public static void init(String flags) {
        ...
        try {
            ...
            Log.m98i(TAG, "Read compatibility WAL flags: legacy_compatibility_wal_enabled=" + sLegacyCompatibilityWalEnabled + ", wal_syncmode=" + sWALSyncMode);
            sInitialized = true;
        } catch (IllegalArgumentException e) {
            Log.m97e(TAG, "Setting has invalid format: " + flags, e);
            sInitialized = true;
        }
    }
