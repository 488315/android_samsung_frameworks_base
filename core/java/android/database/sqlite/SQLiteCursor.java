...
public class SQLiteCursor extends AbstractWindowedCursor {
    ...
    public boolean onMove(int oldPosition, int newPosition) {
        if (this.mWindow == null || newPosition < this.mWindow.getStartPosition() || newPosition >= this.mWindow.getStartPosition() + this.mWindow.getNumRows()) {
            ...
            if (newPosition < startPos || newPosition >= startPos + numRows) {
                Log.m96e(TAG, "onMove() return false. RequiredPos: " + newPosition + " WindowStartPos: " + startPos + " WindowRowCount: " + numRows + "(original count of query: " + this.mCount + NavigationBarInflaterView.KEY_CODE_END);
                return false;
            }
            return true;
        }
        ...
        try {
            ...
            if (this.mCount == -1) {
                ...
                if (SQLiteDebug.NoPreloadHolder.DEBUG_SQL_LOG) {
                    Log.m94d(TAG, "received count(*) from native_fill_window: " + this.mCount);
                    return;
                }
                return;
            }
    ...
    public int getColumnIndex(String columnName) {
        ...
        if (periodIndex != -1) {
            ...
            Log.m97e(TAG, "requesting column name with table name -- " + columnName, e);
