...
public class SQLiteQueryBuilder {
  ...
  public Cursor query(
    ...
    if (Log.isLoggable(TAG, 3)) {
      if (Build.IS_DEBUGGABLE) {
        Log.m94d(TAG, sql + " with args " + Arrays.toString(selectionArgs));
      } else {
        Log.m94d(TAG, sql);
      }
    }
  ...
  public long insert(SQLiteDatabase db, ContentValues values) {
    ...
    if (Log.isLoggable(TAG, 3)) {
      if (Build.IS_DEBUGGABLE) {
        Log.m94d(TAG, sql + " with args " + Arrays.toString(sqlArgs));
      } else {
        Log.m94d(TAG, sql);
      }
    }
  ...
  private String computeSingleProjection(String userColumn) {
    ...
    if (collection != null) {
      ...
      if (match) {
        Log.m102w(TAG, "Allowing abusive custom column: " + userColumn);
        return maybeWithOperator(operator, userColumn);
      }
      return null;
    }
