...
public final class SQLiteConnectionPool implements Closeable {
    ...
    private void dispose(boolean finalized) {
        ...
        if (!finalized) {
            synchronized (this.mLock) {
                ...
                if (pendingCount != 0) {
                    Log.m98i(TAG, "The connection pool for " + this.mConfiguration.label + " has been closed but there are still " + pendingCount + " connections in use.  They will be closed as they are released back to the pool.");
                }
    ...
    public void setCheckpointOnClose(boolean set) {
        synchronized (this.mLock) {
            ...
            if (sQLiteConnection != null) {
                ...
                Log.m96e(TAG, "Could not change 'checkpointOnClose' to " + set + " because the primary connection is on used.");
            }
        }
    }
    ...
    */
    public void reconfigure(SQLiteDatabaseConfiguration configuration) {
        ...
        synchronized (this.mLock) {
            ...
            if (!onlyCompatWalChanged && this.mConfiguration.openFlags != configuration.openFlags) {
                ...
                if (this.mConfiguration.sharedConfig.isSecureDb) {
                    newPrimaryConnection = openSecureConnectionLocked(configuration, true, this.mConnectionKey.decryptAndGet(SQLiteGlobal.getRandArray(this.mRandArrPtr)));
                } else {
    ...
    void onConnectionLeaked() {
        Log.m102w(TAG, "A SQLiteConnection object for database '" + this.mConfiguration.label + "' was leaked!  Please fix your application to end transactions in progress properly and to close the database when it is no longer needed.");
    ...
    private void closeConnectionAndLogExceptionsLocked(SQLiteConnection connection) {
        try {
            ...
            Log.m97e(TAG, "Failed to close connection, its fate is now in the hands of the merciful GC: " + connection, ex);
        }
    }
    ...
    private void reconfigureAllConnectionsLocked() {
        ...
        if (sQLiteConnection != null) {
            try {
                ...
                Log.m97e(TAG, "Failed to reconfigure available primary connection, closing it: " + this.mAvailablePrimaryConnection, ex);
                closeConnectionAndLogExceptionsLocked(this.mAvailablePrimaryConnection);
                this.mAvailablePrimaryConnection = null;
            }
        }
        ...
        while (i < count) {
            ...
            try {
                ...
                Log.m97e(TAG, "Failed to reconfigure available non-primary connection, closing it: " + connection, ex2);
                closeConnectionAndLogExceptionsLocked(connection);
                this.mAvailableNonPrimaryConnections.remove(i);
                count--;
                i--;
    ...
    */
    ...
    private void finishAcquireConnectionLocked(SQLiteConnection connection, int connectionFlags) {
        ...
        try {
            ...
            Log.m96e(TAG, "Failed to prepare acquired connection for session, closing it: " + connection + ", connectionFlags=" + connectionFlags);
            closeConnectionAndLogExceptionsLocked(connection);
            throw ex;
        }
    }
        ...
        public void handleMessage(Message msg) {
            synchronized (SQLiteConnectionPool.this.mLock) {
                ...
                if (SQLiteConnectionPool.this.closeAvailableConnectionLocked(msg.what) && Log.isLoggable(SQLiteConnectionPool.TAG, 3)) {
                    Log.m94d(SQLiteConnectionPool.TAG, "Closed idle connection " + SQLiteConnectionPool.this.mConfiguration.label + " " + msg.what + " after " + this.mTimeout);
                }
    ...
    private class IdleConnectionShrinkHandler extends IdleConnectionHandler {
        ...
        @Override // android.database.sqlite.SQLiteConnectionPool.IdleConnectionHandler, android.p009os.Handler
        public void handleMessage(Message msg) {
            synchronized (SQLiteConnectionPool.this.mLock) {
                if (this != SQLiteConnectionPool.this.mIdleConnectionHandler) {
                    return;
                ...
                if (SQLiteConnectionPool.this.releaseAvailableConnectionMemoryLocked(msg.what) && Log.isLoggable(SQLiteConnectionPool.TAG, 3)) {
                    Log.m94d(SQLiteConnectionPool.TAG, "Released idle connection's memory " + SQLiteConnectionPool.this.mConfiguration.label + " " + msg.what + " after " + this.mTimeout);
                }
            }
        }
    }
        ...
        public void encryptAndSave(char[] password, byte[] data) {
            synchronized (this.mLock) {
                ...
                try {
                    ...
                    Log.m96e(SQLiteConnectionPool.TAG, "Could not encrypt the data of " + this.mConfiguration.label);
            ...
            synchronized (this.mLock) {
                try {
                    ...
                    try {
                        ...
                        Log.m96e(SQLiteConnectionPool.TAG, "Could not decrypt the data of " + this.mConfiguration.label);
