...
public class SQLiteUtils {
  ...
  public static boolean isPackageInstalled(Context context, String packageName) {
    try {
      ...
      if (packageManager == null) {
        Log.m94d(TAG, "The PackageManager is null.");
        return false;
      }
  ...
  private static class ReportRunnable implements Runnable {
    ...
    public void run() {
      ...
      issueTrackerIntent.putExtra(
          "ERRCODE", Integer.valueOf(PackageManager.INSTALL_FAILED_PROCESS_NOT_DEFINED));
      issueTrackerIntent.putExtra("ERRNAME", "DBSIZE");
      String packageName = this.context.getPackageName();
      issueTrackerIntent.putExtra("ERRPKG", packageName);
      issueTrackerIntent.putExtra(
          "ERRMSG",
          "Abnormal database file size has been detected from '"
              + packageName
              + "' size: "
              + ((this.size / 1024) / 1024)
              + "MB");
