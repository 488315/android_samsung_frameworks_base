...
public abstract class ContentObserver {
  ...
  public void onChange(boolean selfChange, Collection<Uri> uris, int flags, int userId) {
    if (!CompatChanges.isChangeEnabled(ADD_CONTENT_OBSERVER_FLAGS) || Process.myUid() == 1000) {
      onChange(selfChange, uris, flags, UserHandle.m55of(userId));
    } else {
  ...
  public final void dispatchChange(
      final boolean selfChange, final Collection<Uri> uris, final int flags, final int userId) {
    Handler handler = this.mHandler;
    if (handler == null) {
      onChange(selfChange, uris, flags, userId);
    } else {
      handler.post(
          new Runnable() { // from class: android.database.ContentObserver$$ExternalSyntheticLambda0
