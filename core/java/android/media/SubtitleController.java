    ...
    synchronized (this.mTracks) {
      ...
      while (it.hasNext()) {
        ...
        if (locale != null
          ...
          score =
              (i2 == 0 ? 0 : 8)
                  + ((selectedLocale == null || i4 == 0) ? 0 : 4)
                  + (i3 == 0 ? 0 : 2)
                  + (!languageMatches ? 1 : 0);
          if (!selectForced && i2 == 0) {
            i = 1;
          } else {
            if (((selectedLocale == null && i4 != 0)
                    || (languageMatches && (i3 != 0 || i2 != 0 || selectedLocale != null)))
                && score > bestScore) {
              bestScore = score;
              bestTrack = track;
            }
    ...
    if (this.mTrackIsExplicit) {
      if (!this.mVisibilityIsExplicit) {
        if (this.mCaptioningManager.isEnabled()
            || ((subtitleTrack = this.mSelectedTrack) != null
                && subtitleTrack.getFormat().getInteger(MediaFormat.KEY_IS_FORCED_SUBTITLE, 0)
                    != 0)) {
          show();
        } else {
          SubtitleTrack subtitleTrack2 = this.mSelectedTrack;
