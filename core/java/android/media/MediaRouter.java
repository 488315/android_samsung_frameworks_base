...
public class MediaRouter {
  ...
  static class Static implements DisplayManager.DisplayListener {
    ...
    final IAudioRoutesObserver.Stub mAudioRoutesObserver =
        new IAudioRoutesObserver.Stub() { // from class: android.media.MediaRouter.Static.1
          ...
          public void dispatchAudioRoutesChanged(final AudioRoutesInfo newRoutes) {
            try {
              ...
              Log.m97e(MediaRouter.TAG, "Error querying Bluetooth A2DP state", e);
            }
            Static.this.mHandler.post(
                new Runnable() { // from class: android.media.MediaRouter.Static.1.1
    ...
    Static(Context appContext) {
      ...
      RouteCategory routeCategory =
          new RouteCategory(C4337R.string.default_audio_route_category_name, 3, false);
      this.mSystemCategory = routeCategory;
      routeCategory.mIsSystem = true;
      this.mCanConfigureWifiDisplays =
          appContext.checkPermission(
                  Manifest.permission.CONFIGURE_WIFI_DISPLAY, Process.myPid(), Process.myUid())
              == 0;
    }
    ...
    void startMonitoringRoutes(Context appContext) {
      ...
      routeInfo.mNameResId = C4337R.string.default_audio_route_name;
      ...
      appContext.registerReceiver(
          ...
          new IntentFilter("android.hardware.display.action.WIFI_DISPLAY_STATUS_CHANGED"));
      appContext.registerReceiver(
          new VolumeChangeReceiver(), new IntentFilter("android.media.VOLUME_CHANGED_ACTION"));
    ...
    void updateAudioRoutes(AudioRoutesInfo newRoutes) {
      ...
      if (newRoutes.mainType != this.mCurAudioRoutesInfo.mainType) {
        ...
        if ((newRoutes.mainType & 2) != 0 || (newRoutes.mainType & 1) != 0) {
          name = C4337R.string.default_audio_route_name_headphones;
        } else if ((newRoutes.mainType & 4) != 0) {
          name = C4337R.string.default_audio_route_name_dock_speakers;
        } else {
          int name2 = newRoutes.mainType;
          if ((name2 & 8) != 0) {
            name = C4337R.string.default_audio_route_name_external_device;
          } else {
            int name3 = newRoutes.mainType;
            if ((name3 & 16) != 0) {
              name = C4337R.string.default_audio_route_name_usb;
            } else {
              name = C4337R.string.default_audio_route_name;
            }
          }
        }
      ...
      if (!TextUtils.equals(newRoutes.bluetoothName, this.mCurAudioRoutesInfo.bluetoothName)) {
        ...
        if (newRoutes.bluetoothName != null) {
          ...
          if (routeInfo2 == null) {
            ...
            info.mDescription =
                this.mResources.getText(C4337R.string.bluetooth_a2dp_audio_route_name);
            info.mSupportedTypes = 1;
            info.mDeviceType = 3;
            info.mGlobalRouteId =
                MediaRouter.sStatic.mResources.getString(
                    C4337R.string.bluetooth_a2dp_audio_route_id);
        ...
        audioRoutesChanged = true;
      } else if (TextUtils.equals(newRoutes.bluetoothName, this.mCurAudioRoutesInfo.bluetoothName)
          && this.mSelectedRoute != this.mBluetoothA2dpRoute
          && isBluetoothA2dpOn()) {
        Log.m98i(MediaRouter.TAG, "force audioRouteschanged true ");
        audioRoutesChanged = true;
      }
      if (audioRoutesChanged) {
        Log.m100v(
            MediaRouter.TAG,
            "Audio routes updated: " + newRoutes + ", a2dp=" + isBluetoothA2dpOn());
      ...
      if (idx < 0) {
        ...
        try {
          try {
            ...
            Log.m97e(MediaRouter.TAG, "Error getting local stream volume", e);
            return volume;
          }
        } catch (Throwable th) {
          return volume;
    ...
    void updateDiscoveryRequest() {
      ...
      if (routeTypes != this.mDiscoveryRequestRouteTypes
          || activeScan != this.mDiscoverRequestActiveScan) {
    ...
    public Display[] getAllPresentationDisplays() {
      try {
        ...
        Log.m97e(MediaRouter.TAG, "Unable to get displays.", ex);
        return null;
      }
    }
    ...
    private void updatePresentationDisplays(int changedDisplayId) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (route.updatePresentationDisplay()
            || (route.mPresentationDisplay != null
    ...
    void handleGroupRouteSelected(String routeId) {
      RouteInfo routeToSelect =
          isBluetoothA2dpOn() ? this.mBluetoothA2dpRoute : this.mDefaultAudioVideo;
      int count = this.mRoutes.size();
      for (int i = 0; i < count; i++) {
        RouteInfo route = this.mRoutes.get(i);
    ...
    void rebindAsUser(int userId) {
      if (this.mCurrentUserId != userId || userId < 0 || this.mClient == null) {
        ...
        try {
          ...
          Log.m97e(MediaRouter.TAG, "Unable to register media router client.", ex2);
        }
        publishClientDiscoveryRequest();
    ...
    void publishClientDiscoveryRequest() {
      ...
      if (iMediaRouterClient != null) {
        try {
          this.mMediaRouterService.setDiscoveryRequest(
              iMediaRouterClient,
              this.mDiscoveryRequestRouteTypes,
              this.mDiscoverRequestActiveScan);
        } catch (RemoteException ex) {
          ex.rethrowFromSystemServer();
        }
      }
    ...
    void publishClientSelectedRoute(boolean explicit) {
      ...
      if (iMediaRouterClient != null) {
        try {
          ...
          RouteInfo routeInfo = this.mSelectedRoute;
          iMediaRouterService.setSelectedRoute(
              iMediaRouterClient, routeInfo != null ? routeInfo.mGlobalRouteId : null, explicit);
        } catch (RemoteException ex) {
          ex.rethrowFromSystemServer();
        }
      }
    ...
    void updateClientState() {
      ...
      ArrayList<MediaRouterClientState.RouteInfo> globalRoutes =
          mediaRouterClientState != null ? mediaRouterClientState.routes : null;
      ...
      for (int i = 0; i < globalRouteCount; i++) {
        ...
        RouteInfo route = findGlobalRoute(globalRoute.f288id);
        if (route == null) {
          ...
          if (!route2.isDefault() && !route2.isBluetooth() && globalRouteId != null) {
            ...
            while (true) {
              if (j < globalRouteCount) {
                if (globalRouteId.equals(globalRoutes.get(j).f288id)) {
                  break;
                } else {
                  j++;
                }
    ...
    void requestSetVolume(RouteInfo route, int volume) {
      ...
      if (route.mGlobalRouteId != null && (iMediaRouterClient = this.mClient) != null) {
        try {
          this.mMediaRouterService.requestSetVolume(
              iMediaRouterClient, route.mGlobalRouteId, volume);
        } catch (RemoteException ex) {
          ex.rethrowFromSystemServer();
        }
      }
    ...
    void requestUpdateVolume(RouteInfo route, int direction) {
      ...
      if (route.mGlobalRouteId != null && (iMediaRouterClient = this.mClient) != null) {
        try {
          this.mMediaRouterService.requestUpdateVolume(
              iMediaRouterClient, route.mGlobalRouteId, direction);
        } catch (RemoteException ex) {
          ex.rethrowFromSystemServer();
        }
      }
    ...
    RouteInfo makeGlobalRoute(MediaRouterClientState.RouteInfo globalRoute) {
      ...
      route.mGlobalRouteId = globalRoute.f288id;
    ...
    final class Client extends IMediaRouterClient.Stub {
      ...
      public void onStateChanged() {
        Static.this.mHandler.post(
            new Runnable() { // from class: android.media.MediaRouter.Static.Client.1
              @Override // java.lang.Runnable
              public void run() {
      ...
      public /* synthetic */ void lambda$onRestoreRoute$0() {
        if (this == Static.this.mClient && Static.this.mSelectedRoute != null) {
          if (!Static.this.mSelectedRoute.isDefault()
              && !Static.this.mSelectedRoute.isBluetooth()) {
            return;
          }
          if (Static.this.mSelectedRoute.isDefault() && Static.this.mBluetoothA2dpRoute != null) {
            Log.m94d(
                MediaRouter.TAG,
                "onRestoreRoute() : selectedRoute="
                    + Static.this.mSelectedRoute
                    + ", a2dpRoute="
                    + Static.this.mBluetoothA2dpRoute);
          } else {
            Log.m94d(MediaRouter.TAG, "onRestoreRoute() : route=" + Static.this.mSelectedRoute);
          }
  ...
  public void removeCallback(Callback cb) {
    ...
    if (index >= 0) {
      ...
      Log.m102w(TAG, "removeCallback(" + cb + "): callback not registered");
    }
  }
  ...
  static void selectRouteStatic(int types, RouteInfo route, boolean explicit) {
    Log.m100v(TAG, "Selecting route: " + route);
    RouteInfo oldRoute = sStatic.mSelectedRoute;
    RouteInfo currentSystemRoute =
        sStatic.isBluetoothA2dpOn() ? sStatic.mBluetoothA2dpRoute : sStatic.mDefaultAudioVideo;
    boolean wasDefaultOrBluetoothRoute =
        oldRoute != null && (oldRoute.isDefault() || oldRoute.isBluetooth());
    ...
    if (!route.matchesTypes(types)) {
      Log.m102w(
          TAG,
          "selectRoute ignored; cannot select route with supported types "
              + typesToString(route.getSupportedTypes())
              + " into route types "
              + typesToString(types));
      return;
    }
    if (sStatic.isPlaybackActive()
      ...
      try {
        ...
        Log.m97e(TAG, "Error changing Bluetooth A2DP state", e);
      }
    } else {
      Log.m98i(
          TAG,
          "Skip setBluetoothA2dpOn(): types="
              + types
              + ", isPlaybackActive()="
              + sStatic.isPlaybackActive()
              + ", BT route="
              + sStatic.mBluetoothA2dpRoute);
    }
    WifiDisplay activeDisplay = sStatic.mDisplayService.getWifiDisplayStatus().getActiveDisplay();
    boolean oldRouteHasAddress =
        (oldRoute == null || oldRoute.mDeviceAddress == null) ? false : true;
    ...
    if (activeDisplay != null || oldRouteHasAddress || newRouteHasAddress) {
      if (newRouteHasAddress && !matchesDeviceAddress(activeDisplay, route)) {
        if (sStatic.mCanConfigureWifiDisplays) {
          ...
          Log.m96e(
              TAG, "Cannot connect to wifi displays because this process is not allowed to do so.");
        }
      } else if (activeDisplay != null
          && !newRouteHasAddress
          && oldRouteHasAddress
          && activeDisplay.getDeviceAddress().equals(oldRoute.mDeviceAddress)) {
  ...
  static void addRouteStatic(RouteInfo info) {
    if (DEBUG) {
      Log.m94d(TAG, "Adding route: " + info);
    }
  ...
  static void removeRouteStatic(RouteInfo info) {
    if (DEBUG) {
      Log.m94d(TAG, "Removing route: " + info);
    }
  ...
  static void dispatchRouteSelected(int type, RouteInfo info) {
    ...
    while (it.hasNext()) {
      ...
      if (cbi.filterRouteEvent(info)) {
        cbi.f287cb.onRouteSelected(cbi.router, type, info);
      }
    }
  }
  ...
  static void dispatchRouteUnselected(int type, RouteInfo info) {
    ...
    while (it.hasNext()) {
      CallbackInfo cbi = it.next();
      if (cbi.filterRouteEvent(info)) {
        cbi.f287cb.onRouteUnselected(cbi.router, type, info);
      }
    }
  }
  ...
  static void dispatchRouteChanged(RouteInfo info, int oldSupportedTypes) {
    if (DEBUG) {
      Log.m94d(TAG, "Dispatching route change: " + info);
    }
    ...
    while (it.hasNext()) {
      ...
      if (!oldVisibility && newVisibility) {
        cbi.f287cb.onRouteAdded(cbi.router, info);
        if (info.isSelected()) {
          cbi.f287cb.onRouteSelected(cbi.router, newSupportedTypes, info);
        }
      }
      if (oldVisibility || newVisibility) {
        cbi.f287cb.onRouteChanged(cbi.router, info);
      }
      if (oldVisibility && !newVisibility) {
        if (info.isSelected()) {
          cbi.f287cb.onRouteUnselected(cbi.router, oldSupportedTypes, info);
        }
        cbi.f287cb.onRouteRemoved(cbi.router, info);
      }
    }
  }
  ...
  static void dispatchRouteGrouped(RouteInfo info, RouteGroup group, int index) {
    ...
    while (it.hasNext()) {
      ...
      if (cbi.filterRouteEvent(group)) {
        cbi.f287cb.onRouteGrouped(cbi.router, info, group, index);
      }
    }
  }
  ...
  static void dispatchRouteUngrouped(RouteInfo info, RouteGroup group) {
    ...
    while (it.hasNext()) {
      CallbackInfo cbi = it.next();
      if (cbi.filterRouteEvent(group)) {
        cbi.f287cb.onRouteUngrouped(cbi.router, info, group);
      }
    }
  }
  ...
  static void dispatchRouteVolumeChanged(RouteInfo info) {
    ...
    while (it.hasNext()) {
      CallbackInfo cbi = it.next();
      if (cbi.filterRouteEvent(info)) {
        cbi.f287cb.onRouteVolumeChanged(cbi.router, info);
      }
    }
  }
  ...
  static void dispatchRoutePresentationDisplayChanged(RouteInfo info) {
    ...
    while (it.hasNext()) {
      CallbackInfo cbi = it.next();
      if (cbi.filterRouteEvent(info)) {
        cbi.f287cb.onRoutePresentationDisplayChanged(cbi.router, info);
      }
    }
  }
  ...
  static void systemVolumeChanged(int newValue) {
    ...
    if (selectedRoute.isBluetooth() || selectedRoute.isDefault()) {
      ...
      dispatchRouteVolumeChanged(
          sStatic.mIsBluetoothA2dpOn ? sStatic.mBluetoothA2dpRoute : sStatic.mDefaultAudioVideo);
    } else {
      ...
      if (shouldShowWifiDisplay(d2, activeDisplay)) {
        ...
        if (route == null) {
          ...
          boolean disconnected =
              !address.equals(activeDisplayAddress)
    ...
    while (true) {
      ...
      if (i > 0) {
        ...
        if (route2.mDeviceAddress != null
            && ((d = findWifiDisplay(displays, route2.mDeviceAddress)) == null
                || !shouldShowWifiDisplay(d, activeDisplay))) {
  ...
  static int getWifiDisplayStatusCode(WifiDisplay d, WifiDisplayStatus wfdStatus) {
    ...
    if (d.equals(wfdStatus.getActiveDisplay())) {
      ...
      switch (activeState) {
        case 0:
          Log.m96e(TAG, "Active display is not connected!");
          break;
      }
      return newStatus;
    }
  ...
  static RouteInfo makeWifiDisplayRoute(WifiDisplay display, WifiDisplayStatus wfdStatus) {
    ...
    newRoute.mDescription =
        sStatic.mResources.getText(C4337R.string.wireless_display_route_description);
  ...
  private static void updateWifiDisplayRoute(
      RouteInfo route, WifiDisplay display, WifiDisplayStatus wfdStatus, boolean disconnected) {
    boolean changed = false;
  ...
  public static class RouteInfo {
    ...
    final IRemoteVolumeObserver.Stub mRemoteVolObserver =
        new IRemoteVolumeObserver.Stub() { // from class: android.media.MediaRouter.RouteInfo.1
          @Override // android.media.IRemoteVolumeObserver
          public void dispatchRemoteVolumeUpdate(final int direction, final int value) {
            MediaRouter.sStatic.mHandler.post(
                new Runnable() { // from class: android.media.MediaRouter.RouteInfo.1.1
                  @Override // java.lang.Runnable
                  public void run() {
                    if (RouteInfo.this.mVcb != null) {
                      if (direction != 0) {
                        RouteInfo.this.mVcb.vcb.onVolumeUpdateRequest(
                            RouteInfo.this.mVcb.route, direction);
                      } else {
                        RouteInfo.this.mVcb.vcb.onVolumeSetRequest(
                            RouteInfo.this.mVcb.route, value);
                      }
                    }
                  }
                });
    ...
    boolean resolveStatusCode() {
      ...
      switch (statusCode) {
        case 1:
          resId = C4337R.string.media_route_status_scanning;
          break;
        case 2:
          resId = C4337R.string.media_route_status_connecting;
          break;
        case 3:
          resId = C4337R.string.media_route_status_available;
          break;
        case 4:
          resId = C4337R.string.media_route_status_not_available;
          break;
        case 5:
          resId = C4337R.string.media_route_status_in_use;
          break;
        default:
          resId = 0;
          break;
    ...
    public void requestSetVolume(int volume) {
      if (this.mPlaybackType == 0) {
        try {
          MediaRouter.sStatic.mAudioService.setStreamVolumeWithAttribution(
              this.mPlaybackStream, volume, 0, ActivityThread.currentPackageName(), null);
          return;
        } catch (RemoteException e) {
          Log.m97e(MediaRouter.TAG, "Error setting local stream volume", e);
          return;
        }
      }
    ...
    private Display choosePresentationDisplay() {
      Display[] displays;
      if ((getSupportedTypes() & 2) == 0
          || (displays = getAllPresentationDisplays()) == null
          || displays.length == 0) {
        return null;
      }
  ...
  public static class UserRouteInfo extends RouteInfo {
    ...
    public void requestSetVolume(int volume) {
      if (this.mVolumeHandling == 1) {
        if (this.mVcb == null) {
          Log.m96e(
              MediaRouter.TAG, "Cannot requestSetVolume on user route - no volume callback set");
        } else {
    ...
    public void requestUpdateVolume(int direction) {
      if (this.mVolumeHandling == 1) {
        if (this.mVcb == null) {
          Log.m96e(
              MediaRouter.TAG,
              "Cannot requestChangeVolume on user route - no volumec callback set");
        } else {
    ...
    private void configureSessionVolume() {
      ...
      if (remoteControlClient == null) {
        if (MediaRouter.DEBUG) {
          Log.m94d(MediaRouter.TAG, "No Rcc to configure volume for route " + ((Object) getName()));
          return;
        }
        return;
      }
      ...
      if (session == null) {
        if (MediaRouter.DEBUG) {
          Log.m94d(MediaRouter.TAG, "Rcc has no session to configure volume");
          return;
        }
        return;
      }
      if (this.mPlaybackType == 1) {
        ...
        SessionVolumeProvider sessionVolumeProvider = this.mSvp;
        if (sessionVolumeProvider == null
            || sessionVolumeProvider.getVolumeControl() != volumeControl
            || this.mSvp.getMaxVolume() != this.mVolumeMax) {
          SessionVolumeProvider sessionVolumeProvider2 =
              new SessionVolumeProvider(volumeControl, this.mVolumeMax, this.mVolume);
    ...
    class SessionVolumeProvider extends VolumeProvider {
      ...
      public void onSetVolumeTo(final int volume) {
        MediaRouter.sStatic.mHandler.post(
            new Runnable() { // from class:
                             // android.media.MediaRouter.UserRouteInfo.SessionVolumeProvider.1
              @Override // java.lang.Runnable
              public void run() {
                if (UserRouteInfo.this.mVcb != null) {
                  UserRouteInfo.this.mVcb.vcb.onVolumeSetRequest(
                      UserRouteInfo.this.mVcb.route, volume);
                }
              }
            });
      }
      ...
      public void onAdjustVolume(final int direction) {
        MediaRouter.sStatic.mHandler.post(
            new Runnable() { // from class:
                             // android.media.MediaRouter.UserRouteInfo.SessionVolumeProvider.2
              @Override // java.lang.Runnable
              public void run() {
                if (UserRouteInfo.this.mVcb != null) {
                  UserRouteInfo.this.mVcb.vcb.onVolumeUpdateRequest(
                      UserRouteInfo.this.mVcb.route, direction);
                }
              }
            });
      }
    ...
    public void addRoute(RouteInfo route) {
      ...
      if (route.getCategory() != this.mCategory) {
        throw new IllegalArgumentException(
            "Route cannot be added to a group with a different category. (Route category="
                + route.getCategory()
                + " group category="
                + this.mCategory
  ...
  public static class RouteCategory {
    ...
    public String toString() {
      return "RouteCategory{ name="
          + ((Object) getName())
          + " types="
          + MediaRouter.typesToString(this.mTypes)
          + " groupable="
          + this.mGroupable
          + " }";
    }
  ...
  public abstract static class Callback {
    ...
    public abstract void onRouteGrouped(
        MediaRouter mediaRouter, RouteInfo routeInfo, RouteGroup routeGroup, int i);
    ...
    public abstract void onRouteUngrouped(
        MediaRouter mediaRouter, RouteInfo routeInfo, RouteGroup routeGroup);
  ...
  static class WifiDisplayStatusChangedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      if (intent
          .getAction()
          .equals("android.hardware.display.action.WIFI_DISPLAY_STATUS_CHANGED")) {
        MediaRouter.updateWifiDisplayStatus(
            (WifiDisplayStatus)
                intent.getParcelableExtra("android.hardware.display.extra.WIFI_DISPLAY_STATUS"));
      }
    }
  }
  ...
  static boolean isAudioPathA2DPStatic() {
    ...
    if (sStatic.mSelectedRoute != null
        && (playbackStream = sStatic.mSelectedRoute.getPlaybackStream()) >= 0
        && playbackStream <= AudioSystem.getNumStreamTypes()) {
