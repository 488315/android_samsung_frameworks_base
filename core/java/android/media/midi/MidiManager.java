...
public final class MidiManager {
  ...
  class DeviceListener extends IMidiDeviceListener.Stub {
    ...
    private boolean shouldInvokeCallback(MidiDeviceInfo device) {
      ...
      Log.m96e(MidiManager.TAG, "Invalid transport type: " + this.mTransport);
      return false;
    }
  }
  ...
  public void openBluetoothDevice(
      ...
      final OnDeviceOpenedListener listener,
      final Handler handler) {
    Log.m94d(TAG, "openBluetoothDevice() " + bluetoothDevice);
    IMidiDeviceOpenCallback callback =
        new IMidiDeviceOpenCallback.Stub() { // from class: android.media.midi.MidiManager.3
          @Override // android.media.midi.IMidiDeviceOpenCallback
          public void onDeviceOpened(IMidiDeviceServer server, IBinder deviceToken) {
            Log.m94d(MidiManager.TAG, "onDeviceOpened() server:" + server);
            ...
            if (server != null) {
              try {
                ...
                Log.m96e(MidiManager.TAG, "remote exception in getDeviceInfo()");
              }
            }
            MidiManager.this.sendOpenDeviceResponse(device, listener, handler);
          }
  ...
  public void closeBluetoothDevice(MidiDevice midiDevice) {
    try {
      ...
      Log.m96e(TAG, "Exception closing BLE-MIDI device" + ex);
    }
  }
    ...
    try {
      try {
        MidiDeviceServer server =
            new MidiDeviceServer(this.mService, inputPortReceivers, numOutputPorts, callback);
        MidiDeviceInfo deviceInfo =
            this.mService.registerDeviceServer(
                ...
                defaultProtocol);
        if (deviceInfo == null) {
          Log.m96e(TAG, "registerVirtualDevice failed");
          return null;
        }
        return server;
      } catch (RemoteException e) {
