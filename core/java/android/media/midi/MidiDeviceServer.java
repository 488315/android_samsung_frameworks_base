...
public final class MidiDeviceServer implements Closeable {
  ...
  MidiDeviceServer(
        ...
        new IMidiDeviceServer.Stub() { // from class: android.media.midi.MidiDeviceServer.1
          ...
          public FileDescriptor openInputPort(IBinder token, int portNumber) {
            ...
            if (portNumber < 0 || portNumber >= MidiDeviceServer.this.mInputPortCount) {
              Log.m96e(
                  MidiDeviceServer.TAG, "portNumber out of range in openInputPort: " + portNumber);
              return null;
            }
            synchronized (MidiDeviceServer.this.mInputPortOutputPorts) {
              if (MidiDeviceServer.this.mInputPortOutputPorts[portNumber] != null) {
                Log.m94d(MidiDeviceServer.TAG, "port " + portNumber + " already open");
                return null;
              }
              try {
                ...
                InputPortClient client =
                    MidiDeviceServer.this.new InputPortClient(token, outputPort);
                ...
                Log.m96e(MidiDeviceServer.TAG, "unable to create FileDescriptors in openInputPort");
                return null;
              }
            }
          }
          ...
          public FileDescriptor openOutputPort(IBinder token, int portNumber) {
            ...
            if (portNumber < 0 || portNumber >= MidiDeviceServer.this.mOutputPortCount) {
              Log.m96e(
                  MidiDeviceServer.TAG, "portNumber out of range in openOutputPort: " + portNumber);
              return null;
            }
            try {
              FileDescriptor[] pair = MidiDeviceServer.createSeqPacketSocketPair();
              ...
              Log.m96e(MidiDeviceServer.TAG, "unable to create FileDescriptors in openOutputPort");
              return null;
            }
          }
    ...
    this.mInputPortFailureHandler =
        new MidiDispatcher
            .MidiReceiverFailureHandler() { // from class: android.media.midi.MidiDeviceServer.2
          ...
          public void onReceiverFailure(MidiReceiver receiver, IOException failure) {
            PortClient client;
            Log.m97e(MidiDeviceServer.TAG, "MidiInputPort failed to send data", failure);
            synchronized (MidiDeviceServer.this.mInputPortClients) {
    ...
    try {
      try {
        MidiDeviceStatus status =
            new MidiDeviceStatus(this.mDeviceInfo, this.mInputPortOpen, this.mOutputPortOpenCount);
        ...
        Log.m96e(TAG, "RemoteException in updateDeviceStatus");
      }
    } finally {
  ...
  public void close() throws IOException {
    synchronized (this.mGuard) {
      ...
      try {
        ...
        Log.m96e(TAG, "RemoteException in unregisterDeviceServer");
      }
      this.mIsClosed = true;
    }
  }
  ...
  public void updateTotalBytes() {
    try {
      ...
      Log.m96e(TAG, "RemoteException in updateTotalBytes");
    }
  }
}
