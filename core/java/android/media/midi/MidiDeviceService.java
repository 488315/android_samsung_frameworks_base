...
public abstract class MidiDeviceService extends Service {
  ...
  private final MidiDeviceServer.Callback mCallback =
      new MidiDeviceServer.Callback() { // from class: android.media.midi.MidiDeviceService.1
  ...
  public void onCreate() {
    ...
    try {
      ...
      Log.m96e(TAG, "RemoteException in IMidiManager.getServiceDeviceInfo");
      server = null;
    }
    if (deviceInfo == null) {
      Log.m96e(TAG, "Could not find MidiDeviceInfo for MidiDeviceService " + this);
      return;
    }
    ...
    server =
        new MidiDeviceServer(this.mMidiManager, inputPortReceivers, deviceInfo, this.mCallback);
    this.mServer = server;
  }
