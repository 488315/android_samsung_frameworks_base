...
class WebVttRenderingWidget extends ViewGroup implements SubtitleTrack.RenderingWidget {
  ...
  public WebVttRenderingWidget(
    ...
    this.mCaptioningListener =
        new CaptioningManager
            .CaptioningChangeListener() { // from class: android.media.WebVttRenderingWidget.1
  ...
  public void setActiveCues(Vector<SubtitleTrack.Cue> activeCues) {
    ...
    if (width > 0 && height > 0) {
      ...
      Log.m102w(getClass().getName(), "setActiveCues width : " + width + ", height : " + height);
    }
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    setCaptionStyle(
        this.mCaptionStyle, this.mManager.getFontScale() * LINE_HEIGHT_RATIO * viewportHeight);
    int regionCount = this.mRegionBoxes.size();
    for (int i = 0; i < regionCount; i++) {
      RegionLayout regionBox = this.mRegionBoxes.valueAt(i);
  ...
  private static class CueLayout extends LinearLayout {
    ...
    public void update() {
      ...
      removeAllViews();
      int cueAlignment =
          WebVttRenderingWidget.resolveCueAlignment(getLayoutDirection(), this.mCue.mAlignment);
      switch (cueAlignment) {
        case 203:
