...
import android.media.p008tv.ITvInputService;
...
public abstract class TvInputService extends Service {
        ...
        private final class TimeShiftPositionTrackingRunnable implements Runnable {
            ...
            public void run() {
                ...
                if (currentPositionMs < Session.this.mStartPositionMs) {
                    Log.m102w(TvInputService.TAG, "Current position (" + currentPositionMs + ") cannot be earlier than start position (" + Session.this.mStartPositionMs + "). Reset to the start position.");
        ...
        public Void doInBackground(View... views) {
            ...
            try {
                ...
                if (!isCancelled() && overlayViewParent.isAttachedToWindow()) {
                    Log.m96e(TvInputService.TAG, "Time out on releasing overlay view. Killing " + overlayViewParent.getContext().getPackageName());
    ...
    public static abstract class RecordingSession {
        ...
        public void notifyError(int error) {
            if (error < 0 || error > 2) {
                Log.m102w(TvInputService.TAG, "notifyError - invalid error code (" + error + ") is changed to RECORDING_ERROR_UNKNOWN.");
                error = 0;
            }
