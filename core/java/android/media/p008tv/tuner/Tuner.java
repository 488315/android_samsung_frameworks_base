...
import android.media.p008tv.tuner.Tuner;
...
public class Tuner implements AutoCloseable {
    ...
    public Tuner(Context context, String tvInputSessionId, int useCase) {
        ...
        if (nativeGetTunerVersion == 0) {
            ...
            Log.m94d(TAG, "Current Tuner version is " + TunerVersionChecker.getMajorVersion(sTunerVersion) + MediaMetrics.SEPARATOR + TunerVersionChecker.getMinorVersion(sTunerVersion) + MediaMetrics.SEPARATOR);
        }
        ...
        try {
            if (src == null) {
                ...
                if (DEBUG) {
                    ...
                    Log.m94d(TAG, "mFrontendCiCamHandle:" + src.mFrontendCiCamHandle + ", mFrontendCiCamId:" + src.mFrontendCiCamId);
                }
    ...
    private void releaseFrontend() {
        ...
        try {
            if (this.mFrontendHandle != null) {
                ...
                if (z) {
                    Log.m94d(TAG, "call TRM#releaseFrontend :" + this.mFrontendHandle + ", " + this.mClientId);
                }
    ...
    private void releaseCiCam() {
        ...
        try {
            if (this.mFrontendCiCamHandle != null) {
                if (DEBUG) {
                    Log.m94d(TAG, "unlinking CiCam : " + this.mFrontendCiCamHandle + " for " + this.mClientId);
                }
                ...
                if (result == 0) {
                    ...
                    Log.m96e(TAG, "nativeUnlinkCiCam(" + this.mFrontendCiCamHandle + ") for mClientId:" + this.mClientId + "failed with result:" + result);
                }
            } else if (DEBUG) {
    ...
    public int tune(FrontendSettings settings) {
        ...
        try {
            ...
            if (this.mFrontendHandle != null && type != this.mFrontendType) {
                Log.m96e(TAG, "Frontend was opened with type " + this.mFrontendType + ", new type is " + type);
                return 3;
            }
    ...
    public int getMaxNumberOfFrontends(int frontendType) {
        ...
        if (maxNumFromHAL != maxNumFromTRM) {
            Log.m102w(TAG, "max num of usable frontend is out-of-sync b/w " + maxNumFromHAL + " != " + maxNumFromTRM);
        }
    ...
    public int applyFrontend(FrontendInfo desiredFrontendInfo) {
        ...
        try {
            ...
            if (DEBUG) {
                Log.m94d(TAG, "Applying frontend with type " + this.mFrontendType + ", id " + this.mDesiredFrontendId);
            }
            if (checkResource(0, this.mFrontendLock)) {
                return 0;
            }
    ...
    public int configureDemux(DemuxInfo desiredDemuxInfo) {
        ...
        TunerVersionChecker.getMajorVersion(sTunerVersion);
        if (sTunerVersion < 196608) {
            Log.m96e(TAG, "configureDemux() is not supported for tuner version:" + TunerVersionChecker.getMajorVersion(sTunerVersion) + MediaMetrics.SEPARATOR + TunerVersionChecker.getMinorVersion(sTunerVersion) + MediaMetrics.SEPARATOR);
            return 1;
        }
    ...
    private int configureDemuxInternal(DemuxInfo desiredDemuxInfo, boolean reduceDesiredFilterTypes) {
        ...
        if (filterCapsList.length <= 0) {
            Log.m96e(TAG, "configureDemuxInternal: getFilterTypeCapabilityList() returned an empty array");
            return 1;
        }
        ...
        if (!supported) {
            Log.m96e(TAG, "configureDemuxInternal: requested caps:" + desiredFilterTypes + " is not supported by the system");
            return 1;
        }
        Integer num = this.mDemuxHandle;
    ...
    private void acquireTRMSLock(String functionNameForLog) {
        if (DEBUG) {
            Log.m94d(TAG, "ATTEMPT:acquireLock() in " + functionNameForLog + "for clientId:" + this.mClientId);
        }
        if (!this.mTunerResourceManager.acquireLock(this.mClientId)) {
            Log.m96e(TAG, "FAILED:acquireLock() in " + functionNameForLog + " for clientId:" + this.mClientId + " - this can cause deadlock between Tuner API calls and onReclaimResources()");
        }
    }
