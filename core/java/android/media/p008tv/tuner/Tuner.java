...
import android.media.p008tv.tuner.dvr.DvrPlayback;
import android.media.p008tv.tuner.dvr.DvrRecorder;
import android.media.p008tv.tuner.dvr.OnPlaybackStatusChangedListener;
import android.media.p008tv.tuner.dvr.OnRecordStatusChangedListener;
import android.media.p008tv.tuner.filter.Filter;
import android.media.p008tv.tuner.filter.FilterCallback;
import android.media.p008tv.tuner.filter.SharedFilter;
import android.media.p008tv.tuner.filter.SharedFilterCallback;
import android.media.p008tv.tuner.filter.TimeFilter;
import android.media.p008tv.tuner.frontend.Atsc3PlpInfo;
import android.media.p008tv.tuner.frontend.FrontendInfo;
import android.media.p008tv.tuner.frontend.FrontendSettings;
import android.media.p008tv.tuner.frontend.FrontendStatus;
import android.media.p008tv.tuner.frontend.FrontendStatusReadiness;
import android.media.p008tv.tuner.frontend.OnTuneEventListener;
import android.media.p008tv.tuner.frontend.ScanCallback;
import android.media.p008tv.tunerresourcemanager.ResourceClientProfile;
import android.media.p008tv.tunerresourcemanager.TunerCiCamRequest;
import android.media.p008tv.tunerresourcemanager.TunerDemuxRequest;
import android.media.p008tv.tunerresourcemanager.TunerDescramblerRequest;
import android.media.p008tv.tunerresourcemanager.TunerFrontendRequest;
import android.media.p008tv.tunerresourcemanager.TunerLnbRequest;
...
public class Tuner implements AutoCloseable {
  ...
  static {
    try {
      ...
      Log.m94d(TAG, "tuner JNI library not found!");
    }
    sTunerVersion = 0;
  }

  public Tuner(Context context, String tvInputSessionId, int useCase) {
    TunerResourceManager.ResourcesReclaimListener resourcesReclaimListener =
        new TunerResourceManager
            .ResourcesReclaimListener() { // from class: android.media.tv.tuner.Tuner.1
    ...
    TunerResourceManager tunerResourceManager =
        (TunerResourceManager) context.getSystemService(TunerResourceManager.class);
    ...
    if (nativeGetTunerVersion == 0) {
      Log.m96e(TAG, "Unknown Tuner version!");
    } else {
      Log.m94d(
          TAG,
          "Current Tuner version is "
    ...
    tunerResourceManager.registerClientProfile(
        profile, new PendingIntent$$ExternalSyntheticLambda1(), resourcesReclaimListener, clientId);
  ...
  private FrontendInfo[] getFrontendInfoListInternal() {
    ...
    for (int i = 0; i < ids.size(); i++) {
      ...
      if (frontendInfo == null) {
        Log.m96e(TAG, "Failed to get a FrontendInfo on frontend id:" + id + "!");
      } else {
        ...
        Arrays.stream(infos)
            .filter(
                new Predicate() { // from class:
                                  // android.media.tv.tuner.Tuner$$ExternalSyntheticLambda9
            ...
            .toArray(
                new IntFunction() { // from class:
                                    // android.media.tv.tuner.Tuner$$ExternalSyntheticLambda10
  ...
  private void replicateFrontendSettings(Tuner src) {
    ...
    try {
      if (src == null) {
        if (DEBUG) {
          Log.m94d(TAG, "resetting Frontend params for " + this.mClientId);
        }
        this.mFrontend = null;
        ...
        if (DEBUG) {
          Log.m94d(TAG, "copying Frontend params from " + src.mClientId + " to " + this.mClientId);
        }
  ...
  private void replicateCiCamSettings(Tuner src) {
    ...
    try {
      if (src == null) {
        if (DEBUG) {
          Log.m94d(TAG, "resetting CiCamParams: " + this.mClientId);
        }
        this.mFrontendCiCamHandle = null;
        this.mFrontendCiCamId = null;
      } else {
        if (DEBUG) {
          Log.m94d(TAG, "copying CiCamParams from " + src.mClientId + " to " + this.mClientId);
          Log.m94d(
              TAG,
              "mFrontendCiCamHandle:"
                  + src.mFrontendCiCamHandle
                  + ", mFrontendCiCamId:"
                  + src.mFrontendCiCamId);
        }
  ...
  private void replicateLnbSettings(Tuner src) {
    ...
    try {
      if (src == null) {
        if (DEBUG) {
          Log.m94d(TAG, "resetting Lnb params");
        }
        this.mLnb = null;
        this.mLnbHandle = null;
      } else {
        if (DEBUG) {
          Log.m94d(TAG, "copying Lnb params from " + src.mClientId + " to " + this.mClientId);
        }
        this.mLnb = src.mLnb;
  ...
  private boolean isFrontendOwner() {
    ...
    Log.m96e(TAG, "transferOwner() - cannot be called on the non-owner");
    return false;
  }

  private boolean isNewOwnerQualifiedForTransfer(Tuner newOwner) {
    boolean newOwnerIsTheCurrentSharee =
        newOwner.mFeOwnerTuner == this && newOwner.mFrontendHandle.equals(this.mFrontendHandle);
    if (!newOwnerIsTheCurrentSharee) {
      Log.m96e(TAG, "transferOwner() - new owner must be the current sharee");
      return false;
    }
    boolean newOwnerAlreadyHoldsToBeSharedResource =
        (newOwner.mFrontendCiCamHandle == null && newOwner.mLnb == null) ? false : true;
    if (!newOwnerAlreadyHoldsToBeSharedResource) {
      return true;
    }
    Log.m96e(TAG, "transferOwner() - new owner cannot be holding CiCam nor Lnb resource");
    return false;
  }
  ...
  private void releaseFrontend() {
    boolean z = DEBUG;
    if (z) {
      Log.m94d(TAG, "Tuner#releaseFrontend");
    }
    this.mFrontendLock.lock();
    try {
      if (this.mFrontendHandle != null) {
        if (z) {
          Log.m94d(TAG, "mFrontendHandle not null");
        }
        if (this.mFeOwnerTuner != null) {
          if (z) {
            Log.m94d(TAG, "mFeOwnerTuner not null - sharee");
          }
          ...
          if (z) {
            Log.m94d(TAG, "mFeOwnerTuner null - owner");
          }
        ...
        if (z) {
          Log.m94d(
              TAG, "call TRM#releaseFrontend :" + this.mFrontendHandle + ", " + this.mClientId);
        }
  ...
  private void releaseCiCam() {
    ...
    try {
      if (this.mFrontendCiCamHandle != null) {
        if (DEBUG) {
          Log.m94d(
              TAG, "unlinking CiCam : " + this.mFrontendCiCamHandle + " for " + this.mClientId);
        }
        int result = nativeUnlinkCiCam(this.mFrontendCiCamId.intValue());
        if (result == 0) {
          this.mTunerResourceManager.releaseCiCam(
              this.mFrontendCiCamHandle.intValue(), this.mClientId);
          replicateCiCamSettings(null);
        } else {
          Log.m96e(
              TAG,
              "nativeUnlinkCiCam("
                  + this.mFrontendCiCamHandle
                  + ") for mClientId:"
                  + this.mClientId
                  + "failed with result:"
                  + result);
        ...
        Log.m94d(TAG, "NOT unlinking CiCam : " + this.mClientId);
      }
    } finally {
      this.mFrontendCiCamLock.unlock();
    }
  ...
  private void closeLnb() {
    ...
    try {
      if (this.mLnb != null) {
        if (DEBUG) {
          Log.m94d(TAG, "calling mLnb.close() : " + this.mClientId);
        }
        this.mLnb.close();
      } else if (DEBUG) {
        Log.m94d(TAG, "NOT calling mLnb.close() : " + this.mClientId);
      }
    } finally {
      this.mLnbLock.unlock();
    }
        ...
        case 1:
          synchronized (Tuner.this.mOnResourceLostListenerLock) {
            if (Tuner.this.mOnResourceLostListener != null
                && Tuner.this.mOnResourceLostListenerExecutor != null) {
              Tuner.this.mOnResourceLostListenerExecutor.execute(
                  new Runnable() { // from class:
                                   // android.media.tv.tuner.Tuner$EventHandler$$ExternalSyntheticLambda0
  ...
  public int tune(FrontendSettings settings) {
    ...
    try {
      if (this.mFeOwnerTuner != null) {
        Log.m94d(TAG, "Operation cannot be done by sharee of tuner");
        return 3;
      }
      int type = settings.getType();
      if (this.mFrontendHandle != null && type != this.mFrontendType) {
        Log.m96e(
            TAG, "Frontend was opened with type " + this.mFrontendType + ", new type is " + type);
        return 3;
      }
      Log.m94d(TAG, "Tune to " + settings.getFrequencyLong());
      this.mFrontendType = type;
      if (type == 10
          && !TunerVersionChecker.checkHigherOrEqualVersionTo(65537, "Tuner with DTMB Frontend")) {
        return 1;
      }
      if (this.mFrontendType == 11
          && !TunerVersionChecker.checkHigherOrEqualVersionTo(196608, "Tuner with IPTV Frontend")) {
        return 1;
      }
      ...
      this.mFrontendInfo = null;
      Log.m94d(TAG, "Write Stats Log for tuning.");
  ...
  public int scan(
      FrontendSettings settings, int scanType, Executor executor, ScanCallback scanCallback) {
    Executor executor2;
    this.mFrontendLock.lock();
    try {
      if (this.mFeOwnerTuner != null) {
      ...
      synchronized (this.mScanCallbackLock) {
        ...
        if ((scanCallback2 != null && scanCallback2 != scanCallback)
            || ((executor2 = this.mScanCallbackExecutor) != null && executor2 != executor)) {
          throw new IllegalStateException(
              "Different Scan session already in progress.  stopScan must be called before a new"
                  + " scan session can be started.");
        }
        int type = settings.getType();
        this.mFrontendType = type;
        if (type == 10
            && !TunerVersionChecker.checkHigherOrEqualVersionTo(65537, "Scan with DTMB Frontend")) {
          return 1;
        }
        if (this.mFrontendType == 11
            && !TunerVersionChecker.checkHigherOrEqualVersionTo(
                196608, "Tuner with IPTV Frontend")) {
          return 1;
        }
        if (!checkResource(0, this.mFrontendLock)) {
          return 1;
  ...
  public int disconnectFrontendToCiCam(int ciCamId) {
    ...
    try {
      ...
      if (TunerVersionChecker.checkHigherOrEqualVersionTo(65537, "unlinkFrontendToCiCam")) {
        this.mFrontendCiCamLock.lock();
        if (this.mFrontendCiCamHandle != null
            && (num = this.mFrontendCiCamId) != null
            && num.intValue() == ciCamId) {
  ...
  public List<FrontendStatusReadiness> getFrontendStatusReadiness(int[] statusTypes) {
    ...
    try {
      if (!TunerVersionChecker.checkHigherOrEqualVersionTo(
          131072, "Get fronted status readiness")) {
  ...
  public int getMaxNumberOfFrontends(int frontendType) {
    ...
    if (maxNumFromHAL != maxNumFromTRM) {
      Log.m102w(
          TAG,
          "max num of usable frontend is out-of-sync b/w "
              + maxNumFromHAL
              + " != "
              + maxNumFromTRM);
    }
  ...
  private void onFrontendEvent(final int eventType) {
    Log.m94d(TAG, "Got event from tuning. Event type: " + eventType + " for " + this);
    synchronized (this.mOnTuneEventLock) {
      Executor executor = this.mOnTuneEventExecutor;
      if (executor != null && this.mOnTuneEventListener != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda7
              @Override // java.lang.Runnable
              public final void run() {
    ...
    Log.m94d(TAG, "Wrote Stats Log for the events from tuning.");
    if (eventType == 0) {
  ...
  private void onLocked() {
    Log.m94d(TAG, "Wrote Stats Log for locked event from scanning.");
    FrameworkStatsLog.write(276, this.mUserId, 2);
    synchronized (this.mScanCallbackLock) {
      Executor executor = this.mScanCallbackExecutor;
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda14
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onUnlocked() {
    Log.m94d(TAG, "Wrote Stats Log for unlocked event from scanning.");
    FrameworkStatsLog.write(276, this.mUserId, 2);
    synchronized (this.mScanCallbackLock) {
      Executor executor = this.mScanCallbackExecutor;
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda15
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onScanStopped() {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda4
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onProgress(final int percent) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda12
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onFrequenciesReport(final long[] frequencies) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda3
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onSymbolRates(final int[] rate) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda11
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onHierarchy(final int hierarchy) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda16
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onSignalType(final int signalType) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda8
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onPlpIds(final int[] plpIds) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda1
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onGroupIds(final int[] groupIds) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda21
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onInputStreamIds(final int[] inputStreamIds) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda19
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onDvbsStandard(final int dvbsStandandard) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda18
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onDvbtStandard(final int dvbtStandard) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda5
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onAnalogSifStandard(final int sif) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda13
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onAtsc3PlpInfos(final Atsc3PlpInfo[] atsc3PlpInfos) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda6
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onModulationReported(final int modulation) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda20
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onPriorityReported(final boolean isHighPriority) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda0
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onDvbcAnnexReported(final int dvbcAnnex) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda2
              @Override // java.lang.Runnable
              public final void run() {
  ...
  private void onDvbtCellIdsReported(final int[] dvbtCellIds) {
    synchronized (this.mScanCallbackLock) {
      ...
      if (executor != null && this.mScanCallback != null) {
        executor.execute(
            new Runnable() { // from class: android.media.tv.tuner.Tuner$$ExternalSyntheticLambda17
              @Override // java.lang.Runnable
              public final void run() {
  ...
  public Filter openFilter(
      int mainType, int subType, long bufferSize, Executor executor, FilterCallback cb) {
    this.mDemuxLock.lock();
    try {
      TunerVersionChecker.getMajorVersion(sTunerVersion);
      if (sTunerVersion >= 196608) {
        ...
        if (res != 0) {
          Log.m96e(TAG, "openFilter called for unsupported mainType: " + mainType);
          return null;
        }
      }
      if (!checkResource(1, this.mDemuxLock)) {
      ...
      Filter filter =
          nativeOpenFilter(mainType, TunerUtils.getFilterSubtype(mainType, subType), bufferSize);
      if (filter != null) {
  ...
  public DvrRecorder openDvrRecorder(
      long bufferSize, Executor executor, OnRecordStatusChangedListener l) {
    this.mDemuxLock.lock();
    try {
      Objects.requireNonNull(executor, "executor must not be null");
  ...
  public DvrPlayback openDvrPlayback(
      long bufferSize, Executor executor, OnPlaybackStatusChangedListener l) {
    this.mDemuxLock.lock();
    try {
      Objects.requireNonNull(executor, "executor must not be null");
  ...
  public int applyFrontend(FrontendInfo desiredFrontendInfo) {
    ...
    try {
      if (this.mFeOwnerTuner != null) {
        Log.m96e(TAG, "Operation connot be done by sharee of tuner");
        return 3;
      }
      if (this.mFrontendHandle != null) {
        Log.m96e(TAG, "A frontend has been opened before");
        return 3;
      }
      ...
      if (DEBUG) {
        Log.m94d(
            TAG,
            "Applying frontend with type "
                + this.mFrontendType
                + ", id "
                + this.mDesiredFrontendId);
      }
      if (checkResource(0, this.mFrontendLock)) {
        return 0;
      }
  ...
  public static SharedFilter openSharedFilter(
      Context context, String sharedFilterToken, Executor executor, SharedFilterCallback cb) {
  ...
  public int configureDemux(DemuxInfo desiredDemuxInfo) {
    ...
    if (sTunerVersion < 196608) {
      Log.m96e(
          TAG,
          "configureDemux() is not supported for tuner version:"
              + TunerVersionChecker.getMajorVersion(sTunerVersion)
              + MediaMetrics.SEPARATOR
              + TunerVersionChecker.getMinorVersion(sTunerVersion)
              + MediaMetrics.SEPARATOR);
  ...
  private int configureDemuxInternal(DemuxInfo desiredDemuxInfo, boolean reduceDesiredFilterTypes) {
    ...
    if (caps == null) {
      Log.m96e(TAG, "configureDemuxInternal:failed to get DemuxCapabilities");
      return 1;
    }
    int[] filterCapsList = caps.getFilterTypeCapabilityList();
    if (filterCapsList.length <= 0) {
      Log.m96e(
          TAG, "configureDemuxInternal: getFilterTypeCapabilityList() returned an empty array");
      return 1;
    }
    ...
    if (!supported) {
      Log.m96e(
          TAG,
          "configureDemuxInternal: requested caps:"
              + desiredFilterTypes
    ...
    Integer num = this.mDemuxHandle;
    if (num != null
        && desiredFilterTypes != 0
        && (currentDemuxInfo = nativeGetDemuxInfo(num.intValue())) != null
        && (currentDemuxInfo.getFilterTypes() & desiredFilterTypes) != desiredFilterTypes) {
      releaseFilters();
      releaseDemux();
    }
    this.mDesiredDemuxInfo.setFilterTypes(desiredFilterTypes);
  ...
  void releaseLnb() {
    ...
    try {
      if (this.mLnbHandle != null) {
        if (DEBUG) {
          Log.m94d(TAG, "releasing Lnb");
        }
        this.mTunerResourceManager.releaseLnb(this.mLnbHandle.intValue(), this.mClientId);
        this.mLnbHandle = null;
      } else if (DEBUG) {
        Log.m94d(TAG, "NOT releasing Lnb because mLnbHandle is null");
      }
      this.mLnb = null;
    } finally {
      releaseTRMSLock();
  ...
  private void acquireTRMSLock(String functionNameForLog) {
    if (DEBUG) {
      Log.m94d(
          TAG, "ATTEMPT:acquireLock() in " + functionNameForLog + "for clientId:" + this.mClientId);
    }
    if (!this.mTunerResourceManager.acquireLock(this.mClientId)) {
      Log.m96e(
          ...
          "FAILED:acquireLock() in "
              ...
              + " for clientId:"
              + this.mClientId
              + " - this can cause deadlock between Tuner API calls and onReclaimResources()");
    }
  }
