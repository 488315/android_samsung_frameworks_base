...
public final class TunerVersionChecker {
    ...
    public static boolean checkHigherOrEqualVersionTo(int version, String methodName) {
        if (!isHigherOrEqualVersionTo(version)) {
            Log.m96e(TAG, "Current Tuner version " + getMajorVersion(Tuner.getTunerVersion()) + MediaMetrics.SEPARATOR + getMinorVersion(Tuner.getTunerVersion()) + " does not support " + methodName + MediaMetrics.SEPARATOR);
            return false;
        }
        return true;
    }
