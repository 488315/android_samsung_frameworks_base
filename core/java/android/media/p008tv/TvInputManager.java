...
import android.media.p008tv.ITvInputClient;
import android.media.p008tv.ITvInputHardwareCallback;
import android.media.p008tv.ITvInputManagerCallback;
...
public final class TvInputManager {
    ...
    private Hardware acquireTvInputHardwareInternal(int deviceId, TvInputInfo info, String tvInputSessionId, int priorityHint, Executor executor, HardwareCallback callback) {
        try {
            ITvInputHardware hardware = this.mService.acquireTvInputHardware(deviceId, new BinderC20464(executor, callback), info, this.mUserId, tvInputSessionId, priorityHint);
            if (hardware == null) {
                return null;
            }
    ...
    public static final class Session {
        ...
        private int sendInputEventOnMainLooperLocked(PendingEvent p) {
            ...
            if (inputChannel != null) {
                ...
                Log.m102w(TvInputManager.TAG, "Unable to send input event to session: " + this.mToken + " dropping:" + event);
                return 0;
            }
            return 0;
        }
        ...
        void finishedInputEvent(int seq, boolean handled, boolean timeout) {
            synchronized (this.mHandler) {
                ...
                if (timeout) {
                    Log.m102w(TvInputManager.TAG, "Timeout waiting for session to handle input event after 2500 ms: " + this.mToken);
                } else {
