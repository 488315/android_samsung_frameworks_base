...
public class TvRecordingClient {
  ...
  public void startRecording(Uri programUri, Bundle params) {
    ...
    if (this.mIsRecordingStarted) {
      Log.m102w(TAG, "startRecording failed - recording already started");
    }
    TvInputManager.Session session = this.mSession;
    if (session != null) {
  ...
  public void stopRecording() {
    if (!this.mIsRecordingStarted) {
      Log.m102w(TAG, "stopRecording failed - recording not yet started");
    }
    TvInputManager.Session session = this.mSession;
    if (session != null) {
  ...
  public void pauseRecording(Bundle params) {
    ...
    if (this.mIsPaused) {
      Log.m102w(TAG, "pauseRecording failed - recording already paused");
    }
    TvInputManager.Session session = this.mSession;
    if (session != null) {
  ...
  public void resumeRecording(Bundle params) {
    if (!this.mIsRecordingStarted || this.mIsRecordingStopping || !this.mIsTuned) {
      throw new IllegalStateException(
          "resumeRecording failed - recording not yet started or stopping or not yet tuned");
    }
    if (!this.mIsPaused) {
      Log.m102w(TAG, "resumeRecording failed - recording not yet paused");
    }
    TvInputManager.Session session = this.mSession;
    if (session != null) {
  ...
  private class MySessionCallback extends TvInputManager.SessionCallback {
    ...
    public void onSessionCreated(TvInputManager.Session session) {
      if (this != TvRecordingClient.this.mSessionCallback) {
        Log.m102w(TvRecordingClient.TAG, "onSessionCreated - session already created");
        if (session != null) {
          session.release();
          return;
        }
    ...
    public void onTuned(TvInputManager.Session session, Uri channelUri) {
      if (this != TvRecordingClient.this.mSessionCallback) {
        Log.m102w(TvRecordingClient.TAG, "onTuned - session not created");
        return;
      }
      if (TvRecordingClient.this.mIsTuned || !Objects.equals(this.mChannelUri, channelUri)) {
        Log.m102w(
            TvRecordingClient.TAG, "onTuned - already tuned or not yet tuned to last channel");
        return;
      }
    ...
    public void onSessionReleased(TvInputManager.Session session) {
      if (this != TvRecordingClient.this.mSessionCallback) {
        Log.m102w(TvRecordingClient.TAG, "onSessionReleased - session not created");
        return;
      }
    ...
    public void onRecordingStopped(TvInputManager.Session session, Uri recordedProgramUri) {
      if (this != TvRecordingClient.this.mSessionCallback) {
        Log.m102w(TvRecordingClient.TAG, "onRecordingStopped - session not created");
        return;
      }
      if (!TvRecordingClient.this.mIsRecordingStarted) {
        Log.m102w(TvRecordingClient.TAG, "onRecordingStopped - recording not yet started");
        return;
      }
      TvRecordingClient.this.mIsRecordingStarted = false;
      TvRecordingClient.this.mIsPaused = false;
    ...
    public void onError(TvInputManager.Session session, int error) {
      if (this != TvRecordingClient.this.mSessionCallback) {
        Log.m102w(TvRecordingClient.TAG, "onError - session not created");
        return;
      }
      if (TvRecordingClient.this.mCallback != null) {
    ...
    public void onSessionEvent(TvInputManager.Session session, String eventType, Bundle eventArgs) {
      if (this != TvRecordingClient.this.mSessionCallback) {
        Log.m102w(TvRecordingClient.TAG, "onSessionEvent - session not created");
