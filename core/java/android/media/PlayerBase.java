...
public abstract class PlayerBase {
    ...
    protected void baseRegisterPlayer(int sessionId) {
        try {
            ...
            Log.m97e(TAG, "Error talking to audio service, player will not be tracked", e);
        }
    }
    ...
    void baseUpdateAudioAttributes(AudioAttributes attr) {
        ...
        try {
            ...
            Log.m97e(TAG, "Error talking to audio service, audio attributes will not be updated", e);
        }
        synchronized (this.mLock) {
            this.mAttributes = attr;
        }
    ...
    void baseUpdateSessionId(int sessionId) {
        try {
            ...
            Log.m97e(TAG, "Error talking to audio service, the session ID will not be updated", e);
        }
    }
    ...
    void baseUpdateDeviceId(AudioDeviceInfo deviceInfo) {
        ...
        try {
            ...
            Log.m97e(TAG, "Error talking to audio service, " + deviceId + " device id will not be tracked for piid=" + piid, e);
        }
    }
    ...
    private void updateState(int state, int deviceId) {
        ...
        try {
            ...
            Log.m97e(TAG, "Error talking to audio service, " + AudioPlaybackConfiguration.toLogFriendlyPlayerState(state) + " state will not be tracked for piid=" + piid, e);
        }
    }
    ...
    void baseRelease() {
        ...
        if (releasePlayer) {
            try {
                ...
                Log.m97e(TAG, "Error talking to audio service, the player will still be tracked", e);
            }
        }
        try {
    ...
    public static void deprecateStreamTypeForPlayback(int streamType, String className, String opName) throws IllegalArgumentException {
        ...
        Log.m102w(className, "Use of stream types is deprecated for operations other than volume control");
        Log.m102w(className, "See the documentation of " + opName + " for what to use instead with android.media.AudioAttributes to qualify your playback use case");
    }
