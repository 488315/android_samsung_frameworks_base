...
public abstract class MediaRoute2ProviderService extends Service {
    ...
    public final void notifySessionCreated(long requestId, RoutingSessionInfo sessionInfo) {
        ...
        if (DEBUG) {
            Log.m94d(TAG, "notifySessionCreated: Creating a session. requestId=" + requestId + ", sessionInfo=" + sessionInfo);
        }
    ...
    final class MediaRoute2ProviderServiceStub extends IMediaRoute2ProviderService.Stub {
        ...
        private boolean checkSessionIdIsValid(String sessionId, String description) {
            if (TextUtils.isEmpty(sessionId)) {
                Log.m102w(MediaRoute2ProviderService.TAG, description + ": Ignoring empty sessionId from system service.");
                return false;
            }
            if (MediaRoute2ProviderService.this.getSessionInfo(sessionId) == null) {
                Log.m102w(MediaRoute2ProviderService.TAG, description + ": Ignoring unknown session from system service. sessionId=" + sessionId);
                return false;
            }
            return true;
        }
        ...
        private boolean checkRouteIdIsValid(String routeId, String description) {
            if (TextUtils.isEmpty(routeId)) {
                Log.m102w(MediaRoute2ProviderService.TAG, description + ": Ignoring empty routeId from system service.");
                return false;
            }
            if (MediaRoute2ProviderService.this.mProviderInfo == null || MediaRoute2ProviderService.this.mProviderInfo.getRoute(routeId) == null) {
                Log.m102w(MediaRoute2ProviderService.TAG, description + ": Ignoring unknown route from system service. routeId=" + routeId);
                return false;
            }
            return true;
        }
