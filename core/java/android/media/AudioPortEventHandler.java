...
class AudioPortEventHandler {
                            ...
                            public void handleMessage(Message msg) {
                                ...
                                synchronized (AudioPortEventHandler.this.mLock) {
                                    if (msg.what == 4) {
                                        ...
                                        if (AudioPortEventHandler.this.mListeners.contains(
                                                msg.obj)) {
                                            listeners.add(
                                        ...
                                        listeners =
                                                (ArrayList)
                                                        AudioPortEventHandler.this.mListeners
                                                                .clone();
                                    }
                                }
                                ...
                                if (msg.what != 3) {
                                    int status =
                                            AudioManager.updateAudioPortCache(ports, patches, null);
                                    if (status != 0) {
                                        sendMessageDelayed(
                                                obtainMessage(msg.what, msg.obj),
                                                AudioPortEventHandler.RESCHEDULE_MESSAGE_DELAY_MS);
                                        return;
                                    }
                                }
                                ...
                                switch (status2) {
                                    ...
                                    case 4:
                                        AudioPort[] portList =
                                                (AudioPort[]) ports.toArray(new AudioPort[0]);
                                ...
                                AudioPatch[] patchList =
                                        (AudioPatch[]) patches.toArray(new AudioPatch[0]);
    ...
    private static void postEventFromNative(
            Object module_ref, int what, int arg1, int arg2, Object obj) {
        Handler handler;
        AudioPortEventHandler eventHandler =
                (AudioPortEventHandler) ((WeakReference) module_ref).get();
        if (eventHandler != null
                && eventHandler != null
                && (handler = eventHandler.handler()) != null) {
