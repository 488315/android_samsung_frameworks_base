...
class WebVttTrack extends SubtitleTrack implements WebVttCueListener {
    ...
    public void onData(byte[] data, boolean eos, long runID) {
        try {
            ...
            Log.m102w(TAG, "subtitle data is not UTF-8 encoded: " + e);
        }
    }
    ...
    public void onCueParsed(TextTrackCue cue) {
        synchronized (this.mParser) {
            ...
            if (this.DEBUG) {
                Log.m100v(TAG, cue.appendLinesToBuilder(cue.appendStringsToBuilder(new StringBuilder()).append(" simplified to: ")).toString());
            }
    ...
    public void updateView(Vector<SubtitleTrack.Cue> activeCues) {
        ...
        if (this.DEBUG && this.mTimeProvider != null) {
            try {
                Log.m94d(TAG, "at " + (this.mTimeProvider.getCurrentTimeUs(false, true) / 1000) + " ms the active cues are:");
            } catch (IllegalStateException e) {
                Log.m94d(TAG, "at (illegal state) the active cues are:");
            }
        }
