...
public class Metadata {
  ...
  private boolean scanAllRecords(Parcel parcel, int bytesLeft) {
    ...
    while (true) {
      ...
      if (size <= 12) {
        Log.m96e(TAG, "Record is too short");
        error = true;
        break;
      }
      ...
      if (this.mKeyToPosMap.containsKey(Integer.valueOf(metadataId))) {
        Log.m96e(TAG, "Duplicate metadata ID found");
        error = true;
        break;
      }
      ...
      try {
        ...
        Log.m96e(TAG, "Invalid size: " + e.getMessage());
        error = true;
      }
    }
    Log.m96e(TAG, "Invalid metadata type " + metadataType);
    error = true;
    if (bytesLeft != 0 || error) {
      Log.m96e(TAG, "Ran out of data or error on record " + recCount);
      this.mKeyToPosMap.clear();
      return false;
    }
    return true;
  ...
  public boolean parse(Parcel parcel) {
    if (parcel.dataAvail() < 8) {
      Log.m96e(TAG, "Not enough data " + parcel.dataAvail());
      return false;
    }
    int pin = parcel.dataPosition();
    int size = parcel.readInt();
    if (parcel.dataAvail() + 4 < size || size < 8) {
      Log.m96e(TAG, "Bad size " + size + " avail " + parcel.dataAvail() + " position " + pin);
      parcel.setDataPosition(pin);
      return false;
    }
    int kShouldBeMetaMarker = parcel.readInt();
    if (kShouldBeMetaMarker != kMetaMarker) {
      Log.m96e(TAG, "Marker missing " + Integer.toHexString(kShouldBeMetaMarker));
      parcel.setDataPosition(pin);
      return false;
    }
    if (!scanAllRecords(parcel, size - 8)) {
      parcel.setDataPosition(pin);
      return false;
    }
    this.mParcel = parcel;
  ...
  private boolean checkMetadataId(int val) {
    if (val <= 0 || (31 < val && val < 8192)) {
      Log.m96e(TAG, "Invalid metadata ID " + val);
      return false;
    }
    return true;
  }
