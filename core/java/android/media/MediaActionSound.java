...
public class MediaActionSound {
  ...
  private SoundPool.OnLoadCompleteListener mLoadCompleteListener =
      new SoundPool.OnLoadCompleteListener() { // from class: android.media.MediaActionSound.1
        @Override // android.media.SoundPool.OnLoadCompleteListener
        public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
          for (SoundState sound : MediaActionSound.this.mSounds) {
            if (sound.f281id == sampleId) {
              int playSoundId = 0;
              synchronized (sound) {
                if (status != 0) {
                  sound.state = 0;
                  sound.f281id = 0;
                  Log.m96e(
                      MediaActionSound.TAG,
                      "OnLoadCompleteListener() error: "
                          + status
                          + " loading sound: "
                          + sound.name);
                  return;
                ...
                switch (sound.state) {
                  case 1:
                    ...
                    break;
                  case 2:
                    playSoundId = sound.f281id;
                    sound.state = 3;
                    break;
                  default:
                    Log.m96e(
                        MediaActionSound.TAG,
                        "OnLoadCompleteListener() called in wrong state: "
                            + sound.state
                            + " for sound: "
                            + sound.name);
                    break;
  ...
  public static boolean mustPlayShutterSound() {
    ...
    try {
      ...
      Log.m96e(TAG, "audio service is unavailable for queries, defaulting to false");
      return false;
    }
  }
  ...
  public MediaActionSound() {
    SoundPool build =
        new SoundPool.Builder()
            .setMaxStreams(1)
            .setAudioAttributes(
                new AudioAttributes.Builder().setUsage(13).setFlags(1).setContentType(4).build())
            .build();
      ...
      switch (sound.state) {
        case 0:
          if (loadSound(sound) <= 0) {
            Log.m96e(TAG, "load() error loading sound: " + soundName);
            break;
          }
          break;
        default:
          Log.m96e(TAG, "load() called in wrong state: " + sound + " for sound: " + soundName);
          break;
      }
    }
  }
    ...
    synchronized (sound) {
      switch (sound.state) {
        case 0:
          loadSound(sound);
          if (loadSound(sound) <= 0) {
            Log.m96e(TAG, "play() error loading sound: " + soundName);
            break;
          }
        case 1:
          sound.state = 2;
        ...
        default:
          Log.m96e(
              TAG, "play() called in wrong state: " + sound.state + " for sound: " + soundName);
          break;
        case 3:
          this.mSoundPool.play(sound.f281id, 1.0f, 1.0f, 0, 0, 1.0f);
          break;
      }
    }
  }
