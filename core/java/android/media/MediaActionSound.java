                ...
                if (sound.f281id == sampleId) {
                    ...
                    synchronized (sound) {
                        if (status != 0) {
                            sound.state = 0;
                            sound.f281id = 0;
                            Log.m96e(MediaActionSound.TAG, "OnLoadCompleteListener() error: " + status + " loading sound: " + sound.name);
                            return;
                        }
                        switch (sound.state) {
                            case 1:
                            ...
                            case 2:
                                ...
                                break;
                            default:
                                Log.m96e(MediaActionSound.TAG, "OnLoadCompleteListener() called in wrong state: " + sound.state + " for sound: " + sound.name);
                                break;
                        }
                        if (playSoundId != 0) {
    ...
    public void play(int soundName) {
        ...
        synchronized (sound) {
            switch (sound.state) {
                ...
                case 2:
                default:
                    Log.m96e(TAG, "play() called in wrong state: " + sound.state + " for sound: " + soundName);
                    break;
                case 3:
