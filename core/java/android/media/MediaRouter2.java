...
public final class MediaRouter2 {
    ...
    void requestCreateController(RoutingController controller, MediaRoute2Info route, long managerRequestId) {
        ...
        if (stub != null) {
            try {
                ...
                Log.m97e(TAG, "createControllerForTransfer: Failed to request for creating a controller.", ex);
    ...
    void syncRoutesOnHandler(List<MediaRoute2Info> currentRoutes, RoutingSessionInfo currentSystemSessionInfo) {
        if (currentRoutes == null || currentRoutes.isEmpty() || currentSystemSessionInfo == null) {
            Log.m96e(TAG, "syncRoutesOnHandler: Received wrong data. currentRoutes=" + currentRoutes + ", currentSystemSessionInfo=" + currentSystemSessionInfo);
            return;
        }
        synchronized (this.mLock) {
            this.mRoutes.clear();
    ...
    void createControllerOnHandler(int requestId, RoutingSessionInfo sessionInfo) {
        ...
        if (!TextUtils.equals(requestedRoute.getProviderId(), sessionInfo.getProviderId())) {
            Log.m102w(TAG, "The session's provider ID does not match the requested route's. (requested route's providerId=" + requestedRoute.getProviderId() + ", actual providerId=" + sessionInfo.getProviderId() + NavigationBarInflaterView.KEY_CODE_END);
            notifyTransferFailure(requestedRoute);
            return;
        }
        RoutingController oldController = matchingRequest.mOldController;
        if (!oldController.scheduleRelease()) {
            Log.m102w(TAG, "createControllerOnHandler: Ignoring controller creation for released old controller. oldController=" + oldController);
    ...
    void updateControllerOnHandler(RoutingSessionInfo sessionInfo) {
        ...
        if (matchingController == null) {
            Log.m102w(TAG, "updateControllerOnHandler: Matching controller not found. uniqueSessionId=" + sessionInfo.getId());
            return;
        }
        ...
        if (!TextUtils.equals(oldInfo.getProviderId(), sessionInfo.getProviderId())) {
            Log.m102w(TAG, "updateControllerOnHandler: Provider IDs are not matched. old=" + oldInfo.getProviderId() + ", new=" + sessionInfo.getProviderId());
        } else {
    ...
    void releaseControllerOnHandler(RoutingSessionInfo sessionInfo) {
        ...
        if (matchingController == null) {
            if (DEBUG) {
                Log.m94d(TAG, "releaseControllerOnHandler: Matching controller not found. uniqueSessionId=" + sessionInfo.getId());
            }
        } else {
            RoutingSessionInfo oldInfo = matchingController.getRoutingSessionInfo();
            if (!TextUtils.equals(oldInfo.getProviderId(), sessionInfo.getProviderId())) {
                Log.m102w(TAG, "releaseControllerOnHandler: Provider IDs are not matched. old=" + oldInfo.getProviderId() + ", new=" + sessionInfo.getProviderId());
            } else {
    ...
    public class RoutingController {
        ...
        public void selectRoute(MediaRoute2Info route) {
            ...
            if (MediaRouter2.checkRouteListContainsRouteId(selectedRoutes, route.getId())) {
                Log.m102w(MediaRouter2.TAG, "Ignoring selecting a route that is already selected. route=" + route);
                return;
            }
        ...
        public void deselectRoute(MediaRoute2Info route) {
            ...
            List<MediaRoute2Info> selectedRoutes = getSelectedRoutes();
            if (!MediaRouter2.checkRouteListContainsRouteId(selectedRoutes, route.getId())) {
                Log.m102w(MediaRouter2.TAG, "Ignoring deselecting a route that is not selected. route=" + route);
                return;
            }
