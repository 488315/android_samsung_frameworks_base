...
public final class MediaDescrambler implements AutoCloseable {
  ...
  private interface DescramblerWrapper {
    ...
    int descramble(ByteBuffer byteBuffer, ByteBuffer byteBuffer2, MediaCodec.CryptoInfo cryptoInfo)
    ...
    try {
      try {
        if (MediaCas.getService() != null) {
          ...
          this.mIDescrambler =
              new HidlDescrambler(MediaCas.getServiceHidl().createDescrambler(CA_system_id));
        ...
        Log.m96e(TAG, "Failed to create descrambler: " + e);
        this.mIDescrambler = null;
        throw new MediaCasException.UnsupportedCasException(
            "Unsupported CA_system_id " + CA_system_id);
      }
