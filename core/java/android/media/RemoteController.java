...
public final class RemoteController {
  ...
  public boolean seekTo(long timeMs) throws IllegalArgumentException {
    if (!this.mEnabled) {
      Log.m96e(TAG, "Cannot use seekTo() from a disabled RemoteController");
      return false;
    }
    if (timeMs < 0) {
  ...
  public boolean setSynchronizationMode(int sync) throws IllegalArgumentException {
    ...
    Log.m96e(TAG, "Cannot set synchronization mode on an unregistered RemoteController");
    return false;
  }
    ...
    public void handleMessage(Message msg) {
      switch (msg.what) {
        ...
        case 2:
          ...
          break;
        default:
          Log.m96e(RemoteController.TAG, "unknown event " + msg.what);
          break;
      }
    }
  }
  ...
  private static void sendMsg(
      ...
      int delayMs) {
    if (handler == null) {
      Log.m96e(TAG, "null event handler, will not deliver message " + msg);
      return;
    }
    ...
    synchronized (mInfoLock) {
      if (controller == null) {
        ...
        if (mediaController != null) {
          ...
          || !controller.getSessionToken().equals(this.mCurrentSession.getSessionToken())) {
  ...
  public void onNewMediaMetadata(MediaMetadata metadata) {
    ...
    synchronized (mInfoLock) {
      ...
      MediaController mediaController = this.mCurrentSession;
      boolean canRate =
          (mediaController == null || mediaController.getRatingType() == 0) ? false : true;
