...
public final class MediaRouter2Manager {
  ...
  private final ConcurrentMap<String, RouteListingPreference> mPackageToRouteListingPreferenceMap =
      new ConcurrentHashMap();
  ...
  public void registerCallback(Executor executor, Callback callback) {
    ...
    if (!this.mCallbackRecords.addIfAbsent(callbackRecord)) {
      Log.m102w(TAG, "Ignoring to register the same callback twice.");
    }
  }
  ...
  public void unregisterCallback(Callback callback) {
    ...
    if (!this.mCallbackRecords.remove(new CallbackRecord(null, callback))) {
      Log.m102w(TAG, "unregisterCallback: Ignore unknown callback. " + callback);
    }
  }
  ...
  public List<MediaRoute2Info> getTransferableRoutes(final RoutingSessionInfo sessionInfo) {
    return getFilteredRoutes(
        sessionInfo,
        false,
        new Predicate() { // from class: android.media.MediaRouter2Manager$$ExternalSyntheticLambda6
  ...
  private List<MediaRoute2Info> getFilteredRoutes(
    ...
    RouteDiscoveryPreference discoveryPreference =
        this.mDiscoveryPreferenceMap.getOrDefault(packageName, RouteDiscoveryPreference.EMPTY);
  ...
  public void transfer(RoutingSessionInfo sessionInfo, MediaRoute2Info route) {
    Objects.requireNonNull(sessionInfo, "sessionInfo must not be null");
    Objects.requireNonNull(route, "route must not be null");
    Log.m100v(TAG, "Transferring routing session. session= " + sessionInfo + ", route=" + route);
    synchronized (this.mRoutesLock) {
      if (!this.mRoutes.containsKey(route.getId())) {
        Log.m102w(TAG, "transfer: Ignoring an unknown route id=" + route.getId());
  ...
  public void setRouteVolume(MediaRoute2Info route, int volume) {
    ...
    if (route.getVolumeHandling() == 0) {
      Log.m102w(TAG, "setRouteVolume: the route has fixed volume. Ignoring.");
      return;
    }
    if (volume < 0 || volume > route.getVolumeMax()) {
      Log.m102w(TAG, "setRouteVolume: the target volume is out of range. Ignoring");
      return;
    }
    try {
  ...
  public void setSessionVolume(RoutingSessionInfo sessionInfo, int volume) {
    ...
    if (sessionInfo.getVolumeHandling() == 0) {
      Log.m102w(TAG, "setSessionVolume: the route has fixed volume. Ignoring.");
      return;
    }
    if (volume < 0 || volume > sessionInfo.getVolumeMax()) {
      Log.m102w(TAG, "setSessionVolume: the target volume is out of range. Ignoring");
      return;
    }
    try {
      int requestId = this.mNextRequestId.getAndIncrement();
    ...
    while (it.hasNext()) {
      ...
      record.mExecutor.execute(
          new Runnable() { // from class:
            ...
            @Override // java.lang.Runnable
            public final void run() {
              MediaRouter2Manager.CallbackRecord.this.mCallback.onDiscoveryPreferenceChanged(
                  packageName, preference);
            }
          });
    }
  }
  ...
  public void updateRouteListingPreference(
    ...
    if (routeListingPreference == null) {
      ...
      oldRouteListingPreference =
          this.mPackageToRouteListingPreferenceMap.put(packageName, routeListingPreference);
    }
    ...
    while (it.hasNext()) {
      ...
      record.mExecutor.execute(
          new Runnable() { // from class:
            ...
            @Override // java.lang.Runnable
            public final void run() {
              MediaRouter2Manager.CallbackRecord.this.mCallback.onRouteListingPreferenceUpdated(
          ...
          sessionInfo.getSelectableRoutes().stream()
              .filter(
                  new Predicate() { // from class:
                    ...
                    @Override // java.util.function.Predicate
                    public final boolean test(Object obj) {
                      return MediaRouter2Manager.lambda$getSelectableRoutes$11(
  ...
  public void selectRoute(RoutingSessionInfo sessionInfo, MediaRoute2Info route) {
    ...
    if (sessionInfo.getSelectedRoutes().contains(route.getId())) {
      Log.m102w(TAG, "Ignoring selecting a route that is already selected. route=" + route);
      return;
    }
    if (!sessionInfo.getSelectableRoutes().contains(route.getId())) {
      Log.m102w(TAG, "Ignoring selecting a non-selectable route=" + route);
      return;
    }
    try {
      int requestId = this.mNextRequestId.getAndIncrement();
  ...
  public void deselectRoute(RoutingSessionInfo sessionInfo, MediaRoute2Info route) {
    ...
    Objects.requireNonNull(route, "route must not be null");
    if (!sessionInfo.getSelectedRoutes().contains(route.getId())) {
      Log.m102w(TAG, "Ignoring deselecting a route that is not selected. route=" + route);
      return;
    }
    if (!sessionInfo.getDeselectableRoutes().contains(route.getId())) {
      Log.m102w(TAG, "Ignoring deselecting a non-deselectable route=" + route);
      return;
    }
    try {
      int requestId = this.mNextRequestId.getAndIncrement();
  ...
  private void transferToRoute(RoutingSessionInfo session, MediaRoute2Info route) {
    ...
    try {
      this.mMediaRouterService.transferToRouteWithManager(
          this.mClient, requestId, session.getId(), route);
    } catch (RemoteException ex) {
      throw ex.rethrowFromSystemServer();
    }
  }
  ...
  private void requestCreateSession(RoutingSessionInfo oldSession, MediaRoute2Info route) {
    if (TextUtils.isEmpty(oldSession.getClientPackageName())) {
      Log.m102w(TAG, "requestCreateSession: Can't create a session without package name.");
      ...
      try {
        this.mMediaRouterService.requestCreateSessionWithManager(
            this.mClient, requestId, oldSession, route);
      } catch (RemoteException ex) {
        throw ex.rethrowFromSystemServer();
      }
    }
            ...
            new BiConsumer() { // from class:
              ...
              public final void accept(Object obj, Object obj2) {
                ((MediaRouter2Manager) obj)
                    .handleTransferTimeout((MediaRouter2Manager.TransferRequest) obj2);
              }
            },
            this,
            transferRequest);
  ...
  private boolean areSessionsMatched(
    ...
    return TextUtils.equals(volumeControlId, sessionInfo.getOriginalId())
        && TextUtils.equals(mediaController.getPackageName(), sessionInfo.getOwnerPackageName());
  }
      ...
      MediaRouter2Manager.this.mHandler.sendMessage(
          PooledLambda.obtainMessage(
              new BiConsumer() { // from class:
                ...
                public final void accept(Object obj, Object obj2) {
                  ((MediaRouter2Manager) obj)
                      .handleSessionsUpdatedOnHandler((RoutingSessionInfo) obj2);
                }
              },
              MediaRouter2Manager.this,
              session));
      ...
      MediaRouter2Manager.this.mHandler.sendMessage(
          PooledLambda.obtainMessage(
              new TriConsumer() { // from class:
                ...
                public final void accept(Object obj, Object obj2, Object obj3) {
                  ((MediaRouter2Manager) obj)
                      .updateDiscoveryPreference((String) obj2, (RouteDiscoveryPreference) obj3);
                }
              },
              MediaRouter2Manager.this,
              packageName,
      ...
      MediaRouter2Manager.this.mHandler.sendMessage(
          PooledLambda.obtainMessage(
              new TriConsumer() { // from class:
                ...
                public final void accept(Object obj, Object obj2, Object obj3) {
                  ((MediaRouter2Manager) obj)
                      .updateRouteListingPreference((String) obj2, (RouteListingPreference) obj3);
                }
              },
              MediaRouter2Manager.this,
              packageName,
