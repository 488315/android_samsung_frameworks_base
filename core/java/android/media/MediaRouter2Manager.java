...
public final class MediaRouter2Manager {
    ...
    void createSessionOnHandler(int requestId, RoutingSessionInfo sessionInfo) {
        ...
        if (!sessionInfo.getSelectedRoutes().contains(requestedRoute.getId())) {
            Log.m102w(TAG, "The session does not contain the requested route. (requestedRouteId=" + requestedRoute.getId() + ", actualRoutes=" + sessionInfo.getSelectedRoutes() + NavigationBarInflaterView.KEY_CODE_END);
            notifyTransferFailed(matchingRequest.mOldSessionInfo, requestedRoute);
