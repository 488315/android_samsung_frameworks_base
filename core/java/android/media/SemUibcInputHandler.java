...
public class SemUibcInputHandler {
  ...
  static {
    try {
      Log.m94d(TAG, "try to load libuibc.so");
      System.loadLibrary("uibc");
    } catch (Exception e) {
      Log.m94d(TAG, "library loading is failed");
  ...
  public static boolean isActive() {
    try {
      ...
      Log.m96e(TAG, "NoSuchMethod - mWfdSinkManager.isActiveUIBC()");
      return false;
    }
  }
  ...
  public static boolean isPenAvailable() {
    try {
      ...
      Log.m96e(TAG, "NoSuchMethod - mWfdSinkManager.isPenAvailable()");
      return false;
    }
  }

  public static void handlePenEvent(
      Context context, int states, int x, int y, float pressure, float tilt, float orientation) {
    context.enforceCallingOrSelfPermission(
        "android.permission.INJECT_EVENTS", "Need INJECT_EVENT Permission");
    penEvent(states, x, y, pressure, tilt, orientation);
  }

  public static void handleTouchMove(
      Context context, int pointers, int[] id, int[] moveX, int[] moveY) {
    context.enforceCallingOrSelfPermission(
        "android.permission.INJECT_EVENTS", "Need INJECT_EVENT Permission");
