...
public final class TimedText {
  ...
  public TimedText(Parcel parcel) {
    ...
    if (!parseParcel(this.mParcel)) {
      hashMap.clear();
      Log.m102w(TAG, "parseParcel() fails");
    }
  }
  ...
  private synchronized boolean parseParcel(Parcel parcel) {
    ...
    if (mTextIndex == 102) {
      ...
      Log.m102w(TAG, "Invalid timed text key found: " + mTextIndex);
      return false;
    }
    while (parcel.dataAvail() > 0) {
      int key = parcel.readInt();
      if (!isValidKey(key)) {
        Log.m102w(TAG, "Invalid timed text key found: " + key);
        return false;
      }
      Object object = null;
      switch (key) {
        ...
        case 15:
          ...
          Justification justification = new Justification(horizontal, vertical);
  ...
  private void readStyle(Parcel parcel) {
    ...
    Style style =
        new Style(startChar, endChar, fontId, isBold, isItalic, isUnderlined, fontSize, colorRGBA);
