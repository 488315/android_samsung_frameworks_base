...
public final class MediaBrowser {
  ...
  public void connect() {
    if (this.mState != 0 && this.mState != 1) {
      throw new IllegalStateException(
          "connect() called while neither disconnecting nor disconnected (state="
              + getStateLabel(this.mState)
              + NavigationBarInflaterView.KEY_CODE_END);
    }
    this.mState = 2;
    this.mHandler.post(
        new Runnable() { // from class: android.media.browse.MediaBrowser.1
          ...
          public void run() {
            ...
            try {
              bound =
                  MediaBrowser.this.mContext.bindService(
                      intent, MediaBrowser.this.mServiceConnection, 4097);
            } catch (Exception e) {
              Log.m96e(
                  MediaBrowser.TAG,
                  "Failed binding to service " + MediaBrowser.this.mServiceComponent);
            }
            if (!bound) {
  ...
  public void disconnect() {
    this.mState = 0;
    this.mHandler.post(
        new Runnable() { // from class: android.media.browse.MediaBrowser.2
          @Override // java.lang.Runnable
          public void run() {
            if (MediaBrowser.this.mServiceCallbacks != null) {
              try {
                ...
                Log.m102w(
                    MediaBrowser.TAG,
                    "RemoteException during connect for " + MediaBrowser.this.mServiceComponent);
              }
            }
  ...
  public void getItem(final String mediaId, final ItemCallback cb) {
    ...
    if (this.mState != 3) {
      Log.m98i(TAG, "Not connected, unable to retrieve the MediaItem.");
      this.mHandler.post(
          new Runnable() { // from class: android.media.browse.MediaBrowser.3
            @Override // java.lang.Runnable
            public void run() {
              cb.onError(mediaId);
            }
    ...
    try {
      ...
      Log.m98i(TAG, "Remote error getting media item.");
      this.mHandler.post(
          new Runnable() { // from class: android.media.browse.MediaBrowser.5
            @Override // java.lang.Runnable
            public void run() {
              cb.onError(mediaId);
            }
    ...
    if (isConnected()) {
      if (options == null) {
        try {
          ...
          Log.m94d(TAG, "addSubscription failed with RemoteException parentId=" + parentId);
          return;
        }
      }
      this.mServiceBinder.addSubscription(
          parentId, callback.mToken, options, this.mServiceCallbacks);
    }
  }
  ...
  private void unsubscribeInternal(String parentId, SubscriptionCallback callback) {
    ...
    try {
      if (callback == null) {
        ...
        for (int i = callbacks.size() - 1; i >= 0; i--) {
          if (callbacks.get(i) == callback) {
            if (isConnected()) {
              this.mServiceBinder.removeSubscription(
                  parentId, callback.mToken, this.mServiceCallbacks);
            }
            callbacks.remove(i);
      ...
      Log.m94d(TAG, "removeSubscription failed with RemoteException parentId=" + parentId);
    }
  ...
  public void onServiceConnected(
      ...
      final String root,
      final MediaSession.Token session,
      final Bundle extra) {
    this.mHandler.post(
        new Runnable() { // from class: android.media.browse.MediaBrowser.6
          @Override // java.lang.Runnable
          public void run() {
            ...
            if (MediaBrowser.this.mState != 2) {
              Log.m102w(
                  ...
                  "onConnect from service while mState="
                      + MediaBrowser.getStateLabel(MediaBrowser.this.mState)
                      + "... ignoring");
              return;
            }
            ...
            for (Map.Entry<String, Subscription> subscriptionEntry :
                MediaBrowser.this.mSubscriptions.entrySet()) {
              ...
              for (int i = 0; i < callbackList.size(); i++) {
                try {
                  MediaBrowser.this.mServiceBinder.addSubscription(
                      id,
                  ...
                  Log.m94d(
                      MediaBrowser.TAG,
                      "addSubscription failed with RemoteException parentId=" + id);
                }
              }
            }
          }
  ...
  public void onConnectionFailed(final IMediaBrowserServiceCallbacks callback) {
    this.mHandler.post(
        new Runnable() { // from class: android.media.browse.MediaBrowser.7
          ...
          public void run() {
            Log.m96e(
                MediaBrowser.TAG, "onConnectFailed for " + MediaBrowser.this.mServiceComponent);
  ...
  public void onLoadChildren(
    ...
    this.mHandler.post(
        new Runnable() { // from class: android.media.browse.MediaBrowser.8
          @Override // java.lang.Runnable
          public void run() {
            Subscription subscription;
            SubscriptionCallback subscriptionCallback;
            if (MediaBrowser.this.isCurrent(callback, "onLoadChildren")
                && (subscription = (Subscription) MediaBrowser.this.mSubscriptions.get(parentId))
                    != null
                && (subscriptionCallback =
                        subscription.getCallback(MediaBrowser.this.mContext, options))
                    != null) {
  ...
  void dump() {
    Log.m94d(TAG, "MediaBrowser...");
    Log.m94d(TAG, "  mServiceComponent=" + this.mServiceComponent);
    Log.m94d(TAG, "  mCallback=" + this.mCallback);
    Log.m94d(TAG, "  mRootHints=" + this.mRootHints);
    Log.m94d(TAG, "  mState=" + getStateLabel(this.mState));
    Log.m94d(TAG, "  mServiceConnection=" + this.mServiceConnection);
    Log.m94d(TAG, "  mServiceBinder=" + this.mServiceBinder);
    Log.m94d(TAG, "  mServiceCallbacks=" + this.mServiceCallbacks);
    Log.m94d(TAG, "  mRootId=" + this.mRootId);
    Log.m94d(TAG, "  mMediaSessionToken=" + this.mMediaSessionToken);
  }
  ...
  private class MediaServiceConnection implements ServiceConnection {
    ...
    public void onServiceConnected(final ComponentName name, final IBinder binder) {
      postOrRun(
          new Runnable() { // from class: android.media.browse.MediaBrowser.MediaServiceConnection.1
            @Override // java.lang.Runnable
            public void run() {
              ...
              MediaBrowser.this.mState = 2;
              try {
                MediaBrowser.this.mServiceBinder.connect(
                    MediaBrowser.this.mContext.getPackageName(),
                    MediaBrowser.this.mRootHints,
                    MediaBrowser.this.mServiceCallbacks);
              } catch (RemoteException e) {
                Log.m102w(
                    MediaBrowser.TAG,
    ...
    public void onServiceDisconnected(final ComponentName name) {
      postOrRun(
          new Runnable() { // from class: android.media.browse.MediaBrowser.MediaServiceConnection.2
            @Override // java.lang.Runnable
            public void run() {
    ...
    public boolean isCurrent(String funcName) {
      ...
      if (MediaBrowser.this.mState != 0 && MediaBrowser.this.mState != 1) {
        Log.m98i(
            ...
            funcName
                + " for "
                + MediaBrowser.this.mServiceComponent
                + " with mServiceConnection="
                + MediaBrowser.this.mServiceConnection
                + " this="
                + this);
        return false;
      }
