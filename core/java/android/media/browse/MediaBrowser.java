...
public final class MediaBrowser {
        ...
        this.mHandler.post(new Runnable() { // from class: android.media.browse.MediaBrowser.1
            ...
            public void run() {
                ...
                try {
                    ...
                    Log.m96e(MediaBrowser.TAG, "Failed binding to service " + MediaBrowser.this.mServiceComponent);
                }
                if (!bound) {
            ...
            public void run() {
                if (MediaBrowser.this.mServiceCallbacks != null) {
                    try {
                        ...
                        Log.m102w(MediaBrowser.TAG, "RemoteException during connect for " + MediaBrowser.this.mServiceComponent);
                    }
                }
        ...
        this.mHandler.post(new Runnable() { // from class: android.media.browse.MediaBrowser.6
            ...
            public void run() {
                ...
                if (MediaBrowser.this.mState != 2) {
                    Log.m102w(MediaBrowser.TAG, "onConnect from service while mState=" + MediaBrowser.getStateLabel(MediaBrowser.this.mState) + "... ignoring");
                    return;
                }
                ...
                for (Map.Entry<String, Subscription> subscriptionEntry : MediaBrowser.this.mSubscriptions.entrySet()) {
                    ...
                    for (int i = 0; i < callbackList.size(); i++) {
                        try {
                            ...
                            Log.m94d(MediaBrowser.TAG, "addSubscription failed with RemoteException parentId=" + id);
                        }
                    }
                }
            }
    ...
    public void onConnectionFailed(final IMediaBrowserServiceCallbacks callback) {
        this.mHandler.post(new Runnable() { // from class: android.media.browse.MediaBrowser.7
            @Override // java.lang.Runnable
            public void run() {
                Log.m96e(MediaBrowser.TAG, "onConnectFailed for " + MediaBrowser.this.mServiceComponent);
    ...
    public boolean isCurrent(IMediaBrowserServiceCallbacks callback, String funcName) {
        ...
        if (this.mState != 0 && this.mState != 1) {
            Log.m98i(TAG, funcName + " for " + this.mServiceComponent + " with mServiceConnection=" + this.mServiceCallbacks + " this=" + this);
            return false;
        }
        return false;
    }
    ...
    private class MediaServiceConnection implements ServiceConnection {
        ...
        public boolean isCurrent(String funcName) {
            ...
            if (MediaBrowser.this.mState != 0 && MediaBrowser.this.mState != 1) {
                Log.m98i(MediaBrowser.TAG, funcName + " for " + MediaBrowser.this.mServiceComponent + " with mServiceConnection=" + MediaBrowser.this.mServiceConnection + " this=" + this);
                return false;
            }
            return false;
        }
