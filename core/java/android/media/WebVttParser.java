...
class WebVttParser {
  ...
  WebVttParser(WebVttCueListener listener) {
    ...
    this.mParseHeader =
        new Phase() { // from class: android.media.WebVttParser.3
                ...
                if (name.equals("id")) {
                  ...
                  try {
                    ...
                    WebVttParser.this.log_warning(
                        "region setting", name, "has invalid value", e.getMessage(), value);
                    i = 0;
                  }
                  ...
                  if (!value.equals(MediaMetrics.Value.f285UP)) {
                    WebVttParser.this.log_warning(
    ...
    this.mParseCueId =
        new Phase() { // from class: android.media.WebVttParser.4
          static final /* synthetic */ boolean $assertionsDisabled = false;
    ...
    this.mParseCueTime =
        new Phase() { // from class: android.media.WebVttParser.5
          static final /* synthetic */ boolean $assertionsDisabled = false;
          ...
          public void parse(String line) {
            ...
            String rest =
                line.substring(arrowAt2 + 3).replaceFirst("^\\s+", "").replaceFirst("\\s+", " ");
                      ...
                      try {
                        if (value.endsWith("%")) {
                          ...
                          WebVttParser.this.mCue.mLinePosition =
                              Integer.valueOf(WebVttParser.parseIntPercentage(value));
                        } else if (value.matches(".*[^0-9].*")) {
                          WebVttParser.this.log_warning(
                              "cue setting", name, "contains an invalid character", value);
                        } else {
                        ...
                        WebVttParser.this.log_warning(
                            "cue setting", name, "is not numeric or percentage", value);
                      }
                      i = 0;
                    } else {
                      i = 0;
    ...
    this.mParseCueText =
        new Phase() { // from class: android.media.WebVttParser.6
          @Override // android.media.WebVttParser.Phase
          public void parse(String line) {
            if (line.length() == 0) {
  ...
  public void log_warning(String nameType, String name, String message, String value) {
    Log.m102w(getClass().getName(), nameType + " '" + name + "' " + message + " ('" + value + "')");
  }

  /* JADX INFO: Access modifiers changed from: private */
  public void log_warning(String message, String value) {
    Log.m102w(getClass().getName(), message + " ('" + value + "')");
  }
}
