...
public class MediaPlayer extends PlayerBase implements SubtitleController.Listener, VolumeAutomation, AudioRouting {
    ...
    class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case 100:
                    Log.m96e(MediaPlayer.TAG, "Error (" + msg.arg1 + "," + msg.arg2 + NavigationBarInflaterView.KEY_CODE_END);
                    ...
                    if (onErrorListener != null) {
                        if ((msg.arg2 == -49 || msg.arg2 == -60 || msg.arg2 == -61 || msg.arg2 == -64) && MediaPlayer.this.mOnPlayReadyErrorListener != null) {
                            if (msg.obj != null) {
                                Log.m96e(MediaPlayer.TAG, "PlayReadyAcquistion Failed \n sending onPlayReadyError " + ((String) msg.obj));
                            ...
                            Log.m96e(MediaPlayer.TAG, "License Not Found, propagate error to MoviePlaybackService.java");
                ...
                case 200:
                    switch (msg.arg1) {
                        case 700:
                            Log.m98i(MediaPlayer.TAG, "Info (" + msg.arg1 + "," + msg.arg2 + NavigationBarInflaterView.KEY_CODE_END);
                            break;
                        case 701:
                        case 702:
                ...
                case 202:
                    ...
                    return;
                case 210:
                    Log.m100v(MediaPlayer.TAG, "MEDIA_DRM_INFO " + MediaPlayer.this.mOnDrmInfoHandlerDelegate);
                    if (msg.obj == null) {
    ...
    private class OnDrmPreparedHandlerDelegate {
        ...
        void notifyClient(final int status) {
            ...
            if (handler != null) {
                ...
                Log.m96e(MediaPlayer.TAG, "OnDrmPreparedHandlerDelegate:notifyClient: Unexpected null mHandler");
            }
        }
    }
    ...
    public void prepareDrm(UUID uuid) throws UnsupportedSchemeException, ResourceBusyException, ProvisioningNetworkErrorException, ProvisioningServerErrorException {
        ...
        synchronized (this.mDrmLock) {
            if (this.mDrmInfo == null) {
                Log.m96e(TAG, "prepareDrm(): Wrong usage: The player must be prepared and DRM info be retrieved before this call.");
        ...
        synchronized (this.mDrmLock) {
            ...
            try {
                try {
                    ...
                    switch (result) {
                        case 1:
                            Log.m96e(TAG, "prepareDrm: Provisioning was required but failed due to a network error.");
                            throw new ProvisioningNetworkErrorException("prepareDrm: Provisioning was required but failed due to a network error.");
                        case 2:
                            Log.m96e(TAG, "prepareDrm: Provisioning was required but the request was denied by the server.");
                ...
                Log.m96e(TAG, "prepareDrm(): Wrong usage: The player must be in the prepared state to call prepareDrm().");
    ...
    public MediaDrm.KeyRequest getKeyRequest(byte[] keySetId, byte[] initData, String mimeType, int keyType, Map<String, String> optionalParameters) throws NoDrmSchemeException {
        ...
        Log.m100v(TAG, "getKeyRequest:  keySetId: " + Arrays.toString(keySetId) + " initData:" + Arrays.toString(initData) + " mimeType: " + mimeType + " keyType: " + keyType + " optionalParameters: " + optionalParameters);
        synchronized (this.mDrmLock) {
            if (!this.mActiveDrmScheme) {
            ...
            if (keyType != 3) {
                try {
                    ...
                    Log.m102w(TAG, "getKeyRequest NotProvisionedException: Unexpected. Shouldn't have reached here.");
    ...
    public byte[] provideKeyResponse(byte[] keySetId, byte[] response) throws NoDrmSchemeException, DeniedByServerException {
        byte[] scope;
        byte[] keySetResult;
        Log.m100v(TAG, "provideKeyResponse: keySetId: " + Arrays.toString(keySetId) + " response: " + Arrays.toString(response));
        synchronized (this.mDrmLock) {
            if (!this.mActiveDrmScheme) {
                Log.m96e(TAG, "getKeyRequest NoDrmSchemeException");
                throw new NoDrmSchemeException("getKeyRequest: Has to set a DRM scheme first.");
            ...
            if (keySetId == null) {
                try {
                    ...
                    Log.m102w(TAG, "provideKeyResponse NotProvisionedException: Unexpected. Shouldn't have reached here.");
            ...
            Log.m100v(TAG, "provideKeyResponse: keySetId: " + Arrays.toString(keySetId) + " response: " + Arrays.toString(response) + " --> " + Arrays.toString(keySetResult));
        }
        return keySetResult;
    }
    ...
    public static final class DrmInfo {
        ...
        private DrmInfo(Parcel parcel) {
            ...
            for (int i = 0; i < supportedDRMsCount; i++) {
                ...
                Log.m100v(MediaPlayer.TAG, "DrmInfo() supportedScheme[" + i + "]: " + this.supportedSchemes[i]);
            }
            Log.m100v(MediaPlayer.TAG, "DrmInfo() Parcel psshsize: " + psshsize + " supportedDRMsCount: " + supportedDRMsCount);
        }
        ...
        private Map<UUID, byte[]> parsePSSH(byte[] pssh, int psshsize) {
            ...
            while (len > 0) {
                if (len < 16) {
                    Log.m102w(MediaPlayer.TAG, String.format("parsePSSH: len is too short to parse UUID: (%d < 16) pssh: %d", Integer.valueOf(len), Integer.valueOf(psshsize)));
                    return null;
                }
                ...
                int len2 = len - 16;
                if (len2 < 4) {
                    Log.m102w(MediaPlayer.TAG, String.format("parsePSSH: len is too short to parse datalen: (%d < 4) pssh: %d", Integer.valueOf(len2), Integer.valueOf(psshsize)));
                    return null;
                }
                ...
                int len3 = len2 - 4;
                if (len3 < datalen) {
                    Log.m102w(MediaPlayer.TAG, String.format("parsePSSH: len is too short to parse data: (%d < %d) pssh: %d", Integer.valueOf(len3), Integer.valueOf(datalen), Integer.valueOf(psshsize)));
                    return null;
                }
                byte[] data = Arrays.copyOfRange(pssh, i3, i3 + datalen);
                i = i3 + datalen;
                len = len3 - datalen;
                Log.m100v(MediaPlayer.TAG, String.format("parsePSSH[%d]: <%s, %s> pssh: %d", Integer.valueOf(numentries), uuid, arrToHex(data), Integer.valueOf(psshsize)));
                numentries++;
    ...
    private void prepareDrm_openSessionStep(UUID uuid) throws NotProvisionedException, ResourceBusyException {
        ...
        try {
            ...
            Log.m100v(TAG, "prepareDrm_openSessionStep: mDrmSessionId=" + Arrays.toString(this.mDrmSessionId));
        ...
        public void run() {
            ...
            try {
                try {
                    ...
                    Log.m100v(MediaPlayer.TAG, "HandleProvisioninig: Thread run: response " + response.length + " " + Arrays.toString(response));
                } catch (Exception e2) {
                    this.status = 1;
                    Log.m102w(MediaPlayer.TAG, "HandleProvisioninig: Thread run: connect " + e2 + " url: " + url);
                }
                connection.disconnect();
                if (response != null) {
                    try {
                        MediaPlayer.this.mDrmObj.provideProvisionResponse(response);
                        Log.m100v(MediaPlayer.TAG, "HandleProvisioninig: Thread run: provideProvisionResponse SUCCEEDED!");
                        provisioningSucceeded = true;
                    } catch (Exception e3) {
                        this.status = 2;
                        Log.m102w(MediaPlayer.TAG, "HandleProvisioninig: Thread run: provideProvisionResponse " + e3);
                    }
                }
    ...
    private int HandleProvisioninig(UUID uuid) {
        ...
        Log.m100v(TAG, "HandleProvisioninig provReq  data: " + Arrays.toString(provReq.getData()) + " url: " + provReq.getDefaultUrl());
    ...
    private void resetDrmState() {
        synchronized (this.mDrmLock) {
            Log.m100v(TAG, "resetDrmState:  mDrmInfo=" + this.mDrmInfo + " mDrmProvisioningThread=" + this.mDrmProvisioningThread + " mPrepareDrmInProgress=" + this.mPrepareDrmInProgress + " mActiveDrmScheme=" + this.mActiveDrmScheme);
    ...
    public void cleanDrmObj() {
        Log.m100v(TAG, "cleanDrmObj: mDrmObj=" + this.mDrmObj + " mDrmSessionId=" + Arrays.toString(this.mDrmSessionId));
