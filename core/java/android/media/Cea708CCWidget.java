...
class Cea708CCWidget extends ClosedCaptionWidget implements Cea708CCParser.DisplayListener {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
      ...
      while (i < count) {
        ...
        if (scaleStartRow < 0.0f || scaleStartRow > 1.0f) {
          throw new RuntimeException(
              "A child of ScaledLayout should have a range of scaleStartRow between 0 and 1");
        }
        if (scaleEndRow < scaleStartRow || scaleStartRow > 1.0f) {
          throw new RuntimeException(
              "A child of ScaledLayout should have a range of scaleEndRow between scaleStartRow and"
                  + " 1");
        }
        if (scaleEndCol < 0.0f || scaleEndCol > 1.0f) {
          throw new RuntimeException(
              "A child of ScaledLayout should have a range of scaleStartCol between 0 and 1");
        }
        if (scaleEndCol < scaleStartCol || scaleEndCol > 1.0f) {
          throw new RuntimeException(
              "A child of ScaledLayout should have a range of scaleEndCol between scaleStartCol and"
                  + " 1");
        }
  ...
  static class CCLayout extends ScaledLayout implements ClosedCaptionWidget.ClosedCaptionLayout {
    ...
    public void addOrUpdateViewToSafeTitleArea(
        CCWindowLayout captionWindowLayout, ScaledLayout.ScaledLayoutParams scaledLayoutParams) {
  ...
  static class CCHandler implements Handler.Callback {
    ...
    private void setCurrentWindowLayout(int windowId) {
      ...
      if (windowId >= cCWindowLayoutArr.length
          || (windowLayout = cCWindowLayoutArr[windowId]) == null) {
        return;
      }
    ...
    private void defineWindow(Cea708CCParser.CaptionWindow window) {
      int windowId;
      if (window != null && (windowId = window.f277id) >= 0) {
        CCWindowLayout[] cCWindowLayoutArr = this.mCaptionWindowLayouts;
        if (windowId >= cCWindowLayoutArr.length) {
          return;
        }
  ...
  static class CCWindowLayout extends RelativeLayout implements View.OnLayoutChangeListener {
    ...
    public void initWindow(CCLayout ccLayout, Cea708CCParser.CaptionWindow captionWindow) {
      ...
      if (scaleRow < 0.0f || scaleRow > 1.0f) {
        Log.m98i(
            TAG,
            "The vertical position of the anchor point should be at the range of 0 and 1 but "
                + scaleRow);
      ...
      if (scaleCol < 0.0f || scaleCol > 1.0f) {
        Log.m98i(
            TAG,
            "The horizontal position of the anchor point should be at the range of 0 and 1 but "
                + scaleCol);
      ...
      setCaptionWindowId(captionWindow.f277id);
    ...
    private void updateText(String text, boolean appended) {
      ...
      if (text != null && text.length() > 0) {
        ...
        for (CharacterStyle characterStyle : this.mCharacterStyles) {
          ...
          spannableStringBuilder.setSpan(
              characterStyle, length, spannableStringBuilder.length(), 33);
        }
      }
      ...
      String truncatedText =
          TextUtils.join(
              "\n",
              Arrays.copyOfRange(
                  lines, Math.max(0, lines.length - (this.mRowLimit + 1)), lines.length));
