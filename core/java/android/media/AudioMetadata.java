...
public final class AudioMetadata {
  ...
  public static class Format {
    ...
    public static final Key<String> KEY_MIME =
        AudioMetadata.createKey(MediaFormat.KEY_MIME, String.class);
  ...
  private static class ObjectPackage implements DataPackage<Pair<Class, Object>> {
    ...
    public Pair<Class, Object> unpack(ByteBuffer buffer) {
      ...
      if (dataPackage == null) {
        Log.m96e(AudioMetadata.TAG, "Cannot find DataPackage for type:" + dataType);
        return null;
      }
      int dataSize = buffer.getInt();
      ...
      if (buffer.position() - position != dataSize) {
        Log.m96e(AudioMetadata.TAG, "Broken data package");
        return null;
      }
    ...
    public boolean pack(AutoGrowByteBuffer output, Pair<Class, Object> obj) {
      ...
      if (dataType == null) {
        Log.m96e(AudioMetadata.TAG, "Cannot find data type for " + obj.first);
        return false;
      }
      ...
      if (!dataPackage.pack(output, obj.second)) {
        Log.m98i(AudioMetadata.TAG, "Failed to pack object: " + obj.second);
        return false;
      }
  ...
  private static class BaseMapPackage implements DataPackage<BaseMap> {
    ...
    public BaseMap unpack(ByteBuffer buffer) {
      ...
      if (strDataPackage == null) {
        Log.m96e(AudioMetadata.TAG, "Cannot find DataPackage for String");
        return null;
      }
      for (int i = 0; i < mapSize; i++) {
        String key = strDataPackage.unpack(buffer);
        if (key == null) {
          Log.m96e(AudioMetadata.TAG, "Failed to unpack key for map");
          return null;
        }
        Pair<Class, Object> value = AudioMetadata.OBJECT_PACKAGE.unpack(buffer);
        if (value == null) {
          Log.m96e(AudioMetadata.TAG, "Failed to unpack value for map");
          return null;
        }
        if (key.equals(Format.KEY_HAS_ATMOS.getName())
            && value.first == Format.KEY_HAS_ATMOS.getValueClass()) {
          ret.set(
              Format.KEY_ATMOS_PRESENT, Boolean.valueOf(((Integer) value.second).intValue() != 0));
        } else {
    ...
    public boolean pack(AutoGrowByteBuffer autoGrowByteBuffer, BaseMap baseMap) {
      ...
      while (it.hasNext()) {
        ...
        if (!dataPackage.pack(autoGrowByteBuffer, next.getName())) {
          Log.m98i(AudioMetadata.TAG, "Failed to pack key: " + next.getName());
          return false;
        }
        if (!AudioMetadata.OBJECT_PACKAGE.pack(
            autoGrowByteBuffer, new Pair<>(next.getValueClass(), obj))) {
          Log.m98i(AudioMetadata.TAG, "Failed to pack value: " + baseMap.get(next));
          return false;
        }
      }
      return true;
  ...
  public static BaseMap fromByteBuffer(ByteBuffer buffer) {
    ...
    if (dataPackage == null) {
      Log.m96e(TAG, "Cannot find DataPackage for BaseMap");
      return null;
    }
    try {
      return (BaseMap) dataPackage.unpack(buffer);
    } catch (BufferUnderflowException e) {
      Log.m96e(TAG, "No enough data to unpack");
      return null;
    }
  }
