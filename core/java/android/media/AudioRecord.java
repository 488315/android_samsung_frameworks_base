...
public class AudioRecord
  ...
  public AudioRecord(
    ...
    this(
        new AudioAttributes.Builder().setInternalCapturePreset(audioSource).build(),
        new AudioFormat.Builder()
            .setChannelMask(getChannelMaskFromLegacyConfig(channelConfig, true))
  ...
  @SystemApi
  public AudioRecord(
      AudioAttributes attributes, AudioFormat format, int bufferSizeInBytes, int sessionId)
      throws IllegalArgumentException {
    this(
        attributes,
        format,
  ...
  private AudioRecord(
    ...
    if (attributes.getCapturePreset() != 8) {
      ...
      for (String tag : attributes.getTags()) {
        if (tag.equalsIgnoreCase(SUBMIX_FIXED_VOLUME)) {
          ...
          Log.m100v(TAG, "Will record from REMOTE_SUBMIX at full fixed volume");
        } else {
    ...
    AttributionSource attributionSource2 =
        context != null ? context.getAttributionSource() : AttributionSource.myAttributionSource();
  ...
  public static class Builder {
    ...
    public Builder setAudioSource(int source) throws IllegalArgumentException {
      Preconditions.checkState(
          this.mAudioPlaybackCaptureConfiguration == null, ERROR_MESSAGE_SOURCE_MISMATCH);
      if (!isSupportHotWordUsageInSystemApp(source)
          && (source < 0 || source > MediaRecorder.getAudioSourceMax())) {
    ...
    public Builder setSessionId(int sessionId) throws IllegalArgumentException {
      ...
      if (this.mSessionId == 0) {
        ...
        Log.m96e(AudioRecord.TAG, "setSessionId() called twice or after setSharedAudioEvent()");
      }
      return this;
    }
    ...
    private AudioRecord buildAudioPlaybackCaptureRecord() {
      android.media.audiopolicy.AudioMix audioMix =
          this.mAudioPlaybackCaptureConfiguration.createAudioMix(this.mFormat);
      MediaProjection projection = this.mAudioPlaybackCaptureConfiguration.getMediaProjection();
      AudioPolicy audioPolicy =
          new AudioPolicy.Builder(null).setMediaProjection(projection).addMix(audioMix).build();
    ...
    private AudioRecord buildCallExtractionRecord() {
      AudioMixingRule audioMixingRule =
          new AudioMixingRule.Builder()
              .addMixRule(
                  1, new AudioAttributes.Builder().setUsage(2).setForCallRedirection().build())
              .addMixRule(
                  1, new AudioAttributes.Builder().setUsage(3).setForCallRedirection().build())
              .setTargetMixRole(0)
              .build();
      android.media.audiopolicy.AudioMix audioMix =
    ...
    @SystemApi
    public Builder setMaxSharedAudioHistoryMillis(long maxSharedAudioHistoryMillis)
        throws IllegalArgumentException {
    ...
    public AudioRecord build() throws UnsupportedOperationException {
      ...
      if (this.mIsHotwordStream) {
        if (this.mIsHotwordLookback) {
          throw new UnsupportedOperationException(
              "setRequestHotwordLookbackStream and setRequestHotwordStream used concurrently");
        }
        halInputFlags = 512;
      ...
      if (this.mIsHotwordStream || this.mIsHotwordLookback) {
        this.mAttributes =
            new AudioAttributes.Builder(this.mAttributes).setInternalCapturePreset(6).build();
      }
      if (this.mPrivacySensitive != -1) {
        ...
        this.mAttributes =
            new AudioAttributes.Builder(this.mAttributes)
                ...
                .setPrivacySensitive(this.mPrivacySensitive == 1)
                .build();
      }
      ...
      try {
        if (this.mBufferSizeInBytes == 0) {
          this.mBufferSizeInBytes =
              this.mFormat.getChannelCount()
                  * AudioFormat.getBytesPerSample(this.mFormat.getEncoding());
        }
        AudioRecord record =
            new AudioRecord(
                this.mAttributes,
        ...
        if (audioAttributes != null) {
          this.mAttributes =
              new AudioAttributes.Builder(audioAttributes)
                  .setInternalCapturePreset(this.mAttributes.getCapturePreset())
                  .allowConcurrentCapture()
                  .build();
        } else {
          this.mAttributes =
              new AudioAttributes.Builder()
                  .setInternalCapturePreset(0)
                  .allowConcurrentCapture()
                  .build();
        }
      }
  ...
  private static int getChannelMaskFromLegacyConfig(
      int inChannelConfig, boolean allowLegacyConfig) {
    int mask;
      ...
      default:
        throw new IllegalArgumentException(
            "Unsupported sample encoding "
                + audioFormat
                + ". Should be ENCODING_PCM_8BIT, ENCODING_PCM_16BIT, ENCODING_PCM_24BIT_PACKED,"
                + " ENCODING_PCM_32BIT, or ENCODING_PCM_FLOAT.");
    }
  }
  ...
  private void handleFullVolumeRec(boolean starting) {
    ...
    try {
      ...
      Log.m97e(TAG, "Error talking to AudioService when handling full submix volume", e);
    }
  }
  ...
  public int read(byte[] audioData, int offsetInBytes, int sizeInBytes, int readMode) {
    ...
    if (readMode != 0 && readMode != 1) {
      Log.m96e(TAG, "AudioRecord.read() called with invalid blocking mode");
      return -2;
    }
    if (audioData == null
        || offsetInBytes < 0
  ...
  public int read(float[] audioData, int offsetInFloats, int sizeInFloats, int readMode) {
    if (this.mState == 0) {
      Log.m96e(TAG, "AudioRecord.read() called in invalid state STATE_UNINITIALIZED");
      return -3;
    }
    if (this.mAudioFormat != 4) {
      Log.m96e(TAG, "AudioRecord.read(float[] ...) requires format ENCODING_PCM_FLOAT");
      return -3;
    }
    if (readMode != 0 && readMode != 1) {
      Log.m96e(TAG, "AudioRecord.read() called with invalid blocking mode");
  ...
  public List<MicrophoneInfo> getActiveMicrophones() throws IOException {
    ...
    if (status != 0) {
      if (status != -3) {
        Log.m96e(TAG, "getActiveMicrophones failed:" + status);
      }
      Log.m98i(TAG, "getActiveMicrophones failed, fallback on routed device info");
    }
  ...
  private static void postEventFromNative(
      Object audiorecord_ref, int what, int arg1, int arg2, Object obj) {
  ...
  @Deprecated
  private int native_setup(
      Object audiorecordThis,
      Object attributes,
      int[] sampleRate,
      int channelMask,
  ...
  public static final class MetricsConstants {
    ...
    @Deprecated public static final String LATENCY = "android.media.audiorecord.latency";
