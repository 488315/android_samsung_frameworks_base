...
public final class AudioRecordingConfiguration implements Parcelable {
  ...
  public static final Parcelable.Creator<AudioRecordingConfiguration> CREATOR =
      new Parcelable.Creator<
          AudioRecordingConfiguration>() { // from class:
  ...
  public AudioDeviceInfo getAudioDevice() {
    ...
    if (AudioManager.listAudioPatches(patches) != 0) {
      Log.m96e(TAG, "Error retrieving list of audio patches");
      return null;
    }
    int i = 0;
    while (true) {
      ...
      if (patch.m49id() != this.mPatchHandle) {
        i++;
      } else {
        AudioPortConfig[] sources = patch.sources();
        if (sources != null && sources.length > 0) {
          int devId = sources[0].port().m50id();
          return AudioManager.getDeviceForPortId(devId, 1);
        }
      }
    }
    Log.m96e(TAG, "Couldn't find device for recording, did recording end already?");
    return null;
  }
