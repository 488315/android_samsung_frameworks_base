...
class ImageUtils {
        ...
        switch (format) {
            ...
            default:
                if (Log.isLoggable(IMAGEUTILS_LOG_TAG, 2)) {
                    Log.m100v(IMAGEUTILS_LOG_TAG, "getEstimatedNativeAllocBytes() uses defaultestimated native allocation size.");
                }
                estimatedBytePerPixel = 1.0d;
                break;
        }
    ...
    private static Size getEffectivePlaneSizeForImage(Image image, int planeIdx) {
        switch (image.getFormat()) {
            ...
            default:
                if (Log.isLoggable(IMAGEUTILS_LOG_TAG, 2)) {
                    Log.m100v(IMAGEUTILS_LOG_TAG, "getEffectivePlaneSizeForImage() usesimage's width and height for plane size.");
                }
                return new Size(image.getWidth(), image.getHeight());
        }
    }
