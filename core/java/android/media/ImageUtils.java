...
class ImageUtils {
  ...
  public static void imageCopy(Image src, Image dst) {
    ...
    if (!(dst.getOwner() instanceof ImageWriter)) {
      throw new IllegalArgumentException(
          "Destination image is not from ImageWriter. Only the images from ImageWriter are"
              + " writable");
    }
    ...
    if (!srcSize.equals(dstSize)) {
      throw new IllegalArgumentException(
          "source image size " + srcSize + " is different with destination image size " + dstSize);
    }
      ...
      if (srcRowStride == dstRowStride) {
        ...
        for (int row = 0; row < effectivePlaneSize.getHeight(); row++) {
          if (row == effectivePlaneSize.getHeight() - 1
              && srcByteCount > (remainingBytes = srcBuffer.remaining() - srcOffset)) {
    ...
    switch (format) {
      ...
      default:
        if (Log.isLoggable(IMAGEUTILS_LOG_TAG, 2)) {
          Log.m100v(
              IMAGEUTILS_LOG_TAG,
              "getEstimatedNativeAllocBytes() uses defaultestimated native allocation size.");
        }
        estimatedBytePerPixel = 1.0d;
        break;
    }
    ...
    switch (image.getFormat()) {
      ...
      default:
        if (Log.isLoggable(IMAGEUTILS_LOG_TAG, 2)) {
          Log.m100v(
              IMAGEUTILS_LOG_TAG,
              "getEffectivePlaneSizeForImage() usesimage's width and height for plane size.");
        }
        return new Size(image.getWidth(), image.getHeight());
    }
  }
  ...
  private static void directByteBufferCopy(
      ByteBuffer srcBuffer, int srcOffset, ByteBuffer dstBuffer, int dstOffset, int srcByteCount) {
