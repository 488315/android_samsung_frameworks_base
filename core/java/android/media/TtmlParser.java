...
class TtmlParser {
      ...
      if (active2) {
        if (eventType == 2) {
          if (!isSupportedTag(this.mParser.getName())) {
            Log.m102w(TAG, "Unsupported tag " + this.mParser.getName() + " is ignored.");
          ...
          if (TextUtils.isEmpty(text)) {
            ...
            parent.mChildren.add(
                new TtmlNode(
                    TtmlUtils.PCDATA, "", text, 0L, Long.MAX_VALUE, parent, this.mCurrentRunId));
          } else {
            active = active2;
          }
        } else {
  ...
  private TtmlNode parseNode(TtmlNode parent) throws XmlPullParserException, IOException {
    ...
    if (dur > 0) {
      if (end2 != Long.MAX_VALUE) {
        Log.m96e(
            TAG, "'dur' and 'end' attributes are defined at the same time.'end' value is ignored.");
      }
      end2 = start + dur;
    }
    if (parent != null
    ...
    TtmlNode node =
        new TtmlNode(this.mParser.getName(), null, null, start, end, parent, this.mCurrentRunId);
    return node;
  }
