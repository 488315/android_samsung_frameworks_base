...
public class AudioPolicyConfig implements Parcelable {
  public static final Parcelable.Creator<AudioPolicyConfig> CREATOR =
      new Parcelable.Creator<
          AudioPolicyConfig>() { // from class: android.media.audiopolicy.AudioPolicyConfig.1
  ...
  public String toLogFriendlyString() {
    ...
    while (it.hasNext()) {
      AudioMix mix = it.next();
      textDump3 =
          (((((((textDump3 + "* route flags=0x" + Integer.toHexString(mix.getRouteFlags()) + "\n")
                                      + "  rate="
                                  ...
                                  + "\n")
                              + "  channels=0x")
                          + Integer.toHexString(mix.getFormat().getChannelMask()).toUpperCase()
                          + "\n")
      ...
      while (it2.hasNext()) {
        ...
        switch (criterion.mRule) {
          ...
          case 32770:
            textDump =
                (textDump3 + "  exclude capture preset ") + criterion.mAttr.getCapturePreset();
            break;
          case 32772:
  ...
  protected void setRegistration(String regId) {
    ...
    if (!currentRegNull && !newRegNull && !this.mRegistrationId.equals(regId)) {
      Log.m96e(
          TAG, "Invalid registration transition from " + this.mRegistrationId + " to " + regId);
      return;
    }
