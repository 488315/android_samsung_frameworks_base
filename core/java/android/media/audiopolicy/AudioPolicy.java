...
public class AudioPolicy {
    ...
    private AudioPolicy(AudioPolicyConfig config, Context context, Looper looper, AudioPolicyFocusListener fl, AudioPolicyStatusListener sl, boolean isFocusPolicy, boolean isTestFocusPolicy, AudioPolicyVolumeCallback vc, MediaProjection projection) {
        ...
        if (looper != null) {
            ...
            Log.m96e(TAG, "No event handler due to looper without a thread");
        }
    ...
    public int attachMixes(List<AudioMix> mixes) {
        ...
        synchronized (this.mLock) {
            ...
            try {
                status = service.addMixForPolicy(cfg, m54cb());
                if (status == 0) {
                    this.mConfig.add(zeMixes);
                }
            } catch (RemoteException e) {
                Log.m97e(TAG, "Dead object in attachMixes", e);
                return -1;
            }
        }
        return status;
    ...
    public int detachMixes(List<AudioMix> mixes) {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                status = service.removeMixForPolicy(cfg, m54cb());
                if (status == 0) {
                    this.mConfig.remove(zeMixes);
                }
            } catch (RemoteException e) {
                Log.m97e(TAG, "Dead object in detachMixes", e);
                return -1;
            }
        }
        return status;
    ...
    public boolean setUidDeviceAffinity(int uid, List<AudioDeviceInfo> devices) {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                int status = service.setUidDeviceAffinity(m54cb(), uid, deviceTypes, deviceAdresses);
                z = status == 0;
            } catch (RemoteException e) {
                Log.m97e(TAG, "Dead object in setUidDeviceAffinity", e);
                return false;
            }
        }
        return z;
    ...
    public boolean removeUidDeviceAffinity(int uid) {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                int status = service.removeUidDeviceAffinity(m54cb(), uid);
                z = status == 0;
            } catch (RemoteException e) {
                Log.m97e(TAG, "Dead object in removeUidDeviceAffinity", e);
                return false;
            }
        }
        return z;
    ...
    public boolean removeUserIdDeviceAffinity(int userId) {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                int status = service.removeUserIdDeviceAffinity(m54cb(), userId);
                z = status == 0;
            } catch (RemoteException e) {
                Log.m97e(TAG, "Dead object in removeUserIdDeviceAffinity", e);
                return false;
            }
        }
        return z;
    ...
    public boolean setUserIdDeviceAffinity(int userId, List<AudioDeviceInfo> devices) {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                int status = service.setUserIdDeviceAffinity(m54cb(), userId, deviceTypes, deviceAddresses);
                z = status == 0;
            } catch (RemoteException e) {
                Log.m97e(TAG, "Dead object in setUserIdDeviceAffinity", e);
                return false;
            }
        }
        return z;
    ...
    private boolean policyReadyToUse() {
        ...
        synchronized (this.mLock) {
            if (this.mStatus != 2) {
                Log.m96e(TAG, "Cannot use unregistered AudioPolicy");
                return false;
            }
            ...
            try {
                ...
                if (mediaProjection != null) {
                    if (mediaProjection.getProjection().canProjectAudio()) {
                        ...
                        Slog.m121w(TAG, "Cannot use AudioPolicy for pid " + Binder.getCallingPid() + " / uid " + Binder.getCallingUid() + ", needs MODIFY_AUDIO_ROUTING or MediaProjection that can project audio.");
                        return false;
                    }
                }
                ...
                Log.m96e(TAG, "Failed to check if MediaProjection#canProjectAudio");
    ...
    private int checkCallingOrSelfPermission(String permission) {
        ...
        Slog.m119v(TAG, "Null context, checking permission via ActivityManager");
    ...
    public int setFocusDuckingBehavior(int behavior) throws IllegalArgumentException, IllegalStateException {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                status = service.setFocusPropertiesForPolicy(behavior, m54cb());
                if (status == 0) {
                    this.mConfig.mDuckingPolicy = behavior;
                }
            } catch (RemoteException e) {
                Log.m97e(TAG, "Dead object in setFocusPropertiesForPolicy for behavior", e);
                return -1;
            }
        }
        return status;
    ...
    public boolean sendFocusLoss(AudioFocusInfo focusLoser) throws IllegalStateException {
        ...
        try {
            return getService().sendFocusLoss(focusLoser, m54cb());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public AudioRecord createAudioRecordSink(AudioMix mix) throws IllegalArgumentException {
        if (!policyReadyToUse()) {
            Log.m96e(TAG, "Cannot create AudioRecord sink for AudioMix");
            return null;
        }
    ...
    public AudioTrack createAudioTrackSource(AudioMix mix) throws IllegalArgumentException {
        if (!policyReadyToUse()) {
            Log.m96e(TAG, "Cannot create AudioTrack source for AudioMix");
            return null;
        }
    ...
    /* renamed from: cb */
    public IAudioPolicyCallback m54cb() {
            ...
            switch (msg.what) {
                ...
                case 4:
                    if (AudioPolicy.this.mFocusListener != null) {
                        ...
                        Log.m96e(AudioPolicy.TAG, "Invalid null focus listener for focus request event");
                        break;
                    }
                case 5:
                    if (AudioPolicy.this.mFocusListener != null) {
                        ...
                        Log.m96e(AudioPolicy.TAG, "Invalid null focus listener for focus abandon event");
                        break;
                    }
                case 6:
                    if (AudioPolicy.this.mVolCb != null) {
                        ...
                        Log.m96e(AudioPolicy.TAG, "Invalid null volume event");
                        break;
                    }
                default:
                    Log.m96e(AudioPolicy.TAG, "Unknown event " + msg.what);
                    break;
            }
        }
    }
