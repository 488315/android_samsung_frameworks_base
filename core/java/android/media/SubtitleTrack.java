  ...
  */
  protected synchronized void updateActiveCues(boolean rebuild, long timeMs) {
    ...
    Iterator<Pair<Long, Cue>> it =
        this.mCues.entriesBetween(this.mLastUpdateTimeMs, timeMs).iterator();
    while (it.hasNext()) {
      Pair<Long, Cue> event = it.next();
      Cue cue = event.second;
      if (cue.mEndTimeMs == event.first.longValue()) {
        if (this.DEBUG) {
          Log.m100v(TAG, "Removing " + cue);
        }
  ...
  protected synchronized void clearActiveCues() {
    if (this.DEBUG) {
      Log.m100v(TAG, "Clearing " + this.mActiveCues.size() + " active cues");
    }
  ...
  protected void scheduleTimedEvents() {
    if (this.mTimeProvider != null) {
      ...
      if (this.DEBUG) {
        Log.m94d(TAG, "sched @" + this.mNextScheduledTimeMs + " after " + this.mLastTimeMs);
      }
  ...
  public void onTimedEvent(long timeUs) {
    if (this.DEBUG) {
      Log.m94d(TAG, "onTimedEvent " + timeUs);
    }
    synchronized (this) {
  ...
  protected synchronized boolean addCue(Cue cue) {
    ...
    if (this.mVisible && cue.mStartTimeMs <= nowMs && cue.mEndTimeMs >= this.mLastTimeMs) {
      ...
      if (this.mHandler.postDelayed(runnable2, 10L)) {
        if (this.DEBUG) {
          Log.m100v(TAG, "scheduling update");
        }
      } else if (this.DEBUG) {
        Log.m102w(TAG, "failed to schedule subtitle view update");
      }
      return true;
    }
      ...
      return new Iterable<Pair<Long, Cue>>() { // from class: android.media.SubtitleTrack.CueList.1
        ...
        public Iterator<Pair<Long, Cue>> iterator() {
          if (CueList.this.DEBUG) {
            Log.m94d(CueList.TAG, "slice (" + lastTimeMs + ", " + timeMs + "]=");
          }
          try {
            CueList cueList = CueList.this;
            return cueList
            .new EntryIterator(
                cueList.mCues.subMap(Long.valueOf(lastTimeMs + 1), Long.valueOf(timeMs + 1)));
      ...
      private void nextKey() {
        do {
          try {
            ...
            try {
              this.mRemainingCues =
                  this.mRemainingCues.tailMap(Long.valueOf(this.mCurrentTimeMs + 1));
            } catch (IllegalArgumentException e) {
