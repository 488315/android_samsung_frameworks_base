...
public class AudioDevicePort extends AudioPort {
  ...
  AudioDevicePort(
    ...
    super(
        handle, AudioManager.isInputDevice(type) ? 1 : 2, deviceName, profiles, gains, descriptors);
    this.mType = type;
    this.mAddress = address;
    this.mEncapsulationModes = encapsulationModes;
    this.mEncapsulationMetadataTypes = encapsulationMetadataTypes;
  ...
  public int[] encapsulationModes() {
    ...
    return Arrays.stream(iArr)
        .boxed()
        .filter(
            new Predicate() { // from class: android.media.AudioDevicePort$$ExternalSyntheticLambda0
