      ...
      while (this.mNumDisconnectingThreads.get() <= 0) {
        ...
        Log.m98i(
            ...
            NavigationBarInflaterView.SIZE_MOD_START
                + offset
                + "] - setReadTimeout and setConnectTimeout with 8000ms");
        ...
        if (offset > 0) {
          this.mConnection.setRequestProperty(
              "Range", "bytes=" + offset + NativeLibraryHelper.CLEAR_ABI_OVERRIDE);
        }
        int response = this.mConnection.getResponseCode();
        Log.m98i(
            TAG,
            NavigationBarInflaterView.SIZE_MOD_START + offset + "] - response code = " + response);
        if (response == 300
            || response == 301
            || response == 302
            || response == 303
          ...
          if (response == 307
              && !method.equals(SemShareConstants.HTTP_CONN_REQUEST_METHOD)
              && !method.equals("HEAD")) {
          ...
          if (!url2.getProtocol().equals(IntentFilter.SCHEME_HTTPS)
              && !url2.getProtocol().equals(IntentFilter.SCHEME_HTTP)) {
