...
class CallbackUtil {
  ...
  static <T, S> Pair<ArrayList<ListenerInfo<T>>, S> addListener(
      String methodName,
      Executor executor,
      T listener,
      ArrayList<ListenerInfo<T>> listeners,
      S dispatchStub,
      Supplier<S> newStub,
      Consumer<S> registerStub) {
    Objects.requireNonNull(executor);
    Objects.requireNonNull(listener);
    if (hasListener(listener, listeners)) {
      throw new IllegalArgumentException(
          "attempt to call " + methodName + "on a previously registered listener");
    }
    if (listeners == null) {
    ...
    if (listeners.size() == 0) {
      if (dispatchStub == null) {
        try {
          ...
          Log.m97e(TAG, "Exception while creating stub in " + methodName, e);
  ...
  static <T, S> Pair<ArrayList<ListenerInfo<T>>, S> removeListener(
      String methodName,
      T listener,
      ArrayList<ListenerInfo<T>> listeners,
      S dispatchStub,
      Consumer<S> unregisterStub) {
    Objects.requireNonNull(listener);
    if (!removeListener(listener, listeners)) {
      throw new IllegalArgumentException(
          "attempt to call " + methodName + " on an unregistered listener");
    }
    if (listeners.size() == 0) {
  ...
  static <T> void callListeners(
      ArrayList<ListenerInfo<T>> listeners, Object listenerLock, final CallbackMethod<T> callback) {
            ...
            while (it.hasNext()) {
              ...
              info.mExecutor.execute(
                  new Runnable() { // from class:
                                   // android.media.CallbackUtil$$ExternalSyntheticLambda0
      ...
      synchronized (this.mListenerLock) {
        Pair removeListener =
            CallbackUtil.removeListener(
                ...
                this.mDispatcherStub,
                new Consumer() { // from class:
                                 // android.media.CallbackUtil$LazyListenerManager$$ExternalSyntheticLambda1
                  @Override // java.util.function.Consumer
                  public final void accept(Object obj) {
