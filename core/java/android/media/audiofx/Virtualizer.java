...
public class Virtualizer extends AudioEffect {
    ...
    public Virtualizer(int priority, int audioSession) throws IllegalStateException, IllegalArgumentException, UnsupportedOperationException, RuntimeException {
        ...
        if (audioSession == 0) {
            Log.m102w(TAG, "WARNING: attaching a Virtualizer to global output mix is deprecated!");
        }
        int[] value = new int[1];
    ...
    private boolean getAnglesInt(int inputChannelMask, int deviceType, int[] angles) throws IllegalStateException, IllegalArgumentException, UnsupportedOperationException {
        ...
        if (angles != null && angles.length < nbChannels * 3) {
            Log.m96e(TAG, "Size of array for angles cannot accomodate number of channels in mask (" + nbChannels + NavigationBarInflaterView.KEY_CODE_END);
        ...
        checkStatus(status);
        Log.m96e(TAG, "unexpected status code " + status + " after getParameter(PARAM_VIRTUAL_SPEAKER_ANGLES)");
        return false;
    }
    ...
    public boolean forceVirtualizationMode(int virtualizationMode) throws IllegalStateException, IllegalArgumentException, UnsupportedOperationException {
        ...
        checkStatus(status);
        Log.m96e(TAG, "unexpected status code " + status + " after setParameter(PARAM_FORCE_VIRTUALIZATION_MODE)");
        return false;
    }
    ...
    public int getVirtualizationMode() throws IllegalStateException, UnsupportedOperationException {
        ...
        checkStatus(status);
        Log.m96e(TAG, "unexpected status code " + status + " after getParameter(PARAM_VIRTUALIZATION_MODE)");
        return 0;
    }
