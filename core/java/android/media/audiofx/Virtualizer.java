...
public class Virtualizer extends AudioEffect {
  ...
  private boolean getAnglesInt(int inputChannelMask, int deviceType, int[] angles)
    ...
    if (status >= 0) {
      if (angles != null) {
        ...
        for (int i = 0; i < nbChannels; i++) {
          angles[i * 3] =
              AudioFormat.convertNativeChannelMaskToOutMask(resultConverter.getInt(i * 4 * 3));
    ...
    Log.m96e(
        TAG,
        "unexpected status code " + status + " after getParameter(PARAM_VIRTUAL_SPEAKER_ANGLES)");
    return false;
  }
  ...
  public int getVirtualizationMode() throws IllegalStateException, UnsupportedOperationException {
    ...
    checkStatus(status);
    Log.m96e(
        TAG, "unexpected status code " + status + " after getParameter(PARAM_VIRTUALIZATION_MODE)");
    return 0;
  }
