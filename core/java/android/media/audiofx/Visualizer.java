...
public class Visualizer {
  ...
  public Visualizer(int audioSession) throws UnsupportedOperationException, RuntimeException {
    ...
    synchronized (obj) {
      ...
      try {
        int result =
            native_setup(
                new WeakReference(this), audioSession, id, attributionSourceState.getParcel());
        if (attributionSourceState != null) {
          attributionSourceState.close();
        }
        if (result != 0 && result != -2) {
          Log.m96e(TAG, "Error code " + result + " when initializing Visualizer.");
          switch (result) {
            case -5:
  ...
  public int getMeasurementPeakRms(MeasurementPeakRms measurement) {
    ...
    if (measurement == null) {
      Log.m96e(TAG, "Cannot store measurements in a null object");
      return -4;
    }
    synchronized (this.mStateLock) {
      if (this.mState != 2) {
  ...
  private static void postEventFromNative(
    ...
    switch (what) {
      ...
      case 2:
        ...
        return;
      default:
        Log.m96e(TAG, "Unknown native event in postEventFromNative: " + what);
        return;
    }
  }
