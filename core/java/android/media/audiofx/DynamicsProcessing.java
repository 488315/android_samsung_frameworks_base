...
public final class DynamicsProcessing extends AudioEffect {
  ...
  public DynamicsProcessing(int priority, int audioSession, Config cfg) {
    ...
    if (audioSession == 0) {
      Log.m102w(TAG, "WARNING: attaching a DynamicsProcessing to global output mix isdeprecated!");
    }
  ...
  public static final class C1937Eq extends BandStage {
    private final EqBand[] mBands;

    public C1937Eq(boolean inUse, boolean enabled, int bandCount) {
          ...
          if (bandCount > 1) {
            freq =
                (float)
                    Math.pow(
                        10.0d,
                        DynamicsProcessing.mMinFreqLog
                            + ((b
                                    * (DynamicsProcessing.mMaxFreqLog
                                        - DynamicsProcessing.mMinFreqLog))
                                / (bandCount - 1)));
          }
    ...
    @Override // android.media.audiofx.DynamicsProcessing.BandStage,
        ...
        for (int b = 0; b < bandCount; b++) {
          ...
          this.mBands[b] =
              new MbcBand(
                  ...
                  freq,
                  3.0f,
                  DynamicsProcessing.MBC_DEFAULT_RELEASE_TIME,
                  1.0f,
                  DynamicsProcessing.MBC_DEFAULT_THRESHOLD,
                  0.0f,
                  DynamicsProcessing.MBC_DEFAULT_NOISE_GATE_THRESHOLD,
                  1.0f,
                  0.0f,
                  0.0f);
        }
  ...
  public static final class Channel {
    ...
    private Mbc mMbc;
    private C1937Eq mPostEq;
    private C1937Eq mPreEq;
    ...
    public Channel(
      ...
      this.mPreEq = new C1937Eq(preEqInUse, true, preEqBandCount);
      this.mMbc = new Mbc(mbcInUse, true, mbcBandCount);
      this.mPostEq = new C1937Eq(postEqInUse, true, postEqBandCount);
      this.mLimiter =
          new Limiter(
              limiterInUse,
              true,
              ...
              60.0f,
              10.0f,
              DynamicsProcessing.LIMITER_DEFAULT_THRESHOLD,
              0.0f);
    }

    public Channel(Channel cfg) {
      this.mInputGain = cfg.mInputGain;
      this.mPreEq = new C1937Eq(cfg.mPreEq);
      this.mMbc = new Mbc(cfg.mMbc);
      this.mPostEq = new C1937Eq(cfg.mPostEq);
    ...
    public String toString() {
      return String.format(" InputGain: %f\n", Float.valueOf(this.mInputGain))
          + "-->PreEq\n"
          + this.mPreEq.toString()
          + "-->MBC\n"
          + this.mMbc.toString()
          + "-->PostEq\n"
          + this.mPostEq.toString()
          + "-->Limiter\n"
    ...
    public C1937Eq getPreEq() {
      return this.mPreEq;
    }

    public void setPreEq(C1937Eq preEq) {
      if (preEq.getBandCount() != this.mPreEq.getBandCount()) {
        throw new IllegalArgumentException(
            "PreEqBandCount changed from "
                + this.mPreEq.getBandCount()
                + " to "
                + preEq.getBandCount());
      }
      this.mPreEq = new C1937Eq(preEq);
    }
    ...
    public void setMbc(Mbc mbc) {
      if (mbc.getBandCount() != this.mMbc.getBandCount()) {
        throw new IllegalArgumentException(
            "MbcBandCount changed from " + this.mMbc.getBandCount() + " to " + mbc.getBandCount());
      }
    ...
    public C1937Eq getPostEq() {
      return this.mPostEq;
    }

    public void setPostEq(C1937Eq postEq) {
      if (postEq.getBandCount() != this.mPostEq.getBandCount()) {
        throw new IllegalArgumentException(
            "PostEqBandCount changed from "
                + this.mPostEq.getBandCount()
                + " to "
                + postEq.getBandCount());
      }
      this.mPostEq = new C1937Eq(postEq);
    }
  ...
  public static final class Config {
    ...
    public Config(int channelCount, Config cfg) {
      ...
      if (i != cfg.mChannel.length) {
        throw new IllegalArgumentException(
            "configuration channel counts differ " + i + " !=" + cfg.mChannel.length);
      }
      if (channelCount < 1) {
    ...
    public C1937Eq getPreEqByChannelIndex(int channelIndex) {
      checkChannel(channelIndex);
      return this.mChannel[channelIndex].getPreEq();
    }

    public void setPreEqByChannelIndex(int channelIndex, C1937Eq preEq) {
      checkChannel(channelIndex);
      this.mChannel[channelIndex].setPreEq(preEq);
    }

    public void setPreEqAllChannelsTo(C1937Eq preEq) {
      int ch = 0;
      while (true) {
        Channel[] channelArr = this.mChannel;
        if (ch < channelArr.length) {
    ...
    public C1937Eq getPostEqByChannelIndex(int channelIndex) {
      checkChannel(channelIndex);
      return this.mChannel[channelIndex].getPostEq();
    }

    public void setPostEqByChannelIndex(int channelIndex, C1937Eq postEq) {
      checkChannel(channelIndex);
      this.mChannel[channelIndex].setPostEq(postEq);
    }

    public void setPostEqAllChannelsTo(C1937Eq postEq) {
      int ch = 0;
      while (true) {
        Channel[] channelArr = this.mChannel;
        if (ch < channelArr.length) {
    ...
    public static final class Builder {
      ...
      public Builder setPreEqByChannelIndex(int channelIndex, C1937Eq preEq) {
        checkChannel(channelIndex);
        this.mChannel[channelIndex].setPreEq(preEq);
        return this;
      }

      public Builder setPreEqAllChannelsTo(C1937Eq preEq) {
        for (int ch = 0; ch < this.mChannel.length; ch++) {
      ...
      public Builder setPostEqByChannelIndex(int channelIndex, C1937Eq postEq) {
        checkChannel(channelIndex);
        this.mChannel[channelIndex].setPostEq(postEq);
        return this;
      }

      public Builder setPostEqAllChannelsTo(C1937Eq postEq) {
        for (int ch = 0; ch < this.mChannel.length; ch++) {
  ...
  private void setEngineArchitecture(
      int i, float f, boolean z, int i2, boolean z2, int i3, boolean z3, int i4, boolean z4) {
    setNumberArray(
        new Number[] {48},
        new Number[] {
          Integer.valueOf(i),
  ...
  private void updateEngineEqBandByChannelIndex(int i, int i2, int i3, EqBand eqBand) {
    setNumberArray(
        new Number[] {Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(i3)},
        new Number[] {
          Integer.valueOf(eqBand.isEnabled() ? 1 : 0),
          Float.valueOf(eqBand.getCutoffFrequency()),
          Float.valueOf(eqBand.getGain())
        });
  }

  private C1937Eq queryEngineEqByChannelIndex(int param, int channelIndex) {
    ...
    C1937Eq eq = new C1937Eq(values[0].intValue() > 0, values[1].intValue() > 0, bandCount);
    for (int b = 0; b < bandCount; b++) {
  ...
  private EqBand queryEngineEqBandByChannelIndex(int param, int channelIndex, int bandIndex) {
    Number[] params = {
      Integer.valueOf(param), Integer.valueOf(channelIndex), Integer.valueOf(bandIndex)
    };
  ...
  private void updateEngineEqByChannelIndex(int i, int i2, C1937Eq c1937Eq) {
    int bandCount = c1937Eq.getBandCount();
    setNumberArray(
        new Number[] {Integer.valueOf(i), Integer.valueOf(i2)},
        new Number[] {
          Integer.valueOf(c1937Eq.isInUse() ? 1 : 0),
          Integer.valueOf(c1937Eq.isEnabled() ? 1 : 0),
          Integer.valueOf(bandCount)
        });
    for (int i3 = 0; i3 < bandCount; i3++) {
      updateEngineEqBandByChannelIndex(i == 64 ? 69 : 101, i2, i3, c1937Eq.getBand(i3));
    }
  }
  ...
  private MbcBand queryEngineMbcBandByChannelIndex(int channelIndex, int bandIndex) {
    ...
    Float valueOf = Float.valueOf(0.0f);
    Number[] values = {
      0, valueOf, valueOf, valueOf, valueOf, valueOf, valueOf, valueOf, valueOf, valueOf, valueOf
    };
    byte[] paramBytes = numberArrayToByteArray(params);
    byte[] valueBytes = numberArrayToByteArray(values);
    getParameter(paramBytes, valueBytes);
    ...
    return new MbcBand(
        ...
        values[8].floatValue(),
        values[9].floatValue(),
        values[10].floatValue());
  }

  private void updateEngineMbcBandByChannelIndex(int i, int i2, MbcBand mbcBand) {
    setNumberArray(
        new Number[] {85, Integer.valueOf(i), Integer.valueOf(i2)},
        new Number[] {
          Integer.valueOf(mbcBand.isEnabled() ? 1 : 0),
          Float.valueOf(mbcBand.getCutoffFrequency()),
          Float.valueOf(mbcBand.getAttackTime()),
          Float.valueOf(mbcBand.getReleaseTime()),
          Float.valueOf(mbcBand.getRatio()),
          Float.valueOf(mbcBand.getThreshold()),
          Float.valueOf(mbcBand.getKneeWidth()),
          Float.valueOf(mbcBand.getNoiseGateThreshold()),
          Float.valueOf(mbcBand.getExpanderRatio()),
          Float.valueOf(mbcBand.getPreGain()),
          Float.valueOf(mbcBand.getPostGain())
        });
  }
  ...
  private void updateEngineMbcByChannelIndex(int i, Mbc mbc) {
    ...
    setNumberArray(
        new Number[] {80, Integer.valueOf(i)},
        new Number[] {
          Integer.valueOf(mbc.isInUse() ? 1 : 0),
          Integer.valueOf(mbc.isEnabled() ? 1 : 0),
          Integer.valueOf(bandCount)
        });
  ...
  private void updateEngineLimiterByChannelIndex(int i, Limiter limiter) {
    setNumberArray(
        new Number[] {112, Integer.valueOf(i)},
        new Number[] {
          Integer.valueOf(limiter.isInUse() ? 1 : 0),
          Integer.valueOf(limiter.isEnabled() ? 1 : 0),
          Integer.valueOf(limiter.getLinkGroup()),
          Float.valueOf(limiter.getAttackTime()),
          Float.valueOf(limiter.getReleaseTime()),
          Float.valueOf(limiter.getRatio()),
          Float.valueOf(limiter.getThreshold()),
          Float.valueOf(limiter.getPostGain())
        });
  }
  ...
  private Channel queryEngineByChannelIndex(int channelIndex) {
    ...
    C1937Eq preEq = queryEngineEqByChannelIndex(64, channelIndex);
    Mbc mbc = queryEngineMbcByChannelIndex(channelIndex);
    C1937Eq postEq = queryEngineEqByChannelIndex(96, channelIndex);
  ...
  private void updateEngineChannelByChannelIndex(int channelIndex, Channel channel) {
    ...
    C1937Eq preEq = channel.getPreEq();
    updateEngineEqByChannelIndex(64, channelIndex, preEq);
    Mbc mbc = channel.getMbc();
    updateEngineMbcByChannelIndex(channelIndex, mbc);
    C1937Eq postEq = channel.getPostEq();
    updateEngineEqByChannelIndex(96, channelIndex, postEq);
  ...
  private void byteArrayToNumberArray(byte[] valuesIn, Number[] valuesOut) {
    ...
    if (outIndex != valuesOut.length) {
      throw new IllegalArgumentException(
          "only converted " + outIndex + " values out of " + valuesOut.length + " expected");
    }
  }
  ...
  public static class Settings {
    ...
    public String toString() {
      String str =
          new String("DynamicsProcessing;channelCount=" + Integer.toString(this.channelCount));
      for (int ch = 0; ch < this.channelCount; ch++) {
        str =
            str.concat(
                NavigationBarInflaterView.GRAVITY_SEPARATOR
                    + ch
                    + "_inputGain="
                    + Float.toString(this.inputGain[ch]));
      }
      return str;
    }
  }
  ...
  public void setProperties(Settings settings) {
    if (settings.channelCount != settings.inputGain.length
        || settings.channelCount != this.mChannelCount) {
      throw new IllegalArgumentException(
          "settings invalid channel count: " + settings.channelCount);
    }
    for (int ch = 0; ch < this.mChannelCount; ch++) {}
  }
}
