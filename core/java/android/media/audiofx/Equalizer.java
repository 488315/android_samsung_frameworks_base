...
public class Equalizer extends AudioEffect {
  ...
  public Equalizer(int priority, int audioSession)
    ...
    if (audioSession == 0) {
      Log.m102w(TAG, "WARNING: attaching an Equalizer to global output mix is deprecated!");
    }
    getNumberOfBands();
    ...
    if (numberOfPresets != 0) {
      ...
      for (int i = 0; i < this.mNumPresets; i++) {
        ...
        try {
          ...
          Log.m96e(TAG, "preset name decode error");
        }
      }
    }
  }
  ...
  public static class Settings {
    ...
    public String toString() {
      String str =
          new String(
              "Equalizer;curPreset="
                  + Short.toString(this.curPreset)
                  + ";numBands="
                  + Short.toString(this.numBands));
  ...
  public void setProperties(Settings settings)
    ...
    byte[] param =
        concatArrays(shortToByteArray(settings.curPreset), shortToByteArray(this.mNumBands));
    for (int i = 0; i < this.mNumBands; i++) {
