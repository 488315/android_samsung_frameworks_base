...
class Cea608CCParser {
  ...
  /* renamed from: BS */
  private static final int f273BS = 33;

  /* renamed from: CR */
  private static final int f274CR = 45;
  ...
  /* renamed from: TR */
  private static final int f275TR = 42;

  /* renamed from: TS */
  private static final char f276TS = 160;
  ...
  public void parse(byte[] data) {
    ...
    for (int i = 0; i < ccData.length; i++) {
      if (DEBUG) {
        Log.m94d(TAG, ccData[i].toString());
      }
  ...
  private CCMemory getMemory() {
    switch (this.mMode) {
      ...
      default:
        Log.m102w(TAG, "unrecoginized mode: " + this.mMode);
        return this.mDisplay;
    }
  }
  ...
  private static class PAC extends StyleCode {
    ...
    public String toString() {
      return String.format(
          "{%d, %d}, %s", Integer.valueOf(this.mRow), Integer.valueOf(this.mCol), super.toString());
    }
  }
  ...
  private static class CCMemory {
    ...
    CCMemory() {
      ...
      Arrays.fill(blank, Cea608CCParser.f276TS);
        ...
        for (int i = 0; i < this.mCol; i++) {
          if (this.mLines[this.mRow].charAt(i) != 160) {
            for (int j = this.mCol; j < this.mLines[this.mRow].length(); j++) {
              this.mLines[j].setCharAt(j, Cea608CCParser.f276TS);
            }
            return;
          }
        }
    ...
    void m51bs() {
      ...
      if (cCLineBuilder != null) {
        cCLineBuilder.setCharAt(this.mCol, Cea608CCParser.f276TS);
        if (this.mCol == 31) {
          this.mLines[this.mRow].setCharAt(32, Cea608CCParser.f276TS);
        }
      }
    }
