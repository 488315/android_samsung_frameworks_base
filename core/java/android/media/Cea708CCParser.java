...
class Cea708CCParser {
  ...
  Cea708CCParser(DisplayListener listener) {
    this.mListener =
        new DisplayListener() { // from class: android.media.Cea708CCParser.1
      ...
      if (i == 24) {
        try {
          if (data[pos] == 0) {
            ...
            String value =
                new String(Arrays.copyOfRange(data, pos, pos + 2), CharacterSets.MIMENAME_EUC_KR);
            this.mBuffer.append(value);
          }
        } catch (UnsupportedEncodingException e) {
          Log.m97e(TAG, "P16 Code - Could not find supported encoding", e);
        }
      }
      return pos + 2;
    }
  ...
  private int parseC1(byte[] data, int pos) {
    ...
    switch (windowBitmap) {
      ...
      case 145:
        ...
        int pos11 = pos10 + 1;
        emitCaptionEvent(
            new CaptionEvent(13, new CaptionPenColor(foregroundColor, backgroundColor, edgeColor)));
        return pos11;
      case 146:
      ...
      case 151:
        ...
        int borderType =
            ((data[pos + 2] & 128) >> 5)
                | ((data[pos + 1] & MidiConstants.STATUS_PROGRAM_CHANGE) >> 6);
  ...
  public static class CaptionPenColor {
    ...
    public CaptionPenColor(
        CaptionColor foregroundColor, CaptionColor backgroundColor, CaptionColor edgeColor) {
