    ...
    private class Transport extends IAccountAuthenticator.Stub {
        ...
        public void addAccount(IAccountAuthenticatorResponse response, String accountType, String authTokenType, String[] features, Bundle options) throws RemoteException {
            ...
            if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                Log.m100v(AbstractAccountAuthenticator.TAG, "addAccount: accountType " + accountType + ", authTokenType " + authTokenType + ", features " + (features == null ? "[]" : Arrays.toString(features)));
            }
            try {
                ...
                if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                    ...
                    Log.m100v(AbstractAccountAuthenticator.TAG, "addAccount: result " + AccountManager.sanitizeResult(result));
                }
                if (result != null) {
        ...
        public void confirmCredentials(IAccountAuthenticatorResponse response, Account account, Bundle options) throws RemoteException {
            ...
            try {
                ...
                if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                    ...
                    Log.m100v(AbstractAccountAuthenticator.TAG, "confirmCredentials: result " + AccountManager.sanitizeResult(result));
                }
                if (result != null) {
                    response.onResult(result);
                }
        ...
        public void getAuthTokenLabel(IAccountAuthenticatorResponse response, String authTokenType) throws RemoteException {
            ...
            if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                Log.m100v(AbstractAccountAuthenticator.TAG, "getAuthTokenLabel: authTokenType " + authTokenType);
            }
            try {
                ...
                if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                    result.keySet();
                    Log.m100v(AbstractAccountAuthenticator.TAG, "getAuthTokenLabel: result " + AccountManager.sanitizeResult(result));
                }
                response.onResult(result);
            } catch (Exception e) {
        ...
        public void getAuthToken(IAccountAuthenticatorResponse response, Account account, String authTokenType, Bundle loginOptions) throws RemoteException {
            ...
            if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                Log.m100v(AbstractAccountAuthenticator.TAG, "getAuthToken: " + account + ", authTokenType " + authTokenType);
            }
            try {
                ...
                if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                    ...
                    Log.m100v(AbstractAccountAuthenticator.TAG, "getAuthToken: result " + AccountManager.sanitizeResult(result));
                }
                if (result != null) {
                    response.onResult(result);
                }
        ...
        public void updateCredentials(IAccountAuthenticatorResponse response, Account account, String authTokenType, Bundle loginOptions) throws RemoteException {
            ...
            if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                Log.m100v(AbstractAccountAuthenticator.TAG, "updateCredentials: " + account + ", authTokenType " + authTokenType);
            }
            try {
                ...
                if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                    ...
                    Log.m100v(AbstractAccountAuthenticator.TAG, "updateCredentials: result " + AccountManager.sanitizeResult(result));
                }
                if (result != null) {
                    response.onResult(result);
                }
        ...
        public void startAddAccountSession(IAccountAuthenticatorResponse response, String accountType, String authTokenType, String[] features, Bundle options) throws RemoteException {
            ...
            if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                Log.m100v(AbstractAccountAuthenticator.TAG, "startAddAccountSession: accountType " + accountType + ", authTokenType " + authTokenType + ", features " + (features == null ? "[]" : Arrays.toString(features)));
            }
            try {
                ...
                if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                    ...
                    Log.m100v(AbstractAccountAuthenticator.TAG, "startAddAccountSession: result " + AccountManager.sanitizeResult(result));
                }
                if (result != null) {
                    response.onResult(result);
                }
        ...
        public void startUpdateCredentialsSession(IAccountAuthenticatorResponse response, Account account, String authTokenType, Bundle loginOptions) throws RemoteException {
            ...
            if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                Log.m100v(AbstractAccountAuthenticator.TAG, "startUpdateCredentialsSession: " + account + ", authTokenType " + authTokenType);
            }
            try {
                ...
                if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                    ...
                    Log.m100v(AbstractAccountAuthenticator.TAG, "startUpdateCredentialsSession: result " + AccountManager.sanitizeResult(result));
                }
                if (result != null) {
                    response.onResult(result);
                }
        ...
        public void finishSession(IAccountAuthenticatorResponse response, String accountType, Bundle sessionBundle) throws RemoteException {
            ...
            try {
                ...
                if (Log.isLoggable(AbstractAccountAuthenticator.TAG, 2)) {
                    Log.m100v(AbstractAccountAuthenticator.TAG, "finishSession: result " + AccountManager.sanitizeResult(result));
                }
                if (result != null) {
                    response.onResult(result);
                }
    ...
    public void handleException(IAccountAuthenticatorResponse response, String method, String data, Exception e) throws RemoteException {
        if (e instanceof NetworkErrorException) {
            if (Log.isLoggable(TAG, 2)) {
                Log.m101v(TAG, method + NavigationBarInflaterView.KEY_CODE_START + data + NavigationBarInflaterView.KEY_CODE_END, e);
            }
