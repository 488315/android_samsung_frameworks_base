...
public class ChooseTypeAndAccountActivity extends Activity implements AccountManagerCallback<Bundle> {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (Log.isLoggable(TAG, 2)) {
            Log.m100v(TAG, "ChooseTypeAndAccountActivity.onCreate(savedInstanceState=" + savedInstanceState + NavigationBarInflaterView.KEY_CODE_END);
        }
        ...
        if (SemViewUtils.isOpenThemeApplied(this)) {
            ...
            Log.m98i(TAG, "onCreate: colorPalette=true isNightMode=" + isNightMode + " titleColor=0x" + Integer.toHexString(titleColor) + " buttonTextColor=0x" + Integer.toHexString(buttonTextColor2));
        }
        ...
        Log.m98i(TAG, "Ok textColors=" + okTextColors + " defaultColor=0x" + Integer.toHexString(okTextColors.getDefaultColor()));
        ColorStateList cancelTextColors = this.mCancelButton.getTextColors();
        Log.m98i(TAG, "Cancel textColors=" + cancelTextColors + " defaultColor=0x" + Integer.toHexString(cancelTextColors.getDefaultColor()));
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (Log.isLoggable(TAG, 2)) {
            ...
            Log.m100v(TAG, "ChooseTypeAndAccountActivity.onActivityResult(reqCode=" + requestCode + ", resCode=" + resultCode + NavigationBarInflaterView.KEY_CODE_END);
        }
        this.mPendingRequest = 0;
        if (resultCode == 0) {
        ...
        if (resultCode == -1) {
            if (requestCode == 1) {
                if (data == null || (accountType = data.getStringExtra("accountType")) == null) {
                    Log.m94d(TAG, "ChooseTypeAndAccountActivity.onActivityResult: unable to find account type, pretending the request was canceled");
                } else {
            ...
            Log.m94d(TAG, "ChooseTypeAndAccountActivity.onActivityResult: unable to find added account, pretending the request was canceled");
        }
        if (Log.isLoggable(TAG, 2)) {
    ...
    private void setResultAndFinish(String accountName, String accountType) {
        ...
        setResult(-1, new Intent().putExtras(bundle));
        if (Log.isLoggable(TAG, 2)) {
            Log.m100v(TAG, "ChooseTypeAndAccountActivity.setResultAndFinish: selected account " + account.toSafeString());
        }
        finish();
    }
    ...
    private final void populateUIAccountList(String[] listItems) {
        ...
        list.setAdapter((ListAdapter) new ArrayAdapter(this, C4337R.layout.sem_choose_type_and_account_dialog_item_layout, listItems));
