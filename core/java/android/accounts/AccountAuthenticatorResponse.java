...
public class AccountAuthenticatorResponse implements Parcelable {
  public static final Parcelable.Creator<AccountAuthenticatorResponse> CREATOR =
      new Parcelable.Creator<
          AccountAuthenticatorResponse>() { // from class:
  ...
  public void onResult(Bundle result) {
    if (Log.isLoggable(TAG, 2)) {
      result.keySet();
      Log.m100v(
          TAG, "AccountAuthenticatorResponse.onResult: " + AccountManager.sanitizeResult(result));
    }
    try {
  ...
  public void onRequestContinued() {
    if (Log.isLoggable(TAG, 2)) {
      Log.m100v(TAG, "AccountAuthenticatorResponse.onRequestContinued");
    }
    try {
  ...
  public void onError(int errorCode, String errorMessage) {
    if (Log.isLoggable(TAG, 2)) {
      Log.m100v(TAG, "AccountAuthenticatorResponse.onError: " + errorCode + ", " + errorMessage);
    }
    try {
