...
public final class PrinterDiscoverySession {
    ...
    PrinterDiscoverySession(IPrintManager printManager, Context context, int userId) {
        ...
        try {
            ...
            Log.m97e(LOG_TAG, "Error creating printer discovery session", re);
        }
    }

    public final void startPrinterDiscovery(List<PrinterId> priorityList) {
        if (isDestroyed()) {
            Log.m102w(LOG_TAG, "Ignoring start printers discovery - session destroyed");
            return;
        }
        if (!this.mIsPrinterDiscoveryStarted) {
            ...
            try {
                ...
                Log.m97e(LOG_TAG, "Error starting printer discovery", re);
            }
        }
    }
    ...
    public final void stopPrinterDiscovery() {
        if (isDestroyed()) {
            Log.m102w(LOG_TAG, "Ignoring stop printers discovery - session destroyed");
            return;
        }
        if (this.mIsPrinterDiscoveryStarted) {
            ...
            try {
                ...
                Log.m97e(LOG_TAG, "Error stopping printer discovery", re);
            }
        }
    }
    ...
    public final void startPrinterStateTracking(PrinterId printerId) {
        if (isDestroyed()) {
            Log.m102w(LOG_TAG, "Ignoring start printer state tracking - session destroyed");
            return;
        }
        try {
            this.mPrintManager.startPrinterStateTracking(printerId, this.mUserId);
        } catch (RemoteException re) {
            Log.m97e(LOG_TAG, "Error starting printer state tracking", re);
        }
    }

    public final void stopPrinterStateTracking(PrinterId printerId) {
        if (isDestroyed()) {
            Log.m102w(LOG_TAG, "Ignoring stop printer state tracking - session destroyed");
            return;
        }
        try {
            this.mPrintManager.stopPrinterStateTracking(printerId, this.mUserId);
        } catch (RemoteException re) {
            Log.m97e(LOG_TAG, "Error stopping printer state tracking", re);
        }
    }

    public final void validatePrinters(List<PrinterId> printerIds) {
        if (isDestroyed()) {
            Log.m102w(LOG_TAG, "Ignoring validate printers - session destroyed");
            return;
        }
        try {
            this.mPrintManager.validatePrinters(printerIds, this.mUserId);
        } catch (RemoteException re) {
            Log.m97e(LOG_TAG, "Error validating printers", re);
        }
    }

    public final void destroy() {
        if (isDestroyed()) {
            Log.m102w(LOG_TAG, "Ignoring destroy - session destroyed");
        }
        destroyNoCheck();
    }
    ...
    public final List<PrinterInfo> getPrinters() {
        if (isDestroyed()) {
            Log.m102w(LOG_TAG, "Ignoring get printers - session destroyed");
    ...
    protected final void finalize() throws Throwable {
        if (!isDestroyedNoCheck()) {
            Log.m96e(LOG_TAG, "Destroying leaked printer discovery session");
            destroyNoCheck();
        }
        super.finalize();
    }
    ...
    private void destroyNoCheck() {
        ...
        try {
            try {
                ...
                Log.m97e(LOG_TAG, "Error destroying printer discovery session", e);
            }
        } finally {
            this.mObserver = r0;
