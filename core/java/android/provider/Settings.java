...
public final class Settings {
  ...
  public static final String ACTION_NOTIFICATION_POPUP_STYLE_SETTINGS =
      "android.settings.NOTIFICATION_POPUP_STYLE_SETTINGS";
  ...
  @SystemApi public static final String ACTION_TETHER_SETTINGS = "android.settings.TETHER_SETTINGS";
  ...
  @SystemApi public static final String ACTION_USER_SETTINGS = "android.settings.USER_SETTINGS";
  ...
  static {
    ...
    PM_CHANGE_NETWORK_STATE =
        new String[] {Manifest.permission.CHANGE_NETWORK_STATE, Manifest.permission.WRITE_SETTINGS};
  ...
  public static class NameValueTable implements BaseColumns {
    ...
    protected static boolean putString(
        ContentResolver resolver, Uri uri, String name, String value) {
      try {
  ...
  private static final class GenerationTracker {
    ...
    public boolean isGenerationChanged() {
      ...
      if (DEBUG) {
        Log.m94d(
            Settings.TAG,
            "isGenerationChanged() : " + currentGeneration + " : " + this.mCurrentGeneration);
      }
    ...
    private int readCurrentGeneration() {
      try {
        ...
        Log.m97e(Settings.TAG, "Error getting current generation", e);
    ...
    public void destroy() {
      try {
        ...
        Log.m97e(Settings.TAG, "Error closing backing array", e);
      }
    }
  ...
  static class NameValueCache {
    ...
    public /* synthetic */ void lambda$new$0(String name) {
      synchronized (this) {
        Log.m96e(Settings.TAG, "Error accessing generation tracker - removing");
    ...
    <T extends NameValueTable> NameValueCache(
        Uri uri,
        String getCommand,
        String setCommand,
    ...
    private <T extends NameValueTable> NameValueCache(
        Uri uri,
        String getCommand,
        String setCommand,
        String deleteCommand,
        String listCommand,
        String setAllCommand,
        ContentProviderHolder providerHolder,
        Class<T> callerClass) {
      this.mValues = new ArrayMap<>();
      this.mGenerationTrackers = new ArrayMap<>();
      this.mGenerationTrackerErrorHandler =
    ...
    public boolean putStringForUser(
      ...
      try {
        ...
        cp.call(
            ...
            this.mProviderHolder.mUri.getAuthority(),
            this.mCallSetCommand,
            name,
            arg);
    ...
    private void semDumpCallStackIfNeeded(
        String name, String value, String packageName, int userId) {
      if (!Settings.CALL_METHOD_PUT_SECURE.equals(this.mCallSetCommand)
          || !Secure.ENABLED_ACCESSIBILITY_SERVICES.equals(name)) {
        return;
      }
      Context context = ActivityThread.currentApplication();
      if (context == null) {
        Log.m94d(Settings.TAG, "can't get context for a11y callstack");
        return;
      }
    ...
    public int setStringsForPrefix(
        ContentResolver cr, String prefix, HashMap<String, String> keyValues) {
    ...
    public String getStringForUser(ContentResolver cr, String name, int userHandle) {
      ...
      if (useCache) {
        synchronized (this) {
          ...
          if (generationTracker != null) {
            if (generationTracker.isGenerationChanged()) {
              if (DEBUG) {
                Log.m98i(
                    Settings.TAG,
                    "Generation changed for setting:"
                        + name
                        + " type:"
                        + this.mUri.getPath()
                        + " in package:"
          ...
          if (DEBUG) {
            Log.m98i(Settings.TAG, "Cache miss for setting:" + name + " for user:" + userHandle);
          }
      ...
      if (!isCallerExemptFromReadableRestriction() && this.mAllFields.contains(name)) {
        if (!this.mReadableFields.contains(name)) {
          throw new SecurityException(
              "Settings key: <"
                  + name
                  + "> is not readable. From S+, settings keys annotated with @hide are restricted"
                  + " to system_server and system apps only, unless they are annotated with"
                  + " @Readable.");
        }
        if (this.mReadableFieldsWithMaxTargetSdk.containsKey(name)) {
          ...
          if (!targetSdkCheckOk) {
            throw new SecurityException(
                "Settings key: <"
                    + name
                    + "> is only readable to apps with targetSdkVersion lower than or equal to: "
                    + maxTargetSdk);
          }
        }
      }
        ...
        try {
          ...
          if (needsGenerationTracker) {
            ...
            if (DEBUG) {
              Log.m98i(
                  Settings.TAG,
                  "Requested generation tracker for setting:"
                      + name
                      + " type:"
                      + this.mUri.getPath()
                      + " in package:"
              ...
              if (isSelf) {
                synchronized (this) {
                  try {
                        ...
                        if (array == null || index < 0) {
                          ...
                          if (DEBUG) {
                            try {
                              Log.m98i(
                                  Settings.TAG,
                                  "Received generation tracker for setting:"
                                      + name
                                      + " type:"
                                      + this.mUri.getPath()
                                      + " in package:"
                    ...
                    if (this.mGenerationTrackers.get(name) != null
                        && !this.mGenerationTrackers.get(name).isGenerationChanged()) {
                      if (DEBUG) {
                        Log.m98i(Settings.TAG, "Updating cache for setting:" + name);
                      }
                ...
                if (DEBUG) {
                  Log.m98i(
                      ...
                      "call-query of user "
                          ...
                          + " by "
                          + UserHandle.myUserId()
                          + (Settings.isInSystemServer() ? " in system_server" : "")
      ...
      try {
        try {
          try {
            ...
            if (c2 == null) {
              Log.m102w(Settings.TAG, "Can't get key " + name + " from " + this.mUri);
              return str;
            }
            ...
            synchronized (this) {
              if (this.mGenerationTrackers.get(name) != null
                  && !this.mGenerationTrackers.get(name).isGenerationChanged()) {
                if (DEBUG) {
                  Log.m98i(Settings.TAG, "Updating cache for setting:" + name + " using query");
                }
    ...
    */
    public ArrayMap<String, String> getStringsForPrefix(
      ...
      boolean needsGenerationTracker;
      Bundle b;
      int currentGeneration2;
      String str;
      int currentGeneration3;
      ...
      synchronized (this) {
        try {
          ...
          if (generationTracker != null) {
            try {
              if (generationTracker.isGenerationChanged()) {
                ...
                if (prefixCached) {
                  if (DEBUG) {
                    Log.m98i(Settings.TAG, "Cache hit for prefix:" + prefix);
                  }
                  if (names.isEmpty()) {
              ...
              int currentGeneration4 = generationTracker.getCurrentGeneration();
              currentGeneration = currentGeneration4;
              needsGenerationTracker = false;
            } catch (Throwable th) {
              e = th;
              while (true) {
          ...
          try {
            ...
            boolean z = DEBUG;
            if (z) {
              Log.m98i(Settings.TAG, "Cache miss for prefix:" + prefix);
            }
            IContentProvider cp = this.mProviderHolder.getProvider(cr);
            try {
              Bundle args = new Bundle();
              ...
              if (needsGenerationTracker) {
                try {
                  ...
                  if (z) {
                    Log.m98i(
                        Settings.TAG,
                        "Requested generation tracker for prefix:"
                            + prefix
                            + " type: "
                            + this.mUri.getPath()
                            + " in package:"
              ...
              if ((namespace.equals("device_policy_manager")
                      || namespace.equals(Context.AUTOFILL_MANAGER_SERVICE))
              ...
              Map<String, String> flagsToValues =
                  (HashMap) b.getSerializable("value", HashMap.class);
              if (names.isEmpty()) {
              ...
              synchronized (this) {
                try {
                  if (needsGenerationTracker) {
                    try {
                      MemoryIntArray array =
                          (MemoryIntArray)
                              b.getParcelable(
                                  Settings.CALL_METHOD_TRACK_GENERATION_KEY, MemoryIntArray.class);
                      int index = b.getInt(Settings.CALL_METHOD_GENERATION_INDEX_KEY, -1);
                      if (array != null && index >= 0) {
                        int generation = b.getInt(Settings.CALL_METHOD_GENERATION_KEY, 0);
                        if (DEBUG) {
                          try {
                            Log.m98i(
                                Settings.TAG,
                                "Received generation tracker for prefix:"
                                    + prefix
                                    + " type:"
                                    + this.mUri.getPath()
                                    + " in package:"
                        ...
                        try {
                          this.mGenerationTrackers.put(
                              ...
                              new GenerationTracker(
                                  ...
                                  generation,
                                  this.mGenerationTrackerErrorHandler));
                          currentGeneration3 = generation;
                          if (this.mGenerationTrackers.get(prefix) != null
                              && currentGeneration3
                                  == this.mGenerationTrackers.get(prefix).getCurrentGeneration()) {
                            if (DEBUG) {
                              Log.m98i(Settings.TAG, "Updating cache for prefix:" + prefix);
                            }
                            this.mValues.putAll(flagsToValues);
                            this.mValues.put(prefix, str);
                          }
                          return keyValues;
                        } catch (Throwable th5) {
                          th = th5;
                      ...
                      currentGeneration2 = currentGeneration;
                      str = null;
                    } catch (Throwable th6) {
                      th = th6;
                    }
                  ...
                  currentGeneration3 = currentGeneration2;
                  if (this.mGenerationTrackers.get(prefix) != null) {
                    if (DEBUG) {}
                    this.mValues.putAll(flagsToValues);
                    this.mValues.put(prefix, str);
  ...
  public static boolean canDrawOverlays(Context context) {
    return isCallingPackageAllowedToDrawOverlays(
            context, Process.myUid(), context.getOpPackageName(), false)
  ...
  public static final class System extends NameValueTable {
    ...
    @Readable public static final String ACCELEROMETER_ROTATION = "accelerometer_rotation";
    ...
    @Readable public static final String ACTION_MEMO_ON_OFF_SCREEN = "action_memo_on_off_screen";
    public static final String ADAPTIVE_BRIGHTNESS = "adaptive_brightness";

    @Readable public static final String ADAPTIVE_FAST_CHARGING = "adaptive_fast_charging";

    @Readable @Deprecated public static final String ADAPTIVE_SLEEP = "adaptive_sleep";

    @Deprecated public static final String ADB_ENABLED = "adb_enabled";

    @Readable public static final String ADVANCED_SETTINGS = "advanced_settings";
    public static final int ADVANCED_SETTINGS_DEFAULT = 0;

    @Deprecated public static final String AIRPLANE_MODE_ON = "airplane_mode_on";

    @Deprecated public static final String AIRPLANE_MODE_RADIOS = "airplane_mode_radios";

    @Deprecated
    public static final String AIRPLANE_MODE_TOGGLEABLE_RADIOS = "airplane_mode_toggleable_radios";

    @Readable public static final String AIR_VIEW_MASTER_ONOFF = "air_view_master_onoff";

    @Readable public static final String ALARM_ALERT = "alarm_alert";

    @Readable public static final String ALARM_ALERT_CACHE = "alarm_alert_cache";
    ...
    @Deprecated public static final String ALWAYS_FINISH_ACTIVITIES = "always_finish_activities";

    @Deprecated public static final String ANDROID_ID = "android_id";

    @Deprecated public static final String ANIMATOR_DURATION_SCALE = "animator_duration_scale";

    @Readable
    public static final String AOD_SHOW_LOCKSCREEN_WALLPAPER = "aod_show_lockscreen_wallpaper";

    @Readable public static final String APPEND_FOR_LAST_AUDIBLE = "_last_audible";

    @Readable public static final String APPLY_RAMPING_RINGER = "apply_ramping_ringer";

    @Readable public static final String APP_VOLUME_ENABLED = "app_volume_enabled";

    @Readable public static final String ATC_MODE_ENABLED = "atc_mode_enabled";

    @Readable public static final String AUTO_ADJUST_TOUCH = "auto_adjust_touch";
    public static final String AUTO_LAUNCH_MEDIA_CONTROLS = "auto_launch_media_controls";

    @Deprecated public static final String AUTO_TIME = "auto_time";

    @Deprecated public static final String AUTO_TIME_ZONE = "auto_time_zone";

    @Readable
    public static final String BIOMETRICS_SCREEN_TRANSITION_EFFECT = "screen_transition_effect";

    @Readable public static final String BLUETOOTH_DISCOVERABILITY = "bluetooth_discoverability";

    @Readable
    public static final String BLUETOOTH_DISCOVERABILITY_TIMEOUT =
        "bluetooth_discoverability_timeout";

    @Deprecated public static final String BLUETOOTH_ON = "bluetooth_on";
    public static final String BLUETOOTH_SECURITY_ON_CHECK = "bluetooth_security_on_check";

    @Readable public static final String BLUE_LIGHT_FILTER = "blue_light_filter";
    ...
    @Readable public static final String BLUE_LIGHT_FILTER_OFF_TIME = "blue_light_filter_off_time";

    @Readable public static final String BLUE_LIGHT_FILTER_ON_TIME = "blue_light_filter_on_time";

    @Readable public static final String BLUE_LIGHT_FILTER_OPACITY = "blue_light_filter_opacity";

    @Readable
    public static final String BLUE_LIGHT_FILTER_SCHEDULED = "blue_light_filter_scheduled";

    @Readable public static final String BLUE_LIGHT_FILTER_TYPE = "blue_light_filter_type";

    @Readable
    public static final String CALL_ANSWERRING_MESSAGE_AUTO_ANSWER_WITH_MEMO =
        "callsettings_answer_memo";
    ...
    @Readable public static final String CALL_AUTOMATIC_ANSWERING_SEC = "answeringmode_auto_time";

    @Readable
    public static final String CALL_AUTO_RECORD_CALLS = "record_calls_automatically_on_off";

    @Readable public static final String CALL_DISPLAY_CALLER_INFO_CARD = "display_caller_info_card";
    ...
    @Readable public static final String CALL_VIDEO_CALL_QUALITY_INFO = "videocallmessage_settings";

    @Readable public static final String CALL_VIDEO_SPEAKER = "videocall_speaker";

    @Readable
    public static final String CALL_VOICE_CALL_PROTECTION = "enable_call_protect_when_calling";

    @Readable public static final String CAMERA_FEEDBACK_VIBRATE = "camera_feedback_vibrate";
    public static final String CAMERA_FLASH_NOTIFICATION = "camera_flash_notification";

    @Deprecated public static final String CAR_DOCK_SOUND = "car_dock_sound";

    @Deprecated public static final String CAR_UNDOCK_SOUND = "car_undock_sound";

    @Readable public static final String CHAMELEON_DOMROAMMAXUSER = "chameleon_domroammaxuser";

    @Readable public static final String CHAMELEON_GSMMAXUSER = "chameleon_gsmmaxuser";

    @Readable public static final String CHAMELEON_INTROAMMAXUSER = "chameleon_introammaxuser";

    @Readable public static final String CHAMELEON_MAXUSER = "chameleon_maxuser";

    @Readable public static final String CHAMELEON_SSID = "chameleon_ssid";
    ...
    @Deprecated public static final String DATA_ROAMING = "data_roaming";

    @Readable @Deprecated public static final String DATE_FORMAT = "date_format";

    @Readable public static final String DAY_OF_WEEK = "day_of_week";

    @Deprecated public static final String DEBUG_APP = "debug_app";
    ...
    @Readable
    public static final String DEFAULT_ASSIST_VIBRATION_FEEDBACK =
        "default_assist_vibration_feedback";
    ...
    @Readable public static final String DESKTOP_MODE = "desktop_mode";

    @Deprecated public static final String DESK_DOCK_SOUND = "desk_dock_sound";

    @Deprecated public static final String DESK_UNDOCK_SOUND = "desk_undock_sound";

    @Deprecated public static final String DEVICE_PROVISIONED = "device_provisioned";

    @Readable public static final String DIALING_KEYPAD_VIBRATE = "dialing_keypad_vibrate";

    @Readable @Deprecated public static final String DIM_SCREEN = "dim_screen";

    @Readable public static final String DIRECT_SHARE = "direct_share";
    public static final String DISABLE_DEXCOMPAT_RESTART_DIALOG =
        "disable_dexcompat_restart_dialog";
    public static final String DISPLAY_CHOOSER_DO_NOT_SHOW_AGAIN =
        "display_chooser_do_not_show_again";

    @Readable public static final String DISPLAY_COLOR_MODE = "display_color_mode";
    public static final String DISPLAY_COLOR_MODE_VENDOR_HINT = "display_color_mode_vendor_hint";

    @Readable public static final String DISPLAY_NIGHT_THEME = "display_night_theme";
    ...
    @Readable
    public static final String DISPLAY_NIGHT_THEME_SCHEDULED_TYPE =
        "display_night_theme_scheduled_type";

    @Readable
    public static final String DISPLAY_NIGHT_THEME_WALLPAPER = "display_night_theme_wallpaper";

    @Readable public static final String DOCK_SCREEN_OFF_TIMEOUT = "dock_screen_off_timeout";

    @Readable
    public static final String DOCK_SCREEN_OFF_TIMEOUT_ENABLED = "dock_screen_off_timeout_enabled";

    @Deprecated public static final String DOCK_SOUNDS_ENABLED = "dock_sounds_enabled";

    @Readable public static final String DOUBLE_TAB_TO_WAKE_UP = "double_tab_to_wake_up";

    @Readable public static final String DOUBLE_TAP_TO_SLEEP = "double_tap_to_sleep";

    @Readable public static final String DQE_TUNE_ENABLED = "dqe_tune_enabled";

    @Readable public static final String DTMF_TONE_TYPE_WHEN_DIALING = "dtmf_tone_type";

    @Readable public static final String DTMF_TONE_WHEN_DIALING = "dtmf_tone";
    ...
    @Readable public static final String EDGE_INFORMATION_STREAM = "edge_information_stream";

    @Readable public static final String EGG_MODE = "egg_mode";

    @Readable public static final String ENABLED_SIM2_ONLY = "enabled_sim2_only";

    @Readable public static final String END_BUTTON_BEHAVIOR = "end_button_behavior";
    ...
    @Readable public static final String ENHANCE_POINTER_PRECISION = "enhance_pointer_precision";
    public static final String ETH_DEVICE_CONNECTED = "eth_device_conn";
    public static final String ETH_DISABLED = "eth_disabled";

    @Readable public static final String FINGER_AIR_VIEW = "finger_air_view";

    @Readable public static final String FINGER_AIR_VIEW_HIGHLIGHT = "finger_air_view_highlight";

    @Readable
    public static final String FINGER_AIR_VIEW_INFORMATION_PREVIEW =
        "finger_air_view_information_preview";

    @Readable public static final String FLEX_MODE_PANEL_SHOW_SETTINGS_ICON = "show_setting_icon";

    @Readable public static final String FLIPFONT = "flipfont";
    public static final String FOLDABLE_RESTART_DIALOG_DO_NOT_SHOW_AGAIN =
        "foldable_restart_dialog_do_not_show_again";

    @Readable public static final String FOLDER_SOUNDS_ENABLED = "folder_sounds_enabled";

    @Readable public static final String FONT_SCALE = "font_scale";

    @Readable @Deprecated
    public static final String HAPTIC_FEEDBACK_ENABLED = "haptic_feedback_enabled";

    @Readable public static final String HAPTIC_FEEDBACK_INTENSITY = "haptic_feedback_intensity";
    public static final String HARDWARE_HAPTIC_FEEDBACK_INTENSITY =
        "hardware_haptic_feedback_intensity";

    @Readable public static final String HDR_EFFECT = "hdr_effect";

    @Readable public static final String HEARING_AID = "hearing_aid";

    @Readable
    public static final String HIDE_ROTATION_LOCK_TOGGLE_FOR_ACCESSIBILITY =
        "hide_rotation_lock_toggle_for_accessibility";

    @Readable public static final String HOVER_MAGNIFIER_SCALE = "hover_zoom_value";

    @Readable public static final String HOVER_ZOOM_MAGNIFIER_SIZE = "hover_zoom_magnifier_size";
    ...
    @Readable public static final String INTELLIGENT_SLEEP_MODE = "intelligent_sleep_mode";

    @Readable public static final String KEY_BACKLIGHT_TIMEOUT = "key_backlight_timeout";

    @Readable public static final String KEY_NIGHT_MODE = "key_night_mode";

    @Readable public static final String KG_MULTIPLE_LOCKSCREEN = "kg_multiple_lockscreen";
    ...
    @Readable public static final String LARGE_COVER_SCREEN_APPS = "large_cover_screen_apps";
    ...
    @Readable public static final String LIFT_TO_WAKE = "lift_to_wake";
    public static final String LOCALE_PREFERENCES = "locale_preferences";

    @Deprecated
    public static final String LOCATION_PROVIDERS_ALLOWED = "location_providers_allowed";

    @Readable public static final String LOCKSCREEN_DISABLED = "lockscreen.disabled";

    @Readable public static final String LOCKSCREEN_SOUNDS_ENABLED = "lockscreen_sounds_enabled";

    @Readable public static final String LOCK_APPLICATION_SHORTCUT = "lock_application_shortcut";

    @Readable public static final String LOCK_NOTICARD_OPACITY = "lock_noticard_opacity";

    @Readable
    public static final String LOCK_NOTICARD_OPACITY_DARK_MODE = "lock_noticard_opacity_dark_mode";

    @Deprecated public static final String LOCK_PATTERN_ENABLED = "lock_pattern_autolock";
    ...
    @Readable public static final String LOCK_SCREEN_SHORTCUT = "lock_screen_shortcut";

    @Deprecated public static final String LOCK_SOUND = "lock_sound";

    @Readable public static final String LOCK_TO_APP_ENABLED = "lock_to_app_enabled";

    @Deprecated public static final String LOGGING_ID = "logging_id";

    @Deprecated public static final String LOW_BATTERY_SOUND = "low_battery_sound";

    @Readable public static final String MASTER_BALANCE = "master_balance";

    @Readable public static final String MASTER_MONO = "master_mono";

    @Readable public static final String MASTER_MOTION = "master_motion";
    ...
    @Readable public static final String MIN_REFRESH_RATE = "min_refresh_rate";

    @Readable public static final String MMS_USER_AGENT = "mms_user_agent";

    @Readable public static final String MMS_X_WAP_PROFILE_URL = "mms_x_wap_profile_url";

    @Deprecated public static final String MODE_RINGER = "mode_ringer";

    @Readable
    public static final String MODE_RINGER_STREAMS_AFFECTED = "mode_ringer_streams_affected";

    @Readable public static final String MONO_AUDIO_TYPE = "mono_audio_type";

    @Readable public static final String MOTION_ENGINE = "motion_engine";

    @Readable public static final String MOUSE_ADDITIONAL_1_OPTION = "mouse_additional_1_option";

    @Readable public static final String MOUSE_ADDITIONAL_2_OPTION = "mouse_additional_2_option";

    @Readable public static final String MOUSE_MIDDLE_OPTION = "mouse_middle_button_option";

    @Readable public static final String MOUSE_POINTER_COLOR = "mouse_pointer_color";

    @Readable public static final String MOUSE_POINTER_SIZE = "mouse_pointer_size_step";

    @Readable public static final String MOUSE_SCROLLING_SPEED = "mouse_scrolling_speed";

    @Readable public static final String MOUSE_SECONDARY_OPTION = "mouse_secondary_button_option";
    ...
    @Readable public static final String MULTICORE_PACKET_SCHEDULER = "multicore_packet_scheduler";

    @Readable public static final String MULTISOUND_APP = "multisound_app";

    @Readable public static final String MULTISOUND_DEVICE_TYPE = "multisound_devicetype";

    @Readable public static final String MULTISOUND_STATE = "multisound_state";

    @Readable public static final String MULTI_AUDIO_FOCUS_ENABLED = "multi_audio_focus_enabled";

    @Readable public static final String MUTE_STREAMS_AFFECTED = "mute_streams_affected";

    @Readable
    public static final String NAVIGATION_GESTURES_VIBRATE = "navigation_gestures_vibrate";

    @Readable public static final String NEARBY_SCANNING_ENABLED = "nearby_scanning_enabled";

    @Readable
    public static final String NEARBY_SCANNING_PERMISSION_ALLOWED =
        "nearby_scanning_permission_allowed";

    @Deprecated public static final String NETWORK_PREFERENCE = "network_preference";

    @Readable public static final String NETWORK_SPEED = "network_speed";

    @Readable public static final String NEW_DEX = "new_dex";

    @Readable @Deprecated public static final String NEXT_ALARM_FORMATTED = "next_alarm_formatted";

    @Readable public static final String NIGHT_MODE_ON = "night_mode";

    @Readable @Deprecated
    public static final String NOTIFICATIONS_USE_RING_VOLUME = "notifications_use_ring_volume";

    @Readable public static final String NOTIFICATION_LIGHT_PULSE = "notification_light_pulse";

    @Readable public static final String NOTIFICATION_SOUND = "notification_sound";

    @Readable public static final String NOTIFICATION_SOUND2_CACHE = "notification_sound2_cache";
    public static final Uri NOTIFICATION_SOUND2_CACHE_URI;

    @Readable public static final String NOTIFICATION_SOUND_2 = "notification_sound_2";

    @Readable public static final String NOTIFICATION_SOUND_CACHE = "notification_sound_cache";
    ...
    @Readable
    public static final String NOTIFICATION_VIBRATION_SEP_INDEX =
        "notification_vibration_sep_index";

    @Readable public static final String ONEHAND_ANY_SCREEN = "any_screen_enabled";

    @Readable public static final String PALM_TOUCH_TO_SLEEP = "palm_touch_to_sleep";

    @Deprecated public static final String PARENTAL_CONTROL_ENABLED = "parental_control_enabled";
    ...
    @Readable public static final String PEAK_REFRESH_RATE = "peak_refresh_rate";
    ...
    @Readable public static final String PEN_HOVERING_AIR_MENU = "pen_hovering_air_menu";

    @Readable public static final String PEN_HOVERING_LINK_PREVIEW = "pen_hovering_link_preview";

    @Readable public static final String PEN_WRITING_SOUND = "pen_writing_sound";

    @Readable public static final String PEOPLE_STRIPE = "people_stripe";

    @Readable public static final String POINTER_LOCATION = "pointer_location";

    @Readable public static final String POINTER_SPEED = "pointer_speed";

    @Deprecated public static final String POWER_SOUNDS_ENABLED = "power_sounds_enabled";

    @Readable
    public static final String PREMIUM_TAP_FOR_WATCH_FACE_SWITCH_ON_OFF =
        "premium_tap_for_watch_face_switch_on_off";

    @Readable public static final String PREMIUM_WATCH_SOUND = "premium_watch_sound";

    @Readable public static final String PREMIUM_WATCH_STYLE_OPTION = "premium_watch_style_option";

    @Readable public static final String PREMIUM_WATCH_SWITCH_ONOFF = "premium_watch_switch_onoff";
    ...
    @Deprecated public static final String RADIO_BLUETOOTH = "bluetooth";

    @Deprecated public static final String RADIO_CELL = "cell";

    @Deprecated public static final String RADIO_NFC = "nfc";

    @Deprecated public static final String RADIO_WIFI = "wifi";

    @Deprecated public static final String RADIO_WIMAX = "wimax";

    @Readable public static final String RAMPART_SUW_MAIN_ON = "rampart_suw_main_on";

    @Readable
    public static final String RCP_POLICY_MIGRATION_COMPLETED = "rcp_profile_migration_completed";

    @Readable public static final String REMOVE_ANIMATIONS = "remove_animations";

    @Readable public static final String RINGTONE = "ringtone";

    @Readable public static final String RINGTONE2_CACHE = "ringtone2_cache";
    public static final Uri RINGTONE2_CACHE_URI;

    @Readable public static final String RINGTONE_2 = "ringtone_2";

    @Readable public static final String RINGTONE_3 = "ringtone_3";

    @Readable public static final String RINGTONE_CACHE = "ringtone_cache";
    ...
    @Readable public static final String RING_VIBRATION_INTENSITY = "ring_vibration_intensity";
    ...
    @Readable public static final String SCREEN_AUTO_BRIGHTNESS_ADJ = "screen_auto_brightness_adj";

    @Readable public static final String SCREEN_BRIGHTNESS = "screen_brightness";

    @Readable public static final String SCREEN_BRIGHTNESS_FLOAT = "screen_brightness_float";

    @Readable public static final String SCREEN_BRIGHTNESS_MODE = "screen_brightness_mode";
    ...
    public static final String SCREEN_FLASH_NOTIFICATION_COLOR_MODE =
    ...
    @Readable public static final String SCREEN_MODE_SETTING = "screen_mode_setting";

    @Readable public static final String SCREEN_OFF_MEMO = "screen_off_memo";
    public static final String SCREEN_OFF_POCKET = "screen_off_pocket";

    @Readable public static final String SCREEN_OFF_TIMEOUT = "screen_off_timeout";

    @Readable
    public static final String SEM_ACCELEROMETER_ROTATION_SECOND = "accelerometer_rotation_second";

    @Readable
    public static final String SEM_ACCESSIBILITY_AM_MAGNIFICATION_MODE =
        "accessibility_am_magnification_mode";

    @Readable
    public static final String SEM_ACCESSIBILITY_EDIT_MAGNIFICATION_SIZE =
        "accessibility_edit_magnification_size";

    @Readable
    public static final String SEM_ACCESSIBILITY_LOCK_MAGNIFICATION_HORIZONTAL_FOCUS =
        "accessibility_lock_magnification_horizontal_focus";

    @Readable public static final String SEM_ACCESSIBILITY_MAGNIFIER = "accessibility_magnifier";

    @Readable
    public static final String SEM_ACCESSIBILITY_REDUCE_TRANSPARENCY =
        "accessibility_reduce_transparency";

    @Readable public static final String SEM_ACCESS_CONTROL_ENABLED = "access_control_enabled";

    @Readable public static final String SEM_ACCESS_CONTROL_USE = "access_control_use";

    @Readable public static final String SEM_AIR_BUTTON_ONOFF = "air_button_onoff";
    public static final String SEM_AIR_COMMAND_ENABLE_FLOATING_ICON = "air_cmd_use_minimized";

    @Readable public static final String SEM_AMBIENT_SOUND_AMPLIFICATION = "amplify_ambient_sound";

    @Readable
    public static final String SEM_AMBIENT_SOUND_AMPLIFICATION_RUNNING =
        "run_amplify_ambient_sound";

    @Readable
    public static final String SEM_AMBIENT_SOUND_AMPLIFICATION_WITH_MEDIA =
        "mix_amplify_ambient_sound";

    @Readable public static final String SEM_ASSISTANT_MENU = "assistant_menu";

    @Readable public static final String SEM_ASSISTANT_PLUS = "assistant_menu_eam_enable";

    @Readable
    public static final String SEM_CAMERA_FLASH_NOTIFICATION = "camera_flash_notification";

    @Readable public static final String SEM_COLOR_BLIND_SWITCH = "color_blind";

    @Readable public static final String SEM_COLOR_BLIND_TEST_CHECK = "color_blind_test";
    ...
    @Readable
    public static final String SEM_CURRENT_THEME_SUPPORT_NIGHT_MODE =
        "current_theme_support_night_mode";

    @Readable public static final String SEM_CURSOR_THICKNESS = "cursor_thickness";

    @Readable public static final String SEM_DB_MULTI_WINDOW_MODE = "multi_window_enabled";

    @Readable
    public static final String SEM_DEFAULT_NOTIFICATION_VIBRATION_PATTERN =
    ...
    @Readable public static final String SEM_EASY_INTERACTION = "easy_interaction";

    @Readable @Deprecated public static final String SEM_EASY_MODE_CAMERA = "easy_mode_camera";

    @Readable @Deprecated public static final String SEM_EASY_MODE_CONTACTS = "easy_mode_contacts";

    @Readable @Deprecated public static final String SEM_EASY_MODE_EMAIL = "easy_mode_email";

    @Readable @Deprecated public static final String SEM_EASY_MODE_GALLERY = "easy_mode_gallery";

    @Readable @Deprecated public static final String SEM_EASY_MODE_INTERNET = "easy_mode_internet";

    @Readable @Deprecated public static final String SEM_EASY_MODE_MESSAGES = "easy_mode_messages";

    @Readable @Deprecated public static final String SEM_EASY_MODE_MUSIC = "easy_mode_music";

    @Readable @Deprecated public static final String SEM_EASY_MODE_SPLANNER = "easy_mode_splanner";

    @Readable public static final String SEM_EASY_MODE_SWITCH = "easy_mode_switch";

    @Readable @Deprecated public static final String SEM_EASY_MODE_VIDEO = "easy_mode_video";

    @Readable public static final String SEM_EMERGENCY_MODE = "emergency_mode";

    @Readable
    public static final String SEM_EMERGENCY_MODE_USER_AGREEMENT = "safety_care_user_agree";

    @Readable public static final String SEM_FINGER_MAGNIFIER = "finger_magnifier";
    ...
    @Readable public static final String SEM_GREYSCALE_MODE = "greyscale_mode";
    public static final String SEM_HEARING_AID = "hearing_aid";

    @Readable public static final String SEM_HIGH_CONTRAST = "high_contrast";
    public static final String SEM_MASTER_BALANCE = "master_balance";
    public static final String SEM_MASTER_MONO = "master_mono";

    @Readable public static final String SEM_MINIMAL_BATTERY_USE = "minimal_battery_use";

    @Readable public static final String SEM_MOTION_MUTE = "motion_merged_mute_pause";

    @Readable public static final String SEM_MOTION_OVERTURN = "motion_overturn";

    @Readable public static final String SEM_MOTION_PICK_UP = "motion_pick_up";

    @Readable
    public static final String SEM_MOTION_PICK_UP_TO_CALL_OUT = "motion_pick_up_to_call_out";

    @Readable public static final String SEM_NOTIFICATION_REMINDER = "notification_reminder";

    @Readable
    public static final String SEM_NOTIFICATION_REMINDER_APP_LIST =
        "notification_reminder_app_list";

    @Readable
    public static final String SEM_NOTIFICATION_REMINDER_LED_INDICATOR =
        "notification_reminder_led_indicator";

    @Readable
    public static final String SEM_NOTIFICATION_REMINDER_SELECTABLE =
    ...
    @Readable public static final String SEM_ONE_HAND_ANY_SCREEN_RUNNING = "any_screen_running";

    @Readable
    public static final String SEM_OPEN_AIR_CMD_USING_SPEN_BTN = "open_air_cmd_using_spen_btn";

    @Readable public static final String SEM_PALM_SWIPE = "surface_palm_swipe";

    @Readable public static final String SEM_PALM_TOUCH = "surface_palm_touch";

    @Readable public static final String SEM_PEN_DETACHMENT_ALERT = "pen_detachment_alert";

    @Readable public static final String SEM_PEN_DETACHMENT_OPTION = "pen_detachment_option";

    @Readable public static final String SEM_PEN_HOVERING = "pen_hovering";

    @Readable public static final String SEM_PEN_HOVERING_ICON_LABEL = "pen_hovering_icon_label";

    @Readable
    public static final String SEM_PEN_HOVERING_INFORMATION_PREVIEW =
        "pen_hovering_information_preview";

    @Readable public static final String SEM_PEN_HOVERING_LIST_SCROLL = "pen_hovering_list_scroll";

    @Readable public static final String SEM_PEN_HOVERING_POINTER = "pen_hovering_pointer";

    @Readable public static final String SEM_PEN_WRITING_BUDDY = "pen_writing_buddy";

    @Readable public static final String SEM_PHONE1_ON = "phone1_on";

    @Readable public static final String SEM_PHONE2_ON = "phone2_on";

    @Readable @Deprecated
    public static final String SEM_PREFERED_VOICE_CALL = "prefered_voice_call";

    @Readable public static final String SEM_PREV_SYSTEM_SOUND = "prev_system_sound";

    @Readable public static final String SEM_RAPID_KEY_INPUT = "rapid_key_input";

    @Readable
    public static final String SEM_RAPID_KEY_INPUT_MENU_CHECKED = "rapid_key_input_menu_checked";

    @Readable public static final String SEM_SCREEN_CURTAIN = "lcd_curtain";
    ...
    @Readable public static final String SEM_SELECT_ICON_1 = "select_icon_1";

    @Readable public static final String SEM_SELECT_ICON_2 = "select_icon_2";

    @Readable public static final String SEM_SELECT_NAME_1 = "select_name_1";

    @Readable public static final String SEM_SELECT_NAME_2 = "select_name_2";

    @Readable public static final String SEM_SIP_KEY_FEEDBACK_SOUND = "sip_key_feedback_sound";

    @Readable
    public static final String SEM_SIP_KEY_FEEDBACK_VIBRATION = "sip_key_feedback_vibration";

    @Readable public static final String SEM_SOUND_BALANCE = "sound_balance";

    @Readable
    @Deprecated(forRemoval = true, since = "15.0")
    public static final String SEM_SOUND_DETECTOR_SWITCH = "sound_detector";

    @Readable public static final String SEM_SPEN_AIR_ACTION = "spen_air_action";

    @Readable public static final String SEM_SPEN_FEEDBACK_HAPTIC = "spen_feedback_haptic";

    @Readable public static final String SEM_SPEN_UNLOCK = "spen_unlock";

    @Readable public static final String SEM_SPEN_WRITING_COLOR = "spen_writing_color";

    @Readable
    public static final String SEM_SPEN_WRITING_COLOR_SWITCH = "spen_writing_color_switch";

    @Readable public static final String SEM_STATUS_BAR_SHOW_DATE = "status_bar_show_date";

    @Readable
    public static final String SEM_STATUS_BAR_SHOW_NETWORK_INFORMATION =
        "status_bar_show_network_information";

    @Readable public static final String SEM_SUB_SCREEN_AUTO_LOCK = "sub_lcd_auto_lock";

    @Readable public static final String SEM_SYSTEM_SOUND = "system_sound";

    @Readable public static final String SEM_TIME_KEY = "time_key";

    @Readable public static final String SEM_TIME_KEY_SELECTABLE = "time_key_selectable";

    @Readable public static final String SEM_TORCH_LIGHT = "torch_light";

    @Readable public static final String SEM_ULTRA_POWERSAVING_MODE = "ultra_powersaving_mode";
    public static final String SEM_VIBRATION_FORCE_TOUCH_INTENSITY =
        "SEM_VIBRATION_FORCE_TOUCH_INTENSITY";

    @Readable
    public static final String SEM_VIBRATION_NOTIFICATION_INTENSITY =
        "SEM_VIBRATION_NOTIFICATION_INTENSITY";

    @Readable public static final String SEM_WALLPAPERTHEME_STATE = "wallpapertheme_state";

    @Deprecated public static final String SETTINGS_CLASSNAME = "settings_classname";

    @Readable public static final String SETUP_WIZARD_HAS_RUN = "setup_wizard_has_run";
    public static final String SET_SHORTCUTS_MODE = "set_shortcuts_mode";

    @Readable public static final String SHOW_BATTERY_PERCENT = "status_bar_show_battery_percent";

    @Readable public static final String SHOW_BUTTON_BACKGROUND = "show_button_background";

    @Readable public static final String SHOW_GTALK_SERVICE_STATUS = "SHOW_GTALK_SERVICE_STATUS";
    ...
    @Readable public static final String SHOW_TOUCHES = "show_touches";

    @Readable @Deprecated public static final String SHOW_WEB_SUGGESTIONS = "show_web_suggestions";

    @Readable public static final String SIP_ADDRESS_ONLY = "SIP_ADDRESS_ONLY";

    @Readable public static final String SIP_ALWAYS = "SIP_ALWAYS";

    @Readable @Deprecated public static final String SIP_ASK_ME_EACH_TIME = "SIP_ASK_ME_EACH_TIME";

    @Readable public static final String SIP_CALL_OPTIONS = "sip_call_options";

    @Readable public static final String SIP_RECEIVE_CALLS = "sip_receive_calls";

    @Readable public static final String SOUND_EFFECTS_ENABLED = "sound_effects_enabled";

    @Readable public static final String SPEAKER_BALANCE = "speaker_balance";

    @Readable
    public static final String SPEN_FEEDBACK_HAPTIC_AIR_COMMAND =
        "spen_feedback_haptic_air_command";

    @Readable
    public static final String SPEN_FEEDBACK_HAPTIC_AIR_VIEW = "spen_feedback_haptic_air_view";

    @Readable
    public static final String SPEN_FEEDBACK_HAPTIC_PEN_GESTURE =
        "spen_feedback_haptic_pen_gesture";

    @Readable public static final String SPEN_FEEDBACK_SOUND = "spen_feedback_sound";
    ...
    @Readable public static final String SPEN_SCREEN_ON = "spen_screen_on";
    ...
    @Readable public static final String SUB_SCREEN_BRIGHTNESS_MODE = "sub_screen_brightness_mode";

    @Readable public static final String SUPER_FAST_CHARGING = "super_fast_charging";

    @Readable public static final String SURFACE_MOTION_ENGINE = "surface_motion_engine";

    @Readable
    public static final String SYNC_VIBRATION_WITH_NOTIFICATION =
    ...
    @Readable public static final String SYSTEM_LOCALES = "system_locales";

    @Readable public static final String TASK_EDGE = "task_edge";

    @Readable public static final String TEMPERATURE_UNIT = "temperature_unit";

    @Readable public static final String TEXT_AUTO_CAPS = "auto_caps";

    @Readable public static final String TEXT_AUTO_PUNCTUATE = "auto_punctuate";

    @Readable public static final String TEXT_AUTO_REPLACE = "auto_replace";

    @Readable public static final String TEXT_SHOW_PASSWORD = "show_password";

    @Readable public static final String TIME_12_24 = "time_12_24";

    @Readable public static final String TOOLBOX_ONOFF = "toolbox_onoff";
    ...
    @Readable public static final String TTY_MODE = "tty_mode";

    @Readable public static final String TURN_OVER_LIGHTING = "turn_over_lighting";

    @Deprecated public static final String UNLOCK_SOUND = "unlock_sound";
    public static final String UNREAD_NOTIFICATION_DOT_INDICATOR =
        "unread_notification_dot_indicator";

    @Deprecated public static final String USB_MASS_STORAGE_ENABLED = "usb_mass_storage_enabled";

    @Readable public static final String USER_ACTIVITY_TIMEOUT = "user_activity_timeout";

    @Readable public static final String USER_ROTATION = "user_rotation";

    @Deprecated public static final String USE_GOOGLE_MAIL = "use_google_mail";

    @Readable public static final String VIBRATE_INPUT_DEVICES = "vibrate_input_devices";

    @Readable public static final String VIBRATE_IN_SILENT = "vibrate_in_silent";

    @Readable public static final String VIBRATE_ON = "vibrate_on";

    @Readable @Deprecated public static final String VIBRATE_WHEN_RINGING = "vibrate_when_ringing";

    @Readable public static final String VIBRATION_SOUND_ENABLED = "vibration_sound_enabled";

    @Readable public static final String VIB_FEEDBACK_MAGNITUDE = "VIB_FEEDBACK_MAGNITUDE";

    @Readable public static final String VIB_RECVCALL_MAGNITUDE = "VIB_RECVCALL_MAGNITUDE";

    @Readable public static final String VIVIDNESS_INTENSITY = "vividness_intensity";

    @Readable public static final String VOIP_ANTI_HOWLING = "voip_anti_howling";

    @Readable public static final String VOIP_EXTRA_VOLUME = "voip_extra_volume";

    @Readable public static final String VOLUME_ACCESSIBILITY = "volume_a11y";

    @Readable public static final String VOLUME_ALARM = "volume_alarm";

    @Readable public static final String VOLUME_ASSISTANT = "volume_assistant";

    @Readable public static final String VOLUME_BLUETOOTH_SCO = "volume_bluetooth_sco";

    @Readable public static final String VOLUME_LIMITER_ON = "volumelimit_on";

    @Readable public static final String VOLUME_LIMITER_PASSWORD = "volumelimit_set_password";

    @Readable public static final String VOLUME_LIMITER_VALUE = "volume_limiter_value";

    @Readable public static final String VOLUME_MASTER = "volume_master";

    @Readable public static final String VOLUME_MUSIC = "volume_music";

    @Readable public static final String VOLUME_NOTIFICATION = "volume_notification";

    @Readable public static final String VOLUME_RING = "volume_ring";
    public static final String[] VOLUME_SETTINGS;
    public static final String[] VOLUME_SETTINGS_INT;

    @Readable public static final String VOLUME_SYSTEM = "volume_system";

    @Readable public static final String VOLUME_VOICE = "volume_voice";

    @Readable public static final String VOLUME_WAITING_TONE = "volume_waiting_tone";

    @Deprecated public static final String WAIT_FOR_DEBUGGER = "wait_for_debugger";

    @Readable @Deprecated public static final String WALLPAPER_ACTIVITY = "wallpaper_activity";
    public static final String WEAR_ACCESSIBILITY_GESTURE_ENABLED =
        "wear_accessibility_gesture_enabled";

    @Readable public static final String WHEN_TO_MAKE_WIFI_CALLS = "when_to_make_wifi_calls";

    @Readable
    public static final String WIFISPEAKER_CHROMECAST_MODE_ENABLED =
        "wifispeaker_chromecast_mode_enabled";

    @Deprecated public static final String WIFI_MAX_DHCP_RETRY_COUNT = "wifi_max_dhcp_retry_count";
    ...
    @Deprecated public static final String WIFI_ON = "wifi_on";

    @Deprecated public static final String WIFI_SLEEP_POLICY = "wifi_sleep_policy";

    @Deprecated public static final int WIFI_SLEEP_POLICY_DEFAULT = 0;

    @Deprecated public static final int WIFI_SLEEP_POLICY_NEVER = 2;

    @Deprecated public static final int WIFI_SLEEP_POLICY_NEVER_WHILE_PLUGGED = 1;

    @Readable @Deprecated public static final String WIFI_STATIC_DNS1 = "wifi_static_dns1";

    @Readable @Deprecated public static final String WIFI_STATIC_DNS2 = "wifi_static_dns2";

    @Readable @Deprecated public static final String WIFI_STATIC_GATEWAY = "wifi_static_gateway";

    @Readable @Deprecated public static final String WIFI_STATIC_IP = "wifi_static_ip";

    @Readable @Deprecated public static final String WIFI_STATIC_NETMASK = "wifi_static_netmask";

    @Readable @Deprecated public static final String WIFI_USE_STATIC_IP = "wifi_use_static_ip";
    ...
    @Deprecated public static final String WIFI_WATCHDOG_ON = "wifi_watchdog_on";
    ...
    @Deprecated public static final String WINDOW_ANIMATION_SCALE = "window_animation_scale";
    ...
    static {
      ...
      sNameValueCache =
          new NameValueCache(
              ...
              Settings.CALL_METHOD_DELETE_SYSTEM,
    ...
    public static void getPublicSettings(
        Set<String> allKeys,
        Set<String> readableKeys,
        ArrayMap<String, Integer> readableKeysWithMaxTargetSdk) {
      Settings.getPublicSettingsForClass(
          System.class, allKeys, readableKeys, readableKeysWithMaxTargetSdk);
    }
    ...
    public static String getStringForUser(ContentResolver resolver, String name, int userHandle) {
      if (MOVED_TO_SECURE.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.System to"
    ...
    public static boolean putString(
        ContentResolver resolver, String name, String value, boolean overrideableByRestore) {
      ...
      if (MOVED_TO_SECURE.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.System to"
                + " android.provider.Settings.Secure, value is unchanged.");
        return false;
      }
      if (MOVED_TO_GLOBAL.contains(name) || MOVED_TO_SECURE_THEN_GLOBAL.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.System to"
                + " android.provider.Settings.Global, value is unchanged.");
        return false;
      }
      ...
      if (MOVED_TO_SECURE.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.System to"
                + " android.provider.Settings.Secure, returning Secure URI.");
      ...
      if (MOVED_TO_GLOBAL.contains(name) || MOVED_TO_SECURE_THEN_GLOBAL.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.System to"
                + " android.provider.Settings.Global, returning read-only global URI.");
    ...
    public static boolean semPutIntForUser(
        ContentResolver cr, String name, int value, int userHandle) {
    ...
    public static boolean putLongForUser(
        ContentResolver cr, String name, long value, int userHandle) {
    ...
    public static float semGetFloatForUser(
        ContentResolver cr, String name, float def, int userHandle) {
    ...
    public static boolean semPutFloatForUser(
        ContentResolver cr, String name, float value, int userHandle) {
    ...
    public static void adjustConfigurationForUser(
        ContentResolver cr,
        Configuration outConfig,
        int userHandle,
        boolean updateSettingsIfEmpty) {
      outConfig.fontScale = getFloatForUser(cr, FONT_SCALE, 1.0f, userHandle);
      if (outConfig.fontScale < 0.0f) {
        outConfig.fontScale = 1.0f;
      }
      outConfig.fontWeightAdjustment =
          Secure.getIntForUser(cr, Secure.FONT_WEIGHT_ADJUSTMENT, 0, userHandle);
      ...
      if (localeValue != null) {
        ...
        putStringForUser(
            cr, SYSTEM_LOCALES, outConfig.getLocales().toLanguageTags(), userHandle, false);
      }
    }
    ...
    public static void clearConfiguration(Configuration inoutConfig) {
      ...
      if (!inoutConfig.userSetLocale
          && !inoutConfig.getLocales().isEmpty()
          && !inoutConfig.rilSetLocale) {
    ...
    public static boolean putConfigurationForUser(
        ContentResolver cr, Configuration config, int userHandle) {
      return putFloatForUser(cr, FONT_SCALE, config.fontScale, userHandle)
          && putStringForUser(
              cr, SYSTEM_LOCALES, config.getLocales().toLanguageTags(), userHandle, false);
    }
    ...
    public static void setShowGTalkServiceStatusForUser(
        ContentResolver contentResolver, boolean z, int i) {
    ...
    public static boolean canWrite(Context context) {
      return Settings.isCallingPackageAllowedToWriteSettings(
          context, Process.myUid(), context.getOpPackageName(), false);
    }
  }
  ...
  public static final class Secure extends NameValueTable {
    ...
    @Readable
    public static final String ACCESSIBILITY_CHANGE_MAGNIFICATION_SIZE =
        "accessibility_change_magnification_size";

    @Readable public static final String ACCESSIBILITY_CURSOR_COLOR = "accessibility_cursor_color";

    @Readable
    public static final String ACCESSIBILITY_DIRECT_ACCESS_TARGET_SERVICE =
    ...
    @Readable
    public static final String ACCESSIBILITY_EDIT_MAGNIFICATION_SIZE =
        "accessibility_edit_magnification_size";

    @Readable public static final String ACCESSIBILITY_ENABLED = "accessibility_enabled";
    ...
    @Readable
    public static final String ACCESSIBILITY_HWKEY_DOUBLETAP_ENABLED =
        "accessibility_hwkey_doubletap_enabled";

    @Readable
    public static final String ACCESSIBILITY_INTERACTIVE_UI_TIMEOUT_MS =
        "accessibility_interactive_ui_timeout_ms";

    @Readable public static final String ACCESSIBILITY_LARGE_CURSOR = "accessibility_large_cursor";
    ...
    @Readable public static final String ADAPTIVE_SLEEP = "adaptive_sleep";
    ...
    @Readable @Deprecated public static final String ALLOW_MOCK_LOCATION = "mock_location";
    public static final String ALWAYS_ON_VPN_APP = "always_on_vpn_app";

    @Readable public static final String ALWAYS_ON_VPN_LOCKDOWN = "always_on_vpn_lockdown";
    ...
    @Readable public static final String ANDROID_ID = "android_id";

    @Readable public static final String ANR_SHOW_BACKGROUND = "anr_show_background";

    @Readable
    public static final String APP_CONTINUITY_GUIDE_CONFIRMATION_UNFOLDED =
        "app_continuity_guide_confirmation_unfolded";

    @Readable public static final String ASSISTANT = "assistant";

    @Readable public static final String ASSIST_DISCLOSURE_ENABLED = "assist_disclosure_enabled";

    @Readable public static final String ASSIST_GESTURE_ENABLED = "assist_gesture_enabled";

    @Readable public static final String ASSIST_GESTURE_SENSITIVITY = "assist_gesture_sensitivity";
    ...
    @Readable public static final String ASSIST_SCREENSHOT_ENABLED = "assist_screenshot_enabled";

    @Readable public static final String ASSIST_STRUCTURE_ENABLED = "assist_structure_enabled";
    public static final String ASSIST_TOUCH_GESTURE_ENABLED = "assist_touch_gesture_enabled";

    @Readable public static final String ATTENTIVE_TIMEOUT = "attentive_timeout";
    ...
    @Readable public static final String AUTOFILL_SERVICE = "autofill_service";
    ...
    @SystemApi @Readable public static final String AUTO_REVOKE_DISABLED = "auto_revoke_disabled";

    @Readable public static final String AUTO_SWIPE_MAIN_USER = "auto_swipe_main_user";

    @Readable public static final String AWARE_ENABLED = "aware_enabled";

    @Readable public static final String AWARE_LOCK_ENABLED = "aware_lock_enabled";
    ...
    @Readable @Deprecated public static final String BACKGROUND_DATA = "background_data";

    @Readable public static final String BACKUP_AUTO_RESTORE = "backup_auto_restore";

    @Readable public static final String BACKUP_ENABLED = "backup_enabled";

    @Readable
    public static final String BACKUP_LOCAL_TRANSPORT_PARAMETERS =
        "backup_local_transport_parameters";

    @Readable public static final String BACKUP_MANAGER_CONSTANTS = "backup_manager_constants";

    @Readable public static final String BACKUP_PROVISIONED = "backup_provisioned";
    public static final String BACKUP_SCHEDULING_ENABLED = "backup_scheduling_enabled";

    @Readable public static final String BACKUP_TRANSPORT = "backup_transport";
    ...
    @Readable public static final String BADGE_APP_ICON_TYPE = "badge_app_icon_type";

    @Readable public static final String BIOMETRIC_APP_ENABLED = "biometric_app_enabled";

    @Readable public static final String BIOMETRIC_DEBUG_ENABLED = "biometric_debug_enabled";

    @Readable public static final String BIOMETRIC_KEYGUARD_ENABLED = "biometric_keyguard_enabled";

    @Readable public static final String BIOMETRIC_VIRTUAL_ENABLED = "biometric_virtual_enabled";

    @Readable public static final String BLOCK_USB_LOCK = "block_usb_lock";
    ...
    public static final String BLUETOOTH_LE_BROADCAST_NAME_INFO =
        "bluetooth_le_broadcast_name_info";
    public static final String BLUETOOTH_LE_BROADCAST_NEED_START_POPUP =
    ...
    public static final String BLUETOOTH_LE_ENCRYPTED_BROADCAST =
    ...
    @Readable public static final String BLUETOOTH_ON_WHILE_DRIVING = "bluetooth_on_while_driving";

    @Readable
    public static final String BUBBLE_IMPORTANT_CONVERSATIONS = "bubble_important_conversations";

    @Readable public static final String BUGREPORT_IN_POWER_MENU = "bugreport_in_power_menu";

    @Readable public static final String CALL_RTT_AUTOMATIC_MODE = "preferred_rtt_automatic_mode";

    @Readable
    public static final String CALL_SCREENING_DEFAULT_COMPONENT =
        "call_screening_default_component";

    @Readable public static final String CALL_SUPPORT_RTT = "preferred_rtt_mode";
    ...
    public static final String CAMERA_FLASH_NOTIFICATION_APP_LIST =
        "camera_flash_notification_app_list";

    @Readable public static final String CAMERA_GESTURE_DISABLED = "camera_gesture_disabled";
    ...
    @Readable public static final String CARRIER_APPS_HANDLED = "carrier_apps_handled";
    ...
    public static final String CDMA_ROAM_GUARD_DATA_LTE_INTERNATIONAL =
        "roam_guard_data_lte_international";
    ...
    public static final String CDMA_ROAM_SETTING_CALL_INTERNATIONAL =
        "roam_setting_call_international";
    public static final String CDMA_ROAM_SETTING_DATA_DOMESTIC = "roam_setting_data_domestic";
    public static final String CDMA_ROAM_SETTING_DATA_INTERNATIONAL =
        "roam_setting_data_international";
    public static final String CDMA_ROAM_SETTING_DATA_LTE = "roam_setting_data_lte";
    public static final String CDMA_ROAM_SETTING_DATA_LTE_INTERNATIONAL =
        "roam_setting_data_lte_international";

    @Readable public static final String CHAMELEON_TETHEREDDATA = "chameleon_tethereddata";

    @Readable public static final String CHARGING_SOUNDS_ENABLED = "charging_sounds_enabled";

    @Readable public static final String CHARGING_VIBRATION_ENABLED = "charging_vibration_enabled";
    ...
    @Readable public static final String COLOR_ENHANCEMENT_MODE = "color_enhancement_mode";
    ...
    @Readable public static final String CONTENT_CAPTURE_ENABLED = "content_capture_enabled";
    public static final Uri CONTENT_URI;
    public static final String CONTRAST_LEVEL = "contrast_level";

    @Readable @Deprecated public static final String CONTROLS_ENABLED = "controls_enabled";
    ...
    @Readable public static final String DARK_MODE_DIALOG_SEEN = "dark_mode_dialog_seen";

    @Readable public static final String DARK_THEME_CUSTOM_END_TIME = "dark_theme_custom_end_time";
    ...
    @Readable public static final String DEFAULT_INPUT_METHOD = "default_input_method";
    public static final String DEFAULT_VOICE_INPUT_METHOD = "default_voice_input_method";
    public static final String DEVELOPMENT_CUSTOM_BUGREPORT_WRITER =
    ...
    @Readable public static final String DEVICE_PAIRED = "device_paired";
    ...
    @Readable public static final String DIALER_DEFAULT_APPLICATION = "dialer_default_application";

    @Readable public static final String DISABLED_PRINT_SERVICES = "disabled_print_services";

    @Readable(maxTargetSdk = 33)
    public static final String DISABLED_SYSTEM_INPUT_METHODS = "disabled_system_input_methods";

    @Readable public static final String DISPLAY_DENSITY_FORCED = "display_density_forced";

    @Readable
    public static final String DISPLAY_WHITE_BALANCE_ENABLED = "display_white_balance_enabled";

    @Readable public static final String DOCKED_CLOCK_FACE = "docked_clock_face";
    ...
    @Readable public static final String DOUBLE_TAP_TO_WAKE = "double_tap_to_wake";

    @SystemApi @Readable public static final String DOZE_ALWAYS_ON = "doze_always_on";

    @Readable public static final String DOZE_DOUBLE_TAP_GESTURE = "doze_pulse_on_double_tap";

    @Readable public static final String DOZE_ENABLED = "doze_enabled";

    @Readable public static final String DOZE_PICK_UP_GESTURE = "doze_pulse_on_pick_up";

    @Readable public static final String DOZE_PULSE_ON_LONG_PRESS = "doze_pulse_on_long_press";
    public static final String DOZE_QUICK_PICKUP_GESTURE = "doze_quick_pickup_gesture";

    @Readable public static final String DOZE_TAP_SCREEN_GESTURE = "doze_tap_gesture";

    @Readable public static final String DOZE_WAKE_DISPLAY_GESTURE = "doze_wake_display_gesture";

    @Readable public static final String DOZE_WAKE_LOCK_SCREEN_GESTURE = "doze_wake_screen_gesture";

    @Readable
    public static final String DSG_USER_DATA_ROAM_SETTING_INTERNATIONAL =
        "dsg_user_data_roam_setting_international";

    @Readable public static final String EDGE_ENABLE = "edge_enable";

    @Readable public static final String EDGE_HANDLE_SIZE_PERCENT = "edge_handle_size_percent";

    @Readable public static final String EDGE_HANDLE_TRANSPARENCY = "edge_handle_transparency";

    @Readable
    public static final String EDGE_SHARPNESS_DISPLAY_ACTIVATED =
        "edge_sharpness_display_activated";

    @Readable
    public static final String EDGE_SHARPNESS_DISPLAY_INTENSITY_LEVEL =
        "edge_sharpness_display_intensity_level";

    @Readable
    public static final String EMERGENCY_ASSISTANCE_APPLICATION =
    ...
    @Readable public static final String ENABLED_PRINT_SERVICES = "enabled_print_services";

    @Readable public static final String ENABLED_VR_LISTENERS = "enabled_vr_listeners";
    ...
    @Readable public static final String EYE_TEMP_DISPLAY_ACTIVATED = "eye_temp_display_activated";
    ...
    @Readable public static final String FACE_UNLOCK_APP_ENABLED = "face_unlock_app_enabled";
    ...
    @Readable public static final String FACE_UNLOCK_RE_ENROLL = "face_unlock_re_enroll";
    ...
    @Readable public static final String FLASHLIGHT_AVAILABLE = "flashlight_available";

    @Readable public static final String FLASHLIGHT_ENABLED = "flashlight_enabled";

    @Readable public static final String FONT_WEIGHT_ADJUSTMENT = "font_weight_adjustment";

    @Readable public static final String GAME_DASHBOARD_ALWAYS_ON = "game_dashboard_always_on";

    @Readable
    public static final String GESTURE_IMMERSIVE_MODE_CONFIRMATIONS =
        "gesture_immersive_mode_confirmations";
    ...
    @Readable public static final String HIDE_SECURE_FOLDER_FLAG = "hide_secure_folder_flag";

    @Readable public static final String HSV_GAIN_DISPLAY_ACTIVATED = "hsv_gain_display_activated";

    @Readable public static final String HSV_GAIN_DISPLAY_HUE_LEVEL = "hsv_gain_display_hue_level";

    @Readable public static final String HSV_GAIN_DISPLAY_SAT_LEVEL = "hsv_gain_display_sat_level";

    @Readable public static final String HSV_GAIN_DISPLAY_VAL_LEVEL = "hsv_gain_display_val_level";

    @Deprecated public static final String HTTP_PROXY = "http_proxy";

    @SystemApi @Readable public static final String HUSH_GESTURE_USED = "hush_gesture_used";
    ...
    @SystemApi @Readable public static final String INSTANT_APPS_ENABLED = "instant_apps_enabled";
    public static final Set<String> INSTANT_APP_SETTINGS;

    @Readable
    public static final String IN_CALL_NOTIFICATION_ENABLED = "in_call_notification_enabled";

    @Readable public static final String KDDI_CPA_APN = "kddi_cpa_apn";

    @Readable
    public static final String KDDI_CPA_AUTHENTICATION_TYPE = "kddi_cpa_authentication_type";

    @Readable public static final String KDDI_CPA_ID = "kddi_cpa_id";

    @Readable public static final String KDDI_CPA_ON = "kddi_cpa_on";

    @Readable public static final String KDDI_CPA_PASSWORD = "kddi_cpa_password";

    @Readable public static final String KDDI_CPA_PORT = "kddi_cpa_port";

    @Readable public static final String KDDI_CPA_PROXY = "kddi_cpa_proxy";

    @Readable public static final String KDDI_CPA_STATE = "kddi_cpa_state";

    @Readable public static final String KDDI_CPA_STATIC_DNS1 = "kddi_cpa_static_dns1";

    @Readable public static final String KDDI_CPA_STATIC_DNS2 = "kddi_cpa_static_dns2";

    @Readable public static final String KDDI_CPA_VJ_COMPRESS = "kddi_cpa_vj_compress";

    @Readable public static final String KEYGUARD_SLICE_URI = "keyguard_slice_uri";
    public static final String KNOWN_TRUST_AGENTS_INITIALIZED = "known_trust_agents_initialized";

    @Readable public static final String KNOX_FINGER_PRINT_PLUS = "knox_finger_print_plus";

    @Readable public static final String KNOX_SCREEN_OFF_TIMEOUT = "knox_screen_off_timeout";

    @SystemApi @Readable public static final String LAST_SETUP_SHOWN = "last_setup_shown";
    public static final String LAUNCHER_TASKBAR_EDUCATION_SHOWING =
        "launcher_taskbar_education_showing";

    @Readable public static final String[] LEGACY_RESTORE_SETTINGS;

    @SystemApi @Readable @Deprecated
    ...
    @Readable public static final String LOCATION_CHANGER = "location_changer";
    ...
    @Readable public static final String LOCATION_COARSE_ACCURACY_M = "locationCoarseAccuracy";

    @Readable @Deprecated public static final String LOCATION_MODE = "location_mode";

    @Deprecated public static final int LOCATION_MODE_BATTERY_SAVING = 2;

    @Deprecated public static final int LOCATION_MODE_HIGH_ACCURACY = 3;
    public static final int LOCATION_MODE_OFF = 0;

    @SystemApi public static final int LOCATION_MODE_ON = 3;

    @Deprecated public static final int LOCATION_MODE_SENSORS_ONLY = 1;
    ...
    @Readable public static final String LOCKDOWN_IN_POWER_MENU = "lockdown_in_power_menu";
    ...
    @Readable public static final String LOCK_NOTI_AND_SECURITY = "lock_function_val";

    @Readable @Deprecated public static final String LOCK_PATTERN_ENABLED = "lock_pattern_autolock";
    ...
    @Readable
    public static final String LOCK_SCREEN_ALLOW_PRIVATE_NOTIFICATIONS_WHEN_UNSECURE =
    ...
    @Readable
    public static final String LOCK_SCREEN_SHOW_NOTIFICATIONS_ON_KEYGUARD =
        "lock_screen_show_notifications_on_keyguard";
    ...
    @Readable public static final String LOCK_TO_APP_EXIT_LOCKED = "lock_to_app_exit_locked";

    @Readable @Deprecated public static final String LOGGING_ID = "logging_id";

    @Readable public static final String LONG_PRESS_TIMEOUT = "long_press_timeout";
    ...
    @Readable public static final String LTW_CLIPBOARD_SYNC_STATE = "ltw_clipboard_sync_state";
    ...
    @Readable public static final String MANUAL_RINGER_TOGGLE_COUNT = "manual_ringer_toggle_count";
    ...
    @Readable
    public static final String MCF_CONTINUITY_NEARBY_DEVICE_STATE =
        "mcf_continuity_nearby_device_state";
    ...
    @Readable public static final String MEDIA_CONTROLS_RESUME = "qs_media_resumption";

    @Readable
    public static final String MINIMAL_POST_PROCESSING_ALLOWED = "minimal_post_processing_allowed";

    @Readable public static final String MOUNT_PLAY_NOTIFICATION_SND = "mount_play_not_snd";

    @Readable public static final String MOUNT_UMS_AUTOSTART = "mount_ums_autostart";

    @Readable public static final String MOUNT_UMS_NOTIFY_ENABLED = "mount_ums_notify_enabled";

    @Readable public static final String MOUNT_UMS_PROMPT = "mount_ums_prompt";
    private static final HashSet<String> MOVED_TO_GLOBAL;
    ...
    @Readable public static final String MULTI_PRESS_TIMEOUT = "multi_press_timeout";

    @Readable public static final String NAS_SETTINGS_UPDATED = "nas_settings_updated";

    @Readable public static final String NAVIGATION_MODE = "navigation_mode";
    ...
    @Readable public static final String NEARBY_SHARING_COMPONENT = "nearby_sharing_component";
    public static final String NEARBY_SHARING_SLICE_URI = "nearby_sharing_slice_uri";

    @Deprecated public static final String NETWORK_PREFERENCE = "network_preference";
    public static final String NFC_PAYMENT_DEFAULT_COMPONENT = "nfc_payment_default_component";

    @Readable public static final String NFC_PAYMENT_FOREGROUND = "nfc_payment_foreground";

    @Readable public static final String NIGHT_DISPLAY_ACTIVATED = "night_display_activated";

    @Readable public static final String NIGHT_DISPLAY_AUTO_MODE = "night_display_auto_mode";
    ...
    @Readable public static final String NOTIFICATION_BADGING = "notification_badging";

    @Readable public static final String NOTIFICATION_BUBBLES = "notification_bubbles";

    @Readable public static final String NOTIFICATION_DISMISS_RTL = "notification_dismiss_rtl";
    ...
    @SystemApi @Readable public static final String ODI_CAPTIONS_ENABLED = "odi_captions_enabled";
    ...
    @Readable public static final String PARENTAL_CONTROL_ENABLED = "parental_control_enabled";
    ...
    @Readable public static final String PAYMENT_SERVICE_SEARCH_URI = "payment_service_search_uri";

    @Readable public static final String PEOPLE_STRIP = "people_strip";

    @Readable
    public static final String POWER_MENU_LOCKED_SHOW_CONTENT = "power_menu_locked_show_content";

    @Readable public static final String PPPD_EXIT_CODE = "pppd_exit_port";

    @Readable
    public static final String PREFERRED_TIME_ZONE_DETECTION_METHOD =
        "preferred_time_zone_detection_method";

    @Readable public static final String PREFERRED_TTY_MODE = "preferred_tty_mode";

    @Readable public static final String PRINT_SERVICE_SEARCH_URI = "print_service_search_uri";

    @Readable public static final String QS_AUTO_ADDED_TILES = "qs_auto_tiles";

    @Readable(maxTargetSdk = 33)
    public static final String QS_TILES = "sysui_qs_tiles";

    @Readable public static final String RAKUTEN_DENWA_PREFIX = "rakuten_denwa_prefix";
    ...
    public static final String RAMPART_BLOCKED_AUTO_DOWNLOAD_MESSAGES =
        "rampart_blocked_auto_download_messages";
    public static final String RAMPART_BLOCKED_COMMANDS = "rampart_blocked_commands";
    public static final String RAMPART_BLOCKED_DEVICE_ADMIN_APPS =
        "rampart_blocked_device_admin_apps";
    public static final String RAMPART_BLOCKED_KEYSTRING = "rampart_blocked_keystring";
    public static final String RAMPART_BLOCKED_LINK_PREVIEW_MESSAGES =
        "rampart_blocked_link_preview_messages";
    public static final String RAMPART_BLOCKED_LOCATION_GALLERY =
        "rampart_blocked_location_gallery";
    public static final String RAMPART_BLOCKED_LOCATION_MESSAGES =
        "rampart_blocked_location_messages";
    public static final String RAMPART_BLOCKED_SHARED_ALBUM_GALLERY =
        "rampart_blocked_shared_album_gallery";
    public static final String RAMPART_BLOCKED_UNKNOWN_APPS = "rampart_blocked_unknown_apps";
    public static final String RAMPART_BLOCKED_USB_DATA_TRANSFER =
        "rampart_blocked_usb_data_transfer";
    public static final String RAMPART_ENABLED_DEVICE_PROTECTION =
        "rampart_enabled_device_protection";
    ...
    public static final String RAMPART_STRICT_PROTECTION_SWITCH_ENABLED =
    ...
    @Readable public static final String REFRESH_RATE_MODE = "refresh_rate_mode";
    public static final int REFRESH_RATE_MODE_ALWAYS = 2;

    @Readable public static final String REFRESH_RATE_MODE_COVER = "refresh_rate_mode_cover";
    ...
    @Readable
    public static final String RGB_GAIN_API_DISPLAY_GREEN_LEVEL =
        "rgb_gain_api_display_green_level";

    @Readable
    public static final String RGB_GAIN_API_DISPLAY_RED_LEVEL = "rgb_gain_api_display_red_level";

    @Readable public static final String RGB_GAIN_DISPLAY_ACTIVATED = "rgb_gain_display_activated";
    ...
    @Readable public static final String RGB_GAIN_DISPLAY_RED_LEVEL = "rgb_gain_display_red_level";

    @Readable public static final String RTT_CALLING_MODE = "rtt_calling_mode";

    @Readable
    public static final String SAMSUNGFLOW_CLIPBOARD_SYNC_STATE =
        "samsungflow_clipboard_sync_state";
    ...
    @Readable public static final String SCREENSAVER_COMPONENTS = "screensaver_components";

    @Readable
    public static final String SCREENSAVER_DEFAULT_COMPONENT = "screensaver_default_component";

    @Readable public static final String SCREENSAVER_ENABLED = "screensaver_enabled";
    public static final String SCREENSAVER_HOME_CONTROLS_ENABLED =
        "screensaver_home_controls_enabled";

    @Readable public static final String SCREEN_EXTRA_BRIGHTNESS = "screen_extra_brightness";
    public static final String SCREEN_FLASH_NOTIFICATION_COLOR_APPS =
    ...
    @Readable public static final String SEARCH_MAX_STAT_AGE_MILLIS = "search_max_stat_age_millis";
    ...
    @Readable public static final String SEARCH_PREFILL_MILLIS = "search_prefill_millis";
    ...
    @Readable
    public static final String SECURE_FOLDER_AUTOMATIC_DATA_DECRYPTION =
        "automatic_data_decryption";

    @Readable public static final String SECURE_FOLDER_IMAGE_NAME = "secure_folder_image_name";

    @Readable public static final String SECURE_FOLDER_NAME = "secure_folder_name";

    @Readable @Deprecated public static final String SECURE_FRP_MODE = "secure_frp_mode";

    @Readable
    public static final String SELECTED_INPUT_METHOD_SUBTYPE = "selected_input_method_subtype";

    @Readable public static final String SELECTED_SPELL_CHECKER = "selected_spell_checker";

    @Readable
    public static final String SELECTED_SPELL_CHECKER_SUBTYPE = "selected_spell_checker_subtype";

    public static final String SEM_ACCESSIBILITY_CAPTIONING_BACKGROUND_COLOR =
        "accessibility_captioning_background_color";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_EDGE_COLOR =
        "accessibility_captioning_edge_color";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_EDGE_TYPE =
        "accessibility_captioning_edge_type";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_ENABLED =
        "accessibility_captioning_enabled";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_FONT_SCALE =
        "accessibility_captioning_font_scale";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_FOREGROUND_COLOR =
        "accessibility_captioning_foreground_color";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_LOCALE =
        "accessibility_captioning_locale";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_PRESET =
        "accessibility_captioning_preset";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_TYPEFACE =
        "accessibility_captioning_typeface";
    public static final String SEM_ACCESSIBILITY_CAPTIONING_WINDOW_COLOR =
        "accessibility_captioning_window_color";
    public static final String SEM_ACCESSIBILITY_DISPLAY_MAGNIFICATION_ENABLED =
        "accessibility_display_magnification_enabled";

    @Readable
    public static final String SEM_ACCESSIBILITY_MAGNIFICATION_ACTIVATED =
        "accessibility_magnification_activated";

    @Readable
    public static final String SEM_ACCESSIBILITY_SCRIPT_INJECTION =
        "accessibility_script_injection";

    @Readable public static final String SEM_APPLOCK_ENABLED = "app_lock_enabled";
    ...
    @Readable public static final String SEM_APPLOCK_LOCK_TYPE = "applock_lock_type";

    @Readable
    public static final String SEM_ASSISTANT_MENU_CURSOR_PAD_SIZE = "assistant_menu_pad_size";

    @Readable
    public static final String SEM_ASSISTANT_MENU_CURSOR_POINTER_SIZE =
        "assistant_menu_pointer_size";

    @Readable
    public static final String SEM_ASSISTANT_MENU_CURSOR_POINTER_SPEED =
        "assistant_menu_pointer_speed";

    @Readable
    public static final String SEM_ASSISTANT_MENU_DOMINANT_HAND_TYPE =
        "assistant_menu_dominant_hand_type";

    @Readable
    public static final String SEM_AUTO_WIFI_AVERAGE_TIME_TO_FAVORITE_AP =
        "sem_auto_wifi_average_time_to_favorite_ap";

    @Readable
    public static final String SEM_AUTO_WIFI_TIME_BECOME_FAVORITE_AP =
        "sem_auto_wifi_time_become_favorite_ap";

    @Readable public static final String SEM_AUTO_WIFI_TURN_ON_TIME = "sem_auto_wifi_turn_on_time";

    @Deprecated public static final String SEM_BOLD_TEXT = "bold_text";

    @Readable public static final String SEM_COLOR_ADJUSTMENT_TYPE = "color_adjustment_type";

    @Readable public static final String SEM_COLOR_BLIND_SEVERITY = "color_blind_cvdseverity";

    @Readable public static final String SEM_COLOR_BLIND_TYPE = "color_blind_cvdtype";

    @Readable
    public static final String SEM_COLOR_BLIND_USER_PARAMETER = "color_blind_user_parameter";

    @Readable public static final String SEM_COLOR_LENS_OPACTITY = "color_lens_opacity";

    @Readable public static final String SEM_COLOR_LENS_SWITCH = "color_lens_switch";

    @Readable public static final String SEM_COLOR_LENS_TYPE = "color_lens_type";

    @Readable public static final String SEM_DIRECT_WRITING_TOOLBAR = "direct_writing_toolbar";
    ...
    @Readable public static final String SEM_PERFORMANCE_MODE = "sem_perfomance_mode";

    @Readable
    public static final String SEM_PREDEFINED_COLOR_BLIND_INTENSITY =
        "predefined_color_blind_intensity";

    @Readable public static final String SEM_RELUMINO_EDGE_THICKNESS = "relumino_edge_thickness";

    @Readable public static final String SEM_RELUMINO_SWITCH = "relumino_switch";

    @Readable public static final String SEM_RELUMINO_TYPE = "relumino_type";
    public static final String SEM_SIP_KEYBOARD_TYPE_MOUSE_ID_LIST =
        "sip_keyboard_type_mouse_id_list";
    ...
    @Readable public static final String SETTINGS_CLASSNAME = "settings_classname";
    ...
    @Readable public static final String SHOW_MEDIA_WHEN_BYPASSING = "show_media_when_bypassing";
    ...
    @Readable
    public static final String SHOW_NOTIFICATION_CATEGORY_SETTING =
        "show_notification_category_setting";

    @Readable public static final String SHOW_NOTIFICATION_SNOOZE = "show_notification_snooze";
    public static final String SHOW_QR_CODE_SCANNER_SETTING = "show_qr_code_scanner_setting";

    @Readable public static final String SHOW_ROTATION_SUGGESTIONS = "show_rotation_suggestions";
    ...
    @Readable public static final String SILENCE_ALARMS_TOUCH_COUNT = "silence_alarms_touch_count";

    @Readable public static final String SILENCE_CALL_GESTURE_COUNT = "silence_call_gesture_count";

    @Readable public static final String SILENCE_CALL_TOUCH_COUNT = "silence_call_touch_count";

    @Readable public static final String SILENCE_GESTURE = "silence_gesture";

    @Readable
    public static final String SILENCE_TIMER_GESTURE_COUNT = "silence_timer_gesture_count";

    @Readable public static final String SILENCE_TIMER_TOUCH_COUNT = "silence_timer_touch_count";
    ...
    @Readable public static final String SKIP_DIRECTION = "skip_gesture_direction";

    @Readable public static final String SKIP_FIRST_USE_HINTS = "skip_first_use_hints";

    @Readable public static final String SKIP_GESTURE = "skip_gesture";

    @Readable public static final String SKIP_GESTURE_COUNT = "skip_gesture_count";

    @Readable public static final String SKIP_TOUCH_COUNT = "skip_touch_count";

    @Readable public static final String SLEEP_TIMEOUT = "sleep_timeout";

    @Readable
    public static final String SMART_TETHERING_MHS_FIRST_TIME_CONNECT =
        "wifi_ap_smart_tethering_first_time_connect";

    @Readable public static final String SMS_DEFAULT_APPLICATION = "sms_default_application";

    @Readable public static final String SMS_PREFMODE = "sms_prefmode";

    @Readable public static final String SMS_PREFMODE2 = "sms_prefmode2";

    @Readable public static final String SMS_PREFMODE_DOMESTIC = "sms_prefmode_domestic";

    @Readable public static final String SMS_PREFMODE_DOMESTIC2 = "sms_prefmode_domestic2";

    @Readable public static final String SMS_PREFMODE_FOREIGN = "sms_prefmode_foreign";

    @Readable public static final String SMS_PREFMODE_FOREIGN2 = "sms_prefmode_foreign2";
    public static final String SPATIAL_AUDIO_ENABLED = "spatial_audio_enabled";

    @Readable public static final String SPELL_CHECKER_ENABLED = "spell_checker_enabled";
    public static final String STATUS_BAR_SHOW_VIBRATE_ICON = "status_bar_show_vibrate_icon";

    @Readable public static final String STYLUS_BUTTONS_ENABLED = "stylus_buttons_enabled";

    @Readable public static final int STYLUS_HANDWRITING_DEFAULT_VALUE = 1;

    @Readable public static final String STYLUS_HANDWRITING_ENABLED = "stylus_handwriting_enabled";

    @Readable
    public static final String SUPPRESS_AUTO_BATTERY_SAVER_SUGGESTION =
        "suppress_auto_battery_saver_suggestion";

    @Readable public static final String SUPPRESS_DOZE = "suppress_doze";
    public static final String SWIPE_BOTTOM_TO_NOTIFICATION_ENABLED =
        "swipe_bottom_to_notification_enabled";

    @Readable public static final String SYNC_PARENT_SOUNDS = "sync_parent_sounds";
    ...
    @Readable public static final String TAP_DURATION_ENABLED = "tap_duration_enabled";

    @Readable public static final String TAP_DURATION_THRESHOLD = "tap_duration_threshold";

    @Readable public static final String TAP_GESTURE = "tap_gesture";
    ...
    @Readable public static final String TOUCH_BLOCKING_ENABLED = "touch_blocking_enabled";

    @Readable public static final String TOUCH_BLOCKING_PERIOD = "touch_blocking_period";

    @Readable public static final String TOUCH_EXPLORATION_ENABLED = "touch_exploration_enabled";
    ...
    @Readable public static final String TRUST_AGENTS_EXTEND_UNLOCK = "trust_agents_extend_unlock";

    @Readable public static final String TRUST_AGENTS_INITIALIZED = "trust_agents_initialized";

    @Readable @Deprecated public static final String TTS_DEFAULT_COUNTRY = "tts_default_country";

    @Readable @Deprecated public static final String TTS_DEFAULT_LANG = "tts_default_lang";

    @Readable public static final String TTS_DEFAULT_LOCALE = "tts_default_locale";

    @Readable public static final String TTS_DEFAULT_PITCH = "tts_default_pitch";

    @Readable public static final String TTS_DEFAULT_RATE = "tts_default_rate";

    @Readable public static final String TTS_DEFAULT_SYNTH = "tts_default_synth";

    @Readable @Deprecated public static final String TTS_DEFAULT_VARIANT = "tts_default_variant";

    @Readable public static final String TTS_ENABLED_PLUGINS = "tts_enabled_plugins";

    @Readable @Deprecated public static final String TTS_USE_DEFAULTS = "tts_use_defaults";

    @Readable public static final String TTY_MODE_ENABLED = "tty_mode_enabled";

    @Readable
    public static final String TV_APP_USES_NON_SYSTEM_INPUTS = "tv_app_uses_non_system_inputs";

    @Readable public static final String TV_INPUT_CUSTOM_LABELS = "tv_input_custom_labels";

    @Readable public static final String TV_INPUT_HIDDEN_INPUTS = "tv_input_hidden_inputs";

    @Readable public static final String TV_USER_SETUP_COMPLETE = "tv_user_setup_complete";

    @Readable public static final String UI_NIGHT_MODE = "ui_night_mode";

    @Readable public static final String UI_NIGHT_MODE_CUSTOM_TYPE = "ui_night_mode_custom_type";
    public static final String UI_NIGHT_MODE_LAST_COMPUTED = "ui_night_mode_last_computed";

    @Readable public static final String UI_NIGHT_MODE_OVERRIDE_OFF = "ui_night_mode_override_off";

    @Readable public static final String UI_NIGHT_MODE_OVERRIDE_ON = "ui_night_mode_override_on";
    ...
    @Readable
    public static final String USCC_USER_DATA_ROAM_SETTING_DOMESTIC =
        "uscc_user_data_roam_setting_domestic";

    @Readable
    public static final String USCC_USER_DATA_ROAM_SETTING_INTERNATIONAL =
        "uscc_user_data_roam_setting_international";

    @SystemApi @Readable public static final String USER_SETUP_COMPLETE = "user_setup_complete";

    @SystemApi public static final int USER_SETUP_PERSONALIZATION_COMPLETE = 10;

    @SystemApi public static final int USER_SETUP_PERSONALIZATION_NOT_STARTED = 0;

    @SystemApi public static final int USER_SETUP_PERSONALIZATION_PAUSED = 2;

    @SystemApi public static final int USER_SETUP_PERSONALIZATION_STARTED = 1;
    ...
    @Readable
    public static final String USER_USED_APP_CONTINUITY_SETTING =
        "user_used_app_continuity_setting";

    @Deprecated public static final String USE_GOOGLE_MAIL = "use_google_mail";

    @Readable public static final String VOICE_INTERACTION_SERVICE = "voice_interaction_service";

    @Readable public static final String VOICE_RECOGNITION_SERVICE = "voice_recognition_service";

    @SystemApi @Readable public static final String VOLUME_HUSH_GESTURE = "volume_hush_gesture";

    @SystemApi public static final int VOLUME_HUSH_MUTE = 2;

    @SystemApi public static final int VOLUME_HUSH_OFF = 0;

    @SystemApi public static final int VOLUME_HUSH_VIBRATE = 1;

    @Readable
    public static final String VOLUME_LIMITER_SECURE_PASSWORD = "volumelimit_secure_password";

    @Readable public static final String VR_DISPLAY_MODE = "vr_display_mode";
    public static final int VR_DISPLAY_MODE_LOW_PERSISTENCE = 0;
    public static final int VR_DISPLAY_MODE_OFF = 1;

    @Readable public static final String WAKE_GESTURE_ENABLED = "wake_gesture_enabled";
    ...
    @Readable public static final String WIFI_7_MODE_ENABLED = "sec_wifi_7_mode_enabled";

    @Readable public static final String WIFI_ADPS = "wifi_adps_enable";

    @Readable public static final String WIFI_AP_11AX_MODE_CHECKED = "wifi_ap_11ax_mode_checked";

    @Readable public static final String WIFI_AP_5G_CHECKED = "wifi_ap_5G_checked";

    @Readable public static final String WIFI_AP_CHIP_MAXCLIENT = "wifi_ap_chip_maxclient";

    @Readable public static final String WIFI_AP_CHIP_SUPPORT5G = "wifi_ap_chip_support5g";

    @Readable
    public static final String WIFI_AP_CHIP_SUPPORT5G_BASEON_COUNTRY =
        "wifi_ap_chip_support5g_baseon_country";

    @Readable public static final String WIFI_AP_CUSTOMER = "wifi_ap_customer";

    @Readable
    public static final String WIFI_AP_DEFAULT_CONF_GENERATED = "wifi_ap_SoftAp_conf_present";

    @Readable public static final String WIFI_AP_DELAY_SCAN_LCD_ON = "wifi_ap_delay_scan_lcd_on";

    @Readable public static final String WIFI_AP_DISABLE_RANDOM_MAC = "wifi_ap_disable_random_mac";

    @Readable public static final String WIFI_AP_DUAL_POPUP = "wifi_ap_dual_popup";

    @Readable
    public static final String WIFI_AP_ENABLE_WIFI_SHARING = "wifi_ap_enable_wifi_sharing";

    @Readable
    public static final String WIFI_AP_FIRST_TIME_WIFI_SHARING_DIALOG =
    ...
    @Readable public static final String WIFI_AP_LAST_2G_CHANNEL = "wifi_ap_last_2g_channel";

    @Readable public static final String WIFI_AP_MOBILE_DATA_LIMIT = "wifi_ap_mobile_data_limit";
    ...
    @Readable
    public static final String WIFI_AP_NUMBER_OF_MAX_MACADDR_CLIENT =
        "wifi_ap_number_of_max_macaddr_client";

    @Readable public static final String WIFI_AP_PLUGGED_TYPE = "wifi_ap_plugged_type";

    @Readable public static final String WIFI_AP_PMF_CHECKED = "wifi_ap_pmf_checked";
    ...
    @Readable public static final String WIFI_AP_PROVISION_SUCCESS = "wifi_ap_provision_success";

    @Readable public static final String WIFI_AP_QR_CODE_GENERATED = "wifi_ap_qr_code_generated";

    @Readable public static final String WIFI_AP_RANDOM_PASSWORD = "wifi_ap_random_password";

    @Readable public static final String WIFI_AP_RVFMODE = "wifi_ap_rvfmode";

    @Readable public static final String WIFI_AP_SAVED_STATE = "wifi_ap_saved_state";

    @Readable public static final String WIFI_AP_SECURITY_TYPE = "wifi_ap_security_type";

    @Readable public static final String WIFI_AP_SHOW_PASSWORD = "wifi_ap_show_passwd";

    @Readable public static final String WIFI_AP_SMART_D2D_TETHERING = "wifi_ap_smart_d2d_mhs";
    ...
    @Readable
    public static final String WIFI_AP_SMART_TETHERING_USER_TYPE =
        "wifi_ap_smart_tethering_user_type";

    @Readable
    public static final String WIFI_AP_SMART_TETHERING_WITH_FAMLIY =
        "wifi_ap_smart_tethering_settings_with_family";

    @Readable
    public static final String WIFI_AP_SMART_TETHERING_WITH_FAMLIY_RESTORING_REQUIRED =
        "wifi_ap_smart_tethering_settings_with_family_restoring_required";

    @Readable public static final String WIFI_AP_SUPPORTTEMP_SPR = "wifi_ap_supporttemp_spr";

    @Readable public static final String WIFI_AP_TIMEOUT_SETTING = "wifi_ap_timeout_setting";

    @Readable public static final String WIFI_AP_TIME_OUT_VALUE = "wifi_ap_time_out_value";
    public static final String WIFI_AP_TX_POWER_CHANGED_BY_SERVICE =
        "wifi_ap_tx_power_changed_by_service";

    @Readable public static final String WIFI_AP_WIFI_SHARING = "wifi_ap_wifi_sharing";
    ...
    public static final String WIFI_HOTSPOT20_CONNECTED_HISTORY =
        "wifi_hotspot20_connected_history";
    public static final String WIFI_HOTSPOT20_ENABLE = "wifi_hotspot20_enable";
    public static final String WIFI_HOTSPOT20_USEABLE_VENDOR_USIM =
        "wifi_hotspot20_useable_vendor_usim";

    @Deprecated public static final String WIFI_IDLE_MS = "wifi_idle_ms";

    @Readable @Deprecated
    public static final String WIFI_INTERNET_SERVICE_CHECK_WARNING =
        "wifi_internet_service_check_warning";

    @Deprecated public static final String WIFI_MAX_DHCP_RETRY_COUNT = "wifi_max_dhcp_retry_count";

    @Readable public static final String WIFI_MLO_LINK_COUNT = "sec_wifi_mlo_link_count";

    @Deprecated
    public static final String WIFI_MOBILE_DATA_TRANSITION_WAKELOCK_TIMEOUT_MS =
        "wifi_mobile_data_transition_wakelock_timeout_ms";

    @Readable public static final String WIFI_MWIPS = "wifi_mwips";
    ...
    @Readable public static final String WIFI_SAVED_STATE = "wifi_saved_state";

    @Readable public static final String WIFI_SENSOR_MONITOR_ENABLE = "wifi_sensor_monitor_enable";

    @Readable
    public static final String WIFI_SHARING_LITE_POPUP_FLAG = "wifi_sharing_lite_popup_status";

    @Readable
    public static final String WIFI_SNS_DIALOG_FOR_STARTING_SETTINGS =
        "wifi_sns_dialog_for_starting_settings";

    @Readable
    public static final String WIFI_SNS_VISITED_COUNTRY_ISO = "wifi_sns_visited_country_iso";

    @Readable
    public static final String WIFI_SUSPEND_HOTSPOT_CONNECTION_DURING_SLEEP =
        "smart_wifi_ap_advanced_connect_lcd_off";
    ...
    @Readable @Deprecated public static final String WIFI_WATCHDOG_ON = "wifi_watchdog_on";
    ...
    @Readable public static final String WIFI_WWSM_PATCH_KEY = "wifi_wwsm_patch_key";

    @Readable
    public static final String WIFI_WWSM_PATCH_NEED_TO_CHECK_APPSTORE =
        "wifi_wwsm_patch_need_to_check_appstore";

    @Readable
    public static final String WIFI_WWSM_PATCH_REMOVE_SNS_MENU_FROM_SETTINGS =
        "wifi_wwsm_patch_remove_sns_menu_from_settings";

    @Readable
    public static final String WIFI_WWSM_PATCH_RESTORE_SNS_ENABLED =
        "wifi_wwsm_patch_restore_sns_enabled";

    @Readable
    public static final String WIFI_WWSM_PATCH_TEST_MODE_ENABLED =
        "wifi_wwsm_patch_test_mode_enabled";

    @Readable
    public static final String WIFI_WWSM_PATCH_UPDATE_AVAILABLE =
        "wifi_wwsm_patch_update_available";

    @Readable public static final String ZEN_DURATION = "zen_duration";
    ...
    @Readable public static final String ZEN_SETTINGS_UPDATED = "zen_settings_updated";
    private static final NameValueCache sNameValueCache;
    private static final ContentProviderHolder sProviderHolder;
    ...
    public static void getPublicSettings(
        ...
        ArrayMap<String, Integer> readableKeysWithMaxTargetSdk) {
      Settings.getPublicSettingsForClass(
          Secure.class, allKeys, readableKeys, readableKeysWithMaxTargetSdk);
    }
    ...
    public static String getStringForUser(ContentResolver resolver, String name, int userHandle) {
      if (MOVED_TO_GLOBAL.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.Secure to"
      ...
      if (MOVED_TO_LOCK_SETTINGS.contains(name) && Process.myUid() != 1000) {
        ...
        boolean isPreMnc =
            (application == null
                    || application.getApplicationInfo() == null
                    || application.getApplicationInfo().targetSdkVersion > 22)
                ? false
                : true;
        if (isPreMnc) {
          return "0";
        ...
        throw new SecurityException(
            "Settings.Secure."
                + name
                + " is deprecated and no longer accessible. See API documentation for potential"
                + " replacements.");
      }
      return sNameValueCache.getStringForUser(resolver, name, userHandle);
    }
    ...
    public static boolean putString(
        ContentResolver resolver, String name, String value, boolean overrideableByRestore) {
      return putStringForUser(
          resolver, name, value, null, false, resolver.getUserId(), overrideableByRestore);
    }
    ...
    @SystemApi
    public static boolean putString(
        ContentResolver resolver, String name, String value, String tag, boolean makeDefault) {
    ...
    public static void resetToDefaultsAsUser(
        ContentResolver resolver, String tag, int mode, int userHandle) {
      try {
        Bundle arg = new Bundle();
        arg.putInt(Settings.CALL_METHOD_USER_KEY, userHandle);
        if (tag != null) {
        ...
        cp.call(
            resolver.getAttributionSource(),
            contentProviderHolder.mUri.getAuthority(),
            Settings.CALL_METHOD_RESET_SECURE,
            null,
            arg);
      } catch (RemoteException e) {
      ...
      if (MOVED_TO_GLOBAL.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.Secure to"
                + " android.provider.Settings.Global, returning global URI.");
        return Global.getUriFor(Global.CONTENT_URI, name);
      }
      return getUriFor(CONTENT_URI, name);
    }
    ...
    public static boolean isLocationProviderEnabled(ContentResolver cr, String provider) {
      ...
      ILocationManager lm =
          (ILocationManager) Objects.requireNonNull(ILocationManager.Stub.asInterface(binder));
      try {
    ...
    @Deprecated
    public static void setLocationProviderEnabled(
        ContentResolver cr, String provider, boolean enabled) {}
  }

  public static final class Global extends NameValueTable {

    @Readable public static final String ACTIVITY_MANAGER_CONSTANTS = "activity_manager_constants";
    ...
    @Readable public static final String ADB_ENABLED = "adb_enabled";

    @Readable public static final String ADB_WIFI_ENABLED = "adb_wifi_enabled";

    @Readable public static final String ADD_USERS_WHEN_LOCKED = "add_users_when_locked";

    @Readable
    public static final String ADVANCED_BATTERY_USAGE_AMOUNT = "advanced_battery_usage_amount";

    @Readable public static final String AIRPLANE_MODE_ON = "airplane_mode_on";

    @Readable public static final String AIRPLANE_MODE_RADIOS = "airplane_mode_radios";
    ...
    @Readable public static final String ALL_SOUND_OFF = "all_sound_off";

    @Readable public static final String ALWAYS_FINISH_ACTIVITIES = "always_finish_activities";

    @Readable
    public static final String ALWAYS_ON_DISPLAY_CONSTANTS = "always_on_display_constants";

    @Readable public static final String ANGLE_DEBUG_PACKAGE = "angle_debug_package";

    @Readable public static final String ANGLE_EGL_FEATURES = "angle_egl_features";

    @Readable public static final String ANGLE_GL_DRIVER_ALL_ANGLE = "angle_gl_driver_all_angle";
    ...
    @Readable public static final String ANIMATOR_DURATION_SCALE = "animator_duration_scale";

    @Readable public static final String ANOMALY_CONFIG = "anomaly_config";

    @Readable public static final String ANOMALY_CONFIG_VERSION = "anomaly_config_version";

    @Readable
    public static final String ANOMALY_DETECTION_CONSTANTS = "anomaly_detection_constants";

    @Readable public static final String APN_DB_UPDATE_CONTENT_URL = "apn_db_content_url";

    @Readable public static final String APN_DB_UPDATE_METADATA_URL = "apn_db_metadata_url";

    @Readable @Deprecated public static final String APPLY_RAMPING_RINGER = "apply_ramping_ringer";

    @Readable public static final String APPOP_HISTORY_BASE_INTERVAL_MILLIS = "baseIntervalMillis";

    @Readable public static final String APPOP_HISTORY_INTERVAL_MULTIPLIER = "intervalMultiplier";

    @Readable public static final String APPOP_HISTORY_MODE = "mode";

    @Readable public static final String APPOP_HISTORY_PARAMETERS = "appop_history_parameters";

    @Readable
    public static final String APP_AUTO_RESTRICTION_ENABLED = "app_auto_restriction_enabled";

    @Readable public static final String APP_BINDING_CONSTANTS = "app_binding_constants";

    @Readable
    public static final String APP_INTEGRITY_VERIFICATION_TIMEOUT =
        "app_integrity_verification_timeout";

    @Readable public static final String APP_OPS_CONSTANTS = "app_ops_constants";

    @SystemApi @Readable public static final String APP_STANDBY_ENABLED = "app_standby_enabled";
    ...
    @Readable public static final String ASSISTED_GPS_ENABLED = "assisted_gps_enabled";
    ...
    @Readable public static final String AUDIO_SAFE_VOLUME_STATE = "audio_safe_volume_state";

    @SystemApi @Readable @Deprecated
    public static final String AUTOFILL_COMPAT_MODE_ALLOWED_PACKAGES =
        "autofill_compat_mode_allowed_packages";

    @Readable public static final String AUTOFILL_LOGGING_LEVEL = "autofill_logging_level";
    ...
    @Readable public static final String AUTOMATIC_POWER_SAVE_MODE = "automatic_power_save_mode";

    @Readable public static final String AUTO_OMC_UPDATE = "auto_omc_update";

    @Readable public static final String AUTO_ON_PROTECT_BATTERY = "auto_on_protect_battery";

    @Readable public static final String AUTO_REVOKE_PARAMETERS = "auto_revoke_parameters";

    @Readable public static final String AUTO_TIME = "auto_time";

    @Readable public static final String AUTO_TIME_ZONE = "auto_time_zone";
    ...
    @Readable public static final String AWARE_ALLOWED = "aware_allowed";
    ...
    @Readable
    public static final String BATTERY_PROTECTION_RECHARGE_LEVEL =
        "battery_protection_recharge_level";

    @Readable public static final int BATTERY_PROTECTION_RECHARGE_LEVEL_DEFAULT_VALUE = 95;

    @Readable
    public static final String BATTERY_PROTECTION_THRESHOLD = "battery_protection_threshold";

    @Readable public static final int BATTERY_PROTECTION_THRESHOLD_DEFAULT_VALUE;

    @Readable public static final String BATTERY_SAVER_CONSTANTS = "battery_saver_constants";

    @Readable
    public static final String BATTERY_SAVER_DEVICE_SPECIFIC_CONSTANTS =
        "battery_saver_device_specific_constants";

    @Readable public static final String BATTERY_STATS_CONSTANTS = "battery_stats_constants";

    @Readable public static final String BATTERY_TIP_CONSTANTS = "battery_tip_constants";

    @Readable public static final String BINDER_CALLS_STATS = "binder_calls_stats";

    @Readable public static final String BIXBY_TEXT_CALL_LANGUAGE = "screen_call_language";
    ...
    @Readable public static final String BLOCKED_SLICES = "blocked_slices";
    ...
    public static final String BLUETOOTH_A2DP_SINK_PRIORITY_PREFIX =
        "bluetooth_a2dp_sink_priority_";

    @Readable
    public static final String BLUETOOTH_A2DP_SRC_PRIORITY_PREFIX = "bluetooth_a2dp_src_priority_";

    public static final String BLUETOOTH_A2DP_SUPPORTS_LDAC_PREFIX =
        "bluetooth_a2dp_supports_ldac_";
    ...
    @Readable
    public static final String BLUETOOTH_HEARING_AID_PRIORITY_PREFIX =
    ...
    @Readable
    public static final String BLUETOOTH_MAP_CLIENT_PRIORITY_PREFIX =
        "bluetooth_map_client_priority_";

    @Readable public static final String BLUETOOTH_MAP_PRIORITY_PREFIX = "bluetooth_map_priority_";

    @Readable public static final String BLUETOOTH_ON = "bluetooth_on";

    @Readable public static final String BLUETOOTH_PAN_PRIORITY_PREFIX = "bluetooth_pan_priority_";

    @Readable
    public static final String BLUETOOTH_PBAP_CLIENT_PRIORITY_PREFIX =
        "bluetooth_pbap_client_priority_";

    @Readable public static final String BLUETOOTH_SAP_PRIORITY_PREFIX = "bluetooth_sap_priority_";

    @Readable
    public static final String BLUETOOTH_SECURITY_ON_CHECK = "bluetooth_security_on_check";

    @Readable public static final String BOOT_COUNT = "boot_count";

    @Readable public static final String BOTTOM_GESTURE_INSET_SCALE = "bottom_gesture_inset_scale";

    @Readable public static final String BROADCAST_BG_CONSTANTS = "bcast_bg_constants";

    @Readable public static final String BROADCAST_FG_CONSTANTS = "bcast_fg_constants";

    @Readable public static final String BROADCAST_OFFLOAD_CONSTANTS = "bcast_offload_constants";
    ...
    @Readable public static final String CACHED_APPS_FREEZER_ENABLED = "cached_apps_freezer";

    @Readable public static final String CALL_AUTO_RETRY = "call_auto_retry";

    @Readable public static final String CALL_AUTO_ROTATION = "call_auto_rotation";

    @Readable public static final String CALL_READ_CALLER_ID = "call_read_caller_id";
    ...
    @Readable public static final String CAPTIVE_PORTAL_HTTPS_URL = "captive_portal_https_url";

    @Readable public static final String CAPTIVE_PORTAL_HTTP_URL = "captive_portal_http_url";

    @Readable public static final String CAPTIVE_PORTAL_MODE = "captive_portal_mode";
    ...
    @Readable public static final String CAPTIVE_PORTAL_SERVER = "captive_portal_server";

    @Readable public static final String CAPTIVE_PORTAL_USER_AGENT = "captive_portal_user_agent";

    @Readable public static final String CAPTIVE_PORTAL_USE_HTTPS = "captive_portal_use_https";

    @SystemApi @Readable public static final String CARRIER_APP_NAMES = "carrier_app_names";

    @SystemApi @Readable public static final String CARRIER_APP_WHITELIST = "carrier_app_whitelist";

    @Readable public static final String CAR_DOCK_SOUND = "car_dock_sound";

    @Readable public static final String CAR_UNDOCK_SOUND = "car_undock_sound";

    @Readable public static final String CDMA_CELL_BROADCAST_SMS = "cdma_cell_broadcast_sms";

    @Readable public static final String CDMA_ROAMING_MODE = "roaming_settings";

    @Readable public static final String CDMA_SUBSCRIPTION_MODE = "subscription_mode";

    @Readable public static final String CELL_ON = "cell_on";

    @Readable public static final String CERT_PIN_UPDATE_CONTENT_URL = "cert_pin_content_url";

    @Readable public static final String CERT_PIN_UPDATE_METADATA_URL = "cert_pin_metadata_url";

    @Readable
    public static final String CHAINED_BATTERY_ATTRIBUTION_ENABLED =
        "chained_battery_attribution_enabled";

    @Readable public static final String CHARGER_CONNECTED_TIME = "charger_connected_time";

    @Deprecated public static final String CHARGING_SOUNDS_ENABLED = "charging_sounds_enabled";

    @Readable public static final String CHARGING_STARTED_SOUND = "charging_started_sound";

    @Deprecated
    public static final String CHARGING_VIBRATION_ENABLED = "charging_vibration_enabled";

    @Readable public static final String CHECK_PRIVATE_IP_MODE = "check_private_ip_mode";
    ...
    @Readable public static final String COMPATIBILITY_MODE = "compatibility_mode";

    @Readable public static final String CONNECTIVITY_CHANGE_DELAY = "connectivity_change_delay";
    ...
    @Readable public static final String CONTACT_SWIPE_TO_CALL_MESSAGE = "swipe_to_call_message";
    public static final Uri CONTENT_URI;
    ...
    @Readable public static final String DATABASE_CREATION_BUILDID = "database_creation_buildid";

    @Readable public static final String DATABASE_DOWNGRADE_REASON = "database_downgrade_reason";

    @Readable
    public static final String DATA_ACTIVITY_TIMEOUT_MOBILE = "data_activity_timeout_mobile";

    @Readable public static final String DATA_ACTIVITY_TIMEOUT_WIFI = "data_activity_timeout_wifi";
    ...
    @Readable
    public static final String DBSC_CONSENT_CROSS_BORDER_AGREE_VALUE =
        "dbsc_consent_cross_border_agree_value";

    @Readable
    public static final String DBSC_CONSENT_CUSTOMIZED_SERVICE_AGREE_DATE =
        "dbsc_consent_customized_service_agree_date";

    @Readable
    public static final String DBSC_CONSENT_CUSTOMIZED_SERVICE_AGREE_VALUE =
        "dbsc_consent_customized_service_agree_value";

    @Readable
    public static final String DBSC_CONSENT_MARKETING_AGREE_DATE =
        "dbsc_consent_marketing_agree_date";

    @Readable
    public static final String DBSC_CONSENT_MARKETING_AGREE_VALUE =
        "dbsc_consent_marketing_agree_value";

    @Readable
    public static final String DBSC_CONSENT_MARKETING_DATA_PROCESSING_AGREE_DATE =
        "dbsc_consent_marketing_data_processing_agree_date";

    @Readable
    public static final String DBSC_CONSENT_MARKETING_DATA_PROCESSING_AGREE_VALUE =
        "dbsc_consent_marketing_data_processing_agree_value";

    @Readable
    public static final String DBSC_CONSENT_PERSONAL_AD_AGREE_DATE =
        "dbsc_consent_personal_ad_agree_date";

    @Readable
    public static final String DBSC_CONSENT_PERSONAL_AD_AGREE_VALUE =
        "dbsc_consent_personal_ad_agree_value";
    ...
    @Readable public static final String DBSC_CONSENT_TNC_COUNTRY = "dbsc_consent_tnc_country";

    @Readable public static final String DEBUG_APP = "debug_app";

    @Readable public static final String DEBUG_VIEW_ATTRIBUTES = "debug_view_attributes";
    ...
    @Readable public static final String DEFAULT_DEVICE_NAME = "default_device_name";

    @Readable public static final String DEFAULT_DNS_SERVER = "default_dns_server";

    @Readable public static final String DEFAULT_INSTALL_LOCATION = "default_install_location";

    @Readable
    public static final String DEFAULT_RESTRICT_BACKGROUND_DATA =
        "default_restrict_background_data";

    @SystemApi @Readable public static final String DEFAULT_SM_DP_PLUS = "default_sm_dp_plus";

    @Readable public static final String DESK_DOCK_SOUND = "desk_dock_sound";

    @Readable public static final String DESK_UNDOCK_SOUND = "desk_undock_sound";
    ...
    @Readable public static final String DEVELOPMENT_FORCE_RTL = "debug.force_rtl";
    ...
    @Readable public static final String DEVELOPMENT_USE_BLAST_ADAPTER_VR = "use_blast_adapter_vr";
    ...
    @SystemApi @Readable public static final String DEVICE_DEMO_MODE = "device_demo_mode";

    @Readable public static final String DEVICE_NAME = "device_name";

    @Readable public static final String DEVICE_POLICY_CONSTANTS = "device_policy_constants";

    @Readable public static final String DEVICE_PROVISIONED = "device_provisioned";
    ...
    @Readable public static final String DISABLE_WINDOW_BLURS = "disable_window_blurs";

    @Readable
    public static final String DISK_FREE_CHANGE_REPORTING_THRESHOLD =
        "disk_free_change_reporting_threshold";

    @Readable public static final String DISPLAY_PANEL_LPM = "display_panel_lpm";

    @Readable public static final String DISPLAY_SCALING_FORCE = "display_scaling_force";

    @Readable public static final String DISPLAY_SIZE_FORCED = "display_size_forced";

    @Readable public static final String DNS_RESOLVER_MAX_SAMPLES = "dns_resolver_max_samples";

    @Readable public static final String DNS_RESOLVER_MIN_SAMPLES = "dns_resolver_min_samples";
    ...
    @Readable public static final String DOCK_AUDIO_MEDIA_ENABLED = "dock_audio_media_enabled";

    @Readable public static final String DOCK_SOUNDS_ENABLED = "dock_sounds_enabled";

    @Readable
    public static final String DOCK_SOUNDS_ENABLED_WHEN_ACCESSIBILITY =
    ...
    @Readable public static final String DROPBOX_AGE_SECONDS = "dropbox_age_seconds";

    @Readable public static final String DROPBOX_MAX_FILES = "dropbox_max_files";

    @Readable public static final String DROPBOX_QUOTA_KB = "dropbox_quota_kb";

    @Readable public static final String DROPBOX_QUOTA_PERCENT = "dropbox_quota_percent";

    @Readable public static final String DROPBOX_RESERVE_PERCENT = "dropbox_reserve_percent";

    @Readable public static final String DROPBOX_TAG_PREFIX = "dropbox:";
    ...
    @Readable @Deprecated public static final String EDGE_ENABLE = "edge_enable";
    ...
    @Readable public static final String EMERGENCY_TONE = "emergency_tone";

    @Readable public static final String EMULATE_DISPLAY_CUTOUT = "emulate_display_cutout";
    ...
    @Readable public static final String ENABLE_CELLULAR_ON_BOOT = "enable_cellular_on_boot";

    @Readable
    public static final String ENABLE_DELETION_HELPER_NO_THRESHOLD_TOGGLE =
        "enable_deletion_helper_no_threshold_toggle";

    @Readable public static final String ENABLE_DISKSTATS_LOGGING = "enable_diskstats_logging";

    @Readable public static final String ENABLE_EPHEMERAL_FEATURE = "enable_ephemeral_feature";

    @Readable
    public static final String ENABLE_GNSS_RAW_MEAS_FULL_TRACKING =
        "enable_gnss_raw_meas_full_tracking";

    @Readable public static final String ENABLE_GPU_DEBUG_LAYERS = "enable_gpu_debug_layers";
    public static final String ENABLE_MULTI_SLOT_TIMEOUT_MILLIS =
        "enable_multi_slot_timeout_millis";

    @Readable public static final String ENABLE_RADIO_BUG_DETECTION = "enable_radio_bug_detection";
    public static final String ENABLE_TARE = "enable_tare";

    @Readable public static final String ENCODED_SURROUND_OUTPUT = "encoded_surround_output";
    ...
    @Readable @Deprecated public static final String ENHANCED_4G_MODE_ENABLED = "volte_vt_enabled";

    @Readable
    public static final String EPHEMERAL_COOKIE_MAX_SIZE_BYTES = "ephemeral_cookie_max_size_bytes";

    @Readable public static final String ERROR_LOGCAT_PREFIX = "logcat_for_";

    @Readable
    public static final String EUICC_FACTORY_RESET_TIMEOUT_MILLIS =
        "euicc_factory_reset_timeout_millis";

    @SystemApi @Readable public static final String EUICC_PROVISIONED = "euicc_provisioned";
    ...
    @Readable public static final String FANCY_IME_ANIMATIONS = "fancy_ime_animations";

    @Readable public static final String FLEX_MODE_PANEL_ENABLED = "flex_mode_panel_enabled";

    @Readable public static final String FLEX_MODE_SCROLL_WHEEL_POS = "flex_mode_scroll_wheel_pos";

    @Readable
    public static final String FORCED_APP_STANDBY_FOR_SMALL_BATTERY_ENABLED =
        "forced_app_standby_for_small_battery_enabled";

    @Readable public static final String FORCE_ALLOW_ON_EXTERNAL = "force_allow_on_external";
    ...
    @Readable public static final String FPS_DEVISOR = "fps_divisor";
    ...
    @Readable
    public static final String FREEFORM_HANDLER_HELP_POPUP_COUNT =
        "freeform_handler_help_popup_count";

    @Readable public static final String FSTRIM_MANDATORY_INTERVAL = "fstrim_mandatory_interval";

    @Readable
    public static final String GLOBAL_HTTP_PROXY_EXCLUSION_LIST =
        "global_http_proxy_exclusion_list";

    @Readable public static final String GLOBAL_HTTP_PROXY_HOST = "global_http_proxy_host";

    @Readable public static final String GLOBAL_HTTP_PROXY_PAC = "global_proxy_pac_url";

    @Readable public static final String GLOBAL_HTTP_PROXY_PORT = "global_http_proxy_port";
    ...
    @Readable public static final String GOOGLE_CORE_CONTROL = "google_core_control";

    @Readable
    public static final String GPRS_REGISTER_CHECK_PERIOD_MS = "gprs_register_check_period_ms";

    @Readable public static final String GPU_DEBUG_APP = "gpu_debug_app";

    @Readable public static final String GPU_DEBUG_LAYERS = "gpu_debug_layers";

    @Readable public static final String GPU_DEBUG_LAYERS_GLES = "gpu_debug_layers_gles";

    @Readable public static final String GPU_DEBUG_LAYER_APP = "gpu_debug_layer_app";

    @Readable public static final String HDR_CONVERSION_MODE = "hdr_conversion_mode";

    @Readable public static final String HDR_FORCE_CONVERSION_TYPE = "hdr_force_conversion_type";
    ...
    @Readable public static final String HIDDEN_API_POLICY = "hidden_api_policy";

    @Readable public static final String HIDE_ERROR_DIALOGS = "hide_error_dialogs";

    @Readable public static final String HTTP_PROXY = "http_proxy";
    ...
    @Readable public static final String INSTANT_APP_DEXOPT_ENABLED = "instant_app_dexopt_enabled";
    public static final Set<String> INSTANT_APP_SETTINGS;
    ...
    @Readable public static final String KEEP_PROFILE_IN_BACKGROUND = "keep_profile_in_background";

    @Readable public static final String KERNEL_CPU_THREAD_READER = "kernel_cpu_thread_reader";

    @Readable public static final String KEY_CALL_REMINDER = "call_reminder";

    @Readable public static final String KEY_CHORD_POWER_VOLUME_UP = "key_chord_power_volume_up";

    @Readable public static final String LANG_ID_UPDATE_CONTENT_URL = "lang_id_content_url";

    @Readable public static final String LANG_ID_UPDATE_METADATA_URL = "lang_id_metadata_url";
    public static final String[] LEGACY_RESTORE_SETTINGS;

    @Readable public static final String LID_BEHAVIOR = "lid_behavior";
    ...
    @Readable public static final String LOCK_SOUND = "lock_sound";

    @Readable public static final String LOOPER_STATS = "looper_stats";

    @Readable public static final String LOW_BATTERY_SOUND = "low_battery_sound";

    @Readable public static final String LOW_BATTERY_SOUND_TIMEOUT = "low_battery_sound_timeout";

    @Readable public static final String LOW_POWER_MODE = "low_power";

    @Readable public static final String LOW_POWER_MODE_BACK_DATA_OFF = "low_power_back_data_off";
    public static final String LOW_POWER_MODE_REMINDER_ENABLED = "low_power_mode_reminder_enabled";

    @Readable public static final String LOW_POWER_MODE_STICKY = "low_power_sticky";
    ...
    @Readable public static final String LOW_POWER_MODE_TRIGGER_LEVEL = "low_power_trigger_level";
    ...
    @Readable public static final String LTC_HIGHSOC_DURATION = "ltc_highsoc_duration";

    @Readable public static final int LTC_HIGHSOC_DURATION_DEFAULT_VALUE = 10080;

    @Readable public static final String LTC_HIGHSOC_EXCEED_TIME = "ltc_highsoc_exceed_time";

    @Readable public static final String LTC_HIGHSOC_THRESHOLD = "ltc_highsoc_threshold";

    @Readable public static final int LTC_HIGHSOC_THRESHOLD_DEFAULT_VALUE = 95;

    @Readable public static final String LTC_RELEASE_THRESHOLD = "ltc_release_threshold";

    @Readable public static final int LTC_RELEASE_THRESHOLD_DEFAULT_VALUE = 75;

    @Readable public static final String LTE_SERVICE_FORCED = "lte_service_forced";
    ...
    @Readable public static final String MAX_ERROR_BYTES_PREFIX = "max_error_bytes_for_";
    ...
    @Readable public static final String MDC_INITIAL_MAX_RETRY = "mdc_initial_max_retry";

    @Readable
    public static final String MHL_INPUT_SWITCHING_ENABLED = "mhl_input_switching_enabled";

    @Readable public static final String MHL_POWER_CHARGE_ENABLED = "mhl_power_charge_enabled";

    @Readable
    public static final String MIN_DURATION_BETWEEN_RECOVERY_STEPS_IN_MS =
        "min_duration_between_recovery_steps";

    @Readable public static final String MOBILE_DATA = "mobile_data";

    @Readable public static final String MOBILE_DATA_ALWAYS_ON = "mobile_data_always_on";

    @Readable
    public static final String MOBILE_NETWORK_EXPERIENCE_BOOSTER =
        "mobile_network_experience_booster";

    @Readable
    public static final String MODEM_STACK_ENABLED_FOR_SLOT = "modem_stack_enabled_for_slot";

    @Readable public static final String MODE_RINGER = "mode_ringer";

    @Readable public static final String MODE_RINGER_MUTE_TIME = "mode_ringer_time";

    @Readable public static final String MODE_RINGER_MUTE_TIME_ON = "mode_ringer_time_on";
    private static final HashSet<String> MOVED_TO_SECURE;
    private static final HashSet<String> MOVED_TO_SYSTEM;

    @Readable public static final String MULTI_SIM_DATACROSS_SLOT = "multi_sim_datacross_slot";

    @Readable public static final String MULTI_SIM_DATA_CALL_SLOT = "multi_sim_data_call_slot";

    @Readable public static final String MULTI_SIM_DATA_CALL_SUBSCRIPTION = "multi_sim_data_call";

    @Readable public static final String MULTI_SIM_DDS_PROGRESSING = "multi_sim_dds_progressing";

    @Readable public static final String MULTI_SIM_SMS_PROMPT = "multi_sim_sms_prompt";

    @Readable public static final String MULTI_SIM_SMS_SLOT = "multi_sim_sms_slot";

    @Readable public static final String MULTI_SIM_SMS_SUBSCRIPTION = "multi_sim_sms";

    @Readable public static final String[] MULTI_SIM_USER_PREFERRED_SUBS;

    @Readable public static final String MULTI_SIM_VOICE_CALL_SLOT = "multi_sim_voice_call_slot";

    @Readable public static final String MULTI_SIM_VOICE_CALL_SUBSCRIPTION = "multi_sim_voice_call";

    @Readable public static final String MULTI_SIM_VOICE_PROMPT = "multi_sim_voice_prompt";
    ...
    @Readable
    public static final String NAVIGATIONBAR_BACK_GESTURE_SENSITIVITY =
        "navigation_bar_back_gesture_sensitivity";

    @Readable
    public static final String NAVIGATIONBAR_BACK_GESTURE_SENSITIVITY_SUB =
        "navigation_bar_back_gesture_sensitivity_sub";

    @Readable
    public static final String NAVIGATIONBAR_BLOCK_GESTURES_WITH_SPEN =
        "navigation_bar_block_gestures_with_spen";

    @Readable public static final String NAVIGATIONBAR_COLOR = "navigationbar_color";

    @Readable
    public static final String NAVIGATIONBAR_CURRENT_COLOR = "navigationbar_current_color";

    @Readable
    public static final String NAVIGATIONBAR_GESTURES_DETAIL_TYPE =
        "navigation_bar_gesture_detail_type";

    @Readable public static final String NAVIGATIONBAR_GESTURE_HINT = "navigation_bar_gesture_hint";

    @Readable public static final String NAVIGATIONBAR_KEY_ORDER = "navigationbar_key_order";

    @Readable public static final String NAVIGATIONBAR_KEY_POSITION = "navigationbar_key_position";

    @Readable
    public static final String NAVIGATIONBAR_SWITCH_APPS_WHEN_HINT_HIDDEN =
    ...
    @Readable
    public static final String NAVIGATION_BAR_BUTTON_TO_HIDE_KEYBOARD =
        "navigation_bar_button_to_hide_keyboard";

    @Readable
    public static final String NAVIGATION_BAR_GESTURE_WHILE_HIDDEN =
        "navigation_bar_gesture_while_hidden";

    @Readable public static final String NETPOLICY_OVERRIDE_ENABLED = "netpolicy_override_enabled";

    @Readable public static final String NETPOLICY_QUOTA_ENABLED = "netpolicy_quota_enabled";

    @Readable public static final String NETPOLICY_QUOTA_FRAC_JOBS = "netpolicy_quota_frac_jobs";

    @Readable
    public static final String NETPOLICY_QUOTA_FRAC_MULTIPATH = "netpolicy_quota_frac_multipath";

    @Readable public static final String NETPOLICY_QUOTA_LIMITED = "netpolicy_quota_limited";

    @Readable public static final String NETPOLICY_QUOTA_UNLIMITED = "netpolicy_quota_unlimited";

    @Readable public static final String NETSTATS_AUGMENT_ENABLED = "netstats_augment_enabled";
    ...
    @Readable public static final String NETSTATS_DEV_DELETE_AGE = "netstats_dev_delete_age";

    @Readable public static final String NETSTATS_DEV_PERSIST_BYTES = "netstats_dev_persist_bytes";

    @Readable public static final String NETSTATS_DEV_ROTATE_AGE = "netstats_dev_rotate_age";

    @Readable public static final String NETSTATS_ENABLED = "netstats_enabled";

    @Readable
    public static final String NETSTATS_GLOBAL_ALERT_BYTES = "netstats_global_alert_bytes";

    @Readable public static final String NETSTATS_POLL_INTERVAL = "netstats_poll_interval";

    @Readable public static final String NETSTATS_SAMPLE_ENABLED = "netstats_sample_enabled";
    ...
    @Readable public static final String NETSTATS_UID_DELETE_AGE = "netstats_uid_delete_age";

    @Readable public static final String NETSTATS_UID_PERSIST_BYTES = "netstats_uid_persist_bytes";

    @Readable public static final String NETSTATS_UID_ROTATE_AGE = "netstats_uid_rotate_age";
    ...
    @Readable public static final String NETWORK_AVOID_BAD_WIFI = "network_avoid_bad_wifi";
    ...
    @Readable public static final String NETWORK_PREFERENCE = "network_preference";
    ...
    @Readable public static final String NETWORK_SCORER_APP = "network_scorer_app";
    ...
    @Readable public static final String NETWORK_WATCHLIST_ENABLED = "network_watchlist_enabled";

    @Readable
    public static final String NETWORK_WATCHLIST_LAST_REPORT_TIME =
        "network_watchlist_last_report_time";

    @Readable public static final String NEW_CONTACT_AGGREGATOR = "new_contact_aggregator";

    @Readable public static final String NFC_PREFERRED_SIM_INDEX = "nfc_preferred_sim_index";
    ...
    @Readable public static final String NITZ_UPDATE_DIFF = "nitz_update_diff";

    @Readable public static final String NITZ_UPDATE_SPACING = "nitz_update_spacing";

    @Readable @Deprecated public static final String NOTIFICATION_BUBBLES = "notification_bubbles";
    ...
    @Readable public static final String NTP_SERVER = "ntp_server";

    @Readable public static final String NTP_TIMEOUT = "ntp_timeout";
    ...
    @Readable public static final String OPEN_IN_POP_UP_VIEW = "open_in_pop_up_view";

    @Readable public static final String OPEN_IN_SPLIT_SCREEN_VIEW = "open_in_split_screen_view";

    @SystemApi @Readable
    public static final String OTA_DISABLE_AUTOMATIC_UPDATE = "ota_disable_automatic_update";

    @Readable public static final String OVERLAY_DISPLAY_DEVICES = "overlay_display_devices";
    ...
    @Readable public static final String PACKAGE_VERIFIER_TIMEOUT = "verifier_timeout";

    @Readable
    public static final String PACKAGE_VERIFIER_TIMEOUT_SAMSUNG = "verifier_timeout_samsung";

    @Readable public static final String PAC_CHANGE_DELAY = "pac_change_delay";
    ...
    @Readable public static final String POLICY_CONTROL = "policy_control";

    @Readable public static final String POWER_BUTTON_LONG_PRESS = "power_button_long_press";
    ...
    @Readable public static final String POWER_KEY_MAPPING = "power_key_mapping";

    @Readable public static final String POWER_MANAGER_CONSTANTS = "power_manager_constants";

    @Readable public static final String POWER_SOUNDS_ENABLED = "power_sounds_enabled";

    @Readable public static final String PREFERRED_NETWORK_MODE = "preferred_network_mode";

    @Readable public static final String PRIVATE_DNS_DEFAULT_MODE = "private_dns_default_mode";

    @Readable public static final String PRIVATE_DNS_MODE = "private_dns_mode";

    @Readable public static final String PRIVATE_DNS_SPECIFIER = "private_dns_specifier";

    @Readable
    public static final String PROVISIONING_APN_ALARM_DELAY_IN_MS =
        "provisioning_apn_alarm_delay_in_ms";

    @Readable public static final String RADIO_BLUETOOTH = "bluetooth";

    @Readable
    public static final String RADIO_BUG_SYSTEM_ERROR_COUNT_THRESHOLD =
        "radio_bug_system_error_count_threshold";

    @Readable
    public static final String RADIO_BUG_WAKELOCK_TIMEOUT_COUNT_THRESHOLD =
        "radio_bug_wakelock_timeout_count_threshold";

    @Readable public static final String RADIO_CELL = "cell";

    @Readable public static final String RADIO_NFC = "nfc";
    public static final String RADIO_UWB = "uwb";

    @Readable public static final String RADIO_WIFI = "wifi";

    @Readable public static final String RADIO_WIMAX = "wimax";
    ...
    @Readable public static final String SAFE_BOOT_DISALLOWED = "safe_boot_disallowed";

    @Readable public static final String SAFE_WIFI = "safe_wifi";

    @Readable public static final String SATELLITE_MODE_ENABLED = "satellite_mode_enabled";

    @Readable public static final String SATELLITE_MODE_RADIOS = "satellite_mode_radios";

    @Readable public static final String SCREEN_CALLS = "screen_call";

    @Readable public static final String SECURE_FRP_MODE = "secure_frp_mode";

    @Readable public static final String SEHOME_PORTRAIT_MODE_ONLY = "sehome_portrait_mode_only";

    @Readable public static final String SELINUX_STATUS = "selinux_status";

    @Readable public static final String SELINUX_UPDATE_CONTENT_URL = "selinux_content_url";

    @Readable public static final String SELINUX_UPDATE_METADATA_URL = "selinux_metadata_url";

    @Readable
    public static final String SEM_ACCESSIBILITY_REDUCE_TRANSPARENCY =
        "accessibility_reduce_transparency";

    public static final String SEM_ADAPTIVE_BATTERY_MANAGEMENT_ENABLED =
        "adaptive_battery_management_enabled";

    @Readable public static final String SEM_AUTO_BRIGHTNESS_LIMIT = "auto_brightness_limit";
    ...
    @Readable public static final String SEM_BOLD_TEXT = "bold_text";
    ...
    @Readable public static final String SEM_FONT_SIZE = "font_size";

    @Readable
    public static final String SEM_LOW_POWER_BRIGHTNESS_LIMIT = "sem_low_power_brightness_limit";

    @Readable
    public static final String SEM_MHS_NETWORK_AUTO_HIGH_PERFORMANCE =
        "sem_mhs_network_auto_high_performance";

    @Readable
    public static final String SEM_MHS_NETWORK_AUTO_WIFI_OFF = "sem_mhs_network_auto_wifi_off";

    @Readable
    public static final String SEM_MHS_NETWORK_AUTO_WIFI_OFF_WAITING_TIME =
        "sem_mhs_network_auto_wifi_off_waiting_time";

    @Readable
    public static final String SEM_MHS_NETWORK_PERFORMANCE = "sem_mhs_network_performance";

    @Readable
    public static final String SEM_MULTI_WINDOW_MENU_IN_FULL_SCREEN =
        "multi_window_menu_in_full_screen";
    ...
    @Readable
    public static final String SEM_NAVIGATIONBAR_USE_THEME_DEFAULT =
        "navigationbar_use_theme_default";

    @Readable public static final String SEM_PROCESS_TEXT_MANAGE_APPS = "process_text_manager_apps";

    @Readable public static final String SEM_TASKBAR_MAX_RECENT_COUNT = "taskbar_max_recent_count";

    @Readable
    public static final String SEM_TASKBAR_RECENT_APPS_ENABLED = "taskbar_recent_apps_enabled";

    @Readable
    public static final String SEM_TASKBAR_SHOW_HIDE_ON_HOLD_ENABLED =
        "taskbar_show_hide_on_hold_enabled";

    @Readable public static final String SEM_TASKBAR_TYPE = "task_bar_type";

    @Readable public static final String SEM_TASK_BAR = "task_bar";

    @Readable
    public static final String SEM_WIFI_ALLOWED_OAUTH_PROVIDER = "sem_wifi_allowed_oauth_provider";

    @Readable public static final String SEM_WIFI_APE_ENABLED = "sem_wifi_ape_enabled";

    @Readable
    public static final String SEM_WIFI_CARRIER_NETWORK_OFFLOAD_ENABLED =
        "sem_wifi_carrier_network_offload_enabled";

    @Readable
    public static final String SEM_WIFI_DEVELOPER_OPTION_VISIBLE =
        "sem_wifi_developer_option_visible";

    public static final String SEM_WIFI_DISPLAY_ON = "wifi_display_on";

    @Readable
    public static final String SEM_WIFI_INTELLIGENT_WIFI_ADDED_REMOVED_LIST =
        "sem_auto_wifi_added_removed_list";

    @Readable
    public static final String SEM_WIFI_LAST_NETWORK_RATING_SCORER =
        "sem_wifi_last_network_rating_scorer";

    @Readable
    public static final String SEM_WIFI_NETWORK_RATING_ENABLED =
        "sem_wifi_network_rating_scorer_enabled";

    @Readable public static final String SEM_WIFI_P2P_DEVICE_NAME = "wifi_p2p_device_name";

    @Readable
    public static final String SEM_WIFI_RECOMMEND_LEARNING_SCORE =
        "sem_wifi_recommend_learning_score";

    @Readable
    public static final String SEM_WIFI_RECOMMEND_MANUAL_CONNECT_COUNT =
        "sem_wifi_recommend_manual_connect_count";

    @Readable
    public static final String SEM_WIFI_RECOMMEND_NETWORK_USER_LEVEL =
        "sem_wifi_recommend_network_user_level";

    @Readable
    public static final String SEM_WIFI_SETTINGS_FRAMEWORK_SCAN_INTERVAL =
        "sem_wifi_settings_framework_scan_interval";

    @Readable
    public static final String SEM_WIFI_SWITCH_TO_BETTER_WIFI_ENABLED =
        "sem_wifi_switch_to_better_wifi_enabled";

    @Readable
    public static final String SEM_WIFI_SWITCH_TO_BETTER_WIFI_SUPPORTED =
    ...
    @Readable
    public static final String SEM_WIFI_WHAT_HINTCARD_HAVE_TO_BE_SHOWN =
        "sem_what_hintcard_have_to_be_shown";

    @Readable public static final String SEND_ACTION_APP_ERROR = "send_action_app_error";

    @Readable
    public static final String SETTINGS_USE_EXTERNAL_PROVIDER_API =
        "settings_use_external_provider_api";

    @Readable public static final String SETTINGS_USE_PSD_API = "settings_use_psd_api";
    ...
    @Readable public static final String SET_GLOBAL_HTTP_PROXY = "set_global_http_proxy";

    @Readable public static final String SET_INSTALL_LOCATION = "set_install_location";

    @Readable public static final String SHORTCUT_MANAGER_CONSTANTS = "shortcut_manager_constants";
    ...
    @Readable public static final String SHOW_FIRST_CRASH_DIALOG = "show_first_crash_dialog";

    @Readable
    public static final String SHOW_HIDDEN_LAUNCHER_ICON_APPS_ENABLED =
        "show_hidden_icon_apps_enabled";

    @Readable public static final String SHOW_MEDIA_ON_QUICK_SETTINGS = "qs_media_controls";

    @Readable public static final String SHOW_MUTE_IN_CRASH_DIALOG = "show_mute_in_crash_dialog";
    ...
    @Readable
    public static final String SHOW_PLAYBACK_CONTROLS_BELOW_VIDEOS =
        "show_playback_controls_below_videos";

    @Readable @Deprecated public static final String SHOW_PROCESSES = "show_processes";
    ...
    @Readable public static final String SHOW_TEMPERATURE_WARNING = "show_temperature_warning";

    @Readable public static final String SHOW_USB_TEMPERATURE_ALARM = "show_usb_temperature_alarm";
    ...
    @Readable public static final String SIGNED_CONFIG_VERSION = "signed_config_version";

    @Readable public static final String SIM_SELECT_ICON_1 = "select_icon_1";

    @Readable public static final String SIM_SELECT_ICON_2 = "select_icon_2";

    @Readable public static final String SIM_SELECT_NAME_1 = "select_name_1";

    @Readable public static final String SIM_SELECT_NAME_2 = "select_name_2";
    ...
    @Readable public static final String SMS_SHORT_CODE_RULE = "sms_short_code_rule";

    @Readable public static final String SOFTSIM_SUBID = "softsim_subid";
    ...
    @Readable
    public static final String SPLIT_HANDLER_HELP_POPUP_COUNT =
        "multi_split_quick_options_help_count";

    @Readable
    public static final String SQLITE_COMPATIBILITY_WAL_FLAGS = "sqlite_compatibility_wal_flags";

    @Readable public static final String STAY_ON_WHILE_PLUGGED_IN = "stay_on_while_plugged_in";

    @Readable public static final String STORAGE_BENCHMARK_INTERVAL = "storage_benchmark_interval";

    @Readable
    public static final String STORAGE_SETTINGS_CLOBBER_THRESHOLD =
        "storage_settings_clobber_threshold";

    @Readable public static final String STYLUS_EVER_USED = "stylus_ever_used";

    @Readable public static final String SUGGESTION_RESPONSES = "suggestion_responses";

    @Readable public static final String SYNC_MANAGER_CONSTANTS = "sync_manager_constants";
    ...
    @Readable public static final String SYS_TRACED = "sys_traced";

    @Readable public static final String SYS_UIDCPUPOWER = "sys_uidcpupower";
    ...
    @Readable public static final String TCP_DEFAULT_INIT_RWND = "tcp_default_init_rwnd";
    public static final String TETHERING_BLOCKED = "tethering_blocked";

    @Readable
    public static final String TETHERING_DATA_WARNING_SIM_SLOT_0 =
        "tethering_data_warning_sim_slot_0";

    @Readable
    public static final String TETHERING_DATA_WARNING_SIM_SLOT_1 =
        "tethering_data_warning_sim_slot_1";

    @Readable public static final String TETHER_DUN_APN = "tether_dun_apn";

    @Readable public static final String TETHER_DUN_REQUIRED = "tether_dun_required";
    ...
    @SystemApi @Readable public static final String TETHER_SUPPORTED = "tether_supported";

    @Readable
    public static final String TEXT_CLASSIFIER_ACTION_MODEL_PARAMS =
        "text_classifier_action_model_params";

    @Readable public static final String TEXT_CLASSIFIER_CONSTANTS = "text_classifier_constants";

    @SystemApi @Readable public static final String THEATER_MODE_ON = "theater_mode_on";

    @Readable public static final String TIME_ONLY_MODE_CONSTANTS = "time_only_mode_constants";
    ...
    @Readable public static final String TRANSITION_ANIMATION_SCALE = "transition_animation_scale";

    @Readable public static final String TRUSTED_SOUND = "trusted_sound";

    @Readable public static final String TZINFO_UPDATE_CONTENT_URL = "tzinfo_content_url";

    @Readable public static final String TZINFO_UPDATE_METADATA_URL = "tzinfo_metadata_url";

    @Readable public static final String UNGAZE_SLEEP_ENABLED = "ungaze_sleep_enabled";
    ...
    @Readable public static final String UNLOCK_SOUND = "unlock_sound";

    @Readable
    public static final String UNUSED_STATIC_SHARED_LIB_MIN_CACHE_PERIOD =
        "unused_static_shared_lib_min_cache_period";

    @Readable public static final String UPDATABLE_DRIVER_ALL_APPS = "updatable_driver_all_apps";
    ...
    @Readable public static final String USB_MASS_STORAGE_ENABLED = "usb_mass_storage_enabled";
    ...
    @Readable public static final String USER_DISABLED_HDR_FORMATS = "user_disabled_hdr_formats";
    ...
    @Readable public static final String USER_SWITCHER_ENABLED = "user_switcher_enabled";

    @Readable public static final String USE_GOOGLE_MAIL = "use_google_mail";
    ...
    @Readable @Deprecated public static final String VT_IMS_ENABLED = "vt_ims_enabled";

    @Readable public static final String WAIT_FOR_DEBUGGER = "wait_for_debugger";

    @Readable public static final String WARNING_TEMPERATURE = "warning_temperature";
    ...
    @SystemApi @Readable public static final String WEBVIEW_MULTIPROCESS = "webview_multiprocess";

    @Readable public static final String WEBVIEW_PROVIDER = "webview_provider";

    @Readable @Deprecated public static final String WFC_IMS_ENABLED = "wfc_ims_enabled";

    @Readable @Deprecated public static final String WFC_IMS_MODE = "wfc_ims_mode";

    @Readable @Deprecated
    public static final String WFC_IMS_ROAMING_ENABLED = "wfc_ims_roaming_enabled";

    @Readable @Deprecated public static final String WFC_IMS_ROAMING_MODE = "wfc_ims_roaming_mode";

    @Readable
    public static final String WIFI_ADAPTIVE_WIFI_CONTROL_ENABLED = "sem_auto_wifi_control_enabled";

    @Readable public static final String WIFI_ALWAYS_REQUESTED = "wifi_always_requested";
    ...
    @Readable public static final String WIFI_COUNTRY_CODE = "wifi_country_code";
    ...
    @Readable public static final String WIFI_DISPLAY_ON = "wifi_display_on";

    @Readable public static final String WIFI_DISPLAY_WPS_CONFIG = "wifi_display_wps_config";

    @Readable public static final String WIFI_ENHANCED_AUTO_JOIN = "wifi_enhanced_auto_join";
    ...
    @Readable public static final String WIFI_FREQUENCY_BAND = "wifi_frequency_band";

    @Readable
    public static final String WIFI_GUIDER_FEATURE_CONTROL = "wifi_guider_feature_control";

    @Readable public static final String WIFI_IDLE_MS = "wifi_idle_ms";

    @Readable
    public static final String WIFI_IWC_LAST_TIME_SWITCH_TO_MOBILE_ON =
        "wifi_iwc_last_time_switch_to_mobile_on";

    @Readable
    public static final String WIFI_IWC_USER_DATA_PREFERENCE = "wifi_iwc_user_data_preference";

    @Readable public static final String WIFI_MAX_DHCP_RETRY_COUNT = "wifi_max_dhcp_retry_count";

    @Readable public static final String WIFI_MIGRATION_COMPLETED = "wifi_migration_completed";
    ...
    @Readable public static final String WIFI_NETWORK_SHOW_RSSI = "wifi_network_show_rssi";
    public static final String WIFI_NUM_OF_SWITCH_TO_MOBILE_DATA_TOGGLE =
        "wifi_num_of_switch_to_mobile_data_toggle";

    @Readable @Deprecated
    public static final String WIFI_NUM_OPEN_NETWORKS_KEPT = "wifi_num_open_networks_kept";

    @Readable public static final String WIFI_ON = "wifi_on";

    @Readable
    public static final String WIFI_ON_WHEN_PROXY_DISCONNECTED = "wifi_on_when_proxy_disconnected";

    @Readable @Deprecated public static final String WIFI_P2P_DEVICE_NAME = "wifi_p2p_device_name";

    @Readable @Deprecated
    public static final String WIFI_P2P_PENDING_FACTORY_RESET = "wifi_p2p_pending_factory_reset";

    @Readable
    public static final String WIFI_RECOMMEND_NETWORKS_NOTIFICATION_ON =
        "sem_wifi_recommend_networks_notification_on";
    ...
    @Readable @Deprecated public static final String WIFI_SCORE_PARAMS = "wifi_score_params";

    @Readable @Deprecated public static final String WIFI_SLEEP_POLICY = "wifi_sleep_policy";
    ...
    @Readable
    public static final String WIFI_SWITCH_FOR_INDIVIDUAL_APPS_DETECTION_MODE =
        "wifi_switch_for_individual_apps_detection_mode";

    @Readable
    public static final String WIFI_SWITCH_FOR_INDIVIDUAL_APPS_ENABLED =
        "wifi_switch_for_individual_apps_enabled";

    @Readable
    public static final String WIFI_SWITCH_FOR_INDIVIDUAL_APPS_EVER_DETECTED =
        "wifi_switch_for_individual_apps_ever_detected";

    @Readable
    public static final String WIFI_SWITCH_TO_MOBILE_DATA_SUPER_AGGRESSIVE_MODE_ON =
    ...
    @Readable public static final String WIFI_WATCHDOG_ON = "wifi_watchdog_on";

    @Readable
    public static final String WIFI_WATCHDOG_POOR_NETWORK_AGGRESSIVE_MODE_ON =
    ...
    public static final String WIFI_WCM_COUNTRY_CODE_FROM_SCAN_RESULT =
    ...
    @Readable
    public static final String WIFI_WCM_QOS_SHARING_SCORE_SUMMARY =
        "wifi_wcm_qos_sharing_score_summary";

    @Readable
    public static final String WIMAX_NETWORKS_AVAILABLE_NOTIFICATION_ON =
        "wimax_networks_available_notification_on";

    @Readable public static final String WINDOW_ANIMATION_SCALE = "window_animation_scale";

    @Readable
    public static final String WIRELESS_CHARGING_STARTED_SOUND = "wireless_charging_started_sound";

    @Readable public static final String WTF_IS_FATAL = "wtf_is_fatal";

    @Deprecated public static final String ZEN_DURATION = "zen_duration";

    @Deprecated public static final int ZEN_DURATION_FOREVER = 0;

    @Deprecated public static final int ZEN_DURATION_PROMPT = -1;

    @Readable public static final String ZEN_MODE = "zen_mode";
    public static final int ZEN_MODE_ALARMS = 3;

    @Readable public static final String ZEN_MODE_CONFIG_ETAG = "zen_mode_config_etag";
    public static final int ZEN_MODE_IMPORTANT_INTERRUPTIONS = 1;
    public static final int ZEN_MODE_NO_INTERRUPTIONS = 2;
    public static final int ZEN_MODE_OFF = 0;

    @Readable public static final String ZEN_MODE_RINGER_LEVEL = "zen_mode_ringer_level";

    @Deprecated
    public static final String ZEN_SETTINGS_SUGGESTION_VIEWED = "zen_settings_suggestion_viewed";

    @Deprecated public static final String ZEN_SETTINGS_UPDATED = "zen_settings_updated";

    @Readable public static final String ZRAM_ENABLED = "zram_enabled";
    private static final NameValueCache sNameValueCache;
    private static final ContentProviderHolder sProviderHolder;
    ...
    public static class Wearable {
      ...
      @Deprecated public static final String COMBINED_LOCATION_ENABLED = "combined_location_enable";
    ...
    static {
      ...
      LEGACY_RESTORE_SETTINGS = new String[0];
      BATTERY_PROTECTION_THRESHOLD_DEFAULT_VALUE =
          (SemFloatingFeature.getInstance()
                      .getBoolean("SEC_FLOATING_FEATURE_BATTERY_DISABLE_ECO_BATTERY")
                  || SystemProperties.getInt("ro.build.version.sep", 0) < 150100)
              ? 85
              : 80;
      ContentProviderHolder contentProviderHolder = new ContentProviderHolder(parse);
      sProviderHolder = contentProviderHolder;
      ...
      MULTI_SIM_USER_PREFERRED_SUBS =
          new String[] {"user_preferred_sub1", "user_preferred_sub2", "user_preferred_sub3"};
      ArraySet arraySet = new ArraySet();
    ...
    public static String zenModeToString(int mode) {
      return mode == 1
          ...
          : mode == 3
              ? "ZEN_MODE_ALARMS"
              : mode == 2 ? "ZEN_MODE_NO_INTERRUPTIONS" : "ZEN_MODE_OFF";
    }
    ...
    public static void getPublicSettings(
        ...
        ArrayMap<String, Integer> readableKeysWithMaxTargetSdk) {
      Settings.getPublicSettingsForClass(
          Global.class, allKeys, readableKeys, readableKeysWithMaxTargetSdk);
    }
    ...
    public static String getStringForUser(ContentResolver resolver, String name, int userHandle) {
      if (MOVED_TO_SECURE.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.Global to"
                + " android.provider.Settings.Secure, returning read-only value.");
        return Secure.getStringForUser(resolver, name, userHandle);
      }
      if (MOVED_TO_SYSTEM.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.Global to"
                + " android.provider.Settings.System, returning read-only value.");
    ...
    public static boolean putString(
        ...
        boolean overrideableByRestore) {
      return putStringForUser(
          resolver, name, value, tag, makeDefault, resolver.getUserId(), overrideableByRestore);
    }
    ...
    public static void resetToDefaultsAsUser(
      ...
      try {
        ...
        cp.call(
            resolver.getAttributionSource(),
            contentProviderHolder.mUri.getAuthority(),
            Settings.CALL_METHOD_RESET_GLOBAL,
            null,
            arg);
      } catch (RemoteException e) {
        Log.m103w(Settings.TAG, "Can't reset do defaults for " + CONTENT_URI, e);
      ...
      if (MOVED_TO_SYSTEM.contains(name)) {
        Log.m102w(
            ...
            "Setting "
                + name
                + " has moved from android.provider.Settings.Global to"
                + " android.provider.Settings.System, value is unchanged.");
        return System.putStringForUser(
            resolver, name, value, tag, makeDefault, userHandle, overrideableByRestore);
      }
      return sNameValueCache.putStringForUser(
          resolver, name, value, tag, makeDefault, userHandle, overrideableByRestore);
  ...
  public static final class Config extends NameValueTable {
    ...
    @Deprecated public static final int SYNC_DISABLED_MODE_NONE = 0;

    @Deprecated public static final int SYNC_DISABLED_MODE_PERSISTENT = 1;

    @Deprecated public static final int SYNC_DISABLED_MODE_UNTIL_REBOOT = 2;
    private static final NameValueCache sNameValueCache;
    private static final ContentProviderHolder sProviderHolder;
    ...
    static {
      ...
      sNameValueCache =
          new NameValueCache(
              parse,
              Settings.CALL_METHOD_GET_CONFIG,
              Settings.CALL_METHOD_PUT_CONFIG,
              Settings.CALL_METHOD_DELETE_CONFIG,
              Settings.CALL_METHOD_LIST_CONFIG,
              Settings.CALL_METHOD_SET_ALL_CONFIG,
              contentProviderHolder,
              Config.class);
    }
    ...
    public static Map<String, String> getStrings(
      ...
      ArrayMap<String, String> rawKeyValues =
          sNameValueCache.getStringsForPrefix(resolver, prefix, compositeNames);
    ...
    public static boolean putString(
      ...
      return sNameValueCache.putStringForUser(
          resolver,
          createCompositeName(namespace, name),
          value,
          null,
          makeDefault,
          resolver.getUserId(),
    ...
    @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
    public static boolean setStrings(String namespace, Map<String, String> keyValues)
        throws DeviceConfig.BadConfigException {
      return setStrings(getContentResolver(), namespace, keyValues);
    }

    public static boolean setStrings(
        ContentResolver resolver, String namespace, Map<String, String> keyValues)
        throws DeviceConfig.BadConfigException {
      ...
      int result =
          sNameValueCache.setStringsForPrefix(
    ...
    public static boolean deleteString(String namespace, String name) {
      ContentResolver resolver = getContentResolver();
      return sNameValueCache.deleteStringForUser(
          resolver, createCompositeName(namespace, name), resolver.getUserId());
    }
    ...
    public static void resetToDefaults(int resetMode, String namespace) {
      try {
        ...
        cp.call(
            resolver.getAttributionSource(),
            contentProviderHolder.mUri.getAuthority(),
            Settings.CALL_METHOD_RESET_CONFIG,
            null,
            arg);
      } catch (RemoteException e) {
    ...
    public static void setSyncDisabledMode(int disableSyncMode) {
      try {
        ...
        cp.call(
            resolver.getAttributionSource(),
            contentProviderHolder.mUri.getAuthority(),
            Settings.CALL_METHOD_SET_SYNC_DISABLED_MODE_CONFIG,
            null,
            args);
      } catch (RemoteException e) {
      ...
      try {
        ...
        Bundle bundle =
            cp.call(
                resolver.getAttributionSource(),
                contentProviderHolder.mUri.getAuthority(),
                Settings.CALL_METHOD_GET_SYNC_DISABLED_MODE_CONFIG,
                null,
                args);
    ...
    public static void setMonitorCallback(
        ContentResolver resolver, Executor executor, DeviceConfig.MonitorCallback callback) {
    ...
    public static void clearMonitorCallback(ContentResolver resolver) {
      try {
        ...
        cp.call(
            resolver.getAttributionSource(),
            contentProviderHolder.mUri.getAuthority(),
            Settings.CALL_METHOD_UNREGISTER_MONITOR_CALLBACK_CONFIG,
            null,
            arg);
      } catch (RemoteException e) {
    ...
    public static void registerContentObserver(
        String namespace, boolean notifyForDescendants, ContentObserver observer) {
      ActivityThread.currentApplication()
          .getContentResolver()
          .registerContentObserver(createNamespaceUri(namespace), notifyForDescendants, observer);
    }
    ...
    public static void enforceReadPermission(String namespace) {
      if (ActivityThread.currentApplication()
              ...
              != 0
          && !DeviceConfig.getPublicNamespaces().contains(namespace)) {
        throw new SecurityException(
            "Permission denial: reading from settings"
                + " requires:android.permission.READ_DEVICE_CONFIG");
      }
    }
    ...
    private static void setMonitorCallbackAsUser(
        ...
        ContentResolver resolver,
        int userHandle,
        final DeviceConfig.MonitorCallback callback) {
      try {
        Bundle arg = new Bundle();
        arg.putInt(Settings.CALL_METHOD_USER_KEY, userHandle);
        arg.putParcelable(
            ...
            new RemoteCallback(
                new RemoteCallback
                    .OnResultListener() { // from class:
                                          // android.provider.Settings$Config$$ExternalSyntheticLambda2
        ...
        cp.call(
            resolver.getAttributionSource(),
            contentProviderHolder.mUri.getAuthority(),
            Settings.CALL_METHOD_REGISTER_MONITOR_CALLBACK_CONFIG,
            null,
            arg);
      } catch (RemoteException e) {
    ...
    public static void handleMonitorCallback(
        Bundle result, Executor executor, final DeviceConfig.MonitorCallback monitorCallback) {
      char c;
      ...
      switch (c) {
          ...
          if (updatedNamespace != null) {
            executor.execute(
                new Runnable() { // from class:
                                 // android.provider.Settings$Config$$ExternalSyntheticLambda0
        ...
        case 1:
          ...
          if (namespace != null && callingPackage != null) {
            executor.execute(
                new Runnable() { // from class:
                                 // android.provider.Settings$Config$$ExternalSyntheticLambda1
                  @Override // java.lang.Runnable
                  public final void run() {
          ...
          break;
        default:
          Slog.m121w(Settings.TAG, "Unrecognized DeviceConfig callback");
          break;
      }
    }
  ...
  public static final class Bookmarks implements BaseColumns {
    ...
    /* renamed from: ID */
    public static final String f353ID = "_id";
      ...
      while (intent == null) {
        try {
          ...
          try {
            ...
            Log.m103w("Bookmarks", "Intent column not found", e);
  ...
  public static boolean isCallingPackageAllowedToWriteSettings(
      Context context, int uid, String callingPackage, boolean throwException) {
    return isCallingPackageAllowedToPerformAppOpsProtectedOperation(
        context, uid, callingPackage, null, throwException, 23, PM_WRITE_SETTINGS, false);
  }

  @SystemApi
  @Deprecated
  public static boolean checkAndNoteWriteSettingsOperation(
      Context context, int uid, String callingPackage, boolean throwException) {
  ...
  public static boolean isCallingPackageAllowedToDrawOverlays(
      Context context, int uid, String callingPackage, boolean throwException) {
    return isCallingPackageAllowedToPerformAppOpsProtectedOperation(
        context, uid, callingPackage, null, throwException, 24, PM_SYSTEM_ALERT_WINDOW, false);
  }

  public static boolean checkAndNoteDrawOverlaysOperation(
      Context context,
      int uid,
      String callingPackage,
      String callingAttributionTag,
  ...
  @Deprecated
  public static boolean isCallingPackageAllowedToPerformAppOpsProtectedOperation(
      Context context,
      int uid,
      String callingPackage,
      boolean throwException,
      ...
      boolean makeNote) {
    return isCallingPackageAllowedToPerformAppOpsProtectedOperation(
        context, uid, callingPackage, null, throwException, appOpsOpCode, permissions, makeNote);
  }
  ...
  public static boolean isCallingPackageAllowedToPerformAppOpsProtectedOperation(
    ...
    int mode =
        makeNote
            ? appOpsMgr.noteOpNoThrow(
                appOpsOpCode, uid, callingPackage, callingAttributionTag, (String) null)
            : appOpsMgr.checkOpNoThrow(appOpsOpCode, uid, callingPackage);
    switch (mode) {
      case 0:
        return true;
      case 3:
