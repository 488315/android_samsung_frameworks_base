...
public class ContactsInternal {
  ...
  public static void startQuickContactWithErrorToastForUser(
    ...
    try {
      ...
      Toast.makeText(context, C4337R.string.quick_contacts_not_available, 0).show();
    }
  }
  ...
  private static boolean maybeStartManagedQuickContact(Context context, Intent originalIntent) {
    ...
    if (!TextUtils.isEmpty(lookupKey)
        && lookupKey.startsWith(ContactsContract.Contacts.ENTERPRISE_CONTACT_LOOKUP_PREFIX)) {
      ...
      String actualLookupKey =
          lookupKey.substring(ContactsContract.Contacts.ENTERPRISE_CONTACT_LOOKUP_PREFIX.length());
      ...
      dpm.startManagedQuickContact(
          actualLookupKey, actualContactId, isContactIdIgnored, actualDirectoryId, originalIntent);
      return true;
    }
    return false;
  }
