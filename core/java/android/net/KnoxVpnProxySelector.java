...
public class KnoxVpnProxySelector extends ProxySelector {
    ...
    public List<java.net.Proxy> select(URI uri) {
        ...
        try {
            ...
            boolean z = DBG;
            if (z) {
                Log.m94d(TAG, "pac url being recieved is " + urlString + "for the caller " + Process.myUid());
            }
            ...
            if (ret.size() == 0) {
                Log.m94d(TAG, "in-valid knox vpn proxy is added for caller during exception " + Process.myUid());
                ret.add(java.net.Proxy.NO_PROXY);
            }
        } catch (Exception e2) {
