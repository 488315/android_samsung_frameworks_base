...
public class SntpClient {
    ...
    public boolean requestTime(String host, int port, int timeout, Network network) {
        try {
            ...
            if (lService != null && lService.shallForceNtpMdmValues()) {
                ...
                if (!TextUtils.isEmpty(newHost)) {
                    host = newHost;
                    Log.m94d(TAG, "host set by MDM: " + host);
                }
                if (newTimeout != 0) {
                    timeout = newTimeout;
                    Log.m94d(TAG, "timeout set by MDM: " + timeout);
                }
            }
        } catch (RemoteException e) {
            Log.m94d(TAG, "Remote Exception: " + e);
        }
        ...
        try {
            ...
            Log.m102w(TAG, "Unknown host: " + host);
            EventLogTags.writeNtpFailure(host, e2.toString());
        }
        Log.m94d(TAG, "request time failed");
        return false;
    }
    ...
    */
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Log.m94d(TAG, "round trip: " + roundTripTimeMillis + "ms, clock offset: " + clockOffsetMillis + "ms");
                    ...
                    try {
                        ...
                        Log.m94d(TAG, "request time failed: " + e);
                        if (socket != null) {
                            socket.close();
                        }
                        TrafficStats.setThreadStatsTag(oldTag);
    ...
    public boolean requestTime(String host, int timeout) {
        Log.m102w(TAG, "Shame on you for calling the hidden API requestTime()!");
        return false;
    }
