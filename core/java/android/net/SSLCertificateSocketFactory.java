...
public class SSLCertificateSocketFactory extends SSLSocketFactory {
  ...
  private SSLSocketFactory makeSocketFactory(
    ...
    try {
      SSLContext sslContext =
          SSLContext.getInstance(org.apache.http.conn.ssl.SSLSocketFactory.TLS, "AndroidOpenSSL");
  ...
  private synchronized SSLSocketFactory getDelegate() {
    ...
    if (this.mInsecureFactory == null) {
      if (this.mSecure) {
        Log.m102w(TAG, "*** BYPASSING SSL SECURITY CHECKS (socket.relaxsslcheck=yes) ***");
      } else {
        Log.m102w(TAG, "Bypassing SSL security checks at caller's request");
      }
