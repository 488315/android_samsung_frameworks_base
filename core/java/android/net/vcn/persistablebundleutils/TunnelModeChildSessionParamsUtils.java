  ...
  static class ConfigRequest {
    ...
    ConfigRequest(TunnelModeChildSessionParams.TunnelModeChildConfigRequest config) {
      ...
      if (config instanceof TunnelModeChildSessionParams.ConfigRequestIpv4Address) {
        this.type = 1;
        this.address =
            ((TunnelModeChildSessionParams.ConfigRequestIpv4Address) config).getAddress();
      } else if (config instanceof TunnelModeChildSessionParams.ConfigRequestIpv6Address) {
        this.type = 2;
        Inet6Address address =
            ((TunnelModeChildSessionParams.ConfigRequestIpv6Address) config).getAddress();
        this.address = address;
        if (address != null) {
          prefixLen =
              ((TunnelModeChildSessionParams.ConfigRequestIpv6Address) config).getPrefixLength();
        }
  ...
  public static TunnelModeChildSessionParams fromPersistableBundle(PersistableBundle in) {
    ...
    for (ConfigRequest req : reqList) {
      switch (req.type) {
        ...
        case 3:
          if (req.address != null) {
            Log.m102w(TAG, "Requesting a specific IPv4 DNS server is unsupported");
          }
          builder.addInternalDnsServerRequest(OsConstants.AF_INET);
          break;
        case 4:
          if (req.address != null) {
            Log.m102w(TAG, "Requesting a specific IPv6 DNS server is unsupported");
          }
          builder.addInternalDnsServerRequest(OsConstants.AF_INET6);
          break;
        case 5:
          if (req.address != null) {
            Log.m102w(TAG, "Requesting a specific IPv4 DHCP server is unsupported");
          }
    ...
    if (hasIpv4AddressReq != hasIpv4NetmaskReq) {
      Log.m102w(
          TAG,
          String.format(
              "Expect IPv4 address request and IPv4 netmask request either both exist or both"
                  + " absent, but found hasIpv4AddressReq exists? %b, hasIpv4AddressReq exists? %b,"
                  + " ",
              Boolean.valueOf(hasIpv4AddressReq), Boolean.valueOf(hasIpv4NetmaskReq)));
    }
