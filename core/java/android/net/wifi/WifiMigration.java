...
public final class WifiMigration {
  ...
  private static AtomicFile getUserAtomicFile(int storeFileId, int userId) {
    return new AtomicFile(
        new File(getLegacyWifiUserDirectory(userId), STORE_ID_TO_FILE_NAME.get(storeFileId)));
  }
        ...
        new SettingsMigrationData.Builder()
            .setScanAlwaysAvailable(
                Settings.Global.getInt(
                        context.getContentResolver(), Settings.Global.WIFI_SCAN_ALWAYS_AVAILABLE, 0)
                    == 1)
            .setP2pFactoryResetPending(
                Settings.Global.getInt(
                    ...
                    == 1)
            .setP2pDeviceName(
                Settings.Global.getString(context.getContentResolver(), "wifi_p2p_device_name"))
            .setSoftApTimeoutEnabled(
                Settings.Global.getInt(
                        context.getContentResolver(), Settings.Global.SOFT_AP_TIMEOUT_ENABLED, 1)
                    == 1)
            .setWakeUpEnabled(
                Settings.Global.getInt(
                        context.getContentResolver(), Settings.Global.WIFI_WAKEUP_ENABLED, 0)
                    == 1)
            .setScanThrottleEnabled(
                Settings.Global.getInt(
                        context.getContentResolver(), Settings.Global.WIFI_SCAN_THROTTLE_ENABLED, 1)
                    == 1)
