...
public class WifiNl80211Manager {
    ...
    class ApInterfaceEventCallback extends IApInterfaceEventCallback.Stub {
        ...
        public void onConnectedClientsChanged(final NativeWifiClient client, final boolean isConnected) {
            if (WifiNl80211Manager.this.mVerboseLoggingEnabled) {
                Log.m94d(WifiNl80211Manager.TAG, "onConnectedClientsChanged called with " + client.getMacAddress() + " isConnected: " + isConnected);
            }
            long token = Binder.clearCallingIdentity();
            try {
    ...
    public boolean singleScanIsProgress() {
        ...
        if (elapsedTimeFromLastSingleScanTriggered > 0 && elapsedTimeFromLastSingleScanTriggered < 4000) {
            Log.m98i(TAG, "single scan is in progress : elapsed=" + elapsedTimeFromLastSingleScanTriggered);
            return true;
        }
        this.mLastSingleScanTriggeredTime.set(0L);
        return false;
