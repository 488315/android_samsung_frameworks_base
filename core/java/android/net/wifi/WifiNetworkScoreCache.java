...
public class WifiNetworkScoreCache extends INetworkScoreCache.Stub {
    ...
    public final void updateScores(List<ScoredNetwork> networks) {
        ...
        if (DBG) {
            Log.m94d(TAG, "updateScores list size=" + networks.size());
        }
        ...
        synchronized (this.mLock) {
            for (ScoredNetwork network : networks) {
                ...
                if (networkKey == null) {
                    if (DBG) {
                        Log.m94d(TAG, "Failed to build network key for ScoredNetwork" + network);
                    }
                } else {
    ...
    public int getNetworkScore(ScanResult result) {
        ...
        if (network != null && network.rssiCurve != null) {
            ...
            if (DBG) {
                Log.m94d(TAG, "getNetworkScore found scored network " + network.networkKey + " score " + Integer.toString(score) + " RSSI " + result.level);
            }
        }
        return score;
    }
    ...
    public int getNetworkScore(ScanResult result, boolean isActiveNetwork) {
        ...
        if (network != null && network.rssiCurve != null) {
            ...
            if (DBG) {
                Log.m94d(TAG, "getNetworkScore found scored network " + network.networkKey + " score " + Integer.toString(score) + " RSSI " + result.level + " isActiveNetwork " + isActiveNetwork);
            }
        }
        return score;
    }
    ...
    public ScoredNetwork getScoredNetwork(NetworkKey networkKey) {
        ...
        if (key == null) {
            if (DBG) {
                Log.m94d(TAG, "Could not build key string for Network Key: " + networkKey);
                return null;
            }
            return null;
        }
