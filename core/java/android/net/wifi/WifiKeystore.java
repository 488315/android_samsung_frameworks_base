...
public final class WifiKeystore {
    ...
    public static boolean put(String alias, byte[] blob) {
        try {
            Log.m98i(TAG, "put blob. alias " + alias);
            getService().put(alias, 1010, blob);
            return true;
        } catch (Exception e) {
            Log.m97e(TAG, "Failed to put blob.", e);
            return false;
        }
    }
    ...
    public static byte[] get(String alias) {
        try {
            Log.m98i(TAG, "get blob. alias " + alias);
            return getService().get(alias, 1010);
        } catch (ServiceSpecificException e) {
            if (e.errorCode != 7) {
                Log.m97e(TAG, "Failed to get blob.", e);
                return null;
            }
            return null;
        } catch (Exception e2) {
            Log.m97e(TAG, "Failed to get blob.", e2);
            return null;
        }
    }
    ...
    public static boolean remove(String alias) {
        try {
            ...
            if (e.errorCode != 7) {
                Log.m97e(TAG, "Failed to remove blob.", e);
                return false;
            }
            return false;
        } catch (Exception e2) {
            Log.m97e(TAG, "Failed to remove blob.", e2);
            return false;
        }
    }
    ...
    public static String[] list(String prefix) {
        try {
            ...
            Log.m97e(TAG, "Failed to list blobs.", e);
            return new String[0];
        }
    }
}
