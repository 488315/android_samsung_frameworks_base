...
public class SslCertificate {
            ...
            if (dName != null) {
                ...
                try {
                    ...
                    for (int i = 0; i < oid.size(); i++) {
                        if (oid.elementAt(i).equals(X509Name.f720CN)) {
                            if (this.mCName == null) {
                                this.mCName = (String) val.elementAt(i);
                            }
                        } else if (oid.elementAt(i).equals(X509Name.f724O) && this.mOName == null) {
                            this.mOName = (String) val.elementAt(i);
                        } else if (oid.elementAt(i).equals(X509Name.f725OU) && this.mUName == null) {
    ...
    public View inflateCertificateView(Context context) {
        ...
        View certificateView = factory.inflate(C4337R.layout.ssl_certificate, (ViewGroup) null);
        DName issuedTo = getIssuedTo();
        if (issuedTo != null) {
            ((TextView) certificateView.findViewById(C4337R.id.to_common)).setText(issuedTo.getCName());
            ((TextView) certificateView.findViewById(C4337R.id.to_org)).setText(issuedTo.getOName());
            ((TextView) certificateView.findViewById(C4337R.id.to_org_unit)).setText(issuedTo.getUName());
        }
        ((TextView) certificateView.findViewById(C4337R.id.serial_number)).setText(getSerialNumber(this.mX509Certificate));
        DName issuedBy = getIssuedBy();
        if (issuedBy != null) {
            ((TextView) certificateView.findViewById(C4337R.id.by_common)).setText(issuedBy.getCName());
            ((TextView) certificateView.findViewById(C4337R.id.by_org)).setText(issuedBy.getOName());
            ((TextView) certificateView.findViewById(C4337R.id.by_org_unit)).setText(issuedBy.getUName());
        }
        String issuedOn = formatCertificateDate(context, getValidNotBeforeDate());
        ((TextView) certificateView.findViewById(C4337R.id.issued_on)).setText(issuedOn);
        String expiresOn = formatCertificateDate(context, getValidNotAfterDate());
        ((TextView) certificateView.findViewById(C4337R.id.expires_on)).setText(expiresOn);
        ((TextView) certificateView.findViewById(C4337R.id.sha256_fingerprint)).setText(getDigest(this.mX509Certificate, "SHA256"));
        ((TextView) certificateView.findViewById(C4337R.id.sha1_fingerprint)).setText(getDigest(this.mX509Certificate, "SHA1"));
