...
public class DdmHandleViewDebug extends DdmHandle {
  ...
  private Chunk dumpHierarchy(View rootView, ByteBuffer in) {
    ...
    try {
      ...
      Log.m94d(TAG, "Time to obtain view hierarchy (ms): " + (end - start));
      i = b.toByteArray();
    ...
    try {
      try {
        ...
        Chunk createFailChunk =
            createFailChunk(
                1, "Unexpected error while obtaining view hierarchy: " + e.getMessage());
        try {
          dos.close();
  ...
  private Chunk invokeViewMethod(View rootView, View targetView, ByteBuffer in) {
    ...
    try {
      ...
      try {
        ...
        Log.m96e(TAG, "Exception while invoking method: " + e2.getCause().getMessage());
      ...
      Log.m96e(TAG, "No such method: " + e3.getMessage());
  ...
  private Chunk setLayoutParameter(View rootView, View targetView, ByteBuffer in) {
    ...
    try {
      ...
      Log.m96e(TAG, "Exception setting layout parameter: " + e);
  ...
  private Chunk profileView(View rootView, View targetView) {
    ...
    try {
      ...
      Chunk createFailChunk =
          createFailChunk(1, "Unexpected error while profiling view: " + e2.getMessage());
      try {
        bw.close();
      } catch (IOException e3) {
      }
  ...
  public static void deserializeMethodParameters(Object[] args, Class<?>[] argTypes, ByteBuffer in)
      ...
      if (isArray) {
        ...
        switch (typeSignature) {
          ...
          case 'Z':
            ...
            break;
          default:
            Log.m96e(TAG, "arg " + i + ", unrecognized type: " + typeSignature);
            throw new ViewMethodInvocationSerializationException(
                "Unsupported parameter type (" + typeSignature + ") to invoke view method.");
        }
      }
    }
  }
  ...
  public static byte[] serializeReturnValue(Class<?> cls, Object obj)
    ...
    if (cls.isArray()) {
      if (!cls.equals(byte[].class)) {
        throw new ViewMethodInvocationSerializationException(
            "Unsupported array return type (" + cls + NavigationBarInflaterView.KEY_CODE_END);
      }
      byte[] bArr = (byte[]) obj;
