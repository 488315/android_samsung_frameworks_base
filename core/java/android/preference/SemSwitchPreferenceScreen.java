...
public class SemSwitchPreferenceScreen extends SwitchPreference {
    ...
    public SemSwitchPreferenceScreen(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.Preference, defStyleAttr, defStyleRes);
        Configuration configuration = context.getResources().getConfiguration();
        String fragment = a.getString(13);
        if ("".equals(fragment)) {
            Log.m102w("SemSwitchPreferenceScreen", "SwitchPreferenceScreen should get fragment property. Fragment property does not exist in SwitchPreferenceScreen");
        }
        if ((configuration.screenWidthDp <= 320 && configuration.fontScale >= 1.1f) || (configuration.screenWidthDp < 411 && configuration.fontScale >= 1.3f)) {
            setLayoutResource(C4337R.layout.tw_switch_preference_screen_large);
        } else {
            setLayoutResource(C4337R.layout.tw_switch_preference_screen_material);
        }
        if (this.mIsDeviceDefaultDark) {
            setWidgetLayoutResource(C4337R.layout.tw_switch_preference_screen_widget_divider_dark);
        } else {
            setWidgetLayoutResource(C4337R.layout.tw_switch_preference_screen_widget_divider);
        }
