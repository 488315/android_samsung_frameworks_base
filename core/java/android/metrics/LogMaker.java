...
public class LogMaker {
  ...
  public LogMaker setCategory(int category) {
    this.entries.put(
        MetricsProto.MetricsEvent.RESERVED_FOR_LOGBUILDER_CATEGORY, Integer.valueOf(category));
    return this;
  }
  ...
  public LogMaker setSubtype(int subtype) {
    this.entries.put(
        MetricsProto.MetricsEvent.RESERVED_FOR_LOGBUILDER_SUBTYPE, Integer.valueOf(subtype));
    return this;
  }
  ...
  public LogMaker setComponentName(ComponentName component) {
    this.entries.put(
        MetricsProto.MetricsEvent.RESERVED_FOR_LOGBUILDER_PACKAGENAME, component.getPackageName());
  ...
  public LogMaker addTaggedData(int tag, Object value) {
    ...
    if (value.toString().getBytes().length > 4000) {
      Log.m98i(TAG, "Log value too long, omitted: " + value.toString());
    } else {
  ...
  public Object[] serialize() {
    ...
    if (size > 4000) {
      Log.m98i(TAG, "Log line too long, did not emit: " + size + " bytes.");
  ...
  public void deserialize(Object[] items) {
    ...
    for (int i2 = 0; items != null && i2 < items.length; i2 = i) {
      ...
      if (key instanceof Integer) {
        ...
        Log.m98i(
            TAG,
            "Invalid key " + (key == null ? SemCapabilities.FEATURE_TAG_NULL : key.toString()));
      }
    }
  }
