...
public class BluetoothDeviceFilterUtils {
  ...
  static boolean matchesServiceUuids(
      List<ParcelUuid> serviceUuids, List<ParcelUuid> serviceUuidMasks, BluetoothDevice device) {
  ...
  static boolean matchesServiceUuid(
    ...
    List<ParcelUuid> deviceUuids =
        device.getUuids() == null ? Collections.emptyList() : Arrays.asList(device.getUuids());
    if (serviceUuid == null) {
      return true;
    }
  ...
  private static void debugLogMatchResult(boolean result, BluetoothDevice device, Object criteria) {
    Log.m98i(LOG_TAG, getDeviceDisplayNameInternal(device) + (result ? " ~ " : " !~ ") + criteria);
  }
