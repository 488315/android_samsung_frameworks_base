...
public final class BluetoothLeDeviceFilter implements DeviceFilter<ScanResult> {
            ...
            new Parcelable.Creator<BluetoothLeDeviceFilter>() { // from class:
                ...
                public BluetoothLeDeviceFilter createFromParcel(Parcel in) {
                    Builder builder =
                            new Builder()
                                    .setNamePattern(
                                            BluetoothDeviceFilterUtils.patternFromString(
                                                    in.readString()))
                                    .setScanFilter(
                                            (ScanFilter) in.readParcelable(null, ScanFilter.class));
    ...
    private BluetoothLeDeviceFilter(
        ...
        this.mScanFilter =
                (ScanFilter) ObjectUtils.firstNotNull(scanFilter, new ScanFilter.Builder().build());
    ...
    public String getDeviceDisplayName(ScanResult sr) {
        ...
        if (this.mRenameBytesFrom >= 0) {
            ...
            sb.append(
                    BluetoothDeviceFilterUtils.getDeviceDisplayNameInternal(sr.getDevice())
                            .substring(
                                    this.mRenameNameFrom,
                                    this.mRenameNameFrom + this.mRenameNameLength));
        }
    ...
    public boolean equals(Object o) {
        ...
        if (this.mRenameBytesFrom == that.mRenameBytesFrom
                && this.mRenameBytesLength == that.mRenameBytesLength
                && this.mRenameNameFrom == that.mRenameNameFrom
                && this.mRenameNameLength == that.mRenameNameLength
                && this.mRenameBytesReverseOrder == that.mRenameBytesReverseOrder
                && Objects.equals(this.mNamePattern, that.mNamePattern)
                && Objects.equals(this.mScanFilter, that.mScanFilter)
                && Arrays.equals(this.mRawDataFilter, that.mRawDataFilter)
                && Arrays.equals(this.mRawDataFilterMask, that.mRawDataFilterMask)
                && Objects.equals(this.mRenamePrefix, that.mRenamePrefix)
                && Objects.equals(this.mRenameSuffix, that.mRenameSuffix)) {
            return true;
        }
        return false;
    }
    ...
    public int hashCode() {
        return Objects.hash(
                this.mNamePattern,
                this.mScanFilter,
                Integer.valueOf(Arrays.hashCode(this.mRawDataFilter)),
                Integer.valueOf(Arrays.hashCode(this.mRawDataFilterMask)),
                this.mRenamePrefix,
                this.mRenameSuffix,
                Integer.valueOf(this.mRenameBytesFrom),
                Integer.valueOf(this.mRenameBytesLength),
                Integer.valueOf(this.mRenameNameFrom),
                Integer.valueOf(this.mRenameNameLength),
    ...
    public String toString() {
        return "BluetoothLEDeviceFilter{mNamePattern="
                + this.mNamePattern
                + ", mScanFilter="
                + this.mScanFilter
                + ", mRawDataFilter="
                + Arrays.toString(this.mRawDataFilter)
                + ", mRawDataFilterMask="
                + Arrays.toString(this.mRawDataFilterMask)
                + ", mRenamePrefix='"
                + this.mRenamePrefix
                + DateFormat.QUOTE
                + ", mRenameSuffix='"
                ...
                + DateFormat.QUOTE
                + ", mRenameBytesFrom="
                + this.mRenameBytesFrom
                + ", mRenameBytesLength="
                + this.mRenameBytesLength
                + ", mRenameNameFrom="
                + this.mRenameNameFrom
                + ", mRenameNameLength="
                + this.mRenameNameLength
                + ", mRenameBytesReverseOrder="
                + this.mRenameBytesReverseOrder
                + '}';
    }
    ...
    public static final class Builder extends OneTimeUseBuilder<BluetoothLeDeviceFilter> {
        ...
        public Builder setRawDataFilter(byte[] rawDataFilter, byte[] rawDataFilterMask) {
            ...
            Preconditions.checkArgument(
                    rawDataFilterMask == null || rawDataFilter.length == rawDataFilterMask.length,
        ...
        public Builder setRenameFromBytes(
                String prefix, String suffix, int bytesFrom, int bytesLength, ByteOrder byteOrder) {
            checkRenameNotSet();
        ...
        public Builder setRenameFromName(
                String prefix, String suffix, int nameFrom, int nameLength) {
            checkRenameNotSet();
        ...
        private void checkRenameNotSet() {
            Preconditions.checkState(
                    this.mRenamePrefix == null, "Renaming rule can only be set once");
        }
        ...
        private Builder setRename(String prefix, String suffix) {
            checkNotUsed();
            Preconditions.checkArgument(
                    TextUtils.length(prefix)
                            <= BluetoothLeDeviceFilter.getRenamePrefixLengthLimit(),
                    "Prefix is too long");
        ...
        public BluetoothLeDeviceFilter build() {
            markUsed();
            return new BluetoothLeDeviceFilter(
                    this.mNamePattern,
                    this.mScanFilter,
                    this.mRawDataFilter,
                    this.mRawDataFilterMask,
