...
final class DeadZone {
  ...
  private static final FloatProperty<DeadZone> FLASH_PROPERTY =
      new FloatProperty<DeadZone>(
          "DeadZoneFlash") { // from class: android.inputmethodservice.navigationbar.DeadZone.1
  ...
  private float getSize(long now) {
    ...
    return (int) lerp(i, this.mSizeMin, (dt - i2) / i3);
  }
      ...
      if (!this.mVertical) {
        ...
        consumeEvent =
            this.mDisplayRotation == 3
                ? event.getX() > ((float) (this.mNavigationBarView.getWidth() - size))
