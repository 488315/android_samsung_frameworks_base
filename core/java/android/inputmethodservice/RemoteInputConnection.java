...
final class RemoteInputConnection implements InputConnection {
    ...
    private static final class InputMethodServiceInternalHolder {
        ...
        public InputMethodServiceInternal getAndWarnIfNull() {
            ...
            if (ims == null) {
                Log.m97e(RemoteInputConnection.TAG, "InputMethodService is already destroyed.  InputConnection instances cannot be used beyond InputMethodService lifetime.", new Throwable());
            }
            return ims;
        }
    }
    ...
    public SurroundingText getSurroundingText(int beforeLength, int afterLength, int flags) {
        if (beforeLength < 0) {
            Log.m96e(TAG, "beforeLength=" + beforeLength + " is invalid and always results in null result.");
        }
        if (afterLength < 0) {
            Log.m96e(TAG, "afterLength=" + afterLength + " is invalid and always results in null result.");
        }
        if (this.mCancellationGroup.isCanceled()) {
            return null;
        }
