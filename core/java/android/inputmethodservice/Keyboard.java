...
public class Keyboard {
  ...
  public static class Row {
    ...
    public Row(Resources res, Keyboard parent, XmlResourceParser parser) {
      this.parent = parent;
      TypedArray a = res.obtainAttributes(Xml.asAttributeSet(parser), C4337R.styleable.Keyboard);
      this.defaultWidth =
          Keyboard.getDimensionOrFraction(a, 0, parent.mDisplayWidth, parent.mDefaultWidth);
      this.defaultHeight =
          Keyboard.getDimensionOrFraction(a, 1, parent.mDisplayHeight, parent.mDefaultHeight);
      this.defaultHorizontalGap =
          Keyboard.getDimensionOrFraction(a, 2, parent.mDisplayWidth, parent.mDefaultHorizontalGap);
      this.verticalGap =
          Keyboard.getDimensionOrFraction(a, 3, parent.mDisplayHeight, parent.mDefaultVerticalGap);
      a.recycle();
      TypedArray a2 =
          res.obtainAttributes(Xml.asAttributeSet(parser), C4337R.styleable.Keyboard_Row);
  ...
  public static class Key {
    ...
    public Key(Resources res, Row parent, int x, int y, XmlResourceParser parser) {
      ...
      TypedArray a = res.obtainAttributes(Xml.asAttributeSet(parser), C4337R.styleable.Keyboard);
      this.width =
          Keyboard.getDimensionOrFraction(a, 0, this.keyboard.mDisplayWidth, parent.defaultWidth);
      this.height =
          Keyboard.getDimensionOrFraction(a, 1, this.keyboard.mDisplayHeight, parent.defaultHeight);
      this.gap =
          Keyboard.getDimensionOrFraction(
              a, 2, this.keyboard.mDisplayWidth, parent.defaultHorizontalGap);
      a.recycle();
      TypedArray a2 =
          res.obtainAttributes(Xml.asAttributeSet(parser), C4337R.styleable.Keyboard_Key);
      this.f266x += this.gap;
      ...
      if (drawable != null) {
        drawable.setBounds(
            0, 0, drawable.getIntrinsicWidth(), this.iconPreview.getIntrinsicHeight());
      }
    ...
    int[] parseCSV(String value) {
      ...
      while (st.hasMoreTokens()) {
        ...
        try {
          ...
          Log.m96e("Keyboard", "Error parsing keycodes " + value);
        }
        count2 = count3;
      }
      return values;
    ...
    public boolean isInside(int x, int y) {
      ...
      int i3 = this.f266x;
      return (x >= i3 || (leftEdge && x <= this.width + i3))
          && (x < this.width + i3 || (rightEdge && x >= i3))
          && ((y >= (i = this.f267y) || (topEdge && y <= this.height + i))
              && (y < this.height + i || (bottomEdge && y >= i)));
    }

    public int squaredDistanceFrom(int x, int y) {
      int xDist = (this.f266x + (this.width / 2)) - x;
      int yDist = (this.f267y + (this.height / 2)) - y;
      ...
      while (y < gridHeight) {
        ...
        for (int i = 0; i < this.mKeys.size(); i++) {
          ...
          if (key.squaredDistanceFrom(x, y) < this.mProximityThreshold
              || key.squaredDistanceFrom((this.mCellWidth + x) - 1, y) < this.mProximityThreshold
              || key.squaredDistanceFrom((this.mCellWidth + x) - 1, (this.mCellHeight + y) - 1)
        ...
        if (event == 2) {
          ...
          if (TAG_ROW.equals(tag)) {
            try {
              ...
              boolean skipRow =
                  (currentRow.mode == 0 || currentRow.mode == this.mKeyboardMode) ? false : true;
              if (skipRow) {
                try {
                  ...
                  e = e;
                  Log.m96e("Keyboard", "Parse error:" + e);
                  e.printStackTrace();
