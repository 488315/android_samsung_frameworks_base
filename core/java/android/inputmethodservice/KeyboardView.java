...
public class KeyboardView extends View implements View.OnClickListener {
  ...
  public KeyboardView(Context context, AttributeSet attrs) {
    this(context, attrs, C4337R.attr.keyboardViewStyle);
  }
  ...
  public KeyboardView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(
            attrs, C0000R.styleable.KeyboardView, defStyleAttr, defStyleRes);
    ...
    a.recycle();
    TypedArray a2 = this.mContext.obtainStyledAttributes(C4337R.styleable.Theme);
    ...
    this.mDisambiguateSwipe = getResources().getBoolean(C4337R.bool.config_swipeDisambiguation);
          ...
          new GestureDetector(
              ...
              new GestureDetector
                ...
                public boolean onFling(
                  ...
                  if (velocityX <= KeyboardView.this.mSwipeThreshold
                      || absY >= absX
                      || deltaX <= travelX) {
                    if (velocityX >= (-KeyboardView.this.mSwipeThreshold)
                        || absY >= absX
                        || deltaX >= (-travelX)) {
                      if (velocityY >= (-KeyboardView.this.mSwipeThreshold)
                          || absX >= absY
                          || deltaY >= (-travelY)) {
                        if (velocityY > KeyboardView.this.mSwipeThreshold
                            && absX < absY / 2.0f
                            && deltaY > travelY) {
                          if (KeyboardView.this.mDisambiguateSwipe
                              && endingVelocityY < velocityY / 4.0f) {
                            sendDownKey = true;
                          } else {
                      ...
                      } else if (KeyboardView.this.mDisambiguateSwipe
                          && endingVelocityY > velocityY / 4.0f) {
                        sendDownKey = true;
                      } else {
                        ...
                        && endingVelocityX > velocityX / 4.0f) {
                      sendDownKey = true;
                    } else {
                  ...
                  if (sendDownKey) {
                    ...
                    keyboardView.detectAndSendKey(
                        keyboardView.mDownKey,
                        KeyboardView.this.mStartX,
                        KeyboardView.this.mStartY,
                        me1.getEventTime());
                  }
                  return false;
                }
  ...
  public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (keyboard == null) {
      setMeasuredDimension(
          this.mPaddingLeft + this.mPaddingRight, this.mPaddingTop + this.mPaddingBottom);
      return;
    }
    ...
    setMeasuredDimension(
        width, this.mKeyboard.getHeight() + this.mPaddingTop + this.mPaddingBottom);
  }
  ...
  private void onBufferDraw() {
    ...
    if (bitmap == null || this.mKeyboardChanged) {
      if (bitmap == null
          || (this.mKeyboardChanged
              && (bitmap.getWidth() != getWidth() || this.mBuffer.getHeight() != getHeight()))) {
    ...
    if (invalidKey2 != null
        ...
        && (invalidKey2.f266x + kbdPaddingLeft) - 1 <= clipRegion.left
        && (invalidKey2.f267y + kbdPaddingTop) - 1 <= clipRegion.top
        && invalidKey2.f266x + invalidKey2.width + kbdPaddingLeft + 1 >= clipRegion.right
        && invalidKey2.f267y + invalidKey2.height + kbdPaddingTop + 1 >= clipRegion.bottom) {
      drawSingleKey = true;
    } else {
      drawSingleKey = false;
    }
    ...
    while (i < keyCount2) {
      ...
      if (drawSingleKey && invalidKey2 != key) {
        ...
        canvas.translate(key.f266x + kbdPaddingLeft, key.f267y + kbdPaddingTop);
        ...
        if (label != null) {
          ...
          canvas.drawText(
              ...
              (((key.height - padding.top) - padding.bottom) / 2)
                  + ((paint.getTextSize() - paint.descent()) / 2.0f)
                  + padding.top,
              paint);
  ...
  */
  /* JADX WARN: Code restructure failed: missing block: B:21:0x0058, code lost:
     ...
     if (r15 >= r1.length) goto L41;
  */
  /* JADX WARN: Code restructure failed: missing block: B:23:0x005c, code lost:

     if (r1[r15] <= r12) goto L28;
  */
  ...
  */
  /* JADX WARN: Code restructure failed: missing block: B:9:0x003a, code lost:
     ...
     if (r14 >= r17.mProximityThreshold) goto L11;
  */
  /*
  ...
  */
  ...
  public void showKey(int keyIndex) {
    ...
    if (key.icon != null) {
      this.mPreviewText.setCompoundDrawables(
          null, null, null, key.iconPreview != null ? key.iconPreview : key.icon);
    ...
    this.mPreviewText.measure(
        View.MeasureSpec.makeMeasureSpec(0, 0), View.MeasureSpec.makeMeasureSpec(0, 0));
    int popupWidth =
        Math.max(
            this.mPreviewText.getMeasuredWidth(),
            key.width + this.mPreviewText.getPaddingLeft() + this.mPreviewText.getPaddingRight());
    ...
    if (!this.mPreviewCentered) {
      this.mPopupPreviewX = (key.f266x - this.mPreviewText.getPaddingLeft()) + this.mPaddingLeft;
      this.mPopupPreviewY = (key.f267y - popupHeight) + this.mPreviewOffset;
    } else {
    ...
    this.mPreviewText
        .getBackground()
        .setState(key.popupResId != 0 ? LONG_PRESSABLE_STATE_SET : EMPTY_STATE_SET);
    ...
    if (this.mPopupPreviewY + this.mCoordinates[1] < 0) {
      if (key.f266x + key.width <= getWidth() / 2) {
  ...
  private void sendAccessibilityEventForUnicodeCharacter(int eventType, int code) {
    ...
    if (this.mAccessibilityManager.isEnabled()) {
      ...
      switch (code) {
        case -6:
          text = this.mContext.getString(C4337R.string.keyboardview_keycode_alt);
          break;
        case -5:
          text = this.mContext.getString(C4337R.string.keyboardview_keycode_delete);
          break;
        case -4:
          text = this.mContext.getString(C4337R.string.keyboardview_keycode_done);
          break;
        case -3:
          text = this.mContext.getString(C4337R.string.keyboardview_keycode_cancel);
          break;
        case -2:
          text = this.mContext.getString(C4337R.string.keyboardview_keycode_mode_change);
          break;
        case -1:
          text = this.mContext.getString(C4337R.string.keyboardview_keycode_shift);
          break;
        case 10:
          text = this.mContext.getString(C4337R.string.keyboardview_keycode_enter);
          break;
        default:
  ...
  public void invalidateKey(int keyIndex) {
    ...
    this.mDirtyRect.union(
        key.f266x + this.mPaddingLeft,
        key.f267y + this.mPaddingTop,
        key.f266x + key.width + this.mPaddingLeft,
        key.f267y + key.height + this.mPaddingTop);
    onBufferDraw();
    invalidate(
        key.f266x + this.mPaddingLeft,
        key.f267y + this.mPaddingTop,
  ...
  protected boolean onLongPress(Keyboard.Key popupKey) {
    ...
    if (view == null) {
      ...
      this.mMiniKeyboard.setOnKeyboardActionListener(
          new OnKeyboardActionListener() { // from class: android.inputmethodservice.KeyboardView.3
      ...
      this.mMiniKeyboardContainer.measure(
          View.MeasureSpec.makeMeasureSpec(getWidth(), Integer.MIN_VALUE),
          View.MeasureSpec.makeMeasureSpec(getHeight(), Integer.MIN_VALUE));
    ...
    getLocationInWindow(this.mCoordinates);
    this.mPopupX = popupKey.f266x + this.mPaddingLeft;
    this.mPopupY = popupKey.f267y + this.mPaddingTop;
  ...
  public boolean onTouchEvent(MotionEvent me) {
    ...
    if (pointerCount != this.mOldPointerCount) {
      if (pointerCount == 1) {
        ...
        MotionEvent up =
            MotionEvent.obtain(
                now, now, 1, this.mOldPointerX, this.mOldPointerY, me.getMetaState());
  ...
  public boolean repeatKey() {
    ...
    detectAndSendKey(this.mCurrentKey, key.f266x, key.f267y, this.mLastTapTime);
    return true;
  }
  ...
  private static class SwipeTracker {
    ...
    public void computeCurrentVelocity(int units, float maxVelocity) {
      ...
      this.mXVelocity =
          accumX < 0.0f ? Math.max(accumX, -maxVelocity) : Math.min(accumX, maxVelocity);
      this.mYVelocity =
          accumY < 0.0f ? Math.max(accumY, -maxVelocity) : Math.min(accumY, maxVelocity);
    }
