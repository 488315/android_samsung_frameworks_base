...
final class IRemoteInputConnectionInvoker {
  ...
  static final class TextBoundsInfoResultReceiver
    ...
    @Override // android.inputmethodservice.IRemoteInputConnectionInvoker.OnceResultReceiver
    public void dispatch(
        Executor executor, final Consumer<TextBoundsInfoResult> consumer, int code, Bundle data) {
    ...
    try {
      ...
      try {
        this.mConnection.performHandwritingGesture(
            createHeader(), ParcelableHandwritingGesture.m165of(gesture), resultReceiver);
        if (ignored != null) {
          ignored.close();
        }
      } finally {
    ...
    try {
      ...
      try {
        this.mConnection.previewHandwritingGesture(
            createHeader(), ParcelableHandwritingGesture.m165of(gesture), csToken);
        if (csToken != null) {
          csToken.close();
          return true;
        }
  ...
  private CancellationSignalBeamer.Sender getCancellationSignalBeamer() {
    ...
    CancellationSignalBeamer.Sender sender2 =
        new CancellationSignalBeamer
            .Sender() { // from class: android.inputmethodservice.IRemoteInputConnectionInvoker.1
