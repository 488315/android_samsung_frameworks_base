...
final class SoftInputWindow extends Dialog {
  ...
  void setToken(IBinder token) {
    switch (this.mWindowState) {
      ...
      case 4:
        Log.m98i(TAG, "Ignoring setToken() because window is already destroyed.");
        return;
      default:
  ...
  public boolean dispatchTouchEvent(MotionEvent ev) {
    ...
    if (ev.isWithinBoundsNoHistory(
        this.mBounds.left, this.mBounds.top, this.mBounds.right - 1, this.mBounds.bottom - 1)) {
    ...
    switch (this.mWindowState) {
      ...
      case 2:
        try {
          ...
          Log.m98i(
              TAG, "Probably the IME window token is already invalidated. show() does nothing.");
          updateWindowState(3);
          return;
        } catch (WindowManager.InvalidDisplayException e2) {
          Log.m98i(TAG, "Probably the specified display can not be found. show() does nothing.");
          updateWindowState(3);
          return;
        }
      case 3:
        Log.m98i(TAG, "Not trying to call show() because it was already rejected once.");
        return;
      case 4:
        Log.m98i(TAG, "Ignoring show() because the window is already destroyed.");
        return;
      default:
        throw new IllegalStateException("Unexpected state=" + this.mWindowState);
    }
    ...
    switch (this.mWindowState) {
      ...
      case 2:
        try {
          ...
          Log.m98i(
              TAG, "Probably the IME window token is already invalidated. No need to dismiss it.");
        }
        updateWindowState(4);
        return;
      case 3:
        Log.m98i(TAG, "Not trying to dismiss the window because it is most likely unnecessary.");
        updateWindowState(4);
        return;
      case 4:
