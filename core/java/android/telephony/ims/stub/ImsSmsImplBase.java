...
public class ImsSmsImplBase {
  ...
  public void sendSms(
    ...
    try {
      ...
      Log.m96e(LOG_TAG, "Can not send sms: " + e.getMessage());
    }
  }
  ...
  public void acknowledgeSms(int token, int messageRef, int result) {
    Log.m96e(LOG_TAG, "acknowledgeSms() not implemented.");
  }

  public void acknowledgeSms(int token, int messageRef, int result, byte[] pdu) {
    Log.m96e(LOG_TAG, "acknowledgeSms() not implemented. acknowledgeSms(int, int, int) called.");
    acknowledgeSms(token, messageRef, result);
  }

  public void acknowledgeSmsReport(int token, int messageRef, int result) {
    Log.m96e(LOG_TAG, "acknowledgeSmsReport() not implemented.");
  }
  ...
  public final void onSmsReceived(int token, String format, byte[] pdu) throws RuntimeException {
    ...
    try {
      ...
      Log.m96e(LOG_TAG, "Can not deliver sms: " + e.getMessage());
      SmsMessage message = SmsMessage.createFromPdu(pdu, format);
      if (message != null && message.mWrappedSmsMessage != null) {
        acknowledgeSms(token, message.mWrappedSmsMessage.mMessageRef, 2);
      } else {
        Log.m102w(LOG_TAG, "onSmsReceived: Invalid pdu entered.");
  ...
  @Deprecated
  public final void onSmsStatusReportReceived(int token, int messageRef, String format, byte[] pdu)
      throws RuntimeException {
    IImsSmsListener listener;
    synchronized (this.mLock) {
      listener = this.mListener;
    ...
    try {
      ...
      Log.m96e(LOG_TAG, "Can not process sms status report: " + e.getMessage());
  ...
  public final void onSmsStatusReportReceived(int token, String format, byte[] pdu)
    ...
    try {
      ...
      if (message != null && message.mWrappedSmsMessage != null) {
        ...
        Log.m102w(LOG_TAG, "onSmsStatusReportReceived: Invalid pdu entered.");
