...
public final class RcsConfig {
      ...
      try {
        ...
        for (int i = 1; eventType != i && characteristic2 != null; i = 1) {
          if (eventType == 2) {
            ...
            if (TAG_CHARACTERISTIC.equals(lowerCase)) {
              ...
              if (attributeCount > 0) {
                ...
                while (true) {
                  ...
                  String lowerCase2 =
                      newPullParser.getAttributeName(i2).trim().toLowerCase(Locale.ROOT);
                  if (!"type".equals(lowerCase2)) {
                    i2++;
                  } else {
                    str =
                        newPullParser
                            .getAttributeValue(newPullParser.getAttributeNamespace(i2), lowerCase2)
                            .trim()
              ...
              if (attributeCount2 > 1) {
                for (int i3 = 0; i3 < attributeCount2; i3++) {
                  String lowerCase3 =
                      newPullParser.getAttributeName(i3).trim().toLowerCase(Locale.ROOT);
                  if ("name".equals(lowerCase3)) {
                    str2 =
                        newPullParser
                            .getAttributeValue(newPullParser.getAttributeNamespace(i3), lowerCase3)
                            .trim()
                            .toLowerCase(Locale.ROOT);
            ...
            characteristic2 =
                TAG_CHARACTERISTIC.equals(newPullParser.getName().trim().toLowerCase(Locale.ROOT))
                    ? characteristic2.getParent()
                    : characteristic2;
          }
  ...
  */
  public static byte[] loadRcsConfigForSub(Context cxt, int subId, boolean isCompressed) {
    ...
    Cursor cursor =
        cxt.getContentResolver()
            .query(Telephony.SimInfo.CONTENT_URI, null, "_id=" + subId, null, null);
    try {
      if (cursor != null) {
        try {
          if (cursor.moveToFirst()) {
            data =
                cursor.getBlob(cursor.getColumnIndexOrThrow(Telephony.SimInfo.COLUMN_RCS_CONFIG));
          }
        } catch (Exception e) {
  ...
  private static void logd(String msg) {
    Rlog.m238d(LOG_TAG, msg);
  }

  private static void loge(String msg) {
    Rlog.m240e(LOG_TAG, msg);
  }
}
