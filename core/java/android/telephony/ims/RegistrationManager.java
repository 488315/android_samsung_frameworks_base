...
public interface RegistrationManager {
  ...
  void registerImsRegistrationCallback(Executor executor, RegistrationCallback registrationCallback)
      throws ImsException;
  ...
  static int getAccessType(int regtech) {
    ...
    if (!map.containsKey(Integer.valueOf(regtech))) {
      Log.m102w("RegistrationManager", "getAccessType - invalid regType returned: " + regtech);
      return -1;
    }
        ...
        try {
          this.mExecutor.execute(
              new Runnable() { // from class:
                ...
                public final void run() {
                  RegistrationManager.RegistrationCallback.RegistrationBinder.this
                      .lambda$onDeregistered$2(info, suggestedAction, imsRadioTech);
                }
              });
        } finally {
          restoreCallingIdentity(callingIdentity);
        ...
        try {
          this.mExecutor.execute(
              new Runnable() { // from class:
                ...
                public final void run() {
                  RegistrationManager.RegistrationCallback.RegistrationBinder.this
                      .lambda$onDeregisteredWithDetails$3(info, suggestedAction, imsRadioTech);
                }
              });
          this.mExecutor.execute(
              new Runnable() { // from class:
