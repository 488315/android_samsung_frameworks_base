...
public class ImsService extends Service {
  ...
  protected final IBinder mImsServiceController =
      new IImsServiceController.Stub() { // from class: android.telephony.ims.compat.ImsService.1
  ...
  public void addImsFeatureStatusCallback(
    ...
    synchronized (this.mFeaturesBySlot) {
      ...
      if (features == null) {
        Log.m102w(
            LOG_TAG,
            "Can not add ImsFeatureStatusCallback. No ImsFeatures exist on slot " + slotId);
        return;
      }
  ...
  public void removeImsFeatureStatusCallback(
    ...
    synchronized (this.mFeaturesBySlot) {
      ...
      if (features == null) {
        Log.m102w(
            LOG_TAG,
            "Can not remove ImsFeatureStatusCallback. No ImsFeatures exist on slot " + slotId);
        return;
      }
      ImsFeature f = features.get(featureType);
      if (f != null) {
