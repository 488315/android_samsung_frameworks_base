...
public class MbmsDownloadSession implements AutoCloseable {
    ...
    ServiceConnection serviceConnection =
        new ServiceConnection() { // from class: android.telephony.MbmsDownloadSession.3
            ...
            try {
              ...
              if (result == 0) {
                try {
                  downloadService
                      .asBinder()
                      .linkToDeath(MbmsDownloadSession.this.mDeathRecipient, 0);
              ...
              Log.m96e(MbmsDownloadSession.LOG_TAG, "Service died before initialization");
              MbmsDownloadSession.sIsInitialized.set(false);
            } catch (RuntimeException e3) {
              Log.m96e(MbmsDownloadSession.LOG_TAG, "Runtime exception during initialization");
          ...
          public void onServiceDisconnected(ComponentName name) {
            Log.m102w(
                MbmsDownloadSession.LOG_TAG, "bindAndInitialize: Remote service disconnected");
            MbmsDownloadSession.sIsInitialized.set(false);
          ...
          public void onNullBinding(ComponentName name) {
            Log.m102w(
                MbmsDownloadSession.LOG_TAG, "bindAndInitialize: Remote service returned null");
  ...
  public void requestUpdateFileServices(List<String> classList) {
    ...
    try {
      ...
      Log.m102w(LOG_TAG, "Remote process died");
  ...
  public File getTempFileRootDirectory() {
    SharedPreferences prefs =
        this.mContext.getSharedPreferences(MbmsTempFileProvider.TEMP_FILE_ROOT_PREF_FILE_NAME, 0);
  ...
  public void close() {
    ...
    try {
      try {
        ...
        Log.m98i(LOG_TAG, "Remote exception while disposing of service");
      }
      ...
      Log.m98i(LOG_TAG, "Service already dead");
    } finally {
      this.mService.set(null);
      sIsInitialized.set(false);
  ...
  private void writeDownloadRequestToken(DownloadRequest request) {
    ...
    if (token.exists()) {
      Log.m102w(LOG_TAG, "Download token " + token.getName() + " already exists");
      return;
    }
    try {
  ...
  private void deleteDownloadRequestToken(DownloadRequest request) {
    File token = getDownloadRequestTokenPath(request);
    if (!token.isFile()) {
      Log.m102w(LOG_TAG, "Attempting to delete non-existent download token at " + token);
    } else if (!token.delete()) {
      Log.m102w(LOG_TAG, "Couldn't delete download token at " + token);
    }
  }
      ...
      try {
        ...
        if (!testFile.renameTo(testFileDestination)) {
          throw new IllegalArgumentException(
              "Destination provided in the download request is invalid -- files in the temp file"
                  + " directory cannot be directly moved there.");
        }
      } catch (IOException e) {
