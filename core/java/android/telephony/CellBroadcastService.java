...
public abstract class CellBroadcastService extends Service {
    ...
    public class ICellBroadcastServiceWrapper extends ICellBroadcastService.Stub {
        ...
        public void handleGsmCellBroadcastSms(int slotIndex, byte[] message) {
            ...
            com.android.telephony.Rlog.m238d(CellBroadcastService.TAG, "header=" + header);
            if (header == null) {
                return;
            }
            ...
            if (pageCount == 1) {
                if (!header.isUmtsFormat()) {
                    try {
                        com.android.telephony.Rlog.m242i(CellBroadcastService.TAG, "Single page. Not UMTS format");
                        ...
                        if (header.getEtwsInfo() != null && pdu.length > 56) {
                            com.android.telephony.Rlog.m242i(CellBroadcastService.TAG, "Remove padding bit and convert GSM to UMTS.");
                            int i2 = pdu.length - 1;
                            while (i2 >= 0 && pdu[i2] == 0) {
                                i2--;
                                pduLength--;
                        ...
                        com.android.telephony.Rlog.m240e(CellBroadcastService.TAG, "Error in decoding SMS CB pdu " + e.toString());
                        CellBroadcastService.this.onGsmCellBroadcastSms(slotIndex, pdu);
                        return;
                    }
                }
                com.android.telephony.Rlog.m238d(CellBroadcastService.TAG, "Converting is not needed");
                CellBroadcastService.this.onGsmCellBroadcastSms(slotIndex, pdu);
                return;
            }
            if (pageCount <= 1) {
            ...
            com.android.telephony.Rlog.m238d(CellBroadcastService.TAG, "pdus size=" + pdus.length);
            pdus[header.getPageIndex() - 1] = pdu;
            for (byte[] bArr : pdus) {
                if (bArr == null) {
                    com.android.telephony.Rlog.m238d(CellBroadcastService.TAG, "still missing pdu");
                    return;
                }
            }
            try {
                ...
                while (iter.hasNext()) {
                    try {
                        ...
                        if (info.overTime()) {
                            com.android.telephony.Rlog.m238d(CellBroadcastService.TAG, "Remove saved message over 5min");
                            iter.remove();
                        }
                ...
                try {
                    if (wacLength2 > 0) {
                        try {
                            com.android.telephony.Rlog.m242i(CellBroadcastService.TAG, "WAC included in GSM format multipage");
                        ...
                        try {
                            if (header.getEtwsInfo() == null) {
                                com.android.telephony.Rlog.m242i(CellBroadcastService.TAG, "No WAC. Deliver CB without converting.");
                                for (byte[] bArr2 : pdus2) {
                                    CellBroadcastService.this.onGsmCellBroadcastSms(slotIndex, bArr2);
                                }
                                return;
                            }
                            com.android.telephony.Rlog.m242i(CellBroadcastService.TAG, "Remove padding bit and convert GSM to UMTS for multipage.");
            ...
            com.android.telephony.Rlog.m240e(CellBroadcastService.TAG, errorMessage);
            if (pdus2 != null) {
    ...
    public SmsCbHeader createSmsCbHeader(byte[] bytes) {
        try {
            ...
            com.android.telephony.Rlog.m240e(TAG, "Can't create SmsCbHeader, ex = " + ex.toString());
            return null;
        }
    }
    ...
    public void convertGsmToUmts(byte[] pdu, byte[] wac, int pduLength, int wacLength, int slotIndex) {
        ...
        try {
            ...
            com.android.telephony.Rlog.m240e(TAG, "Error in convertGsmToUmts: " + e.toString());
        ...
        try {
            ...
            if (wacLength != 0) {
                try {
                    ...
                    e = e;
                    com.android.telephony.Rlog.m240e(TAG, "Error in convertGsmToUmtsForMultiPage: " + e.toString());
                    for (byte[] bArr : pdus) {
