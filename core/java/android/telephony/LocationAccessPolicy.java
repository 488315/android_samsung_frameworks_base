...
public final class LocationAccessPolicy {
      ...
      public LocationPermissionQuery build() {
        ...
        if (i2 < 0 || (i = this.mMinSdkVersionForFine) < 0) {
          throw new IllegalArgumentException(
              "Must specify min sdk versions for enforcement for both coarse and fine permissions");
        }
        ...
        if (i3 < i4) {
          throw new IllegalArgumentException(
              "Since fine location permission includes access to coarse location, the min sdk level"
                  + " for enforcement of the fine location permission must not be less than the min"
                  + " sdk level for enforcement of the coarse location permission.");
        }
  ...
  private static void logError(Context context, LocationPermissionQuery query, String errorMsg) {
    if (query.logAsInfo) {
      Log.m98i(TAG, errorMsg);
      return;
    }
    Log.m96e(TAG, errorMsg);
    try {
  ...
  private static LocationPermissionResult checkAppLocationPermissionHelper(
    ...
    String locationTypeForLog =
        Manifest.permission.ACCESS_FINE_LOCATION.equals(permissionToCheck) ? "fine" : "coarse";
    boolean hasManifestPermission =
        checkManifestPermission(context, query.callingPid, query.callingUid, permissionToCheck);
    ...
    if (minSdkVersion > 10000) {
      String errorMsg =
          "Allowing "
              ...
              + minSdkVersion
              + " yet. Please fix this app because it will break in the future. Called from "
              + query.method;
  ...
  public static boolean isLocationModeEnabled(Context context, int userId) {
    ...
    if (locationManager == null) {
      Log.m102w(TAG, "Couldn't get location manager, denying location access");
      return false;
    }
    return locationManager.isLocationEnabledForUser(UserHandle.m55of(userId));
  }
  ...
  public static String[] getLocationBypassPackages(Context context) {
    return context
        .getResources()
        .getStringArray(C4337R.array.config_serviceStateLocationAllowedPackages);
  }
