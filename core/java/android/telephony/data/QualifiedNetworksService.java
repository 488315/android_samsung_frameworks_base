  ...
  public abstract class NetworkAvailabilityProvider implements AutoCloseable {
    ...
    public void onUpdateQualifiedNetworkTypes(int apnTypes, int[] qualifiedNetworkTypes) {
      ...
      if (iQualifiedNetworksServiceCallback != null) {
        try {
          iQualifiedNetworksServiceCallback.onQualifiedNetworkTypesChanged(
    ...
    public final void updateHandoverEnabled(int supportedApnTypes) {
      QualifiedNetworksService.this
          .mHandler
          .obtainMessage(7, this.mSlotIndex, supportedApnTypes)
          .sendToTarget();
    }
  ...
  private class QualifiedNetworksServiceHandler extends Handler {
    ...
    public void handleMessage(Message message) {
      ...
      NetworkAvailabilityProvider provider =
          (NetworkAvailabilityProvider) QualifiedNetworksService.this.mProviders.get(slotIndex);
      switch (message.what) {
        case 1:
          if (QualifiedNetworksService.this.mProviders.get(slotIndex) != null) {
            QualifiedNetworksService.this.loge(
                "Network availability provider for slot " + slotIndex + " already existed.");
            break;
          } else {
            NetworkAvailabilityProvider provider2 =
                QualifiedNetworksService.this.onCreateNetworkAvailabilityProvider(slotIndex);
            if (provider2 != null) {
              ...
              QualifiedNetworksService.this.loge(
                  "Failed to create network availability provider. slot index = " + slotIndex);
              break;
            }
          }
        case 2:
        ...
        case 3:
          for (int i = 0; i < QualifiedNetworksService.this.mProviders.size(); i++) {
            NetworkAvailabilityProvider provider3 =
                (NetworkAvailabilityProvider) QualifiedNetworksService.this.mProviders.get(i);
            if (provider3 != null) {
              provider3.close();
            }
          }
