...
public class EuiccManager {
      ...
      PendingIntent callbackIntent =
          (PendingIntent)
              resolutionIntent.getParcelableExtra(
                  EXTRA_EMBEDDED_SUBSCRIPTION_RESOLUTION_CALLBACK_INTENT, PendingIntent.class);
    ...
    try {
      getIEuiccController()
          .getDownloadableSubscriptionMetadata(
              this.mCardId, subscription, this.mContext.getOpPackageName(), callbackIntent);
    } catch (RemoteException e) {
      throw e.rethrowFromSystemServer();
    }
  }
  ...
  public void deleteSubscription(int subscriptionId, PendingIntent callbackIntent) {
    ...
    try {
      getIEuiccController()
          .deleteSubscription(
              this.mCardId, subscriptionId, this.mContext.getOpPackageName(), callbackIntent);
    } catch (RemoteException e) {
      throw e.rethrowFromSystemServer();
    }
  }
      ...
      try {
        if (getIEuiccController()
            .isCompatChangeEnabled(
                this.mContext.getOpPackageName(), SWITCH_WITHOUT_PORT_INDEX_EXCEPTION_ON_DISABLE)) {
          Log.m96e(
              TAG, "switchToSubscription without portIndex is not allowed for disable operation");
          throw new IllegalArgumentException(
              "Must use switchToSubscription with portIndex API for disable operation");
        }
      } catch (RemoteException e) {
        throw e.rethrowFromSystemServer();
      }
  ...
  public void switchToSubscription(
    ...
    try {
      ...
      if (!canWriteEmbeddedSubscriptions
          && !getIEuiccController()
              .hasCarrierPrivilegesForPackageOnAnyPhone(this.mContext.getOpPackageName())) {
        Log.m96e(TAG, "Not permitted to use switchToSubscription with portIndex");
