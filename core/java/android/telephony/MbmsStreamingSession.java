...
public class MbmsStreamingSession implements AutoCloseable {
  ...
  public void requestUpdateStreamingServices(List<String> serviceClassList) {
    ...
    try {
      int returnCode =
          streamingService.requestUpdateStreamingServices(this.mSubscriptionId, serviceClassList);
      if (returnCode == -1) {
        close();
  ...
  public StreamingService startStreaming(
      StreamingServiceInfo serviceInfo, Executor executor, StreamingServiceCallback callback) {
    IMbmsStreamingService streamingService = this.mService.get();
    if (streamingService == null) {
      throw new IllegalStateException("Middleware not yet bound");
    }
        ...
        new ServiceConnection() { // from class: android.telephony.MbmsStreamingSession.3
          ...
          public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            try {
              ...
              Log.m96e(MbmsStreamingSession.LOG_TAG, "Service died before initialization");
              MbmsStreamingSession.this.sendErrorToApp(103, e2.toString());
              MbmsStreamingSession.sIsInitialized.set(false);
            } catch (RuntimeException e3) {
              Log.m96e(MbmsStreamingSession.LOG_TAG, "Runtime exception during initialization");
          ...
          public void onNullBinding(ComponentName name) {
            Log.m102w(
                MbmsStreamingSession.LOG_TAG, "bindAndInitialize: Remote service returned null");
