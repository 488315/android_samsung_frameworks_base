...
public class AccessNetworkUtils {
  ...
  public static int getOperatingBandForUarfcn(int uarfcn) {
    ...
    int[] addlBand4 = {
      SettingsEnums.SECURITY_WARNINGS,
    ...
    int[] addlBand19 = {
      787, 812, MetricsProto.MetricsEvent.WIFI_NETWORK_RECOMMENDATION_CONNECTION_SUCCESS
    };
    int[] addlBand25 = {
      6292,
      6317,
  ...
  */
  public static int getFrequencyFromEarfcn(int band, int earfcn, boolean isUplink) {
    int low = 0;
    int offset = 0;
    AccessNetworkConstants.EutranBandArfcnFrequency[] values =
        AccessNetworkConstants.EutranBandArfcnFrequency.values();
    int length = values.length;
    int i = 0;
    while (true) {
      ...
      if (band != earfcnFrequency.band) {
        ...
        Rlog.m85w(
            TAG,
            "Band and the range of EARFCN are not consistent: band = "
                + band
                + " ,earfcn = "
                + earfcn
                + " ,isUplink = "
  ...
  private static boolean isInEarfcnRange(
    ...
    return isUplink
        ...
        : earfcn >= earfcnFrequency.downlinkOffset && earfcn <= earfcnFrequency.downlinkRange;
  }
