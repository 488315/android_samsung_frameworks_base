...
public class MbmsStreamingServiceBase extends IMbmsStreamingService.Stub {
    ...
    public final int initialize(
            final IMbmsStreamingSessionCallback callback, final int subscriptionId)
            throws RemoteException {
        if (callback == null) {
        ...
        int result =
                initialize(
                        new MbmsStreamingSessionCallback() { // from class:
                                                             // android.telephony.mbms.vendor.MbmsStreamingServiceBase.1
                            ...
                            public void onError(int errorCode, String message) {
                                try {
                                    ...
                                    MbmsStreamingServiceBase.this.onAppCallbackDied(
                                            uid, subscriptionId);
                                }
                            }

                            @Override // android.telephony.mbms.MbmsStreamingSessionCallback
                            public void onStreamingServicesUpdated(
                                    List<StreamingServiceInfo> services) {
                                try {
                    ...
                    .linkToDeath(
                            new IBinder
                                    .DeathRecipient() { // from class:
                                                        // android.telephony.mbms.vendor.MbmsStreamingServiceBase.2
    ...
    public int startStreaming(
            int subscriptionId, String serviceId, StreamingServiceCallback callback)
            throws RemoteException {
        return 0;
    }
    ...
    @Override // android.telephony.mbms.vendor.IMbmsStreamingService
    public int startStreaming(
            final int subscriptionId, String serviceId, final IStreamingServiceCallback callback)
            throws RemoteException {
        if (callback == null) {
            throw new NullPointerException("Callback must not be null");
        }
        ...
        int result =
                startStreaming(
                        subscriptionId,
                        serviceId,
                        new StreamingServiceCallback() { // from class:
                                                         // android.telephony.mbms.vendor.MbmsStreamingServiceBase.3
        ...
        if (result == 0) {
            callback.asBinder()
                    .linkToDeath(
                            new IBinder
                                    .DeathRecipient() { // from class:
                                                        // android.telephony.mbms.vendor.MbmsStreamingServiceBase.4
                                @Override // android.os.IBinder.DeathRecipient
                                public void binderDied() {
                                    MbmsStreamingServiceBase.this.onAppCallbackDied(
                                            uid, subscriptionId);
