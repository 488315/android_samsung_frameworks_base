...
public final class BarringInfo implements Parcelable {
  ...
  public BarringInfo createLocationInfoSanitizedCopy() {
    ...
    return cellIdentity == null
        ? this
        : new BarringInfo(cellIdentity.sanitizeLocationInfo(), this.mBarringServiceInfos);
  }

  public BarringInfo(Parcel p) {
    this.mCellIdentity =
        (CellIdentity) p.readParcelable(CellIdentity.class.getClassLoader(), CellIdentity.class);
  ...
  public boolean equals(Object rhs) {
    ...
    for (int i = 0; i < this.mBarringServiceInfos.size(); i++) {
      if (this.mBarringServiceInfos.keyAt(i) != bi.mBarringServiceInfos.keyAt(i)
          || !Objects.equals(
              this.mBarringServiceInfos.valueAt(i), bi.mBarringServiceInfos.valueAt(i))) {
        return false;
      }
    }
    return true;
