...
public class CbGeoUtils {
  ...
  public static class Polygon implements Geometry {
                  ...
                  .map(
                      new Function() { // from class:
                        ...
                        public final Object apply(Object obj) {
                          ...
                          lambda$new$0 =
                              CbGeoUtils.Polygon.this.lambda$new$0((CbGeoUtils.LatLng) obj);
                          return lambda$new$0;
                        }
                      })
    ...
    public boolean contains(LatLng latLng) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (ccw == 0) {
          if (Math.min(a.f513x, b.f513x) <= p.f513x
              && p.f513x <= Math.max(a.f513x, b.f513x)
              && Math.min(a.f514y, b.f514y) <= p.f514y
              && p.f514y <= Math.max(a.f514y, b.f514y)) {
            return true;
          }
        } else if (CbGeoUtils.sign(a.f514y - p.f514y) <= 0) {
          if (ccw > 0 && CbGeoUtils.sign(b.f514y - p.f514y) > 0) {
            windingNumber++;
          }
        } else if (ccw < 0 && CbGeoUtils.sign(b.f514y - p.f514y) <= 0) {
          windingNumber--;
        }
      }
    ...
    private static double crossProduct(Point a, Point b) {
      return (a.f513x * b.f514y) - (a.f514y * b.f513x);
    }

    static final class Point {

      /* renamed from: x */
      public final double f513x;

      /* renamed from: y */
      public final double f514y;
      ...
      public Point subtract(Point p) {
        return new Point(this.f513x - p.f513x, this.f514y - p.f514y);
      }
    }
    ...
    for (String geometryStr : str.split("\\s*;\\s*")) {
      ...
      switch (c) {
        case 0:
          geometries.add(
              new Circle(
                  parseLatLngFromString(geoParameters[1]), Double.parseDouble(geoParameters[2])));
          break;
        case 1:
          ...
          break;
        default:
          com.android.telephony.Rlog.m240e(TAG, "Invalid geometry format " + geometryStr);
          break;
      }
    }
    return geometries;
                ...
                new Function() { // from class:
                  ...
                  public final Object apply(Object obj) {
                    ...
                    encodeGeometryToString =
                        CbGeoUtils.encodeGeometryToString((CbGeoUtils.Geometry) obj);
  ...
  public static String encodeGeometryToString(Geometry geometry) {
    ...
    if (geometry instanceof Polygon) {
      ...
      com.android.telephony.Rlog.m240e(TAG, "Unsupported geometry object " + geometry);
      return null;
    }
    return sb.toString();
  }
