...
class QuickAccessWalletServiceInfo {
    ...
    static QuickAccessWalletServiceInfo tryCreate(Context context) {
        ...
        if (!Manifest.permission.BIND_QUICK_ACCESS_WALLET_SERVICE.equals(serviceInfo.permission)) {
            Log.m102w(TAG, String.format("%s.%s does not require permission %s", serviceInfo.packageName, serviceInfo.name, Manifest.permission.BIND_QUICK_ACCESS_WALLET_SERVICE));
            return null;
        }
    ...
    private static Drawable parseTileServiceMetadata(Context context, ServiceInfo serviceInfo) {
        ...
        if (tileIconDrawableId != 0) {
            try {
                ...
                Log.m97e(TAG, "Error parsing quickaccesswallet tile service meta-data", e);
            }
        }
        return null;
    }
    ...
    static ServiceMetadata parseServiceMetadata(Context context, ServiceInfo serviceInfo) {
        ...
        try {
            ...
            Log.m97e(TAG, "Error parsing quickaccesswallet service meta-data", e);
        }
        if (TAG_WALLET_SERVICE.equals(parser.getName())) {
            AttributeSet allAttributes = Xml.asAttributeSet(parser);
            TypedArray afsAttributes = null;
            try {
                afsAttributes = resources.obtainAttributes(allAttributes, C4337R.styleable.QuickAccessWalletService);
        ...
        Log.m96e(TAG, "Meta-data does not start with quickaccesswallet-service tag");
        return ServiceMetadata.empty();
    }
