...
class QuickAccessWalletServiceInfo {
  ...
  private QuickAccessWalletServiceInfo(
      ServiceInfo serviceInfo, ServiceMetadata metadata, TileServiceMetadata tileServiceMetadata) {
  ...
  static QuickAccessWalletServiceInfo tryCreate(Context context) {
    ...
    if (defaultPaymentApp == null
        || (serviceInfo = getWalletServiceInfo(context, defaultPaymentApp.getPackageName()))
            == null) {
      return null;
    }
  ...
  static ServiceMetadata parseServiceMetadata(Context context, ServiceInfo serviceInfo) {
    ...
    if (TAG_WALLET_SERVICE.equals(parser.getName())) {
      ...
      try {
        afsAttributes =
            resources.obtainAttributes(allAttributes, C4337R.styleable.QuickAccessWalletService);
