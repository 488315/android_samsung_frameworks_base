...
public class StatusBarNotification implements Parcelable {
    ...
    private String key() {
        String sbnKey = this.user.getIdentifier() + NtpTrustedTime.NTP_SETTING_SERVER_NAME_DELIMITER + this.pkg + NtpTrustedTime.NTP_SETTING_SERVER_NAME_DELIMITER + this.f481id + NtpTrustedTime.NTP_SETTING_SERVER_NAME_DELIMITER + this.tag + NtpTrustedTime.NTP_SETTING_SERVER_NAME_DELIMITER + this.uid;
    ...
    public StatusBarNotification m4292clone() {
        return cloneShallow(this.notification.m866clone());
    }

    StatusBarNotification cloneShallow(Notification notification) {
        StatusBarNotification result = new StatusBarNotification(this.pkg, this.opPkg, this.f481id, this.tag, this.uid, this.initialPid, notification, this.user, this.overrideGroupKey, this.postTime);
        result.setInstanceId(this.mInstanceId);
        return result;
    }

    public String toString() {
        return TextUtils.formatSimple("StatusBarNotification(pkg=%s user=%s id=%d tag=%s key=%s: %s)", this.pkg, this.user, Integer.valueOf(this.f481id), this.tag, this.key, this.notification);
    }
