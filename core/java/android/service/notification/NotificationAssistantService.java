    ...
    public final void adjustNotification(Adjustment adjustment) {
        if (isBound()) {
            try {
                ...
                Log.m101v(TAG, "Unable to contact notification manager", ex);
    ...
    private class NotificationAssistantServiceWrapper extends NotificationListenerService.NotificationListenerWrapper {
        ...
        public void onNotificationEnqueuedWithChannel(IStatusBarNotificationHolder sbnHolder, NotificationChannel channel, NotificationRankingUpdate update) {
            try {
                ...
                if (sbn == null) {
                    Log.m102w(NotificationAssistantService.TAG, "onNotificationEnqueuedWithChannel: Error receiving StatusBarNotification");
                    return;
                }
        ...
        public void onNotificationSnoozedUntilContext(IStatusBarNotificationHolder sbnHolder, String snoozeCriterionId) {
            try {
                StatusBarNotification sbn = sbnHolder.get();
                if (sbn == null) {
                    Log.m102w(NotificationAssistantService.TAG, "onNotificationSnoozed: Error receiving StatusBarNotification");
                    return;
                }
                SomeArgs args = SomeArgs.obtain();
                args.arg1 = sbn;
                ...
                case 1:
                    ...
                    if (adjustment != null) {
                        if (!NotificationAssistantService.this.isBound()) {
                            Log.m102w(NotificationAssistantService.TAG, "MSG_ON_NOTIFICATION_ENQUEUED: service not bound, skip.");
                            return;
                        }
                        try {
                            ...
                            Log.m101v(NotificationAssistantService.TAG, "Unable to contact notification manager", ex);
                            throw ex.rethrowFromSystemServer();
