...
public final class GameSessionTrampolineActivity extends Activity {
  ...
  public static Intent createIntent(
      Intent targetIntent, Bundle options, AndroidFuture<GameSessionActivityResult> resultFuture) {
    Intent trampolineIntent = new Intent();
    trampolineIntent.setComponent(
        new ComponentName("android", "android.service.games.GameSessionTrampolineActivity"));
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    try {
      startActivityAsCaller(
          (Intent) getIntent().getParcelableExtra(INTENT_KEY, Intent.class),
          getIntent().getBundleExtra(OPTIONS_KEY),
          false,
          getUserId(),
          1);
    } catch (Exception e) {
      Slog.m121w(TAG, "Unable to launch activity from game session");
      AndroidFuture<GameSessionActivityResult> future =
          (AndroidFuture) getIntent().getParcelableExtra(FUTURE_KEY, AndroidFuture.class);
