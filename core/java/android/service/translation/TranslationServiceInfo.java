...
public final class TranslationServiceInfo {
    ...
    private TranslationServiceInfo(Context context, ServiceInfo si) {
        if (!Manifest.permission.BIND_TRANSLATION_SERVICE.equals(si.permission)) {
            Slog.m121w(TAG, "TranslationServiceInfo from '" + si.packageName + "' does not require permission " + Manifest.permission.BIND_TRANSLATION_SERVICE);
        ...
        try {
            ...
            if (XML_TAG_SERVICE.equals(parser.getName())) {
                ...
                try {
                    afsAttributes = resources.obtainAttributes(allAttributes, C4337R.styleable.TranslationService);
