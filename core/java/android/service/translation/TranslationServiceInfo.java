...
public final class TranslationServiceInfo {
  ...
  private TranslationServiceInfo(Context context, ServiceInfo si) {
    if (!Manifest.permission.BIND_TRANSLATION_SERVICE.equals(si.permission)) {
      Slog.m121w(
          TAG,
          "TranslationServiceInfo from '"
              + si.packageName
              + "' does not require permission "
              + Manifest.permission.BIND_TRANSLATION_SERVICE);
      throw new SecurityException(
          "Service does not require permission android.permission.BIND_TRANSLATION_SERVICE");
    }
    this.mServiceInfo = si;
    XmlResourceParser parser =
        si.loadXmlMetaData(context.getPackageManager(), TranslationService.SERVICE_META_DATA);
    if (parser == null) {
    ...
    try {
      ...
      if (XML_TAG_SERVICE.equals(parser.getName())) {
        ...
        try {
          afsAttributes =
              resources.obtainAttributes(allAttributes, C4337R.styleable.TranslationService);
        ...
        Log.m96e(TAG, "Meta-data does not start with translation-service tag");
      }
