...
public final class CarrierMessagingServiceWrapper implements AutoCloseable {
    ...
    if (this.mICarrierMessagingService != null) {
      try {
        this.mICarrierMessagingService.filterSms(
            pdu, format, destPort, subId, new CarrierMessagingCallbackInternal(callback, executor));
  ...
  @SystemApi
  public void sendMms(
      Uri pduUri, int subId, Uri location, Executor executor, CarrierMessagingCallback callback) {
    Objects.requireNonNull(this.mICarrierMessagingService);
    try {
      this.mICarrierMessagingService.sendMms(
          pduUri, subId, location, new CarrierMessagingCallbackInternal(callback, executor));
    } catch (RemoteException e) {
      throw new RuntimeException(e);
    }
  }
