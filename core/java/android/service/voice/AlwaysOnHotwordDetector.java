...
public class AlwaysOnHotwordDetector extends AbstractDetector {
        ...
        synchronized (this.mLock) {
            ...
            if (i != -3 && i != -2 && i != 3) {
                if (this.mIsAvailabilityOverriddenByTestApi) {
                    Slog.m121w(TAG, "Suppressing system availability update. Availability is overridden by test API.");
                    return;
                }
                if (i == 2) {
                    try {
            ...
            Slog.m121w(TAG, "Received onSoundModelsChanged for an unsupported keyphrase/config or in the error state");
        }
    }
    ...
    static final class SoundTriggerListener extends IHotwordRecognitionStatusCallback.Stub {
        ...
        public void onGenericSoundTriggerDetected(SoundTrigger.GenericRecognitionEvent event) {
            Slog.m121w(AlwaysOnHotwordDetector.TAG, "Generic sound trigger event detected at AOHD: " + event);
        }
        ...
        public void onHotwordDetectionServiceFailure(HotwordDetectionServiceFailure hotwordDetectionServiceFailure) {
            Slog.m119v(AlwaysOnHotwordDetector.TAG, "onHotwordDetectionServiceFailure: " + hotwordDetectionServiceFailure);
        ...
        public void onVisualQueryDetectionServiceFailure(VisualQueryDetectionServiceFailure visualQueryDetectionServiceFailure) throws RemoteException {
            Slog.m121w(AlwaysOnHotwordDetector.TAG, "onVisualQueryDetectionServiceFailure: " + visualQueryDetectionServiceFailure);
        }
        ...
        public void handleMessage(Message msg) {
            synchronized (AlwaysOnHotwordDetector.this.mLock) {
                if (AlwaysOnHotwordDetector.this.mAvailability == -3) {
                    Slog.m121w(AlwaysOnHotwordDetector.TAG, "Received message: " + msg.what + " for an invalid detector");
                } else {
