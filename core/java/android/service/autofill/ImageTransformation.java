...
public final class ImageTransformation extends InternalTransformation implements Transformation, Parcelable {
    ...
    public void apply(ValueFinder finder, RemoteViews parentTemplate, int childViewId) throws Exception {
        ...
        if (value == null) {
            Log.m102w(TAG, "No view for id " + this.mId);
            return;
        }
        int size = this.mOptions.size();
        if (Helper.sDebug) {
            Log.m94d(TAG, size + " multiple options on id " + childViewId + " to compare against");
        }
        for (int i = 0; i < size; i++) {
            Option option = this.mOptions.get(i);
            try {
                if (option.pattern.matcher(value).matches()) {
                    Log.m94d(TAG, "Found match at " + i + ": " + option);
                ...
                Log.m102w(TAG, "Error matching regex #" + i + NavigationBarInflaterView.KEY_CODE_START + option.pattern + ") on id " + option.resId + ": " + e.getClass());
                throw e;
            }
        }
        if (Helper.sDebug) {
            Log.m94d(TAG, "No match for " + value);
        }
    }
