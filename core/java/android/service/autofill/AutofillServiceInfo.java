    ...
    public AutofillServiceInfo(Context context, ServiceInfo si) {
        if (!Manifest.permission.BIND_AUTOFILL_SERVICE.equals(si.permission)) {
            if (Manifest.permission.BIND_AUTOFILL.equals(si.permission)) {
                Log.m102w(TAG, "AutofillService from '" + si.packageName + "' uses unsupported permission " + Manifest.permission.BIND_AUTOFILL + ". It works for now, but might not be supported on future releases");
                new MetricsLogger().write(new LogMaker(MetricsProto.MetricsEvent.AUTOFILL_INVALID_PERMISSION).setPackageName(si.packageName));
            } else {
                Log.m102w(TAG, "AutofillService from '" + si.packageName + "' does not require permission " + Manifest.permission.BIND_AUTOFILL_SERVICE);
        ...
        try {
            ...
            if (TAG_AUTOFILL_SERVICE.equals(parser.getName())) {
                ...
                try {
                    afsAttributes = resources.obtainAttributes(allAttributes, C4337R.styleable.AutofillService);
            ...
            if (type != 1 && (type != 3 || parser.getDepth() > outerDepth)) {
                if (type != 3 && type != 4 && TAG_COMPATIBILITY_PACKAGE.equals(parser.getName())) {
                    ...
                    try {
                        AttributeSet allAttributes = Xml.asAttributeSet(parser);
                        try {
                            cpAttributes = resources.obtainAttributes(allAttributes, C4337R.styleable.AutofillService_CompatibilityPackage);
