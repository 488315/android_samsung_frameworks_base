...
public final class DateValueSanitizer extends InternalSanitizer implements Sanitizer, Parcelable {
  public static final Parcelable.Creator<DateValueSanitizer> CREATOR =
      new Parcelable.Creator<
          DateValueSanitizer>() { // from class: android.service.autofill.DateValueSanitizer.1
        /* JADX WARN: Can't rename method to resolve collision */
        @Override // android.os.Parcelable.Creator
        public DateValueSanitizer createFromParcel(Parcel parcel) {
          return new DateValueSanitizer(
              (DateFormat)
                  parcel.readSerializable(DateFormat.class.getClassLoader(), DateFormat.class));
        }
  ...
  public AutofillValue sanitize(AutofillValue value) {
    if (value == null) {
      Log.m102w(TAG, "sanitize() called with null value");
      return null;
    }
    if (!value.isDate()) {
      if (Helper.sDebug) {
        Log.m94d(TAG, value + " is not a date");
      }
      return null;
    }
    try {
      ...
      if (Helper.sDebug) {
        Log.m94d(TAG, "Transformed " + date + " to " + converted);
      }
      Date sanitized = this.mDateFormat.parse(converted);
      if (Helper.sDebug) {
        Log.m94d(TAG, "Sanitized to " + sanitized);
      }
