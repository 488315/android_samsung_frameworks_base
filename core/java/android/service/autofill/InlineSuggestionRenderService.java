...
public abstract class InlineSuggestionRenderService extends Service {
    ...
    private final LruCache<InlineSuggestionUiImpl, Boolean> mActiveInlineSuggestions = new LruCache<InlineSuggestionUiImpl, Boolean>(30) { // from class: android.service.autofill.InlineSuggestionRenderService.1
        ...
        public void entryRemoved(boolean evicted, InlineSuggestionUiImpl key, Boolean oldValue, Boolean newValue) {
            if (evicted) {
                Log.m102w(InlineSuggestionRenderService.TAG, "Hit max=30 entries in the cache. Releasing oldest one to make space.");
    ...
    final class InlineSuggestionUiImpl {
        ...
        public /* synthetic */ void lambda$releaseSurfaceControlViewHost$0() {
            ...
            Log.m100v(InlineSuggestionRenderService.TAG, "Removed the inline suggestion from the cache, current size=" + InlineSuggestionRenderService.this.mActiveInlineSuggestions.size());
        }
    ...
    public final IBinder onBind(Intent intent) {
        ...
        Log.m102w(TAG, "Tried to bind to wrong intent (should be android.service.autofill.InlineSuggestionRenderService: " + intent);
        return null;
    }
    ...
    public View onRenderSuggestion(InlinePresentation presentation, int width, int height) {
        Log.m96e(TAG, "service implementation (" + getClass() + " does not implement onRenderSuggestion()");
        return null;
    }
}
