...
public abstract class AugmentedAutofillService extends Service {
    ...
    public final IBinder onBind(Intent intent) {
        ...
        Log.m102w(TAG, "Tried to bind to wrong intent (should be android.service.autofill.augmented.AugmentedAutofillService: " + intent);
        return null;
    }
    ...
    public void handleOnFillRequest(int sessionId, IBinder client, int taskId, ComponentName componentName, AutofillId focusedId, AutofillValue focusedValue, long requestTime, InlineSuggestionsRequest inlineSuggestionsRequest, IFillCallback callback) {
        ...
        if (proxy2 == null) {
            ...
            transport = transport2;
            if (sDebug) {
                Log.m94d(TAG, "Reusing proxy for session " + sessionId);
            }
            iFillCallback = callback;
            cancellationSignal = cancellationSignal2;
    ...
    public void handleOnDestroyAllFillWindowsRequest() {
        ...
        if (sparseArray != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (proxy == null) {
                    Log.m102w(TAG, "No proxy for session " + sessionId);
                    return;
                }
                if (proxy.mCallback != null) {
                    try {
                        ...
                        Log.m97e(TAG, "failed to check current pending request status", e);
                    }
                }
                proxy.destroy();
            }
    ...
    public void handleOnUnbind() {
        ...
        if (sparseArray == null) {
            if (sDebug) {
                Log.m94d(TAG, "onUnbind(): no proxy to destroy");
                return;
            }
            return;
        }
        int size = sparseArray.size();
        if (sDebug) {
            Log.m94d(TAG, "onUnbind(): destroying " + size + " proxies");
        }
        for (int i = 0; i < size; i++) {
            AutofillProxy proxy = this.mAutofillProxies.valueAt(i);
            try {
                ...
                Log.m102w(TAG, "error destroying " + proxy);
            }
        }
    ...
    static final class AutofillProxy {
                ...
                try {
                    ...
                    if (rect == null) {
                        if (AugmentedAutofillService.sDebug) {
                            Log.m94d(AugmentedAutofillService.TAG, "getViewCoordinates(" + this.mFocusedId + ") returned null");
                        }
                        return null;
                    }
                    ...
                    Log.m102w(AugmentedAutofillService.TAG, "Could not get coordinates for " + this.mFocusedId);
                    return null;
                }
            }
        }
        ...
        public void requestShowFillUi(int width, int height, Rect anchorBounds, IAutofillWindowPresenter presenter) throws RemoteException {
            if (this.mCancellationSignal.isCanceled()) {
                if (AugmentedAutofillService.sVerbose) {
                    Log.m100v(AugmentedAutofillService.TAG, "requestShowFillUi() not showing because request is cancelled");
                    return;
                }
                return;
            }
            ...
            synchronized (this.mLock) {
                ...
                if (iFillCallback != null) {
                    try {
                        ...
                        Log.m97e(AugmentedAutofillService.TAG, "failed to check current pending request status", e);
                    }
                    Log.m94d(AugmentedAutofillService.TAG, "mCallback is updated.");
                }
                this.mCallback = callback;
                this.mCancellationSignal = cancellationSignal;
            }
        ...
        void reportResult(List<Dataset> inlineSuggestionsData, Bundle clientState, boolean showingFillWindow) {
            try {
                ...
                Log.m96e(AugmentedAutofillService.TAG, "Error calling back with the inline suggestions data: " + e);
            }
        }
            ...
            synchronized (this.mLock) {
                if (this.mFocusedViewNode == null) {
                    try {
                        ...
                        Log.m96e(AugmentedAutofillService.TAG, "Error getting the ViewNode of the focused view: " + e);
                        return null;
                    }
                }
        ...
        void logEvent(int event) {
            if (AugmentedAutofillService.sVerbose) {
                Log.m100v(AugmentedAutofillService.TAG, "returnAndLogResult(): " + event);
            }
            long duration = -1;
            int type = 0;
            switch (event) {
                case 1:
                    ...
                    if (this.mFirstOnSuccessTime == 0) {
                        ...
                        if (AugmentedAutofillService.sDebug) {
                            Log.m94d(AugmentedAutofillService.TAG, "Service responded nothing in " + TimeUtils.formatDuration(duration));
                            break;
                        }
                    }
                    break;
                case 2:
                    ...
                    if (this.mUiFirstShownTime == 0) {
                        ...
                        if (AugmentedAutofillService.sDebug) {
                            Log.m94d(AugmentedAutofillService.TAG, "UI shown in " + TimeUtils.formatDuration(duration));
                            break;
                        }
                    }
                    break;
                case 3:
                    ...
                    if (this.mUiFirstDestroyedTime == 0) {
                        ...
                        if (AugmentedAutofillService.sDebug) {
                            Log.m94d(AugmentedAutofillService.TAG, "UI destroyed in " + TimeUtils.formatDuration(duration));
                            break;
                        }
                    }
                    break;
                case 4:
                    if (this.mFirstOnSuccessTime == 0) {
                        ...
                        if (AugmentedAutofillService.sDebug) {
                            Log.m94d(AugmentedAutofillService.TAG, "Inline response in " + TimeUtils.formatDuration(duration));
                            break;
                        }
                    }
                    break;
                default:
                    Log.m102w(AugmentedAutofillService.TAG, "invalid event reported: " + event);
                    break;
            }
        ...
        public void destroy() {
            synchronized (this.mLock) {
                if (this.mFillWindow != null) {
                    if (AugmentedAutofillService.sDebug) {
                        Log.m94d(AugmentedAutofillService.TAG, "destroying window");
                    }
