...
public final class FillWindow implements AutoCloseable {
    ...
    public boolean update(PresentationParams.Area area, View rootView, long flags) {
        if (AugmentedAutofillService.sDebug) {
            Log.m94d(TAG, "Updating " + area + " + with " + rootView);
        }
        ...
        if (smartSuggestion == null) {
            Log.m102w(TAG, "No SmartSuggestionParams");
            return false;
        }
        ...
        synchronized (this.mLock) {
            ...
            if (AugmentedAutofillService.sDebug) {
                Log.m94d(TAG, "Created FillWindow: params= " + smartSuggestion + " view=" + rootView);
            }
    ...
    public /* synthetic */ boolean lambda$update$0(View view, MotionEvent motionEvent) {
        if (motionEvent.getAction() == 4) {
            if (AugmentedAutofillService.sVerbose) {
                Log.m100v(TAG, "Outside touch detected, hiding the window");
            }
            hide();
            return false;
        }
        ...
        synchronized (this.mLock) {
            if (this.mWm != null && this.mFillView != null) {
                try {
                    ...
                    if (AugmentedAutofillService.sDebug) {
                        Log.m94d(TAG, "Filed with token " + p.token + " gone.");
                    }
                } catch (IllegalStateException e2) {
                    if (AugmentedAutofillService.sDebug) {
                        Log.m94d(TAG, "Exception showing window.");
                    }
                }
            }
        }
        ...
        synchronized (this.mLock) {
            ...
            if (windowManager != null && (view = this.mFillView) != null && this.mShowing) {
                try {
                    ...
                    if (AugmentedAutofillService.sDebug) {
                        Log.m94d(TAG, "Exception hiding window.");
                    }
                }
            }
        }
    ...
    public void destroy() {
        if (AugmentedAutofillService.sDebug) {
            Log.m94d(TAG, "destroy(): mDestroyed=" + this.mDestroyed + " mShowing=" + this.mShowing + " mFillView=" + this.mFillView);
        }
        synchronized (this.mLock) {
            if (this.mDestroyed) {
                return;
    ...
    static final class FillWindowPresenter extends IAutofillWindowPresenter.Stub {
        ...
        public void show(WindowManager.LayoutParams p, Rect transitionEpicenter, boolean fitsSystemWindows, int layoutDirection) {
            if (AugmentedAutofillService.sDebug) {
                Log.m94d(FillWindow.TAG, "FillWindowPresenter.show()");
            }
        ...
        public void hide(Rect transitionEpicenter) {
            if (AugmentedAutofillService.sDebug) {
                Log.m94d(FillWindow.TAG, "FillWindowPresenter.hide()");
            }
            FillWindow fillWindow = this.mFillWindowReference.get();
            if (fillWindow != null) {
