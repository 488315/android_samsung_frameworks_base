...
public final class UserData implements FieldClassificationUserData, Parcelable {
          ...
          if (size > 0) {
            for (int i2 = 0; i2 < size; i2++) {
              ...
              builder.setFieldClassificationAlgorithmForCategory(
                  categoryId, categoryAlgorithms.valueAt(i2), categoryArgs.get(categoryId));
            }
          }
  ...
  public static final class Builder {
    ...
    private void addMapping(String value, String categoryId) {
      ...
      if (this.mUniqueValueCategoryPairs.contains(pair)) {
        Log.m102w(UserData.TAG, "Ignoring entry with same value / category");
        return;
      }
  ...
  private static int getInt(String settings, int defaultValue) {
    ...
    if (cr == null) {
      Log.m102w(TAG, "Could not read from " + settings + "; hardcoding " + defaultValue);
      return defaultValue;
    }
