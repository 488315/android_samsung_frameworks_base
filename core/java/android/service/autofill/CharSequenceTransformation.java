...
public final class CharSequenceTransformation extends InternalTransformation implements Transformation, Parcelable {
    ...
    public void apply(ValueFinder finder, RemoteViews parentTemplate, int childViewId) throws Exception {
        ...
        if (Helper.sDebug) {
            Log.m94d(TAG, size + " fields on id " + childViewId);
        }
        for (Map.Entry<AutofillId, Pair<Pattern, String>> entry : this.mFields.entrySet()) {
            ...
            if (value == null) {
                Log.m102w(TAG, "No value for id " + id);
                return;
            }
            try {
                ...
                if (!matcher.find()) {
                    if (Helper.sDebug) {
                        Log.m94d(TAG, "Match for " + field.first + " failed on id " + id);
                        return;
                    }
                    return;
                }
                ...
                Log.m102w(TAG, "Cannot apply " + field.first.pattern() + Session.SUBSESSION_SEPARATION_CHAR + field.second + " to field with autofill id" + id + ": " + e.getClass());
                throw e;
            }
        }
        Log.m94d(TAG, "Converting text on child " + childViewId + " to " + converted.length() + "_chars");
