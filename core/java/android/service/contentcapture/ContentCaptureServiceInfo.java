...
public final class ContentCaptureServiceInfo {
    ...
    private ContentCaptureServiceInfo(Context context, ServiceInfo si) {
        if (!Manifest.permission.BIND_CONTENT_CAPTURE_SERVICE.equals(si.permission)) {
            Slog.m121w(TAG, "ContentCaptureService from '" + si.packageName + "' does not require permission " + Manifest.permission.BIND_CONTENT_CAPTURE_SERVICE);
        ...
        try {
            ...
            if (XML_TAG_SERVICE.equals(parser.getName())) {
                ...
                try {
                    afsAttributes = resources.obtainAttributes(allAttributes, C4337R.styleable.ContentCaptureService);
