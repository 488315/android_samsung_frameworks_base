  ...
  private ContentCaptureServiceInfo(Context context, ServiceInfo si) {
    if (!Manifest.permission.BIND_CONTENT_CAPTURE_SERVICE.equals(si.permission)) {
      ...
      throw new SecurityException(
          "Service does not require permission android.permission.BIND_CONTENT_CAPTURE_SERVICE");
    }
    this.mServiceInfo = si;
    XmlResourceParser parser =
        si.loadXmlMetaData(context.getPackageManager(), ContentCaptureService.SERVICE_META_DATA);
    if (parser == null) {
    ...
    try {
      ...
      if (XML_TAG_SERVICE.equals(parser.getName())) {
        ...
        try {
          afsAttributes =
              resources.obtainAttributes(allAttributes, C4337R.styleable.ContentCaptureService);
        ...
        Log.m96e(TAG, "Meta-data does not start with content-capture-service tag");
      }
