...
public abstract class ContentCaptureService extends Service {
    ...
    public final IBinder onBind(Intent intent) {
        ...
        Log.m102w(TAG, "Tried to bind to wrong intent (should be android.service.contentcapture.ContentCaptureService: " + intent);
        return null;
    }
    ...
    public void onCreateContentCaptureSession(ContentCaptureContext context, ContentCaptureSessionId sessionId) {
        if (ContentCaptureHelper.sVerbose) {
            Log.m100v(TAG, "onCreateContentCaptureSession(id=" + sessionId + ", ctx=" + context + NavigationBarInflaterView.KEY_CODE_END);
        }
    }

    public void onContentCaptureEvent(ContentCaptureSessionId sessionId, ContentCaptureEvent event) {
        if (ContentCaptureHelper.sVerbose) {
            Log.m100v(TAG, "onContentCaptureEventsRequest(id=" + sessionId + NavigationBarInflaterView.KEY_CODE_END);
        }
    }
    ...
    public void onDestroyContentCaptureSession(ContentCaptureSessionId sessionId) {
        if (ContentCaptureHelper.sVerbose) {
            Log.m100v(TAG, "onDestroyContentCaptureSession(id=" + sessionId + NavigationBarInflaterView.KEY_CODE_END);
        }
    }
    ...
    private boolean handleIsRightCallerFor(ContentCaptureEvent event, int uid) {
        ...
        if (this.mSessionUids.indexOfKey(sessionId) < 0) {
            if (ContentCaptureHelper.sVerbose) {
                Log.m100v(TAG, "handleIsRightCallerFor(" + event + "): no session for " + sessionId + ": " + this.mSessionUids);
            }
            return false;
        }
        int rightUid = this.mSessionUids.get(sessionId);
        if (rightUid != uid) {
            Log.m96e(TAG, "invalid call from UID " + uid + ": session " + sessionId + " belongs to " + rightUid);
    ...
    static class DataShareReadAdapterDelegate extends IDataShareReadAdapter.Stub {
        ...
        private void executeAdapterMethodLocked(final Consumer<DataShareReadAdapter> adapterFn, String methodName) {
            ...
            if (resourceManager == null) {
                Slog.m121w(ContentCaptureService.TAG, "Can't execute " + methodName + "(), resource manager has been GC'ed");
                return;
            }
            ...
            if (adapter == null || executor == null) {
                Slog.m121w(ContentCaptureService.TAG, "Can't execute " + methodName + "(), references are null");
                return;
            }
        ...
        private void clearHardReferences() {
            LocalDataShareAdapterResourceManager resourceManager = this.mResourceManagerReference.get();
            if (resourceManager == null) {
                Slog.m121w(ContentCaptureService.TAG, "Can't clear references, resource manager has been GC'ed");
            } else {
