...
public class TileService extends Service {
  ...
  private final HandlerC2769H mHandler = new HandlerC2769H(Looper.getMainLooper());
  ...
  public final void startActivityAndCollapse(Intent intent) {
    if (CompatChanges.isChangeEnabled(START_ACTIVITY_NEEDS_PENDING_INTENT)) {
      throw new UnsupportedOperationException(
          "startActivityAndCollapse: Starting activity from TileService using an Intent is not"
              + " allowed.");
    }
  ...
  public final Tile getQsTile() {
    ...
    IBinder iBinder;
    if (this.mTile == null
        && (iQSService = this.mService) != null
        && (iBinder = this.mTileToken) != null) {
      try {
        this.mTile = iQSService.getTile(iBinder);
      } catch (RemoteException e) {
        if (e instanceof DeadObjectException) {
          Log.m94d(TAG, "getQsTile : Unable to reach IQSService : " + e);
        } else {
          throw new RuntimeException("getQsTile : Unable to reach IQSService", e);
        }
      }
      Log.m94d(TAG, "getQsTile : mTile = " + this.mTile);
  ...
  public IBinder onBind(Intent intent) {
    ...
    try {
      ...
      Log.m94d(TAG, "onBind : mTile = " + this.mTile);
      Tile tile = this.mTile;
      if (tile != null) {
        tile.setService(this.mService, this.mTileToken);
        this.mHandler.sendEmptyMessage(7);
  ...
  private class HandlerC2769H extends Handler {
    ...
    public HandlerC2769H(Looper looper) {
      super(looper);
      this.mTileServiceName = TileService.this.getClass().getSimpleName();
    }

    private void logMessage(String message) {
      Log.m94d(TileService.TAG, this.mTileServiceName + " Handler - " + message);
    }
  ...
  public static boolean isQuickSettingsSupported() {
    return Resources.getSystem().getBoolean(C4337R.bool.config_quickSettingsSupported);
  }

  public static final void requestListeningState(Context context, ComponentName component) {
    StatusBarManager sbm = (StatusBarManager) context.getSystemService(StatusBarManager.class);
    if (sbm == null) {
      Log.m96e(TAG, "No StatusBarManager service found");
    } else {
