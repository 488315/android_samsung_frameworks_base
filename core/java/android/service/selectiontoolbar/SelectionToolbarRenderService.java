...
public abstract class SelectionToolbarRenderService extends Service {
    ...
    class BinderC27931 extends ISelectionToolbarRenderService.Stub {
        ...
        public void onShow(int callingUid, ShowInfo showInfo, ISelectionToolbarCallback callback) {
            if (SelectionToolbarRenderService.this.mCache.indexOfKey(callingUid) < 0) {
                SelectionToolbarRenderService.this.mCache.put(callingUid, new Pair(new RemoteCallbackWrapper(callback), SelectionToolbarRenderService.this.new CleanCacheRunnable(callingUid)));
            }
    ...
    public final IBinder onBind(Intent intent) {
        ...
        Log.m102w(TAG, "Tried to bind to wrong intent (should be android.service.selectiontoolbar.SelectionToolbarRenderService: " + intent);
        return null;
    }
    ...
    private class CleanCacheRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (toolbarPair != null) {
                Log.m102w(SelectionToolbarRenderService.TAG, "CleanCacheRunnable: remove " + this.mCleanUid + " from cache.");
