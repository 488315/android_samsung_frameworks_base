...
public final class DefaultSelectionToolbarRenderService extends SelectionToolbarRenderService {
  ...
  public void onShow(
    ...
    if (!canShowToolbar(callingUid, showInfo)) {
      Slog.m115e(TAG, "Do not allow multiple toolbar for the app.");
    ...
    if (this.mToolbarCache.indexOfKey(callingUid) < 0) {
      RemoteSelectionToolbar toolbar =
          new RemoteSelectionToolbar(
              this,
              widgetToken,
              showInfo,
              callbackWrapper,
              new SelectionToolbarRenderService
                  .TransferTouchListener() { // from class:
                                             // android.service.selectiontoolbar.DefaultSelectionToolbarRenderService$$ExternalSyntheticLambda0
    ...
    Slog.m119v(TAG, "onShow() for " + widgetToken);
    ...
    if (toolbarPair.first.longValue() == widgetToken) {
      ...
      Slog.m121w(TAG, "onShow() for unknown " + widgetToken);
    }
  }
  ...
  public void onHide(long widgetToken) {
    ...
    if (toolbar != null) {
      Slog.m119v(TAG, "onHide() for " + widgetToken);
  ...
  public void onDismiss(long widgetToken) {
    RemoteSelectionToolbar toolbar = getRemoteSelectionToolbarByTokenLocked(widgetToken);
    if (toolbar != null) {
      Slog.m119v(TAG, "onDismiss() for " + widgetToken);
  ...
  public void onToolbarShowTimeout(int callingUid) {
    Slog.m121w(TAG, "onToolbarShowTimeout for callingUid = " + callingUid);
    Pair<Long, RemoteSelectionToolbar> toolbarPair = this.mToolbarCache.get(callingUid);
    if (toolbarPair != null) {
