...
public class AppWidgetHost {
  ...
  static class Callbacks extends IAppWidgetHost.Stub {
    ...
    public void updateAppWidget(int appWidgetId, RemoteViews views) {
      if (isLocalBinder() && views != null) {
        views = views.mo897clone();
      }
      Handler handler = this.mWeakHandler.get();
      if (handler == null) {
        Log.m94d(AppWidgetHost.TAG, "Handler is not available.");
      } else {
  ...
  private static void bindService(Context context) {
    synchronized (sServiceLock) {
      ...
      if (packageManager.hasSystemFeature(PackageManager.FEATURE_APP_WIDGETS)
          || context.getResources().getBoolean(C4337R.bool.config_enableAppWidgetService)) {
  ...
  public void startListening() {
    ...
    try {
      List<PendingHostUpdate> updates =
          sService
              .startListening(
                  this.mCallbacks, this.mContextOpPackageName, this.mHostId, idsToUpdate)
              .getList();
      int N = updates.size();
  ...
  public final void startAppWidgetConfigureActivityForResult(
    ...
    try {
      IntentSender intentSender =
          iAppWidgetService.createAppWidgetConfigIntentSender(
  ...
  public final void semStartAppWidgetConfigureActivityForResult(
    ...
    try {
      IntentSender intentSender =
          iAppWidgetService.semCreateAppWidgetConfigIntentSender(
              this.mContextOpPackageName, appWidgetId, intentFlags);
      if (intentSender != null) {
        activity.startIntentSenderForResult(
            intentSender, requestCode, (Intent) null, 0, 0, 0, options);
  ...
  public void deleteAppWidgetId(int appWidgetId) {
    ...
    try {
      Log.m94d(TAG, "deleteAppWidgetId() appWidgetId = " + appWidgetId);
  ...
  void updateAppWidgetView(int appWidgetId, RemoteViews views) {
    AppWidgetHostListener v = getListener(appWidgetId);
    Log.m98i(TAG, "updateAppWidgetView, appWidgetId = " + appWidgetId + ", v = " + v);
    if (v != null) {
  ...
  void viewDataChanged(int appWidgetId, int viewId) {
    AppWidgetHostListener v = getListener(appWidgetId);
    Log.m98i(TAG, "viewDataChanged, appWidgetId = " + appWidgetId + ", v = " + v);
    if (v != null) {
