...
public class FileA3D extends BaseObj {
  ...
  public static class IndexEntry {
    ...
    static synchronized BaseObj internalCreate(RenderScript rs, IndexEntry entry) {
      synchronized (IndexEntry.class) {
        ...
        switch (C25221.$SwitchMap$android$renderscript$FileA3D$EntryType[
            entry.mEntryType.ordinal()]) {
          case 1:
  ...
  private void initEntries() {
    ...
    for (int i = 0; i < numFileEntries; i++) {
      this.mFileEntries[i] =
          new IndexEntry(this.mRS, i, getID(this.mRS), names[i], EntryType.toEntryType(ids[i]));
    }
  }
