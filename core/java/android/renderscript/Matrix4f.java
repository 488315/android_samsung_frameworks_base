...
public class Matrix4f {
  ...
  private float computeCofactor(int i, int j) {
    ...
    float minor =
        ((fArr[(r0 * 4) + c0]
                    * ((fArr[(r1 * 4) + c1] * fArr[(r2 * 4) + c2])
                        - (fArr[(r2 * 4) + c1] * fArr[(r1 * 4) + c2])))
                - (fArr[(r1 * 4) + c0]
                    * ((fArr[(r0 * 4) + c1] * fArr[(r2 * 4) + c2])
                        - (fArr[(r2 * 4) + c1] * fArr[(r0 * 4) + c2]))))
            + (fArr[(r2 * 4) + c0]
                * ((fArr[(r0 * 4) + c1] * fArr[(r1 * 4) + c2])
                    - (fArr[(r1 * 4) + c1] * fArr[(r0 * 4) + c2])));
    if (((i + j) & 1) == 0) {
      return minor;
    }
  ...
  public boolean inverse() {
    ...
    float det =
        (f * fArr2[0]) + (fArr[4] * fArr2[1]) + (fArr[8] * fArr2[2]) + (fArr[12] * fArr2[3]);
  ...
  public boolean inverseTranspose() {
    ...
    float[] fArr2 = result.mMat;
    float det =
        (f * fArr2[0]) + (fArr[4] * fArr2[4]) + (fArr[8] * fArr2[8]) + (fArr[12] * fArr2[12]);
    if (Math.abs(det) < 1.0E-6d) {
      return false;
    }
    float det2 = 1.0f / det;
