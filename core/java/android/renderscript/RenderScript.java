...
public class RenderScript {
  ...
  native long rsnClosureCreate(
      long j, long j2, long j3, long[] jArr, long[] jArr2, int[] iArr, long[] jArr3, long[] jArr4);
    ...
    if (!SystemProperties.getBoolean("config.disable_renderscript", false)) {
      try {
        ...
        try {
          ...
          Log.m96e(LOG_TAG, "Error loading RS jni library: " + e);
          throw new RSRuntimeException("Error loading RS jni library: " + e);
        }
      } catch (Exception e2) {
        Log.m96e(LOG_TAG, "Error loading GC methods: " + e2);
  ...
  synchronized void nAllocationData2D(
    ...
    validate();
    rsnAllocationData2D(
        this.mContext, id, xoff, yoff, mip, face, w, h, d, sizeBytes, dt.mID, mSize, usePadding);
  }
      ...
      while (this.mRun) {
        ...
        if (msg == 4) {
          ...
          if (this.mRS.mErrorCallback != null) {
            ...
            Log.m96e(RenderScript.LOG_TAG, "non fatal RS error, " + e);
          }
        } else if (msg != 5) {
          try {
            sleep(1L, 0);
  ...
  private static RenderScript internalCreate(
    ...
    if (!sInitialized) {
      Log.m96e(LOG_TAG, "RenderScript.create() called when disabled; someone is likely to crash");
      return null;
    }
    if ((flags & (-15)) != 0) {
  ...
  private void helpDestroy() {
    ...
    if (shouldDestroy) {
      ...
      if (interrupted) {
        Log.m100v(LOG_TAG, "Interrupted during wait for MessageThread to join");
