...
public final class ScriptGroup extends BaseObj {
  ...
  static class C2525IO {
    Allocation mAllocation;
    Script.KernelID mKID;

    C2525IO(Script.KernelID s) {
      this.mKID = s;
    }
  }
  ...
  public static final class Closure extends BaseObj {
    ...
    Closure(
      ...
      long id =
          rs.nInvokeClosureCreate(invokeID.getID(rs), this.mFP.getData(), fieldIDs, values, sizes);
      setID(id);
      this.guard.open("destroy");
    }
  ...
  ScriptGroup(
      RenderScript rs, String name, List<Closure> closures, List<Input> inputs, Future[] outputs) {
    super(0L, rs);
    this.mName = name;
  ...
  public Object[] execute(Object... inputs) {
    ...
    for (int i = 0; i < this.mInputs2.size(); i++) {
      ...
      if ((obj instanceof Future) || (obj instanceof Input)) {
        Log.m96e(TAG, toString() + ": input " + i + " is a future or unbound value");
        return null;
      }
  ...
  public void setInput(Script.KernelID s, Allocation a) {
    int ct = 0;
    while (true) {
      C2525IO[] c2525ioArr = this.mInputs;
      if (ct < c2525ioArr.length) {
        if (c2525ioArr[ct].mKID != s) {
          ct++;
        } else {
  ...
  public void setOutput(Script.KernelID s, Allocation a) {
    int ct = 0;
    while (true) {
      C2525IO[] c2525ioArr = this.mOutputs;
      if (ct < c2525ioArr.length) {
        if (c2525ioArr[ct].mKID != s) {
          ct++;
        } else {
  ...
  public static final class Builder {
    ...
    public ScriptGroup create() {
      ...
      validateDAG();
      ArrayList<C2525IO> inputs = new ArrayList<>();
      ArrayList<C2525IO> outputs = new ArrayList<>();
      ...
      for (int ct2 = 0; ct2 < this.mNodes.size(); ct2++) {
        ...
        while (ct22 < n.mKernels.size()) {
          ...
          if (!hasInput) {
            inputs.add(new C2525IO(kid));
          }
          if (!hasOutput) {
            outputs.add(new C2525IO(kid));
          }
          ct22++;
          idx = idx2;
        }
  ...
  public static final class Builder2 {
    ...
    public ScriptGroup create(String name, Future... outputs) {
      if (name == null
          ...
          || name.length() > 100
          || !name.equals(
              name.replaceAll("[^a-zA-Z0-9-]", Session.SESSION_SEPARATION_CHAR_CHILD))) {
    ...
    private boolean seperateArgsAndBindings(
        Object[] argsAndBindings, ArrayList<Object> args, Map<Script.FieldID, Object> bindingMap) {
      int i = 0;
