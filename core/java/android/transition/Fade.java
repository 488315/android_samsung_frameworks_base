...
public class Fade extends Visibility {
  ...
  /* renamed from: IN */
  public static final int f527IN = 1;
  ...
  public Fade(Context context, AttributeSet attrs) {
    super(context, attrs);
    TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.Fade);
  ...
  public void captureStartValues(TransitionValues transitionValues) {
    ...
    transitionValues.values.put(
        PROPNAME_TRANSITION_ALPHA, Float.valueOf(transitionValues.view.getTransitionAlpha()));
  }
  ...
  private Animator createAnimation(final View view, float startAlpha, float endAlpha) {
    ...
    if (DBG) {
      Log.m94d(LOG_TAG, "Created animator " + anim);
    }
    FadeAnimatorListener listener = new FadeAnimatorListener(view);
    anim.addListener(listener);
    addListener(
        new TransitionListenerAdapter() { // from class: android.transition.Fade.1
  ...
  public Animator onAppear(
    ...
    if (DBG) {
      ...
      Log.m94d(
          LOG_TAG,
          "Fade.onAppear: startView, startVis, endView, endVis = " + startView + ", " + view);
    }
  ...
  private static float getStartAlpha(TransitionValues startValues, float fallbackValue) {
    ...
    if (startValues == null
        || (startAlphaFloat = (Float) startValues.values.get(PROPNAME_TRANSITION_ALPHA)) == null) {
