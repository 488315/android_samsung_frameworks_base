...
public final class Scene {
    ...
    public static Scene getSceneForLayout(ViewGroup sceneRoot, int layoutId, Context context) {
        SparseArray<Scene> scenes = (SparseArray) sceneRoot.getTag(C4337R.id.scene_layoutid_cache);
        if (scenes == null) {
            scenes = new SparseArray<>();
            sceneRoot.setTagInternal(C4337R.id.scene_layoutid_cache, scenes);
        }
    ...
    static void setCurrentScene(ViewGroup sceneRoot, Scene scene) {
        sceneRoot.setTagInternal(C4337R.id.current_scene, scene);
    }

    public static Scene getCurrentScene(ViewGroup sceneRoot) {
        return (Scene) sceneRoot.getTag(C4337R.id.current_scene);
    }
