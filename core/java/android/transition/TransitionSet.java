...
public class TransitionSet extends Transition {
  ...
  public TransitionSet(Context context, AttributeSet attrs) {
    ...
    this.mStarted = false;
    this.mChangeFlags = 0;
    TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.TransitionSet);
    ...
    if (!this.mPlayTogether) {
      for (int i = 1; i < numTransitions; i++) {
        ...
        previousTransition.addListener(
            new TransitionListenerAdapter() { // from class: android.transition.TransitionSet.1
              @Override // android.transition.TransitionListenerAdapter,
                        // android.transition.Transition.TransitionListener
              public void onTransitionEnd(Transition transition) {
  ...
  public TransitionSet mo5406clone() {
    TransitionSet clone = (TransitionSet) super.mo5406clone();
    clone.mTransitions = new ArrayList<>();
    int numTransitions = this.mTransitions.size();
    for (int i = 0; i < numTransitions; i++) {
      clone.addTransitionInternal(this.mTransitions.get(i).mo5406clone());
    }
    return clone;
  }
}
