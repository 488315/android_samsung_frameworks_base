...
public class TransitionInflater {
  ...
  */
  private Transition createTransitionFromXml(
      XmlPullParser parser, AttributeSet attrs, Transition parent)
    ...
    while (true) {
      ...
      if ((type != 3 || parser.getDepth() > depth) && type != 1) {
        if (type == 2) {
          ...
          } else if (SemLocationManager.GEOFENCE_TRANSITION.equals(name)) {
            transition =
                (Transition)
                    createCustom(attrs, Transition.class, SemLocationManager.GEOFENCE_TRANSITION);
  ...
  private Object createCustom(AttributeSet attrs, Class expectedType, String tag) {
    ...
    try {
      ...
      synchronized (arrayMap) {
        ...
        if (constructor == null
            && (c = this.mContext.getClassLoader().loadClass(className).asSubclass(expectedType))
                != null) {
      ...
      if ((type != 3 || parser.getDepth() > depth) && type != 1) {
        if (type == 2) {
          ...
          if (name.equals("target")) {
            TypedArray a =
                this.mContext.obtainStyledAttributes(attrs, C4337R.styleable.TransitionTarget);
  ...
  */
  /*
      Code decompiled incorrectly, please refer to instructions dump.
  */
  private TransitionManager createTransitionManagerFromXml(
      XmlPullParser parser, AttributeSet attrs, ViewGroup sceneRoot)
      throws XmlPullParserException, IOException {
    int depth = parser.getDepth();
      ...
      if ((type != 3 || parser.getDepth() > depth) && type != 1) {
        if (type == 2) {
          ...
          if (name.equals("transitionManager")) {
            ...
            if (!name.equals(SemLocationManager.GEOFENCE_TRANSITION) || transitionManager == null) {
              break;
            }
  ...
  private void loadTransition(
    ...
    TypedArray a = this.mContext.obtainStyledAttributes(attrs, C4337R.styleable.TransitionManager);
