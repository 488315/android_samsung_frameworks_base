...
public class VintfObject {
    ...
    public static int verify(String[] packageInfo) {
        if (packageInfo == null || packageInfo.length <= 0) {
            Slog.m121w(LOG_TAG, "VintfObject.verify() is deprecated. Call verifyWithoutAvb() instead.");
            return verifyWithoutAvb();
        }
        Slog.m121w(LOG_TAG, "VintfObject.verify() with non-empty packageInfo is deprecated. Skipping compatibility checks for update package.");
        return 0;
    }
