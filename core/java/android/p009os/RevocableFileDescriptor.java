...
public class RevocableFileDescriptor {
  ...
  private final ProxyFileDescriptorCallback mCallback =
      new ProxyFileDescriptorCallback() { // from class: android.os.RevocableFileDescriptor.1
        ...
        public int onRead(long offset, int size, byte[] data) throws ErrnoException {
          ...
          while (n < size) {
            try {
              return n
                  + Os.pread(RevocableFileDescriptor.this.mInner, data, n, size - n, offset + n);
        ...
        public int onWrite(long offset, int size, byte[] data) throws ErrnoException {
          ...
          while (n < size) {
            try {
              return n
                  + Os.pwrite(RevocableFileDescriptor.this.mInner, data, n, size - n, offset + n);
            } catch (InterruptedIOException e) {
              n += e.bytesTransferred;
            }
          }
        ...
        public void onFsync() throws ErrnoException {
          Slog.m119v(RevocableFileDescriptor.TAG, "onFsync()");
          checkRevoked();
        ...
        public void onRelease() {
          Slog.m119v(RevocableFileDescriptor.TAG, "onRelease()");
  ...
  public void init(Context context, FileDescriptor fd) throws IOException {
    ...
    this.mOuter =
        ((StorageManager) context.getSystemService(StorageManager.class))
            .openProxyFileDescriptor(805306368, this.mCallback);
  }
