...
public final class SharedMemory implements Parcelable, Closeable {
  ...
  private SharedMemory(FileDescriptor fileDescriptor) {
    ...
    this.mCleaner =
        Cleaner.create(fileDescriptor, new Closer(fileDescriptor.getInt$(), memoryRegistration));
  }
  ...
  public ByteBuffer map(int prot, int offset, int length) throws ErrnoException {
    ...
    if (length > 0) {
      ...
      long address =
          Os.mmap(0L, length, prot, OsConstants.MAP_SHARED, this.mFileDescriptor, offset);
