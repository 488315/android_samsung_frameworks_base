...
import android.p009os.vibrator.VibrationConfig;
import android.p009os.vibrator.VibratorFrequencyProfile;
...
public abstract class Vibrator {
    ...
    public void vibrate(long milliseconds, AudioAttributes attributes) {
        try {
            ...
            Log.m97e(TAG, "Failed to create VibrationEffect", iae);
        }
    }
    ...
    public void vibrate(long[] pattern, int repeat, AudioAttributes attributes) {
        if (repeat < -1 || repeat >= pattern.length) {
            Log.m96e(TAG, "vibrate called with repeat index out of bounds (pattern.length=" + pattern.length + ", index=" + repeat + NavigationBarInflaterView.KEY_CODE_END);
