...
public class UpdateLock {
  ...
  private static void checkService() {
    if (sService == null) {
      sService =
          IUpdateLock.Stub.asInterface(ServiceManager.getService(Context.UPDATE_LOCK_SERVICE));
    }
  }
  ...
  private void acquireLocked() {
    ...
    if (iUpdateLock != null) {
      try {
        ...
        Log.m96e(TAG, "Unable to contact service to acquire");
      }
    }
    this.mHeld = true;
  }
  ...
  private void releaseLocked() {
    ...
    if (iUpdateLock != null) {
      try {
        ...
        Log.m96e(TAG, "Unable to contact service to release");
      }
    }
    this.mHeld = false;
    if (this.mCount < 0) {
