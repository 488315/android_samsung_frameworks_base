        ...
        synchronized (this.mLock) {
            ...
            for (int i = 0; i < vibratorIds.length; i++) {
                ...
                if (vibrator instanceof NullVibrator) {
                    Log.m102w(TAG, "Vibrator manager service not ready; Info not yet available for vibrator: " + vibratorIds[i]);
                    return VibratorInfo.EMPTY_VIBRATOR_INFO;
                }
        ...
        private static VibratorInfo.FrequencyProfile frequencyProfileIntersection(VibratorInfo[] infos) {
            ...
            for (VibratorInfo info : infos) {
                ...
                if (vibratorStartIdx < 0 || vibratorEndIdx >= vibratorMaxAmplitudes.length) {
                    Slog.m121w(SystemVibrator.TAG, "Error calculating the intersection of vibrator frequency profiles: attempted to fetch from vibrator " + info.getId() + " max amplitude with bad index " + vibratorStartIdx);
