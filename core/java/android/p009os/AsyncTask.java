...
public abstract class AsyncTask<Params, Progress, Result> {
  ...
  static {
    ...
    RejectedExecutionHandler rejectedExecutionHandler =
        new RejectedExecutionHandler() { // from class: android.os.AsyncTask.2
          ...
          public void rejectedExecution(Runnable r, ThreadPoolExecutor e) {
            Log.m102w(AsyncTask.LOG_TAG, "Exceeded ThreadPoolExecutor pool size");
            synchronized (this) {
    ...
    ThreadPoolExecutor threadPoolExecutor =
        new ThreadPoolExecutor(1, 20, 3L, TimeUnit.SECONDS, new SynchronousQueue(), threadFactory);
  ...
  public AsyncTask(Looper callbackLooper) {
    ...
    this.mFuture =
        new FutureTask<Result>(workerRunnable) { // from class: android.os.AsyncTask.4
          @Override // java.util.concurrent.FutureTask
          protected void done() {
            try {
              AsyncTask.this.postResultIfNotInvoked(get());
            } catch (InterruptedException e) {
              Log.m104w(AsyncTask.LOG_TAG, e);
            } catch (CancellationException e2) {
              AsyncTask.this.postResultIfNotInvoked(null);
            } catch (ExecutionException e3) {
              throw new RuntimeException(
                  "An error occurred while executing doInBackground()", e3.getCause());
            }
          }
        };
  }
  ...
  public final AsyncTask<Params, Progress, Result> executeOnExecutor(
    ...
    if (this.mStatus != Status.PENDING) {
      switch (C22925.$SwitchMap$android$os$AsyncTask$Status[this.mStatus.ordinal()]) {
        case 1:
