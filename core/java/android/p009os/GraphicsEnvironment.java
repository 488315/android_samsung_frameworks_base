    ...
    */
    ...
    private boolean isAngleEnabledByGameMode(Context context, String packageName) {
        try {
            ...
            Log.m96e(TAG, "Caught exception while querying GameManagerService if ANGLE is enabled for package: " + packageName);
            return false;
        }
    }
    ...
    private boolean shouldUseAngleInternal(Context context, Bundle bundle, String packageName) {
        ...
        if (optInPackages.size() != optInValues.size()) {
            Log.m100v(TAG, "Global.Settings values are invalid: number of packages: " + optInPackages.size() + ", number of values: " + optInValues.size());
    ...
    private boolean chooseDriver(Context context, Bundle coreSettings, PackageManager pm, String packageName, ApplicationInfo ai) {
        ...
        try {
            ...
            Log.m100v(TAG, "Updatable driver package search path: " + paths + ", required sphal libraries: " + sphalLibraries);
