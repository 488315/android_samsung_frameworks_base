...
public final class Debug {
      ...
      switch (stringInfo) {
        ...
        default:
          throw new IllegalStateException(
              "Unexpected getStringInfo(" + propertyName + ") return value " + stringInfo);
      }
    }
  ...
  public static void setFieldsOn(Class<?> cl, boolean partial) {
    Log.wtf(
        TAG,
        "setFieldsOn("
            + (cl == null ? SemCapabilities.FEATURE_TAG_NULL : cl.getName())
  ...
  public static boolean dumpService(String name, FileDescriptor fd, String[] args) {
    ...
    if (service == null) {
      Log.m96e(TAG, "Can't find service to dump: " + name);
      return false;
    }
    try {
      ...
      Log.m97e(TAG, "Can't dump service: " + name, e);
      return false;
    }
  }
  ...
  public static boolean semIsOemUnlockEnabled(Context context) {
    ...
    if (manager == null) {
      Log.m96e(TAG, "PersistentDataBlockManager is null ");
      return false;
    }
    ...
    synchronized (Debug.class) {
      if (reason != null) {
        ...
        SystemProperties.set(
            "sys.reset_info", extra_info.substring(0, Math.min(extra_info.length(), 91)));
        SystemProperties.set("ctl.start", "resetreason");
      } else {
        Log.m94d(TAG, "!@ saveResetReason, but reason is null");
      }
    }
  }
  ...
  public static synchronized void saveDump(BugreportParams params, String packageName) {
    ...
    synchronized (Debug.class) {
      ...
      if ("sys_error".equals(nxt_brmode) && PLATFORM_SILENT_RESET.equals(resetReason)) {
        Log.m94d(TAG, "No need to trigger dumpstate in PF_SR case");
        return;
      }
      if ("0x4f4c".equals(debugLevel) && "true".equals(product_ship)) {
        try {
          if (nxt_brmode.startsWith("app")
              && (packageName == null || !isContainingDumpPackage(packageName))) {
            Log.m94d(TAG, "low && ship && 3rdparty app crash, do not dump");
            return;
          }
        } catch (NullPointerException e) {
          Log.m94d(TAG, "NullPointerException Occurred from saveDump");
          return;
        }
      }
      if ("1".equals(dump_running)) {
        if (!pre_brmode.startsWith(Notification.CATEGORY_SYSTEM)
          ...
          Log.m94d(TAG, "cancel previous dumsptate, and start new one");
          ...
          Log.m94d(TAG, "dumpstate is already running, so skip");
          return;
        }
      }
  ...
  private static boolean isContainingDumpPackage(String packageName) {
    String[] validPackages = {
      "com.sec.", "com.samsung.", "com.android.phone", AsPackageName.SYSTEMUI
    };
