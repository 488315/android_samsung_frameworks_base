...
public class PatternMatcher implements Parcelable {
  ...
  public boolean check() {
    try {
      ...
      Log.m102w(TAG, "Failed to verify advanced pattern: " + e.getMessage());
      return false;
    }
  }
  ...
  */
                  ...
                  if (!inSet) {
                    if (inCharClass) {
                      ...
                      inCharClass = false;
                        ...
                        && pattern.charAt(it2 + 1) == '-'
                        && pattern.charAt(it2 + 2) != ']') {
                      it3 = it + 1;
                      sParsedPatternScratch[it] = c;
                      it2++;
                      inCharClass = true;
          ...
          case '{':
            if (!inSet) {
              if (it4 != 0) {
                ...
                if (!isParsedModifier(iArr6[it4 - 1])) {
                  it = it4 + 1;
                  iArr6[it4] = -5;
                  inRange = true;
                  it2 = ip + 1;
  ...
  static boolean matchAdvancedPattern(int[] parsedPattern, String match) {
    ...
    while (ip3 < LP) {
      ...
      int matched =
          matchChars(
              match, im, LM, tokenType, i, i2, parsedPattern, maxRepetition3, minRepetition3);
      if (matched == -1) {
        return false;
      }
      im += matched;
  ...
  private static int matchChars(
    ...
    while (matched < maxRepetition
        && matchChar(match, im + matched, lm, tokenType, parsedPattern, tokenStart, tokenEnd)) {
      matched++;
    }
