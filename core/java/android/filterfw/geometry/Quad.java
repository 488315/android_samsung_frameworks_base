...
public class Quad {
  ...
  public boolean IsInUnitRange() {
    return this.f61p0.IsInUnitRange()
        && this.f62p1.IsInUnitRange()
        && this.f63p2.IsInUnitRange()
        && this.f64p3.IsInUnitRange();
  }
  ...
  public Quad translated(float x, float y) {
    return new Quad(
        this.f61p0.plus(x, y), this.f62p1.plus(x, y), this.f63p2.plus(x, y), this.f64p3.plus(x, y));
  }

  public Quad scaled(float s) {
    return new Quad(
        this.f61p0.times(s), this.f62p1.times(s), this.f63p2.times(s), this.f64p3.times(s));
  }

  public Quad scaled(float x, float y) {
    return new Quad(
        this.f61p0.mult(x, y), this.f62p1.mult(x, y), this.f63p2.mult(x, y), this.f64p3.mult(x, y));
  }

  public Rectangle boundingBox() {
    List<Float> xs =
        Arrays.asList(
            Float.valueOf(this.f61p0.f59x),
            Float.valueOf(this.f62p1.f59x),
            Float.valueOf(this.f63p2.f59x),
            Float.valueOf(this.f64p3.f59x));
    List<Float> ys =
        Arrays.asList(
            Float.valueOf(this.f61p0.f60y),
            Float.valueOf(this.f62p1.f60y),
            Float.valueOf(this.f63p2.f60y),
            Float.valueOf(this.f64p3.f60y));
