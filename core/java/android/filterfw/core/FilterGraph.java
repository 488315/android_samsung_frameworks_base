...
public class FilterGraph {
  ...
  public void beginProcessing() {
    if (this.mLogVerbose) {
      Log.m100v(this.TAG, "Opening all filter connections...");
    }
  ...
  public void closeFilters(FilterContext context) {
    if (this.mLogVerbose) {
      Log.m100v(this.TAG, "Closing all filters...");
    }
    Iterator<Filter> it = this.mFilters.iterator();
    while (it.hasNext()) {
      Filter filter = it.next();
  ...
  private void runTypeCheck() {
    ...
    while (!filterStack.empty()) {
      ...
      if (this.mLogVerbose) {
        Log.m100v(this.TAG, "Running type check on " + filter + Session.TRUNCATE_STRING);
      }
  ...
  private void runTypeCheckOn(Filter filter) {
    for (InputPort inputPort : filter.getInputPorts()) {
      if (this.mLogVerbose) {
        Log.m100v(this.TAG, "Type checking port " + inputPort);
      }
      ...
      if (sourceFormat != null && targetFormat != null) {
        if (this.mLogVerbose) {
          Log.m100v(
              this.TAG,
              "Checking " + sourceFormat + " against " + targetFormat + MediaMetrics.SEPARATOR);
        }
  ...
  private void discardUnconnectedOutputs() {
    ...
    while (it.hasNext()) {
      ...
      for (OutputPort port : filter.getOutputPorts()) {
        if (!port.isConnected()) {
          if (this.mLogVerbose) {
            Log.m100v(this.TAG, "Autoconnecting unconnected " + port + " to Null filter.");
          }
        ...
        if (this.mAutoBranchMode == 0) {
          throw new RuntimeException(
              "Attempting to connect "
                  + outputPort
                  + " to multiple filter ports! Enable auto-branching to allow this.");
        }
        if (this.mLogVerbose) {
          Log.m100v(this.TAG, "Creating branch for " + outputPort + "!");
        }
  ...
  private HashSet<Filter> getSourceFilters() {
    ...
    for (Filter filter : getFilters()) {
      if (filter.getNumberOfConnectedInputs() == 0) {
        if (this.mLogVerbose) {
          Log.m100v(this.TAG, "Found source filter: " + filter);
        }
