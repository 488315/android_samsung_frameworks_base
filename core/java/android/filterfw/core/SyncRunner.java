...
public class SyncRunner extends GraphRunner {
    ...
    public SyncRunner(FilterContext context, FilterGraph graph, Class schedulerClass) {
        ...
        if (isLoggable) {
            Log.m100v(TAG, "Initializing SyncRunner");
        }
        if (Scheduler.class.isAssignableFrom(schedulerClass)) {
            try {
                ...
                if (isLoggable) {
                    Log.m100v(TAG, "Setting up filters");
                }
                graph.setupFilters();
                return;
    ...
    public void run() {
        ...
        if (this.mDoneListener != null) {
            if (this.mLogVerbose) {
                Log.m100v(TAG, "Calling completion listener.");
            }
            this.mDoneListener.onRunnerDone(determinePostRunState());
        }
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Run complete");
        }
    }
    ...
    protected void processFilterNode(Filter filter) {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Processing filter node");
        }
        ...
        if (filter.getStatus() == 4) {
            if (this.mLogVerbose) {
                Log.m100v(TAG, "Scheduling filter wakeup");
            }
    ...
    boolean performStep() {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Performing one step.");
        }
