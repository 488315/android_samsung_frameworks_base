...
public class FilterFactory {
  ...
  public static void addFilterLibrary(String libraryPath) {
    if (mLogVerbose) {
      Log.m100v(TAG, "Adding filter library " + libraryPath);
    }
    synchronized (mClassLoaderGuard) {
      if (mLibraries.contains(libraryPath)) {
        if (mLogVerbose) {
          Log.m100v(TAG, "Library already added");
        }
      } else {
  ...
  public void addPackage(String packageName) {
    if (mLogVerbose) {
      Log.m100v(TAG, "Adding package " + packageName);
    }
  ...
  public Filter createFilterByClassName(String className, String filterName) {
    if (mLogVerbose) {
      Log.m100v(TAG, "Looking up class " + className);
    }
    ...
    while (it.hasNext()) {
      ...
      try {
        if (mLogVerbose) {
          Log.m100v(TAG, "Trying " + packageName + MediaMetrics.SEPARATOR + className);
        }
        synchronized (mClassLoaderGuard) {
          filterClass =
              mCurrentClassLoader.loadClass(packageName + MediaMetrics.SEPARATOR + className);
        }
  ...
  public Filter createFilterByClass(Class filterClass, String filterName) {
    if (!Filter.class.isAssignableFrom(filterClass)) {
      throw new IllegalArgumentException(
          "Attempting to allocate class '" + filterClass + "' which is not a subclass of Filter!");
    }
    try {
