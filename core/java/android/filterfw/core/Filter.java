...
public abstract class Filter {
    ...
    protected final void pushOutput(String name, Frame frame) {
        if (frame.getTimestamp() == -2) {
            if (this.mLogVerbose) {
                Log.m100v(TAG, "Default-setting output Frame timestamp on port " + name + " to " + this.mCurrentTimestamp);
            }
    ...
    protected final Frame pullInput(String name) {
        ...
        if (this.mCurrentTimestamp == -1) {
            ...
            if (this.mLogVerbose) {
                Log.m100v(TAG, "Default-setting current timestamp from input port " + name + " to " + this.mCurrentTimestamp);
            }
        }
    ...
    protected void addMaskedInputPort(String name, FrameFormat formatMask) {
        ...
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Filter " + this + " adding " + port);
        }
    ...
    protected void addFieldPort(String name, Field field, boolean hasDefault, boolean isFinal) {
        ...
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Filter " + this + " adding " + fieldPort);
        }
    ...
    protected void addProgramPort(String name, String varName, Field field, Class varType, boolean hasDefault) {
        ...
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Filter " + this + " adding " + programPort);
        }
    ...
    final synchronized boolean canProcess() {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Checking if can process: " + this + " (" + this.mStatus + ").");
        }
        boolean z = false;
        if (this.mStatus > 3) {
            return false;
    ...
    final void openOutputs() {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Opening all output ports on " + this + "!");
        }
        for (OutputPort outputPort : this.mOutputPorts.values()) {
    ...
    private final void setImmediateInputValue(String name, Object value) {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Setting immediate value " + value + " for port " + name + "!");
        }
    ...
    private final boolean inputConditionsMet() {
        for (FilterPort port : this.mInputPorts.values()) {
            if (!port.isReady()) {
                if (this.mLogVerbose) {
                    Log.m100v(TAG, "Input condition not met: " + port + "!");
                    return false;
                }
                return false;
            }
    ...
    private final boolean outputConditionsMet() {
        for (FilterPort port : this.mOutputPorts.values()) {
            if (!port.isReady()) {
                if (this.mLogVerbose) {
                    Log.m100v(TAG, "Output condition not met: " + port + "!");
                    return false;
                }
                return false;
            }
    ...
    private final void closePorts() {
        if (this.mLogVerbose) {
            Log.m100v(TAG, "Closing all ports on " + this + "!");
        }
        for (InputPort inputPort : this.mInputPorts.values()) {
            inputPort.close();
        }
    ...
    private final boolean filterMustClose() {
        for (InputPort inputPort : this.mInputPorts.values()) {
            if (inputPort.filterMustClose()) {
                if (this.mLogVerbose) {
                    Log.m100v(TAG, "Filter " + this + " must close due to port " + inputPort);
                }
                return true;
            }
        }
        for (OutputPort outputPort : this.mOutputPorts.values()) {
            if (outputPort.filterMustClose()) {
                if (this.mLogVerbose) {
                    Log.m100v(TAG, "Filter " + this + " must close due to port " + outputPort);
                }
                return true;
            }
        }
