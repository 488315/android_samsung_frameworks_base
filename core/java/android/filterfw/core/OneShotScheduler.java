...
public class OneShotScheduler extends RoundRobinScheduler {
    ...
    public Filter scheduleNextNode() {
        ...
        while (true) {
            ...
            if (filter == null) {
                if (this.mLogVerbose) {
                    Log.m100v(TAG, "No filters available to run.");
                }
                return null;
            }
            if (!this.scheduled.containsKey(filter.getName())) {
                ...
                if (this.mLogVerbose) {
                    Log.m100v(TAG, "Scheduling filter \"" + filter.getName() + "\" of type " + filter.getFilterClassName());
                }
                return filter;
            }
            if (first != filter) {
                ...
                if (this.mLogVerbose) {
                    Log.m100v(TAG, "One pass through graph completed.");
                }
                return null;
            }
        }
