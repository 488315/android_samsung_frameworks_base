...
public final class RemoteAnimationRunnerHelper {
  ...
  public static boolean mergeAnimation(
      IBinder token,
      TransitionInfo info,
      SurfaceControl.Transaction t,
      IBinder mergeTarget,
      IRemoteTransitionFinishedCallback finishCallback,
      HashMap<Integer, Runnable> animCallbacks,
      ArrayMap<SurfaceControl, SurfaceControl> leashMap)
      throws RemoteException {
    if (CoreRune.FW_CUSTOM_SHELL_TRANSITION_MERGE
        && info.canMergeAnimation()
        && animCallbacks.get(1) != null) {
    ...
    if (!CoreRune.FW_CUSTOM_SHELL_TRANSITION_MERGE_TRANSFER
        || !info.canTransferAnimation()
        || animCallbacks.get(2) == null) {
      return false;
    }
    ...
    for (SurfaceControl leash : leashMap.keySet()) {
      ...
      if (transitionLeash == null || !transitionLeash.isValid()) {
        Log.m102w(TAG, "Failed to transfer animation due to invalid transition leash");
        return false;
      }
