...
public interface SplashScreen {
  ...
  public static class SplashScreenImpl implements SplashScreen {
    ...
    public void setSplashScreenTheme(int themeId) {
      if (this.mActivityToken == null) {
        Log.m102w(TAG, "Couldn't persist the starting theme. This instance is not an Activity");
        return;
      }
      ...
      try {
        AppGlobals.getPackageManager()
            .setSplashScreenTheme(
                activity.getComponentName().getPackageName(), themeName, activity.getUserId());
    ...
    private void dispatchOnExitAnimation(IBinder token, SplashScreenView view) {
      synchronized (this.mGlobalLock) {
        ...
        if (impl.mExitAnimationListener == null) {
          Slog.m115e(TAG, "cannot dispatch onExitAnimation to listener " + token);
        } else {
