...
public class WindowOnBackInvokedDispatcher implements OnBackInvokedDispatcher {
    ...
    static class OnBackInvokedCallbackWrapper extends IOnBackInvokedCallback.Stub {
        ...
        public /* synthetic */ void lambda$onBackInvoked$5() {
            ...
            if (callback == null) {
                Log.m94d(WindowOnBackInvokedDispatcher.TAG, "Trying to call onBackInvoked() on a null callback reference.");
            } else if (!(callback instanceof OnBackAnimationCallback) || isInProgress) {
                Log.m94d(WindowOnBackInvokedDispatcher.TAG, "onBackInvoked, owner=" + WindowOnBackInvokedDispatcher.sOwnerTag + ", callback=" + callback);
                callback.onBackInvoked();
            } else {
                Log.m102w(WindowOnBackInvokedDispatcher.TAG, "ProgressAnimator was not in progress, skip onBackInvoked().");
            }
        }
    ...
    public static class Checker {
        ...
        public boolean checkApplicationCallbackRegistration(int priority, OnBackInvokedCallback callback) {
            if (!isOnBackInvokedCallbackEnabled(getContext()) && !(callback instanceof CompatOnBackInvokedCallback)) {
                Log.m102w(WindowOnBackInvokedDispatcher.TAG, "OnBackInvokedCallback is not enabled for the application.\nSet 'android:enableOnBackInvokedCallback=\"true\"' in the application manifest.");
                return false;
            }
            if (priority < 0) {
        ...
        public static boolean isOnBackInvokedCallbackEnabled(Context context) {
            ...
            if (context == null) {
                Log.m102w(WindowOnBackInvokedDispatcher.TAG, "OnBackInvokedCallback is not enabled because context is null.");
                return false;
            }
            ...
            if (context instanceof Activity) {
                ...
                if (activityInfo == null) {
                    Log.m102w(WindowOnBackInvokedDispatcher.TAG, "The ActivityInfo is null, so we cannot verify if this Activity has the 'android:enableOnBackInvokedCallback' attribute. The application attribute will be used as a fallback.");
