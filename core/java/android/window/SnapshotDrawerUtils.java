...
public class SnapshotDrawerUtils {
  ...
  public static class SnapshotSurface {
    ...
    public SnapshotSurface(
        SurfaceControl rootSurface, TaskSnapshot snapshot, CharSequence title, Rect taskBounds) {
    ...
    void initiateSystemBarPainter(
        int windowFlags,
        int windowPrivateFlags,
        int appearance,
        ActivityManager.TaskDescription taskDescription,
        int requestedVisibleTypes) {
      this.mSystemBarBackgroundPainter =
          new SystemBarBackgroundPainter(
              windowFlags,
              windowPrivateFlags,
              appearance,
              taskDescription,
    ...
    void setFrames(Rect frame, Rect systemBarInsets) {
      ...
      this.mSizeMismatch =
          (this.mFrame.width() == snapshot.getWidth()
                  && this.mFrame.height() == snapshot.getHeight())
              ? false
              : true;
    ...
    public void drawSnapshot(boolean releaseAfterDraw) {
      Log.m100v(
          SnapshotDrawerUtils.TAG, "Drawing snapshot surface sizeMismatch=" + this.mSizeMismatch);
    ...
    private void drawSizeMatchSnapshot() {
      this.mTransaction
          .setBuffer(this.mRootSurface, this.mSnapshot.getHardwareBuffer())
          .setColorSpace(this.mRootSurface, this.mSnapshot.getColorSpace())
          .apply();
    }
    ...
    private void drawSizeMismatchSnapshot() {
      ...
      boolean aspectRatioMismatch =
          !SnapshotDrawerUtils.isAspectRatioMatch(this.mFrame, this.mSnapshot);
      SurfaceControl childSurfaceControl =
          new SurfaceControl.Builder(session)
              .setName(((Object) this.mTitle) + " - task-snapshot-surface")
              .setBLASTLayer()
              .setFormat(buffer.getFormat())
              .setParent(this.mRootSurface)
      ...
      SnapshotDrawerUtils.sSnapshotMatrix.setRectToRect(
          SnapshotDrawerUtils.sTmpSnapshotSize,
          SnapshotDrawerUtils.sTmpDstFrame,
          Matrix.ScaleToFit.FILL);
      this.mTransaction.setMatrix(
          childSurfaceControl, SnapshotDrawerUtils.sSnapshotMatrix, SnapshotDrawerUtils.sTmpFloat9);
      ...
      if (aspectRatioMismatch) {
        GraphicBuffer background =
            GraphicBuffer.create(
                this.mFrame.width(),
                this.mFrame.height(),
                1,
                SemExtendedFormat.DataType.FOOD_SHOT_INFO);
        if (background == null) {
          Log.m96e(
              SnapshotDrawerUtils.TAG,
              "Unable to draw snapshot: failed to allocate graphic buffer for "
                  + ((Object) this.mTitle));
          return;
        }
        ...
        this.mTransaction.setBuffer(
            this.mRootSurface, HardwareBuffer.createFromGraphicBuffer(background));
      }
    ...
    Rect calculateSnapshotCrop(Rect insets) {
      ...
      float scaleX = snapshot.getWidth() / this.mSnapshot.getTaskSize().f85x;
      float scaleY = snapshot.getHeight() / this.mSnapshot.getTaskSize().f86y;
      boolean isTop = this.mTaskBounds.top == 0 && this.mFrame.top == 0;
      rect.inset(
          (int) (insets.left * scaleX),
    ...
    void drawBackgroundAndBars(Canvas c, Rect frame) {
      ...
      if (fillHorizontally) {
        c.drawRect(
            frame.right,
            Color.alpha(this.mSystemBarBackgroundPainter.mStatusBarColor) == 255
                ? statusBarHeight
                : 0.0f,
            c.getWidth(),
            fillVertically ? frame.bottom : c.getHeight(),
            SnapshotDrawerUtils.sBackgroundPaint);
      }
      if (fillVertically) {
        c.drawRect(
            0.0f, frame.bottom, c.getWidth(), c.getHeight(), SnapshotDrawerUtils.sBackgroundPaint);
      }
    ...
    void drawStatusBarBackground(Canvas c, Rect alreadyDrawnFrame) {
      ...
      systemBarBackgroundPainter.drawStatusBarBackground(
          c, alreadyDrawnFrame, systemBarBackgroundPainter.getStatusBarColorViewHeight());
    }
  ...
  public static boolean isAspectRatioMatch(Rect frame, TaskSnapshot snapshot) {
    ...
    return Math.abs(
            (((float) buffer.getWidth()) / ((float) buffer.getHeight()))
  ...
  public static boolean isAspectRatioMatch(Rect frame1, Rect frame2) {
    return (frame1.isEmpty()
            || frame2.isEmpty()
            || Math.abs(
                    (((float) frame2.width()) / ((float) frame2.height()))
                        - (((float) frame1.width()) / ((float) frame1.height())))
                > 0.01f)
        ? false
        : true;
  }
  ...
  public static void drawSnapshotOnSurface(
      ...
      Rect configBounds,
      Rect windowBounds,
      InsetsState topWindowInsetsState,
      boolean releaseAfterDraw) {
    if (windowBounds.isEmpty()) {
      Log.m96e(TAG, "Unable to draw snapshot on an empty windowBounds");
      return;
    }
    SnapshotSurface drawSurface =
        new SnapshotSurface(rootSurface, snapshot, lp.getTitle(), configBounds);
  ...
  public static WindowManager.LayoutParams createLayoutParameters(
      StartingWindowInfo info, CharSequence title, int windowType, int pixelFormat, IBinder token) {
    WindowManager.LayoutParams attrs = info.topOpaqueWindowLayoutParams;
    ...
    if (attrs == null || mainWindowParams == null || topWindowInsetsState == null) {
      Log.m102w(TAG, "unable to create taskSnapshot surface ");
      return null;
    }
  ...
  public static class SystemBarBackgroundPainter {
    ...
    public SystemBarBackgroundPainter(
      ...
      int semiTransparent = context.getColor(C4337R.color.system_bar_background_semi_transparent);
      int calculateBarColor =
          DecorView.calculateBarColor(
              windowFlags,
              67108864,
              semiTransparent,
              taskDescription.getStatusBarColor(),
              appearance,
              8,
              taskDescription.getEnsureStatusBarContrastWhenTransparent());
      this.mStatusBarColor = calculateBarColor;
      int calculateBarColor2 =
          DecorView.calculateBarColor(
              windowFlags,
              134217728,
              semiTransparent,
              ...
              taskDescription.getEnsureNavigationBarContrastWhenTransparent()
                  && context.getResources().getBoolean(C4337R.bool.config_navBarNeedsScrim),
    ...
    int getStatusBarColorViewHeight() {
      ...
      if (DecorView.STATUS_BAR_COLOR_VIEW_ATTRIBUTES.isVisible(
    ...
    private boolean isNavigationBarColorViewVisible() {
      boolean forceBarBackground = (this.mWindowPrivateFlags & 32768) != 0;
      return DecorView.NAVIGATION_BAR_COLOR_VIEW_ATTRIBUTES.isVisible(
          this.mRequestedVisibleTypes,
    ...
    void drawNavigationBarBackground(Canvas c) {
      ...
      DecorView.getNavigationBarRect(
          c.getWidth(), c.getHeight(), this.mSystemBarInsets, navigationBarRect, this.mScale);
