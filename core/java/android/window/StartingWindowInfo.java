...
public final class StartingWindowInfo implements Parcelable {
  ...
  public void notifyAddComplete(SurfaceControl addedSurface) {
    IWindowlessStartingSurfaceCallback iWindowlessStartingSurfaceCallback =
        this.windowlessStartingSurfaceCallback;
  ...
  void readFromParcel(Parcel source) {
    this.taskInfo =
        (ActivityManager.RunningTaskInfo)
            source.readTypedObject(ActivityManager.RunningTaskInfo.CREATOR);
    ...
    this.topOpaqueWindowLayoutParams =
        (WindowManager.LayoutParams) source.readTypedObject(WindowManager.LayoutParams.CREATOR);
    this.mainWindowLayoutParams =
        (WindowManager.LayoutParams) source.readTypedObject(WindowManager.LayoutParams.CREATOR);
    ...
    this.windowlessStartingSurfaceCallback =
        IWindowlessStartingSurfaceCallback.Stub.asInterface(source.readStrongBinder());
    this.rootSurface = (SurfaceControl) source.readTypedObject(SurfaceControl.CREATOR);
  }

  public String toString() {
    return "StartingWindowInfo{taskId="
        + this.taskInfo.taskId
        + " targetActivityInfo="
        + this.targetActivityInfo
        + " displayId="
        + this.taskInfo.displayId
        + " topActivityType="
        + this.taskInfo.topActivityType
        + " preferredStartingWindowType="
        + Integer.toHexString(this.startingWindowTypeParameter)
        + " insetsState="
        + this.topOpaqueWindowInsetsState
        + " topWindowLayoutParams="
        + this.topOpaqueWindowLayoutParams
        + " mainWindowLayoutParams="
        + this.mainWindowLayoutParams
        + " splashScreenThemeResId "
        + Integer.toHexString(this.splashScreenThemeResId);
  }
}
