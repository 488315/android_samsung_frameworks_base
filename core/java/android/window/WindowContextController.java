...
public class WindowContextController {
  ...
  public void attachToDisplayArea(int type, int displayId, Bundle options) {
    if (this.mAttachedToDisplayArea == 1) {
      throw new IllegalStateException(
          "A Window Context can be only attached to a DisplayArea once.");
    }
    int i = this.mToken.attachToDisplayArea(type, displayId, options) ? 1 : 3;
    this.mAttachedToDisplayArea = i;
    if (i == 3) {
      Log.m102w(TAG, "attachToDisplayArea fail, type:" + type + ", displayId:" + displayId);
    }
  }
  ...
  public void attachToWindowToken(IBinder windowToken) {
    if (this.mAttachedToDisplayArea != 1) {
      throw new IllegalStateException(
          "The Window Context should have been attached to a DisplayArea. AttachToDisplayArea:"
              + this.mAttachedToDisplayArea);
    }
