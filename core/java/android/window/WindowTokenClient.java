...
public class WindowTokenClient extends IWindowToken.Stub {
    ...
    public void onConfigurationChanged(Configuration newConfig, int newDisplayId, boolean shouldReportConfigChange) {
        ...
        if (!shouldUpdateResources && this.mShouldDumpConfigForIme) {
            Log.m94d(TAG, "Configuration not dispatch to IME because configuration is up to date. Current config=" + context.getResources().getConfiguration() + ", reported config=" + currentConfig + ", updated config=" + newConfig);
        }
        if (shouldUpdateResources) {
            ...
            if (this.mShouldDumpConfigForIme) {
                if (!shouldReportConfigChange) {
                    Log.m94d(TAG, "Only apply configuration update to Resources because shouldReportConfigChange is false.\n" + Debug.getCallers(5));
                } else if (diff == 0) {
                    Log.m94d(TAG, "Configuration not dispatch to IME because configuration has no  public difference with updated config.  Current config=" + context.getResources().getConfiguration() + ", reported config=" + currentConfig + ", updated config=" + newConfig);
                }
            }
        }
        if (displayChanged) {
