...
public final class SplashScreenView extends FrameLayout {
    ...
    public static class Builder {
        ...
        public Builder createFromParcel(SplashScreenViewParcelable parcelable) {
            ...
            if (SplashScreenView.DEBUG) {
                Log.m94d(SplashScreenView.TAG, String.format("Building from parcel drawable: %s", this.mIconDrawable));
            }
            return this;
        }
        ...
        public SplashScreenView build() {
            ...
            final SplashScreenView view = (SplashScreenView) layoutInflater.inflate(C4337R.layout.splash_screen_view, (ViewGroup) null, false);
        ...
        private SurfaceView createSurfaceView(SplashScreenView splashScreenView) {
            ...
            if (this.mSurfacePackage == null) {
                if (SplashScreenView.DEBUG) {
                    Log.m94d(SplashScreenView.TAG, "SurfaceControlViewHost created on thread " + Thread.currentThread().getId());
                }
        ...
        if (DEBUG) {
            surfacePackage.getSurfaceControl().addOnReparentListener(new SurfaceControl.OnReparentListener() { // from class: android.window.SplashScreenView$$ExternalSyntheticLambda1
                ...
                public final void onReparent(SurfaceControl.Transaction transaction, SurfaceControl surfaceControl) {
                    Log.m96e(SplashScreenView.TAG, String.format("SurfacePackage'surface reparented to %s", surfaceControl));
                }
            });
    ...
    private void releaseAnimationSurfaceHost() {
        ...
        if (surfaceControlViewHost != null && !this.mIsCopied) {
            if (DEBUG) {
                Log.m94d(TAG, "Shell removed splash screen. Releasing SurfaceControlViewHost on thread #" + Thread.currentThread().getId());
            }
