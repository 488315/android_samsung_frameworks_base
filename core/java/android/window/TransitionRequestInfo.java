...
public final class TransitionRequestInfo implements Parcelable {
  ...
  public TransitionRequestInfo(
      int type, ActivityManager.RunningTaskInfo triggerTask, RemoteTransition remoteTransition) {
    this(type, triggerTask, remoteTransition, null);
  }

  public static class DisplayChange implements Parcelable {
    public static final Parcelable.Creator<DisplayChange> CREATOR =
        new Parcelable.Creator<
            DisplayChange>() { // from class: android.window.TransitionRequestInfo.DisplayChange.1
          /* JADX WARN: Can't rename method to resolve collision */
          @Override // android.os.Parcelable.Creator
    ...
    public String toString() {
      return "DisplayChange { displayId = "
          + this.mDisplayId
          + ", startAbsBounds = "
          + this.mStartAbsBounds
          + ", endAbsBounds = "
          + this.mEndAbsBounds
          + ", startRotation = "
          + this.mStartRotation
          + ", endRotation = "
          + this.mEndRotation
          + ", physicalDisplayChanged = "
          + this.mPhysicalDisplayChanged
          + ", uiModeChanged = "
          + this.mUiModeChanged
          + " }";
    }
  ...
  public TransitionRequestInfo(
    ...
    AnnotationValidations.validate(
        (Class<? extends Annotation>) WindowManager.TransitionType.class, (Annotation) null, type);
  ...
  public String toString() {
    return "TransitionRequestInfo { type = "
        + WindowManager.transitTypeToString(this.mType)
        + ", triggerTask = "
        + this.mTriggerTask
        + ", remoteTransition = "
  ...
  TransitionRequestInfo(Parcel in) {
    ...
    RemoteTransition remoteTransition =
        (flg & 4) == 0 ? null : (RemoteTransition) in.readTypedObject(RemoteTransition.CREATOR);
    DisplayChange displayChange =
        (flg & 8) == 0 ? null : (DisplayChange) in.readTypedObject(DisplayChange.CREATOR);
    this.mType = type;
    AnnotationValidations.validate(
        (Class<? extends Annotation>) WindowManager.TransitionType.class, (Annotation) null, type);
    this.mTriggerTask = triggerTask;
