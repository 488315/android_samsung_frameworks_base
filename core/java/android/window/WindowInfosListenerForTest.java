...
public class WindowInfosListenerForTest {
  ...
  public void addWindowInfosListener(final Consumer<List<WindowInfo>> consumer) {
    ...
    WindowInfosListener windowInfosListener =
        new WindowInfosListener() { // from class: android.window.WindowInfosListenerForTest.1
          ...
          public void onWindowInfosChanged(
            ...
            try {
              ...
              Log.m96e(
                  ...
                  "Exception thrown while waiting for listener to be called with initial state");
            }
            consumer.accept(
                WindowInfosListenerForTest.buildWindowInfos(windowHandles, displayInfos));
          }
        };
  ...
  public static List<WindowInfo> buildWindowInfos(
    ...
    for (InputWindowHandle handle : windowHandles) {
      Rect bounds =
          new Rect(handle.frameLeft, handle.frameTop, handle.frameRight, handle.frameBottom);
      ...
      windowInfos.add(
          new WindowInfo(
              handle.getWindowToken(), handle.name, handle.displayId, bounds, handle.inputConfig));
    }
    return windowInfos;
  }
}
