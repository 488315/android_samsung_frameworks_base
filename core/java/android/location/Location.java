...
public class Location implements Parcelable {
  ...
  private static final ThreadLocal<BearingDistanceCache> sBearingDistanceCache =
      ThreadLocal.withInitial(
          new Supplier() { // from class: android.location.Location$$ExternalSyntheticLambda0
  ...
  public double getMslAltitudeMeters() {
    Preconditions.checkState(
        hasMslAltitude(), "The Mean Sea Level altitude of this location is not set.");
  ...
  public boolean equals(Object o) {
    ...
    return this.mTimeMs == location.mTimeMs
        ...
        && (!hasElapsedRealtimeUncertaintyNanos()
            || Double.compare(
                    location.mElapsedRealtimeUncertaintyNs, this.mElapsedRealtimeUncertaintyNs)
                == 0)
        ...
        && ((!hasAltitude() || Double.compare(location.mAltitudeMeters, this.mAltitudeMeters) == 0)
            && hasSpeed() == location.hasSpeed()
            && ((!hasSpeed()
                    || Float.compare(location.mSpeedMetersPerSecond, this.mSpeedMetersPerSecond)
                        == 0)
                && hasBearing() == location.hasBearing()
                && ((!hasBearing()
                        || Float.compare(location.mBearingDegrees, this.mBearingDegrees) == 0)
                    && hasAccuracy() == location.hasAccuracy()
                    && ((!hasAccuracy()
                            || Float.compare(
                                    location.mHorizontalAccuracyMeters,
                                    this.mHorizontalAccuracyMeters)
                                == 0)
                        && hasVerticalAccuracy() == location.hasVerticalAccuracy()
                        && ((!hasVerticalAccuracy()
                                || Float.compare(
                                        location.mAltitudeAccuracyMeters,
                                        this.mAltitudeAccuracyMeters)
                                    == 0)
                            && hasSpeedAccuracy() == location.hasSpeedAccuracy()
                            && ((!hasSpeedAccuracy()
                                    || Float.compare(
                                            location.mSpeedAccuracyMetersPerSecond,
                                            this.mSpeedAccuracyMetersPerSecond)
                                        == 0)
                                && hasBearingAccuracy() == location.hasBearingAccuracy()
                                && ((!hasBearingAccuracy()
                                        || Float.compare(
                                                location.mBearingAccuracyDegrees,
                                                this.mBearingAccuracyDegrees)
                                            == 0)
                                    && hasMslAltitude() == location.hasMslAltitude()
                                    && ((!hasMslAltitude()
                                            || Double.compare(
                                                    location.mMslAltitudeMeters,
                                                    this.mMslAltitudeMeters)
                                                == 0)
                                        && hasMslAltitudeAccuracy()
                                        ...
                                        && ((!hasMslAltitudeAccuracy()
                                                || Float.compare(
                                                        ...
                                                        this.mMslAltitudeAccuracyMeters)
                                                    == 0)
                                            && Objects.equals(this.mProvider, location.mProvider)
                                            && areExtrasEqual(
                                                this.mExtras, location.mExtras))))))))));
  }
  ...
  private static void computeDistanceAndBearing(
    ...
    while (true) {
      ...
      aA =
          ((uSquared / 16384.0d)
                  * (((((320.0d - (175.0d * uSquared)) * uSquared) - 768.0d) * uSquared) + 4096.0d))
              + 1.0d;
      double bB =
          (uSquared / 1024.0d)
              * (((((74.0d - (47.0d * uSquared)) * uSquared) - 128.0d) * uSquared) + 256.0d);
              ...
              * (cos2SM
                  + ((bB / 4.0d)
                      * ((((cos2SMSq * 2.0d) - 1.0d) * lat23)
                          - ((((bB / 6.0d) * cos2SM) * (((sinSigma2 * 4.0d) * sinSigma2) - 3.0d))
                              * ((4.0d * cos2SMSq) - 3.0d)))));
      lambda =
          l
              + ((1.0d - cC)
                  ...
                  * (sigma
                      + (cC
                          * sinSigma2
                          * (cos2SM + (cC * lat23 * (((2.0d * cos2SM) * cos2SM) - 1.0d))))));
