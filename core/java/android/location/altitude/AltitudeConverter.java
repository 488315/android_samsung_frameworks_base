...
public final class AltitudeConverter {
  ...
  private static void validate(Location location) {
    ...
    Preconditions.checkArgument(
        Double.isFinite(location.getAltitude()),
        "Invalid altitude above WGS84: %f",
  ...
  private static void addMslAltitude(
      MapParamsProto params, long[] s2CellIds, double[] geoidHeightsMeters, Location location) {
    long s0 = s2CellIds[0];
    ...
    double offsetMeters =
        h0 + ((h1 - h0) * wi) + ((h2 - h0) * wj) + ((((h3 - h1) - h2) + h0) * wi * wj);
    location.setMslAltitudeMeters(location.getAltitude() - offsetMeters);
    if (location.hasVerticalAccuracy()) {
      double verticalAccuracyMeters = location.getVerticalAccuracyMeters();
      if (Double.isFinite(verticalAccuracyMeters)
          && verticalAccuracyMeters >= SContextConstants.ENVIRONMENT_VALUE_UNKNOWN) {
  ...
  public boolean addMslAltitudeToLocation(Location location) {
    ...
    if (params == null
        || (geoidHeightsMeters =
                this.mGeoidHeightMap.readGeoidHeights(
                    params, (s2CellIds = findMapSquare(params, location))))
            == null) {
      return false;
    }
    addMslAltitude(params, s2CellIds, geoidHeightsMeters, location);
