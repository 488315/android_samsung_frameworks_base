...
public abstract class SettingInjectorService extends Service {
    ...
    private void sendStatus(Intent intent, String summary, boolean enabled) {
        ...
        if (Log.isLoggable(TAG, 3)) {
            Log.m94d(TAG, this.mName + ": received " + intent + ", summary=" + summary + ", enabled=" + enabled + ", sending message: " + message);
        }
        try {
            messenger.send(message);
        } catch (RemoteException e) {
            Log.m97e(TAG, this.mName + ": sending dynamic status failed", e);
        }
    }
