...
public final class GnssMeasurementCorrections implements Parcelable {
  public static final Parcelable.Creator<GnssMeasurementCorrections> CREATOR =
      new Parcelable.Creator<
          GnssMeasurementCorrections>() { // from class:
  ...
  private GnssMeasurementCorrections(Builder builder) {
    ...
    Preconditions.checkArgument(
        (singleSatCorrList == null || singleSatCorrList.isEmpty()) ? false : true);
    this.mSingleSatCorrectionList = Collections.unmodifiableList(new ArrayList(singleSatCorrList));
    this.mHasEnvironmentBearing =
        builder.mEnvironmentBearingIsSet && builder.mEnvironmentBearingUncertaintyIsSet;
  ...
  public String toString() {
    return "GnssMeasurementCorrections:\n"
        + String.format(
            "   %-29s = %s\n", "LatitudeDegrees = ", Double.valueOf(this.mLatitudeDegrees))
        + String.format(
            "   %-29s = %s\n", "LongitudeDegrees = ", Double.valueOf(this.mLongitudeDegrees))
        + String.format(
            "   %-29s = %s\n", "AltitudeMeters = ", Double.valueOf(this.mAltitudeMeters))
        + String.format(
            "   %-29s = %s\n",
            "HorizontalPositionUncertaintyMeters = ",
            Double.valueOf(this.mHorizontalPositionUncertaintyMeters))
        + String.format(
            "   %-29s = %s\n",
            "VerticalPositionUncertaintyMeters = ",
            Double.valueOf(this.mVerticalPositionUncertaintyMeters))
        + String.format(
            "   %-29s = %s\n",
            "ToaGpsNanosecondsOfWeek = ", Long.valueOf(this.mToaGpsNanosecondsOfWeek))
        + String.format(
            "   %-29s = %s\n", "mSingleSatCorrectionList = ", this.mSingleSatCorrectionList)
        + String.format(
            "   %-29s = %s\n",
            "HasEnvironmentBearing = ", Boolean.valueOf(this.mHasEnvironmentBearing))
        + String.format(
            "   %-29s = %s\n",
            "EnvironmentBearingDegrees = ", Float.valueOf(this.mEnvironmentBearingDegrees))
        + String.format(
            "   %-29s = %s\n",
  ...
  public static final class Builder {
    ...
    public Builder setSingleSatelliteCorrectionList(
        List<GnssSingleSatCorrection> singleSatCorrectionList) {
    ...
    public Builder setEnvironmentBearingUncertaintyDegrees(
    ...
    public GnssMeasurementCorrections build() {
      if (this.mEnvironmentBearingIsSet ^ this.mEnvironmentBearingUncertaintyIsSet) {
        throw new IllegalStateException(
            "Both environment bearing and environment bearing uncertainty must be set.");
      }
