...
public final class AdbNotifications {
    ...
    public static Notification createNotification(Context context, byte transportType) {
        ...
        return new Notification.Builder(context, SystemNotificationChannels.DEVELOPER_IMPORTANT).setSmallIcon(C4337R.drawable.stat_sys_adb).setWhen(0L).setOngoing(true).setTicker(title).setDefaults(0).setColor(context.getColor(17170460)).setContentTitle(title).setContentText(message).setContentIntent(pIntent).setVisibility(1).extend(new Notification.TvExtender().setChannelId(ADB_NOTIFICATION_CHANNEL_ID_TV)).build();
    }
}
