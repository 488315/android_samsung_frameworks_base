...
final class ConnectionServiceAdapter implements IBinder.DeathRecipient {
    ...
    if (this.mAdapters.size() == 1) {
      try {
        this.mAdapters
            .iterator()
            .next()
            .queryRemoteConnectionServices(callback, callingPackage, Log.getExternalSession());
        return;
      } catch (RemoteException e) {
  ...
  void setConferenceableConnections(String callId, List<String> conferenceableCallIds) {
    Rlog.m77d(
        LOG_TAG,
        "setConferenceableConnections - callId: "
            + callId
    ...
    for (IConnectionServiceAdapter adapter : this.mAdapters) {
      try {
        adapter.requestCallEndpointChange(
            callId,
            endpoint,
            new ResultReceiverC29411(null, executor, callback),
                  ...
                  OutcomeReceiver.this.onError(
                      (CallEndpointException)
                          result.getParcelable(
                              CallEndpointException.CHANGE_ERROR, CallEndpointException.class));
                }
              });
        }
      } finally {
    ...
    for (IConnectionServiceAdapter adapter : this.mAdapters) {
      try {
        adapter.queryLocation(
            ...
            timeoutMillis,
            provider,
            new ResultReceiverC29422(null, executor, callback),
            Log.getExternalSession());
      } catch (RemoteException e) {
            ...
            new Runnable() { // from class:
              ...
              public final void run() {
                OutcomeReceiver.this.onResult(
                    (Location)
                        result.getParcelable(Connection.EXTRA_KEY_QUERY_LOCATION, Location.class));
              }
            });
      } else {
        Executor executor2 = this.val$executor;
