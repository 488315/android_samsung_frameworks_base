...
public class PhoneAccountSuggestionService extends Service {
  ...
  private IPhoneAccountSuggestionService mInterface =
      new IPhoneAccountSuggestionService
          .Stub() { // from class: android.telecom.PhoneAccountSuggestionService.1
  ...
  public final void suggestPhoneAccounts(String number, List<PhoneAccountSuggestion> suggestions) {
    ...
    if (callback == null) {
      Log.m75w(this, "No suggestions requested for the number %s", Log.pii(number));
      return;
    }
    try {
      callback.suggestPhoneAccounts(number, suggestions);
    } catch (RemoteException e) {
      Log.m75w(this, "Remote exception calling suggestPhoneAccounts", new Object[0]);
    }
  }
}
