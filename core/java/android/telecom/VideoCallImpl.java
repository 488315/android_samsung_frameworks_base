...
public class VideoCallImpl extends InCallService.VideoCall {
  ...
  private final class VideoCallListenerBinder extends IVideoCallback.Stub {
    ...
    public void receiveSessionModifyRequest(VideoProfile videoProfile) {
      Rlog.m77d(
          VideoCallImpl.LOG_TAG, "receiveSessionModifyRequest - videoProfile: " + videoProfile);
    ...
    public void receiveSessionModifyResponse(
      ...
      Rlog.m77d(
          VideoCallImpl.LOG_TAG,
          "receiveSessionModifyResponse - status: "
              + status
              + ", request: "
              + requestProfile
              + ", response: "
    ...
    public void changeCameraCapabilities(VideoProfile.CameraCapabilities cameraCapabilities) {
      Rlog.m77d(
          VideoCallImpl.LOG_TAG, "changeCameraCapabilities - capabilities: " + cameraCapabilities);
      if (VideoCallImpl.this.mHandler == null) {
        return;
      }
  ...
  VideoCallImpl(IVideoProvider videoProvider, String callingPackageName, int targetSdkVersion)
    ...
    Rlog.m77d(
        LOG_TAG,
        "New VideoCallImpl - videoProvider: "
            + videoProvider
            + ", binder: "
            + videoCallListenerBinder);
  }
  ...
  public void registerCallback(InCallService.VideoCall.Callback callback, Handler handler) {
    ...
    Rlog.m77d(
        LOG_TAG, "registerCallback - callback: " + this.mCallback + ", handler: " + this.mHandler);
  }
  ...
  public void unregisterCallback(InCallService.VideoCall.Callback callback) {
    if (callback != this.mCallback) {
      Rlog.m77d(
          LOG_TAG,
          "unregisterCallback is ignored. callback is different. mCallback: "
              + this.mCallback
              + ", callback: "
              + callback);
      return;
