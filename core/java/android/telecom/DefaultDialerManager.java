...
public class DefaultDialerManager {
    ...
    public static boolean setDefaultDialerApplication(Context context, String packageName, int user) {
        ...
        try {
            try {
                ...
                ((RoleManager) context.getSystemService(RoleManager.class)).addRoleHolderAsUser("android.app.role.DIALER", packageName, 0, UserHandle.m55of(user), AsyncTask.THREAD_POOL_EXECUTOR, callback);
    ...
    public static String getDefaultDialerApplication(Context context, int user) {
        long identity = Binder.clearCallingIdentity();
        try {
            return (String) CollectionUtils.firstOrNull(((RoleManager) context.getSystemService(RoleManager.class)).getRoleHoldersAsUser("android.app.role.DIALER", UserHandle.m55of(user)));
        } finally {
            Binder.restoreCallingIdentity(identity);
        }
    }
