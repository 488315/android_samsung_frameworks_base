  ...
  public static boolean setDefaultDialerApplication(Context context, String packageName, int user) {
    ...
    try {
      try {
        ...
        Slog.m116e(TAG, "Failed to set default dialer to " + packageName + " for user " + user, e);
        Binder.restoreCallingIdentity(identity);
        return false;
      }
    } catch (Throwable th) {
    ...
    try {
      return (String)
          CollectionUtils.firstOrNull(
              ((RoleManager) context.getSystemService(RoleManager.class))
                  .getRoleHoldersAsUser("android.app.role.DIALER", UserHandle.m55of(user)));
    } finally {
      Binder.restoreCallingIdentity(identity);
    }
  }
