...
public class EventManager {
    ...
    public Event(String eventId, String sessionId, long time, Object data) {
      ...
      this.timestampString =
          ZonedDateTime.ofInstant(Instant.ofEpochMilli(time), ZoneId.systemDefault())
              .format(EventManager.DATE_TIME_FORMATTER);
      this.data = data;
    }
  }
  ...
  public class EventRecord {
    ...
    private class PendingResponse {
      ...
      public PendingResponse(
          String requestEventId, long requestEventTimeMillis, long timeoutMillis, String name) {
    ...
    public void addEvent(String event, String sessionId, Object data) {
      ...
      Log.m72i("Event", "RecordEntry %s: %s, %s", this.mRecordEntry.getId(), event, data);
    }
      ...
      synchronized (this.mEvents) {
        ...
        while (it2.hasNext()) {
          ...
          if (!EventManager.this.requestResponsePairs.containsKey(event.eventId)) {
            ...
            Iterator it3 =
                ((List) EventManager.this.requestResponsePairs.get(event.eventId)).iterator();
            while (it3.hasNext()) {
              TimedEventPair p = (TimedEventPair) it3.next();
              pendingResponses.put(
                  p.mResponse,
                  new PendingResponse(event.eventId, event.time, p.mTimeoutMillis, p.mName));
              it3 = it3;
              it2 = it2;
            }
            it = it2;
      ...
      for (Event event : getEvents()) {
        ...
        if (event.data != null) {
          ...
          if ((data instanceof Loggable)
              && (record = (EventRecord) EventManager.this.mCallEventRecordMap.get(data)) != null) {
  ...
  public void event(Loggable recordEntry, String event, Object data) {
    ...
    if (recordEntry == null) {
      Log.m72i(TAG, "Non-call EVENT: %s, %s", event, data);
      return;
    }
    synchronized (this.mEventRecords) {
      ...
      if (record == null) {
        Log.m72i(TAG, "No EventRecord in CallEventRecordMap", new Object[0]);
      } else {
    ...
    if (args != null) {
      try {
        ...
        Log.m69e(
            this,
            ife,
            "IllegalFormatException: formatString='%s' numArgs=%d",
            format,
    ...
    Comparator<Pair<Loggable, Event>> byEventTime =
        Comparator.comparingLong(
            new ToLongFunction() { // from class:
                                   // android.telecom.Logging.EventManager$$ExternalSyntheticLambda0
  ...
  public void changeEventCacheSize(int newSize) {
    ...
    oldEventLog.forEach(
        new Consumer() { // from class:
                         // android.telecom.Logging.EventManager$$ExternalSyntheticLambda1
  ...
  public /* synthetic */ void lambda$changeEventCacheSize$1(EventRecord newRecord) {
    ...
    if (this.mEventRecords.remainingCapacity() == 0
        && (record = this.mEventRecords.poll()) != null) {
  ...
  private void addEventRecord(EventRecord newRecord) {
    ...
    try {
      ...
      Log.m72i(TAG, "addEventRecord - Can't add new record", new Object[0]);
    }
  }
}
