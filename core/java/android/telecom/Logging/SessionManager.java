...
public class SessionManager {
    ...
    public synchronized void startSession(String shortMethodName, String callerIdentification) {
        ...
        if (activeSession != null) {
            ...
            Log.m68d(LOGGING_TAG, Session.START_SESSION, new Object[0]);
    ...
    public synchronized void startExternalSession(Session.Info sessionInfo, String shortMethodName) {
        ...
        if (threadSession != null) {
            Log.m76w(LOGGING_TAG, "trying to start an external session with a session already active.", new Object[0]);
            return;
        }
        Log.m68d(LOGGING_TAG, Session.START_EXTERNAL_SESSION, new Object[0]);
    ...
    public synchronized Session createSubsession(boolean isStartedFromActiveSession) {
        ...
        Session threadSession = this.mSessionMapper.get(Integer.valueOf(threadId));
        if (threadSession == null) {
            Log.m68d(LOGGING_TAG, "Log.createSubsession was called with no session active.", new Object[0]);
            return null;
        }
        ...
        if (!isStartedFromActiveSession) {
            Log.m74v(LOGGING_TAG, "CREATE_SUBSESSION " + newSubsession.toString(), new Object[0]);
        } else {
            Log.m74v(LOGGING_TAG, "CREATE_SUBSESSION (Invisible subsession)", new Object[0]);
        }
    ...
    public synchronized Session.Info getExternalSession(String ownerInfo) {
        ...
        Session threadSession = this.mSessionMapper.get(Integer.valueOf(threadId));
        if (threadSession == null) {
            Log.m68d(LOGGING_TAG, "Log.getExternalSession was called with no session active.", new Object[0]);
            return null;
        }
    ...
    public synchronized void continueSession(Session subsession, String shortMethodName) {
        ...
        if (parentSession == null) {
            Log.m72i(LOGGING_TAG, "Log.continueSession was called with no session active for method " + shortMethodName, new Object[0]);
            return;
        }
        this.mSessionMapper.put(Integer.valueOf(getCallingThreadId()), subsession);
        if (!subsession.isStartedFromActiveSession()) {
            Log.m74v(LOGGING_TAG, Session.CONTINUE_SUBSESSION, new Object[0]);
        } else {
            Log.m74v(LOGGING_TAG, "CONTINUE_SUBSESSION (Invisible Subsession) with Method " + shortMethodName, new Object[0]);
        }
    }
    ...
    public synchronized void endSession() {
        ...
        if (completedSession == null) {
            Log.m76w(LOGGING_TAG, "Log.endSession was called with no session active.", new Object[0]);
            return;
        }
        completedSession.markSessionCompleted(System.currentTimeMillis());
        if (!completedSession.isStartedFromActiveSession()) {
            Log.m74v(LOGGING_TAG, "END_SUBSESSION (dur: " + completedSession.getLocalExecutionTime() + " mS)", new Object[0]);
        } else {
            Log.m74v(LOGGING_TAG, "END_SUBSESSION (Invisible Subsession) (dur: " + completedSession.getLocalExecutionTime() + " ms)", new Object[0]);
        }
    ...
    private void endParentSessions(Session subsession) {
        ...
        Log.m68d(LOGGING_TAG, "END_SESSION (dur: " + fullSessionTimeMs + " ms): " + subsession.toString(), new Object[0]);
    ...
    public synchronized void cleanupStaleSessions(long timeoutMs) {
        ...
        if (isSessionsStale) {
            Log.m76w(LOGGING_TAG, logMessage, new Object[0]);
        } else {
            Log.m74v(LOGGING_TAG, "No stale logging sessions needed to be cleaned...", new Object[0]);
        }
    }
