...
public class CallerInfoAsyncQuery {
      ...
      try {
        Context otherContext =
            context.createPackageContextAsUser(
                context.getPackageName(), 0, UserHandle.m55of(currentUser));
    ...
    protected class CallerInfoWorkerHandler extends AsyncQueryHandler.WorkerHandler {
      ...
      private void handleGeoDescription(Message msg) {
        ...
        if (!TextUtils.isEmpty(cw.number)
          ...
          cw.geoDescription =
              CallerInfo.getGeoDescription(CallerInfoAsyncQueryHandler.this.mContext, cw.number);
        ...
        if (this.mCallerInfo == null) {
          ...
          if (cw.event == 4) {
            ...
            CallerInfo newCallerInfo =
                CallerInfo.doSecondaryLookupIfNecessary(this.mContext, cw.number, this.mCallerInfo);
              ...
              new Runnable() { // from class:
                ...
                public void run() {
                  cw.listener.onQueryComplete(
                      token, cw.cookie, CallerInfoAsyncQueryHandler.this.mCallerInfo);
                }
              });
        } else {
          Log.m76w(
  ...
  public static CallerInfoAsyncQuery startQuery(
    ...
    Uri contactRef =
        ContactsContract.PhoneLookup.ENTERPRISE_CONTENT_FILTER_URI
            .buildUpon()
            .appendPath(number)
            .appendQueryParameter("sip", String.valueOf(PhoneNumberUtils.isUriNumber(number)))
            .build();
    CallerInfoAsyncQuery c = new CallerInfoAsyncQuery();
    c.allocate(context, contactRef);
    CookieWrapper cw = new CookieWrapper();
  ...
  private void allocate(Context context, Uri contactRef) {
    ...
    CallerInfoAsyncQueryHandler callerInfoAsyncQueryHandler =
        new CallerInfoAsyncQueryHandler(context);
