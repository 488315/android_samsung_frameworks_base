...
public class CallerInfoAsyncQuery {
    ...
    static ContentResolver getCurrentProfileContentResolver(Context context) {
        ...
        if (myUser != currentUser) {
            try {
                Context otherContext = context.createPackageContextAsUser(context.getPackageName(), 0, UserHandle.m55of(currentUser));
                return otherContext.getContentResolver();
            } catch (PackageManager.NameNotFoundException e) {
                Log.m70e(LOG_TAG, (Throwable) e, "Can't find self package", new Object[0]);
            }
        }
    ...
    private class CallerInfoAsyncQueryHandler extends AsyncQueryHandler {
        ...
        protected class CallerInfoWorkerHandler extends AsyncQueryHandler.WorkerHandler {
            ...
            public void handleMessage(Message msg) {
                ...
                if (cw == null) {
                    Log.m72i(CallerInfoAsyncQuery.LOG_TAG, "Unexpected command (CookieWrapper is null): " + msg.what + " ignored by CallerInfoWorkerHandler, passing onto parent.", new Object[0]);
                    super.handleMessage(msg);
                    return;
                }
                Log.m68d(CallerInfoAsyncQuery.LOG_TAG, "Processing event: " + cw.event + " token (arg1): " + msg.arg1 + " command: " + msg.what + " query URI: " + CallerInfoAsyncQuery.sanitizeUriToString(args.uri), new Object[0]);
                switch (cw.event) {
                    case 1:
                        super.handleMessage(msg);
                        break;
        ...
        protected void onQueryComplete(final int token, Object cookie, Cursor cursor) {
            ...
            Log.m68d(CallerInfoAsyncQuery.LOG_TAG, "##### onQueryComplete() #####   query complete for token: " + token, new Object[0]);
            final CookieWrapper cw = (CookieWrapper) cookie;
            if (cw == null) {
                Log.m72i(CallerInfoAsyncQuery.LOG_TAG, "Cookie is null, ignoring onQueryComplete() request.", new Object[0]);
                if (cursor != null) {
                    return;
                } else {
                    return;
            ...
            try {
                ...
                if (cw.listener != null) {
                    ...
                    Log.m76w(CallerInfoAsyncQuery.LOG_TAG, "There is no listener to notify for this query.", new Object[0]);
                }
                if (cursor != null) {
                    cursor.close();
                }
    ...
    public static CallerInfoAsyncQuery startQuery(int token, Context context, Uri contactRef, String number, OnQueryCompleteListener listener, Object cookie) {
        ...
        try {
            ...
            Log.m68d(LOG_TAG, "startQuery - isEmergencyNumber is fail. " + re, new Object[0]);
