...
public class AnimatorInflater {
    ...
    */
    throw new UnsupportedOperationException(
        "Method not decompiled:"
            ...
            + " android.content.res.Resources$Theme, int, float):android.animation.Animator");
  }
  ...
  */
  ...
  public static android.animation.StateListAnimator loadStateListAnimator(
      android.content.Context r14, int r15) throws android.content.res.Resources.NotFoundException {
    /*
          ...
          if (ImsConfig.EXTRA_CHANGED_ITEM.equals(parser.getName())) {
            ...
            for (int i = 0; i < attributeCount; i++) {
              ...
              if (attrName == 16843213) {
                ...
                states[stateIndex] =
                    attributeSet.getAttributeBooleanValue(i, false) ? attrName : -attrName;
                stateIndex = stateIndex2;
              }
            }
            if (animator == null) {
              animator =
                  createAnimatorFromXml(context.getResources(), context.getTheme(), parser, 1.0f);
            }
            if (animator == null) {
  ...
  private static class PathDataEvaluator implements TypeEvaluator<PathParser.PathData> {
    ...
    public PathParser.PathData evaluate(
        float fraction, PathParser.PathData startPathData, PathParser.PathData endPathData) {
    ...
    if (valueType2 == 2) {
      ...
      if (nodesFrom == null && nodesTo == null) {
        ...
        if (nodesFrom == null) {
          ...
          if (nodesTo != null) {
            returnValue2 =
                PropertyValuesHolder.ofObject(propertyName, new PathDataEvaluator(), nodesTo);
          }
        } else {
          ...
          if (nodesTo != null) {
            ...
            returnValue2 =
                PropertyValuesHolder.ofObject(propertyName, evaluator, nodesFrom, nodesTo);
            toType = toType2;
          } else {
            toType = toType2;
      ...
      if ((type != 3 || parser.getDepth() > depth) && type != 1) {
        if (type == 2) {
          ...
          if (name.equals("objectAnimator")) {
            ...
            if (theme == null) {
              a = res.obtainAttributes(attrs, C4337R.styleable.AnimatorSet);
            } else {
              a = theme.obtainStyledAttributes(attrs, C4337R.styleable.AnimatorSet, 0, 0);
            }
            anim2.appendChangingConfigurations(a.getChangingConfigurations());
            int ordering = a.getInt(0, 0);
            createAnimatorFromXml(
                res, theme, parser, attrs, (AnimatorSet) anim2, ordering, pixelSize);
            a.recycle();
            anim = anim2;
          } else {
  ...
  private static PropertyValuesHolder[] loadValues(
    ...
    while (true) {
      ...
      if (type != 2) {
        ...
        if (name.equals("propertyValuesHolder")) {
          if (theme != null) {
            a = theme.obtainStyledAttributes(attrs, C4337R.styleable.PropertyValuesHolder, 0, 0);
          } else {
            a = res.obtainAttributes(attrs, C4337R.styleable.PropertyValuesHolder);
          }
  ...
  private static int inferValueTypeOfKeyframe(
    ...
    int valueType;
    if (theme != null) {
      a = theme.obtainStyledAttributes(attrs, C4337R.styleable.Keyframe, 0, 0);
    } else {
      a = res.obtainAttributes(attrs, C4337R.styleable.Keyframe);
    }
  ...
  private static void dumpKeyframes(Object[] keyframes, String header) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      StringBuilder append =
          new StringBuilder().append("Keyframe ").append(i).append(": fraction ");
      ...
      Log.m94d(TAG, append2.append(obj).toString());
    }
  }
      ...
      if (size > 0) {
        ...
        while (i2 < count) {
          ...
          if (keyframe2.getFraction() >= startIndex) {
            ...
            int j2 = endIndex2 + 1;
            float gap =
                keyframeArray[j2].getFraction() - keyframeArray[startIndex2 - 1].getFraction();
  ...
  private static ValueAnimator loadAnimator(
    ...
    if (theme != null) {
      arrayAnimator = theme.obtainStyledAttributes(attrs, C4337R.styleable.Animator, 0, 0);
    } else {
      arrayAnimator = res.obtainAttributes(attrs, C4337R.styleable.Animator);
    }
    if (anim != null) {
      if (theme != null) {
        arrayObjectAnimator =
            theme.obtainStyledAttributes(attrs, C4337R.styleable.PropertyAnimator, 0, 0);
      } else {
        arrayObjectAnimator = res.obtainAttributes(attrs, C4337R.styleable.PropertyAnimator);
      }
