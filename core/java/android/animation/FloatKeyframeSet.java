...
class FloatKeyframeSet extends KeyframeSet implements Keyframes.FloatKeyframes {
  ...
  public FloatKeyframeSet mo554clone() {
    ...
    for (int i = 0; i < numKeyframes; i++) {
      newKeyframes[i] = (Keyframe.FloatKeyframe) keyframes.get(i).mo555clone();
    }
  ...
  public float getFloatValue(float fraction) {
    if (fraction <= 0.0f) {
      ...
      return ((Number)
              this.mEvaluator.evaluate(
                  intervalFraction, Float.valueOf(prevValue), Float.valueOf(nextValue)))
          .floatValue();
    }
    if (fraction >= 1.0f) {
      Keyframe.FloatKeyframe prevKeyframe2 =
          (Keyframe.FloatKeyframe) this.mKeyframes.get(this.mNumKeyframes - 2);
      Keyframe.FloatKeyframe nextKeyframe2 =
          (Keyframe.FloatKeyframe) this.mKeyframes.get(this.mNumKeyframes - 1);
      ...
      return ((Number)
              this.mEvaluator.evaluate(
                  intervalFraction2, Float.valueOf(prevValue2), Float.valueOf(nextValue2)))
          .floatValue();
    }
    ...
    for (int i = 1; i < this.mNumKeyframes; i++) {
      ...
      if (fraction < nextKeyframe3.getFraction()) {
        ...
        float intervalFraction3 =
            (fraction - prevKeyframe3.getFraction())
                / (nextKeyframe3.getFraction() - prevKeyframe3.getFraction());
        ...
        return ((Number)
                this.mEvaluator.evaluate(
                    intervalFraction3, Float.valueOf(prevValue3), Float.valueOf(nextValue3)))
            .floatValue();
      }
