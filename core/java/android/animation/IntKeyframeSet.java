...
class IntKeyframeSet extends KeyframeSet implements Keyframes.IntKeyframes {
  ...
  /* renamed from: clone */
  public IntKeyframeSet mo554clone() {
    ...
    for (int i = 0; i < numKeyframes; i++) {
      newKeyframes[i] = (Keyframe.IntKeyframe) keyframes.get(i).mo555clone();
    }
  ...
  public int getIntValue(float fraction) {
    ...
    if (fraction >= 1.0f) {
      Keyframe.IntKeyframe prevKeyframe2 =
          (Keyframe.IntKeyframe) this.mKeyframes.get(this.mNumKeyframes - 2);
      Keyframe.IntKeyframe nextKeyframe2 =
          (Keyframe.IntKeyframe) this.mKeyframes.get(this.mNumKeyframes - 1);
      ...
      return ((Number)
              this.mEvaluator.evaluate(
                  intervalFraction2, Integer.valueOf(prevValue2), Integer.valueOf(nextValue2)))
          .intValue();
    }
    ...
    for (int i = 1; i < this.mNumKeyframes; i++) {
      ...
      if (fraction < nextKeyframe3.getFraction()) {
        ...
        return ((Number)
                this.mEvaluator.evaluate(
                    intervalFraction3, Integer.valueOf(prevValue3), Integer.valueOf(nextValue3)))
            .intValue();
      }
