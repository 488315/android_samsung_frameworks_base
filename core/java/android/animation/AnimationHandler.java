...
public class AnimationHandler {
  ...
  private Choreographer.FrameCallback mPauser =
      new Choreographer
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (callback != null && isCallbackDue(callback, currentTime)) {
        ...
        if (this.mCommitCallbacks.contains(callback)) {
          getProvider()
              .postCommitCallback(
                  new Runnable() { // from class: android.animation.AnimationHandler.2
                    @Override // java.lang.Runnable
                    public void run() {
                      AnimationHandler animationHandler = AnimationHandler.this;
                      animationHandler.commitAnimationFrame(
                          callback, animationHandler.getProvider().getFrameTime());
                    }
                  });
        }
      }
