...
public class RestrictionsManager {
  ...
  public List<Bundle> getApplicationRestrictionsPerAdmin() {
    try {
      IRestrictionsManager iRestrictionsManager = this.mService;
      if (iRestrictionsManager != null) {
        return iRestrictionsManager.getApplicationRestrictionsPerAdminForUser(
  ...
  public void requestPermission(String requestType, String requestId, PersistableBundle request) {
    ...
    try {
      IRestrictionsManager iRestrictionsManager = this.mService;
      if (iRestrictionsManager != null) {
        iRestrictionsManager.requestPermission(
  ...
  public Intent createLocalApprovalIntent() {
    ...
    try {
      IRestrictionsManager iRestrictionsManager = this.mService;
      if (iRestrictionsManager != null
          && (result = iRestrictionsManager.createLocalApprovalIntent()) != null) {
  ...
  public List<RestrictionEntry> getManifestRestrictions(String packageName) {
    try {
      ...
      XmlResourceParser xml =
          appInfo.loadXmlMetaData(this.mContext.getPackageManager(), META_DATA_APP_RESTRICTIONS);
  ...
  private RestrictionEntry loadRestrictionElement(Context appContext, XmlResourceParser xml)
    ...
    if (xml.getName().equals(TAG_RESTRICTION) && (attrSet = Xml.asAttributeSet(xml)) != null) {
      TypedArray a = appContext.obtainStyledAttributes(attrSet, C4337R.styleable.RestrictionEntry);
  ...
  private RestrictionEntry loadRestriction(Context appContext, TypedArray a, XmlResourceParser xml)
    ...
    if (restrictionType == -1) {
      Log.m102w(TAG, "restrictionType cannot be omitted");
      return null;
    }
    if (key == null) {
      Log.m102w(TAG, "key cannot be omitted");
      return null;
    }
    ...
    switch (restrictionType) {
      ...
      case 3:
      default:
        Log.m102w(TAG, "Unknown restriction type " + restrictionType);
        return restriction;
      case 4:
      ...
      case 8:
        ...
        while (XmlUtils.nextElementWithin(xml, outerDepth)) {
          ...
          if (childEntry == null) {
            Log.m102w(TAG, "Child entry cannot be loaded for bundle restriction " + key);
          } else {
            restrictionEntries.add(childEntry);
            if (restrictionType == 8 && childEntry.getType() != 7) {
              Log.m102w(TAG, "bundle_array " + key + " can only contain entries of type bundle");
            }
          }
          context = appContext;
        }
        restriction.setRestrictions(
            (RestrictionEntry[])
                restrictionEntries.toArray(new RestrictionEntry[restrictionEntries.size()]));
        return restriction;
    }
  }
      ...
      case 8:
        ...
        for (int i = 0; i < bundleRestrictionArray.length; i++) {
          ...
          if (bundleRestrictions == null) {
            Log.m102w(TAG, "addRestrictionToBundle: Non-bundle entry found in bundle array");
