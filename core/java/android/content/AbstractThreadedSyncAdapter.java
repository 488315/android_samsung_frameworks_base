...
public abstract class AbstractThreadedSyncAdapter {
                  ...
                  new BiConsumer() { // from class:
                    ...
                    public final void accept(Object obj, Object obj2) {
                      ((AbstractThreadedSyncAdapter) obj)
                          .handleOnUnsyncableAccount((ISyncAdapterUnsyncableAccountCallback) obj2);
                    }
                  },
    ...
    public void startSync(
      ...
      if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
        ...
        Log.m94d(
            ...
            "startSync() start " + authority + " " + account + " " + extras);
      }
      try {
        try {
          try {
            ...
            synchronized (AbstractThreadedSyncAdapter.this.mSyncThreadLock) {
              if (AbstractThreadedSyncAdapter.this.mSyncThreads.containsKey(threadsKey)) {
                ...
                if (alreadyInProgress2) {
                  Log.m94d(AbstractThreadedSyncAdapter.TAG, "  alreadyInProgress");
                }
                alreadyInProgress = true;
              } else {
                if (AbstractThreadedSyncAdapter.this.mAutoInitialize
                    && extras != null
                  ...
                  try {
                    ...
                    if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
                      Log.m94d(AbstractThreadedSyncAdapter.TAG, "startSync() finishing");
                      return;
                    }
                    return;
                  } finally {
                ...
                SyncThread syncThread =
                    new SyncThread(
                        "SyncAdapterThread-"
                            + AbstractThreadedSyncAdapter.this.mNumSyncStarts.incrementAndGet(),
                        syncContextClient,
                        authority,
                        account,
                        extras);
            ...
            th = e;
            if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
              Log.m95d(AbstractThreadedSyncAdapter.TAG, "startSync() caught exception", th);
            }
            throw th;
          }
        } catch (Throwable th) {
      ...
      try {
        try {
          ...
          if (info != null) {
            ...
            Log.m102w(AbstractThreadedSyncAdapter.TAG, "cancelSync() unknown context");
          }
        } catch (Error | RuntimeException th) {
          if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
            Log.m95d(AbstractThreadedSyncAdapter.TAG, "cancelSync() caught exception", th);
          }
          throw th;
        }
      } finally {
        if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
          Log.m94d(AbstractThreadedSyncAdapter.TAG, "cancelSync() finishing");
        }
      }
    }
  }
  ...
  private class SyncThread extends Thread {
    ...
    private SyncThread(
        String name, SyncContext syncContext, String authority, Account account, Bundle extras) {
      super(name);
    ...
    public void run() {
      ...
      if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
        Log.m94d(AbstractThreadedSyncAdapter.TAG, "Thread started");
      }
      ...
      try {
        try {
          try {
            if (isCanceled()) {
              if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
                Log.m94d(AbstractThreadedSyncAdapter.TAG, "Already canceled");
              }
              Trace.traceEnd(128L);
              if (0 != 0) {
                provider.release();
              ...
              if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
                Log.m94d(AbstractThreadedSyncAdapter.TAG, "Thread finished");
                return;
              }
              return;
            }
            if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
              Log.m94d(AbstractThreadedSyncAdapter.TAG, "Calling onPerformSync...");
            }
            ...
            try {
              if (provider2 != null) {
                AbstractThreadedSyncAdapter.this.onPerformSync(
                    this.mAccount, this.mExtras, this.mAuthority, provider2, syncResult);
              } else {
                syncResult.databaseError = true;
              }
              if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
                Log.m94d(AbstractThreadedSyncAdapter.TAG, "onPerformSync done");
              }
              Trace.traceEnd(128L);
              if (provider2 != null) {
                provider2.release();
              ...
              th = e;
              if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
                Log.m95d(AbstractThreadedSyncAdapter.TAG, "caught exception", th);
              }
              throw th;
              ...
              provider = provider2;
              if (AbstractThreadedSyncAdapter.ENABLE_LOG) {
                Log.m95d(AbstractThreadedSyncAdapter.TAG, "SecurityException", e);
              }
  ...
  public void handleOnUnsyncableAccount(ISyncAdapterUnsyncableAccountCallback cb) {
    ...
    try {
      ...
      Log.m97e(TAG, "Exception while calling onUnsyncableAccount, assuming 'true'", e);
      doSync = true;
    }
    try {
      cb.onUnsyncableAccountDone(doSync);
    } catch (RemoteException e2) {
      Log.m97e(TAG, "Could not report result of onUnsyncableAccount", e2);
    }
  }
