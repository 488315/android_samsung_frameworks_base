...
public abstract class ContentProvider implements ContentInterface, ComponentCallbacks2 {
  ...
  class Transport extends ContentProviderNative {
        ...
        try {
          try {
            return this.mInterface.query(
                uri2, projection, queryArgs, CancellationSignal.fromTransport(cancellationSignal));
    ...
    private void logGetTypeData(
      ...
      try {
        ProviderInfo cpi =
            ContentProvider.this
                ...
                .resolveContentProvider(
                    uri.getAuthority(), PackageManager.ComponentInfoFlags.m10of(128L));
        int callingUserId = UserHandle.getUserId(callingUid);
        Uri userUri =
            (!ContentProvider.this.mSingleUser
                    || UserHandle.isSameUser(ContentProvider.this.mMyUid, callingUid))
                ? uri
        ...
        if (cpi.forceUriPermissions
            ...
            && ContentProvider.this
                    .getContext()
                    .checkUriPermission(userUri, Binder.getCallingPid(), callingUid, 1)
                != 0) {
    ...
    @Override // android.content.IContentProvider
    public Uri insert(
        AttributionSource attributionSource, Uri uri, ContentValues initialValues, Bundle extras) {
      ...
      try {
        try {
          return this.mInterface.openTypedAssetFile(
              uri2, mimeType, opts, CancellationSignal.fromTransport(cancellationSignal));
        } catch (RemoteException e) {
          throw e.rethrowAsRuntimeException();
        }
      } finally {
    ...
    private void enforceFilePermission(AttributionSource attributionSource, Uri uri, String mode)
    ...
    private int enforceReadPermission(AttributionSource attributionSource, Uri uri)
      ...
      if (ContentProvider.this.mTransport.mReadOp != -1
          && ContentProvider.this.mTransport.mReadOp
              != AppOpsManager.permissionToOpCode(ContentProvider.this.mReadPermission)) {
    ...
    private int enforceWritePermission(AttributionSource attributionSource, Uri uri)
      ...
      if (ContentProvider.this.mTransport.mWriteOp != -1
          && ContentProvider.this.mTransport.mWriteOp
              != AppOpsManager.permissionToOpCode(ContentProvider.this.mWritePermission)) {
        return PermissionChecker.checkOpForDataDelivery(
            ContentProvider.this.getContext(),
  ...
  boolean checkUser(int pid, int uid, Context context) {
    ...
    if (callingUserId == context.getUserId()
        || this.mSingleUser
        || ((SemDualAppManager.isDualAppId(UserHandle.getUserId(uid)) && context.getUserId() == 0)
            || ((SemDualAppManager.isDualAppId(context.getUserId())
                    && UserHandle.getUserId(uid) == 0)
                || context.checkPermission(Manifest.permission.INTERACT_ACROSS_USERS, pid, uid) == 0
                || context.checkPermission(Manifest.permission.INTERACT_ACROSS_USERS_FULL, pid, uid)
                    == 0))) {
      return true;
    }
  ...
  private boolean isContentRedirectionAllowedForUser(int incomingUserId) {
    ...
    if (um != null
        && um.getUserProperties(UserHandle.m55of(incomingUserId)).isMediaSharedWithParent()
        && (parent = um.getProfileParent(UserHandle.m55of(incomingUserId))) != null
  ...
  protected int semEnforceReadPermission(
      Uri uri, String callingPkg, String attributionTag, IBinder callerToken)
      throws SecurityException {
    return 2;
  }
  ...
  @Deprecated(forRemoval = true, since = "13.0")
  protected int semEnforceWritePermission(
      Uri uri, String callingPkg, String attributionTag, IBinder callerToken)
      throws SecurityException {
    return 2;
  }
  ...
  public void shutdown() {
    Log.m102w(
        TAG,
        "implement ContentProvider shutdown() to make sure all database connections are gracefully"
            + " shutdown");
  }
  ...
  public void validateIncomingAuthority(String authority) throws SecurityException {
    ...
    if (!matchesOurAuthorities(getAuthorityWithoutUserId(authority))) {
      String message2 =
          "The authority " + authority + " does not match the one of the contentProvider: ";
  ...
  public Uri validateIncomingUri(Uri uri) throws SecurityException {
    ...
    if (!this.mSingleUser
        && (userId = getUserIdFromAuthority(auth, -2)) != -2
    ...
    if (encodedPath != null && encodedPath.indexOf("//") != -1) {
      ...
      Log.m102w(
          TAG, "Normalized " + uri + " to " + normalized + " to avoid possible security issues");
      return normalized;
    }
    return uri;
  }
  ...
  public static UserHandle getUserHandleFromUri(Uri uri) {
    return UserHandle.m55of(getUserIdFromUri(uri, Process.myUserHandle().getIdentifier()));
  }
