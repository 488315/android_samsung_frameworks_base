...
public class ApexEnvironment {
  ...
  public File getDeviceProtectedDataDirForUser(UserHandle user) {
    return Environment.buildPath(
        Environment.getDataMiscDeDirectory(user.getIdentifier()), APEX_DATA, this.mApexModuleName);
  }

  public File getCredentialProtectedDataDirForUser(UserHandle user) {
    return Environment.buildPath(
        Environment.getDataMiscCeDirectory(user.getIdentifier()), APEX_DATA, this.mApexModuleName);
  }
}
