...
import android.p009os.incremental.IIncrementalServiceConnector;
...
public class FileSystemControlParcel implements Parcelable {
  public static final Parcelable.Creator<FileSystemControlParcel> CREATOR =
      new Parcelable.Creator<
          FileSystemControlParcel>() { // from class: android.content.pm.FileSystemControlParcel.1
    ...
    try {
      ...
      this.incremental =
          (IncrementalFileSystemControlParcel)
              _aidl_parcel.readTypedObject(IncrementalFileSystemControlParcel.CREATOR);
      if (_aidl_parcel.dataPosition() - _aidl_start_pos >= _aidl_parcelable_size) {
        if (_aidl_start_pos > Integer.MAX_VALUE - _aidl_parcelable_size) {
          throw new BadParcelableException("Overflow in the size of parcelable");
        }
