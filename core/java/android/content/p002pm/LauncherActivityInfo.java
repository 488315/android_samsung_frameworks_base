...
public class LauncherActivityInfo {
    ...
    public Drawable semGetBadgedIconForIconTray(int density) {
        ...
        if (PmUtils.supportLiveIcon(activityInfo.applicationInfo, this.mContext)) {
            Log.m98i(TAG, "Trying to load live icon for " + pkgName);
        ...
        if (badgedIcon != null) {
            Log.m98i(TAG, "packageName: " + pkgName + ", useThemeIcon: " + useThemeIcon + ", height: " + badgedIcon.getIntrinsicHeight() + ", width: " + badgedIcon.getIntrinsicWidth() + ", density: " + density);
        }
        return badgedIcon;
    }
