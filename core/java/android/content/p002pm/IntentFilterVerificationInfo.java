...
public final class IntentFilterVerificationInfo implements Parcelable {
  ...
  public static final Parcelable.Creator<IntentFilterVerificationInfo> CREATOR =
      new Parcelable.Creator<
          IntentFilterVerificationInfo>() { // from class:
  ...
  public void setStatus(int s) {
    if (s >= 0 && s <= 3) {
      ...
      Log.m102w(TAG, "Trying to set a non supported status: " + s);
    }
  }
  ...
  public void readFromXml(TypedXmlPullParser parser) throws XmlPullParserException, IOException {
    ...
    if (stringFromXml == null) {
      Log.m96e(TAG, "Package name cannot be null!");
    }
    int status = getIntFromXml(parser, "status", -1);
    if (status == -1) {
      Log.m96e(TAG, "Unknown status value: " + status);
    }
    this.mStatus = status;
