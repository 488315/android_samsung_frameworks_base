...
public abstract class RegisteredServicesCache<V> {
  ...
  private UserServices<V> findOrCreateUserLocked(int userId, boolean loadFromFileIfNew) {
    ...
    if (services == null) {
      ...
      if (loadFromFileIfNew
        ...
        AtomicFile file = createFileForUser(user.f54id);
        if (file.getBaseFile().exists()) {
          ...
          try {
            try {
              ...
              Log.m103w(TAG, "Error reading persistent services for user " + user.f54id, e);
            }
          } finally {
  ...
  private void generateServicesMap(int[] changedUids, int userId) {
    ...
    for (ResolveInfo resolveInfo : resolveInfos) {
      try {
        ...
        if (info == null) {
          Log.m102w(TAG, "Unable to load service info " + resolveInfo.toString());
        } else {
        ...
        while (it.hasNext()) {
          ...
          if (previousUid == null) {
            ...
            if (inSystemImage(info2.uid)
                || !containsTypeAndUid(serviceInfos, info2.type, previousUid.intValue())) {
              user.services.put(info2.type, info2);
              user.persistentServices.put(info2.type, Integer.valueOf(info2.uid));
              notifyListener(info2.type, userId, false);
              changed = true;
  ...
  protected ServiceInfo<V> parseServiceInfo(ResolveInfo service)
    ...
    int type;
    android.content.p002pm.ServiceInfo si = service.serviceInfo;
    ...
    try {
      try {
        ...
        if (v == null) {}
        android.content.p002pm.ServiceInfo serviceInfo = service.serviceInfo;
      ...
      try {
        ...
        try {
          for (UserInfo user : getUsers()) {
            UserServices<V> userServices = this.mUserServices.get(user.f54id);
            if (userServices != null) {
              writePersistentServicesLocked(userServices, user.f54id);
            }
          }
