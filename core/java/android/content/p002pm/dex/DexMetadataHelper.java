...
public class DexMetadataHelper {
  ...
  */
  public static ParseResult validateDexMetadataFile(
    ...
    if (DEBUG) {
      Log.m100v(
          TAG, "validateDexMetadataFile: " + dmaPath + ", " + packageName + ", " + versionCode);
    }
    try {
      try {
        jarFile = new StrictJarFile(dmaPath, false, false);
        try {
          ParseResult validateDexMetadataManifest =
              validateDexMetadataManifest(
                  input, dmaPath, jarFile, packageName, versionCode, requireManifest);
          try {
            jarFile.close();
          ...
          ParseResult error =
              input.error(
                  PackageManager.INSTALL_FAILED_BAD_DEX_METADATA, "Error opening " + dmaPath, e2);
          if (jarFile != null) {
            try {
              jarFile.close();
  ...
  private static ParseResult validateDexMetadataManifest(
    ...
    if (!requireManifest) {
      if (DEBUG) {
        Log.m100v(TAG, "validateDexMetadataManifest: " + dmaPath + " manifest.json check skipped");
      }
    ...
    if (zipEntry == null) {
      return input.error(
          PackageManager.INSTALL_FAILED_BAD_DEX_METADATA, "Missing manifest.json in " + dmaPath);
    }
    ...
    try {
      ...
      if (jsonPackageName == null || jsonVersionCode == -1) {
        return input.error(
            ...
            "manifest.json in " + dmaPath + " is missing 'packageName' and/or 'versionCode'");
      }
