  ...
  private void updateCache() {
    this.mNeverConstrainDisplayApisAllPackages =
        DeviceConfig.getBoolean(
            "constrain_display_apis", FLAG_NEVER_CONSTRAIN_DISPLAY_APIS_ALL_PACKAGES, false);
    String neverConstrainConfigStr =
        DeviceConfig.getString("constrain_display_apis", FLAG_NEVER_CONSTRAIN_DISPLAY_APIS, "");
    this.mNeverConstrainConfigMap = buildConfigMap(neverConstrainConfigStr);
    String alwaysConstrainConfigStr =
        DeviceConfig.getString("constrain_display_apis", FLAG_ALWAYS_CONSTRAIN_DISPLAY_APIS, "");
  ...
  private static ArrayMap<String, Pair<Long, Long>> buildConfigMap(String configStr) {
    ...
    while (i2 < length) {
      ...
      if (packageAndVersions.size() != 3) {
        ...
        try {
          long minVersion =
              minVersionCodeStr.isEmpty() ? Long.MIN_VALUE : Long.parseLong(minVersionCodeStr);
          long maxVersion =
              maxVersionCodeStr.isEmpty() ? Long.MAX_VALUE : Long.parseLong(maxVersionCodeStr);
          ...
          Slog.m121w(TAG, "Invalid APK version code in package entry: " + packageEntryString);
        }
      }
      i2++;
      i = 0;
