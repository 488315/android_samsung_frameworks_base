...
public class ResourcesProvider implements AutoCloseable, Closeable {
  ...
  public static ResourcesProvider loadOverlay(OverlayInfo overlayInfo) throws IOException {
    ...
    String path =
        (String)
            Preconditions.checkStringNotEmpty(overlayInfo.getBaseCodePath(), "Invalid base path");
  ...
  public static ResourcesProvider loadFromApk(
    ...
    return new ResourcesProvider(
        ApkAssets.loadFromFd(
            fileDescriptor.getFileDescriptor(), fileDescriptor.toString(), 4, assetsProvider));
  }

  public static ResourcesProvider loadFromApk(
      ParcelFileDescriptor fileDescriptor, long offset, long length, AssetsProvider assetsProvider)
      throws IOException {
    return new ResourcesProvider(
        ApkAssets.loadFromFd(
      ...
      if (this.mOpen) {
        if (this.mOpenCount != 0) {
          throw new IllegalStateException(
              "Failed to close provider used by " + this.mOpenCount + " ResourcesLoader instances");
        }
        this.mOpen = false;
        try {
  ...
  protected void finalize() throws Throwable {
    synchronized (this.mLock) {
      if (this.mOpenCount != 0) {
        Log.m102w(
            TAG, "ResourcesProvider " + this + " finalized with non-zero refs: " + this.mOpenCount);
      }
    }
  }
}
