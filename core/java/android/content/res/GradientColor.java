    ...
    */
        ...
        while (true) {
            ...
            if (type != i && ((depth = parser.getDepth()) >= innerDepth2 || type != 3)) {
                if (type != 2) {
                    ...
                    TypedArray a = Resources.obtainAttributes(r, theme, attrs, C4337R.styleable.GradientColorItem);
    ...
    private void onColorsChange() {
        ...
        if (tempColors.length < 2) {
            Log.m102w(TAG, "<gradient> tag requires 2 color values specified!" + tempColors.length + " " + Arrays.toString(tempColors));
        }
