...
public class PermissionCheckerManager {
    ...
    private final IPermissionChecker mService =
            IPermissionChecker.Stub.asInterface(
                    ServiceManager.getServiceOrThrow(Context.PERMISSION_CHECKER_SERVICE));

    @Retention(RetentionPolicy.SOURCE)
    public @interface PermissionResult {}

    public PermissionCheckerManager(Context context)
    ...
    public int checkPermission(
        ...
        if (AppOpsManager.permissionToOpCode(permission) == -1) {
            if (!fromDatasource) {
                return this.mContext.checkPermission(
                                        permission, attributionSource.pid, attributionSource.uid)
                                == 0
                        ? 0
                        : 2;
            }
        ...
        try {
            return this.mService.checkPermission(
                    permission,
                    attributionSource,
                    message,
                    forDataDelivery,
    ...
    public int checkOp(
        ...
        Objects.requireNonNull(attributionSource);
        try {
            return this.mService.checkOp(
                    op, attributionSource, message, forDataDelivery, startDataDelivery);
        } catch (RemoteException e) {
            e.rethrowFromSystemServer();
            return 2;
        }
