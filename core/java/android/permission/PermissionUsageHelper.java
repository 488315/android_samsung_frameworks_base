...
import android.content.p002pm.Attribution;
...
public class PermissionUsageHelper
  ...
  private static boolean shouldShowIndicators() {
    return DeviceConfig.getBoolean("privacy", "camera_mic_icons_enabled", true)
  ...
  private static long getRecentThreshold(Long now) {
    return now.longValue()
        - DeviceConfig.getLong("privacy", RECENT_ACCESS_TIME_MS, DEFAULT_RECENT_TIME_MS);
  }
        ...
        this.mAttributionChains.computeIfAbsent(
            ...
            new Function() { // from class:
              ...
              public final Object apply(Object obj) {
                return PermissionUsageHelper.lambda$addLinkToChainIfNotPresentLocked$0(
                    (Integer) obj);
              }
            });
  ...
  public List<PermissionGroupUsage> getOpUsageData(boolean isMicMuted) {
    ...
    if (hasPhoneCall
      ...
      for (int usageNum = 0; usageNum < permUsages.size(); usageNum++) {
        if (telephonyManager.checkCarrierPrivilegesForPackage(permUsages.get(usageNum).packageName)
            == 1) {
    ...
    while (permGroupNum < usedPermGroups.size()) {
      ...
      ArrayMap<OpUsage, CharSequence> usagesWithLabels =
          permissionUsageHelper.getUniqueUsagesWithLabels(permGroup, rawUsages.get(permGroup));
      permissionUsageHelper.updateSubattributionLabelsMap(
          rawUsages.get(permGroup), subAttributionLabelsMap);
    ...
    for (OpUsage usage : usages) {
      if (usage.attributionTag != null && !subAttributionLabelsMap.containsKey(usage.packageName)) {
        subAttributionLabelsMap.put(
            usage.packageName, getSubattributionLabelsForPackage(usage.packageName, usage.uid));
      }
    }
  }
  ...
  private ArrayMap<String, String> getSubattributionLabelsForPackage(String packageName, int uid) {
    ...
    PackageInfo packageInfo =
        userContext
            .getPackageManager()
            .getPackageInfo(packageName, PackageManager.PackageInfoFlags.m11of(2147487744L));
    ...
    try {
      ...
      ApplicationInfo appInfo =
          userPkgManager.getApplicationInfoAsUser(
              packageName, PackageManager.ApplicationInfoFlags.m9of(0L), UserHandle.getUserId(uid));
      if (appInfo == null) {
        return false;
      }
        ...
        while (opEntryNum2 < numOpEntries2) {
          ...
          while (numPkgOps3 < numAttrEntries) {
            ...
            if (lastAccessTime < recentThreshold && !attrOpEntry.isRunning()) {
              ...
              OpUsage usage =
                  new OpUsage(
                      packageName, attributionTag, op, uid, lastAccessTime, isRunning, proxyUsage);
      ...
      if (currentUsage2 == null) {
        ...
        while (currentUsage2.proxy != null) {
          ...
          if (!currentUsage.packageName.equals(start.packageName)
            ...
            try {
              PackageManager userPkgManager =
                  permissionUsageHelper.getUserContext(currentUsage.getUser()).getPackageManager();
              ApplicationInfo appInfo =
      ...
      while (i3 < permissionUsageHelper.mAttributionChains.size()) {
        ...
        if (!usageList.isEmpty() && usageList.get(lastVisible).isEnd()) {
          if (usageList.get(0).isStart()
              && str.equals(getGroupForOp(usageList.get(0).usage.f350op))
              && Manifest.permission_group.MICROPHONE.equals(str)) {
            ...
            while (lastVisible2 > 0
                && !permissionUsageHelper.shouldShowPackage(lastVisibleLink.usage.packageName)) {
              lastVisible2--;
              ...
              try {
                ...
                try {
                  ApplicationInfo appInfo2 =
                      userPkgManager2.getApplicationInfo(lastVisibleLink.usage.packageName, 0);
  ...
  private static class OpUsage {
    ...
    public boolean equals(Object obj) {
      ...
      return Objects.equals(this.packageName, other.packageName)
          ...
          && Objects.equals(this.f350op, other.f350op)
          && this.uid == other.uid
  ...
  private static class AccessChainLink {
    ...
    AccessChainLink(String op, String packageName, String attributionTag, int uid, int flags) {
      this.usage =
          new OpUsage(packageName, attributionTag, op, uid, System.currentTimeMillis(), true, null);
      this.flags = flags;
    }
    ...
    public boolean packageAndOpEquals(
      ...
      return Objects.equals(op, this.usage.f350op)
