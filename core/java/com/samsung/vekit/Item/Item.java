...
public class Item extends Element implements ItemStatusListener {
  ...
  private int getRegionIndex(Region region) {
    if (region == null) {
      Log.m96e(this.TAG, "failed to get Region Index");
      return -1;
    }
  ...
  public Region getRegion(int index) {
    if (this.regionList.size() <= index || index < 0) {
      Log.m96e(this.TAG, "failed to get region (invalid index) : " + index);
      return null;
    }
  ...
  public void checkValidAnimation(Animation animation) throws Exception {
    ...
    if (!valid.booleanValue()) {
      throw new Exception(
          "isInvalidElement : please attach correct Animation(not TransitionAnimation) to Item.");
    }
  }
