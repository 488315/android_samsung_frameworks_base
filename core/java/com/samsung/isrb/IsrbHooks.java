...
public class IsrbHooks {
  ...
  private static final String[] ISRBSKIPSERVICE = {
    ...
    "com.android.server.cover.CoverManagerService",
  ...
  private static class LoggingHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread t, Throwable e) {
      ...
      if (IsrbHooks.mApplicationObject == null && 1000 == Process.myUid()) {
        IsrbHooks.Clog_e(
            IsrbHooks.TAG, "!@*** FATAL EXCEPTION IN SYSTEM PROCESS: " + t.getName(), e);
  ...
  private static class ISRBExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    private ISRBExceptionHandler(
        Thread.UncaughtExceptionHandler handler, LoggingHandler loggingHandler) {
    ...
    private void ensureLogging(Thread t, Throwable e) {
      if (!this.mLoggingHandler.mTriggered) {
        try {
          ...
          Slog.m113d(IsrbHooks.TAG, "Ignored !!!");
        }
      }
    }
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
      ...
      if (IsrbHooks.mApplicationObject == null
          && 1000 == Process.myUid()
          && !IsrbHooks.checkServiceState()) {
        Slog.m113d(IsrbHooks.TAG, "checkServiceState is NULL");
        IsrbHooks.mState = 0;
      ...
      if (IsrbHooks.mState == 0 || IsrbHooks.mState == 1) {
        ...
        Slog.m113d(IsrbHooks.TAG, "back to RESCUEPARTY,begin to default handler!");
      ...
      if (!handleException(ex) && this.mHandler != null) {
        Slog.m113d(IsrbHooks.TAG, "Use DefaultHanlder!");
        IsrbHooks.mState = 0;
        this.mHandler.uncaughtException(thread, ex);
        return;
      }
      if ("android.bg".equals(thread.getName())) {
        Slog.m113d(IsrbHooks.TAG, "set NULL to instance");
        BackgroundThread.isrbresetInstance();
      }
      if ("WifiHandlerThread".equals(thread.getName())) {
        Slog.m113d(IsrbHooks.TAG, "set SystemProperties for wifi");
        SystemProperties.set("sys.isrb.wificrash", Boolean.toString(true));
      }
      if (thread.getName().indexOf("Wifi") >= 0
          || thread.getName().indexOf("Network") >= 0
          || thread.getName().indexOf("Connectivity") >= 0) {
        Slog.m113d(IsrbHooks.TAG, "set SystemProperties for networkcrash");
        SystemProperties.set("sys.isrb.networkcrash", Boolean.toString(true));
      }
      if ("android.fg".equals(thread.getName())
          || "android.ui".equals(thread.getName())
          || "ActivityManager".equals(thread.getName())
          || "PackageManager".equals(thread.getName())
          || "android.anim".equals(thread.getName())
          || "android.display".equals(thread.getName())
          || "ObserverHandler".equals(thread.getName())
          || "android.io".equals(thread.getName())) {
        Slog.m113d(IsrbHooks.TAG, "android thread loop");
        while (true) {
          try {
            Looper.loop();
          } catch (Throwable th) {
            Slog.m113d(IsrbHooks.TAG, "Catch Exception in thread again!");
          }
        }
      } else {
        if (IsrbHooks.mApplicationObject == null && 1000 == Process.myUid()) {
          ...
          checkhandler.postDelayed(
              new Runnable() { // from class: com.samsung.isrb.IsrbHooks.ISRBExceptionHandler.1
                @Override // java.lang.Runnable
                public void run() {
                  if (IsrbHooks.getEnterIdle()) {
                    Slog.m113d(IsrbHooks.TAG, "successfully enter idle");
                  } else {
                    IsrbHooks.mState = 2;
                    Slog.m113d(IsrbHooks.TAG, "can not enter idle, we should back to rescue party");
          ...
          while (true) {
            try {
              ...
              if (!isInHandleMessage(e)) {
                Slog.m113d(IsrbHooks.TAG, "count ++ !");
                mCrashMainCount++;
              }
              if (IsrbHooks.mState == 2 || mCrashMainCount >= 5) {
                Slog.m113d(IsrbHooks.TAG, "back to RESCUEPARTY,call default handler!");
                IsrbHooks.useDefaultSetting();
    ...
    private boolean isChoreographerException(Throwable e) {
      ...
      for (int i = elements.length - 1; i > -1; i--) {
        if (elements.length - i > 30) {
          Slog.m113d(IsrbHooks.TAG, "isChoreographerException---stack to long");
          return false;
        }
        ...
        if (("android.view.Choreographer".equals(element.getClassName())
            ...
            || "onStart".equals(element.getMethodName())
            || "onResume".equals(element.getMethodName())
            || "onPause".equals(element.getMethodName())
            || "onStop".equals(element.getMethodName())
            || "onDestroy".equals(element.getMethodName())) {
          return true;
        }
      }
      return false;
    ...
    private boolean isInHandleMessage(Throwable e) {
      ...
      for (int i = elements.length - 1; i > -1; i--) {
        if (elements.length - i > 30) {
          Slog.m113d(IsrbHooks.TAG, "isInHandleMessage---stack to long");
          return false;
        }
        StackTraceElement element = elements[i];
    ...
    private boolean isBooting(Throwable e) {
      ...
      for (int i = elements.length - 1; i > -1; i--) {
        if (elements.length - i > 30) {
          Slog.m113d(IsrbHooks.TAG, "isBooting---stack to long");
          return false;
        }
        StackTraceElement element = elements[i];
        if ("startOtherServices".equals(element.getMethodName())
            || "startCoreServices".equals(element.getMethodName())) {
          return true;
        }
      }
      return false;
    ...
    private boolean handleException(Throwable ex) {
      ...
      if (IsrbHooks.mApplicationObject == null && 1000 == Process.myUid()) {
        if (isBooting(ex)) {
          Slog.m113d(IsrbHooks.TAG, "is booting cause crash!");
          return false;
        }
        return true;
      }
      if (isChoreographerException(ex)) {
        Slog.m113d(IsrbHooks.TAG, "is viewroot cause crash!");
        return false;
      }
      return true;
    }
  ...
  public static void init() {
    ...
    if (SystemProperties.getBoolean(PROP_ENABLE_ISRB, false) && !"isrb_boot".equals(str)) {
      ...
      if (!("system_server".equals(currentProcessName)
          || AsPackageName.SYSTEMUI.equals(currentProcessName)
          || "com.android.networkstack.process".equals(currentProcessName)
          || "com.android.phone".equals(currentProcessName))) {
        return;
      }
      Thread.setDefaultUncaughtExceptionHandler(
          new ISRBExceptionHandler(
              Thread.getDefaultUncaughtExceptionHandler(), new LoggingHandler()));
      mState = 0;
    }
  }
  ...
  public static boolean getEnterIdle() {
    try {
      ...
      Log.m97e(TAG, "RemoteException in isBootCompleteState : ", e);
      return false;
    }
  }
  ...
  public static void useDefaultSetting() {
    try {
      ...
      Log.m97e(TAG, "RemoteException in setIsrbEnable : ", e);
    }
  }
  ...
  public static void setFakeTime() {
    try {
      ...
      Log.m97e(TAG, "RemoteException in setFakeTime : ", e);
    }
  }
