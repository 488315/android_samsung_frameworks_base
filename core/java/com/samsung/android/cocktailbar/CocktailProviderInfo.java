...
public class CocktailProviderInfo implements Parcelable {
    ...
    try {
      Resources resources =
          pkgMgr.getResourcesForApplicationAsUser(
              cn.getPackageName(), UserHandle.getUserId(ri.activityInfo.applicationInfo.uid));
      Binder.restoreCallingIdentity(identity);
      try {
        CocktailProviderInfo pInfo =
            new CocktailProviderInfo(context, pkgMgr, resources, cn, xml, ri, version);
        try {
          ...
          Log.m96e(TAG, "XML resources failed");
          return null;
        } catch (IllegalArgumentException e3) {
          Log.m96e(TAG, "IllegalArgumentException");
          return null;
        }
      ...
      e = e6;
      try {
        Log.m97e(TAG, "failed to load find package", e);
        Binder.restoreCallingIdentity(identity);
        return null;
      } catch (Throwable th2) {
        e = th2;
  ...
  private CocktailProviderInfo(
    ...
    if (TextUtils.isEmpty(category)) {
      ...
      while (categorySplitter.hasNext()) {
        ...
        switch (categoryId) {
          case -1:
            Log.m96e(TAG, "Provider: " + provider + " specified an invalid catetory of " + c);
            this.category = -1;
            return;
          case 4:
          case 32:
  ...
  private static boolean enforceValidCategory(
    ...
    Log.m98i(TAG, "enforceValidCategory: there is no category filters");
    return true;
  }
