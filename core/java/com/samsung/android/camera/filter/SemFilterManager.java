...
public class SemFilterManager {
  ...
  private static final String[] FILTER_PROJECTION = {
    "name", FILTER_FILE_NAME, "package_name", "vendor", "category", "version", FILTER_TITLE_ID
  };
    ...
    switch (type) {
      case 100:
        String filterIdentifier =
            filterPackageName + "," + filterFileName.substring(filterPackageName.length() + 1);
      ...
      case 101:
        SemFilter selfieFaceCorrection =
            new SemFilterImpl(
                ...
                TYPE_EFFECT_DISTORTION_CORRECTION,
                "Selfie Face Correction",
                "SAMSUNG_MOBILE",
                0,
                0);
        FilterList.add(selfieFaceCorrection);
        SemFilter customcolorFilter =
            new SemFilterImpl(
                "com.samsung.android.provider",
                "CustomColor",
                TYPE_EFFECT_CUSTOMCOLOR,
                "Custom Color",
                "SAMSUNG_MOBILE",
                0,
                0);
        FilterList.add(customcolorFilter);
        SemFilter foodFilter =
            new SemFilterImpl(
                "com.samsung.android.provider", "Food", 450, "Food", "SAMSUNG_MOBILE", 0, 0);
  ...
  */
    ...
    try {
      try {
        cursor =
            this.mContext
                .getContentResolver()
                .query(MYFILTER_URI, null, null, null, "filter_order");
        if (cursor == null) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
          while (cursor.moveToNext()) {
            ...
            if (myFilterName != null
                && !myFilterName.equals("Unnamed filter")
                && (myFilterFileName = cursor.getString(2)) != null
                && !myFilterFileName.equals("Unnamed filter")) {
              Log.m98i(
                  TAG,
                  "myFilterName : " + myFilterName + ", myFilterFileName : " + myFilterFileName);
              SemFilter Myfilter =
                  new SemFilterImpl(
                      "",
                      myFilterName,
                      MYFILTER_SEPERATOR + myFilterFileName,
                      myFilterName,
                      "",
                      0,
                      0);
  ...
  private List<SemFilter> loadFilter() {
    ...
    ArrayList<SemFilter> FilterList = new ArrayList<>();
    Cursor cursor =
        this.mContext.getContentResolver().query(FILTER_URI, FILTER_PROJECTION, null, null, null);
    try {
      if (cursor == null) {
