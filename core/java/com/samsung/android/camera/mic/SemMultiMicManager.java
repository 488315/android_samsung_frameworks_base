...
public class SemMultiMicManager {
  ...
  public void initialize(int soundLocation, int deviceOrientation, float maxZoom, float minZoom) {
    String parameters =
        new AudioParameter.Builder()
            ...
            .setParam(AUDIO_PARAMETER_SUBKEY_MULTI_MIC_PHONE_ORIENTATION, deviceOrientation)
  ...
  public boolean setMicSensitivity(int channel, int level) {
    int i = this.mMode;
    if (i != 1 && i != 2) {
      Log.m96e(TAG, "Current mode is not MODE_ADJUSTING_SENSITIVITY");
      return false;
    }
    if (level < -12 || level > 12) {
      Log.m96e(TAG, "Invalid level " + level + " in setMicSensitivity");
      return false;
    }
    String parameters =
        new AudioParameter.Builder()
  ...
  public boolean setMode(int mode) {
    if (!isValidMode(mode)) {
      Log.m96e(TAG, "Invalid mode " + mode + " in setMode");
      return false;
    }
  ...
  public boolean setSoundLocation(int soundLocation) {
    if (soundLocation < 0 || soundLocation > 1) {
      Log.m96e(TAG, "Invalid location " + soundLocation + " in setSoundLocation");
      return false;
    }
    String parameters =
        new AudioParameter.Builder()
  ...
  public static boolean isSupported(int mode) {
    if (!isValidMode(mode)) {
      Log.m96e(TAG, "Invalid mode " + mode + " in isSupported");
      return false;
    }
    int version = 0;
    try {
    ...
    return mode == 0
        ...
        : mode == 1
            ...
            : mode == 2 ? version >= 8020 || (version >= 7020 && version < 8000) : version >= 8001;
  }
  ...
  private static void setAudioServiceConfig(String keyValuePairs) {
    ...
    try {
      ...
      Log.m97e(TAG, "Not allowed to audio routing", e2);
    }
  }
