  ...
  private static String get(ContentResolver resolver, String pref) {
    ...
    try {
      try {
        String selection = "pref='" + pref + "'";
        cursor =
            resolver.query(SemEmergencyConstants.URI_PREFSETTINGS, null, selection, null, null);
  ...
  */
  ...
  private static String checkForMCC() {
    ...
    if (size_readystate > 0 && arrIsReady[0].equals("READY")) {
      Elog.m254d(
          TAG,
          "checkForMCC: requested Country : mcc1[0] " + mcc1[0] + " sim ready = " + arrIsReady[0]);
      return mcc1[0];
    }
    if (size_readystate > 1 && arrIsReady[1].equals("READY")) {
      Elog.m254d(
          TAG,
          "checkForMCC: requested Country : mcc1[1] " + mcc1[1] + " sim ready = " + arrIsReady[1]);
      return mcc1[1];
    }
    ...
    Elog.m254d(
        TAG, "checkForMCC: requested Country : default cond. sim ready = " + arrIsReadyValue);
    return null;
  }
