...
public final class WifiControlHistoryProvider extends ContentProvider {
  ...
  public static final class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void createTable(SQLiteDatabase db) {
      ...
      try {
        db.execSQL(
            "CREATE TABLE WifiHistory(conrol_id INTEGER PRIMARY KEY AUTOINCREMENT,package_name"
                + " TEXT, time_stamp LONG, enable_number INTEGER, disable_number INTEGER,"
                + " first_control LONG, last_control INTEGER) ");
    ...
    ContentValues checkAndGetContentValues(Cursor cursor, ContentValues values) {
      ...
      filteredValues.put(
          ENABLE_NUMBER,
          Integer.valueOf(
              getInteger(ENABLE_NUMBER, cursor, values, 0) + (lastControl == 1 ? 1 : 0)));
      filteredValues.put(
          DISABLE_NUMBER,
          Integer.valueOf(
              getInteger(DISABLE_NUMBER, cursor, values, 0) + (lastControl == 0 ? 1 : 0)));
  ...
  public Cursor query(
    ...
    StringBuilder append =
        new StringBuilder()
            .append("query table ")
            .append(this.mDbHelper.getTableName())
            .append(" where ")
            .append(selection)
            .append(" arg length ");
    ...
    StringBuilder append2 =
        append
            .append(
                selectionArgs != null
                    ? Integer.valueOf(selectionArgs.length)
                    : SemCapabilities.FEATURE_TAG_NULL)
            .append(" projection length ");
    if (projection != null) {
    ...
    try {
      cursor =
          query(
              ...
              this.mDbHelper.getDefaultSortOrder());
    } catch (SQLiteException e) {
