...
public class SemWifiApContentProviderHelper {
  ...
  public static void insert(Context mContext, String key, String val) {
    boolean hasPermission =
        mContext.checkPermission(
                Manifest.permission.OVERRIDE_WIFI_CONFIG, -1, Binder.getCallingUid())
            == 0;
    if (hasPermission) {
      ...
      try {
        try {
          ...
          Log.m96e(TAG, "SemWifiApContentProviderHelper insert:" + e);
        }
        if (isKeypresent(mContext, key)) {
          ...
          Log.m98i(TAG, "Inserting Key:" + key);
        }
      } finally {
  ...
  public static String get(Context mContext, String key) {
    ...
    try {
      try {
        Cursor c =
            mContext.getContentResolver().query(CONTENT_URI, null, "name = ?", selectionArgs, null);
        if (c != null) {
          try {
        ...
        Log.m96e(TAG, "SemWifiApContentProviderHelper get:" + e);
      }
      return returnValue;
    } finally {
      Binder.restoreCallingIdentity(ident);
