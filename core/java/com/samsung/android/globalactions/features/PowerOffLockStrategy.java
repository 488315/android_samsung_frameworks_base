...
public class PowerOffLockStrategy implements SecureConfirmStrategy {
    ...
    public PowerOffLockStrategy(
            ConditionChecker conditionChecker, BiometricPromptWrapper biometricPromptWrapper) {
    ...
    public boolean doActionBeforeSecureConfirm(
            final ActionViewModel viewModel, final SamsungGlobalActions globalActions) {
        if (this.mConditionChecker.isEnabled(SystemConditions.IS_SECURE_KEYGUARD)) {
            return true;
        }
        if (this.mConditionChecker.isEnabled(
                SystemConditions.IS_POWER_OFF_UNLOCK_ALWAYS_REQUIRED)) {
            CancellationSignal signal = new CancellationSignal();
            this.mBiometricPromptWrapper.initPrompt(" ", 33023);
            this.mBiometricPromptWrapper.setRunnable(
                    new Runnable() { // from class:
                                     // com.samsung.android.globalactions.features.PowerOffLockStrategy$$ExternalSyntheticLambda0
                    ...
                    new Runnable() { // from class:
                                     // com.samsung.android.globalactions.features.PowerOffLockStrategy$$ExternalSyntheticLambda1
                        @Override // java.lang.Runnable
                        public final void run() {
    ...
    public boolean isNeedSecureConfirm() {
        return (this.mConditionChecker.isEnabled(SystemConditions.IS_RMM_LOCKED)
                        ...
                        || this.mConditionChecker.isEnabled(
                                SystemConditions.IS_POWER_OFF_UNLOCK_NOT_REQUIRED)
                        || !this.mBiometricPromptWrapper.canAuthenticate(33023))
                ? false
                : true;
    }
