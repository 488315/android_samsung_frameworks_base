...
public class SktStrategy implements ActionInteractionStrategy {
    ...
    public boolean onPressPowerAction() {
        this.mLogWrapper.m266v(TAG, "onPressPowerAction");
        if (this.mConditionChecker.isEnabled(SystemConditions.IS_SECURE_KEYGUARD) && this.mConditionChecker.isEnabled(SystemConditions.IS_TSAFE_LOCK)) {
            this.mToastController.showToast(this.mResourcesWrapper.getString(C4337R.string.globalactions_unable_power_off_msg), 1);
            return true;
        }
        if (this.mConditionChecker.isEnabled(SystemConditions.IS_CARRIRER_LOCK_PLUS_ENABLED)) {
            this.mToastController.showToast(this.mResourcesWrapper.getString(C4337R.string.globalactions_unable_power_off_msg_fmm), 1);
            return true;
        }
        return false;
    }
    ...
    public boolean onPressRestartAction() {
        this.mLogWrapper.m266v(TAG, "onPressRestartAction");
        if (this.mConditionChecker.isEnabled(SystemConditions.IS_CARRIRER_LOCK_PLUS_ENABLED)) {
            this.mToastController.showToast(this.mResourcesWrapper.getString(C4337R.string.globalactions_unable_restart_msg_fmm), 1);
            return true;
        }
        return false;
    }
    ...
    public boolean onPressEmergencyModeAction() {
        this.mLogWrapper.m266v(TAG, "onPressEmergencyModeAction");
        if (this.mConditionChecker.isEnabled(SystemConditions.IS_CARRIRER_LOCK_PLUS_ENABLED)) {
            this.mToastController.showToast(this.mResourcesWrapper.getString(C4337R.string.globalactions_unable_emergency_msg_fmm), 1);
            return true;
        }
        return false;
    }
