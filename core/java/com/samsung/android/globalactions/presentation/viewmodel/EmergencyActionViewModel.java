...
public class EmergencyActionViewModel implements ActionViewModel {
  ...
  public EmergencyActionViewModel(
  ...
  public void setStateLabel() {
    if (this.mConditionChecker.isEnabled(SystemConditions.IS_EMERGENCY_MODE)) {
      ...
      this.mInfo.setStateLabel(
          this.mResourcesWrapper.getString(C4337R.string.global_action_emergency_mode_on_status));
    } else {
      this.mToggleState = ActionViewModel.ToggleState.off;
      this.mInfo.setStateLabel(
          this.mResourcesWrapper.getString(C4337R.string.global_action_emergency_mode_off_status));
    }
  }
  ...
  public void onPress() {
    ...
    if (this.mConditionChecker.isEnabled(SystemConditions.IS_FMM_LOCKED)) {
      if (this.mToggleState == ActionViewModel.ToggleState.off) {
        if (this.mConditionChecker.isEnabled(SystemConditions.IS_TABLET_DEVICE)) {
          res = C4337R.string.globalactions_unable_emergency_msg_fmm_tablet;
        } else {
          res = C4337R.string.globalactions_unable_emergency_msg_fmm;
        }
      } else if (this.mConditionChecker.isEnabled(SystemConditions.IS_TABLET_DEVICE)) {
        res = C4337R.string.globalactions_unable_emergency_off_msg_fmm_tablet;
      } else {
        res = C4337R.string.globalactions_unable_emergency_off_msg_fmm;
      }
    ...
    if (this.mConditionChecker.isEnabled(SystemConditions.IS_BIKE_MODE)
        && !this.mConditionChecker.isEnabled(SystemConditions.IS_EMERGENCY_MODE_USER_AGREEMENT)) {
      String emergencyModeText =
          this.mResourcesWrapper.getString(C4337R.string.global_action_toggle_emergency_mode);
      String sBikeModeText = this.mResourcesWrapper.getString(C4337R.string.globalactions_bikemode);
      this.mToastController.showToast(
          this.mResourcesWrapper.getString(
              C4337R.string.globalactions_unable_emergency_mode_msg_in_bike_mode,
              emergencyModeText,
              sBikeModeText),
          1);
    } else {
      ...
      this.mSAnalytics.sendEventLog(
          ...
          SamsungGlobalActionsAnalytics.DID_EMERGENCY_MODE,
          3L);
