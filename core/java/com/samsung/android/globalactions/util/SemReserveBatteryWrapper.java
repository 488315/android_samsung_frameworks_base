...
public class SemReserveBatteryWrapper {
  ...
  public boolean isReserveBatteryMode() {
    ...
    if (this.mConfigYuvaDownloadable) {
      mRBM =
          Settings.Secure.getInt(this.mContext.getContentResolver(), "reserve_battery_on", 0) != 0;
      mEnableRBM =
          Settings.Secure.getInt(this.mContext.getContentResolver(), "enable_reserve_max_mode", 0)
              != 0;
    } else {
      mRBM =
          Settings.System.getInt(this.mContext.getContentResolver(), "reserve_battery_on", 0) != 0;
      mEnableRBM =
          Settings.System.getInt(this.mContext.getContentResolver(), "enable_reserve_max_mode", 0)
              != 0;
    }
