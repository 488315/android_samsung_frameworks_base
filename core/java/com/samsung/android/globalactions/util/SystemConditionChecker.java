...
public class SystemConditionChecker implements ConditionChecker {
    ...
    static /* synthetic */ class C50321 {
        ...
        static {
            ...
            try {
                f2969x921795bd[SystemConditions.IS_POWER_OFF_UNLOCK_ONLY_LOCKSCREEN_REQUIRED.ordinal()] = 48;
    ...
    public boolean isEnabled(Object name) {
        try {
            ...
            this.mLogWrapper.m265i(TAG, NavigationBarInflaterView.SIZE_MOD_START + condition.name().toLowerCase() + "] " + ret + " (" + (current.longValue() - time.longValue()) + NavigationBarInflaterView.KEY_CODE_END);
            return ret;
        } catch (Exception ex) {
            ex.printStackTrace();
            return false;
