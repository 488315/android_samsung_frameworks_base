...
public class ColorThemeExtractor {
  ...
  public static int[] getSeedColors(Bitmap bitmap) {
    ...
    int[] clusterGroupArray =
        ColorPaletteExtractor.makeClusterGroupColorBandBasedFromHueInterval(
            30, clusterGroupSVvalues);
    float bitmapScale =
        BitmapHelper.fineScaleValueBySquareRootSize(bitmap.getWidth(), bitmap.getHeight(), 150);
    float bitmapScale2 = bitmapScale <= 1.0f ? bitmapScale : 1.0f;
    Bitmap resizedSampleImage =
        Bitmap.createScaledBitmap(
            bitmap,
            (int) (bitmap.getWidth() * bitmapScale2),
            (int) (bitmap.getHeight() * bitmapScale2),
            false);
    ...
    ColorExtractor.DominantColorResult[] resultDominantColors =
        ColorPaletteExtractor.kMeansHsv(resizedSampleImage, clusterGroupArray);
    ColorPaletteExtractor.discardSameColorFromDominantColorsForColorPalette(
        resultDominantColors, 45.0f / 360.0d, ColorPaletteExtractor.ColorSpace.HUE, true);
    int[] resultColors =
        ColorPaletteExtractor.getOnlyColorsFromDominantColor(resultDominantColors, 45.0f);
