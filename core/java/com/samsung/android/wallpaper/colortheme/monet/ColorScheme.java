...
public class ColorScheme {
  ...
  public static List<Integer> getSeedColors(WallpaperColors wallpaperColors, boolean filter) {
    ...
    for (Map.Entry<Integer, Cam> entry4 : filteredIntToCam.entrySet()) {
      intToScoreIntermediate.put(
          entry4.getKey(),
          Double.valueOf(
              score(entry4.getValue(), intToHueProportion.get(entry4.getKey()).doubleValue())));
    }
  ...
  private static final String stringForColor(int color) {
    ...
    String t =
        'T' + String.format("%04d", Integer.valueOf(Math.round(CamUtils.lstarFromInt(color))));
    String hex =
        String.format("%-06s", Integer.toHexString(16777215 & color).toUpperCase(Locale.ROOT));
    return h + c + t + " = #" + hex;
  }
  ...
  private static <K, V extends Comparable<? super V>> List<Map.Entry<K, V>> entriesSortedByValues(
    ...
    Collections.sort(
        ...
        new Comparator<
            Map.Entry<
                K,
                V>>() { // from class: com.samsung.android.wallpaper.colortheme.monet.ColorScheme.1
