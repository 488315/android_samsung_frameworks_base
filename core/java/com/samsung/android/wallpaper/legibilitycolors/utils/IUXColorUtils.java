  ...
  public static float calculateLuminance(int r, int g, int b) {
    ...
    return (float)
        ((0.2126d * (nr < 0.04045d ? nr / 12.92d : Math.pow((nr + 0.055d) / 1.055d, 2.4d)))
            + (0.7152d * (ng < 0.04045d ? ng / 12.92d : Math.pow((ng + 0.055d) / 1.055d, 2.4d)))
            + (0.0722d * (nb < 0.04045d ? nb / 12.92d : Math.pow((nb + 0.055d) / 1.055d, 2.4d))));
  }
    ...
    if (hsvA[0] == hsvB[0]) {
      ...
      boolean cw =
          (hueVector_color_B[0] * ((double) IUXMathUtils.cos(shiftedAngleA)))
                  + (hueVector_color_B[1] * ((double) IUXMathUtils.sin(shiftedAngleA)))
      ...
      adjustedHue =
          hue > 180.0f
              ? (((hue - 180.0f) / (252.0f - 180.0f)) * hueScopeLength_cb)
                  ...
                  + hueScopeLength_lc
              : hue > 72.0f
                  ? (((hue - 72.0f) / (180.0f - 72.0f)) * hueScopeLength_lc) + 72.0f
                  : hue;
    }
