        ...
        try {
            ...
            if (parser != null) {
                ...
                applyComponentRuntimeManifest(
                        packageName,
                        getActivityInfoList(pkg.activities),
                        overlay.getActivityPolicies());
                applyComponentRuntimeManifest(
                        packageName,
                        getServiceInfoList(pkg.services),
                        overlay.getServicePolicies());
                applyComponentRuntimeManifest(
                        packageName,
                        getProviderInfoList(pkg.providers),
                        overlay.getProviderPolicies());
                applyComponentRuntimeManifest(
                        packageName,
                        getActivityInfoList(pkg.receivers),
                        overlay.getReceiverPolicies());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for (Map.Entry<String, List<RuntimeManifestPolicies.PolicyInfo>> entry :
            ...
            if (targetName != null && policies != null) {
                ...
                if (matchingComponent == null) {
                    Slog.d(
                            "RuntimeManifestUtils",
                            "Target " + targetName2 + " not found in manifest");
                } else {
