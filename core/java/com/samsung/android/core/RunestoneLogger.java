...
public class RunestoneLogger {
    ...
    private static void sendMultiWindowState(Context context, ArrayList<String> packageList) {
        ...
        switch (packageList.size()) {
            ...
            case 3:
                ...
                break;
            default:
                Log.m102w(TAG, "Warning sendPairLoggingLocked [" + packageList + NavigationBarInflaterView.SIZE_MOD_END);
                return;
        }
    ...
    public static void sendRunestoneLogging(Context context, int screenType, int screenState, ArrayList<String> packageList) {
        ...
        if (DEBUG) {
            Log.m94d(TAG, "insertLog screenType : " + screenType + ", screenState : " + screenState + ", timeStamp : " + timeStamp + ", packageList : " + (packageList != null ? packageList.toString() : SemCapabilities.FEATURE_TAG_NULL));
        }
        ...
        if (packageList != null && !packageList.isEmpty()) {
            ...
            Log.m94d(TAG, "Send failed. it's MULTI_WINDOW Type, but package list is null");
            return;
        }
