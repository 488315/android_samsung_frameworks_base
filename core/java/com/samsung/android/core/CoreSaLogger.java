...
public class CoreSaLogger {
  ...
  private static void sendLogToServer(
    ...
    try {
      ActivityTaskManager.getService()
          .sendSaLoggingBroadcast(trackingId, eventId, eventDetail, value, customDimension);
            ...
            new Runnable() { // from class:
              ...
              public final void run() {
                CoreSaLogger.lambda$sendSaLoggingBroadcast$0(
                    eventId, detail, value, customDimension, mode, trackingId, context);
              }
            });
  }
  ...
  static /* synthetic */ void lambda$sendSaLoggingBroadcast$0(
    ...
    if (CoreRune.SAFE_DEBUG) {
      Log.m94d(
          TAG,
          "sendSaLoggingBroadcast eventId : "
              + eventId
              + ", eventDetail : "
              + detail
              + ", value : "
              + (value != -1 ? Long.valueOf(value) : SemCapabilities.FEATURE_TAG_NULL)
              + ", customDimension : "
              + customDimension
              + ", mode : "
              + mode);
  ...
  public static void sendSaLoggingBroadcastForSetting(
      final Context context, final String trackingId, final String settingId, final String value) {
