...
public class DatabaseCleanResult {
  ...
  public static DatabaseCleanResult fromBundle(Bundle in) {
    if (!in.containsKey(Contract.DatabaseClean.Extra.DELETED_SIZE_BYTES)
        || !in.containsKey(Contract.DatabaseClean.Extra.DELETED_EVENTS_COUNT)) {
      Log.m287e(TAG, "fromBundle(): invalid bundle.");
      return null;
    }
    return new DatabaseCleanResult(
        in.getLong(Contract.DatabaseClean.Extra.DELETED_SIZE_BYTES),
        in.getLong(Contract.DatabaseClean.Extra.DELETED_EVENTS_COUNT));
  }
