...
public class ZipHandler {
    ...
    public static ZipResult deflate(byte[] content) {
        if (content.length <= 0) {
            Log.m287e(TAG, "deflate(): Empty object byte array");
            return null;
        }
        ...
        deflater.end();
        Log.m286d(TAG, "deflate(): bytes size: " + content.length + ", bytes size after compression: " + zippedLength + ",  bytes saved: " + (content.length - zippedLength));
    ...
    public static EventList inflate(ZipResult zip) throws JSONException, DataFormatException {
        ...
        String str = TAG;
        Log.m286d(str, "inflate(): actual number of uncompressed bytes: " + origLength + " original number of uncompressed bytes: " + zip.getOriginalLength());
        inflate.end();
        if (origLength <= 0) {
            Log.m286d(str, "inflate(): Could not return to decompress data");
            return null;
        }
