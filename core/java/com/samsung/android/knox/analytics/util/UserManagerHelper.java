...
public class UserManagerHelper {
    ...
    public boolean isAnyPOActive() {
        ...
        try {
            ...
            for (UserInfo userInfo : userInfoList) {
                try {
                    if (devicePolicyManager.getProfileOwnerAsUser(userInfo.f54id) != null) {
                        hasAnyPo = true;
                    }
                } catch (RemoteException e) {
                    Log.m288e(TAG, "isAnyPOActive() - Remote exception: ", e);
                }
            }
            Log.m286d(TAG, "isAnyPOActive(): " + String.valueOf(hasAnyPo));
            return hasAnyPo;
        } catch (RuntimeException e2) {
            Log.m288e(TAG, "isAnyPOActive() - Runtime exception: ", e2);
            return false;
        }
    }
    ...
    public String getPoPackageName(int userId) {
        ...
        try {
            ...
            Log.m288e(TAG, "getPoPackageName() - Remote exception: ", e);
        }
        Log.m286d(TAG, "getPoPackageName(int userId): " + userId + " - " + poPackageName);
    ...
    public String getPoPackageName() {
        ...
        try {
            ...
            if (userInfoList != null) {
                for (UserInfo userInfo : userInfoList) {
                    try {
                        ComponentName profileComponent = devicePolicyManager.getProfileOwnerAsUser(userInfo.f54id);
                        if (profileComponent != null) {
                            poPackageName = profileComponent.getPackageName();
                        }
                    } catch (RemoteException e) {
            ...
            Log.m286d(TAG, "getPoPackageName(): " + poPackageName);
            return poPackageName;
        } catch (RuntimeException e2) {
            Log.m288e(TAG, "getPoPackageName() - Runtime exception: ", e2);
            return null;
        }
    }
    ...
    public String getDoPackageName() {
        ...
        try {
            ...
            Log.m288e(TAG, "isDoActive(): Exception in DPMS.getDeviceOwnerComponent - ", e);
        }
        Log.m286d(TAG, "getDoPackageName(): " + doPackageName);
    ...
    public boolean isDoActive() {
        ...
        try {
            ...
            Log.m288e(TAG, "isDoActive(): Exception in DPMS.hasDeviceOwner - ", e);
        }
        Log.m286d(TAG, "isDoActive(): " + String.valueOf(res));
        return res;
    }

    public int getUserType(int userId) {
        String str = TAG;
        Log.m286d(str, "getUserType(" + userId + NavigationBarInflaterView.KEY_CODE_END);
        UserInfo ui = getUserInfo(userId);
        if (ui == null) {
            Log.m287e(str, "getUserType(): UserInfo is null!");
            return -1;
        }
    ...
    private IDevicePolicyManager getDevicePolicyManagerService() {
        if (this.mDevicePolicyManagerService == null) {
            ...
            if (iDevicePolicyManager == null) {
                Log.m287e(TAG, "getDevicePolicyManagerService(): could not get DevicePolicyManager!");
            }
        }
