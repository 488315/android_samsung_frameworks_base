...
public class SecurityUtils {
  ...
  private static final boolean IS_SHIP =
      Boolean.parseBoolean(SemSystemProperties.get("ro.product_ship", "false"));
  private static final String[] WHITELIST_FOR_TEST = {
    "com.android.frameworks.knoxservicestests",
    "com.samsung.android.knox.analytics.testapp",
    "com.samsung.android.knox.kpu.demo",
    "com.samsung.android.knox.kpu.poc",
    "com.samsung.knoxautomation",
    "android",
    "root"
  };
  ...
  public static void enforcePackageNameForContentProvider(String callingPackage, int pid) {
    if (pid == Process.myPid()) {
      Log.m286d(TAG, "enforcePackageNameForContentProvider(): MyPid");
    } else if (UploaderBroadcaster.UPLOADER_PACKAGENAME.equals(callingPackage)) {
      Log.m286d(TAG, "enforcePackageNameForContentProvider(): allowed");
    } else if (isPackageWhitelisted(callingPackage)) {
    } else {
      throw new SecurityException(
          TAG
              + "Security Exception Occurred while caller "
              + callingPackage
              + " tried to access Content Provider");
    }
  }

  public static final void enforceInternalOnly(String callingPackage, int pid) {
    if (pid == Process.myPid()) {
      Log.m286d(TAG, "enforceInternalOnly(): MyPid");
    } else if (isPackageWhitelisted(callingPackage)) {
    } else {
      throw new SecurityException(
          TAG
  ...
  public static void enforceCallingPermissionForLog(
      Context context, int callingPid, int callingUid) {
    if (callingPid == Process.myPid()) {
      Log.m286d(TAG, "enforceCallingPermissionForLog(): MyPid");
    } else {
      if (context.checkCallingOrSelfPermission(Manifest.permission.KNOX_ANALYTICS_INTERNAL) == 0
          || context.checkCallingOrSelfPermission(Manifest.permission.KNOX_SOLUTION_SDK) == 0) {
        return;
      }
      ...
      if (isPackageWhitelisted(callingPackage)) {
        ...
        throw new SecurityException(
            TAG
                ...
                + SERVICE_KEEPER_SERVICE_NAME
                + "] in ["
                + SERVICE_KEEPER_METHOD_NAME
                + "] service");
      }
    }
  }
  ...
  private static boolean isPackageWhitelisted(String packageName) {
    ...
    for (String whitelistedPackage : WHITELIST_FOR_TEST) {
      if (whitelistedPackage.equals(packageName)) {
        Log.m286d(TAG, "Allowing whitelisted package for tests: " + packageName);
        return true;
      }
    }
    return false;
