...
class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String COMPRESSED_EVENTS_CREATE_TABLE =
            "CREATE TABLE compressed_events ( id INTEGER PRIMARY KEY, length INTEGER,"
                + " original_length INTEGER, bulk INTEGER DEFAULT 1000, content BLOB )";
    ...
    private static final String EVENTS_CREATE_TABLE =
            "CREATE TABLE events ( id INTEGER PRIMARY KEY, vid INTEGER, bulk INTEGER DEFAULT 1,"
                + " data BLOB )";
    private static final String EVENTS_DELETE =
    ...
    public Cursor getLastId() {
        ...
        Cursor cursor =
                db.query(
                        LAST_EVENT_ID_TABLE,
                        new String[] {LAST_EVENT_ID_FIELD},
                        null,
                        null,
                        null,
                        null,
    ...
    public Cursor getEventCount() {
        ...
        MatrixCursor cursor =
                new MatrixCursor(new String[] {Contract.Events.Projection.COUNT_ONLY}, 1);
    ...
    Cursor getCompressedEventChunk(Integer limit) {
        ...
        Cursor cursor =
                db.query("compressed_events", null, null, null, null, null, "id ASC", sLimit);
        if (cursor == null || cursor.getCount() <= 0) {
    ...
    public Cursor getEventCountCursor() {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor =
                db.query("events", new String[] {"bulk"}, null, null, null, null, null, null);
        if (cursor == null || cursor.getCount() <= 0) {
            if (cursor != null) {
                cursor.close();
                return null;
    ...
    public Cursor getTotalCompressedEventCursor(String limit) {
        Log.d(
                TAG,
                "getTotalCompressedEventCursor(" + limit + NavigationBarInflaterView.KEY_CODE_END);
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor =
                db.query(
                        "compressed_events",
    ...
    public Cursor getVersioningBlob() {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor =
                db.query("version", new String[] {"data", "id"}, null, null, null, null, null);
        return cursor;
    }
            ...
            try {
                if (cursor.getCount() > 0) {
                    cv.put(
                            "row_id",
                            Integer.valueOf(
                                    cursor.getInt(cursor.getColumnIndex(LAST_EVENT_ID_FIELD))));
        ...
        try {
            ...
            if (db.insert("compressed_events", null, values) == -1) {
                throw new SQLException(
                        "Transaction Failure. Not possible to insert compressed events.");
            }
            if (db.delete("events", EVENTS_DELETE, new String[] {String.valueOf(plainEventsSize)})
                    == 0) {
                throw new SQLException(
                        "Transaction Failure. Not possible to delete plain-text events.");
            }
