...
class EncryptedCursor extends AbstractCursor {
  ...
  public String getString(int column) {
    Log.m286d(this.TAG, "getString(" + column + NavigationBarInflaterView.KEY_CODE_END);
    if (this.mDatabaseCursor.getColumnName(column).equals("data")) {
      ...
      try {
        ...
        if (eventCounter > 1) {
          ...
          decrypted =
              this.mCryptoHandler.decrypt(this.mDatabaseCursor.getBlob(column), useLegacyKey());
        }
      } catch (UnsupportedEncodingException e) {
        Log.m288e(this.TAG, "getString(): UnsupportedEncodingException", e);
      } catch (InvalidKeyException e2) {
        Log.m288e(this.TAG, "getString(): InvalidKeyException", e2);
        this.mCryptoHandler.deleteAnalyticsLegacyKey();
      } catch (GeneralSecurityException e3) {
        Log.m288e(this.TAG, "getString(): GeneralSecurityException", e3);
      }
      if (decrypted == null) {
        Log.m287e(this.TAG, "getString(): null data.");
  ...
  private boolean useLegacyKey() {
    Log.m286d(this.TAG, "useLegacyKey()");
    ...
    if (this.mSyntheticRowId == -1) {
      Log.m286d(this.TAG, "useLegacyKey(): There is no marked event ID");
      return false;
    }
  ...
  public int getType(int column) {
    if (this.mDatabaseCursor.getColumnName(column).equals("data")) {
      Log.m286d(this.TAG, "getType(" + column + "): returning string for encrypted data");
      return 3;
    }
  ...
  public byte[] getBlob(int column) {
    Log.m286d(this.TAG, "getBlob(" + column + NavigationBarInflaterView.KEY_CODE_END);
