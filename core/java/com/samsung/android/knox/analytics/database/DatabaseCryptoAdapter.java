...
class DatabaseCryptoAdapter {
    ...
    private Cursor createMergedCursor(Integer requestedSize) {
        ...
        Log.m286d(str, "createCursorWith: " + compressedEventsCount + " compressed events and " + numberOfPlainEvents + " plain events");
    ...
    */
    private int getTotalPlainEvents(int requestedSize, int totalCompressedEvents) {
        ...
        if (requestedSize == totalCompressedEvents || (eventsCursor = this.mDbHelper.getEventCountCursor()) == null) {
            return 0;
        }
    ...
    private long deleteMergedChunk(long size, long numberOfEvents) {
        String str = TAG;
        Log.m286d(str, "deleteMergedChunk(" + size + ", " + numberOfEvents + NavigationBarInflaterView.KEY_CODE_END);
    ...
    private int checkCompressedChunksLimit(long numberOfEvents) {
        Log.m286d(TAG, "checkCompressedChunksLimit(" + numberOfEvents + NavigationBarInflaterView.KEY_CODE_END);
    ...
    private int calculateRemainingEventsForDelete(int numberOfEvents, int compressedDeleted) {
        ...
        Log.m286d(TAG, "calculateRemainingEventsForDelete(" + numberOfEvents + ", " + compressedDeleted + "): totalToDelete: " + totalEventsToDelete);
    ...
    public long deleteCompressedEventChunk(long size) {
        Log.m286d(TAG, "deleteCompressedEventChunk(size: " + size + NavigationBarInflaterView.KEY_CODE_END);
    ...
    public long deleteUntilTargetDbSize(long targetDbSize) {
        Log.m286d(TAG, "deleteUntilTargetDbSize(" + targetDbSize + NavigationBarInflaterView.KEY_CODE_END);
        return -1L;
    }
