...
public class SemPersonaManager {
  ...
  public static boolean isKnoxVersionSupported(int version) {
    ...
    if (version > 0
        && (currentVersion = getKnoxContainerVersion()) != null
        && currentVersion.getVersionNumber() >= version) {
      return true;
    }
    return false;
  }
  ...
  public void setAppSeparationDefaultPolicy(int userId) {
    ...
    if (iSemPersonaManager != null) {
      try {
        ...
        Log.m95d(
            TAG,
            "Could not call setAppSeparationDefaultPolicy , inside SemPersonaManager with"
                + " exception:",
            re);
      }
    }
  ...
  public boolean isInstallableAppInContainer(Context appContext, int containerId, String pkgName) {
    ...
    if (pkgName == null
        ...
        || SemCapabilities.FEATURE_TAG_NULL.equalsIgnoreCase(pkgName)
        || !isUserManaged()) {
      return false;
    }
    ...
    if (iSemPersonaManager != null) {
      try {
        ...
        Log.m95d(
            TAG,
            "Could not get isPossibleAddAppsToContainer , inside SemPersonaManager with exception:",
            re);
      }
    }
    return false;
    ...
    try {
      int masterSettingsVal =
          Settings.Secure.getIntForUser(
              context.getContentResolver(), "notifications_master_activation", 0, userId);
  ...
  public static Drawable getCustomReverseBadgeForCustomContainer(
    ...
    if (isSecureFolderId(userId)) {
      return Resources.getSystem()
          .getDrawableForDensity(C4337R.drawable.ic_sf_reverse_badge_bottom, density);
    }
    if (isDualAppId(userId)) {
      return Resources.getSystem()
          .getDrawableForDensity(C4337R.drawable.ic_reverse_dualapp_corner, density);
    }
    if (isAppSeparationUserId(userId)) {
      return Resources.getSystem()
          .getDrawableForDensity(C4337R.drawable.screen_badge_270_separated, density);
    }
    return Resources.getSystem()
        .getDrawableForDensity(C4337R.drawable.ws_reverse_ic_screen_work_bottom, density);
  }
  ...
  public static Pair<Boolean, Drawable> getCustomBadgeForCustomContainer(
    ...
    if (isSecureFolderId(user.getIdentifier())) {
      return new Pair<>(
          true,
          Resources.getSystem().getDrawableForDensity(C4337R.drawable.ic_sf_badge_bottom, density));
    }
    if (isAppSeparationUserId(user.getIdentifier())) {
      return new Pair<>(
          true,
          Resources.getSystem()
              .getDrawableForDensity(C4337R.drawable.screen_badge_separated, density));
    }
    if (isKnoxId(user.getIdentifier())) {
      return new Pair<>(
          true,
          Resources.getSystem()
              .getDrawableForDensity(C4337R.drawable.ws_ic_screen_work_bottom, density));
    }
  ...
  public static Pair<Boolean, Drawable> getNotificationBadge(
    ...
    if (badge != null) {
      return new Pair<>(
          true,
          new BitmapDrawable(
              locContext.getResources(), BitmapFactory.decodeByteArray(badge, 0, badge.length)));
    }
    ...
    if (isSecureFolderId(user.getIdentifier())) {
      Drawable rawDrawable =
          Resources.getSystem()
              .getDrawableForDensity(C4337R.drawable.stat_sys_secure_folder, density);
      rawDrawable.setTint(
          Resources.getSystem().getColor(C4337R.color.SecureFolder_notification_badge, null));
      return new Pair<>(true, rawDrawable);
    }
    if (isAppSeparationUserId(user.getIdentifier())) {
      Drawable rawDrawable2 =
          Resources.getSystem()
              .getDrawableForDensity(C4337R.drawable.knox_basic_separated, density);
      rawDrawable2.setTint(
          Resources.getSystem().getColor(C4337R.color.SeparatedApps_notification_badge, null));
  ...
  public static boolean isPkgAllowedToListenKnoxNoti(Context context, String pkgName) {
    try {
      ...
      if (appInfo != null && (appInfo.flags & 1) == 0) {
        if (context
                .getPackageManager()
                .checkPermission(Manifest.permission.READ_KNOX_NOTIFICATION, pkgName)
            != 0) {
          if (DEBUG) {
  ...
  public Intent createConfirmProfileCredentialIntent(
    ...
    Intent intent =
        getKeyguardManager().createConfirmDeviceCredentialIntent(title, description, userId);
    if (intent != null) {
  ...
  public static Drawable getSecureFolderIcon(Context context) {
    ...
    try {
      iconName =
          Settings.Secure.getStringForUser(
              context.getContentResolver(), Settings.Secure.SECURE_FOLDER_IMAGE_NAME, 0);
    } catch (Exception e) {
    ...
    sfDrawable =
        context.getPackageManager().semGetApplicationIconForIconTray(SECUREFOLDER_PACKAGE, 32);
    return sfDrawable;
  }
          ...
          if (isKnoxVersionSupported(220)) {
            ...
            if (lService != null) {
              try {
                ...
                Log.m94d(
                    TAG,
                    "Knox:: claimInterface : calling isPackageAllowedToAccessExternalSdcard for"
                        + " user- "
                        + userId
                        + " and callingUid-"
                        + callingUid);
                ...
                Log.m94d(
                    TAG,
                    "Knox:: claimInterface : calling isPackageAllowedToAccessExternalSdcard"
                        + " allowed-"
                        + allowed);
              } catch (RemoteException re) {
                Log.m103w(
                    TAG,
                    "doBind(): isPackageAllowedToAccessExternalSdcard on EDMProxy failed! ",
                    re);
              }
            }
            if (!allowed) {
    ...
    try {
      ...
      try {
        IServiceConnection sd =
            context.getServiceDispatcher(conn, context.getMainThreadHandler(), flags);
        try {
  ...
  public boolean setRCPDataPolicy(String appName, String policyProperty, String value) {
    try {
      ...
      Log.m95d(
          TAG, "Could not get setRCPDataPolicy , inside PersonaPolicyManager with exception:", re);
      return false;
    }
  }
