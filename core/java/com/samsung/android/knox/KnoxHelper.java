...
public class KnoxHelper {
        ...
        for (LauncherActivityInfoInternal internal : internals.getList()) {
            if (cList != null && cList.size() > 0) {
                ...
                while (true) {
                    ...
                    if (DEBUG) {
                        Log.m100v(TAG, "convertToActivityList " + user + " : " + cName + "," + internal.getComponentName() + ".equals():" + cName.equals(internal.getComponentName()));
                    }
    ...
    public static List<LauncherActivityInfo> getActivityList(Context context, ILauncherApps service, String packageName, UserHandle user) {
        ...
        Log.m94d(TAG, "getActivityList callingUserId: " + callingUid + ", target user: " + user.getIdentifier());
        ...
        if (SemPersonaManager.isSecureFolderId(user.getIdentifier()) || SemPersonaManager.isAppSeparationUserId(user.getIdentifier())) {
            Log.m100v(TAG, "source and target users are different, and caller is knox container or target user is for secure folder/ separated apps, so request cannot be granted!");
            return Collections.EMPTY_LIST;
        }
        return null;
    }
