...
public class WM7SearchMediaGetter implements IMediaGetter {
  ...
  private static class SearchRequest {
    ...
    public boolean isSame(SearchCriteria criteria, int startIndex) {
      if (criteria == null
          || !criteria.equals(this.mSearchCriteria)
          || this.mStartIndex != startIndex) {
        return false;
      }
      return true;
  ...
  private class FlatProviderConnectionInfo implements FlatProvider.IFlatProviderConnection {
    ...
    public FlatProviderConnectionInfo(
        FlatProvider.IFlatProviderConnection conn, Provider provider) {
      this.mConn = null;
      this.mConn = conn;
  ...
  public void start(
      ...
      FlatProvider.IFlatProviderConnection callback) {
  ...
  private void startFlatSearching(
      Provider provider,
      ArrayList<Item.MediaType> types,
      FlatProvider.IFlatProviderConnection callback) {
    callback.onStart();
    ...
    for (int i = 0; i < types.size(); i++) {
      ...
      if (type != Item.MediaType.ITEM_AUDIO
          && type != Item.MediaType.ITEM_VIDEO
          && type != Item.MediaType.ITEM_IMAGE) {
    ...
    provider.setSearchResponseListener(
        new Provider
            .IProviderSearchResponseListener() { // from class:
                                                 // com.samsung.android.allshare.extension.impl.WM7SearchMediaGetter.1
          @Override // com.samsung.android.allshare.media.Provider.IProviderSearchResponseListener
          public void onSearchResponseReceived(
            ...
            while (it.hasNext()) {
              ...
              if (searchCriteria == null) {
                ...
                SearchRequest request =
                    conn2.getCurrentSearchRequest(searchCriteria, requestedStartIndex);
                if (request == null) {
                  continue;
                  ...
                  if (endOfItems) {
                    ...
                    request.updateSearchRequest(
                        searchCriteria, requestedStartIndex + returnedCount);
                    WM7SearchMediaGetter.this.mProvider.search(
                        searchCriteria, requestedStartIndex + returnedCount, 50);
                  }
                }
              }
            }
