...
final class PlaylistPlayerImpl extends PlaylistPlayer {
  ...
  private PlaylistPlayer.IPlaylistPlayerPlaybackResponseListener mPlaylistPlayerResponseListener =
      null;
  ...
  AllShareEventHandler mEventHandler =
      new AllShareEventHandler(
          ServiceConnector
              .getMainLooper()) { // from class: com.samsung.android.allshare.PlaylistPlayerImpl.1
...

        {
          ...
          hashMap.put(
              AllShareEvent.EVENT_RENDERER_STATE_BUFFERING, PlaylistPlayer.PlayerState.BUFFERING);
          this.mPlaylistStateMap.put(
              AllShareEvent.EVENT_RENDERER_STATE_PAUSED, PlaylistPlayer.PlayerState.PAUSED);
          this.mPlaylistStateMap.put(
              AllShareEvent.EVENT_RENDERER_STATE_STOPPED, PlaylistPlayer.PlayerState.STOPPED);
          this.mPlaylistStateMap.put(
              AllShareEvent.EVENT_RENDERER_STATE_PLAYING, PlaylistPlayer.PlayerState.PLAYING);
          this.mPlaylistStateMap.put(
              AllShareEvent.EVENT_RENDERER_STATE_CONTENT_CHANGED,
              PlaylistPlayer.PlayerState.CONTENT_CHANGED);
        }
        ...
        public void handleEventMessage(CVMessage cvm) {
          ...
          try {
            ...
            DLog.w_api(
                PlaylistPlayerImpl.TAG_CLASS, "mEventHandler.handleEventMessage Exception ", e);
          }
          ...
          if (state.equals(PlaylistPlayer.PlayerState.CONTENT_CHANGED)) {
            if (PlaylistPlayerImpl.this.mCurrentPlayingContentUriStrList != null
                && !isContains(resBundle.getString(AllShareKey.BUNDLE_STRING_APP_ITEM_ID))) {
        ...
        private void notifyEvent(PlaylistPlayer.PlayerState state, ERROR error) {
          if (PlaylistPlayerImpl.this.mPlaylistPlayerEventListener != null) {
            try {
              PlaylistPlayerImpl.this.mPlaylistPlayerEventListener.onPlaylistPlayerStateChanged(
                  state, error);
            } catch (Exception e) {
  ...
  AllShareResponseHandler mAllShareRespHandler =
      new AllShareResponseHandler(
          ServiceConnector
              .getMainLooper()) { // from class: com.samsung.android.allshare.PlaylistPlayerImpl.2
        ...
        public void handleResponseMessage(CVMessage cvm) {
          ...
          if (actionID.equals(AllShareAction.ACTION_PLAYLIST_PLAYER_PLAY)) {
            PlaylistPlayerImpl.this.mCurrentPlayingContentUriStrList =
                resBundle.getStringArrayList(AllShareKey.BUNDLE_STRING_SERVER_URI_LIST);
          }
          ...
          if (!actionID.equals(AllShareAction.ACTION_PLAYLIST_PLAYER_PLAY)) {
            if (actionID.equals(AllShareAction.ACTION_PLAYLIST_PLAYER_SEEK)) {
              ...
              PlaylistPlayerImpl.this.mPlaylistPlayerResponseListener.onSeekResponseReceived(
                  trackNum, error);
              return;
            }
            ...
            if (actionID.equals(AllShareAction.ACTION_PLAYLIST_PLAYER_PERVIOUS)) {
              PlaylistPlayerImpl.this.mPlaylistPlayerResponseListener.onPreviousResponseReceived(
                  error);
              return;
            }
            ...
            if (actionID.equals(AllShareAction.ACTION_PLAYLIST_PLAYER_PAUSE)) {
              PlaylistPlayerImpl.this.mPlaylistPlayerResponseListener.onPauseResponseReceived(
                  error);
              return;
            }
            if (actionID.equals(AllShareAction.ACTION_PLAYLIST_PLAYER_RESUME)) {
              PlaylistPlayerImpl.this.mPlaylistPlayerResponseListener.onResumeResponseReceived(
                  error);
              return;
            } else {
              if (actionID.equals(AllShareAction.ACTION_PLAYLIST_PLAYER_REQUEST_PLAY_POSITION)) {
                ...
                PlaylistPlayerImpl.this.mPlaylistPlayerResponseListener
                    .onGetPlayPositionResponseReceived(trackNum2, error);
                return;
              }
              return;
            }
          }
          ArrayList<Bundle> playlist =
              resBundle.getParcelableArrayList(AllShareKey.BUNDLE_PARCELABLE_ARRAYLIST_CONTENT_URI);
          ...
          if (mimeType != null) {
            if (mimeType.contains("audio")) {
              ...
              PlaylistPlayerImpl.this.mPlaylistPlayerResponseListener.onPlayResponseReceived(
                  playlistBuilder.build(), trackNum3, error);
              return;
            }
            if (mimeType.contains("video")) {
              ...
              PlaylistPlayerImpl.this.mPlaylistPlayerResponseListener.onPlayResponseReceived(
                  playlistBuilder2.build(), trackNum4, error);
              return;
            }
            if (mimeType.contains("image")) {
              ...
              PlaylistPlayerImpl.this.mPlaylistPlayerResponseListener.onPlayResponseReceived(
                  playlistBuilder3.build(), trackNum5, error);
            }
          }
        }
      };

  PlaylistPlayerImpl(IAllShareConnector connector, DeviceImpl deviceImpl) {
  ...
  void setCurrentFilePath(String strFilePath) {
  ...
  public void play(Playlist playlist, int trackNumber) {
    ...
    if (iAllShareConnector == null || !iAllShareConnector.isAllShareServiceConnected()) {
      this.mPlaylistPlayerResponseListener.onPlayResponseReceived(
          playlist, trackNumber, ERROR.SERVICE_NOT_CONNECTED);
      return;
    }
    if (playlist == null) {
      this.mPlaylistPlayerResponseListener.onPlayResponseReceived(
          playlist, trackNumber, ERROR.INVALID_ARGUMENT);
      this.mFilePath = null;
      return;
    }
    if (trackNumber < 1) {
  ...
  public void seek(int trackNumber) {
    ...
    if (trackNumber < 1) {
      this.mPlaylistPlayerResponseListener.onSeekResponseReceived(
          trackNumber, ERROR.INVALID_ARGUMENT);
      return;
    }
  ...
  public void next() {
    ...
    if (playlist != null) {
      ...
      this.mTrackNumber = i;
      int size =
          i >= playlist.getPlaylist().size()
              ? this.mPlaylist.getPlaylist().size() - 1
              : this.mTrackNumber;
  ...
  public void getPlayPosition() {
    IAllShareConnector iAllShareConnector = this.mAllShareConnector;
    if (iAllShareConnector == null || !iAllShareConnector.isAllShareServiceConnected()) {
      this.mPlaylistPlayerResponseListener.onGetPlayPositionResponseReceived(
  ...
  @Override // com.samsung.android.allshare.media.PlaylistPlayer
  public void setPlaybackResponseListener(
      PlaylistPlayer.IPlaylistPlayerPlaybackResponseListener l) {
  ...
  public boolean isSupportAudio() {
    ...
    try {
      boolean result =
          res_bundle.getBoolean(AllShareKey.BUNDLE_BOOLEAN_SUPPORT_AUDIO_PLAYLIST_PLAYER);
      return result;
    } catch (Exception e) {
  ...
  public boolean isSupportVideo() {
    ...
    try {
      boolean result =
          res_bundle.getBoolean(AllShareKey.BUNDLE_BOOLEAN_SUPPORT_VIDEO_PLAYLIST_PLAYER);
      return result;
    } catch (Exception e) {
