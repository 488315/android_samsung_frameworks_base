...
public class PermissionAccessInformation {
    ...
    public PermissionAccessInformation(
            int op,
            int uid,
            String packageName,
    ...
    public boolean equals(Object o) {
        ...
        if (!"CHINA".equalsIgnoreCase(SystemProperties.get("ro.csc.country_code"))
                ? this.mAccessTime / 60000 == that.mAccessTime / 60000
                : this.mAccessTime == that.mAccessTime) {
            accessTime = true;
        } else {
            accessTime = false;
        }
        if (this.mOp == that.mOp
                && this.mUid == that.mUid
                && this.mIsBackground == that.mIsBackground
                && accessTime
                && this.mPackageName.equals(that.mPackageName)
                && this.mProxyPackageName.equals(that.mProxyPackageName)
                && this.mProxyAttributionTag.equals(that.mProxyAttributionTag)) {
            return true;
        }
        return false;
    }
    ...
    public int hashCode() {
        return Objects.hash(
                Integer.valueOf(this.mOp),
                Integer.valueOf(this.mUid),
                this.mPackageName,
                this.mProxyPackageName,
                this.mProxyAttributionTag,
                Boolean.valueOf(this.mIsBackground),
                Long.valueOf(this.mAccessTime / 60000));
    }

    public String toString() {
        return "PermissionAccessInformation{op="
                + this.mOp
                + ", uid="
                + this.mUid
                + DateFormat.QUOTE
