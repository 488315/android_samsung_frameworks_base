...
public class NNDirector {
  ...
  public SumeClient newVideoUpscaler(Option option) {
    ...
    MFDescriptorGraph graph =
        this.graphBuilder
            .addNode(parser, audioDecoder, Evaluator.m362eq(MediaType.COMPRESSED_AUDIO), 1)
            .addNode(audioDecoder, audioEncoder)
            .addNode(audioEncoder, muxer, Evaluator.m362eq(MediaType.RAW_AUDIO))
            .addNode(parser, videoDecoder, Evaluator.m362eq(MediaType.COMPRESSED_VIDEO), 1)
            .addNode(videoDecoder, upscaler, 2)
            .addNode(upscaler, videoEncoder, 3)
            .addNode(videoEncoder, muxer, Evaluator.m362eq(MediaType.RAW_VIDEO))
            .build(option);
  ...
  public SumeClient newMotionPhotoVSWUpscaler(Option option) {
    ...
    switch (option.getUpscaler()) {
      case 2:
        vswUpscaler =
            new ImgpDescriptor(
                "com.samsung.android.sume.midas.upscaler.WrapVSWEnginePlugin", "X2_UPSCALER");
        break;
      case 3:
        vswUpscaler =
            new ImgpDescriptor(
                "com.samsung.android.sume.midas.upscaler.WrapVSWEnginePlugin", "X3_UPSCALER");
        break;
      default:
        vswUpscaler =
            new ImgpDescriptor(
                "com.samsung.android.sume.midas.upscaler.WrapVSWEnginePlugin", "X4_UPSCALER");
        break;
    }
  ...
  public SumeClient newMasteringNet(Option option) {
    ImgpDescriptor imgpDescriptor =
        new ImgpDescriptor("com.samsung.android.masteringnet.MasteringNetPlugin", "MasteringNet");
  ...
  public SumeClient newAvailableMasteringNet(Option option) {
    ImgpDescriptor imgpDescriptor =
        new ImgpDescriptor(
            "com.samsung.android.masteringnet.MasteringNetPlugin", "AvailableMasteringNet");
    MFDescriptorGraph graph = this.graphBuilder.addNode(imgpDescriptor).build(option);
    return new SumeClient(this.serviceProxySupplier.get(), graph);
  }
  ...
  public SumeClient newImageUpscaler(Option option) {
    ...
    this.graphBuilder
        .addNode(
            miracleEstimator, upscaler, Evaluator.m363ge(Float.valueOf(miracleFilterThreshold)))
        .addNode(
            miracleEstimator,
            miracleFilter,
            Evaluator.m366lt(Float.valueOf(miracleFilterThreshold)))
        .addNode(miracleFilter, upscaler);
    if (option.isSupportAlphaChannel()) {
      ImgpDescriptor extractAlpha =
          new ImgpDescriptor(
              "com.samsung.android.sume.ext.plugin.AlphaChannelPlugin", "EXTRACT_ALPHA");
      ImgpDescriptor mergeAlpha =
          new ImgpDescriptor(
              "com.samsung.android.sume.ext.plugin.AlphaChannelPlugin", "MERGE_ALPHA");
  ...
  public SumeClient newImageVSWUpscaler(Option option) {
    ...
    this.graphBuilder
        .addNode(
            miracleEstimator, vswUpscaler, Evaluator.m363ge(Float.valueOf(miracleFilterThreshold)))
        .addNode(
            miracleEstimator,
            miracleFilter,
            Evaluator.m366lt(Float.valueOf(miracleFilterThreshold)))
  ...
  public SumeClient newOldPhotoEnhancer(Option option) {
    ...
    ImgpDescriptor composeBgNFaces =
        new ImgpDescriptor("com.samsung.android.sume.nn.plugin.OldPhotoPlugin", "COMPOSE_BG_FACES");
