...
public class MediaFilterController implements MediaController<Response>, MessageConsumer {
    ...
    public Response run(List<MediaBuffer> inBuffers, List<MediaBuffer> outBuffers) {
        ...
        if (this.eventListener != null) {
            this.eventListener.onEvent(
                    (Event)
                            Event.of(502, "timestampMs", Long.valueOf(endTs))
                                    ...
                                    .map(
                                            new Function() { // from class:
                                                ...
                                                public final Object apply(Object obj) {
                                                    return MediaFilterController.this
                                                            .m9128xb8943d28((MediaBuffer) obj);
                                                }
                                            })
                                    .collect(Collectors.toList());
            Log.d(
                    TAG,
                    "buffer-list[" + buffers.size() + "]: " + Arrays.toString(buffers.toArray()));
            outBuffers.clear();
                                ...
                                Optional.ofNullable(request.getInputBuffer())
                                        .map(
                                                new Function() { // from class:
                                                    ...
                                                    public final Object apply(Object obj) {
                                                        return MediaFilterController.this
                                                                .m9126xbf598303((MediaBuffer) obj);
                                                    }
                                                })
        ...
        if (((Boolean) it.getExtra("singular-buffer", false)).booleanValue()) {
            return new ArrayList<
                    MediaBuffer>() { // from class:
                                     // com.samsung.android.sume.core.controller.MediaFilterController.2
                {
                    add(it);
                }
            };
    ...
    public int[] getConsumeMessage() {
        return new int[] {
            993, 501, 502, 509, 510, 505, 506, 507, 508, 2, 511, 512, 513, 514, 515, 516
        };
    }
    ...
    public boolean onMessageReceived(final Message message) throws UnsupportedOperationException {
        ...
        try {
            ...
            if (message.contains(Message.KEY_CONTENTS_ID)
                ...
                if (message.getCode() == 508
                        && contentsInfo.getAsBooleanOr("show-progress", false)) {
                    message.put("show-progress", true);
                    message.put(
                            Message.KEY_WHOLE_FRAMES,
                            Integer.valueOf(contentsInfo.getWholeFrames()));
                    message.put(
                            Message.KEY_DISPLAY_NAME,
                            contentsInfo.getDataOr(Message.KEY_DISPLAY_NAME, ""));
                }
            }
            if (this.eventListener == null) {
                return false;
