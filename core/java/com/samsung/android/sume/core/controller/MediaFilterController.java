...
public class MediaFilterController implements MediaController<Response>, MessageConsumer {
    ...
    public Response run(List<MediaBuffer> inBuffers, List<MediaBuffer> outBuffers) {
        ...
        if (onEventListener2 != null) {
            onEventListener2.onEvent((Event) Event.m422of(502, "timestampMs", Long.valueOf(endTs)).put("id", Integer.valueOf(this.f3050id)));
        }
        String str = TAG;
        Log.m94d(str, "run X: processing total " + (endTs - beginTs) + " ms[#" + outBuffers.size() + NavigationBarInflaterView.SIZE_MOD_END);
    ...
    /* synthetic */ MutableMediaBuffer m354xb8943d28(MediaBuffer it) {
        ...
        if (!mediaType.isMetaData() && !mediaType.isScala() && !((Boolean) buf.getExtra("freezed", false)).booleanValue()) {
            ...
            if (colorFormat != contentsInfo.getOriginalColorFormat()) {
                Log.m94d(str, Def.fmtstr("color-format of output(%s) is differ from one of input(%s)", colorFormat.name(), contentsInfo.getOriginalColorFormat().name()));
            ...
            if (dataType != contentsInfo.getOriginalDataType()) {
                Log.m94d(str, Def.fmtstr("data-type of output(%s) is differ from one of input(%s)", dataType.name(), contentsInfo.getOriginalDataType().name()));
        ...
        try {
            if (message.isError()) {
                ...
                if (message.getException() instanceof ContentFilterOutException) {
                    event = (Event) Event.m421of(701, (String) Optional.ofNullable(message.getException().getMessage()).orElse("none")).put("id", Integer.valueOf(this.f3050id));
                }
            }
