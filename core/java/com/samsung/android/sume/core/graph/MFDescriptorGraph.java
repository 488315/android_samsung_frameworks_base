...
public class MFDescriptorGraph implements Parcelable {
  ...
  public static final Parcelable.Creator<MFDescriptorGraph> CREATOR =
  ...
  protected MFDescriptorGraph(Parcel in) {
    DescriptorNode[] descriptorNodeArr =
        (DescriptorNode[])
            Arrays.stream(in.readParcelableArray(DescriptorNode.class.getClassLoader()))
                .map(
                    new Function() { // from class:
                                     // com.samsung.android.sume.core.graph.MFDescriptorGraph$$ExternalSyntheticLambda1
                ...
                .toArray(
                    new IntFunction() { // from class:
                                        // com.samsung.android.sume.core.graph.MFDescriptorGraph$$ExternalSyntheticLambda2
    ...
    for (DescriptorNode node : descriptorNodeArr) {
      if (node.descriptor instanceof NNDescriptor) {
        ...
        String str = TAG;
        Log.m94d(str, "node: isKeepFilterDatatype " + nnDescriptor.isKeepFilterDatatype());
        Log.m94d(str, "node: isMultipleInputOutput " + nnDescriptor.isBatchIO());
      }
    }
  }
  ...
  public Graph<MediaFilter> toMediaFilterGraph(final MFGraphUnitFactory unitFactory) {
    Log.m94d(TAG, "toMediaFilterGraph: option=" + this.option);
    final MFGraph.Builder builder = new MFGraph.Builder(unitFactory, this.option);
    try {
      final List<Pair<DescriptorNode, GraphNode<MediaFilter>>> nodeList =
          (List)
              Arrays.stream(this.nodes)
                  .sorted(
                      Comparator.comparing(
                          new Function() { // from class:
                                           // com.samsung.android.sume.core.graph.MFDescriptorGraph$$ExternalSyntheticLambda3
                            @Override // java.util.function.Function
                            public final Object apply(Object obj) {
                              return Integer.valueOf(
                                  ((MFDescriptorGraph.DescriptorNode) obj).getId());
                            }
                          }))
                  .map(
                      new Function() { // from class:
                                       // com.samsung.android.sume.core.graph.MFDescriptorGraph$$ExternalSyntheticLambda4
                        @Override // java.util.function.Function
                        public final Object apply(Object obj) {
                          return MFDescriptorGraph.lambda$toMediaFilterGraph$2(
                              MFGraphUnitFactory.this, (MFDescriptorGraph.DescriptorNode) obj);
                        }
                      })
                  .collect(Collectors.toList());
      nodeList.forEach(
          new Consumer() { // from class:
                           // com.samsung.android.sume.core.graph.MFDescriptorGraph$$ExternalSyntheticLambda5
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
              MFDescriptorGraph.lambda$toMediaFilterGraph$4(
                  MFGraph.Builder.this, nodeList, (Pair) obj);
            }
          });
    } catch (Exception e) {
      e.printStackTrace();
  ...
  static /* synthetic */ Pair lambda$toMediaFilterGraph$2(
      MFGraphUnitFactory unitFactory, DescriptorNode it) {
  ...
  static /* synthetic */ void lambda$toMediaFilterGraph$4(
    ...
    final Map<Integer, BufferChannelDescriptor> channelMapMap =
        ((DescriptorNode) it.first).channelMap;
    ...
    if (children.isEmpty()) {
      ...
      children.forEach(
          new Consumer() { // from class:
                           // com.samsung.android.sume.core.graph.MFDescriptorGraph$$ExternalSyntheticLambda0
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
              MFDescriptorGraph.lambda$toMediaFilterGraph$3(
                  nodeList, evaluatorMap, channelMapMap, builder, from, (Integer) obj);
            }
          });
    }
  }

  static /* synthetic */ void lambda$toMediaFilterGraph$3(
      List nodeList,
      Map evaluatorMap,
      Map channelMapMap,
      MFGraph.Builder builder,
      GraphNode from,
      Integer idx) {
  ...
  public static class Builder {
    ...
    /* renamed from: id */
    AtomicInteger f3061id = new AtomicInteger(0);
    ...
    private DescriptorNode getNode(MFDescriptor descriptor) {
      if (!this.nodeMap.containsKey(descriptor)) {
        this.nodeMap.put(
            descriptor, new DescriptorNode(this.f3061id.getAndIncrement(), descriptor));
      }
    ...
    public Builder addNode(MFDescriptor from, MFDescriptor to, int channelType) {
      return addNode(
          ...
          getNode(to),
          (Evaluator) null,
          new BufferChannelDescriptor(channelType, 0));
    }

    public Builder addNode(
        MFDescriptor from, MFDescriptor to, Evaluator evaluator, int channelType) {
      return addNode(
          getNode(from), getNode(to), evaluator, new BufferChannelDescriptor(channelType, 0));
    }

    public Builder addNode(
        MFDescriptor from, MFDescriptor to, BufferChannelDescriptor channelDescriptor) {
    ...
    public MFDescriptorGraph build() {
      return new MFDescriptorGraph(
          (DescriptorNode[]) this.nodeMap.values().toArray(new DescriptorNode[0]),
          new Graph.Option());
    }

    public MFDescriptorGraph build(Graph.Option option) {
      return new MFDescriptorGraph(
          (DescriptorNode[]) this.nodeMap.values().toArray(new DescriptorNode[0]), option);
    }
  }
  ...
  static class DescriptorNode implements Parcelable {
    ...
    void addNode(
        DescriptorNode childNode, Evaluator evaluator, BufferChannelDescriptor channelDescriptor) {
    ...
    protected DescriptorNode(Parcel in) {
      ...
      this.evaluatorMap = new HashMap();
      this.channelMap = new HashMap();
      this.f3062id = in.readInt();
      this.descriptor = (MFDescriptor) in.readSerializable();
      this.children =
          (List) Arrays.stream(in.createIntArray()).boxed().collect(Collectors.toList());
