...
public class ReceiveChannelRouter extends ChannelRouterBase {
    ...
    if (this.receiveType == Type.ANY) {
      ...
      this.receiveOp =
          new Supplier() { // from class:
                           // com.samsung.android.sume.core.channel.ReceiveChannelRouter$$ExternalSyntheticLambda0
      ...
      this.receiveOp =
          new Supplier() { // from class:
                           // com.samsung.android.sume.core.channel.ReceiveChannelRouter$$ExternalSyntheticLambda1
            @Override // java.util.function.Supplier
            public final Object get() {
  ...
  public MediaBuffer receiveAll() {
    final List<MediaBuffer> buffers =
        (List)
            ((Stream) this.channels.stream().parallel())
                .map(new ReceiveChannelRouter$$ExternalSyntheticLambda2())
                .collect(Collectors.toList());
    int primaryId =
        IntStream.range(0, buffers.size())
            .filter(
                new IntPredicate() { // from class:
                                     // com.samsung.android.sume.core.channel.ReceiveChannelRouter$$ExternalSyntheticLambda6
  ...
  public MediaBuffer receiveAny() {
    String str = TAG;
    Log.m94d(str, "anyReceived: # of channel=" + this.channels.size());
    Def.require(!this.channels.isEmpty());
    if (this.channels.size() == 1) {
      Log.m94d(str, "channel: " + this.channels.get(0));
      return (MediaBuffer)
    ...
    Map<Integer, Future<MediaBuffer>> results =
        (Map)
            IntStream.range(0, this.channels.size())
                .boxed()
                .collect(
                    Collectors.toMap(
                        Function.identity(),
                        new Function() { // from class:
                                         // com.samsung.android.sume.core.channel.ReceiveChannelRouter$$ExternalSyntheticLambda3
                          @Override // java.util.function.Function
                          public final Object apply(Object obj) {
                            return ReceiveChannelRouter.this.m346x12a5943b(
                                threadPool, queue, (Integer) obj);
                          }
                        }));
    while (!results.isEmpty()) {
      try {
        try {
          ...
          MediaBuffer buffer =
              (MediaBuffer)
                  ((Future) Objects.requireNonNull(results.remove(Integer.valueOf(id)))).get();
          if (buffer != null) {
            return buffer;
          }
        ...
        results
            ...
            .forEach(
                new Consumer() { // from class:
                                 // com.samsung.android.sume.core.channel.ReceiveChannelRouter$$ExternalSyntheticLambda4
  ...
  /* synthetic */ Future m346x12a5943b(
      ExecutorService threadPool, final BlockingQueue queue, final Integer it) {
    return threadPool.submit(
        new Callable() { // from class:
                         // com.samsung.android.sume.core.channel.ReceiveChannelRouter$$ExternalSyntheticLambda5
          @Override // java.util.concurrent.Callable
          public final Object call() {
            return ReceiveChannelRouter.this.m345xca1c8dc(it, queue);
          }
        });
  }

  /* renamed from: lambda$receiveAny$1$com-samsung-android-sume-core-channel-ReceiveChannelRouter */
  /* synthetic */ MediaBuffer m345xca1c8dc(Integer it, BlockingQueue queue) throws Exception {
    try {
      MediaBuffer buffer = this.channels.get(it.intValue()).receive();
      queue.put(it);
      return buffer;
    } catch (Exception e) {
      Log.m94d(TAG, "buffer-channel receive thread is interrupted: " + e.getMessage());
      queue.put(it);
      return null;
    }
  }
