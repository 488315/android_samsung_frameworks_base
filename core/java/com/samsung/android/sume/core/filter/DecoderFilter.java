    ...
    while (true) {
      if (!this.reachedInputEos || !this.reachedOutputEos) {
        ...
        if (status == -1) {
          ...
          Log.m94d(
              TAG,
              tagged("output format changed: " + this.mediaCodec.getOutputFormat(), new Object[0]));
        } else if (status < 0) {
          continue;
        } else {
          if ((bufferInfo.flags & 4) != 0) {
            ...
            if (outputChannel instanceof SurfaceChannel) {
              this.messageProducer
                  .newMessage(5, "last-timestampUs", Long.valueOf(this.lastTimestampUs.get()))
                  .post();
            } else {
          ...
          if (bufferInfo.size == 0) {
            ...
            if (outputChannel instanceof SurfaceChannel) {
              ...
              MediaBuffer mediaBuffer2 =
                  MediaBuffer.m326of(
                      com.samsung.android.sume.core.format.MediaFormat.mutableAudioOf(
                          Integer.valueOf(decodedBuffer.limit())));
