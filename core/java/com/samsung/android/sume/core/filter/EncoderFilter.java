...
public class EncoderFilter extends MediaCodecFilter {
    ...
    protected void configCodec(final Message configData) {
        ...
        String str2 = TAG;
        Log.m94d(str2, "configCodec: " + configData);
        ...
        try {
            ...
            Log.m94d(str2, "media-format=" + mediaFormat);
        ...
        while (true) {
            ...
            if (!ready2 || !this.reachedOutputEos) {
                ...
                if (!this.reachedInputEos && ready) {
                    ...
                    String str = TAG;
                    Log.m94d(str, "[bhko] buffer=" + mediaBuffer);
                    int bufferIdx = this.mediaCodec.dequeueInputBuffer(JobInfo.MIN_BACKOFF_MILLIS);
                    Log.m94d(str, tag + "dequeue input buffer: " + bufferIdx);
                    if (bufferIdx >= 0) {
                ...
                String str2 = TAG;
                Log.m94d(str2, tag + "dequeue output buffer");
                int status = this.mediaCodec.dequeueOutputBuffer(bufferInfo, JobInfo.MIN_BACKOFF_MILLIS);
                Log.m94d(str2, tag + "buffer st=" + status + ", info=" + bufferInfo);
                if (status == -1) {
                    Log.m94d(str2, tag + "retry dequeue output buffer");
                    descriptor = descriptor3;
                    mediaType = mediaType2;
                    descriptor2 = null;
                } else if (status == -2) {
                    Log.m94d(str2, tag + "track format = " + this.mediaCodec.getOutputFormat());
                    ...
                    Log.m94d(str2, tag + "now ready to start encode");
                    descriptor = descriptor3;
                    mediaType = mediaType2;
                    descriptor2 = null;
                } else if (status < 0) {
                    ...
                    MediaBuffer mediaBuffer2 = MediaBuffer.m331of(mediaType2, outputBuffer);
                    mediaBuffer2.setExtra("track-idx", Integer.valueOf(trackIndex.get()));
                    mediaBuffer2.setExtra("buffer-info", bufferInfo);
                    Log.m94d(str2, "flag=" + Integer.toHexString(bufferInfo.flags));
                    if ((bufferInfo.flags & 2) != 0) {
                        ready = true;
                        bufferInfo.size = 0;
                        mediaBuffer2.release();
                    }
                    Log.m94d(str2, "size=" + bufferInfo.size);
                    if (bufferInfo.size != 0) {
                        this.processedFrames++;
                        descriptor = descriptor3;
                        mediaType = mediaType2;
                        Log.m94d(str2, tag + "# of encoded frames: " + this.processedFrames + NavigationBarInflaterView.SIZE_MOD_START + bufferInfo.presentationTimeUs + "](" + Integer.toHexString(bufferInfo.flags) + NavigationBarInflaterView.KEY_CODE_END);
                        Log.m94d(str2, tag + "total # :" + this.numWholeFrames.get() + ", last ts: " + this.lastTimestampUs.get());
                    ...
                    if ((bufferInfo.flags & 4) != 0) {
                        Log.m98i(str2, tag + "encoder reached eos");
