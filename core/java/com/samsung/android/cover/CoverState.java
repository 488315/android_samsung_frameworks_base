...
public class CoverState implements Parcelable {
  ...
  public static final Parcelable.Creator<CoverState> CREATOR =
      new Parcelable.Creator<CoverState>() { // from class: com.samsung.android.cover.CoverState.1
  ...
  public CoverState(
      boolean switchState, int type, int color, int widthPixel, int heightPixel, boolean attached) {
    this.mVisibleRect = new Rect();
    this.switchState = switchState;
    this.type = type;
    this.color = color;
    ...
    if (resource != null) {
      if (type == 15 || type == 16 || type == 17) {
        ...
        this.mVisibleRect.right =
            resource.getDimensionPixelSize(C4337R.dimen.cover_clear_view_right);
        this.mVisibleRect.bottom =
            resource.getDimensionPixelSize(C4337R.dimen.cover_clear_view_bottom);
      }
