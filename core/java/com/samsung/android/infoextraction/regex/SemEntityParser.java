...
public class SemEntityParser {
    ...
    private static void parsingDateMillisInfo() {
        ...
        while (m1.find()) {
            ...
            Log.m94d(TAG, "add date for millis(type1): " + matchString);
        }
        ...
        while (m2.find()) {
            ...
            Log.m94d(TAG, "add date for millis(type2): " + matchString2);
        }
        ...
        if (countryDateString.length() > 0 && countryDateString.charAt(0) == '|') {
            ...
            while (m3.find()) {
                ...
                Log.m94d(TAG, "add date for millis(type3, country): " + matchString3);
            }
        }
    }
    ...
    private static void parsingTimeMillisInfo() {
        ...
        while (m.find()) {
            ...
            Log.m94d(TAG, "add time for millis : " + matchString);
        }
    }
    ...
    private static void parsingDateInfo() {
        ...
        while (m1.find()) {
            ...
            Log.m94d(TAG, "add date(pattern type1): " + matchString);
        }
        ...
        while (m2.find()) {
            ...
            Log.m94d(TAG, "add date(pattern type2): " + matchString2);
        }
        ...
        if (sb.length() > 0 && sb.charAt(0) == '|') {
            ...
            while (m3.find()) {
                ...
                Log.m94d(TAG, "add date(pattern type3, country): " + matchString3);
            }
        }
    }
    ...
    private static void parsingTimeInfo() {
        ...
        while (m.find()) {
            ...
            Log.m94d(TAG, "add time : " + matchString);
        }
    }
    ...
    private static void parsingPhoneNumInfo() {
        ...
        while (m.find()) {
            ...
            if (matchString2.length() >= 7) {
                ...
                Log.m94d(TAG, "add tel number : " + matchString2);
            }
        }
    ...
    private static void parsingEmailInfo() {
        ...
        while (m.find()) {
            ...
            Log.m94d(TAG, "add email address : " + matchString2);
        }
    }
    ...
    private static void parsingURLInfo() {
        ...
        while (m.find()) {
            ...
            Log.m94d(TAG, "add URL : " + matchString);
        }
    }
        ...
        if (mInfo.getCount(5) == 1) {
            ...
            if (spaceCount > 0 && (str.length() / spaceCount) + 1 > 8) {
                ...
                while (m.find()) {
                    ...
                    Log.m94d(TAG, "add refactoring phone number : " + m.group(0));
                }
            }
        }
    }
        ...
        try {
            if (patternType == 1) {
                ...
                if (separated.length == 3) {
                    ...
                    Log.m94d(TAG, "fail convertDateToMillis() by invalid length. (type:1)");
                    return "";
                }
                ...
                if (separated2.length == 3) {
                    ...
                    Log.m94d(TAG, "fail convertDateToMillis() by invalid length. (type:2)");
                    return "";
                }
            } else {
                Log.m94d(TAG, "fail convertDateToMillis() by invalid patternType : ");
                return "";
            }
            t.hour = 0;
            t.minute = 0;
            t.second = 0;
            Log.m94d(TAG, "convertDateToMillis() completed successfully");
            Log.m94d(TAG, "year:" + t.year + ", month:" + t.month + ", day:" + t.monthDay + ", hour:" + t.hour + ", minute:" + t.minute + ", second:" + t.second);
            long result = t.toMillis(true);
            return Long.toString(result);
        } catch (Exception e) {
            Log.m94d(TAG, "fail convertDateToMillis() by exception : " + e.getMessage());
            return "";
        }
    }
        ...
        try {
            ...
            if (!timeStr2.contains("pm") && !timeStr2.contains("PM") && !timeStr2.contains("오후")) {
                if (!timeStr2.contains(XmlTags.TAG_ACCESS_MODE) && !timeStr2.contains("AM") && !timeStr2.contains("오전")) {
                    ...
                    t.second = 0;
                    Log.m94d(TAG, "convertTimeToMillis() completed successfully");
                    Log.m94d(TAG, "year:" + t.year + ", month:" + t.month + ", day:" + t.monthDay + ", hour:" + t.hour + ", minute:" + t.minute + ", second:" + t.second);
                    long result = t.toMillis(true);
                    return Long.toString(result);
                }
            ...
            Log.m94d(TAG, "fail convertTimeToMillis() by exception : " + e.getMessage());
            return "";
        }
    }
