...
public class SemInfoExtractionManager {
    ...
    public SemInfoExtractionManager(Context context) throws IllegalStateException {
        this.mContext = null;
        Log.m94d(TAG, "SemInfoExtractionManager setting...");
        if (context == null) {
            Log.m94d(TAG, "Could not get the SemInfoExtraction service. -> context is NULL");
        ...
        if (isPenFeatureModel(context)) {
            Log.m94d(TAG, "SemInfoExtractionManager call by : " + this.mContext.getPackageName());
        } else {
            Log.m94d(TAG, "SemInfoExtraction only use for Pen Feature models.");
    ...
    private boolean isPenFeatureModel(Context context) {
        ...
        if (uspLevel <= 0) {
            Log.m94d(TAG, "isPenFeatureModel : Pen is not supported, uspLevel=" + uspLevel);
            return false;
        }
        return true;
    }
    ...
    public void setInfoExtractionListener(InfoExtractionListener infoExtractionListener) throws IllegalArgumentException {
        if (infoExtractionListener == null) {
            Log.m94d(TAG, "infoExtractionListener is null");
    ...
    public void setOnExtractionCompletedListener(OnExtractionCompletedListener onExtractionCompletedListener) throws IllegalArgumentException {
        if (onExtractionCompletedListener == null) {
            Log.m94d(TAG, "onExtractionCompletedListener is null");
    ...
    public void training(String source) throws IllegalStateException, IllegalArgumentException {
        Log.m94d(TAG, "training doesn't support in this version");
    }

    public void addResultRule(int type, String source) throws IllegalStateException, IllegalArgumentException {
        Log.m94d(TAG, "addResultRule doesn't support in this version");
    }

    private boolean bindInfoExtractionService() {
        if (this.mContext == null) {
            Log.m94d(TAG, "mContext is NULL -> can't try to bind with InfoExtractionService! ");
            return false;
        }
        ...
        if (!ret) {
            Log.m94d(TAG, "Failed to bind with InfoExtractionService service!");
        }
        return ret;
    }
    ...
    private void startExtraction(final int dataType, final Object reqObject) {
        if (this.mConnection == null) {
            Log.m94d(TAG, "mConnection is NULL");
            ...
            Log.m94d(TAG, "start : Binding to InfoExtractionService...");
            bindInfoExtractionService();
            return;
        }
        Log.m94d(TAG, "mConnection is not NULL");
        if (this.mInfoExtractionService == null) {
            Log.m94d(TAG, "mInfoExtractionService == null");
            bindInfoExtractionService();
        } else {
            Log.m94d(TAG, "mInfoExtractionService != null");
    ...
    public void requestInfoExtraction(IBinder InfoExtractionService, int dataType, Object reqObject) {
        Log.m94d(TAG, "requestInfoExtraction data type = " + dataType);
        ...
        switch (dataType) {
            ...
            case 3:
                ...
                break;
            default:
                Log.m94d(TAG, "can't make data type = " + dataType);
                break;
        }
        ...
        try {
            if (InfoExtractionService != null) {
                ...
                Log.m94d(TAG, "request Extraction : success");
            } else {
                Log.m94d(TAG, "request Extraction : InfoExtractionService is null!");
            }
    ...
    class IncomingHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (SemInfoExtractionManager.this.mOnExtractionCompletedListener != null) {
                Log.m94d(SemInfoExtractionManager.TAG, "sent to mOnExtractionCompletedListener ReqTime : " + receivedReqTime + " extracted size : " + semExtractedInfoList.size());
                ...
                Log.m94d(SemInfoExtractionManager.TAG, "mInfoExtractionResultListener is NULL");
                if (SemInfoExtractionManager.this.mInfoExtractionListener != null) {
                    Log.m94d(SemInfoExtractionManager.TAG, "sent to InfoExtractionListener ReqTime : " + receivedReqTime + " extracted size : " + semExtractedInfoList.size());
                    SemInfoExtractionManager.this.mInfoExtractionListener.onCompleted((int) receivedReqTime, semExtractedInfoList);
                } else {
                    Log.m94d(SemInfoExtractionManager.TAG, "mInfoExtractionListener is NULL");
                }
                SemInfoExtractionManager.this.mRequestNumber = -1L;
            }
        }
