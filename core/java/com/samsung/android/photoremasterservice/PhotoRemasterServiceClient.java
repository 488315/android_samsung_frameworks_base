...
public class PhotoRemasterServiceClient {
  ...
  static final String SERVICE_CLASS =
      "com.samsung.android.photoremasterservice.PhotoRemasterService";
  ...
  private final ServiceConnection mConnection = new ServiceConnection() { // from class:
        // com.samsung.android.photoremasterservice.PhotoRemasterServiceClient.2
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
          ...
          LogUtil.m294i(PhotoRemasterServiceClient.TAG, "Service is connected(attached).");
        }

        @Override // android.content.ServiceConnection
        public void onServiceDisconnected(ComponentName className) {
          LogUtil.m294i(PhotoRemasterServiceClient.TAG, "Service is disconnected.");
          PhotoRemasterServiceClient.this.mServiceMessenger = null;
          if (!PhotoRemasterServiceClient.this.mDisconnectionRequested) {
            LogUtil.m292e(
                PhotoRemasterServiceClient.TAG,
                "Unexpected disconnection with PhotoRemaster Service! CancellationException will be"
                    + " thrown!");
          ...
          public void handleMessage(Message msg) {
            switch (msg.what) {
              case -1:
                ...
                LogUtil.m292e(PhotoRemasterServiceClient.TAG, "Received exception : " + msg.what);
                break;
              case 0:
                PhotoRemasterServiceClient.this.mServiceReturnValue.complete(msg.getData());
                LogUtil.m291d(PhotoRemasterServiceClient.TAG, "Received: " + msg.what);
                break;
              default:
                LogUtil.m292e(
                    PhotoRemasterServiceClient.TAG, "Wrong Message is received." + msg.what);
  ...
  private static boolean isExceptionContained(Bundle bundle) {
    return (bundle == null || bundle.getSerializable("exception", Exception.class) == null)
        ? false
        : true;
  }
  ...
  private Bundle doGetServiceReturnValue(long timeOut)
    ...
    while (true) {
      try {
        ...
        if (this.mProgressObserver.wasUpdateAndClear()) {
          LogUtil.m294i(
              TAG, "Wait again service return value because there has been update message.");
        } else {
          LogUtil.m292e(TAG, "Time Out! There is no response from service.");
          throw e;
        }
      }
    }
    ...
    try {
      ...
      if (isExceptionContained(retValue)) {
        ...
        RuntimeException rtException =
            new RuntimeException(exception + " in PhotoRemaster Service");
  ...
  public synchronized void unbindService() {
    LogUtil.m294i(TAG, "Try Unbinding...");
    ...
    if (!this.mIsBound) {
      LogUtil.m296w(TAG, "Already unbound!!!");
      return;
    }
  ...
  public synchronized Bundle callService(final int serviceMessage, final Bundle bundle) {
    LogUtil.m294i(TAG, "Call Service. message: " + serviceMessage);
    if (!this.mIsBound) {
      bind();
      if (!this.mIsBound) {
        LogUtil.m292e(TAG, "Service is not bounded!");
        return null;
      }
    }
    if (this.mServiceMessenger == null) {
      LogUtil.m292e(TAG, "ServiceMessenger is null!");
      return null;
    }
    new Thread(
            new Runnable() { // from class:
                             // com.samsung.android.photoremasterservice.PhotoRemasterServiceClient.1ServiceCallRunnable
              @Override // java.lang.Runnable
              public void run() {
                try {
                  LogUtil.m291d(PhotoRemasterServiceClient.TAG, "Send message to service...");
                  ...
                  if (PhotoRemasterServiceClient.this.mIncomingMessenger.getBinder() == null) {
                    LogUtil.m292e(PhotoRemasterServiceClient.TAG, "mIncomingMessenger is null!");
                  }
                  LogUtil.m292e(PhotoRemasterServiceClient.TAG, "Exception!!!! " + e);
                } catch (Exception e2) {
                  e2.printStackTrace();
                }
              }
        ...
        .start();
    LogUtil.m291d(TAG, "Service called!");
    LogUtil.m291d(TAG, "Start waiting the return value.");
    ...
    LogUtil.m294i(TAG, "Service return value: " + retBundle);
    return retBundle;
  }
}
