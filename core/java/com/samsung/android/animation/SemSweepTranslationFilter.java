...
public class SemSweepTranslationFilter extends SemAbsSweepAnimationFilter {
    ...
    public BitmapDrawable getSweepBitmapDrawable() {
        Log.m94d(TAG, "getSweepBitmapDrawable : mDrawSweepBitmapDrawable = " + this.mDrawSweepBitmapDrawable);
    ...
    private Canvas drawRectToBitmapCanvas(View view, float deltaX, float sweepProgress) {
        ...
        if (sweepProgress > 0.0f) {
            ...
            if (d == null) {
                Log.m94d(TAG, "mSweepConfiguration.drawableLeftToRight is null");
                return null;
            }
        ...
        if (d2 == null) {
            Log.m94d(TAG, "mSweepConfiguration.drawableRightToLeft is null");
            return null;
        }
    ...
    public ValueAnimator createActionUpAnimator(View viewForeground, float adjustedVelocityX, int scaledTouchSlop, float deltaX, boolean isSweepPattern) {
        ...
        Log.m94d(TAG, "SemSweepTranslationFilter : createActionUpAnimator() : Math.abs(adjustedVelocityX) = " + Math.abs(adjustedVelocityX));
        Log.m94d(TAG, "SemSweepTranslationFilter : createActionUpAnimator() : scaledTouchSlop * 23 = " + (scaledTouchSlop * 23));
        if (Math.abs(adjustedVelocityX) > scaledTouchSlop * 23 && isSweepPattern) {
            Log.m94d(TAG, "SemSweepTranslationFilter : createActionUpAnimator() : kick in animation with given velocity, point #1");
            ...
            if (deltaXAbs > endAlpha / 2.0f) {
                Log.m94d(TAG, "SemSweepTranslationFilter : createActionUpAnimator() : Greater than a half of the width, point #2");
                duration = (int) ((1.0f - (Math.abs(translationX) / width)) * 600.0f);
                endX = width * Math.signum(deltaX);
                remainingDistance = 0;
            } else {
                Log.m94d(TAG, "SemSweepTranslationFilter : createActionUpAnimator() : Not far enough - animate it back, point #3");
