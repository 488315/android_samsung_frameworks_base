  ...
  */
  ...
  private boolean handleTouchDownEvent(MotionEvent event) {
    ...
    if (positionForView == -1
        || positionForView < this.mListView.getFirstVisiblePosition()
  ...
  private boolean handleTouchMoveEvent(MotionEvent event) {
    ...
    if (listView != null
        && (this.mCurrentPosition < listView.getFirstVisiblePosition()
            || this.mCurrentPosition > this.mListView.getLastVisiblePosition())) {
      return false;
    }
  ...
  private void trackSweepDistanceAndDirection(MotionEvent event) {
    if (this.mSweepPrevPosX == -1.0f) {
      Log.m94d(
          TAG, "trackSweepDistanceAndDirection : first calling trackSweepDistanceAndDirection");
      Log.m94d(
          TAG,
          "trackSweepDistanceAndDirection : mSweepPrevPosX is set to mDownX, mSweepPrevPosX = "
              + this.mDownX);
  ...
  private boolean sweepPatternIsIndeedFling(float velocity) {
    ...
    Log.m94d(
        TAG, "sweepPatternIsIndeedFling : minimalDistanceThreshold = " + minimalDistanceThreshold);
    if ((velocity > 0.0f && this.mSweepRightDistance < minimalDistanceThreshold)
      ...
      Log.m94d(
          TAG,
          "sweepPatternIsIndeedFling : SweepDistance is less than minDistance, return false #1");
