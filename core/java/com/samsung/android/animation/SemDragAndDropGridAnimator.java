            ...
            public void onItemAnimatorEnd() {
                ...
                if (SemDragAndDropGridAnimator.this.mDropDonePending) {
                    ...
                    if (SemDragAndDropGridAnimator.this.mDndListener != null) {
                        Log.m94d(SemDragAndDropGridAnimator.TAG, "dndListener.onDragAndDropEnd() from onAllAnimationsFinished()");
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case 0:
                ...
                if (!isDraggable() || this.mGridView.getCount() <= 1 || (itemPosition = this.mGridView.pointToPosition(this.mDndTouchX, this.mDndTouchY)) == -1 || activatedByLongPress()) {
                    return false;
                }
                if (itemPosition >= 0 && itemPosition < this.mGridView.getCount() && checkStartDnd(this.mDndTouchX, this.mDndTouchY, itemPosition)) {
        ...
        if (dragView == null || dropView == null) {
            ...
            if (dropViewVisible) {
                ...
                if (childCount < numOfColumns2) {
                    Log.m96e(TAG, "Child cound (" + this.mGridView.getChildCount() + ") is smaller than column count (" + numOfColumns2 + NavigationBarInflaterView.KEY_CODE_END);
                    resetDndState();
                    return;
            ...
            Log.m100v(TAG, "dndListener.onTouchUp() dragView == null, distanceX=" + numOfColumns + ", distanceY=" + index);
            ...
            va.addListener(new AnimatorListenerAdapter() { // from class: com.samsung.android.animation.SemDragAndDropGridAnimator.4
                ...
                public void onAnimationEnd(Animator anim) {
                    ...
                    if (SemDragAndDropGridAnimator.this.mDndListener != null) {
                        Log.m94d(SemDragAndDropGridAnimator.TAG, "dndListener.onDragAndDropEnd() from AnimationEnd");
                        SemDragAndDropGridAnimator.this.mDndListener.onDragAndDropEnd();
                    }
                }
            });
