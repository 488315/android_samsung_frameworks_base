...
public class RenderTexture_GL_OES {
  ...
  private static final String BLUR_FRAGMENT_SHADER_CODE =
      "#extension GL_OES_EGL_image_external : require\n"
          ...
          + "\t\t\t\t\t\t\t\t\t\t\t\t\t\n"
          + "\tfor( int i = 0; i < 7; i++ )\t\t\n"
          + "\t\tcolor += texture2D(u_TextureUnit, vec2( v_TextureCoord.x+gaussFilter[i].x*fWidth,"
  ...
  public int createProgram(boolean isBlurMode) {
    if (isBlurMode) {
      this.mProgram =
          OpenGlHelper.createProgram(TEXTURE_VERTEX_SHADER_CODE, BLUR_FRAGMENT_SHADER_CODE);
    } else {
      this.mProgram =
          OpenGlHelper.createProgram(TEXTURE_VERTEX_SHADER_CODE, TEXTURE_FRAGMENT_SHADER_CODE);
    }
    int i = this.mProgram;
    if (i == 0) {
      return 0;
  ...
  public void draw(SurfaceTexture st) {
    ...
    GLES20.glVertexAttribPointer(
        this.ma_TextureCoordinatesHandle, 2, 5126, false, 16, (Buffer) this.mVerticesFloatBuffer);
  ...
  public int loadTexture(int rotationAngle) {
    ...
    if (loadTextureOES == 0) {
      LogS.m473d("TranscodeLib", "not able to load new texture");
    }
  ...
  private boolean checkcallFinish() {
    ...
    if (chipName.contains("exynos") || chipName.contains("s5e")) {
      LogS.m473d("TranscodeLib", "checkcallFinish  chipName :" + chipName + ", return false");
      return false;
    }
    LogS.m473d("TranscodeLib", "checkcallFinish  chipName :" + chipName + ", return true");
    return true;
  }
}
