...
public class NalUnitParser {
          ...
          Arrays.stream(values())
              .filter(
                  new Predicate() { // from class:
                    ...
                    @Override // java.util.function.Predicate
                    public final boolean test(Object obj) {
                      return NalUnitParser.HEVCNalUnitType.lambda$getNalType$0(
                          val, (NalUnitParser.HEVCNalUnitType) obj);
                    }
                  })
              .findFirst()
              .orElse(UNKNOWN);
  ...
  public boolean findHDRStaticMeta() {
    ...
    int sizeOfContentsLevelInfoMeta =
        allMetaInOneNal ? 0 : contentsLevelInfoMetaEndPos - this.mContentsLevelInfoMetaStartPos;
    if (sizeOfMasteringDisplayColorMeta < 0 || sizeOfContentsLevelInfoMeta < 0) {
      LogS.m474e(
          TAG,
          "invalid size : " + sizeOfMasteringDisplayColorMeta + " " + sizeOfContentsLevelInfoMeta);
      return false;
    }
    ByteBuffer allocate =
    ...
    allocate.put(
        this.mBuffer, this.mMasteringDisplayColorMetaStartPos, sizeOfMasteringDisplayColorMeta);
    if (!allMetaInOneNal) {
      ...
      if (isNalStartCode(this.mBuffer, index)) {
        ...
        if (isMasteringDisplayColorInfo(index + 2)) {
          if (findContentLightLevelWithinDisplayMasteringNal(index + 26)) {
            LogS.m474e(
                TAG, "Mastering display color meta and Content light level info meta in one NAL");
          } else {
            LogS.m474e(
                TAG,
                "Mastering display color meta and Content light level info meta not  in one NAL");
          }
          return true;
        }
      }
