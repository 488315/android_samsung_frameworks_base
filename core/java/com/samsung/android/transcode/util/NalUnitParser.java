...
public class NalUnitParser {
    ...
    public NalUnitParser(ByteBuffer byteBuffer) {
        ...
        LogS.m475i(TAG, "input buffer size : " + byteBuffer.remaining());
    ...
    public boolean findHDRStaticMeta() {
        ...
        if (this.mNalStartPos < 0) {
            LogS.m474e(TAG, "there is no nal start code");
            return false;
        }
        if (!findMasteringDisplayStaticMeta()) {
            LogS.m474e(TAG, "fail to find Mastering display color meta in stream.");
            this.mBuffer = null;
            return false;
        }
        ...
        if (i == this.mContentsLevelInfoMetaStartPos) {
            ...
            if (findContentLightLevel()) {
                ...
                LogS.m474e(TAG, "cannot find Content light level info meta");
                return false;
            }
        }
        LogS.m474e(TAG, "Mastering display color meta buffer position : " + this.mMasteringDisplayColorMetaStartPos + " ~ " + masteringDisplayColorMetaEndPos);
        LogS.m474e(TAG, "Content light level info meta buffer position : " + this.mContentsLevelInfoMetaStartPos + " ~ " + contentsLevelInfoMetaEndPos);
        int sizeOfMasteringDisplayColorMeta = masteringDisplayColorMetaEndPos - this.mMasteringDisplayColorMetaStartPos;
        LogS.m474e(TAG, "Mastering display color meta data size : " + sizeOfMasteringDisplayColorMeta);
        ...
        if (sizeOfMasteringDisplayColorMeta < 0 || sizeOfContentsLevelInfoMeta < 0) {
            LogS.m474e(TAG, "invalid size : " + sizeOfMasteringDisplayColorMeta + " " + sizeOfContentsLevelInfoMeta);
            return false;
        }
        ...
        if (!allMetaInOneNal) {
            LogS.m474e(TAG, "Content light level info meta data size : " + sizeOfContentsLevelInfoMeta);
    ...
    public ByteBuffer insertHDRStaticMeta(ByteBuffer orgBuffer, int orgBufferSize, boolean isHEVC) {
        ...
        LogS.m473d(TAG, "ppsPos : " + ppsStartPos);
        int ppsEndPos = -1;
        if (ppsStartPos >= 0) {
            ppsEndPos = findNalStartCode(orgByteBuffer, ppsStartPos + 4);
            LogS.m473d(TAG, "ppsEndPos : " + ppsStartPos);
        }
    ...
    private boolean isPPSNalUnit(byte[] buffer, int index, boolean isHEVC) {
        if (isHEVC) {
            ...
            LogS.m474e(TAG, "NAL type : " + nalType);
        ...
        LogS.m474e(TAG, "NAL type : " + nalType2);
            ...
            if (isNalStartCode(this.mBuffer, index)) {
                ...
                if (isMasteringDisplayColorInfo(index + 2)) {
                    if (findContentLightLevelWithinDisplayMasteringNal(index + 26)) {
                        LogS.m474e(TAG, "Mastering display color meta and Content light level info meta in one NAL");
                    } else {
                        LogS.m474e(TAG, "Mastering display color meta and Content light level info meta not  in one NAL");
                    }
                    return true;
                }
            }
