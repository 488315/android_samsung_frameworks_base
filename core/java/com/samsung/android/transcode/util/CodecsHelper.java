...
public class CodecsHelper {
    ...
    public static MediaCodecInfo getMediaCodec(String mimeType, boolean isEncoder) {
        ...
        if (codec != null) {
            LogS.m473d("TranscodeLib", "getMediaCodec : " + codec.getName());
        }
        return codec;
    }
    ...
    public static int getVideoEncodingBitRate(float sizeFraction, long maxSizeKB, long timeDurationMs, int audioBitRate, int width, int height) {
        ...
        LogS.m475i("TranscodeLib", "getVideoEncodingBitRate maxSizeKB: " + maxSizeKB + " sizeFraction :" + sizeFraction + " bitatre :  " + bitRate);
    ...
    public static int suggestBitrate(ExportMediaInfo outputInfo, MediaInfo.MediaFileInfo sourceInfo) {
        ...
        if (sourceInfo.Is360) {
            ...
            if (isSamsungAuthor(bitrate2)) {
                if (keepOriginalBitrate(outputInfo, sourceInfo)) {
                    ...
                    LogS.m475i("TranscodeLib", "[final] keepOriginalBitrate: " + bitrate3);
                    return bitrate3;
                }
        ...
        LogS.m473d("TranscodeLib", "[1] get from table. bitrate: " + bitrate + ", isHighBitrateMode: " + isHighBitrateMode);
        int outputFramerate = outputInfo.getFrameRate();
        if (outputFramerate >= 60) {
            bitrate = (int) (((bitrate * 0.8f) * outputFramerate) / 30.0f);
            LogS.m473d("TranscodeLib", "[2] over 60fps case. bitrate: " + bitrate);
        }
        ...
        if (!isHdr && "video/hevc".equals(outputVideoCodecType) && bitrate != 80000000) {
            ...
            LogS.m473d("TranscodeLib", "[3] normal hevc case. bitrate: " + bitrate);
        }
        if (sourceInfo.Bitrate != 0) {
            ...
            LogS.m473d("TranscodeLib", "[4] sourceBitrate : " + sourceInfo.Bitrate + ", originalBitrate: " + originalBitrate);
            bitrate = Math.min(bitrate, originalBitrate);
        }
        LogS.m475i("TranscodeLib", "suggestBitRate. bitrate: " + bitrate);
        return bitrate;
    }
    ...
    private static boolean isHighBitrateMode(MediaInfo.MediaFileInfo sourceInfo) {
        LogS.m473d("TranscodeLib", "isHighBitrateMode. codecType: " + sourceInfo.VideoCodecType + ", width: " + sourceInfo.Width + ", height: " + sourceInfo.Height + ", bitrate: " + sourceInfo.Bitrate + ", framerate: " + sourceInfo.Framerate);
        ...
        LogS.m473d("TranscodeLib", "isHighBitrateMode. [1] expected original bitrate: " + bitrate);
        if (sourceInfo.Framerate >= 60) {
            bitrate = (int) (((bitrate * 0.8f) * sourceInfo.Framerate) / 30.0f);
            LogS.m473d("TranscodeLib", "isHighBitrateMode. [2] over 60fps case. bitrate: " + bitrate);
        }
        int bitrate2 = (int) (bitrate * BITRATE_MARGIN_FACTOR);
        LogS.m473d("TranscodeLib", "isHighBitrateMode. [3] check condition. bitrate: " + bitrate2);
    ...
    public static boolean isSupportOMX() {
        ...
        LogS.m473d("TranscodeLib", "isSupportOMX getMediaCodec : " + ((String) Optional.ofNullable(codecInfo).map(new Function() { // from class: com.samsung.android.transcode.util.CodecsHelper$$ExternalSyntheticLambda1
