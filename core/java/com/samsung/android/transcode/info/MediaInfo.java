...
public class MediaInfo {
    ...
    if ((context == null || uri == null) && filepath == null) {
      ...
      try {
        ...
        try {
          ...
          info.RecordingMode =
              Integer.parseInt(
                  (String) Optional.ofNullable(recordingMode).orElse(Integer.toString(0)));
  ...
  */
  public static MediaFormat getTrackInfo(
      String filepath, Context context, Uri uri, boolean isVideo) {
  ...
  private static void setVideoFramerate(MediaExtractor extractor, MediaFormat videoFormat) {
    ...
    LogS.m473d(
        TAG, "setVideoFramerate Framerate: " + Framerate + ", FrameInterval : " + FrameInterval);
  }
  ...
  private static void setFrameRateBySampleInterval(MediaExtractor extractor) {
    ...
    if (bufferSizeV > 0) {
      ...
      if (avgTime <= 0 || frameCount <= 0) {
        ...
        LogS.m473d(
            TAG,
            "Fail to Calculate Framerate  avgTime :" + avgTime + ", frameCount : " + frameCount);
      } else {
        FrameInterval =
