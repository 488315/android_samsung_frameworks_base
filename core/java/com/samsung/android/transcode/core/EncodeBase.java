...
public abstract class EncodeBase extends Encode {
    ...
    protected void startEncoding() throws IOException {
        ...
        LogS.m473d("TranscodeLib", "Encoding abruptly stopped mUserStop ?" + this.mUserStop + " mCodecError? " + this.mCodecError);
    }
    ...
    */
    ...
    private void sendAudioToDecoder() {
        if (!this.mUserStop && !this.mAudioExtractorDone) {
            if (this.mAudioEncoderOutputMediaFormat == null || (this.mMuxerStarted && this.mAudioEncoderInputBufferCount <= 0 && !this.mAudioWaitFrame)) {
                ...
                if (jumpRegion != -1) {
                    LogS.m473d("TranscodeLib", "Seekto region End time :" + (this.mRegionList.get(jumpRegion).mRegionEndTime * 1000));
    ...
    private void getandsendAudioToMuxer() {
        if (!this.mUserStop && this.mCopyAudio && this.mMuxerStarted && !this.mAudioEncoderDone) {
            ...
            if (bufferInfoA.size < 0) {
                ...
                LogS.m473d("TranscodeLib", "Audio writeSampleData bufferInfoA.size : " + bufferInfoA.size + ", bufferInfoA.presentationTimeUs :" + bufferInfoA.presentationTimeUs);
    ...
    protected void audioVolume(ByteBuffer pcmData, int dataLength) {
        int cnt = 0;
        LogS.m473d("TranscodeLib", "AudioVolume  fade_sampleRateConvFactor: 0.1, data_length; " + dataLength);
    ...
    private boolean checkAudioEncoderOutputBufferIndex() {
        ...
        if (audioEncoderOutputBufferIndex == -2) {
            ...
            LogS.m474e("TranscodeLib", "audio encoder: output format changed " + this.mAudioEncoderOutputMediaFormat);
            return false;
        }
        ...
        if (this.mAudioEncoderOutputBufferInfo.size != 0) {
            LogS.m474e("TranscodeLib", "audio encoder writing sample data to muxer  time: " + this.mAudioEncoderOutputBufferInfo.presentationTimeUs);
        ...
        while (!this.mUserStop && !this.mAudioEncoderDone) {
            ...
            if (sNAACHandle != 0) {
                ...
                if (this.mAudioDecoderDone) {
                    this.mAudioEncoderDone = true;
                    LogS.m474e("TranscodeLib", "saw input EOS: Audio audioEncoderDone: " + this.mAudioEncoderDone);
                    return;
                }
                return;
            }
        ...
        if (size >= 0) {
                ...
                if (size > 0 && sVSPHandle != 0) {
                    ...
                    if (f != 1.0f) {
                        if (f > 8.0f) {
                            ...
                            LogS.m473d("TranscodeLib", "Seekto region : " + i3 + ", end time :" + (this.mRegionList.get(i3).mRegionEndTime * 1000) + ", RegionList.size() : " + this.mRegionList.size());
                            ...
                            LogS.m473d("TranscodeLib", "releaseOutputBuffer : " + this.mPendingAudioDecoderOutputBufferIndex);
                        ...
                        tempAudio.clear();
                        LogS.m473d("TranscodeLib", "VSPExe2 original size :" + size + ", mTempAudioLength :" + this.mTempAudioLength + ", mTempAudioEncSize :" + this.mTempAudioEncSize);
                        while (true) {
                            ...
                            LogS.m473d("TranscodeLib", " Enc NAACEncoderExe encoded_size: " + encodedSize + " naac_time : " + this.mNaccTime);
            ...
            if (isSlowV2() && size > 0 && sSRCHandle != 0) {
                ...
                while (true) {
                    ...
                    LogS.m473d(str, " Enc NAACEncoderExe encoded_size: " + encodedSize2 + str2 + this.mNaccTime);
                ...
                LogS.m473d(str, " Enc NAACEncoderExe2 encoded_size: " + encodedSize3 + str2 + this.mNaccTime);
    ...
    private void checkAudioDecoderEOS(long seektime) {
        ...
        LogS.m473d("TranscodeLib", "Seekto region : " + i + ", end time :" + (this.mRegionList.get(i).mRegionEndTime * 1000) + ", RegionList.size() : " + this.mRegionList.size());
        ...
        if (size >= 0) {
            ...
            if ((i == 2 || i == 1) && size > 0 && sVSPHandle != 0) {
                ...
                if (f != 1.0f) {
                    ...
                    this.mTempAudioLength += this.mOutputAudioChannelCount * newSize * 2;
                    tempAudio.clear();
                    LogS.m473d("TranscodeLib", "VSPExe2 original size :" + size + ", mTempAudioLength :" + this.mTempAudioLength);
        ...
        if (list != null && !list.isEmpty()) {
            ...
            if (recordingMode == 1 || recordingMode == 2) {
                ...
                if (tempSampleTime2 < timedelta) {
                    LogS.m473d("TranscodeLib", "[getAudioDrop]SampleTime error tempSampleTime = " + tempSampleTime2 + ",timeDelta :" + timedelta);
                } else {
                    LogS.m473d("TranscodeLib", "[getAudioDrop]SampleTime new tempSampleTime = " + (tempSampleTime2 - timedelta) + ",timeDelta :" + timedelta);
                }
            }
        }
    ...
    private boolean checkAudioDecoderEOSNotWaitFrameCase(long presentationTime) {
        ...
        if (this.mTempAudioLength > 0) {
            ...
            LogS.m473d("TranscodeLib", "Enc Last frame queueInputBuffer size:" + this.mTempAudioLength + ", presentationTime :" + presentationTime);
        ...
        LogS.m473d("TranscodeLib", "Enc EOS queueInputBuffer  time :" + this.mAudioDecoderOutputBufferInfo.presentationTimeUs + ", size : " + this.mAudioDecoderOutputBufferInfo.size);
    ...
    private void checkAudioLoopCount(long seektime) {
        ...
        LogS.m473d("TranscodeLib", "seek to next frame\taudioLoopCount :" + this.mAudioLoopCount + ", seektime: " + seektime);
        ...
        if (this.mAudioWaitFrame) {
            ...
            if (audioEncoderInputBufferIndex == -1) {
                ...
                if (tempPresentationTime > this.mOriginTrimEndUs) {
                    ...
                    LogS.m473d("TranscodeLib", "Forcely EOS  AudioLoopCount :" + this.mAudioLoopCount + ", seek time:" + seektime + ", temp_presentationTime :" + tempPresentationTime);
                    this.mAudioWaitFrame = false;
                    this.mAudioLoopCount = 0;
                    this.mInputAudioDecoder.flush();
                    this.mAudioExtractor.seekTo(seektime, 1);
    ...
    */
    ...
    protected long getSuperslowSeektime(long seekTimeUs) {
        ...
        if (list != null && !list.isEmpty()) {
            ...
            while (true) {
                ...
                j = seekTimeUs;
                LogS.m473d("TranscodeLib", "[getSuperslowSeektime] regStartTime = " + regStartTime + ",regEndTime : " + regEndTime + ",regAudioEndTime: " + regAudioEndTime2 + ", isCancel =" + isCancel + ",tmpSeekTimeUs: " + tmpSeekTimeUs + ", timeDelta:" + timeDelta + ", seekTimeUs:" + j + ", i :" + i);
        ...
        while (!this.mUserStop && !this.mAudioDecoderDone) {
            if ((this.mPendingAudioDecoderOutputBufferIndex != -1 || this.mAudioWaitFrame) && this.mAudioEncoderInputBufferCount <= 0) {
                ...
                if (!this.mSEFVideo) {
                    ...
                    LogS.m473d("TranscodeLib", "presentationTime :" + presentationTime + ", temp_presentationTime: " + tempPresentationTime + ", seektime :" + seektime2 + ", audioWaitFrame:" + this.mAudioWaitFrame + ", timescale : " + this.mTimescale);
                    seektime = seektime2;
                }
    ...
    private boolean checkEncoderOutputBufferIndex(int encoderOutputBufferIndex) {
        ...
        if (encoderOutputBufferIndex == -2) {
            LogS.m474e("TranscodeLib", "video encoder: output format changed " + this.mOutputVideoEncoder.getOutputFormat());
    ...
    public void sendVideoFramesToMuxer(int encoderOutputBufferIndex, MediaCodec.BufferInfo videoEncoderOutputBufferInfo) {
        ...
        if (videoEncoderOutputBufferInfo.size != 0) {
            LogS.m474e("TranscodeLib", "video encoder: writing sample data timestamp " + videoEncoderOutputBufferInfo.presentationTimeUs);
            try {
                this.mMuxer.writeSampleData(this.mVideoTrackIndex, encoderOutputBuffer, videoEncoderOutputBufferInfo);
            } catch (IllegalStateException e) {
                LogS.m474e("TranscodeLib", "fail to writeSampleData videoEncoderDone? " + this.mVideoEncoderDone);
            }
        ...
        try {
            ...
            LogS.m474e("TranscodeLib", "fail to release output buffer of encoder videoEncoderDone? " + this.mVideoEncoderDone);
        }
    }
    ...
    */
    ...
    private boolean checkDecoderOutputBufferIndex(int decoderOutputBufferIndex, MediaCodec.BufferInfo videoDecoderOutputBufferInfo) {
        ...
        if (decoderOutputBufferIndex == -2) {
            LogS.m474e("TranscodeLib", "video decoder: codec info format changed" + this.mInputVideoDecoder.getOutputFormat());
            return false;
        }
    ...
    private void checkSkipFrames(long presentationTimeUs) {
        if (this.mSkipFrames && this.mSkippedFramesCount % this.mFramesSkipInterval != 0) {
            LogS.m473d("TranscodeLib", "input surface: skip this frame: presentationTimeUs " + presentationTimeUs);
        } else {
            ...
            if (releaseOutputBufferOfVideoDecoder(frame)) {
                try {
                    if (!this.mOutputSurface.checkForNewImage(1000)) {
                        LogS.m474e("TranscodeLib", "video decoder: checkForNewImage return false!!  mUserStop : " + this.mUserStop);
                    }
    ...
    private boolean checkRetDropFastMotion(boolean retDrop, int recordingmode, int recordingFps, int layernum, int maxlayernum, SEFHelper.Speed playSpeed) {
        if (isSlow120(recordingmode, recordingFps)) {
            switch (C53123.$SwitchMap$com$samsung$android$transcode$util$SEFHelper$Speed[playSpeed.ordinal()]) {
                case 1:
                case 2:
                    break;
                case 3:
    ...
    private boolean procSVCLayerDrop(long sampleTime, int layerNum, int maxLayerNum, int recordingMode, int recordingFps) {
        ...
        LogS.m473d("TranscodeLib", "[procSVCLayerDrop]SampleTime = tempSampleTime = " + sampleTime2 + ",layernum :" + layerNum + ", maxlayernum : " + maxLayerNum);
        if (!isRegionListExist()) {
    ...
    protected boolean calculateIsDrop(byte[] a, long presentationTimeUs) {
        ...
        if (isSlowFastExceptSlowV2120NoneSVC() || (i = this.mRecordingMode) == 15 || i == 19) {
            ...
            LogS.m473d("TranscodeLib", "layerNumber: " + layerNumber + ", isDrop: " + this.mIsDrop + ", mModifiedVideotime: " + this.mModifiedVideotime);
            return isDrop;
        }
        if (isSuperSlow()) {
            boolean isDrop2 = procSuperSlowVideo(presentationTimeUs, layerNumber, this.mRecordingFps);
            LogS.m473d("TranscodeLib", "isDrop: " + this.mIsDrop + " ,mModifiedVideotime: " + this.mModifiedVideotime);
            return isDrop2;
        }
        ...
        this.mOutputVideoEncoder.setCallback(new MediaCodec.Callback() { // from class: com.samsung.android.transcode.core.EncodeBase.1
            ...
            public void onOutputBufferAvailable(MediaCodec codec, int index, MediaCodec.BufferInfo bufferInfo) {
                if (!EncodeBase.this.mAsyncCodecReleased[ASYNC_CODEC_TYPE.VIDEO_ENCODER.ordinal()]) {
                    if (EncodeBase.this.isVideoEncoderAvailableCondition()) {
                        ...
                        LogS.m474e("TranscodeLib", "video encoder: [onOutputBufferAvailable] condition error");
                    }
                }
            }
            ...
            public void onOutputFormatChanged(MediaCodec codec, MediaFormat format) {
                if (!EncodeBase.this.mCodecError && !EncodeBase.this.mAsyncCodecReleased[ASYNC_CODEC_TYPE.VIDEO_ENCODER.ordinal()]) {
                    ...
                    LogS.m474e("TranscodeLib", "video encoder: onOutputFormatChanged " + EncodeBase.this.mVideoEncoderOutputMediaFormat);
    ...
    protected void prepareVideoCodecNeo() throws IOException {
        ...
        LogS.m475i("TranscodeLib", "input video auth : " + this.mAuthor + ", recordingMode :" + this.mRecordingMode);
        if (this.mUseUri) {
        ...
        if (this.mTrimVideoEndUs == 0) {
            ...
            LogS.m473d("TranscodeLib", "mTrimVideoEndUs was 0 but updated  mTrimVideoEndUs : " + this.mTrimVideoEndUs + ", mOriginTrimEndUs : " + this.mOriginTrimEndUs);
        }
    ...
    private void checkOutputVideoFrameRate() {
        if (this.mOutputVideoTargetFrameRate > 0) {
            ...
            LogS.m473d("TranscodeLib", "mSourceFrameRate : " + this.mSourceFrameRate + ", mOutputVideoTargetFrameRate : " + this.mOutputVideoTargetFrameRate + ", mOutputVideoFrameRate : " + this.mOutputVideoFrameRate);
            return;
        }
        if (this.mMMSMode) {
        ...
        if (this.mSourceFrameRate > 0) {
            if (this.m2ndTimeEncoding) {
                this.mOutputVideoFrameRate = this.mSourceFrameRate / this.mFramesSkipInterval;
                LogS.m473d("TranscodeLib", "m2ndTimeEncoding case mOutputVideoFrameRate : " + this.mOutputVideoFrameRate);
            } else {
                this.mOutputVideoFrameRate = this.mSourceFrameRate;
            }
        }
    ...
    protected void checkTrimAudioEndUs(MediaFormat inputAudioFormat) {
        if (this.mTrimAudioEndUs == 0) {
            ...
            LogS.m473d("TranscodeLib", "mTrimAudioEndUs was 0 but updated mTrimAudioEndUs :" + this.mTrimAudioEndUs + ", mOriginTrimEndUs:" + this.mOriginTrimEndUs);
        }
    }
    ...
    private boolean checkPendingAudioDecoderBufferIndex(int audioDecoderOutputBufferIndex, String mimeType) {
        if (audioDecoderOutputBufferIndex == -1) {
            LogS.m473d("TranscodeLib", "audio decoder output buffer try again later while preparing audio codec");
            return false;
        }
        if (audioDecoderOutputBufferIndex == -3) {
        ...
        if (audioDecoderOutputBufferIndex == -2) {
            ...
            LogS.m474e("TranscodeLib", "audio decoder: output format changed: SampleRate" + this.mOutputAudioSampleRateHZ + ",ChannelCount" + this.mOutputAudioChannelCount);
            this.formatupdated = true;
            return false;
        }
        if (audioDecoderOutputBufferIndex >= 0) {
    ...
    protected void updateProgress(long frametime, boolean isAudio) {
        ...
        if (this.mEncodeProgressListener != null && progress > this.mProgress) {
            LogS.m473d("TranscodeLib", "updateProgress: audioProgressTime: " + this.mAudioProgressTime + ", vidioProgressTime: " + this.mVidioProgressTime + ", time : " + time + ", progress: " + progress);
