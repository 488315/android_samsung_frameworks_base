...
public abstract class Encode {
    ...
    public void setupAndExecuteEncode() throws IOException {
        if (this.mSMConvert) {
            LogS.m473d("TranscodeLib", "starting encoder preparation  - SlowMo");
            prepareForRewrite();
        } else {
            LogS.m474e("TranscodeLib", "starting encoder preparation  mSMEncode  : " + this.mSMEncode + " mConvert : " + this.mConvert);
            prepare();
        }
        LogS.m473d("TranscodeLib", "encoder preparation done.");
        prepareListener();
        ...
        LogS.m474e("TranscodeLib", "encoding finished.");
    }
    ...
    public void encode() throws IOException {
        ...
        try {
            ...
            LogS.m473d("TranscodeLib", "generated output file size : " + size);
            if (!this.mSMConvert && !this.mConvert && !this.mUserStop) {
                ...
                if (j != -1 && size / 1024.0d > j && (this instanceof EncodeVideo)) {
                    if (!file.delete()) {
                        LogS.m474e("TranscodeLib", "Could not clean up file: " + file.getAbsolutePath());
                    }
                    this.mSizeFraction = (((this.mSizeFraction * this.mOutputMaxSizeKB) * 1024.0f) / size) - 0.05f;
                    LogS.m473d("TranscodeLib", "file size(" + size + ") exceeded the expected(" + (this.mOutputMaxSizeKB * 1024) + ") size limit. new fraction :" + this.mSizeFraction);
                    ...
                    try {
                        LogS.m473d("TranscodeLib", "2nd time starting encoder preparation");
                        this.m2ndTimeEncoding = true;
                        prepare();
                        LogS.m473d("TranscodeLib", "2nd time encoder preparation done.");
                        this.mMuxer = new MediaMuxer(this.mOutputFilePath, this.mOutputFormat);
                        this.mMuxerStarted = false;
                        this.mVideoTrackIndex = -1;
                        this.mAudioTrackIndex = -1;
                        LogS.m473d("TranscodeLib", "2nd time starting to encode");
                        if (this.mSMEncode) {
                            startSMEncoding();
                        } else {
                            startEncoding();
                        }
                        LogS.m473d("TranscodeLib", "2nd time encoding finished.");
                        release();
                        LogS.m474e("TranscodeLib", "2nd generated output size : " + new File(this.mOutputFilePath).length());
    ...
    public void rewrite() throws IOException {
        printVersionInfo();
        try {
            LogS.m473d("TranscodeLib", "starting Rewrite prepartion");
            prepareForRewrite();
            LogS.m473d("TranscodeLib", "Rewrite preparation done.");
            prepareListener();
            if (!this.mRewritable) {
                throw new IOException("Can't rewirte");
            }
            startRewriting();
            LogS.m473d("TranscodeLib", "Rewrite finished.");
            release();
            releaseListener();
        } catch (Throwable th) {
            release();
    ...
    private void prepareListener() throws IOException {
        ...
        this.mAudioTrackIndex = -1;
        this.mProgress = 0;
        LogS.m473d("TranscodeLib", "starting to encode");
    ...
    private void releaseListener() {
        if (this.mEncodeEventListener != null) {
            if (!this.mUserStop) {
                LogS.m474e("TranscodeLib", "calling onCompleted");
                this.mEncodeEventListener.onCompleted();
            } else {
                LogS.m473d("TranscodeLib", "user stopped. Not calling onCompleted");
            }
    ...
    private void printVersionInfo() {
        LogS.m474e("TranscodeLib", "Transcode Framework v.4.59");
    }

    public static String getLibraryVersion() {
        LogS.m474e("TranscodeLib", "getLibraryVersion  : Transcode Framework v.4.59");
        return VERSION;
    }
