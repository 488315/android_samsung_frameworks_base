                ...
                case 5:
                    ...
                    if (semContextMovement.getAction() == 1) {
                        try {
                            ...
                            Log.m94d(SemMotionRecognitionManager.TAG, "  >> check setting smart alert enabled : " + isEnabledPickUp);
                        } catch (RemoteException e) {
                            Log.m97e(SemMotionRecognitionManager.TAG, "RemoteException in getPickUpMotionStatus: ", e);
                        }
                        if (isEnabledPickUp) {
                            mrevent.setMotion(67);
                            Log.m94d(SemMotionRecognitionManager.TAG, "mySemContextMotionListener : Send Smart alert event");
        ...
        synchronized (this.sListenerDelegates) {
            ...
            while (it.hasNext()) {
                ...
                if (l.getListener() == listener) {
                    Log.m94d(TAG, "  .registerListener : fail. already registered / listener count = " + this.sListenerDelegates.size() + ", name :" + listener);
                    return;
                }
            }
            ...
            Log.m100v(TAG, "  .registerListener : success. listener count = " + size + Session.SUBSESSION_SEPARATION_CHAR + this.sListenerDelegates.size() + ", motion_events=" + motion_events + ", name :" + listener);
        }
    }
        ...
        synchronized (this.sListenerDelegates) {
            ...
            while (true) {
                ...
                if (l.getListener() != listener) {
                    ...
                    Log.m94d(TAG, "update listener " + i + " = name :" + listener + ",  motionevents = " + motionevents);
                    break;
                }
            }
    ...
    */
    public void unregisterListener(SemMotionEventListener listener) {
        ...
        synchronized (this.sListenerDelegates) {
            ...
            Log.m98i(TAG, "  .unregisterListener : / listener count = " + size + Session.SUBSESSION_SEPARATION_CHAR + this.sListenerDelegates.size() + ", name :" + listener);
        }
        l.resetListener();
        Log.m98i(TAG, "  .unregisterListener : / listener count = " + size + Session.SUBSESSION_SEPARATION_CHAR + this.sListenerDelegates.size() + ", name :" + listener);
    }
                ...
                public void handleMessage(Message msg) {
                    synchronized (SemMotionRecognitionManager.this.sListenerDelegates) {
                        try {
                            ...
                            Log.m97e(SemMotionRecognitionManager.TAG, "ClassCastException in handleMessage: msg.obj = " + msg.obj, e);
                        }
                    }
                }
            };
