package com.samsung.android.p043sm.iafdlib;
...
public class IafdSmAPIManager {
  ...
  private final ConcurrentHashMap<String, CheckUpdateCallback> mUpdateCallbackMap =
  ...
  public void checkUpdate(
      String pkgName, long versionCode, CheckUpdateCallback checkUpdateCallback) {
    if (TextUtils.isEmpty(pkgName)) {
      checkUpdateCallback.onResult(-1, -1L, null, null);
      Log.m96e(TAG, "pkgName is null");
      return;
    }
    if (this.mContentObserver == null) {
              ...
              new Handler(
                  Looper
                      .getMainLooper())) { // from class:
                                           // com.samsung.android.sm.iafdlib.IafdSmAPIManager.1
            @Override // android.database.ContentObserver
            public void onChange(boolean selfChange, Uri uri) {
              Log.m98i(IafdSmAPIManager.TAG, "update check done, content uri " + uri.toString());
              String pkgName2 = uri.getQueryParameter(IafdConstant.KEY_PACKAGE_NAME);
              long versionCode2 =
                  Long.valueOf(uri.getQueryParameter(IafdConstant.KEY_VERSION_CODE)).longValue();
              ...
              CheckUpdateCallback updateCallback =
                  (CheckUpdateCallback) IafdSmAPIManager.this.mUpdateCallbackMap.get(pkgName2);
              Log.m98i(
                  IafdSmAPIManager.TAG, "updateCallback is null = " + (updateCallback == null));
      ...
      this.mContext
          ...
          .registerContentObserver(
              IafdConstant.IAFD_STUB_EX_CHECK_UPDATE_API, true, this.mContentObserver);
    }
    ...
    if (System.currentTimeMillis() - this.mLastCheckTime > AlarmManager.INTERVAL_HALF_DAY) {
      this.mLastCheckTime = System.currentTimeMillis();
      Log.m98i(TAG, "cache is expired clear it");
      this.mCache.clear();
    }
    Result result = this.mCache.get(pkgName);
    if (result != null && checkUpdateCallback != null) {
      Log.m98i(TAG, "using cache ");
      checkUpdateCallback.onResult(
          result.resultCode, result.versionCode, result.versionName, result.pkgName);
      return;
    }
    ...
    this.mContext
        .getContentResolver()
        .call(
            IafdConstant.DC_API_AUTHORITY, IafdConstant.METHOD_CHECK_UPDATE, (String) null, bundle);
  }

  public void reportErrorDataToServer(
      String pkgName,
      int userId,
      int errorType,
      String errorStack,
    ...
    this.mContext
        .getContentResolver()
        .call(
            IafdConstant.DC_API_AUTHORITY,
            IafdConstant.METHOD_REPORT_TO_SERVER,
            (String) null,
            bundle);
  }
  ...
  public void onDestroy() {
    try {
      Log.m96e(TAG, "unregisterContentObserver");
      this.mContext.getContentResolver().unregisterContentObserver(this.mContentObserver);
      this.mContentObserver = null;
      this.mCache.clear();
      this.mUpdateCallbackMap.clear();
    } catch (Exception e) {
      Log.m97e(TAG, "onDestroy ", e);
    }
  }
