...
public class SemExtendedFormat {
    ...
    public static int addSEFDataFiles(String fileName, String[] keyNames, String[] dataFileNames, int[] dataTypes, int option) {
        ...
        if (dataCount != dataFileNames.length) {
            Log.m96e(TAG, "Data Count is different. ( keyNames data count= " + dataCount + ", dataFileNames data count= " + dataFileNames.length + " )");
        } else if (dataCount != dataTypes.length) {
            Log.m96e(TAG, "Data Count is different. ( keyNames data count= " + dataCount + ", dataTypes data count= " + dataTypes.length + " )");
        }
        if (fileName == null || fileName.length() <= 0) {
            Log.m96e(TAG, "Invalid file name: " + fileName);
            return 0;
    ...
    */
    ...
    public static int saveAudioJPEG(String fileName, String keyName, byte[] audioStream) {
        if (fileName == null || audioStream == null || audioStream.length <= 0) {
            Log.m96e(TAG, "Invalid file name: " + fileName);
            if (audioStream != null) {
                Log.m96e(TAG, "saveAudioJPEG input parameter is null :  audioStream.length = " + audioStream.length);
                return 0;
            }
    ...
    public static boolean isMp4ConversionSupported(Context context, String srcPath) {
        ...
        switch (type) {
            case 2048:
                Log.m98i(TAG, "SoundNShot is not supported from P OS. So, MP4 Conversion for SoundNShot is removed from Q OS");
                break;
            case 2256:
                Log.m98i(TAG, "VirtualShot is not supported from R OS. So, MP4 Conversion for VirtualShot is removed from R OS");
                break;
            case DataType.PANORAMA_SHOT_INFO /* 2272 */:
                Log.m98i(TAG, "MotionPanoramaShot is not supported from R OS. So, MP4 Conversion for MotionPanoramaShot is removed from R OS");
                break;
            case DataType.WIDE_SELFIE_INFO /* 2416 */:
                Log.m98i(TAG, "MotionWideSelfie is not supported from R OS. So, MP4 Conversion for MotionWideSelfie is removed from R OS");
                break;
            case 2608:
                break;
            default:
