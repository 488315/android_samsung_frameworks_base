...
public class SemExtendedFormat {
  ...
  public static boolean isValidFile(File sefFile) throws IOException {
    ...
    if (fileName == null || fileName.length() <= 0) {
      Log.m96e(TAG, "Invalid file name: " + fileName);
      return false;
    }
  ...
  public static boolean hasData(File sefFile, int dataType) throws IOException {
    ...
    if (fileName == null || fileName.length() <= 0 || dataType == -1) {
      Log.m96e(TAG, "Invalid file name: " + fileName + ", Data Type: " + dataType);
      return false;
    }
    ...
    if (currentTypes == null) {
      Log.m96e(TAG, "Invalid file : " + fileName);
      return false;
    }
  ...
  public static boolean hasData(File sefFile, String keyName) throws IOException {
    ...
    if (fileName == null || fileName.length() <= 0 || keyName.length() <= 0) {
      Log.m96e(TAG, "Invalid file name: " + fileName + ", keyName: " + keyName);
      return false;
    }
    if (SEFJNI.isSEFFile(fileName) == 0) {
      return false;
  ...
  public static int addData(File sefFile, String keyName, byte[] data, int dataType, int option)
    ...
    if (keyName == null || keyName.length() <= 0) {
      Log.m96e(TAG, "Invalid key name: " + keyName);
      return 0;
    }
    if (data == null || data.length <= 0) {
      Log.m96e(TAG, "Invalid data");
      return 0;
    }
    ...
    if (option == 0 || option == 1) {
      return SEFJNI.addSEFData(
          fileName, keyName, keyName.length(), null, 0, data, data.length, dataType, option);
    }
    Log.m96e(TAG, "Unsupported Option Combination. Please check the option !!!!!");
    return 0;
  }
  ...
  public static int addData(File sefFile, String keyName, File dataFile, int dataType, int option)
    ...
    if (dataFileName == null || dataFileName.length() <= 0) {
      Log.m96e(TAG, "Invalid SEF Data File name: " + dataFileName);
      return 0;
    }
    if (option == 256 || option == 4) {
  ...
  public static long addData(
    ...
    if (option == 0 || option == 1) {
      return addSEFDataByteBufferAddTag(
          buffer, keyName, keyName.length(), null, 0, data, data.length, dataType, option);
    }
    Log.m96e(TAG, "Unsupported Option Combination. Please check the option !!!!!");
    Log.m96e(TAG, "You can use only one of two - TYPE_SKIP_IF_EXISTS, TYPE_OVERWRITE_IF_EXISTS");
    return 0L;
  }
  ...
  public static long getRequiredBufferSize(
    ...
    if (orgDataSize <= 0) {
      Log.m96e(TAG, "invalid orgDataSize : " + orgDataSize);
      return 0L;
    }
    if (dataCount <= 0) {
      Log.m96e(TAG, "invalid dataCount : " + dataCount);
      return 0L;
    }
    if (totalDataSize <= 0) {
      Log.m96e(TAG, "invalid totalDataSize : " + totalDataSize);
      return 0L;
    }
    if (totalkeyNameSize <= 0) {
      Log.m96e(TAG, "invalid totalkeyNameSize : " + totalkeyNameSize);
      return 0L;
    }
  ...
  public static int addSEFData(File sefFile, String keyName, byte[] data, int dataType, int option)
      throws IOException {
  ...
  public static int addSEFData(
      File sefFile, String keyName, File dataFile, int dataType, int option) throws IOException {
    return addSEFData(sefFile, keyName, dataFile, (byte[]) null, dataType, option);
  }

  public static int addSEFData(
      File sefFile, String keyName, File dataFile, byte[] subdataInfo, int dataType, int option)
      throws IOException {
    String fileName = sefFile.getCanonicalPath();
    String dataFileName = dataFile.getCanonicalPath();
    if (fileName == null || fileName.length() <= 0) {
  ...
  public static int addSEFData(
      String fileName, String keyName, byte[] data, byte[] subdataInfo, int dataType, int option) {
    if (fileName != null && fileName.length() > 0) {
      if (keyName != null && keyName.length() > 0) {
        if (data != null && data.length > 0) {
          if (option == 16) {
  ...
  public static int addSEFDataFiles(
      String fileName, String[] keyNames, String[] dataFileNames, int[] dataTypes, int option) {
  ...
  public static boolean deleteAllData(File sefFile) throws IOException {
    String fileName = sefFile.getCanonicalPath();
    if (fileName != null && fileName.length() > 0) {
      return (SEFJNI.isSEFFile(fileName) == 0 && QdioJNI.isJPEG(fileName) == 1)
          ? QdioJNI.DeleteQdioFromFile(fileName) == 1
  ...
  public static boolean deleteAllData(ParcelFileDescriptor pfd) throws IOException {
    return (SEFJNI.isSEFfileDescriptor(pfd) == 0 && QdioJNI.isJPEGfd(pfd) == 1)
        ? QdioJNI.deleteQdioFromFileDescriptor(pfd) == 1
        : SEFJNI.clearSEFDataFileDescriptor(pfd) == 1;
  }
  ...
  */
  public static byte[] getData(File sefFile, String keyName) throws IOException {
    ...
    if (SEFJNI.isSEFFile(fileName) == 0 && QdioJNI.isJPEG(fileName) == 1) {
      QdioJPEGData qdioJpegData = QdioJNI.checkAudioInJPEG(fileName);
      return QdioJNI.getAudioStreamBuffer(qdioJpegData, 0);
    }
    try {
      try {
        fis = new FileInputStream(fileName);
        dataPosition = getDataPosition(sefFile, keyName);
      } catch (Exception e) {
        e.printStackTrace();
      }
      ...
      long j = dataPosition.length + startOffset;
  ...
  */
  /*
      Code decompiled incorrectly, please refer to instructions dump.
  */
  public static byte[] getData(ParcelFileDescriptor pfd, String keyName) throws IOException {
    DataPosition dataPosition;
    if (keyName == null || keyName.length() <= 0) {
      Log.m96e(TAG, "Invalid key name: " + keyName);
      return null;
    ...
    FileInputStream fis = null;
    if (SEFJNI.isSEFfileDescriptor(pfd) == 0 && QdioJNI.isJPEGfd(pfd) == 1) {
      QdioJPEGData qdioJpegData = QdioJNI.checkAudioInJPEGfd(pfd);
      return QdioJNI.getAudioStreamBuffer(qdioJpegData, 0);
    }
    try {
      try {
        fis = new FileInputStream(pfd.getFileDescriptor());
        dataPosition = getDataPosition(pfd, keyName);
      } catch (Exception e) {
        e.printStackTrace();
      }
      if (dataPosition == null) {
  ...
  public static DataPosition getDataPosition(File sefFile, String keyName) throws IOException {
    ...
    if (SEFJNI.isSEFFile(fileName) == 0 && QdioJNI.isJPEG(fileName) == 1) {
      ...
      if (posArray == null) {
        Log.m102w(TAG, "No Sound data is found in file.");
        return null;
      }
    ...
    if (posArray2 == null) {
      Log.m102w(TAG, "No SEF data is found in file.");
      return null;
    }
  ...
  public static long[] getDataPositionArray(File sefFile, String keyName) throws IOException {
    ...
    if (PositionData == null) {
      Log.m102w(TAG, "No SEF data matching to given keyName is found in file.");
      return null;
    }
    return PositionData;
  }
  ...
  public static SEFSubDataPosition getSEFSubDataPosition(String fileName, String keyName) {
    ...
    if (posArray == null) {
      Log.m102w(TAG, "No SEF sub data is found in file.");
      return null;
    }
  ...
  */
  public static byte[] getSEFData(File sefFile, String keyName) throws IOException {
    ...
    try {
      try {
        fis = new FileInputStream(fileName);
        dataPosition = getSEFDataPosition(fileName, keyName);
      } catch (Exception e) {
        e.printStackTrace();
      }
      if (dataPosition == null) {
  ...
  */
  ...
  public static int getMajorDataType(String fileName) {
    ...
    if (dataTypes == null) {
      Log.m96e(TAG, "No data type has been found : " + fileName);
      return -1;
    }
    ...
    Log.m96e(TAG, "No major data type has been found : " + fileName);
    return -1;
  }
  ...
  public static int copyAllData(File srcFile, File dstFile) throws IOException {
    ...
    if (srcFileName == null || srcFileName.length() <= 0) {
      Log.m96e(TAG, "Invalid src file name: " + srcFileName);
      return 0;
    }
    if (dstFileName == null || dstFileName.length() <= 0) {
      Log.m96e(TAG, "Invalid dst file name: " + dstFileName);
      return 0;
    }
  ...
  public static class AudioJPEGData {
    ...
    public int getLength(int idx) {
      ...
      return ((Integer) this.endOffset.get(idx)).intValue()
          - ((Integer) this.startOffset.get(idx)).intValue();
    }
  ...
  public static int saveAudioJPEG(String fileName, String keyName, byte[] audioStream) {
    if (fileName == null || audioStream == null || audioStream.length <= 0) {
      ...
      if (audioStream != null) {
        Log.m96e(
            TAG,
            "saveAudioJPEG input parameter is null :  audioStream.length = " + audioStream.length);
        return 0;
      }
      Log.m96e(TAG, "saveAudioJPEG input parameter is null ");
      return 0;
    }
    return SEFJNI.saveAudioJPEG(
        fileName, audioStream, audioStream.length, keyName, keyName.length());
  }
  ...
  public static int deleteAudioData(String fileName) {
    ...
    return SEFJNI.deleteQdioData(
        fileName, KeyName.SOUND_SHOT_WAVE, KeyName.SOUND_SHOT_WAVE.length());
  }
  ...
  public static byte[] getAudioStreamBuffer(AudioJPEGData qdioJpegData, int index)
      throws IOException {
    return QdioJNI.getAudioStreamBuffer(qdioJpegData, index);
  }

  public static byte[] getAudioStreamBuffer(QdioJPEGData qdioJpegData, int index)
      throws IOException {
    return QdioJNI.getAudioStreamBuffer(qdioJpegData, index);
  }
  ...
  public static void convertImageToMP4(String srcPath, String targetPath) {
    ...
    switch (type) {
      case 2608:
        ...
        break;
      default:
        Log.m96e(TAG, "This type of file is not yet supported. type=" + type);
        break;
    }
  }
  ...
  public static boolean isMp4ConversionSupported(Context context, String srcPath) {
    ...
    switch (type) {
      case 2048:
        Log.m98i(
            TAG,
            "SoundNShot is not supported from P OS. So, MP4 Conversion for SoundNShot is removed"
                + " from Q OS");
        break;
      case 2256:
        Log.m98i(
            TAG,
            "VirtualShot is not supported from R OS. So, MP4 Conversion for VirtualShot is removed"
                + " from R OS");
        break;
      ...
      case DataType.WIDE_SELFIE_INFO /* 2416 */:
        Log.m98i(
            TAG,
            "MotionWideSelfie is not supported from R OS. So, MP4 Conversion for MotionWideSelfie"
