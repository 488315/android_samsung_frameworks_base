...
public class SemPhotoRemaster {
    ...
    public static class Builder {
        ...
        private Builder(Uri inputUri) {
            LogUtil.m294i(TAG, new Throwable().getStackTrace()[0].getMethodName() + " is called!");
            LogUtil.m291d(TAG, "Input file: " + inputUri);
        ...
        private Builder(Bitmap inputImage) {
            LogUtil.m294i(TAG, new Throwable().getStackTrace()[0].getMethodName() + " is called!");
            LogUtil.m291d(TAG, "Input is bitmap");
            this.mInputUri = null;
        ...
        public void updateInputPath(Context context) {
            ...
            try {
                if (cursor == null) {
                    LogUtil.m291d(TAG, "InputPath is set as inputUri.getPath()");
                } else {
                    int column_index = cursor.getColumnIndexOrThrow("_data");
                    cursor.moveToFirst();
                    LogUtil.m294i(TAG, "InputPath is replaced as content provider.");
                    this.mInputPathName = cursor.getString(column_index);
                    LogUtil.m291d(TAG, "mInputPathName is updated as " + this.mInputPathName);
                }
                if (cursor != null) {
                    cursor.close();
                }
    ...
    public static class Result {
        ...
        public String getParameter(int id) {
            try {
                ...
                LogUtil.m292e(TAG, "No such id:" + id);
                return null;
            }
        }
        ...
        public void setParameter(int id, String val) {
            try {
                ...
                LogUtil.m292e(TAG, new Throwable().getStackTrace()[0].getMethodName() + NavigationBarInflaterView.KEY_CODE_START + id + ", " + val + NavigationBarInflaterView.KEY_CODE_END);
    ...
    private Result getParameters() {
        LogUtil.m294i(TAG, new Throwable().getStackTrace()[0].getMethodName() + " is called!");
        final Result result = new Result(this.mBuilder.mBitmap == null ? null : this.mServiceClient.getBitmapParam(ResultParam.OUTPUT_BITMAP.f3032ID));
        ...
        if (this.mBuilder.mRequestFocusRoi && remasteredPathName != null) {
            result.setParameter(ResultParam.FOCUS_ROI.f3032ID, this.mServiceClient.getFocusRoi(this.mBuilder.mInputPathName, remasteredPathName));
        }
    ...
    public /* synthetic */ void lambda$getParameters$2(Result result, ResultParam param) {
        String paramData = this.mServiceClient.getStringParam(param.f3032ID);
        LogUtil.m291d(TAG, "getStringParam(" + param.f3032ID + ") : " + paramData);
        result.setParameter(param.f3032ID, paramData);
    }
    ...
    public /* synthetic */ void lambda$getParameters$5(Result result, ResultParam param) {
        String paramData = String.valueOf(this.mServiceClient.getLongParam(param.f3032ID));
        LogUtil.m291d(TAG, "getLongParam(" + param.f3032ID + ") : " + paramData);
        result.setParameter(param.f3032ID, paramData);
    }
        ...
        synchronized (SemPhotoRemaster.class) {
            ...
            synchronized (obj) {
                if (this.mServiceClient != null) {
                    LogUtil.m292e(TAG, "Error mServiceClient is not null at begin of doRemaster");
                }
                this.mServiceClient = new ClientRemasterDirector();
                if (this.mBuilder.mTryInit) {
                    LogUtil.m291d(TAG, "TryInit is started.");
                    if (!this.mServiceClient.tryInit(context)) {
                        LogUtil.m296w(TAG, "Failed to tryInit()");
                ...
                if (processingSuccess) {
                    LogUtil.m291d(TAG, "Raw Result: " + result.mResultJson);
                }
                synchronized (obj) {
    ...
    public boolean stop() {
        ...
        synchronized (mStopLock) {
            ...
            if (clientRemasterDirector == null) {
                LogUtil.m294i(TAG, "Service is already stopped.");
                return true;
            }
