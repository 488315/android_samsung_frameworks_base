...
public class SemPhotoRemaster {
  ...
  public static class Builder {
    ...
    public void updateInputPath(Context context) {
      ...
      Cursor cursor =
          context.getContentResolver().query(this.mInputUri, projection, null, null, null);
      try {
        if (cursor == null) {
  ...
  private Result getParameters() {
    ...
    final Result result =
        new Result(
            this.mBuilder.mBitmap == null
                ? null
                : this.mServiceClient.getBitmapParam(ResultParam.OUTPUT_BITMAP.f3032ID));
    ...
    if (this.mBuilder.mRequestFocusRoi && remasteredPathName != null) {
      result.setParameter(
          ResultParam.FOCUS_ROI.f3032ID,
          this.mServiceClient.getFocusRoi(this.mBuilder.mInputPathName, remasteredPathName));
    }
