...
public abstract class SemBackgroundMusic {
  ...
  public Parcel writeToParcel(String interfaceName) {
    ...
    Log.m98i(TAG, "BackgroundMusic size : " + this.mBGMInfos.size());
    for (int i = 0; i < this.mBGMInfos.size(); i++) {
      if (interfaceName != null) {
        try {
          p.writeFileDescriptor(this.mBGMInfos.get(i).f3024fd);
        } catch (IOException e) {
          Log.m98i(TAG, "setBackgroundMusic ParcelFileDescriptor.dup failed");
        }
      } else {
        ParcelFileDescriptor pfd = ParcelFileDescriptor.dup(this.mBGMInfos.get(i).f3024fd);
  ...
  protected BGMInfo addInfo(BGMInfo bgmInfo, AssetFileDescriptor afd, int startTime, int endTime) {
    bgmInfo.f3024fd = afd.getFileDescriptor();
