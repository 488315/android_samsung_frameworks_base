...
public class SemImageCrop {
    ...
    private SemImageCrop() {
        ...
        nativeSetup();
        Log.m98i(TAG, "setup : " + this.mNativeHandle);
    }
    ...
    public static SemImageCrop getInstance() {
        if (isSupport) {
            ...
            Log.m102w(TAG, "is not supported");
        }
        return instance;
    }
    ...
    public SemCroppedImageInfo crop(ByteBuffer in, Rect rect) {
        if (in == null) {
            Log.m96e(TAG, "in is null!");
            return null;
        }
        if (rect == null) {
            Log.m96e(TAG, "rect is null!");
            return null;
        }
        if (!isValidRect(rect)) {
            Log.m96e(TAG, "rect is not valid, check rect properties has negative value or width/height is less than or equal to zero!");
            return null;
        }
        if (isHeicFormat(in)) {
            if (!hasHevcEncoder || !isSupportHeifCapture) {
                Log.m96e(TAG, "is heic format, but hevcEncoder : " + hasHevcEncoder + ", supportHeifCapture : " + isSupportHeifCapture);
                return null;
            }
            ...
            in.get(header);
            in.rewind();
            Log.m96e(TAG, "image format is not supported!, {" + bytesToHex(header, 16) + "}");
            return null;
        }
