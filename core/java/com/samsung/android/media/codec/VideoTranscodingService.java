...
public class VideoTranscodingService extends IVideoTranscodingService.Stub {
                            ...
                            try {
                                ...
                                while (true) {
                                    if (VideoTranscodingService.this.mStartingTasks.size() != 0) {
                                        ...
                                        if (top.getState() == 0) {
                                            Log.m98i(VideoTranscodingService.TAG, "Task(" + top.getID() + ") has been started");
                                            top.start();
    ...
    public synchronized String register(int mode, IVideoTranscodingServiceCallback callback) {
        if (callback == null) {
            Log.m102w(TAG, "callback is null");
            return null;
        }
        ...
        boolean ret = task.linkToDeath(new IBinder.DeathRecipient() { // from class: com.samsung.android.media.codec.VideoTranscodingService.2
            ...
            public void binderDied() {
                Log.m96e(VideoTranscodingService.TAG, "binderDied: task(" + task.getID() + NavigationBarInflaterView.KEY_CODE_END);
                try {
        ...
        if (!ret) {
            Log.m96e(TAG, "Failed to link to death.");
            return null;
        }
        addTask(task);
        return id;
    ...
    private void addTask(Task task) {
        Log.m94d(TAG, "addTask(" + task.getID() + NavigationBarInflaterView.KEY_CODE_END);
        this.mTaskLock.lock();
        try {
            try {
    ...
    public void startTask(String id) throws RemoteException {
        Task task;
        Log.m94d(TAG, "startTask(" + id + NavigationBarInflaterView.KEY_CODE_END);
        this.mTaskLock.lock();
        try {
            try {
            ...
            if (task == null) {
                Log.m102w(TAG, "There is no task id(" + id + ") to start");
                return;
            }
    ...
    public void stopTask(String id) throws RemoteException {
        Log.m94d(TAG, "stopTask(" + id + NavigationBarInflaterView.KEY_CODE_END);
        ...
        try {
            try {
                if (this.mWaitingTasks.containsKey(id)) {
                    ...
                    Log.m98i(TAG, "Task(" + id + ") has been removed in w queue.");
                    isRemoved = true;
                } else {
                    ...
                    while (true) {
                        ...
                        if (task.getID().equals(id)) {
                            removedTask = task;
                            if (task.getState() == 1) {
                                Log.m98i(TAG, "Task(" + task.getID() + ") try to stop.");
                                task.stop();
                            }
                            this.mStartingTasks.remove(task);
                            Log.m98i(TAG, "Task(" + id + ") has been removed in s queue.");
                            isRemoved = true;
                        }
                    }
                }
            ...
            if (isRemoved) {
                ...
                Log.m102w(TAG, "There is no task id(" + id + ") to stop");
            }
        } finally {
            this.mTaskLock.unlock();
        }
    ...
    public void printTasks() {
        ...
        Log.m98i(TAG, "Waiting tasks(" + this.mWaitingTasks.size() + NavigationBarInflaterView.KEY_CODE_END + tasks);
        String tasks2 = "";
        int i2 = 1;
        ...
        Log.m98i(TAG, "Starting tasks(" + this.mStartingTasks.size() + NavigationBarInflaterView.KEY_CODE_END + tasks2);
    }
}
