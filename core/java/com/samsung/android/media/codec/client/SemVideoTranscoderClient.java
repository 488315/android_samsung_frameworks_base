...
public class SemVideoTranscoderClient extends ClientImpl {
    ...
    public SemVideoTranscoderClient(
            ...
            try {
                try {
                    try {
                        String inputPath =
                                (String)
                                        SemVideoTranscoderClient.this.mArgs.get(
                        ...
                        switch (SemVideoTranscoderClient.this.mMode) {
                            case 0:
                                if (SemVideoTranscoderClient.isSupportedHdrToSdr()
                                        && hdr10bit != null
                                        && bitDepth != null) {
                            ...
                            default:
                                Log.w(
                                        Context.SEM_VIDEO_TRANSCODING_SERVICE,
                                        "Unsupported mode ("
                                                + SemVideoTranscoderClient.this.mMode
                                                + NavigationBarInflaterView.KEY_CODE_END);
                                dstVideoCodecType = 4;
                        }
                        if (isValid) {
                            SemVideoTranscoderClient.this.mTranscoder.initialize(
                                    outputPath, w, h, inputPath);
                            SemVideoTranscoderClient.this.mTranscoder
                                    .setVideoTranscodingServiceCallback(
                                            SemVideoTranscoderClient.this.mProgressCallback);
                            SemVideoTranscoderClient.this.mTranscoder.setOutputConfig(
                                    1, dstVideoCodecType);
                            SemVideoTranscoderClient.this.mTranscoder.setOutputConfig(2, 2);
                            if (SemVideoTranscoderClient.this.mMode != 0
                                    && SemVideoTranscoderClient.this.mMode != 2) {
                                SemVideoTranscoderClient.this.mIsRunning = true;
                                SemVideoTranscoderClient.this.mTranscoder.encode();
                                Log.i(
                                        Context.SEM_VIDEO_TRANSCODING_SERVICE,
                                        "Task("
                                                + SemVideoTranscoderClient.this.mID
                                                + ") has been finished");
                            }
                        ...
                        SemVideoTranscoderClient.this.mTranscodingService.stopTask(
                                SemVideoTranscoderClient.this.mID);
                    } catch (Exception e) {
                        e.printStackTrace();
                        Log.w(
