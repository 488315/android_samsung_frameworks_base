...
public class ConvertVideo extends Convert {
        ...
        try {
            ...
            try {
                ...
                if (this.mTrimVideoEndUs == 0) {
                    ...
                    Log.m94d(Constants.TAG, "mTrimVideoEndUs was 0 but updated  mTrimVideoEndUs : " + this.mTrimVideoEndUs);
                }
                ...
                Log.m94d(Constants.TAG, "mSourceFrameRate :" + this.mSourceFrameRate + ", mOutputVideoFrameRate :" + this.mOutputVideoFrameRate);
    ...
    */
    protected boolean prepareAudioCodec() {
        ...
        try {
            if (mUseUri) {
                convertVideo2.mAudioExtractor = CodecsHelper.createExtractor(convertVideo2.mContext, convertVideo2.mInputUri);
            } else {
                convertVideo2.mAudioExtractor = CodecsHelper.createExtractor(convertVideo2.mInputFilePath);
            }
            int audioInputTrack2 = CodecsHelper.getAndSelectAudioTrackIndex(convertVideo2.mAudioExtractor);
            ...
            if (convertVideo2.mTrimAudioEndUs == 0) {
                ...
                Log.m94d(Constants.TAG, "mTrimAudioEndUs was 0 but updated mTrimAudioEndUs :" + convertVideo2.mTrimAudioEndUs);
            }
            Log.m94d(Constants.TAG, "Audio input format " + inputAudioFormat);
            convertVideo2.mOutputAudioSampleRateHZ = inputAudioFormat.getInteger(MediaFormat.KEY_SAMPLE_RATE);
            convertVideo2.mOutputAudioChannelCount = inputAudioFormat.getInteger(MediaFormat.KEY_CHANNEL_COUNT);
            ...
            if ("audio/mp4a-latm".equals(mimeType)) {
                try {
                    ...
                    convertVideo2.mInputAudioDecoder = CodecsHelper.createAudioDecoder(audioinputCodecInfo2, inputAudioFormat);
                    if (convertVideo2.mCopyAudio) {
                        try {
                            ...
                            MediaFormat outputAudioFormat = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                            if (maxInputSize != 0) {
                            }
                            outputAudioFormat.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                            outputAudioFormat.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                            Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat);
                            MediaCodecInfo audioCodecInfo = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                            convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo, outputAudioFormat);
                            ...
                            maxInputSize = 0;
                            maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                            MediaFormat outputAudioFormat2 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                            if (maxInputSize != 0) {
                            }
                            outputAudioFormat2.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                            outputAudioFormat2.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                            Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat2);
                            MediaCodecInfo audioCodecInfo2 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                            convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo2, outputAudioFormat2);
                            if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                            }
                            if ("audio/mp4a-latm".equals(mimeType)) {
                            }
                            ...
                            maxInputSize = 0;
                            maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                            MediaFormat outputAudioFormat22 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                            if (maxInputSize != 0) {
                            }
                            outputAudioFormat22.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                            outputAudioFormat22.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                            Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat22);
                            MediaCodecInfo audioCodecInfo22 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                            convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo22, outputAudioFormat22);
                            if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                            }
                            if ("audio/mp4a-latm".equals(mimeType)) {
                            }
                    ...
                    ByteBuffer[] audioDecoderInputBuffers = convertVideo2.mCopyAudio ? convertVideo2.mInputAudioDecoder.getInputBuffers() : null;
                    MediaCodec.BufferInfo audioDecoderOutputBufferInfo2 = new MediaCodec.BufferInfo();
                    Runnable schedulerCallback = new Runnable() { // from class: com.samsung.android.media.convert.core.ConvertVideo.1
                    ...
                    while (true) {
                        ...
                        if (z) {
                            ...
                            try {
                                ...
                                audioDecoderOutputBuffers2 = audioDecoderOutputBuffers;
                                int audioDecoderInputBufferIndex = convertVideo.mInputAudioDecoder.dequeueInputBuffer(10000L);
                                if (audioDecoderInputBufferIndex == -1) {
                                    try {
                                        Log.m94d(Constants.TAG, "audio decoder input try again later while preparing audio codec");
                                        ...
                                        maxInputSize = 0;
                                        maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                        MediaFormat outputAudioFormat222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                        if (maxInputSize != 0) {
                                        }
                                        outputAudioFormat222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                        outputAudioFormat222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                        Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat222);
                                        MediaCodecInfo audioCodecInfo222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                        convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo222, outputAudioFormat222);
                                        if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                        }
                                        if ("audio/mp4a-latm".equals(mimeType)) {
                                        }
                                        ...
                                        maxInputSize = 0;
                                        maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                        MediaFormat outputAudioFormat2222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                        if (maxInputSize != 0) {
                                        }
                                        outputAudioFormat2222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                        outputAudioFormat2222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                        Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat2222);
                                        MediaCodecInfo audioCodecInfo2222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                        convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo2222, outputAudioFormat2222);
                                        if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                        }
                                        if ("audio/mp4a-latm".equals(mimeType)) {
                                        }
                                        ...
                                        maxInputSize = 0;
                                        maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                        MediaFormat outputAudioFormat22222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                        if (maxInputSize != 0) {
                                        }
                                        outputAudioFormat22222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                        outputAudioFormat22222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                        Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat22222);
                                        MediaCodecInfo audioCodecInfo22222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                        convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo22222, outputAudioFormat22222);
                                        if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                        }
                                        if ("audio/mp4a-latm".equals(mimeType)) {
                                        }
                                    ...
                                    audioinputCodecInfo = audioinputCodecInfo3;
                                    int size = convertVideo.mAudioExtractor.readSampleData(audioDecoderInputBuffer, 0);
                                    long presentationTimeUs = convertVideo.mAudioExtractor.getSampleTime();
                                    if (size > 0) {
                                        audioInputTrack = audioInputTrack2;
                                        convertVideo.mInputAudioDecoder.queueInputBuffer(audioDecoderInputBufferIndex, 0, size, presentationTimeUs, convertVideo.mAudioExtractor.getSampleFlags());
                                    } else {
                                        audioInputTrack = audioInputTrack2;
                                        if (size == -1) {
                                ...
                                maxInputSize = 0;
                                maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                MediaFormat outputAudioFormat222222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                if (maxInputSize != 0) {
                                }
                                outputAudioFormat222222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                outputAudioFormat222222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat222222);
                                MediaCodecInfo audioCodecInfo222222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo222222, outputAudioFormat222222);
                                if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                }
                                if ("audio/mp4a-latm".equals(mimeType)) {
                                }
                                ...
                                maxInputSize = 0;
                                maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                MediaFormat outputAudioFormat2222222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                if (maxInputSize != 0) {
                                }
                                outputAudioFormat2222222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                outputAudioFormat2222222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat2222222);
                                MediaCodecInfo audioCodecInfo2222222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo2222222, outputAudioFormat2222222);
                                if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                }
                                if ("audio/mp4a-latm".equals(mimeType)) {
                                }
                                ...
                                maxInputSize = 0;
                                maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                MediaFormat outputAudioFormat22222222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                if (maxInputSize != 0) {
                                }
                                outputAudioFormat22222222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                outputAudioFormat22222222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat22222222);
                                MediaCodecInfo audioCodecInfo22222222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo22222222, outputAudioFormat22222222);
                                if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                }
                                if ("audio/mp4a-latm".equals(mimeType)) {
                                }
                        ...
                        if (convertVideo.formatupdated) {
                            ...
                            if (pendingAudioDecoderOutputBufferIndex3 == -1) {
                                try {
                                    ...
                                    audioDecoderOutputBufferInfo = audioDecoderOutputBufferInfo2;
                                    int audioDecoderOutputBufferIndex = convertVideo.mInputAudioDecoder.dequeueOutputBuffer(audioDecoderOutputBufferInfo, 10000L);
                                    if (audioDecoderOutputBufferIndex == -1) {
                                        Log.m94d(Constants.TAG, "audio decoder output buffer try again later while preparing audio codec");
                                    } else if (audioDecoderOutputBufferIndex == -3) {
                                        Log.m94d(Constants.TAG, "audio decoder: output buffers changed ");
                                        ByteBuffer[] audioDecoderOutputBuffers3 = convertVideo.mInputAudioDecoder.getOutputBuffers();
                                        ...
                                        convertVideo.mOutputAudioSampleRateHZ = convertVideo.mInputAudioDecoder.getOutputFormat().getInteger(MediaFormat.KEY_SAMPLE_RATE);
                                        convertVideo.mOutputAudioChannelCount = convertVideo.mInputAudioDecoder.getOutputFormat().getInteger(MediaFormat.KEY_CHANNEL_COUNT);
                                        Log.m94d(Constants.TAG, "audio decoder: output format changed: SampleRate" + convertVideo.mOutputAudioSampleRateHZ + ",ChannelCount" + convertVideo.mOutputAudioChannelCount);
                                        convertVideo.formatupdated = true;
                                    } else if (audioDecoderOutputBufferIndex < 0) {
                                        Log.m94d(Constants.TAG, "Unexpected result from audio decoder dequeue output format.");
                                    } else if ((audioDecoderOutputBufferInfo.flags & 2) != 0) {
                                        Log.m94d(Constants.TAG, "audio decoder: codec config buffer");
                                        convertVideo.mInputAudioDecoder.releaseOutputBuffer(audioDecoderOutputBufferIndex, false);
                                    } else {
                                    ...
                                    maxInputSize = 0;
                                    maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                    MediaFormat outputAudioFormat222222222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                    if (maxInputSize != 0) {
                                    }
                                    outputAudioFormat222222222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                    outputAudioFormat222222222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                    Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat222222222);
                                    MediaCodecInfo audioCodecInfo222222222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                    convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo222222222, outputAudioFormat222222222);
                                    if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                    }
                                    if ("audio/mp4a-latm".equals(mimeType)) {
                                    }
                                    ...
                                    maxInputSize = 0;
                                    maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                    MediaFormat outputAudioFormat2222222222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                    if (maxInputSize != 0) {
                                    }
                                    outputAudioFormat2222222222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                    outputAudioFormat2222222222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                    Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat2222222222);
                                    MediaCodecInfo audioCodecInfo2222222222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                    convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo2222222222, outputAudioFormat2222222222);
                                    if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                    }
                                    if ("audio/mp4a-latm".equals(mimeType)) {
                                    }
                                    ...
                                    maxInputSize = 0;
                                    maxInputSize = inputAudioFormat.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE);
                                    MediaFormat outputAudioFormat22222222222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                                    if (maxInputSize != 0) {
                                    }
                                    outputAudioFormat22222222222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                                    outputAudioFormat22222222222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                                    Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat22222222222);
                                    MediaCodecInfo audioCodecInfo22222222222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                                    convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo22222222222, outputAudioFormat22222222222);
                                    if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                                    }
                                    if ("audio/mp4a-latm".equals(mimeType)) {
                                    }
                ...
                convertVideo = convertVideo2;
                if (MediaFormat.MIMETYPE_AUDIO_AC3.equals(mimeType) || MediaFormat.MIMETYPE_AUDIO_EAC3.equals(mimeType) || MediaFormat.MIMETYPE_AUDIO_EAC3_JOC.equals(mimeType) || MediaFormat.MIMETYPE_AUDIO_AC4.equals(mimeType)) {
            ...
            try {
                MediaFormat outputAudioFormat222222222222 = MediaFormat.createAudioFormat(convertVideo.mOutputAudioMimeType, convertVideo.mOutputAudioSampleRateHZ, convertVideo.mOutputAudioChannelCount);
                if (maxInputSize != 0) {
                    outputAudioFormat222222222222.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, maxInputSize);
                }
                outputAudioFormat222222222222.setInteger(MediaFormat.KEY_BIT_RATE, convertVideo.mOutputAudioBitRate);
                outputAudioFormat222222222222.setInteger(MediaFormat.KEY_AAC_PROFILE, convertVideo.mOutputAudioAACProfile);
                Log.m94d(Constants.TAG, "Audio output format " + outputAudioFormat222222222222);
                MediaCodecInfo audioCodecInfo222222222222 = CodecsHelper.getEncoderCodec(convertVideo.mOutputAudioMimeType);
                convertVideo.mOutputAudioEncoder = CodecsHelper.createAudioEncoder(audioCodecInfo222222222222, outputAudioFormat222222222222);
                if (inputAudioFormat.containsKey(MediaFormat.KEY_ENCODER_DELAY)) {
                    inputAudioFormat.setInteger(MediaFormat.KEY_ENCODER_DELAY, 0);
                }
                if ("audio/mp4a-latm".equals(mimeType)) {
                    convertVideo.mInputAudioDecoder = CodecsHelper.createAudioDecoder(CodecsHelper.getDecoderCodec(mimeType), inputAudioFormat);
                    return true;
                }
    ...
    */
    protected void startConverting() throws IOException {
        ...
        if (prepareVideoCodec() && prepareAudioCodec()) {
            ...
            ByteBuffer[] audioEncoderOutputBuffers3 = this.mCopyAudio ? this.mOutputAudioEncoder.getOutputBuffers() : null;
            ByteBuffer[] audioEncoderInputBuffers = this.mCopyAudio ? this.mOutputAudioEncoder.getInputBuffers() : null;
            ByteBuffer[] audioDecoderOutputBuffers = this.mCopyAudio ? this.mInputAudioDecoder.getOutputBuffers() : null;
            ByteBuffer[] audioDecoderInputBuffers = this.mCopyAudio ? this.mInputAudioDecoder.getInputBuffers() : null;
                ...
                if (j2 != 0) {
                    ...
                    while (true) {
                        ...
                        if (this.mUserStop && !videoExtractorDone) {
                            if (videoEncoderOutputMediaFormat == null || this.mMuxerStarted) {
                                ...
                                if (decoderInputBufferIndex == -1) {
                                    ...
                                    audioEncoderOutputBufferInfo = videoEncoderOutputBufferInfo3;
                                    Log.m94d(Constants.TAG, " video decoder: get input buffer presentationTime :" + presentationTime + ", size : " + size);
                                    audioDecoderOutputBufferInfo = audioDecoderOutputBufferInfo6;
                                    if (presentationTime <= this.mTrimVideoEndUs && size >= 0) {
                                        this.mInputVideoDecoder.queueInputBuffer(decoderInputBufferIndex, 0, size, presentationTime, this.mVideoExtractor.getSampleFlags());
                        ...
                        while (!this.mUserStop && !videoDecoderDone) {
                            ...
                            decoderOutputBufferIndex = this.mInputVideoDecoder.dequeueOutputBuffer(audioDecoderOutputBufferInfo5, 10000L);
                            ...
                            if (decoderOutputBufferIndex == -3) {
                                ...
                                ByteBuffer[] videoDecoderOutputBuffers3 = this.mInputVideoDecoder.getOutputBuffers();
                            ...
                            if (decoderOutputBufferIndex == -2) {
                                Log.m94d(Constants.TAG, "video decoder: codec info format changed" + this.mInputVideoDecoder.getOutputFormat());
                                ...
                                Log.m94d(Constants.TAG, "video decoder: returned buffer for time " + audioDecoderOutputBufferInfo5.presentationTimeUs);
                                ...
                                if (render) {
                                    ...
                                    try {
                                        if (!this.mOutputSurface.checkForNewImage(1000)) {
                                            try {
                                                Log.m94d(Constants.TAG, "video decoder: checkForNewImage return false!!  mUserStop : " + this.mUserStop);
                                            ...
                                            try {
                                                audioDecoderOutputBufferInfo2 = audioDecoderOutputBufferInfo;
                                                try {
                                                    if (audioDecoderOutputBufferInfo5.presentationTimeUs >= this.mTrimVideoStartUs) {
                                                        this.mInputSurface.setPresentationTime(audioDecoderOutputBufferInfo5.presentationTimeUs * 1000);
                                                        Log.m94d(Constants.TAG, "input surface: swap buffers time:" + audioDecoderOutputBufferInfo5.presentationTimeUs);
                        ...
                        audioDecoderOutputBufferInfo2 = audioDecoderOutputBufferInfo;
                        audioEncoderOutputMediaFormat = audioEncoderOutputMediaFormat4;
                        if (!this.mUserStop && !videoEncoderDone && (videoEncoderOutputMediaFormat == null || this.mMuxerStarted)) {
                            encoderOutputBufferIndex = this.mOutputVideoEncoder.dequeueOutputBuffer(videoEncoderOutputBufferInfo2, 10000L);
                            if (encoderOutputBufferIndex != -1) {
                                ...
                                Log.m94d(Constants.TAG, "video encoder: output format changed " + this.mOutputVideoEncoder.getOutputFormat());
                                if (this.mVideoTrackIndex >= 0) {
                                    throw new RuntimeException("Video encoder output format changed after muxer has started");
                                }
                                MediaFormat videoEncoderOutputMediaFormat2 = this.mOutputVideoEncoder.getOutputFormat();
                                videoEncoderOutputMediaFormat = videoEncoderOutputMediaFormat2;
                            } else if (encoderOutputBufferIndex < 0) {
                                Log.m94d(Constants.TAG, "Unexpected result from video encoder dequeue output format.");
                            } else {
                                ...
                                if ((videoEncoderOutputBufferInfo2.flags & 2) != 0) {
                                    ...
                                    if (videoEncoderOutputBufferInfo2.size != 0) {
                                        Log.m94d(Constants.TAG, "video encoder: writing sample data timestamp " + videoEncoderOutputBufferInfo2.presentationTimeUs);
                                        this.mMuxer.writeSampleData(this.mVideoTrackIndex, encoderOutputBuffer, videoEncoderOutputBufferInfo2);
                                    }
                        ...
                        if (this.mCopyAudio) {
                            ...
                            if (!this.mUserStop && !audioExtractorDone && (audioEncoderOutputMediaFormat == null || this.mMuxerStarted)) {
                                ...
                                if (audioDecoderInputBufferIndex == -1) {
                                    ...
                                    Log.m94d(Constants.TAG, "audioDecoderInput  presentationTimeUs :" + presentationTimeUs + ", size:" + size2);
                                    if (presentationTimeUs <= this.mTrimAudioEndUs && size2 >= 0) {
                                        this.mInputAudioDecoder.queueInputBuffer(audioDecoderInputBufferIndex, 0, size2, presentationTimeUs, this.mAudioExtractor.getSampleFlags());
                                        this.mAudioExtractor.advance();
                                    } else {
                            ...
                            if (!this.mUserStop && !audioDecoderDone && pendingAudioDecoderOutputBufferIndex == -1) {
                                if (audioEncoderOutputMediaFormat == null || this.mMuxerStarted) {
                                    audioDecoderOutputBufferInfo4 = audioDecoderOutputBufferInfo2;
                                    int audioDecoderOutputBufferIndex = this.mInputAudioDecoder.dequeueOutputBuffer(audioDecoderOutputBufferInfo4, 10000L);
                                    if (audioDecoderOutputBufferIndex == -1) {
                                        Log.m94d(Constants.TAG, "audio decoder output buffer try again later while decoding");
                                    } else if (audioDecoderOutputBufferIndex == -3) {
                                        Log.m94d(Constants.TAG, "audio decoder: output buffers changed");
                                        ByteBuffer[] audioDecoderOutputBuffers3 = this.mInputAudioDecoder.getOutputBuffers();
                            ...
                            if (this.mUserStop || audioDecoderDone || pendingAudioDecoderOutputBufferIndex == -1) {
                                ...
                                if (audioEncoderInputBufferIndex == -1) {
                                    ...
                                    if (size3 >= 0 && presentationTime2 >= 0) {
                                        ByteBuffer encoderInputBuffer = audioEncoderInputBuffers[audioEncoderInputBufferIndex];
                                        ByteBuffer audioDecoderOutputBuffer = audioDecoderOutputBuffers2[pendingAudioDecoderOutputBufferIndex].duplicate();
                                        ...
                                        this.mOutputAudioEncoder.queueInputBuffer(audioEncoderInputBufferIndex, 0, size3, presentationTime2, audioDecoderOutputBufferInfo4.flags);
                                    }
                                    this.mInputAudioDecoder.releaseOutputBuffer(pendingAudioDecoderOutputBufferIndex, false);
                            ...
                            if (!this.mUserStop && !audioEncoderDone2) {
                                if (audioEncoderOutputMediaFormat == null || this.mMuxerStarted) {
                                    audioEncoderOutputBufferInfo2 = audioEncoderOutputBufferInfo;
                                    int audioEncoderOutputBufferIndex = this.mOutputAudioEncoder.dequeueOutputBuffer(audioEncoderOutputBufferInfo2, 10000L);
                                    if (audioEncoderOutputBufferIndex == -1) {
                                        ...
                                        Log.m94d(Constants.TAG, "audio encoder: output format changed " + audioEncoderOutputMediaFormat);
                                        audioDecoderOutputBufferInfo3 = audioDecoderOutputBufferInfo4;
                                        audioEncoderOutputBuffers = audioEncoderOutputBuffers2;
                                        lastAudioSampleWrittenTime2 = lastAudioSampleWrittenTime3;
                                        audioEncoderDone = audioEncoderDone2;
                                    } else if (audioEncoderOutputBufferIndex < 0) {
                                        Log.m94d(Constants.TAG, "Unexpected result from audio encoder dequeue output format.");
                                        audioDecoderOutputBufferInfo3 = audioDecoderOutputBufferInfo4;
                                        audioEncoderOutputMediaFormat2 = audioEncoderOutputMediaFormat;
                                        lastAudioSampleWrittenTime2 = lastAudioSampleWrittenTime3;
                                    } else {
                                        ByteBuffer audioEncoderOutputBuffer = audioEncoderOutputBuffers2[audioEncoderOutputBufferIndex];
                                        if ((audioEncoderOutputBufferInfo2.flags & 2) != 0) {
                                            ...
                                            if (audioEncoderOutputBufferInfo2.size == 0) {
                                                ...
                                                lastAudioSampleWrittenTime2 = lastAudioSampleWrittenTime3;
                                            } else if (lastAudioSampleWrittenTime3 <= audioEncoderOutputBufferInfo2.presentationTimeUs) {
                                                Log.m94d(Constants.TAG, "audio encoder writing sample data to muxer  time: " + audioEncoderOutputBufferInfo2.presentationTimeUs);
                                                long lastAudioSampleWrittenTime4 = audioEncoderOutputBufferInfo2.presentationTimeUs;
                                                this.mMuxer.writeSampleData(this.mAudioTrackIndex, audioEncoderOutputBuffer, audioEncoderOutputBufferInfo2);
                                                audioEncoderOutputMediaFormat3 = audioEncoderOutputMediaFormat;
                                                ...
                                                audioDecoderOutputBufferInfo3 = audioDecoderOutputBufferInfo4;
                                                audioEncoderOutputMediaFormat3 = audioEncoderOutputMediaFormat;
                                                Log.m94d(Constants.TAG, "lastAudioSampleWrittenTime : " + lastAudioSampleWrittenTime2 + " > presentationTime :" + audioEncoderOutputBufferInfo2.presentationTimeUs);
                                                if (!audioDecoderDone) {
                        ...
                        if (this.mUserStop && !this.mMuxerStarted && videoEncoderOutputMediaFormat != null) {
                            if (!this.mCopyAudio || audioEncoderOutputMediaFormat != null) {
                                String filepath = mUseUri ? getVEEditFilePath(this.mContext, this.mInputUri) : this.mInputFilePath;
    ...
    @Override // com.samsung.android.media.convert.core.Convert
    public boolean stop() {
        Log.m94d(Constants.TAG, "Stop method called  mConverting :" + this.mConverting + ", mUserStop :" + this.mUserStop);
    ...
    public int getOutputFileSize() {
        ...
        try {
            ...
            if (trimEndTime == 0) {
                ...
                Log.m94d(Constants.TAG, "getOutputFileSize  trimEndTime was 0 but updated  trimEndTime : " + trimEndTime);
            }
            this.mSourceFrameRate = 0;
            this.mSourceFrameRate = inputFormat.getInteger(MediaFormat.KEY_FRAME_RATE);
            if (this.mSourceFrameRate == 0 || this.mSourceFrameRate > 250) {
        ...
        try {
            try {
                try {
                    ...
                    if (width <= 0 || height <= 0) {
                        Log.m94d(Constants.TAG, "Resolution Error appear : width = " + width + ", height= " + height);
                        return false;
                    }
                    ...
                    Log.m94d(Constants.TAG, "mOutputWidth :" + this.mOutputWidth + ", mOutputHeight :" + this.mOutputHeight);
                    return true;
                }
    ...
    */
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (tmpAtomPosition >= 0) {
                                    ...
                                    try {
                                        Log.m94d(Constants.TAG, "Found parent: " + atomName + " move to position: " + tmpAtomPosition);
                                        filePointer += 8;
                                    } catch (Exception e3) {
                                        e = e3;
                                        e.printStackTrace();
    ...
    public boolean getHDRMode(MediaMetadataRetriever retriever) {
        try {
            ...
            Log.m94d(Constants.TAG, "getHDRMode  mAuthor : " + this.mAuthor + ", mRecordingMode : " + this.mRecordingMode);
