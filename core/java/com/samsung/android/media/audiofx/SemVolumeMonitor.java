...
public class SemVolumeMonitor extends AudioEffect {
    ...
    public SemVolumeMonitor(int priority, int audioSession) throws IllegalArgumentException {
        ...
        if (audioSession == 0) {
            Log.m102w(TAG, "WARNING: attaching an SemVolumeMonitor to global output mix is deprecated!");
        }
    }
    ...
    public byte[] getOneMinScoreStatus(int energyValueSizeInByte, int scoreValueSizeInByte) {
        ...
        Log.m98i(TAG, "getOneHourRms: call getParameter. bytes:" + valueSizeInByte + "=max(" + energyValueSizeInByte + "," + scoreValueSizeInByte + NavigationBarInflaterView.KEY_CODE_END);
        getParameter(paramBytes, valueBytes);
        Log.m98i(TAG, "getOneHourRms: getParameter done");
        return valueBytes;
    }
    ...
    public void setBluetoothVolume(int volumeIndex) {
        ...
        Log.m98i(TAG, "setBluetoothVolume: call setParameter");
        try {
            setParameter(paramBytes, valueBytes);
        } catch (IllegalStateException e) {
            Log.m97e(TAG, "setBluetoothVolume#setParameter", e);
        }
        Log.m98i(TAG, "setBluetoothVolume: setParameter done");
    }
    ...
    public void onOff(boolean z) {
        ...
        Log.m98i(TAG, "onOff: call setParameter");
        setParameter(integerArrayToByteArray, integerArrayToByteArray2);
        Log.m98i(TAG, "onOff: setParameter done");
    }
