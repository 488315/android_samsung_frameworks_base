...
public class FlipFontOptimizer {
  ...
  public void setFlipfont(Context context) {
    String sOverrideFont =
        SemCscFeature.getInstance().getString("CscFeature_Common_ConfigSpecialEdition");
  ...
  private void setFlipfont(Context context, String fontName) {
    ...
    if (installedFontList == null) {
      Log.m98i(TAG, "changeFont():Installed font list is null");
      return;
    }
    int i = 0;
    while (true) {
    ...
    File fontDir =
        fontWriter.createFontDirectory(
            fontdir.replaceAll(" ", NativeLibraryHelper.CLEAR_ABI_OVERRIDE));
    if (fontDir == null) {
      Log.m96e(TAG, "create fontDir object is null ");
      return;
    }
    ...
    while (it.hasNext()) {
      ...
      try {
        ...
        Log.m98i(TAG, "Cannot copy FontFile from AssetManager, " + e.getMessage());
    ...
    try {
      ...
      Log.m96e(TAG, "Cannot update Configuration : " + e2.getMessage());
    }
    ...
    for (ActivityManager.RunningTaskInfo aTask : allTasks) {
      ...
      if (s.compareTo("com.android.settings") != 0
          && s.compareTo("com.sec.android.app.SecSetupWizard") != 0
          && s.compareTo("com.samsung.music") != 0
          && s.compareTo("com.sec.android.app.music") != 0) {
        if (s.compareTo("com.infraware.polarisoffice") == 0
            || s.compareTo("com.infraware.polarisoffice4") == 0
            || s.compareTo("com.infraware.polarisviewer4") == 0
            || s.compareTo("com.infraware.PolarisOfficeStdForTablet") == 0
            || s.compareTo("com.infraware.polarisviewer5tablet") == 0
            || s.compareTo("com.infraware.polarisoffice5tablet") == 0
            || s.compareTo("com.infraware.polarisoffice4.document") == 0
            || s.compareTo("com.infraware.polarisoffice5") == 0
            || s.compareTo("com.infraware.polarisoffice5.document") == 0
            || s.compareTo("com.infraware.polarisviewer5") == 0
            || s.compareTo("com.infraware.polarisviewer5.document") == 0) {
          activityManager.forceStopPackage(s);
        } else if (s.compareTo("com.sec.android.app.camera") == 0) {
          Log.m94d(TAG, "com.sec.android.app.camera == 0");
        } else {
  ...
  private ArrayList getFontsVector() {
    ...
    try {
      ...
      this.mTypefaceFinder.getSansEntries(
          this.mContext, packageManager, fontNames, typefaceFiles, fontPackageNames);
      return fontNames;
    } catch (Exception e) {
      Log.m96e(TAG, "Cannot make a Installed Font List : " + e.getMessage());
      return null;
    } finally {
      Binder.restoreCallingIdentity(token);
    }
