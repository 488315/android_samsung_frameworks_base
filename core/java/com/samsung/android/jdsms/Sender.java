...
public final class Sender {
    ...
    /* renamed from: EV */
    private static final String f3012EV = "ev";
    ...
    public Sender(Context context) {
        DsmsLog.m279d(SUBTAG, "Created. context=" + context);
        if (context != null) {
            DsmsLog.m279d(SUBTAG, "context.packageName=[" + context.getPackageName() + NavigationBarInflaterView.SIZE_MOD_END);
            this.mContext = context;
            if ("factory".equals(SystemProperties.get("ro.factory.factory_binary"))) {
                DsmsLog.m279d(SUBTAG, "DSMS disabled");
    ...
    private int sendToDsms(final DsmsMessage message) {
        if (this.DSMS_DISABLED) {
            DsmsLog.m281e(SUBTAG, "DSMS not supported");
            return -19;
        }
        DsmsLog.m279d(SUBTAG, "Sending message " + message);
        if (!PolicyEnforcer.isAValidCaller()) {
            DsmsLog.m281e(SUBTAG, "Unauthorized caller");
            return -1;
        }
    ...
    public void sendMessage(DsmsMessage message) {
        ...
        DsmsLog.m279d(SUBTAG, "Message sent");
    }
}
