...
public class MultiWindowUtils {
    ...
    public static int getRoundedCornerRadius(Context context) {
        int multiWindowRadius = context.getResources().getDimensionPixelSize(C4337R.dimen.rounded_corner_radius_for_multiwindow);
    ...
    public static boolean isSingleInstancePerTask(Context ctx, String pkg) {
        ...
        try {
            ActivityInfo ai = ctx.getPackageManager().getActivityInfo(cn, PackageManager.ComponentInfoFlags.m10of(128L));
    ...
    public static void adjustBoundsForScreenRatio(Rect prevScreenBounds, Rect nextScreenBounds, Rect sourceBounds, Rect outBounds) {
        ...
        if (prevScreenWidth == nextScreenWidth && prevScreenHeight == nextScreenHeight) {
            Slog.m113d("RotationUtils", "adjustBoundsForScreenRatio: Since the screen ratio has not changed, there is no need to calculate new bounds.");
            return;
        }
