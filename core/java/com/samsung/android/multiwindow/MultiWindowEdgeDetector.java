...
public class MultiWindowEdgeDetector {
    ...
    private void loadResources() {
        ...
        this.mCornerRadius = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.multiwindow_freeform_gesture_guide_corner_radius);
        this.mGestureThreshold = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.multiwindow_freeform_gesture_threshold);
        if (SAFE_DEBUG) {
    ...
    private void updateScreenInfo() {
        ...
        if (display == null) {
            Slog.m121w(TAG, this.mPrefixLog + "display is null, mContext=" + this.mContext);
            return;
        }
        ...
        if (needUpdate) {
            ...
            if (SAFE_DEBUG) {
                Slog.m117i(TAG, this.mPrefixLog + "updateScreenInfo: mScreenWidth=" + this.mScreenWidth + ", mScreenHeight=" + this.mScreenHeight + ", mScreenOrientation=" + this.mScreenOrientation + ", sWidth=" + sWidth);
            }
        }
    }
    ...
    private void ensureScreenInfo() {
        ...
        if (this.mScreenOrientation != currentOrientation) {
            Slog.m121w(TAG, this.mPrefixLog + "ensureScreenInfo: ScreenInfo is wrong, mScreenOr=" + this.mScreenOrientation + ", currentOr=" + currentOrientation);
            updateScreenInfo();
        }
    }
    ...
    private boolean isTopTaskHomeOrRecents() {
        ...
        Log.m98i(TAG, this.mPrefixLog + "isTopTaskHomeOrRecents");
        return true;
    }
    ...
    private int getEdgeFlags(MotionEvent ev) {
        ...
        Log.m98i(TAG, this.mPrefixLog + "checkEdgeFlags: " + Utils.edgeFlagToString(flags) + ", [" + x + "," + y + "], w=" + sWidth + ", h=" + sHeight + ", screenWidth=" + this.mScreenWidth);
        return flags;
    }
    ...
    private boolean isExceedThreshold(int dx, int dy) {
        ...
        Log.m98i(TAG, this.mPrefixLog + "isExceedThreshold: " + result + ", dx=" + Math.abs(dx) + ", dy=" + Math.abs(dy) + ", distance=" + distance + ", threshold=" + this.mGestureThreshold);
        return result;
    }
    ...
    private boolean isEffectiveAngle(int dx, int dy) {
        ...
        Log.m98i(TAG, this.mPrefixLog + "isEffectiveAngle: " + result + ", degrees=" + degrees);
        return result;
    }
    ...
    private void updateFromSystemProperties() {
        if (SAFE_DEBUG) {
            ...
            if (changed) {
                Log.m98i(TAG, this.mPrefixLog + "updateFromSystemProperties: sWidth=" + sWidth + ", sHeight=" + sHeight + ", mCornerRadius=" + this.mCornerRadius + ", mMaxDegrees=" + this.mMaxDegrees);
            }
        }
    }
        ...
        switch (event.getAction()) {
            ...
            case 2:
                ...
                if (isValidGesture(x - this.mStartX, y - this.mStartY)) {
                    Slog.m113d(TAG, this.mPrefixLog + "Gesture valid");
                    this.mIsGestureDetected = true;
                    return true;
                }
                Slog.m113d(TAG, this.mPrefixLog + "Gesture invalid, reset");
                reset();
                return false;
            case 3:
                break;
