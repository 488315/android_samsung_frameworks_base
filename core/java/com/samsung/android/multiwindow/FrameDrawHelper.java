...
public class FrameDrawHelper {
  ...
  public void updateResources(Configuration activityConfig) {
    ...
    this.mThickness = f;
    this.mStrokeRadiusInPopOver =
        res.getDimensionPixelSize(C4337R.dimen.sem_decor_outline_radius_popover);
  ...
  public void drawFrame(Canvas canvas) {
    ...
    if (isPopOver) {
      canvas.drawPath(
          SemViewUtils.getSmoothRoundedRect(width, height, left, top, (int) radius),
          this.mPaintRoot);
    }
    canvas.drawPath(
        SemViewUtils.getRoundedCorner(1, 0, 0, (int) this.mStrokeRadius), this.mPaintContent);
    float width2 = this.mView.getWidth();
    float f = this.mStrokeRadius;
    canvas.drawPath(
        SemViewUtils.getRoundedCorner(2, (int) (width2 - f), 0, (int) f), this.mPaintContent);
    float height2 = this.mView.getHeight();
    float f2 = this.mStrokeRadius;
    canvas.drawPath(
        SemViewUtils.getRoundedCorner(4, 0, (int) (height2 - f2), (int) f2), this.mPaintContent);
    int width3 = (int) (this.mView.getWidth() - this.mStrokeRadius);
    float height3 = this.mView.getHeight();
    float f3 = this.mStrokeRadius;
    canvas.drawPath(
        SemViewUtils.getRoundedCorner(8, width3, (int) (height3 - f3), (int) f3),
        this.mPaintContent);
  }
}
