...
public class FreeformResizeGuide {
  ...
  public FreeformResizeGuide(Context context, int dexDockingState, String packageName) {
    ...
    Context systemUiContext =
        context != null ? context : ActivityThread.currentActivityThread().getSystemUiContext();
    ...
    FreeformResizeGuideView freeformResizeGuideView =
        (FreeformResizeGuideView)
            LayoutInflater.from(systemUiContext)
                .inflate(C4337R.layout.freeform_resize_guide, (ViewGroup) null);
  ...
  public void show(Rect bounds) {
    ...
    if (transitionInfo != null) {
      this.mView.show(
          this.mLastBounds, this.mBounds, true, this.mNeedToFullscreenTransition, transitionInfo);
    ...
    if (isDexTaskDocked(dexTaskDockingState)
        && (dockingMaxWidth =
                MultiWindowManager.getInstance()
                    .calculateMaxWidth(dexTaskDockingState, displaySize.width(), this.mMinWidth))
            != -1) {
  ...
  public void updateResizeGestureInfo(Rect displayFrame, Rect stableBounds) {
    ...
    this.mFreeformGuideViewFullscreenMargin =
        this.mContext
            .getResources()
            .getDimensionPixelSize(C4337R.dimen.freeform_resize_guide_view_fullscreen_margin);
    this.mMinimizeFreeformPadding =
        this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.minimize_freeform_padding);
  }
