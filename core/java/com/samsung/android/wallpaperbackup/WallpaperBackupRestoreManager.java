...
public class WallpaperBackupRestoreManager {
  ...
  public void startBackupWallpaper(
      Context context,
      int which,
      String basePath,
      String source,
  ...
  public void startBackupWallpaper(
    ...
    Slog.m113d(
        str,
        "startBackupWallpaper which = "
            + Integer.toHexString(which)
            + " action= "
            + action
            + " basePath="
            + basePath2
    ...
    if (!errorCode.equals(BnRFileHelper.ErrorCode.ERROR_NONE)) {
      ...
      Slog.m113d(str, "startBackupWallpaper is return because precondition fail");
      sendResponse(
          context,
          which,
          action2,
          ...
          errorCode,
          BnRFileHelper.REQ_MINIMUM_SIZE,
          source,
          sessionTime,
          extraResults,
          null);
    ...
    while (true) {
      ...
      if (i < iArr.length) {
        if (isSupportedScreen(iArr[i], which)) {
          if (BnRConstants.BNR_SOURCE_SCLOUD.equals(source)
              && wallpaperManager.isSystemAndLockPaired(this.mModeFlagSet[i])
              && which == 2
              && wallpaperManager.semGetWallpaperType(which) == 7) {
            Log.m98i(
                TAG,
                "pushBackupFile() : home and lock layered wallpaper is paired for mode "
                    + this.mModeFlagSet[i]
                    + ", backup home wallpaper");
            int homeWhich = this.mModeFlagSet[i] | 1;
            WallpaperBNRHelper helper =
                new WallpaperBNRHelper(
                    context,
                    wallpaperManager,
                    action,
                    homeWhich,
  ...
  public void startRestoreWallpaper(
      Context context,
      int which,
      String basePath,
      String source,
      int securityLevel,
      String saveKey,
      String restoreScreen) {
    startRestoreWallpaper(
        context, "", which, basePath, source, securityLevel, saveKey, restoreScreen);
  }
  ...
  public void startRestoreWallpaper(
    ...
    if (!errorCode.equals(BnRFileHelper.ErrorCode.ERROR_NONE)) {
      ...
      HashMap<Integer, ResultCode> extraResults = new HashMap<>();
      extraResults.put(Integer.valueOf(which), err);
      Slog.m113d(str, "startRestoreWallpaper is return because precondition fail");
      sendResponse(
          context,
          which,
          action2,
    ...
    pushRestoreFile(
        context, action2, which, basePath2, securityLevel, saveKey, source, restoreScreen);
  }

  private void pushRestoreFile(
      Context context,
      String action,
      int which,
      String basePath,
    ...
    int i2 = helpers.size();
    if (i2 == 0) {
      Log.m94d(TAG, "pushRestoreFile: Nothing to restore.");
      WallpaperBNRHelper helper2 =
          new WallpaperBNRHelper(
              context,
              wallpaperManager,
              action,
  ...
  private boolean handleDifferentTypeRestore(
      WallpaperBNRHelper helper, int mode, int which, String restoreScreen) {
    if (Rune.isFolder() && !"folder".equals(helper.getDeviceType())) {
      if (mode == 16) {
        Log.m98i(TAG, "skip restoring sub display of phone models");
        return false;
      }
      if (mode == 4) {
        helper.setWhich(which | 16);
        Log.m98i(TAG, "restoring main display of phone model to sub display of fold model");
        return true;
      }
    }
    if (!Rune.isFolder() && !Rune.isTablet()) {
      if ("folder".equals(helper.getDeviceType())) {
        if (mode == 16) {
          ...
          Log.m98i(TAG, "restoring sub display of fold model to phone");
          return true;
        }
        if (mode == 4) {
          Log.m98i(TAG, "do not restore main display of fold model to phone");
          return false;
        }
      } else if (Rune.SUPPORT_COVER_DISPLAY_WATCHFACE) {
        if (mode == 16
            && (Rune.SUPPORT_LARGE_FRONT_SUB_DISPLAY
                ^ BnRConstants.COVER_TYPE_LARGE_SCREEN.equals(helper.getCoverType()))) {
          Log.m98i(TAG, "skip different size cover screen");
          return false;
        }
      } else if (mode == 16) {
        Log.m98i(TAG, "skip sub display for phones");
        return false;
      }
    }
    return true;
  ...
  */
  private boolean isSupportedScreen(int mode, int type) {
    switch (mode) {
      case 8:
      default:
        return true;
      case 16:
        if (Rune.BNR_SUPPORT_BETWEEN_FOLD_AND_PHONE && !Rune.isTablet()) {
          return true;
        }
        if (!Rune.SUPPORT_SUB_DISPLAY_MODE) {
          return false;
        }
        if (Rune.SUPPORT_COVER_DISPLAY_WATCHFACE && (type & 1) == 0) {
          return false;
        }
        break;
      case 32:
        if (!Rune.VIRTUAL_DISPLAY_WALLPAPER || (type & 1) == 0) {
          return false;
        }
        break;
    }
  ...
  private boolean isRestorableDeviceType(ArrayList<WallpaperBNRHelper> helpers) {
    ...
    switch (c) {
      ...
      case 2:
        ...
        break;
      default:
        Log.m94d(TAG, "isRestorableDeviceType: deviceTypeBackup = " + deviceTypeBackup);
        break;
    }
    Slog.m113d(TAG, "isRestorableDeviceType: Device type not matching");
  ...
  public static void response(ArrayList<WallpaperBNRHelper> helpers) {
    ...
    sendResponse(
        helper2.getContext(),
        helper2.getType(),
        helper2.getResponseAction(),
        successCount == 0 ? ResultCode.RESULT_FAIL : ResultCode.RESULT_SUCCESS,
        errorCode,
        BnRFileHelper.REQ_MINIMUM_SIZE,
        helper2.getSource(),
        helper2.getSessionTime(),
        extraResults,
        packageList);
  }
  ...
  private static void sendResponse(
      ...
      BnRFileHelper.ErrorCode errorCode,
      int requiredSize,
      String source,
      String sessionTime,
      HashMap extraResultCodes,
    ...
    String str = TAG;
    Slog.m113d(str, logBuffer.toString());
    ...
    Slog.m113d(str, "sendBroadcast. " + which);
  }
  ...
  static class WallpaperBNRHelper {
    ...
    WallpaperBNRHelper(
      ...
      if (!TextUtils.isEmpty(action) && action.contains("BACKUP")) {
        ...
        Log.m94d(TAG, "WallpaperBNRHelper: Set backup case true.");
      }
      this.mWhich = which;
      this.mAction = action;
    ...
    private String extractPackageName(Uri uri) {
      if (uri != null) {
        try {
          ...
          Log.m96e(TAG, "extractPackageName: uri = " + uri + ", error = " + e.getMessage());
          return null;
        }
      }
      return null;
    ...
    private void createBackupInfo() {
      ...
      if (!this.mWallpaperManager.isWallpaperDataExists(this.mWhich)) {
        Log.m94d(TAG, "createBackupInfo: WallpaperData for [" + this.mWhich + "] does not exist.");
      ...
      if ((Rune.VIRTUAL_DISPLAY_WALLPAPER && this.mMode == 32)
          || (Rune.SUPPORT_COVER_DISPLAY_WATCHFACE
              && this.mMode == 16
              && this.mWallpaperType == 8)) {
        this.mIsCustomWallpaper = true;
      } else {
        this.mIsCustomWallpaper =
            Settings.System.getIntForUser(
                    this.mContext.getContentResolver(), this.mSettingsName, 1, -2)
                == 0;
      }
      this.mIsBackupAllowed = this.mWallpaperManager.isWallpaperBackupAllowed(this.mWhich);
      this.mIsDownloadedThemeWallpaper =
          Settings.System.getIntForUser(
                  this.mContext.getContentResolver(), this.mSettingsName, 1, -2)
              == 3;
      if (Rune.isFolder()) {
      ...
      if (!canBackup()) {
        Log.m94d(TAG, "createBackupInfo: Not allowed to backup wallpaper");
      }
        ...
        if (gyroSensor != null) {
          this.mTiltValue =
              Settings.System.getInt(
                  ...
                  BnRConstants.SETTINGS_WALLPAPER_TILT_STATUS,
                  0);
        }
      }
      Bundle extrasBundle =
          this.mWallpaperManager.getWallpaperExtras(this.mWhich, this.mContext.getUserId());
      ...
      switch (this.mWallpaperType) {
        case 0:
          ...
          if (getMode() == 4 && !TextUtils.isEmpty(this.mDeviceType)) {
            if (this.mDeviceType.equals("folder")
                || this.mDeviceType.equals(BnRConstants.DEVICETYPE_TABLET)) {
              this.mOrientation =
                  this.mWallpaperManager.getWallpaperOrientation(
                      this.mWhich, this.mContext.getUserId());
              break;
            }
          }
          break;
        ...
        default:
          Log.m96e(
              TAG,
              "createBackupInfo: Unhandled wallpaper type, mWallpaperType = "
                  + this.mWallpaperType);
          break;
        case 3:
          ...
          if (customPackUri != null) {
            ...
            this.mSourceFilePath =
                BnRConstants.CUSTOM_MULTIPACK_SOURCE_PATH
                    + customPackUri.getHost()
                    + customPackUri.getPath();
            break;
          }
          break;
        case 5:
        ...
        case 7:
          this.mIsHomeAndLockPaired =
              this.mWallpaperManager.isSystemAndLockPaired(WhichChecker.getMode(this.mWhich));
          ComponentName cn =
              this.mWallpaperManager.semGetWallpaperComponent(
                  this.mWhich, this.mContext.getUserId());
          if (cn != null) {
        ...
        case 8:
          ...
          if (Rune.SUPPORT_COVER_DISPLAY_WATCHFACE && this.mMode == 16) {
            this.mDescriptor =
                this.mWallpaperManager.getWallpaperFile(this.mWhich, this.mContext.getUserId(), -1);
            this.mCropHint = this.mWallpaperManager.semGetWallpaperCropHint(this.mWhich);
            break;
          }
          break;
    ...
    public void convertToImageWallpaperForSCloud() {
      addErrorDescription(
          "convertImageWallpaperForSCloud: Backup with Samsung cloud, change layered type to"
              + " image");
      ...
      try {
        String fileName =
            this.mWallpaperManager
                .getWallpaperExtras(whichForAssets, this.mContext.getUserId())
                .getString(BnRConstants.KEY_REPRESENTATIVE_IMAGE_FILE);
        if (TextUtils.isEmpty(fileName)) {
          Log.m98i(
              TAG,
              "representative file name is empty, try thumbnail file name and remove crop hints");
          this.mExternalParams = null;
          fileName =
              this.mWallpaperManager
                  .getWallpaperExtras(whichForAssets, this.mContext.getUserId())
                  .getString(BnRConstants.KEY_THUNBNAIL_FILENAME);
        }
        this.mDescriptor =
            this.mWallpaperManager.getWallpaperAssetFile(
                whichForAssets, this.mContext.getUserId(), fileName);
      } catch (Exception e) {
        addErrorDescription(
            "Error occured getting representative file : "
                + e.toString()
                + ", try hard-coding way..");
        this.mDescriptor =
            this.mWallpaperManager.getWallpaperAssetFile(
                whichForAssets, this.mContext.getUserId(), "thumbnail.jpg");
      }
    }
    ...
    private void createRestoreInfo() {
      ...
      this.mCropHint =
          new Rect(
              wallpaperUser.getLeftValue(),
              wallpaperUser.getTopValue(),
              wallpaperUser.getRightValue(),
              wallpaperUser.getBottomValue());
      ...
      if (Rune.isFolder() && TextUtils.isEmpty(wallpaperUser.getDeviceType()) && getMode() == 4) {
        if (this.mCropHint.isEmpty()) {
          ...
          if (ratio > 0.74f) {
            addErrorDescription(
                "createRestoreInfo: Consider backup device is table. ratio = " + ratio);
            this.mDeviceType = "folder";
      ...
      if (min2 == 0
          && getMode() == 4
          && !TextUtils.isEmpty(this.mDeviceType)
          && (this.mDeviceType.equals("folder")
              || this.mDeviceType.equals(BnRConstants.DEVICETYPE_TABLET))) {
        ...
        case 8:
          ...
          if (TextUtils.isEmpty(fileName)) {
            fileName = createVideoFileName();
              ...
              && getMode() == 4
              && getType() == 2
              && fileName.endsWith("_6.mp4")) {
            fileName = fileName.replace("_6.mp4", "_2.mp4");
          }
          this.mTargetFilePath =
              BnRConstants.WALLPAPER_VIDEO_RESTORE_PATH + File.separator + fileName;
          break;
      }
    }
    ...
    public boolean canBackup() {
      ...
      if (Build.VERSION.SEM_PLATFORM_INT > 150000 && this.mWallpaperType == 7) {
        try {
          if (isHomeAndLockPaired()) {
            assetFiles =
                this.mWallpaperManager.getWallpaperAssets(
                    this.mMode | 1, this.mContext.getUserId());
          } else {
            assetFiles =
                this.mWallpaperManager.getWallpaperAssets(this.mWhich, this.mContext.getUserId());
          }
      ...
      if (this.mWallpaperType == 3 && !canBackup && (uri = this.mUri) != null) {
        ...
        if (!TextUtils.isEmpty(stringUri)
            && stringUri.startsWith(BnRConstants.CUSTOM_PACK_PREFIX)) {
          canBackup = true;
        }
      }
      Slog.m113d(TAG, "canBackup: which = " + this.mWhich + " canBackup = " + canBackup);
      return canBackup;
    }
    ...
    public String getOriginalXmlFilePath() {
      switch (this.mMode) {
        ...
        case 8:
          return this.mType == 2
              ? BnRConstants.ORIGINAL_LOCK_XML_NAME
              : BnRConstants.ORIGINAL_XML_NAME;
        case 16:
          return this.mType == 2
              ? BnRConstants.SUB_DISPLAY_LOCK_XML_ORIGINAL_NAME
              : BnRConstants.SUB_DISPLAY_XML_ORIGINAL_NAME;
        case 32:
    ...
    public String getXmlFilePath() {
      switch (this.mMode) {
        case 4:
          return this.mType == 2
              ? BnRConstants.DEFAULT_LOCK_XML_NAME
              : BnRConstants.DEFAULT_XML_NAME;
        case 8:
          String path =
              this.mType == 2 ? BnRConstants.DEX_LOCK_XML_NAME : BnRConstants.DEX_XML_NAME;
        ...
        case 16:
          return this.mType == 2
              ? BnRConstants.SUB_DISPLAY_LOCK_XML_NAME
              : BnRConstants.SUB_DISPLAY_XML_NAME;
        case 32:
          return this.mType == 2 ? "" : BnRConstants.VIRTUAL_DISPLAY_XML_NAME;
        default:
          return null;
    ...
    private String getFilePath(WallpaperUser wallpaperUser) {
      ...
      switch (this.mWallpaperType) {
        ...
        case 6:
        default:
          Log.m96e(TAG, "getFilePath: mWallpaperType = " + this.mWallpaperType);
          return null;
        case 3:
    ...
    private String getGifPath() {
      if (Rune.SUPPORT_COVER_DISPLAY_WATCHFACE) {
        if (this.mMode == 16 && this.mType == 1) {
          return BnRConstants.SUB_DISPLAY_FOLDER_NAME
              + File.separator
              + BnRConstants.CUSTOM_GIF_FILE_NAME;
        }
        Log.m94d(TAG, "getGifPath: Invalid which for gif wallpaper. mWhich = " + this.mWhich);
        return null;
      }
      if (Rune.VIRTUAL_DISPLAY_WALLPAPER) {
        if (this.mMode == 32 && this.mType == 1) {
          return BnRConstants.VIRTUAL_DISPLAY_FOLDER_NAME
              + File.separator
              + BnRConstants.CUSTOM_GIF_FILE_NAME;
        }
        Log.m94d(TAG, "getGifPath: Invalid which for gif wallpaper. mWhich = " + this.mWhich);
      ...
      Log.m94d(TAG, "getGifPath: NOT SUPPORTED YET!");
      return null;
    }
    ...
    private String getImagePath() {
      String imageFileName =
          this.mType == 2
              ? BnRConstants.LOCK_WALLPAPER_FILE_NAME
              : BnRConstants.WALLPAPER_IMAGE_FILE_NAME;
      switch (this.mMode) {
        case 4:
    ...
    private String getMultipackPath() {
      switch (this.mMode) {
        ...
        case 16:
          return BnRConstants.SUB_DISPLAY_FOLDER_NAME
              + File.separator
              + BnRConstants.CUSTOM_MULTIPACK_PATH;
        case 32:
          return BnRConstants.VIRTUAL_DISPLAY_FOLDER_NAME
              + File.separator
              + BnRConstants.CUSTOM_MULTIPACK_PATH;
    ...
    private String getLiveWallpaperPath() {
      switch (this.mMode) {
        ...
        case 8:
          return BnRConstants.DEX_FOLDER_NAME
              + File.separator
              + BnRConstants.LIVE_WALLPAPER_ASSETS_PATH;
        case 16:
          return BnRConstants.SUB_DISPLAY_FOLDER_NAME
              + File.separator
              + BnRConstants.LIVE_WALLPAPER_ASSETS_PATH;
    ...
    private String createVideoFileName() {
      ...
      if (this.mContext != null) {
        return "video_wallpaper_"
            + this.mContext.getUserId()
            + Session.SESSION_SEPARATION_CHAR_CHILD
            + which
            + MediaMetrics.SEPARATOR
            + BnRConstants.VIDEO_FILE_EXTENSION;
      }
      Log.m94d(TAG, "createVideoFileName: context is null!");
      return "video_wallpaper_0_"
          + which
          + MediaMetrics.SEPARATOR
          + BnRConstants.VIDEO_FILE_EXTENSION;
    }
    ...
    private XmlParser getXmlParser() {
      try {
        ...
        Log.m96e(TAG, "getXmlParser: " + e.getMessage());
    ...
    public String toString() {
      ...
      buffer
          ...
          .append("\n\t\tmWallpaperType = " + this.mWallpaperType)
          .append("\n\t\tmIsBackupCase = " + this.mIsBackupCase);
      if (this.mIsBackupCase) {
        buffer
            .append("\n\t\tmIsCustomWallpaper = " + this.mIsCustomWallpaper)
      ...
      buffer
          ...
          .append("\n\t\tmSecurityLevel = " + this.mSecurityLevel)
          .append("\n\t\tmSaveKey = " + this.mSaveKey)
          .append("\n\t\tmSessionTime = " + this.mSessionTime)
          .append("\n\t\tmTiltValue = " + this.mTiltValue)
          .append("\n\t\tmCropHint = " + this.mCropHint)
          .append("\n\t\tmRotation = " + this.mRotation)
          .append("\n\t\tmAction = " + this.mAction)
          .append("\n\t\tmBasePath = " + this.mBasePath)
          .append("\n\t\tmSource = " + this.mSource)
          .append("\n\t\tmDescriptor = " + this.mDescriptor)
          .append("\n\t\tmSourceFilePath = " + this.mSourceFilePath)
          .append("\n\t\tmTargetFilePath = " + this.mTargetFilePath)
          .append("\n\t\tmFilePath = " + this.mFilePath)
          .append("\n\t\tmXmlPath = " + this.mXmlPath)
          .append("\n\t\tmSettingsName = " + this.mSettingsName)
          .append("\n\t\tmExternalParams = " + this.mExternalParams)
  ...
  static class WallpaperBackupAsyncTask
    ...
    public void onProgressUpdate(WallpaperBNRHelper... helpers) {
      try {
        ...
        Slog.m113d(WallpaperBackupRestoreManager.TAG, "onProgressUpdate:" + helper.toString());
        helper.close();
      } catch (Exception e) {
        e.printStackTrace();
      }
    ...
    private boolean backupWallpaper(WallpaperBNRHelper helper) {
      switch (helper.getWallpaperType()) {
        ...
        case 0:
          ...
          return BnRFileHelper.copyFile(
              helper.getTargetFilePath(), helper.getDescriptor(), helper.getFileSaveKey());
        case 1:
        case 2:
        case 4:
        case 6:
        default:
          return false;
        case 3:
          return BnRFileHelper.copyDir(
              helper.getTargetFilePath(), helper.getSourceFilePath(), helper.getFileSaveKey());
        case 5:
          return BnRFileHelper.copyFile(
              helper.getTargetFilePath(), helper.getSourceFilePath(), helper.getFileSaveKey());
        case 7:
          ...
          return BnRFileHelper.copyAssets(
              helper.getTargetFilePath(),
              helper
                  .getWallpaperManager()
                  .getWallpaperAssets(helper.getWhich(), helper.getContext().getUserId()),
              helper.getFileSaveKey());
        case 8:
          if (Rune.SUPPORT_COVER_DISPLAY_WATCHFACE && helper.getMode() == 16) {
            BnRFileHelper.copyFile(
                helper.getBasePath()
                    + BnRConstants.SUB_DISPLAY_FOLDER_NAME
                    + File.separator
                    + BnRConstants.VIDEO_DIR_PATH
                    + BnRConstants.VIDEO_THUMBNAIL_FILE_NAME,
    ...
    private void backupXml(WallpaperBNRHelper helper) {
      if (helper == null) {
        Log.m94d(WallpaperBackupRestoreManager.TAG, "backupXml: Cannot create backup xml file.");
        return;
      }
      ...
      if (Rune.SUPPORT_COVER_DISPLAY_WATCHFACE && helper.getMode() == 16 && wallpaperType == 8) {
        wallpaperUser.setPath(
            BnRConstants.SUB_DISPLAY_FOLDER_NAME + File.separator + BnRConstants.VIDEO_DIR_PATH);
      } else {
      ...
      if (wallpaperType == 0 && helper.getMode() == 4) {
        ...
        if (!TextUtils.isEmpty(deviceType)
            && (deviceType.equals("folder") || deviceType.equals(BnRConstants.DEVICETYPE_TABLET))) {
  ...
  static class WallpaperRestoreAsyncTask
      extends AsyncTask<ArrayList, WallpaperBNRHelper, ArrayList<WallpaperBNRHelper>> {
      ...
      for (int i = 0; i < arrayList.size(); i++) {
        ...
        if (retoreSuccess) {
          ...
          if (Build.VERSION.SEM_PLATFORM_INT >= 140100) {
            WallpaperManager.getInstance(helper.getContext())
                .semClearBackupWallpapers(helper.getWhich());
          }
        } else {
    ...
    private void writeSettingValue(WallpaperBNRHelper helper) {
        ...
        switch (helper.getMode()) {
          case 4:
            Settings.System.putIntForUser(
                helper.getContext().getContentResolver(),
                WallpaperManager.SETTINGS_LOCKSCREEN_WALLPAPER,
                1,
                -2);
            break;
          case 16:
            Settings.System.putIntForUser(
                helper.getContext().getContentResolver(),
                WallpaperManager.SETTINGS_LOCKSCREEN_WALLPAPER_SUB,
                1,
                -2);
            break;
        }
      ...
      if (helper.getMode() != 8
          && (sensorManager =
                  (SensorManager) helper.getContext().getSystemService(Context.SENSOR_SERVICE))
              != null) {
        Sensor gyroSensor = sensorManager.getDefaultSensor(65579);
        if (gyroSensor != null) {
    ...
    private boolean restoreWallpaper(WallpaperBNRHelper helper) {
      ...
      switch (helper.getWallpaperType()) {
        ...
        case 0:
          if (Build.VERSION.SEM_PLATFORM_INT >= 140100
              && (WhichChecker.isWatchFaceDisplay(helper.getWhich())
                  || WhichChecker.isVirtualDisplay(helper.getWhich()))) {
            break;
        ...
        case 6:
        default:
          helper.addErrorDescription(
              "restoreWallpaper: Unhandled wallpaper type [" + helper.getWallpaperType() + "].");
          break;
        case 3:
          if (helper.getMode() != 8) {
            break;
    ...
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    private boolean setBitmap(WallpaperBNRHelper helper) {
      ...
      Bitmap bitmap =
          BnRFileHelper.getBitmapFromPath(
              helper.getSourceFilePath(), helper.getSecurityLevel(), helper.getFileSaveKey());
      if (bitmap == null) {
        return false;
      }
      try {
        Matrix matrix = new Matrix();
        matrix.postRotate(helper.getRotationValue());
        rotatedBitmap =
            Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, true);
      } catch (Exception e) {
        e.printStackTrace();
        helper.addErrorDescription("setBitmap: Exception <" + e.getMessage() + ">");
      }
      if (rotatedBitmap == null) {
        helper.addErrorDescription("setBitmap: rotatedBitmap is null.");
        return false;
      }
      ...
      Bundle extrasBundle2 = null;
      String extrasStr = helper.getExternalParams();
      if (!TextUtils.isEmpty(extrasStr)) {
        extrasBundle2 = WallpaperExtraBundleHelper.fromJson(extrasStr);
      }
      if (helper.getMode() == 4) {
        String deviceType = helper.getDeviceType();
        if (!TextUtils.isEmpty(deviceType)
            && (deviceType.equals("folder") || deviceType.equals(BnRConstants.DEVICETYPE_TABLET))) {
          if (extrasBundle2 == null) {
            extrasBundle2 = new Bundle();
          }
          extrasBundle2.putInt("orientation", helper.getOrientation());
          extrasBundle = extrasBundle2;
          result =
              WallpaperManager.getInstance(helper.getContext())
                  .setBitmap(rotatedBitmap, visibleRect, true, helper.getWhich(), extrasBundle);
          if (WallpaperBackupRestoreManager.DEBUG) {
            Log.m94d(
                WallpaperBackupRestoreManager.TAG,
                "setBitmap: Elapsed Time\t\t ["
                    + (SystemClock.elapsedRealtime() - startTime)
                    + NavigationBarInflaterView.SIZE_MOD_END);
          }
          if (result > 0) {
            if (helper.isDownloadedThemeWallpaper() && (uri = helper.getUri()) != null) {
              WallpaperManager.getInstance(helper.getContext())
                  .semSetUri(uri, true, helper.getWhich());
            }
            return true;
          }
          return false;
    ...
    private boolean setStream(WallpaperBNRHelper helper) {
      ...
      InputStream inputStream =
          BnRFileHelper.getInputStreamFromPath(
              helper.getSourceFilePath(), helper.getSecurityLevel(), helper.getFileSaveKey());
      try {
        if (inputStream == null) {
          helper.addErrorDescription(
              "setStream: inputStream is null. path = <" + helper.getSourceFilePath() + ">");
          return false;
        }
        Rect visibleRect = helper.getCropHint();
        ...
        int result =
            WallpaperManager.getInstance(helper.getContext())
                .setStream(
                    inputStream, visibleRect, true, helper.getWhich(), 0, false, extrasBundle);
        if (WallpaperBackupRestoreManager.DEBUG) {
          Log.m94d(
              WallpaperBackupRestoreManager.TAG,
              "setStream: Elapsed Time\t\t ["
                  + (SystemClock.elapsedRealtime() - startTime)
                  + NavigationBarInflaterView.SIZE_MOD_END);
        }
        if (result > 0) {
    ...
    private boolean setVideoWallpaper(WallpaperBNRHelper helper) {
      if (helper.isDownloadedThemeWallpaper()) {
        ...
        WallpaperManager.getInstance(helper.getContext())
            .setVideoLockscreenWallpaper(
                (String) null, helper.getPackageName(), (String) null, helper.getWhich(), true);
        return true;
      }
      ...
      boolean result =
          BnRFileHelper.copyEncryptFile(
              helper.getSourceFilePath(), helper.getTargetFilePath(), helper.getFileSaveKey());
      if (!result) {
        return false;
      }
      WallpaperManager.getInstance(helper.getContext())
          .setVideoLockscreenWallpaper(
              helper.getTargetFilePath(), (String) null, (String) null, helper.getWhich(), true);
      return true;
    }
    ...
    private boolean setCoverVideoWallpaper(WallpaperBNRHelper helper) {
      try {
        ...
        Log.m96e(
            WallpaperBackupRestoreManager.TAG, "restoreWallpaper: Exception " + e.getMessage());
      }
      return false;
    }
    ...
    private boolean setGifWallpaper(WallpaperBNRHelper helper) {
      if (Rune.SUPPORT_COVER_DISPLAY_WATCHFACE && helper.getType() == 1 && helper.getMode() == 16) {
        try {
          ...
          Log.m96e(
              WallpaperBackupRestoreManager.TAG, "setGifWallpaper: Exception " + e.getMessage());
        }
      }
      return false;
    }
    ...
    private boolean setMultipackWallpaper(WallpaperBNRHelper helper) {
      ...
      if (helper.isDownloadedThemeWallpaper()) {
        WallpaperManager.getInstance(helper.getContext())
            .semSetUri(uri, true, helper.getWhich(), 3);
        return true;
      }
      if (helper.getUri() != null) {
        Bundle param = new Bundle();
    ...
    private boolean setMotionWallpaper(WallpaperBNRHelper helper) {
      ...
      WallpaperManager.getInstance(helper.getContext())
          .setMotionWallpaper(helper.getPackageName(), helper.getWhich(), true);
      return true;
    }
    ...
    private boolean setAnimatedWallpaper(WallpaperBNRHelper helper) {
      if (helper.isDownloadedThemeWallpaper()) {
        ...
        try {
          WallpaperManager.getInstance(helper.getContext())
              .setAnimatedLockscreenWallpaper(helper.getPackageName(), helper.getWhich(), true);
          return true;
        } catch (Exception e) {
          e.printStackTrace();
    ...
    private boolean setLiveWallpaper(WallpaperBNRHelper helper) {
      try {
        ...
        param.putString("key", helper.getFileSaveKey());
        Log.m94d(WallpaperBackupRestoreManager.TAG, "setLiveWallpaper: param = " + param);
    ...
    private Bundle sendEventToDressRoom(Context context, String method, Bundle param) {
      return context
          .getContentResolver()
          .call(
              Uri.parse("content://com.samsung.android.app.dressroom.provider"),
              method,
              (String) null,
              param);
    }
    ...
    private boolean isSourceFileValid(WallpaperBNRHelper helper) {
      ...
      if (!BnRFileHelper.isExist(helper.getSourceFilePath())) {
        helper.addErrorDescription(
            "isSourceFileValid: Source file not exist. path = <"
                + helper.getSourceFilePath()
                + ">");
        return false;
      }
      return true;
