...
public class SemPressGestureDetector {
  ...
  private Runnable mLongLongTouchRunnable =
      new Runnable() { // from class: com.samsung.android.widget.SemPressGestureDetector.1
        ...
        public void run() {
          ...
          semPressGestureDetector.mResponeLongLongTouch =
              semPressGestureDetector.sendBixbyLongClickedEvent(2);
        }
      };
      ...
      new Runnable() { // from class: com.samsung.android.widget.SemPressGestureDetector.2
        ...
        public void run() {
          ...
          if (SemPressGestureDetector.this.mView != null) {
            ...
            semPressGestureDetector2.mResponeLongTouch =
                semPressGestureDetector2.sendBixbyLongClickedEvent(1);
  ...
  private Runnable mCheckRestrictTouchRunnable =
      new Runnable() { // from class: com.samsung.android.widget.SemPressGestureDetector.3
        ...
        public void run() {
          SemPressGestureDetector semPressGestureDetector = SemPressGestureDetector.this;
          semPressGestureDetector.mTouchDownRestricted =
              semPressGestureDetector.isFingerPrintInDisplay();
        }
      };
  private Runnable mGetSettingRunnable =
      new Runnable() { // from class: com.samsung.android.widget.SemPressGestureDetector.4
        @Override // java.lang.Runnable
        public void run() {
          try {
            long version =
                SemPressGestureDetector.getAppVersionCode(
                    SemPressGestureDetector.this.mContext, SemPressGestureDetector.TAEGET_PKG_NAME);
            if (version < SemPressGestureDetector.SUPPORT_DOUBLE_FINGER_MODE_MIN_VERSION
                && version != SemPressGestureDetector.this.mBixbyTouchVersion) {
              if (SemPressGestureDetector.this.mContext.checkSelfPermission(
                      "android.permission.WRITE_SECURE_SETTINGS")
                  == 0) {
                Settings.Secure.putInt(
                    SemPressGestureDetector.this.mContext.getContentResolver(),
                    SemPressGestureDetector.KEY_BIXBYTOUCH_FINGER_TOUCH_TRIGGER,
                    1);
              }
              SemPressGestureDetector.sCurrentTouchMode = 1;
            } else {
              SemPressGestureDetector.sCurrentTouchMode =
                  Settings.Secure.getInt(
                      SemPressGestureDetector.this.mContext.getContentResolver(),
                      SemPressGestureDetector.KEY_BIXBYTOUCH_FINGER_TOUCH_TRIGGER,
                      1);
            }
            SemPressGestureDetector.sLongPressTime =
                Settings.Secure.getInt(
                    SemPressGestureDetector.this.mContext.getContentResolver(),
                    SemPressGestureDetector.KEY_BIXBYTOUCH_LONG_PRESS_TIME,
                    500);
            SemPressGestureDetector.sLongLongPressTime =
                Settings.Secure.getInt(
                    SemPressGestureDetector.this.mContext.getContentResolver(),
                    SemPressGestureDetector.KEY_LONG_LONG_PRESS_TIME,
                    1500);
            SemPressGestureDetector.sCheckTouchDownDelayTime =
                Settings.Secure.getInt(
                    SemPressGestureDetector.this.mContext.getContentResolver(),
                    SemPressGestureDetector.KEY_CHECK_FP_DELAY_TIME,
                    100);
            SemPressGestureDetector.sFingerDownThreshold =
                Settings.Secure.getInt(
                    SemPressGestureDetector.this.mContext.getContentResolver(),
                    SemPressGestureDetector.KEY_BIXBYTOUCH_FINGER_DOWN_THRESHOLD,
                    100);
    ...
    try {
      if (sCurrentTouchMode == 2) {
        if (this.mTouchedRawPoints.size() == 2) {
          float mTouchedX0 = this.mTouchedPoints.get(0).f3087x;
          float mTouchedY0 = this.mTouchedPoints.get(0).f3088y;
          float mTouchedX1 = this.mTouchedPoints.get(1).f3087x;
          float mTouchedY1 = this.mTouchedPoints.get(1).f3088y;
        ...
        float mTouchedX = this.mTouchedPoints.get(0).f3087x;
        float mTouchedY = this.mTouchedPoints.get(0).f3088y;
      ...
      if (this.mView != null) {
        ...
        if (activity != null) {
          ...
          if (configuration != null) {
            ...
            this.mWindowingMode =
                this.mView.getResources().getConfiguration().windowConfiguration.getWindowingMode();
          }
          this.mAppBounds =
              activity.getWindow().getWindowManager().getCurrentWindowMetrics().getBounds();
          this.mMaxBounds =
              activity.getWindow().getWindowManager().getMaximumWindowMetrics().getBounds();
          ...
          Log.secD(
              TAG, "parseInfoFromView: mDecorViewBounds = " + this.mDecorViewBounds.toString());
          try {
        ...
        this.mWindowConfig =
            this.mView.getResources().getConfiguration().windowConfiguration.toString();
        this.mAppBounds =
            this.mView.getResources().getConfiguration().windowConfiguration.getAppBounds();
        this.mWindowingMode =
            this.mView.getResources().getConfiguration().windowConfiguration.getWindowingMode();
      }
    } catch (Exception e2) {
  ...
  public boolean isFingerPrintInDisplay() {
    ...
    try {
      FingerprintManager fpm =
          (FingerprintManager) this.mContext.getSystemService(Context.FINGERPRINT_SERVICE);
  ...
  public boolean sendBixbyLongClickedEvent(int flag) {
    ...
    if (this.mDetachedFromWindow
        || (str = this.mActivityName) == null
        || str.startsWith(TAEGET_PKG_NAME)) {
      return false;
    }
    try {
      Bundle bundle = new Bundle();
      ...
      if (flag == 1) {
        ...
        if (sCurrentTouchMode != 2) {
          if (this.mTouchedRawPoints.size() == 1) {
            bundle.putFloat("touch_raw_start_x1", this.mTouchedRawPoints.get(0).f3087x);
            bundle.putFloat("touch_raw_start_y1", this.mTouchedRawPoints.get(0).f3088y);
          }
        } else if (this.mTouchedRawPoints.size() == 2) {
          bundle.putFloat("touch_raw_start_x1", this.mTouchedRawPoints.get(0).f3087x);
          bundle.putFloat("touch_raw_start_y1", this.mTouchedRawPoints.get(0).f3088y);
          bundle.putFloat("touch_raw_start_x2", this.mTouchedRawPoints.get(1).f3087x);
          bundle.putFloat("touch_raw_start_y2", this.mTouchedRawPoints.get(1).f3088y);
        }
      ...
      if (result != null) {
        if (flag == 1) {
          ...
          sFingerDownThreshold =
              result.getInt(KEY_BIXBYTOUCH_FINGER_DOWN_THRESHOLD, sFingerDownThreshold);
        }
    ...
    if (touchedView != null) {
      try {
        ...
        if (touchedView.getId() != -1) {
          bundle.putString(
              "found_widget_id", this.mContext.getResources().getResourceName(touchedView.getId()));
        }
  ...
  private void checkBlockApp() {
    new Thread(
            new Runnable() { // from class: com.samsung.android.widget.SemPressGestureDetector.5
              @Override // java.lang.Runnable
              public void run() {
                boolean z;
                try {
                  SemPressGestureDetector semPressGestureDetector = SemPressGestureDetector.this;
                  if (!semPressGestureDetector.isLauncherApp()
                      && !SemPressGestureDetector.this.matchPackage(
                          SemPressGestureDetector.TAEGET_PKG_NAME)) {
                    z = false;
  ...
  private boolean dispatchTouchEventDoubleFingers(MotionEvent event) {
    if (event.getActionMasked() == 0) {
      ...
      this.mView.postDelayed(
          this.mCheckRestrictTouchRunnable, sCheckTouchDownDelayTime - sFingerDownThreshold);
  ...
  private boolean checkTouchedPointIsMoved(MotionEvent event) {
    ...
    for (int i = 0; i < this.mTouchedPoints.size() && i < event.getPointerCount(); i++) {
      float mTouchX = this.mTouchedPoints.get(i).f3087x;
      float mTouchY = this.mTouchedPoints.get(i).f3088y;
      boolean z = false;
