...
public class SemLockPatternUtils {
  ...
  public boolean setLockCredential(
    ...
    try {
      boolean result =
          this.mLockPatternUtils.setLockCredential(newCredential, currentCredential, userId);
      return result;
    } catch (Exception e) {
      Log.m99i(TAG, "setLockCredential : catch exception", e);
      return false;
    }
  }
  ...
  public boolean verifyCredential(String password, int userId) {
    ...
    if (credential.isNone()) {
      Log.m98i(TAG, "verifyCredential : credential is none.");
      return false;
    }
    try {
      ...
      if (response.getResponseCode() != 0) {
        Log.m98i(TAG, "verifyCredential : return " + response.getResponseCode());
        return false;
      }
      return true;
    } catch (Exception e) {
      Log.m99i(TAG, "verifyCredential : catch exception", e);
      return false;
    }
  }
  ...
  public boolean clearLock(String currentPassword, int userId) {
    ...
    try {
      ...
      Log.m99i(TAG, "clearLock : catch exception", e);
      return false;
    }
  }
  ...
  private LockscreenCredential createCredential(String password, int credentialType) {
    switch (credentialType) {
      ...
      case 5:
      default:
        Log.m98i(TAG, "createCredential : wrong credential type : " + credentialType);
