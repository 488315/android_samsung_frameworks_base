...
public class SemCursorIndexer extends SemAbstractIndexer {
    ...
    public SemCursorIndexer(Cursor cursor, int sortedColumnIndex, CharSequence indexCharacters) {
        ...
        Log.m94d("SemCursorIndexer", "SemCursorIndexer constructor");
        if (sortedColumnIndex < 0) {
            RuntimeException e = new RuntimeException("here");
            e.fillInStackTrace();
            Log.m103w("SemCursorIndexer", "SemCursorIndexer() called with " + sortedColumnIndex, e);
        }
    }
    ...
    public SemCursorIndexer(Cursor cursor, int sortedColumnIndex, CharSequence indexCharacters, int profileCount, int favoriteCount) {
        ...
        this.mCursor = cursor;
        this.mColumnIndex = sortedColumnIndex;
        Log.m96e("SemCursorIndexer", "SemCursorIndexer constructor, profileCount:" + profileCount + ", favoriteCount:" + favoriteCount);
        if (sortedColumnIndex < 0) {
            RuntimeException e = new RuntimeException("here");
            e.fillInStackTrace();
            Log.m103w("SemCursorIndexer", "SemCursorIndexer() called with " + sortedColumnIndex, e);
    ...
    protected String getItemAt(int pos) {
        if (this.mCursor.isClosed()) {
            Log.m94d("SemCursorIndexer", "SemCursorIndexer getItemCount : mCursor is closed  ");
            return null;
        }
        if (this.mColumnIndex < 0) {
            Log.m94d("SemCursorIndexer", "getItemAt() mColumnIndex : " + this.mColumnIndex);
        }
    ...
    protected Bundle getBundle() {
        Log.m94d("SemCursorIndexer", "SemCursorIndexer getBundle : Bundle was used by Indexer");
    ...
    protected void onBeginTransaction() {
        ...
        Log.m94d("SemCursorIndexer", "SemCursorIndexer.onBeginTransaction() : Current cursor pos to save is :  " + this.mSavedCursorPos);
    }

    @Override // com.samsung.android.widget.SemAbstractIndexer
    protected void onEndTransaction() {
        Log.m94d("SemCursorIndexer", "SemCursorIndexer.onEndTransaction() : Saved cursor pos to restore  is :  " + this.mSavedCursorPos);
