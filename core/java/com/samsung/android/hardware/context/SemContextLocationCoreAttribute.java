...
public class SemContextLocationCoreAttribute extends SemContextAttribute {
  ...
  public boolean checkAttribute() {
    int i = this.mMode;
    if (i < -1 || i > 3) {
      Log.m94d(TAG, "Mode value is wrong!!");
      return false;
    }
    if (i == 0) {
      int i2 = this.mAction;
      if ((i2 < -1 || i2 > 10) && i2 != 23) {
        Log.m94d(TAG, "Action value is wrong!!");
        return false;
      }
    } else if (i == 1) {
      int i3 = this.mAction;
      ...
      if ((i4 == 16 || i4 == 17) && !this.mFusedBatchOption.isValid()) {
        Log.m94d(TAG, "FusedBatchOption is wrong");
        return false;
      }
    }
    if (this.mFenceId < 0) {
      Log.m94d(TAG, "FenceID is wrong!!");
      return false;
    }
    if (this.mRadius < 0) {
      Log.m94d(TAG, "Radius is wrong!!");
      return false;
    }
    if (this.mStatus < 0) {
      Log.m94d(TAG, "Status is wrong!1");
      return false;
    }
    if (this.mTotalGpsCnt < 0) {
      Log.m94d(TAG, "TotalGpsCount is wrong!!");
      return false;
    }
    if (this.mSuccessGpsCnt < 0) {
      Log.m94d(TAG, "Success gps count is wrong");
      return false;
    }
    if (this.mMinDistance < 0) {
      Log.m94d(TAG, "Minimum distance is wrong");
      return false;
    }
    if (this.mMinTime < 0) {
      Log.m94d(TAG, "Minimum time is wrong");
      return false;
    }
    if (this.mAccuracy < 0) {
      Log.m94d(TAG, "Accuracy is wrong");
      return false;
    }
    if (this.mTimeStamp < 0) {
      Log.m94d(TAG, "Timestamp is wrong");
      return false;
    }
    double d = this.mLongitude;
    if (d < -180.0d || d > 180.0d) {
      Log.m94d(TAG, "Longitude is wrong");
      return false;
    }
    double d2 = this.mLatitude;
    if (d2 < -90.0d || d2 > 90.0d) {
      Log.m94d(TAG, "Latitude is wrong");
      return false;
    }
    if (this.mRequestId < 0) {
      Log.m94d(TAG, "RequestId is wrong");
      return false;
    }
    if (this.mBatchingSize >= 0) {
      return true;
    }
    Log.m94d(TAG, "BatchingSize is wrong");
    return false;
  }
  ...
  private void setAttribute() {
    ...
    switch (this.mMode) {
      case 0:
        ...
        if (i == 1) {
          ...
          attribute.putIntArray(
              "IntType",
              new int[] {this.mFenceId, this.mRadius, this.mTotalGpsCnt, this.mSuccessGpsCnt});
          ...
          Log.m94d(TAG, "This Type is default attribute type");
          break;
        }
      case 1:
        if (this.mAction == 8) {
      ...
      case 3:
        ...
        if (i2 == 16 || i2 == 17) {
          int[] intType3 = {
            this.mRequestId, this.mFusedBatchOption.user_info, this.mFusedBatchOption.flags
          };
          ...
          float[] floatType2 = {
            this.mLocation.getSpeed(), this.mLocation.getBearing(), this.mLocation.getAccuracy()
          };
  ...
  private static class FusedBatchOption {
    ...
    boolean isValid() {
      if (this.period < 0) {
        Log.m94d(SemContextLocationCoreAttribute.TAG, "FusedBatchOption.period is wrong.");
        return false;
      }
      if (this.user_info < 0) {
        Log.m94d(SemContextLocationCoreAttribute.TAG, "FusedBatchOption.user_info is wrong.");
        return false;
      }
      if (this.flags < 0) {
        Log.m94d(SemContextLocationCoreAttribute.TAG, "FusedBatchOption.flags is wrong.");
        return false;
      }
      if (this.max_power < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN) {
        Log.m94d(SemContextLocationCoreAttribute.TAG, "FusedBatchOption.max_power is wrong.");
        return false;
      }
      if (this.distance_thrs < 0.0f) {
        Log.m94d(SemContextLocationCoreAttribute.TAG, "FusedBatchOption.distance_thrs is wrong.");
        return false;
      }
      return true;
    }
