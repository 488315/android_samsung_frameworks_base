...
public class SemSoundTriggerModule {
  ...
  SemSoundTriggerModule(
      int moduleId, final SemSoundTrigger.StatusListener listener, Handler handler) {
    this.instance = null;
    SoundTrigger.StatusListener stListener =
        new SoundTrigger
            .StatusListener() { // from class:
                                // com.samsung.android.hardware.soundtrigger.SemSoundTriggerModule.1
          ...
          public void onRecognition(SoundTrigger.RecognitionEvent recognitionEvent) {
            ...
            if (recognitionEvent instanceof SoundTrigger.KeyphraseRecognitionEvent) {
              SoundTrigger.KeyphraseRecognitionExtra[] stKeyphraseExtras =
                  ((SoundTrigger.KeyphraseRecognitionEvent) recognitionEvent).keyphraseExtras;
              SemSoundTrigger.KeyphraseRecognitionExtra[] KeyphraseExtras =
                  new SemSoundTrigger.KeyphraseRecognitionExtra[stKeyphraseExtras.length];
              for (int i = 0; i < stKeyphraseExtras.length; i++) {
                SemSoundTrigger.ConfidenceLevel[] confidenceLevels =
                    new SemSoundTrigger.ConfidenceLevel
                        [stKeyphraseExtras[i].confidenceLevels.length];
                for (int j = 0; j < stKeyphraseExtras[i].confidenceLevels.length; j++) {
                  confidenceLevels[j] =
                      new SemSoundTrigger.ConfidenceLevel(
                          stKeyphraseExtras[i].confidenceLevels[j].userId,
                          stKeyphraseExtras[i].confidenceLevels[j].confidenceLevel);
                }
                KeyphraseExtras[i] =
                    new SemSoundTrigger.KeyphraseRecognitionExtra(
                        stKeyphraseExtras[i].f227id,
                        stKeyphraseExtras[i].recognitionModes,
                        stKeyphraseExtras[i].coarseConfidenceLevel,
                        confidenceLevels);
              }
              listener.onRecognition(
                  new SemSoundTrigger.KeyphraseRecognitionEvent(
                      recognitionEvent.status,
                      recognitionEvent.soundModelHandle,
                      recognitionEvent.captureAvailable,
                      recognitionEvent.captureSession,
                      recognitionEvent.captureDelayMs,
                      recognitionEvent.capturePreambleMs,
                      recognitionEvent.triggerInData,
                      recognitionEvent.captureFormat,
                      recognitionEvent.data,
                      KeyphraseExtras));
              return;
            }
            listener.onRecognition(
                new SemSoundTrigger.RecognitionEvent(
                    recognitionEvent.status,
                    recognitionEvent.soundModelHandle,
                    recognitionEvent.captureAvailable,
                    recognitionEvent.captureSession,
