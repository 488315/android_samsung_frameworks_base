...
public class SemSmartClipDataRepository implements Parcelable {
  ...
  public static final Parcelable.Creator<SemSmartClipDataRepository> CREATOR =
      new Parcelable.Creator<
        ...
        public SemSmartClipDataRepository createFromParcel(Parcel in) {
          Log.m94d(
              SemSmartClipDataRepository.TAG, "SemSmartClipDataRepository.createFromParcel called");
      ...
      if (view != null) {
        ...
        if (bHaveUrlTag) {
          if ((view instanceof WebView)
              ...
              || view.getClass().getName().equals("org.chromium.content.browser.ChromeView")
              || view.getClass().getName().equals("org.samsung.content.sbrowser.SbrContentView")
              || view.getClass().getName().equals("com.sec.chromium.content.browser.SbrContentView")
              || view.getClass()
                  .getName()
    ...
    if (this.mScaleRect.width() != 1.0f || this.mScaleRect.height() != 1.0f) {
      ...
      if (hScale != 0.0f && vScale != 0.0f) {
        ...
        unScaledWinFrame.right =
            (int) (this.mWinFrameRect.left + (this.mWinFrameRect.width() / hScale) + 0.5f);
        unScaledWinFrame.bottom =
            (int) (this.mWinFrameRect.top + (this.mWinFrameRect.height() / vScale) + 0.5f);
  ...
  public boolean dump(boolean dumpMetaTags) {
    Log.m94d(TAG, "----- Start of SmartClip repository informations -----");
    Log.m94d(TAG, "** Content type : " + getContentType());
    Log.m94d(TAG, "** Meta area rect : " + getContentRect().toString());
    Log.m94d(TAG, "** Captured image file path : " + this.mCapturedImageFilePath);
    if (dumpMetaTags) {
      Log.m94d(TAG, "** mTags");
      SmartClipMetaTagArrayImpl smartClipMetaTagArrayImpl = this.mTags;
      if (smartClipMetaTagArrayImpl != null) {
        ...
        Log.m94d(TAG, "mTags is null");
      }
      Log.m94d(TAG, "** Element tree **");
      SmartClipDataRootElement smartClipDataRootElement = this.mRootElement;
      if (smartClipDataRootElement != null) {
        smartClipDataRootElement.dump();
      }
    }
    Log.m94d(TAG, "----- End of SmartClip repository informations -----");
    return true;
  }
  ...
  public void writeToParcel(Parcel out, int flags) {
    ...
    SmartClipMetaTagArrayImpl smartClipMetaTagArrayImpl =
        (SmartClipMetaTagArrayImpl) getAllMetaTags();
    ...
    try {
      ...
      if (json_repository.has(FIELD_CONTENT_RECT)) {
        ...
        repository.mContentRect =
            new Rect(
                json_rect.getInt(0), json_rect.getInt(1), json_rect.getInt(2), json_rect.getInt(3));
      }
      ...
      if (json_repository.has(FIELD_META_TAGS)) {
        ...
        for (int i = 0; i < tagCount; i++) {
          ...
          try {
            ...
            Log.m96e(TAG, "There is no meta value! type=" + tagType);
          }
          try {
            String extraDataStr = json_tag.getString(FIELD_META_TAG_EXTRA_DATA);
            byte[] extraData = Base64.decode(extraDataStr, 0);
            Log.m94d(TAG, "Decoding : Length of extra data = " + extraData.length);
      ...
      e = e4;
      Log.m96e(TAG, "JSONException throwed : " + e.toString());
      e.printStackTrace();
    }
  }
        ...
        if (curTag != null) {
          ...
          if (curTag instanceof SemSmartClipExtendedMetaTag) {
            ...
            if (curTagImpl.getExtraData() != null) {
              ...
              Log.m94d(TAG, "Encoding : Length of extra data = " + extraData.length);
