...
public class SmartClipDataCropperImpl extends SemSmartClipDataCropper {
    ...
    public boolean doExtractSmartClipData(View rootView) {
        ...
        Log.m94d(TAG, "doExtractSmartClipData : Extraction start! reqId = " + this.mExtractionRequest.mRequestId + "  Cropped area = " + rectAreaStr + "  Package = " + this.mPackageName);
    ...
    public boolean setPendingExtractionResult(SemSmartClipDataElement resultElement) {
        ...
        if (!elementImpl.isEmptyTag(false)) {
            if (DEBUG) {
                Log.m94d(TAG, "setPendingExtractionResult : Contains meta data : " + elementImpl.getDumpString(false, true));
            } else {
                Log.m94d(TAG, "setPendingExtractionResult : Contains meta data : " + elementImpl.getDumpString(false, false));
            }
        }
    ...
    public boolean sendExtractionResultToSmartClipService(SemSmartClipDataRepository result) {
        ...
        if (result != null) {
            Log.m94d(TAG, "sendExtractionResultToSmartClipService : -- Extracted SmartClip data information --");
            Log.m94d(TAG, "sendExtractionResultToSmartClipService : Request Id : " + this.mExtractionRequest.mRequestId);
            Log.m94d(TAG, "sendExtractionResultToSmartClipService : Extraction mode : " + this.mExtractionRequest.mExtractionMode);
            result.dump(DEBUG);
        } else {
        ...
        Log.m94d(TAG, "sendExtractionResultToSmartClipService : Elapsed = " + (System.currentTimeMillis() - this.mExtractionStartTime));
        return true;
    }
    ...
    private int extractDefaultSmartClipData_GoogleChromeView(View view, SemSmartClipCroppedArea croppedArea, SmartClipDataElementImpl resultElement) {
        try {
            ...
            if (extractSmartClipDataMethod != null && setSmartClipResultHandlerMethod != null) {
                ...
                Handler handler = new Handler(resultElement, view, setSmartClipResultHandlerMethod) { // from class: com.samsung.android.content.smartclip.SmartClipDataCropperImpl.1
                    ...
                    public void handleMessage(Message msg) {
                        ...
                        if (SmartClipDataCropperImpl.DEBUG) {
                            Log.m94d(SmartClipDataCropperImpl.TAG, String.format("Title:%s\nURL:%s\nArea:%s\nText:%s\nHTML:%s", title, url, area, text, html));
                        }
                        ...
                        try {
                            ...
                            Log.m96e(SmartClipDataCropperImpl.TAG, "Could not invoke set smartclip handler API");
                            e.printStackTrace();
                        }
                        SmartClipDataCropperImpl.this.setPendingExtractionResult(this.mResult);
                    }
                ...
                if (DEBUG) {
                    Log.m94d(TAG, "Converting coordinate : " + croppedArea.getRect().toString() + " -> " + cropRect.toString());
                }
        ...
        try {
            ...
            if (invokeObj != null && extractSmartClipDataMethod != null) {
                ...
                Handler handler = new Handler(resultElement, view) { // from class: com.samsung.android.content.smartclip.SmartClipDataCropperImpl.2
                    ...
                    @Override // android.p009os.Handler
                    public void handleMessage(Message msg) {
                        Log.m94d(SmartClipDataCropperImpl.TAG, "Pending meta data arrived from third party");
                        Bundle bundle = msg.getData();
                        if (bundle == null) {
                            Log.m96e(SmartClipDataCropperImpl.TAG, "The bundle is null!");
                            SmartClipDataCropperImpl.this.setPendingExtractionResult(this.mResult);
            ...
            Log.m96e(TAG, "Exception is thrown during execute the third party smartclip interface. e=" + e);
            e.printStackTrace();
        }
        return 1;
    }
    ...
    private Rect adjustMetaAreaRect(View view, Rect rect) {
        ...
        Log.m96e(TAG, "adjustMetaAreaRect : there is no intersection " + rect + " and " + viewBoundsOnScreen);
        return null;
    }
    ...
    private boolean removeSmartClipDataElementByRect(SmartClipDataElementImpl element, Rect rectToDelete) {
        ...
        if (element.getFirstChild() == null) {
            ...
            if (metaAreaRect != null && Rect.intersects(rectToDelete, metaAreaRect)) {
                Log.m94d(TAG, "removeSmartClipDataElementByRect : Removing element due to RECT intersection. element = " + element.getDumpString(false, true));
                element.getParent().removeChild(element);
                return true;
            }
        }
                ...
                if (htmlTagCount > 0 && textTagCount > 0) {
                    switch (smartClipDataCropperImpl.mExtractionLevel) {
                        ...
                        default:
                            ...
                            Log.m94d(TAG, "filterMetaTagForBrowserViews : The TEXT tag changed to HTML_TEXT. View=" + viewName);
                            break;
                    }
                }
                ...
                while (it2.hasNext()) {
                    ...
                    if (SemSmartClipMetaTagType.HTML.equals(type) == z) {
                        ...
                        if (value.length() > MAX_META_VALUE_SIZE) {
                            Log.m96e(TAG, "filterMetaTagForBrowserViews : Have large HTML data(" + value.length() + " bytes). Converting tag..");
                            String filePathName = allocateMetaTagFilePath();
                            if (!smartClipDataCropperImpl.writeStringToFile(filePathName, value)) {
                                Log.m96e(TAG, "filterMetaTagForBrowserViews : Failed to save meta tag! - " + filePathName);
                            } else {
                                Log.m94d(TAG, "filterMetaTagForBrowserViews : Saved the meta tag to " + filePathName);
                            }
    ...
    private ArrayList<View> getChildViewsByZOrder(ViewGroup viewGroup) {
        ...
        if (zOrderExist) {
            ...
            while (it.hasNext()) {
                View child = it.next();
                Log.m94d(TAG, "getChildViewsByZOrder : Parent=" + viewGroup + " / View=" + child + " / Z=" + child.getZ());
            }
        }
        return childViews;
    }
    ...
    private boolean isThirdPartyExtractionInterfaceEnabledOnManifest(Context context, String packageName) {
        ...
        if (ai == null) {
            Log.m96e(TAG, "isSupportThirdPartyExtractionInterface : Could not get appInfo! - " + packageName);
            return false;
        }
    ...
    public boolean updateDataElementWithBundle(View view, Bundle bundle, SmartClipDataElementImpl resultElement) {
        ...
        Rect area = (Rect) bundle.getParcelable("rect");
        if (DEBUG) {
            Log.m94d(TAG, String.format("fillDataElementWithBundle : Title:%s\nLink:%s\nURL:%s\nArea:%s", title, appLink, url, area));
        }
        if (!TextUtils.isEmpty(title)) {
            resultElement.setTag(new SemSmartClipMetaTag("title", title));
            ...
            if (Rect.intersects(croppedArea.getRect(), screenRectOfView)) {
                ...
                switch (mainResult) {
                    ...
                    default:
                        ...
                        Log.m96e(TAG, "Unknown main extraction result value : " + mainResult + " / View = " + viewString);
                        element.clearMetaData();
                        break;
                }
                ...
                if (!element.isEmptyTag(false)) {
                    if (DEBUG) {
                        Log.m94d(TAG, "traverseView : Contains meta data : " + element.getDumpString(false, true));
                    } else {
                        Log.m94d(TAG, "traverseView : Contains meta data : " + element.getDumpString(false, false));
                    }
                }
                if (haveCroppedView) {
    ...
    */
    private boolean traverseViewForDragAndDrop(View view, SemSmartClipCroppedArea croppedArea, SemSmartClipDataRepository smartClipDataRepository, SmartClipDataElementImpl parentDataElement) {
        int extractionResult;
        ...
        if (view != null && view.getVisibility() == 0 && view.getWidth() > 0 && view.getHeight() > 0) {
            ...
            if (Rect.intersects(croppedArea.getRect(), screenRectOfView)) {
                ...
                for (SmartClipDataElementImpl elementTraveler = element; elementTraveler != null; elementTraveler = elementTraveler.traverseNextElement(element)) {
                    Rect adjustedMetaAreaRect = smartClipDataCropperImpl.adjustMetaAreaRect(view, elementTraveler.getMetaAreaRect());
                    elementTraveler.setMetaAreaRect(adjustedMetaAreaRect);
                }
                int mainResult = smartClipDataCropperImpl.getMainResultFromExtractionResult(extractionResult);
                switch (mainResult) {
                    case 0:
                        element.clearMetaData();
                        break;
                ...
                if (!(view instanceof ViewGroup) || skipExtractionFromChildView) {
                    ...
                    ArrayList<View> childViewArray = smartClipDataCropperImpl.getChildViewsByZOrder((ViewGroup) view);
                    int childCount = childViewArray.size();
                    int i = childCount - 1;
                    while (i >= 0) {
                        View child = childViewArray.get(i);
                        boolean haveCroppedView3 = haveCroppedView2;
                        boolean bFound = smartClipDataCropperImpl.traverseViewForDragAndDrop(child, croppedArea, smartClipDataRepository, element);
                        if (!bFound) {
                            i--;
                            smartClipDataCropperImpl = this;
