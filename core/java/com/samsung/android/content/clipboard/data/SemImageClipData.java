...
public class SemImageClipData extends SemClipData {
  ...
  public boolean setAlternateClipData(int type, SemClipData altData) {
    ...
    switch (type) {
      case 2:
        ...
        boolean result2 =
            ((SemImageClipData) altData).setBitmapPath(getBitmapPath(), getExtraDataPath());
        return result2;
      default:
        return false;
    }
  ...
  public String toString() {
    return "SemImageClipData class. Value is "
        + ((Object)
            (this.mImagePath.length() > 20 ? this.mImagePath.subSequence(0, 20) : this.mImagePath));
  }

  private boolean compareFile(FileInputStream src, FileInputStream dest) {
    Throwable th;
    BufferedInputStream bisSrc;
    int compareSize = 128;
    ...
    try {
      ...
      if (srcSize == destSize && srcSize >= 1) {
        if (destSize >= 1) {
          ...
          try {
            bisSrc = new BufferedInputStream(src);
          } catch (IOException e) {
            e = e;
            try {
              e.printStackTrace();
              ...
              try {
                ...
                try {
                  ...
                  while (i2 < buffSize) {
                    ...
                    result = readSrcData[i2] == readDestData[i2];
                    i2++;
                    bisDest = bisDest2;
                  }
                  compareCount++;
  ...
  public void insertContentUri(Context context, String path) {
    ...
    try {
      try {
        ...
        Log.m60e(TAG, "Exception occurs in insertContentUri because " + e.getMessage());
      }
    } finally {
