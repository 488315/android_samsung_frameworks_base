...
public class SemImageClipDataProvider extends ContentProvider {
    ...
    /* renamed from: ID */
    public static final String f2965ID = "id";
    ...
    private static class DBHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            Log.m102w(DBHelper.class.getName(), "Upgrading database from version " + oldVersion + " to " + newVersion + ". Old data will be destroyed");
    ...
    public Uri insert(Uri uri, ContentValues values) {
        if (Binder.getCallingUid() != 1000) {
            Log.m96e(TAG, "SecurityException when insert in SemClipboardProvider. blocked package : " + getContext().getPackageManager().getNameForUid(Binder.getCallingUid()));
            return null;
        }
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        int count;
        if (Binder.getCallingUid() != 1000) {
            Log.m96e(TAG, "SecurityException when update in SemClipboardProvider. blocked package : " + getContext().getPackageManager().getNameForUid(Binder.getCallingUid()));
            return 0;
        }
        switch (uriMatcher.match(uri)) {
            case 1:
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        int count;
        if (Binder.getCallingUid() != 1000) {
            Log.m96e(TAG, "SecurityException when delete in SemClipboardProvider. blocked package : " + getContext().getPackageManager().getNameForUid(Binder.getCallingUid()));
            return 0;
        }
        switch (uriMatcher.match(uri)) {
            case 1:
