...
public class SamsungTelecomServiceConnection {
  ...
  private static final ComponentName SERVICE_COMPONENT =
      new ComponentName(
          "com.android.server.telecom", "com.samsung.server.telecom.SamsungTelecomService");
        ...
        service.linkToDeath(
            new IBinder
                .DeathRecipient() { // from class:
                                    // com.samsung.android.telecom.SamsungTelecomServiceConnection.TelecomServiceConnection.1
            ...
            0);
        Slog.m117i(
            SamsungTelecomServiceConnection.TAG,
            "connectToSamsungTelecom - ServiceManager.addService : " + service);
        ServiceManager.addService(Context.SEM_TELECOM_SERVICE, service);
      } catch (RemoteException e) {
        Slog.m117i(SamsungTelecomServiceConnection.TAG, "Failed linking to death.");
      }
    }
  ...
  public void connectToSamsungTelecom() {
    if (hasSamsungTelecomSystemFeature()) {
      synchronized (this.mLock) {
        ...
        Slog.m117i(TAG, "connectToSamsungTelecom - Attempting to bind to : " + componentName);
        if (this.mContext.bindServiceAsUser(
            intent, telecomServiceConnection2, 67108929, UserHandle.SYSTEM)) {
          Slog.m117i(TAG, "connectToSamsungTelecom - Succeeded to connect");
          this.mTelecomServiceConnection = telecomServiceConnection2;
        } else {
          Slog.m117i(TAG, "connectToSamsungTelecom - Failed to connect");
        }
      }
    }
  }
