...
public class MdfUtils {
  ...
  public static void logMdf(
      boolean condition, String logMessage, boolean outcome, int severity, String swComponent) {
      ...
      if (certificateChain[i] != null) {
        for (int j = 0; j < allowedIssuers.length; j++) {
          if (allowedIssuers[j] != null
              && allowedIssuers[j].equals(certificateChain[i].getIssuerX500Principal())) {
            return true;
          }
        }
      }
                ...
                buildMethod.invoke(
                    setEncryptionPaddingsMethod.invoke(
                        setKeySizeMethod.invoke(
                            setBlockModesMethod.invoke(specBuilder, new String[] {"GCM"}), 256),
  ...
  public static byte[] decryptMdf(byte[] value, String keyAlias) {
    ...
    try {
      ...
      if (secretKey == null) {
        System.err.println(
            "MDFUtils::MDF decryption failed, unable to get encryption key from AndroidKeystore");
        return null;
      }
