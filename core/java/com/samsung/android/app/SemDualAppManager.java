    ...
    */
    public void addDualAppAccounts(LinearLayout contents, int resId, int resHeight) {
        ...
        try {
            ...
            AuthenticatorDescription[] dualAppDescs = AccountManager.get(mContext).getAuthenticatorTypesAsUser(dualAppId2);
                ...
                if (desc == null) {
                    ...
                    try {
                        if (desc.iconId == 0) {
                            ...
                            dualAppId = dualAppId2;
                            try {
                                Context authContext = mContext.createPackageContextAsUser(desc.packageName, 0, userHandle);
                                PackageManager pm = mContext.getPackageManager();
                                if (pm.semShouldPackIntoIconTray(desc.packageName)) {
                                    mgr = mgr2;
                                    try {
                                        icon = pm.getUserBadgedIcon(pm.semGetDrawableForIconTray(authContext.getDrawable(desc.iconId), 1), userHandle);
                                    } catch (PackageManager.NameNotFoundException e) {
                                        Log.m102w(TAG, "Bad package name for account type " + desc.type);
                                        if (icon == null) {
                                        }
                                        TextView child = (TextView) inflater.inflate(resId, (ViewGroup) contents, false);
                                        ...
                                        TextView child2 = (TextView) inflater.inflate(resId, (ViewGroup) contents, false);
    ...
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    private void addResolveInfoFromOtherUser(PackageManager pm, List<DisplayResolveInfo> target, ActivityInfo ai, Intent ii) {
        int userId;
        ActivityInfo ai2;
        ResolveInfo ri;
        ActivityInfo ai3 = ai;
        ...
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() { // from class: com.samsung.android.app.SemDualAppManager.1
            ...
            public void run() {
                try {
                    ...
                    Drawable badgeicon = Resources.getSystem().getDrawableForDensity(C4337R.drawable.ic_dualapp_widget_badge, density);
                    if (badgeicon != null) {
