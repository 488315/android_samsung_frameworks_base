...
public class SprCacheManager {
    ...
    public void printDebug() {
        synchronized (this.mCacheList) {
            Log.m94d("SprDrawable", this.mName + NavigationBarInflaterView.KEY_CODE_START + this.mHashCode + ") printDebug start");
            Iterator<SprCache> it = this.mCacheList.iterator();
            while (it.hasNext()) {
                SprCache cache = it.next();
                Log.m94d("SprDrawable", this.mName + NavigationBarInflaterView.KEY_CODE_START + this.mHashCode + ")Cache (" + cache.width + ", " + cache.height + NavigationBarInflaterView.SIZE_MOD_START + cache.dpi + "]) " + cache.refCount);
            }
            Log.m94d("SprDrawable", this.mName + NavigationBarInflaterView.KEY_CODE_START + this.mHashCode + ") printDebug end");
        }
    }
