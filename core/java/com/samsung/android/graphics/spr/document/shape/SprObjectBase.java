...
public abstract class SprObjectBase implements Cloneable {
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (id) {
                ...
                case 112:
                    ...
                    break;
                default:
                    Log.m96e(TAG, "Unknown attribute id:" + id);
                    in.skip(size);
                    break;
            }
        }
        ...
        while (it.hasNext()) {
            ...
            switch (attribute.mType) {
                case 1:
                    ...
                    canvas.clipRect(clip.left, clip.top, clip.right, clip.bottom, Region.EnumC0836Op.INTERSECT);
                    break;
                case 3:
                    ...
                    if (reference == null) {
                        ...
                        switch (reference.mType) {
                            case 1:
                                Path path = new Path();
                                path.addCircle(((SprObjectShapeCircle) reference).f2985cx, ((SprObjectShapeCircle) reference).f2986cy, ((SprObjectShapeCircle) reference).f2984cr, Path.Direction.CW);
                            ...
                            case 2:
                                ...
                                break;
                            case 4:
                                canvas.clipPath(((SprObjectShapePath) reference).path, Region.EnumC0836Op.INTERSECT);
                                break;
                            case 5:
                                canvas.clipRect(((SprObjectShapeRectangle) reference).left, ((SprObjectShapeRectangle) reference).top, ((SprObjectShapeRectangle) reference).right, ((SprObjectShapeRectangle) reference).bottom, Region.EnumC0836Op.INTERSECT);
                                break;
                        }
                    }
                case 32:
                ...
                case 64:
                    ...
                    break;
                default:
                    Log.m94d(TAG, "Attribute type = " + ((int) attribute.mType) + "is not supported type");
                    break;
            }
        }
    }
    ...
    protected void setShadowLayer() {
        ...
        if (this.isVisibleFill) {
            ...
            this.fillPaint.setShadowLayer(radius <= 0.5f ? radius : (radius - 0.5f) / 0.57735f, this.shadow.f2975dx, this.shadow.f2976dy, this.shadow.shadowColor);
            return;
        }
        if (this.isVisibleStroke) {
            float radius2 = sprAttributeShadow.radius;
            float f = radius2 <= 0.5f ? radius2 : (radius2 - 0.5f) / 0.57735f;
            this.strokePaint.setShadowLayer(this.shadow.radius, this.shadow.f2975dx, this.shadow.f2976dy, this.shadow.shadowColor);
        }
    }
    ...
    public SprObjectBase mo8906clone() throws CloneNotSupportedException {
        ...
        while (it.hasNext()) {
            SprAttributeBase attribute = it.next();
            object.mAttributeList.add(attribute.mo8903clone());
        }
        if (this.strokePaint != null) {
