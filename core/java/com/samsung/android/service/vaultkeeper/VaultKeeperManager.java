...
public final class VaultKeeperManager {
    ...
    public static VaultKeeperManager getInstance(String vaultName) {
        if (vaultName == null) {
            Log.m96e(TAG, "vaultName is null");
            return null;
        }
        if (vaultName.length() == 0 || vaultName.length() > 32) {
            Log.m96e(TAG, "vaultName length is wrong(" + vaultName.length() + "). It should be less than (32" + NavigationBarInflaterView.KEY_CODE_END);
            return null;
        }
