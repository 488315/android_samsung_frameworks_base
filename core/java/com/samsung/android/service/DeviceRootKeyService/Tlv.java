...
public final class Tlv {
  ...
  public static Tlv decodeTlv(byte[] buffer, int offset, int length) {
    ...
    int parsed = 0 + 1;
    short totLen =
        ByteBuffer.wrap(buffer, offset + parsed, 2).order(ByteOrder.LITTLE_ENDIAN).getShort();
    ...
    while (parsed2 < length) {
      ...
      int i =
          ByteBuffer.wrap(buffer, offset + parsed3, 2).order(ByteOrder.LITTLE_ENDIAN).getShort();
