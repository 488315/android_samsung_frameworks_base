...
public final class DeviceRootKeyServiceManager {
  ...
  public byte[] createServiceKeySession(String serviceName, int keyType, Tlv tlv) {
    Log.m98i(TAG, "createServiceKeySession() is called.");
    try {
      if (tlv == null) {
  ...
  public int setDeviceRootKey(byte[] keyBlob) {
    Log.m98i(TAG, "setDeviceRootKey() has been deprecated.");
    return -10000;
  }

  public byte[] doSelfTestProvService(int keyType, Tlv tlv) {
    Log.m98i(TAG, "doSelfTestProvService() is called.");
    try {
      if (tlv == null) {
  ...
  public DeviceInfo getDeviceInfo(int infoType) {
    Log.m98i(TAG, "getDeviceInfo() is called.");
    if (infoType <= 0 || infoType > 14) {
      Log.m96e(TAG, "Invalid argument");
      return null;
    }
    try {
      if (this.mTlvEx == null) {
  ...
  public final class DeviceInfo {
    ...
    public DeviceInfo(int infoType) {
      ...
      if ((infoType & 2) == 2
          && (tByteArray3 = DeviceRootKeyServiceManager.this.mTlvEx.getTlvValue(102)) != null
          && isValidLength(102, tByteArray3.length)) {
        this.mModemHash = (byte[]) tByteArray3.clone();
      }
      if ((infoType & 4) == 4
          && (tByteArray2 = DeviceRootKeyServiceManager.this.mTlvEx.getTlvValue(104)) != null
          && isValidLength(104, tByteArray2.length)) {
        this.mImeiHash = (byte[]) tByteArray2.clone();
      }
      if ((infoType & 8) == 8
          && (tByteArray = DeviceRootKeyServiceManager.this.mTlvEx.getTlvValue(108)) != null
    ...
    */
    private boolean isValidLength(int tag, int len) {
      switch (tag - 100) {
        case 1:
          return len == 1;
        case 2:
  ...
  private final class TlvEx {
    ...
    private boolean parseTlv(byte[] tlvBuffer) {
      if (tlvBuffer == null || tlvBuffer.length < 3) {
        Log.m96e(DeviceRootKeyServiceManager.TAG, "Invalid argument");
        return false;
      }
      ...
      if (tag != 254 || pos2 + len != tlvBuffer.length) {
        Log.m96e(DeviceRootKeyServiceManager.TAG, "Failed to read TLV header");
        return false;
      }
