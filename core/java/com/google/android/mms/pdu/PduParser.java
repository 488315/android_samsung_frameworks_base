...
public class PduParser {
    ...
    */
    protected PduBody parseParts(ByteArrayInputStream pduDataStream, byte[] contType) {
        char c;
        ...
        if (ctTypeStr.equals(ContentType.MULTIPART_MIXED) || ctTypeStr.equals(ContentType.MULTIPART_RELATED)) {
            ...
            while (i < count) {
                ...
                int endPos = headerLength - (startPos - pduDataStream.available());
                if (endPos > 0) {
                    if (!pduParser.parsePartHeaders(pduDataStream, part, endPos)) {
                        return null;
                    }
                } else if (endPos < 0) {
                    return null;
                ...
                c2 = c;
                count = count2;
                ctTypeStr = ctTypeStr2;
                pduBody = pduBody2;
                pduParser = this;
            }
        } else {
            if (!ctTypeStr.equals("text/plain")) {
                return null;
            }
            String smilDoc = new String("<smil><head><layout><root-layout width=\"320px\" height=\"480px\"/><region id=\"Text\" left=\"0\" top=\"320\" width=\"320px\" height=\"160px\" fit=\"meet\"/></layout></head><body><par><text src=\"attach.txt\" region=\"Text\"/></par></body></smil>");
            PduPart smilPart = new PduPart();
            smilPart.setContentId("smil.txt".getBytes());
            smilPart.setContentLocation("smil.txt".getBytes());
            smilPart.setContentType(ContentType.APP_SMIL.getBytes());
            smilPart.setData(smilDoc.getBytes());
            body.addPart(0, smilPart);
            PduPart part2 = new PduPart();
            part2.setContentLocation("attach.txt".getBytes());
            part2.setContentId("attach.txt".getBytes());
            part2.setContentType("text/plain".getBytes());
            int dataLength2 = pduDataStream.available();
            byte[] partData = new byte[dataLength2];
            pduDataStream.read(partData, 0, dataLength2);
            part2.setData(partData);
            body.addPart(part2);
        }
        return body;
    }
            ...
            switch (param) {
                case 129:
                    ...
                    if ((firstValue > 32 && firstValue < 127) || firstValue == 0) {
                        ...
                        try {
                            ...
                            Log.m97e(LOG_TAG, Arrays.toString(charsetStr), e);
                            map.put(129, 0);
                        }
                    } else {
                ...
                default:
                    if (-1 == skipWapValue(pduDataStream, lastLen)) {
                        Log.m96e(LOG_TAG, "Corrupt Content-Type");
                        break;
                    } else {
                        lastLen = 0;
                        break;
    ...
    protected static byte[] parseContentType(ByteArrayInputStream pduDataStream, HashMap<Integer, Object> map) {
        ...
        if (cur < 32) {
            try {
                ...
                int startPos = pduDataStream.available();
                if (length > startPos) {
                    Log.m96e(LOG_TAG, "parseContentType: Invalid length " + length + " when available bytes are " + startPos);
                ...
                if (parameterLen < 0) {
                    Log.m96e(LOG_TAG, "Corrupt MMS message");
                    return PduContentTypes.contentTypes[0].getBytes();
                }
                return contentType;
            } catch (IllegalArgumentException e) {
        ...
        while (lastLen > 0) {
            ...
            if (header > 127) {
                switch (header) {
                    ...
                    default:
                        if (-1 == skipWapValue(pduDataStream, lastLen)) {
                            Log.m96e(LOG_TAG, "Corrupt Part headers");
                            return false;
                        }
                        lastLen = 0;
                        break;
