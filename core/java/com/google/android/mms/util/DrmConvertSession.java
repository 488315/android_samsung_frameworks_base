    ...
    if (context != null && str != null && !str.equals("")) {
      try {
        ...
        try {
          ...
          Log.m103w(TAG, "Conversion of Mimetype: " + str + " is not supported.", e);
          str2 = str2;
        ...
        Log.m102w(str2, "DrmManagerClient instance could not be created, context is Illegal.");
      } catch (IllegalStateException e4) {
        Log.m102w(str2, "DrmManagerClient didn't initialize properly.");
      }
    }
  ...
  public byte[] convert(byte[] inBuffer, int size) {
    ...
    if (inBuffer != null) {
      try {
        ...
        Log.m103w(
            TAG,
            "Buffer with data to convert is illegal. Convertsession: " + this.mConvertSessionId,
            e);
        return null;
      } catch (IllegalStateException e2) {
    ...
    try {
      ...
      try {
        try {
          ...
          Log.m103w(
              TAG, "Access to File: " + filename + " was denied denied by SecurityManager.", e8);
          if (rndAccessFile == null) {
            return 491;
          }
          try {
      ...
      Log.m103w(
          TAG, "Could not close convertsession. Convertsession: " + this.mConvertSessionId, e10);
      return result2;
    }
  }
}
