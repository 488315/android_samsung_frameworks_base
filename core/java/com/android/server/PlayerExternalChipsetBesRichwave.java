    ...
    private synchronized void seekTune(int cmd, int value) {
        ...
        if (!this.mIsScanning) {
            ...
            if (this.mIsRDSEnabled) {
                ...
                needRestoreRds = true;
                byte[] seektune_cmd = {-1};
                int ret = this.mUsbDeviceConnection.controlTransfer(192, 161, cmd, value, seektune_cmd, 1, 4000);
                log("cmd:" + cmd + ", value: " + value + ", ret: " + ret + ", data: " + toHex(seektune_cmd) + ", ascii: " + toASCII(seektune_cmd));
                int timeOutCnt = 0;
                while (true) {
        ...
        int ret2 = this.mUsbDeviceConnection.controlTransfer(192, 161, cmd, value, seektune_cmd2, 1, 4000);
        log("cmd:" + cmd + ", value: " + value + ", ret: " + ret2 + ", data: " + toHex(seektune_cmd2) + ", ascii: " + toASCII(seektune_cmd2));
        int timeOutCnt2 = 0;
        while (true) {
            if (this.mIsSeekTuneing) {
            }
