        ...
        public void onAvailable(Network network) {
            synchronized (NetworkPinner.sLock) {
                ...
                if (NetworkPinner.sCM.getBoundNetworkForProcess() == null && NetworkPinner.sNetwork == null) {
                    ...
                    Log.m94d(NetworkPinner.TAG, "Wifi alternate reality enabled on network " + network);
                }
        ...
        public void onLost(Network network) {
            synchronized (NetworkPinner.sLock) {
                ...
                if (network.equals(NetworkPinner.sNetwork) && network.equals(NetworkPinner.sCM.getBoundNetworkForProcess())) {
                    NetworkPinner.unpin();
                    Log.m94d(NetworkPinner.TAG, "Wifi alternate reality disabled on network " + network);
                }
                NetworkPinner.sLock.notify();
            }
        }
        ...
        synchronized (sLock) {
            if (sCallback == null) {
                ...
                try {
                    ...
                    Log.m95d(TAG, "Failed to register network callback", e);
                    sCallback = null;
                }
            }
        }
        ...
        synchronized (sLock) {
            if (sCallback != null) {
                try {
                    ...
                    Log.m95d(TAG, "Failed to unregister network callback", e);
                }
                sCallback = null;
                sNetwork = null;
            }
