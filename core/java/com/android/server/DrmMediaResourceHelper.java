...
class DrmMediaResourceHelper {
    ...
    private final SemMediaResourceHelper.CodecStateChangedListener mMediaResourceCodecStateChangeListener = new SemMediaResourceHelper.CodecStateChangedListener() { // from class: com.android.server.DrmMediaResourceHelper.1
        ...
        public void onStateChanged(ArrayList<SemMediaResourceHelper.MediaResourceInfo> mediaResourceInfo) {
            if (mediaResourceInfo != null && mediaResourceInfo.size() > 0) {
                Log.m98i(DrmMediaResourceHelper.this.TAG, "onStateChanged size = " + mediaResourceInfo.size());
                for (int i = 0; i < mediaResourceInfo.size(); i++) {
                    Log.m98i(DrmMediaResourceHelper.this.TAG, "resource type: " + mediaResourceInfo.get(i).getResourceType() + ",,,isSecured :  " + mediaResourceInfo.get(i).isSecured() + "IsEncoder : " + mediaResourceInfo.get(i).isEncoder() + "getCodecState : " + mediaResourceInfo.get(i).getCodecState() + "getpid : " + mediaResourceInfo.get(i).getPid());
                    ...
                    if (pid > 0 && mediaResourceInfo.get(i).isSecured() && !mediaResourceInfo.get(i).isEncoder() && mediaResourceInfo.get(i).getCodecState() == 1) {
                        Log.m96e(DrmMediaResourceHelper.this.TAG, "CodecStateChangedListener resource type: " + mediaResourceInfo.get(i).getResourceType() + ",,,isSecured :  " + mediaResourceInfo.get(i).isSecured() + "IsEncoder : " + mediaResourceInfo.get(i).isEncoder() + "getCodecState : " + mediaResourceInfo.get(i).getCodecState() + "getpid : " + mediaResourceInfo.get(i).getPid());
    ...
    private final SemMediaResourceHelper.ResourceInfoChangedListener mMediaResourceInfoChangedListener = new SemMediaResourceHelper.ResourceInfoChangedListener() { // from class: com.android.server.DrmMediaResourceHelper.2
        ...
        public void onAdd(ArrayList<SemMediaResourceHelper.MediaResourceInfo> mediaResourceInfo) {
            if (mediaResourceInfo != null && mediaResourceInfo.size() > 0) {
                Log.m98i(DrmMediaResourceHelper.this.TAG, "onAdd size = " + mediaResourceInfo.size());
                for (int i = 0; i < mediaResourceInfo.size(); i++) {
                    int pid = mediaResourceInfo.get(i).getPid();
                    if (pid > 0 && mediaResourceInfo.get(i).isSecured() && !mediaResourceInfo.get(i).isEncoder() && !DrmMediaResourceHelper.this.isResourcePresent(pid)) {
                        ...
                        Log.m98i(DrmMediaResourceHelper.this.TAG, "onAdd ret of toggle CPU boost to ON =  " + ret);
                    }
                }
            }
        }
        ...
        public void onRemove(ArrayList<SemMediaResourceHelper.MediaResourceInfo> mediaResourceInfo) {
            if (mediaResourceInfo != null && mediaResourceInfo.size() == 0 && DrmMediaResourceHelper.this.BoostedPIDs.size() > 0) {
                Log.m98i(DrmMediaResourceHelper.this.TAG, "onRemove size = 0 (All resources removed)");
                ...
                while (i.hasNext()) {
                    ...
                    Log.m98i(DrmMediaResourceHelper.this.TAG, "onRemove toggle CPU boost to OFF for pid =  " + pid);
                }
                Log.m96e(DrmMediaResourceHelper.this.TAG, "onRemove making Dpdrm to 0  at first point ");
                DrmEventService.mDrmManagerClient.dpDRM(0);
                return;
            }
            if (mediaResourceInfo != null && mediaResourceInfo.size() > 0) {
                Log.m98i(DrmMediaResourceHelper.this.TAG, "onRemove size = " + mediaResourceInfo.size());
                ...
                for (int i2 = 0; i2 < DrmMediaResourceHelper.this.BoostedPIDs.size(); i2++) {
                    ...
                    if (!flag) {
                        ...
                        Log.m98i(DrmMediaResourceHelper.this.TAG, "onRemove toggle CPU boost to OFF for pid for multiple objs =  " + pid2);
                ...
                if (DrmMediaResourceHelper.this.BoostedPIDs.size() == 0) {
                    Log.m96e(DrmMediaResourceHelper.this.TAG, "onRemove making Dpdrm to 0 second point ");
                    DrmEventService.mDrmManagerClient.dpDRM(0);
                }
            }
        }
