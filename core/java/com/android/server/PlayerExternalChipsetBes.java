...
public abstract class PlayerExternalChipsetBes extends PlayerExternalChipsetBase {
    ...
    protected void initEndpointBes() {
        ...
        Log.m94d("FMRadioBestechnic", "interface Count - " + this.mUsbDevice.getInterfaceCount() + " End ID - " + this.mUsbDevice.getInterface(this.mUsbDevice.getInterfaceCount() - 1).getId());
        if (this.mUsbDevice.getInterface(this.mUsbDevice.getInterfaceCount() - 1).getId() == 4 && (usbInterface = this.mUsbDevice.getInterface(this.mUsbDevice.getInterfaceCount() - 1)) != null) {
            Log.m94d("FMRadioBestechnic", "claim HID " + usbInterface.toString());
            if (this.mUsbDeviceConnection.claimInterface(usbInterface, true)) {
                ...
                for (int j = 0; j < usbInterface.getEndpointCount(); j++) {
                    ...
                    switch (endpoint.getType()) {
                        case 3:
                            Log.m94d("FMRadioBestechnic", "found USB endpoint the Type is  USB_ENDPOINT_XFER_INT");
                            if (endpoint.getDirection() == 0) {
                                Log.m94d("FMRadioBestechnic", "found USB_DIR_OUT");
                                Log.m94d("FMRadioBestechnic", "endpoint.getEndpointNumber:" + endpoint.getEndpointNumber());
                                this.mUsbEndpoint = endpoint;
                                break;
                            } else if (endpoint.getDirection() == 128) {
                                Log.m94d("FMRadioBestechnic", "found USB_DIR_IN");
                                this.mUsbEndpoint = endpoint;
                                Log.m94d("FMRadioBestechnic", "endpoint.getEndpointNumber:" + endpoint.getEndpointNumber());
                                break;
                            } else {
            ...
            Log.m94d("FMRadioBestechnic", "Cannot claim interface");
        }
    }
    ...
    protected void startNotifyThreadBes() {
        if (this.notifyWorkerThread == null && this.mUsbEndpoint != null) {
            ...
            Log.m94d("FMRadioBestechnic", "start Notify Thread");
        }
    }
    ...
    protected void stopNotifyThreadBes() {
        ...
        if (notifyWorkerThread != null) {
            ...
            Log.m94d("FMRadioBestechnic", "Notify Thread is stopped");
        }
    }

    protected void releaseInterfaceBes() {
        Log.m94d("FMRadioBestechnic", "release()");
    ...
    private class NotifyWorkerThread extends Thread {
        ...
        private int verifyInfo(byte[] datas) {
            ...
            if (PlayerExternalChipsetBes.this.startsWith(datas, num) && PlayerExternalChipsetBes.this.mUsbDevice.getDeviceClass() != 2) {
                ...
                if (buffer != null) {
                    Log.m94d("FMRadioBestechnic", "buffer: " + PlayerExternalChipsetBes.this.toHex(buffer));
                }
                if (buffer == null) {
                    return 0;
                }
                Log.m94d("FMRadioBestechnic", "has result");
                if (PlayerExternalChipsetBes.this.startsWith(buffer, new byte[]{1, 1}) || PlayerExternalChipsetBes.this.startsWith(buffer, new byte[]{1, 0})) {
                    if (!PlayerExternalChipsetBes.this.startsWith(buffer, new byte[]{1, 0})) {
                        ...
                        Log.m94d("FMRadioBestechnic", "tune fail");
                    }
                    return 1;
                }
                if (PlayerExternalChipsetBes.this.startsWith(buffer, new byte[]{0, 1}) || PlayerExternalChipsetBes.this.startsWith(buffer, new byte[]{0, 0})) {
                    if (!PlayerExternalChipsetBes.this.startsWith(buffer, new byte[]{0, 0})) {
                        ...
                        Log.m94d("FMRadioBestechnic", "seek fail");
                    }
                    return 2;
                }
                if (PlayerExternalChipsetBes.this.startsWith(buffer, new byte[]{2}) || PlayerExternalChipsetBes.this.mIsRDSEnabled) {
                    ...
                    if (PlayerExternalChipsetBes.this.rdsParser.isRDSDataValid()) {
                        Log.m94d("FMRadioBestechnic", "RDSDataValid, PS: " + PlayerExternalChipsetBes.this.rdsParser.getProgramService() + " - RT: " + PlayerExternalChipsetBes.this.rdsParser.getRadioText());
                        ...
                        Log.m94d("FMRadioBestechnic", RDSdata.toString());
        ...
        public void run() {
            Log.m94d("FMRadioBestechnic", "notify thread is running");
            ...
            while (true) {
                if (PlayerExternalChipsetBes.this.mIsRDSEnabled || (PlayerExternalChipsetBes.this.mIsRunning && !Thread.currentThread().isInterrupted())) {
                    ...
                    if (ret2 < 0) {
                        ...
                        count = 0;
                        Log.m94d("FMRadioBestechnic", "Received NOTIFY: " + PlayerExternalChipsetBes.this.toHex(buffer));
                        int infoRet = verifyInfo(Arrays.copyOfRange(buffer, 0, ret2));
                        switch (infoRet) {
                            case 0:
                                Log.m94d("FMRadioBestechnic", "no result complete");
                                break;
                            case 1:
                            case 2:
                                Log.m94d("FMRadioBestechnic", "seek or tune complete");
                                PlayerExternalChipsetBes.this.mIsSeekTuneing = false;
                                break;
                            case 3:
                                Log.m94d("FMRadioBestechnic", "rds segment complete");
    ...
    protected void threadSleep(long ms) {
        try {
            ...
            Log.m94d("FMRadioBestechnic", "Thread sleep interrupted");
        }
    }
