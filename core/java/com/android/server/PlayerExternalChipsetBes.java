  ...
  protected void initEndpointBes() {
    ...
    Log.m94d(
        ...
        "interface Count - "
            ...
            + " End ID - "
            + this.mUsbDevice.getInterface(this.mUsbDevice.getInterfaceCount() - 1).getId());
    if (this.mUsbDevice.getInterface(this.mUsbDevice.getInterfaceCount() - 1).getId() == 4
        && (usbInterface = this.mUsbDevice.getInterface(this.mUsbDevice.getInterfaceCount() - 1))
            != null) {
      ...
      if (this.mUsbDeviceConnection.claimInterface(usbInterface, true)) {
        ...
        for (int j = 0; j < usbInterface.getEndpointCount(); j++) {
          ...
          switch (endpoint.getType()) {
            case 3:
              Log.m94d(
                  "FMRadioBestechnic", "found USB endpoint the Type is  USB_ENDPOINT_XFER_INT");
              if (endpoint.getDirection() == 0) {
                Log.m94d("FMRadioBestechnic", "found USB_DIR_OUT");
                Log.m94d(
                    "FMRadioBestechnic",
                    "endpoint.getEndpointNumber:" + endpoint.getEndpointNumber());
        ...
        if (PlayerExternalChipsetBes.this.mIsRDSEnabled
          ...
          int ret2 =
              PlayerExternalChipsetBes.this.mUsbDeviceConnection.bulkTransfer(
                  PlayerExternalChipsetBes.this.mUsbEndpoint, buffer, length, 1000);
          if (ret2 < 0) {
            count++;
