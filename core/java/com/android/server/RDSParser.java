  ...
  */
  public void parseData(ExtRDSData data) {
    ...
    Log(
        "Group code: "
            + (RDSGroupCode / 2)
            + (RDSGroupCode % 2 == 0 ? "A" : GnssSignalType.CODE_TYPE_B));
      ...
      case 1:
        if (data.blerd == 3) {
          ...
          for (int index = 0; index < 2; index++) {
            if (this.mProgramService.buffer_ps[PS_data_segment + index]
                != ((char) data.rdsd[index])) {
      ...
      case 5:
        ...
        for (int i = 0; i < data_segment_size; i++) {
          if (rds_data[i] == '\r' || rds_data[i] == 0) {
            ...
            Log.m94d("MonitorRDS", "Radio Text Buffer: " + new String(this.mRadioText.buffer_rt));
            ...
            if (!this.mRadioText.endReceived
                && this.mRadioText.receivedChar > this.mRadioText.length) {
              this.mRadioText.resetBuffer();
              break;
            }
          } else {
  ...
  private class RadioText {
    ...
    public void latch() {
      Log.m94d(RDSParser.TAG, "latch Radio Text");
    ...
    public boolean isRTValid(int maxLength) {
      return (RDSParser.this.mRadioText.endReceived
              && RDSParser.this.mRadioText.length == RDSParser.this.mRadioText.receivedChar)
