...
public final class MacAddressUtils {
    ...
    private static final long LOCALLY_ASSIGNED_MASK =
            longAddrFromByteAddr(MacAddress.fromString("2:0:0:0:0:0").toByteArray());
    private static final long MULTICAST_MASK =
            longAddrFromByteAddr(MacAddress.fromString("1:0:0:0:0:0").toByteArray());
    private static final long OUI_MASK =
            longAddrFromByteAddr(MacAddress.fromString("ff:ff:ff:0:0:0").toByteArray());
    private static final long NIC_MASK =
            longAddrFromByteAddr(MacAddress.fromString("0:0:0:ff:ff:ff").toByteArray());
    ...
    public static MacAddress createRandomUnicastAddress(MacAddress base, Random r) {
        ...
        if (base == null) {
            ...
            addr =
                    (longAddrFromByteAddr(base.toByteArray()) & OUI_MASK)
                            | (NIC_MASK & r.nextLong());
        }
        MacAddress mac =
                MacAddress.fromBytes(
                        byteAddrFromLongAddr((addr | LOCALLY_ASSIGNED_MASK) & (~MULTICAST_MASK)));
