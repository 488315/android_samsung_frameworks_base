...
public class LocationPermissionChecker {
  ...
  */
  public int checkLocationPermissionWithDetailInfo(
    ...
    switch (result) {
      case 1:
        Log.m96e(TAG, "Location mode is disabled for the device");
        break;
      case 2:
        Log.m96e(TAG, "UID " + uid + " has no location permission");
        break;
    }
  }
  ...
  public boolean checkCallersLocationPermission(
    ...
    boolean isFineLocationAllowed =
        noteAppOpAllowed(AppOpsManager.OPSTR_FINE_LOCATION, pkgName, featureId, uid, message);
  ...
  public boolean isLocationModeEnabled() {
    ...
    try {
      return LocationManager.isLocationEnabledForUser(UserHandle.m55of(getCurrentUser()));
    } catch (Exception e) {
      Log.m97e(TAG, "Failure to get location mode via API, falling back to settings", e);
      return false;
    }
  }
  ...
  private boolean isTargetSdkLessThan(String packageName, int versionCode, int callingUid) {
    ...
    try {
      if (this.mContext
              .getPackageManager()
              .getApplicationInfoAsUser(packageName, 0, UserHandle.getUserHandleForUid(callingUid))
              .targetSdkVersion
          < versionCode) {
