...
public final class IpRange {
  ...
  public IpRange(InetAddress startAddr, InetAddress endAddr) {
    ...
    if (addrToBigInteger(startAddr.getAddress()).compareTo(addrToBigInteger(endAddr.getAddress()))
        >= 0) {
  ...
  public List<IpPrefix> asIpPrefixes() {
    ...
    workingSet.add(
        new IpPrefix(isIpv6 ? getAsInetAddress(new byte[16]) : getAsInetAddress(new byte[4]), 0));
  ...
  public boolean overlapsRange(IpRange other) {
    return addrToBigInteger(this.mStartAddr).compareTo(addrToBigInteger(other.mEndAddr)) <= 0
        && addrToBigInteger(other.mStartAddr).compareTo(addrToBigInteger(this.mEndAddr)) <= 0;
  }
