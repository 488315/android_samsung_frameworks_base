...
public class BackdropFrameRenderer extends Thread implements Choreographer.FrameCallback {
  ...
  public BackdropFrameRenderer(
    ...
    onResourcesLoaded(
        decorView,
        resizingBackgroundDrawable,
        captionBackgroundDrawable,
  ...
  private void doFrameUncheckedLocked() {
    ...
    if (!this.mNewTargetRect.equals(this.mOldTargetRect)
        || this.mOldFullscreen != this.mFullscreen
        || !this.mSystemBarInsets.equals(this.mOldSystemBarInsets)
        || this.mReportNextDraw) {
      this.mOldFullscreen = this.mFullscreen;
  ...
  private void redrawLocked(Rect newBounds, boolean fullscreen) {
    ...
    if ((this.mLastCaptionHeight == 0 && this.mDecorView.isShowingCaption())
        || this.mLastContentWidth == 0
