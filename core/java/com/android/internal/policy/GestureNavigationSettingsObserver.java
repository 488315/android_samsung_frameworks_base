...
public class GestureNavigationSettingsObserver extends ContentObserver {
  ...
  public GestureNavigationSettingsObserver(
      Handler handler, Context context, Runnable onChangeRunnable) {
    super(handler);
        ...
        new DeviceConfig
          ...
          public void onPropertiesChanged(DeviceConfig.Properties properties) {
            if ("systemui".equals(properties.getNamespace())
                && GestureNavigationSettingsObserver.this.mOnChangeRunnable != null) {
  ...
  public void register() {
    ...
    r.registerContentObserver(
        Settings.Secure.getUriFor(Settings.Secure.BACK_GESTURE_INSET_SCALE_LEFT), false, this, -1);
    r.registerContentObserver(
        Settings.Secure.getUriFor(Settings.Secure.BACK_GESTURE_INSET_SCALE_RIGHT), false, this, -1);
    r.registerContentObserver(
        Settings.Secure.getUriFor(Settings.Secure.USER_SETUP_COMPLETE), false, this, -1);
    r.registerContentObserver(
        Settings.Global.getUriFor(Settings.Global.BOTTOM_GESTURE_INSET_SCALE), false, this, -1);
    r.registerContentObserver(
        Settings.Global.getUriFor(Settings.Global.NAVIGATIONBAR_GESTURES_DETAIL_TYPE),
        false,
        this,
        -1);
    DeviceConfig.addOnPropertiesChangedListener(
        ...
        new Executor() { // from class:
                         // com.android.internal.policy.GestureNavigationSettingsObserver$$ExternalSyntheticLambda1
  ...
  public void registerForCallingUser() {
    ContentResolver r = this.mContext.getContentResolver();
    r.registerContentObserver(
        Settings.Secure.getUriFor(Settings.Secure.BACK_GESTURE_INSET_SCALE_LEFT), false, this);
    r.registerContentObserver(
        Settings.Secure.getUriFor(Settings.Secure.BACK_GESTURE_INSET_SCALE_RIGHT), false, this);
    r.registerContentObserver(
        Settings.Secure.getUriFor(Settings.Secure.USER_SETUP_COMPLETE), false, this);
    DeviceConfig.addOnPropertiesChangedListener(
        "systemui",
        new Executor() { // from class:
  ...
  private float getUnscaledInset(Resources userRes) {
    ...
    float defaultInset = userRes.getDimension(C4337R.dimen.config_backGestureInset) / dm.density;
    if (defaultInset > 0.0f) {
      backGestureInset =
          DeviceConfig.getFloat(
              "systemui", SystemUiDeviceConfigFlags.BACK_GESTURE_EDGE_WIDTH, defaultInset);
    } else {
  ...
  public int getBottomSensitivity(Resources userRes) {
    ...
    if (isButtonMode || gestureHint) {
      return userRes.getDimensionPixelSize(C4337R.dimen.navigation_bar_gesture_height);
    }
    DisplayMetrics dm = userRes.getDisplayMetrics();
    float bottomInset =
        userRes.getDimension(C4337R.dimen.navigation_bar_gesture_height) / dm.density;
    float inset = TypedValue.applyDimension(1, bottomInset, dm);
    float scale =
        Settings.Global.getFloat(
            this.mContext.getContentResolver(), Settings.Global.BOTTOM_GESTURE_INSET_SCALE, 1.0f);
