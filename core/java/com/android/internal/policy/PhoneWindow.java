...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
  ...
  private static final Window.OnContentApplyWindowInsetsListener sDefaultContentInsetsApplier =
      new Window
          .OnContentApplyWindowInsetsListener() { // from class:
  ...
  public PhoneWindow(Context context) {
    ...
    this.mAudioMode = 0;
    this.mUiOptions = 0;
    this.mInvalidatePanelMenuRunnable =
        new Runnable() { // from class: com.android.internal.policy.PhoneWindow.1
    ...
    this.mRenderShadowsInCompositor =
  ...
  public PhoneWindow(
    ...
    if (Settings.Global.getInt(
            context.getContentResolver(), Settings.Global.DEVELOPMENT_FORCE_RESIZABLE_ACTIVITIES, 0)
        != 0) {
      forceResizable = true;
    } else {
  ...
  public void addContentView(View view, ViewGroup.LayoutParams params) {
    ...
    if (hasFeature(12)) {
      Log.m100v(TAG, "addContentView does not support content transitions");
    }
    this.mContentParent.addView(view, params);
    this.mContentParent.requestApplyInsets();
    Window.Callback cb = getCallback();
  ...
  public final void openPanel(int featureId, KeyEvent event) {
    DecorContentParent decorContentParent;
    if (featureId == 0
        && (decorContentParent = this.mDecorContentParent) != null
  ...
  private void openPanel(PanelFeatureState st, KeyEvent event) {
    ...
    if (st.decorView == null || st.refreshDecorView) {
      ...
      width = -1;
        ...
        && (lp = st.createdPanelView.getLayoutParams()) != null
        && lp.width == -1) {
      width = -1;
    }
    if (!st.hasPanelItems()) {
    ...
    WindowManager.LayoutParams lp3 =
        new WindowManager.LayoutParams(
            width, -2, st.f1000x, st.f1001y, 1003, 8519680, st.decorView.mDefaultOpacity);
    if (st.isCompact) {
    ...
    if (this.mPanelChordingKey != 0) {
      ...
      if (playSoundEffect) {
        ...
        if (audioManager != null) {
          ...
          Log.m102w(TAG, "Couldn't get audio manager");
        }
      }
    }
  }
  ...
  private void reopenMenu(boolean toggleMenuMode) {
    ...
    if (decorContentParent != null
        && decorContentParent.canShowOverflowMenu()
        && (!ViewConfiguration.get(getContext()).hasPermanentMenuKey()
            || this.mDecorContentParent.isOverflowMenuShowPending())) {
      Window.Callback cb = getCallback();
      if (!this.mDecorContentParent.isOverflowMenuShowing() || !toggleMenuMode) {
        if (cb != null && !isDestroyed()) {
          ...
          if (st != null
              && st.menu != null
              && !st.refreshMenuContent
              && cb.onPreparePanel(0, st.createdPanelView, st.menu)) {
  ...
  protected boolean initializePanelDecor(PanelFeatureState st) {
    ...
    TypedArray a =
        getContext()
            .obtainStyledAttributes(null, C4337R.styleable.Window, 0, st.listPresenterTheme);
  ...
  private int getOptionsPanelGravity() {
    try {
      ...
      Log.m97e(TAG, "Couldn't getOptionsPanelGravity; using default", ex);
      return 81;
    }
  }
  ...
  void onOptionsPanelRotationChanged() {
    ...
    WindowManager.LayoutParams lp =
        st.decorView != null ? (WindowManager.LayoutParams) st.decorView.getLayoutParams() : null;
    if (lp != null) {
  ...
  protected void onIntChanged(int featureId, int value) {
    ...
    if (featureId == 2 || featureId == 5) {
      updateProgressBars(value);
        ...
        && (titleContainer = (FrameLayout) findViewById(C4337R.id.title_container)) != null) {
  ...
  private void updateProgressBars(int value) {
    ...
    if (value == -1) {
      if ((features & 4) != 0) {
        if (horizontalProgressBar != null) {
          ...
          Log.m96e(TAG, "Horizontal progress bar not located in current window decor");
        }
      }
      if ((features & 32) != 0) {
        if (circularProgressBar != null) {
          ...
          Log.m96e(TAG, "Circular progress bar not located in current window decor");
          return;
        }
      }
      return;
    ...
    if (20000 <= value && value <= 30000) {
      if (horizontalProgressBar != null) {
        horizontalProgressBar.setSecondaryProgress(
            value + TaskConstants.TASK_CHILD_LAYER_LETTERBOX_BACKGROUND);
      } else {
        Log.m96e(TAG, "Horizontal progress bar not located in current window decor");
      }
      showProgressBars(horizontalProgressBar, circularProgressBar);
  ...
  private void showProgressBars(ProgressBar horizontalProgressBar, ProgressBar spinnyProgressBar) {
    ...
    if ((features & 32) != 0
        ...
        && spinnyProgressBar.getVisibility() == 4) {
  ...
  private void hideProgressBars(ProgressBar horizontalProgressBar, ProgressBar spinnyProgressBar) {
    ...
    if ((features & 32) != 0
        && spinnyProgressBar != null
        && spinnyProgressBar.getVisibility() == 0) {
      spinnyProgressBar.startAnimation(anim);
      spinnyProgressBar.setVisibility(4);
    }
    if ((features & 4) != 0
        && horizontalProgressBar != null
        && horizontalProgressBar.getVisibility() == 0) {
    ...
    if (decorContentParent != null) {
      if (!decorContentParent.hasIcon() || (this.mResourcesSetFlags & 4) != 0) {
        if (resId != 0) {
          ...
          this.mDecorContentParent.setIcon(
  ...
  protected boolean onKeyDown(int featureId, int keyCode, KeyEvent event) {
    ...
    KeyEvent.DispatcherState dispatcher =
        decorView != null ? decorView.getKeyDispatcherState() : null;
    switch (keyCode) {
      case 4:
        ...
        if (this.mMediaController != null && !isActivePhoneCallOngoing()) {
          getMediaSessionManager()
              .dispatchVolumeKeyEventToSessionAsSystemService(
                  event, this.mMediaController.getSessionToken());
        } else {
          getMediaSessionManager()
              .dispatchVolumeKeyEventAsSystemService(event, this.mVolumeControlStreamType);
        }
        return true;
      case 79:
      case 85:
    ...
    switch (keyCode) {
      case 4:
        if (featureId >= 0 && event.isTracking() && !event.isCanceled()) {
          if (featureId == 0
              && (st = getPanelState(featureId, false)) != null
              && st.isInExpandedMode) {
            reopenMenu(true);
            return true;
          }
      ...
      case 84:
        if (!isNotInstantAppAndKeyguardRestricted()
            && (getContext().getResources().getConfiguration().uiMode & 15) != 6) {
  ...
  public void restoreHierarchyState(Bundle savedInstanceState) {
    ...
    if (focusedViewId != -1) {
      ...
      if (needsFocus == null) {
        Log.m102w(
            TAG,
            "Previously focused view reported id "
                + focusedViewId
    ...
    if (this.mDecorContentParent != null) {
      ...
      if (actionBarStates == null) {
        Log.m102w(
            TAG, "Missing saved instance states for action bar views! State will not be restored.");
      } else {
  ...
  private class PanelMenuPresenterCallback implements MenuPresenter.Callback {
    ...
    public boolean onOpenSubMenu(MenuBuilder subMenu) {
      Window.Callback cb;
      if (subMenu == null
          && PhoneWindow.this.hasFeature(8)
          && (cb = PhoneWindow.this.getCallback()) != null
          && !PhoneWindow.this.isDestroyed()) {
  ...
  */
  protected ViewGroup generateLayout(DecorView decor) {
    ...
    if (a.hasValue(66)) {
      ...
      a.getValue(66, this.mFixedWidthMajor);
    }
    if (a.hasValue(67)) {
      if (this.mFixedWidthMinor == null) {
        this.mFixedWidthMinor = new TypedValue();
      }
      a.getValue(67, this.mFixedWidthMinor);
    }
    if (a.hasValue(64)) {
      if (this.mFixedHeightMajor == null) {
        this.mFixedHeightMajor = new TypedValue();
      }
      a.getValue(64, this.mFixedHeightMajor);
    }
    if (a.hasValue(65)) {
      if (this.mFixedHeightMinor == null) {
        this.mFixedHeightMinor = new TypedValue();
      }
      a.getValue(65, this.mFixedHeightMinor);
    }
    ...
    if (!this.mForcedNavigationBarColor) {
      int navBarCompatibleColor = context.getColor(C4337R.color.navigation_bar_compatible);
      int navBarDefaultColor = context.getColor(C4337R.color.navigation_bar_default);
      int navBarColor = a.getColor(35, navBarDefaultColor);
      if (navBarColor == navBarDefaultColor
          && !context.getResources().getBoolean(C4337R.bool.config_navBarDefaultTransparent)) {
    ...
    boolean appUseDeviceDefault =
        this.mDeviceDefaultNavigationBarColor == appColor
            || context.getColor(C4337R.color.navigation_bar_default) == appColor;
    int defaultNavigationBarColor = getDefaultNavigationBarColor();
    this.mDefaultNavigationBarColor = defaultNavigationBarColor;
    if (!this.mForcedNavigationBarColor) {
      this.mNavigationBarColor = appUseDeviceDefault ? defaultNavigationBarColor : appColor;
    }
    int i2 = this.mNavigationBarColor;
    if (i2 == defaultNavigationBarColor && needLightNavigationBar(i2)) {
    ...
    if (!this.mIsFloating) {
      ...
      if (!CoreRune.MW_FREEFORM_SHELL_TRANSITION
          || (configuration = this.mActivityCurrentConfig) == null
    ...
    try {
      if (context.getResources().getAssets().getSamsungThemeOverlays().size() > 0) {
        ...
        try {
          context
              .getTheme()
              .resolveAttribute(C4337R.attr.parentIsDeviceDefault, isDeviceDefault, false);
          if (isDeviceDefault.data != 0 && a.getBoolean(45, true) && this.mStatusBarColor != 0) {
            if (context.getResources().getBoolean(C4337R.bool.sem_window_light_status_bar)) {
            ...
            this.mStatusBarColor =
                context.getResources().getColor(C4337R.color.tw_status_bar_color);
          }
        } catch (Exception e) {
          e = e;
          e.printStackTrace();
          ...
          contentParent = (ViewGroup) findViewById(16908290);
          if (contentParent != null) {}
        }
      }
      if (context.getResources().getAssets().getSamsungThemeOverlays().size() > 0
          || WallpaperThemeUtils.hasWallpaperThemeOverlays(context)) {
        this.mThemeApplied = true;
        int themeColor = getDefaultNavigationBarColor();
        if (this.mNavigationBarColor == this.mDefaultNavigationBarColor
            && !this.mForcedNavigationBarColor) {
    ...
    if (a.hasValue(50)) {
      ...
      if (mode < 0 || mode > 3) {
        throw new UnsupportedOperationException(
            "Unknown windowLayoutInDisplayCutoutMode: " + a.getString(50));
      }
      params.layoutInDisplayCutoutMode = mode;
    }
    if ((!this.mAlwaysReadCloseOnTouchAttr
            || getContext().getApplicationInfo().targetSdkVersion >= 11)
        && a.getBoolean(21, false)) {
      setCloseOnTouchOutsideIfNotSet(true);
    }
    if (!hasSoftInputMode()) {
    ...
    if (getContainer() == null) {
      ...
      if (this.mLoadElevation) {
        ...
        } else if (!CoreRune.MW_SHELL_FREEFORM_SHADOW_WITH_VIEW_ELEVATION
            || !this.mDecor.isNonFullscreenWindowInFreeform()) {
    ...
    if ((features & 24) == 0) {
      if (this.mIsFloating) {
        ...
        getContext()
            .getTheme()
            .resolveAttribute(C4337R.attr.dialogTitleIconsDecorLayout, res, true);
        layoutResource = res.resourceId;
      } else {
        layoutResource = C4337R.layout.screen_title_icons;
      }
      removeFeature(8);
    } else {
      int layoutResource2 = features & 36;
      if (layoutResource2 != 0 && (features & 256) == 0) {
        layoutResource = C4337R.layout.screen_progress;
      } else {
        ...
        if (layoutResource3 != 0) {
          if (this.mIsFloating) {
            ...
            getContext()
                .getTheme()
                .resolveAttribute(C4337R.attr.dialogCustomTitleDecorLayout, res2, true);
            layoutResource = res2.resourceId;
          } else {
            layoutResource = C4337R.layout.screen_custom_title;
          }
          removeFeature(8);
        } else {
          ...
          if (layoutResource4 == 0) {
            if (this.mIsFloating) {
              ...
              getContext()
                  .getTheme()
                  .resolveAttribute(C4337R.attr.dialogTitleDecorLayout, res3, true);
              layoutResource = res3.resourceId;
            } else {
              int layoutResource5 = features & 256;
              if (layoutResource5 != 0) {
                layoutResource = a.getResourceId(63, C4337R.layout.screen_action_bar);
              } else {
                layoutResource = C4337R.layout.screen_title;
              }
            }
          } else {
            int layoutResource6 = features & 1024;
            if (layoutResource6 != 0) {
              layoutResource = C4337R.layout.screen_simple_overlay_action_mode;
            } else {
              layoutResource = C4337R.layout.screen_simple;
            }
          }
        }
      }
  ...
  private void installDecor() {
    ...
    if (this.mContentParent == null) {
      ...
      DecorContentParent decorContentParent =
          (DecorContentParent) this.mDecor.findViewById(C4337R.id.decor_content_parent);
      if (decorContentParent != null) {
        ...
        if ((this.mResourcesSetFlags & 1) != 0
            || (this.mIconRes != 0 && !this.mDecorContentParent.hasIcon())) {
          this.mDecorContentParent.setIcon(this.mIconRes);
        } else if ((this.mResourcesSetFlags & 1) == 0
            && this.mIconRes == 0
            && !this.mDecorContentParent.hasIcon()) {
          this.mDecorContentParent.setIcon(
              getContext().getPackageManager().getDefaultActivityIcon());
          this.mResourcesSetFlags |= 4;
        }
        if ((this.mResourcesSetFlags & 2) != 0
            || (this.mLogoRes != 0 && !this.mDecorContentParent.hasLogo())) {
        ...
        if (textView != null) {
          if ((getLocalFeatures() & 2) != 0) {
            View titleContainer = findViewById(C4337R.id.title_container);
      ...
      if (this.mDecor.getBackground() == null
          && (drawable = this.mBackgroundFallbackDrawable) != null) {
      ...
      if (hasFeature(13)) {
        if (this.mTransitionManager == null) {
          ...
          if (transitionRes != 0) {
            ...
            this.mTransitionManager =
                inflater.inflateTransitionManager(transitionRes, this.mContentParent);
          } else {
        ...
        this.mSharedElementEnterTransition =
            getTransition(this.mSharedElementEnterTransition, null, 29);
        this.mSharedElementReturnTransition =
            getTransition(this.mSharedElementReturnTransition, transition2, 41);
        this.mSharedElementExitTransition =
            getTransition(this.mSharedElementExitTransition, null, 30);
        this.mSharedElementReenterTransition =
            getTransition(this.mSharedElementReenterTransition, transition2, 42);
        if (this.mAllowEnterTransitionOverlap == null) {
          this.mAllowEnterTransitionOverlap =
              Boolean.valueOf(getWindowStyle().getBoolean(32, true));
        }
        if (this.mAllowReturnTransitionOverlap == null) {
          this.mAllowReturnTransitionOverlap =
              Boolean.valueOf(getWindowStyle().getBoolean(31, true));
        }
  ...
  private Drawable loadImageURI(Uri uri) {
    try {
      ...
      Log.m102w(TAG, "Unable to open content: " + uri);
      return null;
    }
  }
  ...
  private PanelFeatureState getPanelState(
      int featureId, boolean required, PanelFeatureState convertPanelState) {
  ...
  private ImageView getLeftIconView() {
    ...
    ImageView imageView2 = (ImageView) findViewById(C4337R.id.left_icon);
  ...
  private ProgressBar getCircularProgressBar(boolean shouldInstallDecor) {
    ...
    ProgressBar progressBar2 = (ProgressBar) findViewById(C4337R.id.progress_circular);
  ...
  private ProgressBar getHorizontalProgressBar(boolean shouldInstallDecor) {
    ...
    ProgressBar progressBar2 = (ProgressBar) findViewById(C4337R.id.progress_horizontal);
  ...
  private ImageView getRightIconView() {
    ...
    ImageView imageView2 = (ImageView) findViewById(C4337R.id.right_icon);
  ...
  private boolean isTvUserSetupComplete() {
    boolean isTvSetupComplete =
        Settings.Secure.getInt(
                getContext().getContentResolver(), Settings.Secure.USER_SETUP_COMPLETE, 0)
            != 0;
    return isTvSetupComplete
        & (Settings.Secure.getInt(
                getContext().getContentResolver(), Settings.Secure.TV_USER_SETUP_COMPLETE, 0)
            != 0);
  }
    ...
    if (cb == null || isDestroyed()) {
      ...
      try {
        ...
        Log.m97e(
            ...
            "WindowCallback "
                + cb.getClass().getName()
                + " does not implement method onSearchRequested(SearchEvent); fa",
            e);
  ...
  public void setMediaController(MediaController controller) {
    ...
    if (controller != null && this.mOnModeChangedListener == null) {
      ...
      this.mOnModeChangedListener =
          new AudioManager
              .OnModeChangedListener() { // from class:
                                         // com.android.internal.policy.PhoneWindow$$ExternalSyntheticLambda1
      ...
      getAudioManager()
          .addOnModeChangedListener(getContext().getMainExecutor(), this.mOnModeChangedListener);
  ...
  static final class PanelFeatureState {
    ...
    public boolean hasPanelItems() {
      ...
      return (this.isCompact || this.isInExpandedMode)
          ? this.listMenuPresenter.getAdapter().getCount() > 0
    ...
    void setStyle(Context context) {
      TypedArray a = context.obtainStyledAttributes(C4337R.styleable.Theme);
      ...
      this.listPresenterTheme = a.getResourceId(377, C4337R.style.Theme_ExpandedMenu);
      a.recycle();
    }
    ...
    MenuView getListMenuView(Context context, MenuPresenter.Callback cb) {
      ...
      if (this.listMenuPresenter == null) {
        ListMenuPresenter listMenuPresenter =
            new ListMenuPresenter(C4337R.layout.list_menu_item_layout, this.listPresenterTheme);
        this.listMenuPresenter = listMenuPresenter;
        listMenuPresenter.setCallback(cb);
        this.listMenuPresenter.setId(C4337R.id.list_menu_presenter);
    ...
    MenuView getIconMenuView(Context context, MenuPresenter.Callback cb) {
      ...
      if (this.iconMenuPresenter == null) {
        ...
        this.iconMenuPresenter.setId(C4337R.id.icon_menu_presenter);
  ...
  static class RotationWatcher extends IRotationWatcher.Stub {
    ...
    private final Runnable mRotationChanged =
        new Runnable() { // from class: com.android.internal.policy.PhoneWindow.RotationWatcher.1
          @Override // java.lang.Runnable
          public void run() {
    ...
    public void addWindow(PhoneWindow phoneWindow) {
      synchronized (this.mWindows) {
        if (!this.mIsWatching) {
          try {
            WindowManagerHolder.sWindowManager.watchRotation(
                this, phoneWindow.getContext().getDisplayId());
            this.mHandler = new Handler();
            this.mIsWatching = true;
          } catch (RemoteException ex) {
            Log.m97e(PhoneWindow.TAG, "Couldn't start watching for device rotation", ex);
          }
        }
  ...
  public static final class PhoneWindowMenuCallback
    ...
    public boolean onMenuItemSelected(MenuBuilder menu, MenuItem item) {
      ...
      return (callback == null
              ...
              || !callback.onMenuItemSelected(6, item))
          ? false
          : true;
    }
  ...
  void updateDeviceDefaultNavigationBarColor() {
    this.mDeviceDefaultNavigationBarColor =
        getContext().getResources().getColor(C4337R.color.navbar_light_theme_color, null);
  }
  ...
  public void updateDefaultNavigationBarColor() {
    ...
    boolean needUpdate =
        (i != this.mDefaultNavigationBarColor
                || i == defaultColor
                || this.mForcedNavigationBarColor)
            ? false
            : true;
  ...
  private int getDefaultNavigationBarColor() {
    if (this.mThemeApplied) {
      boolean useThemeDefault =
          Settings.Global.getInt(
                  getContext().getContentResolver(), "navigationbar_use_theme_default", 0)
              != 0;
      if (useThemeDefault) {
        return getContext().getResources().getColor(C4337R.color.tw_navigation_bar_color);
      }
    }
  ...
  private boolean needLightNavigationBar(int color) {
    if (this.mThemeApplied) {
      ...
      if (useThemeDefault) {
        boolean needLight =
            getContext().getResources().getBoolean(C4337R.bool.sem_window_light_navigation_bar);
        if (!needLight) {
  ...
  private void setForceLightNavigationBar(boolean enable) {
    ...
    if (oldValue != attr.samsungFlags) {
      Log.m94d(
          TAG,
          "forceLight changed to "
              + enable
              + " ["
              + ((Object) attr.getTitle())
              + "] from "
              + Debug.getCallers(5));
      setAttributes(attr);
      DecorView decorView = this.mDecor;
  ...
  public void setActivityCurrentConfig(Configuration config) {
    ...
    if (this.mDecor != null && oldWindowingMode != config.windowConfiguration.getWindowingMode()) {
      if (config.windowConfiguration.getWindowingMode() == 6
          && WindowConfiguration.isSplitScreenWindowingMode(config.windowConfiguration)) {
        split = true;
      }
