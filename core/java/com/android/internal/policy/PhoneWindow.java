...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    private void openPanel(PanelFeatureState st, KeyEvent event) {
        ...
        st.isHandled = false;
        WindowManager.LayoutParams lp3 = new WindowManager.LayoutParams(width, -2, st.f1000x, st.f1001y, 1003, 8519680, st.decorView.mDefaultOpacity);
        if (st.isCompact) {
    ...
    protected boolean initializePanelDecor(PanelFeatureState st) {
        ...
        TypedArray a = getContext().obtainStyledAttributes(null, C4337R.styleable.Window, 0, st.listPresenterTheme);
    ...
    protected void onIntChanged(int featureId, int value) {
        ...
        } else if (featureId == 7 && (titleContainer = (FrameLayout) findViewById(C4337R.id.title_container)) != null) {
    ...
    public void restoreHierarchyState(Bundle savedInstanceState) {
        ...
        if (focusedViewId != -1) {
            ...
            if (needsFocus == null) {
                Log.m102w(TAG, "Previously focused view reported id " + focusedViewId + " during save, but can't be found during restore.");
            } else {
        ...
        if (this.mDecorContentParent != null) {
            ...
            if (actionBarStates == null) {
                Log.m102w(TAG, "Missing saved instance states for action bar views! State will not be restored.");
            } else {
    ...
    */
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        if (!this.mForcedNavigationBarColor) {
            ...
            if (navBarColor == navBarDefaultColor && !context.getResources().getBoolean(C4337R.bool.config_navBarDefaultTransparent)) {
        ...
        boolean appUseDeviceDefault = this.mDeviceDefaultNavigationBarColor == appColor || context.getColor(C4337R.color.navigation_bar_default) == appColor;
        ...
        if (!this.mIsFloating) {
            ...
            if (!CoreRune.MW_FREEFORM_SHELL_TRANSITION || (configuration = this.mActivityCurrentConfig) == null || configuration.windowConfiguration.getWindowingMode() != 5) {
        ...
        try {
            if (context.getResources().getAssets().getSamsungThemeOverlays().size() > 0) {
                ...
                try {
                    context.getTheme().resolveAttribute(C4337R.attr.parentIsDeviceDefault, isDeviceDefault, false);
                    if (isDeviceDefault.data != 0 && a.getBoolean(45, true) && this.mStatusBarColor != 0) {
                        ...
                        this.mStatusBarColor = context.getResources().getColor(C4337R.color.tw_status_bar_color);
                    }
                } catch (Exception e) {
                    e = e;
                    e.printStackTrace();
        ...
        if ((features & 24) == 0) {
            if (this.mIsFloating) {
                ...
                getContext().getTheme().resolveAttribute(C4337R.attr.dialogTitleIconsDecorLayout, res, true);
            ...
            if (layoutResource2 != 0 && (features & 256) == 0) {
                ...
                if (layoutResource3 != 0) {
                    if (this.mIsFloating) {
                        ...
                        getContext().getTheme().resolveAttribute(C4337R.attr.dialogCustomTitleDecorLayout, res2, true);
                    ...
                    if (layoutResource4 == 0) {
                        if (this.mIsFloating) {
                            ...
                            getContext().getTheme().resolveAttribute(C4337R.attr.dialogTitleDecorLayout, res3, true);
    ...
    private void installDecor() {
        ...
        if (this.mContentParent == null) {
            ...
            DecorContentParent decorContentParent = (DecorContentParent) this.mDecor.findViewById(C4337R.id.decor_content_parent);
            if (decorContentParent != null) {
    ...
    private boolean launchDefaultSearch(KeyEvent event) {
        ...
        if (cb == null || isDestroyed()) {
            ...
            try {
                ...
                Log.m97e(TAG, "WindowCallback " + cb.getClass().getName() + " does not implement method onSearchRequested(SearchEvent); fa", e);
    ...
    static final class PanelFeatureState {
        ...
        MenuView getListMenuView(Context context, MenuPresenter.Callback cb) {
            ...
            if (this.listMenuPresenter == null) {
                ListMenuPresenter listMenuPresenter = new ListMenuPresenter(C4337R.layout.list_menu_item_layout, this.listPresenterTheme);
    ...
    void updateDeviceDefaultNavigationBarColor() {
        this.mDeviceDefaultNavigationBarColor = getContext().getResources().getColor(C4337R.color.navbar_light_theme_color, null);
    }
    ...
    private boolean needLightNavigationBar(int color) {
        if (this.mThemeApplied) {
            boolean useThemeDefault = Settings.Global.getInt(getContext().getContentResolver(), "navigationbar_use_theme_default", 0) != 0;
            if (useThemeDefault) {
                boolean needLight = getContext().getResources().getBoolean(C4337R.bool.sem_window_light_navigation_bar);
                if (!needLight) {
    ...
    private void setForceLightNavigationBar(boolean enable) {
        ...
        if (oldValue != attr.samsungFlags) {
            Log.m94d(TAG, "forceLight changed to " + enable + " [" + ((Object) attr.getTitle()) + "] from " + Debug.getCallers(5));
            setAttributes(attr);
            DecorView decorView = this.mDecor;
