...
public class DecorView extends FrameLayout implements RootViewSurfaceTaker, WindowCallbacks {
    ...
    DecorView(Context context, int featureId, PhoneWindow window, WindowManager.LayoutParams params) {
        ...
        this.FREEFORM_OUTLINE_PROVIDER = new ViewOutlineProvider() { // from class: com.android.internal.policy.DecorView.12
            ...
            public void getOutline(View view, Outline outline) {
                if (DecorView.this.isFreeformMode()) {
                    int radius = view.getResources().getDimensionPixelSize(C4337R.dimen.sem_decor_outline_radius_freeform);
        ...
        this.mBarEnterExitDuration = context.getResources().getInteger(C4337R.integer.dock_enter_exit_duration);
        if (!context.getResources().getBoolean(C4337R.bool.config_forceWindowDrawsStatusBarBackground) || params.type == 2011 || context.getApplicationInfo().targetSdkVersion < 24) {
            z = false;
        } else {
            z = true;
        }
        this.mForceWindowDrawsBarBackgrounds = z;
        this.mSemiTransparentBarColor = context.getResources().getColor(C4337R.color.system_bar_background_semi_transparent, null);
        setWindow(window);
        updateLogTag(params);
        ...
        this.mRoundedCornerRadiusForLetterBox = res.getDimensionPixelSize(C4337R.dimen.rounded_corner_radius_for_letterbox);
    ...
    public boolean gatherTransparentRegion(Region region) {
        ...
        if (this.mKnoxBadgeViewGroupOverlay != null) {
            region.m20op(this.mKnoxBadgeStartX, this.mKnoxBadgeStartY, getWidth(), getHeight(), Region.EnumC0836Op.DIFFERENCE);
        }
    ...
    */
    ...
    public void setWindowBackground(Drawable drawable) {
        ...
        Log.m98i(TAG, "setWindowBackground: isPopOver=" + this.mIsPopOver + " color=" + Integer.toHexString(this.mPopOverBackgroundColor) + " d=" + drawable);
    ...
    */
    WindowInsets updateColorViews(WindowInsets insets, boolean animate) {
        ...
        if (!this.mWindow.mIsFloating || isImeWindow) {
            ...
            if (insets != null) {
                ...
                boolean clearsCompatInsets = InsetsState.clearsCompatInsets(attrs.type, attrs.flags, getResources().getConfiguration().windowConfiguration.getActivityType(), this.mLastForceConsumingTypes);
                ...
                this.mKnoxBadgeInsets = insets.getInsets(WindowInsets.Type.systemBars() | WindowInsets.Type.displayCutout());
            ...
            updateColorViewInt(this.mNavigationColorViewState, calculateNavigationBarColor(appearance2), this.mWindow.mNavigationBarDividerColor, navBarSize, navBarToRightEdge || navBarToLeftEdge, navBarToLeftEdge, 0, animate && !disallowAnimate, this.mForceWindowDrawsBarBackgrounds, requestedVisibleTypes);
            boolean oldDrawLegacy = this.mDrawLegacyNavigationBarBackground;
            this.mDrawLegacyNavigationBarBackground = (this.mWindow.getAttributes().flags & Integer.MIN_VALUE) == 0;
            ...
            if (oldDrawLegacy == this.mDrawLegacyNavigationBarBackground) {
                ...
                updateColorViewInt(this.mStatusColorViewState, statusBarColor, 0, this.mLastTopInset, false, statusBarNeedsLeftInset, statusBarSideInset, (animate || disallowAnimate) ? false : true, this.mForceWindowDrawsBarBackgrounds, requestedVisibleTypes);
                if (!this.mHasCaption) {
            ...
            statusBarNeedsRightInset = !navBarToRightEdge && this.mNavigationColorViewState.present;
            boolean statusBarNeedsLeftInset2 = !navBarToLeftEdge && this.mNavigationColorViewState.present;
            if (!statusBarNeedsRightInset) {
            }
            int statusBarColor2 = calculateStatusBarColor(appearance2);
            updateColorViewInt(this.mStatusColorViewState, statusBarColor2, 0, this.mLastTopInset, false, statusBarNeedsLeftInset2, statusBarSideInset, (animate || disallowAnimate) ? false : true, this.mForceWindowDrawsBarBackgrounds, requestedVisibleTypes);
            if (!this.mHasCaption) {
            }
        } else {
            controller = controller2;
        ...
        boolean forceConsumingNavBar = ((this.mForceWindowDrawsBarBackgrounds || this.mDrawLegacyNavigationBarBackgroundHandled) && (attrs.flags & Integer.MIN_VALUE) == 0 && (sysUiVisibility & 512) == 0 && decorFitsSystemWindows && !hideNavigation) || ((this.mLastForceConsumingTypes & WindowInsets.Type.navigationBars()) != 0 && hideNavigation);
        ...
        boolean consumingStatusBar = ((sysUiVisibility & 1024) == 0 && decorFitsSystemWindows && (attrs.flags & 256) == 0 && (attrs.flags & 65536) == 0 && this.mForceWindowDrawsBarBackgrounds && this.mLastTopInset != 0) || ((this.mLastForceConsumingTypes & WindowInsets.Type.statusBars()) != 0 && fullscreen);
        ...
        if (CoreRune.MW_CAPTION_SHELL_INSETS) {
            ...
            if (!consumingStatusBar && shouldConsumeCaptionBar()) {
                ...
                captionBarBackgroundDrawableNeeded = (isOpaque() || Color.alpha(this.mWindow.mStatusBarColor) == 255) ? false : true;
            }
            if (!captionBarBackgroundDrawableNeeded) {
                ...
                if (colorDrawable != null) {
                    color = color2;
                    if (colorDrawable.getColor() == color && this.mCaptionBarBackgroundDrawable.getBounds().height() == consumedTop2) {
                        consumedTop = consumedTop2;
                    }
                } else {
                    color = color2;
                ...
                this.mCaptionBarBackgroundDrawable = new ColorDrawable(color) { // from class: com.android.internal.policy.DecorView.5
                ...
                if (CoreRune.SAFE_DEBUG) {
                    consumedTop = consumedTop2;
                    Log.m94d(this.mLogTag, "CaptionBarBackgroundDrawable is created. " + this + ", color=0x" + Integer.toHexString(color) + ", captionHeight=" + captionHeight);
                } else {
                    consumedTop = consumedTop2;
                }
            }
        ...
        if (viewGroup == null) {
            ...
            if (lp.topMargin == consumedTop2 && lp.rightMargin == consumedRight && lp.bottomMargin == consumedBottom && lp.leftMargin == consumedLeft) {
                insets2 = insets;
            } else {
        ...
        if (!forceConsumingNavBar || hideNavigation || this.mDrawLegacyNavigationBarBackgroundHandled) {
            ...
            this.mBackgroundInsets = Insets.m14of(this.mLastLeftInset, 0, this.mLastRightInset, this.mLastBottomInset);
        }
        updateBackgroundDrawable();
        return insets2;
    }
    ...
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    private void updateColorViewInt(ColorViewState state, int color, int dividerColor, int size, boolean verticalBar, boolean seascape, int sideMargin, boolean animate, boolean force, int requestedVisibleTypes) {
        int resolvedGravity;
        int vis;
        boolean visibilityChanged;
        int leftMargin;
        int type = state.attributes.insetsType;
        state.present = state.attributes.isPresent(((requestedVisibleTypes & type) == 0 && (this.mLastForceConsumingTypes & type) == 0) ? false : true, this.mWindow.getAttributes().flags, force);
        ...
        if (type == WindowInsets.Type.navigationBars()) {
            WindowManager.LayoutParams attrs = this.mWindow.getAttributes();
            if (this.mContext.getResources().getConfiguration().isDexMode() && attrs != null && (attrs.type == 2011 || attrs.type == 2021)) {
                showView = false;
            }
        }
        ...
        if (visibilityChanged2) {
            ...
            if (!animate || isResizing()) {
                ...
                view.animate().alpha(0.0f).setInterpolator(this.mHideInterpolator).setDuration(this.mBarEnterExitDuration).withEndAction(new Runnable() { // from class: com.android.internal.policy.DecorView.7
    ...
    private ActionMode createStandaloneActionMode(ActionMode.Callback callback) {
        ...
        if (actionBarContextView == null || !actionBarContextView.isAttachedToWindow()) {
            if (this.mWindow.isFloating()) {
                ...
                PopupWindow popupWindow = new PopupWindow(actionBarContext, (AttributeSet) null, C4337R.attr.actionModePopupWindowStyle);
    ...
    private void loadBackgroundDrawablesIfNeeded() {
        if (this.mResizingBackgroundDrawable == null) {
            ...
            if (resizingBackgroundDrawable == null) {
                Log.m102w(this.mLogTag, "Failed to find background drawable for PhoneWindow=" + this.mWindow);
            }
        }
        if (this.mCaptionBackgroundDrawable == null) {
            this.mCaptionBackgroundDrawable = getContext().getDrawable(C4337R.drawable.decor_caption_title_focused);
        }
    ...
    private DecorCaptionView inflateDecorCaptionView(LayoutInflater inflater) {
        ...
        DecorCaptionView view = (DecorCaptionView) inflater2.inflate(C4337R.layout.decor_caption, (ViewGroup) null);
    ...
    private void setLightDecorCaptionShade(DecorCaptionView view) {
        view.findViewById(C4337R.id.maximize_window).setBackgroundResource(C4337R.drawable.decor_maximize_button_light);
        view.findViewById(C4337R.id.close_window).setBackgroundResource(C4337R.drawable.decor_close_button_light);
    }

    private void setDarkDecorCaptionShade(DecorCaptionView view) {
        view.findViewById(C4337R.id.maximize_window).setBackgroundResource(C4337R.drawable.decor_maximize_button_dark);
        view.findViewById(C4337R.id.close_window).setBackgroundResource(C4337R.drawable.decor_close_button_dark);
    }
        ...
        public void onDestroyActionMode(ActionMode mode) {
            ...
            if (isMncApp) {
                ...
                if (!isPrimary && mode.getType() == 0) {
                    Log.m96e(DecorView.this.mLogTag, "Destroying unexpected ActionMode instance of TYPE_PRIMARY; " + mode + " was not the current primary action mode! Expected " + DecorView.this.mPrimaryActionMode);
                }
                if (!isFloating && mode.getType() == 1) {
                    Log.m96e(DecorView.this.mLogTag, "Destroying unexpected ActionMode instance of TYPE_FLOATING; " + mode + " was not the current floating action mode! Expected " + DecorView.this.mFloatingActionMode);
                }
            } else {
