...
public class DividerSnapAlgorithm {
    ...
    public static DividerSnapAlgorithm create(Context ctx, Rect insets) {
        ...
        int dividerWindowWidth = ctx.getResources().getDimensionPixelSize(C4337R.dimen.docked_stack_divider_thickness);
        int dividerInsets = ctx.getResources().getDimensionPixelSize(C4337R.dimen.docked_stack_divider_insets);
        if (CoreRune.MW_MULTI_SPLIT_DIVIDER) {
            dividerWindowWidth = ctx.getResources().getDimensionPixelSize(C4337R.dimen.multi_split_screen_divider_thickness);
            if (CoreRune.MW_MULTI_SPLIT_DIVIDER_SIZE_FOLD) {
                dividerInsets = ctx.getResources().getDimensionPixelSize(C4337R.dimen.multi_split_screen_divider_insets_fold);
            } else {
                dividerInsets = ctx.getResources().getDimensionPixelSize(C4337R.dimen.multi_split_screen_divider_insets);
            }
        }
    ...
    public DividerSnapAlgorithm(Resources res, int displayWidth, int displayHeight, int dividerSize, boolean isHorizontalDivision, Rect insets, int dockSide, boolean isMinimizedMode, boolean isHomeResizable) {
        ...
        this.mMinimalSizeResizableTask = res.getDimensionPixelSize(C4337R.dimen.default_minimal_size_resizable_task);
        this.mTaskHeightInMinimizedMode = isHomeResizable ? res.getDimensionPixelSize(C4337R.dimen.task_height_of_minimized_mode) : 0;
        this.mDensityDpi = res.getConfiguration().densityDpi;
        this.mMultiSplitMinimalSize = res.getDimensionPixelSize(C4337R.dimen.multi_split_minimal_size_resizable_task);
        this.mMinimumSplitTargetGap = res.getDimensionPixelSize(C4337R.dimen.multi_split_minimum_target_gap);
