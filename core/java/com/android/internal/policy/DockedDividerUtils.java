...
public class DockedDividerUtils {
  ...
  public static void calculateBoundsForPosition(
      int position,
      int dockSide,
      Rect outRect,
      int displayWidth,
    ...
    calculateBoundsForPosition(
        position, dockSide, outRect, displayWidth, displayHeight, dividerSize, null);
  }
  ...
  public static void calculateBoundsForCellWithPosition(
      int position, int dockSide, Rect outRect, Rect splitArea, int dividerSize) {
  ...
  public static int calculateMiddlePosition(
  ...
  public static int getDividerInsets(Resources res) {
    return res.getDimensionPixelSize(C4337R.dimen.docked_stack_divider_insets);
  }

  public static int getDividerSize(Resources res, int dividerInsets) {
    int windowWidth = res.getDimensionPixelSize(C4337R.dimen.docked_stack_divider_thickness);
  ...
  public static float calculateSplitRatio(
      int dividerSize, Rect main, Rect splitArea, Rect applyInsets) {
    ...
    int positionInDisplay =
        calculatePositionForBounds(main, dockSide, dividerSize)
            - (isHorizontalDivision(dockSide) ? stableBounds.top : stableBounds.left);
    return positionInDisplay
        / ((isHorizontalDivision(dockSide) ? stableBounds.height() : splitArea.width())
            - dividerSize);
  }
}
