...
public class EnergyConsumerStats {
  ...
  public static class Config {
    ...
    public Config(
      ...
      this.mSupportedMultiStateBuckets =
          new boolean[supportedStandardBuckets.length + strArr.length];
    ...
    public boolean isCompatible(Config other) {
      return Arrays.equals(this.mSupportedStandardBuckets, other.mSupportedStandardBuckets)
          ...
          && Arrays.equals(this.mSupportedMultiStateBuckets, other.mSupportedMultiStateBuckets)
    ...
    public static Config createFromParcel(Parcel in) {
      ...
      return new Config(
          supportedStandardBuckets, customBucketNames, supportedMultiStateBuckets, stateNames);
    }
    ...
    for (int entry = 0; entry < numWrittenEntries; entry++) {
      ...
      if (in.readBoolean()) {
        ...
        if (this.mConfig == null
            || multiStateCounter.getStateCount() != this.mConfig.getStateNames().length) {
  ...
  public void updateCustomBucket(int customBucket, long chargeDeltaUC, long timestampMs) {
    if (!isValidCustomBucket(customBucket)) {
      Slog.m115e(TAG, "Attempted to update invalid custom bucket " + customBucket);
    } else {
  ...
  private void reset() {
    ...
    for (int index = 0; index < numIndices; index++) {
      ...
      LongMultiStateCounter[] longMultiStateCounterArr =
          this.mAccumulatedMultiStateChargeMicroCoulomb;
      if (longMultiStateCounterArr != null
          && (longMultiStateCounter = longMultiStateCounterArr[index]) != null) {
