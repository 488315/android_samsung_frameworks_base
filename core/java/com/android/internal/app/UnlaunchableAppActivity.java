...
public class UnlaunchableAppActivity extends Activity
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (this.mUserId != -10000) {
      ...
      if (SemPersonaManager.isSecureFolderId(this.mUserId)) {
        ...
        i = C4337R.string.secure_folder_decrypt_btn;
      } else {
        resources = getResources();
        i = C4337R.string.work_mode_turn_on_btn;
      }
      ...
      if (targetPackageName != null
          && targetPackageName.equals(
              this.mTelecomManager.getDefaultDialerPackage(UserHandle.m55of(this.mUserId)))) {
        showEmergencyCallButton = true;
      } else {
        showEmergencyCallButton = false;
      }
      if (showEmergencyCallButton) {
        builder = new AlertDialog.Builder(this, C4337R.style.AlertDialogWithEmergencyButton);
        builder.setNeutralButton(C4337R.string.work_mode_emergency_call_button, this);
      } else {
      ...
      dialog.create();
      if (showEmergencyCallButton) {
        dialog.getWindow().findViewById(C4337R.id.parentPanel).setPadding(0, 0, 0, 30);
  ...
  private String getDialogTitle() {
    if (SemPersonaManager.isSecureFolderId(this.mUserId)) {
      return getResources().getString(C4337R.string.secure_folder_decrypt_title);
    }
  ...
  public /* synthetic */ String lambda$getDialogTitle$0() {
    return getString(C4337R.string.work_mode_off_title_alert, getWorkProfileName());
  }

  private String getDialogMessage() {
    if (SemPersonaManager.isSecureFolderId(this.mUserId)) {
      return getResources().getString(C4337R.string.secure_folder_decrypt_message);
    }
    return getResources().getString(C4337R.string.work_profile_on_message);
  }
  ...
  public /* synthetic */ void lambda$onClick$1(UserManager userManager) {
    userManager.requestQuietModeEnabled(false, UserHandle.m55of(this.mUserId), this.mTarget);
  }
  ...
  public static Intent createInQuietModeDialogIntent(
    ...
    if (resolveInfo != null) {
      intent.putExtra(
          "android.intent.extra.PACKAGE_NAME", resolveInfo.getComponentInfo().packageName);
    }
    return intent;
  }
}
