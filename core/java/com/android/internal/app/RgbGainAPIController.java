...
public final class RgbGainAPIController {
            ...
            new Handler(
                Looper
                    .getMainLooper())) { // from class:
                                         // com.android.internal.app.RgbGainAPIController.1
  ...
  public boolean isActivated() {
    return Settings.Secure.getIntForUser(
            ...
            Settings.Secure.RGB_GAIN_API_DISPLAY_ACTIVATED,
            0,
            this.mUserId)
        == 1;
  }
  ...
  public int getRgbGainRedLevel() {
    int level =
        Settings.Secure.getIntForUser(
            this.mContext.getContentResolver(),
            Settings.Secure.RGB_GAIN_API_DISPLAY_RED_LEVEL,
            -1,
            this.mUserId);
    if (level == -1) {
      Slog.m113d(TAG, "Using default value for setting: rgb_gain_api_display_red_level");
  ...
  public int getRgbGainGreenLevel() {
    int level =
        Settings.Secure.getIntForUser(
            this.mContext.getContentResolver(),
            Settings.Secure.RGB_GAIN_API_DISPLAY_GREEN_LEVEL,
            -1,
            this.mUserId);
    if (level == -1) {
      Slog.m113d(TAG, "Using default value for setting: rgb_gain_api_display_green_level");
      return getDefaultRgbGainLevel();
    }
    return level;
  }
  ...
  public int getRgbGainBlueLevel() {
    int level =
        Settings.Secure.getIntForUser(
            this.mContext.getContentResolver(),
            Settings.Secure.RGB_GAIN_API_DISPLAY_BLUE_LEVEL,
            -1,
            this.mUserId);
    if (level == -1) {
      Slog.m113d(TAG, "Using default value for setting: rgb_gain_api_display_blue_level");
      return getDefaultRgbGainLevel();
    }
    return level;
  }
  ...
  public void onSettingChanged(String setting) {
    char c;
    Slog.m113d(TAG, "onSettingChanged: " + setting);
    ...
    if (oldCallback != callback) {
      ...
      if (oldCallback == null) {
        ...
        cr.registerContentObserver(
            Settings.Secure.getUriFor(Settings.Secure.RGB_GAIN_API_DISPLAY_ACTIVATED),
            false,
            this.mContentObserver,
            this.mUserId);
        cr.registerContentObserver(
            Settings.Secure.getUriFor(Settings.Secure.RGB_GAIN_API_DISPLAY_RED_LEVEL),
            false,
            this.mContentObserver,
            this.mUserId);
        cr.registerContentObserver(
            Settings.Secure.getUriFor(Settings.Secure.RGB_GAIN_API_DISPLAY_GREEN_LEVEL),
            false,
            this.mContentObserver,
            this.mUserId);
        cr.registerContentObserver(
            Settings.Secure.getUriFor(Settings.Secure.RGB_GAIN_API_DISPLAY_BLUE_LEVEL),
            false,
            this.mContentObserver,
            this.mUserId);
      }
