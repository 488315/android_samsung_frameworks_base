...
public final class ProcessState {
  ...
  static final int[] PROCESS_STATE_TO_STATE = {
    0, 0, 1, 2, 3, 4, 5, 6, 6, 7, 8, 10, 1, 11, 12, 13, 14, 14, 14, 14
  };
  public static final Comparator<ProcessState> COMPARATOR =
      new Comparator<
          ProcessState>() { // from class: com.android.internal.app.procstats.ProcessState.1
  ...
  public ProcessState(
      ProcessState commonProcess, String pkg, int uid, long vers, String name, long now) {
    this.mTotalRunningPss = new long[10];
    this.mCurCombinedState = -1;
    this.mLastPssState = -1;
  ...
  public ProcessState clone(long now) {
    ProcessState pnew =
        new ProcessState(this, this.mPackage, this.mUid, this.mVersion, this.mName, now);
    ...
    if (numOfSources > 0) {
      ...
      for (int i2 = 0; i2 < numOfSources; i2++) {
        ...
        AssociationState.SourceState src =
            new AssociationState.SourceState(this.mStats, null, this, key);
  ...
  public boolean hasAnyData() {
    return (this.mDurations.getKeyCount() == 0
            && this.mCurCombinedState == -1
            && this.mPssTable.getKeyCount() == 0
            && this.mTotalRunningPss[0] == 0)
        ? false
        : true;
  }
  ...
  public void decActiveServices(String serviceName) {
    ...
    if (i < 0) {
      Slog.wtfStack(
          "ProcessStats",
          "Proc active services underrun: pkg="
              + this.mPackage
              + " uid="
              + this.mUid
              + " proc="
  ...
  public void decStartedServices(int memFactor, long now, String serviceName) {
    ...
    if (i == 0 && this.mCurCombinedState % 16 == 9) {
      ...
      Slog.wtfStack(
          "ProcessStats",
          "Proc started services underrun: pkg="
              + this.mPackage
              + " uid="
              + this.mUid
              + " name="
  ...
  public void addPss(
    ...
    if (always
        ...
        || SystemClock.uptimeMillis()
            >= this.mLastPssTime + JobInfo.DEFAULT_INITIAL_BACKOFF_MILLIS) {
      this.mLastPssState = this.mCurCombinedState;
      ...
      if (i != -1) {
        ...
        PssTable.mergeStats(
            this.mCommonProcess.mTotalRunningPss,
            0,
            1,
            pss,
            pss,
        ...
        if (this.mCommonProcess.mMultiPackage && pkgList != null) {
          for (int ip = pkgList.size() - 1; ip >= 0; ip--) {
            ...
            fixedProc.mPssTable.mergeStats(
                this.mCurCombinedState, 1, pss, pss, pss, uss, uss, uss, rss, rss, rss);
            PssTable.mergeStats(
                fixedProc.mTotalRunningPss, 0, 1, pss, pss, pss, uss, uss, uss, rss, rss, rss);
          }
        }
      }
    }
  ...
  public void reportCachedKill(
      ArrayMap<String, ProcessStats.ProcessStateHolder> pkgList, long pss) {
    ensureNotDead();
  ...
  public ProcessState pullFixedProc(String pkgName) {
    if (this.mMultiPackage) {
      LongSparseArray<ProcessStats.PackageState> vpkg =
          this.mStats.mPackages.get(pkgName, this.mUid);
      if (vpkg == null) {
      ...
      if (pkg == null) {
        throw new IllegalStateException(
            "Didn't find package " + pkgName + " / " + this.mUid + " vers " + this.mVersion);
      }
      ProcessState proc = pkg.mProcesses.get(this.mName);
      if (proc == null) {
        throw new IllegalStateException(
            "Didn't create per-package process "
                + this.mName
                + " in pkg "
                + pkgName
                + " / "
  ...
  private ProcessState pullFixedProc(
      ArrayMap<String, ProcessStats.ProcessStateHolder> pkgList, int index) {
    ...
    if (proc.mMultiPackage) {
      LongSparseArray<ProcessStats.PackageState> vpkg =
          this.mStats.mPackages.get(pkgList.keyAt(index), proc.mUid);
      if (vpkg == null) {
        throw new IllegalStateException(
            "No existing package "
                + pkgList.keyAt(index)
  ...
  AssociationState.SourceState getOrCreateSourceState(AssociationState.SourceKey key) {
    ...
    AssociationState.SourceState state = this.mCommonSources.get(key);
    if (state == null) {
      AssociationState.SourceState state2 =
          new AssociationState.SourceState(this.mStats, null, this, key);
  ...
  public void aggregatePss(ProcessStats.TotalMemoryUseCollection data, long now) {
    ...
    while (type2 < processState.mDurations.getKeyCount()) {
      ...
      int i2 = type2;
      double newAvg =
          ((totalMemoryUseCollection.processStatePss[procState2]
                      * totalMemoryUseCollection.processStateSamples[procState2])
                  + (avg * samples))
              / (totalMemoryUseCollection.processStateSamples[procState2] + samples);
  ...
  public long computeProcessTimeLocked(
  ...
  private void dumpProcessSummaryDetails(
      PrintWriter pw,
      String prefix,
      String label,
      int[] screenStates,
      ...
      long totalTime,
      boolean full) {
    ProcessStats.ProcessDataCollection totals =
        new ProcessStats.ProcessDataCollection(screenStates, memStates, procStates);
  ...
  void dumpInternalLocked(
      PrintWriter pw, String prefix, String reqPackage, long totalTime, long now, boolean dumpAll) {
    if (dumpAll) {
      pw.print(prefix);
      pw.print("myID=");
      ...
      if (this.mCommonSources != null) {
        ...
        AssociationState.dumpSources(
            ...
            prefix + "    ",
            prefix + "        ",
            AssociationState.createSortedAssociations(now, totalTime, this.mCommonSources),
            now,
            totalTime,
            reqPackage,
            true,
        ...
        while (ip2 < data.procStates.length) {
          ...
          if (samples <= maxRss) {
            ...
            if (data.numPss == 0) {
              ...
              avgPss = samples;
              data.avgPss =
                  (long) (((data.avgPss * data.numPss) + (d * avgPss)) / (data.numPss + avgPss));
              ...
              data.avgUss =
                  (long)
                      (((data.avgUss * data.numPss) + (avgUss * avgPss)) / (data.numPss + avgPss));
              ...
              data.avgRss =
                  (long)
                      (((data.avgRss * data.numPss) + (avgRss * avgPss)) / (data.numPss + avgPss));
  ...
  public void dumpPackageProcCheckin(
      PrintWriter pw, String pkgName, int uid, long vers, String itemName, long now) {
    pw.print("pkgproc,");
    pw.print(pkgName);
    pw.print(",");
    pw.print(uid);
  ...
  public void dumpAllPssCheckin(PrintWriter pw) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      dumpPssSamplesCheckin(
          pw, this.mPssTable.getArrayForKey(key), SparseMappingTable.getIndexFromKey(key));
    }
  }
  ...
  public void dumpDebug(ProtoOutputStream proto, long fieldId, String procName, int uid, long now) {
    ...
    if (this.mNumExcessiveCpu > 0 || this.mNumCachedKill > 0) {
      ...
      ProtoUtils.toAggStatsProto(
          proto,
          1146756268035L,
          this.mMinCachedKillPss,
          this.mAvgCachedKillPss,
          this.mMaxCachedKillPss);
      proto.end(killToken);
    }
    ...
    while (true) {
      ...
      if (durationByState.indexOfKey(type2) < 0) {
        ...
        i = i4;
        DumpUtils.printProcStateTagProto(
            proto, 1159641169921L, 1159641169922L, 1159641169923L, type2);
    ...
    while (i5 < durationByState.size()) {
      ...
      int i6 = i5;
      DumpUtils.printProcStateTagProto(
          proto, 1159641169921L, 1159641169922L, 1159641169923L, durationByState.keyAt(i5));
  ...
  static void writeCompressedProcessName(
    ...
    int seconds3 =
        (int)
            TimeUnit.MILLISECONDS.toSeconds(
                processStats.mTimePeriodEndUptime - processStats.mTimePeriodStartUptime);
  ...
  */
  public void dumpAggregatedProtoForStatsd(
      ...
      int uid,
      long now,
      ProcessMap<ArraySet<ProcessStats.PackageState>> procToPkgMap,
    ...
    if (!didCurState && (i2 = this.mCurCombinedState) != -1 && i2 % 16 != 9) {
      int aggregatedType2 = DumpUtils.aggregateCurrentProcessState(i2);
      int index2 = durationByState.indexOfKey(aggregatedType2);
      if (index2 >= 0) {
        durationByState.put(
            aggregatedType2, (now - this.mStartTime) + durationByState.valueAt(index2));
      } else {
    ...
    for (int i4 = 0; i4 < this.mPssTable.getKeyCount(); i4++) {
      ...
      if (durationByState.indexOfKey(aggregatedType3) >= 0) {
        ...
        if (meanRssByState.indexOfKey(aggregatedType3) >= 0) {
          meanRssByState.put(
              aggregatedType3, meanRssByState.get(aggregatedType3) + meanTimesDuration);
        } else {
          meanRssByState.put(aggregatedType3, meanTimesDuration);
        }
        if (maxRssByState.indexOfKey(aggregatedType3) >= 0
            && maxRssByState.get(aggregatedType3) < rssMeanAndMax[1]) {
    ...
    for (int i5 = 0; i5 < durationByState.size(); i5++) {
      ...
      if (meanRssByState.indexOfKey(aggregatedKey) >= 0) {
        ...
        meanRssByState.put(
            aggregatedKey,
            duration > 0
                ? meanRssByState.get(aggregatedKey) / duration
    ...
    if (!this.mMultiPackage && uidToPkgMap.get(this.mUid).size() <= 1) {
      ...
      while (i < durationByState.size()) {
        ...
        DumpUtils.printAggregatedProcStateTagProto(
            proto, 1159641169921L, 1159641169930L, aggregatedKey2);
        ...
        ProtoUtils.toAggStatsProto(
            ...
            0L,
            0L,
            (int) meanRssByState.get(aggregatedKey2),
            (int) maxRssByState.get(aggregatedKey2));
        proto.end(stateToken);
        i++;
        token = token;
      ...
      this.mStats.dumpFilteredAssociationStatesProtoForProc(
          proto, 2246267895815L, now, this, uidToPkgMap);
      proto.end(token);
    }
    z = true;
    writeCompressedProcessName(proto, 1138166333441L, procName, str, z);
