...
public class PssTable extends SparseMappingTable.Table {
  ...
  public static void mergeStats(
      long[] stats, int statsIndex, long[] thatStats, int thatStatsIndex) {
    mergeStats(
        stats,
        statsIndex,
        (int) thatStats[thatStatsIndex + 0],
        thatStats[thatStatsIndex + 1],
        thatStats[thatStatsIndex + 2],
        thatStats[thatStatsIndex + 3],
        thatStats[thatStatsIndex + 4],
        thatStats[thatStatsIndex + 5],
        thatStats[thatStatsIndex + 6],
        thatStats[thatStatsIndex + 7],
        thatStats[thatStatsIndex + 8],
        thatStats[thatStatsIndex + 9]);
  }
  ...
  public static void mergeStats(
    ...
    stats[statsIndex + 2] =
        (long) (((stats[statsIndex + 2] * count) + (avgPss * inCount)) / (inCount + count));
    ...
    stats[statsIndex + 5] =
        (long) (((stats[statsIndex + 5] * count) + (avgUss * inCount)) / (inCount + count));
    if (stats[statsIndex + 6] < maxUss) {
      stats[statsIndex + 6] = maxUss;
    }
    long j = stats[statsIndex + 7];
    stats[statsIndex + 8] =
        (long) (((stats[statsIndex + 8] * count) + (avgRss * inCount)) / (inCount + count));
  ...
  public static void writeStatsToProto(ProtoOutputStream proto, long[] stats, int statsIndex) {
    ...
    ProtoUtils.toAggStatsProto(
        proto, 1146756268038L, stats[statsIndex + 1], stats[statsIndex + 2], stats[statsIndex + 3]);
    ProtoUtils.toAggStatsProto(
        proto, 1146756268039L, stats[statsIndex + 4], stats[statsIndex + 5], stats[statsIndex + 6]);
    ProtoUtils.toAggStatsProto(
        proto, 1146756268040L, stats[statsIndex + 7], stats[statsIndex + 8], stats[statsIndex + 9]);
  }
