  ...
  public static class Table {
    ...
    public void copyFrom(Table copyFrom, int valueCount) {
      ...
      for (int i = 0; i < N; i++) {
        ...
        long[] theirLongs =
            (long[]) copyFrom.mParent.mLongs.get(SparseMappingTable.getArrayFromKey(theirKey));
    ...
    public long getValue(int key, int index) {
      ...
      try {
        ...
        SparseMappingTable.logOrThrow(
            "key=0x" + Integer.toHexString(key) + " index=" + index + " -- " + dumpInternalState(),
            ex);
        return 0L;
      }
    }
