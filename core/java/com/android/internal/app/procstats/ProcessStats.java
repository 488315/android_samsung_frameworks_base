        ...
        if (!commonProc.isMultiPackage()) {
            if (pkgState.mPackageName.equals(commonProc.getPackage()) && pkgState.mVersionCode == commonProc.getVersion()) {
                ...
                if (commonPkgState != null) {
                    ...
                    Slog.m121w(TAG, "Cloning proc state: no package state " + commonProc.getPackage() + "/" + pkgState.mUid + " for proc " + commonProc.getName());
                }
                str = processName;
                        ...
                        if (act.mProcState == procState) {
                            ...
                            if (act.mProcState < procState) {
                                ...
                                if (this.mNextInverseProcStateWarningUptime <= nowUptime) {
                                    Slog.m121w(TAG, "Tracking association " + act + " whose proc state " + act.mProcState + " is better than process " + proc + " proc state " + procState + " (" + this.mSkippedInverseProcStateWarningCount + " skipped)");
        ...
        while (true) {
            ...
            if (protoStreams[shardIndex].getRawSize() > maxRawShardSizeBytes) {
                ...
                if (shardIndex < protoStreams.length) {
                    ...
                    Slog.m113d(TAG, String.format("Dropping process indices from %d to %d from statsd proto (too large)", Integer.valueOf(ip), Integer.valueOf(procMap.size())));
                    break;
                }
            }
            SparseArray<ProcessState> uids = procMap.valueAt(ip);
