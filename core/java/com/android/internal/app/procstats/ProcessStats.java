...
public final class ProcessStats implements Parcelable {
  ...
  public static final Parcelable.Creator<ProcessStats> CREATOR =
      new Parcelable.Creator<
          ProcessStats>() { // from class: com.android.internal.app.procstats.ProcessStats.1
  ...
  public void add(ProcessStats other) {
          ...
          for (int NSRVS4 = NSRVS2; isvc < NSRVS4; NSRVS4 = NSRVS4) {
            ...
            ServiceState thisSvc =
                getServiceStateLocked(
                    pkgName, uid, vers, otherSvc.getProcessName(), otherSvc.getName());
          ...
          for (int iasc = 0; iasc < NASCS4; iasc++) {
            ...
            AssociationState thisAsc =
                getAssociationStateLocked(
                    pkgName, uid, vers3, otherAsc.getProcessName(), otherAsc.getName());
    ...
    for (int ip2 = 0; ip2 < size; ip2++) {
      ...
      if (uidState == null) {
        this.mUidStates.put(uid2, uidStates2.valueAt(ip2).m7717clone());
      } else {
  ...
  public boolean evaluateSystemProperties(boolean update) {
    ...
    String runtime =
        SystemProperties.get("persist.sys.dalvik.vm.lib.2", VMRuntime.getRuntime().vmLibrary());
  ...
  private void writeCompactedLongArray(Parcel out, long[] array, int num) {
    for (int i = 0; i < num; i++) {
      long val = array[i];
      if (val < 0) {
        Slog.m121w(TAG, "Time val negative: " + val);
        val = 0;
      }
      if (val > 2147483647L) {
              ...
              while (NVERS2 > 0) {
                ...
                while (NASCS > 0) {
                  ...
                  AssociationState asc2 =
                      hadData ? pkgState3.mAssociations.get(associationName2) : null;
                  if (asc2 != null) {
                    NSRVS = NSRVS3;
                    ...
                    associationName = associationName2;
                    asc =
                        new AssociationState(this, pkgState3, associationName2, processName2, null);
                  }
    ...
    if (!commonProc.isMultiPackage()) {
      if (pkgState.mPackageName.equals(commonProc.getPackage())
        ...
        PackageState commonPkgState =
            getPackageStateLocked(commonProc.getPackage(), pkgState.mUid, commonProc.getVersion());
  ...
  public ServiceState getServiceStateLocked(
    ...
    ProcessState ps =
        processName != null ? getProcessStateLocked(packageName, uid, vers, processName) : null;
  ...
  public void dumpLocked(
    ...
    long totalTime3 =
        DumpUtils.dumpSingleTime(
            null,
            null,
            processStats4.mMemFactorDurations,
            processStats4.mMemFactor,
            processStats4.mStartTime,
            now);
    pw.print("          Start time: ");
    ...
    TimeUtils.formatDuration(
        (processStats4.mRunning ? SystemClock.uptimeMillis() : processStats4.mTimePeriodEndUptime)
            - processStats4.mTimePeriodStartUptime,
        pw);
    pw.println();
    pw.print("  Total elapsed time: ");
    TimeUtils.formatDuration(
        (processStats4.mRunning
                ? SystemClock.elapsedRealtime()
                : processStats4.mTimePeriodEndRealtime)
            - processStats4.mTimePeriodStartRealtime,
        pw);
    ...
    if (i != 0) {
      ArrayMap<String, SparseArray<LongSparseArray<PackageState>>> pkgMap =
          processStats4.mPackages.getMap();
      int ip4 = 0;
      while (true) {
            ...
            if (iv2 < ip5) {
              ...
              if ((section & 8) != 0) {
                ...
                while (iasc3 < NASCS4) {
                  ...
                  iu4 = iu6;
                  cont.mSources =
                      AssociationState.createSortedAssociations(now, totalTime2, asc.mSources);
                  ...
                  if (activeOnly && !asc2.isInUse()) {
                    ...
                    asc2.dumpStats(
                        ...
                        (!onlyAssociations
                                || pkgMatch
                                || procMatch
                                || asc2.getProcessName().equals(reqPackage))
                            ? null
                            : reqPackage,
                        dumpDetails,
                        dumpAll);
  ...
  public void dumpSummaryLocked(PrintWriter pw, String reqPackage, long now, boolean activeOnly) {
    long totalTime =
        DumpUtils.dumpSingleTime(
            null, null, this.mMemFactorDurations, this.mMemFactor, this.mStartTime, now);
  ...
  private void dumpFragmentationLocked(PrintWriter pw) {
    ...
    for (int i = 0; i < NPAGETYPES; i++) {
      pw.format(
          "Node %3d Zone %7s  %14s ",
          this.mPageTypeNodes.get(i), this.mPageTypeZones.get(i), this.mPageTypeLabels.get(i));
  ...
  void dumpFilteredSummaryLocked(
    ...
    ArrayList<ProcessState> procs =
        collectProcessesLocked(
            screenStates, memStates, procStates, sortProcStates, now, reqPackage, activeOnly);
    if (procs.size() > 0) {
      if (header != null) {
        pw.println();
        pw.println(header);
      ...
      DumpUtils.dumpProcessSummaryLocked(
          pw, prefix, prcLabel, procs, screenStates, memStates, sortProcStates, now, totalTime);
    }
  }
            ...
            while (iv < vpkgs2.size()) {
              ...
              if ((section & 2) != 0) {
                ...
                while (iproc < NPROCS) {
                  ...
                  proc.dumpPackageProcCheckin(
                      pw, pkgName2, uid, vers, pkgState2.mProcesses.keyAt(iproc), now);
                  iproc++;
                  NSRVS2 = NSRVS2;
                  pkgName2 = pkgName2;
                  pkgState2 = pkgState2;
              ...
              if (NPROCS2 != 0) {
                for (int isvc = 0; isvc < NSRVS; isvc++) {
                  String serviceName =
                      DumpUtils.collapseString(pkgName, pkgState.mServices.keyAt(isvc));
              ...
              if ((section & 8) != 0) {
                for (int iasc = 0; iasc < NASCS; iasc++) {
                  String associationName =
                      DumpUtils.collapseString(pkgName, pkgState.mAssociations.keyAt(iasc));
      ...
      if (protoStreams[shardIndex].getRawSize() > maxRawShardSizeBytes) {
        ...
        if (shardIndex < protoStreams.length) {
          ...
          Slog.m113d(
              TAG,
              String.format(
                  "Dropping process indices from %d to %d from statsd proto (too large)",
  ...
  public void dumpFilteredAssociationStatesProtoForProc(
    ...
    if ((!procState.isMultiPackage() || procState.getCommonProcess() == procState)
        ...
        && !sources.isEmpty()
        && (procStatsService =
                IProcessStats.Stub.asInterface(ServiceManager.getService(SERVICE_NAME)))
            != null) {
      try {
