...
public final class ServiceState {
  ...
  public ServiceState(
      ProcessStats processStats, String pkg, String name, String processName, ProcessState proc) {
    this.mPackage = pkg;
    this.mName = name;
  ...
  public long dumpTime(
      PrintWriter pw, String prefix, int serviceType, int curState, long curStartTime, long now) {
  ...
  public void dumpDebug(ProtoOutputStream proto, long fieldId, long now) {
    ...
    writeTypeToProto(
        proto, 2246267895810L, 1, 0, this.mRunCount, this.mRunState, this.mRunStartTime, now);
    writeTypeToProto(
        proto,
        2246267895810L,
        2,
    ...
    writeTypeToProto(
        ...
        now);
    writeTypeToProto(
        proto, 2246267895810L, 4, 2, this.mBoundCount, this.mBoundState, this.mBoundStartTime, now);
    writeTypeToProto(
        proto, 2246267895810L, 5, 3, this.mExecCount, this.mExecState, this.mExecStartTime, now);
    proto.end(token);
  }
