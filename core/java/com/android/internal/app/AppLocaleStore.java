      ...
      if (localeConfig.getStatus() == 0) {
        ...
        localeStatus =
            appSupportedLocales.size() > 0
                ? AppLocaleResult.LocaleStatus.GET_SUPPORTED_LANGUAGE_FROM_LOCAL_CONFIG
      ...
      String[] locales =
          packageManager
              .getResourcesForApplication(
                  packageManager.getPackageInfo(packageName, 131072).applicationInfo)
              .getAssets()
              .getNonSystemLocales();
      if (locales == null) {
        Log.m98i(
            TAG, NavigationBarInflaterView.SIZE_MOD_START + packageName + "] locales are null.");
      } else if (locales.length <= 0) {
        Log.m98i(
            TAG, NavigationBarInflaterView.SIZE_MOD_START + packageName + "] locales length is 0.");
      } else {
            ...
            .filter(
                new Predicate() { // from class:
                  ...
                  public final boolean test(Object obj) {
                    ...
                    matchLanguageInSet =
                        AppLocaleStore.matchLanguageInSet((Locale) obj, assetLocale);
  ...
  private static boolean isSystemApp(Context context, String packageName) {
    try {
      ApplicationInfo applicationInfo =
          context.getPackageManager().getApplicationInfoAsUser(packageName, 0, context.getUserId());
