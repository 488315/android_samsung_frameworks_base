...
public class AppLocaleStore {
    ...
    public static AppLocaleResult getAppSupportedLocales(Context context, String packageName) {
        ...
        try {
            ...
            Log.m102w(TAG, "Can not found the package name : " + packageName + " / " + e);
        }
        if (localeConfig != null) {
            if (localeConfig.getStatus() == 0) {
                ...
                Log.m94d(TAG, "filterNonMatchingLocale. , shouldFilterNotMatchingLocale: " + shouldFilterNotMatchingLocale + ", assetLocale size: " + assetLocale.size() + ", packageLocaleList size: " + packageLocaleList.size());
        ...
        Log.m94d(TAG, "getAppSupportedLocales(). package: " + packageName + ", status: " + localeStatus + ", appSupportedLocales:" + appSupportedLocales.size());
    ...
    private static HashSet<Locale> getAssetLocales(Context context, String packageName) {
        ...
        try {
            ...
            if (locales == null) {
                Log.m98i(TAG, NavigationBarInflaterView.SIZE_MOD_START + packageName + "] locales are null.");
            } else if (locales.length <= 0) {
                Log.m98i(TAG, NavigationBarInflaterView.SIZE_MOD_START + packageName + "] locales length is 0.");
            } else {
    ...
    private static boolean hasInstallerInfo(Context context, String packageName) {
        try {
            ...
            Log.m102w(TAG, "Installer info not found for: " + packageName);
            return false;
        }
    }
    ...
    private static boolean isSystemApp(Context context, String packageName) {
        try {
            ...
            Log.m102w(TAG, "Application info not found for: " + packageName);
            return false;
        }
    }
