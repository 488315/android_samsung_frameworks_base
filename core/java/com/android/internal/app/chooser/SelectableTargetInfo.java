...
public final class SelectableTargetInfo implements ChooserTargetInfo {
  ...
  private Drawable getChooserTargetIconDrawable(ChooserTarget target, ShortcutInfo shortcutInfo) {
    ...
    try {
      ...
      Log.m96e(TAG, "Could not find activity associated with ChooserTarget");
    }
    if (info == null) {
      return null;
    }
    Bitmap appIcon =
        this.mSelectableTargetInfoCommunicator.makePresentationGetter(info).getIconBitmap(null);
  ...
  private Intent getBaseIntentToSend() {
    ...
    if (result == null) {
      Log.m96e(TAG, "ChooserTargetInfo: no base intent available to send");
    } else {
  ...
  public ResolveInfo getResolveInfo() {
    DisplayResolveInfo displayResolveInfo = this.mSourceInfo;
    return displayResolveInfo != null
