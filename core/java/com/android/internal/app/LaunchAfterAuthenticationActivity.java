...
public class LaunchAfterAuthenticationActivity extends Activity {
  ...
  public static Intent createLaunchAfterAuthenticationIntent(IntentSender onSuccessIntent) {
    return new Intent()
        .setClassName("android", LaunchAfterAuthenticationActivity.class.getName())
  ...
  public void onCreate(Bundle icicle) {
    ...
    IntentSender onSuccessIntent =
        (IntentSender) getIntent().getParcelableExtra(EXTRA_ON_SUCCESS_INTENT, IntentSender.class);
  ...
  private void requestDismissKeyguardIfNeeded(final IntentSender onSuccessIntent) {
    KeyguardManager km =
        (KeyguardManager)
            Objects.requireNonNull((KeyguardManager) getSystemService(KeyguardManager.class));
    if (km.isKeyguardLocked()) {
      km.requestDismissKeyguard(
          this,
          new KeyguardManager
              .KeyguardDismissCallback() { // from class:
                                           // com.android.internal.app.LaunchAfterAuthenticationActivity.1
            ...
            public void onDismissError() {
              Slog.m115e(LaunchAfterAuthenticationActivity.TAG, "Error while dismissing keyguard.");
              LaunchAfterAuthenticationActivity.this.finish();
            }
          });
    } else {
  ...
  public void onUnlocked(IntentSender targetIntent) {
    try {
      ...
      Slog.m116e(TAG, "Error while sending original intent", e);
    }
  }
}
