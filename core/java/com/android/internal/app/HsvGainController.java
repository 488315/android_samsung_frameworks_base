  ...
  public HsvGainController(Context context, int userId) {
    ...
    this.mContentObserver =
        new ContentObserver(
            new Handler(
                Looper
                    .getMainLooper())) { // from class: com.android.internal.app.HsvGainController.1
  ...
  public int getHsvGainHueLevel() {
    int level =
        Settings.Secure.getIntForUser(
            ...
            this.mUserId);
    if (level == -1) {
      Slog.m113d(TAG, "Using default value for setting: hsv_gain_display_hue_level");
  ...
  public int getHsvGainSatLevel() {
    int level =
        Settings.Secure.getIntForUser(
            ...
            this.mUserId);
    if (level == -1) {
      Slog.m113d(TAG, "Using default value for setting: hsv_gain_display_sat_level");
      return getDefaultHsvGainLevel();
    }
    return level;
  }
  ...
  public int getHsvGainValLevel() {
    int level =
        Settings.Secure.getIntForUser(
            ...
            this.mUserId);
    if (level == -1) {
      Slog.m113d(TAG, "Using default value for setting: hsv_gain_display_val_level");
      return getDefaultHsvGainLevel();
    }
    return level;
  }
