    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!TextUtils.isEmpty(streamedDeviceName)) {
            if (TextUtils.equals(activityInfo.packageName, getPackageManager().getPermissionControllerPackageName())) {
                this.mAlertParams.mTitle = getString(C4337R.string.app_streaming_blocked_title_for_permission_dialog);
                this.mAlertParams.mMessage = getString(C4337R.string.app_streaming_blocked_message, streamedDeviceName);
            } else if (TextUtils.equals(activityInfo.packageName, BLOCKED_COMPONENT_PLAYSTORE)) {
                this.mAlertParams.mTitle = getString(C4337R.string.app_streaming_blocked_title_for_playstore_dialog);
                this.mAlertParams.mMessage = getString(C4337R.string.app_streaming_blocked_message, streamedDeviceName);
            } else if (TextUtils.equals(activityInfo.packageName, "com.android.settings")) {
                this.mAlertParams.mTitle = getString(C4337R.string.app_streaming_blocked_title_for_settings_dialog);
                this.mAlertParams.mMessage = getString(C4337R.string.app_streaming_blocked_message_for_settings_dialog, streamedDeviceName);
            } else {
                this.mAlertParams.mMessage = getString(C4337R.string.app_streaming_blocked_message, streamedDeviceName);
            }
        } else {
