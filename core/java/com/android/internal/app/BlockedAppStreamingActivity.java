...
public class BlockedAppStreamingActivity extends AlertActivity {
  ...
  private static final String EXTRA_BLOCKED_ACTIVITY_INFO =
      "com.android.internal.app.extra.BLOCKED_ACTIVITY_INFO";
  private static final String EXTRA_STREAMED_DEVICE =
      "com.android.internal.app.extra.STREAMED_DEVICE";
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    ActivityInfo activityInfo =
        (ActivityInfo) intent.getParcelableExtra(EXTRA_BLOCKED_ACTIVITY_INFO, ActivityInfo.class);
    ...
    if (!TextUtils.isEmpty(streamedDeviceName)) {
      if (TextUtils.equals(
          activityInfo.packageName, getPackageManager().getPermissionControllerPackageName())) {
        this.mAlertParams.mTitle =
            getString(C4337R.string.app_streaming_blocked_title_for_permission_dialog);
        this.mAlertParams.mMessage =
            getString(C4337R.string.app_streaming_blocked_message, streamedDeviceName);
      } else if (TextUtils.equals(activityInfo.packageName, BLOCKED_COMPONENT_PLAYSTORE)) {
        this.mAlertParams.mTitle =
            getString(C4337R.string.app_streaming_blocked_title_for_playstore_dialog);
        this.mAlertParams.mMessage =
            getString(C4337R.string.app_streaming_blocked_message, streamedDeviceName);
      } else if (TextUtils.equals(activityInfo.packageName, "com.android.settings")) {
        this.mAlertParams.mTitle =
            getString(C4337R.string.app_streaming_blocked_title_for_settings_dialog);
        this.mAlertParams.mMessage =
            getString(
                C4337R.string.app_streaming_blocked_message_for_settings_dialog,
                streamedDeviceName);
      } else {
        this.mAlertParams.mMessage =
            getString(C4337R.string.app_streaming_blocked_message, streamedDeviceName);
      ...
      this.mAlertParams.mMessage = getString(C4337R.string.app_blocked_message, appLabel);
    }
