...
class AppPredictionServiceResolverComparator extends AbstractResolverComparator {
  ...
  void handleResultMessage(Message msg) {
    if (msg.what == 0 && msg.obj != null) {
      ...
      Log.m96e(TAG, "Unexpected null result");
    }
  }
  ...
  void doCompute(final List<ResolverActivity.ResolvedComponentInfo> targets) {
    ...
    this.mAppPredictor.sortTargets(
        appTargets,
        Executors.newSingleThreadExecutor(),
        new Consumer() { // from class:
                         // com.android.internal.app.AppPredictionServiceResolverComparator$$ExternalSyntheticLambda0
  ...
  public /* synthetic */ void lambda$doCompute$0(List targets, List sortedAppTargets) {
    if (sortedAppTargets.isEmpty()) {
      Log.m98i(TAG, "AppPredictionService disabled. Using resolver.");
      setupFallbackModel(targets);
    } else {
      Log.m98i(TAG, "AppPredictionService response received");
  ...
  private void setupFallbackModel(List<ResolverActivity.ResolvedComponentInfo> targets) {
    ResolverRankerServiceResolverComparator resolverRankerServiceResolverComparator =
        new ResolverRankerServiceResolverComparator(
            this.mContext,
            this.mIntent,
            this.mReferrerPackage,
            new AbstractResolverComparator
                .AfterCompute() { // from class:
                                  // com.android.internal.app.AppPredictionServiceResolverComparator$$ExternalSyntheticLambda1
    ...
    this.mComparatorModel =
        this.mModelBuilder.buildFallbackModel(resolverRankerServiceResolverComparator);
  ...
  static class ModelBuilder {
    ...
    ResolverComparatorModel buildFromRankedList(List<AppTarget> sortedAppTargets) {
      return new AppPredictionServiceComparatorModel(
          this.mAppPredictor, this.mUser, buildTargetRanksMapFromSortedTargets(sortedAppTargets));
    }
    ...
    private Map<ComponentName, Integer> buildTargetRanksMapFromSortedTargets(
      ...
      for (int i = 0; i < sortedAppTargets.size(); i++) {
        ComponentName componentName =
            new ComponentName(
                sortedAppTargets.get(i).getPackageName(), sortedAppTargets.get(i).getClassName());
        targetRanks.put(componentName, Integer.valueOf(i));
        Log.m98i(
            AppPredictionServiceResolverComparator.TAG,
            "handleSortedAppTargets, sortedAppTargets #" + i + ": " + componentName);
      }
      return targetRanks;
    }

    /* renamed from: com.android.internal.app.AppPredictionServiceResolverComparator$ModelBuilder$1 */
    class C43611 implements ResolverComparatorModel {
      final /* synthetic */ AbstractResolverComparator val$comparator;

      C43611(AbstractResolverComparator abstractResolverComparator) {
      ...
      public Comparator<ResolveInfo> getComparator() {
        ...
        return new Comparator() { // from class:
                                  // com.android.internal.app.AppPredictionServiceResolverComparator$ModelBuilder$1$$ExternalSyntheticLambda0
          @Override // java.util.Comparator
          public final int compare(Object obj, Object obj2) {
            int compare;
            compare =
                AbstractResolverComparator.this.compare((ResolveInfo) obj, (ResolveInfo) obj2);
            return compare;
          }
        };
      }
  ...
  static class AppPredictionServiceComparatorModel implements ResolverComparatorModel {
    ...
    AppPredictionServiceComparatorModel(
        AppPredictor appPredictor, UserHandle user, Map<ComponentName, Integer> targetRanks) {
      this.mAppPredictor = appPredictor;
      this.mUser = user;
    ...
    public Comparator<ResolveInfo> getComparator() {
      return new Comparator() { // from class:
                                // com.android.internal.app.AppPredictionServiceResolverComparator$AppPredictionServiceComparatorModel$$ExternalSyntheticLambda0
        @Override // java.util.Comparator
        public final int compare(Object obj, Object obj2) {
          int lambda$getComparator$0;
          lambda$getComparator$0 =
              AppPredictionServiceResolverComparator.AppPredictionServiceComparatorModel.this
    ...
    public /* synthetic */ int lambda$getComparator$0(ResolveInfo lhs, ResolveInfo rhs) {
      Integer lhsRank =
          this.mTargetRanks.get(
              new ComponentName(lhs.activityInfo.packageName, lhs.activityInfo.name));
      Integer rhsRank =
          this.mTargetRanks.get(
              new ComponentName(rhs.activityInfo.packageName, rhs.activityInfo.name));
    ...
    public float getScore(TargetInfo targetInfo) {
      ...
      if (rank == null) {
        Log.m102w(
            AppPredictionServiceResolverComparator.TAG,
            "Score requested for unknown component. Did you call compute yet?");
        return 0.0f;
      }
    ...
    @Override // com.android.internal.app.ResolverComparatorModel
    public void notifyOnTargetSelected(TargetInfo targetInfo) {
      this.mAppPredictor.notifyAppTargetEvent(
          new AppTargetEvent.Builder(
                  new AppTarget.Builder(
                          new AppTargetId(targetInfo.getResolvedComponentName().toString()),
                          targetInfo.getResolvedComponentName().getPackageName(),
                          this.mUser)
                      .setClassName(targetInfo.getResolvedComponentName().getClassName())
                      .build(),
                  1)
              .build());
    }
