...
public class ResolverListController {
  ...
  public ResolveInfo getLastChosen() throws RemoteException {
    ...
    return packageManager.getLastChosenActivity(
        intent, intent.resolveTypeIfNeeded(this.mContext.getContentResolver()), 65536);
  }
  ...
  public void addResolveListDedupe(
      List<ResolverActivity.ResolvedComponentInfo> into, Intent intent, List<ResolveInfo> from) {
    int fromCount = from.size();
    int intoCount = into.size();
    for (int i = 0; i < fromCount; i++) {
      ResolveInfo newInfo = from.get(i);
      if (newInfo.userHandle == null) {
        Log.m102w(TAG, "Skipping ResolveInfo with no userHandle: " + newInfo);
      } else {
        boolean found = false;
        int j = 0;
        while (true) {
    ...
    for (int i = inputList.size() - 1; i >= 0; i--) {
      ...
      int granted =
          ActivityManager.checkComponentPermission(
              ai.permission, this.mLaunchedFromUid, ai.applicationInfo.uid, ai.exported);
  ...
  private void compute(List<ResolverActivity.ResolvedComponentInfo> inputList)
    ...
    if (this.mResolverComparator == null) {
      Log.m94d(TAG, "Comparator has already been destroyed; skipped.");
      return;
    }
  ...
  public void sort(List<ResolverActivity.ResolvedComponentInfo> inputList) {
    try {
      ...
      Log.m96e(TAG, "Compute & Sort was interrupted: " + e);
    }
  }
  ...
  public void topK(List<ResolverActivity.ResolvedComponentInfo> inputList, int k) {
    ...
    try {
      ...
      Log.m96e(TAG, "Compute & greatestOf was interrupted: " + e);
    }
  }
