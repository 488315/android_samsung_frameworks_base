...
public class SuspendedAppActivity extends AlertActivity implements DialogInterface.OnClickListener {
  ...
  public static final String EXTRA_ACTIVITY_OPTIONS =
      "com.android.internal.app.extra.ACTIVITY_OPTIONS";
  ...
  public static final String EXTRA_SUSPENDED_PACKAGE =
      "com.android.internal.app.extra.SUSPENDED_PACKAGE";
  public static final String EXTRA_SUSPENDING_PACKAGE =
      "com.android.internal.app.extra.SUSPENDING_PACKAGE";
  public static final String EXTRA_UNSUSPEND_INTENT =
      "com.android.internal.app.extra.UNSUSPEND_INTENT";
  ...
  private BroadcastReceiver mSuspendModifiedReceiver =
      new BroadcastReceiver() { // from class: com.android.internal.app.SuspendedAppActivity.1
        ...
        public void onReceive(Context context, Intent intent) {
          if (Intent.ACTION_PACKAGES_SUSPENSION_CHANGED.equals(intent.getAction())) {
            ...
            if (ArrayUtils.contains(modified, SuspendedAppActivity.this.mSuspendedPackage)) {
              ...
              if (!suspendedAppActivity.isPackageSuspended(suspendedAppActivity.mSuspendedPackage)
                  && !SuspendedAppActivity.this.isFinishing()) {
                Slog.m121w(
                    SuspendedAppActivity.TAG,
                    "Package "
                        + SuspendedAppActivity.this.mSuspendedPackage
                        + " has modified suspension conditions while dialog was visible."
                        + " Finishing.");
  ...
  public boolean isPackageSuspended(String packageName) {
    try {
      ...
      Slog.m116e(TAG, "Package " + packageName + " not found", ne);
      return false;
    }
  }
  ...
  private Intent getMoreDetailsActivity() {
    Intent moreDetailsIntent =
        new Intent(Intent.ACTION_SHOW_SUSPENDED_APP_DETAILS).setPackage(this.mSuspendingPackage);
    ResolveInfo resolvedInfo =
        this.mPm.resolveActivityAsUser(moreDetailsIntent, 786432, this.mUserId);
    if (resolvedInfo != null
        && resolvedInfo.activityInfo != null
        && Manifest.permission.SEND_SHOW_SUSPENDED_APP_DETAILS.equals(
            resolvedInfo.activityInfo.permission)) {
      if (isDigitalWellbingPackage(this.mSuspendingPackage)) {
        moreDetailsIntent
            .putExtra("android.intent.extra.PACKAGE_NAME", this.mSuspendedPackage)
            .setFlags(335577088);
      } else {
        moreDetailsIntent
            .putExtra("android.intent.extra.PACKAGE_NAME", this.mSuspendedPackage)
  ...
  private Drawable resolveIcon() {
    ...
    if (iconId != 0 && (resources = this.mSuspendingAppResources) != null) {
      try {
        ...
        Slog.m115e(TAG, "Could not resolve drawable resource id " + iconId);
        return null;
      }
    }
    return null;
    ...
    if (suspendDialogInfo != null) {
      ...
      if (titleId != 0 && (resources = this.mSuspendingAppResources) != null) {
        try {
          ...
          Slog.m115e(TAG, "Could not resolve string resource id " + titleId);
        }
      } else if (title != null) {
        return title;
      }
    }
    return getString(C4337R.string.app_suspended_title);
  }
    ...
    if (suspendDialogInfo != null) {
      ...
      if (messageId != 0 && (resources = this.mSuspendingAppResources) != null) {
        try {
          ...
          Slog.m115e(TAG, "Could not resolve string resource id " + messageId);
        }
      } else if (message != null) {
        return String.format(
            getResources().getConfiguration().getLocales().get(0), message, suspendedAppLabel);
      }
    }
    return getString(
        C4337R.string.app_suspended_default_message,
        suspendedAppLabel,
        getAppLabel(this.mSuspendingPackage));
  }
    ...
    switch (this.mNeutralButtonAction) {
      case 0:
        if (this.mMoreDetailsIntent != null) {
          defaultButtonTextId = C4337R.string.app_suspended_more_details;
          break;
        } else {
          return null;
        }
      case 1:
        defaultButtonTextId = C4337R.string.app_suspended_unsuspend_message;
        break;
      default:
        Slog.m121w(TAG, "Unknown neutral button action: " + this.mNeutralButtonAction);
        return null;
    }
    SuspendDialogInfo suspendDialogInfo = this.mSuppliedDialogInfo;
    if (suspendDialogInfo != null) {
      ...
      if (buttonTextId != 0 && (resources = this.mSuspendingAppResources) != null) {
        try {
          ...
          Slog.m115e(TAG, "Could not resolve string resource id " + buttonTextId);
        }
  ...
  public void onCreate(Bundle icicle) {
    ...
    this.mSuppliedDialogInfo =
        (SuspendDialogInfo) intent.getParcelableExtra(EXTRA_DIALOG_INFO, SuspendDialogInfo.class);
    this.mOnUnsuspend =
        (IntentSender) intent.getParcelableExtra(EXTRA_UNSUSPEND_INTENT, IntentSender.class);
    if (isDigitalWellbingPackage(this.mSuspendingPackage)
        && (getResources().getConfiguration().uiMode & 32) != 0) {
      setTheme(16974545);
    }
    if (this.mSuppliedDialogInfo != null) {
      try {
        this.mSuspendingAppResources =
            createContextAsUser(UserHandle.m55of(this.mUserId), 0)
                .getPackageManager()
                .getResourcesForApplication(this.mSuspendingPackage);
      } catch (PackageManager.NameNotFoundException ne) {
        Slog.m116e(TAG, "Could not find resources for " + this.mSuspendingPackage, ne);
      }
    }
    SuspendDialogInfo suspendDialogInfo = this.mSuppliedDialogInfo;
    int neutralButtonAction =
        suspendDialogInfo != null ? suspendDialogInfo.getNeutralButtonAction() : 0;
    ...
    setupAlert();
    IntentFilter suspendModifiedFilter =
        new IntentFilter(Intent.ACTION_PACKAGES_SUSPENSION_CHANGED);
    registerReceiverAsUser(
        this.mSuspendModifiedReceiver,
        UserHandle.m55of(this.mUserId),
        suspendModifiedFilter,
        null,
        null);
  }
  ...
  private void requestDismissKeyguardIfNeeded(CharSequence dismissMessage) {
    ...
    if (km.isKeyguardLocked()) {
      ...
      km.requestDismissKeyguard(
          ...
          dismissMessage,
          new KeyguardManager
              .KeyguardDismissCallback() { // from class:
                                           // com.android.internal.app.SuspendedAppActivity.2
            @Override // android.app.KeyguardManager.KeyguardDismissCallback
            public void onDismissError() {
              Slog.m115e(
                  SuspendedAppActivity.TAG,
                  "Error while dismissing keyguard. Keeping the dialog visible.");
            }

            @Override // android.app.KeyguardManager.KeyguardDismissCallback
            public void onDismissCancelled() {
              Slog.m121w(SuspendedAppActivity.TAG, "Keyguard dismiss was cancelled. Finishing.");
              SuspendedAppActivity.this.finish();
            }
          });
    }
      ...
      case -3:
        switch (this.mNeutralButtonAction) {
          case 0:
            ...
            if (intent != null) {
              startActivityAsUser(intent, this.mOptions, UserHandle.m55of(this.mUserId));
              break;
            } else {
            ...
            try {
              String[] errored =
                  ipm.setPackagesSuspendedAsUser(
                      new String[] {this.mSuspendedPackage},
                      false,
                      null,
                      null,
                      null,
                      this.mSuspendingPackage,
                      this.mUserId);
              if (ArrayUtils.contains(errored, this.mSuspendedPackage)) {
                Slog.m115e(TAG, "Could not unsuspend " + this.mSuspendedPackage);
                break;
              } else {
                Intent reportUnsuspend =
                    new Intent()
                        .setAction(Intent.ACTION_PACKAGE_UNSUSPENDED_MANUALLY)
                        .putExtra("android.intent.extra.PACKAGE_NAME", this.mSuspendedPackage)
                        .setPackage(this.mSuspendingPackage)
                        .addFlags(16777216);
                sendBroadcastAsUser(reportUnsuspend, UserHandle.m55of(this.mUserId));
                if (this.mOnUnsuspend != null) {
                  ...
                  try {
                    ...
                    Slog.m116e(TAG, "Error while starting intent " + this.mOnUnsuspend, e);
                    break;
                  }
                }
              }
            } catch (RemoteException re) {
              Slog.m116e(TAG, "Can't talk to system process", re);
              break;
            }
            break;
          default:
            Slog.m115e(TAG, "Unexpected action on neutral button: " + this.mNeutralButtonAction);
            break;
        }
    }
  ...
  public static Intent createSuspendedAppInterceptIntent(
      String suspendedPackage,
      String suspendingPackage,
      SuspendDialogInfo dialogInfo,
      Bundle options,
      IntentSender onUnsuspend,
      int userId) {
