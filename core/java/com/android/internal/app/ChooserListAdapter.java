...
public class ChooserListAdapter extends ResolverListAdapter {
  ...
  class ViewOnLayoutChangeListenerC43731 implements View.OnLayoutChangeListener {
    ViewOnLayoutChangeListenerC43731() {}
  ...
  public ChooserListAdapter(
    ...
    this.mPinTextSpacingListener = new ViewOnLayoutChangeListenerC43731();
    this.mMaxShortcutTargetsPerApp =
        context.getResources().getInteger(C4337R.integer.config_maxShortcutTargetsPerApp);
    ...
    this.mApplySharingAppLimits =
        DeviceConfig.getBoolean(
            "systemui", SystemUiDeviceConfigFlags.APPLY_SHARING_APP_LIMITS_IN_SYSUI, true);
  }
  ...
  View onCreateView(ViewGroup parent) {
    return this.mInflater.inflate(C4337R.layout.resolve_grid_item, parent, false);
  }
  ...
  protected void onBindView(View view, TargetInfo info, int position) {
    ...
    if (info == null) {
      holder.icon.lambda$setImageURIAsync$2(
    ...
    if (info instanceof ChooserActivity.PlaceHolderTargetInfo) {
      int maxWidth =
          this.mContext
              .getResources()
              .getDimensionPixelSize(C4337R.dimen.chooser_direct_share_label_placeholder_max_width);
      holder.text.setMaxWidth(maxWidth);
      holder.text.setBackground(
          this.mContext
              .getResources()
              .getDrawable(
                  C4337R.drawable.chooser_direct_share_label_placeholder,
    ...
    if (info instanceof MultiDisplayResolveInfo) {
      Drawable bkg = this.mContext.getDrawable(C4337R.drawable.chooser_group_background);
      holder.text.setPaddingRelative(0, 0, bkg.getIntrinsicWidth(), 0);
      holder.text.setBackground(bkg);
    } else {
      if (info.isPinned()
          && (getPositionTargetType(position) == 2 || getPositionTargetType(position) == 1)) {
        Drawable bkg2 = this.mContext.getDrawable(C4337R.drawable.chooser_pinned_background);
  ...
  void updateAlphabeticalList() {
    new AsyncTask<
      ...
      public List<DisplayResolveInfo> doInBackground(Void... voids) {
          ...
          String resolvedTarget =
              info.getResolvedComponentName().getPackageName()
                  ...
                  + ResolverActivity.getResolveInfoUserHandle(
                          info.getResolveInfo(), ChooserListAdapter.this.getUserHandle())
                      .getIdentifier();
        ...
        Collections.sort(
            groupedTargets, new ChooserActivity.AzInfoComparator(ChooserListAdapter.this.mContext));
  ...
  public int getServiceTargetCount() {
    ...
    if (chooserListCommunicator.isSendAction(chooserListCommunicator.getTargetIntent())
      ...
      return Math.min(
          this.mServiceTargets.size(), this.mChooserListCommunicator.getMaxRankedTargets());
    }
    return 0;
  }
  ...
  public void addServiceResults(
    ...
    if (targets.size() != 0) {
      ...
      int targetsLimit =
          this.mApplySharingAppLimits ? Math.min(targets.size(), maxTargets) : targets.size();
    ...
    return new AsyncTask<
      ...
      public List<ResolverActivity.ResolvedComponentInfo> doInBackground(
        ...
        ChooserListAdapter.this.mResolverListController.topK(
            params[0], ChooserListAdapter.this.mChooserListCommunicator.getMaxRankedTargets());
        Trace.endSection();
        return params[0];
      }
