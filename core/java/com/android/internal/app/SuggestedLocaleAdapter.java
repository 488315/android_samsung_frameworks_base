...
public class SuggestedLocaleAdapter extends BaseAdapter implements Filterable {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        switch (itemType) {
            ...
            case 1:
            case 3:
                TextView textView = (TextView) itemView.findViewById(C4337R.id.section_header);
                if (itemType == 0) {
                    setTextTo(textView, C4337R.string.language_picker_section_suggested);
                } else if (itemType == 3) {
                    if (SemCscFeature.getInstance().getBoolean("CscFeature_Common_ReplaceSecBrandAsGalaxy", false)) {
                        setTextTo(textView, C4337R.string.language_picker_section_suggested_sec_jpn);
                    } else {
                        setTextTo(textView, C4337R.string.language_picker_section_suggested_sec);
                    }
                } else if (this.mCountryMode) {
                    setTextTo(textView, C4337R.string.region_picker_section_all);
                } else if (!this.mHasSpecificAppPackageName) {
                    if (getCount() > 1) {
                        setTextTo(textView, C4337R.string.language_picker_section_show_other);
                    }
                } else {
                    setTextTo(textView, C4337R.string.language_picker_section_show_all);
                }
            ...
            case 2:
            default:
                updateTextView(itemView, (TextView) itemView.findViewById(C4337R.id.locale), position);
                return itemView;
            case 4:
                itemView.findViewById(C4337R.id.language_picker_item).findViewById(C4337R.id.divider).setVisibility(8);
                itemView.findViewById(C4337R.id.external_divider).setVisibility(0);
                updateTextView(itemView, (TextView) itemView.findViewById(C4337R.id.language_picker_item).findViewById(C4337R.id.locale), position);
                return itemView;
            case 5:
                View externalDivider = itemView.findViewById(C4337R.id.external_divider);
                if (externalDivider != null) {
                    itemView.findViewById(C4337R.id.language_picker_item).findViewById(C4337R.id.divider).setVisibility(8);
                    externalDivider.setVisibility(0);
                }
                if (((LocaleStore.LocaleInfo) getItem(position)).isAppCurrentLocale()) {
                    title = (TextView) itemView.findViewById(C4337R.id.language_picker_item).findViewById(C4337R.id.locale);
                } else {
                    title = (TextView) itemView.findViewById(C4337R.id.locale);
                }
                title.setText(C4337R.string.system_locale_title);
                title.setContentDescription(itemView.getContext().getResources().getString(C4337R.string.system_locale_title));
                return itemView;
        }
    }
    ...
    private View getNewViewIfNeeded(View convertView, ViewGroup parent, int itemType, int position) {
        ...
        switch (itemType) {
            ...
            case 1:
            case 3:
                boolean shouldReuseView = (convertView instanceof TextView) && convertView.findViewById(C4337R.id.section_header) != null;
                if (!shouldReuseView) {
                    break;
                }
                break;
            case 2:
            default:
                boolean shouldReuseView2 = (!(convertView instanceof ViewGroup) || convertView.findViewById(C4337R.id.locale) == null || convertView.findViewById(C4337R.id.divider) == null) ? false : true;
                if (!shouldReuseView2) {
                    updatedView = new SemLocalePickerItemView(parent.getContext(), 1, this.mInflater);
                }
                TextView text = (TextView) updatedView.findViewById(C4337R.id.locale);
                LocaleStore.LocaleInfo item = (LocaleStore.LocaleInfo) getItem(position);
                ...
                View divder = updatedView.findViewById(C4337R.id.divider);
            ...
            case 4:
                ...
                boolean shouldReuseView4 = shouldReuseView3 && convertView.findViewById(C4337R.id.external_divider) != null;
                if (!shouldReuseView4) {
                    View updatedView2 = this.mInflater.inflate(C4337R.layout.sec_app_language_picker_current_locale_item, parent, false);
                    addStateDescriptionIntoCurrentLocaleItem(updatedView2);
                    updatedView2.findViewById(C4337R.id.language_picker_item).findViewById(C4337R.id.divider).setVisibility(8);
                    updatedView2.findViewById(C4337R.id.external_divider).setVisibility(0);
            ...
            case 5:
                if (((LocaleStore.LocaleInfo) getItem(position)).isAppCurrentLocale()) {
                    boolean shouldReuseView5 = (convertView instanceof LinearLayout) && convertView.findViewById(C4337R.id.language_picker_item) != null;
                    if (!shouldReuseView5) {
                        View updatedView3 = this.mInflater.inflate(C4337R.layout.sec_app_language_picker_current_locale_item, parent, false);
                        addStateDescriptionIntoCurrentLocaleItem(updatedView3);
                        updatedView3.findViewById(C4337R.id.language_picker_item).findViewById(C4337R.id.divider).setVisibility(8);
                        updatedView3.findViewById(C4337R.id.external_divider).setVisibility(0);
                        semApplyRoundedCorner(updatedView3, position);
                        break;
                    }
                } else {
                    boolean shouldReuseView6 = (convertView instanceof TextView) && convertView.findViewById(C4337R.id.locale) != null;
                    if (!shouldReuseView6) {
                        View updatedView4 = this.mInflater.inflate(C4337R.layout.language_picker_item, parent, false);
    ...
    private void addStateDescriptionIntoCurrentLocaleItem(View root) {
        String description = root.getContext().getResources().getString(C4337R.string.checked);
