...
public class ConfirmUserCreationActivity extends AlertActivity
    ...
    try {
      ...
      if (this.mAccountName != null
          ...
          && (AccountManager.get(this).someUserHasAccount(account)
              | this.mUserManager.someUserHasSeedAccount(this.mAccountName, this.mAccountType))) {
        accountExists = true;
      }
      ...
      if (!isUserPropertyWithinLimit(this.mUserName, 100)
          ...
          || ((persistableBundle = this.mAccountOptions) != null
              && !persistableBundle.isBundleContentsWithinLengthLimit(1000))) {
        setResult(1);
        Log.m98i(TAG, "User properties must not exceed their character limits");
        return null;
      }
      ...
      if (accountExists) {
        String message =
            getString(C4337R.string.user_creation_account_exists, appName, this.mAccountName);
        return message;
      }
      String message2 = this.mAccountName;
      return getString(C4337R.string.user_creation_adding, appName, message2);
  ...
  public void onClick(DialogInterface dialog, int which) {
    ...
    if (which == -1 && this.mCanProceed && this.mIsFirstClick) {
      ...
      Log.m98i(TAG, "Ok, creating user");
      UserInfo user =
          this.mUserManager.createUser(this.mUserName, "android.os.usertype.full.SECONDARY", 0);
      if (user == null) {
        Log.m96e(TAG, "Couldn't create user");
        finish();
        return;
      } else {
        this.mUserManager.setSeedAccountData(
            user.f54id, this.mAccountName, this.mAccountType, this.mAccountOptions);
        setResult(-1);
      }
    }
    finish();
