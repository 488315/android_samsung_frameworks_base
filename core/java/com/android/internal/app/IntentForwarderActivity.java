...
public class IntentForwarderActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Slog.m113d(TAG, "forward intent from : " + this.mFinalCallingUserId + " to " + this.mFinalTargetUserId);
        if (newIntent == null) {
    ...
    private void maybeShowUserConsentMiniResolver(ResolveInfo target, final Intent launchIntent, UserInfo managedProfile) {
        ...
        if (privilegedCallerAskedToSkipUserConsent || intentToLaunchProfileOwner) {
            Log.m98i("IntentForwarderActivity", String.format("Skipping user consent for redirection into the managed profile for intent [%s], privilegedCallerAskedToSkipUserConsent=[%s], intentToLaunchProfileOwner=[%s]", launchIntent, Boolean.valueOf(privilegedCallerAskedToSkipUserConsent), Boolean.valueOf(intentToLaunchProfileOwner)));
            startActivityAsCaller(launchIntent, targetUserId);
            finish();
            return;
        }
        Log.m98i("IntentForwarderActivity", String.format("Showing user consent for redirection into the managed profile for intent [%s] and  calling package [%s]", launchIntent, callingPackage));
        setContentView(C4337R.layout.miniresolver);
        findViewById(C4337R.id.title_container).setElevation(0.0f);
        PackageManager packageManagerForTargetUser = createContextAsUser(UserHandle.m55of(targetUserId), 0).getPackageManager();
        ...
        ((TextView) findViewById(C4337R.id.open_cross_profile)).setText(getOpenInWorkMessage(launchIntent, target.loadLabel(packageManagerForTargetUser)));
        ((Button) findViewById(C4337R.id.use_same_profile_browser)).setText(17039360);
        findViewById(C4337R.id.use_same_profile_browser).setOnClickListener(new View.OnClickListener() { // from class: com.android.internal.app.IntentForwarderActivity$$ExternalSyntheticLambda8
        ...
        findViewById(C4337R.id.button_open).setOnClickListener(new View.OnClickListener() { // from class: com.android.internal.app.IntentForwarderActivity$$ExternalSyntheticLambda9
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
        ...
        if ((isDialerIntent(launchIntent) || isTextMessageIntent(launchIntent)) && devicePolicyManager.getManagedSubscriptionsPolicy().getPolicyType() == 1) {
            ...
            ((TextView) findViewById(C4337R.id.miniresolver_info_section_text)).setText(getWorkTelephonyInfoSectionMessage(launchIntent));
        } else {
    ...
    public /* synthetic */ void lambda$maybeShowUserConsentMiniResolver$3(Intent launchIntent, int targetUserId, View v) {
        startActivityAsCaller(launchIntent, ActivityOptions.makeCustomAnimation(getApplicationContext(), C4337R.anim.activity_open_enter, C4337R.anim.push_down_out).toBundle(), false, targetUserId);
        finish();
    }
    ...
    private Drawable getAppIcon(ResolveInfo target, Intent launchIntent, int targetUserId, PackageManager packageManagerForTargetUser) {
        if (isDialerIntent(launchIntent)) {
            ...
            String defaultDialerPackageName = telecomManager.getDefaultDialerPackage(UserHandle.m55of(targetUserId));
            try {
    ...
    private String getForwardToPersonalMessageFromSecureFolder() {
        return String.format(getString(C4337R.string.forward_intent_from), getString(C4337R.string.secure_folder));
    }
    ...
    private void startActivityAsCaller(Intent newIntent, int userId) {
        ...
        try {
            ...
            if (drivingIntent != null && ((drivingIntent.getAction().equals(Intent.ACTION_SEND) || drivingIntent.getAction().equals(Intent.ACTION_SEND_MULTIPLE)) && (SemPersonaManager.isSecureFolderId(userId) || SemPersonaManager.isSecureFolderId(callingUserId)))) {
                ...
                getResources();
                Intent ChooserIntent = Intent.createChooser(drivingIntent, Resources.getSystem().getText(C4337R.string.whichSendApplication));
    ...
    private void launchResolverActivityWithCorrectTab(Intent intentReceived, String className, Intent newIntent, int callingUserId, int targetUserId) {
        ...
        sanitizeIntent(intentReceived);
        intentReceived.putExtra("com.android.internal.app.ResolverActivity.EXTRA_SELECTED_PROFILE", selectedProfile);
        intentReceived.putExtra("com.android.internal.app.ResolverActivity.EXTRA_CALLING_USER", UserHandle.m55of(callingUserId));
        startActivityAsCaller(intentReceived, null, false, userId);
        finish();
    }
            ...
            while (true) {
                ...
                if (tempUri != null) {
                    if (tempUri.getUserInfo() != null && !tempUri.getUserInfo().equals(String.valueOf(0))) {
                        ...
                        Slog.m113d(TAG, "isAbleToUsingCopyToSecureFolder | false | due to userId | " + tempUri.getUserInfo());
                        break;
                    }
