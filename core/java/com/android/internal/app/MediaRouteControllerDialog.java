...
public class MediaRouteControllerDialog extends AlertDialog {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setButton(
        -2,
        res.getString(C4337R.string.media_route_controller_disconnect),
        new DialogInterface
            .OnClickListener() { // from class:
                                 // com.android.internal.app.MediaRouteControllerDialog.1
    ...
    View customView =
        getLayoutInflater().inflate(C4337R.layout.media_route_controller_dialog, (ViewGroup) null);
    setView(customView, 0, 0, 0, 0);
    super.onCreate(savedInstanceState);
    View customPanelView = getWindow().findViewById(C4337R.id.customPanel);
    if (customPanelView != null) {
      customPanelView.setMinimumHeight(0);
    }
    this.mVolumeLayout =
        (LinearLayout) customView.findViewById(C4337R.id.media_route_volume_layout);
    SeekBar seekBar = (SeekBar) customView.findViewById(C4337R.id.media_route_volume_slider);
    this.mVolumeSlider = seekBar;
    seekBar.setOnSeekBarChangeListener(
        new SeekBar
            .OnSeekBarChangeListener() { // from class:
                                         // com.android.internal.app.MediaRouteControllerDialog.2
          private final Runnable mStopTrackingTouch =
              new Runnable() { // from class:
                               // com.android.internal.app.MediaRouteControllerDialog.2.1
          ...
          public void onStartTrackingTouch(SeekBar seekBar2) {
            if (MediaRouteControllerDialog.this.mVolumeSliderTouched) {
              MediaRouteControllerDialog.this.mVolumeSlider.removeCallbacks(
                  this.mStopTrackingTouch);
            } else {
          ...
          public void onStopTrackingTouch(SeekBar seekBar2) {
            MediaRouteControllerDialog.this.mVolumeSlider.postDelayed(
                this.mStopTrackingTouch, 250L);
          }
    ...
    if (update()) {
      ...
      FrameLayout controlFrame =
          (FrameLayout) customView.findViewById(C4337R.id.media_route_control_frame);
  ...
  private Drawable obtainMediaRouteButtonDrawable() {
    ...
    int[] drawableAttrs = {C4337R.attr.externalRouteEnabledDrawable};
  ...
  private final class MediaRouterCallback extends MediaRouter.SimpleCallback {
    ...
    public void onRouteGrouped(
        MediaRouter router, MediaRouter.RouteInfo info, MediaRouter.RouteGroup group, int index) {
      MediaRouteControllerDialog.this.update();
    }
