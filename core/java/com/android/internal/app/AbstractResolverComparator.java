...
public abstract class AbstractResolverComparator
  ...
  public AbstractResolverComparator(
      Context launchedFromContext, Intent intent, UserHandle targetUserSpace) {
  ...
  public AbstractResolverComparator(
    ...
    this.mHandler =
        new Handler(
          ...
          public void handleMessage(Message msg) {
            switch (msg.what) {
              case 0:
                Log.m94d(AbstractResolverComparator.TAG, "RANKER_SERVICE_RESULT");
              ...
              case 1:
                Log.m94d(
                    AbstractResolverComparator.TAG, "RANKER_RESULT_TIMEOUT; unbinding services");
                AbstractResolverComparator.this.mHandler.removeMessages(0);
                AbstractResolverComparator.this.afterCompute();
                if (AbstractResolverComparator.this.mChooserActivityLogger != null) {
                  AbstractResolverComparator.this.mChooserActivityLogger
    ...
    this.mHttp =
        IntentFilter.SCHEME_HTTP.equals(scheme) || IntentFilter.SCHEME_HTTPS.equals(scheme);
    ...
    for (UserHandle user : targetUserSpaceList) {
      ...
      this.mUsmMap.put(
          user, (UsageStatsManager) userContext.getSystemService(Context.USAGE_STATS_SERVICE));
    }
  ...
  private void getContentAnnotations(Intent intent) {
    try {
      ArrayList<String> annotations =
          intent.getStringArrayListExtra(Intent.EXTRA_CONTENT_ANNOTATIONS);
      if (annotations != null) {
      ...
      Log.m98i(TAG, "Couldn't unparcel intent annotations. Ignoring.");
    ...
    if (this.mUsmMap.containsKey(user)) {
      this.mUsmMap
          ...
          .reportChooserSelection(
              packageName, user.getIdentifier(), this.mContentType, this.mAnnotations, action);
    }
  }
  ...
  void beforeCompute() {
    Log.m94d(TAG, "Setting watchdog timer for 500ms");
    Handler handler = this.mHandler;
    if (handler == null) {
      Log.m94d(TAG, "Error: Handler is Null; Needs to be initialized.");
    } else {
