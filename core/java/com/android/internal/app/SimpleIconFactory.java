...
public class SimpleIconFactory {
  ...
  private static int getIconSizeFromContext(Context ctx) {
    return getAttrDimFromContext(
        ctx, C4337R.attr.iconfactoryIconSize, "Expected theme to define iconfactoryIconSize.");
  }

  private static int getBadgeSizeFromContext(Context ctx) {
    return getAttrDimFromContext(
        ctx, C4337R.attr.iconfactoryBadgeSize, "Expected theme to define iconfactoryBadgeSize.");
  }
  ...
  private Bitmap createIconBitmap(
      Drawable icon, float scale, int size, boolean insetAdiForShadow, boolean ignoreAdiMask) {
    ...
    if (icon instanceof AdaptiveIconDrawable) {
      ...
      if (ignoreAdiMask) {
        ...
        final Rect childRect =
            new Rect(cX - insetWidth, cY - insetHeight, cX + insetWidth, cY + insetHeight);
  ...
  private Drawable normalizeAndWrapToAdaptiveIcon(
    ...
    if (this.mWrapperIcon == null) {
      this.mWrapperIcon =
          this.mContext
              .getDrawable(C4337R.drawable.iconfactory_adaptive_icon_drawable_wrapper)
              .mutate();
    }
  ...
  */
    ...
    if (width > 0 && height > 0) {
      ...
      if (topY == -1 && rightX != -1) {
        ...
        if (outBounds == null) {
          ...
          outBounds.set(
              this.mBounds.left / width,
              this.mBounds.top / rectArea2,
              1.0f - (this.mBounds.right / width),
              1.0f - (this.mBounds.bottom / height));
        }
  ...
  private synchronized void recreateIcon(
      Bitmap icon, BlurMaskFilter blurMaskFilter, int ambientAlpha, int keyAlpha, Canvas out) {
  ...
  public static class FixedScaleDrawable extends DrawableWrapper {
    ...
    public void draw(Canvas canvas) {
      ...
      canvas.scale(
          this.mScaleX, this.mScaleY, getBounds().exactCenterX(), getBounds().exactCenterY());
      super.draw(canvas);
