...
public class SimpleIconFactory {
    ...
    private static int getIconSizeFromContext(Context ctx) {
        return getAttrDimFromContext(ctx, C4337R.attr.iconfactoryIconSize, "Expected theme to define iconfactoryIconSize.");
    }

    private static int getBadgeSizeFromContext(Context ctx) {
        return getAttrDimFromContext(ctx, C4337R.attr.iconfactoryBadgeSize, "Expected theme to define iconfactoryBadgeSize.");
    }
    ...
    private Drawable normalizeAndWrapToAdaptiveIcon(Drawable icon, RectF outIconBounds, float[] outScale) {
        if (this.mWrapperIcon == null) {
            this.mWrapperIcon = this.mContext.getDrawable(C4337R.drawable.iconfactory_adaptive_icon_drawable_wrapper).mutate();
        }
