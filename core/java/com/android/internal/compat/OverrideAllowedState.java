  ...
  public void enforce(long changeId, String packageName) throws SecurityException {
    switch (this.state) {
      ...
      case 3:
        throw new SecurityException(
            String.format(
                "Cannot override %1$d for %2$s because the app's targetSdk (%3$d) is above the"
                    + " change's targetSdk threshold (%4$d)",
      ...
      case 6:
        throw new SecurityException(
            String.format(
                "Cannot override %1$d for %2$s because the change's targetSdk threshold (%3$d) is"
                    + " above the platform sdk.",
                Long.valueOf(changeId), packageName, Integer.valueOf(this.changeIdTargetSdk)));
      default:
        return;
    }
  }
