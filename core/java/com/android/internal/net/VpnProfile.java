...
public final class VpnProfile implements Cloneable, Parcelable {
  ...
  public static final Parcelable.Creator<VpnProfile> CREATOR =
      new Parcelable.Creator<VpnProfile>() { // from class: com.android.internal.net.VpnProfile.1
  ...
  public VpnProfile(Parcel in) {
    ...
    PersistableBundle bundle =
        (PersistableBundle)
            in.readParcelable(PersistableBundle.class.getClassLoader(), PersistableBundle.class);
    this.ikeTunConnParams =
        bundle != null ? TunnelConnectionParamsUtils.fromPersistableBundle(bundle) : null;
  ...
  public void writeToParcel(Parcel parcel, int i) {
    ...
    parcel.writeParcelable(
        ikeTunnelConnectionParams == null
            ? null
            : TunnelConnectionParamsUtils.toPersistableBundle(ikeTunnelConnectionParams),
        i);
  ...
  public static VpnProfile decode(String key, byte[] value) {
    ...
    try {
      ...
      if (values.length <= 23 || values.length >= 28) {
        if ((values.length <= 32 || values.length >= 34) && values.length <= 34) {
          ...
          if (values.length >= 32 && values[31].length() != 0) {
            ...
            IkeTunnelConnectionParams tempIkeTunConnParams2 =
          ...
          VpnProfile profile =
              new VpnProfile(
                  ...
                  excludeLocalRoutes,
                  requiresInternetValidation,
                  tempIkeTunConnParams,
                  automaticNattKeepaliveTimerEnabled,
      ...
      e = e2;
      Log.m95d(TAG, "Got exception in decode.", e);
      return null;
    }
  }

  public byte[] encode(boolean encryptRequired) {
    Log.m98i(TAG, "encode: encrypt=" + encryptRequired);
    if (encryptRequired) {
      encrypt(this);
    }
    return encode();
  ...
  public int hashCode() {
    return Objects.hash(
        ...
        Boolean.valueOf(this.requiresInternetValidation),
        this.ikeTunConnParams,
        Boolean.valueOf(this.automaticNattKeepaliveTimerEnabled),
        Boolean.valueOf(this.automaticIpVersionSelectionEnabled));
  }
  ...
  public boolean equals(Object obj) {
    ...
    return Objects.equals(this.key, other.key)
        ...
        && Objects.equals(this.ikeTunConnParams, other.ikeTunConnParams)
        && this.automaticNattKeepaliveTimerEnabled == other.automaticNattKeepaliveTimerEnabled
  ...
  private static String bytes2Hex(byte[] bytes) {
    ...
    for (int i = 0; i < bytes.length; i++) {
      result
          .append("0123456789ABCDEF".charAt((bytes[i] >> 4) & 15))
  ...
  private static String[] doEncrypt(Key secretKey, String plainValue) {
    try {
      ...
      IvParameterSpec ivParamSpec =
          (IvParameterSpec) cipher.getParameters().getParameterSpec(IvParameterSpec.class);
      ...
      Log.m96e(TAG, "Failed to encrypt: " + e.toString());
      e.printStackTrace();
      return null;
    }
  }
  ...
  private static String doDecrypt(Key secretKey, String encryptedBase64, String ivHexBase64) {
    try {
      ...
      return new String(Base64.decode(decryptedBuffer, 2), StandardCharsets.UTF_8).intern();
        ...
        | IllegalBlockSizeException
        | NoSuchPaddingException e) {
      Log.m96e(TAG, "Failed to decrypt: " + e.toString());
      e.printStackTrace();
      return null;
    }
  }
    ...
    try {
      ...
      if (generate && !keyStore.containsAlias(VPN_SECRET_KEY)) {
        try {
          ...
          Log.m96e(TAG, "Failed to create key: " + e.toString());
          e.printStackTrace();
        }
      }
      ...
      Log.m96e(TAG, "Failed to get key: " + e2.toString());
      e2.printStackTrace();
      return null;
    }
  }
    ...
    if (!profile.isIpsecSecretIvParams.isEmpty() || !profile.isPasswordIvParams.isEmpty()) {
      try {
        ...
        if (secretKey != null) {
          ...
          if (!profile.ipsecSecret.isEmpty()
              && (ret2 = doDecrypt(secretKey, profile.ipsecSecret, profile.isIpsecSecretIvParams))
                  != null) {
            profile.ipsecSecret = ret2;
          }
          if (!profile.password.isEmpty()
              && (ret = doDecrypt(secretKey, profile.password, profile.isPasswordIvParams))
                  != null) {
        ...
        Log.m96e(TAG, "Error while decrypting profile: " + e);
        return;
      }
    }
    Log.m98i(TAG, "This profile was not encrypted:" + profile.name);
  }
