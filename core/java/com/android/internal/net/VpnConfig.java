...
public class VpnConfig implements Parcelable {
    ...
    public static Intent getIntentForConfirmation() {
        ...
        ComponentName componentName = ComponentName.unflattenFromString(Resources.getSystem().getString(C4337R.string.config_customVpnConfirmDialogComponent));
        ...
        if (!configOption) {
            ...
            while (true) {
                ...
                if (configLocal.session.equals(configSession)) {
                    Log.m94d(TAG, "getIntentForStatusPanelEnterpriseVpn : Removing iterator for profile : " + configSession);
                    iterator.remove();
                    break;
                }
            }
            Log.m94d(TAG, "getIntentForStatusPanelEnterpriseVpn : config size =  " + mConfigsReceived.size());
            if (mConfigsReceived.size() == 0) {
                Log.m94d(TAG, "getIntentForStatusPanelEnterpriseVpn : Returning null");
                return null;
            }
        } else {
            if (config != null) {
                Log.m94d(TAG, "getIntentForStatusPanelEnterpriseVpn : Adding iterator for profile : " + config.session);
            }
