...
import android.content.p002pm.dex.DexMetadataHelper;
...
public class InstallLocationUtils {
        ...
        sDefaultTestableInterface =
            new TestableInterface() { // from class:
              ...
              public boolean getAllow3rdPartyOnInternalConfig(Context context) {
                return context
                    .getResources()
                    .getBoolean(C4337R.bool.config_allow3rdPartyAppOnInternal);
              }
              ...
              public ApplicationInfo getExistingAppInfo(Context context, String packageName) {
                try {
                  ApplicationInfo existingInfo =
                      context.getPackageManager().getApplicationInfo(packageName, 4194304);
                  return existingInfo;
  ...
  public static String resolveInstallVolume(Context context, PackageInstaller.SessionParams params)
      throws IOException {
  ...
  public static String resolveInstallVolume(
    ...
    for (VolumeInfo vol : storageManager.getVolumes()) {
      if (vol.type == 1 && vol.isMountedWritable()) {
        boolean isInternalStorage = VolumeInfo.ID_PRIVATE_INTERNAL.equals(vol.f345id);
    ...
    if (!forceAllowOnExternal && params.installLocation == 1) {
      if (existingInfo != null
          && !Objects.equals(existingInfo.volumeUuid, StorageManager.UUID_PRIVATE_INTERNAL)) {
        throw new IOException(
    ...
    if (volumePaths.size() == 1) {
      ...
      for (String vol2 : volumePaths.keySet()) {
        ...
        UUID target = storageManager.getUuidForPath(new File(volumePath));
        long availBytes =
            storageManager.getAllocatableBytes(target, translateAllocateFlags(params.installFlags));
    ...
    if (!volumePaths.isEmpty()
        ...
        && SystemProperties.getBoolean("debug.pm.install_skip_size_check_for_maxint", false)) {
      return bestCandidate;
    }
    throw new IOException(
    ...
    if (storageVolumes != null) {
      for (StorageVolume volume : storageVolumes) {
        if ("sd".equals(volume.getSubSystem()) && volume.isRemovable()) {
          Log.m94d(TAG, "getExternalStorageSdPath: " + volume.getPath());
          return params.sizeBytes > 0
              && !emulated
              && params.sizeBytes <= storage.getStorageBytesUntilLow(volume.getPathFile());
        }
      }
      Log.m96e(TAG, "Cannot find fitsOnExternal volume");
      return false;
    }
    Log.m96e(TAG, "storageVolumes is null");
    return false;
  }

  private static boolean hasActiveContainer(StorageManager storage) {
    if (storage == null) {
      Log.m96e(TAG, "hasActiveContainer, StorageManager is null");
      return false;
    }
    StorageVolume[] storageVolumes = storage.getVolumeList();
  ...
  public static long calculateInstalledSize(PackageLite pkg, String abiOverride, FileDescriptor fd)
      throws IOException {
  ...
  public static long calculateInstalledSize(
    ...
    return sizeBytes
        + DexMetadataHelper.getPackageDexMetadataSize(pkg)
