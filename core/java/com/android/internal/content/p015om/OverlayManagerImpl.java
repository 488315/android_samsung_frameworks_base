...
import android.content.p001om.OverlayIdentifier;
import android.content.p001om.OverlayInfo;
import android.content.p001om.OverlayManagerTransaction;
import android.content.p002pm.ApplicationInfo;
import android.content.p002pm.PackageManager;
import android.content.p002pm.parsing.FrameworkParsingPackageUtils;
import android.p009os.FabricatedOverlayInfo;
import android.p009os.FabricatedOverlayInternal;
import android.p009os.FabricatedOverlayInternalEntry;
...
public class OverlayManagerImpl {
  ...
  private static native void createFrroFile(
      String str, FabricatedOverlayInternal fabricatedOverlayInternal) throws IOException;
  ...
  private static native FabricatedOverlayInfo getFabricatedOverlayInfo(String str)
      throws IOException;
  ...
  private static void cleanExpiredOverlays(
      Path selfTargetingBasePath, Path folderForCurrentBaseApk) {
    try {
      ...
      Files.walkFileTree(
          ...
          new SimpleFileVisitor<
              Path>() { // from class: com.android.internal.content.om.OverlayManagerImpl.1
            ...
            public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs)
              ...
              return super.preVisitDirectory((C44351) dir, attrs);
            }
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
              ...
              if (!file.toFile().delete()) {
                Log.m102w(OverlayManagerImpl.TAG, "Failed to delete file " + file);
              }
              return super.visitFile((C44351) file, attrs);
            }
            ...
            public FileVisitResult postVisitDirectory(Path dir, IOException exc)
              ...
              if (!fileName.equals(currentBaseFolder)
                  ...
                  && !dir.toFile().delete()) {
                Log.m102w(OverlayManagerImpl.TAG, "Failed to delete dir " + dir);
              }
              return super.postVisitDirectory((C44351) dir, exc);
            }
          });
    } catch (IOException e) {
      Log.m102w(TAG, "Unknown fail " + e);
    }
  }
  ...
  public void ensureBaseDir() {
    ...
    if (!baseFile.exists()) {
      if (!baseFile.mkdirs()) {
        Log.m102w(TAG, "Failed to create directory " + baseFile);
      }
  ...
  private boolean isSameWithTargetSignature(String targetPackage) {
    ...
    return TextUtils.equals(packageName, targetPackage)
        || packageManager.checkSignatures(packageName, targetPackage) == 0;
  }
  ...
  public static String checkOverlayNameValid(String name) {
    ...
    String checkOverlayNameResult =
        FrameworkParsingPackageUtils.validateName(overlayName, false, true);
    Preconditions.checkArgument(
  ...
  private void checkPackageName(String packageName) {
    ...
    Preconditions.checkArgument(
        TextUtils.equals(this.mContext.getPackageName(), packageName),
        TextUtils.formatSimple(
            "UID %d doesn't own the package %s", Integer.valueOf(Process.myUid()), packageName));
  }
  ...
  */
  public void registerFabricatedOverlay(FabricatedOverlayInternal overlayInternal)
    ...
    List<FabricatedOverlayInternalEntry> entryList =
        (List) Objects.requireNonNull(overlayInternal.entries);
    boolean z2 = true;
    ...
    String targetPackage =
        (String) Preconditions.checkStringNotEmpty(applicationInfo.getBaseCodePath());
      ...
      if (!applicationInfo.isSystemApp()) {
        try {
          ...
          if (!frroPath.toFile().delete()) {
            Log.m102w(TAG, "Failed to delete file " + frroPath);
          }
          throw e;
        }
      }
  ...
  public void unregisterFabricatedOverlay(String overlayName) {
    ...
    if (!idmapPath.toFile().delete()) {
      Log.m102w(TAG, "Failed to delete file " + idmapPath);
    }
  }
    ...
    while (it.hasNext()) {
      ...
      if (request.type == 2) {
        ...
        OverlayIdentifier overlayIdentifier =
            (OverlayIdentifier) Objects.requireNonNull(request.overlay);
    ...
    File[] frroFiles =
        base.listFiles(
            new FilenameFilter() { // from class:
                                   // com.android.internal.content.om.OverlayManagerImpl$$ExternalSyntheticLambda0
  ...
  static /* synthetic */ boolean lambda$getOverlayInfosForTarget$0(File dir, String name) {
    ...
    String idmapFileName =
        name.substring(0, name.length() - FRRO_EXTENSION.length()) + IDMAP_EXTENSION;
