...
public abstract class PackageMonitor extends BroadcastReceiver {
    ...
    if (user != null) {
      ...
      if (externalStorage) {
        context.registerReceiverAsUser(
            this, user, this.mExternalFilt, null, this.mRegisteredHandler);
        return;
      }
      return;
    }
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    if (intExtra == -10000) {
      Slog.m121w(TAG, "Intent broadcast does not contain user handle: " + intent);
      return;
    }
    ...
    if ("android.intent.action.PACKAGE_ADDED".equals(action)) {
      ...
      boolean canRestart =
          onHandleForceStop(
              intent, stringArrayExtra2, intent.getIntExtra(Intent.EXTRA_UID, 0), false);
      if (canRestart) {
        setResultCode(-1);
      }
