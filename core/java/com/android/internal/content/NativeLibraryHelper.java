...
import android.content.p002pm.parsing.ApkLiteParseUtils;
import android.content.p002pm.parsing.PackageLite;
...
public class NativeLibraryHelper {
  ...
  private static native int nativeCopyNativeBinaries(
      long j, String str, String str2, boolean z, boolean z2);
  ...
  public static class Handle implements Closeable {
    ...
    public static Handle create(File packageFile) throws IOException {
      ...
      ParseResult<PackageLite> ret =
          ApkLiteParseUtils.parsePackageLite(input.reset(), packageFile, 0);
      if (ret.isError()) {
    ...
    public static Handle create(PackageLite lite) throws IOException {
      return create(
          lite.getAllApkPaths(),
          lite.isMultiArch(),
          lite.isExtractNativeLibs(),
          lite.isDebuggable());
    }

    public static Handle create(
        List<String> codePaths, boolean multiArch, boolean extractNativeLibs, boolean debuggable)
        throws IOException {
      ...
      if (files != null) {
        for (int nn = 0; nn < files.length; nn++) {
          if (files[nn].isDirectory()) {
            ...
            Slog.m121w(TAG, "Could not delete native binary: " + files[nn].getPath());
          }
        }
      }
      if (deleteRootDir && !nativeLibraryRoot.delete()) {
        Slog.m121w(TAG, "Could not delete native binary directory: " + nativeLibraryRoot.getPath());
      }
    }
  }
  ...
  public static int copyNativeBinariesForSupportedAbi(
      Handle handle,
      File libraryRoot,
      String[] abiList,
  ...
  public static int copyNativeBinariesWithOverride(
      Handle handle, File libraryRoot, String abiOverride, boolean isIncremental) {
    int copyRet;
    int copyRet2;
    try {
      if (handle.multiArch) {
        if (abiOverride != null && !CLEAR_ABI_OVERRIDE.equals(abiOverride)) {
          Slog.m121w(TAG, "Ignoring abiOverride for multi arch application.");
        }
        if (Build.SUPPORTED_32_BIT_ABIS.length > 0
            && (copyRet2 =
                    copyNativeBinariesForSupportedAbi(
                        handle, libraryRoot, Build.SUPPORTED_32_BIT_ABIS, true, isIncremental))
                < 0
            && copyRet2 != -114
            && copyRet2 != -113) {
          Slog.m121w(TAG, "Failure copying 32 bit native libraries; copyRet=" + copyRet2);
          return copyRet2;
        }
        if (Build.SUPPORTED_64_BIT_ABIS.length > 0
            && (copyRet =
                    copyNativeBinariesForSupportedAbi(
                        handle, libraryRoot, Build.SUPPORTED_64_BIT_ABIS, true, isIncremental))
                < 0
            && copyRet != -114
            && copyRet != -113) {
          Slog.m121w(TAG, "Failure copying 64 bit native libraries; copyRet=" + copyRet);
          return copyRet;
        }
      } else {
        ...
        String[] abiList =
            cpuAbiOverride != null ? new String[] {cpuAbiOverride} : Build.SUPPORTED_ABIS;
        if (Build.SUPPORTED_64_BIT_ABIS.length > 0
            && cpuAbiOverride == null
        ...
        int copyRet3 =
            copyNativeBinariesForSupportedAbi(handle, libraryRoot, abiList, true, isIncremental);
        if (copyRet3 < 0 && copyRet3 != -114) {
          Slog.m121w(
              TAG,
              "Failure copying native libraries [errorCode="
                  + copyRet3
      ...
      Slog.m116e(TAG, "Copying native libraries failed", e);
      return -110;
    }
  }
    ...
    if (handle.multiArch) {
      ...
      long sum =
          Build.SUPPORTED_32_BIT_ABIS.length > 0
              ? 0 + sumNativeBinariesForSupportedAbi(handle, Build.SUPPORTED_32_BIT_ABIS)
              : 0L;
      if (Build.SUPPORTED_64_BIT_ABIS.length > 0) {
  ...
  private static int incrementalConfigureNativeBinariesForSupportedAbi(
      Handle handle, File libSubDir, String abi) {
    String[] apkPaths = handle.apkPaths;
    if (apkPaths == null || apkPaths.length == 0) {
      Slog.m115e(TAG, "No apks to extract native libraries from.");
      return -110;
    }
    ...
    IncrementalManager incrementalManager =
        new IncrementalManager(IIncrementalService.Stub.asInterface(incrementalService));
    ...
    if (incrementalStorage == null) {
      Slog.m115e(TAG, "Failed to find incremental storage");
      return -110;
    }
    ...
    for (String str : apkPaths) {
      if (!incrementalStorage.configureNativeBinaries(
          str, libRelativeDir, abi, handle.extractNativeLibs)) {
        return -110;
      }
    }
    return 1;
  ...
  private static String getRelativePath(File base, File target) {
    try {
      ...
      Slog.m115e(
          TAG,
          "Failed to find relative path between: "
