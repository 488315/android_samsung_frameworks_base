  ...
  class BinderC46651 extends ICallEventCallback.Stub {
    ...
    private void handleCallEventCallback(
      ...
      TransactionalCall call =
          (TransactionalCall)
              ClientTransactionalServiceWrapper.this.mCallIdToTransactionalCall.get(callId);
      if (call != null) {
        final CallControlCallback callback = call.getCallControlCallback();
        final ReceiverWrapper outcomeReceiverWrapper =
            ClientTransactionalServiceWrapper.this.new ReceiverWrapper(ackResultReceiver);
                ...
                .execute(
                    new Runnable() { // from class:
                      ...
                      public final void run() {
                        ClientTransactionalServiceWrapper.BinderC46651.this
                            .lambda$handleCallEventCallback$0(
                                action, callback, outcomeReceiverWrapper, args, callId);
                      }
                    });
          } catch (Exception e) {
            Log.m96e(
    ...
    public void onAddCallControl(
      ...
      Log.m98i(
          ClientTransactionalServiceWrapper.TAG,
          TextUtils.formatSimple("oACC: id=[%s], code=[%d]", callId, Integer.valueOf(resultCode)));
      TransactionalCall call =
          (TransactionalCall)
              ClientTransactionalServiceWrapper.this.mCallIdToTransactionalCall.get(callId);
      if (call != null) {
        ...
        if (resultCode == 0) {
          CallControl control =
              new CallControl(
                  callId,
                  callControl,
                  ClientTransactionalServiceWrapper.this.mRepository,
                  ClientTransactionalServiceWrapper.this.mPhoneAccountHandle);
        ...
        try {
          executor.execute(
              new Runnable() { // from class:
                ...
                @Override // java.lang.Runnable
                public final void run() {
                  ClientTransactionalServiceWrapper.BinderC46651.lambda$handleEventCallback$1(
                      action, callback, arg);
                }
              });
        } finally {
    ...
    public void onCallStreamingFailed(String callId, int reason) {
      Log.m98i(
          ClientTransactionalServiceWrapper.TAG,
          TextUtils.formatSimple("oCSF: id=[%s], reason=[%s]", callId, Integer.valueOf(reason)));
