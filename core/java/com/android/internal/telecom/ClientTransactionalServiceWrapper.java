...
public class ClientTransactionalServiceWrapper {
    ...
    private final ICallEventCallback mCallEventCallback = new BinderC46651();
    ...
    public void untrackCall(String callId) {
        Log.m98i(TAG, TextUtils.formatSimple("removeCall: with id=[%s]", callId));
    ...
    class BinderC46651 extends ICallEventCallback.Stub {
        ...
        private void handleCallEventCallback(final String action, final String callId, ResultReceiver ackResultReceiver, final Object... args) {
            Log.m98i(ClientTransactionalServiceWrapper.TAG, TextUtils.formatSimple("hCEC: id=[%s], action=[%s]", callId, action));
                    ...
                    try {
                        call.getExecutor().execute(new Runnable() { // from class: com.android.internal.telecom.ClientTransactionalServiceWrapper$1$$ExternalSyntheticLambda0
                            ...
                            public final void run() {
                                ClientTransactionalServiceWrapper.BinderC46651.this.lambda$handleCallEventCallback$0(action, callback, outcomeReceiverWrapper, args, callId);
                            }
                        });
                    } catch (Exception e) {
                        Log.m96e(ClientTransactionalServiceWrapper.TAG, ClientTransactionalServiceWrapper.EXECUTOR_FAIL_MSG + e);
                    }
                } finally {
        ...
        public void onAddCallControl(String callId, int resultCode, ICallControl callControl, CallException transactionalException) {
            Log.m98i(ClientTransactionalServiceWrapper.TAG, TextUtils.formatSimple("oACC: id=[%s], code=[%d]", callId, Integer.valueOf(resultCode)));
            TransactionalCall call = (TransactionalCall) ClientTransactionalServiceWrapper.this.mCallIdToTransactionalCall.get(callId);
            if (call != null) {
            ...
            ClientTransactionalServiceWrapper.this.untrackCall(callId);
            Log.m96e(ClientTransactionalServiceWrapper.TAG, "oACC: TransactionalCall object not found for call w/ id=" + callId);
        }
        ...
        public void handleEventCallback(String callId, final String action, final Object arg) {
            Log.m94d(ClientTransactionalServiceWrapper.TAG, TextUtils.formatSimple("hEC: [%s], callId=[%s]", action, callId));
            TransactionalCall call = (TransactionalCall) ClientTransactionalServiceWrapper.this.mCallIdToTransactionalCall.get(callId);
            if (call != null) {
                ...
                try {
                    executor.execute(new Runnable() { // from class: com.android.internal.telecom.ClientTransactionalServiceWrapper$1$$ExternalSyntheticLambda1
                        @Override // java.lang.Runnable
                        public final void run() {
                            ClientTransactionalServiceWrapper.BinderC46651.lambda$handleEventCallback$1(action, callback, arg);
                        }
                    });
                } finally {
                    Binder.restoreCallingIdentity(identity);
        ...
        public void onCallStreamingFailed(String callId, int reason) {
            Log.m98i(ClientTransactionalServiceWrapper.TAG, TextUtils.formatSimple("oCSF: id=[%s], reason=[%s]", callId, Integer.valueOf(reason)));
