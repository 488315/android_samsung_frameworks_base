...
public class PersistentServiceConnection<T> extends ObservableServiceConnection<T> {
  ...
  public PersistentServiceConnection(
    ...
    this.mConnectionCallback =
        new ObservableServiceConnection.Callback<
            T>() { // from class: com.android.internal.util.PersistentServiceConnection.1
          ...
          public void onDisconnected(ObservableServiceConnection<T> connection, int reason) {
            ...
            synchronized (PersistentServiceConnection.this.mLock) {
              if (PersistentServiceConnection.this.mInjector.uptimeMillis() - this.mConnectedTime
    ...
    this.mConnectRunnable =
        new Runnable() { // from class: com.android.internal.util.PersistentServiceConnection.2
  ...
  public void scheduleConnectionAttemptLocked() {
    ...
    long reconnectDelayMs = (long) Math.scalb(this.mBaseReconnectDelayMs, i);
