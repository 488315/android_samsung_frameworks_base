...
public class ContrastColorUtil {
  ...
  private ContrastColorUtil(Context context) {
    this.mGrayscaleIconMaxSize =
        context
            .getResources()
            .getDimensionPixelSize(C4337R.dimen.notification_gray_scale_size_limit);
  }
    ...
    synchronized (obj) {
      ...
      synchronized (obj) {
        this.mGrayscaleBitmapCache.put(
            bitmap, Pair.create(Boolean.valueOf(result), Integer.valueOf(generationId)));
      }
      if (!result) {
        Log.m94d(TAG, "GrayScale=false. Bitmap is not grayscale.");
      }
      return result;
    }
  }
  ...
  public boolean isGrayscaleIcon(Context context, int drawableResId) {
    ...
    try {
      ...
      Log.m96e(TAG, "Drawable not found: " + drawableResId);
      return false;
    }
  }
            ...
            if (fullLength) {
              ...
              resultSpan =
                  new ForegroundColorSpan(
                      ensureLargeTextContrast(foregroundColor, background, isBgDark2));
            }
          } else {
            resultSpan = span;
          }
  ...
  public static int resolveColor(Context context, int color, boolean defaultBackgroundIsDark) {
    ...
    if (color == 0) {
      if (defaultBackgroundIsDark) {
        res = C4337R.color.notification_default_color_dark;
      } else {
        res = C4337R.color.notification_default_color_light;
      }
  ...
  public static int changeColorLightness(int baseColor, int amount) {
    ...
    result[0] =
        Math.max(Math.min(100.0d, result[0] + amount), SContextConstants.ENVIRONMENT_VALUE_UNKNOWN);
  ...
  public static int resolvePrimaryColor(
    ...
    if (useDark) {
      return context.getColor(C4337R.color.notification_primary_text_color_light);
    }
    return context.getColor(C4337R.color.notification_primary_text_color_dark);
  }
  ...
  public static int resolveSecondaryColor(
    ...
    boolean useDark = shouldUseDark(backgroundColor, defaultBackgroundIsDark);
    if (useDark) {
      return context.getColor(C4337R.color.notification_secondary_text_color_light);
    }
    return context.getColor(C4337R.color.notification_secondary_text_color_dark);
  }
  ...
  public static int resolveThirdColor(
    ...
    boolean useDark = shouldUseDark(backgroundColor, defaultBackgroundIsDark);
    if (useDark) {
      return context.getColor(C4337R.color.notification_third_text_color_light);
    }
    return context.getColor(C4337R.color.notification_third_text_color_dark);
  }
  ...
  public static int resolveDefaultColor(
    ...
    boolean useDark = shouldUseDark(backgroundColor, defaultBackgroundIsDark);
    if (useDark) {
      return context.getColor(C4337R.color.notification_default_color_light);
    }
    return context.getColor(C4337R.color.notification_default_color_dark);
  }
      ...
      return Color.rgb(
          constrain(
              (int)
                  Math.round(
                      (r > 0.0031308d
                              ? (Math.pow(r, 0.4166666666666667d) * 1.055d) - 0.055d
                              : r * 12.92d)
                          * 255.0d),
              0,
              255),
          constrain(
              (int)
                  Math.round(
                      (g > 0.0031308d
                              ? (Math.pow(g, 0.4166666666666667d) * 1.055d) - 0.055d
                              : g * 12.92d)
                          * 255.0d),
              0,
              255),
          constrain(
              (int)
                  Math.round(
                      255.0d
                          * (b > 0.0031308d
                              ? (Math.pow(b, 0.4166666666666667d) * 1.055d) - 0.055d
                              : b * 12.92d)),
              0,
              255));
    }
