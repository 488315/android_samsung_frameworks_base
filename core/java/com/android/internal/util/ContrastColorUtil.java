...
public class ContrastColorUtil {
    ...
    private ContrastColorUtil(Context context) {
        this.mGrayscaleIconMaxSize = context.getResources().getDimensionPixelSize(C4337R.dimen.notification_gray_scale_size_limit);
    }
    ...
    public boolean isGrayscaleIcon(Bitmap bitmap) {
        ...
        if (bitmap.getWidth() > this.mGrayscaleIconMaxSize || bitmap.getHeight() > this.mGrayscaleIconMaxSize) {
            Log.m94d(TAG, "GrayScale=false. Bitmap(Width=" + bitmap.getWidth() + "px, Height=" + bitmap.getHeight() + "px) is larger than " + this.mGrayscaleIconMaxSize + "px.");
            return false;
        }
        Object obj = sLock;
        synchronized (obj) {
