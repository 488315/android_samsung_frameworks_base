...
public final class GeoidHeightMap {
  ...
  public static MapParamsProto getParams(Context context) throws IOException {
    ...
    synchronized (sLock) {
      if (sParams == null) {
        InputStream is =
            context.getApplicationContext().getAssets().open("geoid_height_map/map-params.pb");
        try {
  ...
  private static boolean getUnitIntervalValues(
      MapParamsProto params, TileFunction tileFunction, long[] s2CellIds, double[] values) {
  ...
  private static void mergeByteBufferValues(
      MapParamsProto params,
      long[] s2CellIds,
      S2TileProto[] tiles,
      int tileIndex,
      double[] values) {
    ...
    for (int i = tileIndex; i < tiles.length; i++) {
      if (tiles[i] == tiles[tileIndex]) {
        ...
        values[i] =
            Double.isNaN(values[i]) ? SContextConstants.ENVIRONMENT_VALUE_UNKNOWN : values[i];
        values[i] = values[i] + (byteBuffer.get(bufferIndex) & 255);
      }
    }
  }

  private static void mergeByteJpegValues(
      MapParamsProto params,
      long[] s2CellIds,
      S2TileProto[] tiles,
      int tileIndex,
  ...
  private static void mergeBytePngValues(
      MapParamsProto params,
      long[] s2CellIds,
      S2TileProto[] tiles,
      int tileIndex,
  ...
  private static void mergeByteImageValues(
      MapParamsProto params,
      byte[] bytes,
      long[] s2CellIds,
      S2TileProto[] tiles,
    ...
    for (int i = tileIndex; i < tiles.length; i++) {
      if (s2CellIds[i] != 0 && tiles[i] == tiles[tileIndex]) {
        ...
        values[i] =
            values[i]
                + (bitmap.getPixel(
                        getIndexX(params, s2CellIds[i], bitmap.getWidth()),
                        getIndexY(params, s2CellIds[i], bitmap.getHeight()))
                    & 255);
      }
    }
  }
  ...
  public double[] readGeoidHeights(MapParamsProto params, Context context, long[] s2CellIds)
      throws IOException {
    ...
    if (getGeoidHeights(
        params, new GeoidHeightMap$$ExternalSyntheticLambda0(lruCache), s2CellIds, heightsMeters)) {
  ...
  private boolean getGeoidHeights(
      MapParamsProto params, TileFunction tileFunction, long[] s2CellIds, double[] heightsMeters) {
  ...
  private TileFunction loadFromCacheAndDisk(
      MapParamsProto params, Context context, long[] s2CellIds) throws IOException {
    int i;
    int len = s2CellIds.length;
      ...
      if (s2CellIds[i4] == 0) {
        ...
        InputStream is =
            context
                ...
                .getAssets()
                .open("geoid_height_map/tile-" + diskTokens[i4] + ".pb");
        try {
    ...
    return new TileFunction() { // from class:
                                // com.android.internal.location.altitude.GeoidHeightMap$$ExternalSyntheticLambda1
  ...
  static /* synthetic */ S2TileProto lambda$loadFromCacheAndDisk$0(
      long[] cacheKeys, S2TileProto[] loadedTiles, long cacheKey) {
  ...
  private void mergeFromDiskTile(
      MapParamsProto params,
      final S2TileProto diskTile,
      long[] cacheKeys,
      String[] diskTokens,
      int diskTokenIndex,
      S2TileProto[] loadedTiles)
      throws IOException {
    ...
    TileFunction diskTileFunction =
        new TileFunction() { // from class:
                             // com.android.internal.location.altitude.GeoidHeightMap$$ExternalSyntheticLambda2
          @Override // com.android.internal.location.altitude.GeoidHeightMap.TileFunction
  ...
  static /* synthetic */ S2TileProto lambda$mergeFromDiskTile$1(
      S2TileProto diskTile, long cacheKey) {
    return diskTile;
  }
}
