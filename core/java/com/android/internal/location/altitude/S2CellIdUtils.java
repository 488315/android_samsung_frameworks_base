...
public final class S2CellIdUtils {
  ...
  private static final int[][] POS_TO_IJ = {
    new int[] {0, 1, 3, 2}, new int[] {0, 2, 3, 1}, new int[] {3, 2, 0, 1}, new int[] {3, 1, 0, 2}
  };
  ...
  private static int xyzToFace(double x, double y, double z) {
    ...
    return absX > absY
        ? absX > absZ
            ? x < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN ? 3 : 0
            : z < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN ? 5 : 2
        : absY > absZ
            ? y < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN ? 4 : 1
            : z < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN ? 5 : 2;
  }
  ...
  private static int uToI(double u) {
    ...
    return Math.max(
        0, Math.min(View.LAST_APP_AUTOFILL_ID, (int) Math.round((1.073741824E9d * s) - 0.5d)));
  }
  ...
  private static long toIjo(long s2CellId) {
    ...
    while (k >= 0) {
      ...
      int bits2 =
          LOOKUP_IJ[
              bits
                  + ((((int) (s2CellId >>> (((k * 2) * 4) + 1))) & ((1 << (nbits * 2)) - 1)) << 2)];
      i += (bits2 >> 6) << (k * 4);
      j += ((bits2 >> 2) & 15) << (k * 4);
      bits = bits2 & 3;
      k--;
  ...
  private static UvTransform[] createUvTransforms() {
    UvTransform[] uvTransforms = {
      new UvTransform() { // from class: com.android.internal.location.altitude.S2CellIdUtils.1
  ...
  private static XyzTransform[] createXyzTransforms() {
    XyzTransform[] xyzTransforms = {
      new XyzTransform() { // from class: com.android.internal.location.altitude.S2CellIdUtils.7
