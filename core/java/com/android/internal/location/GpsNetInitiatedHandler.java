...
public class GpsNetInitiatedHandler {
    ...
    private class EmergencyCallListener extends TelephonyCallback implements TelephonyCallback.OutgoingEmergencyCallListener, TelephonyCallback.CallStateListener, TelephonyCallback.PreciseCallStateListener {
        ...
        public void onOutgoingEmergencyCall(EmergencyNumber placedEmergencyNumber, int subscriptionId) {
            if (GpsNetInitiatedHandler.this.mIzatServiceEnabled) {
                ...
                if (GpsNetInitiatedHandler.DEBUG) {
                    Log.m94d(GpsNetInitiatedHandler.TAG, "onOutgoingEmergencyCall(): inEmergency = " + GpsNetInitiatedHandler.this.getInEmergency());
                }
        ...
        public void onPreciseCallStateChanged(PreciseCallState callState) {
            if (!GpsNetInitiatedHandler.this.mIzatServiceEnabled) {
                ...
                if (GpsNetInitiatedHandler.DEBUG) {
                    Log.m94d(GpsNetInitiatedHandler.TAG, "onPreciseCallStateChanged. fgCallState = " + fgCallState);
                }
                    ...
                    try {
                        if (GpsNetInitiatedHandler.this.mTelephonyManager.isEmergencyNumber(phoneNumber)) {
                            ...
                            if (callState == 1) {
                                ...
                                Log.m94d(GpsNetInitiatedHandler.TAG, "additional emergency call is disconnected, the number of remaining call is " + GpsNetInitiatedHandler.this.mNumOfEmergencyCalls);
                                return;
                            }
                            if (callState == 2) {
                                GpsNetInitiatedHandler.this.mNumOfEmergencyCalls++;
                        ...
                        Log.m97e("SystemEmergencyHelper", "isEmergencyNumber throws IllegalStateException:", e);
                    }
                }
            }
        };
        ...
        BroadcastReceiver broadcastReceiver2 = new BroadcastReceiver() { // from class: com.android.internal.location.GpsNetInitiatedHandler.2
            ...
            public void onReceive(Context context2, Intent intent) {
                ...
                if (action.equals(LocationManager.MODE_CHANGED_ACTION)) {
                    ...
                    if (GpsNetInitiatedHandler.DEBUG) {
                        Log.m94d(GpsNetInitiatedHandler.TAG, "location enabled :" + GpsNetInitiatedHandler.this.getLocationEnabled());
                    }
                }
            }
        };
    ...
    public void handleNiNotification(GpsNiNotification notif) {
        if (DEBUG) {
            Log.m94d(TAG, "in handleNiNotification () : notificationId: " + notif.notificationId + " requestorId: " + notif.requestorId + " text: " + notif.text + " mIsSuplEsEnabled" + getSuplEsEnabled() + " mIsLocationEnabled" + getLocationEnabled());
        }
        if (getSuplEsEnabled()) {
            handleNiInEs(notif);
        } else {
    ...
    private void handleNi(GpsNiNotification notif) {
        if (DEBUG) {
            Log.m94d(TAG, "in handleNi () : needNotify: " + notif.needNotify + " needVerify: " + notif.needVerify + " privacyOverride: " + notif.privacyOverride + " mPopupImmediately: " + this.mPopupImmediately + " mInEmergency: " + getInEmergency());
        }
    ...
    private void handleNiInEs(GpsNiNotification notif) {
        if (DEBUG) {
            Log.m94d(TAG, "in handleNiInEs () : niType: " + notif.niType + " notificationId: " + notif.notificationId);
        }
    ...
    private synchronized void setNiNotification(GpsNiNotification notif) {
        ...
        if (DEBUG) {
            Log.m94d(TAG, "setNiNotification, notifyId: " + notif.notificationId + ", title: " + title + ", message: " + message);
        }
        if (this.mNiNotificationBuilder == null) {
            this.mNiNotificationBuilder = new Notification.Builder(this.mContext, SystemNotificationChannels.NETWORK_ALERTS).setSmallIcon(C4337R.drawable.stat_sys_gps_on).setWhen(0L).setOngoing(true).setAutoCancel(true).setColor(this.mContext.getColor(17170460));
        }
        if (this.mPlaySounds) {
    ...
    private void openNiDialog(GpsNiNotification notif) {
        ...
        if (DEBUG) {
            Log.m94d(TAG, "openNiDialog, notifyId: " + notif.notificationId + ", requestorId: " + notif.requestorId + ", text: " + notif.text);
        }
    ...
    private Intent getDlgIntent(GpsNiNotification notif) {
        ...
        if (DEBUG) {
            Log.m94d(TAG, "generateIntent, title: " + title + ", message: " + message + ", timeout: " + notif.timeout);
        }
        return intent;
    }
    ...
    private static String getNotifTicker(GpsNiNotification notif, Context context) {
        String ticker = String.format(context.getString(C4337R.string.gpsNotifTicker), decodeString(notif.requestorId, mIsHexInput, notif.requestorIdEncoding), decodeString(notif.text, mIsHexInput, notif.textEncoding));
        return ticker;
    }
    ...
    private static String getNotifMessage(GpsNiNotification notif, Context context) {
        String message = String.format(context.getString(C4337R.string.gpsNotifMessage), decodeString(notif.requestorId, mIsHexInput, notif.requestorIdEncoding), decodeString(notif.text, mIsHexInput, notif.textEncoding));
        return message;
    }
