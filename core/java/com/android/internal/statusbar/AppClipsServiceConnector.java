...
public class AppClipsServiceConnector {
  ...
  public boolean canLaunchCaptureContentActivityForNote(int taskId) {
    try {
      ...
      Log.m94d(TAG, "Exception from service\n" + e);
      return false;
    }
  }

  private void connectToServiceAndProcessRequest(
      final int taskId, final CompletableFuture<Boolean> future) {
    ServiceConnection serviceConnection =
        new ServiceConnection() { // from class:
                                  // com.android.internal.statusbar.AppClipsServiceConnector.1
          @Override // android.content.ServiceConnection
          public void onServiceConnected(ComponentName name, IBinder service) {
            try {
              future.complete(
                  Boolean.valueOf(
                      IAppClipsService.Stub.asInterface(service)
                          .canLaunchCaptureContentActivityForNote(taskId)));
            } catch (Exception e) {
              Log.m94d(AppClipsServiceConnector.TAG, "Exception from service\n" + e);
            }
    ...
    ComponentName serviceComponent =
        ComponentName.unflattenFromString(
            this.mContext
                .getResources()
                .getString(C4337R.string.config_screenshotAppClipsServiceComponent));
