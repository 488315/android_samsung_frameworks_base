...
public class InteractionJankMonitor {
  ...
  private static class InstanceHolder {
    public static final InteractionJankMonitor INSTANCE =
        new InteractionJankMonitor(new HandlerThread(InteractionJankMonitor.DEFAULT_WORKER_NAME));
  ...
  public /* synthetic */ void lambda$new$0(Context context) {
    try {
      this.mPropertiesChangedListener.onPropertiesChanged(
          DeviceConfig.getProperties("interaction_jank_monitor", new String[0]));
      DeviceConfig.addOnPropertiesChangedListener(
          "interaction_jank_monitor",
          new HandlerExecutor(this.mWorker.getThreadHandler()),
          this.mPropertiesChangedListener);
  ...
  public FrameTracker createFrameTracker(Configuration config, Session session) {
    ...
    if (!config.hasValidView()) {
      ...
      Log.m95d(
          TAG,
          "create FrameTracker fails: view="
              + view
              + ", attached="
              + attached
              + ", hasViewRoot="
    ...
    FrameTracker.ThreadedRendererWrapper threadedRenderer =
        view == null ? null : new FrameTracker.ThreadedRendererWrapper(view.getThreadedRenderer());
    FrameTracker.ViewRootWrapper viewRoot =
        view == null ? null : new FrameTracker.ViewRootWrapper(view.getViewRootImpl());
    ...
    FrameTracker.FrameTrackerListener eventsListener =
        new FrameTracker
            .FrameTrackerListener() { // from class:
                                      // com.android.internal.jank.InteractionJankMonitor$$ExternalSyntheticLambda12
  ...
  private boolean needRemoveTasks(String action, Session session) {
    ...
    boolean badCancel =
        (!action.equals(ACTION_SESSION_CANCEL)
                || session.getReason() == 16
                || session.getReason() == 19)
            ? false
            : true;
  ...
  public boolean begin(View v, int cujType) {
    try {
      ...
      Log.m95d(TAG, "Build configuration failed!", ex);
      return false;
    }
  }
          ...
          new TimeFunction() { // from class:
            ...
            public final void invoke(long j, long j2, long j3) {
              EventLogTags.writeJankCujEventsBeginRequest(
                  r0.mCujType, j, j2, j3, InteractionJankMonitor.Configuration.this.mTag);
            }
          });
      ...
      boolean success =
          config
              ...
              .runWithScissors(
                  ...
                  EXECUTOR_TASK_TIMEOUT);
      if (!success) {
        Log.m94d(TAG, "begin failed due to timeout, CUJ=" + getNameOfCuj(config.mCujType));
        return false;
      }
  ...
  private boolean beginInternal(Configuration conf) {
    ...
    if (!shouldMonitor(cujType)
        || getTracker(cujType) != null
        || (tracker = createFrameTracker(conf, new Session(cujType, conf.mTag))) == null) {
      return false;
    }
      ...
      boolean success =
          tracker
              ...
              .runWithScissors(
                  ...
                  EXECUTOR_TASK_TIMEOUT);
      if (!success) {
        Log.m94d(TAG, "end failed due to timeout, CUJ=" + getNameOfCuj(cujType));
        return false;
      }
      return result.mResult;
    } catch (IllegalArgumentException ex) {
      Log.m95d(TAG, "Execute end task failed!", ex);
      return false;
    }
  }
    ...
    try {
      ...
      boolean success =
          tracker
              ...
              .runWithScissors(
                  new Runnable() { // from class:
                                   // com.android.internal.jank.InteractionJankMonitor$$ExternalSyntheticLambda11
                    @Override // java.lang.Runnable
                    public final void run() {
                  ...
                  EXECUTOR_TASK_TIMEOUT);
      if (!success) {
        Log.m94d(TAG, "cancel failed due to timeout, CUJ=" + getNameOfCuj(cujType));
        return false;
      }
      return result.mResult;
    } catch (IllegalArgumentException ex) {
      Log.m95d(TAG, "Execute cancel task failed!", ex);
      return false;
    }
  }
  ...
  public void updateProperties(DeviceConfig.Properties properties) {
    ...
    this.mTraceThresholdFrameTimeMillis =
        properties.getInt(SETTINGS_THRESHOLD_FRAME_TIME_MILLIS_KEY, 64);
    boolean debugOverlayEnabled =
        Build.IS_DEBUGGABLE && properties.getBoolean(SETTINGS_DEBUG_OVERLAY_ENABLED_KEY, false);
    if (debugOverlayEnabled && this.mDebugOverlay == null) {
      this.mDebugOverlay =
          new InteractionMonitorDebugOverlay(this.mLock, this.mDebugBgColor, this.mDebugYOffset);
    } else if (!debugOverlayEnabled
        && (interactionMonitorDebugOverlay = this.mDebugOverlay) != null) {
  ...
  public static class Session {
    ...
    private String generateSessionName(String cujName, String cujPostfix) {
      ...
      if (cujName.length() > 80) {
        throw new IllegalArgumentException(
            TextUtils.formatSimple("The length of cuj name <%s> exceeds %d", cujName, 80));
      }
      if (hasPostfix && cujPostfix.length() > (remaining = 100 - cujName.length())) {
        cujPostfix =
            cujPostfix
                .substring(0, remaining - 3)
                .concat(android.telecom.Logging.Session.TRUNCATE_STRING);
      }
      if (hasPostfix) {
    ...
    public int getStatsdInteractionType() {
      return this.mCujType >= 10000
          ? InteractionJankMonitor.SEC_CUJ_TO_STATSD_INTERACTION_TYPE[this.mCujType - 10000]
          : InteractionJankMonitor.CUJ_TO_STATSD_INTERACTION_TYPE[this.mCujType];
    }
  ...
  private void postEventLogToWorkerThread(final TimeFunction logFunction) {
    ...
    final long unixNanos =
        TimeUnit.NANOSECONDS.convert(now.getEpochSecond(), TimeUnit.SECONDS) + now.getNano();
