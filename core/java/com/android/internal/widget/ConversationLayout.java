...
public class ConversationLayout extends FrameLayout
  ...
  protected void onFinishInflate() {
    ...
    this.mMessagingLinearLayout =
        (MessagingLinearLayout) findViewById(C4337R.id.notification_messaging);
    this.mActions = (NotificationActionListLayout) findViewById(C4337R.id.actions);
    this.mImageMessageContainer =
        (MessagingLinearLayout) findViewById(C4337R.id.conversation_image_message_container);
    DisplayMetrics displayMetrics = getResources().getDisplayMetrics();
    int size = Math.max(displayMetrics.widthPixels, displayMetrics.heightPixels);
    this.mMessagingClipRect = new Rect(0, 0, size, size);
    setMessagingClippingDisabled(false);
    this.mConversationIconView = (CachingIconView) findViewById(C4337R.id.conversation_icon);
    this.mConversationIconContainer = findViewById(C4337R.id.conversation_icon_container);
    this.mIcon = (CachingIconView) findViewById(16908294);
    this.mFeedbackIcon = findViewById(C4337R.id.feedback);
    this.mMinTouchSize = getResources().getDisplayMetrics().density * 48.0f;
    this.mImportanceRingView =
        (CachingIconView) findViewById(C4337R.id.conversation_icon_badge_ring);
    this.mConversationIconBadge = findViewById(C4337R.id.conversation_icon_badge);
    this.mConversationIconBadgeBg =
        (CachingIconView) findViewById(C4337R.id.conversation_icon_badge_bg);
    ...
    this.mConversationText = (TextView) findViewById(C4337R.id.conversation_text);
    this.mExpandButtonContainer = findViewById(C4337R.id.expand_button_container);
    this.mExpandButtonContainerA11yContainer =
        (ViewGroup) findViewById(C4337R.id.expand_button_a11y_container);
    this.mConversationHeader = findViewById(C4337R.id.conversation_header);
    this.mContentContainer = findViewById(C4337R.id.notification_action_list_margin_target);
    this.mExpandButtonAndContentContainer =
        (ViewGroup) findViewById(C4337R.id.expand_button_and_content_container);
    this.mExpandButton = (NotificationExpandButton) findViewById(C4337R.id.expand_button);
    this.mMessageSpacingStandard =
        getResources().getDimensionPixelSize(C4337R.dimen.notification_messaging_spacing);
    this.mMessageSpacingGroup =
        getResources()
            .getDimensionPixelSize(C4337R.dimen.notification_messaging_spacing_conversation_group);
    this.mNotificationHeaderExpandedPadding =
        getResources().getDimensionPixelSize(C4337R.dimen.conversation_header_expanded_padding_end);
    this.mContentMarginEnd =
        getResources().getDimensionPixelSize(C4337R.dimen.notification_content_margin_end);
    this.mBadgeProtrusion =
        getResources().getDimensionPixelSize(C4337R.dimen.conversation_badge_protrusion);
    this.mConversationAvatarSize =
        getResources().getDimensionPixelSize(C4337R.dimen.conversation_avatar_size);
    this.mConversationAvatarSizeExpanded =
        getResources().getDimensionPixelSize(C4337R.dimen.conversation_avatar_size_group_expanded);
    this.mConversationIconTopPaddingExpandedGroup =
        getResources()
            .getDimensionPixelSize(
                C4337R.dimen.conversation_icon_container_top_padding_small_avatar);
    this.mConversationIconTopPadding =
        getFontScaledMarginHeight(
            this.mContext, C4337R.dimen.conversation_icon_container_top_padding);
    this.mExpandedGroupMessagePadding =
        getResources()
            .getDimensionPixelSize(C4337R.dimen.expanded_group_conversation_message_padding);
    this.mExpandedGroupBadgeProtrusion =
        getResources()
            .getDimensionPixelSize(C4337R.dimen.conversation_badge_protrusion_group_expanded);
    this.mExpandedGroupBadgeProtrusionFacePile =
        getResources()
            .getDimensionPixelSize(
                C4337R.dimen.conversation_badge_protrusion_group_expanded_face_pile);
    this.mConversationFacePile = findViewById(C4337R.id.conversation_face_pile);
    this.mFacePileAvatarSize =
        getResources().getDimensionPixelSize(C4337R.dimen.conversation_face_pile_avatar_size);
    this.mFacePileAvatarSizeExpandedGroup =
        getResources()
            .getDimensionPixelSize(C4337R.dimen.conversation_face_pile_avatar_size_group_expanded);
    this.mFacePileProtectionWidth =
        getResources().getDimensionPixelSize(C4337R.dimen.conversation_face_pile_protection_width);
    this.mFacePileProtectionWidthExpanded =
        getResources()
            .getDimensionPixelSize(C4337R.dimen.conversation_face_pile_protection_width_expanded);
    this.mFallbackChatName =
        getResources().getString(C4337R.string.conversation_title_fallback_one_to_one);
    this.mFallbackGroupChatName =
        getResources().getString(C4337R.string.conversation_title_fallback_group_chat);
    this.mAppName = (ObservableTextView) findViewById(C4337R.id.app_name_text);
    this.mAppNameDivider = findViewById(C4337R.id.app_name_divider);
    ...
    this.mConversationStartMargin =
        getResources().getDimensionPixelSize(C4337R.dimen.notification_content_margin_start);
    this.mConversationTopMargin =
        getFontScaledMarginHeight(
            this.mContext, C4337R.dimen.conversation_expand_button_top_margin_expanded);
    this.mConversationMinHeight =
        getResources().getDimensionPixelSize(C4337R.dimen.conversation_expand_button_height);
  }
  ...
  public void setIsImportantConversation(boolean isImportantConversation, boolean animate) {
    ...
    if (animate && isImportantConversation) {
      ...
      bg.mutate();
      final int ringColor = getResources().getColor(C4337R.color.conversation_important_highlight);
      int standardThickness =
          getResources().getDimensionPixelSize(C4337R.dimen.importance_ring_stroke_width);
      int largeThickness =
          getResources().getDimensionPixelSize(C4337R.dimen.importance_ring_anim_max_stroke_width);
      int standardSize = getResources().getDimensionPixelSize(C4337R.dimen.importance_ring_size);
      final int baseSize = standardSize - (standardThickness * 2);
      final int bgSize =
          getResources().getDimensionPixelSize(C4337R.dimen.conversation_icon_size_badged);
  ...
  private void addRemoteInputHistoryToMessages(
    ...
    for (int i = remoteInputHistory.length - 1; i >= 0; i--) {
      ...
      Notification.MessagingStyle.Message message =
          new Notification.MessagingStyle.Message(historyMessage.getText(), 0L, null, true);
      ...
      for (int i = this.mGroups.size() - 1; i >= 0; i--) {
        ...
        if ((messageSender != null && !TextUtils.equals(userKey, getKey(messageSender)))
          ...
          if (this.mConversationIcon == null) {
            ...
            if (avatarIcon == null) {
              avatarIcon =
                  this.mPeopleHelper.createAvatarSymbol(conversationText, "", this.mLayoutColor);
            }
  ...
  private void updateActionListPadding() {
    ...
    if (notificationActionListLayout != null) {
      notificationActionListLayout.setCollapsibleIndentDimen(
    ...
    ImageView bottomBackground =
        (ImageView)
            this.mConversationFacePile.findViewById(
                C4337R.id.conversation_face_pile_bottom_background);
    ImageView bottomView =
        (ImageView)
            this.mConversationFacePile.findViewById(C4337R.id.conversation_face_pile_bottom);
    ImageView topView =
        (ImageView) this.mConversationFacePile.findViewById(C4337R.id.conversation_face_pile_top);
  ...
  private void updatePaddingsBasedOnContentAvailability() {
    ...
    messagingLinearLayout.setPaddingRelative(
        messagingLinearLayout.getPaddingStart(),
        messagingPadding,
          ...
          if (cachedIcon2 == null) {
            cachedIcon2 =
                this.mPeopleHelper.createAvatarSymbol(
                    senderName2, uniqueNames.get(senderName2), this.mLayoutColor);
            cachedAvatars.put(senderName2, cachedIcon2);
          }
          group2.setCreatedAvatar(
              cachedIcon2, senderName2, uniqueNames.get(senderName2), this.mLayoutColor);
        }
      }
    }
  }
  ...
  private void setUser(Person user) {
    ...
    if (user.getIcon() == null) {
      Icon userIcon = Icon.createWithResource(getContext(), C4337R.drawable.messaging_user);
                  ...
                  for (MessagingGroup group : ConversationLayout.this.mAddedQueue) {
                    if (group.isShown()) {
                      ...
                      MessagingPropertyAnimator.startLocalTranslationFrom(
                          group, group.getHeight(), ConversationLayout.LINEAR_OUT_SLOW_IN);
                    }
                  }
  ...
  public CharSequence getConversationSenderName() {
    ...
    return getResources().getString(C4337R.string.conversation_single_line_name_display, name);
  }
  ...
  public CharSequence getConversationText() {
    ...
    CharSequence text =
        messagingMessage.getMessage() != null ? messagingMessage.getMessage().getText() : null;
    if (text == null && (messagingMessage instanceof MessagingImageMessage)) {
      String unformatted =
          getResources().getString(C4337R.string.conversation_single_line_image_placeholder);
