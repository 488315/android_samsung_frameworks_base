...
public class PointerLocationView extends View implements InputManager.InputDeviceListener, WindowManagerPolicyConstants.PointerEventListener {
    ...
    public static class PointerState {
        ...
        public void addTrace(float x, float y, boolean current) {
            ...
            if (i3 > mMaxTraceCntByPointerCnt) {
                clearTrace();
                Log.m98i(PointerLocationView.TAG, "point drawn is initialized because there is a risk of overflow due to too many draws.");
            }
        }
    }
    ...
    public PointerLocationView(Context c) {
        ...
        this.mSystemGestureExclusionListener = new BinderC47581();
        ...
        if (altStrategy.length() != 0) {
            Log.m94d(TAG, "Comparing default velocity tracker strategy with " + altStrategy);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int NP2 = this.mPointers.size(); p < NP2; NP2 = NP) {
            ...
            if (this.mCurDown && ps.mCurDown) {
                canvas.drawLine(0.0f, ps.mCoords.f545y, getWidth(), ps.mCoords.f545y, this.mTargetPaint);
                canvas.drawLine(ps.mCoords.f544x, -getHeight(), ps.mCoords.f544x, Math.max(getHeight(), getWidth()), this.mTargetPaint);
                int pressureLevel = (int) (ps.mCoords.pressure * 255.0f);
                this.mPaint.setARGB(255, pressureLevel, 255, 255 - pressureLevel);
                canvas.drawPoint(ps.mCoords.f544x, ps.mCoords.f545y, this.mPaint);
                this.mPaint.setARGB(255, pressureLevel, 255 - pressureLevel, 128);
                drawOval(canvas, ps.mCoords.f544x, ps.mCoords.f545y, ps.mCoords.touchMajor, ps.mCoords.touchMinor, ps.mCoords.orientation, this.mPaint);
                this.mPaint.setARGB(255, pressureLevel, 128, 255 - pressureLevel);
                drawOval(canvas, ps.mCoords.f544x, ps.mCoords.f545y, ps.mCoords.toolMajor, ps.mCoords.toolMinor, ps.mCoords.orientation, this.mPaint);
                ...
                if (ps.mToolType == 2 || ps.mToolType == 4) {
                    canvas.drawLine(ps.mCoords.f544x, ps.mCoords.f545y, ps.mCoords.f544x + orientationVectorX, ps.mCoords.f545y + orientationVectorY, this.mPaint);
                } else {
                    canvas.drawLine(ps.mCoords.f544x - orientationVectorX, ps.mCoords.f545y - orientationVectorY, ps.mCoords.f544x + orientationVectorX, ps.mCoords.f545y + orientationVectorY, this.mPaint);
                }
                float tiltScale = (float) Math.sin(ps.mCoords.getAxisValue(25));
                canvas.drawCircle(ps.mCoords.f544x + (orientationVectorX * tiltScale), ps.mCoords.f545y + (orientationVectorY * tiltScale), 3.0f, this.mPaint);
    ...
    private void drawLabels(Canvas canvas) {
        ...
        if ((this.mCurDown && ps.mCurDown) || count == 0) {
            ...
            canvas.drawText(this.mText.clear().append("X: ").append(ps.mCoords.f544x, 1).toString(), itemW + 1, base, this.mTextPaint);
            canvas.drawRect(itemW * 2, this.mHeaderPaddingTop, (itemW * 3) - 1, bottom, this.mTextBackgroundPaint);
            canvas.drawText(this.mText.clear().append("Y: ").append(ps.mCoords.f545y, 1).toString(), (itemW * 2) + 1, base, this.mTextPaint);
        } else {
    ...
    public void onPointerEvent(MotionEvent event) {
        ...
        while (historyPos2 < N) {
            ...
            while (i3 < NI) {
                ...
                if (ps2 != null) {
                    ...
                    ps2.addTrace(coords4.f544x, coords4.f545y, false);
                }
                i3 = i + 1;
        ...
        for (int i4 = 0; i4 < NI; i4++) {
            ...
            if (ps != null) {
                ...
                ps.addTrace(coords6.f544x, coords6.f545y, true);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (shouldLogKey(keyCode)) {
            ...
            if (repeatCount == 0) {
                Log.m98i(TAG, "Key Down: " + event);
                return true;
            }
            Log.m98i(TAG, "Key Repeat #" + repeatCount + ": " + event);
            return true;
        }
    ...
    protected void onDetachedFromWindow() {
        ...
        try {
            ...
            Log.m97e(TAG, "Failed to unregister window manager callbacks", e2);
        }
    }
    ...
    class BinderC47581 extends ISystemGestureExclusionListener.Stub {
        BinderC47581() {
        }
        ...
        public void onSystemGestureExclusionChanged(int displayId, Region systemGestureExclusion, Region systemGestureExclusionUnrestricted) {
            ...
            if (systemGestureExclusionUnrestricted != null) {
                ...
                rejected.m23op(exclusion, Region.EnumC0836Op.DIFFERENCE);
            }
            ...
            if (handler != null) {
                handler.post(new Runnable() { // from class: com.android.internal.widget.PointerLocationView$1$$ExternalSyntheticLambda0
                    ...
                    public final void run() {
                        PointerLocationView.BinderC47581.this.lambda$onSystemGestureExclusionChanged$0(exclusion, rejected);
                    }
                });
            }
        }
