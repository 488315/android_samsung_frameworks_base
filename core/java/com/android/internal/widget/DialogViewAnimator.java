...
public class DialogViewAnimator extends ViewAnimator {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    while (true) {
      ...
      if (getMeasureAllChildren() || child.getVisibility() != 8) {
        ...
        if (measureMatchParentChildren && !matchHeight) {
          maxHeight2 =
              Math.max(maxHeight2, child.getMeasuredHeight() + lp.topMargin + lp.bottomMargin);
    ...
    int maxHeight3 =
        Math.max(maxHeight2 + getPaddingTop() + getPaddingBottom(), getSuggestedMinimumHeight());
    ...
    while (i4 < matchCount) {
      ...
      if (lp2.width == maxHeight) {
        childWidthMeasureSpec =
            View.MeasureSpec.makeMeasureSpec(
                (((getMeasuredWidth() - getPaddingLeft()) - getPaddingRight()) - lp2.leftMargin)
                    - lp2.rightMargin,
                i2);
      } else {
        childWidthMeasureSpec =
      ...
      if (lp2.height == -1) {
        childHeightMeasureSpec =
            View.MeasureSpec.makeMeasureSpec(
                (((getMeasuredHeight() - getPaddingTop()) - getPaddingBottom()) - lp2.topMargin)
                    - lp2.bottomMargin,
                1073741824);
      } else {
        childHeightMeasureSpec =
