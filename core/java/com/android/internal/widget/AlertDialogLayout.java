  ...
  private boolean tryOnMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (child.getVisibility() != 8) {
        int id = child.getId();
        switch (id) {
          case C4337R.id.buttonPanel /* 16908859 */:
            buttonPanel = child;
            break;
          case C4337R.id.contentPanel /* 16908921 */:
          case C4337R.id.customPanel /* 16908963 */:
            if (middlePanel != null) {
              return false;
            }
            middlePanel = child;
            break;
          case C4337R.id.topPanel /* 16909916 */:
            topPanel = child;
            break;
          default:
            return false;
    ...
    if (middlePanel != null) {
      if (heightMode == 0) {
        ...
        childHeightSpec =
            View.MeasureSpec.makeMeasureSpec(Math.max(0, heightSize - usedHeight), heightMode);
      }
    ...
    int widthSizeAndState =
        resolveSizeAndState(maxWidth + i2 + getPaddingRight(), widthMeasureSpec, childState);
    ...
    while (i2 < count) {
      ...
      if (child == null || child.getVisibility() == 8) {
        ...
        switch (absoluteGravity & 7) {
          case 1:
            int childLeft2 =
                ((((childSpace - childWidth) / 2) + paddingLeft) + lp.leftMargin) - lp.rightMargin;
