...
public class ActionBarContainer extends FrameLayout {
  ...
  public ActionBarContainer(Context context, AttributeSet attrs) {
    ...
    this.mIsSetOpenTheme =
        z2 && context.getResources().getAssets().getSamsungThemeOverlays().size() > 0;
    this.mSetBackground = false;
    TypedArray a = context.obtainStyledAttributes(attrs, C4337R.styleable.ActionBar);
    if (this.mIsSetOpenTheme) {
      if (a.getResourceId(2, 0) == 17303727) {
        this.mBackground = context.getDrawable(C4337R.drawable.tw_action_bar_background);
      } else if (a.getResourceId(2, 0) == 17303728) {
        this.mBackground = context.getDrawable(C4337R.drawable.tw_action_bar_background_dark);
      } else {
  ...
  protected void onConfigurationChanged(Configuration newConfig) {
    ...
    if (this.mIsThemeDeviceDefaultFamily) {
      TypedArray a =
          getContext().obtainStyledAttributes(null, C4337R.styleable.ActionBar, 16843470, 0);
      if (this.mIsSetOpenTheme) {
        if (a.getResourceId(2, 0) == 17303727) {
          this.mBackground = getContext().getDrawable(C4337R.drawable.tw_action_bar_background);
        } else if (a.getResourceId(2, 0) == 17303728) {
          this.mBackground =
              getContext().getDrawable(C4337R.drawable.tw_action_bar_background_dark);
  ...
  public void onFinishInflate() {
    ...
    this.mActionBarView = findViewById(C4337R.id.action_bar);
    this.mActionContextView = findViewById(C4337R.id.action_context_bar);
  }
  ...
  public void setStackedBackground(Drawable bg) {
    ...
    if (bg != null) {
      ...
      if (this.mIsStacked && (drawable = this.mStackedBackground) != null) {
        drawable.setBounds(
            this.mTabContainer.getLeft(),
            this.mTabContainer.getTop(),
            this.mTabContainer.getRight(),
            this.mTabContainer.getBottom());
      }
    }
    boolean z = true;
    if (this.mIsSplit) {
  ...
  protected boolean verifyDrawable(Drawable who) {
    return (who == this.mBackground && !this.mIsSplit)
        || (who == this.mStackedBackground && this.mIsStacked)
        || ((who == this.mSplitBackground && this.mIsSplit) || super.verifyDrawable(who));
  }
  ...
  public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (this.mActionBarView == null
        && View.MeasureSpec.getMode(heightMeasureSpec) == Integer.MIN_VALUE
        && (i = this.mHeight) >= 0) {
      heightMeasureSpec =
          View.MeasureSpec.makeMeasureSpec(
              Math.min(i, View.MeasureSpec.getSize(heightMeasureSpec)), Integer.MIN_VALUE);
    }
    super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    if (this.mActionBarView != null
        && (view = this.mTabContainer) != null
        && view.getVisibility() != 8) {
      ...
      for (int i2 = 0; i2 < childCount; i2++) {
        ...
        if (child != this.mTabContainer) {
          nonTabMaxHeight =
              Math.max(
                  nonTabMaxHeight, isCollapsed(child) ? 0 : getMeasuredHeightWithMargins(child));
        }
      }
      ...
      setMeasuredDimension(
          getMeasuredWidth(),
          Math.min(getMeasuredHeightWithMargins(this.mTabContainer) + nonTabMaxHeight, maxHeight));
    }
  }
  ...
  public void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    if (tabContainer != null && tabContainer.getVisibility() != 8) {
      ...
      tabContainer.layout(
          l, (containerHeight - tabHeight) - lp.bottomMargin, r, containerHeight - lp.bottomMargin);
    }
      ...
      if (this.mBackground != null) {
        if (this.mActionBarView.getVisibility() == 0) {
          ...
          if (view != null && view.getVisibility() == 0) {
            this.mBackground.setBounds(
                this.mActionContextView.getLeft(),
                this.mActionContextView.getTop(),
                this.mActionContextView.getRight(),
                this.mActionContextView.getBottom());
          } else {
  ...
  private class ActionBarBackgroundDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
      if (ActionBarContainer.this.mIsSplit) {
        ...
        if (ActionBarContainer.this.mStackedBackground != null
            && ActionBarContainer.this.mIsStacked) {
    ...
    public int getOpacity() {
      if (ActionBarContainer.this.mIsSplit) {
        if (ActionBarContainer.this.mSplitBackground != null
            && ActionBarContainer.this.mSplitBackground.getOpacity() == -1) {
          return -1;
        }
      } else if ((!ActionBarContainer.this.mIsStacked
              || (ActionBarContainer.this.mStackedBackground != null
                  && ActionBarContainer.this.mStackedBackground.getOpacity() == -1))
          && !ActionBarContainer.isCollapsed(ActionBarContainer.this.mActionBarView)
          && ActionBarContainer.this.mBackground != null
          && ActionBarContainer.this.mBackground.getOpacity() == -1) {
        return -1;
      }
      return 0;
    }
