    ...
    switch (action) {
      case 0:
        ...
        float xTargetVelocity =
            computeTargetVelocity(0, event.getX(), v.getWidth(), this.mTarget.getWidth());
        float yTargetVelocity =
            computeTargetVelocity(1, event.getY(), v.getHeight(), this.mTarget.getHeight());
  ...
  private static class ClampedScroller {
    ...
    private float getValueAt(long currentTime) {
      ...
      if (j < 0 || currentTime < j) {
        ...
        return AutoScrollHelper.constrain(elapsedSinceStart / this.mRampUpDuration, 0.0f, 1.0f)
            * 0.5f;
      }
      ...
      return (1.0f - f)
          + (f
              * AutoScrollHelper.constrain(elapsedSinceEnd2 / this.mEffectiveRampDown, 0.0f, 1.0f));
    }
