...
public abstract class DrawBase4 extends PaintOperation implements VariableSupport {
    public static final Companion COMPANION =
            new Companion(
                    42) { // from class:
                          // com.android.internal.widget.remotecompose.core.operations.DrawBase4.1
    ...
    public void updateVariables(RemoteContext context) {
        this.mX1 =
                Float.isNaN(this.mX1Value)
                        ? context.getFloat(Utils.idFromNan(this.mX1Value))
                        : this.mX1Value;
        this.mY1 =
                Float.isNaN(this.mY1Value)
                        ? context.getFloat(Utils.idFromNan(this.mY1Value))
                        : this.mY1Value;
        this.mX2 =
                Float.isNaN(this.mX2Value)
                        ? context.getFloat(Utils.idFromNan(this.mX2Value))
                        : this.mX2Value;
        this.mY2 =
                Float.isNaN(this.mY2Value)
                        ? context.getFloat(Utils.idFromNan(this.mY2Value))
                        : this.mY2Value;
    }
    ...
    public String toString() {
        return this.mName
                + " "
                + Utils.floatToString(this.mX1Value, this.mX1)
                + " "
                + Utils.floatToString(this.mY1Value, this.mY1)
                + " "
                + Utils.floatToString(this.mX2Value, this.mX2)
                + " "
                + Utils.floatToString(this.mY2Value, this.mY2);
    }
