...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    private int getSpanGroupIndex(RecyclerView.Recycler recycler, RecyclerView.State state, int viewPosition) {
        ...
        if (adapterPosition == -1) {
            Log.m102w(TAG, "Cannot find span size for pre layout position. " + viewPosition);
            return 0;
        }
    ...
    private int getSpanIndex(RecyclerView.Recycler recycler, RecyclerView.State state, int pos) {
        ...
        if (adapterPosition == -1) {
            Log.m102w(TAG, "Cannot find span size for pre layout position. It is not cached, not in the adapter. Pos:" + pos);
            return 0;
        }
