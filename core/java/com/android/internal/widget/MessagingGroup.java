...
public class MessagingGroup extends LinearLayout implements MessagingLinearLayout.MessagingChild {
  ...
  protected void onFinishInflate() {
    ...
    this.mMessageContainer =
        (MessagingLinearLayout) findViewById(C4337R.id.group_message_container);
    ...
    this.mContentContainer =
        (LinearLayout) findViewById(C4337R.id.messaging_group_content_container);
    this.mSendingSpinnerContainer =
        findViewById(C4337R.id.messaging_group_sending_progress_container);
    ...
    this.mSenderTextPaddingSingleLine =
        res.getDimensionPixelSize(C4337R.dimen.messaging_group_singleline_sender_padding_end);
    this.mConversationContentStart =
        res.getDimensionPixelSize(C4337R.dimen.conversation_content_start);
    this.mNonConversationContentStart =
        res.getDimensionPixelSize(C4337R.dimen.notification_content_margin_start);
    this.mNonConversationPaddingStart =
        res.getDimensionPixelSize(C4337R.dimen.messaging_layout_icon_padding_start);
    this.mConversationAvatarSize = res.getDimensionPixelSize(C4337R.dimen.messaging_avatar_size);
    this.mNonConversationAvatarSize =
        res.getDimensionPixelSize(C4337R.dimen.notification_icon_circle_size);
    this.mNotificationTextMarginTop =
        res.getDimensionPixelSize(C4337R.dimen.notification_text_margin_top);
  }
      ...
      if (this.mSingleLine) {
        ...
        top =
            (getDistanceFromParent(this.mSenderView, this.mContentContainer)
                    - getDistanceFromParent(this.mMessageContainer, this.mContentContainer))
  ...
  public void setSender(Person sender, CharSequence nameOverride) {
    ...
    if (this.mSingleLine && !TextUtils.isEmpty(nameOverride)) {
      nameOverride =
          this.mContext
              .getResources()
              .getString(C4337R.string.conversation_single_line_name_display, nameOverride);
    }
  ...
  private int calculateSendingTextColor() {
    ...
    this.mContext
        .getResources()
        .getValue(C4337R.dimen.notification_secondary_text_disabled_alpha, alphaValue, true);
      ...
      createdGroup =
          (MessagingGroup)
              LayoutInflater.from(layout.getContext())
                  .inflate(
                      C4337R.layout.notification_template_messaging_group,
                      (ViewGroup) layout,
                      false);
      ...
      getViewTreeObserver()
          .addOnPreDrawListener(
              new ViewTreeObserver
                  .OnPreDrawListener() { // from class: com.android.internal.widget.MessagingGroup.1
  ...
  public void setIsInConversation(boolean isInConversation) {
    ...
    if (this.mIsInConversation != isInConversation) {
      ...
      int size =
          this.mIsInConversation ? this.mConversationAvatarSize : this.mNonConversationAvatarSize;
