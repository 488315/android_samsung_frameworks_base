...
public class MessagingGroup extends LinearLayout implements MessagingLinearLayout.MessagingChild {
    ...
    protected void onFinishInflate() {
        ...
        this.mMessageContainer = (MessagingLinearLayout) findViewById(C4337R.id.group_message_container);
        this.mSenderView = (ImageFloatingTextView) findViewById(C4337R.id.message_name);
        this.mAvatarView = (ImageView) findViewById(C4337R.id.message_icon);
        this.mImageContainer = (ViewGroup) findViewById(C4337R.id.messaging_group_icon_container);
        this.mSendingSpinner = (ProgressBar) findViewById(C4337R.id.messaging_group_sending_progress);
        this.mMessagingIconContainer = (ViewGroup) findViewById(C4337R.id.message_icon_container);
        this.mContentContainer = (LinearLayout) findViewById(C4337R.id.messaging_group_content_container);
        this.mSendingSpinnerContainer = findViewById(C4337R.id.messaging_group_sending_progress_container);
        Resources res = getResources();
        DisplayMetrics displayMetrics = res.getDisplayMetrics();
        this.mDisplaySize.f85x = displayMetrics.widthPixels;
        this.mDisplaySize.f86y = displayMetrics.heightPixels;
        this.mSenderTextPaddingSingleLine = res.getDimensionPixelSize(C4337R.dimen.messaging_group_singleline_sender_padding_end);
        this.mConversationContentStart = res.getDimensionPixelSize(C4337R.dimen.conversation_content_start);
        this.mNonConversationContentStart = res.getDimensionPixelSize(C4337R.dimen.notification_content_margin_start);
        this.mNonConversationPaddingStart = res.getDimensionPixelSize(C4337R.dimen.messaging_layout_icon_padding_start);
        this.mConversationAvatarSize = res.getDimensionPixelSize(C4337R.dimen.messaging_avatar_size);
        this.mNonConversationAvatarSize = res.getDimensionPixelSize(C4337R.dimen.notification_icon_circle_size);
        this.mNotificationTextMarginTop = res.getDimensionPixelSize(C4337R.dimen.notification_text_margin_top);
    }
    ...
    public void updateClipRect() {
        ...
        if (this.mSenderView.getVisibility() != 8 && !this.mClippingDisabled) {
            ...
            int size = Math.max(this.mDisplaySize.f85x, this.mDisplaySize.f86y);
    ...
    public void setSender(Person sender, CharSequence nameOverride) {
        ...
        if (this.mSingleLine && !TextUtils.isEmpty(nameOverride)) {
            nameOverride = this.mContext.getResources().getString(C4337R.string.conversation_single_line_name_display, nameOverride);
        }
    ...
    private int calculateSendingTextColor() {
        ...
        this.mContext.getResources().getValue(C4337R.dimen.notification_secondary_text_disabled_alpha, alphaValue, true);
    ...
    static MessagingGroup createGroup(MessagingLinearLayout layout) {
        ...
        if (createdGroup == null) {
            createdGroup = (MessagingGroup) LayoutInflater.from(layout.getContext()).inflate(C4337R.layout.notification_template_messaging_group, (ViewGroup) layout, false);
