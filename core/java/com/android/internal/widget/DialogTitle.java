    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (layout != null && (lineCount = layout.getLineCount()) > 0) {
            ...
            if (ellipsisCount > 0) {
                setSingleLine(false);
                setMaxLines(2);
                TypedArray a = this.mContext.obtainStyledAttributes(null, C0000R.styleable.TextAppearance, 16842817, 16973892);
                TypedValue outValue = new TypedValue();
                this.mContext.getTheme().resolveAttribute(C4337R.attr.parentIsDeviceDefault, outValue, true);
                boolean mIsDeviceDefault = outValue.data != 0;
                if (mIsDeviceDefault) {
                    int textSize = this.mContext.getResources().getDimensionPixelSize(C4337R.dimen.sem_dialog_title_text_size);
