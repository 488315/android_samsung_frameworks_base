...
public final class RemoteFloatingToolbarPopup implements FloatingToolbarPopup {
    ...
    public void show(List<MenuItem> menuItems, MenuItem.OnMenuItemClickListener menuItemClickListener, Rect contentRect) {
        ...
        if (isShowing() && Objects.equals(menuItems, this.mMenuItems) && Objects.equals(contentRect, this.mPreviousContentRect)) {
            if (DEBUG) {
                Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "Ignore duplicate show() for the same content.");
                return;
            }
            return;
        }
        ...
        if (i > 0) {
            ...
            suggestWidth = this.mParent.getResources().getDimensionPixelSize(C4337R.dimen.floating_toolbar_preferred_width);
        }
        ...
        if (DEBUG) {
            Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "RemoteFloatingToolbarPopup.show() for " + showInfo);
        }
    ...
    public void dismiss() {
        if (this.mState == 3) {
            Log.m102w(FloatingToolbar.FLOATING_TOOLBAR_TAG, "The floating toolbar already dismissed.");
            return;
        }
        if (DEBUG) {
            Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "RemoteFloatingToolbarPopup.dismiss().");
        }
        this.mSelectionToolbarManager.dismissToolbar(this.mFloatingToolbarToken);
        doDismissPopupWindow();
    }
    ...
    public void hide() {
        ...
        if (i == 3 || i == 2) {
            if (DEBUG) {
                Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "The floating toolbar already dismissed or hidden.");
            }
        } else {
            if (DEBUG) {
                Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "RemoteFloatingToolbarPopup.hide().");
            }
    ...
    private void updatePopupWindowContent(WidgetInfo widgetInfo) {
        if (DEBUG) {
            Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "updatePopupWindowContent.");
        }
    ...
    private void doDismissPopupWindow() {
        if (DEBUG) {
            Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "RemoteFloatingToolbarPopup.doDismiss().");
        }
        this.mState = 3;
    ...
    public /* synthetic */ void lambda$onShow$0(WidgetInfo info) {
        ...
        this.mPopupWindow.showAtLocation(this.mParent, 0, coords.f85x, coords.f86y);
    }
    ...
    public /* synthetic */ void lambda$onWidgetUpdated$1(WidgetInfo info) {
        if (!isShowing()) {
            Log.m102w(FloatingToolbar.FLOATING_TOOLBAR_TAG, "onWidgetUpdated(): The widget isn't showing.");
            return;
        }
        updatePopupWindowContent(info);
        Rect contentRect = info.getContentRect();
        Point coords = getCoordinatesInWindow(contentRect.left, contentRect.top);
        if (DEBUG) {
            Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "PopupWindow x= " + coords.f85x + " y= " + coords.f86y + " w=" + contentRect.width() + " h=" + contentRect.height());
        }
        this.mPopupWindow.update(coords.f85x, coords.f86y, contentRect.width(), contentRect.height());
    }
    ...
    public /* synthetic */ void lambda$onMenuItemClicked$3(ToolbarMenuItem toolbarMenuItem) {
        ...
        if (DEBUG) {
            Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "SelectionToolbarCallbackImpl onMenuItemClicked. toolbarMenuItem=" + toolbarMenuItem + " item=" + item);
        }
        if (item != null) {
            this.mMenuItemClickListener.onMenuItemClick(item);
        } else {
            Log.m96e(FloatingToolbar.FLOATING_TOOLBAR_TAG, "onMenuItemClicked: cannot find menu item.");
        }
    }
    ...
    private static class SelectionToolbarCallbackImpl extends ISelectionToolbarCallback.Stub {
        ...
        public void onShown(WidgetInfo info) {
            if (RemoteFloatingToolbarPopup.DEBUG) {
                Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "SelectionToolbarCallbackImpl onShown: " + info);
            }
            ...
            if (remoteFloatingToolbarPopup != null) {
                ...
                Log.m102w(FloatingToolbar.FLOATING_TOOLBAR_TAG, "Lost remoteFloatingToolbarPopup reference for onShown.");
            }
        }
        ...
        public void onWidgetUpdated(WidgetInfo info) {
            if (RemoteFloatingToolbarPopup.DEBUG) {
                Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "SelectionToolbarCallbackImpl onWidgetUpdated: info = " + info);
            }
            RemoteFloatingToolbarPopup remoteFloatingToolbarPopup = this.mRemotePopup.get();
            if (remoteFloatingToolbarPopup != null) {
                remoteFloatingToolbarPopup.onWidgetUpdated(info);
            } else {
                Log.m102w(FloatingToolbar.FLOATING_TOOLBAR_TAG, "Lost remoteFloatingToolbarPopup reference for onWidgetUpdated.");
            }
        }
        ...
        public void onToolbarShowTimeout() {
            ...
            if (remoteFloatingToolbarPopup != null) {
                ...
                Log.m102w(FloatingToolbar.FLOATING_TOOLBAR_TAG, "Lost remoteFloatingToolbarPopup reference for onToolbarShowTimeout.");
            }
        }
        ...
        public void onMenuItemClicked(ToolbarMenuItem toolbarMenuItem) {
            ...
            if (remoteFloatingToolbarPopup != null) {
                ...
                Log.m102w(FloatingToolbar.FLOATING_TOOLBAR_TAG, "Lost remoteFloatingToolbarPopup reference for onMenuItemClicked.");
            }
        }
        ...
        public void onError(int errorCode) {
            if (RemoteFloatingToolbarPopup.DEBUG) {
                Log.m100v(FloatingToolbar.FLOATING_TOOLBAR_TAG, "SelectionToolbarCallbackImpl onError: " + errorCode);
            }
        }
    }
    ...
    static final class MenuItemRepr {
        ...
        /* renamed from: of */
        public static MenuItemRepr m231of(MenuItem menuItem) {
        ...
        public static boolean reprEquals(Collection<MenuItem> menuItems1, Collection<MenuItem> menuItems2) {
            ...
            for (MenuItem menuItem1 : menuItems1) {
                ...
                if (!m231of(menuItem1).equals(m231of(menuItem2))) {
                    return false;
                }
            }
            return true;
