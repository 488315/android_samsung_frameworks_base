...
public final class HbpcdUtils {
  ...
  public int getMcc(int sid, int tz, int DSTflag, boolean isNitzTimeZone) {
    ...
    Cursor c2 =
        this.resolver.query(
            HbpcdLookup.ArbitraryMccSidMatch.CONTENT_URI, projection2, "SID=" + sid, null, null);
    if (c2 != null) {
    ...
    if (c3 != null) {
      ...
      if (c3Counter > 0) {
        if (c3Counter > 1) {
          Log.m102w(LOG_TAG, "something wrong, get more results for 1 conflict SID: " + c3);
        }
        c3.moveToFirst();
    ...
    Cursor c5 =
        this.resolver.query(
            ...
            projection5,
            "SID_Range_Low<=" + sid + " and " + HbpcdLookup.MccSidRange.RANGE_HIGH + ">=" + sid,
            null,
            null);
    if (c5 != null) {
      if (c5.getCount() > 0) {
  ...
  public String getIddByMcc(int mcc) {
    ...
    Cursor cur =
        this.resolver.query(HbpcdLookup.MccIdd.CONTENT_URI, projection, "MCC=" + mcc, null, null);
    if (cur != null) {
      if (cur.getCount() > 0) {
        cur.moveToFirst();
        idd = cur.getString(0);
