...
public class SmsMessage extends SmsMessageBase {
  ...
  public static SmsMessage createFromEfRecord(int index, byte[] data) {
    try {
      ...
      if (msg.mStatusOnIcc != 1 && msg.mStatusOnIcc != 3) {
        msg.mMti = 1;
        int size = data[1] & 255;
        Rlog.m238d(LOG_TAG, "msg[" + index + "]statusOnIcc: " + msg.mStatusOnIcc + " size:" + size);
      ...
      Rlog.m238d(LOG_TAG, "msg[" + index + "]statusOnIcc: " + msg.mStatusOnIcc + " size:" + size2);
  ...
  public static int getTPLayerLengthForPDU(String pdu) {
    Rlog.m246w(LOG_TAG, "getTPLayerLengthForPDU: is not supported in CDMA mode.");
    return 0;
  }
  ...
  public static SubmitPdu getSubmitPdu(
      String scAddr, String destAddr, int destPort, byte[] data, boolean statusReportRequested) {
  ...
  public int getProtocolIdentifier() {
    Rlog.m246w(LOG_TAG, "getProtocolIdentifier: is not supported in CDMA mode.");
    return 0;
  }

  @Override // com.android.internal.telephony.SmsMessageBase
  public boolean isReplace() {
    Rlog.m246w(LOG_TAG, "isReplace: is not supported in CDMA mode.");
    return false;
  }

  @Override // com.android.internal.telephony.SmsMessageBase
  public boolean isCphsMwiMessage() {
    Rlog.m246w(LOG_TAG, "isCphsMwiMessage: is not supported in CDMA mode.");
    return false;
  }
  ...
  public boolean isReplyPathPresent() {
    Rlog.m246w(LOG_TAG, "isReplyPathPresent: is not supported in CDMA mode.");
    return false;
  }
  ...
  public int getMessageType() {
    Rlog.m238d(LOG_TAG, "getMessageType = " + this.mMti);
    return this.mMti;
  }
  ...
  private void parsePdu(byte[] pdu) {
    ...
    try {
      ...
      Rlog.m240e(LOG_TAG, "createFromPdu: conversion from byte array to object failed: " + ex2);
    }
    if (length > pdu.length) {
      throw new RuntimeException(
          "createFromPdu: Invalid pdu, addr.numberOfDigits " + length + " > pdu len " + pdu.length);
    }
    ...
    while (dis.available() > 0) {
      ...
      switch (parameterId) {
        case 0:
          ...
          Rlog.m242i(LOG_TAG, "teleservice = " + env.teleService);
          continue;
          subAddr2 = subAddr;
        case 1:
          subAddr = subAddr2;
        ...
        case 4:
          ...
          if (addr.digitMode == 0) {
            ...
            subAddr = subAddr2;
            if (addr.digitMode != 1) {
              Rlog.m240e(LOG_TAG, "Incorrect Digit mode");
              ...
              if (numberType == 2) {
                Rlog.m240e(LOG_TAG, "TODO: Addr is email id");
              } else {
                Rlog.m240e(LOG_TAG, "TODO: Addr is data network address");
              }
            } else {
              Rlog.m240e(LOG_TAG, "Addr is of incorrect type");
            }
          }
          addr.origBytes = data;
        ...
        default:
          throw new Exception(
              "unsupported parameterId (" + parameterId + NavigationBarInflaterView.KEY_CODE_END);
      }
      Rlog.m240e(LOG_TAG, "parsePduFromEfRecord: conversion from pdu to SmsMessage failed" + ex);
      this.mEnvelope = env;
      this.mPdu = pdu;
      parseSms();
    }
  ...
  public boolean preprocessCdmaFdeaWap() {
    try {
      ...
      if (inStream.read(8) != 0) {
        Rlog.m240e(LOG_TAG, "Invalid FDEA WDP Header Message Identifier SUBPARAMETER_ID");
        return false;
      }
      if (inStream.read(8) != 3) {
        Rlog.m240e(LOG_TAG, "Invalid FDEA WDP Header Message Identifier SUBPARAM_LEN");
        return false;
      }
      ...
      if (this.mBearerData.hasUserDataHeader) {
        Rlog.m240e(LOG_TAG, "Invalid FDEA WDP Header Message Identifier HEADER_IND");
        return false;
      }
      inStream.skip(3);
      if (inStream.read(8) != 1) {
        Rlog.m240e(LOG_TAG, "Invalid FDEA WDP Header User Data SUBPARAMETER_ID");
        return false;
      }
      ...
      if (this.mBearerData.userData.msgEncoding != 0) {
        Rlog.m240e(LOG_TAG, "Invalid FDEA WDP Header User Data MSG_ENCODING");
        return false;
      }
      ...
      Rlog.m240e(LOG_TAG, "Fail to preprocess FDEA WAP: " + ex);
      return false;
    }
  }
  ...
  public void parseSms() {
    ...
    if (Rlog.isLoggable(LOGGABLE_TAG, 2)) {
      Rlog.m238d(
          LOG_TAG, "MT raw BearerData = '" + HexDump.toHexString(this.mEnvelope.bearerData) + "'");
      Rlog.m238d(LOG_TAG, "MT (decoded) BearerData = " + this.mBearerData);
    }
    ...
    if (this.mBearerData.callbackNumber != null) {
      Rlog.m238d(LOG_TAG, "parseSms() callback = " + this.mBearerData.callbackNumber);
    ...
    if (SmsManager.getDefault().getSmsSetting(SmsConstants.SMS_SUPPORT_REPLY_ADDRESS)) {
      if (this.mBearerData.callbackNumber != null) {
        Rlog.m240e(LOG_TAG, "SMS callback number: " + this.mBearerData.callbackNumber.address);
    ...
    if (SmsManager.getDefault().getSmsSetting(SmsConstants.SMS_SPECIAL_ADDRESS_HANDLING_FOR)) {
      if (addr.ton == 0) {
        ...
        if (!TextUtils.isEmpty(origAddress) && origAddress.startsWith("00852")) {
          Rlog.m238d(LOG_TAG, "receive sms from HK number Before Address= " + origAddress);
          String origAddress2 = origAddress.substring(2);
          addr.address = "+";
          addr.address += origAddress2;
          Rlog.m238d(LOG_TAG, "After Address Replacement = " + addr.address);
          return;
        }
  ...
  public SmsCbMessage parseBroadcastSms(String plmn, int slotIndex, int subId) {
    ...
    if (bData == null) {
      Rlog.m246w(LOG_TAG, "BearerData.decode() returned null");
      return null;
    }
    ...
    if (Rlog.isLoggable(LOGGABLE_TAG, 2)) {
      Rlog.m238d(LOG_TAG, "MT raw BearerData = " + HexDump.toHexString(this.mEnvelope.bearerData));
    }
  ...
  public static synchronized int getNextMessageId() {
    ...
    synchronized (SmsMessage.class) {
      ...
      try {
        ...
        if (Rlog.isLoggable(LOGGABLE_TAG, 2)) {
          Rlog.m238d(LOG_TAG, "next persist.radio.cdma.msgid = " + nextMsgId);
          Rlog.m238d(LOG_TAG, "readback gets " + TelephonyProperties.cdma_msg_id().orElse(1));
        }
      } catch (RuntimeException ex) {
        Rlog.m240e(LOG_TAG, "set nextMessage ID failed: " + ex);
      }
    }
    return msgId;
  }
  ...
  private static SubmitPdu privateGetSubmitPdu(
    ...
    if (destAddrStr == null || destAddrStr.length() == 0) {
      Log.m96e(LOG_TAG, "privateGetSubmitPdu - destAddrStr is invalid");
      return null;
    }
    CdmaSmsAddress destAddr =
        CdmaSmsAddress.parse(PhoneNumberUtils.cdmaCheckAndProcessPlusCodeForSms(destAddrStr));
    if (destAddr == null) {
      return null;
    }
    ...
    if (Rlog.isLoggable(LOGGABLE_TAG, 2)) {
      Rlog.m238d(LOG_TAG, "MO (encoded) BearerData = " + bearerData);
      Rlog.m238d(LOG_TAG, "MO raw BearerData = '" + HexDump.toHexString(encodedBearerData) + "'");
    }
    ...
    try {
      ...
      Rlog.m240e(LOG_TAG, "creating SubmitPdu failed: " + ex);
      return null;
    }
  }
  ...
  public static SubmitPdu getDeliverPdu(String origAddr, String message, long date) {
    ...
    try {
      ...
      Rlog.m240e(LOG_TAG, "creating Deliver PDU failed: " + ex);
      return null;
    }
  }
  ...
  public void createPdu() {
    ...
    try {
      ...
      Rlog.m240e(LOG_TAG, "createPdu: conversion from object to byte array failed: " + ex);
    }
  }
  ...
  public byte[] getIncomingSmsFingerprint() {
    ...
    output.write(
        this.mEnvelope.origAddress.origBytes, 0, this.mEnvelope.origAddress.origBytes.length);
    if (SmsManager.getDefault().getSmsSetting(SmsConstants.SMS_3GPP2_LGT_NETWORK)
        && this.mEnvelope.teleService == 4098) {
      if (this.mBearerData.userData != null) {
        output.write(
            this.mBearerData.userData.payload, 0, this.mBearerData.userData.payload.length);
      }
      if (this.mBearerData.msgCenterTimeStamp != null) {
        output.write(
  ...
  public static SmsMessage semCreateFromPdu(int phoneId, byte[] pdu) {
    ...
    try {
      ...
      Log.m97e(LOG_TAG, "SMS PDU parsing failed with out of memory: ", e);
      return null;
    } catch (RuntimeException ex) {
      Rlog.m241e(LOG_TAG, "SMS PDU parsing failed: ", ex);
      return null;
  ...
  private static SubmitPdu privateGetSubmitPdu(
    ...
    CdmaSmsAddress destAddr =
        CdmaSmsAddress.parse(
            PhoneNumberUtils.cdmaCheckAndProcessPlusCodeByNumberFormat(destAddrStr, 1, 1));
    if (destAddr == null) {
      return null;
    }
    BearerData bearerData = new BearerData();
    ...
    if (callbackNumber != null && callbackNumber.length() > 0) {
      Rlog.m238d(LOG_TAG, "callback number is set: " + callbackNumber);
    ...
    if (priority == 2) {
      Rlog.m238d(LOG_TAG, "priority is set to high");
      bearerData.priorityIndicatorSet = true;
      bearerData.priority = priority;
    }
  ...
  public static SubmitPdu getDomainChangeNotification(byte type, String doChgAddr) {
    ...
    CdmaSmsAddress destAddr =
        CdmaSmsAddress.parse(
            PhoneNumberUtils.cdmaCheckAndProcessPlusCodeByNumberFormat(doChgAddr, 1, 1));
    if (destAddr == null) {
      return null;
    }
    BearerData bearerData = new BearerData();
    ...
    if (Log.isLoggable(LOGGABLE_TAG, 2)) {
      Log.m94d(LOG_TAG, "MO (encoded) BearerData = " + bearerData);
      if (encodedBearerData != null) {
        Log.m94d(LOG_TAG, "MO raw BearerData = '" + HexDump.toHexString(encodedBearerData) + "'");
      }
    }
    if (encodedBearerData == null) {
      return null;
