...
public class Sms7BitEncodingTranslator {
    ...
    public static String translate(CharSequence message, boolean isCdmaFormat) {
        ...
        if (message == null) {
            Rlog.m246w(TAG, "Null message can not be translated");
            return null;
        }
    ...
    private static char translateIfNeeded(char c, boolean isCdmaFormat) {
        if (noTranslationNeeded(c, isCdmaFormat)) {
            if (DBG) {
                Rlog.m244v(TAG, "No translation needed for " + Integer.toHexString(c));
            }
            return c;
        }
        int translation = -1;
        ...
        if (translation != -1) {
            if (DBG) {
                Rlog.m244v(TAG, Integer.toHexString(c) + " (" + c + ") translated to " + Integer.toHexString(translation) + " (" + ((char) translation) + NavigationBarInflaterView.KEY_CODE_END);
            }
            return (char) translation;
        }
        if (DBG) {
            Rlog.m246w(TAG, "No translation found for " + Integer.toHexString(c) + "! Replacing for empty space");
            return ' ';
        }
        return ' ';
    }
    ...
    private static void load7BitTranslationTableFromXml() {
        ...
        if (0 == 0) {
            if (DBG) {
                Rlog.m238d(TAG, "load7BitTranslationTableFromXml: open normal file");
            }
            parser = r.getXml(C4337R.xml.sms_7bit_translation_table);
        }
        try {
            try {
                ...
                while (true) {
                    ...
                    if (XML_TRANSLATION_TYPE_TAG.equals(tag)) {
                        ...
                        if (type.equals(ContactsContract.CommonDataKinds.PACKAGE_COMMON)) {
                            ...
                            Rlog.m240e(TAG, "Error Parsing 7BitTranslationTable: found incorrect type" + type);
                        }
                    } else {
                        ...
                        if (from == -1 || to == -1) {
                            Rlog.m238d(TAG, "Invalid translation table file format");
                        } else {
                            if (z) {
                                Rlog.m238d(TAG, "Loading mapping " + Integer.toHexString(from).toUpperCase() + " -> " + Integer.toHexString(to).toUpperCase());
                            }
                ...
                if (z) {
                    Rlog.m238d(TAG, "load7BitTranslationTableFromXml: parsing successful, file loaded");
                }
                if (!(parser instanceof XmlResourceParser)) {
                    return;
                }
            } catch (Exception e) {
                Rlog.m241e(TAG, "Got exception while loading 7BitTranslationTable file.", e);
                if (!(parser instanceof XmlResourceParser)) {
                    return;
                }
            }
