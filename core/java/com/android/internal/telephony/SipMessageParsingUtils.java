...
public class SipMessageParsingUtils {
  ...
  private static String getParameterValue(String headerValue, String parameterKey) {
    ...
    for (String param : params) {
      ...
      if (pair.length >= 2) {
        if (pair.length > 2) {
          Log.m102w(TAG, "getParameterValue: unexpected parameter" + Arrays.toString(pair));
        }
        pair[0] = pair[0].trim();
        pair[1] = pair[1].trim();
  ...
  public static String getCallId(String headerString) {
    List<Pair<String, String>> headers =
        parseHeaders(headerString, true, CALL_ID_SIP_HEADER_KEY, CALL_ID_SIP_HEADER_KEY_COMPACT);
    if (headers.isEmpty()) {
      return null;
    }
  ...
  public static List<Pair<String, String>> parseHeaders(
    ...
    for (String line : headerLines) {
      if (line.startsWith("\t") || line.startsWith(" ")) {
        ...
        if (pair.length < 2) {
          Log.m102w(TAG, "parseHeaders - received malformed line: " + line);
        } else {
