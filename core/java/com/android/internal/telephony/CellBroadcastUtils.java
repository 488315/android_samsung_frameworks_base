...
public class CellBroadcastUtils {
  ...
  public static String getDefaultCellBroadcastReceiverPackageName(Context context) {
    ...
    ResolveInfo resolveInfo =
        packageManager.resolveActivity(
            new Intent(Telephony.Sms.Intents.SMS_CB_RECEIVED_ACTION), 1048576);
    if (resolveInfo == null) {
      Log.m96e(TAG, "getDefaultCellBroadcastReceiverPackageName: no package found");
      return null;
    }
    String packageName = resolveInfo.activityInfo.applicationInfo.packageName;
    if (TextUtils.isEmpty(packageName)
        || packageManager.checkPermission(Manifest.permission.READ_CELL_BROADCASTS, packageName)
            == -1) {
      Log.m96e(
          TAG,
  ...
  public static ComponentName getDefaultCellBroadcastAlertDialogComponent(Context context) {
    ...
    return ComponentName.createRelative(
        cellBroadcastReceiverPackageName,
