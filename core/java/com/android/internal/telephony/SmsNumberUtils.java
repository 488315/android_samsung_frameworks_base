...
public class SmsNumberUtils {
  ...
  private static String formatNumber(
    ...
    boolean z = DBG;
    if (z) {
      Log.m94d(TAG, "NANP type: " + getNumberPlanType(nanpState));
    }
    ...
    if (z) {
      Log.m94d(TAG, "International type: " + getNumberPlanType(internationalState));
    }
  ...
  */
  /* JADX WARN: Code restructure failed: missing block: B:27:0x006b, code lost:
     ...
     android.util.Log.m94d(com.android.internal.telephony.SmsNumberUtils.TAG, "MCC = " + r12 + ", all IDDs = " + r2);
  */
    ...
    try {
      try {
        cursor =
            context
                .getContentResolver()
                .query(HbpcdLookup.MccIdd.CONTENT_URI, projection, where, selectionArgs, null);
        ...
        Log.m97e(TAG, "Can't access HbpcdLookup database", e);
      }
    } finally {
      if (cursor != null) {
        cursor.close();
  ...
  private static int getCountryCode(Context context, String number) {
    ...
    for (int tempCC : allCCs) {
      for (int j = 0; j < MAX_COUNTRY_CODES_LENGTH; j++) {
        if (tempCC == ccArray[j]) {
          if (DBG) {
            Log.m94d(TAG, "Country code = " + tempCC);
          }
          return tempCC;
        }
      }
  ...
  */
    ...
    try {
      try {
        ...
        cursor =
            context
                .getContentResolver()
                .query(HbpcdLookup.MccLookup.CONTENT_URI, projection, null, null, null);
        if (cursor.getCount() > 0) {
  ...
  private static boolean inExceptionListForNpCcAreaLocal(NumberEntry numberEntry) {
    ...
    return numberEntry.number.length() == 12
        && (countryCode == 7 || countryCode == 20 || countryCode == 65 || countryCode == 90);
  }
  ...
  public static String filterDestAddr(Context context, int subId, String destAddr) {
    ...
    boolean z = DBG;
    if (z) {
      Log.m94d(TAG, "enter filterDestAddr. destAddr=\"" + pii(TAG, destAddr) + "\"");
    }
    if (destAddr == null || !PhoneNumberUtils.isGlobalPhoneNumber(destAddr)) {
      Log.m102w(
          TAG, "destAddr" + pii(TAG, destAddr) + " is not a global phone number! Nothing changed.");
      return destAddr;
    }
    TelephonyManager telephonyManager =
        ((TelephonyManager) context.getSystemService("phone")).createForSubscriptionId(subId);
    ...
    if (z) {
      Log.m94d(TAG, "destAddr is " + (result != null ? "formatted." : "not formatted."));
      Log.m94d(
          TAG,
  ...
  private static int getNetworkType(TelephonyManager telephonyManager) {
    ...
    if (DBG) {
      Log.m102w(TAG, "warning! unknown mPhoneType value=" + phoneType);
      return -1;
    }
    return -1;
  }
