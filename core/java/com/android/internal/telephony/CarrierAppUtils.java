...
public final class CarrierAppUtils {
    ...
    private static ContentResolver getContentResolverForUser(Context context, int userId) {
        Context userContext = context.createContextAsUser(UserHandle.m55of(userId), 0);
    ...
    */
    public static void disableCarrierAppsUntilPrivileged(String callingPackage, TelephonyManager telephonyManager, ContentResolver contentResolver, int userId, Set<String> systemCarrierAppsDisabledUntilUsed, Map<String, List<CarrierAssociatedAppEntry>> systemCarrierAssociatedAppsDisabledUntilUsed, Context context) {
        ...
        if (candidates != null && !candidates.isEmpty()) {
            ...
            try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            try {
                                if (telephonyManager2 != null) {
                                    try {
                                        if (telephonyManager2.checkCarrierPrivilegesForPackageAnyPhone(packageName) == 1) {
                                            ...
                                            if (associatedAppList == null) {
                                                try {
                                                    ...
                                                    while (it2.hasNext()) {
                                                        ...
                                                        try {
                                                            packageManager.setSystemAppState(it2.next().appInfo.packageName, 0);
                                            ...
                                            int enabledSetting = context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().getApplicationEnabledSetting(packageName);
                                            ...
                                            if (hasPrivileges) {
                                                ...
                                                try {
                                                    if (!isUpdatedSystemApp(ai2) && enabledSetting == 0 && (ai2.flags & 8388608) != 0) {
                                                        Log.m98i(TAG, "Update state (" + packageName + "): DISABLED_UNTIL_USED for user " + userId);
                                                        context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().setSystemAppState(packageName, 3);
                                                    }
                                                    if (associatedAppList != null) {
                                                        for (AssociatedAppInfo associatedApp : associatedAppList) {
                                                            if (hasRunEver) {
                                                                if (hasRunForSdk || associatedApp.addedInSdk == -1) {
                                                                    ai = ai2;
                                                                    carrierAppsHandledSdk3 = carrierAppsHandledSdk;
                                                                } else {
                                                                    carrierAppsHandledSdk3 = carrierAppsHandledSdk;
                                                                    if (associatedApp.addedInSdk > carrierAppsHandledSdk3) {
                                                                        ai = ai2;
                                                                        if (associatedApp.addedInSdk <= Build.VERSION.SDK_INT) {
                                                                        }
                                                                    } else {
                                                                        ai = ai2;
                                                                    }
                                                                ...
                                                                allowDisable = false;
                                                                int associatedAppEnabledSetting = context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().getApplicationEnabledSetting(associatedApp.appInfo.packageName);
                                                                boolean associatedAppInstalled = (associatedApp.appInfo.flags & 8388608) == 0;
                                                                if (!allowDisable && associatedAppEnabledSetting == 0 && associatedAppInstalled) {
                                                                    Log.m98i(TAG, "Update associated state (" + associatedApp.appInfo.packageName + "): DISABLED_UNTIL_USED for user " + userId);
                                                                    i = 3;
                                                                    context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().setSystemAppState(associatedApp.appInfo.packageName, 3);
                                                                } else {
                                                                    i = 3;
                                                                }
                                                            ...
                                                            allowDisable = true;
                                                            int associatedAppEnabledSetting2 = context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().getApplicationEnabledSetting(associatedApp.appInfo.packageName);
                                                            if ((associatedApp.appInfo.flags & 8388608) == 0) {
                                                            }
                                                            if (!allowDisable) {
                                                            }
                                                            i = 3;
                                                ...
                                                try {
                                                    ...
                                                    Log.m98i(TAG, "Update state (" + packageName + "): ENABLED for user " + userId);
                                                    context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().setSystemAppState(packageName, 2);
                                                    context.createPackageContextAsUser(str3, 0, UserHandle.m55of(userId)).getPackageManager().setApplicationEnabledSetting(packageName, 1, 1);
                                                    if (associatedAppList != null) {
                                                        Iterator<AssociatedAppInfo> it3 = associatedAppList.iterator();
                                                        while (it3.hasNext()) {
                                                            AssociatedAppInfo associatedApp2 = it3.next();
                                                            Iterator<AssociatedAppInfo> it4 = it3;
                                                            String str6 = str4;
                                                            int associatedAppEnabledSetting3 = context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().getApplicationEnabledSetting(associatedApp2.appInfo.packageName);
                                                            boolean associatedAppInstalled2 = (associatedApp2.appInfo.flags & 8388608) != 0;
                                                            try {
                                                                ...
                                                                Log.m98i(TAG, "Update associated state (" + associatedApp2.appInfo.packageName + str5 + userId);
                                                                str2 = str5;
                                                                context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().setSystemAppState(associatedApp2.appInfo.packageName, 2);
                                                                context.createPackageContextAsUser(str3, 0, UserHandle.m55of(userId)).getPackageManager().setApplicationEnabledSetting(associatedApp2.appInfo.packageName, 1, 1);
                                                                it3 = it4;
                                                                str4 = str6;
                                                                carrierAppsHandledSdk4 = carrierAppsHandledSdk2;
                                                                str5 = str2;
                                ...
                                int enabledSetting2 = context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().getApplicationEnabledSetting(packageName);
                ...
                try {
                    permissionManager4.grantDefaultPermissionsToEnabledCarrierApps(packageNames, UserHandle.m55of(userId), TelephonyUtils.DIRECT_EXECUTOR, new Consumer() { // from class: com.android.internal.telephony.CarrierAppUtils$$ExternalSyntheticLambda0
    ...
    private static ApplicationInfo getApplicationInfoIfSystemApp(int userId, String packageName, Context context) {
        try {
            ApplicationInfo ai = context.createContextAsUser(UserHandle.m55of(userId), 0).getPackageManager().getApplicationInfo(packageName, 537952256);
            if (ai != null) {
                return ai;
            }
            return null;
