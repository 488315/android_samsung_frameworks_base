...
public class IccUtils {
  ...
  public static String adnStringFieldToString(byte[] data, int offset, int length) {
    ...
    try {
      defaultCharset = resource.getString(C4337R.string.gsm_alphabet_default_charset);
  ...
  public static Bitmap parseToBnW(byte[] data, int length) {
    ...
    if (pixelIndex != numOfPixels) {
      Rlog.m240e(LOG_TAG, "parse end and size error");
    }
  ...
  public static Bitmap parseToRGB(byte[] data, int length, boolean transparency) {
    ...
    if (8 % bits == 0) {
      ...
      resultArray =
          mapToNon2OrderBitColor(data, valueIndex6, width * height, colorIndexArray, bits);
    }
  ...
  private static int[] mapTo2OrderBitColor(
    ...
    if (8 % bits != 0) {
      Rlog.m240e(LOG_TAG, "not event number of color");
  ...
  private static int[] mapToNon2OrderBitColor(
      byte[] data, int valueIndex, int length, int[] colorArray, int bits) {
    if (8 % bits == 0) {
      Rlog.m240e(LOG_TAG, "not odd number of color");
  ...
  private static int[] getCLUT(byte[] rawData, int offset, int number) {
    ...
    while (true) {
      ...
      int valueIndex3 = valueIndex2 + 1;
      int i =
          ((rawData[valueIndex] & 255) << 16) | (-16777216) | ((rawData[valueIndex2] & 255) << 8);
      int valueIndex4 = valueIndex3 + 1;
  ...
  public static int bytesToInt(byte[] src, int offset, int length) {
    ...
    if (offset < 0 || length < 0 || offset + length > src.length) {
      throw new IndexOutOfBoundsException(
          "Out of the bounds: src=[" + src.length + "], offset=" + offset + ", length=" + length);
    }
    int result = 0;
  ...
  public static String mccMncConvert(String s) {
    ...
    if (ret.toString().compareToIgnoreCase("fff") == 0) {
      Rlog.m242i(LOG_TAG, "[MccMncConvert] MCC Value is invalid('fff')!");
      return null;
    }
    try {
      ...
      Rlog.m241e(LOG_TAG, "mccMncConvert Exception:", e);
    }
    ...
    Rlog.m242i(LOG_TAG, "[MccMncConvert] Convert Result :" + ret.toString());
    return ret.toString();
  }
  ...
  public static int getIccType(int phoneId) {
    ...
    try {
      ...
      Rlog.m241e(LOG_TAG, "getIccType Exception:", e);
      return 0;
    }
  }
  ...
  public static void setUiccProperty(int phoneId, String property, String value) {
    ...
    if (phoneId < 0 || phoneId >= TelephonyManager.getDefault().getActiveModemCount()) {
      Rlog.m238d(LOG_TAG, "setUiccProperty: invalid phoneId=" + phoneId);
      return;
    }
    ...
    try {
      ...
      Rlog.m238d(LOG_TAG, "setUiccProperty: utf-8 not supported");
    }
    if (propValLen > 91) {
      Rlog.m238d(LOG_TAG, "setUiccProperty: property too long property=" + property);
    } else {
