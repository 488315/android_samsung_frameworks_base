...
public class GsmSmsAddress extends SmsAddress {
    ...
    public GsmSmsAddress(byte[] data, int offset, int length) throws ParseException {
        ...
        if ((toa & 128) != 128) {
            throw new ParseException(
                    "Invalid TOA - high bit must be set. toa = " + toa, offset + 1);
        }
        if (isAlphanumeric()) {
    ...
    public GsmSmsAddress(int subId, byte[] data, int offset, int length) throws ParseException {
        ...
        if (isAlphanumeric()) {
            ...
            this.address =
                    PhoneNumberUtils.calledPartyBCDToString(this.origBytes, 1, length - 1, 2);
            this.origBytes[length - 1] = lastByte;
        }
        if (this.address != null) {
            ...
            if (SmsManager.getSmsManagerForContextAndSubscriptionId(null, subId)
                ...
                } else if (DOCOMO.equalsIgnoreCase(this.partofaddress)
                        && !DOCOMO.equals(this.partofaddress)) {
                    this.address = NTT_DOCOMO;
                }
            }
        }
    ...
    public boolean isCphsVoiceMessageSet() {
        return isCphsVoiceMessageIndicatorAddress()
                && ((this.origBytes[2] & 255) == 17 || (this.origBytes[2] & 255) == 145);
    }

    public boolean isCphsVoiceMessageClear() {
        return isCphsVoiceMessageIndicatorAddress()
                && ((this.origBytes[2] & 255) == 16 || (this.origBytes[2] & 255) == 144);
    }
}
