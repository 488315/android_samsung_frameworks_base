...
public class ProcessCpuTracker {
  ...
  private static final int[] PROCESS_STATS_FORMAT = {
    32, 544, 32, 32, 32, 32, 32, 32, 32, 8224, 32, 8224, 32, 8224, 8224
  };
  private static final int[] PROCESS_FULL_STATS_FORMAT = {
    32, 4640, 32, 32, 32, 32, 32, 32, 32, 8224, 32, 8224, 32, 8224, 8224, 32, 32, 32, 32, 32, 32,
    32, 8224
  };
  ...
  private static final Comparator<Stats> sLoadComparator =
      new Comparator<Stats>() { // from class: com.android.internal.os.ProcessCpuTracker.1
  ...
  public static class Stats {
    ...
    Stats(int _pid, int parentPid, boolean includeThreads) {
      ...
      File taskDir =
          new File(
              new File(new File("/proc", Integer.toString(parentPid)), "task"),
              Integer.toString(_pid));
    ...
    private static int getUid(String path) {
      try {
        ...
        Slog.m121w(ProcessCpuTracker.TAG, "Failed to stat(" + path + "): " + e);
        return -1;
      }
    }
  }
      ...
      if (stats2 != null && stats2.pid == i12) {
        ...
        if (stats2.interesting) {
          ...
          if (!Process.readProcFile(
              stats2.statFile.toString(), PROCESS_STATS_FORMAT, null, jArr, null)) {
            iArr3 = pids;
            i2 = length;
            i7 = size;
            i8 = i11;
            ...
            if (j8 == stats2.base_utime && j9 == stats2.base_stime) {
              ...
              if (i9 < 0) {
                ...
                if (stats2.threadStats != null) {
                  ...
                  this.mCurThreadPids =
                      collectStats(
                          stats2.threadsDir, i12, false, this.mCurThreadPids, stats2.threadStats);
                } else {
                  stats = stats2;
                  iArr3 = pids;
                  z4 = z3;
        ...
        if (Process.readProcFile(
            stats3.statFile.toString(), PROCESS_FULL_STATS_FORMAT, strArr, jArr2, null)) {
          ...
          Slog.m121w(TAG, "Skipping unknown process pid " + i4);
        ...
        if (i5 < 0) {
          ...
          if (stats3.threadStats != null) {
            this.mCurThreadPids =
                collectStats(stats3.threadsDir, i4, true, this.mCurThreadPids, stats3.threadStats);
          }
  ...
  private void getName(Stats st, String cmdlineFile) {
    ...
    if (st.name == null
      ...
      if (cmdName != null
          && cmdName.length() > 1
          && (i = (newName = cmdName).lastIndexOf("/")) > 0
          && i < newName.length() - 1) {
  ...
  public String printCpuCoreInfo() {
    ...
    String[] CPUINFO_EACH_PATH = {
      "/sys/devices/system/cpu/cpu%d/cpufreq/scaling_cur_freq",
      "/sys/devices/system/cpu/cpu%d/cpufreq/scaling_governor",
      "/sys/devices/system/cpu/cpu%d/cpufreq/scaling_max_freq"
    };
    String[] AP_THERMISTOR_PATH = {
    ...
    while (true) {
      ...
      if (!Process.readProcFile(
          String.format(AP_THERMISTOR_PATH[i3], Integer.valueOf(i3)),
          SINGLE_LONG_FORMAT,
          null,
          thermistor,
          null)) {
    ...
    if (Process.readProcFile(
        "/sys/devices/system/cpu/possible", SINGLE_STRING_FORMAT, cpuinfo, null, null)) {
    ...
    if (core_num > 0) {
      ...
      while (i6 < CPUINFO_EACH_PATH.length) {
        ...
        pw.print(
            String.format(
                "\n%-18s",
                CPUINFO_EACH_PATH[i6].substring(CPUINFO_EACH_PATH[i6].lastIndexOf(i) + 1)));
        for (int j = 0; j <= core_num; j++) {
          if (Process.readProcFile(
              String.format(CPUINFO_EACH_PATH[i6], Integer.valueOf(j)),
              SINGLE_STRING_FORMAT,
              cpuinfo,
              null,
              null)) {
