...
public class KernelCpuThreadReader {
  ...
  public static KernelCpuThreadReader create(int numBuckets, Predicate<Integer> uidPredicate) {
    try {
      ...
      Slog.m116e(TAG, "Failed to initialize KernelCpuThreadReader", e);
      return null;
    }
  }
  ...
  public ArrayList<ProcessCpuUsage> getProcessCpuUsage() {
    ...
    try {
      ...
      try {
        for (Path processPath : processPaths) {
          ...
          if (uid != -1
              ...
              && (processCpuUsage = getProcessCpuUsage(processPath, processId, uid)) != null) {
        ...
        if (processCpuUsages.isEmpty()) {
          Slog.m121w(TAG, "Didn't successfully get any process CPU information for UIDs specified");
          return null;
        }
      ...
      Slog.m122w(TAG, "Failed to iterate over process paths", e);
      return null;
    }
  }
  ...
  void setNumBuckets(int numBuckets) {
    ...
    if (numBuckets != 0) {
      FrequencyBucketCreator frequencyBucketCreator =
          new FrequencyBucketCreator(frequenciesKhz, numBuckets);
  ...
  private ThreadCpuUsage getThreadCpuUsage(Path threadDirectory) {
    ...
    try {
      ...
      Slog.m122w(TAG, "Failed to parse thread ID when iterating over /proc/*/task", e);
      return null;
    }
  }
  ...
  private int getProcessId(Path processPath) {
    ...
    try {
      ...
      Slog.m122w(TAG, "Failed to parse " + fileName + " as process ID", e);
      return -1;
    }
  }
  ...
  public static class FrequencyBucketCreator {
    ...
    public FrequencyBucketCreator(long[] frequencies, int targetNumBuckets) {
      ...
      int[] bucketStartIndices =
          getBucketStartIndices(clusterStartIndices, targetNumBuckets, length);
  ...
  public static class ProcessCpuUsage {
    ...
    public ProcessCpuUsage(
        int processId, String processName, int uid, ArrayList<ThreadCpuUsage> threadCpuUsages) {
