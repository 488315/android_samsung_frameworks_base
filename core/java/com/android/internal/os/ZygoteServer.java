...
class ZygoteServer {
  ...
  ZygoteServer(boolean isPrimaryZygote) {
    ...
    if (isPrimaryZygote) {
      ...
      this.mUsapPoolSocket =
          Zygote.createManagedSocketFromInitSocket(Zygote.USAP_POOL_SECONDARY_SOCKET_NAME);
    }
  ...
  void closeServerSocket() {
    try {
      ...
      Log.m97e(TAG, "Zygote:  error closing descriptor", ex);
    } catch (IOException ex2) {
      Log.m97e(TAG, "Zygote:  error closing sockets", ex2);
    }
    this.mZygoteSocket = null;
  }
  ...
  private void fetchUsapPoolPolicyProps() {
    if (this.mUsapPoolSupported) {
      ...
      if (this.mUsapPoolSizeMin >= this.mUsapPoolSizeMax) {
        Log.m102w(
            TAG,
            "The max size of the USAP pool must be greater than the minimum size.  Restoring"
                + " default values.");
  ...
  Runnable fillUsapPool(int[] sessionSocketRawFDs, boolean isPriorityRefill) {
    ...
    if (isPriorityRefill) {
      ...
      Log.m98i(
          Zygote.PRIMARY_SOCKET_NAME, "Priority USAP Pool refill. New USAPs: " + numUsapsToSpawn);
    } else {
      numUsapsToSpawn = this.mUsapPoolSizeMax - usapPoolCount;
      Log.m98i(
          Zygote.PRIMARY_SOCKET_NAME, "Delayed USAP Pool refill. New USAPs: " + numUsapsToSpawn);
    }
    ZygoteHooks.preFork();
    do {
      numUsapsToSpawn--;
  ...
  Runnable setUsapPoolStatus(boolean newStatus, LocalSocket sessionSocket) {
    if (!this.mUsapPoolSupported) {
      Log.m102w(TAG, "Attempting to enable a USAP pool for a Zygote that doesn't support it.");
      return null;
    }
    if (this.mUsapPoolEnabled == newStatus) {
      return null;
    }
    Log.m98i(TAG, "USAP Pool status change: " + (newStatus ? "ENABLED" : "DISABLED"));
      ...
      if (this.mUsapPoolRefillTriggerTimestamp == j) {
        ...
        if (elapsedTimeMs >= pollTimeoutMs2) {
          ...
          pollTimeoutMs =
              elapsedTimeMs <= 0
                  ? this.mUsapPoolRefillDelayMs
                  : (int) (pollTimeoutMs2 - elapsedTimeMs);
        }
      }
      try {
                ...
                if (pollIndex6 == 0) {
                  ...
                  try {
                    try {
                      ...
                      boolean multipleForksOK =
                          !isUsapPoolEnabled() && ZygoteHooks.isIndefiniteThreadSuspensionSafe();
                    ...
                    if (this.mIsForkChild) {
                      Log.m97e(TAG, "Caught post-fork exception in child process.", e);
                      throw e;
                    }
                    Slog.m116e(TAG, "Exception executing zygote command: ", e);
                  ...
                  try {
                    ...
                    if (pollIndex6 == usapPoolEventFDIndex) {
                      Log.m96e(TAG, "Failed to read from USAP pool event FD: " + ex.getMessage());
                    } else {
                      Log.m96e(TAG, "Failed to read from USAP reporting pipe: " + ex.getMessage());
                    }
                  }
                  if (readBytes == 8) {
                    ...
                    Log.m96e(TAG, "Incomplete read from USAP management FD of size " + readBytes);
                  }
                }
              }
              pollIndex5 = pollIndex6;
