    ...
    try {
      try {
        try {
            ...
            while (true) {
              ...
              if (total == cArr2.length) {
                if (cArr2.length == 1048576) {
                  this.mErrors++;
                  Slog.m115e(TAG, "Proc file too large: " + this.mFile);
                  if (r != null) {
                    r.close();
                  }
                  return null;
          ...
          this.mErrors++;
          Slog.m116e(TAG, "Error reading " + this.mFile, e);
          return null;
        }
      } catch (FileNotFoundException | NoSuchFileException e2) {
        this.mErrors++;
        Slog.m121w(TAG, "File not found. It's normal if not implemented: " + this.mFile);
        return null;
      }
    } finally {
