...
public abstract class KernelCpuUidTimeReader<T> {
  ...
  public static class KernelCpuUidUserSysTimeReader extends KernelCpuUidTimeReader<long[]> {
    ...
    */
    void readDeltaImpl(Callback<long[]> callback, boolean z) {
      ...
      char c = 1;
      KernelCpuProcStringReader.ProcFileIterator open =
          kernelCpuUidUserSysTimeReader.mReader.open(!kernelCpuUidUserSysTimeReader.mThrottle || z);
      if (open == null) {
        if (open != null) {
          open.close();
          return;
      ...
      while (true) {
        try {
          ...
          if (KernelCpuProcStringReader.asLongs(nextLine, kernelCpuUidUserSysTimeReader.mBuffer)
              < 3) {
            ...
            Slog.m115e(
                kernelCpuUidUserSysTimeReader.mTag,
    ...
    private void removeUidsFromKernelModule(int startUid, int endUid) {
      FileWriter writer;
      Slog.m113d(
          this.mTag, "Removing uids " + startUid + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + endUid);
      ...
      try {
        try {
          ...
          Slog.m116e(
              this.mTag,
              "failed to remove uids " + startUid + " - " + endUid + " from uid_cputime module",
              e);
        }
        try {
      ...
      while (true) {
        ...
        if (i2 < i3) {
          ...
          if (j >= 0) {
            ...
            Slog.m115e(
                this.mTag,
                "Negative delta from freq time for uid: " + i + ", delta: " + this.mDeltaTimes[i2]);
            return;
          }
        } else {
          if (z) {
  ...
  public static class KernelCpuUidFullTimeReader extends KernelCpuUidTimeReader<long[]> {
    ...
    public KernelCpuUidFullTimeReader(boolean throttle) {
      this(
          null, KernelCpuUidBpfMapReader.getFullTimeReaderInstance(), throttle, Clock.SYSTEM_CLOCK);
    }
  ...
  public static class KernelCpuUidActiveTimeReader extends KernelCpuUidTimeReader<Long> {
    ...
    public KernelCpuUidActiveTimeReader(
        KernelCpuProcStringReader reader, KernelCpuUidBpfMapReader bpfReader, boolean throttle) {
      ...
      if (sumActiveTime > 0) {
        ...
        if (longValue <= 0) {
          if (longValue < 0) {
            Slog.m115e(
                this.mTag,
                "Negative delta from active time for uid: " + i + ", delta: " + longValue);
          }
        } else {
