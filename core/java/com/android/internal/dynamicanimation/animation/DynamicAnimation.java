...
public abstract class DynamicAnimation<T extends DynamicAnimation<T>>
    ...
    <K> DynamicAnimation(K object, FloatProperty<K> property) {
        ...
        if (this.mProperty == ROTATION
                || this.mProperty == ROTATION_X
                || this.mProperty == ROTATION_Y) {
    ...
    public void cancel() {
        if (!isCurrentThread()) {
            throw new AndroidRuntimeException(
                    "Animations may only be canceled from the same thread as the animation"
                        + " handler");
        }
        if (this.mRunning) {
    ...
    private void endAnimationInternal(boolean canceled) {
        ...
        for (int i = 0; i < this.mEndListeners.size(); i++) {
            if (this.mEndListeners.get(i) != null) {
                this.mEndListeners
                        .get(i)
                        .onAnimationEnd(this, canceled, this.mValue, this.mVelocity);
            }
        }
