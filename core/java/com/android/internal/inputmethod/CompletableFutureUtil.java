...
public final class CompletableFutureUtil {
  ...
  private static <T> T getValueOrNullInternal(
    ...
    boolean needsToUnregister =
        cancellationGroup != null && cancellationGroup.tryRegisterFutureOrCancelImmediately(future);
    boolean interrupted = false;
    while (true) {
      try {
        try {
  ...
  private static void logTimeoutInternal(String tag, String methodName, long timeout) {
    ...
    Log.m102w(tag, methodName + " didn't respond in " + timeout + " msec.");
  }
  ...
  private static void logErrorInternal(String tag, String methodName, String errorString) {
    ...
    Log.m102w(tag, methodName + " was failed with an exception=" + errorString);
  }
  ...
  private static void logCancellationInternal(String tag, String methodName) {
    ...
    Log.m102w(tag, methodName + " was cancelled.");
  }
  ...
  public static boolean getResultOrFalse(
      ...
      long timeoutMillis) {
    Boolean obj =
        (Boolean) getValueOrNullInternal(future, tag, methodName, timeoutMillis, cancellationGroup);
    if (obj != null) {
  ...
  public static int getResultOrZero(
      ...
      long timeoutMillis) {
    Integer obj =
        (Integer) getValueOrNullInternal(future, tag, methodName, timeoutMillis, cancellationGroup);
    if (obj != null) {
      return obj.intValue();
    }
    return 0;
