...
class ImeTracingServerImpl extends ImeTracing {
    ...
    if (isAvailable() && isEnabled()) {
      switch (source) {
        ...
        case 2:
          ...
          break;
        default:
          Log.m102w("imeTracing", "Request to add to buffer, but source not recognised.");
          break;
      }
    }
  }
  ...
  private void writeTracesToFilesLocked() {
    try {
      ...
      Log.m97e("imeTracing", "Unable to write buffer to file", e);
    }
  }
  ...
  public void startTrace(PrintWriter pw) {
    if (Build.IS_USER) {
      Log.m102w("imeTracing", "Warn: Tracing is not supported on user builds.");
      return;
    }
    synchronized (this.mEnabledLock) {
      if (isAvailable() && isEnabled()) {
        Log.m102w("imeTracing", "Warn: Tracing is already started.");
        return;
      }
      logAndPrintln(
          pw,
  ...
  public void stopTrace(PrintWriter pw) {
    ...
    synchronized (this.mEnabledLock) {
      ...
      Log.m102w("imeTracing", "Warn: Tracing is not available or not started.");
    }
  }
