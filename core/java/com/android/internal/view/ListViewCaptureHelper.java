...
public class ListViewCaptureHelper implements ScrollCaptureViewHelper<ListView> {
    ...
    public void onScrollRequested2(ListView listView, Rect scrollBounds, Rect requestRect, CancellationSignal signal, Consumer<ScrollCaptureViewHelper.ScrollResult> resultConsumer) {
        Log.m94d(TAG, "-----------------------------------------------------------");
        Log.m94d(TAG, "onScrollRequested(scrollBounds=" + scrollBounds + ", requestRect=" + requestRect + NavigationBarInflaterView.KEY_CODE_END);
        ...
        if (!listView.isVisibleToUser() || listView.getChildCount() == 0) {
            Log.m102w(TAG, "listView is empty or not visible, cannot continue");
        ...
        if (scrollAmount < 0) {
            Log.m94d(TAG, "About to scroll UP (content moves down within parent)");
        } else if (scrollAmount > 0) {
            Log.m94d(TAG, "About to scroll DOWN (content moves up within parent)");
        }
        Log.m94d(TAG, "scrollAmount: " + scrollAmount);
        ...
        Log.m94d(TAG, "Parent view has scrolled vertically by " + scrollDistance + " px");
        int i = this.mScrollDelta + scrollDistance;
        this.mScrollDelta = i;
        result.scrollDelta = i;
        if (scrollDistance != 0) {
            Log.m94d(TAG, "Scroll delta is now " + this.mScrollDelta + " px");
        }
