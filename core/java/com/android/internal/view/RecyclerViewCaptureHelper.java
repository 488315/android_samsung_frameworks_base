...
public class RecyclerViewCaptureHelper implements ScrollCaptureViewHelper<ViewGroup> {
  ...
  public void onScrollRequested2(
    ...
    if (!recyclerView.isVisibleToUser() || recyclerView.getChildCount() == 0) {
      Log.m102w(TAG, "recyclerView is empty or not visible, cannot continue");
    ...
    if (anchor == null) {
      Log.m102w(TAG, "Failed to locate anchor view");
      resultConsumer.accept(result);
      return;
    }
    ...
    if (recyclerView.requestChildRectangleOnScreen(anchor, input, true)) {
      if (anchor.getParent() == null) {
        Log.m102w(TAG, "Bug: anchor view " + anchor + " is detached after scrolling");
        resultConsumer.accept(result);
        return;
      } else {
