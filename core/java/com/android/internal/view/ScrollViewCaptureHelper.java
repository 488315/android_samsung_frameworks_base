...
public class ScrollViewCaptureHelper implements ScrollCaptureViewHelper<ViewGroup> {
  ...
  public void onScrollRequested2(
    ...
    requestedContentBounds.offset(
        view.getScrollX() - contentView.getLeft(), view.getScrollY() - contentView.getTop());
    Rect input = new Rect(requestedContentBounds);
    int remainingHeight =
        ((view.getHeight() - view.getPaddingTop()) - view.getPaddingBottom()) - input.height();
    ...
    if (!view.getChildVisibleRect(contentView, available, offset)) {
      ...
      available.offset(-offset.f85x, -offset.f86y);
      available.offset(
          contentView.getLeft() - view.getScrollX(), contentView.getTop() - view.getScrollY());
