...
public final class InlineTooltipUi extends PopupWindow implements AutoCloseable {
        ...
        float scale =
            WindowManager.fixScale(
                Settings.Global.getFloat(
                    anchor.getContext().getContentResolver(), "animator_duration_scale"));
  ...
  protected boolean findDropDownPosition(
    ...
    if (parent instanceof View) {
      ...
      if (isAbove) {
        outParams.f574y = r.top - getPreferHeight(anchor);
      } else {
        outParams.f574y = r.bottom + 1;
      }
    }
    return isAbove;
  }
  ...
  private void show(WindowManager.LayoutParams params) {
    ...
    try {
      ...
      Slog.m113d(TAG, "Failed with token " + params.token + " gone.");
  ...
  private void hide() {
    try {
      ...
      Slog.m116e(TAG, "Exception hiding window ", e);
    }
  }
