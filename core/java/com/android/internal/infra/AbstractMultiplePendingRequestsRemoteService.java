...
public abstract class AbstractMultiplePendingRequestsRemoteService<
  ...
  public AbstractMultiplePendingRequestsRemoteService(
      Context context,
      String serviceInterface,
      ComponentName componentName,
      int userId,
      AbstractRemoteService.VultureCallback<S> callback,
      Handler handler,
      int bindingFlags,
      boolean verbose,
      int initialCapacity) {
    super(
        context, serviceInterface, componentName, userId, callback, handler, bindingFlags, verbose);
  ...
  void handlePendingRequests() {
    synchronized (this.mPendingRequests) {
      ...
      if (this.mVerbose) {
        Slog.m119v(this.mTag, "Sending " + size + " pending requests");
      }
  ...
  protected void handleOnDestroy() {
    synchronized (this.mPendingRequests) {
      int size = this.mPendingRequests.size();
      if (this.mVerbose) {
        Slog.m119v(this.mTag, "Canceling " + size + " pending requests");
      }
      for (int i = 0; i < size; i++) {
  ...
  final void handleBindFailure() {
    synchronized (this.mPendingRequests) {
      int size = this.mPendingRequests.size();
      if (this.mVerbose) {
        Slog.m119v(this.mTag, "Sending failure to " + size + " pending requests");
      }
      for (int i = 0; i < size; i++) {
  ...
  public void dump(String prefix, PrintWriter pw) {
    ...
    pw.append((CharSequence) prefix)
        ...
        .append((CharSequence) String.valueOf(this.mInitialCapacity))
        .println();
    synchronized (this.mPendingRequests) {
    ...
    pw.append((CharSequence) prefix)
        ...
        .append((CharSequence) String.valueOf(size))
        .println();
  }

  @Override // com.android.internal.infra.AbstractRemoteService
  void handlePendingRequestWhileUnBound(
      AbstractRemoteService.BasePendingRequest<S, I> pendingRequest) {
    synchronized (this.mPendingRequests) {
      this.mPendingRequests.add(pendingRequest);
      if (this.mVerbose) {
        Slog.m119v(
            this.mTag,
            "queued " + this.mPendingRequests.size() + " requests; last=" + pendingRequest);
      }
    }
  }
}
