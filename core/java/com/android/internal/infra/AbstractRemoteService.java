...
public abstract class AbstractRemoteService<S extends AbstractRemoteService<S, I>, I extends IInterface> implements IBinder.DeathRecipient {
    ...
    private void updateServicelicationExitInfo(ComponentName componentName, int userId) {
        ...
        if (this.mVerbose) {
            Slog.m119v(this.mTag, "updateServicelicationExitInfo: exitReason=" + ApplicationExitInfo.reasonCodeToString(this.mServiceExitReason) + " exitSubReason= " + ApplicationExitInfo.subreasonToString(this.mServiceExitSubReason));
        }
    }
    ...
    private boolean checkIfDestroyed() {
        if (this.mDestroyed && this.mVerbose) {
            Slog.m119v(this.mTag, "Not handling operation as service for " + this.mComponentName + " is already destroyed");
        }
        return this.mDestroyed;
    }
