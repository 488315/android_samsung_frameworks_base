...
public abstract class Nat {
  ...
  /* renamed from: M */
  private static final long f991M = 4294967295L;
  ...
  /* renamed from: eq */
  public static boolean m203eq(int len, int[] x, int[] y) {
    for (int i = len - 1; i >= 0; i--) {
      if (x[i] != y[i]) {
        return false;
      }
  ...
  public static int mulAddTo(int len, int[] x, int[] y, int[] zz) {
    ...
    for (int i = 0; i < len; i++) {
      long zc2 =
          zc + (mulWordAddTo(len, x[i], y, 0, zz, i) & 4294967295L) + (zz[i + len] & 4294967295L);
      zz[i + len] = (int) zc2;
      zc = zc2 >>> 32;
    }
    return (int) zc;
  ...
  public static void square(int len, int[] x, int xOff, int[] zz, int zzOff) {
    ...
    while (i < len) {
      long d2 =
          d + (squareWordAddTo(x, xOff, i, zz, zzOff) & 4294967295L) + (zz[zzPos] & 4294967295L);
      int zzPos2 = zzPos + 1;
      zz[zzPos] = (int) d2;
      long d3 = (d2 >>> 32) + (zz[zzPos2] & 4294967295L);
      zz[zzPos2] = (int) d3;
