...
public class PBES2AlgorithmParameters {
  ...
  private abstract static class BasePBEWithHmacAlgorithmParameters extends BaseAlgorithmParameters {
    ...
    protected byte[] engineGetEncoded() {
      try {
        return new DERSequence(new ASN1Encodable[] {PKCSObjectIdentifiers.id_PBES2, this.params})
            .getEncoded();
      } catch (IOException e) {
    ...
    protected AlgorithmParameterSpec localEngineGetParameterSpec(Class parameterSpec)
        throws InvalidParameterSpecException {
      if (parameterSpec == PBEParameterSpec.class) {
        PBKDF2Params pbeParamSpec =
            (PBKDF2Params) this.params.getKeyDerivationFunc().getParameters();
        byte[] iv =
            ((ASN1OctetString) this.params.getEncryptionScheme().getParameters()).getOctets();
        return PBES2AlgorithmParameters.createPBEParameterSpec(
            pbeParamSpec.getSalt(), pbeParamSpec.getIterationCount().intValue(), iv);
      }
        ...
        this.params =
            new PBES2Parameters(
                new KeyDerivationFunc(
                    ...
                    new PBKDF2Params(
                        pbeSpec.getSalt(), pbeSpec.getIterationCount(), this.keySize, this.kdf)),
                new EncryptionScheme(this.cipherAlgorithm, new DEROctetString(iv)));
        return;
      }
      ...
      for (int keySize : keySizes) {
        for (int shaVariant : shaVariants) {
          provider.addAlgorithm(
              ...
              PREFIX + "$PBEWithHmacSHA" + shaVariant + "AES" + keySize + "AlgorithmParameters");
        }
      }
    }
  }
  ...
  public static PBEParameterSpec createPBEParameterSpec(
    ...
    try {
      return (PBEParameterSpec)
          PBES2AlgorithmParameters.class
              .getClassLoader()
              .loadClass("javax.crypto.spec.PBEParameterSpec")
              .getConstructor(byte[].class, Integer.TYPE, AlgorithmParameterSpec.class)
              .newInstance(salt, Integer.valueOf(iterationCount), new IvParameterSpec(iv));
    } catch (Exception e) {
