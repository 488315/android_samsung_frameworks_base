...
abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
  ...
  public void checkValidity(Date date)
    ...
    if (date.getTime() > getNotAfter().getTime()) {
      throw new CertificateExpiredException(
          "certificate expired on " + this.f904c.getEndDate().getTime());
    }
    if (date.getTime() < getNotBefore().getTime()) {
      throw new CertificateNotYetValidException(
          "certificate not valid till " + this.f904c.getStartDate().getTime());
    }
  }
  ...
  public boolean hasUnsupportedCriticalExtension() {
    ...
    if (getVersion() == 3
        && (extensions = this.f904c.getTBSCertificate().getExtensions()) != null) {
      Enumeration e = extensions.oids();
      while (e.hasMoreElements()) {
        ASN1ObjectIdentifier oid = (ASN1ObjectIdentifier) e.nextElement();
  ...
  private void doVerify(PublicKey key, SignatureCreator signatureCreator)
    ...
    if ((key instanceof CompositePublicKey)
        && X509SignatureUtil.isCompositeAlgorithm(this.f904c.getSignatureAlgorithm())) {
      List<PublicKey> pubKeys = ((CompositePublicKey) key).getPublicKeys();
      ASN1Sequence keySeq =
          ASN1Sequence.getInstance(this.f904c.getSignatureAlgorithm().getParameters());
      ASN1Sequence sigSeq =
          ASN1Sequence.getInstance(DERBitString.getInstance(this.f904c.getSignature()).getBytes());
      ...
      for (int i3 = 0; i3 != keys.size(); i3++) {
        try {
          checkSignature(
              keys.get(i3),
              signature,
              this.f904c.getSignatureAlgorithm().getParameters(),
              getSignature());
          return;
    ...
    checkSignature(
        key, signature, this.f904c.getSignatureAlgorithm().getParameters(), getSignature());
  }
  ...
  private void checkSignature(
      ...
      throws CertificateException,
          ...
          InvalidKeyException {
    if (!isAlgIdEqual(
        this.f904c.getSignatureAlgorithm(), this.f904c.getTBSCertificate().getSignature())) {
