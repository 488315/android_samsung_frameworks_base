...
abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
    ...
    public X500Name getIssuerX500Name() {
        return this.f904c.getIssuer();
    }

    @Override // com.android.internal.org.bouncycastle.jcajce.interfaces.BCX509Certificate
    public TBSCertificate getTBSCertificateNative() {
        return this.f904c.getTBSCertificate();
    }

    @Override // com.android.internal.org.bouncycastle.jcajce.interfaces.BCX509Certificate
    public X500Name getSubjectX500Name() {
        return this.f904c.getSubject();
    }
    ...
    public void checkValidity(Date date) throws CertificateExpiredException, CertificateNotYetValidException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f904c.getEndDate().getTime());
        }
        if (date.getTime() < getNotBefore().getTime()) {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f904c.getStartDate().getTime());
        }
    }

    @Override // java.security.cert.X509Certificate
    public int getVersion() {
        return this.f904c.getVersionNumber();
    }

    @Override // java.security.cert.X509Certificate
    public BigInteger getSerialNumber() {
        return this.f904c.getSerialNumber().getValue();
    }

    @Override // java.security.cert.X509Certificate
    public Principal getIssuerDN() {
        return new X509Principal(this.f904c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getIssuerX500Principal() {
        try {
            byte[] encoding = this.f904c.getIssuer().getEncoded(ASN1Encoding.DER);
    ...
    public Principal getSubjectDN() {
        return new X509Principal(this.f904c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getSubjectX500Principal() {
        try {
            byte[] encoding = this.f904c.getSubject().getEncoded(ASN1Encoding.DER);
            return new X500Principal(encoding);
        } catch (IOException e) {
    ...
    public Date getNotBefore() {
        return this.f904c.getStartDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotAfter() {
        return this.f904c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f904c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
    ...
    public byte[] getSignature() {
        return this.f904c.getSignature().getOctets();
    }
    ...
    public String getSigAlgOID() {
        return this.f904c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public boolean[] getIssuerUniqueID() {
        DERBitString id = this.f904c.getTBSCertificate().getIssuerUniqueId();
        if (id != null) {
    ...
    public boolean[] getSubjectUniqueID() {
        DERBitString id = this.f904c.getTBSCertificate().getSubjectUniqueId();
        if (id != null) {
            byte[] bytes = id.getBytes();
            boolean[] boolId = new boolean[(bytes.length * 8) - id.getPadBits()];
            for (int i = 0; i != boolId.length; i++) {
    ...
    public List getExtendedKeyUsage() throws CertificateParsingException {
        byte[] extOctets = getExtensionOctets(this.f904c, "2.5.29.37");
        if (extOctets == null) {
            return null;
        }
        try {
    ...
    public Collection getSubjectAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f904c, Extension.subjectAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Collection getIssuerAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f904c, Extension.issuerAlternativeName.getId());
    }
    ...
    public Set getCriticalExtensionOIDs() {
        if (getVersion() == 3) {
            Set set = new HashSet();
            Extensions extensions = this.f904c.getTBSCertificate().getExtensions();
            if (extensions != null) {
    ...
    public byte[] getExtensionValue(String oid) {
        ASN1OctetString extValue = getExtensionValue(this.f904c, oid);
        if (extValue != null) {
            try {
    ...
    public boolean hasUnsupportedCriticalExtension() {
        ...
        if (getVersion() == 3 && (extensions = this.f904c.getTBSCertificate().getExtensions()) != null) {
            Enumeration e = extensions.oids();
            while (e.hasMoreElements()) {
                ASN1ObjectIdentifier oid = (ASN1ObjectIdentifier) e.nextElement();
    ...
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f904c.getSubjectPublicKeyInfo());
        } catch (IOException e) {
            return null;
        }
    }
    ...
    public byte[] getEncoded() throws CertificateEncodingException {
        try {
            return this.f904c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    private void doVerify(PublicKey key, SignatureCreator signatureCreator) throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, SignatureException, NoSuchProviderException {
        if ((key instanceof CompositePublicKey) && X509SignatureUtil.isCompositeAlgorithm(this.f904c.getSignatureAlgorithm())) {
            List<PublicKey> pubKeys = ((CompositePublicKey) key).getPublicKeys();
            ASN1Sequence keySeq = ASN1Sequence.getInstance(this.f904c.getSignatureAlgorithm().getParameters());
            ASN1Sequence sigSeq = ASN1Sequence.getInstance(DERBitString.getInstance(this.f904c.getSignature()).getBytes());
        ...
        if (X509SignatureUtil.isCompositeAlgorithm(this.f904c.getSignatureAlgorithm())) {
            ASN1Sequence keySeq2 = ASN1Sequence.getInstance(this.f904c.getSignatureAlgorithm().getParameters());
            ASN1Sequence sigSeq2 = ASN1Sequence.getInstance(DERBitString.getInstance(this.f904c.getSignature()).getBytes());
        ...
        String sigName3 = X509SignatureUtil.getSignatureName(this.f904c.getSignatureAlgorithm());
        ...
        if (key instanceof CompositePublicKey) {
            ...
            for (int i3 = 0; i3 != keys.size(); i3++) {
                try {
                    checkSignature(keys.get(i3), signature, this.f904c.getSignatureAlgorithm().getParameters(), getSignature());
                    return;
        ...
        checkSignature(key, signature, this.f904c.getSignatureAlgorithm().getParameters(), getSignature());
    }
    ...
    private void checkSignature(PublicKey key, Signature signature, ASN1Encodable params, byte[] sigBytes) throws CertificateException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        if (!isAlgIdEqual(this.f904c.getSignatureAlgorithm(), this.f904c.getTBSCertificate().getSignature())) {
        ...
        try {
            ...
            this.f904c.getTBSCertificate().encodeTo(sigOut, ASN1Encoding.DER);
            sigOut.close();
