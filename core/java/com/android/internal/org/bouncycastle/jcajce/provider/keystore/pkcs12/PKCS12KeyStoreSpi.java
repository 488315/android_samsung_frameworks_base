...
public class PKCS12KeyStoreSpi extends KeyStoreSpi implements PKCSObjectIdentifiers, X509ObjectIdentifiers, BCKeyStore {
    ...
    private class CertId {
        ...
        CertId(PublicKey key) {
            this.f905id = PKCS12KeyStoreSpi.this.createSubjectKeyId(key).getKeyIdentifier();
        }
        ...
        public int hashCode() {
            return Arrays.hashCode(this.f905id);
        }
        ...
        public boolean equals(Object o) {
            ...
            CertId cId = (CertId) o;
            return Arrays.areEqual(this.f905id, cId.f905id);
        }
    }
    ...
    protected PrivateKey unwrapKey(AlgorithmIdentifier algId, byte[] data, char[] password, boolean wrongPKCS12Zero) throws IOException {
        ...
        try {
            if (algorithm.m176on(PKCSObjectIdentifiers.pkcs_12PbeIds)) {
    ...
    private void doStore(OutputStream stream, char[] password, boolean useDEREncoding) throws IOException {
        ...
        while (cs.hasMoreElements()) {
            try {
                ...
                ?? engineGetCertificate = engineGetCertificate(name2);
                Enumeration cs2 = cs;
                try {
                    boolean cAttrSet = false;
                    ASN1EncodableVector keyS2 = keyS;
                    try {
                        CertBag cBag = new CertBag(x509Certificate, new DEROctetString(engineGetCertificate.getEncoded()));
                        ASN1EncodableVector fName = new ASN1EncodableVector();
                        if (engineGetCertificate instanceof PKCS12BagAttributeCarrier) {
                            PKCS12BagAttributeCarrier bagAttrs2 = (PKCS12BagAttributeCarrier) engineGetCertificate;
                                    ...
                                    try {
                                        if (nm2.getString().equals(name2)) {
                                            if (bagAttrs2.getBagAttribute(pkcs_9_at_localKeyId) == null) {
                                                bagAttrs2.setBagAttribute(pkcs_9_at_localKeyId, createSubjectKeyId(engineGetCertificate.getPublicKey()));
                                            }
                        ...
                        if (!cAttrSet) {
                            ...
                            fSeq2.add(new DERSet(createSubjectKeyId(engineGetCertificate.getPublicKey())));
        ...
        while (cs3.hasMoreElements()) {
            try {
                ...
                if (this.keys.get(certId) == null) {
                    SafeBag sBag2 = createSafeBag(certId, cert);
                    certSeq.add(sBag2);
                    hashtable.put(cert, cert);
                }
        ...
        while (cs4.hasMoreElements()) {
            try {
                ...
                ?? r1 = (Certificate) this.chainCerts.get(certId2);
                if (usedCertificateSet.contains(r1) && hashtable.get(r1) == null) {
                    CertBag cBag2 = new CertBag(x509Certificate, new DEROctetString(r1.getEncoded()));
                    ASN1EncodableVector fName2 = new ASN1EncodableVector();
                    if (r1 instanceof PKCS12BagAttributeCarrier) {
                        PKCS12BagAttributeCarrier bagAttrs3 = (PKCS12BagAttributeCarrier) r1;
