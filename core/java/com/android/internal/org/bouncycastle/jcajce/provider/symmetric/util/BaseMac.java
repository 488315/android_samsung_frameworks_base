...
public class BaseMac extends MacSpi implements PBE {
    ...
    protected void engineInit(Key key, AlgorithmParameterSpec params)
                    ...
                    if ((k instanceof PBEKey) && pbeSpec == null) {
                        pbeSpec =
                                new PBEParameterSpec(
                                        ((PBEKey) k).getSalt(), ((PBEKey) k).getIterationCount());
                    }
                    int digest = 1;
                    int keySize = 160;
                    if ((this.macEngine instanceof HMac)
                            && !this.macEngine.getAlgorithmName().startsWith("SHA-1")) {
                        if (this.macEngine
                                .getAlgorithmName()
                                .startsWith(KeyProperties.DIGEST_SHA224)) {
                            digest = 7;
                            keySize = 224;
                        } else if (this.macEngine.getAlgorithmName().startsWith("SHA-256")) {
                            digest = 4;
                            keySize = 256;
                        } else if (this.macEngine
                                .getAlgorithmName()
                                .startsWith(KeyProperties.DIGEST_SHA384)) {
                            digest = 8;
                            keySize = 384;
                        } else if (this.macEngine
                                .getAlgorithmName()
                                .startsWith(KeyProperties.DIGEST_SHA512)) {
                            digest = 9;
                            keySize = 512;
                        } else {
                            throw new InvalidAlgorithmParameterException(
                                    "no PKCS12 mapping for HMAC: "
                                            + this.macEngine.getAlgorithmName());
                        }
                    }
                    param = PBE.Util.makePBEMacParameters(k, 2, digest, keySize, pbeSpec);
                } catch (Exception e) {
                    throw new InvalidAlgorithmParameterException(
                            "PKCS12 requires a PBEParameterSpec");
                }
            } catch (Exception e2) {
        ...
        try {
            ...
            throw new InvalidAlgorithmParameterException(
                    "cannot initialize MAC: " + e3.getMessage());
        }
    }
