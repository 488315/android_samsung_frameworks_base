...
public class CMSSignedDataGenerator extends CMSSignedGenerator {
  ...
  public CMSSignedData generate(CMSTypedData content, boolean encapsulate) throws CMSException {
    ...
    if (content.getContent() != null) {
      ...
      OutputStream cOut =
          CMSUtils.getSafeOutputStream(CMSUtils.attachSignersToOutputStream(this.signerGens, bOut));
      try {
        content.write(cOut);
        cOut.close();
        if (encapsulate) {
    ...
    SignedData sd =
        new SignedData(
            new DERSet(digestAlgs), encInfo, certificates, certrevlist, new DERSet(signerInfos));
