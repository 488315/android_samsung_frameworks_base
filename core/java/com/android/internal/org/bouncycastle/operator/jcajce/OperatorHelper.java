...
class OperatorHelper {
      ...
      if (digAlgId.getAlgorithm().equals((ASN1Primitive) NISTObjectIdentifiers.id_shake256_len)) {
        dig =
            this.helper.createMessageDigest(
                "SHAKE256-" + ASN1Integer.getInstance(digAlgId.getParameters()).getValue());
      } else {
        dig =
            this.helper.createMessageDigest(
      ...
      if (notDefaultPSSParams(seq)) {
        try {
          AlgorithmParameters algParams =
              this.helper.createAlgorithmParameters(KeyProperties.SIGNATURE_PADDING_RSA_PSS);
  ...
  private static String getSignatureName(AlgorithmIdentifier sigAlgId) {
    ...
    if (params != null
        ...
        && sigAlgId.getAlgorithm().equals((ASN1Primitive) PKCSObjectIdentifiers.id_RSASSA_PSS)) {
  ...
  public PublicKey convertPublicKey(SubjectPublicKeyInfo publicKeyInfo)
    ...
    try {
      KeyFactory keyFact =
          this.helper.createKeyFactory(publicKeyInfo.getAlgorithm().getAlgorithm().getId());
