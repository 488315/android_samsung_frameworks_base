...
public class JcaContentSignerBuilder {
  ...
  public JcaContentSignerBuilder(String signatureAlgorithm, AlgorithmParameterSpec sigParamSpec) {
    ...
    if (sigParamSpec instanceof PSSParameterSpec) {
      ...
      this.sigAlgId =
          new AlgorithmIdentifier(PKCSObjectIdentifiers.id_RSASSA_PSS, createPSSParams(pssSpec));
    } else {
  ...
  private ContentSigner buildComposite(CompositePrivateKey privateKey)
    ...
    try {
      ...
      for (int i = 0; i != sigAlgIds.size(); i++) {
        sigs[i] =
            this.helper.createSignature(AlgorithmIdentifier.getInstance(sigAlgIds.getObjectAt(i)));
        if (this.random != null) {
  ...
  private static RSASSAPSSparams createPSSParams(PSSParameterSpec pssSpec) {
    ...
    AlgorithmIdentifier mgfDig =
        digFinder.find(((MGF1ParameterSpec) pssSpec.getMGFParameters()).getDigestAlgorithm());
