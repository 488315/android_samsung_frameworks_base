...
public class JcaContentVerifierProviderBuilder {
    ...
    try {
      ...
      return new ContentVerifierProvider() { // from class:
        ...
        public ContentVerifier get(AlgorithmIdentifier algorithm) throws OperatorCreationException {
          ...
          try {
            Signature sig =
                JcaContentVerifierProviderBuilder.this.helper.createSignature(algorithm);
      ...
      public ContentVerifier get(AlgorithmIdentifier algorithm) throws OperatorCreationException {
        ...
        if (publicKey2 instanceof CompositePublicKey) {
          ...
          for (int i = 0; i != keys.size(); i++) {
            try {
              Signature sig =
                  JcaContentVerifierProviderBuilder.this.createSignature(algorithm, keys.get(i));
              Signature rawSig =
                  JcaContentVerifierProviderBuilder.this.createRawSig(algorithm, keys.get(i));
              if (rawSig != null) {
                return JcaContentVerifierProviderBuilder.this
                .new RawSigVerifier(algorithm, sig, rawSig);
              }
        ...
        Signature sig2 =
            JcaContentVerifierProviderBuilder.this.createSignature(algorithm, publicKey2);
        Signature rawSig2 =
  ...
  public ContentVerifier createCompositeVerifier(AlgorithmIdentifier compAlgId, PublicKey publicKey)
      throws OperatorCreationException {
    ...
    public boolean verify(byte[] digest, byte[] expected) {
      try {
        ...
        throw new RuntimeOperatorException(
            "exception obtaining raw signature: " + e2.getMessage(), e2);
      }
    }
  }
    ...
    public boolean verify(byte[] expected) {
      try {
        ...
        for (int i = 0; i != sigSeq.size(); i++) {
          ...
          if (signature != null
              && !signature.verify(DERBitString.getInstance(sigSeq.getObjectAt(i)).getBytes())) {
            failed = true;
          }
        }
        return !failed;
