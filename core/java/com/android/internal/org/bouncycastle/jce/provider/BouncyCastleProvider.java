...
public final class BouncyCastleProvider extends Provider implements ConfigurableProvider {
  ...
  private static final Class revChkClass =
      ClassUtil.loadClass(BouncyCastleProvider.class, "java.security.cert.PKIXRevocationChecker");
  ...
  private void loadAlgorithms(String packageName, String[] names) {
    for (int i = 0; i != names.length; i++) {
      Class clazz =
          ClassUtil.loadClass(BouncyCastleProvider.class, packageName + names[i] + "$Mappings");
      if (clazz != null) {
        try {
  ...
  public void setParameter(String parameterName, Object parameter) {
    ...
    synchronized (providerConfiguration) {
      ((BouncyCastleProviderConfiguration) providerConfiguration)
  ...
  public boolean hasAlgorithm(String type, String name) {
    return containsKey(
            new StringBuilder().append(type).append(MediaMetrics.SEPARATOR).append(name).toString())
        || containsKey(
            new StringBuilder()
                .append("Alg.Alias.")
                .append(type)
  ...
  public static PrivateKey getPrivateKey(PrivateKeyInfo privateKeyInfo) throws IOException {
    try {
      return KeyFactory.getInstance(privateKeyInfo.getPrivateKeyAlgorithm().getAlgorithm().getId())
