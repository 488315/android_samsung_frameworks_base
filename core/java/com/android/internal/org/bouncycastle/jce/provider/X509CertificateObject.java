...
public class X509CertificateObject extends X509Certificate implements PKCS12BagAttributeCarrier {
  ...
  public void checkValidity(Date date)
    ...
    if (date.getTime() > getNotAfter().getTime()) {
      throw new CertificateExpiredException(
          "certificate expired on " + this.f920c.getEndDate().getTime());
    }
    if (date.getTime() < getNotBefore().getTime()) {
      throw new CertificateNotYetValidException(
          "certificate not valid till " + this.f920c.getStartDate().getTime());
    }
  }
  ...
  public boolean hasUnsupportedCriticalExtension() {
    ...
    if (getVersion() == 3
        && (extensions = this.f920c.getTBSCertificate().getExtensions()) != null) {
      Enumeration e = extensions.oids();
      while (e.hasMoreElements()) {
        ASN1ObjectIdentifier oid = (ASN1ObjectIdentifier) e.nextElement();
  ...
  private void checkSignature(PublicKey key, Signature signature)
      throws CertificateException,
          ...
          InvalidKeyException {
    if (!isAlgIdEqual(
        this.f920c.getSignatureAlgorithm(), this.f920c.getTBSCertificate().getSignature())) {
    ...
    try {
      ...
      while (it.hasMoreElements()) {
        ...
        switch (genName.getTagNo()) {
          ...
          case 4:
            list.add(
                X509Name.getInstance(genName.getName()).toString(true, X509Name.DefaultSymbols));
            temp.add(Collections.unmodifiableList(list));
          case 7:
