...
public class X509CertificateObject extends X509Certificate implements PKCS12BagAttributeCarrier {
    ...
    public void checkValidity(Date date) throws CertificateExpiredException, CertificateNotYetValidException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f920c.getEndDate().getTime());
        }
        if (date.getTime() < getNotBefore().getTime()) {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f920c.getStartDate().getTime());
        }
    }

    @Override // java.security.cert.X509Certificate
    public int getVersion() {
        return this.f920c.getVersionNumber();
    }

    @Override // java.security.cert.X509Certificate
    public BigInteger getSerialNumber() {
        return this.f920c.getSerialNumber().getValue();
    }

    @Override // java.security.cert.X509Certificate
    public Principal getIssuerDN() {
        return new X509Principal(this.f920c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f920c.getIssuer().getEncoded());
        } catch (IOException e) {
    ...
    public Principal getSubjectDN() {
        return new X509Principal(this.f920c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f920c.getSubject().getEncoded());
        } catch (IOException e) {
            throw new IllegalStateException("can't encode issuer DN");
        }
    }
    ...
    public Date getNotBefore() {
        return this.f920c.getStartDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotAfter() {
        return this.f920c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f920c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
    ...
    public byte[] getSignature() {
        return this.f920c.getSignature().getOctets();
    }
    ...
    public String getSigAlgOID() {
        return this.f920c.getSignatureAlgorithm().getAlgorithm().getId();
    }

    @Override // java.security.cert.X509Certificate
    public byte[] getSigAlgParams() {
        if (this.f920c.getSignatureAlgorithm().getParameters() == null) {
            return null;
        }
        try {
            return this.f920c.getSignatureAlgorithm().getParameters().toASN1Primitive().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            return null;
        }
    }
    ...
    public boolean[] getIssuerUniqueID() {
        DERBitString id = this.f920c.getTBSCertificate().getIssuerUniqueId();
        if (id != null) {
    ...
    public boolean[] getSubjectUniqueID() {
        DERBitString id = this.f920c.getTBSCertificate().getSubjectUniqueId();
        if (id != null) {
            byte[] bytes = id.getBytes();
            boolean[] boolId = new boolean[(bytes.length * 8) - id.getPadBits()];
            for (int i = 0; i != boolId.length; i++) {
    ...
    public Set getCriticalExtensionOIDs() {
        if (getVersion() == 3) {
            Set set = new HashSet();
            Extensions extensions = this.f920c.getTBSCertificate().getExtensions();
            if (extensions != null) {
    ...
    private byte[] getExtensionBytes(String oid) {
        Extension ext;
        Extensions exts = this.f920c.getTBSCertificate().getExtensions();
    ...
    public boolean hasUnsupportedCriticalExtension() {
        ...
        if (getVersion() == 3 && (extensions = this.f920c.getTBSCertificate().getExtensions()) != null) {
            Enumeration e = extensions.oids();
            while (e.hasMoreElements()) {
                ASN1ObjectIdentifier oid = (ASN1ObjectIdentifier) e.nextElement();
    ...
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f920c.getSubjectPublicKeyInfo());
        } catch (IOException e) {
            return null;
        }
    }
    ...
    public byte[] getEncoded() throws CertificateEncodingException {
        try {
            if (this.encoded == null) {
                this.encoded = this.f920c.getEncoded(ASN1Encoding.DER);
            }
            return this.encoded;
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
    ...
    public final void verify(PublicKey key) throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, NoSuchProviderException, SignatureException {
        Signature signature;
        String sigName = X509SignatureUtil.getSignatureName(this.f920c.getSignatureAlgorithm());
        try {
    ...
    private void checkSignature(PublicKey key, Signature signature) throws CertificateException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        if (!isAlgIdEqual(this.f920c.getSignatureAlgorithm(), this.f920c.getTBSCertificate().getSignature())) {
            throw new CertificateException("signature algorithm in TBS cert not same as outer cert");
        }
        ASN1Encodable params = this.f920c.getSignatureAlgorithm().getParameters();
