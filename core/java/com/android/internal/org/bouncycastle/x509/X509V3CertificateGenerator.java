...
public class X509V3CertificateGenerator {
    ...
    public void setPublicKey(PublicKey key) throws IllegalArgumentException {
        try {
            this.tbsGen.setSubjectPublicKeyInfo(
                    SubjectPublicKeyInfo.getInstance(
                            new ASN1InputStream(key.getEncoded()).readObject()));
        } catch (Exception e) {
    ...
    public void copyAndAddExtension(
            ASN1ObjectIdentifier oid, boolean critical, X509Certificate cert)
            throws CertificateParsingException {
        ...
        try {
            byte[] signature =
                    X509Util.calculateSignature(
                            this.sigOID, this.signatureAlgorithm, key, random, tbsCert);
            try {
                return generateJcaObject(tbsCert, signature);
            } catch (Exception e) {
                throw new ExtCertificateEncodingException(
                        "exception producing certificate object", e);
            }
    ...
    public X509Certificate generate(PrivateKey key, String provider, SecureRandom random)
        ...
        try {
            byte[] signature =
                    X509Util.calculateSignature(
                            this.sigOID, this.signatureAlgorithm, provider, key, random, tbsCert);
            try {
                return generateJcaObject(tbsCert, signature);
            } catch (Exception e) {
                throw new ExtCertificateEncodingException(
    ...
    private X509Certificate generateJcaObject(TBSCertificate tbsCert, byte[] signature)
        ...
        return (X509Certificate)
                this.certificateFactory.engineGenerateCertificate(
