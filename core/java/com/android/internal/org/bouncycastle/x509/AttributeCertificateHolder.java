...
public class AttributeCertificateHolder implements CertSelector, Selector {
  ...
  public AttributeCertificateHolder(X509Certificate cert) throws CertificateParsingException {
    try {
      ...
      this.holder =
          new Holder(
              new IssuerSerial(
                  generateGeneralNames(name), new ASN1Integer(cert.getSerialNumber())));
    } catch (Exception e) {
  ...
  public boolean match(Certificate cert) {
    ...
    try {
      ...
      if (this.holder.getEntityName() != null
          && matchesDN(
              PrincipalUtil.getSubjectX509Principal(x509Cert), this.holder.getEntityName())) {
        return true;
      }
      if (this.holder.getObjectDigestInfo() == null) {
        return false;
      ...
      try {
        MessageDigest md =
            MessageDigest.getInstance(getDigestAlgorithm(), BouncyCastleProvider.PROVIDER_NAME);
