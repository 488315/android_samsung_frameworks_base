...
public class ASN1OutputStream {
  ...
  public static ASN1OutputStream create(OutputStream out, String encoding) {
    ...
    if (encoding.equals(ASN1Encoding.f685DL)) {
  ...
  final void write(byte[] bytes, int off, int len) throws IOException {
    this.f686os.write(bytes, off, len);
  }
  ...
  final void writeEncoded(
      boolean withTag, int tag, byte headByte, byte[] body, int bodyOff, int bodyLen, byte tailByte)
      throws IOException {
    if (withTag) {
      write(tag);
    }
  ...
  DEROutputStream getDERSubStream() {
    return new DEROutputStream(this.f686os);
  }

  ASN1OutputStream getDLSubStream() {
    return new DLOutputStream(this.f686os);
  }
}
