...
public class PKIXNameConstraintValidator implements NameConstraintValidator {
  ...
  public void checkPermitted(GeneralName name) throws NameConstraintValidatorException {
    switch (name.getTagNo()) {
      ...
      case 7:
        checkPermittedIP(
            this.permittedSubtreesIP, ASN1OctetString.getInstance(name.getName()).getOctets());
        break;
    }
  }
  ...
  public void checkExcluded(GeneralName name) throws NameConstraintValidatorException {
    switch (name.getTagNo()) {
      ...
      case 7:
        checkExcludedIP(
            this.excludedSubtreesIP, ASN1OctetString.getInstance(name.getName()).getOctets());
        break;
    }
  }
      ...
      switch (nameType) {
        case 0:
          this.permittedSubtreesOtherName =
              intersectOtherName(this.permittedSubtreesOtherName, (Set) entry.getValue());
          break;
        case 1:
    ...
    switch (base.getTagNo()) {
      case 0:
        this.excludedSubtreesOtherName =
            unionOtherName(this.excludedSubtreesOtherName, OtherName.getInstance(base.getName()));
        return;
      case 1:
      ...
      case 4:
        this.excludedSubtreesDN =
            unionDN(this.excludedSubtreesDN, (ASN1Sequence) base.getName().toASN1Primitive());
        return;
      case 6:
      ...
      case 7:
        this.excludedSubtreesIP =
            unionIP(
                this.excludedSubtreesIP, ASN1OctetString.getInstance(base.getName()).getOctets());
        return;
    }
  }
  ...
  public boolean equals(Object o) {
    ...
    return collectionsAreEqual(constraintValidator.excludedSubtreesDN, this.excludedSubtreesDN)
        ...
        && collectionsAreEqual(
            constraintValidator.excludedSubtreesOtherName, this.excludedSubtreesOtherName)
        ...
        && collectionsAreEqual(
            constraintValidator.permittedSubtreesOtherName, this.permittedSubtreesOtherName);
  }
  ...
  private static boolean withinDNSubtree(ASN1Sequence dns, ASN1Sequence subtree) {
    ...
    for (int j3 = 0; j3 < subtree.size(); j3++) {
      ...
      if (subtreeRdn.size() != dnsRdn.size()
          || !subtreeRdn.getFirst().getType().equals((ASN1Primitive) dnsRdn.getFirst().getType())) {
        return false;
      }
      if (subtreeRdn.size() == 1
          && subtreeRdn.getFirst().getType().equals((ASN1Primitive) RFC4519Style.serialNumber)) {
        if (!dnsRdn
            .getFirst()
            .getValue()
            .toString()
  ...
  private Set intersectIP(Set permitted, Set ips) {
    ...
    while (it.hasNext()) {
      byte[] ip =
          ASN1OctetString.getInstance(((GeneralSubtree) it.next()).getBase().getName()).getOctets();
      if (permitted == null) {
        if (ip != null) {
          intersect.add(ip);
        }
  ...
  private Set intersectIPRange(byte[] ipWithSubmask1, byte[] ipWithSubmask2) {
    ...
    if (compareTo(min, max) != 1) {
      byte[] ip = m177or(minMax[0], minMax[2]);
      byte[] subnetmask = m177or(subnetmask1, subnetmask2);
  ...
  /* renamed from: or */
  private static byte[] m177or(byte[] ip1, byte[] ip2) {
