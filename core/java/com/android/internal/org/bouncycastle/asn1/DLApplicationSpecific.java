...
public class DLApplicationSpecific extends ASN1ApplicationSpecific {
  ...
  public DLApplicationSpecific(boolean constructed, int tag, ASN1Encodable object)
      throws IOException {
    super(
  ...
  private static byte[] getEncoding(boolean explicit, ASN1Encodable object) throws IOException {
    byte[] data = object.toASN1Primitive().getEncoded(ASN1Encoding.f685DL);
    if (explicit) {
      return data;
    }
  ...
  private static byte[] getEncodedVector(ASN1EncodableVector vec) {
    ...
    for (int i = 0; i != vec.size(); i++) {
      try {
        bOut.write(((ASN1Object) vec.get(i)).getEncoded(ASN1Encoding.f685DL));
      } catch (IOException e) {
