...
public class DSAParametersGenerator {
  ...
  public void init(int size, int certainty, SecureRandom random) {
    this.f846L = size;
    this.f847N = getDefaultN(size);
    this.certainty = certainty;
    this.iterations = Math.max(getMinimumIterations(this.f846L), (certainty + 1) / 2);
    this.random = random;
    this.use186_3 = false;
    this.usageIndex = -1;
  }
    ...
    while (true) {
      ...
      if (isProbablePrime(q)) {
        ...
        while (counter < 4096) {
          ...
          if (p.bitLength() != this.f846L || !isProbablePrime(p)) {
            counter++;
            i2 = 0;
          } else {
  ...
  private DSAParameters generateParameters_FIPS186_3() {
    ...
    while (true) {
      ...
      BigInteger U = new BigInteger(1, output).mod(ONE.shiftLeft(this.f847N - 1));
      q = U.setBit(0).setBit(this.f847N - 1);
      if (isProbablePrime(q)) {
        byte[] offset = Arrays.clone(seed);
        int counterLimit = this.f846L * 4;
        counter = 0;
    ...
    if (i3 >= 0 && (g = calculateGenerator_FIPS186_3_Verifiable(d, p, q, seed, i3)) != null) {
      return new DSAParameters(
          p, q, g, new DSAValidationParameters(seed, counter, this.usageIndex));
    }
    return new DSAParameters(
        p,
        q,
