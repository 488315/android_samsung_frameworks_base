...
public class RFC3394WrapEngine implements Wrapper {
  ...
  public RFC3394WrapEngine(BlockCipher engine, boolean useReverseDirection) {
    this.f844iv = new byte[] {-90, -90, -90, -90, -90, -90, -90, -90};
    this.engine = engine;
  ...
  public void init(boolean forWrapping, CipherParameters param) {
    ...
    if (param instanceof KeyParameter) {
      ...
      this.f844iv = ((ParametersWithIV) param).getIV();
      this.param = (KeyParameter) ((ParametersWithIV) param).getParameters();
      if (this.f844iv.length != 8) {
  ...
  public byte[] wrap(byte[] in, int inOff, int inLen) {
    ...
    byte[] bArr = this.f844iv;
    byte[] block = new byte[bArr.length + inLen];
    byte[] buf = new byte[bArr.length + 8];
    System.arraycopy(bArr, 0, block, 0, bArr.length);
    System.arraycopy(in, inOff, block, this.f844iv.length, inLen);
    this.engine.init(this.wrapCipherMode, this.param);
    for (int j = 0; j != 6; j++) {
      for (int i = 1; i <= n; i++) {
        System.arraycopy(block, 0, buf, 0, this.f844iv.length);
        System.arraycopy(block, i * 8, buf, this.f844iv.length, 8);
        this.engine.processBlock(buf, 0, buf, 0);
        int t = (n * j) + i;
        int k = 1;
        while (t != 0) {
          byte v = (byte) t;
          int length = this.f844iv.length - k;
  ...
  public byte[] unwrap(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
    ...
    while (j >= 0) {
      int i3 = n2;
      while (i3 >= i2) {
        System.arraycopy(a, 0, buf, 0, this.f844iv.length);
        System.arraycopy(block, (i3 - 1) * i, buf, this.f844iv.length, i);
        int t = (n2 * j) + i3;
        int k = 1;
        while (t != 0) {
          byte v = (byte) t;
    ...
    if (!Arrays.constantTimeAreEqual(a, this.f844iv)) {
