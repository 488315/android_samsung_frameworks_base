...
public class OAEPEncoding implements AsymmetricBlockCipher {
  ...
  public byte[] encodeBlock(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
    ...
    byte[] mask2 =
        maskGeneratorFunction1(block, bArr2.length, block.length - bArr2.length, bArr2.length);
  ...
  public byte[] decodeBlock(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
    ...
    byte[] mask =
        maskGeneratorFunction1(block, bArr3.length, block.length - bArr3.length, bArr3.length);
    int i = 0;
    while (true) {
      bArr = this.defHash;
      if (i == bArr.length) {
  ...
  private byte[] maskGeneratorFunction1(byte[] Z, int zOff, int zLen, int length) {
    ...
    if (hashBuf.length * counter < length) {
      ...
      this.mgf1Hash.doFinal(hashBuf, 0);
      System.arraycopy(
          hashBuf, 0, mask, hashBuf.length * counter, mask.length - (hashBuf.length * counter));
    }
    return mask;
  }
}
