...
public class GCMBlockCipher implements AEADBlockCipher {
    ...
    public void init(boolean forEncryption, CipherParameters params) throws IllegalArgumentException {
        ...
        if (keyParam != null) {
            ...
            this.multiplier.init(this.f852H);
            this.exp = null;
        } else if (this.f852H == null) {
        ...
        if (bArr5.length == 12) {
            ...
            gHASHBlock(this.f853J0, X);
        }
        this.f854S = new byte[16];
        ...
        this.atLength = 0L;
        this.atLengthPre = 0L;
        this.counter = Arrays.clone(this.f853J0);
    ...
    private void initCipher() {
        ...
        if (this.atLengthPre > 0) {
            System.arraycopy(this.S_atPre, 0, this.f854S, 0, 16);
        }
    }
    ...
    public int doFinal(byte[] out, int outOff) throws IllegalStateException, InvalidCipherTextException {
        ...
        if (j2 > this.atLengthPre) {
            ...
            if (this.exp == null) {
                ...
                basicGCMExponentiator.init(this.f852H);
            }
            this.exp.exponentiateX(c, H_c);
            GCMUtil.multiply(this.S_at, H_c);
            GCMUtil.xor(this.f854S, this.S_at);
        }
        byte[] X = new byte[16];
        Pack.longToBigEndian(this.atLength * 8, X, 0);
        Pack.longToBigEndian(this.totalLength * 8, X, 8);
        gHASHBlock(this.f854S, X);
        byte[] tag = new byte[16];
        this.cipher.processBlock(this.f853J0, 0, tag, 0);
        GCMUtil.xor(tag, this.f854S);
        int resultLen = extra;
        int i3 = this.macSize;
    ...
    private void processBlock(byte[] buf, int bufOff, byte[] out, int outOff) {
        ...
        if (this.forEncryption) {
            ...
            gHASHBlock(this.f854S, ctrBlock);
            System.arraycopy(ctrBlock, 0, out, outOff, 16);
        } else {
            gHASHBlock(this.f854S, buf, bufOff);
    ...
    private void processPartial(byte[] buf, int off, int len, byte[] out, int outOff) {
        ...
        if (this.forEncryption) {
            ...
            gHASHPartial(this.f854S, buf, off, len);
        } else {
            gHASHPartial(this.f854S, buf, off, len);
            GCMUtil.xor(buf, off, ctrBlock, 0, len);
        }
