...
public final class AccessibilityUtils {
  ...
  public static Set<ComponentName> getEnabledServicesFromSettings(Context context, int userId) {
    String enabledServicesSetting =
        Settings.Secure.getStringForUser(
            context.getContentResolver(), Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES, userId);
    if (TextUtils.isEmpty(enabledServicesSetting)) {
      return Collections.emptySet();
    }
    Set<ComponentName> enabledServices = new HashSet<>();
    TextUtils.StringSplitter colonSplitter =
  ...
  public static void setAccessibilityServiceState(
      Context context, ComponentName componentName, boolean enabled, int userId) {
  ...
  public static int getAccessibilityServiceFragmentType(
      AccessibilityServiceInfo accessibilityServiceInfo) {
    int targetSdk =
        accessibilityServiceInfo.getResolveInfo().serviceInfo.applicationInfo.targetSdkVersion;
  ...
  public static boolean isUserSetupCompleted(Context context) {
    return Settings.Secure.getIntForUser(
            context.getContentResolver(), Settings.Secure.USER_SETUP_COMPLETE, 0, -2)
        != 0;
  }
  ...
  public static int textOrSpanChanged(CharSequence before, CharSequence after) {
    ...
    if (((before instanceof Spanned) || (after instanceof Spanned))
        && !parcelableSpansEquals(before, after)) {
      return 2;
    }
    return 0;
  }
  ...
  public static ComponentName getAccessibilityMenuComponentToMigrate(
    ...
    Optional<ComponentName> menuOutsideSystem =
        menuComponentNames.stream()
            .filter(
                new Predicate() { // from class:
                                  // com.android.internal.accessibility.util.AccessibilityUtils$$ExternalSyntheticLambda0
                  @Override // java.util.function.Predicate
                  public final boolean test(Object obj) {
                    return AccessibilityUtils.lambda$getAccessibilityMenuComponentToMigrate$0(
                        (ComponentName) obj);
                  }
                })
            .findFirst();
    boolean shouldMigrateToMenuInSystem =
        menuComponentNames.size() == 2
  ...
  static /* synthetic */ boolean lambda$getAccessibilityMenuComponentToMigrate$0(
      ComponentName name) {
  ...
  private static Set<ComponentName> findA11yMenuComponentNames(
    ...
    PackageManager.ResolveInfoFlags flags = PackageManager.ResolveInfoFlags.m12of(786944L);
  ...
  public static boolean isAccessControlEnabled(Context context) {
    return Settings.System.getIntForUser(
            context.getContentResolver(), Settings.System.SEM_ACCESS_CONTROL_ENABLED, 0, -2)
        != 0;
  }
  ...
  public static boolean makeToastForEmergencyMode(
      Context context, String targetName, String targetLabel) {
    if (!SemEmergencyManager.isEmergencyMode(context) || allowPerformInEmergencyMode(targetName)) {
      return false;
    }
    String message =
        context.getString(
            C4337R.string.accessibility_shortcut_cannot_use_emergency_mode, targetLabel);
  ...
  private static boolean allowPerformInEmergencyMode(String targetName) {
    return targetName.equals(
            "com.samsung.accessibility/com.samsung.accessibility.shortcut.AccessibilityHomepageActivityShortcut")
        || targetName.equals(
            "com.samsung.accessibility/com.samsung.accessibility.assistantmenu.serviceframework.AssistantMenuService")
        || targetName.equals("com.android.server.accessibility.MagnificationController")
        || targetName.equals(AccessibilityShortcutController.TALKBACK_SE);
  }

  public static boolean makeToastForDexMode(
      Context context, String targetName, String targetLabel) {
    if ((!isDexMode(context) || !disallowPerformInDexMode(targetName))
        && (!isDexDualMonitorDisplay(context)
            || !disallowPerformInDexDualMonitorDisplay(targetName))) {
      return false;
    }
    String message =
        context.getString(C4337R.string.accessibility_shortcut_cannot_use_dex_mode, targetLabel);
    Toast.makeText(new ContextThemeWrapper(context, 16974123), message, 0).show();
    return true;
  }
  ...
  private static boolean disallowPerformInDexDualMonitorDisplay(String targetName) {
    return targetName.equals(A11yLogger.COMPONENT_NAME_RELUMINO_SHORTCUT.flattenToString())
        || targetName.equals(A11yLogger.COMPONENT_NAME_COLOR_LENS_SHORTCUT.flattenToString())
        || targetName.equals(
            AccessibilityShortcutController.DALTONIZER_COMPONENT_NAME.flattenToString());
  }

  public static boolean isFoldedLargeCoverScreen() {
    return A11yRune.A11Y_COMMON_BOOL_SUPPORT_LARGE_COVER_SCREEN_FLIP
  ...
  public static boolean makeToastForCoverScreen(Context context, String targetName) {
    if (!isFoldedLargeCoverScreen()
        || (targetName != null && !disallowPerformInCoverScreen(targetName))) {
      return false;
    }
    String message =
        context.getString(C4337R.string.accessibility_shortcut_open_phone_and_try_again);
    Toast.makeText(new ContextThemeWrapper(context, 16974123), message, 0).show();
    return true;
  }
  ...
  public static boolean disallowPerformInCoverScreen(String targetName) {
    return targetName.equals(
            A11yLogger.COMPONENT_NAME_ACCESSIBILITY_HOMEPAGE_SHORTCUT.flattenToString())
        || targetName.equals(A11yLogger.COMPONENT_NAME_UNIVERSAL_SWITCH.flattenToString())
        || targetName.equals(A11yLogger.COMPONENT_NAME_MAGNIFIER_CAMERA_SHORTCUT.flattenToString())
        || targetName.equals(
            A11yLogger.COMPONENT_NAME_INTERACTION_CONTROL_SHORTCUT.flattenToString())
        || targetName.equals(A11yLogger.COMPONENT_NAME_VOICE_ACCESS.flattenToString())
        || targetName.equals(A11yLogger.COMPONENT_NAME_LIVE_TRANSCRIBE.flattenToString())
        || targetName.equals(
            A11yLogger.COMPONENT_NAME_GOOGLE_LIVE_TRANSCRIBE_SHORTCUT.flattenToString())
        || targetName.equals(
            A11yLogger.COMPONENT_NAME_GOOGLE_SOUND_NOTIFICATION_SHORTCUT.flattenToString());
  }

  public static boolean makeToastForFingerprint(
      Context context, String targetName, String targetLabel) {
    ...
    String message =
        context.getString(C4337R.string.accessibility_shortcut_cannot_use_fingerprint, targetLabel);
    Toast.makeText(new ContextThemeWrapper(context, 16974123), message, 0).show();
    return true;
  }

  public static boolean disallowPerformWhileFingerPrint(String targetName) {
    return targetName.equals(A11yLogger.COMPONENT_NAME_COLOR_LENS_SHORTCUT.flattenToString())
        || targetName.equals(A11yLogger.COMPONENT_NAME_COLOR_ADJUSTMENT_SHORTCUT.flattenToString())
        || targetName.equals(
            AccessibilityShortcutController.DALTONIZER_COMPONENT_NAME.flattenToString())
        || targetName.equals(
  ...
  public static boolean needToShowToast(Context context, String targetName, String targetLabel) {
    return makeToastForEmergencyMode(context, targetName, targetLabel)
        || makeToastForDexMode(context, targetName, targetLabel)
        || makeToastForFingerprint(context, targetName, targetLabel)
        || makeToastForCoverScreen(context, targetName);
  }
  ...
  public static void updateProfile(Context context, String targetName) {
    context.sendBroadcastAsUser(
        new Intent("com.samsung.accessibility.action.UPDATE_PROFILE")
            .setClassName(
                "com.android.settings",
                "com.samsung.android.settings.accessibility.recommend.RecommendedForYouReceiver")
            .putExtra("component", targetName),
        UserHandle.CURRENT);
  }
