...
public class AccessibilityDirectAccessController {
    ...
    public static Map<ComponentName, ToggleableFrameworkFeatureInfo> getFrameworkShortcutFeaturesMap() {
        if (sFrameworkShortcutFeaturesMap == null) {
            ...
            featuresMap.put(AccessibilityShortcutController.COLOR_INVERSION_COMPONENT_NAME, new ToggleableFrameworkFeatureInfo(Settings.Secure.ACCESSIBILITY_DISPLAY_INVERSION_ENABLED, "1", "0", C4337R.string.color_inversion_feature_name));
            featuresMap.put(AccessibilityShortcutController.DALTONIZER_COMPONENT_NAME, new ToggleableFrameworkFeatureInfo(Settings.Secure.ACCESSIBILITY_DISPLAY_DALTONIZER_ENABLED, "1", "0", C4337R.string.color_correction_feature_name));
    ...
    private String getShortcutWarningTitle(List<AccessibilityTarget> targets) {
        if (targets.size() == 1 && AccessibilityShortcutController.TALKBACK_SE.equals(targets.get(0).getId())) {
            ...
            return this.mContext.getString(C4337R.string.accessibility_direct_access_warning_title_samsung_talkback, targets.get(0).getLabel());
        }
        if (AccessibilityUtils.isSideKeySupported()) {
            return this.mContext.getString(C4337R.string.accessibility_direct_access_warning_title_samsung_side_key);
        }
    ...
    private String getShortcutWarningMessage(List<AccessibilityTarget> targets) {
        if (targets.size() == 1) {
            if (AccessibilityUtils.isSideKeySupported()) {
                return this.mContext.getString(C4337R.string.f644xe5c5f2ca, targets.get(0).getLabel()) + " " + this.mContext.getString(C4337R.string.accessibility_select_different_function);
            }
            return this.mContext.getString(C4337R.string.accessibility_direct_access_single_service_warning_samsung, targets.get(0).getLabel()) + " " + this.mContext.getString(C4337R.string.accessibility_select_different_function);
        }
        ...
        return this.mContext.getString(C4337R.string.accessibility_direct_access_multiple_service_warning_samsung) + "\n" + sb.toString() + "\n" + this.mContext.getString(C4337R.string.accessibility_select_different_function);
    }
    ...
    class TtsPrompt implements TextToSpeech.OnInitListener {
        ...
        TtsPrompt(String serviceName) {
            if (AccessibilityUtils.isSideKeySupported()) {
                this.mText = AccessibilityDirectAccessController.this.mContext.getString(C4337R.string.accessibility_direct_access_spoken_feedback_side_key, serviceName);
            } else {
                this.mText = AccessibilityDirectAccessController.this.mContext.getString(C4337R.string.accessibility_direct_access_spoken_feedback, serviceName);
            }
        ...
        public void dismiss() {
            this.mDismiss = true;
            AccessibilityDirectAccessController.this.mHandler.sendMessage(PooledLambda.obtainMessage(new C4340x3fc729fb(), this.mTts));
        }

        @Override // android.speech.tts.TextToSpeech.OnInitListener
        public void onInit(int status) {
            if (status != 0) {
                Slog.m113d(AccessibilityDirectAccessController.TAG, "Tts init fail, status=" + Integer.toString(status));
                AccessibilityDirectAccessController.this.playNotificationTone();
            } else {
                AccessibilityDirectAccessController.this.mHandler.sendMessage(PooledLambda.obtainMessage(new C4338x3fc729f9(), this));
            }
        }
        ...
        public void waitForTtsReady() {
            ...
            if (i == 0) {
                ...
                AccessibilityDirectAccessController.this.mHandler.sendMessageDelayed(PooledLambda.obtainMessage(new C4338x3fc729f9(), this), 1000L);
            }
        }
    }
