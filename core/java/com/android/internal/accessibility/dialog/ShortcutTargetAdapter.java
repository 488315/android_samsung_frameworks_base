    ...
    if (convertView == null) {
      convertView2 =
          LayoutInflater.from(context)
              .inflate(C4337R.layout.accessibility_shortcut_chooser_item_samsung, parent, false);
      holder = new TargetAdapter.ViewHolder();
      holder.mIconView =
          (ImageView) convertView2.findViewById(C4337R.id.accessibility_shortcut_target_icon);
      holder.mLabelView =
          (TextView) convertView2.findViewById(C4337R.id.accessibility_shortcut_target_label);
      holder.mStatusView =
          (TextView) convertView2.findViewById(C4337R.id.accessibility_shortcut_target_status);
      ...
      if (AccessibilityUtils.disallowPerformInCoverScreen(target.getId())) {
        convertView2.setContentDescription(
            ((Object) target.getLabel())
                + " "
                + context.getString(
                    C4337R.string.accessibility_shortcut_menu_item_status_disabled));
        ...
        if (target.getStateDescription() == null) {
          ...
          if (i2 == 3) {
            targetSummary =
                context.getString(C4337R.string.accessibility_shortcut_menu_item_status_on);
          } else if (i2 == 4) {
            targetSummary =
                context.getString(C4337R.string.accessibility_shortcut_menu_item_status_off);
          } else if (i2 == 5) {
            targetSummary =
                context.getString(C4337R.string.accessibility_shortcut_menu_item_status_disabled);
          }
        } else {
