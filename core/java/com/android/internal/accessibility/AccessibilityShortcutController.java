...
public class AccessibilityShortcutController {
    ...
    public static Map<ComponentName, FrameworkFeatureInfo> getFrameworkShortcutFeaturesMap() {
        if (sFrameworkShortcutFeaturesMap == null) {
            ...
            featuresMap.put(COLOR_INVERSION_COMPONENT_NAME, new ToggleableFrameworkFeatureInfo(Settings.Secure.ACCESSIBILITY_DISPLAY_INVERSION_ENABLED, "1", "0", C4337R.string.color_inversion_feature_name));
            featuresMap.put(DALTONIZER_COMPONENT_NAME, new ToggleableFrameworkFeatureInfo(Settings.Secure.ACCESSIBILITY_DISPLAY_DALTONIZER_ENABLED, "1", "0", C4337R.string.color_correction_feature_name));
            if (RoSystemProperties.SUPPORT_ONE_HANDED_MODE) {
                featuresMap.put(ONE_HANDED_COMPONENT_NAME, new ToggleableFrameworkFeatureInfo(Settings.Secure.ONE_HANDED_MODE_ACTIVATED, "1", "0", C4337R.string.one_handed_mode_feature_name));
            }
            featuresMap.put(REDUCE_BRIGHT_COLORS_COMPONENT_NAME, new ToggleableFrameworkFeatureInfo(Settings.Secure.REDUCE_BRIGHT_COLORS_ACTIVATED, "1", "0", C4337R.string.reduce_bright_colors_feature_name));
            featuresMap.put(ACCESSIBILITY_HEARING_AIDS_COMPONENT_NAME, new LaunchableFrameworkFeatureInfo(C4337R.string.hearing_aids_feature_name));
    ...
    public void performAccessibilityShortcut() {
        ...
        if (vibrator != null && vibrator.hasVibrator()) {
            if (ShortcutUtils.isSupportDCMotorHapticFeedback(vibrator)) {
                ...
                long[] vibePattern = ArrayUtils.convertToLongArray(this.mContext.getResources().getIntArray(C4337R.array.config_longPressVibePattern));
    ...
    private AlertDialog createShortcutWarningDialog(final int userId) {
        ...
        AlertDialog alertDialog = frameworkObjectProvider.getAlertDialogBuilder(frameworkObjectProvider.getSystemUiContext()).setTitle(getShortcutWarningTitle(targets)).setMessage(getShortcutWarningMessage(targets)).setCancelable(false).setPositiveButton(C4337R.string.accessibility_shortcut_use, (DialogInterface.OnClickListener) null).setNegativeButton(C4337R.string.accessibility_shortcut_dont_use, new DialogInterface.OnClickListener() { // from class: com.android.internal.accessibility.AccessibilityShortcutController$$ExternalSyntheticLambda0
    ...
    private String getShortcutWarningTitle(List<AccessibilityTarget> targets) {
        if (targets.size() == 1 && TALKBACK_SE.equals(targets.get(0).getId())) {
            return this.mContext.getString(C4337R.string.accessibility_shortcut_warning_title_samsung_talkback, targets.get(0).getLabel());
        }
    ...
    private String getShortcutWarningMessage(List<AccessibilityTarget> targets) {
        if (targets.size() == 1) {
            return this.mContext.getString(C4337R.string.accessibility_shortcut_single_service_warning_samsung, targets.get(0).getLabel());
        }
        StringBuilder sb = new StringBuilder();
        for (AccessibilityTarget target : targets) {
            sb.append(this.mContext.getString(C4337R.string.accessibility_shortcut_multiple_service_list, target.getLabel()));
        }
        return this.mContext.getString(C4337R.string.accessibility_shortcut_multiple_service_warning_samsung) + "\n" + sb.toString() + "\n" + this.mContext.getString(C4337R.string.accessibility_select_different_function);
    }
    ...
    public void playNotificationTone() {
        ...
        Uri ringtoneUri = Uri.parse("file://" + this.mContext.getString(C4337R.string.config_defaultAccessibilityNotificationSound));
    ...
    class TtsPrompt implements TextToSpeech.OnInitListener {
        ...
        TtsPrompt(String serviceName) {
            this.mText = AccessibilityShortcutController.this.mContext.getString(C4337R.string.accessibility_shortcut_spoken_feedback, serviceName);
        ...
        public void dismiss() {
            this.mDismiss = true;
            AccessibilityShortcutController.this.mHandler.sendMessage(PooledLambda.obtainMessage(new C4340x3fc729fb(), this.mTts));
        }

        @Override // android.speech.tts.TextToSpeech.OnInitListener
        public void onInit(int status) {
            if (status != 0) {
                Slog.m113d(AccessibilityShortcutController.TAG, "Tts init fail, status=" + Integer.toString(status));
                AccessibilityShortcutController.this.playNotificationTone();
            } else {
                AccessibilityShortcutController.this.mHandler.sendMessage(PooledLambda.obtainMessage(new C4341xe6764ed2(), this));
            }
        }
        ...
        public void waitForTtsReady() {
            ...
            if (i == 0) {
                ...
                AccessibilityShortcutController.this.mHandler.sendMessageDelayed(PooledLambda.obtainMessage(new C4341xe6764ed2(), this), 1000L);
            }
        }
    }
    ...
    private class UserSetupCompleteObserver extends ContentObserver {
        ...
        private void setEmptyShortcutTargetIfNeeded() {
            ...
            String defaultShortcutTarget = AccessibilityShortcutController.this.mContext.getString(C4337R.string.config_defaultAccessibilityService);
