...
public class InstallConstraintsTagParser {
    ...
    public static ParseResult<ParsingPackage> parseInstallConstraints(
        ...
        if (prefixes.isSuccess()) {
            ...
            return input.skip(
                    "Install of this package is restricted on this device; device fingerprint does"
                        + " not start with one of the allowed prefixes");
        }
    ...
    private static ParseResult<String> readFingerprintPrefixValue(
            ParseInput input, Resources res, XmlResourceParser parser) {
        TypedArray sa =
                res.obtainAttributes(
                        parser, R.styleable.AndroidManifestInstallConstraintsFingerprintPrefix);
        try {
