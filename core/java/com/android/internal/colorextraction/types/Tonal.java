...
public class Tonal implements ExtractionType {
  ...
  public void extractInto(
    ...
    boolean success =
        runTonalExtraction(inWallpaperColors, outColorsNormal, outColorsDark, outColorsExtraDark);
    if (!success) {
  ...
  private boolean runTonalExtraction(
    ...
    if (palette == null) {
      Log.m102w(TAG, "Could not find a tonal palette!");
      return false;
    }
    int fitIndex = bestFit(palette, hsl[0], hsl[1], hsl[2]);
    if (fitIndex == -1) {
      Log.m102w(TAG, "Could not find best fit!");
      return false;
    }
    float[] h =
        fit(palette.f663h, hsl[0], fitIndex, Float.NEGATIVE_INFINITY, Float.POSITIVE_INFINITY);
    float[] s = fit(palette.f665s, hsl[1], fitIndex, 0.0f, 1.0f);
    float[] l = fit(palette.f664l, hsl[2], fitIndex, 0.0f, 1.0f);
  ...
  public void applyFallback(
    ...
    int color;
    boolean light =
        (inWallpaperColors == null || (inWallpaperColors.getColorHints() & 1) == 0) ? false : true;
    boolean dark =
        (inWallpaperColors == null || (inWallpaperColors.getColorHints() & 2) == 0) ? false : true;
  ...
  private int[] getColorPalette(TonalPalette palette) {
    return getColorPalette(palette.f663h, palette.f665s, palette.f664l);
  }
  ...
  private static int bestFit(TonalPalette palette, float h, float s, float l) {
    ...
    for (int i = 0; i < palette.f663h.length; i++) {
      float error =
          (Math.abs(h - palette.f663h[i]) * 1.0f)
              + (Math.abs(s - palette.f665s[i]) * 1.0f)
              + (Math.abs(l - palette.f664l[i]) * FIT_WEIGHT_L);
      if (error < minError) {
        minError = error;
        minErrorIndex = i;
      }
  ...
  public static class TonalPalette {
    ...
    /* renamed from: h */
    public final float[] f663h;

    /* renamed from: l */
    public final float[] f664l;
    public final float maxHue;
    public final float minHue;

    /* renamed from: s */
    public final float[] f665s;
    ...
    public float[] getCenter() {
      return new float[] {
        this.mHue.getLower().floatValue()
            + ((this.mHue.getUpper().floatValue() - this.mHue.getLower().floatValue()) / 2.0f),
        this.mSaturation.getLower().floatValue()
            + ((this.mSaturation.getUpper().floatValue() - this.mSaturation.getLower().floatValue())
                / 2.0f),
        this.mLightness.getLower().floatValue()
            + ((this.mLightness.getUpper().floatValue() - this.mLightness.getLower().floatValue())
                / 2.0f)
      };
    }
  ...
  public static class ConfigParser {
    ...
    public ConfigParser(Context context) {
      try {
        XmlPullParser parser = context.getResources().getXml(C4337R.xml.color_extraction);
          ...
          if (eventType != 0 && eventType != 3) {
            if (eventType == 2) {
              ...
              throw new XmlPullParserException(
                  "Invalid XML event " + eventType + " - " + parser.getName(), parser, null);
            }
          }
        }
