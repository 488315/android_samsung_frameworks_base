...
public class SystemNotificationChannels {
  ...
  public static void createAll(Context context) {
    ...
    NotificationChannel physicalKeyboardChannel =
        new NotificationChannel(
            PHYSICAL_KEYBOARD,
            context.getString(C4337R.string.notification_channel_physical_keyboard),
            2);
    physicalKeyboardChannel.setBlockable(true);
    channelsList.add(physicalKeyboardChannel);
    NotificationChannel security =
        new NotificationChannel(
            SECURITY, context.getString(C4337R.string.notification_channel_security), 2);
    channelsList.add(security);
    NotificationChannel car =
        new NotificationChannel(
            CAR_MODE, context.getString(C4337R.string.notification_channel_car_mode), 2);
    car.setBlockable(true);
    channelsList.add(car);
    channelsList.add(newAccountChannel(context));
    NotificationChannel developer =
        new NotificationChannel(
            DEVELOPER, context.getString(C4337R.string.notification_channel_developer), 2);
    developer.setBlockable(true);
    channelsList.add(developer);
    NotificationChannel developerImportant =
        new NotificationChannel(
            DEVELOPER_IMPORTANT,
            context.getString(C4337R.string.notification_channel_developer_important),
            4);
    developer.setBlockable(true);
    channelsList.add(developerImportant);
    NotificationChannel updates =
        new NotificationChannel(
            UPDATES, context.getString(C4337R.string.notification_channel_updates), 2);
    channelsList.add(updates);
    NotificationChannel network =
        new NotificationChannel(
            NETWORK_STATUS,
            context.getString(C4337R.string.notification_channel_network_status),
            2);
    network.setBlockable(true);
    channelsList.add(network);
    NotificationChannel networkAlertsChannel =
        new NotificationChannel(
            NETWORK_ALERTS,
            context.getString(C4337R.string.notification_channel_network_alerts),
            4);
    networkAlertsChannel.setBlockable(true);
    channelsList.add(networkAlertsChannel);
    NotificationChannel networkAvailable =
        new NotificationChannel(
            NETWORK_AVAILABLE,
            context.getString(C4337R.string.notification_channel_network_available),
            2);
    networkAvailable.setBlockable(true);
    channelsList.add(networkAvailable);
    NotificationChannel vpn =
        new NotificationChannel(VPN, context.getString(C4337R.string.notification_channel_vpn), 2);
    channelsList.add(vpn);
    NotificationChannel deviceAdmin =
        new NotificationChannel(DEVICE_ADMIN, getDeviceAdminNotificationChannelName(context), 4);
    channelsList.add(deviceAdmin);
    NotificationChannel alertsChannel =
        new NotificationChannel(
            ALERTS, context.getString(C4337R.string.notification_channel_alerts), 3);
    channelsList.add(alertsChannel);
    NotificationChannel usb =
        new NotificationChannel(USB, context.getString(C4337R.string.notification_channel_usb), 1);
    channelsList.add(usb);
    NotificationChannel foregroundChannel =
        new NotificationChannel(
            FOREGROUND_SERVICE,
            context.getString(C4337R.string.notification_channel_foreground_service),
            2);
    foregroundChannel.setBlockable(true);
    channelsList.add(foregroundChannel);
    NotificationChannel heavyWeightChannel =
        new NotificationChannel(
            HEAVY_WEIGHT_APP,
            context.getString(C4337R.string.notification_channel_heavy_weight_app),
            3);
    heavyWeightChannel.setShowBadge(false);
    heavyWeightChannel.setSound(
        null, new AudioAttributes.Builder().setContentType(4).setUsage(10).build());
    channelsList.add(heavyWeightChannel);
    NotificationChannel systemChanges =
        new NotificationChannel(
            SYSTEM_CHANGES,
            context.getString(C4337R.string.notification_channel_system_changes),
            3);
    systemChanges.setSound(
        null, new AudioAttributes.Builder().setContentType(4).setUsage(5).build());
    channelsList.add(systemChanges);
    NotificationChannel dndChanges =
        new NotificationChannel(
            DO_NOT_DISTURB,
            context.getString(C4337R.string.notification_channel_do_not_disturb),
            2);
    channelsList.add(dndChanges);
    NotificationChannel newFeaturePrompt =
        new NotificationChannel(
            ACCESSIBILITY_MAGNIFICATION,
            context.getString(C4337R.string.notification_channel_accessibility_magnification),
            4);
    newFeaturePrompt.setBlockable(true);
    channelsList.add(newFeaturePrompt);
    NotificationChannel accessibilitySecurityPolicyChannel =
        new NotificationChannel(
            ACCESSIBILITY_SECURITY_POLICY,
            context.getString(C4337R.string.notification_channel_accessibility_security_policy),
            2);
    channelsList.add(accessibilitySecurityPolicyChannel);
    channelsList.add(
        new NotificationChannel(
            MDM_DEXPOLICY, context.getString(C4337R.string.notification_channel_dex_policy), 4));
    NotificationChannel abusiveBackgroundAppsChannel =
        new NotificationChannel(
            ABUSIVE_BACKGROUND_APPS,
            context.getString(C4337R.string.notification_channel_abusive_bg_apps),
            2);
    channelsList.add(abusiveBackgroundAppsChannel);
    channelsList.add(
        new NotificationChannel(
            ETHERNET,
            context.getString(C4337R.string.ethernet_cable_connected_notification_title),
            2));
  ...
  private static String getDeviceAdminNotificationChannelName(final Context context) {
    ...
    return dpm.getResources()
        .getString(
            DevicePolicyResources.Strings.Core.NOTIFICATION_CHANNEL_DEVICE_ADMIN,
            new Supplier() { // from class:
                             // com.android.internal.notification.SystemNotificationChannels$$ExternalSyntheticLambda0
  ...
  public static void createAccountChannelForPackage(String pkg, int uid, Context context) {
    ...
    try {
      iNotificationManager.createNotificationChannelsForPackage(
          pkg, uid, new ParceledListSlice(Arrays.asList(newAccountChannel(context))));
  ...
  private static NotificationChannel newAccountChannel(Context context) {
    return new NotificationChannel(
        ACCOUNT, context.getString(C4337R.string.notification_channel_account), 2);
  }
