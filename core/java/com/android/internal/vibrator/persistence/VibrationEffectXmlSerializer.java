...
public final class VibrationEffectXmlSerializer {
    public static XmlSerializedVibration<VibrationEffect> serialize(
        ...
        XmlValidator.checkSerializerCondition(
                vibration instanceof VibrationEffect.Composed,
                "Unsupported VibrationEffect type %s",
                vibration);
        VibrationEffect.Composed composed = (VibrationEffect.Composed) vibration;
        XmlValidator.checkSerializerCondition(
                !composed.getSegments().isEmpty(),
                "Unsupported empty VibrationEffect %s",
                vibration);
    ...
    private static SerializedVibrationEffect serializePredefinedEffect(
            VibrationEffect.Composed effect, int flags) throws XmlSerializerException {
        List<VibrationEffectSegment> segments = effect.getSegments();
        XmlValidator.checkSerializerCondition(
                effect.getRepeatIndex() == -1,
                "Unsupported repeating predefined effect %s",
                effect);
        XmlValidator.checkSerializerCondition(
                segments.size() == 1,
                "Unsupported multiple segments in predefined effect %s",
                effect);
    ...
    private static SerializedVibrationEffect serializePrimitiveEffect(
        ...
        XmlValidator.checkSerializerCondition(
                effect.getRepeatIndex() == -1,
                "Unsupported repeating primitive composition %s",
                effect);
        SerializedVibrationEffect.SerializedSegment[] primitives =
                new SerializedVibrationEffect.SerializedSegment[segments.size()];
    ...
    private static SerializedVibrationEffect serializeWaveformEffect(
        ...
        for (int i = 0; i < segments.size(); i++) {
            XmlValidator.checkSerializerCondition(
                    segments.get(i) instanceof StepSegment,
                    "Unsupported segment for waveform effect %s",
                    segments.get(i));
            ...
            XmlValidator.checkSerializerCondition(
                    Float.compare(segment.getFrequencyHz(), 0.0f) == 0,
                    "Unsupported segment with non-default frequency %f",
                    Float.valueOf(segment.getFrequencyHz()));
    ...
    private static SerializedPredefinedEffect serializePrebakedSegment(
            VibrationEffectSegment segment, int flags) throws XmlSerializerException {
        XmlValidator.checkSerializerCondition(
                segment instanceof PrebakedSegment,
                "Unsupported segment for predefined effect %s",
                segment);
        PrebakedSegment prebaked = (PrebakedSegment) segment;
        XmlConstants.PredefinedEffectName effectName =
                XmlConstants.PredefinedEffectName.findById(prebaked.getEffectId(), flags);
        XmlValidator.checkSerializerCondition(
                effectName != null,
                "Unsupported predefined effect id %s",
                Integer.valueOf(prebaked.getEffectId()));
        if ((flags & 1) == 0) {
            XmlValidator.checkSerializerCondition(
                    prebaked.shouldFallback(),
                    "Unsupported predefined effect with should fallback %s",
                    Boolean.valueOf(prebaked.shouldFallback()));
        }
    ...
    private static SerializedCompositionPrimitive serializePrimitiveSegment(
        ...
        XmlValidator.checkSerializerCondition(
                segment instanceof PrimitiveSegment,
                "Unsupported segment for primitive composition %s",
                segment);
        ...
        XmlValidator.checkSerializerCondition(
                primitiveName != null,
                "Unsupported primitive effect id %s",
                Integer.valueOf(primitive.getPrimitiveId()));
