...
class ZygoteServer {
    ...
    private void fetchUsapPoolPolicyProps() {
        if (this.mUsapPoolSupported) {
            ...
            if (this.mUsapPoolSizeMin >= this.mUsapPoolSizeMax) {
                Log.m102w(TAG, "The max size of the USAP pool must be greater than the minimum size.  Restoring default values.");
    ...
    Runnable fillUsapPool(int[] sessionSocketRawFDs, boolean isPriorityRefill) {
        ...
        if (isPriorityRefill) {
            ...
            Log.m98i(Zygote.PRIMARY_SOCKET_NAME, "Priority USAP Pool refill. New USAPs: " + numUsapsToSpawn);
        } else {
            numUsapsToSpawn = this.mUsapPoolSizeMax - usapPoolCount;
            Log.m98i(Zygote.PRIMARY_SOCKET_NAME, "Delayed USAP Pool refill. New USAPs: " + numUsapsToSpawn);
        }
        ZygoteHooks.preFork();
        do {
            numUsapsToSpawn--;
