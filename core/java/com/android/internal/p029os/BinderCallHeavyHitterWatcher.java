...
public final class BinderCallHeavyHitterWatcher {
  ...
  public void setConfig(
      boolean enable, int batchSize, float threshold, BinderCallHeavyHitterListener listener) {
    int inputSize;
          ...
          if (enable) {
            ...
            if (threshold >= 1.0E-5f && threshold <= 1.0f) {
              ...
              try {
                ...
                Log.m102w(
                    TAG,
                    "Invalid parameter to heavy hitter watcher: " + batchSize + ", " + capacity);
                return;
              }
            }
            return;
                ...
                if (this.mListener != null
                    && (result =
                            sketch.getTopHeavyHitters(
                                0, this.mCachedCandidateList, this.mCachedCandidateFrequencies))
                        != null
                    && (size = result.size()) > 0) {
                  ArrayList<HeavyHitterContainer> hitters = new ArrayList<>();
                  for (int i3 = 0; i3 < size; i3++) {
                    HeavyHitterContainer container2 =
                        this.mHeavyHitterCandiates.get(result.get(i3).intValue());
                    if (container2 != null) {
                  ...
                  this.mListener.onHeavyHit(
                      ...
                      this.mThreshold,
                      SystemClock.elapsedRealtime() - this.mBatchStartTimeStamp);
                }
