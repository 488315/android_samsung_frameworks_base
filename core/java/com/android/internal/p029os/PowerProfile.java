...
public class PowerProfile {
  ...
  public PowerProfile(Context context, boolean forTest) {
    synchronized (sLock) {
      int xmlId = forTest ? C4337R.xml.power_profile_test : C4337R.xml.power_profile;
  ...
  private void readPowerValuesFromXml(Context context, int xmlId) {
    ...
    try {
      try {
        ...
        parser.close();
        int[] configResIds = {
          C4337R.integer.config_bluetooth_idle_cur_ma,
          C4337R.integer.config_bluetooth_rx_cur_ma,
          C4337R.integer.config_bluetooth_tx_cur_ma,
          C4337R.integer.config_bluetooth_operating_voltage_mv
        };
        ...
        for (int i = 0; i < configResIds.length; i++) {
          ...
          if ((!hashMap.containsKey(key)
                  || hashMap.get(key).doubleValue() <= SContextConstants.ENVIRONMENT_VALUE_UNKNOWN)
    ...
    if (!anyBracketsSpecified || allBracketsSpecified) {
        ...
        for (int cluster2 = 0; cluster2 < this.mCpuClusters.length; cluster2++) {
          ...
          if (data.length != this.mCpuClusters[cluster2].powerBrackets.length) {
            throw new RuntimeException(
                "Wrong number of items in cpu.power_brackets.cluster" + cluster2);
          }
      ...
      for (int cluster6 = 0; cluster6 < this.mCpuClusters.length; cluster6++) {
        ...
        while (step3 < steps4) {
          ...
          int bracket3 =
              (int)
                  ((Math.log(getAveragePowerForCpuCore(cluster6, step3)) - minLogPower)
                      / logBracket);
      ...
      if (cluster < cpuClusterKeyArr.length) {
        ...
        for (int step = 0; step < brackets.length; step++) {
          if (brackets[step] == powerBracket) {
            ...
            sb.append(
                String.format(
                    Locale.US, "%.1f", Double.valueOf(getAveragePowerForCpuCore(cluster, step))));
            sb.append(')');
          }
        }
        cluster++;
  ...
  public int getPowerBracketForCpuCore(int cluster, int step) {
    ...
    if (cluster < cpuClusterKeyArr.length
        && step >= 0
        && step < cpuClusterKeyArr[cluster].powerBrackets.length) {
  ...
  private void initDisplays() {
    ...
    while (true) {
      if (Double.isNaN(
              getAveragePowerForOrdinal(POWER_GROUP_DISPLAY_AMBIENT, this.mNumDisplays, Double.NaN))
          && Double.isNaN(
    ...
    if (deprecatedAmbientDisplay != null && this.mNumDisplays == 0) {
      ...
      Slog.m121w(TAG, "ambient.on is deprecated! Use " + key + " instead.");
    ...
    if (deprecatedScreenOn != null && this.mNumDisplays == 0) {
      ...
      Slog.m121w(TAG, "screen.on is deprecated! Use " + key2 + " instead.");
    ...
    if (deprecatedScreenFull != null && this.mNumDisplays == 0) {
      ...
      Slog.m121w(TAG, "screen.full is deprecated! Use " + key3 + " instead.");
  ...
  public double getBatteryTypicalCapacity() {
    if ("jp".equalsIgnoreCase(SemSystemProperties.getCountryCode())
        && SemSystemProperties.get("ro.product.vendor.device", "").contains("beyond")) {
      return SContextConstants.ENVIRONMENT_VALUE_UNKNOWN;
    }
  ...
  public void dumpDebug(ProtoOutputStream proto) {
    ...
    writePowerConstantArrayToProto(
        proto, POWER_WIFI_CONTROLLER_TX_LEVELS, PowerProfileProto.WIFI_CONTROLLER_TX_LEVELS);
    writePowerConstantToProto(
        proto,
        POWER_WIFI_CONTROLLER_OPERATING_VOLTAGE,
        PowerProfileProto.WIFI_CONTROLLER_OPERATING_VOLTAGE);
    writePowerConstantToProto(
        proto, POWER_BLUETOOTH_CONTROLLER_IDLE, PowerProfileProto.BLUETOOTH_CONTROLLER_IDLE);
    writePowerConstantToProto(
