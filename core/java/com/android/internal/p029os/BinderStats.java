...
public class BinderStats implements Parcelable {
  public static final Parcelable.Creator<BinderStats> CREATOR =
      new Parcelable.Creator<BinderStats>() { // from class: com.android.internal.os.BinderStats.1
  ...
  private void readFromParcel(Parcel in) {
    ...
    int mg = in.readInt();
    if (mg != MAGIC) {
      Slog.m115e(TAG, "MAGIC number mismatch expected=-2130369756 actual=" + mg);
      return;
    }
    int size = in.readInt();
    for (int i = 0; i < size; i++) {
      ...
      if (num > 5) {
        reset();
        Slog.m115e(
            TAG, "The binder_calls_stats file seems to be broken. We discard previous stats.");
        return;
      }
      for (int j = 0; j < num; j++) {
  ...
  private byte[] readFully(InputStream stream, int[] outLen) throws IOException {
    ...
    while (true) {
      ...
      if (amt < 0) {
        Slog.m113d(TAG, "**** FINISHED READING: pos=" + pos + " len=" + data.length);
        outLen[0] = pos;
        return data;
      }
      pos += amt;
  ...
  public void read(InputStream stream) {
    ...
    try {
      try {
        ...
        Slog.m116e(TAG, "Failed to read stat files", e);
      }
    } finally {
      in.recycle();
    }
  ...
  public void addData(ArrayList<BinderStatsEntry> data) {
    ...
    while (this.mData.size() > 1000) {
      try {
        ...
        Slog.m116e(TAG, "IndexOutOfBoundsException occurs.", e2);
        return;
      }
    }
  }
