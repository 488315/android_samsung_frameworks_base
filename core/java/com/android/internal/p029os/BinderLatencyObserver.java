...
public class BinderLatencyObserver {
      ...
      new Runnable() { // from class: com.android.internal.os.BinderLatencyObserver.1
        ...
        public void run() {
          ...
          synchronized (BinderLatencyObserver.this.mLock) {
            histogramMap =
                new ArrayMap<>(
                    (ArrayMap<LatencyDims, int[]>) BinderLatencyObserver.this.mLatencyHistograms);
          ...
          for (LatencyDims dims : histogramMap.keySet()) {
            ...
            String transactionName =
                resolver.getMethodName(dims.getBinderClass(), dims.getTransactionCode());
  ...
  public BinderLatencyObserver(Injector injector, int processSource) {
    ...
    this.mLatencyBuckets =
        new BinderLatencyBuckets(this.mBucketCount, this.mFirstBucketSize, this.mBucketScaleFactor);
  ...
  public void setPushInterval(int pushIntervalMinutes) {
    if (pushIntervalMinutes <= 0) {
      Slog.m121w(
          TAG, "Ignored invalid push interval (value must be positive): " + pushIntervalMinutes);
      return;
    }
    synchronized (this.mLock) {
  ...
  public void setHistogramBucketsParams(
    ...
    synchronized (this.mLock) {
      if (bucketCount != this.mBucketCount
        ...
        this.mLatencyBuckets =
            new BinderLatencyBuckets(bucketCount, firstBucketSize, bucketScaleFactor);
        reset();
      }
    }
  }
