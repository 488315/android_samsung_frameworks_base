...
public abstract class KernelCpuUidBpfMapReader {
    ...
    public static class KernelCpuUidActiveTimeBpfMapReader extends KernelCpuUidBpfMapReader {
        @Override // com.android.internal.p029os.KernelCpuUidBpfMapReader
    ...
    public BpfMapIterator open(boolean ignoreCache) {
        ...
        if (!startTrackingBpfTimes()) {
            Slog.m121w(this.mTag, "Failed to start tracking");
            this.mErrors++;
            return null;
        }
        if (ignoreCache) {
        ...
        this.mWriteLock.unlock();
        this.mErrors++;
        Slog.m121w(this.mTag, "Failed to read bpf times");
        return null;
    }
