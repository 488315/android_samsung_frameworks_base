...
class AppZygoteInit {
    ...
    private static class AppZygoteConnection extends ZygoteConnection {
        ...
        protected void handlePreloadApp(ApplicationInfo appInfo) {
            Log.m98i(AppZygoteInit.TAG, "Beginning application preload for " + appInfo.packageName);
            ...
            if (appInfo.zygotePreloadName != null) {
                try {
                    ...
                    if (!ZygotePreload.class.isAssignableFrom(cl)) {
                        Log.m96e(AppZygoteInit.TAG, preloadName.getClassName() + " does not implement " + ZygotePreload.class.getName());
                    } else {
                    ...
                    Log.m97e(AppZygoteInit.TAG, "AppZygote application preload failed for " + appInfo.zygotePreloadName, e);
                }
            } else {
                Log.m98i(AppZygoteInit.TAG, "No zygotePreloadName attribute specified.");
            }
            try {
                ...
                Log.m98i(AppZygoteInit.TAG, "Application preload done");
