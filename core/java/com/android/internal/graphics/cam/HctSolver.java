...
public class HctSolver {
  static final double[][] SCALED_DISCOUNT_FROM_LINRGB = {
    ...
    new double[] {5.891086651375999E-4d, 0.0029785502573438758d, 3.270666104008398E-4d},
    new double[] {1.0146692491640572E-4d, 5.364214359186694E-4d, 0.0032979401770712076d}
  };
  static final double[][] LINRGB_FROM_SCALED_DISCOUNT = {
    new double[] {1373.2198709594231d, -1100.4251190754821d, -7.278681089101213d},
    new double[] {-271.815969077903d, 559.6580465940733d, -32.46047482791194d},
    new double[] {1.9622899599665666d, -57.173814538844006d, 308.7233197812385d}
  };
  static final double[] Y_FROM_LINRGB = {0.2126d, 0.7152d, 0.0722d};
  static final double[] CRITICAL_PLANES = {
    0.015176349177441876d,
    0.045529047532325624d,
    0.07588174588720938d,
    0.10623444424209313d,
    0.13658714259697685d,
    0.16693984095186062d,
    0.19729253930674434d,
    0.2276452376616281d,
    0.2579979360165119d,
    0.28835063437139563d,
    0.3188300904430532d,
    0.350925934958123d,
    0.3848314933096426d,
    0.42057480301049466d,
    0.458183274052838d,
    0.4976837250274023d,
    0.5391024159806381d,
    0.5824650784040898d,
  ...
  static double hueOf(double[] linrgb) {
    ...
    double rD =
        (linrgb[0] * matrix[0][0]) + (linrgb[1] * matrix[0][1]) + (linrgb[2] * matrix[0][2]);
    double gD =
        (linrgb[0] * matrix[1][0]) + (linrgb[1] * matrix[1][1]) + (linrgb[2] * matrix[1][2]);
    double bD =
        (linrgb[0] * matrix[2][0]) + (linrgb[1] * matrix[2][1]) + (linrgb[2] * matrix[2][2]);
  ...
  static int bisectToLimit(double y, double targetHue) {
    ...
    return CamUtils.argbFromLinrgbComponents(
        (left[0] + right[0]) / 2.0d, (left[1] + right[1]) / 2.0d, (left[2] + right[2]) / 2.0d);
  }
  ...
  static double inverseChromaticAdaptation(double adapted) {
    ...
    double base =
        Math.max(
            ...
            (27.13d * adaptedAbs) / (400.0d - adaptedAbs));
  ...
  static int findResultByJ(double hueRadians, double chroma, double y) {
    ...
    while (iterationRound < 5) {
      ...
      double alpha =
          (chroma == SContextConstants.ENVIRONMENT_VALUE_UNKNOWN
                  || j == SContextConstants.ENVIRONMENT_VALUE_UNKNOWN)
              ? 0.0d
      ...
      double gamma =
          (((p2 + 0.305d) * 23.0d) * t)
              / (((23.0d * p1) + ((t * 11.0d) * hCos)) + ((108.0d * t) * hSin));
      double a = gamma * hCos;
      double b = gamma * hSin;
      ...
      double linrgbR =
          (matrix[0][0] * rCScaled) + (matrix[0][1] * gCScaled) + (matrix[0][2] * bCScaled);
      double linrgbG =
          (matrix[1][0] * rCScaled) + (matrix[1][1] * gCScaled) + (matrix[1][2] * bCScaled);
      double linrgbB =
          (matrix[2][0] * rCScaled) + (matrix[2][1] * gCScaled) + (matrix[2][2] * bCScaled);
      if (linrgbR < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN
          || linrgbG < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN) {
        return 0;
      }
