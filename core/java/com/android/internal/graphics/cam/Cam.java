...
public class Cam {
  ...
  /* renamed from: mJ */
  private final float f670mJ;
  private final float mJstar;

  /* renamed from: mM */
  private final float f671mM;

  /* renamed from: mQ */
  private final float f672mQ;

  /* renamed from: mS */
  private final float f673mS;
  ...
  public static Cam fromIntInFrame(int argb, Frame frame) {
    ...
    float eHue =
        ((float) (Math.cos(((huePrime * 3.141592653589793d) / 180.0d) + 2.0d) + 3.8d)) * 0.25f;
  ...
  public int viewed(Frame frame) {
    ...
    float ac =
        frame.getAw() * ((float) Math.pow(getJ() / 100.0d, (1.0d / frame.getC()) / frame.getZ()));
    float p1 = 3846.1538f * eHue * frame.getNc() * frame.getNcb();
    ...
    float rC =
        Math.signum(rA) * (100.0f / frame.getFl()) * ((float) Math.pow(rCBase, 2.380952380952381d));
    float gCBase =
        (float)
            Math.max(
                SContextConstants.ENVIRONMENT_VALUE_UNKNOWN,
    ...
    float gC =
        Math.signum(gA) * (100.0f / frame.getFl()) * ((float) Math.pow(gCBase, 2.380952380952381d));
    float bCBase =
        (float)
            Math.max(
                SContextConstants.ENVIRONMENT_VALUE_UNKNOWN,
    ...
    float bC =
        Math.signum(bA) * (100.0f / frame.getFl()) * ((float) Math.pow(bCBase, 2.380952380952381d));
