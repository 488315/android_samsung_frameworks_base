...
public final class Frame {
  public static final Frame DEFAULT =
      make(
          ...
          (float) ((CamUtils.yFromLstar(50.0d) * 63.66197723675813d) / 100.0d),
          50.0f,
          2.0f,
          false);
  ...
  /* renamed from: mC */
  private final float f674mC;
  private final float mFl;
  private final float mFlRoot;

  /* renamed from: mN */
  private final float f675mN;
  ...
  public static Frame make(
    ...
    float c =
        ((double) f2) >= 0.9d
            ? MathUtils.lerp(0.59f, 0.69f, (f2 - 0.9f) * 10.0f)
            : MathUtils.lerp(0.525f, 0.59f, (f2 - 0.8f) * 10.0f);
    float d =
        discountingIlluminant
            ? 1.0f
            : (1.0f - (((float) Math.exp(((-adaptingLuminance) - 42.0f) / 92.0f)) * 0.2777778f))
                * f2;
    if (d > 1.0d) {
      f = 1.0f;
    } else {
    ...
    float fl =
        (k4 * adaptingLuminance)
            + (0.1f * k4F * k4F * ((float) Math.cbrt(adaptingLuminance * 5.0d)));
    ...
    float[] rgbAFactors = {
      (float) Math.pow(((rgbD[0] * fl) * rW) / 100.0d, 0.42d),
      (float) Math.pow(((rgbD[1] * fl) * gW) / 100.0d, 0.42d),
      (float) Math.pow(((rgbD[2] * fl) * bW) / 100.0d, 0.42d)
    };
    float[] rgbA = {
