...
public abstract class VerityUtils {
  ...
  public static boolean hasFsverity(String filePath) {
    ...
    if (retval < 0) {
      Slog.m115e(
          TAG,
          "Failed to check whether fs-verity is enabled, errno " + (-retval) + ": " + filePath);
      return false;
    }
    if (retval != 1) {
      return false;
  ...
  public static boolean verifyPkcs7DetachedSignature(
    ...
    if (digest.length != 32) {
      Slog.m121w(TAG, "Only sha256 is currently supported");
      return false;
    }
    try {
      CMSSignedData signedData =
          new CMSSignedData(new CMSProcessableByteArray(toFormattedDigest(digest)), signatureBlock);
      if (!signedData.isDetachedSignature()) {
        Slog.m121w(TAG, "Expect only detached siganture");
        return false;
      }
      if (!signedData.getCertificates().getMatches(null).isEmpty()) {
        Slog.m121w(TAG, "Expect no certificate in signature");
        return false;
      }
      if (!signedData.getCRLs().getMatches(null).isEmpty()) {
        Slog.m121w(TAG, "Expect no CRL in signature");
        return false;
      }
      X509Certificate trustedCert =
          (X509Certificate)
              CertificateFactory.getInstance("X.509").generateCertificate(derCertInputStream);
      ...
      for (SignerInformation si : signedData.getSignerInfos().getSigners()) {
        if (si.getSignedAttributes() != null && si.getSignedAttributes().size() > 0) {
          Slog.m121w(TAG, "Unexpected signed attributes");
          return false;
        }
        if (si.getUnsignedAttributes() != null && si.getUnsignedAttributes().size() > 0) {
          Slog.m121w(TAG, "Unexpected unsigned attributes");
          return false;
        }
        if (!NISTObjectIdentifiers.id_sha256.getId().equals(si.getDigestAlgOID())) {
          Slog.m121w(TAG, "Unsupported digest algorithm OID: " + si.getDigestAlgOID());
          return false;
        }
        if (!PKCSObjectIdentifiers.rsaEncryption.getId().equals(si.getEncryptionAlgOID())) {
          Slog.m121w(TAG, "Unsupported encryption algorithm OID: " + si.getEncryptionAlgOID());
          return false;
        }
      ...
      Slog.m122w(TAG, "Error occurred during the PKCS#7 signature verification", e);
      return false;
    }
  }
  ...
  public static byte[] getFsverityDigest(String filePath) {
    ...
    if (retval < 0) {
      if (retval != (-OsConstants.ENODATA)) {
        Slog.m115e(TAG, "Failed to measure fs-verity, errno " + (-retval) + ": " + filePath);
        return null;
      }
      return null;
    }
