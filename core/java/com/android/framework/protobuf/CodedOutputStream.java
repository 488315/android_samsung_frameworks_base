      ...
      if ((offset | length | (buffer.length - (offset + length))) < 0) {
        throw new IllegalArgumentException(
            String.format(
                ...
                Integer.valueOf(buffer.length), Integer.valueOf(offset), Integer.valueOf(length)));
      }
      this.buffer = buffer;
      this.offset = offset;
  ...
  private static final class UnsafeDirectNioEncoder extends CodedOutputStream {
    ...
    public void write(byte[] value, int offset, int length) throws IOException {
      ...
      throw new OutOfSpaceException(
          String.format(
              "Pos: %d, limit: %d, len: %d",
              Long.valueOf(this.position), Long.valueOf(this.limit), Integer.valueOf(length)));
    }
  ...
  private abstract static class AbstractBufferedEncoder extends CodedOutputStream {
    ...
    public final int spaceLeft() {
      throw new UnsupportedOperationException(
          "spaceLeft() can only be called on CodedOutputStreams that are writing to a flat array or"
              + " ByteBuffer.");
    }
    ...
    public void writeStringNoTag(String value) throws IOException {
      ...
      try {
        ...
        if (minLengthVarIntSize == maxLengthVarIntSize) {
          ...
          int newPosition =
              Utf8.encode(value, this.buffer, this.position, this.limit - this.position);
          this.position = oldPosition;
          int length = (newPosition - oldPosition) - minLengthVarIntSize;
