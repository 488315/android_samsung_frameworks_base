...
final class Utf8 {
    ...
    private static int partialIsValidUtf8(ByteBuffer buffer, int index, int limit) {
      ...
      while (index2 < limit) {
        ...
        if (byte1 >= 0) {
          ...
          if (byte2 > -65
              || ((byte1 == -32 && byte2 < -96)
                  || ((byte1 == -19 && byte2 >= -96) || buffer.get(index4) > -65))) {
            return -1;
          }
          index2 = index4 + 1;
        } else {
      ...
      if ((index | size | ((buffer.limit() - index) - size)) < 0) {
        throw new ArrayIndexOutOfBoundsException(
            String.format(
                ...
                Integer.valueOf(buffer.limit()), Integer.valueOf(index), Integer.valueOf(size)));
      }
      int offset = index;
      ...
      while (offset < limit) {
        ...
        if (DecodeUtil.isOneByte(byte1)) {
          ...
          DecodeUtil.handleFourBytes(
              byte1, b3, buffer.get(offset4), buffer.get(offset5), resultArr, resultPos2);
          offset = offset5 + 1;
      ...
      while (inIx < inLength) {
        ...
        if (c2 < 128) {
          ...
          try {
            ...
            throw new ArrayIndexOutOfBoundsException(
                "Failed writing " + in.charAt(inIx) + " at index " + badWriteIndex2);
          }
          ...
          if (inIx + 1 != inLength) {
            ...
            if (Character.isSurrogatePair(c2, low)) {
              ...
              try {
                ...
                throw new ArrayIndexOutOfBoundsException(
                    "Failed writing " + in.charAt(inIx) + " at index " + badWriteIndex22);
              }
            }
          }
      ...
      if ((index | size | ((bytes.length - index) - size)) < 0) {
        throw new ArrayIndexOutOfBoundsException(
            String.format(
                ...
                Integer.valueOf(bytes.length), Integer.valueOf(index), Integer.valueOf(size)));
      }
      int offset = index;
      int limit = offset + size;
      char[] resultArr = new char[size];
    ...
    int encodeUtf8(CharSequence in, byte[] out, int offset, int length) {
      ...
      while (i < utf16Length) {
        ...
        if (c2 < 128 && j < limit) {
          ...
          int j2 = j + 1;
          out[j] = (byte) ((c2 >>> 6) | EncodeConstants.Resolution.MM_360_EXPORT_HEIGHT_960);
          j = j2 + 1;
          out[j2] = (byte) ((c2 & '?') | 128);
          ...
          if (55296 <= c2
              && c2 <= 57343
              && (i + 1 == in.length() || !Character.isSurrogatePair(c2, in.charAt(i + 1)))) {
            throw new UnpairedSurrogateException(i, utf16Length);
          }
  ...
  static final class UnsafeProcessor extends Processor {
      ...
      if ((index | limit | (bytes.length - limit)) < 0) {
        throw new ArrayIndexOutOfBoundsException(
            String.format(
                ...
                Integer.valueOf(bytes.length), Integer.valueOf(index), Integer.valueOf(limit)));
      }
      long offset = index;
    ...
    int partialIsValidUtf8Direct(int state, ByteBuffer buffer, int index, int limit) {
      if ((index | limit | (buffer.limit() - limit)) < 0) {
        throw new ArrayIndexOutOfBoundsException(
            String.format(
                "buffer limit=%d, index=%d, limit=%d",
                Integer.valueOf(buffer.limit()), Integer.valueOf(index), Integer.valueOf(limit)));
      }
      ...
      while (inIx < inLimit) {
        ...
        if (c3 < c && outIx2 < outLimit) {
          ...
          UnsafeUtil.putByte(
              out,
              outIx2,
              (byte) ((c3 >>> 6) | EncodeConstants.Resolution.MM_360_EXPORT_HEIGHT_960));
          outIx2 = outIx3 + j;
          ...
          if ((c3 >= 55296 && 57343 >= c3) || outIx2 > outLimit - 3) {
            if (outIx2 > outLimit - 4) {
              if (55296 <= c3
                  && c3 <= 57343
                  && (inIx + 1 == inLimit || !Character.isSurrogatePair(c3, in.charAt(inIx + 1)))) {
      ...
      while (inIx < inLimit) {
        ...
        if (c4 < c && outIx2 < outLimit) {
          ...
          if ((c4 >= 55296 && 57343 >= c4) || outIx2 > outLimit - 3) {
            ...
            if (55296 <= c4
                && c4 <= 57343
                && (inIx + 1 == inLimit || !Character.isSurrogatePair(c4, in.charAt(inIx + 1)))) {
              throw new UnpairedSurrogateException(inIx, inLimit);
            }
            throw new ArrayIndexOutOfBoundsException(
          ...
          UnsafeUtil.putByte(
              outIx2, (byte) ((c4 >>> 6) | EncodeConstants.Resolution.MM_360_EXPORT_HEIGHT_960));
    ...
    private static int unsafeEstimateConsecutiveAscii(byte[] bytes, long offset, int maxChars) {
      ...
      while (i + 8 <= maxChars
          && (UnsafeUtil.getLong((Object) bytes, UnsafeUtil.BYTE_ARRAY_BASE_OFFSET + offset)
    ...
    private static int partialIsValidUtf8(long address, int remaining) {
      ...
      while (true) {
        ...
        if (byte1 < -32) {
          ...
          if (byte22 > -65
              || (((byte1 << 28) + (byte22 + SprAttributeBase.TYPE_SHADOW)) >> 30) != 0) {
            break;
          }
      ...
      switch (remaining) {
        ...
        case 2:
          return Utf8.incompleteStateFor(
              byte1, UnsafeUtil.getByte(bytes, offset), UnsafeUtil.getByte(bytes, 1 + offset));
        default:
          throw new AssertionError();
      }
    }
  ...
  private static class DecodeUtil {
    ...
    public static void handleThreeBytes(
      ...
      resultArr[resultPos] =
          (char)
              (((byte1 & 15) << 12) | (trailingByteValue(byte2) << 6) | trailingByteValue(byte3));
    }
