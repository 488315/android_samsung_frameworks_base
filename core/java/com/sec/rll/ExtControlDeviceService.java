...
public class ExtControlDeviceService extends IExtControlDeviceService.Stub {
    ...
    public void setStatus(int deviceType, int status) throws RemoteException {
        Log.m94d(TAG, "setStatus called");
        if (!isAccessPermitted()) {
            return;
        }
        if (deviceType == 4097) {
            Log.m96e(TAG, "Set gps state called with state : " + status);
        ...
        if (deviceType == 8193) {
            Log.m96e(TAG, "Set NFC/Felica state called with state : " + status);
            setNfcState(status);
        }
    }
    ...
    private boolean isAccessPermitted() {
        ...
        if (callerUid == mUid) {
            Log.m94d(TAG, "UID matches - access granted to uid:" + callerUid);
            return true;
        }
        ...
        for (String pkg : packages) {
            boolean z = DEBUG;
            if (z) {
                Log.m94d(TAG, "Looking up pkg info for:" + pkg);
            }
            if (z && pkg.equals("com.rll.test")) {
                Log.m94d(TAG, "Lets allow our test app access RLL");
                return true;
            }
            if (pkg.equals("com.kddi.extcontroldevice") && isSystemApp(pkg)) {
                Log.m94d(TAG, "Allowing RLL access");
                return true;
            }
        }
        Log.m102w(TAG, "Access denied to UID:" + callerUid);
        return false;
    }

    @Override // com.sec.rll.IExtControlDeviceService
    public int getStatus(int deviceType) throws RemoteException {
        Log.m94d(TAG, "getStatus called");
        if (!isAccessPermitted()) {
            return -1;
        }
        if (deviceType == 4097) {
            Binder.clearCallingIdentity();
            Log.m96e(TAG, "get gps state called return value  : " + Settings.Secure.getInt(mContext.getContentResolver(), Settings.Secure.LOCATION_MODE, 0));
            int currentMode = Settings.Secure.getInt(mContext.getContentResolver(), Settings.Secure.LOCATION_MODE, 0);
        ...
        Log.m96e(TAG, "get nfc/felica state called return value : " + getNfcState());
        return getNfcState();
    }
    ...
    public static boolean isENGDevice() {
        ...
        if (Build.DISPLAY.contains("-eng")) {
            Log.m94d(TAG, "isENGDevice: eng build");
            isEngDevice = true;
        }
        ...
        Log.m94d(TAG, "buildType: " + buildType);
