...
class IAFDServiceImpl {
  ...
  private int getDualUserIdAndIsNoSettingsProvidersOfDual() {
    ...
    int dualuserid = 0;
    String[] cmdArr = {
      "/system/bin/sh", "-c", "dumpsys package com.android.providers.settings | grep User"
    };
    try {
  ...
  private final class ServiceHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      switch (msg.what) {
        case 1:
          Log.m94d(IAFDServiceImpl.TAG, "CMD_TYPE_START");
        ...
        case 2:
          ...
          boolean repairResult =
              IAFDServiceImpl.this.mIAFDRepair.repairHandle(IAFDServiceImpl.this.mContext, bundle2);
        ...
        case 5:
          ...
          Log.m94d(IAFDServiceImpl.TAG, "CMD_TYPE_GETUPDATESTATUS");
          IAFDServiceImpl.this.checkUpdate(bundle3);
          break;
        case 6:
          Log.m94d(IAFDServiceImpl.TAG, "CMD_TYPE_GETUPDATESTATUS_RESULT");
          IAFDServiceImpl.this.IAFDstartApp(msg.getData(), true);
          break;
        case 9:
          IAFDServiceImpl.this.mIAFDGetHotfixDataService.getDataFromClient(
              msg.getData().getString("hotfixdata"));
          break;
      }
    }
  }
  ...
  public boolean IAFDstartApp(Bundle bundle, boolean hasGetUpdateResult) {
    ...
    if (hasGetUpdateResult) {
      try {
        ...
        Log.m94d(TAG, "ShowAppErrorUiExt fail, skip");
        return false;
      }
    }
    ...
    if (!hasUpdated && repairType != 0) {
      ...
      if (trigApp.equals("vocApp")) {
        ...
        Log.m94d(TAG, "ShowAppErrorUiExt start voc app");
        return true;
      }
    ...
    Log.m94d(TAG, "ShowAppErrorUiExt start sm app");
    return true;
  }
  ...
  private void showSystemAppDiaglog(final Bundle bundle, final String trigApp) {
    try {
      ...
      builder
          ...
          .setMessage(
              "Happened exception in the running application, you can try to resolve it with the"
                  + " button of [Try to resolve]")
          .setCancelable(true)
          .setPositiveButton(
              "Try to resolve",
              new DialogInterface
                  .OnClickListener() { // from class: com.sec.android.iaft.IAFDServiceImpl.2
                @Override // android.content.DialogInterface.OnClickListener
                public void onClick(DialogInterface dialog, int which) {
                  if (trigApp.equals("SmartMApp")) {
                    Intent intent =
                        new Intent("com.samsung.android.sm.ACTION_START_THIRD_APP_ERROR_DIALOG");
          ...
          .setNegativeButton(
              "Cancel",
              new DialogInterface
                  .OnClickListener() { // from class: com.sec.android.iaft.IAFDServiceImpl.1
                @Override // android.content.DialogInterface.OnClickListener
                public void onClick(DialogInterface dialog, int which) {
                  dialog.cancel();
                }
