    ...
    public static boolean hotfix(Context context, int expType, String pkgName) {
        ...
        try {
            ...
            if (!dexLocation.exists()) {
                Log.m98i(TAG, dexLocation.toString() + " not found.");
                return false;
            }
            ...
            boolean result = ((Boolean) iafdrepairMethod.invoke(obj, context, Integer.valueOf(expType), pkgName)).booleanValue();
            Log.m98i(TAG, "hotfix end");
            return result;
        } catch (Exception e) {
            Log.m98i(TAG, "hotfix fail");
            return false;
        }
    }
}
