    ...
    public void saveFile(String inData) {
        ...
        if (inData != null) {
            try {
                ...
                Log.m98i(TAG, "ToFile fail");
                Log.m98i(TAG, "saveFile completed");
            }
        }
        if (isIAFDDBHOTFIX_UPDATE) {
            Log.m98i(TAG, "IAFDDBHOTFIX_UPDATE");
            try {
        ...
        if (isARDBHOTFIX_UPDATE) {
            Log.m98i(TAG, "ARDBHOTFIX_UPDATE");
        }
        if (isADDBHOTFIX_UPDATE) {
            Log.m98i(TAG, "ADDBHOTFIX_UPDATE");
        }
        if (isIAFTDBHOTFIX_UPDATE) {
            Log.m98i(TAG, "IAFTDBHOTFIX_UPDATE");
        }
        Log.m98i(TAG, "saveFile completed");
    }
}
