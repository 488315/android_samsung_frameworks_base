...
public final class MonotonicCurveFit extends CurveFit {
    ...
    public final void getPos(double d, double[] dArr) {
        double[] dArr2 = this.f4mT;
        int length = dArr2.length;
        int i = 0;
        int length2 = this.f5mY[0].length;
        double d2 = dArr2[0];
        if (d <= d2) {
            getSlope(d2, this.mSlopeTemp);
            for (int i2 = 0; i2 < length2; i2++) {
                dArr[i2] = ((d - this.f4mT[0]) * this.mSlopeTemp[i2]) + this.f5mY[0][i2];
            }
            return;
        }
        int i3 = length - 1;
        ...
        if (d >= d3) {
            ...
            while (i < length2) {
                dArr[i] = ((d - this.f4mT[i3]) * this.mSlopeTemp[i]) + this.f5mY[i3][i];
                i++;
            }
            return;
        }
        ...
        while (i4 < length - 1) {
            if (d == this.f4mT[i4]) {
                for (int i5 = 0; i5 < length2; i5++) {
                    dArr[i5] = this.f5mY[i4][i5];
                }
            }
            double[] dArr3 = this.f4mT;
            int i6 = i4 + 1;
            double d4 = dArr3[i6];
            if (d < d4) {
                double d5 = dArr3[i4];
    ...
    public final void getPos(double d, float[] fArr) {
        ...
        if (d <= d2) {
            getSlope(d2, this.mSlopeTemp);
            for (int i2 = 0; i2 < length2; i2++) {
                fArr[i2] = (float) (((d - this.f4mT[0]) * this.mSlopeTemp[i2]) + this.f5mY[0][i2]);
            }
            return;
        }
        int i3 = length - 1;
        ...
        if (d >= d3) {
            getSlope(d3, this.mSlopeTemp);
            while (i < length2) {
                fArr[i] = (float) (((d - this.f4mT[i3]) * this.mSlopeTemp[i]) + this.f5mY[i3][i]);
                i++;
            }
            return;
        }
