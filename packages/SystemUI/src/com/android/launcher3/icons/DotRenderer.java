...
public class DotRenderer {
    ...
    public DotRenderer(int i, Path path, int i2) {
        new Paint(3);
        int iRound = Math.round(i * 0.228f);
        iRound = iRound <= 0 ? 1 : iRound;
        ...
        float f4 = f / 2.0f;
        builder.radius = f4;
        int iMax = Math.max(Math.round(f3 + f4), Math.round(builder.radius + builder.shadowBlur + builder.keyShadowDistance));
        ...
        Canvas canvasBeginRecording = picture.beginRecording(i3, i3);
        ...
        canvasBeginRecording.drawRoundRect(rectF, f8, f8, paint);
        ...
        canvasBeginRecording.drawRoundRect(rectF2, f10, f10, paint);
        if (Color.alpha(i4) < 255) {
            ...
            canvasBeginRecording.drawRoundRect(rectF3, f11, f11, paint);
            paint.setXfermode(null);
            paint.setColor(i4);
            RectF rectF4 = builder.bounds;
            float f12 = builder.radius;
            canvasBeginRecording.drawRoundRect(rectF4, f12, f12, paint);
        }
    ...
    public static void getPathPoint(Path path, float f, float f2) {
        float f3 = f / 2.0f;
        float f4 = (f2 * f3) + f3;
        Path pathM = PathInterpolator$$ExternalSyntheticOutline0.m(f3, f3);
        pathM.lineTo((f2 * 1.0f) + f4, 0.0f);
        pathM.lineTo(f4, -1.0f);
        pathM.close();
        pathM.op(path, Path.Op.INTERSECT);
        float[] fArr = new float[2];
        new PathMeasure(pathM, false).getPosTan(0.0f, fArr, null);
        fArr[0] = fArr[0] / f;
        fArr[1] = fArr[1] / f;
    }
}
