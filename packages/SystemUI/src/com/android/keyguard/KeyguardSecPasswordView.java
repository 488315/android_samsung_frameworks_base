...
public class KeyguardSecPasswordView extends KeyguardPasswordView {
    ...
    public final boolean startDisappearAnimation(Runnable runnable) {
        if (this.mPasswordEntry.isAttachedToWindow() && this.mPasswordEntry.getRootWindowInsets().isVisible(WindowInsets.Type.ime())) {
            Log.d("KeyguardSecPasswordView", "startDisappearAnimation -> controlWindowInsetsAnimation");
            super.startDisappearAnimation(runnable);
            return true;
        }
        Log.d("KeyguardSecPasswordView", "startDisappearAnimation -> animate view with FastOutLinearInInterpolator");
