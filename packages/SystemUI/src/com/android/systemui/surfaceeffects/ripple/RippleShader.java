...
public final class RippleShader extends RuntimeShader {
    ...
    public final class SizeAtProgress {
        ...
        public final String toString() {
            return DpCornerSize$$ExternalSyntheticOutline0.m(this.height, ")", CubicBezierEasing$$ExternalSyntheticOutline0.m("SizeAtProgress(t=", this.t, ", width=", this.width, ", height="));
        }
    }
    ...
    public final void setRawProgress(float f) {
        ...
        if (((ArrayList) rippleSize.sizes).isEmpty()) {
            ...
            while (interpolation > ((SizeAtProgress) obj).t) {
                int iMin = Math.min(rippleSize.currentSizeIndex + 1, ((ArrayList) rippleSize.sizes).size() - 1);
                rippleSize.currentSizeIndex = iMin;
                obj = ((ArrayList) rippleSize.sizes).get(iMin);
            }
            i = rippleSize.currentSizeIndex;
        }
        int iMax = Math.max(i - 1, 0);
        SizeAtProgress sizeAtProgress = (SizeAtProgress) ((ArrayList) rippleSize.sizes).get(i);
        SizeAtProgress sizeAtProgress2 = (SizeAtProgress) ((ArrayList) rippleSize.sizes).get(iMax);
        Companion companion = Companion;
        float f2 = sizeAtProgress2.t;
        float f3 = sizeAtProgress.t;
        companion.getClass();
        float fSubProgress = Companion.subProgress(f2, f3, interpolation);
        rippleSize.currentWidth = (sizeAtProgress.width * fSubProgress) + sizeAtProgress2.width;
        rippleSize.currentHeight = (sizeAtProgress.height * fSubProgress) + sizeAtProgress2.height;
    ...
    public final class RippleSize {
        ...
        public final void setSizeAtProgresses(SizeAtProgress... sizeAtProgressArr) {
            ((ArrayList) this.sizes).clear();
            this.currentSizeIndex = 0;
            CollectionsKt__MutableCollectionsKt.addAll(this.sizes, sizeAtProgressArr);
