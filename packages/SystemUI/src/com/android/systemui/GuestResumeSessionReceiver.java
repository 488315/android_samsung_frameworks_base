...
public final class GuestResumeSessionReceiver {
        ...
        public final void onUserChanged(int i, Context context) {
            ...
            if (userInfo.isGuest()) {
                ...
                if (userInfo.isGuest()) {
                    ...
                    PendingIntent broadcastAsUser = PendingIntent.getBroadcastAsUser(context2, 0, intent, QuickStepContract.SYSUI_STATE_REQUESTED_RECENT_KEY, userHandle);
                    Notification.Builder contentIntent = new Notification.Builder(guestSessionNotification.mContext, NotificationChannels.ALERTS).setSmallIcon(R.drawable.ic_account_circle).setContentTitle(guestSessionNotification.mContext.getString(R.string.guest_notification_session_active)).setContentText(string).setPriority(0).setOngoing(true).setContentIntent(PendingIntent.getActivityAsUser(guestSessionNotification.mContext, 0, intent2, 335544320, null, UserHandle.of(userInfo.id)));
                    if (!z) {
                        contentIntent.addAction(R.drawable.ic_sysbar_home, guestSessionNotification.mContext.getString(R.string.guest_reset_guest_confirm_button), PendingIntent.getBroadcastAsUser(guestSessionNotification.mContext, 0, new Intent("android.intent.action.GUEST_RESET"), QuickStepContract.SYSUI_STATE_REQUESTED_RECENT_KEY, userHandle));
                    }
    ...
    public class ResetSessionDialog extends SystemUIDialog implements DialogInterface.OnClickListener {
        ...
        public ResetSessionDialog(Context context, UserSwitcherController userSwitcherController, UiEventLogger uiEventLogger, int i) {
            super(context, R.style.Theme_SystemUI_Dialog, false);
