    ...
    public final void run() {
        ...
        if (BasicRune.VOLUME_HOME_IOT) {
            ...
            for (int i = 0; i < length; i++) {
                soundPoolWrapper2.soundIDs[i] = soundPoolBuild.load(SoundPoolWrapper.HOME_HUB_FILES[i], 0);
            }
        } else if (BasicRune.SUPPORT_SOUND_THEME) {
            int length2 = SoundPoolWrapper.SOUND_THEME_FILES.length;
            for (int i2 = 0; i2 < length2; i2++) {
                soundPoolWrapper2.soundIDs[i2] = soundPoolBuild.load(SoundPoolWrapper.SOUND_THEME_FILES[i2], 0);
            }
        } else {
            soundPoolWrapper2.soundID = soundPoolBuild.load("system/media/audio/ui/TW_Volume_control.ogg", 0);
        }
        soundPoolWrapper.soundPool = soundPoolBuild;
    }
}
