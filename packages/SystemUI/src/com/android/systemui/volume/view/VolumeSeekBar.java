    ...
    public final class VolumeSeekbarChangeListener implements SeekBar.OnSeekBarChangeListener {
        ...
        public final void onProgressChanged(SeekBar seekBar, int i, boolean z) {
            ...
            if (volumeSeekBar.isTracking || z) {
                ...
                SeekBarUtil.vibrateIfNeeded(seekBar, VolumeSeekBar.this.stream, i);
            }
        }
        ...
        if (this.enabled) {
            ...
            if (action != 1) {
                if (action == 2) {
                    ...
                    if (getContext().getResources().getConfiguration().getLayoutDirection() == 1 && getRotation() == 0.0f) {
                        x *= -1;
                    }
                    if (this.isTracking) {
                        trackTouchEvent(x);
                    ...
                    if (Math.abs(x) > this.scaledTouchSlop) {
                        trackTouchEvent(x);
