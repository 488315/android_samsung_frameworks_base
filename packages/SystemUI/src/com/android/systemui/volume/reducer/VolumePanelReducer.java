...
public class VolumePanelReducer implements VolumePanelReducerBase {
    ...
    */
    ...
    public static int determineIconState(VolumeStreamState volumeStreamState, VolumeState volumeState) {
        ...
        if (volumeStreamState.isRoutedToMusicFrame()) {
            return 15;
        }
        ...
        if (volumeStreamState.getStreamType() == 2 && volumeState.getRingerModeInternal() == 2) {
            return 3;
        }
        return ((volumeStreamState.getStreamType() != 0 && ((volumeStreamState.getStreamType() != 6 || volumeState.isRemoteMic()) && volumeStreamState.getLevel() == 0)) || volumeStreamState.isMuted()) ? 1 : 3;
    }

    public static int determineRealVolumeLevel(VolumeStreamState volumeStreamState, VolumePanelAction volumePanelAction, boolean z, boolean z2) {
        VolumeState volumeState = volumePanelAction.getVolumeState();
        boolean z3 = (checkStream(volumeStreamState, 3) || volumeStreamState.getStreamType() == 21) && (volumePanelAction.isZenNone() || z);
        if ((!(volumeStreamState.getStreamType() == 2 && volumeState.getRingerModeInternal() == 2) && volumeStreamState.isMuted()) || z3) {
            return 0;
        }
        return (volumeStreamState.getStreamType() != 0 || !z2 || volumeState == null || volumeState.isRemoteMic()) ? volumeStreamState.getLevel() : volumeStreamState.getLevel() + 1;
    }
    ...
    public static String determineRemoteLabel(VolumePanelRow volumePanelRow, VolumeStreamState volumeStreamState, VolumePanelAction volumePanelAction) {
        ...
        } else if (volumeStreamState.getStreamType() != 6 || !volumeState.isRemoteMic()) {
            return volumeStreamState.isDynamic() ? volumeStreamState.getRemoteLabel() : volumePanelRow.getRemoteLabel();
        }
    ...
    public static int determineRowPriority(VolumePanelRow volumePanelRow, int i, boolean z, boolean z2) {
        ...
        if ((volumePanelRow.isVisible() && !z) || (z && volumePanelRow.getPriority() == 1)) {
            return 1;
        }
        if (volumePanelRow.getStreamType() == 23 && i == 3) {
            return 1;
        }
    ...
    public static boolean isStreamSilent(int i, VolumeStreamState volumeStreamState, VolumeState volumeState) {
        return (volumeStreamState.getStreamType() == i) && volumeState.getRingerModeInternal() == 0;
    }
