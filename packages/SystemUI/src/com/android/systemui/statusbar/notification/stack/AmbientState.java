...
public class AmbientState implements Dumpable, PanelScreenShotLogger.LogProvider {
    ...
    public AmbientState(Context context, DumpManager dumpManager, StackScrollAlgorithm.SectionProvider sectionProvider, StackScrollAlgorithm.BypassController bypassController, StatusBarKeyguardViewManager statusBarKeyguardViewManager, LargeScreenShadeInterpolator largeScreenShadeInterpolator, HeadsUpRepository headsUpRepository, AvalancheController avalancheController) {
    ...
    public final ArrayList gatherState() {
        ...
        ExpandableNotificationRow expandableNotificationRow = this.mTrackedHeadsUpRow;
        PanelScreenShotLogger.addLogItem(arrayList, "mTrackedHeadsUpRow.key=", expandableNotificationRow == null ? "null" : expandableNotificationRow.mLoggingKey);
    ...
    public final boolean isDozingAndNotPulsing(ExpandableView expandableView) {
        if (expandableView instanceof ExpandableNotificationRow) {
            ...
            boolean z = this.mPulsing && ((ExpandableNotificationRow) expandableView).getEntryLegacy().mIsHeadsUpEntry;
            if (this.mDozing && !z) {
                return true;
            }
        }
    ...
    public final boolean isNeedsToExpandLocksNoti() {
        return !((SettingsHelper) Dependency.sDependency.getDependencyInner(SettingsHelper.class)).isNotificationAsCard() || (!((SecQsUiDisplayModeInteractor) Dependency.sDependency.getDependencyInner(SecQsUiDisplayModeInteractor.class)).isTablet() && this.mOrientation == 2) || this.mPluginLockMode == 1;
    }
    ...
    public final void setDragDownOnKeyguard(boolean z) {
        EmergencyButtonController$$ExternalSyntheticOutline0.m("setDragDownOnKeyguard: ", "AmbientState", z);
        this.mDragDownOnKeyguard = z;
        ArrayList arrayList = this.mKeyguardNotiExpandListeners;
        int size = arrayList.size();
        int i = 0;
        while (i < size) {
            Object obj = arrayList.get(i);
            i++;
            NotificationShelf notificationShelf = (NotificationShelf) ((KeyguardNotiExpandListener) obj);
    ...
    public final void setSwipingUp(boolean z) {
        int i = SceneContainerFlag.$r8$clinit;
        RefactorFlagUtils refactorFlagUtils = RefactorFlagUtils.INSTANCE;
