    ...
    public void onEndLifetimeExtension(NotifLifetimeExtender notifLifetimeExtender, NotificationEntry notificationEntry) {
        ...
        if (notifCollection.mAttached) {
            ...
            if (!((ArrayList) notificationEntry.mLifetimeExtenders).remove(notifLifetimeExtender)) {
                IllegalStateException illegalStateException = new IllegalStateException(TransitionKt$$ExternalSyntheticOutline0.m(SeslRoundedCorner$SeslRoundedChunkingDrawable$$ExternalSyntheticOutline0.m("Cannot end lifetime extension for extender \"", notifLifetimeExtender.getName(), "\" of entry ", strLogKey, " (collection entry is "), str, ")"));
