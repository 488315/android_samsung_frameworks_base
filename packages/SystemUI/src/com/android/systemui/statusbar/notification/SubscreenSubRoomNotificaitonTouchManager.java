    ...
    public SubscreenSubRoomNotificaitonTouchManager(Context context, SubscreenNotificationInfoManager subscreenNotificationInfoManager, Vibrator vibrator) {
        ...
        ?? r0 = new ItemTouchHelper.SimpleCallback(0, 8) { // from class: com.android.systemui.statusbar.notification.SubscreenSubRoomNotificaitonTouchManager.1
            ...
            public final void onSwiped(RecyclerView.ViewHolder viewHolder) {
                ...
                if (z2) {
                    int size = subscreenNotificationInfoManager3.mGroupDataArray.size();
                    SubscreenNotificationController subscreenNotificationController = subscreenSubRoomNotificaitonTouchManager.mSubscreenNotificationController;
                    if (size == 0) {
                        if (subscreenParentItemViewHolder.mInfo.mRow.isInsignificant()) {
                            ...
                            subscreenNotificationController.mDeviceModel.hideGroupNotification();
                        }
                    } else if (subscreenNotificationInfoManager3.mGroupDataArray.size() <= 1 && !((SubscreenNotificationInfo) subscreenNotificationInfoManager3.mGroupDataArray.get(0)).mRow.isInsignificant()) {
                        subscreenNotificationController.mDeviceModel.hideGroupNotification();
                    }
                }
