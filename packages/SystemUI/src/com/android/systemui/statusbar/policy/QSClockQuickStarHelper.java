...
public class QSClockQuickStarHelper implements SlimIndicatorViewSubscriber {
    ...
    public final String getQuickStarDateViewText() {
        if (TextUtils.isEmpty(this.mQuickStarDateStringFormat)) {
            ...
            if (quickStarDateFormat == 1) {
                this.mQuickStarDateStringPattern = getShorterDatePatternForQuickStar(Boolean.FALSE);
            ...
            } else if (quickStarDateFormat != 5) {
                this.mQuickStarDateStringPattern = this.mContext.getString(R.string.quick_panel_shorten_date_pattern);
            } else {
                this.mQuickStarDateStringPattern = getShorterDatePatternForQuickStar(Boolean.TRUE);
            }
            try {
                if (quickStarDateFormat == 1 || quickStarDateFormat == 5) {
                    this.mQuickStarDateStringFormat = this.mQuickStarDateStringPattern;
                } else {
    ...
    public final String getShorterDatePatternForQuickStar(Boolean bool) {
        String pattern = ((SimpleDateFormat) SimpleDateFormat.getDateInstance(3, Locale.getDefault())).toPattern();
        return bool.booleanValue() ? pattern.contains("dd") ? pattern.replaceAll("\\W?[Yy]+\\W?", "").replace("dd", this.mContext.getString(R.string.quick_panel_day_to_weekday_pattern)) : pattern.replaceAll("\\W?[Yy]+\\W?", "").replace("d", this.mContext.getString(R.string.quick_panel_day_to_weekday_pattern)) : pattern.replaceAll("\\W?[Yy]+\\W?", "");
    }
