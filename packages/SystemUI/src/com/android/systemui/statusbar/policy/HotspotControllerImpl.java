...
public class HotspotControllerImpl implements HotspotController, WifiManager.SoftApCallback {
    ...
    public static void m3103$$Nest$mfireHotspotAvailabilityChanged(HotspotControllerImpl hotspotControllerImpl) {
        ...
        while (i < size) {
            ...
            i++;
            ((HotspotController.Callback) obj).onHotspotAvailabilityChanged(hotspotControllerImpl.isHotspotSupported());
        }
    }
        ...
        synchronized (this.mCallbacks) {
            try {
                ...
                while (i < size) {
                    Object obj = arrayList.get(i);
                    i++;
                    ((HotspotController.Callback) obj).onUpdateConnectedDevices();
                }
            } catch (Throwable th) {
                throw th;
            }
    ...
    public final void onStateChanged(int i, int i2) {
        ...
        switch (this.mHotspotState) {
            case 10:
            case 12:
                Log.i("HotspotController", "fireHotspotAvailabilityChanged - triggered");
                synchronized (this.mCallbacks) {
                ...
                while (i3 < size) {
                    ...
                    i3++;
                    ((HotspotController.Callback) obj).onHotspotPrepared();
                }
                if (arrayList.isEmpty()) {
                    Log.i("HotspotController", "fireHotspotAvailabilityChanged - mCallbacks is empty");
                }
                Log.i("HotspotController", "fireHotspotAvailabilityChanged - end");
                return;
            case 11:
            case 13:
            case 14:
                Log.i("HotspotController", "fireHotspotChangedCallback - triggered");
                synchronized (this.mCallbacks) {
                    arrayList2 = new ArrayList(this.mCallbacks);
                }
                int size2 = arrayList2.size();
                while (i3 < size2) {
                    Object obj2 = arrayList2.get(i3);
                    i3++;
                    ((HotspotController.Callback) obj2).onHotspotChanged(getNumConnectedDevices(), isHotspotEnabled());
                }
                if (arrayList2.isEmpty()) {
                    Log.i("HotspotController", "fireHotspotChangedCallback - mCallbacks is empty");
                }
                Log.i("HotspotController", "fireHotspotChangedCallback - end");
