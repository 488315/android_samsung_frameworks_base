...
public class SemRemoteServiceStateManager {
    ...
    public class ConnectionStateClearHandler extends Handler {
        ...
        public final void handleMessage(Message message) {
            if (message.what != 101) {
                ...
                ArrayList arrayList2 = SemRemoteServiceStateManager.remoteServiceKeySet;
                int size = arrayList2.size();
                int i = 0;
                while (i < size) {
                    Object obj = arrayList2.get(i);
                    i++;
                    semRemoteServiceStateManager.mRemoteServiceStateMap.put((String) obj, 0);
                }
                Intent intent = new Intent("com.samsung.systemui.clipboard.UPDATE_CONNECTION_STATE");
                BroadcastSender broadcastSender = semRemoteServiceStateManager.mBoardcasteSender;
                broadcastSender.getClass();
                broadcastSender.sendInBackground(String.valueOf(intent), new BroadcastSender$$ExternalSyntheticLambda0(broadcastSender, intent, 1));
            }
    ...
    public SemRemoteServiceStateManager(Context context, BroadcastSender broadcastSender) {
        ...
        while (i < size) {
            ...
            i++;
            this.mRemoteServiceStateMap.put((String) obj, 0);
        }
