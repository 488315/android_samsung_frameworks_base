...
public class DefaultUiController {
    ...
    public final void hide() {
        ...
        ArrayList arrayList = invocationLightsView.mAssistInvocationLights;
        int size = arrayList.size();
        int i = 0;
        while (i < size) {
            Object obj = arrayList.get(i);
            i++;
            ((EdgeLight) obj).setEndpoints(0.0f, 0.0f);
        }
            ...
            if (!invocationLightsView.mRegistered && invocationLightsView.mNavigationBarController != null) {
                if (BasicRune.NAVBAR_ENABLED) {
                    ...
                    if (BasicRune.NAVBAR_TASKBAR && ((NavBarStateManagerImpl) navStateManager).isTaskBarEnabled(false)) {
                        LightBarTransitionsController lightBarTransitionsController = ((TaskbarDelegate) Dependency.sDependency.getDependencyInner(TaskbarDelegate.class)).mLightBarTransitionsController;
