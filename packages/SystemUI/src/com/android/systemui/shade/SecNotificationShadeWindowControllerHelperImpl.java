...
public final class SecNotificationShadeWindowControllerHelperImpl implements SecNotificationShadeWindowControllerHelper {
    ...
    public final SamsungNotificationShadeWindowFullscreenHelper fullscreenHelper;
    ...
    public WindowRootView notificationShadeView;
    ...
    public final class Provider {
        ...
        public Provider(Supplier<NotificationShadeWindowState> supplier, Supplier<WindowManager.LayoutParams> supplier2, Supplier<WindowManager.LayoutParams> supplier3, Predicate<Boolean> predicate, Consumer<NotificationShadeWindowState> consumer) {
    ...
    public SecNotificationShadeWindowControllerHelperImpl(Context context, PowerManager powerManager, WindowManager windowManager, DisplayLifecycle displayLifecycle, KeyguardStateController keyguardStateController, KeyguardUpdateMonitor keyguardUpdateMonitor, SettingsHelper settingsHelper, IActivityManager iActivityManager, KeyguardFoldController keyguardFoldController, KeyguardFastBioUnlockController keyguardFastBioUnlockController, KeyguardVisibilityMonitor keyguardVisibilityMonitor, EngineeringModeManagerWrapper engineeringModeManagerWrapper, Lazy lazy, PluginLockMediator pluginLockMediator, KeyguardWallpaper keyguardWallpaper, KeyguardTransitionInteractor keyguardTransitionInteractor, CoroutineScope coroutineScope, AODAmbientWallpaperHelper aODAmbientWallpaperHelper, Lazy lazy2, Lazy lazy3, KeyguardViewMediatorHelper keyguardViewMediatorHelper, SamsungNotificationShadeWindowFullscreenHelper samsungNotificationShadeWindowFullscreenHelper) {
        ...
        this.fullscreenHelper = samsungNotificationShadeWindowFullscreenHelper;
    }
    ...
    public final void applyHelper(NotificationShadeWindowState notificationShadeWindowState) {
        ...
        if (this.isSystemUser) {
            ...
            if (this.settingsHelper.isUltraPowerSavingMode() || this.settingsHelper.isEmergencyMode() || ((this.keyguardUpdateMonitor.mKeyguardOccluded && ((WakefulnessModel) ((PowerInteractor) this.powerInteractorLazy.get()).detailedWakefulness.$$delegate_0.getValue()).isAwake()) || (LsRune.COVER_SUPPORTED && notificationShadeWindowState.isCoverClosed && notificationShadeWindowState.coverType == 16))) {
            ...
            if (LsRune.AOD_FULLSCREEN) {
                ...
                if ((!aODAmbientWallpaperHelper.isAODFullScreenMode() || ((KeyguardViewMediator) aODAmbientWallpaperHelper.keyguardViewMediatorLazy.get()).getViewMediatorCallback().isScreenOn()) && !aODAmbientWallpaperHelper.needWallpaperForUnlockAnimation) {
                    if (!aODAmbientWallpaperHelper.isAODFullScreenMode()) {
                        return;
                    }
                    if (!this.fastUnlockController.isWakeAndUnlockAnimationAODFullScreenMode() && !notificationShadeWindowState.forceVisibleForUnlockAnimation) {
                        return;
                    }
                }
                layoutParamsChanged3.flags |= 1048576;
            }
        }
    }
    ...
    public final boolean isLockScreenRotationAllowed() {
        ...
        return (!z || LsRune.KEYGUARD_SUB_DISPLAY_ROTATIONAL) ? DeviceState.shouldEnableKeyguardScreenRotation(((KeyguardStateControllerImpl) keyguardStateController).mContext) && this.settingsHelper.isLockScreenRotationAllowed() : DeviceState.shouldEnableKeyguardScreenRotation(((KeyguardStateControllerImpl) keyguardStateController).mContext) && this.settingsHelper.isLockScreenRotationAllowed() && ((KeyguardFoldControllerImpl) this.keyguardFoldController).isFoldOpened();
    }

    public final void resetForceInvisible(boolean z) {
        NotificationShadeWindowState currentState = getCurrentState();
        if (currentState.forceInvisible) {
            Log.d(DEBUG_TAG, KeyguardUpdateMonitorLogger$$ExternalSyntheticOutline0.m("resetForceInvisible ", z));
    ...
    public final void setForceInvisible(boolean z) {
        ...
        Log.d(DEBUG_TAG, KeyguardUpdateMonitorLogger$$ExternalSyntheticOutline0.m("setForceInVisible ", z));
