...
public class QRCodeScannerController implements CallbackController {
    ...
    public QRCodeScannerController$$ExternalSyntheticLambda3 mOnDefaultQRCodeScannerChangedListener = null;
    ...
    public QRCodeScannerController(Context context, Executor executor, SecureSettings secureSettings, DeviceConfigProxy deviceConfigProxy, UserTracker userTracker) {
        ...
        executor.execute(new QRCodeScannerController$$ExternalSyntheticLambda0(this, 0));
    }

    public final boolean isAbleToLaunchScannerActivity() {
        Intent intent = this.mIntent;
        if (intent != null) {
            if (intent.getComponent() == null ? false : !this.mContext.getPackageManager().queryIntentActivities(intent, 819200).isEmpty()) {
                return true;
            }
        }
        return false;
    ...
    /* JADX WARN: Type inference failed for: r2v4, types: [android.provider.DeviceConfig$OnPropertiesChangedListener, com.android.systemui.qrcodescanner.controller.QRCodeScannerController$$ExternalSyntheticLambda3] */
        ...
        if (isCameraAvailable()) {
            for (int i : iArr) {
                if (i == 0) {
                    ...
                    ClockEventController$$ExternalSyntheticOutline0.m(i, "Unrecognised event: ", "QRCodeScannerController");
                } else {
