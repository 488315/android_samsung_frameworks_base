...
public class RightShortcutImageCreator extends AbsShortcutImageCreator {
    ...
    public final Bitmap createImage(ImageOptionCreator.ImageOption imageOption, Point point) {
        View viewInflate = ((LayoutInflater) this.mContext.getSystemService("layout_inflater")).inflate(R.layout.keyguard_sec_affordance_view_right, (ViewGroup) null);
        updateCustomShortcutIcon((KeyguardSecAffordanceView) viewInflate.findViewById(R.id.end_button), 1, getShortcutManager().hasShortcut(1));
        Bitmap viewImage = ImageCreator.getViewImage(viewInflate, imageOption, getShortcutManager().isMonotoneIcon(1));
        if (viewImage != null) {
