...
public class SWorkModeTile extends SQSTileImpl implements ManagedProfileController.Callback {
    ...
    public final void onManagedProfileChanged() {
        ...
        if (((ManagedProfileControllerImpl) managedProfileController).hasActiveProfile()) {
            refreshState(Boolean.valueOf(((ManagedProfileControllerImpl) managedProfileController).isWorkModeEnabled()));
        } else {
            onManagedProfileRemoved();
        }
    }
