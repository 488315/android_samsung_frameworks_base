...
public class SecDeviceControlsTile extends QSTileImpl {
    ...
    public SecDeviceControlsTile(QSHost qSHost, QsEventLogger qsEventLogger, Looper looper, Handler handler, FalsingManager falsingManager, MetricsLogger metricsLogger, StatusBarStateController statusBarStateController, ActivityStarter activityStarter, QSLogger qSLogger, SecDeviceControlsController secDeviceControlsController, ControlsComponent controlsComponent) {
        ...
        SecControlsUiController secControlsUiController = controlsComponent.secControlsUiController.isPresent() ? (SecControlsUiController) controlsComponent.secControlsUiController.get() : null;
        this.mSecControlsUiController = secControlsUiController;
        if (secControlsUiController == null) {
            Log.e(this.TAG, "getSecControlsUiController() is empty");
        }
        ControlsListingController controlsListingController = controlsComponent.controlsListingController.isPresent() ? (ControlsListingController) controlsComponent.controlsListingController.get() : null;
        this.mControlsListingController = controlsListingController;
        if (controlsListingController != null) {
            ((ControlsListingControllerImpl) controlsListingController).addCallback((ControlsListingController.ControlsListingCallback) r1);
        } else {
            Log.e(this.TAG, "getSecControlsListingController() is empty");
        }
    }
    ...
    public final void destroy() {
        super.destroy();
        ControlsListingController controlsListingController = this.mControlsListingController;
        if (controlsListingController != null) {
            ((ControlsListingControllerImpl) controlsListingController).removeCallback(this.mListingCallback);
        }
    }
