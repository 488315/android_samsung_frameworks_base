...
public class EdgeLightingStyleManager {
    ...
    public static void setEdgeLightingStyleType(ContentResolver contentResolver, String str) {
    ...
    public final String getEdgeLightingStyleType(ContentResolver contentResolver) {
        ...
        if (!Feature.FEATURE_SUPPORT_BASIC_LIGHTING) {
            getInstance().getClass();
            setEdgeLightingStyleType(contentResolver, "preload/heart");
            return "preload/heart";
        }
        if ("preload/wave".equals(stringForUser) || "preload/bubble".equals(stringForUser) || "preload/gradation".equals(stringForUser) || "preload/glow".equals(stringForUser)) {
            getInstance().getClass();
            setEdgeLightingStyleType(contentResolver, "preload/basic");
            return "preload/basic";
        }
        getInstance().getClass();
        setEdgeLightingStyleType(contentResolver, "preload/noframe");
        return "preload/noframe";
    }

    public final int getPreloadIndex(String str) {
        if (!this.mStyleHashMap.containsKey(str)) {
            Slog.i("EdgeLightingStyleManager", "Not found effect type = " + str);
            Slog.i("EdgeLightingStyleManager", "Style hash map list : " + this.mStyleHashMap.keySet());
            return 0;
        }
