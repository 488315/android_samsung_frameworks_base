...
public class WindowMagnificationAnimationController implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
    ...
    public final void onAnimationUpdate(ValueAnimator valueAnimator) throws Resources.NotFoundException {
        ...
        float fM$1 = DrawerArrowDrawable$$ExternalSyntheticOutline0.m$1(animationSpec2.mScale, f, animatedFraction, f);
        float f2 = animationSpec.mCenterX;
        float fM$12 = DrawerArrowDrawable$$ExternalSyntheticOutline0.m$1(animationSpec2.mCenterX, f2, animatedFraction, f2);
        float f3 = animationSpec.mCenterY;
        this.mController.updateWindowMagnificationInternal(fM$1, fM$12, DrawerArrowDrawable$$ExternalSyntheticOutline0.m$1(animationSpec2.mCenterY, f3, animatedFraction, f3), this.mMagnificationFrameOffsetRatioX, this.mMagnificationFrameOffsetRatioY);
    }
    ...
    public final void setState(int i) {
        if (DEBUG) {
            KeyguardSecPinBasedInputViewController$$ExternalSyntheticOutline0.m(new StringBuilder("setState from "), this.mState, " to ", i, "WindowMagnificationAnimationController");
        }
        this.mState = i;
    }
    ...
    public final void setupEnableAnimationSpecs(float f, float f2, float f3) {
        ...
        float fExactCenterX = windowMagnificationController2.isActivated() ? windowMagnificationController2.mMagnificationFrame.exactCenterX() : Float.NaN;
        WindowMagnificationController windowMagnificationController3 = this.mController;
        float fExactCenterY = windowMagnificationController3.isActivated() ? windowMagnificationController3.mMagnificationFrame.exactCenterY() : Float.NaN;
        if (this.mState == 0) {
            ...
            this.mStartSpec.set(f4, fExactCenterX, fExactCenterY);
            int i = this.mState;
            if (i == 3) {
                f4 = this.mEndSpec.mScale;
            }
            if (i == 3) {
                fExactCenterX = this.mEndSpec.mCenterX;
            }
            if (i == 3) {
                fExactCenterY = this.mEndSpec.mCenterY;
            }
            AnimationSpec animationSpec2 = this.mEndSpec;
            if (Float.isNaN(f)) {
                f = f4;
