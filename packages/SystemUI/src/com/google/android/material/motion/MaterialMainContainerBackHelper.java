...
public class MaterialMainContainerBackHelper extends MaterialBackAnimationHelper {
    ...
    public final int getExpandedCornerSize() {
        ...
        if (this.expandedCornerSize == null) {
            ...
            this.expandedCornerSize = Integer.valueOf(iMax);
        }
    ...
    public void updateBackProgress(float f, boolean z, float f2, float f3) {
        ...
        float fLerp = AnimationUtils.lerp(1.0f, 0.9f, interpolation);
        float fM = SeslRecoilAnimator$Holder$$ExternalSyntheticOutline0.m(width, 0.9f, width, 2.0f);
        float f4 = this.minEdgeGap;
        float fLerp2 = AnimationUtils.lerp(0.0f, Math.max(0.0f, fM - f4), interpolation) * (z ? 1 : -1);
        float fMin = Math.min(Math.max(0.0f, ((height - (fLerp * height)) / 2.0f) - f4), this.maxTranslationY);
