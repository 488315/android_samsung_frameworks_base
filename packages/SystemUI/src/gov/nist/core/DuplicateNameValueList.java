...
public class DuplicateNameValueList implements Serializable, Cloneable {
    ...
    public final Object clone() {
        ...
        ArrayList arrayList = (ArrayList) this.nameValueMap.values();
        int size = arrayList.size();
        int i = 0;
        while (i < size) {
            Object obj = arrayList.get(i);
            i++;
            duplicateNameValueList.set((NameValue) ((NameValue) obj).clone());
        }
    ...
    public final String encode() {
        ...
        if (!this.nameValueMap.isEmpty()) {
            Iterator it = ((ArrayList) this.nameValueMap.values()).iterator();
            if (it.hasNext()) {
                while (true) {
                    Object next = it.next();
